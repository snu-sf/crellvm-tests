...
typedef _Bool (*filterdir_filter_t)
  (const char *dir, const char *file, void *filtarg);

void
filterdir (const char *dir, filterdir_filter_t filter, void *filtarg, filterdir_fun_t fun, void *arg);
# 35 "ssheet.c" 2
# 1 "./versions.h" 1
# 33 "./versions.h"
...
_Bool version_null_p (version_t version);
...
static inline void style_sheets_versions (struct darray * keys, int sum [4], int requirement [4]);
...
new_pattern (char * pattern, size_t len)
{
  struct pattern * res = ((struct pattern *) xmalloc (sizeof (struct pattern) * (1)));
  res->pattern = pattern;
  res->len = len;
  return res;
}
...
{
  do { if (faced_string->string) free (faced_string->string); } while (0);
  free (faced_string);
}
...
{
  if ((((str->face).face == (No_fface).face) && ((str->face).flags == (No_fface).flags)))
    str->face = face;
}
        ...
        const char * version_string)
{
  ...
  string_to_version ("4.14", a2ps_version);
...
rhs_set_no_face (struct darray * dest, struct fface_s face)
{
  ...
  for (i = 0 ; i < dest->len ; i++)
    if ((((((struct faced_string *) dest->content [i])->face).face == (No_fface).face) && ((((struct faced_string *) dest->content [i])->face).flags == (No_fface).flags)))
...
rule_new_internal_word (unsigned char * word,
   struct darray * rhs)
{
  struct rule * res = ((struct rule *) xmalloc (sizeof (struct rule) * (1)));
     ...
     const char *filename, size_t line)
{
  ...
  res->regex = ((struct re_pattern_buffer *) xmalloc (sizeof (struct re_pattern_buffer) * (1)));
  res->regex->translate = ((void*)0);
  res->regex->fastmap = ((void*)0);
  res->regex->buffer = ((void*)0);
  ...
  if (error_msg)
    error_at_line (1, 0, filename, line,
     gettext ("cannot compile regular expression `%s': %s"),
     regexp, error_msg);
  free (pattern);
  res->rhs = rhs;
  return res;
...
{
  int pattern_len = 0;
  char * pattern_to_compile = ((void*)0);

  if (pattern)
    {
      pattern_len = pattern->len + strlen ("\\b()\\b");
      pattern_to_compile = ((char *) __builtin_alloca (sizeof (char) * (pattern_len + 1)));
...
{
  do { if (rule->word) free (rule->word); } while (0);
  if (rule->regex)
    {
      regfree (rule->regex);
      free (rule->regex);
...
{
  int c;
  struct words * res = ((struct words *) xmalloc (sizeof (struct words) * (1)));
  ...
  res->regexps = da_new (name_regexps, size,
    da_linear, increment,
    (da_print_func_t) rule_self_print,
    (da_cmp_func_t) ((void*)0));


  for (c = 0 ; c < 256 ; c ++)
    res->min[c] = res->max[c] = ((void*)0);
...
words_finalize (struct words * list)
{
  ...
  for (i = 0 ; i < list->strings->len ; i++) {
    if (list->min [content[i]->word[0]] == ((void*)0))
...
{
  int i;
  struct style_sheet * ancestor = ((void*)0);
...
sequence_new (struct rule * Open,
       ...
       struct words * exceptions)
{
  struct sequence * res = ((struct sequence *) xmalloc (sizeof (struct sequence) * (1)));
...
new_C_exceptions (void)
{
  ...
  res = words_new ("C usual exceptions: strings",
     ...
     3, 3);
  words_add_string (res, rule_new (((unsigned char *) xstrdup((const char *)("\\\'"))), ((void*)0), rhs_new_single (((unsigned char *) xstrdup((const char *)("\\\'"))), 0, String_fface), "ssheet.c", 672));;
  words_add_string (res, rule_new (((unsigned char *) xstrdup((const char *)("\\\\"))), ((void*)0), rhs_new_single (((unsigned char *) xstrdup((const char *)("\\\\"))), 0, String_fface), "ssheet.c", 673));;
  words_add_string (res, rule_new (((unsigned char *) xstrdup((const char *)("\\\""))), ((void*)0), rhs_new_single (((unsigned char *) xstrdup((const char *)("\\\""))), 0, String_fface), "ssheet.c", 674));;
...
{
  struct sequence * res = ((struct sequence *) xmalloc (sizeof (struct sequence) * (1)));
  res->open = rule_new (((unsigned char *) xstrdup((const char *)(delimitor))), ((void*)0),
   rhs_new_single (((void*)0), 0, Plain_fface),
   "ssheet.c", 686);
  ...
  words_add_string (res->close,
      rule_new (((unsigned char *) xstrdup((const char *)(delimitor))), ((void*)0),
         rhs_new_single (((void*)0), 0, Plain_fface),
         "ssheet.c", 693));
...
{
  struct style_sheet *(res) = ((struct style_sheet *) xmalloc (sizeof (struct style_sheet) * (1)));

  res->name = name;
  res->author = (unsigned char *) ((void*)0);
  version_set_to_null (res->version);
  res->documentation = (unsigned char *) ((void*)0);
...
style_sheet_mixed_new (const unsigned char * ancestors)
{
  ...
  key = ((char *) __builtin_alloca (sizeof (char) * ((strlen((const char *)(ancestors))) + 1)));
  do { const char * _tmp_ = (const char *) (ancestors); ancestors_keys = ((char *) __builtin_alloca (sizeof (char) * (strlen (_tmp_) + 1))); strcpy (ancestors_keys, _tmp_); } while (0);


  ancestors_array = ancestors_new ();
  ancestor_key = strtok (ancestors_keys, ",;+");
  da_append (ancestors_array, ancestor_key);
  ancestor = get_style_sheet (ancestor_key);
  cp = stpcpy (key, ancestor->key);

  while ((ancestor_key = strtok (((void*)0), ",;+")))
    {
      da_append (ancestors_array, ancestor_key);
      ancestor = get_style_sheet (ancestor_key);
      *cp++ =',';
  ...
  *cp = '\0';
  ...
  do { if ((((1 << 2)) & msg_verbosity)) fprintf (stderr, "Creating a mixed style sheet \"%s\"\n", key); } while (0);
...
{
  do { if ((((1 << 2)) & msg_verbosity)) fprintf (stderr, "Finalizing style sheet \"%s\" (%s)\n", sheet->name, sheet->key); } while (0);
  ...
  if (((sheet->alpha1)[0] == 127))
    string_to_array (sheet->alpha1, ((const unsigned char *) "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_"));
  if (((sheet->alpha2)[0] == 127))
    string_to_array (sheet->alpha2, ((const unsigned char *) "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789"));
  ...
  if ((((1 << 2)) & msg_verbosity))
    {
      ...
      do { if ((((1 << 2)) & msg_verbosity)) fprintf (stderr, "Checking coherence of \"%s\" (%s)\n", sheet->name, sheet->key); } while (0);


      if (style_sheet_check (sheet))
 do { if ((((1 << 2)) & msg_verbosity)) fprintf (stderr, "\"%s\" (%s) is sane.\n", sheet->name, sheet->key); } while (0);

      else
 do { if ((((1 << 2)) & msg_verbosity)) fprintf (stderr, "\"%s\" (%s) is corrupted.\n", sheet->name, sheet->key); } while (0);
...
load_style_sheet (const char * pseudo_key)
{
  ...
  if (((strlen (pseudo_key) < strlen (".ssh")) ? 0 : !strcmp (pseudo_key + strlen (pseudo_key) - strlen (".ssh"), ".ssh")))
    {
  ...
  else
    {
      ...
      char * path = pw_find_file (job->common.path, pseudo_key, ".ssh");
      if (!path)
 {
   char *file;
   do { const char * _tmp1_ = (const char *) (pseudo_key); const char * _tmp2_ = (const char *) (".ssh"); file = ((char *) __builtin_alloca (sizeof (char) * ((strlen (_tmp1_) + strlen (_tmp2_) + 1)))); do { stpcpy (stpcpy (file, pseudo_key), ".ssh"); } while (0); } while (0);
   error (0, 0, gettext ("cannot find file `%s'"), quotearg (file));
   return ((void*)0);
 }
...
style_sheet_self_print (struct style_sheet * sheet, FILE * stream)
{
  ...
  _IO_putc ('\n', stream);
  if (!(((const char *) (sheet->author) == ((void*)0)) || (*(sheet->author) == '\0')))
...
list_style_sheets_long (FILE * stream)
{
  ...
  entries = pw_glob_on_suffix (job->common.path, ".ssh");


  entries->cmp = (da_cmp_func_t) sheet_name_cmp;
  da_qsort (entries);

  title (stream, '=', 1, gettext ("Known Style Sheets"));
  _IO_putc ('\n', stream);
...
list_style_sheets_html (FILE * stream)
{
  ...
  fputs ((char *) expand_user_string (job,
          ((struct file_job *) (job->jobs->content[job->jobs->len - 1])),
...
# 1103 "ssheet.c"
  fputs ((char *) expand_user_string (job,
          ((struct file_job *) (job->jobs->content[job->jobs->len - 1])),
          "sheets.html generation",
"#{html.hline:-<hline>}\n<h2>Sumary of current style sheets</h2>\n<p>There is a <a href=\"sheets.tar.gz\">package</a> which contains them all.  Its index (sum of the style sheets version number) is <strong>"), stream);
...
list_style_sheets_texinfo (FILE * stream)
{
  ...
  if ((((1 << 2)) & msg_verbosity))
    {
      fputs ("The current index (sum of all the style sheets version number) is ", stream);
...
check_rules_doubles (const char * name, struct darray * rules)
{
  ...
  for (i = 1 ; i < rules->len ; i++)
    if (!(strcmp((const char *)(((struct rule *) rules->content [i - 1])->word), (const char *)(((struct rule *) rules->content [i])->word))))
...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((((const struct style_sheet *)key)->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0xf)); } while (0); return result; } while (0);
}

static unsigned long
sheet_hash_2 (void const *key)
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((((const struct style_sheet *)key)->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}

static int
sheet_hash_cmp (void const *x, void const *y)
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) (((const struct style_sheet *)x)->key) - 1; unsigned char const *yy = (unsigned char const *) (((const struct style_sheet *)y)->key) - 1; do { if (*++xx == '\0') { yy++; break; } } while (*xx == *++yy); (result) = *xx - *yy; } while (0); return result; } while (0);
  ...
  if (item == ((void*)0))
    {
      ...
      if (item == ((void*)0))
 error (0, 0,
        gettext ("cannot find style sheet `%s': using plain style"), key);
    }
