...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((((const struct delegation *) key)->contract)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}

static int
delegate_hash_cmp (void const *x, void const *y)
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) (((const struct delegation *) x)->contract) - 1; unsigned char const *yy = (unsigned char const *) (((const struct delegation *) y)->contract) - 1; do { if (*++xx == '\0') { yy++; break; } } while (*xx == *++yy); (result) = *xx - *yy; } while (0); return result; } while (0);
...
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) ((*x)->name) - 1; unsigned char const *yy = (unsigned char const *) ((*y)->name) - 1; do { if (*++xx == '\0') { yy++; break; } } while (*xx == *++yy); (result) = *xx - *yy; } while (0); return result; } while (0);
}
  ...
  char *contract_line)
{
  ...
  contract = ((struct delegation *) xmalloc (sizeof (struct delegation) * (1)));



  cp = strtok (contract_line, " \t\n");
  if (cp == ((void*)0)) error_at_line (1, 0, filename, line, gettext ("missing argument for `%s'"), quotearg (contract_line));;
  contract->name = xstrdup (cp);

  cp = strtok (((void*)0), " \t\n:");
  if (cp == ((void*)0)) error_at_line (1, 0, filename, line, gettext ("missing argument for `%s'"), quotearg (contract_line));;
  cp2 = strtok (((void*)0), " \t\n");
  if (cp2 == ((void*)0)) error_at_line (1, 0, filename, line, gettext ("missing argument for `%s'"), quotearg (contract_line));;
  contract->contract = ((char *) xmalloc (sizeof (char) * (strlen (cp) + strlen (cp2) + 2)));
...
get_subcontract (const char *src_type, const char *dest_type)
{
  ...
  token.contract = ((char *) __builtin_alloca (sizeof (char) * (strlen (src_type) + strlen (dest_type) + 2)));
...
{
  char *command, *stdin_content_filename = ((void*)0);
  ...
  if (fjob->name == job->stdin_filename)
    {

      do { (fjob->stdin_tmpname) = (fjob->stdin_tmpname) ? (fjob->stdin_tmpname) : tempnam (((void*)0), "a2_"); } while (0);
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "Delegating `%s' to `%s' (%s)\n", fjob->name, contractor->name, command); } while (0);





  do { (fjob->delegation_tmpname) = (fjob->delegation_tmpname) ? (fjob->delegation_tmpname) : tempnam (((void*)0), "a2_"); } while (0);
  out_stream = fopen (fjob->delegation_tmpname, "w");
  if (!out_stream)
    {
      error (0, (*__errno_location ()), gettext ("cannot create file `%s'"),
  ...
  if (!in_stream)
    {
      fclose (out_stream);
      error (0, (*__errno_location ()), gettext ("cannot open a pipe on `%s'"),
      quotearg (command));
      return 0;
    }
  ...
  while (fgets (buf, sizeof (buf), in_stream))
    {
      ...
      if ((!strncmp("%%Page: ", buf, strlen("%%Page: "))))
 {
   ...
   file_job_synchronize_pages (job);
      ...
      else if ((!strncmp("%%DocumentNeededResources: ", buf, strlen("%%DocumentNeededResources: "))))
 {
   ...
   do { const char * _tmp_ = (const char *) (buf + strlen ("%%DocumentNeededResources: ")); buf_copy = ((char *) __builtin_alloca (sizeof (char) * (strlen (_tmp_) + 1))); strcpy (buf_copy, _tmp_); } while (0);
   res = strtok (buf_copy, " \n\t");
      ...
      else if ((!strncmp("%%+ ", buf, strlen("%%+ "))))
 {
   ...
   do { const char * _tmp_ = (const char *) (buf + strlen ("%%+ ")); buf_copy = ((char *) __builtin_alloca (sizeof (char) * (strlen (_tmp_) + 1))); strcpy (buf_copy, _tmp_); } while (0);
   res = strtok (buf_copy, " \n\t");
   while ((value = strtok (((void*)0), " \n\t")))
     switch (continuation)
       {
...
dump_contract (FILE * stream, struct delegation *contract)
{
  ...
  fprintf (stream, gettext ("Delegation `%s', from %s to %s\n"),
         ...
         FILE * stream)
{
  ...
  ordered_contracts =
    ((struct delegation **)
     hash_dump (contracts, ((void*)0), (qsort_cmp_t) delegate_hash_qcmp));

  fputs (gettext ("Applications configured for delegation"), stream);
  _IO_putc ('\n', stream);
