...
struct pattern_rule *
pattern_rule_new (const char *pattern, int on_file_verdict, _Bool insensitive_p,
    const char *command)
{
  struct pattern_rule *(res) = ((struct pattern_rule *) xmalloc (sizeof (struct pattern_rule) * (1)));
...
void
sheets_map_add (const char * pattern, int on_file_verdict, _Bool insensitive_p,
  const char * key)
{
...
sheets_map_load_main (void)
{
  ...
  file = pw_find_file (job->common.path, "sheets.map", ((void*)0));
  if (!file)
    {
      error (0, (*__errno_location ()), gettext ("cannot find file `%s'"), "sheets.map");

      error (0, 0, gettext ("automatic style selection cancelled"));
      return 0;
    }
...
{
  char *cp = ((void*)0), * command;
  char buf [1024];
  FILE * file_out;

  if ((((const char *) (job->file_command) == ((void*)0)) || (*(job->file_command) == '\0')))
    return ((void*)0);
  ...
  command = ((char *) __builtin_alloca (sizeof (char) * ((4 + strlen (job->file_command) + (strlen((const char *)(filename)))))));


  sprintf (command, "%s '%s'", job->file_command, (const char *) filename);
  free(filename);
  do { if ((((1 << 7)) & msg_verbosity)) fprintf (stderr, "Reading pipe: `%s'\n", command); } while (0);
 ...
 error (0, (*__errno_location ()), gettext ("cannot open a pipe on `%s'"),
  ...
  pclose (file_out);
  do { if ((((1 << 7)) & msg_verbosity)) fprintf (stderr, "file(1): %s", buf); } while (0);
  ...
  if (*cp)
    {
      if (cp)
 do { if ((((1 << 7)) & msg_verbosity)) fprintf (stderr, "File's verdict: %s", cp); } while (0);
...
get_command (const unsigned char *name_to_match, const unsigned char *name_to_file)
{
  ...
  if (name_to_match)
    {
      name_to_match_lc = ((char *) __builtin_alloca (sizeof (char) * (strlen (name_to_match) + 1)));
 ...
 if (file_verdict
     && !fnmatch (((struct pattern_rule *)sheets_map->content[i])->pattern, file_verdict, 0))
   {
     free (file_verdict);
 ...
 if (name_to_match
     && !fnmatch (((struct pattern_rule *)sheets_map->content[i])->pattern,
    (char *) (((struct pattern_rule *)sheets_map->content[i])->insensitive_p
       ? name_to_match_lc : name_to_match),
    0))
   {
     do { if (file_verdict) free (file_verdict); } while (0);
     return ((struct pattern_rule *)sheets_map->content[i])->command;
   }
      }
  do { if (file_verdict) free (file_verdict); } while (0);
