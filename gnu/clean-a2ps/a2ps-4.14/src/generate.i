...
string_to_style_kind (const char * string)
{
  if ((strcmp ((string), ("binary")) == 0))
    return binary;
  else if ((strcmp ((string), ("UNPRINTABLE")) == 0))
    return unprintable;
  else if ((strcmp ((string), ("plain")) == 0))
    return no_style;
  else if ((strcmp ((string), ("delegate")) == 0))
    return delegate;
  return sshparser;
}
...
{
  buffer_t *(res) = ((buffer_t *) xmalloc (sizeof (buffer_t) * (1)));
  ...
  if ((((const char *) (name) == ((void*)0)) || (*(name) == '\0')) || (!(strcmp((const char *)(name), (const char *)("-")))))
    {
      file_job->is_stdin = 1;
      ...
      do { (sample_tmpname) = (sample_tmpname) ? (sample_tmpname) : tempnam (((void*)0), "a2_"); } while (0);
  ...
  else
    {
      ...
      if (isdir ((char *) name))
 {
   error (0, 0, gettext ("`%s' is a directory"), quotearg ((char *) name));
   file_job->printable = 0;
 }

      file_job->name = (unsigned char *) name;
      if ((input_stream = fopen ((char *) name, "r")) == ((void*)0))
 {
   error (0, (*__errno_location ()),
   gettext ("cannot open file `%s'"), quotearg ((char *) name));
   file_job->printable = 0;
 }
      else if (stat ((char *) name, &statbuf) == -1)
 {
   error (0, (*__errno_location ()), gettext ("cannot get informations on file `%s'"),
   quotearg ((char *) name));
   file_job->printable = 0;
 }
      else
 {
   time_t tim = statbuf.st_mtim.tv_sec;
   tm = localtime (&tim);
...
# 123 "generate.c"
  ...
  else if (!(((const char *) (style_request) == ((void*)0)) || (*(style_request) == '\0')))
...
msg_file_pages_printed (a2ps_job * Job, const char * stylename)
{
  ...
  sheets = ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->sheets;
  if (Job->duplex)
    sheets = (sheets + 1) / 2;

  if (((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->pages == 1)

    do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (%s): 1 page on 1 sheet]\n"), ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->name, stylename); } while (0);



  else if (sheets == 1)

    do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (%s): %d pages on 1 sheet]\n"), ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->name, stylename, ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->pages); } while (0);
    ...
    do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (%s): %d pages on %d sheets]\n"), ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))->name, stylename, ((struct file_job *) (Job->jobs->content[Job->jobs->len - 1]))-> pages, sheets); } while (0);
...
msg_job_pages_printed (a2ps_job * Job)
{
  ...
  if (Job->pages == 1)
    ...
    do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[Total: 1 page on 1 sheet] %s\n"), ucp); } while (0);

  else if (sheets == 1)

    do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[Total: %d pages on 1 sheet] %s\n"), Job->pages, ucp); } while (0);


  else

    do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[Total: %d pages on %d sheets] %s\n"), Job-> pages, sheets, ucp); } while (0);
 ...
 do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[1 line wrapped]\n")); } while (0);

      else
 do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[%d lines wrapped]\n"), Job->lines_folded); } while (0);
...
{
  do { if (((((1 << 1) | (1 << 0))) & msg_verbosity)) fprintf (stderr, gettext ("[No output produced]\n")); } while (0);
...
print_toc (const unsigned char * name, const unsigned char * value, int * native_jobs)
{
  ...
  a2ps_open_input_session (job, ((unsigned char *) xstrdup((const char *)(name))));


  ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->is_toc = 1;

  do { const unsigned char * _tmp_ = (const unsigned char *) (expand_user_string (job, ((struct file_job *) (job->jobs->content[job->jobs->len - 1])), name, value)); toc_content = ((unsigned char *) __builtin_alloca (sizeof (unsigned char) * ((strlen((const char *)(_tmp_))) + 1))); (strcpy((char *)(toc_content), (const char *)(_tmp_))); } while (0);
...
{
  char buf[512];
  struct delegation * contract = ((void*)0);
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "Getting ready to print file `%s', with command `%s'\n", file_job->name, file_job->type); } while (0);
  ...
  switch (style_kind)
    ...
    case delegate:
      ...
      page_flush (job);
      sprintf (buf, gettext ("%s, delegated to %s"),
 ...
 do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (%s): failed.  Ignored]\n"), file_job->name, buf); } while (0);

      break;

    case unprintable:

      do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (unprintable): ignored]\n"), file_job->name); } while (0);
 ...
 goto plain_print;
      ...
      do { if ((((1 << 1)) & msg_verbosity)) fprintf (stderr, gettext ("[%s (binary): ignored]\n"), file_job->name); } while (0);
 ...
 goto plain_print;
    ...
    case no_style:
      ...
      msg_file_pages_printed (job, gettext ("plain"));
      (*native_jobs)++;
      break;
    }
