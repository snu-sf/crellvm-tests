       ...
       _Bool pipe_p, enum eol_e eol)
{
  ...
  buffer->len = 0;
  buffer->curr = 0;
  _obstack_begin ((&buffer->obstack), 0, 0, (void *(*) (long)) xmalloc, (void (*) (void *)) free);
}

void
buffer_stream_set (buffer_t * buffer, FILE * stream, enum eol_e eol)
{
  buffer_internal_set (buffer, stream, ((void*)0), 0, 0, eol);
}

void
buffer_pipe_set (buffer_t * buffer, FILE * stream, enum eol_e eol)
{
  buffer_internal_set (buffer, stream, ((void*)0), 0, 1, eol);
}

void
buffer_string_set (buffer_t * buffer, const unsigned char * string, enum eol_e eol)
{
  buffer_internal_set (buffer, ((void*)0), string, strlen (string), 0, eol);
}
...
buffer_buffer_set (buffer_t * buffer, const unsigned char * buf, size_t bufsize,
     enum eol_e eol)
{
  buffer_internal_set (buffer, ((void*)0), buf, bufsize, 0, eol);
}
# 174 "buffer.c"
void
...
buffer_stream_get_line (buffer_t * buffer)
{
  ...
  while ((c = (_IO_getc ((buffer)->stream))) != (-1))
    switch (c)
      {
      case '\n':
 switch (buffer->eol)
 ...
 __extension__ ({ struct obstack *__o = (&buffer->obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (c); (void) 0; });
 return 1;
...
buffer_string_get_line (buffer_t * buffer)
{
  ...
  while ((c = (((buffer)->bufoffset < (buffer)->bufsize) ? ((buffer)->buf[(buffer)->bufoffset++]) : ((buffer)->bufoffset++, (-1)))) != (-1))
    switch (c)
      {
      case '\n':
 switch (buffer->eol)
   ...
   case eol_auto:
     ...
     if ((d = (((buffer)->bufoffset < (buffer)->bufsize) ? ((buffer)->buf[(buffer)->bufoffset++]) : ((buffer)->bufoffset++, (-1)))) != '\r')
       ((buffer)->bufoffset--);
     break;
 ...
 switch (buffer->eol)
   ...
   case eol_auto:
     ...
     if ((d = (((buffer)->bufoffset < (buffer)->bufsize) ? ((buffer)->buf[(buffer)->bufoffset++]) : ((buffer)->bufoffset++, (-1)))) != '\n')
       ((buffer)->bufoffset--);
     c = '\n';
     break;
...
buffer_get (buffer_t * buffer)
{
  ...
  buffer->len = __extension__ ({ struct obstack *__o = (&buffer->obstack); (unsigned) (__o->next_free - __o->object_base); });
  __extension__ ({ struct obstack *__o = (&buffer->obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = ('\0'); (void) 0; });
  buffer->content = (unsigned char *) __extension__ ({ struct obstack *__o1 = (&buffer->obstack); void *value; value = (void *) __o1->object_base; if (__o1->next_free == value) __o1->maybe_empty_object = 1; __o1->next_free = (((((__o1->next_free) - (char *) 0)+__o1->alignment_mask) & ~ (__o1->alignment_mask)) + (char *) 0); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; value; });
 ...
 buffer->allocsize = buffer->len + 1;
      ...
      buffer->value =
 ((unsigned char *) xrealloc ((void *) (buffer->value), sizeof (unsigned char) * (buffer->allocsize)));
