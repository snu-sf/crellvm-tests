...
token_new (void)
{
  struct token *res = ((struct token *) xmalloc (sizeof (struct token) * (1)));
  res->registers = ((struct re_registers *) xmalloc (sizeof (struct re_registers) * (1)));
  res->registers->start = ((regoff_t *) xmalloc (sizeof (regoff_t) * (30)));
  res->registers->end = ((regoff_t *) xmalloc (sizeof (regoff_t) * (30)));
  return res;
}
...
rhs_plain_new (void)
{
  return rhs_new_single (((void*)0), 0, Plain_fface);
}
...
static struct token *token = ((void*)0);
# 114 "sshread.c"
static struct darray *plain_rhs = ((void*)0);
# 123 "sshread.c"
static inline void
match_word (buffer_t * buffer, struct style_sheet *sheet)
{
  ...
  && !(buffer->curr >= buffer->len));

  token->rhs = plain_rhs;
  ((struct faced_string *) token->rhs->content[0])->face = Plain_fface;
        ...
        unsigned char * alphabet)
{
  ...
  ; key--)
      {
 if ((!(strncmp((const char *)((*key)->word), (const char *)(string), ((strlen((const char *)((*key)->word)))))))
     && !alphabet[string[(strlen((const char *)((*key)->word)))]])
   {
     token->rhs = (*key)->rhs;
     do { token->registers->start [0] = buffer->curr; token->registers->end [0] = buffer->curr + (strlen((const char *)((*key)->word))); } while (0);
     buffer->curr += (strlen((const char *)((*key)->word)));
     return 1;
   }
      }
  ...
  for (i = (int) words->regexps->len - 1; i >= 0; i--)
    {
      res = re_match ((((struct rule *) words->regexps->content[i]))->regex, (char *) buffer->value, buffer->len, buffer->curr, token->registers);
      switch (res)
 {
 case -2:
 ...
 case -1:
   continue;
 default:
   token->rhs = (((struct rule *) words->regexps->content[i]))->rhs;
   buffer->curr += res;
   return 1;
 }
    }
...
match_sequence (buffer_t * buffer, struct style_sheet *sheet)
{
      ...
      if (((struct sequence *) sheet->sequences->content [i])->open->regex)
 {
   ...
   res = re_match (((struct sequence *) sheet->sequences->content [i])->open->regex, (char *) buffer->value, buffer->len, buffer->curr, token->registers);
   switch (res)
     {
     case -2:
       fprintf (stderr, "An error occured while matching\n");
     ...
     case -1:
       continue;
     default:
       token->rhs = ((struct sequence *) sheet->sequences->content [i])->open->rhs;
       buffer->curr += res;
      ...
      else
 {
   ...
   if ((!(strncmp((const char *)(((struct sequence *) sheet->sequences->content [i])->open->word), (const char *)(string), ((strlen((const char *)(((struct sequence *) sheet->sequences->content [i])->open->word))))))))
     {
       do { token->registers->start [0] = buffer->curr; token->registers->end [0] = buffer->curr + (strlen((const char *)(((struct sequence *) sheet->sequences->content [i])->open->word))); } while (0);
       token->rhs = ((struct sequence *) sheet->sequences->content [i])->open->rhs;
       buffer->curr += (strlen((const char *)(((struct sequence *) sheet->sequences->content [i])->open->word)));
       return ((struct sequence *) sheet->sequences->content [i]);
     }
 }
    }
...
ssh_get_token (buffer_t * buffer, struct style_sheet *sheet)
{
  ...
  static struct sequence *sequence = ((void*)0);
  ...
  if ((buffer->curr >= buffer->len))
    {
      ...
      (((struct file_job *) (job->jobs->content[job->jobs->len - 1])))->lines = buffer->line;
      ...
      if (match_operator (buffer, sequence->close))
 {
   ...
   return 1;
      ...
      token->rhs = plain_rhs;
      ((struct faced_string *) token->rhs->content[0])->face = sequence->face;
        ...
        struct style_sheet *sheet)
{
 ...
 if (!(((fface).face == ((((struct faced_string *) token->rhs->content[i])->face)).face) && ((fface).flags == ((((struct faced_string *) token->rhs->content[i])->face)).flags)))
   {

     if (((((((struct faced_string *) token->rhs->content[i])->face)).flags)) & ff_Tag1)
       *Job->tag1 = '\0';
     if (((((((struct faced_string *) token->rhs->content[i])->face)).flags)) & ff_Tag2)
       *Job->tag2 = '\0';
     if (((((((struct faced_string *) token->rhs->content[i])->face)).flags)) & ff_Tag3)
       *Job->tag3 = '\0';
     if (((((((struct faced_string *) token->rhs->content[i])->face)).flags)) & ff_Tag4)
       *Job->tag4 = '\0';
     if (grabbing_encoding
  && (!(((((((struct faced_string *) token->rhs->content[i])->face)).flags)) & ff_Encoding)))
       {
  ...
  if (!newenc)
    error (0, 0, gettext ("unknown encoding `%s', ignored"),
  ...
  *bufenc = '\0';
  grabbing_encoding = 0;
     ...
     fface = ((struct faced_string *) token->rhs->content[i])->face;
   }


 if (((struct faced_string *) token->rhs->content[i])->string)
   {
     if (((fface).flags) & ff_Tag1)
       (strcat((char *)(Job->tag1), (const char *)(((struct faced_string *) token->rhs->content[i])->string)));
     if (((fface).flags) & ff_Tag2)
       (strcat((char *)(Job->tag2), (const char *)(((struct faced_string *) token->rhs->content[i])->string)));
     if (((fface).flags) & ff_Tag3)
       (strcat((char *)(Job->tag3), (const char *)(((struct faced_string *) token->rhs->content[i])->string)));
     if (((fface).flags) & ff_Tag4)
       (strcat((char *)(Job->tag4), (const char *)(((struct faced_string *) token->rhs->content[i])->string)));

     if (((fface).flags) & ff_Encoding)
       {
  grabbing_encoding = 1;
  (strcat((char *)(bufenc), (const char *)(((struct faced_string *) token->rhs->content[i])->string)));
       }
   }
 else
   {
     if (((fface).flags) & ff_Tag1)
       do { (strncat(((char *)Job->tag1), (const char *) buffer->content + token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref] - token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref])); } while (0);
     if (((fface).flags) & ff_Tag2)
       do { (strncat(((char *)Job->tag2), (const char *) buffer->content + token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref] - token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref])); } while (0);
     if (((fface).flags) & ff_Tag3)
       do { (strncat(((char *)Job->tag3), (const char *) buffer->content + token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref] - token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref])); } while (0);
     if (((fface).flags) & ff_Tag4)
       do { (strncat(((char *)Job->tag4), (const char *) buffer->content + token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref] - token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref])); } while (0);

     if (((fface).flags) & ff_Encoding)
       {
  grabbing_encoding = 1;
  do { (strncat(((char *)bufenc), (const char *) buffer->content + token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref] - token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref])); } while (0);
       }
   }
 ...
 if (!(((fface).flags) & ff_Invisible))
   {
     if (((struct faced_string *) token->rhs->content[i])->string)
       ps_print_string (Job, ((struct faced_string *) token->rhs->content[i])->string,
          fface.face);
     else
       ps_print_buffer (Job, buffer->content,
          token->registers->start [((struct faced_string *) token->rhs->content[i])->reg_ref], token->registers->end [((struct faced_string *) token->rhs->content[i])->reg_ref],
          fface.face);
   }
      }
