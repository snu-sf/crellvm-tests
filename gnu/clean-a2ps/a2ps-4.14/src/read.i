      ...
      else if ('_' == input[0])
 {
   *face = Keyword;
   buffer->curr += 2;
      ...
      else if (((((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->lines + 3) % 66 == 0)
        || ((((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->lines - 4) % 66 == 0))
 {
   buffer->curr += 2;
 }
...
plain_print_postscript (a2ps_job * Job, buffer_t * buffer)
{
  ...
  while ((c = plain_getc (buffer, &face)) != (-1))
