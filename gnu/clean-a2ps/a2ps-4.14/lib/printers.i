...
void lister_fprint_horizontal
  (struct lister * lister, FILE * stream, void **items, size_t item_number, lister_width_t item_width_fn, lister_print_t item_print_fn);
...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((printer->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}
...
{
  struct printer *(res) = ((struct printer *) xmalloc (sizeof (struct printer) * (1)));
...
# 177 "printers.c"
static inline struct hash_table_s *
printer_table_new (void)
{
  ...
  return (struct hash_table_s *) res;
}
...
static inline void
printer_table_free (struct hash_table_s * table)
{
...
printer_table_find (struct hash_table_s *table, const char *key)
{
...
static inline void
printer_table_add (struct hash_table_s * table,
...
static void
printer_table_short_self_print (struct hash_table_s * table, FILE * stream)
{
...
static void
printer_table_self_print (struct hash_table_s * table, FILE * stream)
{
...
{
  struct a2ps_printers_s *(res) = ((struct a2ps_printers_s *) xmalloc (sizeof (struct a2ps_printers_s) * (1)));
  ...
  printer_create (&res->default_printer, gettext ("Default Printer"));
  printer_create (&res->unknown_printer, gettext ("Unknown Printer"));


  res->request_ppdkey = ((void*)0);
  res->default_ppdkey = xstrdup ("level1");
  res->ppd = ((void*)0);



  res->flag_output_is_printer = 1;
  res->flag_output_name = ((void*)0);
  res->output_is_file = 1;
  res->output_name = ((void*)0);
...
a2ps_printers_free (struct a2ps_printers_s * printers)
{
  ...
  do { if (printers->request_ppdkey) free (printers->request_ppdkey); } while (0);
  do { if (printers->default_ppdkey) free (printers->default_ppdkey); } while (0);
  ppd_free (printers->ppd);


  do { if (printers->flag_output_name) free (printers->flag_output_name); } while (0);
  do { if (printers->output_name) free (printers->output_name); } while (0);
      ...
      const char *key)
{
 ...
 error (1, 0,
        gettext ("no command for the `%s' (%s%s)"),
        gettext ("Default Printer"), "-d", "");
...
destination_to_string (const char *name, _Bool file_p)
{
  unsigned char *res;

  if ((((const char *) (name) == ((void*)0)) || (*(name) == '\0')))
    {
      res = (file_p
      ? ((unsigned char *) xstrdup((const char *)(gettext ("sent to the standard output"))))
      : ((unsigned char *) xstrdup((const char *)(gettext ("sent to the default printer")))));
    }
  else
    {
      char *format = (file_p
        ? gettext ("saved into the file `%s'")
        : gettext ("sent to the printer `%s'"));
      res = ((unsigned char *) xmalloc (sizeof (unsigned char) * (strlen (format) + strlen (name))));
...
a2ps_flag_destination_to_string (a2ps_job * job)
{
  ...
  job->printers->flag_output_is_printer ? 0 : 1);
}
     ...
     const char * key, char * definition)
{
  ...
  if ((strcmp ((key), (gettext ("Default Printer"))) == 0))
    printer_set (&printers->default_printer, ppdkey, command);
  else if ((strcmp ((key), (gettext ("Unknown Printer"))) == 0))
...
a2ps_printers_default_ppdkey_set (struct a2ps_printers_s * printers,
      const char * ppdkey)
{
  do { const char *my_s2 = (ppdkey); do { if (printers->default_ppdkey) free (printers->default_ppdkey); } while (0); printers->default_ppdkey = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
}
...
a2ps_printers_request_ppdkey_set (struct a2ps_printers_s * printers,
      const char * ppdkey)
{
  do { const char *my_s2 = (ppdkey); do { if (printers->request_ppdkey) free (printers->request_ppdkey); } while (0); printers->request_ppdkey = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
}
          ...
          _Bool is_printer)
{
  ...
  if (!is_printer && flag_output_name && (strcmp ((flag_output_name), ("-")) == 0))
    {

      do { if (printers->flag_output_name) free (printers->flag_output_name); } while (0);
      printers->flag_output_name = ((void*)0);
    }
  else
    do { const char *my_s2 = (flag_output_name); do { if (printers->flag_output_name) free (printers->flag_output_name); } while (0); printers->flag_output_name = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
}
...
{
  fputs (gettext ("Known Outputs (Printers, etc.)"), stream);
  _IO_putc ('\n', stream);
...
{
  title (stream, '=', 1, gettext ("Known Outputs (Printers, etc.)"));
  _IO_putc ('\n', stream);
  ...
  if (!printers->flag_output_is_printer)
    {
      ...
      do { const char *my_s2 = (printers->flag_output_name); do { if (printers->output_name) free (printers->output_name); } while (0); printers->output_name = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
      printers->output_is_file = 1;
    }
  else
    {
      ...
      command = (char *) expand_user_string (job, ((struct file_job *) (job->jobs->content[0])),
