...
unprintable_format_to_string (enum unprintable_format format)
{
  switch (format) {
  case space:
    return gettext ("space (i.e., ` ')");
  case octal:
    return gettext ("octal (i.e., `\\001' etc.)");
  case hexa:
    return gettext ("hexadecimal (i.e., `\\x0a' etc.)");
  case caret:
    return gettext ("caret (i.e., `^C', `M-^C' etc.)");
  case Emacs:
    return gettext ("emacs (i.e., `C-c', `M-C-c' etc.)");
  case question_mark:
    return gettext ("question-mark (i.e., `?')");
  }
  error (1, 0,
