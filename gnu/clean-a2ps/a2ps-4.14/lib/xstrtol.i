...
static int
bkm_scale (long int *x, int scale_factor)
{
  long int product = *x * scale_factor;
...
static int
bkm_scale_by_power (long int *x, int base, int power)
{
  while (power--)
    if (bkm_scale (x, base))
      return 1;
...
strtol_error
xstrtol (const char *s, char **ptr, int strtol_base,
    long int *val, const char *valid_suffixes)
{
  ...
  ((0 <= strtol_base && strtol_base <= 36) ? (void) (0) : __assert_fail ("0 <= strtol_base && strtol_base <= 36", "xstrtol.c", 123, __PRETTY_FUNCTION__));

  p = (ptr ? ptr : &t_ptr);

  if (! (! ((long int) 0 < (long int) -1)))
    {
      const char *q = s;
      while ((1 && ((*__ctype_b_loc ())[(int) (((unsigned char) *q))] & (unsigned short int) _ISspace)))
 ++q;
      if (*q == '-')
