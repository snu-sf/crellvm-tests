...
config_hook delegation_hook = ((void*)0);
config_hook toc_entry_hook = ((void*)0);
...
# 93 "confg.gperf"
 {"DefaultPPD:", DefaultPPD, 1, 0},
# 98 "confg.gperf"
 {"LibraryPath:", LibraryPath, 1, 0},
# 102 "confg.gperf"
 {"Options:", Options, 1, 1},
# 94 "confg.gperf"
 {"DefaultPrinter:", DefaultPrinter, 1, 1},
# 103 "confg.gperf"
 {"OutputFirstLine:", OutputFirstLine, 1, 1},
# 105 "confg.gperf"
 {"PassThrough:", Obsolete, 1, 1},
# 106 "confg.gperf"
 {"Pattern:", Obsolete, 1, 1},
# 110 "confg.gperf"
 {"UnknownPrinter:", UnknownPrinter, 1, 1},
# 95 "confg.gperf"
 {"Delegation:", Delegation, 1, 1},
# 96 "confg.gperf"
 {"FileCommand:", FileCommand, 1, 1},
# 99 "confg.gperf"
 {"MacroMetaSequence:", Variable, 2, 1},
# 109 "confg.gperf"
 {"TemporaryDirectory:", Obsolete, 1, 1},
# 111 "confg.gperf"
 {"UserOption:", UserOption, 2, 1},
# 108 "confg.gperf"
 {"Printer:", Printer, 2, 1},
# 107 "confg.gperf"
 {"PrependLibraryPath:", PrependLibraryPath, 1, 1},
# 104 "confg.gperf"
 {"PageLabelFormat:", PageLabelFormat, 1, 1},
# 92 "confg.gperf"
 {"AppendLibraryPath:", AppendLibraryPath, 1, 1}
    };
...
a2_read_config (a2ps_job * job, const char *path, const char *file)
{
  ...
  fname = xpath_concat (path, file, ((void*)0));
  ...
  do { if ((((1 << 5) | (1 << 6)) & msg_verbosity)) fprintf (stderr, "Reading configuration file `%s'\n", fname); } while (0);
 ...
 error_at_line (1, 0, fname, firstline,
         gettext ("invalid option `%s'"), quotearg (argv[0]));


      for (argc = 1 ; argc <= keyword->argc ; argc++)
 {
   if (keyword->line_token && argc == keyword->argc)
     argv [argc] = strtok (((void*)0), "\n");
   else
     argv [argc] = strtok (((void*)0), " \t\n");
   if (argv [argc] == ((void*)0))
     error_at_line (1, 0, fname, firstline,
      gettext ("missing argument for `%s'"), quotearg (argv[0]));
 }
 ...
 case Include:
   {
     ...
     if (!a2_read_config (job, dir, argv[1]))
       {
  char *included_file = xpath_concat (dir, argv[1], ((void*)0));
  error_at_line (0, (*__errno_location ()), fname, firstline,
 ...
 case Options:
   {
     ...
     program_name = ((char *) __builtin_alloca (sizeof (char) * (strlen (program_name) + strlen (fname) + strlen ("%:%:999990"))));
 ...
 case Printer:
   if (!a2ps_printers_add (job->printers, argv[1], argv[2]))
     error_at_line (1, 0, fname, firstline,
      gettext ("invalid definition for printer `%s': %s"),
      argv[1], quotearg (argv[2]));
   break;

 case UnknownPrinter:
   if (!a2ps_printers_add (job->printers, gettext ("Unknown Printer"), argv[1]))
     error_at_line (1, 0, fname, firstline,
      gettext ("invalid definition for printer `%s': %s"),
      gettext ("Unknown Printer"), quotearg (argv[1]));
   break;

 case DefaultPrinter:
   if (!a2ps_printers_add (job->printers, gettext ("Default Printer"), argv[1]))
     error_at_line (1, 0, fname, firstline,
      gettext ("invalid definition for printer `%s': %s"),
      gettext ("Default Printer"), quotearg (argv[1]));
   break;
 ...
 case OutputFirstLine:
   do { const unsigned char *my_s2 = (unsigned char *) (argv[1]); do { if (job->status->magic_number) free (job->status->magic_number); } while (0); job->status->magic_number = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
   break;

 case PageLabelFormat:
   do { const unsigned char *my_s2 = (unsigned char *) (argv[1]); do { if (job->status->page_label_format) free (job->status->page_label_format); } while (0); job->status->page_label_format = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
   break;
 ...
 case LibraryPath:
   do { if (job->common.path) free (job->common.path); } while (0);
 ...
 case FileCommand:
   do { const char *my_s2 = (argv[1]); do { if (job->file_command) free (job->file_command); } while (0); job->file_command = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
   break;
 ...
 case Obsolete:
   ...
   error_at_line (0, 0, fname, firstline,
    gettext ("obsolete `%s' entry.  Ignored"), argv[0]);
   break;
 }
    }
  do { if (fname) free (fname); } while (0);
  fclose (fp);
