...
int from;
{
  ...
  if (vector != ((void*)0))
    {
      for (scan = vector ; (from > 0) && (*scan != ((void*)0)) ; from--)
  scan ++;

      for ( ; *scan != ((void*)0); scan++)
 free (*scan);
      free (vector);
    }
}
  ...
  int * argc;
{
  ...
  if (input != ((void*)0))
    {
      copybuf = __builtin_alloca (strlen (input) + 1);


      do
 {

   while ((1 && ((*__ctype_b_loc ())[(int) (((int) *input))] & (unsigned short int) _ISspace)))
            input++;
       ...
       if (nargv == ((void*)0))
  {
   ...
   while (*input != '\0')
     {
       if ((1 && ((*__ctype_b_loc ())[(int) (((int) *input))] & (unsigned short int) _ISspace)) && !squote && !dquote && !bsquote)
  break;
       else
  {
    if (bsquote)
