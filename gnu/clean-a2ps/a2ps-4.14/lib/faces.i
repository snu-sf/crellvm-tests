...
string_to_face (const char * string)
{
  ...
  for (i = 0 ; face_and_name[i].name ; i++)
    if ((strcmp ((string), (face_and_name[i].name)) == 0))
...
check_face_eo_font (struct a2ps_job * job)
{
  ...
  if (err)
    error (1, 0, gettext ("incomplete knowledge of faces"));
}
...
init_face_eo_font (struct a2ps_job * job)
{
  enum face_e f;
  for (f = First_face ; f <= Last_face ; f++)
    job->face_eo_font [f] = ((void*)0);
}

void
face_eo_font_free (char * face_eo_font [10])
{
  enum face_e f;
  for (f = First_face ; f <= Last_face ; f++)


    do { if (face_eo_font [f]) free (face_eo_font [f]); } while (0);
}
