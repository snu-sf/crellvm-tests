...
{
  struct passwd *passwd = ((void*)0);
  const char *home, *login;
  char *comments = ((void*)0), *name = ((void*)0), *cp;
      ...
      if (login)
 {
   ...
   for (cp = passwd->pw_gecos; *cp; cp++)
     if (*cp == '&')
       amps++;
   gecos = ((char *) __builtin_alloca (sizeof (char) * ((strlen (passwd->pw_gecos) + amps * (strlen (login) - 1) + 1))));
 ...
 {
   gecos = ((char *) __builtin_alloca (sizeof (char) * (strlen (passwd->pw_gecos) + 1)));
      ...
      if ((comments = strchr (gecos, ',')))
 {
  ...
  udata->login = xstrdup (login ? login : gettext ("user"));
  udata->name = xstrdup (name ? name : gettext ("Unknown User"));
  udata->comments = comments ? xstrdup (comments) : ((void*)0);
