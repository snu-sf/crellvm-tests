...
{
  return (string_htable_get (ppd->fonts, fontname) != ((void*)0));
}
...
ppd_new (void)
{
  struct ppd *(res) = ((struct ppd *) xmalloc (sizeof (struct ppd) * (1)));
  res->fonts = string_htable_new ();
  res->key = ((void*)0);
  res->modelname = ((void*)0);
  res->nickname = ((void*)0);
  return res;
}
...
ppd_free (struct ppd * ppd)
{
  if (ppd) {
    ...
    do { if (ppd->key) free (ppd->key); } while (0);
    do { if (ppd->modelname) free (ppd->modelname); } while (0);
    do { if (ppd->nickname) free (ppd->nickname); } while (0);
    free (ppd);
  }
}
...
ppd_self_print (struct ppd * ppd, FILE * stream)
{
  ...
  if (ppd->nickname)
    ...
    cp = ppd->key;
  title (stream, '-', 1, "%s (%s)", cp, ppd->key);


  fonts = string_htable_dump_sorted (ppd->fonts);
  fputs (gettext ("Known Fonts"), stream);
  if (!*fonts)
    ...
    {
      _IO_putc ('\n', stream);
      lister_fprint_separated (((void*)0), stream,
          (void **) fonts, (size_t) -1,
...
{
  fputs (gettext ("Known PostScript Printer Descriptions"), stream);
  _IO_putc ('\n', stream);
...
_a2ps_ppd_list_long (char * const * path, FILE * stream)
{
  ...
  title (stream, '=', 1, gettext ("Known PostScript Printer Descriptions"));
  _IO_putc ('\n', stream);
