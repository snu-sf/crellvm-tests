        ...
        int min, int max, enum range_type_e range_type)
{
  ...
  if ((sscanf (arg, "%d%255s", &res, buf) != 1))
    error (1, 0, gettext ("invalid argument `%s' for `%s'"),
    quotearg (arg), option);
 ...
 sprintf (buf, "%d < n < %d", min, max);
  ...
  if (buf [0])
    {
      error (0, 0,
      gettext ("invalid argument `%s' for `%s'"), quotearg (arg), option);
      fprintf (stderr,
        gettext ("Valid arguments are integers n such that: %s\n"), buf);
      exit (1);
    }
  return res;
}
      ...
      const char * unit, enum range_type_e range_type)
{
  ...
  switch (sscanf (arg, "%f%255s", &res, buf))
    ...
    case 2:
      ...
      res *= (types_list [__xargmatch_internal ((option), (buf), (args_list), (const char *) (types_list), sizeof (*(types_list)), 0, argmatch_die)]);
      break;
  ...
  if (buf [0])
    {
      ...
      gettext ("invalid argument `%s' for `%s'"), quotearg (arg), option);
      fprintf (stderr,
        gettext ("Valid arguments are floats f such that: %s\n"), buf);
      exit (1);
    }
