...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((((const struct medium *)key)->name)) - 1; while (*++kk) (result) += (tolower (*kk) << (tolower (kk[1]) & 0xf)); } while (0); return result; } while (0);
}
...
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) (((const struct medium *)x)->name) - 1; unsigned char const *yy = (unsigned char const *) (((const struct medium *)y)->name) - 1; do { if (*++xx == '\0') { yy++; break; } yy++; } while (tolower (*xx) == tolower (*yy)); (result) = tolower(*xx) - tolower(*yy); } while (0); return result; } while (0);
...
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) ((*(struct medium * const *)x)->name) - 1; unsigned char const *yy = (unsigned char const *) ((*(struct medium * const *)y)->name) - 1; do { if (*++xx == '\0') { yy++; break; } yy++; } while (tolower (*xx) == tolower (*yy)); (result) = tolower(*xx) - tolower(*yy); } while (0); return result; } while (0);
...
a2ps_get_medium (a2ps_job * job, const char * name)
{
  ...
  token.name = ((void*)0);
  if ((!strcasecmp(name, "libpaper")))
    token.name = (char *) ((void*)0);;
  ...
  if (item == ((void*)0))
    error (1, 0, gettext ("unknown medium `%s'"), quotearg (token.name));
...
_Bool
a2ps_medium_libpaper_p (__attribute__ ((unused)) a2ps_job * job, const char * name)
{
  return (strcmp ((name), ("libpaper")) == 0) ? 1 : 0;
}
...
list_media_long (a2ps_job * job, FILE * stream)
{
  int i;
  struct medium ** media;
  media = (struct medium **) hash_dump (job->media, ((void*)0), medium_hash_qcmp);

  fputs (gettext ("Known Media"), stream);
  _IO_putc ('\n', stream);
