...
{
  struct interval * res = ((struct interval *) xmalloc (sizeof (struct interval) * (1)));
  res->min = min;
  res->max = max;
  return res;
}
...
interval_to_buffer (struct interval * interval, unsigned char * buf, int offset)
{
  ...
  return buf + (strlen((const char *)(buf)));
}
# 125 "prange.c"
static inline int
...
page_range_new (void)
{
  struct page_range *(res) = ((struct page_range *) xmalloc (sizeof (struct page_range) * (1)));
  res->intervals = da_new ("Page Range Interval", 5, da_linear, 10,
      (da_print_func_t) interval_self_print, ((void*)0));
  res->toc = 0;
  return res;
}
...
a2ps_page_range_set_string (struct a2ps_job * job, const char * string)
{
  ...
  do { const char * _tmp_ = (const char *) (string); string_copy = ((char *) __builtin_alloca (sizeof (char) * (strlen (_tmp_) + 1))); strcpy (string_copy, _tmp_); } while (0);
  cp = strtok (string_copy, ", \t");

  while (cp)
    {
      min = max = 0;
      if (((*__ctype_b_loc ())[(int) (((int) *cp))] & (unsigned short int) _ISdigit))
 {
   min = atoi (cp);
   while (((*__ctype_b_loc ())[(int) (((int) *cp))] & (unsigned short int) _ISdigit))
     cp++;
 }
      switch (*cp++)
 {
 ...
 case '-':
   ...
   if (!add_pages_interval (job, min, max))
     error (1, 0,
     gettext ("invalid interval `%s'"), quotearg (string));
   break;

 case 't':
   if ((!strncmp("toc", cp - 1, strlen("toc"))))
     {

       job->page_range->toc = 1;
       cp += 2;
 ...
 failed:
 default:
   error (1, 0, gettext ("invalid interval `%s'"), quotearg (string));
   break;
 }
      cp = strtok (((void*)0), ", \t");
    }
...
print_page (struct a2ps_job * job, int page_num)
{
  ...
  if (job->page_range->toc && ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->is_toc)
    return 1;
