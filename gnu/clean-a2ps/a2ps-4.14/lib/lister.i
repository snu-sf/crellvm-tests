...
lister_initialize (struct lister *lister, FILE *stream)
{
  ...
  tterm_initialize (((void*)0), stream);
  l->stream = stream;
}
# 210 "lister.c"
static void
init_worlds (struct lister * l)
{
  ...
  if (l->multicol.worlds == ((void*)0))
    {
      l->multicol.worlds = ((struct world *) xmalloc (sizeof (struct world) * (line_width)));
      for (i = 0; i < line_width; ++i)
 l->multicol.worlds[i].widths = ((size_t *) xmalloc (sizeof (size_t) * (i + 1)));
    }

  max_idx = ((line_width
       - l->before - l->after - l->multicol.between)
             / (1 + l->multicol.between));
  if (max_idx == 0)
    max_idx = 1;
  ...
  for (i = 0; i < max_idx; ++i)
    {
      ...
      l->multicol.worlds[i].width = (i + 1) * 1;

      for (j = 0; j <= i; ++j)
 l->multicol.worlds[i].widths[j] = 1;
    }
}
...
void
lister_fprint_vertical (struct lister * lister, __attribute__ ((unused)) FILE *unused,
   void **items, size_t item_number,
   lister_width_t item_width_fn,
   lister_print_t item_print_fn)
{
      ...
      while (1)
 {
   ...
   nextcolpos += col_width + m->between;
          ...
          do { size_t from = pos; size_t to = nextpos; while (from < to) { if (tabsize > 0 && to / tabsize > (from + 1) / tabsize) { _IO_putc ('\t', stream); from += tabsize - from % tabsize; } else { _IO_putc (' ', stream); from++; } } } while (0);
...
{
  struct lister *l = lister ? lister : &lister_default;
  lister_fprint_vertical (lister, ((void*)0),
     items, item_number,
...
void
lister_fprint_horizontal (struct lister * lister, __attribute__ ((unused)) FILE *unused,
     void **items, size_t item_number,
     lister_width_t item_width_fn,
     lister_print_t item_print_fn)
{
...
{
  struct lister *l = lister ? lister : &lister_default;
  lister_fprint_horizontal (lister, ((void*)0),
      items, item_number,
      l->width_fn, l->print_fn);
}
# 579 "lister.c"
void
lister_fprint_separated (struct lister * lister,__attribute__ ((unused)) FILE *unused,
    void **items, size_t item_number,
    lister_width_t item_width_fn,
    lister_print_t item_print_fn)
{
  ...
  pos = l->before;
  do { size_t from = 0; size_t to = pos; while (from < to) { if (tabsize > 0 && to / tabsize > (from + 1) / tabsize) { _IO_putc ('\t', stream); from += tabsize - from % tabsize; } else { _IO_putc (' ', stream); from++; } } } while (0);
      ...
      if (itemno)
 {
   if (pos + s->between > line_width)
     {
       _IO_putc ('\n', stream);
       do { size_t from = 0; size_t to = l->before; while (from < to) { if (tabsize > 0 && to / tabsize > (from + 1) / tabsize) { _IO_putc ('\t', stream); from += tabsize - from % tabsize; } else { _IO_putc (' ', stream); from++; } } } while (0);
       pos = pos - old_pos + l->before;
     }
   else
     {
       do { size_t from = old_pos; size_t to = old_pos + s->between; while (from < to) { if (tabsize > 0 && to / tabsize > (from + 1) / tabsize) { _IO_putc ('\t', stream); from += tabsize - from % tabsize; } else { _IO_putc (' ', stream); from++; } } } while (0);
       pos += s->between;
     }
 }
      (*item_print_fn) (items[itemno], stream);
...
{
  struct lister *l = lister ? lister : &lister_default;
  lister_fprint_separated (lister, ((void*)0),
      items, item_number,
      l->width_fn, l->print_fn);
}
