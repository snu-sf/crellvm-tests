...
{
  {
    ...
    userdata_get (&u);
    if (u.login)
      macro_meta_sequence_add (job, "user.login", u.login);
    if (u.name)
      macro_meta_sequence_add (job, "user.name", u.name);
    if (u.comments)
      macro_meta_sequence_add (job, "user.comments", u.comments);
    if (u.home)
      macro_meta_sequence_add (job, "user.home", u.home);
    userdata_free (&u);
  }

  {
    char *host = xgethostname ();
    macro_meta_sequence_add (job, "user.host", host);
    free (host);
  }
}
...
tmpfiles_reset (struct a2ps_job * job)
{
  ...
  for (i = 0 ; i < (sizeof (job->tmp_filenames) / sizeof ((job->tmp_filenames)[0])) ; i ++)
    job->tmp_filenames [i] = ((void*)0);
}
...
tmpfiles_free (struct a2ps_job * job)
{
  ...
  for (i = 0 ; i < (sizeof (job->tmp_filenames) / sizeof ((job->tmp_filenames)[0])) ; i ++)
    do { if (job->tmp_filenames[i]) free (job->tmp_filenames[i]); } while (0);
}
...
a2ps_job_new (void)
{
  ...
  res = ((a2ps_job *) xmalloc (sizeof (a2ps_job) * (1)));
  ...
  lister_initialize (((void*)0), stdout);
  lister_before_set (((void*)0), 2);
  ...
  res->file_command = ((void*)0);




  res->media = new_medium_table ();
  ...
  res->output_format = ps;

  res->printers = a2ps_printers_new (&res->common);
  res->output_stream = ((void*)0);

  res->folding = 1;
  res->numbering = 0;
  res->unprintable_format = caret;
  res->interpret = 1;
  res->print_binaries = 0;

  res->file_align = -2;
  res->border = 1;
  res->debug = 0;
  res->prolog = xstrdup ("bw");
  res->medium_request = ((void*)0);
  res->medium = ((void*)0);
  res->tabsize = 8;
  res->lines_requested = 0;
  res->columns_requested = 0;
  res->fontsize = 0.0;
  res->encoding = ((void*)0);
  res->requested_encoding_name = ((void*)0);
  res->requested_encoding = ((void*)0);
  res->saved_encoding = ((void*)0);
  ...
  res->title = ((unsigned char *) xstrdup((const char *)("a2ps output")));


  res->header = (unsigned char *) ((void*)0);
  res->center_title = (unsigned char *) ((void*)0);
  res->left_title = (unsigned char *) ((void*)0);
  res->right_title = (unsigned char *) ((void*)0);
  res->left_footer = (unsigned char *) ((void*)0);
  res->footer = (unsigned char *) ((void*)0);
  res->right_footer = (unsigned char *) ((void*)0);
  res->water = (unsigned char *) ((void*)0);
  * res->tag1 = '\0';
  * res->tag2 = '\0';
  * res->tag3 = '\0';
  * res->tag4 = '\0';
  ...
  res->jobs = da_new ("List of the jobs", 10,
        da_linear, 10,
        (da_print_func_t) file_job_self_print, ((void*)0));
  return res;
}
...
a2ps_job_finalize (struct a2ps_job * job)
{
  ...
  if (!job->requested_encoding)
    error (1, 0, gettext ("unknown encoding `%s'"),
    quotearg (((((void const *) job->requested_encoding_name) == ((void*)0)) ? (const char *) "" : (const char *) (job->requested_encoding_name))));
...
a2ps_job_free (struct a2ps_job * job)
{
  ...
  do { if (job->stdin_filename) free (job->stdin_filename); } while (0);
  ...
  do { if (job->title) free (job->title); } while (0);
