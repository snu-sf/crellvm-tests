...
find_backup_file_name (const char *file, enum backup_type backup_type)
{
  ...
  size_t numbered_suffix_size_max = ((sizeof (int) * 8 - 1) * 302 / 1000 + 2) + 4;
  char *s;
  const char *suffix = simple_backup_suffix;


  backup_suffix_size_max = strlen (simple_backup_suffix) + 1;
  if (1 && backup_suffix_size_max < numbered_suffix_size_max)
...
max_backup_version (const char *file, const char *dir)
{
  ...
  while ((dp = readdir (dirp)) != 0)
    {
      if (!1 || strlen ((dp)->d_name) < file_name_length + 4)
 continue;
...
version_number (const char *base, const char *backup, size_t base_length)
{
  ...
  if (strncmp (base, backup, base_length) == 0
    ...
    {
      for (p = &backup[base_length + 2]; ((unsigned) (*p) - '0' <= 9); ++p)
...
get_version (const char *context, const char *version)
{
  if (version == 0 || *version == 0)
    ...
    return (backup_types [__xargmatch_internal ((context), (version), (backup_args), (const char *) (backup_types), sizeof (*(backup_types)), 1, argmatch_die)]);
}
# 261 "backupfile.c"
enum backup_type
