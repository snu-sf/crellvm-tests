...
{
  for ( ; *string != '\0'; string++)
    if (((*__ctype_b_loc ())[(int) ((*string))] & (unsigned short int) _ISupper))
      return 0;
  return 1;
}
...
_xfopen (const char * filename, const char * rights, const char * format)
{
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "%s-fopen (%s)\n", rights, quotearg (filename)); } while (0);

  res = fopen (filename, rights);
  if (!res)
    error (1, (*__errno_location ()), format, quotearg (filename));
  return res;
}
...
{
  return _xfopen (filename, "r", gettext ("cannot open file `%s'"));
}

FILE *
xwfopen (const char * filename)
{
  return _xfopen (filename, "w", gettext ("cannot create file `%s'"));
}
...
_xpopen (const char * filename, const char * rights, const char * format)
{
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "%s-popen (%s)\n", rights, filename); } while (0);
...
{
  return _xpopen (filename, "r", gettext ("cannot open a pipe on `%s'"));
}

FILE *
xwpopen (const char * filename)
{
  return _xpopen (filename, "w", gettext ("cannot open a pipe on `%s'"));
}
...
void
unlink2 (__attribute__ ((unused)) void * dummy, const char * filename)
{
  do { if ((((1 << 7) | (1 << 6)) & msg_verbosity)) fprintf (stderr, "Unlinking file `%s'\n", filename); } while (0);
