...
static const yytype_uint8 ppdr1[] =
{
...
static const yytype_uint8 ppdr2[] =
{
       0, 2, 1, 0, 3, 5, 5, 2, 2, 2,
       2, 2, 7, 3
};
...
static const yytype_int8 ppdpact[] =
{
      -4, 9, -3, -4, -4, -1, 2, 1, 3, -4,
...
static const yytype_uint8 ppdyytable[] =
{
...
static const yytype_int8 ppdyycheck[] =
{
...
# 800 "parseppd.c"
static void
yy_reduce_print (YYSTYPE *yyvsp, int ppdyyrule)
...
{
  int yynrhs = ppdr2[ppdyyrule];
  int yyi;
  unsigned long int yylno = yyrline[ppdyyrule];
  ...
  for (yyi = 0; yyi < yynrhs; yyi++)
    {
      ...
      yy_symbol_print (stderr, yyrhs[yyprhs[ppdyyrule] + yyi],
         &(yyvsp[(yyi + 1) - (yynrhs)])
                  );
      fprintf (stderr, "\n");
    }
...
# 968 "parseppd.c"
static long unsigned int
yysyntax_error (char *yyresult, int ppdstate, int ppdchar)
{
  int yyn = ppdpact[ppdstate];

  if (! (-4 < yyn && yyn <= 26))
    return 0;
  else
    {
      int yytype = ((unsigned int) (ppdchar) <= 265 ? yytranslate[ppdchar] : 2);
...
# 995 "parseppd.c"
      ...
      int yychecklim = 26 - yyn + 1;
      int yyxend = yychecklim < 12 ? yychecklim : 12;
      int yycount = 1;
 ...
 if (ppdyycheck[yyx + yyn] == yyx && yyx != 1)
   {
...

{
  ...
  if (!yymsg)
    yymsg = "Deleting";
  do { if (ppddebug) { fprintf (stderr, "%s ", yymsg); yy_symbol_print (stderr, yytype, yyvaluep); fprintf (stderr, "\n"); } } while ((0));
...
# 1182 "parseppd.c"
  ...
  do { if (ppddebug) fprintf (stderr, "Starting parse\n"); } while ((0));
...
# 1266 "parseppd.c"
 ...
 if (yyss1 != yyssa)
   free (yyss1);
      ...
      do { if (ppddebug) fprintf (stderr, "Stack size increased to %lu\n", (unsigned long int) yystacksize); } while ((0));
 ...
 goto yyabortlab;
  ...
  do { if (ppddebug) fprintf (stderr, "Entering state %d\n", ppdstate); } while ((0));
...
yybackup:
  ...
  if (ppdchar == (-2))
    {
      do { if (ppddebug) fprintf (stderr, "Reading a token: "); } while ((0));
      ppdchar = ppdlex ();
    }
  ...
  if (ppdchar <= 0)
    {
      ppdchar = yytoken = 0;
      do { if (ppddebug) fprintf (stderr, "Now at end of input.\n"); } while ((0));
    }
  else
    {
      yytoken = ((unsigned int) (ppdchar) <= 265 ? yytranslate[ppdchar] : 2);
      do { if (ppddebug) { fprintf (stderr, "%s ", "Next token is"); yy_symbol_print (stderr, yytoken, &ppdlval); fprintf (stderr, "\n"); } } while ((0));
    }



  yyn += yytoken;
  if (yyn < 0 || 26 < yyn || ppdyycheck[yyn] != yytoken)
    goto yydefault;
  yyn = ppdyytable[yyn];
  if (yyn <= 0)
    {
 ...
 goto yyerrlab;
  ...
  do { if (ppddebug) { fprintf (stderr, "%s ", "Shifting"); yy_symbol_print (stderr, yytoken, &ppdlval); fprintf (stderr, "\n"); } } while ((0));
...
# 1396 "parseppd.c"
  ...
  do { if (ppddebug) yy_reduce_print (yyvsp, yyn); } while ((0));
  switch (yyn)
    {
        case 2:
# 93 "parseppd.y"
...
# 138 "parseppd.y"
 {
  ...
  case 12:
# 148 "parseppd.y"
 { (ppdval.string)= (yyvsp[(2) - (7)].string); }
    break;

  case 13:
# 150 "parseppd.y"
 { (ppdval.string)= (yyvsp[(3) - (3)].string); }
    break;
# 1491 "parseppd.c"
 default: break;
    }
  do { if (ppddebug) { fprintf (stderr, "%s ", "-> $$ ="); yy_symbol_print (stderr, ppdr1[yyn], &ppdval); fprintf (stderr, "\n"); } } while ((0));

  (yyvsp -= (ppdyylen), yyssp -= (ppdyylen));
  ppdyylen = 0;
  do { if (ppddebug) yy_stack_print ((yyss), (yyssp)); } while ((0));
  ...
  *++yyvsp = ppdval;
  ...
  ppdstate = yypgoto[yyn - 12] + *yyssp;
  if (0 <= ppdstate && ppdstate <= 26 && ppdyycheck[ppdstate] == *yyssp)
    ppdstate = ppdyytable[ppdstate];
  else
    ppdstate = yydefgoto[yyn - 12];
...
yyerrlab:
  ...
  if (!yyerrstatus)
    {
      ...
      {
 long unsigned int yysize = yysyntax_error (0, ppdstate, ppdchar);
 ...
 if (0 < yysize && yysize <= yymsg_alloc)
   {
     (void) yysyntax_error (yymsg, ppdstate, ppdchar);
     ppderror (yymsg);
   }
 else
   {
...
yyerrlab1:
 ...
 {
   yyn += 1;
   if (0 <= yyn && yyn <= 26 && ppdyycheck[yyn] == 1)
     {
       yyn = ppdyytable[yyn];
       if (0 < yyn)
  break;
  ...
  *++yyvsp = ppdlval;
  ...
  do { if (ppddebug) { fprintf (stderr, "%s ", "Shifting"); yy_symbol_print (stderr, yystos[yyn], yyvsp); fprintf (stderr, "\n"); } } while ((0));
...
a2ps_ppd_parse (const char * filename, char * const * path)
{
  ...
  do { if ((((1 << 6) | (1 << 9) | (1 << 11)) & msg_verbosity)) fprintf (stderr, "Parsing file `%s'\n", ppdfilename); } while (0);
