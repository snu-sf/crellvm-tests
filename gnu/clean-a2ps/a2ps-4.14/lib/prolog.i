...
a2ps_job_stream_hook ps_comment_hook = ((void*)0);
# 95 "prolog.c"
void
prologues_list_short (a2ps_job * job, FILE * stream)
{
  fputs (gettext ("Known Prologues"), stream);
  _IO_putc ('\n', stream);
  pw_lister_on_suffix (stream, job->common.path, ".pro");
}
     ...
     documentation_fn_t documentation_fn)
{
  ...
  filename = xpw_find_file (job->common.path, prologue_name, ".pro");
  fp = xrfopen (filename);
  ...
  && !done)
    {
      ...
      if ((!strncmp("Documentation", buf, strlen("Documentation"))))
 {
         ...
         documentation_fn_t documentation_fn)
{
  ...
  entries = pw_glob_on_suffix (job->common.path, ".pro");
...
dump_prolog_comments (FILE * stream, struct a2ps_job * job)
{
  ...
  cp = expand_user_string (job, ((struct file_job *) (job->jobs->content[0])),
...
output_prolog (a2ps_job * job)
{
  ...
  if (job->debug)
    output_file (job->divertion, job, "ehandler", ".ps");


  output (job->divertion, "/a2psdict 200 dict def\n");
  output (job->divertion, "a2psdict begin\n");


  output (job->divertion, "%%%%BeginProlog\n");

  output_file (job->divertion, job, job->prolog, ".pro");
  output (job->divertion, "%%%%EndProlog\n");
}
...
output_document_setup (a2ps_job * job)
{
  ...
  output (job->divertion, "%%%%BeginSetup\n");
  ...
  output (job->divertion, "%% Initialize page description variables.\n");
  switch (job->orientation)
    {
    case portrait:

      output (job->divertion, "/sh %d def\n", job->medium->h);
      output (job->divertion, "/sw %d def\n", job->medium->w);
      output (job->divertion, "/llx %d def\n", job->medium->llx);
      output (job->divertion, "/urx %d def\n", job->medium->urx - job->margin);
      output (job->divertion, "/ury %d def\n", job->medium->ury);
      output (job->divertion, "/lly %d def\n", job->medium->lly);
      break;
  ...
  output (job->divertion, "/#copies %d def\n", job->copies);


  if (job->page_prefeed)
    output (job->divertion, "true page_prefeed\n");
  ...
  output (job->divertion, "/th %f def\n", job->status->title_bar_height);



  output (job->divertion, "/fnfs %d def\n", job->status->title_font_size);
  output (job->divertion, "/bfs %f def\n", job->fontsize);
  output (job->divertion, "/cw %f def\n", job->fontsize * 0.6);
  ...
  output (job->divertion, "/hm fnfs 0.25 mul def\n");


  output (job->divertion, "/pw\n");
  output (job->divertion, "   cw %f mul\n",
   (float) job->status->columnsperline + 2 * 0.7);
  output (job->divertion, "def\n");
  output (job->divertion, "/ph\n");
  output (job->divertion, "   %f th add\n",
   (job->status->linesperpage + 0.7) * job->fontsize);
  output (job->divertion, "def\n");
  if (job->columns > 1)
    output (job->divertion, "/pmw urx llx sub pw %d mul sub %d div def\n",
     job->columns, job->columns - 1);
  else
    output (job->divertion, "/pmw 0 def\n");
  if (job->rows > 1)
    output (job->divertion, "/pmh ury lly sub ph %d mul sub %d sub %d div def\n",
     job->rows,
     ((!(((const char *) (job->header) == ((void*)0)) || (*(job->header) == '\0'))) + (!(((const char *) (job->footer) == ((void*)0)) || (*(job->footer) == '\0')) || !(((const char *) (job->left_footer) == ((void*)0)) || (*(job->left_footer) == '\0')) || !(((const char *) (job->right_footer) == ((void*)0)) || (*(job->right_footer) == '\0')))) * 12,
     job->rows - 1);
  else
    output (job->divertion, "/pmh 0 def\n");
  output (job->divertion, "/v 0 def\n");

  switch (job->madir) {
  case madir_rows:
    output (job->divertion, "/x [\n");
    for (j = 1 ; j <= job->rows ; j++) {
      output (job->divertion, "  0\n");
      for (i = 2 ; i <= job->columns ; i++)
 output (job->divertion, "  dup pmw add pw add\n");
    }
    output (job->divertion, "] def\n");

    output (job->divertion, "/y [\n");
    for (j = job->rows ; 1 <= j ; j--) {
      output (job->divertion, "  pmh ph add %d mul ph add\n", j - 1);
      for (i = 2 ; i <= job->columns ; i++)
 output (job->divertion, "  dup\n");
    }
    output (job->divertion, "] def\n");
    break;
  ...
  case madir_columns:
    ...
    for (i = 1 ; i <= job->columns ; i++) {
      output (job->divertion, "  pmw pw add %d mul\n", i - 1);
 ...
 output (job->divertion, "  pmh ph add %d mul ph add\n", j - 1);
  ...
  output (job->divertion, "/scx sw 2 div def\n");
  output (job->divertion, "/scy sh 2 div def\n");

  output (job->divertion, "/snx urx def\n");
  output (job->divertion, "/sny lly 2 add def\n");
  output (job->divertion, "/dx llx def\n");
  output (job->divertion, "/dy sny def\n");
  output (job->divertion, "/fnx scx def\n");
  output (job->divertion, "/fny dy def\n");
  output (job->divertion, "/lx snx def\n");
  output (job->divertion, "/ly ury fnfs 0.8 mul sub def\n");
  output (job->divertion, "/sx %d def\n", (job->numbering ? 5 : 0));
  output (job->divertion, "/tab %d def\n", job->tabsize);
  output (job->divertion, "/x0 0 def\n");
  output (job->divertion, "/y0 0 def\n");


  output (job->divertion, "%%%%EndSetup\n\n");
}
...
end_document (a2ps_job * job)
{
  ...
  if (job->page_prefeed)
    output (job->divertion, "false page_prefeed\n");


  output (job->divertion, "end\n");
  output (job->divertion, "%%%%EOF\n");
}
...
{
  float area_w = 0.0, area_h = 0.0;
  float printing_h, printing_w;
  ...
  if (!(!(((const char *) (job->center_title) == ((void*)0)) || (*(job->center_title) == '\0')) || !(((const char *) (job->left_title) == ((void*)0)) || (*(job->left_title) == '\0')) || !(((const char *) (job->right_title) == ((void*)0)) || (*(job->right_title) == '\0')))) {
    job->status->title_font_size = 11;
    job->status->title_bar_height = 0.0;
  } else if (job->columns * job->rows > 1) {
    job->status->title_font_size = 11;
    job->status->title_bar_height = 15;
  } else {
    job->status->title_font_size = 15;
    job->status->title_bar_height = 20;
  }
...
# 555 "prolog.c"
  if (job->columns_requested != 0) {
    job->fontsize = ((printing_w /
        (job->columns_requested + (job->numbering ? 5 : 0) +
         2 * 0.7))
       / 0.6);
  } else if (job->lines_requested != 0) {
    job->fontsize = (printing_h /
       (job->lines_requested + 0.7));
  ...
  job->status->linesperpage =
    (int) ((printing_h / job->fontsize) - 0.7);
  job->status->columnsperline =
    (int) ((printing_w / (job->fontsize * 0.6)) - 2 * 0.7);

  if (job->columns_requested > 0)
    job->status->columnsperline = job->columns_requested + (job->numbering ? 5 : 0);
  else if (job->lines_requested > 0)
    job->status->linesperpage = job->lines_requested;

  if (job->status->linesperpage <= 0 || job->status->columnsperline <= 0)
    error (1, 0, gettext ("font %f too big"), job->fontsize);

  job->status->wxperline =
    (job->status->columnsperline - (job->numbering ? 5 : 0)) * 600;
