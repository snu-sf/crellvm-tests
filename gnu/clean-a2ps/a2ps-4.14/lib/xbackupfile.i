   ...
   enum backup_type backup_type)
{
  ...
  ((oflag & 0100) ? (void) (0) : __assert_fail ("oflag & 0100", "xbackupfile.c", 153, __PRETTY_FUNCTION__));

  if (backup_type == none)
    return open (file, oflag, mode);


  fd = open (file, oflag | 0200, mode);
# 175 "xbackupfile.c"
  if (fd < 0 && (*__errno_location ()) == 17)
    {


      if (stat (file, &st) == 0 && ((((st.st_mode)) & 0170000) == (0100000)))
 {
...
fopen_backup (const char * filename, enum backup_type backup_type)
{
 ...
 error (1, (*__errno_location ()), gettext ("cannot get informations on file `%s'"),
        quotearg (filename));
    }
  ...
  if (!((((filestat.st_mode)) & 0170000) == (0100000))
      || access (filename, 2))
    backup_type = none ;
 ...
 error (1, (*__errno_location ()), gettext ("cannot rename file `%s' as `%s'"),
  ...
  if (!res)
    {
      error (0, (*__errno_location ()), gettext ("cannot create file `%s'"), quotearg (filename));
      if (backup_name)
 {
   if (rename (filename, backup_name))
     error (0, (*__errno_location ()), gettext ("cannot rename file `%s' as `%s'"),
     quotearg (filename), quotearg (backup_name));
   else
     fprintf (stderr, gettext ("restored file `%s'"), quotearg (filename));
 }
      exit (1);
    }
