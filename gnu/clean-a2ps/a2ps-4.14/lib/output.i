...
# 83 "./dstring.h"
struct dstring *
ds_new (size_t size, enum ds_growth growth, size_t increment);
...
void ds_vsprintf (struct dstring *s, const char *format, va_list args);


void ds_cat_vsprintf (struct dstring *s, const char *format, va_list args);



void ds_unsafe_vsprintf (struct dstring *s, const char *format, va_list args);

void ds_unsafe_cat_vsprintf (struct dstring *s, const char *format, va_list args);
...
{
  struct derivation * res = ((struct derivation *) xmalloc (sizeof (struct derivation) * (1)));
  res->type = type;
  return res;
}
...
{
  struct output * res = ((void*)0);

  res = ((struct output *) xmalloc (sizeof (struct output) * (1)));
  res->name = name;
  res->chunk = ds_new (1024 * 50, ds_geometrical, 2);
  res->derivations = da_new ("derivations", 10,
       da_geometrical, 2,
      ...
      const char *name, const char *suffix)
{
  ...
  FILE * stream;
  char * filename;
  char * token = ((void*)0), * token2 = ((void*)0);
  int line = 0;
  int dont_output = 0;
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "Outputing file %s%s\n", name, ((((void const *) suffix) == ((void*)0)) ? (const char *) "" : (const char *) (suffix))); } while (0);
  ...
  while ((fgets (buf, sizeof (buf), stream))) {
    line++;
    if ((!strncmp("% -- code follows this line --", buf, strlen("% -- code follows this line --"))))
      break;
  }
  ...
  while ((fgets (buf, sizeof (buf), stream))) {
    ...
    if ((!strncmp("%%EndFontUsed", buf, strlen("%%EndFontUsed"))))
      {
 dont_output = 0;
 continue;
      }
    ...
    else if ((!strncmp("%%IfFontUsed:", buf, strlen("%%IfFontUsed:"))))
      {
# 368 "output.c"
 continue;
      }
    ...
    if ((!strncmp("%%IncludeResource:", buf, strlen("%%IncludeResource:"))))
      {
 char * value, * res, * buf2;
 buf2 = xstrdup (buf);
 token = (strtok ((buf), " \t\n"));
 token2 = (strtok ((((void*)0)), " \t\n"));
 if (token2 == ((void*)0)) error_at_line (1, 0, filename, line, gettext ("missing argument for `%s'"), quotearg (token));;
 res = token2;
 if ((strcmp ((res), ("file")) == 0))
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "Including file '%s' upon request given in '%s':%d\n", value, filename, line); } while (0);



  output_file (dest, job, value, ((void*)0));
       }
   }
 else
   {
 ...
 continue;
    ...
    else if ((!strncmp("%%DocumentProcessColors:", buf, strlen("%%DocumentProcessColors:"))))
      {
 token = (strtok ((buf), " \t\n"));
 token2= (strtok ((((void*)0)), " \t\n"));
 if (token2 == ((void*)0)) error_at_line (1, 0, filename, line, gettext ("missing argument for `%s'"), quotearg (token));;
 ...
 continue;
    ...
    else if ((!strncmp("%%BeginResource:", buf, strlen("%%BeginResource:"))))
      {
 char * res;
 output (dest, "%s", buf);
 ...
 continue;
    ...
    else if ((!strncmp("%%EndSetup", buf, strlen("%%EndSetup"))))
      {
 if (dest == out)
 ...
 continue;
    ...
    else if ((!strncmp("%%BeginSetup", buf, strlen("%%BeginSetup"))))
      {
 ...
 continue;
    ...
    else if ((!strncmp("%Face:", buf, strlen("%Face:"))))
      {
 ...
 if (face == No_face)
   ...
   error_at_line (1, 0, filename, line,
    gettext ("invalid face `%s'"), quotearg (token2));
 ...
 continue;
    ...
    else if ((!strncmp("%Font:", buf, strlen("%Font:"))))
      {
 char * basefontname;
 ...
 continue;
    ...
    else if ((!strncmp("%Expand:", buf, strlen("%Expand:"))))
      {
 char * expansion;

 token = (strtok ((buf + strlen ("%Expand:")), "\n"));
 expansion = ((char *)
       expand_user_string (job, ((struct file_job *) (job->jobs->content[0])),
...
output_dump (struct output * out, FILE * stream)
{
  ...
  if ((((1 << 7)) & msg_verbosity))
    output_report (out, stderr);

  fputs ((char *) piece, stream);
  piece += (strlen((const char *)(piece)));
