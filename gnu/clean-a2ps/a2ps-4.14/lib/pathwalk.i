...
pw_internal_string_to_path (const char * path, char sep, int * length)
{
  ...
  res = ((char * *) xcalloc (sizeof (char *), (allocated)));
      ...
      else
 {
   ...
   if (cp [len] == '/')
     len --;
      ...
      res [ entries ] = ((char *) xmalloc (sizeof (char) * (len + 1)));
 ...
 {
   allocated *= 2;
   res = ((char * *) xrealloc ((void *) (res), sizeof (char *) * (allocated)));
 }
    }
  *length = entries;
  ...
  res [*length] = ((void*)0);
  res = ((char * *) xrealloc ((void *) (res), sizeof (char *) * (*length + 1)));
  return res;
}
...
{
  int dummy;
  return pw_internal_string_to_path (path, ':', &dummy);
}
...
pw_path_concat (char ** path1, int len1, char ** path2, int len2)
{
  ...
  if (path2)
    {
      path1 = ((char * *) xrealloc ((void *) (path1), sizeof (char *) * (len1 + len2 + 1)));
      for (i = 0 ; i <= len2 ; i++)
...
pw_append_string_to_path (char ** path1, const char * dir2)
{
  ...
  path2 = pw_internal_string_to_path (dir2, ':', &len2);
      ...
      const char *name, const char *suffix)
{
  ...
  if (suffix)
    do { const char * _tmp1_ = (const char *) (name); const char * _tmp2_ = (const char *) (suffix); filename = ((char *) __builtin_alloca (sizeof (char) * ((strlen (_tmp1_) + strlen (_tmp2_) + 1)))); do { stpcpy (stpcpy (filename, name), suffix); } while (0); } while (0);
  else
    filename = (char *) name;

  do { if ((((1 << 3)) & msg_verbosity)) fprintf (stderr, "pw: looking for `%s'\n", filename); } while (0);


  if (path)
    for (i = 0 ; path [i] ; i ++)
      {
 do { char * tmp1 = (char *) (path [i]); const char * tmp2 = (const char *) (filename); fullpath = ((char *) __builtin_alloca (sizeof (char) * ((strlen (tmp1) + strlen (tmp2) + 2)))); tmp1 = stpcpy (fullpath, tmp1); (*tmp1++) = '/'; stpcpy (tmp1, tmp2); } while (0);
 if (stat (fullpath, &stat_st) == 0)
   {

     do { if ((((1 << 3)) & msg_verbosity)) fprintf (stderr, "pw: success in %s\n", path[i]); } while (0);
     return i + 1;
   }
      }

  if ((((1 << 3)) & msg_verbosity))
    {
  ...
  if (i)
    {
      ...
      if (suffix)
 {
   res = ((char *) xmalloc (sizeof (char) * (strlen (path[i-1]) + 2 + strlen (name) + strlen (suffix))));


   sprintf (res, "%s%c%s%s", path [i-1], '/',
     name, suffix);
 }
      else
 {
   res = ((char *) xmalloc (sizeof (char) * (strlen (path[i-1]) + 2 + strlen (name))));


   sprintf (res, "%s%c%s", path [i-1], '/',
     name);
 }
      return res;
    }
        ...
        const char * name, const char * suffix)
{
  ...
  if (!res)
    {
      char *file;
      file = ((char *) __builtin_alloca (sizeof (char) * (strlen (name) + (suffix ? strlen (suffix) : 0) + 1)));
      sprintf (file, "%s%s", name, ((((void const *) suffix) == ((void*)0)) ? (const char *) "" : (const char *) (suffix)));
      error (1, (*__errno_location ()), gettext ("cannot find file `%s'"), quotearg (file));
    }
  return res;
}
        ...
        const char * name, const char * suffix)
{
  ...
  do { if ((((1 << 3)) & msg_verbosity)) fprintf (stderr, "pw: pasting `%s%s'\n", name, suffix ? suffix : ""); } while (0);
  ...
  while ((fgets (buf, sizeof (buf), fp)))
    {
      line++;
      if ((strncmp ((buf), ("% -- code follows this line --"), (strlen ("% -- code follows this line --"))) == 0))
 break;
    }
  ...
  while ((fgets (buf, sizeof (buf), fp)))
    {
      line++;
      if ((strncmp ((buf), ("% -- include file:"), (strlen ("% -- include file:"))) == 0))
 {
   char * file = buf + strlen ("% -- include file:");
   file = strtok (file, " \n\t");
   do { if ((((1 << 3)) & msg_verbosity)) fprintf (stderr, "pw: including file '%s' upon request given in '%s':%d\n", file, fullpath, line); } while (0);



   if (!pw_paste_file (path, file, ((void*)0)))
     error_at_line (1, (*__errno_location ()), fullpath, line,
      gettext ("cannot find file `%s'"), quotearg (file));
   continue;
 }
      fputs (buf, stdout);
    }
...
static _Bool
pw_filter_fnmatch (__attribute__ ((unused)) const char * dir, const char *file,
     const char *pattern)
{
...
static void
pw_filter_da_append (__attribute__ ((unused)) const char * dir, const char *file,
       struct darray *da)
{
