 ...
 enum ds_growth growth, size_t increment)
{
  ...
  res = ((struct dstring *) xmalloc (sizeof (struct dstring) * (1)));
  res->len = 0;
  res->size = size;
  res->original_size = size;
  res->growth = growth;
  res->increment = increment;
  res->content = ((char *) xmalloc (sizeof (char) * (size)));
  res->content[0] = '\0';
...
ds_print_stats (struct dstring * str, FILE * stream)
{
  ...
  fprintf (stream, gettext ("Dynamic string:\n"));
...
ds_grow (struct dstring *string)
{
  ...
  string->content = ((char *) xrealloc ((void *) (string->content), sizeof (char) * (string->size)));
}
