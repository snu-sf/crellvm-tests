...
void string_htable_add (struct string_htable * table, const char * key);

char * string_htable_get (struct string_htable * table, const char * key);





void string_htable_self_print (struct string_htable * table, FILE * stream);
...
void hash_init (struct string_htable *ht, unsigned long size, hash_func_t hash_1, hash_func_t hash_2, hash_cmp_func_t hash_cmp);

void hash_load (struct string_htable *ht, void *item_table, unsigned long cardinality, unsigned long size);

void **hash_find_slot (struct string_htable *ht, void const *key);
void *hash_find_item (struct string_htable *ht, void const *key);
void *hash_insert (struct string_htable *ht, void *item);
void *hash_insert_at (struct string_htable *ht, void *item, void const *slot);
void *hash_delete (struct string_htable *ht, void const *item);
void *hash_delete_at (struct string_htable *ht, void const *slot);
void hash_delete_items (struct string_htable *ht);
void hash_free_items (struct string_htable *ht, hash_map_func_t free_fn);
void hash_free (struct string_htable *ht, hash_map_func_t free_fn);
void hash_map (struct string_htable *ht, hash_map_func_t map);
void hash_maparg (struct string_htable *ht, hash_maparg_func_t map, void const *arg, qsort_cmp_t compare);

void hash_print_stats (struct string_htable *ht, FILE *out_FILE);
void **hash_dump (struct string_htable *ht, void **vector_0, qsort_cmp_t compare);
void **hash_dump_select (struct string_htable *ht, void **vector_0, hash_select_func_t select_fn, qsort_cmp_t compare);
...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((string)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0xf)); } while (0); return result; } while (0);
}

static unsigned long
string_hash_2 (char *string)
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((string)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}

static int
string_hash_cmp (char *x, char *y)
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) (x) - 1; unsigned char const *yy = (unsigned char const *) (y) - 1; do { if (*++xx == '\0') { yy++; break; } } while (*xx == *++yy); (result) = *xx - *yy; } while (0); return result; } while (0);
}
...
string_htable_new (void)
{
  ...
  res = ((struct string_htable *) xmalloc (sizeof (struct string_htable) * (1)));
  hash_init (res, 8,
...
{
  return (char **) hash_dump (table, ((void*)0), (qsort_cmp_t) string_hash_qcmp);
}
