   ...
   size_t offset)
{
  ...
  if (!*lineptr)
    {
      *n = 64;
      *lineptr = ((char *) xmalloc (sizeof (char) * (*n)));
      if (!*lineptr)
 return -1;
    }
  ...
  for (;;)
    {
      ...
      ((*n + *lineptr == read_pos + nchars_avail) ? (void) (0) : __assert_fail ("*n + *lineptr == read_pos + nchars_avail", "getshline.c", 81, __PRETTY_FUNCTION__));
      if (nchars_avail < 2)
 {
   if (*n > 64)
     *n *= 2;
   ...
   *lineptr = ((char *) xrealloc ((void *) (*lineptr), sizeof (char) * (*n)));
   if (!*lineptr)
     return -1;
   read_pos = *n - nchars_avail + *lineptr;
   ((*n + *lineptr == read_pos + nchars_avail) ? (void) (0) : __assert_fail ("*n + *lineptr == read_pos + nchars_avail", "getshline.c", 94, __PRETTY_FUNCTION__));
 }

      if (c == (-1) || ferror (stream))
 {
