...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((key->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0xf)); } while (0); return result; } while (0);
}

static unsigned long
mv_key_hash_2 (struct multivalued_entry *key)
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((key->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}
...
{
  struct multivalued_entry *(res) = ((struct multivalued_entry *) xmalloc (sizeof (struct multivalued_entry) * (1)));
...
dump_supplied_resources (FILE * stream, a2ps_job * job)
{
  ...
  list = ((multivalued_entry **)
   hash_dump (job->status->supplied_resources, ((void*)0), ((void*)0)));
...
dump_needed_resources (FILE * stream, a2ps_job * job)
{
  ...
  multivalued_entry ** list;
  list = ((multivalued_entry **)
   hash_dump (job->status->needed_resources, ((void*)0), ((void*)0)));

  for (i = 0 ; list [i] ; i ++)
    {

      if ((strcmp ((list [i]-> key), ("color")) == 0)

   || (strcmp ((list [i]-> key), ("file")) == 0))
 continue;
