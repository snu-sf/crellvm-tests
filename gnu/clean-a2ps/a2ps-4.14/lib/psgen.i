...
ps_escape_char (a2ps_job * job, unsigned char c, unsigned char * res)
{
  ...
  if (' ' <= c && c < 0177) {
    if (c == '(' || c == ')' || c == '\\')
      do { int __len = strlen((const char *)res); *(res+__len) = '\\'; *(res+__len+1) = '\0'; } while (0);
    do { int __len = strlen((const char *)res); *(res+__len) = c; *(res+__len+1) = '\0'; } while (0);
    return 1;
  }
...
output_marker (a2ps_job * job, const char * kind, unsigned char * marker)
{
  ...
  cp = expand_user_string (job, ((struct file_job *) (job->jobs->content[job->jobs->len - 1])),
      (const unsigned char *) kind, marker);

  for (i = 0 ; cp[i] ; i++) {
    switch (cp[i]) {
    case 7:
      output_delayed_int (job->divertion, &(((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->last_page));
      break;
    case 8:
      output_delayed_int (job->divertion, &(((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->last_sheet));
      break;
    case 4:
      output_delayed_int (job->divertion, &(((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->pages));
      break;
    case 5:
      output_delayed_int (job->divertion, &(((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->sheets));
      break;
    case 6:
      output_delayed_int (job->divertion, &(((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->lines));
      break;
    case 1:
      output_delayed_int (job->divertion, &job->pages);
      break;
    case 2:
      output_delayed_int (job->divertion, &job->sheets);
      break;
    case 3:
      output_delayed_int (job->divertion, &job->total_files);
      break;
    default:
      *buf = '\0';
      ps_escape_char (job, cp[i], buf);
      output (job->divertion, (char *) buf);
      break;
    }
  }
}
...
ps_end_encoding (a2ps_job * job)
{
  ...
  set_encoding (job, ((void*)0));
  job->status->opened_encoding = ((void*)0);
}
# 275 "psgen.c"
static void
...
page_begin (a2ps_job * job)
{
  ...
  ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->top_page = ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->pages + 1;
  ...
  if (!output_is_to_void (job->divertion))
    {
      output (job->divertion, "%%%%Page: (");
      job->status->page_label = ((unsigned char * *) xmalloc (sizeof (unsigned char *) * (1)));
      output_delayed_string (job->divertion, job->status->page_label);
      output (job->divertion, ") %d\n", job->sheets);
    }


  output (job->divertion, "%%%%BeginPageSetup\n");
  output (job->divertion, "/pagesave save def\n");


  if (job->margin
      && (job->duplex == simplex || (job->sheets % 2)))
    output (job->divertion, "%d 0 translate\n", job->margin);

  if (job->orientation == landscape)
    output (job->divertion, "sh 0 translate 90 rotate\n");

  output (job->divertion, "%%%%EndPageSetup\n");

  if (job->debug)
    output (job->divertion, "%% Display the bounding box\n  gsave\n    llx lly moveto\n    2 setlinewidth\n    0.9 setgray\n    urx lly lineto\n    urx ury lineto\n    llx ury lineto\n    closepath stroke\n  grestore\n\n");
# 394 "psgen.c"
  ps_set_encoding (job, job->requested_encoding);


  if (!(((const char *) (job->water) == ((void*)0)) || (*(job->water) == '\0')))
    {
      output_char (job->divertion, '(');
  ...
  output (job->divertion, "gsave\n");
  output (job->divertion, "llx lly %d add translate\n",
   (!(((const char *) (job->footer) == ((void*)0)) || (*(job->footer) == '\0')) || !(((const char *) (job->left_footer) == ((void*)0)) || (*(job->left_footer) == '\0')) || !(((const char *) (job->right_footer) == ((void*)0)) || (*(job->right_footer) == '\0'))) * 12);
...
page_end (a2ps_job * job)
{
  ...
  *(job->status->page_label) =
    ((unsigned char *) xstrdup((const char *)(expand_user_string (job, ((struct file_job *) (job->jobs->content[job->jobs->len - 1])), (const unsigned char *) "Page label", job->status->page_label_format))));



  output (job->divertion, "grestore\n");


  ps_push_encoding (job, job->requested_encoding);


  if ((!(((const char *) (job->header) == ((void*)0)) || (*(job->header) == '\0')))) {
    output_char (job->divertion, '(');
    output_marker (job, "right header", job->header);
    output (job->divertion, ") rhead\n");
  }




  if ((!(((const char *) (job->footer) == ((void*)0)) || (*(job->footer) == '\0')) || !(((const char *) (job->left_footer) == ((void*)0)) || (*(job->left_footer) == '\0')) || !(((const char *) (job->right_footer) == ((void*)0)) || (*(job->right_footer) == '\0')))) {
    if (!(((const char *) (job->footer) == ((void*)0)) || (*(job->footer) == '\0'))) {
      output_char (job->divertion, '(');
    ...
    if (!(((const char *) (job->right_footer) == ((void*)0)) || (*(job->right_footer) == '\0'))) {
      output_char (job->divertion, '(');
    ...
    if (!(((const char *) (job->left_footer) == ((void*)0)) || (*(job->left_footer) == '\0'))) {
      output_char (job->divertion, '(');
      ...
      output (job->divertion, "() ");
    output (job->divertion, "footer\n");
  }


  ps_end_encoding (job);

  output (job->divertion, "pagesave restore\n");
  output (job->divertion, "showpage\n");
...
virtual_begin (a2ps_job * job)
{
  ...
  if (print_page (job, job->pages)) {
    ...
    output_to_void (job->divertion, 1);
  }
  ...
  output (job->divertion, "/v %d store\n", job->virtual - 1);
  output (job->divertion, "/x0 x v get %f add sx cw mul add store\n",
   0.7 * job->fontsize * 0.6);
  output (job->divertion, "/y0 y v get bfs %s sub store\n",
   (!(((const char *) (job->center_title) == ((void*)0)) || (*(job->center_title) == '\0')) || !(((const char *) (job->left_title) == ((void*)0)) || (*(job->left_title) == '\0')) || !(((const char *) (job->right_title) == ((void*)0)) || (*(job->right_title) == '\0'))) ? "th add" : "");
  output (job->divertion, "x0 y0 moveto\n");
}
...
virtual_end (a2ps_job * job)
{
  ...
  if ((!(((const char *) (job->center_title) == ((void*)0)) || (*(job->center_title) == '\0')) || !(((const char *) (job->left_title) == ((void*)0)) || (*(job->left_title) == '\0')) || !(((const char *) (job->right_title) == ((void*)0)) || (*(job->right_title) == '\0')))) {
    ps_push_encoding (job, job->requested_encoding);
    if (!(((const char *) (job->center_title) == ((void*)0)) || (*(job->center_title) == '\0'))) {
      output_char (job->divertion, '(');
    ...
    if (!(((const char *) (job->right_title) == ((void*)0)) || (*(job->right_title) == '\0'))) {
      output_char (job->divertion, '(');
    ...
    if (!(((const char *) (job->left_title) == ((void*)0)) || (*(job->left_title) == '\0'))) {
      output_char (job->divertion, '(');
    ...
    output (job->divertion, "title\n");
    ps_pop_encoding (job);
  }

  if (job->border)
    output (job->divertion, "border\n");
...
virtual_empty_output (a2ps_job *job)
{
  ...
  if (macro_meta_sequence_get (job, "opt.virtual.force"))
    {
      virtual_begin (job);
      virtual_end (job);
    }
...
page_empty_output (a2ps_job *job)
{
  job->sheets++;
  file_job_synchronize_sheets (job);
  output (job->divertion, "%%%%Page: (*) %d\n", job->sheets);
  output (job->divertion, "%% Empty Page\n");
  output (job->divertion, "showpage\n");
}
...
check_binary_file (a2ps_job * job)
{
 ...
 error (1, 0, gettext ("`%s' is a binary file, printing aborted"),
        ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->name);
    }
}
...
{
  if (!job->status->face_declared) {
    output (job->divertion, ") %s n\n", face_eo_ps (job->status->face));
    job->status->face_declared = 1;
  } else
    output (job->divertion, ") N\n");
  job->status->line++;
  job->status->column = 0;
  job->status->wx = 0;
}
...
# 733 "psgen.c"
      ...
      if (job->status->start_page)
 {
   ...
   if (job->numbering)
     {
       if (((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->lines % job->numbering == 0)
  output (job->divertion, "(%d) # (", ((struct file_job *) (job->jobs->content[job->jobs->len - 1]))->lines);
       else
  output (job->divertion, "0 T (");
     }
   else
...
# 798 "psgen.c"
 ...
 virtual_begin (job);
    ...
    virtual_end (job);
    job->status->start_page = 1;
    break;

  case '\n':
# 828 "psgen.c"
    (((struct file_job *) (job->jobs->content[job->jobs->len - 1])))->lines++;
    job->status->start_line = 1;
    end_of_line (job);
 ...
 output (job->divertion, ") S\n(");
    ...
    job->status->column =
      ((((job->status->wx / 600) > (job->status->column)) ? (job->status->wx / 600) : (job->status->column))
       / job->tabsize + 1) * job->tabsize;
    job->status->wx = job->status->column * 600;
    if ((job->status->wx > job->status->wxperline)) {
      if (job->folding) {
 fold_line (job, new_face);
      } else {
 job->status->is_in_cut = 1;
 return;
      }
    }

    if (!job->status->face_declared) {
      output (job->divertion, ") %s", face_eo_ps(job->status->face));
      job->status->face_declared = 1;
    } else
      output (job->divertion, ") S");
    output (job->divertion, " %d T (", job->status->column);
    break;
  print:
  default:
    {
