...
_stream_ropen (const char * command, _Bool is_file)
{
  struct stream *(res) = ((struct stream *) xmalloc (sizeof (struct stream) * (1)));
  ...
  if (!res->is_file)
    ...
    {
      if (!(((const char *) (command) == ((void*)0)) || (*(command) == '\0')))
...
struct stream *
stream_ropen (const char * command, _Bool is_file)
{
...
static inline struct stream *
_stream_wopen (const char * command, _Bool is_file,
...
struct stream *
stream_wopen (const char * command, _Bool is_file)
{
  return _stream_wopen (command, is_file, none);
}

struct stream *
stream_wopen_backup (const char * command, _Bool is_file,
       enum backup_type backup_type)
{
    ...
    const char **name)
{
  ...
  ((perl_command) ? (void) (0) : __assert_fail ("perl_command", "stream.c", 127, __PRETTY_FUNCTION__));
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "perl-open (%s)\n", quotearg (perl_command)); } while (0);
  ...
  *name = perl_command + strspn (perl_command, "\t >|");
  ...
  switch (*perl_command)
    {
    case '|':
      return _stream_wopen (*name, 0, none);

    case '>':
      return _stream_wopen (*name, 1, backup);
 ...
 case '|':
   ...
   cp = ((char *) __builtin_alloca (sizeof (char) * (len)));
   strncpy (cp, *name, len - 1);
   return _stream_ropen (cp, 0);

 default:

   return _stream_ropen (*name, 1);
 }
    }
}
