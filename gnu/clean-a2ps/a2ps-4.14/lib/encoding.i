...
void font_info_get_wx_for_vector (struct a2ps_job * job, const char * key, char * vector [256], unsigned int * wx);





void font_info_table_dump_special_font_setup (FILE * stream, struct a2ps_job * job);
...
void add_needed_resource (struct a2ps_job * job, const char * key, const char * value);

int exist_resource (struct a2ps_job * job, const char * key, const char * value);


void add_process_color (struct a2ps_job * job, const char * value);

void dump_process_color (FILE * stream, struct a2ps_job * job);


void add_needed_resource (struct a2ps_job * job, const char * key, const char * value);

void dump_needed_resources (FILE * stream, struct a2ps_job * job);


void add_supplied_resource (struct a2ps_job * job, const char * key, const char * value);

void dump_supplied_resources (FILE * stream, struct a2ps_job * job);
...
int
getshdelim (char **_lineptr, size_t *_n, int _delimiter, int _delimiter_quote, int _commentor, FILE *_stream);
# 53 "./getshline.h"
int
getshline_numbered (int *_firstline, int *_lastline, char **_lineptr, size_t *_n, FILE *_stream);



int
getshdelim_numbered (int *_firstline, int *_lastline, char **_lineptr, size_t *_n, int _delimiter, int _delimiter_quote, int _commentor, FILE *_stream);
# 37 "encoding.c" 2
# 1 "./document.h" 1
# 37 "./document.h"
void
authors_print_plain (const unsigned char * authors, FILE * stream, const char * before);

void
authors_print_html (const unsigned char * authors, FILE * stream, const char * before);

void
authors_print_texinfo (const unsigned char * authors, FILE * stream, const char * before);


typedef void (*documentation_fn_t) (const unsigned char * documentation, const char * format, FILE * stream) ;


void
documentation_print_plain (const unsigned char * documentation, const char * format, FILE * stream);


void
documentation_print_html (const unsigned char * documentation, const char * format, FILE * stream);


void
documentation_print_texinfo (const unsigned char * documentation, const char * format, FILE * stream);
# 38 "encoding.c" 2
# 1 "./quotearg.h" 1
# 21 "./quotearg.h"
enum quoting_style
...
load_main_encodings_map (struct a2ps_job * job)
{
  ...
  file = xpw_find_file (job->common.path, "encoding.map", ((void*)0));
...
{
  int i;
  unsigned int *res = ((unsigned int *) xmalloc (sizeof (unsigned int) * (256)));
...
{
  struct font_entry *(res) = ((struct font_entry *) xmalloc (sizeof (struct font_entry) * (1)));
...
{
  return font_entry_get (table, name) != ((void*)0);
}
...
{
  font_entry_get (table, name)->used = 1;
}
# 392 "encoding.c"
struct encoding {
  char * key;
...
encoding_new (const char * key)
{
  ...
  struct encoding *(res) = ((struct encoding *) xmalloc (sizeof (struct encoding) * (1)));

  res->key = xstrdup (key);
  res->name = ((void*)0);
  res->default_font = ((void*)0);
  res->documentation = ((void*)0);
...
encoding_free (struct encoding * encoding)
{
  ...
  do { if (encoding->key) free (encoding->key); } while (0);
  do { if (encoding->name) free (encoding->name); } while (0);
  do { if (encoding->default_font) free (encoding->default_font); } while (0);
  do { if (encoding->documentation) free (encoding->documentation); } while (0);
      ...
      const char * font_list)
{
  ...
  do { const char * _tmp_ = (const char *) (font_list); font_list_copy = ((char *) __builtin_alloca (sizeof (char) * (strlen (_tmp_) + 1))); strcpy (font_list_copy, _tmp_); } while (0);
 ...
 if (font_exists (job, font_name))
  ...
  while (!res && (font_name = strtok (((void*)0), ",<>;")));
 ...
 error (1, 0, "Cannot find font %s, nor any substitute",
  ...
  do { if ((((1 << 4)) & msg_verbosity)) fprintf (stderr, "In encoding %s, font %s is resolved as %s\n", encoding->key, font_name, res); } while (0);
    ...
    struct encoding * encoding)
{
  ...
  if (first_time)
    {
      ...
      first_time = 0;
      _obstack_begin ((&documentation_stack), 0, 0, (void *(*) (long)) xmalloc, (void (*) (void *)) free);
    }

  do { if ((((1 << 4)) & msg_verbosity)) fprintf (stderr, "Loading encoding file `%s.edf'\n", encoding->key); } while (0);
      ...
      if ((strcmp ((token), ("Vector:")) == 0))
 {
  ...
  error_at_line (1, 0, fname, firstline,
          gettext ("missing argument for `%s'"), "`Vector:'");
       lastline++;
       token = (strtok ((buf2), " \t\n"));
       encoding->vector [c++] = xstrdup (token);
       while ((token2 = (strtok ((((void*)0)), " \t\n"))))
      ...
      else if ((strcmp ((token), ("Name:")) == 0))
 {
   ...
   if (token2 == ((void*)0)) error_at_line (1, 0, fname, firstline, gettext ("missing argument for `%s'"), quotearg (token));;
      ...
      else if ((strcmp ((token), ("Documentation")) == 0))
 {
   ...
   && !(!strncmp("EndDocumentation", buf2, strlen("EndDocumentation"))))
     {
       read_length = strlen (buf2);
       if (read_length < sizeof (buf2))
  lastline++;

       __extension__ ({ struct obstack *__o = (&documentation_stack); int __len = (read_length); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((char *) (buf2)), (__len)); __o->next_free += __len; (void) 0; });
     }
   if (!(!strncmp("EndDocumentation", buf2, strlen("EndDocumentation"))))
     error (1, 0, fname, firstline,
     gettext ("missing argument for `%s'"), "`Documentation'");

   __extension__ ({ struct obstack *__o = (&documentation_stack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = ('\0'); (void) 0; });
   documentation =
   ...
   __extension__ ({ struct obstack *__o = (&documentation_stack); void *__obj = (documentation); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
   encoding->documentation = ((unsigned char *) xstrdup((const char *)(documentation)));
     }
      else if ((strcmp ((token), ("Substitute:")) == 0))
 {
   char * orig, * subs;
   ...
   encoding_add_font_substitute (encoding, orig, subs);
        ...
        error_at_line (1, 0, fname, firstline,
                       gettext ("invalid option `%s'"), quotearg (token));
...
encoding_self_print (struct encoding * item, FILE * stream)
{
  ...
  if (! (((const char *) (item->default_font) == ((void*)0)) || (*(item->default_font) == '\0')))
        ...
        struct encoding * encoding)
{
  ...
  switch (job->output_format)
    ...
    case eps:
      ...
      cp = ((char *) __builtin_alloca (sizeof (char) * (strlen (encoding->name) + strlen ("Encoding") + 1)));
        ...
        struct a2ps_job * job)
{
  ...
  encodings = (struct encoding **)
    hash_dump (job->encodings, ((void*)0),
...
char_WX (a2ps_job * job, unsigned char c)
{
  ...
  unsigned int wx = (job->encoding->faces_wx[job->status->face][(int) c]);
 ...
 char buf [3];
 sprintf (buf, "%02x", c);
 return (job->encoding->faces_wx[job->status->face][(int) '\\']) + (job->encoding->faces_wx[job->status->face][(int) 'x']) + (job->encoding->faces_wx[job->status->face][(int) buf[0]]) + (job->encoding->faces_wx[job->status->face][(int) buf[1]]);
      }
 ...
 char buf [4];
 sprintf (buf, "%03o", c);
 return ((job->encoding->faces_wx[job->status->face][(int) '\\'])
  + (job->encoding->faces_wx[job->status->face][(int) buf [0]]) + (job->encoding->faces_wx[job->status->face][(int) buf [1]]) + (job->encoding->faces_wx[job->status->face][(int) buf [2]]));
      }
    ...
    case caret:
      if (0177 < c)
 {
   wx += (job->encoding->faces_wx[job->status->face][(int) 'M']) + (job->encoding->faces_wx[job->status->face][(int) '-']);
   c &= 0177;
 }
      if (c == 0177)
 wx += (job->encoding->faces_wx[job->status->face][(int) '^']) + (job->encoding->faces_wx[job->status->face][(int) '?']);
      else if (c < ' ')
 wx += (job->encoding->faces_wx[job->status->face][(int) '^']) + (job->encoding->faces_wx[job->status->face][(int) c + '@']);
      else
 wx += (job->encoding->faces_wx[job->status->face][(int) c]);
      return wx;
      ...
      if (0177 < c)
 {
   ...
   c &= 0177;
      ...
      if (c == 0177)
 wx += (job->encoding->faces_wx[job->status->face][(int) 'C']) + (job->encoding->faces_wx[job->status->face][(int) '-']) + (job->encoding->faces_wx[job->status->face][(int) '?']);
      else if (c < ' ')
 wx += (job->encoding->faces_wx[job->status->face][(int) 'C']) + (job->encoding->faces_wx[job->status->face][(int) '-']) + (job->encoding->faces_wx[job->status->face][(int) c + '@']);
      else
 wx += (job->encoding->faces_wx[job->status->face][(int) c]);
      return wx;
...
{
  fputs (gettext ("Known Encodings"), stream);
  _IO_putc ('\n', stream);
  pw_lister_on_suffix (stream, job->common.path, ".edf");
}
...
list_encodings_long (a2ps_job * job, FILE * stream)
{
  ...
  entries = pw_glob_on_suffix (job->common.path, ".edf");
