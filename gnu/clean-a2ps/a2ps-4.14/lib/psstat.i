...
{
  do { unsigned long result = 0; do { unsigned char const *kk = (unsigned char const *) ((((const dict_entry *)key)->key)) - 1; while (*++kk) (result) += (*kk << (kk[1] & 0x7)); } while (0); return result; } while (0);
}

static int
key_hash_cmp (void const *x, void const *y)
{
  do { int result; do { unsigned char const *xx = (unsigned char const *) (((const dict_entry *)x)->key) - 1; unsigned char const *yy = (unsigned char const *) (((const dict_entry *)y)->key) - 1; do { if (*++xx == '\0') { yy++; break; } } while (*xx == *++yy); (result) = *xx - *yy; } while (0); return result; } while (0);
...
new_ps_status (void)
{
  struct ps_status * res = ((struct ps_status *) xmalloc (sizeof (struct ps_status) * (1)));


  res->magic_number = ((unsigned char *) xstrdup((const char *)("%!PS-Adobe-3.0")));


  res->page_label_format = ((unsigned char *) xstrdup((const char *)("#!s|$p-|, |")));


  res->opened_encoding = ((void*)0);


  res->page_are_ordered = 1;
...
dict_entry_table_dump (struct hash_table_s * table, FILE * stream)
{
  ...
  items = (struct dict_entry **) hash_dump (table, ((void*)0), ((void*)0));
...
output_statusdict (a2ps_job * job)
{
  ...
  entries = (dict_entry **) hash_dump (job->status->statusdict, ((void*)0), ((void*)0));
  if (*entries) {
