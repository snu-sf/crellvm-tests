 ...
 da_cmp_func_t cmp)
{
  ...
  res = ((struct darray *) xmalloc (sizeof (struct darray) * (1)));
  res->name = name;
  res->original_size = size;
  res->size = size;
  res->content = ((void * *) xcalloc (sizeof (void *), (res->size)));
  res->growth = growth;
...
{
  if (arr) {
    do { if (arr->content) free (arr->content); } while (0);
    free (arr);
  }
}
...
{
  struct darray * res;
  res = (memcpy (xmalloc (sizeof (*array) * (1)), (array), sizeof (*array) * (1)));
  res->content = (memcpy (xmalloc (sizeof (*array->content) * (array->len)), (array->content), sizeof (*array->content) * (array->len)));
  return res;
}
...
da_qsort (struct darray * arr)
{
  ...
  istack = ((int *) xmalloc (sizeof (int) * (100)));
  ir = arr->len - 1;
  l = 0;
  jstack = 0;
 ...
 {
   k = (l + ir) / 2;
   do { tmp = arr->content [k]; arr->content [k] = arr->content [l + 1]; arr->content [l + 1] = tmp ; } while (0);
   if (arr->cmp (arr->content [l], arr->content [ir]) > 0)
     do { tmp = arr->content [l]; arr->content [l] = arr->content [ir]; arr->content [ir] = tmp ; } while (0);
   if (arr->cmp (arr->content [l + 1], arr->content [ir]) > 0)
     do { tmp = arr->content [l + 1]; arr->content [l + 1] = arr->content [ir]; arr->content [ir] = tmp ; } while (0);
   if (arr->cmp (arr->content [l], arr->content [l + 1]) > 0)
     do { tmp = arr->content [l]; arr->content [l] = arr->content [l + 1]; arr->content [l + 1] = tmp ; } while (0);
   i = l + 1;
   j = ir;
   a = arr->content [l + 1];
   for (;;) {
     ...
     if (j < i)
       break;
     do { tmp = arr->content [i]; arr->content [i] = arr->content [j]; arr->content [j] = tmp ; } while (0);
   }
