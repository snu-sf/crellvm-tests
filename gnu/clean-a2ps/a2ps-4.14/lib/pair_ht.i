...
void hash_init (struct pair_htable *ht, unsigned long size, hash_func_t hash_1, hash_func_t hash_2, hash_cmp_func_t hash_cmp);

void hash_load (struct pair_htable *ht, void *item_table, unsigned long cardinality, unsigned long size);

void **hash_find_slot (struct pair_htable *ht, void const *key);
void *hash_find_item (struct pair_htable *ht, void const *key);
void *hash_insert (struct pair_htable *ht, void *item);
void *hash_insert_at (struct pair_htable *ht, void *item, void const *slot);
void *hash_delete (struct pair_htable *ht, void const *item);
void *hash_delete_at (struct pair_htable *ht, void const *slot);
void hash_delete_items (struct pair_htable *ht);
void hash_free_items (struct pair_htable *ht, hash_map_func_t free_fn);
void hash_free (struct pair_htable *ht, hash_map_func_t free_fn);
void hash_map (struct pair_htable *ht, hash_map_func_t map);
void hash_maparg (struct pair_htable *ht, hash_maparg_func_t map, void const *arg, qsort_cmp_t compare);

void hash_print_stats (struct pair_htable *ht, FILE *out_FILE);
void **hash_dump (struct pair_htable *ht, void **vector_0, qsort_cmp_t compare);
void **hash_dump_select (struct pair_htable *ht, void **vector_0, hash_select_func_t select_fn, qsort_cmp_t compare);
...
void
pair_add (struct pair_htable * table,
   const char * key, const char * value)
{
  ...
  if (item) {
    ...
    item = ((struct pair *) xmalloc (sizeof (struct pair) * (1)));
...
void
pair_delete (struct pair_htable * table, const char * key)
{
  struct pair * item, token;
...
char *
pair_get (struct pair_htable * table, const char * key)
{
  struct pair * item, token;
...
void
pair_table_list_short (struct pair_htable * table, FILE * stream)
{
  struct pair ** entries;
  entries = (struct pair **)
    hash_dump (table, ((void*)0),
...
void
pair_table_list_long (struct pair_htable * table, FILE * stream)
{
  int i;
  struct pair ** entries;
  entries = (struct pair **)
...
void
pair_table_self_print (struct pair_htable * table, FILE * stream)
{
  int i;
  struct pair ** entries;
  entries = (struct pair **)
...
# 301 "pair_ht.c"
int
pair_table_load (struct pair_htable * table, const char *file)
{
  FILE * fp;
  char *buf = ((void*)0);
  size_t bufsiz = 0;
  ...
  do { if ((((1 << 6)) & msg_verbosity)) fprintf (stderr, "Loading map file `%s'\n", quotearg (file)); } while (0);
      ...
      if ((strcmp ((token), ("***")) == 0))
 {
   ...
   if (token2 == ((void*)0)) error_at_line (1, 0, file, firstline, gettext ("missing argument for `%s'"), quotearg (token));;
