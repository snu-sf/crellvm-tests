...
option_hook handle_option_hook = ((void*)0);
...
static struct option const long_options[] =
{
  ...
  {"list-options", 0, 0, 139},
  {"list-media", 0, 0, 161},
  {"list-style-sheets", 0, 0, 162},
  {"help-languages", 0, 0, 162},
  {"help-pretty-print", 0, 0, 162},


  {"macro-meta-sequence",1, 0, 'D'},
  {"variable", 1, 0, 'D'},
  {"define", 1, 0, 'D'},
  {"user-option", 1, 0, '='},
  ...
  {"header", 2, 0, 'b'},
  {"no-header", 0, 0, 'B'},
  {"truncate-lines", 1, 0, 151},
  {"line-numbers", 1, 0, 152},

  {"font-size", 1, 0, 'f'},


  {"graphic-symbols", 1, 0, 154},
  {"highlight-level", 1, 0, 173},

  {"interpret", 1, 0, 155},
  {"end-of-line", 1, 0, 169},
  {"borders", 1, 0, 156},
  {"page-prefeed", 0, 0, 'k'},
  {"no-page-prefeed", 0, 0, 'K'},
  {"lines-per-page", 1, 0, 'L'},
  {"chars-per-line", 1, 0, 'l'},
  {"catman", 0, 0, 'm'},
  {"medium", 1, 0, 'M'},
  ...
  {"non-printable-format",1, 0, 135},
  {"print-anyway", 1, 0, 136},

  {"center-title", 2, 0, 149},
  {"left-title", 2, 0, 140},
  {"right-title", 2, 0, 141},
  {"left-footer", 2, 0, 142},
  {"footer", 2, 0, 143},
  {"right-footer", 2, 0, 144},
  {"stdin", 1, 0, 166},

  {"margin", 2, 0, 147},
  {"strip-level", 1, 0, 148},
  {"major", 1, 0, 157},
  {"version-control", 1, 0, 158},
  {"suffix", 1, 0, 159},
...
{
  (((sizeof (((bool_args))) / sizeof (*((bool_args)))) == (sizeof (((bool_types))) / sizeof (*((bool_types)))) + 1) ? (void) (0) : __assert_fail ("(sizeof (((bool_args))) / sizeof (*((bool_args)))) == (sizeof (((bool_types))) / sizeof (*((bool_types)))) + 1", "options.c", 206, __PRETTY_FUNCTION__));
  return (bool_types [__xargmatch_internal ((option), (arg), (bool_args), (const char *) (bool_types), sizeof (*(bool_types)), 0, argmatch_die)]);
}
...
{
  struct opt_optarg *opt_optarg_head = ((void*)0);
  struct opt_optarg *opt_optarg = ((void*)0);
  int res;
  ...
  while (1)
    {
      ...
      option = getopt_long (argc, argv,
       "123456789=:A:a:b::BcCdD:E::f:gGhijkKl:L:mM:n:o:P:qrRs:S:t::T:u::v::VX:Z", long_options, &option_index);
      ...
      if (option == '?')
 {
   ...
   exit (1);
      ...
      new = ((struct opt_optarg *) __builtin_alloca (sizeof (struct opt_optarg) * (1)));
      new->option = option;
      new->optarg = optarg;
      new->next = ((void*)0);
      if (!opt_optarg)
...
# 362 "options.c"
      case '1':
 job->columns = 1; job->rows = 1; job->orientation = portrait; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '2':
 job->columns = 2; job->rows = 1; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '3':
 job->columns = 3; job->rows = 1; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '4':
 job->columns = 2; job->rows = 2; job->orientation = portrait; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '5':
 job->columns = 5; job->rows = 1; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '6':
 job->columns = 3; job->rows = 2; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '7':
 job->columns = 7; job->rows = 1; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '8':
 job->columns = 4; job->rows = 2; job->orientation = landscape; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;

      case '9':
 job->columns = 3; job->rows = 3; job->orientation = portrait; job->columns_requested = 80; job->lines_requested = 0; job->madir = madir_rows;;
 break;
 ...
 break;
      ...
      case 'b':
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->header) free (job->header); } while (0); job->header = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 'B':

 do { if (job->header) free (job->header); } while (0);
 do { if (job->left_footer) free (job->left_footer); } while (0);
 do { if (job->footer) free (job->footer); } while (0);
 do { if (job->right_footer) free (job->right_footer); } while (0);
 do { if (job->left_title) free (job->left_title); } while (0);
 do { if (job->center_title) free (job->center_title); } while (0);
 do { if (job->right_title) free (job->right_title); } while (0);
 do { if (job->water) free (job->water); } while (0);

 job->header = (unsigned char *) ((void*)0);
 job->left_footer = (unsigned char *) ((void*)0);
 job->footer = (unsigned char *) ((void*)0);
 job->right_footer = (unsigned char *) ((void*)0);
 job->left_title = (unsigned char *) ((void*)0);
 job->center_title = (unsigned char *) ((void*)0);
 job->right_title = (unsigned char *) ((void*)0);
 job->water = (unsigned char *) ((void*)0);
 break;
 ...
 break;
      ...
      case 'd':
 a2ps_printers_flag_output_set (job->printers, ((void*)0), 1);
 break;
 ...
 break;
      ...
      case 'm':
 job->lines_requested = 66;
 job->columns_requested = 0;
 break;

      case 'M':
 do { const char *my_s2 = (optarg); do { if (job->medium_request) free (job->medium_request); } while (0); job->medium_request = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
 break;
 ...
 break;
      ...
      case 'o':
 a2ps_printers_flag_output_set (job->printers, optarg, 0);
 break;

      case 'P':
 a2ps_printers_flag_output_set (job->printers, optarg, 1);
 break;
 ...
 (((sizeof (((duplex_args))) / sizeof (*((duplex_args)))) == (sizeof (((duplex_types))) / sizeof (*((duplex_types)))) + 1) ? (void) (0) : __assert_fail ("(sizeof (((duplex_args))) / sizeof (*((duplex_args)))) == (sizeof (((duplex_types))) / sizeof (*((duplex_types)))) + 1", "options.c", 574, __PRETTY_FUNCTION__));
 job->duplex = (duplex_types [__xargmatch_internal (("--sides"), (optarg), (duplex_args), (const char *) (duplex_types), sizeof (*(duplex_types)), 0, argmatch_die)]);
      ...
      case 164:
 {
   char *value;
   value = strchr (optarg, ':');
   if ((((const char *) (value) == ((void*)0)) || (*(value) == '\0')))
     {
   ...
   else
     {
       *value = '\0';
       value ++;
       if (*value == ':')
  setstatusdict (job, optarg, value + 1, 1);
       else
  setstatusdict (job, optarg, value, 0);
     }
 }
 break;

      case 't':
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->title) free (job->title); } while (0); job->title = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;
 ...
 break;
      ...
      case 'u':
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->water) free (job->water); } while (0); job->water = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;
 ...
 if (!getenv ("A2PS_VERBOSITY"))
     {
       if (!(((const char *) (optarg) == ((void*)0)) || (*(optarg) == '\0')))
  {
    msg_verbosity =
      msg_verbosity_argmatch ("--verbose", optarg);
  }
       else
  msg_verbosity = ((1 << 1) | (1 << 0));
     }
 break;

      case 'X':
# 653 "options.c"
 do { if (job->requested_encoding_name) free (job->requested_encoding_name); } while (0);
 ...
 break;
      ...
      case 134:
 do { const char *my_s2 = (optarg); do { if (job->prolog) free (job->prolog); } while (0); job->prolog = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? xstrdup (my_s2) : ((void*)0); } while (0);
 break;

      case 135:
 (((sizeof (((non_printable_args))) / sizeof (*((non_printable_args)))) == (sizeof (((non_printable_types))) / sizeof (*((non_printable_types)))) + 1) ? (void) (0) : __assert_fail ("(sizeof (((non_printable_args))) / sizeof (*((non_printable_args)))) == (sizeof (((non_printable_types))) / sizeof (*((non_printable_types)))) + 1", "options.c", 672, __PRETTY_FUNCTION__));
 job->unprintable_format =
   (non_printable_types [__xargmatch_internal (("--non-printable"), (optarg), (non_printable_args), (const char *) (non_printable_types), sizeof (*(non_printable_types)), 0, argmatch_die)]);
      ...
      case 'D':
 {
  ...
  error (1, 0,
          gettext ("invalid variable identifier `%s'"),
          quotearg (optarg));
     }
   else
     {
 ...
 break;
      ...
      case 140:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->left_title) free (job->left_title); } while (0); job->left_title = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 141:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->right_title) free (job->right_title); } while (0); job->right_title = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 149:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->center_title) free (job->center_title); } while (0); job->center_title = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 142:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->left_footer) free (job->left_footer); } while (0); job->left_footer = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 143:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->footer) free (job->footer); } while (0); job->footer = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;

      case 144:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->right_footer) free (job->right_footer); } while (0); job->right_footer = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;
 ...
 break;
      ...
      case 166:
 do { const unsigned char *my_s2 = (unsigned char *) (optarg); do { if (job->stdin_filename) free (job->stdin_filename); } while (0); job->stdin_filename = !(((const char *) (my_s2) == ((void*)0)) || (*(my_s2) == '\0')) ? ((unsigned char *) xstrdup((const char *)(my_s2))) : (unsigned char *) ((void*)0); } while (0);
 break;
...
a2ps_handle_string_options (a2ps_job * job, const char * string)
{
  ...
  do { if ((((1 << 5)) & msg_verbosity)) fprintf (stderr, "handle_string_options(%s)", string); } while (0);
