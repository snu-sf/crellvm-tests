...
find_cache_boundary (struct region_cache *c, ptrdiff_t pos)
{
  ...
  while (low + 1 < high)
    {
      ...
      ptrdiff_t boundary = ((mid) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(mid)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (mid)].pos);
  ...
  ((void) (0 && (!(((low) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(low)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (low)].pos) > pos || (low + 1 < c->cache_len && ((low + 1) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(low + 1)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (low + 1)].pos) <= pos)))));
         ...
         int value)
{
  ...
  ((void) (0 && (0 < i && i <= c->cache_len)));


  ((void) (0 && ((((i - 1) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(i - 1)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (i - 1)].pos) < pos && (i == c->cache_len || pos < ((i) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(i)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (i)].pos))))));
  ...
  ((void) (0 && (((i - 1) < (c)->gap_start ? (c)->boundaries[(i - 1)].value : (c)->boundaries[(c)->gap_len + (i - 1)].value) != value)));
    ...
    ptrdiff_t start, ptrdiff_t end)
{
  ...
  ((void) (0 && (0 <= start && end <= c->cache_len)));


  ((void) (0 && (start <= end)));


  ((void) (0 && (!(start == 0 && end >= 1))));
...
{
  ((void) (0 && (start <= end)));
  ((void) (0 && (c->buffer_beg <= start && end <= c->buffer_end)));
...
# 380 "region-cache.c"
    ...
    int value_at_end = ((end_ix - 1) < (c)->gap_start ? (c)->boundaries[(end_ix - 1)].value : (c)->boundaries[(c)->gap_len + (end_ix - 1)].value);
    ...
    if (((start_ix) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(start_ix)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (start_ix)].pos) == start)
      {


        if (start_ix > 0
            && ((start_ix - 1) < (c)->gap_start ? (c)->boundaries[(start_ix - 1)].value : (c)->boundaries[(c)->gap_len + (start_ix - 1)].value) == value)
          {
            delete_cache_boundaries (c, start_ix, start_ix + 1);
            start_ix--;
          }
        else
          ((start_ix) < (c)->gap_start ? ((c)->boundaries[(start_ix)].value = (value)) : ((c)->boundaries[(c)->gap_len + (start_ix)].value = (value)));
      }
    else
      {

        if (((start_ix) < (c)->gap_start ? (c)->boundaries[(start_ix)].value : (c)->boundaries[(c)->gap_len + (start_ix)].value) != value)
          {
    ...
    if (end == c->buffer_end)
      ...
      ;
             ...
             || end < ((end_ix) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(end_ix)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (end_ix)].pos))
      {
    ...
    else
      {
        ...
        if (value == ((end_ix) < (c)->gap_start ? (c)->boundaries[(end_ix)].value : (c)->boundaries[(c)->gap_len + (end_ix)].value))
...
{
# 472 "region-cache.c"
  if (((((1)) + c->beg_unchanged) - (((buf)->text->z) - tail)
       > (500))
      || ((((1)) + head) - (((buf)->text->z) - c->end_unchanged)
          > (500)))
...
# 617 "region-cache.c"
      ...
      if (modified_ix < c->cache_len
          && (((modified_ix - 1) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(modified_ix - 1)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (modified_ix - 1)].pos)
              == ((modified_ix) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(modified_ix)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (modified_ix)].pos)))
        {
          int value_after = ((modified_ix) < (c)->gap_start ? (c)->boundaries[(modified_ix)].value : (c)->boundaries[(c)->gap_len + (modified_ix)].value);




          if (modified_ix - 1 > 0
              && value_after == ((modified_ix - 2) < (c)->gap_start ? (c)->boundaries[(modified_ix - 2)].value : (c)->boundaries[(c)->gap_len + (modified_ix - 2)].value))
            delete_cache_boundaries (c, modified_ix - 1, modified_ix + 1);
          else
            {


              ((modified_ix - 1) < (c)->gap_start ? ((c)->boundaries[(modified_ix - 1)].value = (value_after)) : ((c)->boundaries[(c)->gap_len + (modified_ix - 1)].value = (value_after)));
        ...
        ptrdiff_t pos, ptrdiff_t *next)
{
...

  {
    ...
    int i_value = ((i) < (c)->gap_start ? (c)->boundaries[(i)].value : (c)->boundaries[(c)->gap_len + (i)].value);
    ptrdiff_t j;


    if (pos >= ((buf)->text->z))
      {
        if (next) *next = ((buf)->text->z);
        i_value = 0;
      }
    else if (next)
      {
        ...
        for (j = i + 1; j < c->cache_len; j++)
          if (((j) < (c)->gap_start ? (c)->boundaries[(j)].value : (c)->boundaries[(c)->gap_len + (j)].value) != i_value)
            break;

        if (j < c->cache_len)
          *next = ((j) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(j)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (j)].pos);
        else
          *next = ((buf)->text->z);
      }
    ...
    if (next)
      {
        ...
        if (j >= 0)
          *next = ((j + 1) < (c)->gap_start ? (c)->buffer_beg + (c)->boundaries[(j + 1)].pos : (c)->buffer_end + (c)->boundaries[(c)->gap_len + (j + 1)].pos);
        else
          *next = ((1));
      }
