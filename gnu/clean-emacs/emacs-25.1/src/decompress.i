...
# 424 "/usr/include/zconf.h" 3 4
# 1 "../lib/sys/types.h" 1 3 4
# 21 "../lib/sys/types.h" 3
# 425 "/usr/include/zconf.h" 2 3 4
...
unwind_decompress (void *ddata)
{
  ...
  (set_point (((data->old_point) < ((current_buffer->zv)) ? (data->old_point) : ((current_buffer->zv)))));
}

static struct Lisp_Subr _Alignas (8) Szlib_available_p = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fzlib_available_p }, 0, 0, "zlib-available-p", 0, 0}; Lisp_Object Fzlib_available_p
...
static struct Lisp_Subr _Alignas (8) Szlib_decompress_region = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fzlib_decompress_region }, 2, 2, "zlib-decompress-region", 0, 0}; Lisp_Object Fzlib_decompress_region
...
# 142 "decompress.c"
  istart = ((start) >> INTTYPEBITS);
  iend = ((end) >> INTTYPEBITS);
  ...
  if (inflateInit2_((&stream), (15 + 32), "1.2.7", (int)sizeof(z_stream)) != 0)
    return builtin_lisp_symbol (0);

  unwind_data.start = iend;
  unwind_data.stream = &stream;
  unwind_data.old_point = (current_buffer->pt + 0);
  ...
  do
    {
      ...
      ptrdiff_t avail_in = ((iend - pos_byte) < ((2147483647 *2U +1U)) ? (iend - pos_byte) : ((2147483647 *2U +1U)));
      int avail_out = 16 * 1024;
      int decompressed;

      if ((current_buffer->text->gap_size) < avail_out)
 make_gap (avail_out - (current_buffer->text->gap_size));
      stream.next_in = (((pos_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (pos_byte) + (current_buffer->text->beg) - ((1)));
      stream.avail_in = avail_in;
      stream.next_out = (current_buffer->text->beg + current_buffer->text->gpt_byte - ((1)));
      stream.avail_out = avail_out;
      inflate_status = inflate (&stream, 0);
