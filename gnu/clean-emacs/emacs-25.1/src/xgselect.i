...
{
  ((0 <= fd && fd < 1024) ? (void) 0 : __builtin_unreachable ());
  ((void) (((set)->fds_bits)[((fd) / (8 * (int) sizeof (__fd_mask)))] &= ~((__fd_mask) 1 << ((fd) % (8 * (int) sizeof (__fd_mask))))));
}
...
{
  ((0 <= fd && fd < 1024) ? (void) 0 : __builtin_unreachable ());
  return ((((set)->fds_bits)[((fd) / (8 * (int) sizeof (__fd_mask)))] & ((__fd_mask) 1 << ((fd) % (8 * (int) sizeof (__fd_mask))))) != 0) != 0;
}
...
# 1 "../lib/time.h" 1
# 21 "../lib/time.h" 3
# 33 "/usr/include/glib-2.0/glib/gdate.h" 2
    ...
    struct timespec const *timeout, sigset_t const *sigmask)
{
  ...
  GMainContext *context;
  _Bool have_wfds = wfds != ((void*)0);
  GPollFD gfds_buf[128];
  GPollFD *gfds = gfds_buf;
  int gfds_size = (sizeof (gfds_buf) / sizeof (gfds_buf)[0]);
  int n_gfds, retval = 0, our_fds = 0, max_fds = fds_lim - 1;
  _Bool context_acquired = 0;
  int i, nfds, tmo_in_millisec;
  _Bool need_to_dispatch;
  ...
  else do { int __d0, __d1; __asm__ __volatile__ ("cld; rep; " "stosq" : "=c" (__d0), "=D" (__d1) : "a" (0), "0" (sizeof (fd_set) / sizeof (__fd_mask)), "1" (&((&all_rfds)->fds_bits)[0]) : "memory"); } while (0);
  if (wfds) all_wfds = *wfds;
  else do { int __d0, __d1; __asm__ __volatile__ ("cld; rep; " "stosq" : "=c" (__d0), "=D" (__d1) : "a" (0), "0" (sizeof (fd_set) / sizeof (__fd_mask)), "1" (&((&all_wfds)->fds_bits)[0]) : "memory"); } while (0);

  n_gfds = (context_acquired
     ? g_main_context_query (context, 300, &tmo_in_millisec,
        gfds, gfds_size)
     : -1);

  if (gfds_size < n_gfds)
    {
      do { if ((n_gfds) <= sa_avail / sizeof *(gfds) / (sizeof *gfds)) (gfds) = (sa_avail -= (sizeof *(gfds) * (sizeof *gfds) * (n_gfds)), __builtin_alloca (sizeof *(gfds) * (sizeof *gfds) * (n_gfds))); else { (gfds) = xnmalloc (n_gfds, sizeof *(gfds) * (sizeof *gfds)); sa_must_free = 1; record_unwind_protect_ptr (xfree, gfds); } } while (0);
      gfds_size = n_gfds;
      n_gfds = g_main_context_query (context, 300, &tmo_in_millisec,
         gfds, gfds_size);
    }
 ...
 tmop = &tmo;
  ...
  fds_lim = max_fds + 1;
  nfds = pselect (fds_lim, &all_rfds, have_wfds ? &all_wfds : ((void*)0),
    efds, tmop, sigmask);
  ...
  if (need_to_dispatch)
    {
      int pselect_errno = (*__errno_location ());
      ...
      unblock_input ();
      (*__errno_location ()) = pselect_errno;
    }
