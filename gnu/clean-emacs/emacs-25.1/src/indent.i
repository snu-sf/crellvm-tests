...
buffer_display_table (void)
{
  ...
  thisbuf = ((current_buffer)-> display_table_);
  if ((CHAR_TABLE_P (thisbuf) && ((XCHAR_TABLE (thisbuf)->purpose) == (builtin_lisp_symbol (346))) && CHAR_TABLE_EXTRA_SLOTS (XCHAR_TABLE (thisbuf)) == 6))
  ...
  if ((CHAR_TABLE_P (globals.f_Vstandard_display_table) && ((XCHAR_TABLE (globals.f_Vstandard_display_table)->purpose) == (builtin_lisp_symbol (346))) && CHAR_TABLE_EXTRA_SLOTS (XCHAR_TABLE (globals.f_Vstandard_display_table)) == 6))
    return XCHAR_TABLE (globals.f_Vstandard_display_table);
  return 0;
}
...
character_width (int c, struct Lisp_Char_Table *dp)
{
  ...
  if (dp && (elt = ((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[c] : (dp)->ascii)) : disp_char_vector ((dp), (c))), VECTORP (elt)))
    return ASIZE (elt);
...
disptab_matches_widthtab (struct Lisp_Char_Table *disptab, struct Lisp_Vector *widthtab)
{
  ...
  ((void) (0 && (widthtab->header.size == 256)));

  for (i = 0; i < 256; i++)
    if (character_width (i, disptab)
        != ((widthtab->contents[i]) >> INTTYPEBITS))
      return 0;
...
recompute_width_table (struct buffer *buf, struct Lisp_Char_Table *disptab)
{
  ...
  if (!VECTORP (((buf)-> width_table_)))
    bset_width_table (buf, make_uninit_vector (256));
  widthtab = XVECTOR (((buf)-> width_table_));
  ((void) (0 && (widthtab->header.size == 256)));

  for (i = 0; i < 256; i++)
    ((widthtab->contents[i]) = make_natnum (character_width (i, disptab)));
}
...
width_run_cache_on_off (void)
{
  ...
  if (((((current_buffer)-> cache_long_scans_)) == (builtin_lisp_symbol (0)))


      || !((((current_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0))))
    {
      if (!indirect_p
   || ((((cache_buffer)-> cache_long_scans_)) == (builtin_lisp_symbol (0)))
   || !((((cache_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0))))
 {
   ...
   if (cache_buffer->width_run_cache)
     {
       ...
       bset_width_table (current_buffer, builtin_lisp_symbol (0));
     }
        }
      return ((void*)0);
    }
...
skip_invisible (ptrdiff_t pos, ptrdiff_t *next_boundary_p, ptrdiff_t to, Lisp_Object window)
{
  ...
  proplimit = Fnext_property_change (position, buffer, builtin_lisp_symbol (901));
  if (((overlay_limit) >> INTTYPEBITS) < ((proplimit) >> INTTYPEBITS))
    proplimit = overlay_limit;



  if (((proplimit) >> INTTYPEBITS) > pos + 100 || ((proplimit) >> INTTYPEBITS) >= to)
    *next_boundary_p = ((proplimit) >> INTTYPEBITS);

  else
    {

      ((proplimit) = make_natnum (((pos + 100) < (to) ? (pos + 100) : (to))));

      if (((overlay_limit) >> INTTYPEBITS) < ((proplimit) >> INTTYPEBITS))
 proplimit = overlay_limit;
      tmp = Fnext_single_property_change (position, builtin_lisp_symbol (574),
       buffer, proplimit);
      end = ((tmp) >> INTTYPEBITS);
# 265 "indent.c"
      *next_boundary_p = end;
  ...
  prop = Fget_char_property (position, builtin_lisp_symbol (574),
        (!((window) == (builtin_lisp_symbol (0)))
         && ((XWINDOW (window)->contents) == (buffer)))
        ? window : buffer);
  inv_p = (((((current_buffer)-> invisibility_spec_)) == (builtin_lisp_symbol (901))) ? !((prop) == (builtin_lisp_symbol (0))) : invisible_prop (prop, ((current_buffer)-> invisibility_spec_)));

  if (((window) == (builtin_lisp_symbol (0))) ? inv_p == 1 : inv_p)
    return *next_boundary_p;
  return pos;
}
# 304 "indent.c"
static struct Lisp_Subr _Alignas (8) Scurrent_column = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fcurrent_column }, 0, 0, "current-column", 0, 0}; Lisp_Object Fcurrent_column
# 316 "indent.c"
  (void)
{
  Lisp_Object temp;
  ((temp) = make_natnum (current_column ()));
  return temp;
}
...
current_column (void)
{
  ...
  ptrdiff_t post_tab;
  int c;
  int tab_width = sanitize_tab_width (((((current_buffer)-> tab_width_)) >> INTTYPEBITS));
  _Bool ctl_arrow = !((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0)));
  struct Lisp_Char_Table *dp = buffer_display_table ();

  if ((current_buffer->pt + 0) == last_known_column_point
      && (current_buffer->text->modiff) == last_known_column_modified)
  ...
  ptr = ((((current_buffer->pt_byte + 0) - 1) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((current_buffer->pt_byte + 0) - 1) + (current_buffer->text->beg) - ((1))) + 1;


  if ((current_buffer->pt + 0) == (current_buffer->begv))
    stop = ptr;
  else if ((current_buffer->pt + 0) <= (current_buffer->text->gpt) || (current_buffer->begv) > (current_buffer->text->gpt))
    stop = ((((current_buffer->begv_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (current_buffer->begv_byte) + (current_buffer->text->beg) - ((1))));
  else
    stop = (current_buffer->text->beg + current_buffer->text->gpt_byte + current_buffer->text->gap_size - ((1)));
      ...
      if (ptr == stop)
 {
   ...
   if (ptr == ((((current_buffer->begv_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (current_buffer->begv_byte) + (current_buffer->text->beg) - ((1)))))
     break;


   stop = ((((current_buffer->begv_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (current_buffer->begv_byte) + (current_buffer->text->beg) - ((1))));
   ptr = (current_buffer->text->beg + current_buffer->text->gpt_byte - ((1)));


   if ((current_buffer->begv) >= (current_buffer->text->gpt))
     break;
 }

      c = *--ptr;

      if (dp && VECTORP (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[c] : (dp)->ascii)) : disp_char_vector ((dp), (c)))))
 {
   charvec = ((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[c] : (dp)->ascii)) : disp_char_vector ((dp), (c)));
   n = ASIZE (charvec);
 }
      else
 {
   charvec = builtin_lisp_symbol (0);
   n = 1;
 }
      ...
      for (i = n - 1; i >= 0; --i)
 {
  ...
  c = ' ';
   ...
   if (c >= 040 && c < 0177)
     ...
     || (c == '\r'
         && ((((current_buffer)-> selective_display_)) == (builtin_lisp_symbol (901)))))
     {
       ptr++;
 ...
 start_of_line_found:
  ...
  last_known_column_point = (current_buffer->pt + 0);
  last_known_column_modified = (current_buffer->text->modiff);
...
check_display_width (ptrdiff_t pos, ptrdiff_t col, ptrdiff_t *endpos)
{
  ...
  if ((((enum Lisp_Type) ((val = get_char_property_and_overlay (((EMACS_INT) (((EMACS_UINT) (pos) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (344), builtin_lisp_symbol (0), &overlay)) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)

      && ((builtin_lisp_symbol (868)) == ((((void) (0 && ((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((val) - (Lisp_Cons))))->car)))
    {
      Lisp_Object plist = (((void) (0 && ((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((val) - (Lisp_Cons))))->u.cdr, prop;
      int width = -1;
      EMACS_INT align_to_max =
 (col < (9223372036854775807L >> INTTYPEBITS) - 2147483647
  ? (EMACS_INT) 2147483647 + col
  : (9223372036854775807L >> INTTYPEBITS));

      if ((prop = Fplist_get (plist, builtin_lisp_symbol (139)),
    RANGED_INTEGERP (0, prop, 2147483647))
   || (prop = Fplist_get (plist, builtin_lisp_symbol (106)),
       RANGED_INTEGERP (0, prop, 2147483647)))
 width = ((prop) >> INTTYPEBITS);
      else if ((((enum Lisp_Type) ((prop) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Float) && 0 <= XFLOAT_DATA (prop)
        && XFLOAT_DATA (prop) <= 2147483647)
 width = (int)(XFLOAT_DATA (prop) + 0.5);
      else if ((prop = Fplist_get (plist, builtin_lisp_symbol (7)),
  RANGED_INTEGERP (col, prop, align_to_max)))
 width = ((prop) >> INTTYPEBITS) - col;
      else if ((((enum Lisp_Type) ((prop) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Float) && col <= XFLOAT_DATA (prop)
      ...
      if (width >= 0)
 {
   ...
   if (OVERLAYP (overlay))
     *endpos = (((((enum Lisp_Type) ((XOVERLAY (overlay)->end) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Misc) && XMISCTYPE (XOVERLAY (overlay)->end) == Lisp_Misc_Marker) ? marker_position (XOVERLAY (overlay)->end) : (emacs_abort (), 0));
     ...
     get_property_and_range (pos, builtin_lisp_symbol (344), &val, &start, endpos, builtin_lisp_symbol (0));



   if (!((Fplist_get (plist, builtin_lisp_symbol (106))) == (builtin_lisp_symbol (0)))
       && !((((current_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0))))
     {
       int b, wd;
       unsigned char *p = ((((buf_charpos_to_bytepos (current_buffer, pos))) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((buf_charpos_to_bytepos (current_buffer, pos))) + (current_buffer->text->beg) - ((1)));

       do { int ch; ch = (!((p)[0] & 0x80) ? ((b) = 1, (p)[0]) : ! ((p)[0] & 0x20) ? ((b) = 2, (((((p)[0] & 0x1F) << 6) | ((p)[1] & 0x3F)) + (((unsigned char) (p)[0]) < 0xC2 ? 0x3FFF80 : 0))) : ! ((p)[0] & 0x10) ? ((b) = 3, ((((p)[0] & 0x0F) << 12) | (((p)[1] & 0x3F) << 6) | ((p)[2] & 0x3F))) : string_char ((p), ((void*)0), &b)); if ((!((*p) & 0x80) ? 1 : !((*p) & 0x20) ? 2 : !((*p) & 0x10) ? 3 : !((*p) & 0x08) ? 4 : 5) != b) wd = b * 4; else { if (buffer_display_table () != 0 && VECTORP (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((buffer_display_table ())->ascii) == (builtin_lisp_symbol (0))) ? (buffer_display_table ())->defalt : (SUB_CHAR_TABLE_P ((buffer_display_table ())->ascii) ? XSUB_CHAR_TABLE ((buffer_display_table ())->ascii)->contents[ch] : (buffer_display_table ())->ascii)) : disp_char_vector ((buffer_display_table ()), (ch))))) wd = sanitize_char_width (ASIZE (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((buffer_display_table ())->ascii) == (builtin_lisp_symbol (0))) ? (buffer_display_table ())->defalt : (SUB_CHAR_TABLE_P ((buffer_display_table ())->ascii) ? XSUB_CHAR_TABLE ((buffer_display_table ())->ascii)->contents[ch] : (buffer_display_table ())->ascii)) : disp_char_vector ((buffer_display_table ()), (ch))))); else wd = ((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? (ch < 0x20 ? (ch == '\t' ? sanitize_tab_width (((((current_buffer)-> tab_width_)) >> INTTYPEBITS)) : (ch == '\n' ? 0 : (((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2))) : (ch < 0x7f ? 1 : ((((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2)))) : sanitize_char_width (((CHAR_TABLE_REF (globals.f_Vchar_width_table, ch)) >> INTTYPEBITS))); } } while (0);
       width *= wd;
     }
   return width;
 }
...
scan_for_column (ptrdiff_t *endpos, EMACS_INT *goalcol, ptrdiff_t *prevcol)
{
  ...
  EMACS_INT goal = goalcol ? *goalcol : (9223372036854775807L >> INTTYPEBITS);
  ptrdiff_t end = endpos ? *endpos : (current_buffer->pt + 0);
  ptrdiff_t scan, scan_byte, next_boundary;

  scan = find_newline ((current_buffer->pt + 0), (current_buffer->pt_byte + 0), (current_buffer->begv), (current_buffer->begv_byte), -1, ((void*)0), &scan_byte, 1);
  next_boundary = scan;

  window = Fget_buffer_window (Fcurrent_buffer (), builtin_lisp_symbol (0));
  w = ! ((window) == (builtin_lisp_symbol (0))) ? XWINDOW (window) : ((void*)0);

  memset (&cmp_it, 0, sizeof cmp_it);
  cmp_it.id = -1;
  composition_compute_stop_pos (&cmp_it, scan, scan_byte, end, builtin_lisp_symbol (0));
      ...
      while (scan == next_boundary)
 {
   ...
   scan = skip_invisible (scan, &next_boundary, end, builtin_lisp_symbol (0));
   if (scan != old_scan)
     scan_byte = (buf_charpos_to_bytepos (current_buffer, scan));
   if (scan >= end)
     goto endloop;
 }
 ...
 if (width >= 0)
  ...
  continue;
   ...
   || (scan == cmp_it.stop_pos
     ...
     w, ((void*)0), builtin_lisp_symbol (0))))
 composition_update_it (&cmp_it, scan, scan_byte, builtin_lisp_symbol (0));
      if (cmp_it.id >= 0)
 {
   scan += cmp_it.nchars;
   ...
   continue;
      ...
      c = *(((((scan_byte)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((scan_byte)) + (current_buffer->text->beg) - ((1))));




      if (dp != 0
   && ! (multibyte && (((c) & 0xC0) == 0xC0))
   && VECTORP (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[c] : (dp)->ascii)) : disp_char_vector ((dp), (c)))))
 {
   Lisp_Object charvec;
   ptrdiff_t i, n;
  ...
  c = ' ';
       ...
       if (c == '\n')
  goto endloop;
       if (c == '\r' && ((((current_buffer)-> selective_display_)) == (builtin_lisp_symbol (901))))
  goto endloop;
       if (c == '\t')
  {
    col += tab_width;
      ...
      else
 {
   ...
   else if (multibyte && (((c) & 0xC0) == 0xC0))
     {

       unsigned char *ptr;
       int bytes, width;

       ptr = (((scan_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (scan_byte) + (current_buffer->text->beg) - ((1)));
       do { int ch; ch = (!((ptr)[0] & 0x80) ? ((bytes) = 1, (ptr)[0]) : ! ((ptr)[0] & 0x20) ? ((bytes) = 2, (((((ptr)[0] & 0x1F) << 6) | ((ptr)[1] & 0x3F)) + (((unsigned char) (ptr)[0]) < 0xC2 ? 0x3FFF80 : 0))) : ! ((ptr)[0] & 0x10) ? ((bytes) = 3, ((((ptr)[0] & 0x0F) << 12) | (((ptr)[1] & 0x3F) << 6) | ((ptr)[2] & 0x3F))) : string_char ((ptr), ((void*)0), &bytes)); if ((!((*ptr) & 0x80) ? 1 : !((*ptr) & 0x20) ? 2 : !((*ptr) & 0x10) ? 3 : !((*ptr) & 0x08) ? 4 : 5) != bytes) width = bytes * 4; else { if (dp != 0 && VECTORP (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[ch] : (dp)->ascii)) : disp_char_vector ((dp), (ch))))) width = sanitize_char_width (ASIZE (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[ch] : (dp)->ascii)) : disp_char_vector ((dp), (ch))))); else width = ((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? (ch < 0x20 ? (ch == '\t' ? sanitize_tab_width (((((current_buffer)-> tab_width_)) >> INTTYPEBITS)) : (ch == '\n' ? 0 : (((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2))) : (ch < 0x7f ? 1 : ((((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2)))) : sanitize_char_width (((CHAR_TABLE_REF (globals.f_Vchar_width_table, ch)) >> INTTYPEBITS))); } } while (0);
...
{
  EMACS_INT col = (9223372036854775807L >> INTTYPEBITS);
  ptrdiff_t opoint = (current_buffer->pt + 0);

  scan_for_column (&opoint, &col, ((void*)0));
  return col;
}
# 811 "indent.c"
static struct Lisp_Subr _Alignas (8) Sindent_to = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Findent_to }, 1, 2, "indent-to", "NIndent to column: ", 0}; Lisp_Object Findent_to
  ...
  (Lisp_Object column, Lisp_Object minimum)
{
  ...
  ((((((enum Lisp_Type) ((column) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), column));
  if (((minimum) == (builtin_lisp_symbol (0))))
    ((minimum) = make_natnum (0));
  ((((((enum Lisp_Type) ((minimum) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), minimum));

  fromcol = current_column ();
  mincol = fromcol + ((minimum) >> INTTYPEBITS);
  if (mincol < ((column) >> INTTYPEBITS)) mincol = ((column) >> INTTYPEBITS);

  if (fromcol == mincol)
    return ((EMACS_INT) (((EMACS_UINT) (mincol) << INTTYPEBITS) + Lisp_Int0));

  if (globals.f_indent_tabs_mode)
    {
      Lisp_Object n;
      ((n) = make_natnum (mincol / tab_width - fromcol / tab_width));
      if (((n) >> INTTYPEBITS) != 0)
 {
   Finsert_char (((EMACS_INT) (((EMACS_UINT) ('\t') << INTTYPEBITS) + Lisp_Int0)), n, builtin_lisp_symbol (901));

   fromcol = (mincol / tab_width) * tab_width;
 }
    }

  ((column) = make_natnum (mincol - fromcol));
  Finsert_char (((EMACS_INT) (((EMACS_UINT) (' ') << INTTYPEBITS) + Lisp_Int0)), column, builtin_lisp_symbol (901));

  last_known_column = mincol;
  last_known_column_point = (current_buffer->pt + 0);
  last_known_column_modified = (current_buffer->text->modiff);

  ((column) = ((EMACS_INT) (((EMACS_UINT) (mincol) << INTTYPEBITS) + Lisp_Int0)));
  return column;
}


static struct Lisp_Subr _Alignas (8) Scurrent_indentation = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fcurrent_indentation }, 0, 0, "current-indentation", 0, 0}; Lisp_Object Fcurrent_indentation
  ...
  (void)
{
  ...
  find_newline ((current_buffer->pt + 0), (current_buffer->pt_byte + 0), (current_buffer->begv), (current_buffer->begv_byte), -1, ((void*)0), &posbyte, 1);
  return ((EMACS_INT) (((EMACS_UINT) (position_indentation (posbyte)) << INTTYPEBITS) + Lisp_Int0));
}
...
position_indentation (ptrdiff_t pos_byte)
{
      ...
      while (p == stop)
 {
   ...
   if (p != start)
     pos_byte = ((p) - (current_buffer)->text->beg - (p - (current_buffer)->text->beg <= (current_buffer->text->gpt_byte) - ((1)) ? 0 : (current_buffer->text->gap_size)) + ((1)));

   if (pos_byte == (current_buffer->zv_byte))
     return column;
   if (pos_byte == next_boundary_byte)
     {
       ptrdiff_t next_boundary;
       ptrdiff_t pos = (buf_bytepos_to_charpos (current_buffer, pos_byte));
       pos = skip_invisible (pos, &next_boundary, (current_buffer->zv), builtin_lisp_symbol (0));
       pos_byte = (buf_charpos_to_bytepos (current_buffer, pos));
       next_boundary_byte = (buf_charpos_to_bytepos (current_buffer, next_boundary));
     }
   if (pos_byte >= ceiling)
     ceiling = (((pos_byte) < (current_buffer->text->gpt_byte) && (current_buffer->text->gpt) < (current_buffer->zv) ? (current_buffer->text->gpt_byte) : (current_buffer->zv_byte)) - 1) + 1;


   stop_pos_byte = ((ceiling) < (next_boundary_byte) ? (ceiling) : (next_boundary_byte));




   stop = (((stop_pos_byte - 1) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (stop_pos_byte - 1) + (current_buffer->text->beg) - ((1))) + 1;
   p = (((pos_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (pos_byte) + (current_buffer->text->beg) - ((1)));
 }
      switch (*p++)
 {
 ...
 case '\t':
   ...
   break;
 default:
   if ((((sizeof ((p[-1]) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((p[-1]) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((p[-1]) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((p[-1]) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0))
       || ((((current_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0))))
     return column;
   {
     int c;
     pos_byte = ((p - 1) - (current_buffer)->text->beg - (p - 1 - (current_buffer)->text->beg <= (current_buffer->text->gpt_byte) - ((1)) ? 0 : (current_buffer->text->gap_size)) + ((1)));
     c = FETCH_MULTIBYTE_CHAR (pos_byte);
     if (CHAR_HAS_CATEGORY (c, ' '))
       {
  column++;
  do { unsigned char *chp = (((pos_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (pos_byte) + (current_buffer->text->beg) - ((1))); pos_byte += (!((*chp) & 0x80) ? 1 : !((*chp) & 0x20) ? 2 : !((*chp) & 0x10) ? 3 : !((*chp) & 0x08) ? 4 : 5); } while (0);
  p = (((pos_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (pos_byte) + (current_buffer->text->beg) - ((1)));
       }
     else
       return column;
...
{
  while (pos > (current_buffer->begv) && *(((((pos_byte)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((pos_byte)) + (current_buffer->text->beg) - ((1)))) == '\n')
    {
      ...
      pos = find_newline (pos, pos_byte, (current_buffer->begv), (current_buffer->begv_byte),
     -1, ((void*)0), &pos_byte, 0);
    }
  return position_indentation (pos_byte) >= column;
}

static struct Lisp_Subr _Alignas (8) Smove_to_column = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fmove_to_column }, 1, 2, "move-to-column", "NMove to column: ", 0}; Lisp_Object Fmove_to_column
# 990 "indent.c"
  (Lisp_Object column, Lisp_Object force)
{
  ...
  CHECK_NATNUM (column);
  goal = ((column) >> INTTYPEBITS);

  col = goal;
  pos = (current_buffer->zv);
  ...
  if (!((force) == (builtin_lisp_symbol (0))) && col > goal)
    {
      int c;
      ...
      if (c == '\t' && prev_col < goal)
 {
   ...
   (set_point_both ((current_buffer->pt + 0) - 1, (current_buffer->pt_byte + 0) - 1));
   Finsert_char (((EMACS_INT) (((EMACS_UINT) (' ') << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (goal - prev_col) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (901));


   del_range ((current_buffer->pt + 0), (current_buffer->pt + 0) + 1);
   goal_pt = (current_buffer->pt + 0);
   goal_pt_byte = (current_buffer->pt_byte + 0);
   Findent_to (((EMACS_INT) (((EMACS_UINT) (col) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (0));
  ...
  if (col < goal && ((force) == (builtin_lisp_symbol (901))))
    Findent_to (((EMACS_INT) (((EMACS_UINT) (col = goal) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (0));

  last_known_column = col;
  last_known_column_point = (current_buffer->pt + 0);
  last_known_column_modified = (current_buffer->text->modiff);

  return ((EMACS_INT) (((EMACS_UINT) (col) << INTTYPEBITS) + Lisp_Int0));
}
...
compute_motion (ptrdiff_t from, ptrdiff_t frombyte, EMACS_INT fromvpos,
  EMACS_INT fromhpos, _Bool did_motion, ptrdiff_t to,
  ...
  ptrdiff_t hscroll, int tab_offset, struct window *win)
{
  ...
  EMACS_INT selective
    = (((((enum Lisp_Type) ((((current_buffer)-> selective_display_)) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)
       ? ((((current_buffer)-> selective_display_)) >> INTTYPEBITS)
       : !((((current_buffer)-> selective_display_)) == (builtin_lisp_symbol (0))) ? -1 : 0);
  ptrdiff_t selective_rlen
    = (selective && dp && VECTORP (((dp)->extras[4]))
       ? ASIZE (((dp)->extras[4])) : 0);
  ...
  ptrdiff_t prev_pos;
  ptrdiff_t prev_pos_byte;
  EMACS_INT prev_hpos = 0;
  EMACS_INT prev_vpos = 0;
  EMACS_INT contin_hpos;
  int prev_tab_offset;
  ...
  ((((window) = make_lisp_ptr (win, Lisp_Vectorlike)), ((void) (0 && (((((struct vectorlike_header *) ((void *) (intptr_t) ((window) - (Lisp_Vectorlike)))) ->size) & (((9223372036854775807L) - (9223372036854775807L) / 2) | PVEC_TYPE_MASK)) == (((9223372036854775807L) - (9223372036854775807L) / 2) | (PVEC_WINDOW << PSEUDOVECTOR_AREA_BITS)))))));

  if (cache_buffer->base_buffer)
    cache_buffer = cache_buffer->base_buffer;
  width_cache = width_run_cache_on_off ();
  if (dp == buffer_display_table ())
    width_table = (VECTORP (((current_buffer)-> width_table_))
                   ? XVECTOR (((current_buffer)-> width_table_))->contents
                   : 0);
  else
  ...
  if (width < 0)
    {
      ...
      if (!(((((void) (0 && (FRAMEP (win->frame)))), (struct frame *) ((void *) (intptr_t) ((win->frame) - (Lisp_Vectorlike)))))->output_method == output_x_window))
 ...
 width -= 1;
  ...
  ((void) (0 && (from == (buf_bytepos_to_charpos (current_buffer, frombyte)) && frombyte == (buf_charpos_to_bytepos (current_buffer, from)))));
  ...
  memset (&cmp_it, 0, sizeof cmp_it);
  cmp_it.id = -1;
  composition_compute_stop_pos (&cmp_it, pos, pos_byte, to, builtin_lisp_symbol (0));
...
# 1326 "indent.c"
      if (hpos > width)
 {
   ...
   if (!((globals.f_Vtruncate_partial_width_windows) == (builtin_lisp_symbol (0)))
       && (total_width < ((((void) (0 && (FRAMEP (((win)->frame))))), (struct frame *) ((void *) (intptr_t) ((((win)->frame)) - (Lisp_Vectorlike)))))->text_cols))
     {
       if (((((enum Lisp_Type) ((globals.f_Vtruncate_partial_width_windows) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0))
  truncate
    = total_width < ((globals.f_Vtruncate_partial_width_windows) >> INTTYPEBITS);
       else
  truncate = 1;
     }

   if (hscroll || truncate
       || !((((current_buffer)-> truncate_lines_)) == (builtin_lisp_symbol (0))))
     {
      ...
      else
 {
   ...
   if (cmp_it.id >= 0
    ...
    && composition_reseat_it (&cmp_it, pos, pos_byte, to, win,
         ((void*)0), builtin_lisp_symbol (0))))
     composition_update_it (&cmp_it, pos, pos_byte, builtin_lisp_symbol (0));
   if (cmp_it.id >= 0)
     {
       pos += cmp_it.nchars;
  ...
  cmp_it.from = cmp_it.to;
   ...
   c = *(((((pos_byte)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((pos_byte)) + (current_buffer->text->beg) - ((1))));
   pos++, pos_byte++;
   ...
   if (width_cache)
    ...
    && ((width_table[c]) >> INTTYPEBITS) == width_run_width)
  width_run_end = pos;
       ...
       else
  {
    ...
    width_run_width = ((width_table[c]) >> INTTYPEBITS);
       ...
       else if (multibyte && (((c) & 0xC0) == 0xC0))
  {
    ...
    do { int ch; ch = (!((ptr)[0] & 0x80) ? ((mb_bytes) = 1, (ptr)[0]) : ! ((ptr)[0] & 0x20) ? ((mb_bytes) = 2, (((((ptr)[0] & 0x1F) << 6) | ((ptr)[1] & 0x3F)) + (((unsigned char) (ptr)[0]) < 0xC2 ? 0x3FFF80 : 0))) : ! ((ptr)[0] & 0x10) ? ((mb_bytes) = 3, ((((ptr)[0] & 0x0F) << 12) | (((ptr)[1] & 0x3F) << 6) | ((ptr)[2] & 0x3F))) : string_char ((ptr), ((void*)0), &mb_bytes)); if ((!((*ptr) & 0x80) ? 1 : !((*ptr) & 0x20) ? 2 : !((*ptr) & 0x10) ? 3 : !((*ptr) & 0x08) ? 4 : 5) != mb_bytes) mb_width = mb_bytes * 4; else { if (dp != 0 && VECTORP (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[ch] : (dp)->ascii)) : disp_char_vector ((dp), (ch))))) mb_width = sanitize_char_width (ASIZE (((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? ((((dp)->ascii) == (builtin_lisp_symbol (0))) ? (dp)->defalt : (SUB_CHAR_TABLE_P ((dp)->ascii) ? XSUB_CHAR_TABLE ((dp)->ascii)->contents[ch] : (dp)->ascii)) : disp_char_vector ((dp), (ch))))); else mb_width = ((((sizeof ((ch) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((ch) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((ch) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((ch) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) ? (ch < 0x20 ? (ch == '\t' ? sanitize_tab_width (((((current_buffer)-> tab_width_)) >> INTTYPEBITS)) : (ch == '\n' ? 0 : (((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2))) : (ch < 0x7f ? 1 : ((((((current_buffer)-> ctl_arrow_)) == (builtin_lisp_symbol (0))) ? 4 : 2)))) : sanitize_char_width (((CHAR_TABLE_REF (globals.f_Vchar_width_table, ch)) >> INTTYPEBITS))); } } while (0);
    pos_byte += mb_bytes;
    if (mb_width > 1 && (!((*ptr) & 0x80) ? 1 : !((*ptr) & 0x20) ? 2 : !((*ptr) & 0x10) ? 3 : !((*ptr) & 0x08) ? 4 : 5) == mb_bytes)
...
static struct Lisp_Subr _Alignas (8) Scompute_motion = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a7 = Fcompute_motion }, 7, 7, "compute-motion", 0, 0}; Lisp_Object Fcompute_motion
# 1741 "indent.c"
   ...
   Lisp_Object width, Lisp_Object offsets, Lisp_Object window)
{
  ...
  do { if (((((enum Lisp_Type) (((from)) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Misc) && XMISCTYPE ((from)) == Lisp_Misc_Marker)) ((from) = make_natnum (marker_position (from))); else ((((((enum Lisp_Type) ((from) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (558), from)); } while (0);
  CHECK_CONS (frompos);
  CHECK_NUMBER_CAR (frompos);
  CHECK_NUMBER_CDR (frompos);
  do { if (((((enum Lisp_Type) (((to)) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Misc) && XMISCTYPE ((to)) == Lisp_Misc_Marker)) ((to) = make_natnum (marker_position (to))); else ((((((enum Lisp_Type) ((to) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (558), to)); } while (0);
  if (!((topos) == (builtin_lisp_symbol (0))))
    {
      CHECK_CONS (topos);
      CHECK_NUMBER_CAR (topos);
      CHECK_NUMBER_CDR (topos);
    }
  if (!((width) == (builtin_lisp_symbol (0))))
    ((((((enum Lisp_Type) ((width) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), width));

  if (!((offsets) == (builtin_lisp_symbol (0))))
    {
      CHECK_CONS (offsets);
      CHECK_NUMBER_CAR (offsets);
      CHECK_NUMBER_CDR (offsets);
      if (! (0 <= (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->car) >> INTTYPEBITS) && (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->car) >> INTTYPEBITS) <= (9223372036854775807L)
      && 0 <= (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS) && (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS) <= 2147483647))
 args_out_of_range ((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->car, (((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->u.cdr);
      hscroll = (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->car) >> INTTYPEBITS);
      tab_offset = (((((void) (0 && ((((enum Lisp_Type) ((offsets) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((offsets) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS);
    }
  else
  ...
  if (((from) >> INTTYPEBITS) < (current_buffer->begv) || ((from) >> INTTYPEBITS) > (current_buffer->zv))
    args_out_of_range_3 (from, ((EMACS_INT) (((EMACS_UINT) ((current_buffer->begv)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) ((current_buffer->zv)) << INTTYPEBITS) + Lisp_Int0)));
  if (((to) >> INTTYPEBITS) < (current_buffer->begv) || ((to) >> INTTYPEBITS) > (current_buffer->zv))
    args_out_of_range_3 (to, ((EMACS_INT) (((EMACS_UINT) ((current_buffer->begv)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) ((current_buffer->zv)) << INTTYPEBITS) + Lisp_Int0)));

  pos = compute_motion (((from) >> INTTYPEBITS), (buf_charpos_to_bytepos (current_buffer, ((from) >> INTTYPEBITS))),
   (((((void) (0 && ((((enum Lisp_Type) ((frompos) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((frompos) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS),
   (((((void) (0 && ((((enum Lisp_Type) ((frompos) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((frompos) - (Lisp_Cons))))->car) >> INTTYPEBITS), 0,
   ((to) >> INTTYPEBITS),
   (((topos) == (builtin_lisp_symbol (0)))
    ? window_internal_height (w)
    : (((((void) (0 && ((((enum Lisp_Type) ((topos) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((topos) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS)),
   (((topos) == (builtin_lisp_symbol (0)))
    ? (window_body_width (w, 0)
       - (

          (((((void) (0 && (FRAMEP (w->frame)))), (struct frame *) ((void *) (intptr_t) ((w->frame) - (Lisp_Vectorlike)))))->output_method == output_x_window) ? 0 :

          1))
    : (((((void) (0 && ((((enum Lisp_Type) ((topos) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((topos) - (Lisp_Cons))))->car) >> INTTYPEBITS)),
   (((width) == (builtin_lisp_symbol (0))) ? -1 : ((width) >> INTTYPEBITS)),
   hscroll, tab_offset, w);

  ((bufpos) = make_natnum (pos->bufpos));
  ((hpos) = ((EMACS_INT) (((EMACS_UINT) (pos->hpos) << INTTYPEBITS) + Lisp_Int0)));
  ((vpos) = ((EMACS_INT) (((EMACS_UINT) (pos->vpos) << INTTYPEBITS) + Lisp_Int0)));
  ((prevhpos) = ((EMACS_INT) (((EMACS_UINT) (pos->prevhpos) << INTTYPEBITS) + Lisp_Int0)));

  return list5 (bufpos, hpos, vpos, prevhpos, pos->contin ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0));
}
  ...
  register EMACS_INT vtarget, struct window *w)
{
  ...
  ptrdiff_t selective
    = (((((enum Lisp_Type) ((((current_buffer)-> selective_display_)) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)
       ? clip_to_bounds (-1, ((((current_buffer)-> selective_display_)) >> INTTYPEBITS),
  ...
  ((((window) = make_lisp_ptr (w, Lisp_Vectorlike)), ((void) (0 && (((((struct vectorlike_header *) ((void *) (intptr_t) ((window) - (Lisp_Vectorlike)))) ->size) & (((9223372036854775807L) - (9223372036854775807L) / 2) | PVEC_TYPE_MASK)) == (((9223372036854775807L) - (9223372036854775807L) / 2) | (PVEC_WINDOW << PSEUDOVECTOR_AREA_BITS)))))));



  if (((w->contents) == (Fcurrent_buffer ())))
  ...
  if (vpos >= vtarget)
    {
      ...
      while ((vpos > vtarget || first) && from > (current_buffer->begv))
 {
   ptrdiff_t bytepos = from_byte;
   Lisp_Object propval;

   prevline = from;
   do { (prevline)--; if (((((current_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0)))) (bytepos)--; else do { unsigned char *chp; (bytepos)--; if ((bytepos) < (current_buffer->text->gpt_byte)) chp = (current_buffer->text->beg) + (bytepos) - ((1)); else chp = (current_buffer->text->beg) + (current_buffer->text->gap_size) + (bytepos) - ((1)); while (!(((*chp) & 0xC0) != 0x80)) { chp--; (bytepos)--; } } while (0); } while (0);
   prevline = find_newline_no_quit (prevline, bytepos, -1, &bytepos);

   while (prevline > (current_buffer->begv)
   && ((selective > 0
        && indented_beyond_p (prevline, bytepos, selective))


       || (propval = Fget_char_property (((EMACS_INT) (((EMACS_UINT) (prevline - 1) << INTTYPEBITS) + Lisp_Int0)),
             builtin_lisp_symbol (574),
             text_prop_object),
    (((((current_buffer)-> invisibility_spec_)) == (builtin_lisp_symbol (901))) ? !((propval) == (builtin_lisp_symbol (0))) : invisible_prop (propval, ((current_buffer)-> invisibility_spec_))))))
     {
       do { (prevline)--; if (((((current_buffer)-> enable_multibyte_characters_)) == (builtin_lisp_symbol (0)))) (bytepos)--; else do { unsigned char *chp; (bytepos)--; if ((bytepos) < (current_buffer->text->gpt_byte)) chp = (current_buffer->text->beg) + (bytepos) - ((1)); else chp = (current_buffer->text->beg) + (current_buffer->text->gap_size) + (bytepos) - ((1)); while (!(((*chp) & 0xC0) != 0x80)) { chp--; (bytepos)--; } } while (0); } while (0);
       prevline = find_newline_no_quit (prevline, bytepos, -1, &bytepos);
     }
      ...
      if (vpos >= vtarget)
 {
  ...
  if (from > (current_buffer->begv) && *(((((from_byte - 1)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((from_byte - 1)) + (current_buffer->text->beg) - ((1)))) != '\n')
    {
      ptrdiff_t bytepos;
      Lisp_Object propval;
   ...
   || (propval = Fget_char_property (((EMACS_INT) (((EMACS_UINT) (prevline) << INTTYPEBITS) + Lisp_Int0)),
         builtin_lisp_symbol (574),
         text_prop_object),
       (((((current_buffer)-> invisibility_spec_)) == (builtin_lisp_symbol (901))) ? !((propval) == (builtin_lisp_symbol (0))) : invisible_prop (propval, ((current_buffer)-> invisibility_spec_))))))
 {
  ...
  return compute_motion (from, from_byte, vpos, pos.hpos, did_motion,
    (current_buffer->zv), vtarget, - (1 << (BITS_PER_SHORT - 1)),
    -1, hscroll, 0, w);
}
...
{
  double x = col * (((((void) (0 && (FRAMEP (w->frame)))), (struct frame *) ((void *) (intptr_t) ((w->frame) - (Lisp_Vectorlike)))))->column_width) + 0.5;


  if (! ((-2147483647 -1) <= x && x <= 2147483647))
    args_out_of_range (window, column);

  return x;
}

static struct Lisp_Subr _Alignas (8) Svertical_motion = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Fvertical_motion }, 1, 3, "vertical-motion", 0, 0}; Lisp_Object Fvertical_motion
# 1992 "indent.c"
  (Lisp_Object lines, Lisp_Object window, Lisp_Object cur_col)
{
  ...
  _Bool lcols_given = (((enum Lisp_Type) ((lines) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons);
  if (lcols_given)
    {
      lcols = (((void) (0 && ((((enum Lisp_Type) ((lines) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((lines) - (Lisp_Cons))))->car;
      lines = (((void) (0 && ((((enum Lisp_Type) ((lines) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((lines) - (Lisp_Cons))))->u.cdr;
    }

  ((((((enum Lisp_Type) ((lines) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), lines));
  w = decode_live_window (window);

  old_buffer = builtin_lisp_symbol (0);
  if (XBUFFER (w->contents) != current_buffer)
    {
      ...
      set_marker_both (w->pointm, w->contents,
         (current_buffer == current_buffer ? (current_buffer->pt + 0) : ((((current_buffer)-> pt_marker_)) == (builtin_lisp_symbol (0))) ? current_buffer->pt : marker_position (((current_buffer)-> pt_marker_))), (current_buffer == current_buffer ? (current_buffer->pt_byte + 0) : ((((current_buffer)-> pt_marker_)) == (builtin_lisp_symbol (0))) ? current_buffer->pt_byte : marker_byte_position (((current_buffer)-> pt_marker_))));
    }

  if (noninteractive)
    {
      struct position pos;
      pos = *vmotion ((current_buffer->pt + 0), (current_buffer->pt_byte + 0), ((lines) >> INTTYPEBITS), w);
      (set_point_both (pos.bufpos, pos.bytepos));
      it.vpos = pos.vpos;
    }
  else
    {
      ...
      int first_x;
      _Bool overshoot_handled = 0;
      _Bool disp_string_at_start_p = 0;
      ptrdiff_t nlines = ((lines) >> INTTYPEBITS);
      int vpos_init = 0;
      double start_col ;
      int start_x ;
      int to_x = -1;

      _Bool start_x_given = !((cur_col) == (builtin_lisp_symbol (0)));
      if (start_x_given)
 {
   ...
   start_x = window_column_x (w, window, start_col, cur_col);
      ...
      ((pt).charpos = ((current_buffer->pt + 0)), (pt).bytepos = (current_buffer->pt_byte + 0));
      start_display (&it, w, pt);
      first_x = it.first_visible_x;
      it_start = ((it).current.pos).charpos;
      ...
      else
 {
   ...
   it.current_x = it.hpos = 0;
      ...
      if (((it).current.pos).charpos != (current_buffer->pt + 0))
# 2112 "indent.c"
 move_it_to (&it,
      (!disp_string_at_start_p
       || *(((((((it).current.pos).bytepos)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + ((((it).current.pos).bytepos)) + (current_buffer->text->beg) - ((1)))) == '\n')
      ? (current_buffer->pt + 0)
      : (current_buffer->pt + 0) - 1,
      -1, -1, -1, MOVE_TO_POS);
      ...
      if (((it).current.pos).charpos > it_start)
 {
   ...
   overshoot_handled = 1;
      ...
      else if (((it).current.pos).charpos == (current_buffer->pt + 0) - 1
        && *((((((current_buffer->pt_byte + 0) - 1)) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (((current_buffer->pt_byte + 0) - 1)) + (current_buffer->text->beg) - ((1)))) == '\n'
        && nlines <= 0)
 {
      ...
      if (nlines <= 0)
 {
   ...
   if ((nlines < 0 && ((it).current.pos).charpos > 0)
       || (nlines == 0 && !(start_x_given && start_x <= to_x)))
     move_it_by_lines (&it, (((-9223372036854775807L-1)) > (nlines) ? ((-9223372036854775807L-1)) : (nlines)));
 }
      else if (overshoot_handled)
 {
   it.vpos = vpos_init;
   move_it_by_lines (&it, (((9223372036854775807L)) < (nlines) ? ((9223372036854775807L)) : (nlines)));
 }
      else
 {
   ...
   if (it_start < (current_buffer->zv))
     {
       while (((it).current.pos).charpos <= it_start)
  {
    it.vpos = 0;
    move_it_by_lines (&it, 1);
  }
       if (nlines > 1)
  move_it_by_lines (&it, (((9223372036854775807L)) < (nlines - 1) ? ((9223372036854775807L)) : (nlines - 1)));
     }
   else
     {
       it.vpos = 0;
       ...
       if (((it).current.pos).charpos == (pt).charpos && (pt).charpos == it_start)
  it.vpos = 0;
     }
 }
      ...
      if (lcols_given)
 {
   move_it_in_display_line (&it, (current_buffer->zv), first_x + to_x, MOVE_TO_X);
   ...
   if (nlines > 0 && it.area == TEXT_AREA)
     {
       ...
       && !it.string_from_display_prop_p
       && memchr (SSDATA (it.string) + ((it).current.string_pos).bytepos,
    '\n',
    SBYTES (it.string) - ((it).current.string_pos).bytepos))
  {
    move_it_by_lines (&it, 1);
    move_it_in_display_line (&it, (current_buffer->zv), first_x + to_x, MOVE_TO_X);
  }
      ...
      (set_point_both (((it).current.pos).charpos, ((it).current.pos).bytepos));
  ...
  return ((EMACS_INT) (((EMACS_UINT) (it.vpos) << INTTYPEBITS) + Lisp_Int0));
}
...
syms_of_indent (void)
{
  do { static struct Lisp_Boolfwd b_fwd; defvar_bool (&b_fwd, "indent-tabs-mode", &globals.f_indent_tabs_mode); } while (0);

  globals.f_indent_tabs_mode = 1;
