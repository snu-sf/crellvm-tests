...
cmputc (int c)
{
  if (current_tty->termscript)
    _IO_putc (c & 0177, current_tty->termscript);
  _IO_putc (c & 0177, current_tty->output);
  return c;
}
# 112 "cm.c"
void
...
{
  if ((tty)->Wcm->cm_curX == (tty)->Wcm->cm_cols)
    {
      if (!(tty)->Wcm->cm_magicwrap || (tty)->Wcm->cm_curY >= (tty)->Wcm->cm_rows - 1)
 emacs_abort ();
      if (tty->termscript)
 _IO_putc ('\r', tty->termscript);
      _IO_putc ('\r', tty->output);
      if (tty->termscript)
 _IO_putc ('\n', tty->termscript);
      _IO_putc ('\n', tty->output);
      (tty)->Wcm->cm_curX = 0;
...
cmcostinit (struct tty_display_info *tty)
{
    ...
    tty->Wcm->cc_up = (tty->Wcm->cm_up ? (cost = 0, tputs (tty->Wcm->cm_up, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_down = (tty->Wcm->cm_down ? (cost = 0, tputs (tty->Wcm->cm_down, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_left = (tty->Wcm->cm_left ? (cost = 0, tputs (tty->Wcm->cm_left, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_right = (tty->Wcm->cm_right ? (cost = 0, tputs (tty->Wcm->cm_right, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_home = (tty->Wcm->cm_home ? (cost = 0, tputs (tty->Wcm->cm_home, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_cr = (tty->Wcm->cm_cr ? (cost = 0, tputs (tty->Wcm->cm_cr, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_ll = (tty->Wcm->cm_ll ? (cost = 0, tputs (tty->Wcm->cm_ll, 1, evalcost), cost) : 9999);
    tty->Wcm->cc_tab = tty->Wcm->cm_tabwidth ? (tty->Wcm->cm_tab ? (cost = 0, tputs (tty->Wcm->cm_tab, 1, evalcost), cost) : 9999) : 9999;
# 163 "cm.c"
    tty->Wcm->cc_abs = ((tty->Wcm->cm_abs == 0) ? 9999 : (p = tgoto(tty->Wcm->cm_abs, 0, 0), (p ? (cost = 0, tputs (p, 1, evalcost), cost) : 9999)));
    tty->Wcm->cc_habs = ((tty->Wcm->cm_habs == 0) ? 9999 : (p = tgoto(tty->Wcm->cm_habs, 0, 0), (p ? (cost = 0, tputs (p, 1, evalcost), cost) : 9999)));
    tty->Wcm->cc_vabs = ((tty->Wcm->cm_vabs == 0) ? 9999 : (p = tgoto(tty->Wcm->cm_vabs, 0, 0), (p ? (cost = 0, tputs (p, 1, evalcost), cost) : 9999)));
          ...
          int srcy, int srcx, int dsty, int dstx, int doit)
{
 ...
 return c;
    ...
    if (doit)
      do
          (current_tty = (tty), tputs (p, 1, cmputc));
      while (--deltay > 0);
x:
 ...
 goto dodelta;
    ...
    if (tty->Wcm->cc_tab >= 9999 || !tty->Wcm->cm_usetabs)
 goto olddelta;
# 235 "cm.c"
 ...
 if (doit)
     while (--ntabs >= 0)
              (current_tty = (tty), tputs (tty->Wcm->cm_tab, 1, cmputc));
 srcx = tabx;
    }
...
cmgoto (struct tty_display_info *tty, int row, int col)
{
    ...
    if (row == (tty)->Wcm->cm_curY && col == (tty)->Wcm->cm_curX)
    return;

    if ((tty)->Wcm->cm_curY >= 0 && (tty)->Wcm->cm_curX >= 0)
    {
      ...
      relcost = calccost (tty, (tty)->Wcm->cm_curY, (tty)->Wcm->cm_curX, row, col, 0);
      use = 0;
      if ((homecost = tty->Wcm->cc_home) < 9999)
          homecost += calccost (tty, 0, 0, row, col, 0);
      if (homecost < relcost)
          relcost = homecost, use = 1;
      if ((llcost = tty->Wcm->cc_ll) < 9999)
          llcost += calccost (tty, tty->Wcm->cm_rows - 1, 0, row, col, 0);
      if (llcost < relcost)
          relcost = llcost, use = 2;
      if ((crcost = tty->Wcm->cc_cr) < 9999) {
   if (tty->Wcm->cm_autolf)
            if ((tty)->Wcm->cm_curY + 1 >= tty->Wcm->cm_rows)
                crcost = 9999;
       else
                crcost += calccost (tty, (tty)->Wcm->cm_curY + 1, 0, row, col, 0);
   else
            crcost += calccost (tty, (tty)->Wcm->cm_curY, 0, row, col, 0);
      }
      if (crcost < relcost)
   relcost = crcost, use = 3;
      directcost = tty->Wcm->cc_abs, dcm = tty->Wcm->cm_abs;
      if (row == (tty)->Wcm->cm_curY && tty->Wcm->cc_habs < 9999)
   directcost = tty->Wcm->cc_habs, dcm = tty->Wcm->cm_habs;
      else if (col == (tty)->Wcm->cm_curX && tty->Wcm->cc_vabs < 9999)
  ...
  if (directcost <= relcost)
    {
      ...
      p = (dcm == tty->Wcm->cm_habs
           ? tgoto (dcm, row, col)
           : tgoto (dcm, col, row));
      (current_tty = (tty), tputs (p, 1, evalcost));
      if (cost <= relcost)
 {
   (current_tty = (tty), tputs (p, 1, cmputc));
   (tty)->Wcm->cm_curY = row, (tty)->Wcm->cm_curX = col;
   return;
 }
    }
  ...
  switch (use)
    {
    case 1:
      (current_tty = (tty), tputs (tty->Wcm->cm_home, 1, cmputc));
      (tty)->Wcm->cm_curY = 0, (tty)->Wcm->cm_curX = 0;
      break;

    case 2:
      (current_tty = (tty), tputs (tty->Wcm->cm_ll, 1, cmputc));
      (tty)->Wcm->cm_curY = tty->Wcm->cm_rows - 1, (tty)->Wcm->cm_curX = 0;
      break;

    case 3:
      (current_tty = (tty), tputs (tty->Wcm->cm_cr, 1, cmputc));
      if (tty->Wcm->cm_autolf)
 (tty)->Wcm->cm_curY++;
      (tty)->Wcm->cm_curX = 0;
      break;
  ...
  (void) calccost (tty, (tty)->Wcm->cm_curY, (tty)->Wcm->cm_curX, row, col, 1);
  (tty)->Wcm->cm_curY = row, (tty)->Wcm->cm_curX = col;
}
