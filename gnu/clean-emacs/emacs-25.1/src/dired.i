...
open_directory (Lisp_Object dirname, int *fdp)
{
  ...
  block_input ();
# 82 "dired.c"
  fd = emacs_open (name, 00 | 0200000, 0);
  if (fd < 0)
    {
      opendir_errno = (*__errno_location ());
      d = 0;
    }
  else
    {
...
directory_files_internal (Lisp_Object directory, Lisp_Object full,
     Lisp_Object match, Lisp_Object nosort, _Bool attrs,
     Lisp_Object id_format)
{
  ptrdiff_t directory_nbytes;
  Lisp_Object list, dirfilename, encoded_directory;
  struct re_pattern_buffer *bufp = ((void*)0);
  _Bool needsep = 0;
  ...
  list = encoded_directory = dirfilename = builtin_lisp_symbol (0);
  dirfilename = Fdirectory_file_name (directory);

  if (!((match) == (builtin_lisp_symbol (0))))
    {
      CHECK_STRING (match);
# 193 "dired.c"
      bufp = compile_pattern (match, 0, builtin_lisp_symbol (0), 0, 1);
  ...
  dirfilename = encode_file_name (dirfilename);
  encoded_directory = encode_file_name (directory);
...
# 236 "dired.c"
  ...
  if (directory_nbytes == 0
      || !(((SREF (directory, directory_nbytes - 1)) == '/')))
    needsep = 1;
  ...
  for (struct dirent *dp; (dp = read_dirent (d, directory)); )
    {
      ...
      _Bool wanted = (((match) == (builtin_lisp_symbol (0)))
      ...
      if (wanted)
 {
   if (!((full) == (builtin_lisp_symbol (0))))
     {
       ...
       if (needsep)
  SSET (fullname, directory_nbytes, '/');
  ...
  emacs_abort ();
       ...
       if (nchars == nbytes)
  do { if (XSTRING (fullname)->size == 0) (fullname) = empty_unibyte_string; else XSTRING (fullname)->size_byte = -1; } while (0);
  ...
  if (((nosort) == (builtin_lisp_symbol (0))))
    list = Fsort (Fnreverse (list),
    attrs ? builtin_lisp_symbol (408) : builtin_lisp_symbol (884));
...
static struct Lisp_Subr _Alignas (8) Sdirectory_files = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a4 = Fdirectory_files }, 1, 4, "directory-files", 0, 0}; Lisp_Object Fdirectory_files
# 339 "dired.c"
  (Lisp_Object directory, Lisp_Object full, Lisp_Object match, Lisp_Object nosort)
{
  ...
  handler = Ffind_file_name_handler (directory, builtin_lisp_symbol (341));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call5 (handler, builtin_lisp_symbol (341), directory,
                  full, match, nosort);

  return directory_files_internal (directory, full, match, nosort, 0, builtin_lisp_symbol (0));
}

static struct Lisp_Subr _Alignas (8) Sdirectory_files_and_attributes = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a5 = Fdirectory_files_and_attributes }, 1, 5, "directory-files-and-attributes", 0, 0}; Lisp_Object Fdirectory_files_and_attributes
# 367 "dired.c"
  (Lisp_Object directory, Lisp_Object full, Lisp_Object match, Lisp_Object nosort, Lisp_Object id_format)
{
  ...
  handler = Ffind_file_name_handler (directory, builtin_lisp_symbol (342));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call6 (handler, builtin_lisp_symbol (342),
...
static Lisp_Object file_name_completion (Lisp_Object, Lisp_Object, _Bool,
      Lisp_Object);

static struct Lisp_Subr _Alignas (8) Sfile_name_completion = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Ffile_name_completion }, 2, 3, "file-name-completion", 0, 0}; Lisp_Object Ffile_name_completion
# 401 "dired.c"
  (Lisp_Object file, Lisp_Object directory, Lisp_Object predicate)
{
  ...
  handler = Ffind_file_name_handler (directory, builtin_lisp_symbol (417));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call4 (handler, builtin_lisp_symbol (417), file, directory, predicate);



  handler = Ffind_file_name_handler (file, builtin_lisp_symbol (417));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call4 (handler, builtin_lisp_symbol (417), file, directory, predicate);
...
static struct Lisp_Subr _Alignas (8) Sfile_name_all_completions = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Ffile_name_all_completions }, 2, 2, "file-name-all-completions", 0, 0}; Lisp_Object Ffile_name_all_completions
  ...
  (Lisp_Object file, Lisp_Object directory)
{
  ...
  handler = Ffind_file_name_handler (directory, builtin_lisp_symbol (415));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call3 (handler, builtin_lisp_symbol (415), file, directory);



  handler = Ffind_file_name_handler (file, builtin_lisp_symbol (415));
  if (!((handler) == (builtin_lisp_symbol (0))))
    return call3 (handler, builtin_lisp_symbol (415), file, directory);

  return file_name_completion (file, directory, 1, builtin_lisp_symbol (0));
}

static int file_name_completion_stat (int, struct dirent *, struct stat *);

static Lisp_Object
file_name_completion (Lisp_Object file, Lisp_Object dirname, _Bool all_flag,
        Lisp_Object predicate)
{
  ...
  _Bool includeall = 1;
  _Bool check_decoded = 0;
  ptrdiff_t count = SPECPDL_INDEX ();

  elt = builtin_lisp_symbol (0);

  CHECK_STRING (file);

  bestmatch = builtin_lisp_symbol (0);
  encoded_file = encoded_dir = builtin_lisp_symbol (0);
  specbind (builtin_lisp_symbol (325), dirname);
  ...
  encoded_dir = encode_file_name (Fdirectory_file_name (dirname));

  Lisp_Object file_encoding = globals.f_Vfile_name_coding_system;
  if (((globals.f_Vfile_name_coding_system) == (builtin_lisp_symbol (0))))
    file_encoding = globals.f_Vdefault_file_name_coding_system;




  if (!((file_encoding) == (builtin_lisp_symbol (0)))
      && !((Fplist_get (Fcoding_system_plist (file_encoding), builtin_lisp_symbol (322))) == (builtin_lisp_symbol (0))))
      ...
      if (STRING_MULTIBYTE (file))
 {
   ...
   file = decode_file_name (encoded_file);
 }
    }
  int fd;
 ...
 continue;
      ...
      directoryp = ((((st.st_mode)) & 0170000) == (0040000)) != 0;
...
# 552 "dired.c"
  ...
  for (tem = globals.f_Vcompletion_ignored_extensions;
       (((enum Lisp_Type) ((tem) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); tem = (((void) (0 && ((((enum Lisp_Type) ((tem) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tem) - (Lisp_Cons))))->u.cdr)
    {
      ptrdiff_t elt_len;
      char *p1;

      elt = (((void) (0 && ((((enum Lisp_Type) ((tem) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tem) - (Lisp_Cons))))->car;
      if (!STRINGP (elt))
        continue;


      elt = encode_file_name (elt);
   ...
   if (includeall && !canexclude)
     {
      ...
      {
 Lisp_Object regexps, table = (globals.f_completion_ignore_case
          ? Vascii_canon_table : builtin_lisp_symbol (0));


 for (regexps = globals.f_Vcompletion_regexp_list; (((enum Lisp_Type) ((regexps) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons);
      regexps = (((void) (0 && ((((enum Lisp_Type) ((regexps) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((regexps) - (Lisp_Cons))))->u.cdr)
   if (fast_string_match_internal ((((void) (0 && ((((enum Lisp_Type) ((regexps) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((regexps) - (Lisp_Cons))))->car, name, table) < 0)
     break;

 if ((((enum Lisp_Type) ((regexps) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons))
   continue;
      }
 ...
 name = Ffile_name_as_directory (name);
      ...
      if (!((predicate) == (builtin_lisp_symbol (0))) && ((call1 (predicate, name)) == (builtin_lisp_symbol (0))))
 continue;




      Lisp_Object zero = ((EMACS_INT) (((EMACS_UINT) (0) << INTTYPEBITS) + Lisp_Int0));
      ...
      if (check_decoded && SCHARS (file) <= SCHARS (name))
 {
   ...
   Lisp_Object cmp
     = Fcompare_strings (name, zero, ((EMACS_INT) (((EMACS_UINT) (compare) << INTTYPEBITS) + Lisp_Int0)),
    file, zero, ((EMACS_INT) (((EMACS_UINT) (compare) << INTTYPEBITS) + Lisp_Int0)),
    globals.f_completion_ignore_case ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0));
   if (!((cmp) == (builtin_lisp_symbol (901))))
     continue;
 }
 ...
 bestmatch = Fcons (name, bestmatch);
      else if (((bestmatch) == (builtin_lisp_symbol (0))))
 {
   bestmatch = name;
      ...
      else
 {
   ...
   ptrdiff_t compare = ((bestmatchsize) < (SCHARS (name)) ? (bestmatchsize) : (SCHARS (name)));
   Lisp_Object cmp
     = Fcompare_strings (bestmatch, zero, ((EMACS_INT) (((EMACS_UINT) (compare) << INTTYPEBITS) + Lisp_Int0)),
    name, zero, ((EMACS_INT) (((EMACS_UINT) (compare) << INTTYPEBITS) + Lisp_Int0)),
    globals.f_completion_ignore_case ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0));
   ptrdiff_t matchsize = ((cmp) == (builtin_lisp_symbol (901))) ? compare : ((((cmp) >> INTTYPEBITS)) < 0 ? -(((cmp) >> INTTYPEBITS)) : (((cmp) >> INTTYPEBITS))) - 1;

   if (globals.f_completion_ignore_case)
     {
...
# 719 "dired.c"
    (((matchsize == SCHARS (name))
     ...
     && (cmp = Fcompare_strings (name, zero,
            ((EMACS_INT) (((EMACS_UINT) (SCHARS (file)) << INTTYPEBITS) + Lisp_Int0)),
            file, zero,
            builtin_lisp_symbol (0),
            builtin_lisp_symbol (0)),
         ((builtin_lisp_symbol (901)) == (cmp)))
  ...
  bestmatch = name;
   ...
   if (matchsize <= SCHARS (file)
       ...
       && (!globals.f_completion_ignore_case || matchsize == 0)
  ...
  if (all_flag || ((bestmatch) == (builtin_lisp_symbol (0))))
    return bestmatch;


  if (matchcount == 1 && !((Fequal (bestmatch, file)) == (builtin_lisp_symbol (0))))
    return builtin_lisp_symbol (901);
  bestmatch = Fsubstring (bestmatch, ((EMACS_INT) (((EMACS_UINT) (0) << INTTYPEBITS) + Lisp_Int0)),
     ((EMACS_INT) (((EMACS_UINT) (bestmatchsize) << INTTYPEBITS) + Lisp_Int0)));
  return bestmatch;
}
...
file_name_completion_stat (int fd, struct dirent *dp, struct stat *st_addr)
{
  int value;
# 815 "dired.c"
  value = fstatat (fd, dp->d_name, st_addr, 0x100);
  if (value == 0 && ((((st_addr->st_mode)) & 0170000) == (0120000)))
...
static struct Lisp_Subr _Alignas (8) Sfile_attributes = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Ffile_attributes }, 1, 2, "file-attributes", 0, 0}; Lisp_Object Ffile_attributes
# 896 "dired.c"
  (Lisp_Object filename, Lisp_Object id_format)
{
  ...
  filename = internal_condition_case_2 (Fexpand_file_name, filename, builtin_lisp_symbol (0),
     builtin_lisp_symbol (901), Fidentity);
  if (!STRINGP (filename))
    return builtin_lisp_symbol (0);



  handler = Ffind_file_name_handler (filename, builtin_lisp_symbol (407));
  if (!((handler) == (builtin_lisp_symbol (0))))
    {

      if (((id_format) == (builtin_lisp_symbol (0))))
 return call2 (handler, builtin_lisp_symbol (407), filename);
      else
 return call3 (handler, builtin_lisp_symbol (407), filename, id_format);
    }

  encoded = encode_file_name (filename);
  return file_attributes (-100, SSDATA (encoded), id_format);
}
...
file_attributes (int fd, char const *name, Lisp_Object id_format)
{
  ...
  char *uname = ((void*)0), *gname = ((void*)0);
# 942 "dired.c"
  lstat_result = fstatat (fd, name, &s, 0x100);
  ...
  if (!(((id_format) == (builtin_lisp_symbol (0))) || ((id_format) == (builtin_lisp_symbol (557)))))
    {
      block_input ();
      uname = stat_uname (&s);
      gname = stat_gname (&s);
  ...
  return (Flist) ((sizeof (((Lisp_Object []) {(((((s.st_mode)) & 0170000) == (0120000)) ? emacs_readlinkat (fd, name) : ((((s.st_mode)) & 0170000) == (0040000)) ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0)), ((EMACS_INT) (((EMACS_UINT) (s.st_nlink) << INTTYPEBITS) + Lisp_Int0)), (uname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (uname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (uname)) : ((! ((0 <= (s.st_uid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_uid)) && (s.st_uid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_uid) : ((EMACS_INT) (((EMACS_UINT) (s.st_uid) << INTTYPEBITS) + Lisp_Int0)))), (gname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (gname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (gname)) : ((! ((0 <= (s.st_gid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_gid)) && (s.st_gid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_gid) : ((EMACS_INT) (((EMACS_UINT) (s.st_gid) << INTTYPEBITS) + Lisp_Int0)))), make_lisp_time (get_stat_atime (&s)), make_lisp_time (get_stat_mtime (&s)), make_lisp_time (get_stat_ctime (&s)), ((! ((0 <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size)) && (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) : ((EMACS_INT) (((EMACS_UINT) (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) << INTTYPEBITS) + Lisp_Int0))), make_string (modes, 10), builtin_lisp_symbol (901), (! (! ((0 <= (s.st_ino) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_ino)) && (s.st_ino) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_ino) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_ino) - (1)) < 0) ? intbig_to_lisp (s.st_ino) : uintbig_to_lisp (s.st_ino)), (! (! ((0 <= (s.st_dev) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_dev)) && (s.st_dev) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_dev) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_dev) - (1)) < 0) ? intbig_to_lisp (s.st_dev) : uintbig_to_lisp (s.st_dev))})) / sizeof (((Lisp_Object []) {(((((s.st_mode)) & 0170000) == (0120000)) ? emacs_readlinkat (fd, name) : ((((s.st_mode)) & 0170000) == (0040000)) ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0)), ((EMACS_INT) (((EMACS_UINT) (s.st_nlink) << INTTYPEBITS) + Lisp_Int0)), (uname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (uname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (uname)) : ((! ((0 <= (s.st_uid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_uid)) && (s.st_uid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_uid) : ((EMACS_INT) (((EMACS_UINT) (s.st_uid) << INTTYPEBITS) + Lisp_Int0)))), (gname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (gname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (gname)) : ((! ((0 <= (s.st_gid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_gid)) && (s.st_gid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_gid) : ((EMACS_INT) (((EMACS_UINT) (s.st_gid) << INTTYPEBITS) + Lisp_Int0)))), make_lisp_time (get_stat_atime (&s)), make_lisp_time (get_stat_mtime (&s)), make_lisp_time (get_stat_ctime (&s)), ((! ((0 <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size)) && (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) : ((EMACS_INT) (((EMACS_UINT) (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) << INTTYPEBITS) + Lisp_Int0))), make_string (modes, 10), builtin_lisp_symbol (901), (! (! ((0 <= (s.st_ino) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_ino)) && (s.st_ino) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_ino) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_ino) - (1)) < 0) ? intbig_to_lisp (s.st_ino) : uintbig_to_lisp (s.st_ino)), (! (! ((0 <= (s.st_dev) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_dev)) && (s.st_dev) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_dev) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_dev) - (1)) < 0) ? intbig_to_lisp (s.st_dev) : uintbig_to_lisp (s.st_dev))}))[0]), ((Lisp_Object []) {(((((s.st_mode)) & 0170000) == (0120000)) ? emacs_readlinkat (fd, name) : ((((s.st_mode)) & 0170000) == (0040000)) ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0)), ((EMACS_INT) (((EMACS_UINT) (s.st_nlink) << INTTYPEBITS) + Lisp_Int0)), (uname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (uname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (uname)) : ((! ((0 <= (s.st_uid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_uid)) && (s.st_uid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_uid) : ((EMACS_INT) (((EMACS_UINT) (s.st_uid) << INTTYPEBITS) + Lisp_Int0)))), (gname ? (! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_unibyte_string (gname), globals.f_Vlocale_coding_system, 0) : build_unibyte_string (gname)) : ((! ((0 <= (s.st_gid) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_gid)) && (s.st_gid) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (s.st_gid) : ((EMACS_INT) (((EMACS_UINT) (s.st_gid) << INTTYPEBITS) + Lisp_Int0)))), make_lisp_time (get_stat_atime (&s)), make_lisp_time (get_stat_mtime (&s)), make_lisp_time (get_stat_ctime (&s)), ((! ((0 <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size)) && (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) : ((EMACS_INT) (((EMACS_UINT) (sizeof (s.st_size) == 4 ? s.st_size & 0xffffffffu : s.st_size) << INTTYPEBITS) + Lisp_Int0))), make_string (modes, 10), builtin_lisp_symbol (901), (! (! ((0 <= (s.st_ino) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_ino)) && (s.st_ino) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_ino) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_ino) - (1)) < 0) ? intbig_to_lisp (s.st_ino) : uintbig_to_lisp (s.st_ino)), (! (! ((0 <= (s.st_dev) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (s.st_dev)) && (s.st_dev) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (s.st_dev) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (s.st_dev) - (1)) < 0) ? intbig_to_lisp (s.st_dev) : uintbig_to_lisp (s.st_dev))}));
# 987 "dired.c"
}

static struct Lisp_Subr _Alignas (8) Sfile_attributes_lessp = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Ffile_attributes_lessp }, 2, 2, "file-attributes-lessp", 0, 0}; Lisp_Object Ffile_attributes_lessp
...
static struct Lisp_Subr _Alignas (8) Ssystem_users = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fsystem_users }, 0, 0, "system-users", 0, 0}; Lisp_Object Fsystem_users



     (void)
{
  Lisp_Object users = builtin_lisp_symbol (0);

  struct passwd *pw;

  while ((pw = getpwent ()))
    users = Fcons ((! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_string (pw->pw_name), globals.f_Vlocale_coding_system, 0) : build_string (pw->pw_name)), users);

  endpwent ();

  if (((users) == (builtin_lisp_symbol (0))))

    users = list1 (globals.f_Vuser_real_login_name);
  return users;
}

static struct Lisp_Subr _Alignas (8) Ssystem_groups = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fsystem_groups }, 0, 0, "system-groups", 0, 0}; Lisp_Object Fsystem_groups


     (void)
{
  Lisp_Object groups = builtin_lisp_symbol (0);

  struct group *gr;

  while ((gr = getgrent ()))
    groups = Fcons ((! ((globals.f_Vlocale_coding_system) == (builtin_lisp_symbol (0))) ? code_convert_string_norecord (build_string (gr->gr_name), globals.f_Vlocale_coding_system, 0) : build_string (gr->gr_name)), groups);
...
syms_of_dired (void)
{
  ...
  do { static struct Lisp_Objfwd o_fwd; defvar_lisp (&o_fwd, "completion-ignored-extensions", &globals.f_Vcompletion_ignored_extensions); } while (0);





  globals.f_Vcompletion_ignored_extensions = builtin_lisp_symbol (0);
}
