...
# 1555 "/usr/include/elf.h" 3 4
typedef struct
{
  Elf32_Word hwp_flags1;
  Elf32_Word hwp_flags2;
} Elf_Options_Hw;
# 1716 "/usr/include/elf.h" 3 4
typedef struct
{
  Elf32_Word l_name;
...
# 60 "/usr/include/bits/dlfcn.h" 3 4
...
# 166 "unexelf.c" 2
# 186 "unexelf.c"
extern int (*_gl_verify_function5 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((! (! ((Elf64_Half) 0 < (Elf64_Half) -1)) || (-9223372036854775807L-1) <= ((Elf64_Half) (! (! ((Elf64_Half) 0 < (Elf64_Half) -1)) ? (Elf64_Half) 0 : ((Elf64_Half) ~ (Elf64_Half) 0 < (Elf64_Half) -1) ? ~ (Elf64_Half) 0 : ~ ((Elf64_Half) (! (! ((Elf64_Half) 0 < (Elf64_Half) -1)) ? (Elf64_Half) -1 : ((((Elf64_Half) 1 << (sizeof (Elf64_Half) * 8 - 2)) - 1) * 2 + 1)))))) && ((Elf64_Half) (! (! ((Elf64_Half) 0 < (Elf64_Half) -1)) ? (Elf64_Half) -1 : ((((Elf64_Half) 1 << (sizeof (Elf64_Half) * 8 - 2)) - 1) * 2 + 1))) <= (9223372036854775807L)) ? 1 : -1; }))];
# 198 "unexelf.c"
static void *
...
# 241 "unexelf.c"
  Elf64_Ehdr *old_file_h, *new_file_h;
  Elf64_Phdr *old_program_h, *new_program_h;
  Elf64_Shdr *old_section_h, *new_section_h;


  char *old_section_names, *new_section_names;

  Elf64_Phdr *old_bss_seg, *new_bss_seg;
  Elf64_Addr old_bss_addr, new_bss_addr;
  Elf64_Word old_bss_size, bss_size_growth, new_data2_size;
  Elf64_Off old_bss_offset, new_data2_offset;
  ...
  old_file = emacs_open (old_name, 00, 0);

  if (old_file < 0)
    fatal ("Can't open %s for reading: %s", old_name, strerror ((*__errno_location ())));

  if (fstat (old_file, &stat_buf) != 0)
    fatal ("Can't fstat (%s): %s", old_name, strerror ((*__errno_location ())));
# 279 "unexelf.c"
  old_file_size = stat_buf.st_size;
  if (! (0 <= old_file_size && old_file_size <= (18446744073709551615UL)))
    fatal ("File size out of range");
  old_base = mmap (((void*)0), old_file_size, 0x1 | 0x2,
     0x20 | 0x02, -1, 0);
  if (old_base == ((void *) -1))
    fatal ("Can't allocate buffer for %s: %s", old_name, strerror ((*__errno_location ())));

  if (read (old_file, old_base, old_file_size) != old_file_size)
    fatal ("Didn't read all of %s: %s", old_name, strerror ((*__errno_location ())));



  old_file_h = (Elf64_Ehdr *) old_base;
  old_program_h = (Elf64_Phdr *) ((byte *) old_base + old_file_h->e_phoff);
  old_section_h = (Elf64_Shdr *) ((byte *) old_base + old_file_h->e_shoff);
  old_section_names = (char *) old_base
    + (*(Elf64_Shdr *) entry_address (old_section_h, old_file_h->e_shstrndx, old_file_h->e_shentsize)).sh_offset;
  ...
  for (n = old_file_h->e_phnum; --n >= 0; )
    {
      Elf64_Phdr *seg = &(*(Elf64_Phdr *) entry_address (old_program_h, n, old_file_h->e_phentsize));
      if (seg->p_type == 1
   && (old_bss_seg == 0
 ...
 old_bss_seg = seg;
  ...
  for (n = old_file_h->e_shnum; --n > 0; )
    {
      Elf64_Shdr *shdr = &(*(Elf64_Shdr *) entry_address (old_section_h, n, old_file_h->e_shentsize));
      if (shdr->sh_type == 8
   && shdr->sh_addr >= old_bss_addr
   && shdr->sh_addr + shdr->sh_size <= old_bss_addr + old_bss_size
   && (old_bss_index == -1
       || (*(Elf64_Shdr *) entry_address (old_section_h, old_bss_index, old_file_h->e_shentsize)).sh_addr < shdr->sh_addr))
 old_bss_index = n;
    }
  ...
  new_break = sbrk (0);
  new_bss_addr = (Elf64_Addr) new_break;
  bss_size_growth = new_bss_addr - old_bss_addr;
  new_data2_size = bss_size_growth;
  new_data2_size += _Alignof (Elf64_Shdr) - 1;
  new_data2_size -= new_data2_size % _Alignof (Elf64_Shdr);
...
# 351 "unexelf.c"
  ...
  new_file = emacs_open (new_name, 02 | 0100, 0777);
  if (new_file < 0)
    fatal ("Can't creat (%s): %s", new_name, strerror ((*__errno_location ())));

  new_file_size = old_file_size + new_data2_size;

  if (ftruncate (new_file, new_file_size))
    fatal ("Can't ftruncate (%s): %s", new_name, strerror ((*__errno_location ())));

  new_base = mmap (((void*)0), new_file_size, 0x1 | 0x2,
     0x20 | 0x02, -1, 0);
  if (new_base == ((void *) -1))
    fatal ("Can't allocate buffer for %s: %s", old_name, strerror ((*__errno_location ())));
  ...
  new_file_h = (Elf64_Ehdr *) new_base;
  ...
  new_program_h = (Elf64_Phdr *) ((byte *) new_base + new_file_h->e_phoff);
  new_section_h = (Elf64_Shdr *) ((byte *) new_base + new_file_h->e_shoff);
...
# 401 "unexelf.c"
  ...
  for (n = 1; n < old_file_h->e_shnum; n++)
    {
      caddr_t src;
      Elf64_Shdr *old_shdr = &(*(Elf64_Shdr *) entry_address (old_section_h, n, old_file_h->e_shentsize));
      Elf64_Shdr *new_shdr = &(*(Elf64_Shdr *) entry_address (new_section_h, n, new_file_h->e_shentsize));

      if (new_shdr->sh_type == 8
...
# 567 "unexelf.c"
  ...
  for (n = new_file_h->e_shnum; 0 < --n; )
    {
      byte *symnames;
      Elf64_Sym *symp, *symendp;
      Elf64_Shdr *sym_shdr = &(*(Elf64_Shdr *) entry_address (new_section_h, n, new_file_h->e_shentsize));

      if (sym_shdr->sh_type != 11
   && sym_shdr->sh_type != 2)
 continue;

      symnames = ((byte *) new_base
    + (*(Elf64_Shdr *) entry_address (new_section_h, sym_shdr->sh_link, new_file_h->e_shentsize)).sh_offset);
      symp = (Elf64_Sym *) (sym_shdr->sh_offset + new_base);
      symendp = (Elf64_Sym *) ((byte *) symp + sym_shdr->sh_size);
...
# 625 "unexelf.c"
  ...
  new_section_names = ((char *) new_base
         + (*(Elf64_Shdr *) entry_address (new_section_h, new_file_h->e_shstrndx, new_file_h->e_shentsize)).sh_offset);
  for (n = new_file_h->e_shnum; 0 < --n; )
    {
      Elf64_Shdr *old_shdr = &(*(Elf64_Shdr *) entry_address (old_section_h, n, old_file_h->e_shentsize));
      Elf64_Shdr *new_shdr = &(*(Elf64_Shdr *) entry_address (new_section_h, n, new_file_h->e_shentsize));
 ...
 *(new_section_names + new_shdr->sh_name) = ',';
  ...
  for (n = new_file_h->e_shnum; 0 < --n; )
    {
      Elf64_Shdr *rel_shdr = &(*(Elf64_Shdr *) entry_address (new_section_h, n, new_file_h->e_shentsize));
      Elf64_Shdr *shdr;
 ...
 case 4:
   ...
   shdr = &(*(Elf64_Shdr *) entry_address (new_section_h, rel_shdr->sh_info, new_file_h->e_shentsize));
   if (!strcmp (old_section_names + shdr->sh_name, ".data")
     ...
     {
       Elf64_Addr offset = shdr->sh_addr - shdr->sh_offset;
       caddr_t reloc = old_base + rel_shdr->sh_offset, end;
       for (end = reloc + rel_shdr->sh_size;
     reloc < end;
     reloc += rel_shdr->sh_entsize)
  {
    Elf64_Addr addr = ((Elf64_Rel *) reloc)->r_offset - offset;

    if (((Elf64_Rel *) reloc)->r_offset == 0)
      continue;
  ...
  if (write (new_file, new_base, new_file_size) != new_file_size)
    ...
    (unsigned long) new_file_size, new_name, strerror ((*__errno_location ())));
  ...
  if (emacs_close (old_file) != 0)
    fatal ("Can't close (%s): %s", old_name, strerror ((*__errno_location ())));

  if (emacs_close (new_file) != 0)
    fatal ("Can't close (%s): %s", new_name, strerror ((*__errno_location ())));
}
