...
# 21 "/usr/include/sys/timerfd.h" 3 4
# 1 "../lib/time.h" 1 3 4
# 21 "../lib/time.h" 3
# 22 "/usr/include/sys/timerfd.h" 2 3 4
...
# 25 "/usr/include/sys/timerfd.h" 2 3 4
...
extern int timerfd_create (clockid_t __clock_id, int __flags) __attribute__ ((__nothrow__ ));
...
extern int timerfd_settime (int __ufd, int __flags,
       ...
       struct itimerspec *__otmr) __attribute__ ((__nothrow__ ));


extern int timerfd_gettime (int __ufd, struct itimerspec *__otmr) __attribute__ ((__nothrow__ ));
# 31 "atimer.c" 2
  ...
  for (i = 0; i < 2; ++i)
    {
      ...
      for (t = *list, prev = ((void*)0); t && t != timer; prev = t, t = t->next)
 ;
  ...
  if (t)
    {
      ...
      for (p = atimers, prev = ((void*)0); p && p != t; prev = p, p = p->next)
 ;
...
set_alarm (void)
{
      ...
      if (0 <= timerfd || alarm_timer_ok)
 {
   ...
   if (alarm_timer_ok
       && timer_settime (alarm_timer, 1, &ispec, 0) == 0)
     return;
 }
...
{
  struct atimer *a = atimers, *prev = ((void*)0);
...
turn_on_atimers (_Bool on)
{
  if (on)
  ...
  else
    {
      ...
      if (alarm_timer_ok)
 timer_settime (alarm_timer, 1, &ispec, 0);
...
init_atimer (void)
{
  ...
  timerfd = (egetenv ("EMACS_IGNORE_TIMERFD") ? -1 :
      timerfd_create (0, TFD_NONBLOCK | TFD_CLOEXEC));

  if (timerfd < 0)
    {
      struct sigevent sigev;
      sigev.sigev_notify = SIGEV_SIGNAL;
      sigev.sigev_signo = 14;
      sigev.sigev_value.sival_ptr = &alarm_timer;
      alarm_timer_ok
 = timer_create (0, &sigev, &alarm_timer) == 0;
    }

  free_atimers = stopped_atimers = atimers = ((void*)0);


  struct sigaction action;
  emacs_sigaction_init (&action, handle_alarm_signal);
  sigaction (14, &action, 0);
