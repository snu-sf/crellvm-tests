...
int iso_charset_table[3][2][0x80];
# 146 "charset.c"
static struct
{
...
load_charset_map (struct charset *charset, struct charset_map_entries *entries, int n_entries, int control_flag)
{
  ...
  unsigned max_code = ((charset)->max_code);
  _Bool ascii_compatible_p = charset->ascii_compatible_p;
  ...
  if (control_flag)
    {
      if (! globals.f_inhibit_load_charset_map)
 {
   if (control_flag == 1)
     {
       if (charset->method == CHARSET_METHOD_MAP)
  {
    int n = ((charset)->code_linear_p ? (int) ((max_code) - (charset)->min_code) : (((charset)->code_space_mask[(max_code) >> 24] & 0x8) && ((charset)->code_space_mask[((max_code) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((max_code) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(max_code) & 0xFF] & 0x1)) ? (int) (((((max_code) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((max_code) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((max_code) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((max_code) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1) + 1;

    vec = Fmake_vector (((EMACS_INT) (((EMACS_UINT) (n) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (-1) << INTTYPEBITS) + Lisp_Int0)));
       ...
       else
  {
    char_table_set_range (Vchar_unify_table,
     ...
     {
       table = Fmake_char_table (builtin_lisp_symbol (0), builtin_lisp_symbol (0));
 ...
 entries = entries->next;
      ...
      from_index = ((charset)->code_linear_p ? (int) ((from) - (charset)->min_code) : (((charset)->code_space_mask[(from) >> 24] & 0x8) && ((charset)->code_space_mask[((from) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((from) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(from) & 0xFF] & 0x1)) ? (int) (((((from) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((from) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((from) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((from) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1);
      if (from == to)
 {
   to_index = from_index;
   to_c = from_c;
 ...
 {
   to_index = ((charset)->code_linear_p ? (int) ((to) - (charset)->min_code) : (((charset)->code_space_mask[(to) >> 24] & 0x8) && ((charset)->code_space_mask[((to) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((to) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(to) & 0xFF] & 0x1)) ? (int) (((((to) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((to) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((to) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((to) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1);
      ...
      if (control_flag == 1)
 {
   if (charset->method == CHARSET_METHOD_MAP)
     for (; from_index < lim_index; from_index++, from_c++)
       ASET (vec, from_index, ((EMACS_INT) (((EMACS_UINT) (from_c) << INTTYPEBITS) + Lisp_Int0)));
   else
     for (; from_index < lim_index; from_index++, from_c++)
       CHAR_TABLE_SET (Vchar_unify_table,
         ((charset)->code_offset) + from_index,
         ((EMACS_INT) (((EMACS_UINT) (from_c) << INTTYPEBITS) + Lisp_Int0)));
 }
      else if (control_flag == 2)
 {
  ...
  code = ((charset)->code_linear_p ? (code) + (charset)->min_code : (code += (charset)->char_index_offset, (((charset)->code_space[0] + (code) % (charset)->code_space[2]) | (((charset)->code_space[4] + ((code) / (charset)->code_space[3] % (charset)->code_space[6])) << 8) | (((charset)->code_space[8] + ((code) / (charset)->code_space[7] % (charset)->code_space[10])) << 16) | (((charset)->code_space[12] + ((code) / (charset)->code_space[11])) << 24))));

  if (((CHAR_TABLE_REF (table, from_c)) == (builtin_lisp_symbol (0))))
    CHAR_TABLE_SET (table, from_c, ((EMACS_INT) (((EMACS_UINT) (code) << INTTYPEBITS) + Lisp_Int0)));
       }
   else
     for (; from_index < lim_index; from_index++, from_c++)
       {
  if (((CHAR_TABLE_REF (table, from_c)) == (builtin_lisp_symbol (0))))
    CHAR_TABLE_SET (table, from_c, ((EMACS_INT) (((EMACS_UINT) (from_index) << INTTYPEBITS) + Lisp_Int0)));
       }
 }
      else if (control_flag == 3)
 for (; from_index < lim_index; from_index++, from_c++)
   (temp_charset_work->table.decoder[(from_index)] = (from_c));
      else if (control_flag == 4)
 for (; from_index < lim_index; from_index++, from_c++)
   do { if ((from_index) == 0) temp_charset_work->zero_index_char = (from_c); else if ((from_c) < 0x20000) temp_charset_work->table.encoder[(from_c)] = (from_index); else temp_charset_work->table.encoder[(from_c) - 0x10000] = (from_index); } while (0);
      else
 {
   if (ascii_compatible_p)
     {
       if (! (((sizeof ((from_c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((from_c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((from_c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((from_c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)))
  {
    if (from_c < nonascii_min_char)
      nonascii_min_char = from_c;
  }
       else if (! (((sizeof ((to_c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((to_c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((to_c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((to_c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)))
  {
   ...
   for (; from_c <= to_c; from_c++)
     do { if ((from_c) < 0x10000) (fast_map)[(from_c) >> 10] |= 1 << (((from_c) >> 7) & 7); else (fast_map)[((from_c) >> 15) + 62] |= 1 << (((from_c) >> 12) & 7); } while (0);
 }
    }

  if (control_flag == 0)
    {
      ((charset)->min_char) = (ascii_compatible_p
...
static unsigned
read_hex (FILE *fp, _Bool *eof, _Bool *overflow)
{
  int c;
  unsigned n;

  while ((c = _IO_getc (fp)) != (-1))
    {
      if (c == '#')
 {
      ...
      else if (c == '0')
 {
  ...
  n = 0;
  while (c_isxdigit (c = _IO_getc (fp)))
    {
      if ((2147483647 *2U +1U) >> 4 < n)
 *overflow = 1;
      n = ((n << 4)
    | (c - ('0' <= c && c <= '9' ? '0'
     : 'A' <= c && c <= 'F' ? 'A' - 10
...
load_charset_map_from_file (struct charset *charset, Lisp_Object mapfile,
       int control_flag)
{
  unsigned min_code = ((charset)->min_code);
  unsigned max_code = ((charset)->max_code);
  int fd;
  FILE *fp;
  struct charset_map_entries *head, *entries;
  int n_entries;
  Lisp_Object map = (USE_STACK_STRING ? (make_lisp_ptr ((&(union Aligned_String) {{strlen (".map"), -1, 0, (unsigned char *) (".map")}}.s), Lisp_String)) : build_string ((".map")));
  Lisp_Object txt = (USE_STACK_STRING ? (make_lisp_ptr ((&(union Aligned_String) {{strlen (".txt"), -1, 0, (unsigned char *) (".txt")}}.s), Lisp_String)) : build_string ((".txt")));
  Lisp_Object suffixes = (USE_STACK_CONS ? make_lisp_ptr (&(union Aligned_Cons) { { map, { make_lisp_ptr (&(union Aligned_Cons) { { txt, { builtin_lisp_symbol (0) } } }.s, Lisp_Cons) } } }.s, Lisp_Cons) : list2 (map, txt));
  ptrdiff_t count = SPECPDL_INDEX ();
  record_unwind_protect_nothing ();
  specbind (builtin_lisp_symbol (419), builtin_lisp_symbol (0));
  fd = openp (globals.f_Vcharset_map_path, mapfile, suffixes, ((void*)0), builtin_lisp_symbol (0), 0);
  ...
  unbind_to (count + 1, builtin_lisp_symbol (0));
 ...
 break;
      ...
      if (overflow)
 continue;
      if (from < min_code || to > max_code || from > to || c > 0x3FFFFF)
 continue;
...
load_charset_map_from_vector (struct charset *charset, Lisp_Object vec, int control_flag)
{
  ...
  head = ((sizeof *head) <= sa_avail ? (sa_avail -= (sizeof *head), __builtin_alloca (sizeof *head)) : (sa_must_free = 1, record_xmalloc (sizeof *head)));
  entries = head;
  memset (entries, 0, sizeof (struct charset_map_entries));
 ...
 {
   val2 = (((void) (0 && ((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((val) - (Lisp_Cons))))->u.cdr;
   val = (((void) (0 && ((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((val) - (Lisp_Cons))))->car;
   from = ((val) >> INTTYPEBITS);
   to = ((val2) >> INTTYPEBITS);
 }
      else
 ...
 {
   entries->next = ((sizeof *entries->next) <= sa_avail ? (sa_avail -= (sizeof *entries->next), __builtin_alloca (sizeof *entries->next)) : (sa_must_free = 1, record_xmalloc (sizeof *entries->next)));
   entries = entries->next;
   memset (entries, 0, sizeof (struct charset_map_entries));
 }
...
load_charset (struct charset *charset, int control_flag)
{
  ...
  if (((charset)->method) == CHARSET_METHOD_MAP)
    map = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_map));
  else
    {
      if (! ((charset)->unified_p))
 emacs_abort ();
      map = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_unify_map));
    }
  if (STRINGP (map))
...
static struct Lisp_Subr _Alignas (8) Scharsetp = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fcharsetp }, 1, 1, "charsetp", 0, 0}; Lisp_Object Fcharsetp

  (Lisp_Object object)
{
  return ((hash_lookup (XHASH_TABLE (Vcharset_hash_table), object, ((void*)0)) >= 0) ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0));
}
...
{
  int from_idx = ((temp_charset_work->current)->code_linear_p ? (int) ((from) - (temp_charset_work->current)->min_code) : (((temp_charset_work->current)->code_space_mask[(from) >> 24] & 0x8) && ((temp_charset_work->current)->code_space_mask[((from) >> 16) & 0xFF] & 0x4) && ((temp_charset_work->current)->code_space_mask[((from) >> 8) & 0xFF] & 0x2) && ((temp_charset_work->current)->code_space_mask[(from) & 0xFF] & 0x1)) ? (int) (((((from) >> 24) - (temp_charset_work->current)->code_space[12]) * (temp_charset_work->current)->code_space[11]) + (((((from) >> 16) & 0xFF) - (temp_charset_work->current)->code_space[8]) * (temp_charset_work->current)->code_space[7]) + (((((from) >> 8) & 0xFF) - (temp_charset_work->current)->code_space[4]) * (temp_charset_work->current)->code_space[3]) + (((from) & 0xFF) - (temp_charset_work->current)->code_space[0]) - ((temp_charset_work->current)->char_index_offset)) : -1);
  int to_idx = ((temp_charset_work->current)->code_linear_p ? (int) ((to) - (temp_charset_work->current)->min_code) : (((temp_charset_work->current)->code_space_mask[(to) >> 24] & 0x8) && ((temp_charset_work->current)->code_space_mask[((to) >> 16) & 0xFF] & 0x4) && ((temp_charset_work->current)->code_space_mask[((to) >> 8) & 0xFF] & 0x2) && ((temp_charset_work->current)->code_space_mask[(to) & 0xFF] & 0x1)) ? (int) (((((to) >> 24) - (temp_charset_work->current)->code_space[12]) * (temp_charset_work->current)->code_space[11]) + (((((to) >> 16) & 0xFF) - (temp_charset_work->current)->code_space[8]) * (temp_charset_work->current)->code_space[7]) + (((((to) >> 8) & 0xFF) - (temp_charset_work->current)->code_space[4]) * (temp_charset_work->current)->code_space[3]) + (((to) & 0xFF) - (temp_charset_work->current)->code_space[0]) - ((temp_charset_work->current)->char_index_offset)) : -1);
  Lisp_Object range = Fcons (builtin_lisp_symbol (0), builtin_lisp_symbol (0));
  int c, stop;
  ...
  while (1)
    {
      int idx = ((c) == temp_charset_work->zero_index_char ? 0 : (c) < 0x20000 ? (temp_charset_work->table.encoder[(c)] ? (int) temp_charset_work->table.encoder[(c)] : -1) : temp_charset_work->table.encoder[(c) - 0x10000] ? temp_charset_work->table.encoder[(c) - 0x10000] : -1);

      if (idx >= from_idx && idx <= to_idx)
 {
   if ((((((void) (0 && ((((enum Lisp_Type) ((range) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((range) - (Lisp_Cons))))->car) == (builtin_lisp_symbol (0))))
     XSETCAR (range, ((EMACS_INT) (((EMACS_UINT) (c) << INTTYPEBITS) + Lisp_Int0)));
 }
      else if (! (((((void) (0 && ((((enum Lisp_Type) ((range) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((range) - (Lisp_Cons))))->car) == (builtin_lisp_symbol (0))))
 {
   XSETCDR (range, ((EMACS_INT) (((EMACS_UINT) (c - 1) << INTTYPEBITS) + Lisp_Int0)));
   if (c_function)
     (*c_function) (arg, range);
   else
     call2 (function, range, arg);
   XSETCAR (range, builtin_lisp_symbol (0));
 }
      if (c == stop)
 {
   if (c == temp_charset_work->max_char)
     {
       if (! (((((void) (0 && ((((enum Lisp_Type) ((range) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((range) - (Lisp_Cons))))->car) == (builtin_lisp_symbol (0))))
  {
    XSETCDR (range, ((EMACS_INT) (((EMACS_UINT) (c) << INTTYPEBITS) + Lisp_Int0)));
    if (c_function)
      (*c_function) (arg, range);
    else
      call2 (function, range, arg);
...
{
  Lisp_Object range;
  _Bool partial = (from > ((charset)->min_code)
    || to < ((charset)->max_code));

  if (((charset)->method) == CHARSET_METHOD_OFFSET)
    {
      int from_idx = ((charset)->code_linear_p ? (int) ((from) - (charset)->min_code) : (((charset)->code_space_mask[(from) >> 24] & 0x8) && ((charset)->code_space_mask[((from) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((from) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(from) & 0xFF] & 0x1)) ? (int) (((((from) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((from) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((from) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((from) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1);
      int to_idx = ((charset)->code_linear_p ? (int) ((to) - (charset)->min_code) : (((charset)->code_space_mask[(to) >> 24] & 0x8) && ((charset)->code_space_mask[((to) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((to) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(to) & 0xFF] & 0x1)) ? (int) (((((to) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((to) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((to) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((to) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1);
      int from_c = from_idx + ((charset)->code_offset);
      int to_c = to_idx + ((charset)->code_offset);

      if (((charset)->unified_p))
 {
   if (! CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_deunifier))))
     load_charset (charset, 2);
   if (CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_deunifier))))
     map_char_table_for_charset (c_function, function,
     (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_deunifier)), arg,
     partial ? charset : ((void*)0), from, to);
   else
     map_charset_for_dump (c_function, function, arg, from, to);
 }

      range = Fcons (((EMACS_INT) (((EMACS_UINT) (from_c) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (to_c) << INTTYPEBITS) + Lisp_Int0)));
      if (((function) == (builtin_lisp_symbol (0))))
 (*c_function) (arg, range);
      else
 call2 (function, range, arg);
    }
  else if (((charset)->method) == CHARSET_METHOD_MAP)
    {
      if (! CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder))))
 load_charset (charset, 2);
      if (CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder))))
 map_char_table_for_charset (c_function, function,
        (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder)), arg,
        partial ? charset : ((void*)0), from, to);
      else
 map_charset_for_dump (c_function, function, arg, from, to);
    }
  else if (((charset)->method) == CHARSET_METHOD_SUBSET)
    {
      Lisp_Object subset_info;
      int offset;

      subset_info = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_subset));
      charset = (charset_table + (((AREF (subset_info, 0)) >> INTTYPEBITS)));
      offset = ((AREF (subset_info, 3)) >> INTTYPEBITS);
      from -= offset;
      if (from < ((AREF (subset_info, 1)) >> INTTYPEBITS))
 from = ((AREF (subset_info, 1)) >> INTTYPEBITS);
      to -= offset;
      if (to > ((AREF (subset_info, 2)) >> INTTYPEBITS))
 to = ((AREF (subset_info, 2)) >> INTTYPEBITS);
  ...
  else
    {
      ...
      for (parents = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_superset)); (((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons);
    parents = (((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->u.cdr)
 {
   int offset;
   unsigned this_from, this_to;

   charset = (charset_table + ((((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->car) >> INTTYPEBITS)));
   offset = (((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS);
   this_from = from > offset ? from - offset : 0;
   this_to = to > offset ? to - offset : 0;
   if (this_from < ((charset)->min_code))
     this_from = ((charset)->min_code);
   if (this_to > ((charset)->max_code))
     this_to = ((charset)->max_code);
...
static struct Lisp_Subr _Alignas (8) Smap_charset_chars = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a5 = Fmap_charset_chars }, 2, 5, "map-charset-chars", 0, 0}; Lisp_Object Fmap_charset_chars
# 794 "charset.c"
  (Lisp_Object function, Lisp_Object charset, Lisp_Object arg, Lisp_Object from_code, Lisp_Object to_code)
{
  ...
  do { int csid; do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), charset, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (charset)); csid = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0); cs = (charset_table + (csid)); } while (0);
  if (((from_code) == (builtin_lisp_symbol (0))))
    from = ((cs)->min_code);
  else
    {
      from = ((from_code) >> INTTYPEBITS);
      if (from < ((cs)->min_code))
 from = ((cs)->min_code);
    }
  if (((to_code) == (builtin_lisp_symbol (0))))
    to = ((cs)->max_code);
  else
    {
      to = ((to_code) >> INTTYPEBITS);
      if (to > ((cs)->max_code))
 to = ((cs)->max_code);
    }
  map_charset_chars (((void*)0), function, arg, cs, from, to);
...
static struct Lisp_Subr _Alignas (8) Sdefine_charset_internal = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .aMANY = Fdefine_charset_internal }, charset_arg_max, MANY, "define-charset-internal", 0, 0}; Lisp_Object Fdefine_charset_internal
  ...
  (ptrdiff_t nargs, Lisp_Object *args)
{
  ...
  attrs = Fmake_vector (((EMACS_INT) (((EMACS_UINT) (charset_attr_max) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (0));

  (((((enum Lisp_Type) ((args[charset_arg_name]) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (897), args[charset_arg_name]));
  ...
  for (i = 0, dimension = 0, nchars = 1; ; i++)
    {
      ...
      min_byte_obj = Faref (val, ((EMACS_INT) (((EMACS_UINT) (i * 2) << INTTYPEBITS) + Lisp_Int0)));
      max_byte_obj = Faref (val, ((EMACS_INT) (((EMACS_UINT) (i * 2 + 1) << INTTYPEBITS) + Lisp_Int0)));
      do { ((((((enum Lisp_Type) ((min_byte_obj) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), min_byte_obj)); if (! ((0) <= ((min_byte_obj) >> INTTYPEBITS) && ((min_byte_obj) >> INTTYPEBITS) <= (255))) args_out_of_range_3 (min_byte_obj, ((EMACS_INT) (((EMACS_UINT) ((0) < 0 && (0) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (0)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((255) < ((9223372036854775807L >> INTTYPEBITS)) ? (255) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0);
      min_byte = ((min_byte_obj) >> INTTYPEBITS);
      do { ((((((enum Lisp_Type) ((max_byte_obj) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), max_byte_obj)); if (! ((min_byte) <= ((max_byte_obj) >> INTTYPEBITS) && ((max_byte_obj) >> INTTYPEBITS) <= (255))) args_out_of_range_3 (max_byte_obj, ((EMACS_INT) (((EMACS_UINT) ((min_byte) < 0 && (min_byte) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (min_byte)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((255) < ((9223372036854775807L >> INTTYPEBITS)) ? (255) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0);
      max_byte = ((max_byte_obj) >> INTTYPEBITS);
 ...
 break;
  ...
  if (((val) == (builtin_lisp_symbol (0))))
    ...
    {
      do { ((((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), val)); if (! ((1) <= ((val) >> INTTYPEBITS) && ((val) >> INTTYPEBITS) <= (4))) args_out_of_range_3 (val, ((EMACS_INT) (((EMACS_UINT) ((1) < 0 && (1) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (1)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((4) < ((9223372036854775807L >> INTTYPEBITS)) ? (4) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0);
      charset.dimension = ((val) >> INTTYPEBITS);
    }
 ...
 for (j = charset.code_space[i * 4]; j <= charset.code_space[i * 4 + 1];
  ...
  if (! ((val) == (builtin_lisp_symbol (0))))
    {
      unsigned code = cons_to_unsigned (val, (2147483647 *2U +1U));

      if (code < charset.min_code
   || code > charset.max_code)
 args_out_of_range_3 (((! ((0 <= (charset.min_code) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (charset.min_code)) && (charset.min_code) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (charset.min_code) : ((EMACS_INT) (((EMACS_UINT) (charset.min_code) << INTTYPEBITS) + Lisp_Int0))),
        ((! ((0 <= (charset.max_code) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (charset.max_code)) && (charset.max_code) <= (9223372036854775807L >> INTTYPEBITS))) ? make_float (charset.max_code) : ((EMACS_INT) (((EMACS_UINT) (charset.max_code) << INTTYPEBITS) + Lisp_Int0))), val);
      charset.char_index_offset = ((&charset)->code_linear_p ? (int) ((code) - (&charset)->min_code) : (((&charset)->code_space_mask[(code) >> 24] & 0x8) && ((&charset)->code_space_mask[((code) >> 16) & 0xFF] & 0x4) && ((&charset)->code_space_mask[((code) >> 8) & 0xFF] & 0x2) && ((&charset)->code_space_mask[(code) & 0xFF] & 0x1)) ? (int) (((((code) >> 24) - (&charset)->code_space[12]) * (&charset)->code_space[11]) + (((((code) >> 16) & 0xFF) - (&charset)->code_space[8]) * (&charset)->code_space[7]) + (((((code) >> 8) & 0xFF) - (&charset)->code_space[4]) * (&charset)->code_space[3]) + (((code) & 0xFF) - (&charset)->code_space[0]) - ((&charset)->char_index_offset)) : -1);
 ...
 {
   if (charset.max_code < (2147483647 *2U +1U))
    ...
    charset.invalid_code = cons_to_unsigned (val, (2147483647 *2U +1U));
  ...
  if (((val) == (builtin_lisp_symbol (0))))
    ...
    {
      ((((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), val));
      if (((val) >> INTTYPEBITS) < '0' || ((val) >> INTTYPEBITS) > 127)
 error ("Invalid iso-final-char: %""l""d", ((val) >> INTTYPEBITS));
      charset.iso_final = ((val) >> INTTYPEBITS);
    }
  ...
  if (((val) == (builtin_lisp_symbol (0))))
    ...
    {
      do { ((((((enum Lisp_Type) ((val) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), val)); if (! ((-1) <= ((val) >> INTTYPEBITS) && ((val) >> INTTYPEBITS) <= (63))) args_out_of_range_3 (val, ((EMACS_INT) (((EMACS_UINT) ((-1) < 0 && (-1) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (-1)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((63) < ((9223372036854775807L >> INTTYPEBITS)) ? (63) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0);
      charset.iso_revision = ((val) >> INTTYPEBITS);
    }
  ...
  if (((val) == (builtin_lisp_symbol (0))))
    ...
    {
      CHECK_NATNUM (val);
      if ((((val) >> INTTYPEBITS) > 0 && ((val) >> INTTYPEBITS) <= 128) || ((val) >> INTTYPEBITS) >= 256)
 error ("Invalid emacs-mule-id: %""l""d", ((val) >> INTTYPEBITS));
      charset.emacs_mule_id = ((val) >> INTTYPEBITS);
    }

  charset.ascii_compatible_p = ! ((args[charset_arg_ascii_compatible_p]) == (builtin_lisp_symbol (0)));

  charset.supplementary_p = ! ((args[charset_arg_supplementary_p]) == (builtin_lisp_symbol (0)));

  charset.unified_p = 0;

  memset (charset.fast_map, 0, sizeof (charset.fast_map));

  if (! ((args[charset_arg_code_offset]) == (builtin_lisp_symbol (0))))
    {
      ...
      charset.code_offset = ((val) >> INTTYPEBITS);

      i = ((&charset)->code_linear_p ? (int) ((charset.max_code) - (&charset)->min_code) : (((&charset)->code_space_mask[(charset.max_code) >> 24] & 0x8) && ((&charset)->code_space_mask[((charset.max_code) >> 16) & 0xFF] & 0x4) && ((&charset)->code_space_mask[((charset.max_code) >> 8) & 0xFF] & 0x2) && ((&charset)->code_space_mask[(charset.max_code) & 0xFF] & 0x1)) ? (int) (((((charset.max_code) >> 24) - (&charset)->code_space[12]) * (&charset)->code_space[11]) + (((((charset.max_code) >> 16) & 0xFF) - (&charset)->code_space[8]) * (&charset)->code_space[7]) + (((((charset.max_code) >> 8) & 0xFF) - (&charset)->code_space[4]) * (&charset)->code_space[3]) + (((charset.max_code) & 0xFF) - (&charset)->code_space[0]) - ((&charset)->char_index_offset)) : -1);
      if (0x3FFFFF - charset.code_offset < i)
 error ("Unsupported max char: %d", charset.max_char);
      charset.max_char = i + charset.code_offset;
      i = ((&charset)->code_linear_p ? (int) ((charset.min_code) - (&charset)->min_code) : (((&charset)->code_space_mask[(charset.min_code) >> 24] & 0x8) && ((&charset)->code_space_mask[((charset.min_code) >> 16) & 0xFF] & 0x4) && ((&charset)->code_space_mask[((charset.min_code) >> 8) & 0xFF] & 0x2) && ((&charset)->code_space_mask[(charset.min_code) & 0xFF] & 0x1)) ? (int) (((((charset.min_code) >> 24) - (&charset)->code_space[12]) * (&charset)->code_space[11]) + (((((charset.min_code) >> 16) & 0xFF) - (&charset)->code_space[8]) * (&charset)->code_space[7]) + (((((charset.min_code) >> 8) & 0xFF) - (&charset)->code_space[4]) * (&charset)->code_space[3]) + (((charset.min_code) & 0xFF) - (&charset)->code_space[0]) - ((&charset)->char_index_offset)) : -1);
      charset.min_char = i + charset.code_offset;

      i = (charset.min_char >> 7) << 7;
      for (; i < 0x10000 && i <= charset.max_char; i += 128)
 do { if ((i) < 0x10000) (charset.fast_map)[(i) >> 10] |= 1 << (((i) >> 7) & 7); else (charset.fast_map)[((i) >> 15) + 62] |= 1 << (((i) >> 12) & 7); } while (0);
      i = (i >> 12) << 12;
 ...
 charset.ascii_compatible_p = 1;
  ...
  else if (! ((args[charset_arg_map]) == (builtin_lisp_symbol (0))))
    {
      val = args[charset_arg_map];
      ASET (attrs, charset_map, val);
      charset.method = CHARSET_METHOD_MAP;
    }
  else if (! ((args[charset_arg_subset]) == (builtin_lisp_symbol (0))))
    {
      Lisp_Object parent;
      ...
      parent = Fcar (val);
      do { int csid; do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((parent) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), parent, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (parent)); csid = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0); parent_charset = (charset_table + (csid)); } while (0);
      parent_min_code = Fnth (((EMACS_INT) (((EMACS_UINT) (1) << INTTYPEBITS) + Lisp_Int0)), val);
      CHECK_NATNUM (parent_min_code);
      parent_max_code = Fnth (((EMACS_INT) (((EMACS_UINT) (2) << INTTYPEBITS) + Lisp_Int0)), val);
      CHECK_NATNUM (parent_max_code);
      parent_code_offset = Fnth (((EMACS_INT) (((EMACS_UINT) (3) << INTTYPEBITS) + Lisp_Int0)), val);
      ((((((enum Lisp_Type) ((parent_code_offset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), parent_code_offset));
      val = make_uninit_vector (4);
      ASET (val, 0, ((EMACS_INT) (((EMACS_UINT) (parent_charset->id) << INTTYPEBITS) + Lisp_Int0)));
 ...
 charset.fast_map[i] = parent_charset->fast_map[i];
  ...
  else if (! ((args[charset_arg_superset]) == (builtin_lisp_symbol (0))))
    {
      ...
      charset.min_char = 0x3FFFFF;
      charset.max_char = 0;
      for (; ! ((val) == (builtin_lisp_symbol (0))); val = Fcdr (val))
 {
   ...
   if ((((enum Lisp_Type) ((elt) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons))
     {
       car_part = (((void) (0 && ((((enum Lisp_Type) ((elt) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((elt) - (Lisp_Cons))))->car;
       cdr_part = (((void) (0 && ((((enum Lisp_Type) ((elt) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((elt) - (Lisp_Cons))))->u.cdr;
       do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((car_part) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), car_part, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (car_part)); this_id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
       do { if ((! ((int) 0 < (int) -1))) do { ((((((enum Lisp_Type) ((cdr_part) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), cdr_part)); if (! ((((int) (! (! ((int) 0 < (int) -1)) ? (int) 0 : ((int) ~ (int) 0 < (int) -1) ? ~ (int) 0 : ~ ((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))))) <= ((cdr_part) >> INTTYPEBITS) && ((cdr_part) >> INTTYPEBITS) <= (((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))))) args_out_of_range_3 (cdr_part, ((EMACS_INT) (((EMACS_UINT) ((((int) (! (! ((int) 0 < (int) -1)) ? (int) 0 : ((int) ~ (int) 0 < (int) -1) ? ~ (int) 0 : ~ ((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))))) < 0 && (((int) (! (! ((int) 0 < (int) -1)) ? (int) 0 : ((int) ~ (int) 0 < (int) -1) ? ~ (int) 0 : ~ ((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))))) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (((int) (! (! ((int) 0 < (int) -1)) ? (int) 0 : ((int) ~ (int) 0 < (int) -1) ? ~ (int) 0 : ~ ((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1))))))) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))) < ((9223372036854775807L >> INTTYPEBITS)) ? (((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0); else do { ((((((enum Lisp_Type) ((cdr_part) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), cdr_part)); if (! ((0) <= ((cdr_part) >> INTTYPEBITS) && ((cdr_part) >> INTTYPEBITS) <= (((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))))) args_out_of_range_3 (cdr_part, ((EMACS_INT) (((EMACS_UINT) ((0) < 0 && (0) < (-1 - (9223372036854775807L >> INTTYPEBITS)) ? (-1 - (9223372036854775807L >> INTTYPEBITS)) : (0)) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (((((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))) < ((9223372036854775807L >> INTTYPEBITS)) ? (((int) (! (! ((int) 0 < (int) -1)) ? (int) -1 : ((((int) 1 << (sizeof (int) * 8 - 2)) - 1) * 2 + 1)))) : ((9223372036854775807L >> INTTYPEBITS)))) << INTTYPEBITS) + Lisp_Int0))); } while (0); } while (0);
       offset = ((cdr_part) >> INTTYPEBITS);
     }
   else
     {
       do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((elt) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), elt, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (elt)); this_id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
       offset = 0;
     }
   XSETCAR (val, Fcons (((EMACS_INT) (((EMACS_UINT) (this_id) << INTTYPEBITS) + Lisp_Int0)), ((EMACS_INT) (((EMACS_UINT) (offset) << INTTYPEBITS) + Lisp_Int0))));

   this_charset = (charset_table + (this_id));
  ...
  if (! ((val) == (builtin_lisp_symbol (0))) && !STRINGP (val))
    CHECK_VECTOR (val);
  ...
  if (charset.hash_index >= 0)
    {
      new_definition_p = 0;
      id = ((AREF ((Fgethash ((args[charset_arg_name]), Vcharset_hash_table, builtin_lisp_symbol (0))), charset_id)) >> INTTYPEBITS);
      ...
      if (charset_table_used == charset_table_size)
 {
   ...
   struct charset *new_table =
     xpalloc (0, &new_size, 1,
       ((2147483647) < ((9223372036854775807L >> INTTYPEBITS)) ? (2147483647) : ((9223372036854775807L >> INTTYPEBITS))),
  ...
  ASET (attrs, charset_id, ((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)));
  charset.id = id;
  ...
  if (charset.iso_final >= 0)
    {
      iso_charset_table[(charset.dimension) - 1][charset.iso_chars_96][charset.iso_final] = id;

      if (new_definition_p)
 Viso_2022_charset_list = nconc2 (Viso_2022_charset_list,
      list1 (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0))));
      if (iso_charset_table[(1) - 1][0]['J'] == id)
 charset_jisx0201_roman = id;
      else if (iso_charset_table[(2) - 1][0]['@'] == id)
 charset_jisx0208_1978 = id;
      else if (iso_charset_table[(2) - 1][0]['B'] == id)
 charset_jisx0208 = id;
      else if (iso_charset_table[(2) - 1][0]['C'] == id)
 charset_ksc5601 = id;
    }
 ...
 Vemacs_mule_charset_list = nconc2 (Vemacs_mule_charset_list,
  ...
  if (new_definition_p)
    {
      globals.f_Vcharset_list = Fcons (args[charset_arg_name], globals.f_Vcharset_list);
      ...
      else
 {
   ...
   for (tail = Vcharset_ordered_list; (((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); tail = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr)
     {
       struct charset *cs = (charset_table + ((((((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->car) >> INTTYPEBITS)));

       if (cs->supplementary_p)
  break;
     }
   if (((tail) == (Vcharset_ordered_list)))
     Vcharset_ordered_list = Fcons (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)),
        Vcharset_ordered_list);
   else if (((tail) == (builtin_lisp_symbol (0))))
     Vcharset_ordered_list = nconc2 (Vcharset_ordered_list,
         list1 (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0))));
   else
     {
       val = Fcons ((((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->car, (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr);
       XSETCDR (tail, val);
       XSETCAR (tail, ((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)));
     }
 }
...
define_charset_internal (Lisp_Object name,
    ...
    _Bool ascii_compatible, _Bool supplementary,
    int code_offset)
{
  ...
  args[charset_arg_dimension] = ((EMACS_INT) (((EMACS_UINT) (dimension) << INTTYPEBITS) + Lisp_Int0));
  val = make_uninit_vector (8);
  for (i = 0; i < 8; i++)
    ASET (val, i, ((EMACS_INT) (((EMACS_UINT) (code_space[i]) << INTTYPEBITS) + Lisp_Int0)));
  args[charset_arg_code_space] = val;
  args[charset_arg_min_code] = ((EMACS_INT) (((EMACS_UINT) (min_code) << INTTYPEBITS) + Lisp_Int0));
  args[charset_arg_max_code] = ((EMACS_INT) (((EMACS_UINT) (max_code) << INTTYPEBITS) + Lisp_Int0));
  args[charset_arg_iso_final]
    = (iso_final < 0 ? builtin_lisp_symbol (0) : ((EMACS_INT) (((EMACS_UINT) (iso_final) << INTTYPEBITS) + Lisp_Int0)));
  args[charset_arg_iso_revision] = ((EMACS_INT) (((EMACS_UINT) (iso_revision) << INTTYPEBITS) + Lisp_Int0));
  args[charset_arg_emacs_mule_id]
    = (emacs_mule_id < 0 ? builtin_lisp_symbol (0) : ((EMACS_INT) (((EMACS_UINT) (emacs_mule_id) << INTTYPEBITS) + Lisp_Int0)));
  args[charset_arg_ascii_compatible_p] = ascii_compatible ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0);
  args[charset_arg_supplementary_p] = supplementary ? builtin_lisp_symbol (901) : builtin_lisp_symbol (0);
  args[charset_arg_invalid_code] = builtin_lisp_symbol (0);
  args[charset_arg_code_offset] = ((EMACS_INT) (((EMACS_UINT) (code_offset) << INTTYPEBITS) + Lisp_Int0));
  args[charset_arg_map] = builtin_lisp_symbol (0);
  args[charset_arg_subset] = builtin_lisp_symbol (0);
  args[charset_arg_superset] = builtin_lisp_symbol (0);
  args[charset_arg_unify_map] = builtin_lisp_symbol (0);
  ...
  return ((AREF ((Fgethash ((name), Vcharset_hash_table, builtin_lisp_symbol (0))), charset_id)) >> INTTYPEBITS);
}


static struct Lisp_Subr _Alignas (8) Sdefine_charset_alias = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fdefine_charset_alias }, 2, 2, "define-charset-alias", 0, 0}; Lisp_Object Fdefine_charset_alias
  ...
  (Lisp_Object alias, Lisp_Object charset)
{
  ...
  do { if (!(((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || ((attr = Fgethash ((charset), Vcharset_hash_table, builtin_lisp_symbol (0))) == (builtin_lisp_symbol (0)))) wrong_type_argument (builtin_lisp_symbol (262), (charset)); } while (0);
  Fputhash (alias, attr, Vcharset_hash_table);
  globals.f_Vcharset_list = Fcons (alias, globals.f_Vcharset_list);
  return builtin_lisp_symbol (0);
}


static struct Lisp_Subr _Alignas (8) Scharset_plist = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fcharset_plist }, 1, 1, "charset-plist", 0, 0}; Lisp_Object Fcharset_plist

  (Lisp_Object charset)
{
  Lisp_Object attrs;

  do { if (!(((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || ((attrs = Fgethash ((charset), Vcharset_hash_table, builtin_lisp_symbol (0))) == (builtin_lisp_symbol (0)))) wrong_type_argument (builtin_lisp_symbol (262), (charset)); } while (0);
  return AREF ((attrs), charset_plist);
}


static struct Lisp_Subr _Alignas (8) Sset_charset_plist = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fset_charset_plist }, 2, 2, "set-charset-plist", 0, 0}; Lisp_Object Fset_charset_plist
...
static struct Lisp_Subr _Alignas (8) Sunify_charset = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Funify_charset }, 1, 3, "unify-charset", 0, 0}; Lisp_Object Funify_charset
# 1337 "charset.c"
  (Lisp_Object charset, Lisp_Object unify_map, Lisp_Object deunify)
{
  ...
  do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), charset, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (charset)); id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
  cs = (charset_table + (id));
  if (((deunify) == (builtin_lisp_symbol (0)))
      ? ((cs)->unified_p) && ! (((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_deunifier))) == (builtin_lisp_symbol (0)))
      : ! ((cs)->unified_p))
    return builtin_lisp_symbol (0);
  ...
  if (((deunify) == (builtin_lisp_symbol (0))))
    {
      if (((cs)->method) != CHARSET_METHOD_OFFSET
   || ((cs)->code_offset) < 0x110000)
 error ("Can't unify charset: %s", SDATA (SYMBOL_NAME (charset)));
      if (((unify_map) == (builtin_lisp_symbol (0))))
 unify_map = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_unify_map));
      else
 {
   ...
   set_charset_attr (cs, charset_unify_map, unify_map);
      ...
      if (((Vchar_unify_table) == (builtin_lisp_symbol (0))))
 Vchar_unify_table = Fmake_char_table (builtin_lisp_symbol (0), builtin_lisp_symbol (0));
      char_table_set_range (Vchar_unify_table,
    ...
    {
      unsigned min_code = ((cs)->min_code);
      unsigned max_code = ((cs)->max_code);
      int min_char = (((((sizeof ((min_code) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((min_code) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((min_code) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((min_code) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (cs)->ascii_compatible_p) ? (min_code) : ((min_code) < (cs)->min_code || (min_code) > (cs)->max_code) ? -1 : (cs)->unified_p ? decode_char ((cs), (min_code)) : (cs)->method == CHARSET_METHOD_OFFSET ? ((cs)->code_linear_p ? (int) ((min_code) - (cs)->min_code) + (cs)->code_offset : decode_char ((cs), (min_code))) : (cs)->method == CHARSET_METHOD_MAP ? (((cs)->code_linear_p && VECTORP ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_decoder)))) ? ((AREF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_decoder)), (min_code) - (cs)->min_code)) >> INTTYPEBITS) : decode_char ((cs), (min_code))) : decode_char ((cs), (min_code)));
      int max_char = (((((sizeof ((max_code) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((max_code) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((max_code) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((max_code) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (cs)->ascii_compatible_p) ? (max_code) : ((max_code) < (cs)->min_code || (max_code) > (cs)->max_code) ? -1 : (cs)->unified_p ? decode_char ((cs), (max_code)) : (cs)->method == CHARSET_METHOD_OFFSET ? ((cs)->code_linear_p ? (int) ((max_code) - (cs)->min_code) + (cs)->code_offset : decode_char ((cs), (max_code))) : (cs)->method == CHARSET_METHOD_MAP ? (((cs)->code_linear_p && VECTORP ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_decoder)))) ? ((AREF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (cs)->hash_index))), charset_decoder)), (max_code) - (cs)->min_code)) >> INTTYPEBITS) : decode_char ((cs), (max_code))) : decode_char ((cs), (max_code)));

      char_table_set_range (Vchar_unify_table, min_char, max_char, builtin_lisp_symbol (0));
    }
...
{
  ((((((enum Lisp_Type) ((dimension) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), dimension));
  ((((((enum Lisp_Type) ((chars) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (559), chars));
  (((NATNUMP (final_char) && ((final_char) >> INTTYPEBITS) <= 0x3FFFFF)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (260), final_char));

  if (! (1 <= ((dimension) >> INTTYPEBITS) && ((dimension) >> INTTYPEBITS) <= 3))
    error ("Invalid DIMENSION %""l""d, it should be 1, 2, or 3",
    ((dimension) >> INTTYPEBITS));

  _Bool chars_flag = ((chars) >> INTTYPEBITS) == 96;
  if (! (chars_flag || ((chars) >> INTTYPEBITS) == 94))
    error ("Invalid CHARS %""l""d, it should be 94 or 96", ((chars) >> INTTYPEBITS));

  int final_ch = ((final_char) >> INTTYPEBITS);
...
static struct Lisp_Subr _Alignas (8) Sget_unused_iso_final_char = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fget_unused_iso_final_char }, 2, 2, "get-unused-iso-final-char", 0, 0}; Lisp_Object Fget_unused_iso_final_char
# 1418 "charset.c"
  (Lisp_Object dimension, Lisp_Object chars)
{
  _Bool chars_flag = check_iso_charset_parameter (dimension, chars,
       ((EMACS_INT) (((EMACS_UINT) ('0') << INTTYPEBITS) + Lisp_Int0)));
  for (int final_char = '0'; final_char <= '?'; final_char++)
    if (iso_charset_table[(((dimension) >> INTTYPEBITS)) - 1][chars_flag][final_char] < 0)
      return ((EMACS_INT) (((EMACS_UINT) (final_char) << INTTYPEBITS) + Lisp_Int0));
  return builtin_lisp_symbol (0);
}


static struct Lisp_Subr _Alignas (8) Sdeclare_equiv_charset = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a4 = Fdeclare_equiv_charset }, 4, 4, "declare-equiv-charset", 0, 0}; Lisp_Object Fdeclare_equiv_charset
  ...
  (Lisp_Object dimension, Lisp_Object chars, Lisp_Object final_char, Lisp_Object charset)
{
  ...
  do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), charset, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (charset)); id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
  _Bool chars_flag = check_iso_charset_parameter (dimension, chars, final_char);
  iso_charset_table[(((dimension) >> INTTYPEBITS)) - 1][chars_flag][((final_char) >> INTTYPEBITS)] = id;
  return builtin_lisp_symbol (0);
}
# 1460 "charset.c"
int
string_xstring_p (Lisp_Object string)
{
  ...
  while (p < endp)
    {
      int c = (!((p)[0] & 0x80) ? *(p)++ : ! ((p)[0] & 0x20) ? ((p) += 2, ((((p)[-2] & 0x1F) << 6) | ((p)[-1] & 0x3F) | ((unsigned char) ((p)[-2]) < 0xC2 ? 0x3FFF80 : 0))) : ! ((p)[0] & 0x10) ? ((p) += 3, ((((p)[-3] & 0x0F) << 12) | (((p)[-2] & 0x3F) << 6) | ((p)[-1] & 0x3F))) : string_char ((p), &(p), ((void*)0)));
...
find_charsets_in_text (const unsigned char *ptr, ptrdiff_t nchars,
         ...
         Lisp_Object table, _Bool multibyte)
{
  ...
  if (nchars == nbytes)
    {
      if (multibyte)
 ASET (charsets, charset_ascii, builtin_lisp_symbol (901));
      else
 while (ptr < pend)
   {
     int c = *ptr++;

     if (!((table) == (builtin_lisp_symbol (0))))
     ...
     if ((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)))
       ...
       ASET (charsets, charset_eight_bit, builtin_lisp_symbol (901));
   }
    }
  else
    {
      while (ptr < pend)
 {
   int c = (!((ptr)[0] & 0x80) ? *(ptr)++ : ! ((ptr)[0] & 0x20) ? ((ptr) += 2, ((((ptr)[-2] & 0x1F) << 6) | ((ptr)[-1] & 0x3F) | ((unsigned char) ((ptr)[-2]) < 0xC2 ? 0x3FFF80 : 0))) : ! ((ptr)[0] & 0x10) ? ((ptr) += 3, ((((ptr)[-3] & 0x0F) << 12) | (((ptr)[-2] & 0x3F) << 6) | ((ptr)[-1] & 0x3F))) : string_char ((ptr), &(ptr), ((void*)0)));
   struct charset *charset;

   if (!((table) == (builtin_lisp_symbol (0))))
     c = translate_char (table, c);
   charset = ((c) < 0x80 ? (charset_table + (charset_ascii)) : char_charset ((c), builtin_lisp_symbol (0), ((void*)0)));
   ASET (charsets, ((charset)->id), builtin_lisp_symbol (901));
 }
    }
}

static struct Lisp_Subr _Alignas (8) Sfind_charset_region = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Ffind_charset_region }, 2, 3, "find-charset-region", 0, 0}; Lisp_Object Ffind_charset_region
  ...
  (Lisp_Object beg, Lisp_Object end, Lisp_Object table)
{
  ...
  from = ((beg) >> INTTYPEBITS);
  stop = to = ((end) >> INTTYPEBITS);

  if (from < (current_buffer->text->gpt) && (current_buffer->text->gpt) < to)
    {
      stop = (current_buffer->text->gpt);
      stop_byte = (current_buffer->text->gpt_byte);
    }
  else
    stop_byte = (buf_charpos_to_bytepos (current_buffer, stop));

  from_byte = (buf_charpos_to_bytepos (current_buffer, from));

  charsets = Fmake_vector (((EMACS_INT) (((EMACS_UINT) (charset_table_used) << INTTYPEBITS) + Lisp_Int0)), builtin_lisp_symbol (0));
  while (1)
    {
      find_charsets_in_text ((((from_byte) >= (current_buffer->text->gpt_byte) ? (current_buffer->text->gap_size) : 0) + (from_byte) + (current_buffer->text->beg) - ((1))), stop - from,
        stop_byte - from_byte, charsets, table,
        multibyte);
      if (stop < to)
 {
   from = stop, from_byte = stop_byte;
   stop = to, stop_byte = (buf_charpos_to_bytepos (current_buffer, stop));
 }
      else
 break;
    }
  ...
  for (i = charset_table_used - 1; i >= 0; i--)
    if (!((AREF (charsets, i)) == (builtin_lisp_symbol (0))))
      val = Fcons ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset_table + i)->hash_index))), charset_name)), val);
  return val;
}

static struct Lisp_Subr _Alignas (8) Sfind_charset_string = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Ffind_charset_string }, 1, 2, "find-charset-string", 0, 0}; Lisp_Object Ffind_charset_string
...
maybe_unify_char (int c, Lisp_Object val)
{
 ...
 c = ((val) >> INTTYPEBITS);
    ...
    {
      int code_index = c - ((charset)->code_offset);
      int unified = (temp_charset_work->table.decoder[(code_index)]);
...
{
  int c, char_index;
  enum charset_method method = ((charset)->method);
  ...
  if (method == CHARSET_METHOD_SUBSET)
    {
      ...
      subset_info = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_subset));
      charset = (charset_table + (((AREF (subset_info, 0)) >> INTTYPEBITS)));
      code -= ((AREF (subset_info, 3)) >> INTTYPEBITS);
      if (code < ((AREF (subset_info, 1)) >> INTTYPEBITS)
   || code > ((AREF (subset_info, 2)) >> INTTYPEBITS))
 c = -1;
      else
 c = (((((sizeof ((code) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((code) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((code) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((code) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (charset)->ascii_compatible_p) ? (code) : ((code) < (charset)->min_code || (code) > (charset)->max_code) ? -1 : (charset)->unified_p ? decode_char ((charset), (code)) : (charset)->method == CHARSET_METHOD_OFFSET ? ((charset)->code_linear_p ? (int) ((code) - (charset)->min_code) + (charset)->code_offset : decode_char ((charset), (code))) : (charset)->method == CHARSET_METHOD_MAP ? (((charset)->code_linear_p && VECTORP ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder)))) ? ((AREF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder)), (code) - (charset)->min_code)) >> INTTYPEBITS) : decode_char ((charset), (code))) : decode_char ((charset), (code)));
    }
  else if (method == CHARSET_METHOD_SUPERSET)
    {
      Lisp_Object parents;

      parents = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_superset));
      c = -1;
      for (; (((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); parents = (((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->u.cdr)
 {
   int id = (((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->car) >> INTTYPEBITS);
   int code_offset = (((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS);
   unsigned this_code = code - code_offset;

   charset = (charset_table + (id));
   if ((c = (((((sizeof ((this_code) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((this_code) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((this_code) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((this_code) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (charset)->ascii_compatible_p) ? (this_code) : ((this_code) < (charset)->min_code || (this_code) > (charset)->max_code) ? -1 : (charset)->unified_p ? decode_char ((charset), (this_code)) : (charset)->method == CHARSET_METHOD_OFFSET ? ((charset)->code_linear_p ? (int) ((this_code) - (charset)->min_code) + (charset)->code_offset : decode_char ((charset), (this_code))) : (charset)->method == CHARSET_METHOD_MAP ? (((charset)->code_linear_p && VECTORP ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder)))) ? ((AREF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder)), (this_code) - (charset)->min_code)) >> INTTYPEBITS) : decode_char ((charset), (this_code))) : decode_char ((charset), (this_code)))) >= 0)
     break;
 }
    }
  else
    {
      char_index = ((charset)->code_linear_p ? (int) ((code) - (charset)->min_code) : (((charset)->code_space_mask[(code) >> 24] & 0x8) && ((charset)->code_space_mask[((code) >> 16) & 0xFF] & 0x4) && ((charset)->code_space_mask[((code) >> 8) & 0xFF] & 0x2) && ((charset)->code_space_mask[(code) & 0xFF] & 0x1)) ? (int) (((((code) >> 24) - (charset)->code_space[12]) * (charset)->code_space[11]) + (((((code) >> 16) & 0xFF) - (charset)->code_space[8]) * (charset)->code_space[7]) + (((((code) >> 8) & 0xFF) - (charset)->code_space[4]) * (charset)->code_space[3]) + (((code) & 0xFF) - (charset)->code_space[0]) - ((charset)->char_index_offset)) : -1);
      if (char_index < 0)
 return -1;
      ...
      if (method == CHARSET_METHOD_MAP)
 {
   ...
   decoder = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder));
   if (! VECTORP (decoder))
     {
       load_charset (charset, 1);
       decoder = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_decoder));
   ...
   if (VECTORP (decoder))
     c = ((AREF (decoder, char_index)) >> INTTYPEBITS);
   else
     c = (temp_charset_work->table.decoder[(char_index)]);
 }
      else
 {
   c = char_index + ((charset)->code_offset);
   if (((charset)->unified_p)
       && 0x10FFFF < c && c <= 0x3FFF7F)
     {
...
encode_char (struct charset *charset, int c)
{
  ...
  if (((charset)->unified_p))
    {
      ...
      deunifier = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_deunifier));
      ...
      if (CHAR_TABLE_P (deunifier))
 {
   ...
   if (((((enum Lisp_Type) ((deunified) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0))
     code_index = ((deunified) >> INTTYPEBITS);
 }
      else
 {
   code_index = ((c) == temp_charset_work->zero_index_char ? 0 : (c) < 0x20000 ? (temp_charset_work->table.encoder[(c)] ? (int) temp_charset_work->table.encoder[(c)] : -1) : temp_charset_work->table.encoder[(c) - 0x10000] ? temp_charset_work->table.encoder[(c) - 0x10000] : -1);
 }
      if (code_index >= 0)
 c = ((charset)->code_offset) + code_index;
    }
  ...
  if (method == CHARSET_METHOD_SUBSET)
    {
      ...
      subset_info = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_subset));
      this_charset = (charset_table + (((AREF (subset_info, 0)) >> INTTYPEBITS)));
      code = (((!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (c) <= sizeof (int)) ? 1 : -1; })) ? (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (this_charset)->ascii_compatible_p ? (unsigned) (c) : ((this_charset)->unified_p || (this_charset)->method == CHARSET_METHOD_SUBSET || (this_charset)->method == CHARSET_METHOD_SUPERSET) ? encode_char (this_charset, c) : (c) < (this_charset)->min_char || (c) > (this_charset)->max_char ? (this_charset)->invalid_code : (this_charset)->method == CHARSET_METHOD_OFFSET ? ((this_charset)->code_linear_p ? (unsigned) ((c) - (this_charset)->code_offset) + (this_charset)->min_code : encode_char (this_charset, c)) : (this_charset)->method == CHARSET_METHOD_MAP ? (((this_charset)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (this_charset)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (this_charset)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (this_charset)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (this_charset, c)) : encode_char (this_charset, c))) : (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (this_charset)->ascii_compatible_p ? (unsigned) (c) : ((this_charset)->unified_p || (this_charset)->method == CHARSET_METHOD_SUBSET || (this_charset)->method == CHARSET_METHOD_SUPERSET) ? encode_char (this_charset, c) : (c) < (this_charset)->min_char || (c) > (this_charset)->max_char ? (this_charset)->invalid_code : (this_charset)->method == CHARSET_METHOD_OFFSET ? ((this_charset)->code_linear_p ? (unsigned) ((c) - (this_charset)->code_offset) + (this_charset)->min_code : encode_char (this_charset, c)) : (this_charset)->method == CHARSET_METHOD_MAP ? (((this_charset)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (this_charset)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (this_charset)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (this_charset)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (this_charset, c)) : encode_char (this_charset, c)))));
      if (code == ((this_charset)->invalid_code)
   || code < ((AREF (subset_info, 1)) >> INTTYPEBITS)
   || code > ((AREF (subset_info, 2)) >> INTTYPEBITS))
      ...
      for (; (((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); parents = (((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->u.cdr)
 {
   int id = (((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->car) >> INTTYPEBITS);
   int code_offset = (((((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((parents) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((parents) - (Lisp_Cons))))->car) - (Lisp_Cons))))->u.cdr) >> INTTYPEBITS);
   struct charset *this_charset = (charset_table + (id));
  ...
  if (! (((c)) < 0x10000 ? charset->fast_map[((c)) >> 10] & (1 << ((((c)) >> 7) & 7)) : charset->fast_map[(((c)) >> 15) + 62] & (1 << ((((c)) >> 12) & 7)))
  ...
  if (method == CHARSET_METHOD_MAP)
    {
      ...
      encoder = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder));
      if (! CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder))))
 {
   load_charset (charset, 2);
   encoder = (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_encoder));
      ...
      if (CHAR_TABLE_P (encoder))
 {
   ...
   if (((val) == (builtin_lisp_symbol (0))))
     return ((charset)->invalid_code);
   code = ((val) >> INTTYPEBITS);
   if (! ((charset)->compact_codes_p))
     code = ((charset)->code_linear_p ? (code) + (charset)->min_code : (code += (charset)->char_index_offset, (((charset)->code_space[0] + (code) % (charset)->code_space[2]) | (((charset)->code_space[4] + ((code) / (charset)->code_space[3] % (charset)->code_space[6])) << 8) | (((charset)->code_space[8] + ((code) / (charset)->code_space[7] % (charset)->code_space[10])) << 16) | (((charset)->code_space[12] + ((code) / (charset)->code_space[11])) << 24))));
 }
      else
 {
   code = ((c) == temp_charset_work->zero_index_char ? 0 : (c) < 0x20000 ? (temp_charset_work->table.encoder[(c)] ? (int) temp_charset_work->table.encoder[(c)] : -1) : temp_charset_work->table.encoder[(c) - 0x10000] ? temp_charset_work->table.encoder[(c) - 0x10000] : -1);
   code = ((charset)->code_linear_p ? (code) + (charset)->min_code : (code += (charset)->char_index_offset, (((charset)->code_space[0] + (code) % (charset)->code_space[2]) | (((charset)->code_space[4] + ((code) / (charset)->code_space[3] % (charset)->code_space[6])) << 8) | (((charset)->code_space[8] + ((code) / (charset)->code_space[7] % (charset)->code_space[10])) << 16) | (((charset)->code_space[12] + ((code) / (charset)->code_space[11])) << 24))));
 }
    }
  else
    {
      unsigned code_index = c - ((charset)->code_offset);

      code = ((charset)->code_linear_p ? (code_index) + (charset)->min_code : (code_index += (charset)->char_index_offset, (((charset)->code_space[0] + (code_index) % (charset)->code_space[2]) | (((charset)->code_space[4] + ((code_index) / (charset)->code_space[3] % (charset)->code_space[6])) << 8) | (((charset)->code_space[8] + ((code_index) / (charset)->code_space[7] % (charset)->code_space[10])) << 16) | (((charset)->code_space[12] + ((code_index) / (charset)->code_space[11])) << 24))));
    }
...
static struct Lisp_Subr _Alignas (8) Sdecode_char = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Fdecode_char }, 2, 3, "decode-char", 0, 0}; Lisp_Object Fdecode_char
  ...
  (Lisp_Object charset, Lisp_Object code_point, Lisp_Object restriction)
{
  ...
  do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), charset, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (charset)); id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
  code = cons_to_unsigned (code_point, (2147483647 *2U +1U));
  charsetp = (charset_table + (id));
  c = (((((sizeof ((code) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((code) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((code) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((code) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (charsetp)->ascii_compatible_p) ? (code) : ((code) < (charsetp)->min_code || (code) > (charsetp)->max_code) ? -1 : (charsetp)->unified_p ? decode_char ((charsetp), (code)) : (charsetp)->method == CHARSET_METHOD_OFFSET ? ((charsetp)->code_linear_p ? (int) ((code) - (charsetp)->min_code) + (charsetp)->code_offset : decode_char ((charsetp), (code))) : (charsetp)->method == CHARSET_METHOD_MAP ? (((charsetp)->code_linear_p && VECTORP ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_decoder)))) ? ((AREF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_decoder)), (code) - (charsetp)->min_code)) >> INTTYPEBITS) : decode_char ((charsetp), (code))) : decode_char ((charsetp), (code)));
  return (c >= 0 ? ((EMACS_INT) (((EMACS_UINT) (c) << INTTYPEBITS) + Lisp_Int0)) : builtin_lisp_symbol (0));
}


static struct Lisp_Subr _Alignas (8) Sencode_char = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Fencode_char }, 2, 3, "encode-char", 0, 0}; Lisp_Object Fencode_char
  ...
  (Lisp_Object ch, Lisp_Object charset, Lisp_Object restriction)
{
  ...
  c = ((ch) >> INTTYPEBITS);
  charsetp = (charset_table + (id));
  code = (((!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (c) <= sizeof (int)) ? 1 : -1; })) ? (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (charsetp)->ascii_compatible_p ? (unsigned) (c) : ((charsetp)->unified_p || (charsetp)->method == CHARSET_METHOD_SUBSET || (charsetp)->method == CHARSET_METHOD_SUPERSET) ? encode_char (charsetp, c) : (c) < (charsetp)->min_char || (c) > (charsetp)->max_char ? (charsetp)->invalid_code : (charsetp)->method == CHARSET_METHOD_OFFSET ? ((charsetp)->code_linear_p ? (unsigned) ((c) - (charsetp)->code_offset) + (charsetp)->min_code : encode_char (charsetp, c)) : (charsetp)->method == CHARSET_METHOD_MAP ? (((charsetp)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (charsetp)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (charsetp, c)) : encode_char (charsetp, c))) : (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (charsetp)->ascii_compatible_p ? (unsigned) (c) : ((charsetp)->unified_p || (charsetp)->method == CHARSET_METHOD_SUBSET || (charsetp)->method == CHARSET_METHOD_SUPERSET) ? encode_char (charsetp, c) : (c) < (charsetp)->min_char || (c) > (charsetp)->max_char ? (charsetp)->invalid_code : (charsetp)->method == CHARSET_METHOD_OFFSET ? ((charsetp)->code_linear_p ? (unsigned) ((c) - (charsetp)->code_offset) + (charsetp)->min_code : encode_char (charsetp, c)) : (charsetp)->method == CHARSET_METHOD_MAP ? (((charsetp)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charsetp)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (charsetp)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (charsetp, c)) : encode_char (charsetp, c)))));
  if (code == ((charsetp)->invalid_code))
    return builtin_lisp_symbol (0);
  return (! (! ((0 <= (code) || (-1 - (9223372036854775807L >> INTTYPEBITS)) <= (code)) && (code) <= (9223372036854775807L >> INTTYPEBITS))) ? ((EMACS_INT) (((EMACS_UINT) (code) << INTTYPEBITS) + Lisp_Int0)) : ((0 * (code) - (1)) < 0) ? intbig_to_lisp (code) : uintbig_to_lisp (code));
}


static struct Lisp_Subr _Alignas (8) Smake_char = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a5 = Fmake_char }, 1, 5, "make-char", 0, 0}; Lisp_Object Fmake_char
  ...
  (Lisp_Object charset, Lisp_Object code1, Lisp_Object code2, Lisp_Object code3, Lisp_Object code4)
{
  ...
  dimension = ((charsetp)->dimension);
  if (((code1) == (builtin_lisp_symbol (0))))
    code = (((charsetp)->ascii_compatible_p)
     ? 0 : ((charsetp)->min_code));
  else
    {
      CHECK_NATNUM (code1);
      if (((code1) >> INTTYPEBITS) >= 0x100)
 args_out_of_range (((EMACS_INT) (((EMACS_UINT) (0xFF) << INTTYPEBITS) + Lisp_Int0)), code1);
      code = ((code1) >> INTTYPEBITS);

      if (dimension > 1)
 {
   code <<= 8;
   if (((code2) == (builtin_lisp_symbol (0))))
     code |= charsetp->code_space[(dimension - 2) * 4];
   else
     {
       CHECK_NATNUM (code2);
       if (((code2) >> INTTYPEBITS) >= 0x100)
  args_out_of_range (((EMACS_INT) (((EMACS_UINT) (0xFF) << INTTYPEBITS) + Lisp_Int0)), code2);
       code |= ((code2) >> INTTYPEBITS);
     }

   if (dimension > 2)
     {
       code <<= 8;
       if (((code3) == (builtin_lisp_symbol (0))))
  code |= charsetp->code_space[(dimension - 3) * 4];
       else
  {
    CHECK_NATNUM (code3);
    if (((code3) >> INTTYPEBITS) >= 0x100)
      args_out_of_range (((EMACS_INT) (((EMACS_UINT) (0xFF) << INTTYPEBITS) + Lisp_Int0)), code3);
    code |= ((code3) >> INTTYPEBITS);
  }

       if (dimension > 3)
  {
    code <<= 8;
    if (((code4) == (builtin_lisp_symbol (0))))
      code |= charsetp->code_space[0];
    else
      {
        CHECK_NATNUM (code4);
        if (((code4) >> INTTYPEBITS) >= 0x100)
   args_out_of_range (((EMACS_INT) (((EMACS_UINT) (0xFF) << INTTYPEBITS) + Lisp_Int0)), code4);
        code |= ((code4) >> INTTYPEBITS);
      }
  }
     }
 }
...
char_charset (int c, Lisp_Object charset_list, unsigned int *code_return)
{
      ...
      if (code != ((charset)->invalid_code))
 {
   ...
   return charset;
      ...
      if (! maybe_null
   && c <= 0x10FFFF
   && ((charset_list) == (Vcharset_non_preferred_head)))
 return (charset_table + (charset_unicode));
    }
  return (maybe_null ? ((void*)0)
   : c <= 0x3FFF7F ? (charset_table + (charset_emacs))
   : (charset_table + (charset_eight_bit)));
}


static struct Lisp_Subr _Alignas (8) Ssplit_char = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fsplit_char }, 1, 1, "split-char", 0, 0}; Lisp_Object Fsplit_char
  ...
  (Lisp_Object ch)
{
  ...
  for (val = builtin_lisp_symbol (0); dimension > 0; dimension--)
    {
      val = Fcons (((EMACS_INT) (((EMACS_UINT) (code & 0xFF) << INTTYPEBITS) + Lisp_Int0)), val);
      code >>= 8;
    }
  return Fcons ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_name)), val);
}


static struct Lisp_Subr _Alignas (8) Schar_charset = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a2 = Fchar_charset }, 1, 2, "char-charset", 0, 0}; Lisp_Object Fchar_charset
  ...
  (Lisp_Object ch, Lisp_Object restriction)
{
  ...
  (((NATNUMP (ch) && ((ch) >> INTTYPEBITS) <= 0x3FFFFF)) ? (void) 0 : (void) wrong_type_argument (builtin_lisp_symbol (260), ch));
  if (((restriction) == (builtin_lisp_symbol (0))))
    charset = ((((ch) >> INTTYPEBITS)) < 0x80 ? (charset_table + (charset_ascii)) : char_charset ((((ch) >> INTTYPEBITS)), builtin_lisp_symbol (0), ((void*)0)));
  else
    {
      if ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons))
 {
   int c = ((ch) >> INTTYPEBITS);

   for (; (((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); restriction = (((void) (0 && ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((restriction) - (Lisp_Cons))))->u.cdr)
     {
       struct charset *rcharset;

       do { int csid; do { ptrdiff_t idx; if (! (((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((restriction) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), (((void) (0 && ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((restriction) - (Lisp_Cons))))->car, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), ((((void) (0 && ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((restriction) - (Lisp_Cons))))->car)); csid = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0); rcharset = (charset_table + (csid)); } while (0);
       if ((((!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (c) <= sizeof (int)) ? 1 : -1; })) ? (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (rcharset)->ascii_compatible_p ? (unsigned) (c) : ((rcharset)->unified_p || (rcharset)->method == CHARSET_METHOD_SUBSET || (rcharset)->method == CHARSET_METHOD_SUPERSET) ? encode_char (rcharset, c) : (c) < (rcharset)->min_char || (c) > (rcharset)->max_char ? (rcharset)->invalid_code : (rcharset)->method == CHARSET_METHOD_OFFSET ? ((rcharset)->code_linear_p ? (unsigned) ((c) - (rcharset)->code_offset) + (rcharset)->min_code : encode_char (rcharset, c)) : (rcharset)->method == CHARSET_METHOD_MAP ? (((rcharset)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (rcharset)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (rcharset)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (rcharset)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (rcharset, c)) : encode_char (rcharset, c))) : (((((sizeof ((c) + 0)) > (sizeof ((0x80) + 0)) ? (sizeof ((c) + 0)) : (sizeof ((0x80) + 0))) <= sizeof (unsigned) ? ((c) + (unsigned) 0) < ((0x80) + (unsigned) 0) : ((c) + (uintmax_t) 0) < ((0x80) + (uintmax_t) 0)) && (rcharset)->ascii_compatible_p ? (unsigned) (c) : ((rcharset)->unified_p || (rcharset)->method == CHARSET_METHOD_SUBSET || (rcharset)->method == CHARSET_METHOD_SUPERSET) ? encode_char (rcharset, c) : (c) < (rcharset)->min_char || (c) > (rcharset)->max_char ? (rcharset)->invalid_code : (rcharset)->method == CHARSET_METHOD_OFFSET ? ((rcharset)->code_linear_p ? (unsigned) ((c) - (rcharset)->code_offset) + (rcharset)->min_code : encode_char (rcharset, c)) : (rcharset)->method == CHARSET_METHOD_MAP ? (((rcharset)->compact_codes_p && CHAR_TABLE_P ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (rcharset)->hash_index))), charset_encoder)))) ? (charset_work = CHAR_TABLE_REF ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (rcharset)->hash_index))), charset_encoder)), c), (((charset_work) == (builtin_lisp_symbol (0))) ? (rcharset)->invalid_code : (unsigned) ((charset_work) >> INTTYPEBITS))) : encode_char (rcharset, c)) : encode_char (rcharset, c))))) != ((rcharset)->invalid_code))
  return (((void) (0 && ((((enum Lisp_Type) ((restriction) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((restriction) - (Lisp_Cons))))->car;
     }
   return builtin_lisp_symbol (0);
 }
      restriction = coding_system_charset_list (restriction);
      charset = char_charset (((ch) >> INTTYPEBITS), restriction, ((void*)0));
      if (! charset)
 return builtin_lisp_symbol (0);
    }
  return ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_name)));
}


static struct Lisp_Subr _Alignas (8) Scharset_after = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fcharset_after }, 0, 1, "charset-after", 0, 0}; Lisp_Object Fcharset_after
  ...
  (Lisp_Object pos)
{
  ...
  ch = Fchar_after (pos);
  if (! ((((enum Lisp_Type) ((ch) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) & (Lisp_Int0 | ~Lisp_Int1)) == Lisp_Int0))
    return ch;
  charset = ((((ch) >> INTTYPEBITS)) < 0x80 ? (charset_table + (charset_ascii)) : char_charset ((((ch) >> INTTYPEBITS)), builtin_lisp_symbol (0), ((void*)0)));
  return ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), (charset)->hash_index))), charset_name)));
}
...
static struct Lisp_Subr _Alignas (8) Siso_charset = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a3 = Fiso_charset }, 3, 3, "iso-charset", 0, 0}; Lisp_Object Fiso_charset
# 2094 "charset.c"
  (Lisp_Object dimension, Lisp_Object chars, Lisp_Object final_char)
{
  _Bool chars_flag = check_iso_charset_parameter (dimension, chars, final_char);
  int id = iso_charset_table[(((dimension) >> INTTYPEBITS)) - 1][chars_flag][((final_char) >> INTTYPEBITS)];

  return (id >= 0 ? (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), ((charset_table + (id)))->hash_index))), charset_name)) : builtin_lisp_symbol (0));
}


static struct Lisp_Subr _Alignas (8) Sclear_charset_maps = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Fclear_charset_maps }, 0, 0, "clear-charset-maps", 0, 0}; Lisp_Object Fclear_charset_maps
  ...
  if (temp_charset_work)
    {
      ...
      temp_charset_work = ((void*)0);
    }

  if (CHAR_TABLE_P (Vchar_unify_table))
    Foptimize_char_table (Vchar_unify_table, builtin_lisp_symbol (0));

  return builtin_lisp_symbol (0);
}

static struct Lisp_Subr _Alignas (8) Scharset_priority_list = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fcharset_priority_list }, 0, 1, "charset-priority-list", 0, 0}; Lisp_Object Fcharset_priority_list



  (Lisp_Object highestp)
{
  Lisp_Object val = builtin_lisp_symbol (0), list = Vcharset_ordered_list;

  if (!((highestp) == (builtin_lisp_symbol (0))))
    return (AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), ((charset_table + (((Fcar (list)) >> INTTYPEBITS))))->hash_index))), charset_name));

  while (!((list) == (builtin_lisp_symbol (0))))
    {
      val = Fcons ((AREF (((HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), ((charset_table + ((((((void) (0 && ((((enum Lisp_Type) ((list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((list) - (Lisp_Cons))))->car) >> INTTYPEBITS))))->hash_index))), charset_name)), val);
...
static struct Lisp_Subr _Alignas (8) Sset_charset_priority = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .aMANY = Fset_charset_priority }, 1, MANY, "set-charset-priority", 0, 0}; Lisp_Object Fset_charset_priority
  ...
  (ptrdiff_t nargs, Lisp_Object *args)
{
  ...
  new_head = builtin_lisp_symbol (0);
  for (i = 0; i < nargs; i++)
    {
      do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((args[i]) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), args[i], ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (args[i])); id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
      if (! ((Fmemq (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)), old_list)) == (builtin_lisp_symbol (0))))
 {
   old_list = Fdelq (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)), old_list);
   new_head = Fcons (((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0)), new_head);
 }
    }
  Vcharset_non_preferred_head = old_list;
  Vcharset_ordered_list = (Fnconc) ((sizeof (((Lisp_Object []) {Fnreverse (new_head), old_list})) / sizeof (((Lisp_Object []) {Fnreverse (new_head), old_list}))[0]), ((Lisp_Object []) {Fnreverse (new_head), old_list}));

  charset_ordered_list_tick++;

  charset_unibyte = -1;
  for (old_list = Vcharset_ordered_list, list_2022 = list_emacs_mule = builtin_lisp_symbol (0);
       (((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); old_list = (((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->u.cdr)
    {
      if (! ((Fmemq ((((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->car, Viso_2022_charset_list)) == (builtin_lisp_symbol (0))))
 list_2022 = Fcons ((((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->car, list_2022);
      if (! ((Fmemq ((((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->car, Vemacs_mule_charset_list)) == (builtin_lisp_symbol (0))))
 list_emacs_mule = Fcons ((((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->car, list_emacs_mule);
      if (charset_unibyte < 0)
 {
   struct charset *charset = (charset_table + ((((((void) (0 && ((((enum Lisp_Type) ((old_list) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((old_list) - (Lisp_Cons))))->car) >> INTTYPEBITS)));

   if (((charset)->dimension) == 1
       && ((charset)->ascii_compatible_p)
       && ((charset)->max_char) >= 0x80)
     charset_unibyte = ((charset)->id);
 }
    }
...
static struct Lisp_Subr _Alignas (8) Scharset_id_internal = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fcharset_id_internal }, 0, 1, "charset-id-internal", 0, 0}; Lisp_Object Fcharset_id_internal
  ...
  (Lisp_Object charset)
{
  ...
  do { ptrdiff_t idx; if (! (((enum Lisp_Type) ((charset) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || (idx = hash_lookup (XHASH_TABLE (Vcharset_hash_table), charset, ((void*)0))) < 0) wrong_type_argument (builtin_lisp_symbol (262), (charset)); id = ((AREF (HASH_VALUE (XHASH_TABLE (Vcharset_hash_table), idx), charset_id)) >> INTTYPEBITS); } while (0);
  return ((EMACS_INT) (((EMACS_UINT) (id) << INTTYPEBITS) + Lisp_Int0));
}
...
static struct Lisp_Subr _Alignas (8) Ssort_charsets = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Fsort_charsets }, 1, 1, "sort-charsets", 0, 0}; Lisp_Object Fsort_charsets
     ...
     (Lisp_Object charsets)
{
  ...
  ptrdiff_t n = ((len) >> INTTYPEBITS), i, j;
  int done;
  Lisp_Object tail, elt, attrs;
  struct charset_sort_data *sort_data;
  int id, min_id = 2147483647, max_id = (-2147483647 -1);
  ptrdiff_t sa_avail = MAX_ALLOCA; ptrdiff_t sa_count = SPECPDL_INDEX (); _Bool sa_must_free = 0;

  if (n == 0)
    return builtin_lisp_symbol (0);
  do { if ((n) <= sa_avail / sizeof *(sort_data) / (1)) (sort_data) = (sa_avail -= (sizeof *(sort_data) * (1) * (n)), __builtin_alloca (sizeof *(sort_data) * (1) * (n))); else { (sort_data) = xnmalloc (n, sizeof *(sort_data) * (1)); sa_must_free = 1; record_unwind_protect_ptr (xfree, sort_data); } } while (0);
  for (tail = charsets, i = 0; (((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); tail = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr, i++)
    {
      ...
      do { if (!(((enum Lisp_Type) ((elt) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Symbol) || ((attrs = Fgethash ((elt), Vcharset_hash_table, builtin_lisp_symbol (0))) == (builtin_lisp_symbol (0)))) wrong_type_argument (builtin_lisp_symbol (262), (elt)); } while (0);
      sort_data[i].charset = elt;
      sort_data[i].id = id = ((AREF ((attrs), charset_id)) >> INTTYPEBITS);
      if (id < min_id)
 min_id = id;
      if (id > max_id)
 max_id = id;
  ...
  for (done = 0, tail = Vcharset_ordered_list, i = 0;
       done < n && (((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); tail = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr, i++)
    {
      elt = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->car;
      id = ((elt) >> INTTYPEBITS);
 ...
 for (j = 0; j < n; j++)
  ...
  for (i = 0, tail = charsets; (((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons); tail = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr, i++)
...
{
  Lisp_Object tempdir;
  tempdir = Fexpand_file_name (build_string ("charsets"), globals.f_Vdata_directory);
  if (! file_accessible_directory_p (tempdir))
    {
      ...
      fprintf (stderr, "Error: charsets directory not found:\n%s\nEmacs will not function correctly without the character map files.\n%sPlease check your installation!\n",
               ...
               SDATA (tempdir),
               egetenv("EMACSDATA") ? "The EMACSDATA environment variable is set, maybe it has the wrong value?\n" : "");

      exit (1);
    }

  globals.f_Vcharset_map_path = list1 (tempdir);
}
...
syms_of_charset (void)
{
  ...
  Vcharset_ordered_list = builtin_lisp_symbol (0);

  staticpro (&Viso_2022_charset_list);
  Viso_2022_charset_list = builtin_lisp_symbol (0);

  staticpro (&Vemacs_mule_charset_list);
  Vemacs_mule_charset_list = builtin_lisp_symbol (0);

  staticpro (&Vcharset_hash_table);
  Vcharset_hash_table = (Fmake_hash_table) ((sizeof (((Lisp_Object []) {builtin_lisp_symbol (129), builtin_lisp_symbol (369)})) / sizeof (((Lisp_Object []) {builtin_lisp_symbol (129), builtin_lisp_symbol (369)}))[0]), ((Lisp_Object []) {builtin_lisp_symbol (129), builtin_lisp_symbol (369)}));

  charset_table = charset_table_init;
  charset_table_size = (sizeof (charset_table_init) / sizeof (charset_table_init)[0]);
  ...
  do { static struct Lisp_Objfwd o_fwd; defvar_lisp (&o_fwd, "charset-map-path", &globals.f_Vcharset_map_path); } while (0);

  globals.f_Vcharset_map_path = builtin_lisp_symbol (0);

  do { static struct Lisp_Boolfwd b_fwd; defvar_bool (&b_fwd, "inhibit-load-charset-map", &globals.f_inhibit_load_charset_map); } while (0);

  globals.f_inhibit_load_charset_map = 0;

  do { static struct Lisp_Objfwd o_fwd; defvar_lisp (&o_fwd, "charset-list", &globals.f_Vcharset_list); } while (0);

  globals.f_Vcharset_list = builtin_lisp_symbol (0);

  do { static struct Lisp_Objfwd o_fwd; defvar_lisp (&o_fwd, "current-iso639-language", &globals.f_Vcurrent_iso639_language); } while (0);



  globals.f_Vcurrent_iso639_language = builtin_lisp_symbol (0);

  charset_ascii
    = define_charset_internal (builtin_lisp_symbol (181), 1, "\x00\x7F\0\0\0\0\0",
          0, 127, 'B', -1, 0, 1, 0, 0);
  charset_iso_8859_1
    = define_charset_internal (builtin_lisp_symbol (581), 1, "\x00\xFF\0\0\0\0\0",
          0, 255, -1, -1, -1, 1, 0, 0);
  charset_unicode
    = define_charset_internal (builtin_lisp_symbol (971), 3, "\x00\xFF\x00\xFF\x00\x10\0",
          0, 0x10FFFF, -1, 0, -1, 1, 0, 0);
  charset_emacs
    = define_charset_internal (builtin_lisp_symbol (360), 3, "\x00\xFF\x00\xFF\x00\x3F\0",
          0, 0x3FFF7F, -1, 0, -1, 1, 1, 0);
  charset_eight_bit
    = define_charset_internal (builtin_lisp_symbol (359), 1, "\x80\xFF\0\0\0\0\0",
          128, 255, -1, 0, -1, 0, 1,
          0x3FFF7F + 1);
  charset_unibyte = charset_iso_8859_1;
}
