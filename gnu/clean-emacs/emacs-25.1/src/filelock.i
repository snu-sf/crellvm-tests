...
# 23 "/usr/include/utmp.h" 3 4
# 1 "../lib/sys/types.h" 1 3 4
# 21 "../lib/sys/types.h" 3
# 24 "/usr/include/utmp.h" 2 3 4
...
extern int _gl_cxxalias_dummy;
# 25 "/usr/include/bits/utmp.h" 2 3 4
# 1 "../lib/sys/types.h" 1 3 4
# 21 "../lib/sys/types.h" 3
# 26 "/usr/include/bits/utmp.h" 2 3 4
# 1 "/usr/include/bits/wordsize.h" 1 3 4
...
static _Bool boot_time_initialized;


static void get_boot_time_1 (const char *, _Bool);
...
# 163 "filelock.c"
  if ("/var/run/random-seed")
    {
      struct stat st;
      if (stat ("/var/run/random-seed", &st) == 0)
 {
   boot_time = st.st_mtim.tv_sec;
   return boot_time;
 }
    }
  ...
  get_boot_time_1 ("/var/log/wtmp", 1);


  for (counter = 0; counter < 20 && ! boot_time; counter++)
    {
      char cmd_string[sizeof "/var/log/wtmp" ".19.gz"];
      Lisp_Object tempname, filename;
      _Bool delete_flag = 0;

      filename = builtin_lisp_symbol (0);

      tempname = make_formatted_string
 (cmd_string, "%s.%d", "/var/log/wtmp", counter);
      if (! ((Ffile_exists_p (tempname)) == (builtin_lisp_symbol (0))))
 filename = tempname;
      else
 {
   tempname = make_formatted_string (cmd_string, "%s.%d.gz",
         "/var/log/wtmp", counter);
   if (! ((Ffile_exists_p (tempname)) == (builtin_lisp_symbol (0))))
     {
       ...
       (Fcall_process) ((sizeof (((Lisp_Object []) {build_string ("gzip"), builtin_lisp_symbol (0), list2 (builtin_lisp_symbol (41), filename), builtin_lisp_symbol (0), build_string ("-cd"), tempname})) / sizeof (((Lisp_Object []) {build_string ("gzip"), builtin_lisp_symbol (0), list2 (builtin_lisp_symbol (41), filename), builtin_lisp_symbol (0), build_string ("-cd"), tempname}))[0]), ((Lisp_Object []) {build_string ("gzip"), builtin_lisp_symbol (0), list2 (builtin_lisp_symbol (41), filename), builtin_lisp_symbol (0), build_string ("-cd"), tempname}));
      ...
      if (! ((filename) == (builtin_lisp_symbol (0))))
 {
...
# 252 "filelock.c"
void
get_boot_time_1 (const char *filename, _Bool newest)
{
  struct utmp ut, *utp;
  ...
  if (filename)
    {
      ...
      if (faccessat (-100, filename, 4, 0x200) != 0)
 return;
 ...
 break;
      ...
      if (utp->ut_tv.tv_sec > boot_time)
 {
   boot_time = utp->ut_tv.tv_sec;
   if (! newest)
     break;
 }
...
enum { LINKS_MIGHT_NOT_WORK = 1 };




static int
rename_lock_file (char const *old, char const *new, _Bool force)
{
  ...
  if (! force)
    {
      ...
      if (link (old, new) == 0)
 return unlink (old) == 0 || (*__errno_location ()) == 2 ? 0 : -1;
      if ((*__errno_location ()) != 38 && (*__errno_location ()) != LINKS_MIGHT_NOT_WORK)
 return -1;
      ...
      if (lstat (new, &st) == 0 || (*__errno_location ()) == 75)
 {
   (*__errno_location ()) = 17;
   return -1;
 }
...
static int
create_lock_file (char *lfname, char *lock_info_str, _Bool force)
{
  ...
  int err = symlink (lock_info_str, lfname) == 0 ? 0 : (*__errno_location ());


  if (err == 17 && force)
    {
      unlink (lfname);
      err = symlink (lock_info_str, lfname) == 0 ? 0 : (*__errno_location ());
    }

  if (err == 38 || err == LINKS_MIGHT_NOT_WORK || err == 36)
    {
      ...
      char *nonce = ((lfdirlen + sizeof nonce_base) <= sa_avail ? (sa_avail -= (lfdirlen + sizeof nonce_base), __builtin_alloca (lfdirlen + sizeof nonce_base)) : (sa_must_free = 1, record_xmalloc (lfdirlen + sizeof nonce_base)));
      int fd;
      memcpy (nonce, lfname, lfdirlen);
      strcpy (nonce + lfdirlen, nonce_base);

      fd = mkostemp (nonce, 0 | 02000000);
      if (fd < 0)
      ...
      else
 {
   ...
   if (write (fd, lock_info_str, lock_info_len) != lock_info_len
       || fchmod (fd, 0400 | (0400 >> 3) | ((0400 >> 3) >> 3)) != 0)
     err = (*__errno_location ());
...
static int
lock_file_1 (char *lfname, _Bool force)
{

  printmax_t boot = get_boot_time ();

  Lisp_Object luser_name = Fuser_login_name (builtin_lisp_symbol (0));
...
read_lock_data (char *lfname, char lfinfo[MAX_LFINFO + 1])
{
  ...
  while ((nbytes = readlinkat (-100, lfname, lfinfo, MAX_LFINFO + 1)) < 0
  && (*__errno_location ()) == 22)
    {
      int fd = emacs_open (lfname, 00 | 0 | 0400000, 0);
      if (0 <= fd)
 {

   ptrdiff_t read_bytes = read (fd, lfinfo, MAX_LFINFO + 1);
   int read_errno = (*__errno_location ());
   if (emacs_close (fd) != 0)
     return -1;
   (*__errno_location ()) = read_errno;
   return read_bytes;
 }
 ...
 return -1;
  ...
  if (STRINGP (system_name)
      ...
      && memcmp (at + 1, SSDATA (system_name), SBYTES (system_name)) == 0)
    {
      if (pid == getpid ())
        ret = 2;
      else if (0 < pid && pid <= ((pid_t) (! (! ((pid_t) 0 < (pid_t) -1)) ? (pid_t) -1 : ((((pid_t) 1 << (sizeof (pid_t) * 8 - 2)) - 1) * 2 + 1)))
               && (kill (pid, 0) >= 0 || (*__errno_location ()) == 1)
        && (boot_time == 0
     || (boot_time <= ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1)))
...
{
  int err;
  while ((err = lock_file_1 (lfname, 0)) == 17)
    {
...
lock_file (Lisp_Object fn)
{
  ...
  orig_fn = fn;
  fn = Fexpand_file_name (fn, builtin_lisp_symbol (0));
  ...
  encoded_fn = encode_file_name (fn);
...

  {
    ...
    if (!((subject_buf) == (builtin_lisp_symbol (0)))
 && ((Fverify_visited_file_modtime (subject_buf)) == (builtin_lisp_symbol (0)))
 && !((Ffile_exists_p (fn)) == (builtin_lisp_symbol (0))))
      call1 (intern ("ask-user-about-supersession-threat"), fn);

  }


  if (globals.f_create_lockfiles)
    {


      (lfname = ((SBYTES (encoded_fn) + 2 + 1) <= sa_avail ? (sa_avail -= (SBYTES (encoded_fn) + 2 + 1), __builtin_alloca (SBYTES (encoded_fn) + 2 + 1)) : (sa_must_free = 1, record_xmalloc (SBYTES (encoded_fn) + 2 + 1))), fill_in_lock_file_name (lfname, encoded_fn));
      ...
      if (0 < lock_if_free (&lock_info, lfname))
 {
   ...
   if (!((attack) == (builtin_lisp_symbol (0))))
...
unlock_file (Lisp_Object fn)
{
  ...
  fn = Fexpand_file_name (fn, builtin_lisp_symbol (0));
  fn = encode_file_name (fn);

  (lfname = ((SBYTES (fn) + 2 + 1) <= sa_avail ? (sa_avail -= (SBYTES (fn) + 2 + 1), __builtin_alloca (SBYTES (fn) + 2 + 1)) : (sa_must_free = 1, record_xmalloc (SBYTES (fn) + 2 + 1))), fill_in_lock_file_name (lfname, fn));
...
unlock_all_files (void)
{
  ...
  for ((tail) = (Vbuffer_alist); ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons) && ((buf) = (((void) (0 && ((((enum Lisp_Type) (((((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->car) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) (((((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->car) - (Lisp_Cons))))->u.cdr, 1)); (tail) = (((void) (0 && ((((enum Lisp_Type) ((tail) & ~(((9223372036854775807L >> (3 - 1)) / 2 < (9223372036854775807L)) ? - (1 << 3) : (9223372036854775807L >> (3 - 1))))) == Lisp_Cons)))), (struct Lisp_Cons *) ((void *) (intptr_t) ((tail) - (Lisp_Cons))))->u.cdr)
    {
      b = XBUFFER (buf);
      if (STRINGP (((b)-> file_truename_))
   && ((b)->text->save_modiff) < ((b)->text->modiff))
 unlock_file (((b)-> file_truename_));
    }
}

static struct Lisp_Subr _Alignas (8) Slock_buffer = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Flock_buffer }, 0, 1, "lock-buffer", 0, 0}; Lisp_Object Flock_buffer
  ...
  (Lisp_Object file)
{
  if (((file) == (builtin_lisp_symbol (0))))
    file = ((current_buffer)-> file_truename_);
  else
    CHECK_STRING (file);
  if ((current_buffer->text->save_modiff) < (current_buffer->text->modiff)
      && !((file) == (builtin_lisp_symbol (0))))
    lock_file (file);
  return builtin_lisp_symbol (0);
}

static struct Lisp_Subr _Alignas (8) Sunlock_buffer = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a0 = Funlock_buffer }, 0, 0, "unlock-buffer", 0, 0}; Lisp_Object Funlock_buffer
...
{
  if ((current_buffer->text->save_modiff) < (current_buffer->text->modiff)
      && STRINGP (((current_buffer)-> file_truename_)))
...
{
  if (((buffer)->text->save_modiff) < ((buffer)->text->modiff)
      && STRINGP (((buffer)-> file_truename_)))
    unlock_file (((buffer)-> file_truename_));
}

static struct Lisp_Subr _Alignas (8) Sfile_locked_p = { { PVEC_SUBR << PSEUDOVECTOR_AREA_BITS }, { .a1 = Ffile_locked_p }, 1, 1, "file-locked-p", 0, 0}; Lisp_Object Ffile_locked_p
  ...
  (Lisp_Object filename)
{
  ...
  (lfname = ((SBYTES (filename) + 2 + 1) <= sa_avail ? (sa_avail -= (SBYTES (filename) + 2 + 1), __builtin_alloca (SBYTES (filename) + 2 + 1)) : (sa_must_free = 1, record_xmalloc (SBYTES (filename) + 2 + 1))), fill_in_lock_file_name (lfname, filename));

  owner = current_lock_owner (&locker, lfname);
  if (owner <= 0)
    ret = builtin_lisp_symbol (0);
  else if (owner == 2)
    ret = builtin_lisp_symbol (901);
  else
...
{
  do { static struct Lisp_Objfwd o_fwd; defvar_lisp (&o_fwd, "temporary-file-directory", &globals.f_Vtemporary_file_directory); } while (0);

  globals.f_Vtemporary_file_directory = builtin_lisp_symbol (0);

  do { static struct Lisp_Boolfwd b_fwd; defvar_bool (&b_fwd, "create-lockfiles", &globals.f_create_lockfiles); } while (0);

  globals.f_create_lockfiles = 1;
