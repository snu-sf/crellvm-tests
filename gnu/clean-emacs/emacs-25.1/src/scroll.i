...
{
  int i, j;
  int frame_total_lines = ((frame)->total_lines);
  struct matrix_elt *p, *p1;
  int cost, cost1;

  int lines_moved = window_size
    + (((((frame)->output_method == output_termcap || (frame)->output_method == output_msdos_raw) ? (frame)->terminal->display_info.tty : (emacs_abort (), (struct tty_display_info *) 0))->scroll_region_ok) ? 0 : lines_below);



  int *first_insert_cost
    = &(frame)->insert_line_cost[frame_total_lines - 1 - lines_moved];
  int *first_delete_cost
    = &(frame)->delete_line_cost[frame_total_lines - 1 - lines_moved];
  int *next_insert_cost
    = &(frame)->insert_n_lines_cost[frame_total_lines - 1 - lines_moved];
  int *next_delete_cost
    = &(frame)->delete_n_lines_cost[frame_total_lines - 1 - lines_moved];




  int extra_cost = globals.f_baud_rate / (10 * 4 * frame_total_lines);

  if (globals.f_baud_rate <= 0)
    extra_cost = 1;


  matrix->writecost = 0;
  matrix->insertcost = 1000000;
  matrix->deletecost = 1000000;
  ...
  for (j = 1; j <= window_size; j++)
    {
      ...
      matrix[j].writecost = 1000000;
      matrix[j].insertcost = 1000000;
...
# 183 "scroll.c"
 ...
 p->insertcost = ((cost) < (cost1) ? (cost) : (cost1)) + draw_cost[i] + extra_cost;
 ...
 p->deletecost = ((cost) < (cost1) ? (cost) : (cost1));
              ...
              int unchanged_at_top)
{
  ...
  _Bool terminal_window_p = 0;


  struct queue { int count, pos; };
  struct queue *queue_start;
  do { if ((current_matrix->nrows) <= sa_avail / sizeof *(queue_start) / (1)) (queue_start) = (sa_avail -= (sizeof *(queue_start) * (1) * (current_matrix->nrows)), __builtin_alloca (sizeof *(queue_start) * (1) * (current_matrix->nrows))); else { (queue_start) = xnmalloc (current_matrix->nrows, sizeof *(queue_start) * (1)); sa_must_free = 1; record_unwind_protect_ptr (xfree, queue_start); } } while (0);
  struct queue *queue = queue_start;

  char *retained_p = ((window_size) <= sa_avail ? (sa_avail -= (window_size), __builtin_alloca (window_size)) : (sa_must_free = 1, record_xmalloc (window_size)));
  int *copy_from;
  do { if ((window_size) <= sa_avail / sizeof *(copy_from) / (1)) (copy_from) = (sa_avail -= (sizeof *(copy_from) * (1) * (window_size)), __builtin_alloca (sizeof *(copy_from) * (1) * (window_size))); else { (copy_from) = xnmalloc (window_size, sizeof *(copy_from) * (1)); sa_must_free = 1; record_unwind_protect_ptr (xfree, copy_from); } } while (0);
...
# 280 "scroll.c"
      ...
      else
 {
   ...
   --i, --j;
   ((void) (0 && (i >= 0 && i < window_size)));
   ((void) (0 && (j >= 0 && j < window_size)));
   copy_from[i] = j;
   retained_p[j] = 1;
      ...
      do
 {
  ...
  for (k = 0; k < window_size; ++k)
    ((void) (0 && (copy_from[k] >= 0 && copy_from[k] < window_size)));
       ...
       int free_at_end)
{
  ...
  int *first_insert_cost
    = &(frame)->insert_line_cost[frame_total_lines - 1];
  int *first_delete_cost
    = &(frame)->delete_line_cost[frame_total_lines - 1];
  int *next_insert_cost
    = &(frame)->insert_n_lines_cost[frame_total_lines - 1];
  int *next_delete_cost
    = &(frame)->delete_n_lines_cost[frame_total_lines - 1];
  ...
  scroll_overhead
    = ((((frame)->output_method == output_termcap || (frame)->output_method == output_msdos_raw) ? (frame)->terminal->display_info.tty : (emacs_abort (), (struct tty_display_info *) 0))->scroll_region_cost) + extra_cost;
       ...
       int unchanged_at_top)
{
  ...
  do { if ((window_size) <= sa_avail / sizeof *(queue_start) / (1)) (queue_start) = (sa_avail -= (sizeof *(queue_start) * (1) * (window_size)), __builtin_alloca (sizeof *(queue_start) * (1) * (window_size))); else { (queue_start) = xnmalloc (window_size, sizeof *(queue_start) * (1)); sa_must_free = 1; record_unwind_protect_ptr (xfree, queue_start); } } while (0);
      ...
      unsigned *old_hash, unsigned *new_hash, int free_at_end)
{
  ptrdiff_t sa_avail = MAX_ALLOCA; ptrdiff_t sa_count = SPECPDL_INDEX (); _Bool sa_must_free = 0;
  struct matrix_elt *matrix;
  do { if ((window_size + 1) <= sa_avail / sizeof *(matrix) / (window_size + 1)) (matrix) = (sa_avail -= (sizeof *(matrix) * (window_size + 1) * (window_size + 1)), __builtin_alloca (sizeof *(matrix) * (window_size + 1) * (window_size + 1))); else { (matrix) = xnmalloc (window_size + 1, sizeof *(matrix) * (window_size + 1)); sa_must_free = 1; record_unwind_protect_ptr (xfree, matrix); } } while (0);

  if (((((frame)->output_method == output_termcap || (frame)->output_method == output_msdos_raw) ? (frame)->terminal->display_info.tty : (emacs_abort (), (struct tty_display_info *) 0))->scroll_region_ok))
    {
...
{
  int frame_total_lines = ((frame)->total_lines);
  (frame)->insert_line_cost =
    xnrealloc ((frame)->insert_line_cost, frame_total_lines, sizeof (int));
  (frame)->delete_n_lines_cost =
    xnrealloc ((frame)->delete_n_lines_cost, frame_total_lines, sizeof (int));
  (frame)->insert_n_lines_cost =
    xnrealloc ((frame)->insert_n_lines_cost, frame_total_lines, sizeof (int));
  (frame)->delete_line_cost =
    xnrealloc ((frame)->delete_line_cost, frame_total_lines, sizeof (int));

  ins_del_costs (frame,
   ins_line_string, multi_ins_string,
   setup_string, cleanup_string,
   (frame)->insert_line_cost, (frame)->insert_n_lines_cost,
   coefficient);
  ins_del_costs (frame,
   del_line_string, multi_del_string,
   setup_string, cleanup_string,
   (frame)->delete_line_cost, (frame)->delete_n_lines_cost,
   coefficient);
}
