...
enum { ABBR_SIZE_MIN = DEFAULT_MXFAST - __builtin_offsetof(struct tm_zone, abbrs) };
...
tzalloc (char const *name)
{
  ...
  timezone_t tz = malloc (__builtin_offsetof(struct tm_zone, abbrs) + abbr_size);
  if (tz)
    {
      tz->next = ((void*)0);
...
{
  if (emacs_setenv_TZ (tz->tz_is_set ? tz->abbrs : ((void*)0)) != 0)
    return 0;
  tzset ();
  return 1;
}
