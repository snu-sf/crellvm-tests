...
const char *display = ((void*)0);


const char *alt_display = ((void*)0);


char *parent_id = ((void*)0);
...
const char *alternate_editor = ((void*)0);


const char *socket_name = ((void*)0);


const char *server_file = ((void*)0);
...
const char *frame_parameters = ((void*)0);
...
struct option longopts[] =
{
  ...
  { "tty", 0, ((void*)0), 't' },
  { "nw", 0, ((void*)0), 't' },
  { "create-frame", 0, ((void*)0), 'c' },
  { "alternate-editor", 1, ((void*)0), 'a' },
  { "frame-parameters", 1, ((void*)0), 'F' },

  { "socket-name", 1, ((void*)0), 's' },

  { "server-file", 1, ((void*)0), 'f' },
  { "display", 1, ((void*)0), 'd' },
  { "parent-id", 1, ((void*)0), 'p' },
  { 0, 0, 0, 0 }
};
...
# 420 "emacsclient.c"
static void message (_Bool, const char *, ...) __attribute__ ((__format__ (__printf__, 2, 3)));
static void
message (_Bool is_error, const char *format, ...)
{
  va_list args;
...
decode_options (int argc, char **argv)
{
 ...
 default:
   message (1, "Try '%s --help' for more information\n", progname);
   exit (1);
   break;
 }
    }
  ...
  if (alternate_editor)
    {
      ...
      message (1, "%s: error executing alternate editor \"%s\"\n",
        progname, alternate_editor);
    }
  exit (1);
}
# 683 "emacsclient.c"
char send_buffer[4096 + 1];
int sblen = 0;
...
{
# 706 "emacsclient.c"
  message (1, "%s: %s: %s\n", progname, function_name, strerror ((*__errno_location ())));
...
send_to_emacs (int s, const char *data)
{
  ...
  while (*data)
    {
      size_t part = (((dlen) < (4096 - sblen)) ? (dlen) : (4096 - sblen));
   ...
   || (sblen > 0 && send_buffer[sblen-1] == '\n'))
 {
   ...
   if (sent < 0)
     {
       message (1, "%s: failed to send %d bytes to socket: %s\n",
         progname, sblen, strerror ((*__errno_location ())));
       fail ();
     }
   if (sent != sblen)
...
static void
quote_argument (int s, const char *str)
{
...
# 917 "emacsclient.c"
  ...
  if (fgets (dotted, sizeof dotted, config)
    ...
    *port++ = '\0';
    ...
    {
      message (1, "%s: invalid configuration info\n", progname);
      exit (1);
    }
  ...
  if (! fread (authentication, 64, 1, config))
    {
      message (1, "%s: cannot read authentication info\n", progname);
      exit (1);
    }
...
set_tcp_socket (const char *local_server_file)
{
  ...
  setsockopt (s, 1, 13, (char *) &l_arg, sizeof l_arg);
...
find_tty (const char **tty_type, const char **tty_name, int noabort)
{
 ...
 {
   message (1, "%s: could not get terminal name\n", progname);
   fail ();
 }
    }
 ...
 {
   message (1, "%s: please set the TERM variable to your terminal type\n",
     progname);
   fail ();
 }
    }
      ...
      else
 {
   ...
   message (1, "%s: opening a frame in an Emacs term buffer"
     " is not supported\n", progname);
   fail ();
 }
    }
...
handle_sigtstp (int signalnum)
{
  ...
  sigprocmask (0, ((void*)0), &set);
  sigdelset (&set, signalnum);
  signal (signalnum, ((__sighandler_t) 0));
  raise (signalnum);
  sigprocmask (2, &set, ((void*)0));
...
init_signals (void)
{
  ...
  signal (28, pass_signal_to_emacs);
# 1163 "emacsclient.c"
  signal (18, handle_sigcont);
  signal (20, handle_sigtstp);
  signal (22, handle_sigtstp);
}
...
set_local_socket (const char *local_socket_name)
{
  ...
  if ((s = socket (1, SOCK_STREAM, 0)) < 0)
    {
      message (1, "%s: socket: %s\n", progname, strerror ((*__errno_location ())));
      return -1;
    }
...

  {
    ...
    const char *tmpdir ;
    char *tmpdir_storage = ((void*)0);
    char *socket_name_storage = ((void*)0);
...
# 1213 "emacsclient.c"
 ...
 socket_name_storage =
   xmalloc (strlen (tmpdir) + strlen (server_name) + 100);
 ...
 local_socket_name = socket_name_storage;
    ...
    if (strlen (local_socket_name) < sizeof (server.sun_path))
      ...
      {
        message (1, "%s: socket-name %s too long\n",
 ...
 const char *user_name = getenv("LOGNAME");
 ...
 if (0 != geteuid ())
   {
     message (1, "%s: Invalid socket owner\n", progname);
     return -1;
   }
 break;
...
set_socket (int no_exit_if_error)
{
  ...
  if (socket_name)
    {
      ...
      if ((s != -1) || no_exit_if_error)
 return s;
      message (1, "%s: error accessing socket \"%s\"\n",
  ...
  if (!local_server_file)
    local_server_file = getenv("EMACS_SERVER_FILE");
 ...
 return s;
      ...
      message (1, "%s: error accessing server file \"%s\"\n",
  ...
  message (1, "%s: No socket or alternate editor.  Please use:\n\n"
...
start_daemon_and_retry_set_socket (void)
{
  ...
  if (dpid > 0)
    {
      pid_t w;
      w = waitpid (dpid, &status, 2 | 8);

      if ((w == -1) || !((((__extension__ (((union { __typeof(status) __in; int __i; }) { .__in = (status) }).__i))) & 0x7f) == 0) || ((((__extension__ (((union { __typeof(status) __in; int __i; }) { .__in = (status) }).__i))) & 0xff00) >> 8))
 {
   message (1, "Error: Could not start the Emacs daemon\n");
   exit (1);
 }


      message (1, "Emacs daemon should have started, trying to connect again\n");
      if ((emacs_socket = set_socket (1)) == -1)
 {
   message (1, "Error: Cannot connect even after starting the Emacs daemon\n");
   exit (1);
 }
    }
  else if (dpid < 0)
      ...
      if (socket_name != ((void*)0))
 {
   ...
   d_argv[1] = daemon_arg;
      ...
      execvp ("emacs", d_argv);
      message (1, "%s: error starting emacs daemon\n", progname);
    }
# 1586 "emacsclient.c"
}
...
# 1609 "emacsclient.c"
  ...
  if ((argc - optind < 1) && !eval && current_frame)
    {
      message (1, "%s: file name or argument required\n"
      ...
      for (i = optind; i < argc; i++)
 {
   ...
   if (eval)
          ...
          if (*argv[i] == '+')
            {
       char *p = argv[i] + 1;
       while (((*__ctype_b_loc ())[(int) (((unsigned char) *p))] & (unsigned short int) _ISdigit) || *p == ':') p++;
       if (*p == 0)
                {
...
# 1771 "emacsclient.c"
  ...
  else if (eval)
    {
      ...
      while ((str = fgets (string, 8192, stdin)))
 {
   send_to_emacs (emacs_socket, "-eval ");
  ...
  fflush (stdout);
  while (fdatasync (1) != 0 && (*__errno_location ()) == 4)
    continue;
  ...
  while (exit_status == 0)
    {
      ...
      do
        {
          (*__errno_location ()) = 0;
          rl = recv (emacs_socket, string, 8192, 0);
        }


      while (rl < 0 && (*__errno_location ()) == 4);
      ...
      for (end_p = p = string; end_p != ((void*)0) && *end_p != '\0'; p = end_p)
 {
   end_p = strchr (p, '\n');
   if (end_p != ((void*)0))
     *end_p++ = '\0';
          ...
          if (strprefix ("-emacs-pid ", p))
            {
              ...
              emacs_pid = strtol (p + strlen ("-emacs-pid"), ((void*)0), 10);
            }
