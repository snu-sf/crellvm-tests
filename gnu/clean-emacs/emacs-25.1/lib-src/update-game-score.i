# 1 "update-game-score.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "update-game-score.c" 2
# 34 "update-game-score.c"
# 1 "../src/config.h" 1
# 1916 "../src/config.h"
# 1 "../src/conf_post.h" 1
# 35 "../src/conf_post.h"
...
extern int _gl_cxxalias_dummy;
# 39 "update-game-score.c" 2


# 1 "../lib/string.h" 1
# 21 "../lib/string.h" 3
# 42 "update-game-score.c" 2
# 1 "../lib/stdlib.h" 1
# 21 "../lib/stdlib.h" 3
# 43 "update-game-score.c" 2
# 1 "../lib/stdio.h" 1
# 21 "../lib/stdio.h" 3
# 43 "../lib/stdio.h" 3
...
extern int getopt_long_only (int ___argc, char *const *___argv,
        const char *__shortopts,
               const struct option *__longopts, int *__longind)
       __attribute__ ((__nothrow__ ));
# 50 "update-game-score.c" 2
# 62 "update-game-score.c"
...
{
  fprintf (stderr, "%s: %s\n", msg,
    (*__errno_location ()) ? strerror ((*__errno_location ())) : "Invalid data in score file");
  exit (1);
}
...
get_user_id (void)
{
  ...
  if (!buf || strchr (buf->pw_name, ' ') || strchr (buf->pw_name, '\n'))
    {
      intmax_t uid = getuid ();
      char *name = malloc (sizeof uid * 8 / 3 + 4);
      if (name)
 sprintf (name, "%""l" "d", uid);
      return name;
    }
  return buf->pw_name;
}
...
static const char *
get_prefix (_Bool privileged, const char *user_prefix)
{
  if (privileged)
    {

      return "/usr/local/var/games/emacs";




    }
  if (user_prefix == ((void*)0))
...
normalize_integer (char *num)
{
  _Bool neg;
  char *p;
  while (*num != '\n' && ((*__ctype_b_loc ())[(int) ((*num))] & (unsigned short int) _ISspace))
    num++;
  neg = *num == '-';
  num += neg || *num == '-';
...
{
  int c;
  _Bool running_suid, running_sgid;
  void *lockstate;
  char *scorefile;
  char *end, *nl, *user, *data;
  const char *prefix, *user_prefix = ((void*)0);
  struct score_entry *scores;
  struct score_entry newscore;
  _Bool reverse = 0;
  ptrdiff_t scorecount, scorealloc;
  ptrdiff_t max_scores = (((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry)));
      ...
      case 'm':
 {
   ...
   if (optarg == end || *end || m < 0)
     usage (1);
   max_scores = ((m) < ((((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry)))) ? (m) : ((((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry)))));
 }
 break;
      default:
 usage (1);
      ...
      ptrdiff_t *count, ptrdiff_t *alloc)
{
  ...
  if (fstat (fileno (f), &st) != 0)
    return -1;
  if (! (0 <= st.st_size && st.st_size < (((9223372036854775807L)) < ((18446744073709551615UL)) ? ((9223372036854775807L)) : ((18446744073709551615UL)))))
    {
      (*__errno_location ()) = 75;
      return -1;
    }
  ...
  if (*count == *size)
    {
      ...
      if (newsize <= 0)
 newsize = 1;
      else if (newsize <= (((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry))) / 2)
 newsize *= 2;
      else if (newsize < (((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry))))
 newsize = (((9223372036854775807L)) < ((18446744073709551615UL) / sizeof (struct score_entry)) ? ((9223372036854775807L)) : ((18446744073709551615UL) / sizeof (struct score_entry)));
      else
 {
   (*__errno_location ()) = 12;
   return -1;
...
static void
sort_scores (struct score_entry *scores, ptrdiff_t count, _Bool reverse)
{
...
lock_file (const char *filename, void **state)
{
  ...
  *state = lockpath;
  ...
  while ((fd = open (lockpath, 0100 | 0200, 0600)) < 0)
    {
 ...
 return -1;
      ...
      if (5 < attempts
   || (stat (lockpath, &buf) == 0 && 60 * 60 < time (0) - buf.st_ctim.tv_sec))
 {
   if (unlink (lockpath) != 0 && (*__errno_location ()) != 2)
     return -1;
   attempts = 0;
 }
...
unlock_file (const char *filename, void *state)
{
  ...
  int unlink_errno = (*__errno_location ());
  free (lockpath);
  (*__errno_location ()) = ret < 0 ? unlink_errno : saved_errno;
  return ret;
}
