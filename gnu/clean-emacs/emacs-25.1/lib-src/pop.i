...
# 228 "pop.c"
      ...
      if (! password)
 {
  ...
  server->buffer = (char *) malloc (1024);
  if (! server->buffer)
    {
      strcpy (pop_error, "Out of memory in pop_open");
      free ((char *) server);
...
pop_stat (popserver server, int *count, int *size)
{
 ...
 snprintf (pop_error, 160, "%s", fromserver);
      else
 {
   strcpy (pop_error,
  ...
  *count = strtol (&fromserver[4], &end_ptr, 10);
  ...
  if (fromserver + 4 == end_ptr || *end_ptr != ' ' || (*__errno_location ()))
    {
  ...
  *size = strtol (fromserver + 1, &end_ptr, 10);
  if (fromserver + 1 == end_ptr || (*__errno_location ()))
    {
      strcpy (pop_error, "Unexpected response from POP server in pop_stat");
      pop_trash (server);
      return (-1);
...
pop_multi_first (popserver server, const char *command, char **response)
{
  ...
  if (0 == strncmp (*response, "-ERR", 4))
    {
      snprintf (pop_error, 160, "%s", *response);
      return (-1);
    }
...
pop_last (popserver server)
{
  ...
  if (! strncmp (fromserver, "-ERR", 4))
  ...
  else
    {
      ...
      (*__errno_location ()) = 0;
      count = strtol (&fromserver[4], &end_ptr, 10);
      if (fromserver + 4 == end_ptr || (*__errno_location ()))
 {
   strcpy (pop_error, "Unexpected response from server in pop_last");
   pop_trash (server);
   return (-1);
...
# 1028 "pop.c"
      ...
      else
 {
  ...
  if (sock < 0)
    {
      snprintf (pop_error, 160, "%s%s",
  "Could not create socket for POP connection: ", strerror ((*__errno_location ())));
      return (-1);
 ...
 if (! connect (sock, (struct sockaddr *) &addr, sizeof addr))
  ...
  connect_ok = it != ((void*)0);
  if (connect_ok)
    {
      realhost = __builtin_alloca (strlen (it->ai_canonname) + 1);
...
# 1120 "pop.c"
  if (! connect_ok)
    {
      close (sock);
      snprintf (pop_error, 160, "%s%s", "Could not connect to POP server: ", strerror ((*__errno_location ())));
      return (-1);
   ...
   if (! server->buffer)
     {
      ...
      ret = read (server->file,server->buffer + server->data,server->buffer_size - server->data - 1);

      if (ret < 0)
 {
   snprintf (pop_error, 160, "%s%s",
      "Error reading from server: ", strerror ((*__errno_location ())));
   pop_trash (server);
   return (-1);
 }
      else if (ret == 0)
...
sendline (popserver server, const char *line)
{
  ...
  char *buf;
# 1395 "pop.c"
  buf = __builtin_alloca (strlen (line) + 3);
  ...
  if (ret < 0)
    {
      pop_trash (server);
      snprintf (pop_error, 160, "%s%s", "Error writing to POP server: ", strerror ((*__errno_location ())));
      return (ret);
    }
...
fullwrite (int fd, char *buf, int nbytes)
{
  ...
  cp = buf;
  while (nbytes && ((ret = write (fd,cp,nbytes)) > 0))
    {
      cp += ret;
      nbytes -= ret;
    }
...
getok (popserver server)
{
  ...
  if (! strncmp (fromline, "+OK", 3))
    ...
    {
      snprintf (pop_error, 160, "%s", fromline);
      return (-1);
    }
  else
    {
