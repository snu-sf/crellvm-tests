...
int base = 16;
_Bool un_flag = 0, iso_flag = 0, endian = 1;
int group_by = 0x01;
char *progname;
...
# 64 "hexl.c"
      ...
      else
 {
   ...
   if (!strcmp (filename, "-"))
     fp = stdin;
   else if ((fp = fopen (filename, "r")) == ((void*)0))
     {
       perror (filename);
       continue;
     }
      ...
      if (un_flag)
 {
  ...
  break;
       ...
       for (i=0; i < 16; ++i)
  {
    if ((c = _IO_getc (fp)) == ' ' || c == (-1))
      break;

    d = _IO_getc (fp);
    c = (((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISdigit) ? c - '0' : c - 'a' + 10) * 0x10 + (((*__ctype_b_loc ())[(int) ((d))] & (unsigned short int) _ISdigit) ? d - '0' : d - 'a' + 10);
    putchar (c);
 ...
 {
   ((void) set_binary_mode (fileno (fp), 0));
   address = 0;
   string[0] = ' ';
   string[17] = '\0';
   for (;;)
