...
int he;
{
  ...
  do {} while (0);
  if (*display->d_termname == 0 || e_tgetent(tbuf, display->d_termname) != 1)
    {

      Msg(0, "Cannot find terminfo entry for '%s'.", display->d_termname);
  ...
  if ((display->d_tentry = (char *)malloc(2048 + (extra_incap ? strlen(extra_incap) + 1 : 0))) == 0)
    {
      Msg(0, "%s", strnomem);
      return -1;
    }
 ...
 case 2:
   ...
   if (display->d_tcs[i].str && *display->d_tcs[i].str == 0)
     display->d_tcs[i].str = 0;
   break;
 default:
  ...
  if ((display->d_tcs[95].flg))
    {
      ...
      if (!(display->d_tcs[60].str) && (display->d_tcs[55].str) && (InStr((display->d_tcs[55].str), "\033[m") || InStr((display->d_tcs[55].str), "\033[0m")))
 {

   (display->d_tcs[60].str) = "\033[3%p1%dm";
   (display->d_tcs[61].str) = "\033[4%p1%dm";




 }
      if ((display->d_tcs[64].str) && InStr((display->d_tcs[64].str), "\033[39;49m"))
        (display->d_tcs[67].flg) = 1;
      if ((display->d_tcs[64].str) && (InStr((display->d_tcs[64].str), "\033[m") || InStr((display->d_tcs[64].str), "\033[0m")))
        (display->d_tcs[64].str) = 0;

      if (((display->d_tcs[104].str) && InStr((display->d_tcs[104].str), "\033(B")) || ((display->d_tcs[101].str) && InStr((display->d_tcs[101].str), "\033(0")))
 (display->d_tcs[97].flg) = 1;
      if (InStr(display->d_termname, "xterm") || InStr(display->d_termname, "rxvt") ||
   ((display->d_tcs[106].str) && InStr((display->d_tcs[106].str), "\033[M")))
 ...
 (display->d_tcs[13].str) = (display->d_tcs[14].str);
  ...
  if ((display->d_tcs[58].num) > 0)
    (display->d_tcs[48].str) = (display->d_tcs[53].str) = 0;
  if ((display->d_tcs[57].num) > 0)
    (display->d_tcs[51].str) = (display->d_tcs[54].str) = 0;



  if ((display->d_tcs[58].num) > 0 && (display->d_tcs[57].num) > 0)
    (display->d_tcs[47].str) = (display->d_tcs[49].str) = (display->d_tcs[50].str) = (display->d_tcs[52].str) = (display->d_tcs[55].str) = 0;

  xue = (1<<2);
  xse = (1<<1);
  xme = (1<<0);

  if ((display->d_tcs[51].str) && (display->d_tcs[54].str) == 0)
    {
      Msg(0, "Warning: 'so' but no 'se' capability.");
      if ((display->d_tcs[55].str))
 xse = xme;
      else
 (display->d_tcs[51].str) = 0;
    }
  if ((display->d_tcs[48].str) && (display->d_tcs[53].str) == 0)
    {
      Msg(0, "Warning: 'us' but no 'ue' capability.");
      if ((display->d_tcs[55].str))
 xue = xme;
      else
 (display->d_tcs[48].str) = 0;
    }
  if (((display->d_tcs[47].str) || (display->d_tcs[49].str) || (display->d_tcs[50].str) || (display->d_tcs[52].str)) && (display->d_tcs[55].str) == 0)
    {
      Msg(0, "Warning: 'm?' but no 'me' capability.");
      (display->d_tcs[47].str) = (display->d_tcs[49].str) = (display->d_tcs[50].str) = (display->d_tcs[52].str) = 0;
    }
  ...
  if ((display->d_tcs[53].str) && (display->d_tcs[54].str) && strcmp((display->d_tcs[54].str), (display->d_tcs[53].str)) == 0)
    xse = xue;
  if ((display->d_tcs[54].str) && (display->d_tcs[55].str) && strcmp((display->d_tcs[55].str), (display->d_tcs[54].str)) == 0)
    xse = xme;
  if ((display->d_tcs[53].str) && (display->d_tcs[55].str) && strcmp((display->d_tcs[55].str), (display->d_tcs[53].str)) == 0)
    xue = xme;

  for (i = 0; i < 6; i++)
    {
      display->d_attrtab[i] = display->d_tcs[47 + i].str;
      display->d_attrtyp[i] = i == 4 ? xse : (i == 1 ? xue : xme);
    }
  ...
  t = 0;
  for (i = 0; i < 6; i++)
    if ((s = display->d_attrtab[i]))
      {
 t = display->d_attrtyp[i];
 break;
      }
  for (i = 0; i < 6; i++)
    {
      if (display->d_attrtab[i] == 0)
 {
   display->d_attrtab[i] = s;
   display->d_attrtyp[i] = t;
 }
      else
        {
  ...
  if ((display->d_tcs[60].str) || (display->d_tcs[61].str) || (display->d_tcs[62].str) || (display->d_tcs[63].str))
  ...
  if ((display->d_tcs[26].flg))
    (display->d_tcs[29].str) = (display->d_tcs[27].str) = 0;
  if ((display->d_tcs[28].str) == 0)
    (display->d_tcs[27].str) = 0;

  if ((display->d_tcs[29].str) && (display->d_tcs[27].str) && strcmp((display->d_tcs[29].str), (display->d_tcs[27].str)) == 0)
    (display->d_tcs[29].str) = 0;
  if ((display->d_tcs[70].str) == 0)
    (display->d_tcs[69].str) = 0;
  if ((display->d_tcs[94].str) == 0)
    (display->d_tcs[93].str) = 0;
  if ((display->d_tcs[82].str) == 0)
    (display->d_tcs[80].str) = (display->d_tcs[81].str) = 0;
  if ((display->d_tcs[72].str) == 0)
    (display->d_tcs[71].str) = 0;


  if ((display->d_tcs[97].flg))
    {
      if ((display->d_tcs[98].str) == 0)

        (display->d_tcs[98].str) = "\033(%p1%c";



      if ((display->d_tcs[99].str) == 0)
        (display->d_tcs[99].str) = "\033(B";
      (display->d_tcs[103].str) = 0;
      (display->d_tcs[104].str) = 0;
    }
  else if ((display->d_tcs[103].str) || ((display->d_tcs[101].str) && (display->d_tcs[102].str)))
    {
      (display->d_tcs[98].str) = ((display->d_tcs[101].str) && (display->d_tcs[102].str)) ? (display->d_tcs[101].str) : "";
      (display->d_tcs[99].str) = ((display->d_tcs[101].str) && (display->d_tcs[102].str)) ? (display->d_tcs[102].str) : "";
      (display->d_tcs[100].str) = (display->d_tcs[103].str);
    }
  else
    {
      (display->d_tcs[98].str) = (display->d_tcs[99].str) = "";
      (display->d_tcs[100].str) = 0;
      (display->d_tcs[103].str) = "";
    }

  for (i = 0; i < 256; i++)
    display->d_c0_tab[i] = i;
 ...
 display->d_c0_tab[(int)(unsigned char)s[i]] = s[i + 1];
  ...
  if ((display->d_tcs[100].str))
    for (i = (strlen((display->d_tcs[100].str)) - 2) & ~1; i >= 0; i -= 2)
      display->d_c0_tab[(int)(unsigned char)(display->d_tcs[100].str)[i]] = (display->d_tcs[100].str)[i + 1];
  do {} while (0);

  if ((display->d_tcs[74].str) == 0)
    (display->d_tcs[73].str) = 0;
  do {} while (0);


  if ((display->d_tcs[105].str))
    if (CreateTransTable((display->d_tcs[105].str)))
      return -1;



  if ((display->d_tcs[46].str) == 0)
    (display->d_tcs[45].str) = 0;
  Z0width = 132;
  Z1width = 80;

  CheckScreenSize(0);

  if ((display->d_tcs[77].str) == 0 || (display->d_tcs[78].str) == 0 || (display->d_tcs[79].str) == 0)
  ...
  if ((display->d_tcs[75].flg) && !(hardstatusemu & (1<<3)))
    display->d_has_hstatus = 3;


  if ((display->d_tcs[92].str))
    {
      int enc = FindEncoding((display->d_tcs[92].str));
      if (enc != -1)
 display->d_encoding = enc;
    }

  if (!display->d_tcs[158].str && display->d_tcs[158 + 1].str)
    display->d_tcs[158].str = display->d_tcs[158 + 1].str;
  if (!display->d_tcs[158 + 2].str && display->d_tcs[158 + 3].str)
    display->d_tcs[158 + 2].str = display->d_tcs[158 + 3].str;
  ...
  if (display->d_tcs[106].str && display->d_tcs[106 + 10].str && !strcmp(display->d_tcs[106].str, display->d_tcs[106 + 10].str))
    display->d_tcs[106].str = 0;

  if (display->d_tcs[165].str && !strcmp(display->d_tcs[165].str, "\0177"))
    display->d_tcs[165].str = 0;

  if (display->d_tcs[166 + 3].str && !strcmp(display->d_tcs[166 + 3].str, "\008"))
    display->d_tcs[166 + 3].str = 0;
  ...
  for (i = 0; i < kmap_extn; i++)
    remap(i + ((188 -106)+(188 -166)), 1);
  display->d_seqp = display->d_kmaps + 3;
  display->d_seql = 0;
  display->d_seqh = 0;


  display->d_tcinited = 1;
  MakeTermcap(0);
...
int map;
{
  ...
  a1 = 0;
  if (n >= (188 -106)+(188 -166))
    {
      kme = kmap_exts + (n - ((188 -106)+(188 -166)));
      s = kme->str;
      l = kme->fl & ~0x4000;
      fl = kme->fl & 0x4000;
      a1 = &kme->um;
 ...
 a1 = 0;
      if (a2 && a2->nr == -1)
 a2 = 0;
      if (a1 && a1->nr == 158 && a1->args[0] && strcmp(a1->args[0], s) == 0)
 a1 = 0;
      if (a2 && a2->nr == 158 && a2->args[0] && strcmp(a2->args[0], s) == 0)
 a2 = 0;
      domap |= (a1 || a2);
      if (tab == umtab)
 {
...
CheckEscape()
{
  ...
  for (display = displays; display; display = display->d_next)
    {
      for (i = 0; i < display->d_nseqs; i += display->d_kmaps[i + 2] * 2 + 4)
        {
   nr = (display->d_kmaps[i] << 8 | display->d_kmaps[i + 1]) & ~0x4000;
   if (nr < (188 -106)+(188 -166))
     {
       if (umtab[nr].nr == 35)
  break;
       if (umtab[nr].nr == -1 && dmtab[nr].nr == 35)
  break;
     }
   else
     {
       struct kmap_ext *kme = kmap_exts + nr - ((188 -106)+(188 -166));
       if (kme->um.nr == 35)
  break;
       if (kme->um.nr == -1 && kme->dm.nr == 35)
  break;
     }
        }
    }
  ...
  SetEscape((struct acluser *)0, (('a')&037), 'a');
...
unsigned char **sp;
{
  ...
  p = display->d_kmaps;
  while (p - display->d_kmaps < display->d_nseqs)
    {
      l = p[2];
      p += 3;
      for (j = 0; ; j++)
...
int nr;
{
  ...
  if (display->d_nseqs + 2 * k + 4 >= display->d_aseqs)
    {
      display->d_kmaps = (unsigned char *)xrealloc((char *)display->d_kmaps, display->d_aseqs + 256);
      display->d_aseqs += 256;
      p = display->d_kmaps + i;
    }
  display->d_seqp = display->d_kmaps + 3;
  ...
  if (j > 0)
    bcopy((char *)p, (char *)p + 2 * k + 4, display->d_nseqs - i);
  p[0] = nr >> 8;
  p[1] = nr;
  p[2] = k;
  ...
  for (q = display->d_kmaps; q < p; q += 2 * l + 4)
    {
      l = q[2];
      for (m = j = 0; j < l; j++)
 {
...
int k;
{
  ...
  if (display->d_kmaps + display->d_nseqs > p + 2 * k + 4)
    bcopy((char *)p + 2 * k + 4, (char *)p, (display->d_kmaps + display->d_nseqs) - (p + 2 * k + 4));
  display->d_nseqs -= 2 * k + 4;
  display->d_seqp = display->d_kmaps + 3;
  display->d_seql = 0;
  display->d_seqh = 0;
...
char *s;
{
  ...
  if (tcLineLen + (n = strlen(s)) > 55 && Termcaplen < 2048 - 4 - 1)
    {
      strcpy(Termcap + Termcaplen, "\\\n\t:");
      Termcaplen += 4;
      tcLineLen = 0;
    }
  if (Termcaplen + n < 2048 - 1)
    {
...
int aflag;
{
  ...
  do {} while (0);
  if ((s = getenv("SCREENCAP")) && strlen(s) < 2048)
    {
  ...
  do {} while (0);
  do {} while (0);
  if (*screenterm == '\0' || strlen(screenterm) > 768 - 3)
    {
      do {} while (0);
  ...
  do
    {
      strcpy(Term, "TERM=");
      p = Term + 5;
      if (!aflag && strlen(screenterm) + strlen(tname) < 768 -1)
 {
...
# 942 "termcap.c"
      ...
      for (cp = extra_outcap; (p = index(cp, ':')); cp = p)
 {
  ...
  do {} while (0);
  if (Termcaplen + strlen(TermcapConst) < 2048)
    {
  ...
  AddCap(buf);
  AddCap("am:");
  if (aflag || (force_vt && !(display->d_tcs[86].flg)) || (display->d_tcs[87].flg) || !(display->d_tcs[83].flg))
    {
      AddCap("xn:");
      AddCap("xv:");
      AddCap("LP:");
    }
  if (aflag || ((display->d_tcs[18].str) && (display->d_tcs[21].str)) || (display->d_tcs[22].str) || (display->d_tcs[23].str))
    {
      AddCap("sr=\\EM:");
      AddCap("al=\\E[L:");
      AddCap("AL=\\E[%dL:");
    }
  else if ((display->d_tcs[21].str))
    AddCap("sr=\\EM:");
  if (aflag || (display->d_tcs[18].str))
    AddCap("cs=\\E[%i%d;%dr:");
  if (aflag || (display->d_tcs[18].str) || (display->d_tcs[24].str) || (display->d_tcs[25].str))
    {
      AddCap("dl=\\E[M:");
      AddCap("DL=\\E[%dM:");
    }
  if (aflag || (display->d_tcs[31].str) || (display->d_tcs[32].str))
    {
      AddCap("dc=\\E[P:");
      AddCap("DC=\\E[%dP:");
    }
  if (aflag || (display->d_tcs[30].str) || (display->d_tcs[29].str) || (display->d_tcs[27].str))
    {
      AddCap("im=\\E[4h:");
      AddCap("ei=\\E[4l:");
      AddCap("mi:");
 ...
 AddCap("mr=\\E[7m:");
      if ((display->d_tcs[52].str) || (display->d_tcs[49].str) || (display->d_tcs[47].str) || (display->d_tcs[50].str))
 AddCap("me=\\E[m:ms:");
...
# 1048 "termcap.c"
      ...
      if (i < 188)
 {
   ...
   if (i >= 166 && i < 188)
     {
       act = &umtab[i - (166 - 188 + 106)];
       if (act->nr == -1)
  act = &dmtab[i - (166 - 188 + 106)];
     }
   else
     {
       act = &umtab[i - 106];
 ...
 {
 case 2:
   if (display->d_tcs[i].str == 0)
     break;
   MakeString(term[i].tcname, buf, sizeof(buf), display->d_tcs[i].str);
   AddCap(buf);
   break;
 case 0:
...
char *s;
{
  ...
  if ((display->d_xtable = (char ***)calloc(256, sizeof(char **))) == 0)
    {
      Msg(0, "%s", strnomem);
      return -1;
    }
 ...
 curchar = 0;
      ...
      templlen = 0;
      templnsub = 0;
      if (display->d_xtable[curchar] == 0)
        {
          if ((display->d_xtable[curchar] = (char **)calloc(257, sizeof(char *))) == 0)
     {
       Msg(0, "%s", strnomem);
       FreeTransTable();
       return -1;
