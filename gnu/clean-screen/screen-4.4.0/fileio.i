...
register char *str1, *str2;
{
  ...
  if (str2)
    {
      len2 = strlen(str2);
      if ((cp = realloc(str2, (unsigned) len1 + len2 + add_colon + 1)) == ((void*)0))
 Panic(0, "%s", strnomem);
      bcopy(cp, cp + len1 + add_colon, len2 + 1);
    }
  else
    {
      if ((cp = malloc((unsigned) len1 + add_colon + 1)) == ((void*)0))
 Panic(0, "%s", strnomem);
      cp[len1 + add_colon] = '\0';
    }
  bcopy(str1, cp, len1);
...
char *rcfile;
{
  ...
  if (rcfile && *rcfile == '~')
    {
      static char rcfilename_tilde_exp[4096 +1];
      ...
      if (slash_position == rcfile+1)
        {
          ...
          snprintf(rcfilename_tilde_exp, 4096, "%s/%s", home, rcfile+2);
        }
      else if (slash_position)
        {
          ...
          *slash_position = 0;
          ...
          snprintf(rcfilename_tilde_exp, 4096, "%s/%s", p->pw_dir, slash_position+1);
        }
      else
        {
      ...
      if (*rcfile != '/' && rcend && (rcend - rc_name) + strlen(rcfile) + 2 < sizeof(buf))
 {
  ...
  do {} while (0);
  if ((p = getenv("SCREENRC")) != ((void*)0) && *p != '\0')
    {
      do {} while (0);
      return SaveStr(p);
    }
...
int nopanic;
{
  ...
  if (display && (!strncmp(display->d_termname, "vt", 2) || !strncmp(display->d_termname, "xterm", 5)))
  ...
  if (rc_name == ((void*)0) || (fp = secfopen(rc_name, "r")) == ((void*)0))
    {
      ...
      if (!rc_recursion && RcFileName && !strcmp(RcFileName, rc_nonnull))
 {
   ...
   do {} while (0);
      ...
      do {} while (0);
      if (rc_name)
 {if ((rc_name) == 0) abort(); else free((void *)(rc_name)); (rc_name)=0;};
      rc_name = oldrc_name;
      return 1;
    }
 ...
 *p = '\0';
  ...
  *cp++ = '\0';
       ...
       if (p[len - 1] == '*')
  {
    if (!(len - 1) || !strncmp(p, display->d_termname, len - 1))
      break;
  }
       else if (!strcmp(p, display->d_termname))
  break;
     }
   if (!(p && *p))
     continue;
  ...
  if (rc_name == ((void*)0) || (fp = secfopen(rc_name, "r")) == ((void*)0))
    {
      const char *rc_nonnull = rc_name ? rc_name : rcfilename;
      if (rc_recursion)
        Msg((*__errno_location ()), "%s: source %s", oldrc_name, rc_nonnull);
...
int dump;
{
      ...
      if (fn == 0)
 {
   ...
   fn = fnbuf;
      ...
      if (hardcopy_append && !access(fn, 2))
 mode = "a";
      break;
      ...
      if (fn == 0)
 {
   ...
   fn = fnbuf;
      ...
      public = !strcmp(fn, "/tmp/screen-exchange");

      exists = !lstat(fn, &stb);
      if (public && exists && (((((stb.st_mode)) & 0170000) == (0120000)) || stb.st_nlink > 1))
 {
      ...
      else
 {
  ...
  {
    _IO_putc ('>', f);
    for (j = fore->w_layer.l_width - 2; j > 0; j--)
      _IO_putc ('=', f);
    fputs("<\n", f);
  }
       if (dump == 3)
  {
    ...
    for (i = 0; i < fore->w_histheight; i++)
      {
        p = (char *)(((i < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + i) % fore->w_histheight] : &fore->w_mlines[i - fore->w_histheight])->image);
        for (k = fore->w_layer.l_width - 1; k >= 0 && p[k] == ' '; k--)
   ;
        for (j = 0; j <= k; j++)
   _IO_putc (p[j], f);
        _IO_putc ('\n', f);
       ...
       for (i = 0; i < fore->w_layer.l_height; i++)
  {
    ...
    _IO_putc ('\n', f);
       ...
       break;
     case 0:
       if ((p = index(MakeTermcap(fore->w_aflag), '=')) != ((void*)0))
  {
    fputs(++p, f);
    _IO_putc ('\n', f);
  }
...
{
  if (UserContext() > 0)
    UserReturn(unlink(BufferFile) ? (*__errno_location ()) : 0);
  (*__errno_location ()) = UserStatus();
  Msg((*__errno_location ()), "%s %sremoved", BufferFile, (*__errno_location ()) ? "not " : "");
}
...
char *cmd;
{
  ...
  if (pipe(pi))
    {
      WMsg(p, (*__errno_location ()), "printing pipe");
      return -1;
    }
  switch (fork())
    {
    case -1:
      WMsg(p, (*__errno_location ()), "printing fork");
      return -1;
    case 0:
      ...
      if (setgid(real_gid) || setuid(real_uid))
        Panic((*__errno_location ()), "printpipe setuid");

      xsignal(13, ((__sighandler_t) 0));

      execl("/bin/sh", "sh", "-c", cmd, (char *)0);
      Panic((*__errno_location ()), "/bin/sh");
    default:
      break;
    }
  close(pi[0]);
