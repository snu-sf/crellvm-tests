...
struct NewWindow *def, *new, *res;
{
  ...
  res->silence = new->silence != nwin_undef.silence ? new->silence : def->silence;
  res->wrap = new->wrap != nwin_undef.wrap ? new->wrap : def->wrap;
  res->Lflag = new->Lflag != nwin_undef.Lflag ? new->Lflag : def->Lflag;
  res->slow = new->slow != nwin_undef.slow ? new->slow : def->slow;
  res->gr = new->gr != nwin_undef.gr ? new->gr : def->gr;
  res->c1 = new->c1 != nwin_undef.c1 ? new->c1 : def->c1;
  res->bce = new->bce != nwin_undef.bce ? new->bce : def->bce;
  res->encoding = new->encoding != nwin_undef.encoding ? new->encoding : def->encoding;
  ...
  res->poll_zombie_timeout = new->poll_zombie_timeout != nwin_undef.poll_zombie_timeout ? new->poll_zombie_timeout : def->poll_zombie_timeout;
...
int *lenp;
{
 ...
 if (display && fore->w_wlock == 1 &&
     !fore->w_wlockuser && !AclCheckPermWin(display->d_user, 1, fore))
   {
  ...
  if (display && ((fore->w_wlock == 0) ?
      AclCheckPermWin(display->d_user, 1, fore) :
      (display->d_user != fore->w_wlockuser)))
    {
      do {} while (0);
      do {} while (0);

      Msg(0, "write: permission denied (user %s)", display->d_user->u_name);
      *bufpp += *lenp;
      *lenp = 0;
      return;
    }
# 310 "window.c"
  if (((fore)->w_pwin && ((fore)->w_pwin->p_fdpat & 0x1000)))
    {
...
int y, from, to, isblank;
{
  ...
  if (y < 0)
    return;
  fore = (struct win *)flayer->l_data;
  if (from == 0 && y > 0 && fore->w_mlines[y - 1].image[fore->w_layer.l_width] == 0)
...
struct mchar *rend;
{
 ...
 PUTCHAR(*i++);
  ...
  if (((rend->font) && ((rend->font) & 0x60) == 0))
    return 1000;


  if (fore->w_layer.l_encoding && fore->w_layer.l_encoding != 8 && display->d_encoding == 8 && ContainsSpecialDeffont(fore->w_mlines + y, x1, x2, fore->w_layer.l_encoding))
    return 1000;
...
int bufsize;
{
  ...
  if ((w->w_log = logfopen(buf, islogfile(buf) ? ((void*)0) : secfopen(buf, "a"))) == ((void*)0))
    return -2;
...
# 624 "window.c"
  ...
  if (NewWindowAcl(p, display ? display->d_user : users))
    {
      free((char *)p);
      close(f);
      Msg(0, "%s", strnomem);
 ...
 ((p->w_lio_notify)[(i) >> 3]) |= (0x80 >> ((i) & 7));
  ...
  p->w_norefresh = 0;
  strncpy(p->w_tty, TtyName, 768 - 1);
# 743 "window.c"
  if (ChangeWindowSize(p, display ? display->d_forecv->c_xe - display->d_forecv->c_xs + 1: 80,
         display ? display->d_forecv->c_ye - display->d_forecv->c_ys + 1 : 24,
         nwin.histheight))
    {
      FreeWindow(p);
      return -1;
...
struct win *p;
{
  ...
  strncpy(p->w_tty, *TtyName ? TtyName : p->w_title, 768 - 1);
  p->w_ptyfd = f;
  p->w_readev.fd = f;
  p->w_writeev.fd = f;
  evenq(&p->w_readev);
...
char **namep;
{
  ...
  else if ((stat(arg, &st)) == 0 && ((((st.st_mode)) & 0170000) == (0020000)))
    {
      if (access(arg, 4 | 2) == -1)
 {
   Msg((*__errno_location ()), "Cannot access line '%s' for R/W", arg);
   return -1;
 }
      do {} while (0);
      ...
      if (f == -1)
 {
   ...
   return -1;
      ...
      {
 int flag = 1;
 if (ioctl(f, 0x5420, (char *)&flag))
   {
     Msg((*__errno_location ()), "TIOCPKT ioctl");
     close(f);
     return -1;
   }
      }
...
# 1164 "window.c"
  ...
  if (chown(*namep, real_uid, 5) && !eff_uid)



    {
      Msg((*__errno_location ()), "chown tty");
      close(f);
      return -1;
    }
  ...
  if (chmod(*namep, lflag ? TtyMode : (TtyMode & ~022)) && !eff_uid)
    ...
    {
      Msg((*__errno_location ()), "chmod tty");
      close(f);
      return -1;
    }
...
char **args, *ttyn;
{
  ...
  char tebuf[25];
  char ebuf[20];
  char shellbuf[7 + 4096];
  char *proc;



  int newfd;
  int w = win->w_layer.l_width;
  int h = win->w_layer.l_height;
  ...
  if (pty_preopen)
    {
      do {} while (0);
      if ((slave = open(ttyn, 02|0400)) == -1)
 {
   Msg((*__errno_location ()), "ttyn");
   return -1;
 }
    }
 ...
 Panic((*__errno_location ()), "Setuid/gid");
      eff_uid = real_uid;
      eff_gid = real_gid;
   ...
   Panic((*__errno_location ()), "Cannot chdir to %s", win->w_dir);
...
# 1312 "window.c"
      ...
      for(i = 0; i < 3; i++)
 {
       ...
       if (newfd < 0)
  {
    ...
    if (separate_sids)
      ...
      newfd = open(ttyn, 02|0400);



    if (newfd < 0)
      Panic((*__errno_location ()), "Cannot open %s", ttyn);
  }
       else
  dup(newfd);
     }
      ...
      if (wfdused)
 {
     ...
     do {} while (0);
     if (fcntl(win->w_ptyfd, 4, 0))
       Msg((*__errno_location ()), "Warning: clear NBLOCK fcntl failed");
 }
# 1374 "window.c"
      close(win->w_ptyfd);
      if (slave != -1)
      ...
      if (newfd >= 0)
 {
   ...
   if (pwin && (!(pat & 0x1000) || (pat & 0x0002 << 2)))
     {
       do {} while (0);

       modep->tio.c_lflag &= ~0000010;
       modep->tio.c_iflag &= ~0000400;
   ...
   glwz.ws_col = w;
   glwz.ws_row = h;
   (void) ioctl(newfd, 0x5414, (char *)&glwz);


   (void)fcntl(newfd, 4, 0);
 }
 ...
 proc = DefaultShell;
      ...
      do {} while (0);
      Panic((*__errno_location ()), "Cannot exec '%s'", proc);
    default:
      break;
    }
  if (slave != -1)
...
char **av;
{
  ...
  if ((w = display ? fore : windows) == ((void*)0))
    return -1;
  if (!*av || w->w_pwin)
    {
  ...
  *--t = '\0';
  ...
  if (!(pwin->p_fdpat & 0x0001))
    evdeq(&w->w_readev);

  {
    int flag = 0;

    if (ioctl(pwin->p_ptyfd, 0x5420, (char *)&flag))
      {
 Msg((*__errno_location ()), "TIOCPKT pwin ioctl");
 FreePseudowin(w);
 return -1;
      }
    if (w->w_type == 0 && !(pwin->p_fdpat & 0x0001))
      {
 if (ioctl(w->w_ptyfd, 0x5420, (char *)&flag))
   {
     Msg((*__errno_location ()), "TIOCPKT win ioctl");
     FreePseudowin(w);
     return -1;
   }
      }
  ...
  if (pwin->p_fdpat & (0x0001 << 2 * 2 | 0x0001 << 2))
    evenq(&pwin->p_readev);
...
struct win *w;
{
  ...
  do {} while (0);
  if (fcntl(w->w_ptyfd, 4, 04000))
    Msg((*__errno_location ()), "Warning: FreePseudowin: NBLOCK fcntl failed");

  if (w->w_type == 0 && !(pwin->p_fdpat & 0x0001))
    {
      int flag = 1;
      if (ioctl(w->w_ptyfd, 0x5420, (char *)&flag))
 Msg((*__errno_location ()), "Warning: FreePseudowin: TIOCPKT win ioctl");
    }
...
struct win *w;
{
  ...
  if (w->w_wlock == 1 && w->w_wlockuser == dis->d_user)
    {
      struct display *d;
...
struct event *ev;
{
  ...
  for (cv = p->w_layer.l_cvlist; cv; cv = cv->c_lnext)
    {
      display = cv->c_display;
      if (display->d_status == 1 && !display->d_status_bell)
 {
      ...
      if (display->d_obufp - display->d_obuf > display->d_obufmax + display->d_blocked_fuzz)
 {
   ...
   if (display->d_nonblock > 0 && !display->d_blockedev.queued)
     {
       do {} while (0);
...
char *data;
{
  ...
  wtop = p->w_pwin && (((p)->w_pwin->p_fdpat & 0x0003) == (0x0001 | 0x0002));
  if (wtop)
    {
      do {} while (0);
...
char *data;
{
  ...
  ptow = (((p)->w_pwin->p_fdpat & 0x0003 << 2) == (0x0001 | 0x0002) << 2 || ((p)->w_pwin->p_fdpat & 0x0003 << 2 * 2) == (0x0001 | 0x0002) << 2 * 2 );
  if (ptow)
    {
      do {} while (0);
      size = 4096 - p->w_inlen;
...
char *data;
{
 ...
 continue;
      ...
      if (!(((p->w_lio_notify)[(display->d_user->u_id) >> 3]) & (0x80 >> ((display->d_user->u_id) & 7))))
 continue;
...
int len;
{
      ...
      if (p->w_zauto > 5 || *b2 == "**\030B00"[p->w_zauto] || (p->w_zauto == 5 && *b2 == '1') || (p->w_zauto == 5 && p->w_zdisplay && *b2 == '8'))
 {
       ...
       if (i < len)
  {
    ...
    while (len-- > 0)
      do { if (--display->d_obuffree <= 0) Resize_obuf(); *display->d_obufp++ = (*bp++); } while (0);
    Flush(0);
...
int len;
{
  ...
  for (i = 0; i < len ; i++)
    if (bp[i] != 030)
      ...
      return;
  if (zmodem_mode == 3 || (zmodem_mode == 1 && p->w_type != 1))
    {
 ...
 do { if (--display->d_obuffree <= 0) Resize_obuf(); *display->d_obufp++ = (*bp++); } while (0);
  ...
  flayer = &p->w_layer;
  Input(":", 768, 0, zmodem_fin, ((void*)0), 0);
