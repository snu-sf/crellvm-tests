...
extern struct utmp *getutline(), *xpututline();
# 176 "utmp.c"
void
SlotToggle(how)
int how;
...
InitUtmp()
{
  ...
  if ((utmpfd = open(UtmpName, 02)) == -1)
    {
      if ((*__errno_location ()) != 13)
 Msg((*__errno_location ()), "%s", UtmpName);
      do {} while (0);
      utmpok = 0;
      return;
    }
...
RemoveLoginSlot()
{
  ...
  if (display->d_loginslot == (slot_t)0 || display->d_loginslot == (slot_t)-1)
    return;
  ...
  if (!utmpok)
    ...
    {
      display->d_loginslot = 0;
      do {} while (0);
    }
  else
    {
# 341 "utmp.c"
      if ((uu = getutslot(display->d_loginslot)) == 0)
 {
   do {} while (0);
   display->d_loginslot = 0;
 }
      else
 {
   ...
   u = *uu;
   makedead(&u);
   if (pututslot(display->d_loginslot, &u, (char *)0, (struct win *)0) == 0)
     display->d_loginslot = 0;
 }
              ;
    }
  ...
  if (display->d_loginslot == (slot_t)0)
    {
      ...
      if ((tty = ttyname(display->d_userfd)) && stat(tty, &stb) == 0 && (int)stb.st_uid == real_uid && !CheckTtyname(tty) && ((int)stb.st_mode & 0777) != 0666)
 {
   display->d_loginttymode = (int)stb.st_mode & 0777;
   chmod(display->d_usertty, stb.st_mode & 0600);
 }
    }
}
...
RestoreLoginSlot()
{
  ...
  do {} while (0);
  do {} while (0);
  if (utmpok && display->d_loginslot != (slot_t)0 && display->d_loginslot != (slot_t)-1)
    {
      do {} while (0);
      if (pututslot(display->d_loginslot, &display->d_utmp_logintty, display->d_utmp_logintty.ut_host, (struct win *)0) == 0)
        Msg((*__errno_location ()),"Could not write %s", UtmpName);
    }
          ;
  display->d_loginslot = (slot_t)0;
  if (display->d_loginttymode && (tty = ttyname(display->d_userfd)) && !CheckTtyname(tty))
    chmod(tty, display->d_loginttymode);
}
# 405 "utmp.c"
int
SetUtmp(wi)
struct win *wi;
{
  ...
  char *p;
  char host[sizeof(display->d_utmp_logintty.ut_host) + 15];




  wi->w_slot = (slot_t)0;
  if (!utmpok || wi->w_type != 0)
    return -1;
  ...
  if (display)
    {
      strncpy(host, display->d_utmp_logintty.ut_host, sizeof(host) - 15);
      if (display->d_loginslot != (slot_t)0 && display->d_loginslot != (slot_t)-1 && host[0] != '\0')
 {
# 452 "utmp.c"
   for (p = host; *p; p++)
     if ((*p < '0' || *p > '9') && (*p != '.'))
 ...
 {
   strncpy(host + 1, stripdev(display->d_usertty), sizeof(host) - 15 - 1);
   host[0] = ':';
 }
    }
  else
