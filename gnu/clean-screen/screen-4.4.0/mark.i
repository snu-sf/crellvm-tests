...
int y;
{
  ...
  for (x = markdata->left_mar, i = ((y < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + y) % fore->w_histheight] : &fore->w_mlines[y - fore->w_histheight])->image + x; x < fore->w_layer.l_width - 1; x++)
    if (*i++ != ' ')
      break;
...
int y;
{
  ...
  for (x = markdata->right_mar, i = ((y < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + y) % fore->w_histheight] : &fore->w_mlines[y - fore->w_histheight])->image + x; x >= 0; x--)
    if (*i-- != ' ')
      break;
  if (x < 0)
    x = markdata->left_mar;
...
nextchar(int *xp, int *yp, int direction, char target, int num)
{
  ...
  displayed_line = (char *)((*yp < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + *yp) % fore->w_histheight] : &fore->w_mlines[*yp - fore->w_histheight]) -> image;
...
int *xp, *yp, flags, num;
{
  int xx = fore->w_layer.l_width, yy = fore->w_histheight + fore->w_layer.l_height;
  ...
  if ((flags & (1<<1)) && (flags & (1<<2)))
    x += sx;
  ml = ((y < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + y) % fore->w_histheight] : &fore->w_mlines[y - fore->w_histheight]);
  for (oq = -1; ; x += sx, oq = q)
    {
      if (x >= xx || x < 0)
 q = 0;
...
char *pt;
{
 ...
 break;
      ml = ((i < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + i) % fore->w_histheight] : &fore->w_mlines[i - fore->w_histheight]);
      from = (i == y1) ? x1 : 0;
      if (from < markdata->left_mar)
 from = markdata->left_mar;
      for (to = fore->w_layer.l_width, im = ml->image + to; to >= 0; to--)
        if (*im-- != ' ')
   break;
      if (i == y2 && x2 < to)
 to = x2;
 ...
 continue;
      ...
      if (((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[j] == 0xff && (unsigned char)(ml)->image[j] == 0xff : ((unsigned char)(ml)->font[j] & 0xe0) == 0x80 ))
 j--;
      ...
      if (pastefont && font != 0)
 {
   ...
   l += 3;
      ...
      if (i != y2 && (to != fore->w_layer.l_width - 1 || ml->image[to + 1] == ' '))
 {
...
GetHistory()
{
  ...
  for (yy = y - 1; yy >= 0; yy--)
    {
      ml = ((yy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + yy) % fore->w_histheight] : &fore->w_mlines[yy - fore->w_histheight]);
      linep = ml->image;
      if (xx < 0 || eq(linep[xx], q))
 {
   for (i = fore->w_layer.l_width - 1, linep += i; i >= x; i--)
     if (*linep-- != ' ')
       break;
   if (i >= x)
     break;
  ...
  if ((display->d_user->u_plop.buf = (char *)malloc((unsigned) (i - x + 2))) == ((void*)0))
    {
      LMsg(0, "Not enough memory... Sorry.");
      return 0;
    }
  bcopy((char *)linep - i + x + 1, display->d_user->u_plop.buf, i - x + 1);
...
MarkRoutine()
{
  ...
  LMsg(0, "Copy mode - Column %d Line %d(+%d) (%d,%d)",
      x + 1, ((y + 1) - markdata->hist_offset), fore->w_histheight, fore->w_layer.l_width, fore->w_layer.l_height);
...
# 601 "mark.c"
 ...
 if (((*__ctype_b_loc ())[(int) ((od))] & (unsigned short int) _ISgraph)) {
...
# 635 "mark.c"
 ...
 case '\025':
   ...
   break;
 case '\007':
   if (markdata->left_mar == 0 && markdata->right_mar == fore->w_layer.l_width - 1)
     LMsg(0, "Column %d Line %d(+%d)", cx+1, ((cy) - markdata->hist_offset)+1,
  markdata->hist_offset);
   else
     LMsg(0, "Column %d(%d..%d) Line %d(+%d)", cx+1,
  markdata->left_mar+1, markdata->right_mar+1, ((cy) - markdata->hist_offset)+1, markdata->hist_offset);
   break;
 case '\002':
   if (rep_cnt == 0)
     rep_cnt = 1;
 ...
 case '\005':
   ...
   if (cy < ((0) + markdata->hist_offset))
            revto(cx, ((0) + markdata->hist_offset));
   else
            LGotoPos(flayer, cx, ((cy) - markdata->hist_offset));
   break;
 case '\031':
 ...
 case '%':
   ...
   if (rep_cnt > 100)
     rep_cnt = 100;
   revto_line(markdata->left_mar, (rep_cnt * (fore->w_histheight + fore->w_layer.l_height)) / 100, (fore->w_layer.l_height - 1) / 2);
   break;
 case 0201:
 case 'g':
   rep_cnt = 1;
 ...
 case 'G':
   ...
   break;
 case 'H':
   revto(markdata->left_mar, ((0) + markdata->hist_offset));
   break;
 case 'M':
 ...
 case 'E':
   if (rep_cnt == 0)
     rep_cnt = 1;
   nextword(&cx, &cy, (1<<1)|(1<<2) | (od == 'E' ? (1<<3) : 0), rep_cnt);
   revto(cx, cy);
   break;
 case 'b':
 case 'B':
   if (rep_cnt == 0)
     rep_cnt = 1;
   nextword(&cx, &cy, (1<<0)|(1<<1)|(1<<2) | (od == 'B' ? (1<<3) : 0), rep_cnt);
   revto(cx, cy);
   break;
 case 'a':
 ...
 case '\r':
   if (!markdata->second)
     {
       ...
       markdata->y1 = cy;
       revto(cx, cy);
       LMsg(0, "First mark set - Column %d Line %d", cx+1, ((cy) - markdata->hist_offset)+1);
       break;
     }
   else
     {
  ...
  {
    do { struct layer *oldlay = flayer; struct canvas *oldcvlist, *cv; do {} while (0); flayer = flayer->l_next; oldcvlist = flayer->l_cvlist; do {} while (0); flayer->l_cvlist = oldlay->l_cvlist; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; LRefreshAll(flayer, 0); flayer = oldlay; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; flayer->l_next->l_cvlist = oldcvlist; } while(0);
  }
       ExitOverlayPage();
 ...
 case 0222:
       ...
       else if (button == ' ')
  {
    ...
    cy = ((flayer->l_mouseevent.buffer[2]) + markdata->hist_offset);
    revto(cx, cy);
    od = ' ';
  }
       else
...
int tx, ty, line;
{
  ...
  int ystart = 0, yend = fore->w_layer.l_height-1;
  int i, ry;
  unsigned char *wi;
  struct mline *ml;
  struct mchar mc;
  ...
  if (tx < 0)
    tx = 0;
  else if (tx > fore->w_layer.l_width - 1)
    tx = fore->w_layer.l_width -1;
  if (ty < 0)
    ty = 0;
  else if (ty > fore->w_histheight + fore->w_layer.l_height - 1)
  ...
  ml = ((ty < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + ty) % fore->w_histheight] : &fore->w_mlines[ty - fore->w_histheight]);
  if (ty == fy && fx + 1 == tx && ((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[tx] == 0xff && (unsigned char)(ml)->image[tx] == 0xff : ((unsigned char)(ml)->font[tx] & 0xe0) == 0x80 ) && tx < display->d_width - 1)
    tx++;
  if (ty == fy && fx - 1 == tx && ((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[fx] == 0xff && (unsigned char)(ml)->image[fx] == 0xff : ((unsigned char)(ml)->font[fx] & 0xe0) == 0x80 ) && tx)
    tx--;
  ...
  i = 0;
  if (line >= 0 && line < fore->w_layer.l_height)
    i = ((ty) - markdata->hist_offset) - line;
  else if (ty < markdata->hist_offset)
    i = ty - markdata->hist_offset;
  else if (ty > markdata->hist_offset + (fore->w_layer.l_height - 1))
      ...
      if (t == st || x == 0)
 {
   ...
   for (ce = fore->w_layer.l_width; ce >= 0; ce--, wi--)
     if (*wi != ' ')
       break;
 }
      if (x <= ce && x >= markdata->left_mar && x <= markdata->right_mar)
 {
   ...
   if (((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[x] == 0xff && (unsigned char)(ml)->image[x] == 0xff : ((unsigned char)(ml)->font[x] & 0xe0) == 0x80 ))
       {
  if (t == revst)
    revst--;
         t--;
       ...
       if (pastefont)
  {
   ...
   if (((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[(x) + 1] == 0xff && (unsigned char)(ml)->image[(x) + 1] == 0xff : ((unsigned char)(ml)->font[x] & 0x1f) != 0 && ((unsigned char)(ml)->font[x] & 0xe0) == 0 ))
     {
...
int isblank;
{
  ...
  ml = ((wy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + wy) % fore->w_histheight] : &fore->w_mlines[wy - fore->w_histheight]);

  if (markdata->second == 0)
    {
      if (((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[xs] == 0xff && (unsigned char)(ml)->image[xs] == 0xff : ((unsigned char)(ml)->font[xs] & 0xe0) == 0x80 ) && xs > 0)
 xs--;
      if (((fore->w_layer.l_encoding == 8) ? (unsigned char)(ml)->font[(xe) + 1] == 0xff && (unsigned char)(ml)->image[(xe) + 1] == 0xff : ((unsigned char)(ml)->font[xe] & 0x1f) != 0 && ((unsigned char)(ml)->font[xe] & 0xe0) == 0 ) && xe < fore->w_layer.l_width - 1)
 xe++;
      if (xs == 0 && y > 0 && wy > 0 && ((wy - 1 < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + wy - 1) % fore->w_histheight] : &fore->w_mlines[wy - 1 - fore->w_histheight])->image[flayer->l_width] == 0)
  ...
  for (x = fore->w_layer.l_width, wi = ml->image + fore->w_layer.l_width; x >= 0; x--, wi--)
    if (*wi != ' ')
      break;
  if (x < rm)
    rm = x;
...
struct mchar *rend;
{
  ...
  if (fore->w_layer.l_encoding && fore->w_layer.l_encoding != 8 && display->d_encoding == 8 && ContainsSpecialDeffont(ml, xs, xe, fore->w_layer.l_encoding))
    return 1000;
  ...
  for (rm = fore->w_layer.l_width, i = ml->image + fore->w_layer.l_width; rm >= 0; rm--)
    if (*i-- != ' ')
      break;
  ...
  while (dx--)
   ...
   if (!( (rend)->image == (&mchar_marked)->image && (rend)->attr == (&mchar_marked)->attr && (rend)->font == (&mchar_marked)->font && (rend)->fontx == (&mchar_marked)->fontx && (rend)->color == (&mchar_marked)->color ))
     return 1000;
        }
      else
        {
   rend->image = ml->image[x];
   if (!( (rend)->image == (ml)->image[x] && (rend)->attr == (ml)->attr[x] && (rend)->font == (ml)->font[x] && (rend)->fontx == (ml)->fontx[x] && (rend)->color == (ml)->color[x] ))
     return 1000;
        }
      x++;
    }
