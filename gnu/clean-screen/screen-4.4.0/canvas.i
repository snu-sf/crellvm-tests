...
struct canvas *cv;
{
 ...
 display->d_forecv = 0;
      ...
      for (cvp = &display->d_cvlist; *cvp ; cvp = &(*cvp)->c_next)
 if (*cvp == cv)
   {
     *cvp = cv->c_next;
     break;
  ...
  p = cv->c_layer ? ((struct win *)(cv->c_layer)->l_bottom->l_data) : 0;
...
int x, y;
{
      ...
      if (x >= display->d_forecv->c_xs && x <= display->d_forecv->c_xe)
 {
   if (x < cv->c_xs || x > cv->c_xe)
     continue;
          if (y < display->d_forecv->c_ys && y < cv->c_ys)
     continue;
          if (y > display->d_forecv->c_ye + 1 && y > cv->c_ye + 1)
     continue;
   if (y < cv->c_ys)
     m = cv->c_ys - y;
   if (y > cv->c_ye + 1)
     m = y - (cv->c_ye + 1);
      ...
      if (y >= display->d_forecv->c_ys && y <= display->d_forecv->c_ye + 1)
 {
   if (y < cv->c_ys || y > cv->c_ye + 1)
     continue;
          if (x < display->d_forecv->c_xs && x < cv->c_xs)
     continue;
          if (x > display->d_forecv->c_xe && x > cv->c_xe)
     continue;
   if (x < cv->c_xs)
     m = cv->c_xs - x;
   if (x > cv->c_xe)
      ...
      if (m && (!mm || m < mm))
 {
  ...
  return mcv ? mcv : display->d_forecv;
}
...
struct canvas *cv;
{
  ...
  if (focusminwidth || focusminheight)
   ...
   if (cv2->c_slback == cv->c_slback)
     {
       fcv = cv2;
       focusmin = cv->c_slorient == (1 << 0) ? focusminheight : focusminwidth;
       if (focusmin > 0)
  focusmin--;
       else if (focusmin < 0)
  focusmin = cv->c_slorient == (1 << 0) ? ye - ys + 2 : xe - xs + 2;
       do {} while (0);
     }
          cv2 = cv2->c_slback;
        }
  ...
  if (focusmin)
    {
      m = CountCanvas(cv) * 2;
      nh = cv->c_slorient == (1 << 0) ? ye - ys + 2 : xe - xs + 2;
      nh -= m;
      if (nh < 0)
 nh = 0;
      if (focusmin > nh)
 ...
 m += focusmin;
  ...
  nh = cv->c_slorient == (1 << 0) ? ye - ys + 2 : xe - xs + 2;
  i = cv->c_slorient == (1 << 0) ? ys : xs;
  maxi = cv->c_slorient == (1 << 0) ? ye : xe;
  w = wsum;
  for (; cv; cv = cvn)
    {
      cvn = cv->c_slnext;
...
int orient;
{
  ...
  if (cv->c_slorient != 0 && cv->c_slorient != orient)
    if (!AddPerp(cv))
      return -1;
  ...
  ys = cv->c_slback->c_ys;
  ye = cv->c_slback->c_ye;
  if (!captionalways && cv == display->d_canvas.c_slperp && !cv->c_slnext)
    ye--;
  do {} while (0);
  do {} while (0);
...
RemCanvas()
{
  ...
  cv = display->d_forecv;
  while (display->d_forecv->c_slperp)
...
OneCanvas()
{
  struct canvas *cv = display->d_forecv, *ocv = 0;
...
int save;
{
 ...
 {
   struct win *p = cvf->c_layer ? ((struct win *)(cvf->c_layer)->l_bottom->l_data) : 0;
