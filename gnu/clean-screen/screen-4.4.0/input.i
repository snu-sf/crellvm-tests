...
char *p, *s;
{
 ...
 strncpy(inpdata->inp.buf, s, sizeof(inpdata->inp.buf) - 1);
  ...
  flayer->l_x = inpdata->inpstringlen + (inpdata->inpmode & 1 ? 0 : inpdata->inp.pos);
...
int *plen;
{
  ...
  LGotoPos(flayer, inpdata->inpstringlen + (inpdata->inpmode & 1 ? 0 : inpdata->inp.pos), (flayer->l_height - 1));
  if (ppbuf == 0)
    {
      InpAbort();
      return;
      ...
      if (((unsigned char)ch & 0177) >= ' ' && ch != 0177 && inpdata->inp.len < inpdata->inpmaxlen)
 {
       ...
       if (p < inpdata->inp.buf+inpdata->inp.len)
  {
   ...
   do { if (inpdata->search) {if ((inpdata->search) == 0) abort(); else free((void *)(inpdata->search)); (inpdata->search)=0;}; } while (0);
 }
      ...
      else if (ch == '\025')
 {
   ...
   if (inpdata->inp.len && !(inpdata->inpmode & 1))
     {
       LClearArea(flayer, x, (flayer->l_height - 1), x + inpdata->inp.len - 1, (flayer->l_height - 1), 0, 0);
       LGotoPos(flayer, x, (flayer->l_height - 1));
     }
 ...
 {
   x = inpdata->inpstringlen + inpdata->inp.pos;
   if (inpdata->inp.len > inpdata->inp.pos && !(inpdata->inpmode & 1))
     {
       LClearArea(flayer, x, (flayer->l_height - 1), x + inpdata->inp.len - inpdata->inp.pos - 1, (flayer->l_height - 1), 0, 0);
       LGotoPos(flayer, x, (flayer->l_height - 1));
     }
   inpdata->inp.len = inpdata->inp.pos;
 }
      else if (ch == '\027' && inpdata->inp.pos > 0)
 {
   char *oldp = p--;
   ...
   do { if (inpdata->search) {if ((inpdata->search) == 0) abort(); else free((void *)(inpdata->search)); (inpdata->search)=0;}; } while (0);
      ...
      else if (ch == '\004' && inpdata->inp.pos < inpdata->inp.len)
 {
 ...
 {
   LGotoPos(flayer, x -= inpdata->inp.pos, (flayer->l_height - 1));
   inpdata->inp.pos = 0;
 }
 ...
 {
   LGotoPos(flayer, x += inpdata->inp.len - inpdata->inp.pos, (flayer->l_height - 1));
      ...
      else if ((ch == '\006' || (unsigned char)ch == 0206) && inpdata->inp.pos < inpdata->inp.len)
 {
   ...
   inpdata->inp.pos++;
      ...
      else if ((prev = ((ch == '\020' || (unsigned char)ch == 0220) &&
       inpdata->inp.prev)) ||
   (next = ((ch == '\016' || (unsigned char)ch == 0216) &&
     inpdata->inp.next)) ||
   (search = ((ch == '\022' || (unsigned char)ch == 0222) && inpdata->inp.prev)))
 {
  ...
  continue;
   ...
   if (inpdata->inp.len && !(inpdata->inpmode & 1))
     LClearArea(flayer, inpdata->inpstringlen, (flayer->l_height - 1), inpdata->inpstringlen + inpdata->inp.len - 1, (flayer->l_height - 1), 0, 0);
        ...
        ch == '\000' || ch == '\n' || ch == '\r')
 {
   ...
   if (inpdata->inp.len && !(inpdata->inpmode & (1 | 2)))
     {
...
InpAbort()
{
  do { struct layer *oldlay = flayer; struct canvas *oldcvlist, *cv; do {} while (0); flayer = flayer->l_next; oldcvlist = flayer->l_cvlist; do {} while (0); flayer->l_cvlist = oldlay->l_cvlist; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; (*flayer->l_layfn->lf_LayRedisplayLine)((flayer->l_height - 1), 0, flayer->l_width - 1, 0); flayer = oldlay; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; flayer->l_next->l_cvlist = oldcvlist; } while(0);
  ExitOverlayPage();
}
...
int y, xs, xe, isblank;
{
  ...
  if (y != (flayer->l_height - 1))
    {
      do { struct layer *oldlay = flayer; struct canvas *oldcvlist, *cv; do {} while (0); flayer = flayer->l_next; oldcvlist = flayer->l_cvlist; do {} while (0); flayer->l_cvlist = oldlay->l_cvlist; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; (*flayer->l_layfn->lf_LayRedisplayLine)(y, xs, xe, isblank); flayer = oldlay; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; flayer->l_next->l_cvlist = oldcvlist; } while(0);
      return;
    }
  inpdata->inp.buf[inpdata->inp.len] = 0;
  q = xs;
