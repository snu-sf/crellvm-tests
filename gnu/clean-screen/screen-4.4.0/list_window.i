...
gl_Window_add_group(struct ListData *ldata, struct ListRow *row)
{
  ...
  do { if ((wdata)->order == 1) { struct win *_ww; for (_ww = windows; _ww; _ww = _ww->w_next) { w = _ww; if (w->w_group != group) continue; cur = glist_add_row(ldata, w, cur); if (w == wdata->fore) ldata->selected = cur; if (w->w_type == 3) cur = gl_Window_add_group(ldata, cur); } } else { struct win **_ww, *_witer; for (_ww = wtab, _witer = windows; _witer && _ww - wtab < maxwin; _ww++) { if (!(w = *_ww)) continue; if (w->w_group != group) continue; cur = glist_add_row(ldata, w, cur); if (w == wdata->fore) ldata->selected = cur; if (w->w_type == 3) cur = gl_Window_add_group(ldata, cur); _witer = _witer->w_next; } } } while (0);
# 150 "list_window.c"
  return cur;
}
...
{
  struct ListRow *row = ((void*)0);
  struct gl_Window_Data *wdata = ldata->data;
  struct win *w;

  do { if ((wdata)->order == 1) { struct win *_ww; for (_ww = windows; _ww; _ww = _ww->w_next) { w = _ww; if (w->w_group != wdata->group) continue; row = glist_add_row(ldata, w, row); if (w == wdata->fore) ldata->selected = row; if (w->w_type == 3 && wdata->nested) row = gl_Window_add_group(ldata, row); } } else { struct win **_ww, *_witer; for (_ww = wtab, _witer = windows; _witer && _ww - wtab < maxwin; _ww++) { if (!(w = *_ww)) continue; if (w->w_group != wdata->group) continue; row = glist_add_row(ldata, w, row); if (w == wdata->fore) ldata->selected = row; if (w->w_type == 3 && wdata->nested) row = gl_Window_add_group(ldata, row); _witer = _witer->w_next; } } } while (0);
# 169 "list_window.c"
...
gl_Window_row(struct ListData *ldata, struct ListRow *lrow)
{
  ...
  for (xoff = 0, g = w->w_group; g != wdata->group; g = g->w_group)
    xoff += 2;
  display = ((struct win *)(flayer)->l_bottom->l_data) ? 0 : flayer->l_cvlist ? flayer->l_cvlist->c_display : 0;
  str = MakeWinMsgEv(wliststr, w, '%', flayer->l_width - xoff, ((void*)0), 0);
  if (ldata->selected == lrow)
    mchar = &mchar_so;
  else if (w->w_monitor == 3 && renditions[REND_MONITOR] != -1)
    {
      mchar = &mchar_rend;
      ApplyAttrColor(renditions[REND_MONITOR], mchar);
    }
  else if ((w->w_bell == 2 || w->w_bell == 1) && renditions[REND_BELL] != -1)
    {
      mchar = &mchar_rend;
      ApplyAttrColor(renditions[REND_BELL], mchar);
    }
  else if ((w->w_silence == 2 || w->w_silence == 3) && renditions[REND_SILENCE] != -1)
    {
      mchar = &mchar_rend;
...
gl_Window_input(struct ListData *ldata, char **inp, int *len)
{
 ...
 break;
      ...
      if (display && AclCheckPermWin(display->d_user, 2, win))
 return 0;
      ...
      else
 {
   ...
   if (display->d_fore != win)
    ...
    case 'a':
      ...
      if (wdata->group)
 {
   int order = wdata->order | (wdata->nested ? 2 : 0);
   glist_abort();
   display = cd;
   display_windows(1, order, ((void*)0));
   *len = 0;
 }
      ...
      else
 {
   ...
   *len = 0;
    ...
    case ',':
      if (wdata->order == 0 && ldata->selected->prev)
 {
   ...
   WindowChangeNumber(win->w_number, pw->w_number);
    ...
    case '.':
      if (wdata->order == 0 && ldata->selected->next)
 {
 ...
 Input(str, 1, 2, window_kill_confirm, (char *)win, 0);
      }
      break;
...
{
  {if ((ldata->data) == 0) abort(); else free((void *)(ldata->data)); (ldata->data)=0;};
  return 0;
}
...
WListUpdate(struct win *p, struct ListData *ldata)
{
  ...
  if (!p)
    {
      if (ldata->selected)
 wdata->fore = ldata->selected->data;
      glist_remove_rows(ldata);
      gl_Window_rebuild(ldata);
      return;
    }
  ...
  d = 1;
  if (wdata->order == 0 || wdata->order == 1)
    {
      ...
      if (p->w_number != 0)
 {
  ...
  else if (wdata->nested && p->w_group)
...
struct win *p;
{
  ...
  if (ldata->name != ListID)
    return;
  wdata = ldata->data;
  { struct display *olddisplay = display; struct layer *oldflayer = flayer; struct layer *l = cv->c_layer; struct canvas *cvlist = l->l_cvlist; struct canvas *cvlnext = cv->c_lnext; flayer = l; l->l_cvlist = cv; cv->c_lnext = 0; WListUpdate(p, ldata); flayer = oldflayer; l->l_cvlist = cvlist; cv->c_lnext = cvlnext; display = olddisplay; };
}
...
WListLinkChanged()
{
 ...
 if (!(wdata->order & 1))
   continue;
        { struct display *olddisplay = display; struct layer *oldflayer = flayer; struct layer *l = cv->c_layer; struct canvas *cvlist = l->l_cvlist; struct canvas *cvlnext = cv->c_lnext; flayer = l; l->l_cvlist = cv; cv->c_lnext = 0; WListUpdate(0, ldata); flayer = oldflayer; l->l_cvlist = cvlist; cv->c_lnext = cvlnext; display = olddisplay; };
      }
  display = olddisplay;
}
