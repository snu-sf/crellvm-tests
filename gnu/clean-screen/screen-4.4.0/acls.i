...
# 25 "/usr/include/shadow.h" 3 4
# 1 "/usr/include/paths.h" 1 3 4
# 26 "/usr/include/shadow.h" 2 3 4


# 1 "/usr/include/stdio.h" 1 3 4
# 29 "/usr/include/shadow.h" 2 3 4
...
# 175 "/usr/include/sys/syslog.h" 3 4
...
int len, delta, defaultbit;
{
  ...
  if (!(n = (AclBits)calloc(1, (unsigned long)(&(((char *)0)[(len + delta + 1) >> 3])))))
    return -1;
  for (i = 0; i < (len + delta); i++)
    {
      if (((i < len) && ((0x80 >> ((i) & 7)) & ((o)[(i) >> 3]))) ||
          ((i >= len) && (defaultbit)))
 ((n)[(i) >> 3]) |= (0x80 >> ((i) & 7));
    }
  if (len)
...
struct acluser **up;
{
  ...
  strncpy((*up)->u_name, name, 256);
  (*up)->u_password = ((void*)0);
  if (pass)
    (*up)->u_password = SaveStr(pass);
  if (!(*up)->u_password)
  ...
  for ((*up)->u_id = 0; (*up)->u_id < maxusercount; (*up)->u_id++)
    if (!((0x80 >> (((*up)->u_id) & 7)) & ((userbits)[((*up)->u_id) >> 3])))
      break;
  do {} while (0);
  if ((*up)->u_id == maxusercount)
    {
      ...
      if (GrowBitfield(&userbits, maxusercount, 8, 0))
        {
   free((char *)*up); *up = ((void*)0); return -1;
 }
# 225 "acls.c"
      for (j = 0; j <= 187; j++)
        {
   ...
   for (i = 0; i < 1; i++)
     if (GrowBitfield(&comms[j].userbits[i], maxusercount, 8,
         default_c_bit[i]))
       {
         free((char *)*up); *up = ((void*)0); return -1;
       }
   ...
   for (j = 0; j < 3; j++)
     {
       if (GrowBitfield(&u->u_umask_w_bits[j], maxusercount, 8,
    default_w_bit[j]))
  {
    free((char *)*up); *up = ((void*)0); return -1;
  }
      ...
      for (w = windows; w; w = w->w_next)
 {
   ...
   for (j = 0; j < 3; j++)
     if (GrowBitfield(&w->w_userbits[j], maxusercount, 8,
  default_w_bit[j]))
       {
  free((char *)*up); *up = ((void*)0); return -1;
       }
   ...
   if (GrowBitfield(&w->w_mon_notify, maxusercount, 8, 0) ||
       GrowBitfield(&w->w_lio_notify, maxusercount, 8, 0))
     {
       free((char *)*up); *up = ((void*)0); return -1;
     }
 }
  ...
  ((userbits)[((*up)->u_id) >> 3]) |= (0x80 >> (((*up)->u_id) & 7));


  if ((*up)->u_id == 0)
    AclSetPerm(((void*)0), *up, "+a", "#?");


  if (!strcmp((*up)->u_name, "nobody"))
    {
      AclSetPerm(((void*)0), *up, "-rwx", "#?");
      AclSetPerm(((void*)0), *up, "+x", "su");
      AclSetPerm(((void*)0), *up, "+x", "detach");
      AclSetPerm(((void*)0), *up, "+x", "displays");
      AclSetPerm(((void*)0), *up, "+x", "version");
    }
  ...
  for (j = 0; j < 3; j++)
    {
      ...
      (((*up)->u_umask_w_bits[j])[((*up)->u_id) >> 3]) |= (0x80 >> (((*up)->u_id) & 7));
    }
...
struct acluser **up;
{
  ...
  *up = u->u_next;
  ...
  ((userbits)[(u->u_id) >> 3]) &= ~(0x80 >> ((u->u_id) & 7));

  AclSetPerm(((void*)0), u, default_w_bit[2] ? "+r" : "-r", "#");
  AclSetPerm(((void*)0), u, default_w_bit[1]? "+w" : "-w", "#");
  AclSetPerm(((void*)0), u, default_w_bit[0] ? "+x" : "-x", "#");
  AclSetPerm(((void*)0), u, default_c_bit[0] ? "+x" : "-x", "?");
  for (i = 0; i < 3; i++)
...
char *name, *pw1, *pw2;
{
      ...
      if (pw1 && *pw1 && *pw1 != '\377')
 {
  ...
  openlog("screen", 0x01, (4<<3));

  syslog(5, "%s: \"su %s\" %s for \"%s\"", SockPath, name,
      ...
      if (GrowBitfield(&w->w_userbits[j], 0, maxusercount, 0))
 {
   ...
   return -1;
      ...
      for (i = 0; i < maxusercount; i++)
        if (u ? ((0x80 >> ((i) & 7)) & ((u->u_umask_w_bits[j])[(i) >> 3])) :
         default_w_bit[j])
   ((w->w_userbits[j])[(i) >> 3]) |= (0x80 >> ((i) & 7));
    }
  return 0;
}
...
struct comm *cmd;
{
 ...
 case '-':
   ...
   if (neg)
     ((cmd->userbits[0])[(u->u_id) >> 3]) &= ~(0x80 >> ((u->u_id) & 7));
   else
     ((cmd->userbits[0])[(u->u_id) >> 3]) |= (0x80 >> ((u->u_id) & 7));
   break;
        case 'r':
...
struct win *win;
{
      ...
      for (bit = 0; bit < 3; bit++)
 {
   if (!(bits & (1 << bit)))
     continue;
   if (neg)
     ((bitarray[bit])[(u->u_id) >> 3]) &= ~(0x80 >> ((u->u_id) & 7));
   else
     ((bitarray[bit])[(u->u_id) >> 3]) |= (0x80 >> ((u->u_id) & 7));
   if (!uu && (win->w_wlockuser == u) && neg && (bit == 1))
     {
       do {} while (0);
       win->w_wlockuser = ((void*)0);
       if (win->w_wlock == 2)
  win->w_wlock = 1;
     }
 }
  ...
  if (uu && u->u_name[0] == '?' && u->u_name[1] == '\0')
   ...
   for (bit = 0; bit < 3; bit++)
     default_w_bit[bit] =
       (((bitarray[bit])[(u->u_id) >> 3]) & (0x80 >> ((u->u_id) & 7))) ? 1 : 0;
 }
      else
 {
...
char **argv;
{
  ...
  switch (argc)
    ...
    case 1+2:
      do {} while (0);
      return (UserAdd(argv[0], ((void*)0), u) < 0) ||
  ...
  for (w = windows; w; w = w->w_next)
    {
      for (i = 0; i < 3; i++)
        {
   if (((w->w_userbits[i])[(from_id) >> 3]) & (0x80 >> ((from_id) & 7)))
     ((w->w_userbits[i])[(to_id) >> 3]) |= (0x80 >> ((to_id) & 7));
   else
     {
       ((w->w_userbits[i])[(to_id) >> 3]) &= ~(0x80 >> ((to_id) & 7));
       if ((w->w_wlockuser == *to_up) && (i == 1))
  {
    do {} while (0);
  ...
  for (j = 0; j <= 187; j++)
    {
      for (i = 0; i < 1; i++)
        {
   if (((comms[j].userbits[i])[(from_id) >> 3]) & (0x80 >> ((from_id) & 7)))
     ((comms[j].userbits[i])[(to_id) >> 3]) |= (0x80 >> ((to_id) & 7));
   else
     ((comms[j].userbits[i])[(to_id) >> 3]) &= ~(0x80 >> ((to_id) & 7));
 }
    }
...
struct acluser *EffectiveAclUser = ((void*)0);
...
struct win *w;
{
  ...
  ok = ((w->w_userbits[mode])[(u->u_id) >> 3]) & (0x80 >> ((u->u_id) & 7));
  do {} while (0);
...
struct comm *c;
{
  ...
  ok = ((c->userbits[mode])[(u->u_id) >> 3]) & (0x80 >> ((u->u_id) & 7));
  do {} while (0);
  if (!ok)
    {
      struct aclusergroup **g = &u->u_group;
