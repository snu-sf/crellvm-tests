...
# 107 "tty.c"
static void
SigAlrmDummy (sigsig) int sigsig;
{
  do {} while (0);
           ;
}
...
char *line, *opt;
{
  ...
  if ((f = secopen(line, 02 | 04000 | 0400, 0)) == -1)
    {
      if ((*__errno_location ()) == 4)
        Msg(0, "Cannot open line '%s' for R/W: open() blocked, aborted.", line);
      else
        Msg((*__errno_location ()), "Cannot open line '%s' for R/W", line);
      alarm(0);
      xsignal(14, sigalrm);
      return -1;
    }
...
# 160 "tty.c"
 (*__errno_location ()) = 0;
 if (ioctl(f, 0x540C, (char *) 0) < 0)
   Msg((*__errno_location ()), "%s: ioctl TIOCEXCL failed", line);
 do {} while (0);
 do {} while (0);
# 177 "tty.c"
    InitTTY(&Mode, 1);
...
int ttyflag;
{
 ...
 m->tio.c_iflag |= 0000002;


 m->tio.c_iflag |= 0000004;



 m->tio.c_iflag |= 0002000;
 ...
 m->tio.c_iflag |= 0000400;


 m->tio.c_oflag |= 0000004;


 m->tio.c_oflag |= 0014000;
 ...
 m->tio.c_oflag |= 0000001;

    }
# 261 "tty.c"
       cfsetospeed(&m->tio, 0000015);


       cfsetispeed(&m->tio, 0000015);



  m->tio.c_cflag |= 0000060;


 m->tio.c_cflag |= 0000200;


 m->tio.c_cflag |= 0004000;



 m->tio.c_lflag |= 0001000;


 m->tio.c_lflag |= 0004000;


  if (!ttyflag)
    {

 m->tio.c_lflag |= 0000001;


 m->tio.c_lflag |= 0000002;


 m->tio.c_lflag |= 0000010;

    }

 m->tio.c_lflag |= 0000020;


 m->tio.c_lflag |= 0000040;


 m->tio.c_lflag |= 0100000;
...
# 623 "tty.c"
  if ((*__errno_location ()))
    Msg((*__errno_location ()), "SetTTY (fd %d): ioctl failed", fd);
}
...
# 667 "tty.c"
  if ((*__errno_location ()))
    Msg((*__errno_location ()), "GetTTY (fd %d): ioctl failed", fd);
}
...
int on;
{
  ...
  if (on)
    {
      display->d_NewMode.tio.c_cc[0] = iflag ? display->d_OldMode.tio.c_cc[0] : '\0';
 ...
 display->d_NewMode.tio.c_cc[9] = display->d_OldMode.tio.c_cc[9];
      ...
      display->d_NewMode.tio.c_iflag |= display->d_OldMode.tio.c_iflag & 0002000;
    }
  else
    {
      display->d_NewMode.tio.c_cc[0] = '\0';


 display->d_NewMode.tio.c_cc[8] = '\0';




 display->d_NewMode.tio.c_cc[9] = '\0';


      display->d_NewMode.tio.c_iflag &= ~0002000;
    }
  ...
  if (tcsetattr(display->d_userfd, 0, &display->d_NewMode.tio))
...
# 1125 "tty.c"
      do {} while (0);
 {
   ...
   for (i = 0; i < n; i++)
     if (tcsendbreak(fd, 0) < 0)
       {
  Msg((*__errno_location ()), "cannot send BREAK (tcsendbreak SVR4)");
  return;
       }
 }
      ...
      do {} while (0);
 {
   ...
   for (i = 0; i < n; i++)
     if (ioctl(fd, 0x5409, (char *)0) < 0)
       {
  Msg((*__errno_location ()), "Cannot send BREAK (TCSBRK)");
  return;
       }
 }
    ...
    case 0:
      ...
      do {} while (0);
      if (ioctl(fd, 0x5427, (char *)0) < 0)
        {
   Msg((*__errno_location ()), "Can't send BREAK (TIOCSBRK)");
   return;
 }
      sleep1000(n ? n * 250 : 250);
      if (ioctl(fd, 0x5428, (char *)0) < 0)
        {
   Msg((*__errno_location ()), "BREAK stuck!!! -- HELP! (TIOCCBRK)");
   return;
 }
...
# 1408 "tty.c"
  if ((consredirfd[0] = OpenPTY(&slave)) < 0)
    {
      Msg((*__errno_location ()), "%s: could not open detach pty master", rc_name);
      return -1;
    }
  if ((consredirfd[1] = open(slave, 02 | 0400)) < 0)
    {
      Msg((*__errno_location ()), "%s: could not open detach pty slave", rc_name);
      close(consredirfd[0]);
      return -1;
    }
  InitTTY(&new1, 0);
  ...
  if (UserContext() == 1)
    UserReturn(ioctl(consredirfd[1], 0x541D, (char *)&on));
  if (UserStatus())
    {
      Msg((*__errno_location ()), "%s: ioctl TIOCCONS failed", rc_name);
      close(consredirfd[0]);
      close(consredirfd[1]);
      return -1;
    }
...
# 1487 "tty.c"
  ...
  if (!(mtio.tio.c_cflag & 020000000000))
    rtscts = 0;
  else
  ...
  if (ioctl(fd, 0x5419, (char *)&softcar) < 0)
    softcar = 0;




  if (ioctl(fd, 0x5415, (char *)&mflags) < 0)
...
# 1549 "tty.c"
      ...
      s = "!DTR "; if (mflags & 0x002) s++;
      while (*s) *p++ = *s++;


      s = "!DSR "; if (mflags & 0x100) s++;
      while (*s) *p++ = *s++;
...
char *tty;
{
  ...
  if (lstat(real, &st) || !((((st.st_mode)) & 0170000) == (0020000)) ||
