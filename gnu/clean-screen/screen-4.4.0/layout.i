...
struct layout *lay;
{
      ...
      if (*layp == lay)
 {
  ...
  if (display && display->d_layout == lay)
  ...
  if (layouts)
    LoadLayout((display && display->d_layout) ? display->d_layout : *layp ? *layp : layouts,
 display ? &display->d_canvas : (struct canvas *)0);
  Activate(0);
}
...
struct win *wi;
{
  for (; cv; cv = cv->c_slnext)
    {
      if (cv->c_layer && ((struct win *)(cv->c_layer)->l_bottom->l_data) == wi)
 {
