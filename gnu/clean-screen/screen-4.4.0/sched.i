...
# 161 "sched.c"
   ...
   if (ev->type == 1)
     ((void) (((&r)->fds_bits)[((ev->fd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((ev->fd) % (8 * (int) sizeof (__fd_mask))))));
   else if (ev->type == 2)
     ((void) (((&w)->fds_bits)[((ev->fd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((ev->fd) % (8 * (int) sizeof (__fd_mask))))));
        }
# 189 "sched.c"
      nsel = select(1024, &r, &w, (fd_set *)0, timeoutev ? &timeout : (struct timeval *) 0);
      if (nsel < 0)
 {
   if ((*__errno_location ()) != 4)
     {
# 204 "sched.c"
       Panic((*__errno_location ()), "select");
     }
   nsel = 0;
 }
      else if (nsel == 0)
...
# 224 "sched.c"
   ...
   if (ev->type != 3)
     {
       set = ev->type == 1 ? &r : &w;
       if (nsel == 0 || !((((set)->fds_bits)[((ev->fd) / (8 * (int) sizeof (__fd_mask)))] & ((__fd_mask) 1 << ((ev->fd) % (8 * (int) sizeof (__fd_mask))))) != 0))
  continue;
       nsel--;
     }
   if (ev->condpos && *ev->condpos <= (ev->condneg ? *ev->condneg : 0))
...
int timo;
{
  do {} while (0);
  do {} while (0);
  gettimeofday(&ev->timeout, ((void*)0));
