...
int x, y;
{
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; { display = cv->c_display; if (display->d_blocked) continue; if (cv != display->d_forecv) continue; x2 = x + cv->c_xoff; y2 = y + cv->c_yoff; do {} while (0); if (x2 < cv->c_xs) x2 = cv->c_xs; if (y2 < cv->c_ys) y2 = cv->c_ys; if (x2 > cv->c_xe) x2 = cv->c_xe; if (y2 > cv->c_ye) y2 = cv->c_ye; for (vp = cv->c_vplist; vp; vp = vp->v_next) { if (x2 < vp->v_xs || x2 > vp->v_xe) continue; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; GotoPos(x2, y2); break; } } };
# 142 "layer.c"
}
...
struct mline *ol;
{
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; xs2 = xs + vp->v_xoff; xe2 = xe + vp->v_xoff; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; ScrollH(y2, xs2, xe2, n, bce, ol ? mloff(ol, -vp->v_xoff) : 0); if (xe2 - xs2 == xe - xs) continue; if (n > 0) { xs2 = xe2 + 1 - n; xe2 = xe + vp->v_xoff - n; } else { xe2 = xs2 - 1 - n; xs2 = xs + vp->v_xoff - n; } if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 <= xe2) RefreshArea(xs2, y2, xe2, y2, 1); } };
# 197 "layer.c"
}
...
int bce;
{
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { xs2 = vp->v_xoff; xe2 = l->l_width - 1 + vp->v_xoff; ys2 = ys + vp->v_yoff; ye2 = ye + vp->v_yoff; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (ys2 < vp->v_ys) ys2 = vp->v_ys; if (ye2 > vp->v_ye) ye2 = vp->v_ye; if (ys2 > ye2 || xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; ScrollV(vp->v_xs, ys2, vp->v_xe, ye2, n, bce); do {} while (0); do {} while (0); if (ye2 - ys2 == ye - ys) continue; if (n > 0) { ys2 = ye2 + 1 - n; ye2 = ye + vp->v_yoff - n; } else { ye2 = ys2 - 1 - n; ys2 = ys + vp->v_yoff - n; } do {} while (0); if (ys2 < vp->v_ys) ys2 = vp->v_ys; if (ye2 > vp->v_ye) ye2 = vp->v_ye; do {} while (0); if (ys2 <= ye2) RefreshArea(xs2, ys2, xe2, ye2, 1); } };
# 262 "layer.c"
}
...
struct mline *ol;
{
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; xs2 = x + vp->v_xoff; xe2 = l->l_width - 1 + vp->v_xoff; c2 = c; f = 0; if (xs2 < vp->v_xs) { xs2 = vp->v_xs; c2 = &mchar_blank; if (ol) { int i; i = xs2 - vp->v_xoff - 1; if (i >= 0 && i < l->l_width) { do { (&cc)->image = (ol)->image[i]; (&cc)->attr = (ol)->attr[i]; (&cc)->font = (ol)->font[i]; (&cc)->fontx = (ol)->fontx[i]; (&cc)->color = (ol)->color[i]; (&cc)->mbcs = 0; } while (0); c2 = &cc; } } else f = 1; } if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; rol = ((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mline(ol, l->l_width, l->l_encoding, display->d_encoding) : (ol)); InsChar(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mchar(c2, l->l_encoding, display->d_encoding) : (c2)), xs2, xe2, y2, mloff(rol, -vp->v_xoff)); if (f) RefreshArea(xs2, y2, xs2, y2, 1); } };
# 319 "layer.c"
}
...
# 338 "layer.c"
 ...
 , y, y);
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; { display = cv->c_display; if (display->d_blocked) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; x2 = x + vp->v_xoff; if (x2 < vp->v_xs || x2 > vp->v_xe) continue; PutChar(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mchar(c, l->l_encoding, display->d_encoding) : (c)), x2, y2); break; } } };
# 365 "layer.c"
}
...
int x, y;
{
  ...
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; xs2 = x + vp->v_xoff; xe2 = xs2 + n - 1; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; GotoPos(xs2, y2); SetRendition(r); s2 = s + xs2 - x - vp->v_xoff; if (display->d_encoding == 8 && l->l_encoding != 8 && (r->font || r->fontx || l->l_encoding)) { struct mchar mc; mc = *r; while (xs2 <= xe2) { mc.image = *s2++; PutChar(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mchar(&mc, l->l_encoding, display->d_encoding) : (&mc)), xs2++, y2); } continue; } while (xs2++ <= xe2) PUTCHARLP(*s2++); } };
# 429 "layer.c"
}
...
int x, y;
{
  ...
  if (len > n)
    len = n;
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; xs2 = x + vp->v_xoff; xe2 = xs2 + n - 1; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; GotoPos(xs2, y2); SetRendition(r); len2 = xe2 - (x + vp->v_xoff) + 1; if (len2 > len) len2 = len; PutWinMsg(s, xs2 - x - vp->v_xoff, len2); xs2 = x + vp->v_xoff + len2; if (xs2 < vp->v_xs) xs2 = vp->v_xs; or = display->d_rend; GotoPos(xs2, y2); SetRendition(&or); while (xs2++ <= xe2) PUTCHARLP(' '); } };
# 491 "layer.c"
}
...
struct mline *ol;
{
  ...
  if (l->l_pause.d)
      LayPauseUpdateRegion(l, xs, xe, y, y);
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { xs2 = xs + vp->v_xoff; xe2 = xe + vp->v_xoff; y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; if (display->d_blocked) continue; ClearLine(ol ? mloff(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mline(ol, l->l_width, l->l_encoding, display->d_encoding) : (ol)), -vp->v_xoff) : (struct mline *)0, y2, xs2, xe2, bce); } };
# 530 "layer.c"
}
...
int isblank;
{
  ...
  if (l->l_pause.d)
    LayPauseUpdateRegion(l, xs, xe, y, y);
  for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; { display = cv->c_display; if (display->d_blocked) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { xs2 = xs + vp->v_xoff; xe2 = xe + vp->v_xoff; y2 = y + vp->v_yoff; if (y2 < vp->v_ys || y2 > vp->v_ye) continue; if (xs2 < vp->v_xs) xs2 = vp->v_xs; if (xe2 > vp->v_xe) xe2 = vp->v_xe; if (xs2 > xe2) continue; display = cv->c_display; do {} while (0); do {} while (0); DisplayLine(isblank ? &mline_blank : &mline_null, mloff(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mline(ml, l->l_width, l->l_encoding, display->d_encoding) : (ml)), -vp->v_xoff), y2, xs2, xe2); } } };
# 664 "layer.c"
}
  ...
  if (y != bot)
    {
      ...
      for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; { y2 = 0; display = cv->c_display; if (display->d_blocked) continue; for (vp = cv->c_vplist; vp; vp = vp->v_next) { y2 = y + vp->v_yoff; yy2 = yy + vp->v_yoff; if (yy2 >= vp->v_ys && yy2 <= vp->v_ye && vp->v_xoff >= vp->v_xs && vp->v_xoff <= vp->v_xe) break; } if (vp == 0) continue; for (evp = cv->c_vplist; evp; evp = evp->v_next) if (y2 >= evp->v_ys && y2 <= evp->v_ye && evp->v_xoff + l->l_width - 1 >= evp->v_xs && evp->v_xoff + l->l_width - 1 <= evp->v_xe) break; if (evp == 0 || (ins && vp->v_xoff + l->l_width - 1 > vp->v_ye)) { do {} while (0); cvlist = l->l_cvlist; cvlnext = cv->c_lnext; l->l_cvlist = cv; cv->c_lnext = 0; if (ins) LInsChar(l, c, 0, yy, 0); else LPutChar(l, c, 0, yy); l->l_cvlist = cvlist; cv->c_lnext = cvlnext; } else { WrapChar(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mchar(c, l->l_encoding, display->d_encoding) : (c)), vp->v_xoff + l->l_width, y2, vp->v_xoff, -1, vp->v_xoff + l->l_width - 1, -1, ins); } } };
# 773 "layer.c"
    }
  else
    {
      ...
      for (cv = (l)->l_cvlist; cv; cv = cv->c_lnext) { if ((l)->l_pause.d && cv->c_slorient) continue; { display = cv->c_display; if (display->d_blocked) continue; for (vpp = &cv->c_vplist; (vp = *vpp); vpp = &vp->v_next) { yy2 = bot + vp->v_yoff; if (yy2 >= vp->v_ys && yy2 <= vp->v_ye && vp->v_xoff >= vp->v_xs && vp->v_xoff + l->l_width - 1 <= vp->v_xe) break; } if (vp) { *vpp = vp->v_next; } if (cv->c_vplist) { cvlist = l->l_cvlist; cvlnext = cv->c_lnext; l->l_cvlist = cv; cv->c_lnext = 0; LScrollV(l, 1, top, bot, bce); if (!vp) { if (ins) LInsChar(l, c, 0, bot, 0); else LPutChar(l, c, 0, bot); } l->l_cvlist = cvlist; cv->c_lnext = cvlnext; } if (vp) { *vpp = vp; top2 = top + vp->v_yoff; bot2 = bot + vp->v_yoff; if (top2 < vp->v_ys) top2 = vp->v_ys; WrapChar(((l->l_encoding == 8) != (display->d_encoding == 8) ? recode_mchar(c, l->l_encoding, display->d_encoding) : (c)), vp->v_xoff + l->l_width, bot2, vp->v_xoff, top2, vp->v_xoff + l->l_width - 1, bot2, ins); } } };
# 827 "layer.c"
    }
}
...
int block;
{
  ...
  cv = 0;
  if (display && display->d_forecv->c_layer == flayer)
    cv = display->d_forecv;
 ...
 if (cvp == cv)
   break;
    ...
    {
      do { struct display *olddisplay = display; struct canvas *cv; for (display = displays; display; display = display->d_next) { for (cv = display->d_cvlist; cv; cv = cv->c_next) if (cv->c_layer == flayer) break; if (cv == 0) continue; RemoveStatus(); } display = olddisplay; } while(0);
      if (block)
        do {} while (0);
...
int pause;
{
      ...
      for (vp = cv->c_vplist; vp; vp = vp->v_next)
 {
    ...
    ((xe = layer->l_pause.right[line]) >= 0))
  {
    ...
    if (layer->l_encoding == 8 && xe < vp->v_xe && win)
      {
        struct mline *ml = win->w_mlines + line;
        if (((8 == 8) ? (unsigned char)(ml)->font[(xe) + 1] == 0xff && (unsigned char)(ml)->image[(xe) + 1] == 0xff : ((unsigned char)(ml)->font[xe] & 0x1f) != 0 && ((unsigned char)(ml)->font[xe] & 0xe0) == 0 ))
   xe++;
      }
