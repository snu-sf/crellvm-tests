  ...
  if (dir == 0)
    {
      ...
      if (markdata->isdir > 0)
 searchend(0, 0, ((void*)0));
      else if (markdata->isdir < 0)
 backsearchend(0, 0, ((void*)0));
      else
 LMsg(0, "No previous pattern");
    }
  else
    Input((dir > 0 ? "/" : "?"), sizeof(markdata->isstr)-1, 0,
          (dir > 0 ? searchend : backsearchend), ((void*)0), 0);
}
...
int y, sx, ex;
{
  ...
  for (;sx <= ex; sx++)
    {
   cy = y;
   cp = ((cy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + cy) % fore->w_histheight] : &fore->w_mlines[cy - fore->w_histheight])->image + sx;
   cpe = ((cy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + cy) % fore->w_histheight] : &fore->w_mlines[cy - fore->w_histheight])->image + flayer->l_width;
      ...
      for (;;)
 {
  ...
  cy++;
  cp = ((cy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + cy) % fore->w_histheight] : &fore->w_mlines[cy - fore->w_histheight])->image;
  cpe = ((cy < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + cy) % fore->w_histheight] : &fore->w_mlines[cy - fore->w_histheight])->image + flayer->l_width;
  }
 }
    }
  ...
  while (p >= 0 && p < end)
    {
      ...
      for (i = 0;;)
 {
          c = (((q / w < fore->w_histheight) ? &fore->w_hlines[(fore->w_histidx + q / w) % fore->w_histheight] : &fore->w_mlines[q / w - fore->w_histheight]))->image[q % w];
   if (i == 0)
  ...
  if (pos >= 0)
    {
      ...
      do { struct layer *oldlay = flayer; struct canvas *oldcvlist, *cv; do {} while (0); flayer = flayer->l_next; oldcvlist = flayer->l_cvlist; do {} while (0); flayer->l_cvlist = oldlay->l_cvlist; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; (*flayer->l_layfn->lf_LayRedisplayLine)((flayer->l_height - 1), 0, flayer->l_width - 1, 0); revto(x, y); if (((markdata->cy) - markdata->hist_offset) == (flayer->l_height - 1)) revto_line(markdata->cx, markdata->cy, (flayer->l_height - 1) > 0 ? (flayer->l_height - 1) - 1 : 1);; flayer = oldlay; for (cv = flayer->l_cvlist; cv; cv = cv->c_lnext) cv->c_layer = flayer; flayer->l_next->l_cvlist = oldcvlist; } while(0);
...
int dir;
{
  ...
  if (((markdata->cy) - markdata->hist_offset) == (flayer->l_height - 1))
    revto_line(markdata->cx, markdata->cy, (flayer->l_height - 1) > 0 ? (flayer->l_height - 1) - 1 : 1);
  Input(isprompts[dir + 1], sizeof(markdata->isstr) - 1, 2,
        is_process, ((void*)0), 0);
  LGotoPos(flayer, markdata->cx, ((markdata->cy) - markdata->hist_offset));
  flayer->l_x = markdata->cx;
  flayer->l_y = ((markdata->cy) - markdata->hist_offset);
