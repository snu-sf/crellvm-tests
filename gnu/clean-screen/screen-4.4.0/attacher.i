...
static void
AttachSigCont (sigsig) int sigsig;
{
  do {} while (0);
  ContinuePlease = 1;
           ;
...
static void
QueryResultSuccess (sigsig) int sigsig;
{
  QueryResult = 1;
           ;
}
...
static void
QueryResultFail (sigsig) int sigsig;
{
  QueryResult = 2;
           ;
}
...
int how;
{
  ...
  do {} while (0);
# 208 "attacher.c"
  if ((how == 2 || how == 3) && multiattach)
    {
      real_uid = multi_uid;
      eff_uid = own_uid;

      if (setresuid(multi_uid, own_uid, multi_uid))
 Panic((*__errno_location ()), "setresuid");




      if (chmod(attach_tty, 0666))
 Panic((*__errno_location ()), "chmod %s", attach_tty);
 ...
 break;
  ...
  if (stat(SockPath, &st) == -1)
    Panic((*__errno_location ()), "stat %s", SockPath);
 ...
 Panic((*__errno_location ()), "WriteMessage");
      close(lasts);
 ...
 Panic(0, "Cannot contact screen again. Sigh.");
  ...
  do {} while (0);
  strncpy(m.m.attach.envterm, attach_term, 32);
  m.m.attach.envterm[32] = 0;
  do {} while (0);
...
# 443 "attacher.c"
static void
AttacherSigAlarm (sigsig) int sigsig;
{
...
static void
AttacherSigInt (sigsig) int sigsig;
{
...
void
AttacherFinit (sigsig) int sigsig;
{
  struct stat statb;
  struct msg m;
  int s;
...
static void
AttacherFinitBye (sigsig) int sigsig;
{
  int ppid;
  do {} while (0);
...
static void
AttacherWinch (sigsig) int sigsig;
{
  do {} while (0);
  SigWinchPlease = 1;
           ;
  ...
  for (;;)
    {
      ...
      pause();
      alarm(0);
      if (kill(MasterPid, 0) < 0 && (*__errno_location ()) != 1)
        {
   do {} while (0);
   AttacherPanic++;
 }
...
LockTerminal()
{
  ...
  for (sig = 1; sig < 65; sig++)
    sigs[sig] = xsignal(sig, sig == 17 ? ((__sighandler_t) 0) : ((__sighandler_t) 1));
  xsignal(1, LockHup);
  printf("\n");

  prg = getenv("LOCKPRG");
  if (prg && strcmp(prg, "builtin") && !access(prg, 1))
    {
      ...
      if ((pid = fork()) == 0)
        {
          ...
          closeallfiles(0);
          execl(prg, "SCREEN-LOCK", ((void*)0));
          exit((*__errno_location ()));
        }
      if (pid == -1)
        Msg((*__errno_location ()), "Cannot lock terminal - fork failed");
      else
        {
          ...
          ((wret == -1) && ((*__errno_location ()) == 4))
          )
     (*__errno_location ()) = 0;

          if ((*__errno_location ()))
     {
       Msg((*__errno_location ()), "Lock");
       sleep(2);
     }
   else if ((((__extension__ (((union { __typeof(wstat) __in; int __i; }) { .__in = (wstat) }).__i))) & 0x7f) != 0)
     {
       fprintf(stderr, "Lock: %s: Killed by signal: %d%s\n", prg,
        (((__extension__ (((union { __typeof(wstat) __in; int __i; }) { .__in = (wstat) }).__i))) & 0x7f), (((__extension__ (((union { __typeof(wstat) __in; int __i; }) { .__in = (wstat) }).__i))) & 0x80) ? " (Core dumped)" : "");
       sleep(2);
     }
   else if (((((__extension__ (((union { __typeof(wstat) __in; int __i; }) { .__in = (wstat) }).__i))) & 0xff00) >> 8))
     {
       do {} while (0);
     }
          else
