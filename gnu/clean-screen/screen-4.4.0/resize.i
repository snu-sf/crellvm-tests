...
int change_flag;
{
  ...
  if (ioctl(display->d_userfd, 0x5413, (char *)&glwz) != 0)
    {
      do {} while (0);
...
int change_fore;
{
  ...
  if ((display->d_tcs[44].str))
    ...
    {
      if ((display->d_tcs[45].str) && (wi == Z0width || wi == Z1width) &&
          ((display->d_tcs[1].num) == Z0width || (display->d_tcs[1].num) == Z1width))
        display->d_defwidth = (display->d_tcs[1].num);
      else
  ...
  if (change_fore == 2 && (display->d_tcs[44].str) == ((void*)0) && displays->d_next == 0)
    {
...
struct display *norefdisp;
{
  ...
  if (oldflayer && (l == oldflayer || ((struct win *)(oldflayer)->l_bottom->l_data) == p))
    oldflayer = ((void*)0);
 ...
 for (cv = d->d_cvlist; cv; cv = cv->c_next)
  ...
  do { struct layer *_last = ((void*)0); flayer = (cv->c_layer); while (flayer->l_next) { if ((*flayer->l_layfn->lf_LayResize)(wi, he) == 0) { _last = flayer; flayer = flayer->l_next; } else { struct canvas *_cv; for (_cv = flayer->l_cvlist; _cv; _cv = _cv->c_lnext) _cv->c_display->d_kaablamm = 1; ExitOverlayPage(); if (_last) _last->l_next = flayer; } } (*flayer->l_layfn->lf_LayResize)(wi, he); } while (0);
       }
   }
    }
  else
    {
      ...
      do { struct layer *_last = ((void*)0); flayer = (flayer); while (flayer->l_next) { if ((*flayer->l_layfn->lf_LayResize)(wi, he) == 0) { _last = flayer; flayer = flayer->l_next; } else { struct canvas *_cv; for (_cv = flayer->l_cvlist; _cv; _cv = _cv->c_lnext) _cv->c_display->d_kaablamm = 1; ExitOverlayPage(); if (_last) _last->l_next = flayer; } } (*flayer->l_layfn->lf_LayResize)(wi, he); } while (0);
    }
 ...
 if (((struct win *)(cv->c_layer)->l_bottom->l_data) == p)
   {
            { struct display *olddisplay = display; struct layer *oldflayer = flayer; struct layer *l = cv->c_layer; struct canvas *cvlist = l->l_cvlist; struct canvas *cvlnext = cv->c_lnext; flayer = l; l->l_cvlist = cv; cv->c_lnext = 0; (*flayer->l_layfn->lf_LayRedisplayLine)(-1, -1, -1, 0); flayer = oldflayer; l->l_cvlist = cvlist; cv->c_lnext = cvlnext; display = olddisplay; };
...
# 601 "resize.c"
  for (p = windows; p; p = p->w_next)
    {
      do { ml = p->w_mlines; for (i = 0; i < p->w_layer.l_height; i++, ml++) { do { if (ml->image == oldblank) ml->image = blank; } while (0); do { if (ml->attr == oldnull) ml->attr = null; } while (0); do { if (ml->font == oldnull) ml->font = null; } while (0); do { if (ml->fontx == oldnull) ml->fontx = null; } while (0); do { if (ml->color == oldnull) ml->color = null; } while (0); ; } } while (0);


      do { ml = p->w_hlines; for (i = 0; i < p->w_histheight; i++, ml++) { do { if (ml->image == oldblank) ml->image = blank; } while (0); do { if (ml->attr == oldnull) ml->attr = null; } while (0); do { if (ml->font == oldnull) ml->font = null; } while (0); do { if (ml->fontx == oldnull) ml->fontx = null; } while (0); do { if (ml->color == oldnull) ml->color = null; } while (0); ; } } while (0);
      do { ml = p->w_alt.hlines; for (i = 0; i < p->w_alt.histheight; i++, ml++) { do { if (ml->image == oldblank) ml->image = blank; } while (0); do { if (ml->attr == oldnull) ml->attr = null; } while (0); do { if (ml->font == oldnull) ml->font = null; } while (0); do { if (ml->fontx == oldnull) ml->fontx = null; } while (0); do { if (ml->color == oldnull) ml->color = null; } while (0); ; } } while (0);


      do { ml = p->w_alt.mlines; for (i = 0; i < p->w_alt.height; i++, ml++) { do { if (ml->image == oldblank) ml->image = blank; } while (0); do { if (ml->attr == oldnull) ml->attr = null; } while (0); do { if (ml->font == oldnull) ml->font = null; } while (0); do { if (ml->fontx == oldnull) ml->fontx = null; } while (0); do { if (ml->color == oldnull) ml->color = null; } while (0); ; } } while (0);
    }
}
...
int wi, he, hi;
{
  ...
  if (wi > 1000)
    {
      Msg(0, "Window width too large. Truncated to %d.", 1000);
      wi = 1000;
    }

  if (he > 1000)
    {
      Msg(0, "Window height too large. Truncated to %d.", 1000);
      he = 1000;
    }

  if (p->w_layer.l_width == wi && p->w_layer.l_height == he && p->w_histheight == hi)
    {
      do {} while (0);
      return 0;
    }
...
# 704 "resize.c"
      ...
      if ((nhlines = (struct mline *)calloc(hi, sizeof(struct mline))) == 0)
 {
  ...
  addone = 0;
  if (p->w_layer.l_width && p->w_layer.l_x == p->w_layer.l_width)
    {
      do {} while (0);
      addone = 1;
      p->w_layer.l_x--;
  ...
  if (p->w_layer.l_width == wi)
    {
      ...
      shift = -ncy;
      for (yy = p->w_layer.l_y + p->w_histheight - 1; yy >= 0 && ncy + shift < he; yy--)
 {
   ml = ((yy < p->w_histheight) ? &p->w_hlines[(p->w_histidx + yy) % p->w_histheight] : &p->w_mlines[yy - p->w_histheight]);
   if (!ml->image)
     break;
   if (ml->image[p->w_layer.l_width] == ' ')
     break;
   shift++;
 }
      if (shift < 0)
      ...
      if (p->w_autoaka > 0)
 {
   ...
   if (naka < 1 || naka > he)
     naka = 0;
      ...
      while (shift-- > 0)
 {
   ml = ((fy < p->w_histheight) ? &p->w_hlines[(p->w_histidx + fy) % p->w_histheight] : &p->w_mlines[fy - p->w_histheight]);
   FreeMline(ml);
   fy--;
 }
    }
  do {} while (0);
  if (fy >= 0)
    mlf = ((fy < p->w_histheight) ? &p->w_hlines[(p->w_histidx + fy) % p->w_histheight] : &p->w_mlines[fy - p->w_histheight]);
  if (ty >= 0)
    mlt = ((ty < hi) ? &nhlines[ty] : &nmlines[ty - hi]);
      ...
      if (p->w_layer.l_width == wi)
 {
   ...
   *mlt = *mlf;
   ...
   continue;
      ...
      for (l = p->w_layer.l_width - 1; l > 0; l--)
      ...
      while (l > 0 && fy >= 0 && ty >= 0)
 {
  ...
  goto nomem;
   ...
   if (fy == p->w_layer.l_y + p->w_histheight && lf - lx <= p->w_layer.l_x && lf > p->w_layer.l_x)
     {
       ...
       if (shift > 0)
  {
    ...
    for (y = hi + he - 1; y >= ty; y--)
      {
        mlt = ((y < hi) ? &nhlines[y] : &nmlines[y - hi]);
        FreeMline(mlt);
        if (y - shift < ty)
   continue;
        ml = ((y - shift < hi) ? &nhlines[y - shift] : &nmlines[y - shift - hi]);
        *mlt = *ml;
        *ml = mline_zero;
      }
    ncy += shift;
...
# 923 "resize.c"
  ...
  if (p->w_layer.l_width != wi)
    {
      if (wi)
 {
   t = p->w_tabs ? p->w_layer.l_width : 0;
      ...
      else
 {
  ...
  if (wi && (p->w_layer.l_width != wi || p->w_layer.l_height != he)
      && p->w_layer.l_width != 0 && p->w_layer.l_height != 0 && p->w_ptyfd >= 0 && p->w_pid)
    {
      glwz.ws_col = wi;
      glwz.ws_row = he;
      do {} while (0);
      if (ioctl(p->w_ptyfd, 0x5414, (char *)&glwz))
 do {} while (0);
    }
...
struct win *p;
{
  ...
  do { (ml) = p->w_alt. mlines; p->w_alt. mlines = p->w_mlines; p->w_mlines = (ml); } while (0);
  do { (t) = p->w_alt. width; p->w_alt. width = p->w_layer.l_width; p->w_layer.l_width = (t); } while (0);
  do { (t) = p->w_alt. height; p->w_alt. height = p->w_layer.l_height; p->w_layer.l_height = (t); } while (0);


  do { (t) = p->w_alt. histheight; p->w_alt. histheight = p->w_histheight; p->w_histheight = (t); } while (0);
  do { (ml) = p->w_alt. hlines; p->w_alt. hlines = p->w_hlines; p->w_hlines = (ml); } while (0);
  do { (t) = p->w_alt. histidx; p->w_alt. histidx = p->w_histidx; p->w_histidx = (t); } while (0);
