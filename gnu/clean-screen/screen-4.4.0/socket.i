...
char *match;
{
 ...
 matchlen = 255;
  ...
  if ((dirp = opendir(SockPath)) == 0)
    Panic((*__errno_location ()), "Cannot opendir %s", SockPath);
  ...
  while ((dp = readdir(dirp)))
    {
      ...
      name = dp->d_name;
      do {} while (0);
      if (*name == 0 || *name == '.' || strlen(name) > 2*768)
 continue;
      if (matchlen)
 {
   n = name;
...
MakeServerSocket()
{
  ...
  if ((s = socket(1, SOCK_STREAM, 0)) == -1)
    Panic((*__errno_location ()), "socket");
  a.sun_family = 1;
 ...
 Panic(0, "It is not detached.");
  ...
  if (bind(s, (struct sockaddr *) & a, strlen(SockPath) + 2) == -1)
    Panic((*__errno_location ()), "bind (%s)", SockPath);
# 588 "socket.c"
  chmod(SockPath, (0200 | 0400 | (displays ? 0100 : 0) | (multi ? 1 : 0)));




  if (listen(s, 5) == -1)
    Panic((*__errno_location ()), "listen");
...
# 631 "socket.c"
  if (connect(s, (struct sockaddr *) &a, strlen(SockPath) + 2) == -1)
    {
      if (err)
 Msg((*__errno_location ()), "%s: connect", SockPath);
      do {} while (0);
      close(s);
      s = -1;
    }
...
struct NewWindow *nwin;
{
  ...
  if (nwin->args != nwin_undef.args)
    for (av = nwin->args; *av && n < 64 - 1; ++av, ++n)
      {
        len = strlen(*av) + 1;
  ...
  if (getcwd(m.m.create.dir, sizeof(m.m.create.dir)) == 0)
    {
      Msg((*__errno_location ()), "getcwd");
      return;
    }
  if (nwin->term != nwin_undef.term)
    strncpy(m.m.create.screenterm, nwin->term, 32);
  m.m.create.screenterm[32] = '\0';
  m.protocol_revision = (('m'<<24) | ('s'<<16) | ('g'<<8) | 5);
  do {} while (0);
  if (write(s, (char *) &m, sizeof m) != sizeof m)
    Msg((*__errno_location ()), "write");
  close(s);
}
...
struct win *wi;
{
 ...
 {
   Msg((*__errno_location ()), "Attach: passed fd does not match tty: %s - %s!", m->m_tty, myttyname ? myttyname : "NULL");
   close(i);
   Kill(pid, 1);
   return -1;
 }
    }
  else if ((i = secopen(m->m_tty, 02 | 04000, 0)) < 0)
    {
      Msg((*__errno_location ()), "Attach: Could not open %s!", m->m_tty);
      Kill(pid, 1);
      return -1;
    }
  ...
  if (attach)
    {
      ...
      display->d_encoding = m->m.attach.encoding == 1 ? 8 : m->m.attach.encoding ? m->m.attach.encoding - 1 : 0;



      if (display->d_encoding < 0 || !EncodingName(display->d_encoding))
  ...
  if (iflag && olddisplays)
    {
      ...
      olddisplays->d_NewMode.tio.c_cc[0] = '\0';
      olddisplays->d_NewMode.tio.c_lflag &= ~0000001;
...
# 993 "socket.c"
 ...
 {
   struct cmsghdr *cmsg;
   for (cmsg = ((size_t) (&msg)->msg_controllen >= sizeof (struct cmsghdr) ? (struct cmsghdr *) (&msg)->msg_control : (struct cmsghdr *) 0); cmsg; cmsg = __cmsg_nxthdr (&msg, cmsg))
     {
       int cl;
       char *cp;
...
# 1077 "socket.c"
    ...
    case 9:
 {
   ...
   Kill(m.m.command.apid, (queryflag >= 0) ? 18 : 1);
   queryflag = -1;
 }
      break;
    case 8:
...
chsock()
{
  ...
  r = chmod(SockPath, (0200 | 0400 | (displays ? 0100 : 0) | (multi ? 1 : 0)));





  (void)utimes(SockPath, ((void*)0));
...
# 1361 "socket.c"
      ...
      if (lay)
 {
   ...
   SetCanvasWindow(display->d_forecv, 0);
 }
    }
      ...
      else if (!strcmp(m->m.attach.preselect, "+"))
 {
  ...
  if (fore)
  ...
  else if (!noshowwin)
    {
      ...
      if (!AclCheckPermCmd(display->d_user, 0, &comms[178]))
...
struct msg *mp;
{
      ...
      if (strcmp(mp->m.command.preselect, "-"))
 {
   ...
   if (i < 0 || !wtab[i])
    ...
    {
      if (display && display->d_user == user)
...
int fd;
{
  ...
  struct msghdr msg;
  struct iovec iov;
  char buf[((((sizeof(int)) + sizeof (size_t) - 1) & (size_t) ~(sizeof (size_t) - 1)) + (((sizeof (struct cmsghdr)) + sizeof (size_t) - 1) & (size_t) ~(sizeof (size_t) - 1)))];
  struct cmsghdr *cmsg;
  ...
  cmsg->cmsg_len = ((((sizeof (struct cmsghdr)) + sizeof (size_t) - 1) & (size_t) ~(sizeof (size_t) - 1)) + (sizeof(int)));
  bcopy(&fd, ((cmsg)->__cmsg_data), sizeof(int));
