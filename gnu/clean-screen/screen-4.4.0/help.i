      ...
      else if (helpdata->grow >= 2 && helpdata->grow-2 < helpdata->numrows)
 {
  ...
  if (ktabp[key].nr == n && ktabp[key].args == noargs && strlen(buf) < sizeof(buf) - 7)
   ...
   helpdata->grow++;
        ...
        {
   while ((n = ktabp[helpdata->command_search].nr) == -1
   || ktabp[helpdata->command_search].args == noargs)
     {
       if (++helpdata->command_search >= 256 + (188 -106))
  return -1;
     }
   buf[0] = '\0';
...
int x, y;
{
  ...
  pp = act->args;
  lp = act->argl;
  while (pp && (cp = *pp) != ((void*)0))
    {
      del = 0;
      bp = buf;
      ll = *lp++;
      if (!ll || (index(cp, ' ') != ((void*)0)))
 {
   if (index(cp, '\'') != ((void*)0))
     *bp++ = del = '"';
   else
...
struct action *tab;
{
  ...
  n = 0;
  for (i = 0; i < (188 -106)+(188 -166)+kmap_extn; i++)
    {
      if (tab[i].nr != -1)
 n++;
    }
...
bindkeypage()
{
  ...
  centerline(tbuf, 0);
  y = 2;
  for (i = bindkeydata->pos; i < (188 -106)+(188 -166)+kmap_extn && y < flayer->l_height - 3; i++)
    {
      p = tbuf;
      xch = "   ";
      if (i < (188 -106))
 {
   ...
   sl = s ? strlen(s) : 0;
      ...
      else if (i < (188 -106)+(188 -166))
 {
   act = &bindkeydata->tab[i];
   if (act->nr == -1)
     continue;
   del = *p++ = ':';
   s = term[i + (106 - 188 + 166)].tcname;
   sl = s ? strlen(s) : 0;
   xch = "[A]";
 }
      else
 {
   kme = kmap_exts + (i - ((188 -106)+(188 -166)));
   del = 0;
   s = kme->str;
   sl = kme->fl & ~0x4000;
   if ((kme->fl & 0x4000) != 0)
