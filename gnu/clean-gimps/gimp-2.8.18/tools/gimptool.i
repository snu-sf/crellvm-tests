...
static gboolean dry_run = (0);
static gchar *prefix;
static const gchar *exec_prefix;

static gboolean msvc_syntax = (0);
...
static struct {
  ...
  { "gimpplugindir", "/usr/local/lib/gimp/2.0" },
  { "gimpdatadir", "/usr/local/share/gimp/2.0" }
};


static void usage (int exit_status) __attribute__((__noreturn__));
# 116 "gimptool.c"
static gboolean
starts_with_dir (const gchar *string,
   const gchar *test)
{
  return g_str_has_prefix (string, g_strconcat (test, "/", ((void*)0))) ||
...
one_line_output (const gchar *program,
   const gchar *args)
{
  gchar *command = g_strconcat (program, " ", args, ((void*)0));
  ...
  if (fgets (line, sizeof (line), pipe) == ((void*)0))
    line[0] = '\0';
...
expand_and_munge (const gchar *value)
{
  ...
  if (starts_with_dir (value, "${prefix}"))
    retval = g_strconcat ("/usr/local", value + strlen ("${prefix}"), ((void*)0));
  else if (starts_with_dir (value, "${exec_prefix}"))
    retval = g_strconcat ("/usr/local", value + strlen ("${exec_prefix}"), ((void*)0));
  else
    retval = g_strdup (value);

  if (starts_with_dir (retval, "/usr/local"))
    retval = g_strconcat (get_exec_prefix ('/'), retval + strlen ("/usr/local"), ((void*)0));

  if (starts_with_dir (retval, "/usr/local"))
    retval = g_strconcat (get_runtime_prefix ('/'), retval + strlen ("/usr/local"), ((void*)0));
...
find_out_env_flags (void)
{
  ...
  if ((p = getenv ("CC")) != ((void*)0) && *p != '\0')
    env_cc = p;
  else if (msvc_syntax)
    env_cc = "cl -MD";
  else
    env_cc = "/home/juneyoung.lee/llvm-prototypes/llvm-install-3.7.0/bin/clang";

  if (g_ascii_strncasecmp (env_cc, "cl", 2) == 0 &&
      g_ascii_strncasecmp (env_cc, "clang", 5) != 0)
    msvc_syntax = (!(0));

  if ((p = getenv ("CFLAGS")) != ((void*)0))
    env_cflags = p;
  else
    env_cflags = "";

  if ((p = getenv ("LDFLAGS")) != ((void*)0))
    env_ldflags = p;
  else
    env_ldflags = "";

  if ((p = getenv ("LIBS")) != ((void*)0) && *p != '\0')
    env_libs = p;
  else
    env_libs = "";
}
     ...
     const gchar *what)
{
  ...
  if (install_dir != ((void*)0))
    dest_dir = g_strconcat (install_dir, "/", ((void*)0));
  else
    dest_dir = "";
  ...
  *p = '\0';
  ...
  dest_exe = g_strconcat (dest_dir, q, "", ((void*)0));
...
{
  do_build_2 (get_cflags (), get_libs (), ((void*)0), what);
}

static void
do_build_noui (const gchar *what)
{
  do_build_2 (get_cflags_noui (), get_libs_noui (), ((void*)0), what);
}
...
{
# 570 "gimptool.c"
  return g_build_path (forward_slashes ? "/" : "/",
                       "/usr/local/lib",
                       "gimp",
                       "2.0",
                       "plug-ins",
...
{
  do_build_2 (get_cflags (), get_libs (), get_sys_plugin_dir ((!(0))), what);
}

static void
do_install_admin_noui (const gchar *what)
{
  do_build_2 (get_cflags_noui (), get_libs_noui (), get_sys_plugin_dir ((!(0))), what);
}
...
{
  do_install_bin_2 (get_sys_plugin_dir ((0)), what);
}
...
do_uninstall (const gchar *dir,
       const gchar *what)
{
  maybe_run (g_strconcat ("rm -f", " ",
                          g_shell_quote (g_strconcat (dir, "/",
                                                      what, ((void*)0))),
                          ((void*)0)));
}
...
{
  do_uninstall (get_sys_plugin_dir ((0)), maybe_append_exe (what));
}
...
{
  return g_build_filename (get_runtime_prefix ('/'),
                           "share", "gimp",
                           "2.0",
                           "scripts",
                           ((void*)0));
  ...
  while (++argi < argc)
    {
      for (i = 0; i < (sizeof (dirs) / sizeof ((dirs)[0])); i++)
        {
          if (strcmp (argv[argi],
                      g_strconcat ("--", dirs[i].option, ((void*)0))) == 0)
            break;
        }
