...
evaluate_spline (spline_type s, real t)
{
  ...
  polynomial_degree degree = ((s).degree);
    ...
    for (i = 0; i <= degree - j; i++)
      {
        ...
        real_coordinate_type t1 = ((real_coordinate_type) { (V[j - 1].v[i]).x * (one_minus_t), (V[j - 1].v[i]).y * (one_minus_t) });
        real_coordinate_type t2 = ((real_coordinate_type) { (V[j - 1].v[i + 1]).x * (t), (V[j - 1].v[i + 1]).y * (t) });
        V[j].v[i] = ((real_coordinate_type) { (t1).x + (t2).x, (t1).y + (t2).y });
# 109 "spline.c"
      }
...
{
  spline_list_type *answer = ((spline_list_type *) g_malloc_n ((1), sizeof (spline_list_type)));
...
init_spline_list (spline_type spline)
{
  ...
  ((*answer).data) = ((spline_type *) g_malloc_n ((1), sizeof (spline_type)));
...
{
  if (((*spline_list).data) != ((void*)0))
    safe_free ((address *) &(((*spline_list).data)));
}
...
{
  ((l != ((void*)0)) ? (void) (0) : __assert_fail ("l != ((void*)0)", "spline.c", 162, __PRETTY_FUNCTION__));

  ((*l).length)++;
  ((*l).data) = g_realloc (((*l).data),
                               ((*l).length) * sizeof (spline_type));
  (((*l).data)[((*l).length) - 1]) = s;
}
...
concat_spline_lists (spline_list_type *s1, spline_list_type s2)
{
  ...
  ((s1 != ((void*)0)) ? (void) (0) : __assert_fail ("s1 != ((void*)0)", "spline.c", 180, __PRETTY_FUNCTION__));

  new_length = ((*s1).length) + ((s2).length);

  ((*s1).data) = g_realloc(((*s1).data),new_length * sizeof(spline_type));

  for (this_spline = 0; this_spline < ((s2).length); this_spline++)
    (((*s1).data)[((*s1).length)++])
      = (((s2).data)[this_spline]);
}
...
free_spline_list_array (spline_list_array_type *spline_list_array)
{
  ...
  for (this_list = 0;
       this_list < ((*spline_list_array).length);
       this_list++)
    free_spline_list (&(((*spline_list_array).data)[this_list]));

  if (((*spline_list_array).data) != ((void*)0))
    safe_free ((address *) &(((*spline_list_array).data)));
}
...
append_spline_list (spline_list_array_type *l, spline_list_type s)
{
  ...
  ((*l).data) = g_realloc(((*l).data),(((*l).length))*sizeof(spline_list_type));
  (((*l).data)[((*l).length) - 1]) = s;
}
