...
normalize (const vector_type v)
{
  ...
  ((m > 0.0) ? (void) (0) : __assert_fail ("m > 0.0", "vector.c", 73, __PRETTY_FUNCTION__));
...
Vadd_int_point (const coordinate_type c, const vector_type v)
{
  ...
  a.x = ((int) ((int) ((real) c.x + v.dx) + .5 * (((real) c.x + v.dx) > 0 ? 1 : ((real) c.x + v.dx) < 0 ? -1 : 0)));
  a.y = ((int) ((int) ((real) c.y + v.dy) + .5 * (((real) c.y + v.dy) > 0 ? 1 : ((real) c.y + v.dy) < 0 ? -1 : 0)));
  return a;
}
