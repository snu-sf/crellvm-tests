...
fitted_splines (pixel_outline_list_type pixel_outline_list)
{
  ...
  for (this_list = 0; this_list < ((curve_array).length);
       this_list++)
    {
      spline_list_type curve_list_splines;
      curve_list_type curves = ((curve_array).data[this_list]);
  ...
  for (this_list = 0; this_list < ((char_splines).length);
       this_list++)
    total
      += (((((char_splines).data)[this_list])).length);
...
fit_curve_list (curve_list_type curve_list)
{
  ...
  unsigned curve_list_length = ((curve_list).length);
      ...
      for (this_curve = 0; this_curve < curve_list_length; this_curve++)
        {
          ...
          remove_knee_points (((curve_list).data[this_curve]),
                              ((curve_list).clockwise));
        }
    }
  ...
  for (this_curve = 0; this_curve < curve_list.length; this_curve++)
    {
      ...
      filter (((curve_list).data[this_curve]));

    }
# 316 "fit.c"
  curve = ((curve_list).data[0]);
  if (((curve)->cyclic) && ((curve)->length) != 3)
  ...
  for (this_curve = 0; this_curve < curve_list_length; this_curve++)
    {
      ...
      curve_type current_curve = ((curve_list).data[this_curve]);
      ...
      if (curve_splines == ((void*)0))
      ...
      else
        {
          ...
          for (this_spline = 0;
               this_spline < ((*curve_splines).length);
               this_spline++)
            {
...
split_at_corners (pixel_outline_list_type pixel_list)
{
  ...
  for (this_pixel_o = 0; this_pixel_o < ((pixel_list).length);
       this_pixel_o++)
    {
      ...
      pixel_outline_type pixel_o = ((pixel_list).data[this_pixel_o]);

      ((curve_list).clockwise) = ((pixel_o).clockwise);
# 440 "fit.c"
      if (((pixel_o).length) > corner_surround * 2 + 2)
        {
          corner_list = find_corners (pixel_o);
        }
      else
        {
          corner_list.data = ((void*)0);
      ...
      else
        {
          ...
          for (this_corner = 0; this_corner < corner_list.length - 1;
               this_corner++)
            {
              ...
              unsigned corner = ((corner_list).data[this_corner]);
              unsigned next_corner = ((corner_list).data[this_corner + 1]);
          ...
          for (p = ((corner_list).data[((corner_list).length) - 1]); p < ((pixel_o).length);
               p++)
            append_pixel (curve, ((pixel_o).data[p]));

          for (p = 0; p <= ((corner_list).data[0]); p++)
            append_pixel (curve, ((pixel_o).data[p]));
        }
...
find_corners (pixel_outline_type pixel_outline)
{
  ...
  for (p = 0; p < ((pixel_outline).length); p++)
    {
      real corner_angle;
...
# 574 "fit.c"
              if (corner_angle <= corner_always_threshold && q >= p)
                do { append_index (&corner_list, q); } while (0);




              if (i >= best_corner_index + corner_surround
                  || i >= ((pixel_outline).length))
                break;
          ...
          if (best_corner_angle > corner_always_threshold
              && best_corner_index >= p)
            {
              ...
              do { append_index (&corner_list, best_corner_index); } while (0);

              for (i = 0; i < ((equally_good_list).length); i++)
                do { append_index (&corner_list, ((equally_good_list).data[i])); } while (0);
  ...
  if (((corner_list).length) > 0)
    ...
    remove_adjacent_corners (&corner_list, ((pixel_outline).length) - 1);
              ...
              vector_type *in, vector_type *out)
{
  int i;
  unsigned n_done;
  coordinate_type candidate = ((outline).data[test_index]);
  ...
  for (i = ((test_index) == 0 ? ((outline).length) - 1 : (test_index) - 1), n_done = 0; n_done < corner_surround;
       i = ((i) == 0 ? ((outline).length) - 1 : (i) - 1), n_done++)
    *in = Vadd (*in, IPsubtract (((outline).data[i]), candidate));
# 696 "fit.c"
  for (i = (((test_index) + 1) % ((outline).length)), n_done = 0; n_done < corner_surround;
       i = (((i) + 1) % ((outline).length)), n_done++)
    *out = Vadd (*out, IPsubtract (((outline).data[i]), candidate));
# 708 "fit.c"
}
# 721 "fit.c"
static void
remove_adjacent_corners (index_list_type *list, unsigned last_index)
{
  ...
  for (j = 0; j < ((*list).length) - 1; j++)
    {
      unsigned current = ((*list).data[j]);
      unsigned next = ((*list).data[j + 1]);


      ((current != next) ? (void) (0) : __assert_fail ("current != next", "fit.c", 757, __PRETTY_FUNCTION__));
  ...
  last = ((*list).data[((*list).length) - 1]);
  if (((new).length) == 0
      || !(last == ((new).data[((new).length) - 1]) + 1
           || (last == last_index && ((*list).data[0]) == 0)))
...
remove_knee_points (curve_type curve, boolean clockwise)
{
  ...
  coordinate_type previous
    = real_to_int_coord (((curve)->point_list[((int) (offset) - 1 < 0 ? ((curve)->cyclic) ? ((curve)->length) + (int) (offset) - 1 : -1 : (int) (offset) - 1)].coord));
  curve_type trimmed_curve = copy_most_of_curve (curve);

  if (!((curve)->cyclic))
    append_pixel (trimmed_curve, real_to_int_coord (((curve)->point_list[0].coord)));

  for (i = offset; i < ((curve)->length) - offset; i++)
    {
      coordinate_type current
        = real_to_int_coord (((curve)->point_list[i].coord));
      coordinate_type next
        = real_to_int_coord (((curve)->point_list[((i) + 1 >= ((curve)->length) ? ((curve)->cyclic) ? ((i) + 1) % ((curve)->length) : ((curve)->length) : (i) + 1)].coord));
      vector_type prev_delta = IPsubtract (previous, current);
      vector_type next_delta = IPsubtract (next, current);

      if ((((prev_delta).dx == 0.0 && (prev_delta).dy != 0.0) || ((prev_delta).dy == 0.0 && (prev_delta).dx != 0.0)) && (((next_delta).dx == 0.0 && (next_delta).dy != 0.0) || ((next_delta).dy == 0.0 && (next_delta).dx != 0.0))
          && ((clockwise && ((prev_delta.dx == -1.0 && next_delta.dy == 1.0) || (prev_delta.dy == 1.0 && next_delta.dx == 1.0) || (prev_delta.dx == 1.0 && next_delta.dy == -1.0) || (prev_delta.dy == -1.0 && next_delta.dx == -1.0)))
              || (!clockwise
                  && ((prev_delta.dy == 1.0 && next_delta.dx == -1.0) || (prev_delta.dx == 1.0 && next_delta.dy == 1.0) || (prev_delta.dy == -1.0 && next_delta.dx == 1.0) || (prev_delta.dx == -1.0 && next_delta.dy == -1.0)))))
 {
  ...
  if (!((curve)->cyclic))
    append_pixel (trimmed_curve, real_to_int_coord (((curve)->point_list[(curve)->length-1].coord)));
  ...
  for (iteration = 0; iteration < filter_iteration_count; iteration++)
    {
      ...
      if (offset)
        append_point (new_curve, ((curve)->point_list[0].coord));

      for (this_point = offset; this_point < ((curve)->length) - offset;
           this_point++)
        {
...
# 943 "fit.c"
          ...
          if (angle_alt - angle >= filter_epsilon
              && sum.dx != 0 && sum.dy != 0)
            do { in = in_alt; out = out_alt; count = alt_count; angle = angle_alt; } while (0);
# 970 "fit.c"
          new_point = ((curve)->point_list[this_point].coord);
          sum = Vadd (in, out);
     ...
     if (offset)
       append_point (new_curve, ((curve)->point_list[(curve)->length-1].coord));
                    ...
                    vector_type *in, vector_type *out, unsigned *count)
{
  ...
  unsigned n_done;
  real_coordinate_type candidate = ((curve)->point_list[test_index].coord);
  ...
  for (i = ((int) (test_index) - 1 < 0 ? ((curve)->cyclic) ? ((curve)->length) + (int) (test_index) - 1 : -1 : (int) (test_index) - 1), n_done = 0;
       i >= 0 && n_done < surround;
       i = ((int) (i) - 1 < 0 ? ((curve)->cyclic) ? ((curve)->length) + (int) (i) - 1 : -1 : (int) (i) - 1), n_done++)
    *in = Vadd (*in, Psubtract (((curve)->point_list[i].coord), candidate));
  in_count = n_done;
  ...
  for (i = ((test_index) + 1 >= ((curve)->length) ? ((curve)->cyclic) ? ((test_index) + 1) % ((curve)->length) : ((curve)->length) : (test_index) + 1), n_done = 0;
       i < ((curve)->length) && n_done < surround && n_done < in_count;
       i = ((i) + 1 >= ((curve)->length) ? ((curve)->cyclic) ? ((i) + 1) % ((curve)->length) : ((curve)->length) : (i) + 1), n_done++)
    *out = Vadd (*out, Psubtract (((curve)->point_list[i].coord), candidate));
  out_count = n_done;
  ...
  ((in_count == out_count) ? (void) (0) : __assert_fail ("in_count == out_count", "fit.c", 1046, __PRETTY_FUNCTION__));
  *count = in_count;
}
...
fit_with_line (curve_type curve)
{
  ...
  ((line).degree) = LINEAR;
  ((line).v[0]) = ((line).v[1]) = ((curve)->point_list[0].coord);
  ((line).v[3]) = ((line).v[2]) = ((curve)->point_list[(curve)->length-1].coord);
...
{
  real error, best_error = 3.40282347e+38F;
  spline_type spline, best_spline;
  spline_list_type *spline_list;
  unsigned worst_point;
  unsigned iteration = 0;
  real previous_error = 3.40282347e+38F;
  real improvement = 3.40282347e+38F;
...
# 1133 "fit.c"
  find_tangent (curve, true, false);
  find_tangent (curve, false, false);
 ...
 break;
  ...
  if (error < error_threshold)
  ...
  else
    {
      ...
      ((left_curve)->length) = subdivision_index + 1;
      ...
      if (left_spline_list == ((void*)0) && right_spline_list == ((void*)0))
        return ((void*)0);
      ...
      if (left_spline_list == ((void*)0))
        {
          do { fputs ("warning: ", stderr); fprintf (stderr, "%s", "could not fit left spline list"); fputs (".\n", stderr); fflush (stderr); } while (0);
      ...
      if (right_spline_list == ((void*)0))
        {
          do { fputs ("warning: ", stderr); fprintf (stderr, "%s", "could not fit right spline list"); fputs (".\n", stderr); fflush (stderr); } while (0);
...
fit_one_spline (curve_type curve)
{
  ...
  unsigned i;
  vector_type t1_hat = *((curve)->start_tangent);
  vector_type t2_hat = *((curve)->end_tangent);
  real C[2][2] = { { 0.0, 0.0 }, { 0.0, 0.0 } };
  real X[2] = { 0.0, 0.0 };
  int Alen = ((curve)->length);
  vector_type *A;

  A = ((vector_type *) g_malloc0_n ((Alen * 2), sizeof (vector_type)));

  ((spline).v[0]) = ((curve)->point_list[0].coord);
  ((spline).v[3]) = ((curve)->point_list[(curve)->length-1].coord);
  ((spline).linearity) = 0;
  ...
  for (i = 0; i < ((curve)->length); i++)
    {
      A[i*2+0] = Vmult_scalar (t1_hat, (3.0 * (((curve)->point_list[i].t)) * ((1 - (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t))))));
      A[i*2+1] = Vmult_scalar (t2_hat, (3.0 * ((((curve)->point_list[i].t)) * (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t)))));
    }
  ...
  for (i = 0; i < ((curve)->length); i++)
    {
      ...
      temp0 = Vmult_scalar (start_vector, ((1 - (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t)))));
      temp1 = Vmult_scalar (start_vector, (3.0 * (((curve)->point_list[i].t)) * ((1 - (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t))))));
      temp2 = Vmult_scalar (end_vector, (3.0 * ((((curve)->point_list[i].t)) * (((curve)->point_list[i].t))) * (1 - (((curve)->point_list[i].t)))));
      temp3 = Vmult_scalar (end_vector, ((((curve)->point_list[i].t)) * (((curve)->point_list[i].t)) * (((curve)->point_list[i].t))));

      temp = make_vector (Vsubtract_point (((curve)->point_list[i].coord),
  ...
  if (C0_C1_det == 0.0)
    do { fputs ("fatal: ", stderr); fprintf (stderr, "%s", "zero determinant of C0*C1"); fputs (".\n", stderr); exit (1); } while (0);

  alpha1 = X_C1_det / C0_C1_det;
  alpha2 = C0_X_det / C0_C1_det;

  ((spline).v[1]) = Vadd_point (((spline).v[0]),
                                  Vmult_scalar (t1_hat, alpha1));
  ((spline).v[2]) = Vadd_point (((spline).v[3]),
...
reparameterize (curve_type curve, spline_type S)
{
  ...
  for (p = 0; p < ((curve)->length); p++)
    {
      real new_distance, old_distance;
      real_coordinate_type new_point;
      real_coordinate_type point = ((curve)->point_list[p].coord);
      real t = ((curve)->point_list[p].t);
      ...
      denominator = (((S1_t.x) * (S1_t.x)) + d.x * S2_t.x
       + ((S1_t.y) * (S1_t.y)) + d.y * S2_t.y);
      ...
      ((curve)->point_list[p].t) -= numerator / denominator;
      new_point = evaluate_spline (S, ((curve)->point_list[p].t));
...
find_subdivision (curve_type curve, unsigned initial)
{
  unsigned i, n_done;
  int best_point = -1;
  vector_type best = { 3.40282347e+38F, 3.40282347e+38F };
  unsigned search = subdivide_search * ((curve)->length);
  ...
  for (i = ((initial) + 1 >= ((curve)->length) ? ((curve)->cyclic) ? ((initial) + 1) % ((curve)->length) : ((curve)->length) : (initial) + 1), n_done = 0;
       i < ((curve)->length) - 1 && n_done < search;
       i = ((i) + 1 >= ((curve)->length) ? ((curve)->cyclic) ? ((i) + 1) % ((curve)->length) : ((curve)->length) : (i) + 1), n_done++)
    {
      if (test_subdivision_point (curve, i, &best))
        {
  ...
  if (count == subdivide_surround)
    {
      ...
      join = (((((in).dx < subdivide_threshold && (in).dy > subdivide_threshold) || ((in).dy < subdivide_threshold && (in).dx > subdivide_threshold)) && ((out).dx > subdivide_threshold && (out).dy > subdivide_threshold)) || ((((out).dx < subdivide_threshold && (out).dy > subdivide_threshold) || ((out).dy < subdivide_threshold && (out).dx > subdivide_threshold)) && ((in).dx > subdivide_threshold && (in).dy > subdivide_threshold)));
      if (join)
        {
          do { if ((in).dx < subdivide_threshold && (in).dx < best->dx) best->dx = (in).dx; } while (0);
          do { if ((in).dy < subdivide_threshold && (in).dy < best->dy) best->dy = (in).dy; } while (0);
          do { if ((out).dx < subdivide_threshold && (out).dx < best->dx) best->dx = (out).dx; } while (0);
          do { if ((out).dy < subdivide_threshold && (out).dy < best->dy) best->dy = (out).dy; } while (0);
        }
    }
...
set_initial_parameter_values (curve_type curve)
{
  ...
  ((curve)->point_list[0].t) = 0.0;

  for (p = 1; p < ((curve)->length); p++)
    {
      real_coordinate_type point = ((curve)->point_list[p].coord),
                           previous_p = ((curve)->point_list[p - 1].coord);
      real d = distance (point, previous_p);
      ((curve)->point_list[p].t) = ((curve)->point_list[p - 1].t) + d;
    }

  ((((curve)->point_list[(curve)->length-1].t) != 0.0) ? (void) (0) : __assert_fail ("((curve)->point_list[(curve)->length-1].t) != 0.0", "fit.c", 1608, __PRETTY_FUNCTION__));

  for (p = 1; p < ((curve)->length); p++)
    ((curve)->point_list[p].t) = ((curve)->point_list[p].t) / ((curve)->point_list[(curve)->length-1].t);
...
{
  vector_type tangent;
  vector_type **curve_tangent = to_start_point ? &(((curve)->start_tangent))
                                               : &(((curve)->end_tangent));
  unsigned n_points = 0;
  ...
  if (*curve_tangent == ((void*)0))
    {
      *curve_tangent = ((vector_type *) g_malloc_n ((1), sizeof (vector_type)));
      tangent = find_half_tangent (curve, to_start_point, &n_points);

      if (cross_curve)
        {
          curve_type adjacent_curve
            = to_start_point ? ((curve)->previous) : ((curve)->next);
      ...
      ((n_points > 0) ? (void) (0) : __assert_fail ("n_points > 0", "fit.c", 1652, __PRETTY_FUNCTION__));
...
find_half_tangent (curve_type c, boolean to_start_point, unsigned *n_points)
{
  ...
  real_coordinate_type tangent_point = ((c)->point_list[tangent_index].coord);
  vector_type tangent;
 ...
 break;
      ...
      this_point = ((c)->point_list[p * factor + tangent_index].coord);
...
find_error (curve_type curve, spline_type spline, unsigned *worst_point)
{
  ...
  real worst_error = 1.17549435e-38F;

  *worst_point = ((curve)->length) + 1;

  for (this_point = 0; this_point < ((curve)->length); this_point++)
    {
      real_coordinate_type curve_point = ((curve)->point_list[this_point].coord);
      real t = ((curve)->point_list[this_point].t);
...
spline_linear_enough (spline_type *spline, curve_type curve)
{
  ...
  real distance = 0.0;
# 1775 "fit.c"
  if (epsilon_equal (((*spline).v[0]).x, ((*spline).v[3]).x))
    {
      A = 1;
      B = 0;
      C = -((*spline).v[0]).x;
  ...
  else
    {
      ...
      slope = ((((*spline).v[3]).y - ((*spline).v[0]).y)
        / (((*spline).v[3]).x - ((*spline).v[0]).x));
      A = -slope;
      B = 1;
      C = slope * ((*spline).v[0]).x - ((*spline).v[0]).y;
    }
  ...
  ((*spline).linearity) = distance / ((curve)->length);
...
change_bad_lines (spline_list_type *spline_list)
{
  ...
  unsigned length = ((*spline_list).length);
  ...
  for (this_spline = 0; this_spline < length; this_spline++)
    {
      if ((((((*spline_list).data)[this_spline])).degree) == CUBIC)
        {
          found_cubic = true;
          break;
        }
  ...
  if (found_cubic)
    for (this_spline = 0; this_spline < length; this_spline++)
      {
        spline_type s = (((*spline_list).data)[this_spline]);

        if (((s).degree) == LINEAR)
          {

            if (((s).linearity) > line_reversion_threshold)
              {

                (((((*spline_list).data)[this_spline])).degree)
                  = CUBIC;
              }
      ...
      for (this_spline = 0; this_spline < length; this_spline++)
        {
          ...
          spline_type *s = &(((*l).data)[this_spline]);
          real_coordinate_type start = ((*s).v[0]);
          real_coordinate_type end = ((*s).v[3]);

          do { real delta = fabs (end.x - start.x); if (!epsilon_equal (delta, 0.0) && delta <= align_threshold) { spline_type *next = &(((*l).data)[((this_spline) + 1) % ((*l).length)]); spline_type *prev = &(((*l).data)[this_spline == 0 ? ((*l).length) - 1 : this_spline - 1]); ((*s).v[0]).x = ((*s).v[3]).x = ((*prev).v[3]).x = ((*next).v[0]).x = (start.x + end.x) / 2; spline_change = true; } } while (0);
          do { real delta = fabs (end.y - start.y); if (!epsilon_equal (delta, 0.0) && delta <= align_threshold) { spline_type *next = &(((*l).data)[((this_spline) + 1) % ((*l).length)]); spline_type *prev = &(((*l).data)[this_spline == 0 ? ((*l).length) - 1 : this_spline - 1]); ((*s).v[0]).y = ((*s).v[3]).y = ((*prev).v[3]).y = ((*next).v[0]).y = (start.y + end.y) / 2; spline_change = true; } } while (0);
          if (spline_change)
            {
...
{
  ((*list).length)++;
  list->data = (unsigned *)g_realloc(list->data,(((*list).length)) * sizeof(unsigned));

  list->data[((*list).length) - 1] = new_index;
}
