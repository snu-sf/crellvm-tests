...
new_curve (void)
{
  curve_type curve = ((struct curve *) g_malloc_n ((1), sizeof (struct curve)));

  curve->point_list = ((void*)0);
  ((curve)->length) = 0;
  ((curve)->cyclic) = false;
  ((curve)->start_tangent) = ((curve)->end_tangent) = ((void*)0);
  ((curve)->previous) = ((curve)->next) = ((void*)0);
...
init_curve (coordinate_type coord)
{
  ...
  curve->point_list = ((point_type *) g_malloc_n ((1), sizeof (point_type)));
  ((curve)->length) = 1;

  ((curve)->point_list[0].coord) = int_to_real_coord (coord);
...
{
  ((curve)->length)++;
  curve->point_list = g_realloc (curve->point_list,((curve)->length) * sizeof(point_type));
  ((curve)->point_list[(curve)->length-1].coord) = coord;
...
new_curve_list (void)
{
  ...
  curve_list.length = 0;
  curve_list.data = ((void*)0);
  curve_list.clockwise = (0);
...
free_curve_list_array (curve_list_array_type *curve_list_array)
{
  ...
  for (this_list = 0; this_list < ((*curve_list_array).length);
       this_list++)
...
{
  ((*l).length)++;
  l->data = g_realloc (l->data,( ((*l).length))*sizeof(curve_list_type));
  ((*l).data[((*l).length) - 1]) = curve_list;
}
