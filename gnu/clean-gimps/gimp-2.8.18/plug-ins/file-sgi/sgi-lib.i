...
sgiClose(sgi_t *sgip)
{
  ...
  if (sgip->mode == 1 && sgip->comp != 0)
  {




    fseek(sgip->file, 512, 0);
  ...
  if (sgip->table != ((void*)0))
  {
    free(sgip->table[0]);
    free(sgip->table);
  };

  if (sgip->length != ((void*)0))
  {
    free(sgip->length[0]);
    free(sgip->length);
  };
  ...
  switch (sgip->comp)
  {
    case 0 :
        ...
        if (offset != ftell(sgip->file))
          fseek(sgip->file, offset, 0);

        if (sgip->bpp == 1)
        {
          for (x = sgip->xsize; x > 0; x --, row ++)
            *row = _IO_getc (sgip->file);
        }
        else
        {
          for (x = sgip->xsize; x > 0; x --, row ++)
        ...
        int zsize)
{
  ...
  if ((sgip = sgiOpenFile(file, mode, comp, bpp, xsize, ysize, zsize)) == ((void*)0))
    fclose(file);
            ...
            int zsize)
{
  ...
  if ((sgip = calloc(sizeof(sgi_t), 1)) == ((void*)0))
    return (((void*)0));
  ...
  switch (mode)
  {
   ...
   if(magic != 474) {
     ...
     sgip->swapBytes = 1;
        ...
        sgip->comp = _IO_getc (sgip->file);
        sgip->bpp = _IO_getc (sgip->file);
        getshort(sgip);
 ...
 if (xsize < 1 ||
     ...
     comp < 0 || comp > 2)
        ...
        putshort(474, sgip);
        _IO_putc ((sgip->comp = comp) != 0, sgip->file);
        _IO_putc (sgip->bpp = bpp, sgip->file);
        putshort(3, sgip);
          ...
          int z)
{
  ...
  switch (sgip->comp)
  {
    ...
    case 2 :
        ...
        fseek(sgip->file, sgip->firstrow, 0);
...
putlong(long n,
        sgi_t *sgip)
{
  if (_IO_putc (n >> 24, sgip->file) == (-1))
    return ((-1));
  if (_IO_putc (n >> 16, sgip->file) == (-1))
    return ((-1));
  if (_IO_putc (n >> 8, sgip->file) == (-1))
    return ((-1));
  if (_IO_putc (n, sgip->file) == (-1))
    return ((-1));
  else
    return (0);
}
          ...
          int xsize)
{
 ...
 length;
  ...
  while (xsize > 0)
  {
    if ((ch = _IO_getc (sgip->file)) == (-1))
      return (-1);
    length ++;

    count = (((ch & 127) < (xsize)) ? (ch & 127) : (xsize));
    if (count == 0)
      break;
           ...
           int xsize)
{
  ...
  for (x = xsize, length = 0; x > 0;)
  {
    ...
    while (count > 0)
    {
      ...
      if (_IO_putc (128 | i, sgip->file) == (-1))
        return (-1);
      length ++;

      while (i > 0)
      {
 if (_IO_putc (*start, sgip->file) == (-1))
          return (-1);
        start ++;
        i --;
        length ++;
    ...
    while (count > 0)
    {
      ...
      if (_IO_putc (i, sgip->file) == (-1))
        return (-1);
      length ++;

      if (_IO_putc (repeat, sgip->file) == (-1))
        return (-1);
      length ++;
    };
  };
            ...
            int xsize)
{
  ...
  for (x = xsize, length = 0; x > 0;)
  {
    ...
    while (count > 0)
    {
      ...
      if (putshort(128 | i, sgip) == (-1))
        return (-1);
      length ++;

      while (i > 0)
      {
 if (putshort(*start, sgip) == (-1))
          return (-1);
        start ++;
        i --;
        length ++;
    ...
    while (count > 0)
    {
      ...
      if (putshort(i, sgip) == (-1))
        return (-1);
      length ++;

      if (putshort(repeat, sgip) == (-1))
        return (-1);
      length ++;
    };
  };
