...
static int fits_scanfdouble (const char *buf, double *value)
{
 ...
 if ( bufcopy )
 {
   ...
   else
   {
     ...
     if (((g_ascii_table[(guchar) (*bufptr)] & G_ASCII_DIGIT) != 0))
     {
       gchar *endptr;
...
{static char errmsg[256];
 ...
 if (fits_n_error <= 0) return (((void*)0));
...

{
  if (fits_n_error < 16)
  {
    strncpy (fits_error[fits_n_error], errmsg, 256);
    fits_error[fits_n_error++][256 -1] = '\0';
  }
}
# 522 "fits-io.c"
static void fits_drop_error (void)
...
{int reading, writing, n_rec, n_hdr;
 ...
 if ((filename == ((void*)0)) || (*filename == '\0') || (openmode == ((void*)0)))
   { fits_set_error ("fits_open: Invalid parameters"); return (((void*)0)); };

 reading = (strcmp (openmode, "r") == 0);
 writing = (strcmp (openmode, "w") == 0);
 if ((!reading) && (!writing))
   { fits_set_error ("fits_open: Invalid openmode"); return (((void*)0)); };

 fp = fopen (filename, reading ? "rb" : "wb");
 if (fp == ((void*)0)) { fits_set_error ("fits_open: fopen() failed"); return (((void*)0)); };

 ff = fits_new_filestruct ();
 if (ff == ((void*)0))
 {
   fclose (fp);
   { fits_set_error ("fits_open: No more memory"); return (((void*)0)); };
 }
 ...
 for (n_hdr = 0; ; n_hdr++)
 {
   ...
   if (fseek (fp, hdulist->data_offset+hdulist->data_size, 0) < 0)
     break;
 }
...
{
 if (ff == ((void*)0)) { fits_set_error ("fits_close: Invalid parameter"); return; };
...
{FITS_HDU_LIST *newhdu, *hdu;
 ...
 if (ff->openmode != 'w')
   { fits_set_error ("fits_add_hdu: file not open for writing"); return (((void*)0)); };

 newhdu = fits_new_hdulist ();
 if (newhdu == ((void*)0)) return (((void*)0));

 if (ff->hdu_list == ((void*)0))
 {
   ff->hdu_list = newhdu;
 }
 else
...
{int k;
 ...
 if (hdulist->naddcards >= 128) return (-1);

 k = strlen (card);
 if (k < 80)
 {
   memset (&(hdulist->addcards[hdulist->naddcards][k]), ' ', 80 -k);
   memcpy (hdulist->addcards[(hdulist->naddcards)++], card, k);
 }
 else
 {
   memcpy (hdulist->addcards[(hdulist->naddcards)++], card, 80);
 }
 return (0);
}
# 758 "fits-io.c"
...
{int k;
 ...
 printf ("pixmin        : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->pixmin)));
 printf ("pixmax        : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->pixmax)));
 ...
 if (hdr->used.datamin)
   printf ("datamin       : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->datamin)));
 else
   printf ("datamin       : not used\n");
 if (hdr->used.datamax)
   printf ("datamax       : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->datamax)));
 else
 ...
 if (hdr->used.bscale)
   printf ("bscale        : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->bscale)));
 else
   printf ("bscale        : not used\n");
 if (hdr->used.bzero)
   printf ("bzero         : %s\n", g_ascii_dtostr (buf, sizeof(buf), (gdouble)(hdr->bzero)));
 else
...
{char record[2880];
 FITS_RECORD_LIST *start_list = ((void*)0), *cu_record = ((void*)0), *new_record;
 FITS_DATA *fdat;
 int k, simple, xtension;

 *nrec = 0;

 k = fread (record, 1, 2880, fp);
 if (k != 2880)
   { fits_set_error ("fits_read_header: Error in read of first record"); return (((void*)0)); };

 simple = (strncmp (record, "SIMPLE  ", 8) == 0);
 xtension = (strncmp (record, "XTENSION", 8) == 0);
 if ((!simple) && (!xtension))
   { fits_set_error ("fits_read_header: Missing keyword SIMPLE or XTENSION"); return (((void*)0)); };

 if (simple)
 {
   fdat = fits_decode_card (record, typ_fbool);
   if (fdat && !fdat->fbool)
     fits_set_error ("fits_read_header (warning): keyword SIMPLE does not have value T");
 ...
 for (;;)
 {
   ...
   if (new_record == ((void*)0))
   {
     ...
     { fits_set_error ("fits_read_header: Not enough memory"); return (((void*)0)); };
   }
   memcpy (new_record->data, record, 2880);
   new_record->next_record = ((void*)0);
   (*nrec)++;

   if (start_list == ((void*)0))
   ...
   if (fits_search_card (cu_record, "END") != ((void*)0)) break;

    k = fread (record, 1, 2880, fp);
    if (k != 2880)
      { fits_set_error ("fits_read_header: Error in read of record"); return (((void*)0)); };
 }
 return (start_list);
}
# 903 "fits-io.c"
...
{int numcards;
 ...
 if (ff->openmode != 'w')
   { fits_set_error ("fits_write_header: file not open for writing"); return (-1); };

 numcards = 0;

 if (hdulist->used.simple)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20s%50s", "SIMPLE", 1 ? "T" : "F", " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }
 else if (hdulist->used.xtension)
 {
   {char card[81]; int k; sprintf (card, "%-8.8s= \'%s", "XTENSION", hdulist->xtension); for (k = strlen (card); k < 81; k++) card[k] = ' '; k = strlen ("XTENSION"); if (k < 8) card[19] = '\''; else card[11+k] = '\''; fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", "BITPIX", (long)hdulist->bitpix, " "); fwrite (card, 1, 80, ff->fp); };
 numcards++;

 {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", "NAXIS", (long)hdulist->naxis, " "); fwrite (card, 1, 80, ff->fp); };
 numcards++;

 for (k = 0; k < hdulist->naxis; k++)
 {char naxisn[10];
   sprintf (naxisn, "NAXIS%d", k+1);
   {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", naxisn, (long)hdulist->naxisn[k], " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.extend)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20s%50s", "EXTEND", hdulist->extend ? "T" : "F", " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.groups)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20s%50s", "GROUPS", hdulist->groups ? "T" : "F", " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.pcount)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", "PCOUNT", (long)hdulist->pcount, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }
 if (hdulist->used.gcount)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", "GCOUNT", (long)hdulist->gcount, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.bzero)
 {
   {char card[81], dbl[21], *istr; g_ascii_formatd (dbl, sizeof(dbl), "%f", (gdouble)hdulist->bzero); istr = strstr (dbl, "e"); if (istr) *istr = 'E'; sprintf (card, "%-8.8s= %20.20s%50s", "BZERO", dbl, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }
 if (hdulist->used.bscale)
 {
   {char card[81], dbl[21], *istr; g_ascii_formatd (dbl, sizeof(dbl), "%f", (gdouble)hdulist->bscale); istr = strstr (dbl, "e"); if (istr) *istr = 'E'; sprintf (card, "%-8.8s= %20.20s%50s", "BSCALE", dbl, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.datamin)
 {
   {char card[81], dbl[21], *istr; g_ascii_formatd (dbl, sizeof(dbl), "%f", (gdouble)hdulist->datamin); istr = strstr (dbl, "e"); if (istr) *istr = 'E'; sprintf (card, "%-8.8s= %20.20s%50s", "DATAMIN", dbl, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }
 if (hdulist->used.datamax)
 {
   {char card[81], dbl[21], *istr; g_ascii_formatd (dbl, sizeof(dbl), "%f", (gdouble)hdulist->datamax); istr = strstr (dbl, "e"); if (istr) *istr = 'E'; sprintf (card, "%-8.8s= %20.20s%50s", "DATAMAX", dbl, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }

 if (hdulist->used.blank)
 {
   {char card[81]; sprintf (card, "%-8.8s= %20ld%50s", "BLANK", (long)hdulist->blank, " "); fwrite (card, 1, 80, ff->fp); };
   numcards++;
 }
 ...
 {
   fwrite (hdulist->addcards, 80, hdulist->naddcards, ff->fp);
   numcards += hdulist->naddcards;
 }

 {char card[81]; sprintf (card, "%-80.80s", "END"); fwrite (card, 1, 80, ff->fp); };
 numcards++;
...
# 1046 "fits-io.c"
 ...
 if (hdulist == ((void*)0))
   { fits_set_error ("fits_decode_header: Not enough memory"); return (((void*)0)); };
 ...
 {strcpy (key, "NAXIS"); fdat = fits_decode_card (fits_search_card (hdr, "NAXIS"), typ_flong); if (fdat == ((void*)0)) goto err_missing; };
 hdulist->naxis = fdat->flong;

 {strcpy (key, "BITPIX"); fdat = fits_decode_card (fits_search_card (hdr, "BITPIX"), typ_flong); if (fdat == ((void*)0)) goto err_missing; };
 ...
 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"GCOUNT"), typ_flong); hdulist->used.gcount = (mfdat != ((void*)0)); if (hdulist->used.gcount) hdulist->gcount = mfdat->flong; };
 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"PCOUNT"), typ_flong); hdulist->used.pcount = (mfdat != ((void*)0)); if (hdulist->used.pcount) hdulist->pcount = mfdat->flong; };

 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"GROUPS"), typ_fbool); hdulist->used.groups = (mfdat != ((void*)0)); if (hdulist->used.groups) hdulist->groups = mfdat->fbool; };
 random_groups = hdulist->used.groups && hdulist->groups;

 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"EXTEND"), typ_fbool); hdulist->used.extend = (mfdat != ((void*)0)); if (hdulist->used.extend) hdulist->extend = mfdat->fbool; };
 ...
 data_size = (data_size + 2880 - 1) / 2880;
 data_size *= 2880;
 hdulist->data_size = data_size;


 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"BLANK"), typ_flong); hdulist->used.blank = (mfdat != ((void*)0)); if (hdulist->used.blank) hdulist->blank = mfdat->flong; };

 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"DATAMIN"), typ_fdouble); hdulist->used.datamin = (mfdat != ((void*)0)); if (hdulist->used.datamin) hdulist->datamin = mfdat->fdouble; };
 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"DATAMAX"), typ_fdouble); hdulist->used.datamax = (mfdat != ((void*)0)); if (hdulist->used.datamax) hdulist->datamax = mfdat->fdouble; };

 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"BZERO"), typ_fdouble); hdulist->used.bzero = (mfdat != ((void*)0)); if (hdulist->used.bzero) hdulist->bzero = mfdat->fdouble; };
 {FITS_DATA *mfdat = fits_decode_card (fits_search_card (hdr,"BSCALE"), typ_fdouble); hdulist->used.bscale = (mfdat != ((void*)0)); if (hdulist->used.bscale) hdulist->bscale = mfdat->fdouble; };
 ...
 {char msg[160];
   sprintf (msg, "fits_decode_header: IEEE floating point format required for BITPIX=%d\nis not supported on this machine", hdulist->bitpix);
...
{register int maxelem;
 ...
 unsigned char pixdat[4096];
 int nelem, bpp;
 int blank_found = 0, nan_found = 0;

 if (fseek (fp, hdu->data_offset, 0) < 0)
   { fits_set_error ("fits_eval_pixrange: cant position file"); return (-1); };

 bpp = hdu->bpp;
 nelem = hdu->udata_size / bpp;
 ...
 switch (hdu->bitpix)
 {
   case 8: {
     ...
     while (nelem > 0)
     {
       ...
       if (fread ((char *)pixdat, bpp, maxelem, fp) != maxelem)
         { fits_set_error ("fits_eval_pixrange: error on read bitpix 8 data"); return (-1); };
   ...
   case 16: {
     ...
     while (nelem > 0)
     {
       ...
       nelem -= maxelem;
       if (fread ((char *)pixdat, bpp, maxelem, fp) != maxelem)
         { fits_set_error ("fits_eval_pixrange: error on read bitpix 16 data"); return (-1); };
   ...
   case 32: {
     ...
     while (nelem > 0)
     {
       ...
       nelem -= maxelem;
       if (fread ((char *)pixdat, bpp, maxelem, fp) != maxelem)
         { fits_set_error ("fits_eval_pixrange: error on read bitpix 32 data"); return (-1); };
       ...
       {FITS_BITPIX32 blankval = (FITS_BITPIX32)hdu->blank;
         ...
         while (maxelem-- > 0)
         {
           pixval = ((ptr[0] << 24) | (ptr[1] << 16) | (ptr[2] << 8) | ptr[3]);
           ptr += 4;
           if (pixval != blankval)
           {
             if (pixval < minval) minval = pixval;
   ...
   case -32: {
     ...
     while (nelem > 0)
     {
       ...
       while (maxelem-- > 0)
       {
         if (!fits_nan_32 (ptr))
         {
           { if (fits_ieee32_intel) {unsigned char uc[4]; uc[0] = ptr[3]; uc[1] = ptr[2]; uc[2] = ptr[1]; uc[3] = ptr[0]; pixval = *(FITS_BITPIXM32 *)uc; } else if (fits_ieee32_motorola) { pixval = *(FITS_BITPIXM32 *)ptr; } else if (fits_ieee64_motorola) {FITS_BITPIXM64 m64; unsigned char *uc= (unsigned char *)&m64; uc[0]=ptr[0]; uc[1]=ptr[1]; uc[2]=ptr[2]; uc[3]=ptr[3]; uc[4]=uc[5]=uc[6]=uc[7]=0; pixval = (FITS_BITPIXM32)m64; } else if (fits_ieee64_intel) {FITS_BITPIXM64 i64; unsigned char *uc= (unsigned char *)&i64; uc[0]=uc[1]=uc[2]=uc[3]=0; uc[7]=ptr[0]; uc[6]=ptr[1]; uc[5]=ptr[2]; uc[4]=ptr[3]; pixval = (FITS_BITPIXM32)i64;}};
           ptr += 4;
           if (first)
           {
             first = 0;
   ...
   case -64: {
     ...
     while (nelem > 0)
     {
       ...
       nelem -= maxelem;
       if (fread ((char *)pixdat, bpp, maxelem, fp) != maxelem)
         { fits_set_error ("fits_eval_pixrange: error on read bitpix -64 data"); return (-1); };
       ...
       while (maxelem-- > 0)
       {
         if (!fits_nan_64 (ptr))
         {
           { if (fits_ieee64_intel) {unsigned char uc[8]; uc[0] = ptr[7]; uc[1] = ptr[6]; uc[2] = ptr[5]; uc[3] = ptr[4]; uc[4] = ptr[3]; uc[5] = ptr[2]; uc[6] = ptr[1]; uc[7] = ptr[0]; pixval = *(FITS_BITPIXM64 *)uc; } else pixval = *(FITS_BITPIXM64 *)ptr; };
           ptr += 8;
           if (first)
           {
             first = 0;
...
{static FITS_DATA data;
 long l_long;
 double l_double;
 char l_card[80 +1], msg[256];
 char *cp, *dst, *end;
 int ErrCount = 0;

 if (card == ((void*)0)) return (((void*)0));

 memcpy (l_card, card, 80);
 l_card[80] = '\0';

 if (strncmp (card+8, "= ", 2) != 0)
 {
   sprintf (msg, "fits_decode_card (warning): Missing value indicator '= ' for %8.8s", l_card);
...
{int key_len, k;
 ...
 if (key_len == 0)
   { fits_set_error ("fits_search_card: Invalid parameter"); return (((void*)0)); };
...
{FITS_HDU_LIST *hdulist;
 ...
 if (ff == ((void*)0))
   { fits_set_error ("fits_image_info: ff is NULL"); return (((void*)0)); };

 if (ff->openmode != 'r')
   { fits_set_error ("fits_image_info: file not open for reading"); return (((void*)0)); };

 if ((picind < 1) || (picind > ff->n_pic))
   { fits_set_error ("fits_image_info: picind out of range"); return (((void*)0)); };

 firstpic = 1;
 for (hdulist = ff->hdu_list; hdulist != ((void*)0); hdulist = hdulist->next_hdu)
 {
...
{FITS_HDU_LIST *hdulist;
 ...
 if (hdulist == ((void*)0)) return (((void*)0));

 pic_size = hdulist->bpp * hdulist->naxisn[0] * hdulist->naxisn[1];
 offset = hdulist->data_offset + (hdupicind-1)*pic_size;
 if (fseek (ff->fp, offset, 0) < 0)
   { fits_set_error ("fits_seek_image: Unable to position to image"); return (((void*)0)); };
...
{double offs, scale;
 ...
 if (ff->openmode != 'r') return (-1);
 if (trans->dsttyp != 'c') return (-1);
   ...
   case 32:
     while (npix > 0)
     {
       ...
       if (hdulist->used.blank)
       {
         ...
         while (maxelem--)
         {
           bp32 = ((pix[0] << 24) | (pix[1] << 16) | (pix[2] << 8) | pix[3]);
           if (bp32 == bp32blank)
             *(cdata++) = creplace;
           else
           {
   ...
   case -32:
     while (npix > 0)
     {
       ...
       while (maxelem--)
       {
         ...
         {
           { if (fits_ieee32_intel) {unsigned char uc[4]; uc[0] = pix[3]; uc[1] = pix[2]; uc[2] = pix[1]; uc[3] = pix[0]; bpm32 = *(FITS_BITPIXM32 *)uc; } else if (fits_ieee32_motorola) { bpm32 = *(FITS_BITPIXM32 *)pix; } else if (fits_ieee64_motorola) {FITS_BITPIXM64 m64; unsigned char *uc= (unsigned char *)&m64; uc[0]=pix[0]; uc[1]=pix[1]; uc[2]=pix[2]; uc[3]=pix[3]; uc[4]=uc[5]=uc[6]=uc[7]=0; bpm32 = (FITS_BITPIXM32)m64; } else if (fits_ieee64_intel) {FITS_BITPIXM64 i64; unsigned char *uc= (unsigned char *)&i64; uc[0]=uc[1]=uc[2]=uc[3]=0; uc[7]=pix[0]; uc[6]=pix[1]; uc[5]=pix[2]; uc[4]=pix[3]; bpm32 = (FITS_BITPIXM32)i64;}};
   ...
   case -64:
     while (npix > 0)
     {
       ...
       while (maxelem--)
       {
         ...
         {
           { if (fits_ieee64_intel) {unsigned char uc[8]; uc[0] = pix[7]; uc[1] = pix[6]; uc[2] = pix[5]; uc[3] = pix[4]; uc[4] = pix[3]; uc[5] = pix[2]; uc[6] = pix[1]; uc[7] = pix[0]; bpm64 = *(FITS_BITPIXM64 *)uc; } else bpm64 = *(FITS_BITPIXM64 *)pix; };
