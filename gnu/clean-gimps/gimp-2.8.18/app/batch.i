...
# 25 "./core/core-types.h" 2

# 1 "./base/base-types.h" 1
# 22 "./base/base-types.h"
...
# 30 "batch.c" 2
# 1 "./core/gimpparamspecs.h" 1
# 36 "./core/gimpparamspecs.h"
GType gimp_int32_get_type (void) __attribute__((__const__));
# 47 "./core/gimpparamspecs.h"
...
static void batch_exit_after_callback (Gimp *gimp) __attribute__((__noreturn__));
           ...
           const gchar **batch_commands)
{
  ...
  exit_id = g_signal_connect_data ((gimp), ("exit"), (((GCallback) (batch_exit_after_callback))), (((void*)0)), ((void*)0), G_CONNECT_AFTER);
      ...
      if (! batch_interpreter)
        {
          ...
          if (gimp->be_verbose)
            g_printerr (gettext ("No batch interpreter specified, using the default " "'%s'.\n"), batch_interpreter);
  ...
  if (strcmp (batch_interpreter, "plug-in-script-fu-eval") == 0 &&
      strcmp (batch_commands[0], "-") == 0)
    {
      ...
      if (procedure)
        batch_run_cmd (gimp, proc_name, procedure,
                       GIMP_RUN_NONINTERACTIVE, ((void*)0));
      else
        g_message (gettext ("The batch interpreter '%s' is not available. " "Batch mode disabled."), proc_name);
  ...
  else
    {
      ...
      if (eval_proc)
        ...
        {
          g_message (gettext ("The batch interpreter '%s' is not available. " "Batch mode disabled."), batch_interpreter);
               ...
               const gchar *cmd)
{
  ...
  if (procedure->num_args > i && (((__extension__ ({ GTypeInstance *__inst = (GTypeInstance*) ((procedure->args[i])); GType __t = ((gimp_param_int32_get_type ())); gboolean __r; if (!__inst) __r = (0); else if (__inst->g_class && __inst->g_class->g_type == __t) __r = (!(0)); else __r = g_type_check_instance_is_a (__inst, __t); __r; })))))
    g_value_set_int (&args->values[i++], run_mode);

  if (procedure->num_args > i && (((__extension__ ({ GTypeInstance *__inst = (GTypeInstance*) ((procedure->args[i])); GType __t = ((gimp_param_string_get_type ())); gboolean __r; if (!__inst) __r = (0); else if (__inst->g_class && __inst->g_class->g_type == __t) __r = (!(0)); else __r = g_type_check_instance_is_a (__inst, __t); __r; })))))
