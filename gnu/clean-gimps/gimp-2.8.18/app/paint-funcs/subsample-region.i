# 1 "subsample-region.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 317 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "subsample-region.c" 2
# 18 "subsample-region.c"
# 1 "../../config.h" 1
# 19 "subsample-region.c" 2
...
# 27 "subsample-region.c" 2

# 1 "./subsample-region.h" 1
# 22 "./subsample-region.h"
                  ...
                  gint subsample)
{
  ...
  src = ((guchar *) g_malloc_n ((orig_width * bytes), sizeof (guchar)));
  dest = destPR->data;


  row = ((gdouble *) g_malloc_n ((width * bytes), sizeof (gdouble)));
  x_frac = ((gdouble *) g_malloc_n ((width + orig_width), sizeof (gdouble)));
  ...
  for (i = 0; i < width + orig_width; i++)
    {
      if (x_sum + x_ratio <= (src_col + 1 + (0.0001)))
        {
          x_sum += x_ratio;
  ...
  for (i = 0; i < height; )
    {
      ...
      if (y_sum + y_ratio <= (src_row + 1 + (0.0001)))
        {
          y_sum += y_ratio;
          y_frac = y_sum - y_last;
          advance_dest = (!(0));
                          ...
                          gint subsample)
{
  ...
  do{ if (cmap != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Paint-Funcs", ((const char*) (__func__)), "cmap != NULL"); return; }; }while (0);
  ...
  row = ((gdouble *) g_malloc0_n ((width * bytes), sizeof (gdouble)));
  x_frac = ((gdouble *) g_malloc_n ((width + orig_width), sizeof (gdouble)));
          ...
          if (has_alpha)
            {
              if (s[1] & 0x80)
                {
                  r[0] += cmap[index++] * tot_frac;
                  r[1] += cmap[index++] * tot_frac;
                  r[2] += cmap[index++] * tot_frac;
                  r[3] += tot_frac;
                }
