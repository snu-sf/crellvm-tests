...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimp-composite.c" 2
# 21 "gimp-composite.c"
# 1 "../../config.h" 1
# 22 "gimp-composite.c" 2
...
gimp_composite_dispatch (GimpCompositeContext *ctx)
{
  ...
  if (function)
    {
      if (gimp_composite_options.bits & 0x4)
        {
          g_print ("%s %s %s %s = %p\n",
                     ...
                     gboolean use_cpu_accel)
{
  ...
  if ((p = g_getenv ("GIMP_COMPOSITE")))
    {
      gimp_composite_options.bits = strtoul(p, ((void*)0), 16);
    }

  if (! use_cpu_accel)
    gimp_composite_options.bits |= 0x2;

  if (be_verbose)
    g_printerr ("gimp_composite: verbose=%s\n",
                (gimp_composite_options.bits & 0x4) ?
                "yes" : "no");

  gimp_composite_generic_install ();
# 357 "gimp-composite.c"
  if (! (gimp_composite_options.bits & 0x2))
    {
      extern gboolean gimp_composite_mmx_install (void);
      extern gboolean gimp_composite_sse_install (void);
...
{
  return ! (gimp_composite_options.bits & 0x2);
}
