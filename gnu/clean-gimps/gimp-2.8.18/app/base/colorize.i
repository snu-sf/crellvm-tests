...
colorize_init (Colorize *colorize)
{
  ...
  do{ if (colorize != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "colorize != NULL"); return; }; }while (0);
  ...
  for (i = 0; i < 256; i ++)
    {
      colorize->lum_red_lookup[i] = i * (0.2126);
      colorize->lum_green_lookup[i] = i * (0.7152);
      colorize->lum_blue_lookup[i] = i * (0.0722);
    }
}
  ...
  while (h--)
    {
      ...
      while (w--)
        {
          lum = (colorize->lum_red_lookup[s[0]] +
                 colorize->lum_green_lookup[s[1]] +
                 colorize->lum_blue_lookup[s[2]]);
          ...
          d[0] = colorize->final_red_lookup[lum];
          d[1] = colorize->final_green_lookup[lum];
          d[2] = colorize->final_blue_lookup[lum];
