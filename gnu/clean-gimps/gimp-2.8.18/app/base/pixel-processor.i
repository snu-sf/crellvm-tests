...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "pixel-processor.c" 2
# 20 "pixel-processor.c"
# 1 "../../config.h" 1
# 21 "pixel-processor.c" 2
...
static GThreadPool *pool = ((void*)0);
static GMutex *pool_mutex = ((void*)0);
static GCond *pool_cond = ((void*)0);
      ...
      switch (processor->num_regions)
        ...
        case 1:
          ((p1_func) processor->func) (processor->data,
                                       processor->regions[0] ? &tr[0] : ((void*)0));
          break;

        case 2:
          ((p2_func) processor->func) (processor->data,
                                       processor->regions[0] ? &tr[0] : ((void*)0),
                                       processor->regions[1] ? &tr[1] : ((void*)0));
          break;
        ...
        case 3:
          ((p3_func) processor->func) (processor->data,
                                       processor->regions[0] ? &tr[0] : ((void*)0),
                                       processor->regions[1] ? &tr[1] : ((void*)0),
                                       processor->regions[2] ? &tr[2] : ((void*)0));
          break;
        ...
        case 4:
          ((p4_func) processor->func) (processor->data,
                                       ...
                                       processor->regions[1] ? &tr[1] : ((void*)0),
                                       processor->regions[2] ? &tr[2] : ((void*)0),
                                       processor->regions[3] ? &tr[3] : ((void*)0));
          break;
      ...
      if (progress_func)
        {
          ...
          if (((now.tv_sec - last_time.tv_sec) * 1024 +
               (now.tv_usec - last_time.tv_usec) / 1024) > 64)
            {
                           ...
                           gpointer progress_data)
{
  ...
  gulong tiles = pixels / (64 * 64);


  if (pool && tiles > 8)
    {
      GError *error = ((void*)0);
      gint tasks = (((tiles / 8) < (g_thread_pool_get_max_threads (pool))) ? (tiles / 8) : (g_thread_pool_get_max_threads (pool)));
          ...
          while (processor->threads != 0)
            {
              ...
              g_time_val_add (&timeout, 64 * 1024);
...
pixel_regions_process_parallel_valist (PixelProcessorFunc func,
                                       ...
                                       va_list ap)
{
  PixelProcessor processor = { ((void*)0), };
  gint i;

  for (i = 0; i < num_regions; i++)
    processor.regions[i] = __builtin_va_arg(ap, PixelRegion *);
...
pixel_processor_set_num_threads (gint num_threads)
{
  ...
  do{ if (num_threads > 0 && num_threads <= 16) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "num_threads > 0 && num_threads <= GIMP_MAX_NUM_THREADS"); return; }; }while (0);
  ...
  if (num_threads < 2)
  ...
  else
    {
      ...
      if (pool)
        ...
        {
          pool = g_thread_pool_new ((GFunc) do_parallel_regions, ((void*)0),
                                    num_threads, (!(0)), &error);
                                ...
                                ...)
{
  ...
  __builtin_va_start(va, num_regions);
