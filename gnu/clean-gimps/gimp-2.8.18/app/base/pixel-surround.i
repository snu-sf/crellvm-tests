...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "pixel-surround.c" 2
# 18 "pixel-surround.c"
# 1 "../../config.h" 1
# 19 "pixel-surround.c" 2
...
# 27 "pixel-surround.c" 2
# 1 "./pixel-surround.h" 1
# 26 "./pixel-surround.h"
typedef enum
{
                    ...
                    PixelSurroundMode mode)
{
  ...
  do{ if (width < 64) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "width < TILE_WIDTH"); return (((void*)0)); }; }while (0);
  do{ if (height < 64) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "height < TILE_WIDTH"); return (((void*)0)); }; }while (0);

  surround = ((PixelSurround*) g_slice_alloc0 (sizeof (PixelSurround)));
  ...
  surround->bg = ((guchar *) g_malloc0_n ((surround->rowstride * height), sizeof (guchar)));
  surround->buf = ((guchar *) g_malloc_n ((surround->rowstride * height), sizeof (guchar)));
  surround->mode = mode;

  return surround;
}
# 109 "pixel-surround.c"
void
      ...
      while (i < surround->w)
        {
          ...
          for (j = 0; j < surround->h;)
            ...
            start:
              ...
              w = (((w) < (surround->w - i)) ? (w) : (surround->w - i));
              h = (((h) < (surround->h - j)) ? (h) : (surround->h - j));
...
pixel_surround_release (PixelSurround *surround)
{
  if (surround->tile)
    {
      tile_release (surround->tile, (0));
      surround->tile = ((void*)0);
    }
}
# 238 "pixel-surround.c"
void
pixel_surround_destroy (PixelSurround *surround)
{
  do{ if (surround != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "surround != NULL"); return; }; }while (0);
  ...
  do { if (1) g_slice_free1 (sizeof (PixelSurround), (surround)); else (void) ((PixelSurround*) 0 == (surround)); } while (0);
}
...
{
  guchar *dest = surround->bg;
  gint bytes = (((w) < (surround->w)) ? (w) : (surround->w)) * surround->bpp;
                         ...
                         gint h)
{
  ...
  gint rows = (((h) < (surround->h)) ? (h) : (surround->h));
                         ...
                         gint *rowstride)
{
  ...
  if (! surround->tile)
    {
      surround->tile = tile_manager_get_tile (surround->mgr, x, y, (!(0)), (0));

      if (surround->tile)
        {

          surround->tile_x = x & ~(64 - 1);
          surround->tile_y = y & ~(64 - 1);
  ...
  if (x < 0)
    *w = (((- x) < (surround->w)) ? (- x) : (surround->w));
  else
    *w = surround->w;

  if (y < 0)
    *h = (((- y) < (surround->h)) ? (- y) : (surround->h));
  else
    *h = surround->h;
