...
tile_swap_init (const gchar *path)
{
  ...
  do{ if (gimp_swap_file == ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "gimp_swap_file == NULL"); return; }; }while (0);
  do{ if (path != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "path != NULL"); return; }; }while (0);
...
# 279 "tile-swap.c"
  ...
  do { if (1) g_slice_free1 (sizeof (SwapFile), (gimp_swap_file)); else (void) ((SwapFile*) 0 == (gimp_swap_file)); } while (0);
...
tile_swap_test (void)
{
  do{ if (gimp_swap_file != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "gimp_swap_file != NULL"); return ((0)); }; }while (0);


  gimp_swap_file->fd = open (gimp_swap_file->filename,
                               0100 | 02 | 0 | 0,
                               0400 | 0200);
      ...
      if (err <= 0)
        {
          if (read_err_msg)
            g_message ("unable to read tile data from disk: "
                       "%s (%d/%d bytes read)",
                       g_strerror ((*__errno_location ())), err, nleft);
          read_err_msg = (0);
          return;
        }
...
{
  do{ if (swap_file->fd == -1) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "swap_file->fd == -1"); return; }; }while (0);
  ...
  if (swap_file->fd == -1)
    g_message (gettext ("Unable to open swap file. GIMP has run out of memory " "and cannot use the swap file. Some parts of your images " "may be corrupted. Try to save your work using different " "filenames, restart GIMP and check the location of the " "swap directory in your Preferences."));
  ...
  if (swap_file->swap_file_end > new_size)
    {
      if (ftruncate (swap_file->fd, new_size) != 0)
        {
          g_message (gettext ("Failed to resize swap file: %s"), g_strerror ((*__errno_location ())));
          return;
        }
    }
...
{
  do { if (1) g_slice_free1 (sizeof (SwapFileGap), (gap)); else (void) ((SwapFileGap*) 0 == (gap)); } while (0);
}
