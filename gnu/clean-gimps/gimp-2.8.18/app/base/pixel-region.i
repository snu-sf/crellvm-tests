                      ...
                      gint subsample)
{
  ...
  if (subsample == 1)
  ...
  else
    {
      while (x < end)
        {
          tile = tile_manager_get_tile (PR->tiles, x, y, (!(0)), (0));
          tile_data = tile_data_pointer (tile, x, y);
          npixels = tile_ewidth (tile) - (x % 64);
  ...
  while (y < end)
    {
      tile = tile_manager_get_tile (PR->tiles, x, y, (!(0)), (0));
      tile_data = tile_data_pointer (tile, x, y);
      boundary = y + (tile_eheight(tile) - (y % 64));
                        ...
                        ...)
{
  ...
  PRI = ((PixelRegionIterator*) g_slice_alloc0 (sizeof (PixelRegionIterator)));

  __builtin_va_start(ap, num_regions);
  ...
  while (num_regions --)
    {
      ...
      PR = __builtin_va_arg(ap, PixelRegion *);

      PRH = ((PixelRegionHolder*) g_slice_alloc0 (sizeof (PixelRegionHolder)));
      PRH->PR = PR;
...
pixel_regions_process (PixelRegionIterator *PRI)
{
  ...
  for (list = PRI->pixel_regions; list; list = ((list) ? (((GSList *)(list))->next) : ((void*)0)))
    {
      PixelRegionHolder *PRH = list->data;

      if ((PRH->PR != ((void*)0)) && (PRH->PR->process_count != PRI->process_count))
        {
      ...
      if (PRH->PR)
        {
          ...
          if (PRH->PR->tiles)
            {
              height = 64 - (PRH->PR->y % 64);
              height = (((height) > ((PRI->region_height - (PRH->PR->y - PRH->starty)))) ? ((PRI->region_height - (PRH->PR->y - PRH->starty))) : (((height) < (0)) ? (0) : (height)));
      ...
      if (PRH->PR)
        {
          ...
          if (PRH->PR->tiles)
            {
              width = 64 - (PRH->PR->x % 64);
              width = (((width) > ((PRI->region_width - (PRH->PR->x - PRH->startx)))) ? ((PRI->region_width - (PRH->PR->x - PRH->startx))) : (((width) < (0)) ? (0) : (width)));
  ...
  if (PRI->pixel_regions)
    {
      ...
      for (list = PRI->pixel_regions; list; list = ((list) ? (((GSList *)(list))->next) : ((void*)0)))
        do { if (1) g_slice_free1 (sizeof (PixelRegionHolder), (list->data)); else (void) ((PixelRegionHolder*) 0 == (list->data)); } while (0);

      g_slist_free (PRI->pixel_regions);

      do { if (1) g_slice_free1 (sizeof (PixelRegionIterator), (PRI)); else (void) ((PixelRegionIterator*) 0 == (PRI)); } while (0);
    }
}
  ...
  if (PRH->PR->tiles)
    {
      ...
      PRH->PR->offx = PRH->PR->x % 64;
      PRH->PR->offy = PRH->PR->y % 64;
