...
desaturate_region (GimpDesaturateMode *mode,
                   ...
                   PixelRegion *destPR)
{
  do{ if (mode != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "mode != NULL"); return; }; }while (0);
  do{ if (srcPR->bytes == destPR->bytes) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "srcPR->bytes == destPR->bytes"); return; }; }while (0);
  do{ if (srcPR->bytes == 3 || srcPR->bytes == 4) { } else { g_return_if_fail_warning ("Gimp-Base", ((const char*) (__func__)), "srcPR->bytes == 3 || srcPR->bytes == 4"); return; }; }while (0);
  ...
  while (h--)
    {
      ...
      for (j = 0; j < srcPR->w; j++)
        {
          gint luminosity = ((s[0]) * (0.2126) + (s[1]) * (0.7152) + (s[2]) * (0.0722)) + 0.5;
