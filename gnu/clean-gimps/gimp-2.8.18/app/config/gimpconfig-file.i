...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimpconfig-file.c" 2
# 21 "gimpconfig-file.c"
# 1 "../../config.h" 1
# 22 "gimpconfig-file.c" 2
  ...
  if (sfile == ((void*)0))
    {
      g_set_error (error, g_file_error_quark (), g_file_error_from_errno ((*__errno_location ())),
                   ...
                   gimp_filename_to_utf8 (source), g_strerror ((*__errno_location ())));
      return (0);
    }
  ...
  if (dfile == ((void*)0))
    {
      g_set_error (error, g_file_error_quark (), g_file_error_from_errno ((*__errno_location ())),
                   ...
                   gimp_filename_to_utf8 (dest), g_strerror ((*__errno_location ())));
      fclose (sfile);
      return (0);
    }
                                  ...
                                  GError **error)
{
  ...
  do{ if (filename != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Config", ((const char*) (__func__)), "filename != NULL"); return ((0)); }; }while (0);
  do{ if (name != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Config", ((const char*) (__func__)), "name != NULL"); return ((0)); }; }while (0);
  do{ if (error == ((void*)0) || *error == ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Config", ((const char*) (__func__)), "error == NULL || *error == NULL"); return ((0)); }; }while (0);

  backup = g_strconcat (filename, "~", ((void*)0));

  success = gimp_config_file_copy (filename, backup, error);

  if (success)
    g_message (gettext ("There was an error parsing your '%s' file. " "Default values will be used. A backup of your " "configuration has been created at '%s'."),
