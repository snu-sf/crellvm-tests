         ...
         ...)
{
  ...
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_ERROR, format, args);
           ...
           ...)
{
  va_list args;
  __builtin_va_start(args, format);
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_MESSAGE, format, args);
  __builtin_va_end(args);
}
static void
            ...
            ...)
{
  va_list args;
  __builtin_va_start(args, format);
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_CRITICAL, format, args);
  __builtin_va_end(args);
}
static void
           ...
           ...)
{
  va_list args;
  __builtin_va_start(args, format);
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_WARNING, format, args);
  __builtin_va_end(args);
}
static void
        ...
        ...)
{
  va_list args;
  __builtin_va_start(args, format);
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_INFO, format, args);
  __builtin_va_end(args);
}
static void
         ...
         ...)
{
  va_list args;
  __builtin_va_start(args, format);
  g_logv ("Gimp-Dialogs", G_LOG_LEVEL_DEBUG, format, args);
  __builtin_va_end(args);
}
...
gimp_tip_new (const gchar *title,
              ...
              ...)
{
  GimpTip *tip = ((GimpTip*) g_slice_alloc0 (sizeof (GimpTip)));
...
gimp_tip_free (GimpTip *tip)
{
  ...
  do { if (1) g_slice_free1 (sizeof (GimpTip), (tip)); else (void) ((GimpTip*) 0 == (tip)); } while (0);
}
# 168 "tips-parser.c"
GList *
                     ...
                     GError **error)
{
  ...
  do{ if (filename != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Dialogs", ((const char*) (__func__)), "filename != NULL"); return (((void*)0)); }; }while (0);
  do{ if (error == ((void*)0) || *error == ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Dialogs", ((const char*) (__func__)), "error == NULL || *error == NULL"); return (((void*)0)); }; }while (0);
  ...
  tips_locale = gettext ("tips-locale:C");
      ...
      if (strcmp (element_name, "tip") == 0)
        {
          ...
          parser->current_tip = ((GimpTip*) g_slice_alloc0 (sizeof (GimpTip)));
                         ...
                         GError **error)
{
  ...
  switch (parser->state)
    {
    case TIPS_START:
      g_warning ("%s: shouldn't get here", "tips-parser.c" ":" "306");
      break;
          ...
          for (i = 0; i < text_len; i++)
            {
              if (text[i] != ' ' &&
                  text[i] != '\t' && text[i] != '\n' && text[i] != '\r')
                {
                  g_string_append_c_inline (parser->value, text[i]);
                }
              else if (parser->value->len > 0 &&
                       parser->value->str[parser->value->len - 1] != ' ')
                {
                  g_string_append_c_inline (parser->value, ' ');
                }
            }
        }
      break;
...
tips_parser_end_markup (TipsParser *parser,
                        const gchar *markup_name)
{
  do { if (parser->markup_depth > 0) ; else g_assertion_message_expr ("Gimp-Dialogs", "tips-parser.c", 385, ((const char*) (__func__)), "parser->markup_depth > 0"); } while (0);
...
{
  do { if (parser->unknown_depth > 0 && parser->state == TIPS_IN_UNKNOWN) ; else g_assertion_message_expr ("Gimp-Dialogs", "tips-parser.c", 404, ((const char*) (__func__)), "parser->unknown_depth > 0 && parser->state == TIPS_IN_UNKNOWN"); } while (0);
