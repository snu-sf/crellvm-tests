...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimpdashpattern.c" 2
# 22 "gimpdashpattern.c"
# 1 "../../config.h" 1
# 23 "gimpdashpattern.c" 2
...
# 27 "gimpdashpattern.c" 2

# 1 "./gimpdashpattern.h" 1
# 30 "./gimpdashpattern.h"
GType gimp_dash_pattern_get_type (void) __attribute__((__const__));
...
gimp_dash_pattern_new_from_preset (GimpDashPreset preset)
{
  ...
  pattern = g_array_new ((0), (0), sizeof (gdouble));
  ...
  switch (preset)
    ...
    case GIMP_DASH_LONG_DASH:
      dash = 9.0; g_array_append_vals (pattern, &(dash), 1);
      dash = 3.0; g_array_append_vals (pattern, &(dash), 1);
      break;

    case GIMP_DASH_MEDIUM_DASH:
      dash = 6.0; g_array_append_vals (pattern, &(dash), 1);
      dash = 6.0; g_array_append_vals (pattern, &(dash), 1);
      break;
    ...
    case GIMP_DASH_SPARSE_DOTS:
      for (i = 0; i < 2; i++)
        {
          dash = 1.0; g_array_append_vals (pattern, &(dash), 1);
          dash = 5.0; g_array_append_vals (pattern, &(dash), 1);
        }
      break;
    ...
    case GIMP_DASH_STIPPLES:
      for (i = 0; i < 24; i++)
        {
          dash = 0.5; g_array_append_vals (pattern, &(dash), 1);
        }
      break;

    case GIMP_DASH_DASH_DOT:
      dash = 7.0; g_array_append_vals (pattern, &(dash), 1);
      dash = 2.0; g_array_append_vals (pattern, &(dash), 1);
      dash = 1.0; g_array_append_vals (pattern, &(dash), 1);
      dash = 2.0; g_array_append_vals (pattern, &(dash), 1);
      break;
                                     ...
                                     gdouble dash_length)
{
  ...
  do{ if (segments != ((void*)0) || n_segments == 0) { } else { g_return_if_fail_warning ("Gimp-Core", ((const char*) (__func__)), "segments != NULL || n_segments == 0"); return (((void*)0)); }; }while (0);

  pattern = g_array_new ((0), (0), sizeof (gdouble));

  for (i = 0, count = 0, state = (!(0)); i <= n_segments; i++)
    {
      if (i < n_segments && segments[i] == state)
      ...
      else
        {
          ...
          g_array_append_vals (pattern, &(l), 1);
                                 ...
                                 gint n_segments)
{
  ...
  do{ if (segments != ((void*)0) || n_segments == 0) { } else { g_return_if_fail_warning ("Gimp-Core", ((const char*) (__func__)), "segments != NULL || n_segments == 0"); return; }; }while (0);

  if (pattern == ((void*)0) || pattern->len <= 1)
    {
  ...
  for (i = 0, sum = 0; i < pattern->len ; i++)
    {
      sum += (((gdouble*) (void *) (pattern)->data) [(i)]);
    }

  factor = ((gdouble) n_segments) / sum;

  j = 0;
  sum = (((gdouble*) (void *) (pattern)->data) [(j)]) * factor;
  paint = (!(0));
...
{
  if (value_array == ((void*)0) || value_array->n_values == 0)
    {
      return ((void*)0);
    }
  else
      ...
      for (i = 0; i < value_array->n_values; i++)
        {
          ...
          do{ if ((((__extension__ ({ GValue *__val = (GValue*) ((item)); GType __t = (((GType) ((15) << (2)))); gboolean __r; if (!__val) __r = (0); else if (__val->g_type == __t) __r = (!(0)); else __r = g_type_check_value_holds (__val, __t); __r; }))))) { } else { g_return_if_fail_warning ("Gimp-Core", ((const char*) (__func__)), "G_VALUE_HOLDS_DOUBLE (item)"); return (((void*)0)); }; }while (0);

          val = g_value_get_double (item);

          g_array_append_vals (pattern, &(val), 1);
        }
...
{
  if (pattern == ((void*)0) || pattern->len == 0)
    {
      return ((void*)0);
    }
  else
    {
      ...
      g_value_init (&item, ((GType) ((15) << (2))));

      for (i = 0; i < pattern->len; i++)
        {
          g_value_set_double (&item, (((gdouble*) (void *) (pattern)->data) [(i)]));
  ...
  if (pattern)
    {
      ...
      copy = g_array_sized_new ((0), (0), sizeof (gdouble), pattern->len);

      for (i = 0; i < pattern->len; i++)
        g_array_append_vals (copy, &((((gdouble*) (void *) (pattern)->data) [(i)])), 1);
