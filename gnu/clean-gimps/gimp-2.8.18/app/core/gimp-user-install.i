# 1 "gimp-user-install.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 317 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimp-user-install.c" 2
# 25 "gimp-user-install.c"
# 1 "../../config.h" 1
# 26 "gimp-user-install.c" 2
...
gboolean gimp_config_file_backup_on_error (const gchar *filename,
                                              const gchar *name,
                                              GError **error);
# 54 "gimp-user-install.c" 2
# 1 "../../app/config/gimprc.h" 1
# 24 "../../app/config/gimprc.h"
...
# 25 "../../app/config/gimprc.h" 2
# 34 "../../app/config/gimprc.h"
...
# 57 "gimp-user-install.c" 2
# 1 "./gimp-tags.h" 1
# 22 "./gimp-tags.h"
gboolean gimp_tags_user_install (void);
# 58 "gimp-user-install.c" 2
# 1 "./gimp-user-install.h" 1
# 22 "./gimp-user-install.h"
...
{
  GimpUserInstall *install = ((GimpUserInstall*) g_slice_alloc0 (sizeof (GimpUserInstall)));

  install->verbose = verbose;

  gimp_user_install_detect_old (install, gimp_directory ());
# 189 "gimp-user-install.c"
  return install;
}
...
gimp_user_install_run (GimpUserInstall *install)
{
  ...
  do{ if (install != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Core", ((const char*) (__func__)), "install != NULL"); return ((0)); }; }while (0);

  dirname = g_filename_display_name (gimp_directory ());

  if (install->migrate)
    user_install_log (install,
        gettext ("It seems you have used GIMP %s before.  " "GIMP will now migrate your user settings to '%s'."),

        install->migrate, dirname);
  else
    user_install_log (install,
        gettext ("It appears that you are using GIMP for the " "first time.  GIMP will now create a folder " "named '%s' and copy some files to it."),
...
{
  do{ if (install != ((void*)0)) { } else { g_return_if_fail_warning ("Gimp-Core", ((const char*) (__func__)), "install != NULL"); return; }; }while (0);

  g_free (install->old_dir);

  do { if (1) g_slice_free1 (sizeof (GimpUserInstall), (install)); else (void) ((GimpUserInstall*) 0 == (install)); } while (0);
}
  ...
  if (format)
    {
      ...
      if (install->log)
        install->log (message, (0), install->log_data);
...
user_install_log_newline (GimpUserInstall *install)
{
  ...
  if (install->log)
    install->log (((void*)0), (0), install->log_data);
}
                        ...
                        const gchar *dest)
{
  ...
  user_install_log (install, gettext ("Copying file '%s' from '%s'..."),
...
user_install_mkdir (GimpUserInstall *install,
                    const gchar *dirname)
{
  user_install_log (install, gettext ("Creating folder '%s'..."),
  ...
  if (mkdir (dirname,
               ...
               ((0400 >> 3) >> 3) | ((0100 >> 3) >> 3)) == -1)
    {
      ...
      g_set_error (&error, g_file_error_quark (), g_file_error_from_errno ((*__errno_location ())),
                   gettext ("Cannot create folder '%s': %s"),
                   gimp_filename_to_utf8 (dirname), g_strerror ((*__errno_location ())));
...
user_install_dir_copy (GimpUserInstall *install,
                       ...
                       const gchar *base)
{
  GDir *source_dir = ((void*)0);
  GDir *dest_dir = ((void*)0);
  gchar dest[1024];
  const gchar *basename;
  gchar *dirname;
  gboolean success;
...

  {
    ...
    dirname = g_build_filename (base, basename, ((void*)0));
    g_free (basename);
  }
  ...
  success = (dest_dir = g_dir_open (dirname, 0, &error)) != ((void*)0);
  if (! success)
    goto error;

  success = (source_dir = g_dir_open (source, 0, &error)) != ((void*)0);
  if (! success)
    goto error;

  while ((basename = g_dir_read_name (source_dir)) != ((void*)0))
    {
      gchar *name = g_build_filename (source, basename, ((void*)0));
...
user_install_create_files (GimpUserInstall *install)
{
  ...
  for (i = 0; i < (sizeof (gimp_user_install_items) / sizeof ((gimp_user_install_items)[0])); i++)
    {
      g_snprintf (dest, sizeof (dest), "%s%c%s",
                  gimp_directory (),
                  '/',
...
user_install_migrate_files (GimpUserInstall *install)
{
  ...
  while ((basename = g_dir_read_name (dir)) != ((void*)0))
    {
      gchar *source = g_build_filename (install->old_dir, basename, ((void*)0));
  ...
  gimprc = gimp_rc_new (((void*)0), ((void*)0), (0));
