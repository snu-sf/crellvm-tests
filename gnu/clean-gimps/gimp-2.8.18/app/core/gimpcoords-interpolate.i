# 1 "gimpcoords-interpolate.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 317 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimpcoords-interpolate.c" 2
# 20 "gimpcoords-interpolate.c"
# 1 "../../config.h" 1
# 21 "gimpcoords-interpolate.c" 2
...
gdouble gimp_coords_direction (const GimpCoords *a,
                                     const GimpCoords *b);
# 29 "gimpcoords-interpolate.c" 2
# 1 "./gimpcoords-interpolate.h" 1
# 23 "./gimpcoords-interpolate.h"
  ...
  for (n = 1; n <= num_points; n++)
    {
      ...
      coords.pressure = (((pressure) > (1.0)) ? (1.0) : (((pressure) < (0.0)) ? (0.0) : (pressure)));
      ...
      coords.velocity = (((velocity) > (1.0)) ? (1.0) : (((velocity) < (0.0)) ? (0.0) : (velocity)));

      coords.direction = start_coords.direction + dir_step * n;

      coords.direction = coords.direction - floor (coords.direction);

      g_array_append_vals (*ret_coords, &(coords), 1);

      if (ret_params)
        g_array_append_vals (*ret_params, &(p), 1);
    }
}
