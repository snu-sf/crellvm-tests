# 1 "gimpbrushgenerated-load.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 317 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gimpbrushgenerated-load.c" 2
# 20 "gimpbrushgenerated-load.c"
# 1 "../../config.h" 1
# 21 "gimpbrushgenerated-load.c" 2
...
# 23 "gimpbrushgenerated-load.c" 2
...
# 24 "gimpbrushgenerated-load.c" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 25 "gimpbrushgenerated-load.c" 2
...
# 28 "gimpbrushgenerated-load.c" 2
...
# 31 "gimpbrushgenerated-load.c" 2
...
gboolean g_close (gint fd,
                  GError **error);
# 32 "gimpbrushgenerated-load.c" 2
...
# 34 "gimpbrushgenerated-load.c" 2
...
# 36 "gimpbrushgenerated-load.c" 2
...
void temp_buf_dump (TempBuf *buf,
                                  const gchar *filename);
# 38 "gimpbrushgenerated-load.c" 2

# 1 "./gimpbrushgenerated.h" 1
# 24 "./gimpbrushgenerated.h"
# 1 "./gimpbrush.h" 1
# 22 "./gimpbrush.h"
# 1 "./gimpdata.h" 1
# 26 "./gimpdata.h"
...
void gimp_brush_set_spacing (GimpBrush *brush,
                                                      gint spacing);
# 25 "./gimpbrushgenerated.h" 2
# 35 "./gimpbrushgenerated.h"
...
GType gimp_brush_generated_get_type (void) __attribute__((__const__));
...
# 40 "gimpbrushgenerated-load.c" 2
# 1 "./gimpbrushgenerated-load.h" 1
# 27 "./gimpbrushgenerated-load.h"
GList * gimp_brush_generated_load (GimpContext *context,
                                   const gchar *filename,
                                   GError **error);
# 41 "gimpbrushgenerated-load.c" 2
...
# 43 "gimpbrushgenerated-load.c" 2
                           ...
                           GError **error)
{
  ...
  if (! g_str_has_prefix (string, "GIMP-VBR"))
    {
      g_set_error (error, (gimp_data_error_quark ()), GIMP_DATA_ERROR_READ,
                   gettext ("Fatal parse error in brush file '%s': " "Not a GIMP brush file."),
  ...
  if (! g_str_has_prefix (string, "1.0"))
    {
      if (! g_str_has_prefix (string, "1.5"))
        {
          g_set_error (error, (gimp_data_error_quark ()), GIMP_DATA_ERROR_READ,
                       gettext ("Fatal parse error in brush file '%s': " "Unknown GIMP brush version in line %d."),
  ...
  if (strlen (string) < 1)
    g_strlcpy (string, gettext ("Untitled"), sizeof (string));
  ...
  if (have_shape)
    {
      ...
      enum_class = g_type_class_peek ((gimp_brush_generated_shape_get_type ()));
      ...
      if (! shape_val)
        {
          g_set_error (error, (gimp_data_error_quark ()), GIMP_DATA_ERROR_READ,
                       gettext ("Fatal parse error in brush file '%s': " "Unknown GIMP brush shape in line %d."),
  ...
  if (! fgets (string, sizeof (string), file))
    goto failed;
  spacing = g_ascii_strtod (string, ((void*)0));
  ...
  if (! fgets (string, sizeof (string), file))
    goto failed;
  radius = g_ascii_strtod (string, ((void*)0));
  ...
  if (have_shape)
    {
      ...
      if (! fgets (string, sizeof (string), file))
        goto failed;
      spikes = (((atoi (string)) > (20)) ? (20) : (((atoi (string)) < (2)) ? (2) : (atoi (string))));
    }
  ...
  if (! fgets (string, sizeof (string), file))
    goto failed;
  hardness = g_ascii_strtod (string, ((void*)0));
  ...
  if (! fgets (string, sizeof (string), file))
    goto failed;
  aspect_ratio = g_ascii_strtod (string, ((void*)0));
  ...
  if (! fgets (string, sizeof (string), file))
    goto failed;
  angle = g_ascii_strtod (string, ((void*)0));

  fclose (file);

  brush = ((((GimpBrush*) g_type_check_instance_cast ((GTypeInstance*) ((gimp_brush_generated_new (name, shape, radius, spikes, hardness, aspect_ratio, angle))), ((gimp_brush_get_type ()))))));

  g_free (name);

  brush->spacing = spacing;

  return g_list_prepend (((void*)0), brush);
  ...
  if (error && *error == ((void*)0))
    {
      ...
      if ((*__errno_location ()))
        msg = g_strdup_printf (gettext ("Line %d: %s"), linenum, g_strerror ((*__errno_location ())));
      else
        msg = g_strdup_printf (gettext ("File is truncated in line %d"), linenum);

      g_set_error (error, (gimp_data_error_quark ()), GIMP_DATA_ERROR_READ,
                   gettext ("Error while reading brush file '%s': %s"),
