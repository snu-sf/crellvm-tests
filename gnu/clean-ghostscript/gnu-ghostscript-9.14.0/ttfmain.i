...
static const bool skip_instructions = 0;
...
{ pt->x = ttMulDiv(dx, m->a, 65536) + ttMulDiv(dy, m->c, 65536) + (m->tx >> 10);
    pt->y = ttMulDiv(dx, m->b, 65536) + ttMulDiv(dy, m->d, 65536) + (m->ty >> 10);
}
...
{ char sVersion[4], sVersion1[4] = {0, 1, 0, 0};
    ...
    r->Seek(r, self->t_head.nPos + ((int) ( (char *)&((sfnt_FontHeader *)0)->flags - (char *)((sfnt_FontHeader *)0) )));
    self->nFlags = ttfReader__UShort(r);
    r->Seek(r, self->t_head.nPos + ((int) ( (char *)&((sfnt_FontHeader *)0)->unitsPerEm - (char *)((sfnt_FontHeader *)0) )));
    self->nUnitsPerEm = ttfReader__UShort(r);
    r->Seek(r, self->t_head.nPos + ((int) ( (char *)&((sfnt_FontHeader *)0)->indexToLocFormat - (char *)((sfnt_FontHeader *)0) )));
    self->nIndexToLocFormat = ttfReader__UShort(r);
    r->Seek(r, self->t_maxp.nPos + ((int) ( (char *)&((sfnt_maxProfileTable *)0)->numGlyphs - (char *)((sfnt_maxProfileTable *)0) )));
    self->nNumGlyphs = ttfReader__UShort(r);
    r->Seek(r, self->t_maxp.nPos + ((int) ( (char *)&((sfnt_maxProfileTable *)0)->maxComponentElements - (char *)((sfnt_maxProfileTable *)0) )));
    self->nMaxComponents = ttfReader__UShort(r);
    if(self->nMaxComponents < 10)
        self->nMaxComponents = 10;
    r->Seek(r, self->t_hhea.nPos + ((int) ( (char *)&((sfnt_MetricsHeader *)0)->numberLongMetrics - (char *)((sfnt_MetricsHeader *)0) )));
    self->nLongMetricsHorz = ttfReader__UShort(r);
    if (self->t_vhea.nPos != 0) {
        r->Seek(r, self->t_vhea.nPos + ((int) ( (char *)&((sfnt_MetricsHeader *)0)->numberLongMetrics - (char *)((sfnt_MetricsHeader *)0) )));
        self->nLongMetricsVert = ttfReader__UShort(r);
    } else
        self->nLongMetricsVert = 0;
    if (tti->usage_size < self->nMaxComponents * 3) {
        tti->ttf_memory->free(tti->ttf_memory, tti->usage, "ttfFont__Open");
        tti->usage_size = 0;
        tti->usage = mem->alloc_bytes(mem,
                sizeof(ttfSubGlyphUsage) * self->nMaxComponents * 3,
                "ttfFont__Open");
        if (tti->usage == ((void*)0))
            return fMemoryError;
        tti->usage_size = self->nMaxComponents * 3;
    }
    ...
    code = Context_Create(self->exec, self->face);
    if (code == 0x100)
        return fMemoryError;
...
{ ttfFont *pFont = self->pFont;
    ...
    const byte *glyph = ((void*)0);
    int glyph_size;
    bool execute_bytecode = ((bool)1);
    int nPoints = 0;
...
retry:
    ...
    Init_Glyph_Component(&subglyph, ((void*)0), pFont->exec);
    ...
    if (gOutline->contourCount == 0)
        ...
        if (execute_bytecode && !skip_instructions && n_ins &&
            ...
            if (nPos + n_ins > glyph_size)
                goto errex;
            code = Set_CodeRange(exec, 3, (byte *)glyph + nPos, n_ins);
            if (!code) {
                int k;
                F26Dot6 x;
                ...
                for (k = 0; k < nPoints; k++)
                    pts->touch[k] = pts->touch[k] & 1;
                org_to_cur(nPoints, pts);
                exec->is_composite = 1;
                if (pFont->patented)
                    code = 0x020;
                else
                    code = Context_Run(exec, 0);
                if (!code)
        ...
        if (r->Error(r))
            goto errex;
        bInsOK = !Set_CodeRange(exec, 3, (byte *)glyph + nPos, n_ins);
        onCurve = pts->touch;
