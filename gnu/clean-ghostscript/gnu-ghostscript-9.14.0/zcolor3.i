...
zsetuseciecolor(i_ctx_t * i_ctx_p)
{
    ...
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->use_cie_color = *op;
...
zcurrentrenderingintent(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.intval = (gs_currentrenderingintent((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zsetrenderingintent(i_ctx_t * i_ctx_p)
{
    ...
    if (code < 0 || (code = gs_setrenderingintent((i_ctx_p->pgs), param)) < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
...
zcurrentblackptcomp(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.intval = (gs_currentblackptcomp((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zsetblackptcomp(i_ctx_t * i_ctx_p)
{
    ...
    if (code < 0 || (code = gs_setblackptcomp((i_ctx_p->pgs), param)) < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
