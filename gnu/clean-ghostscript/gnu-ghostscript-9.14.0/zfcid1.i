...
zbuildfont10(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    (((&build.BuildChar)->tas.type_attrs = ((t_null) << 8) + (0)) );
...
{
    const ref *pcidmap = &((font_data *)((pfont)->client_data))->u.type42.CIDMap;
    ...
    switch ((((const byte *)&((pcidmap)->tas.type_attrs))[1])) {
    case t_string:
        if (cid >= ((pcidmap)->tas.rsize) / gdbytes)
            return ((-15));
                     ...
                     int members, gs_glyph_info_t *info)
{
    ...
    uint glyph_index;
    int code = (glyph > (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1))
                         ...
                         gs_glyph_space_t glyph_space, gs_glyph *pglyph)
{
    ...
    int code0 = z11_CIDMap_proc(pfont, ((gs_glyph)0x80000000L));
    int code;
    ...
    for (;;) {
        code = z11_CIDMap_proc(pfont, ((gs_glyph)0x80000000L) + *pindex);
    ...
    if (glyph_space == GLYPH_SPACE_INDEX)
        *pglyph = (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)) + (uint)code;
    else
        *pglyph = ((gs_glyph)0x80000000L) + (uint)(*pindex - 1);
    return 0;
}
                  ...
                  gx_path *ppath, double sbw[4])
{
    return gs_type42_glyph_outline(font, WMode,
            z11_get_glyph_index((gs_font_type42 *)font, glyph) + (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)),
                                   pmat, ppath, sbw);
}
...
get_subst_CID_on_WMode(gs_subst_CID_on_WMode_t *subst, ref *t, int WMode)
{
    ...
    ((&r)->value.intval = (WMode), ((&r)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    if (dict_find(t, &r, &a) > 0 && (((const byte *)&((a)->tas.type_attrs))[1]) == t_array) {
        int n = ((a)->tas.rsize), i;
        uint *s;

        s = (uint *)(*(subst->rc.memory)->procs.alloc_byte_array)(subst->rc.memory, n, sizeof(int), "zbuildfont11");
        if (s == ((void*)0))
            return ((-25));
        for (i = 0; i < n; i++) {
            array_get(subst->rc.memory, a, (long)i, &e);
            if ((((const byte *)&((&e)->tas.type_attrs))[1]) != t_integer)
                return ((-10));
            s[i] = e.value.intval;
        }
        subst->data[WMode] = s;
...
static int release_subst_CID_on_WMode(void *proc_data, void *event_data);
...
release_subst_CID_on_WMode(void *data, void *event)
{
    ...
    pfcid->subst_CID_on_WMode = ((void*)0);
    do { if (subst) { do { do { } while (0); (subst)->rc.ref_count += (-2); } while (0); if (!(subst)->rc.ref_count) { do { do { } while (0); (subst)->rc.free((subst)->rc.memory, (void *)(subst), "release_subst_CID_on_WMode"); } while (0); (subst) = 0; } else do { } while (0); } } while (0);
    return 0;
}
                                          ...
                                          int WMode, gs_glyph glyph)
{
    ...
    uint cid = (glyph >= ((gs_glyph)0x80000000L) ? glyph - ((gs_glyph)0x80000000L) : glyph);
    int WMode1 = !WMode; gs_subst_CID_on_WMode_t *s = pfcid->subst_CID_on_WMode;
...
zbuildfont11(i_ctx_t *i_ctx_p)
{
    ...
    if (code > 0) {
        ...
        file = *pfile;
        if ( !(((&file)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&file)->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&file) : (-7)));
        code = dict_find_string(op, "file_table_pos", &file_table_pos);
        if (code <= 0 || (((const byte *)&((file_table_pos)->tas.type_attrs))[1]) != t_dictionary)
            return ((-10));
        for (i = 0; i < 2; i++) {
            code = dict_find_string(file_table_pos, name[i], &a);
            if (code <= 0 || (((const byte *)&((a)->tas.type_attrs))[1]) != t_array)
                return ((-10));
            for (j = 0; j < 2; j++) {
                code = array_get(((gs_memory_t *)((i_ctx_p->memory).current)), a, j, &v);
                if (code < 0 || (((const byte *)&((&v)->tas.type_attrs))[1]) != t_integer)
                    return ((-10));
        ...
        pfile = ((void*)0);
    code = font_string_array_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, "CIDMap", &rcidmap);
    switch (code) {
    case 0:
    gdb:
    ...
    default:
        return code;
    case (-20):
        switch ((((const byte *)&((&rcidmap)->tas.type_attrs))[1])) {
        case t_string:
            goto gdb;
        case t_dictionary:
        case t_integer:
    ...
    code = build_gs_TrueType_font(i_ctx_p, op, &pfont, ft_CID_TrueType,
                                  ...
                                  (pfile != ((void*)0) ? bf_has_font_file : 0));
    if (code < 0)
        return code;
    pfcid = (gs_font_cid2 *)pfont;
    if (dict_find_string(op, "subst_CID_on_WMode", &t) > 0 && (((const byte *)&((t)->tas.type_attrs))[1]) == t_dictionary) {
        gs_subst_CID_on_WMode_t *subst = ((void*)0);
        ref *o;
        gs_font *font;

        if (dict_find_string(t, "Ordering", &o) <= 0 || (((const byte *)&((o)->tas.type_attrs))[1]) != t_string)
            return ((-10));
        for (font = (gs_lib_ctx_get_interp_instance(((gs_memory_t *)((i_ctx_p->memory).current)))->font_dir)->orig_fonts; font != ((void*)0); font = font->next) {
            if (font->FontType == ft_CID_TrueType) {
                gs_font_cid2 *pfcid1 = (gs_font_cid2 *)font;
                if (pfcid1->subst_CID_on_WMode != ((void*)0) &&
                    bytes_compare(o->value.const_bytes, ((o)->tas.rsize),
        ...
        if (subst == ((void*)0)) {
            do { if ( ((subst) = (gs_subst_CID_on_WMode_t *)(*(pfcid->memory)->procs.alloc_struct)(pfcid->memory, &st_subst_CID_on_WMode, "zbuildfont11")) == 0 ) { return ((-25)); } else { do { (subst)->rc.ref_count = 1; (subst)->rc.memory = pfcid->memory; (subst)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
    ...
    (*(&((font_data *)((pfont)->client_data))->u.type42.CIDMap) = *(&rcidmap));
    ...
    if (MetricsCount) {
        ...
        do { if ( !(((&*(&file))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(&file))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(&file)) : (-7))); do { s = (&file)->value.pfile; if (s->read_id != ((&file)->tas.rsize)) { if (s->read_id == 0 && s->write_id == ((&file)->tas.rsize)) { int fcode = file_switch_to_read(&file); if (fcode < 0) return(fcode); } else { s = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
...
# 521 "./psi/zfcid1.c"
    ...
    if (code < 0)
        return code;
    ((op - 1)->value.intval = (code), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
...
zfillCIDMap(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*Decoding)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*Decoding));
    if ( !((((const byte *)&((&*TT_cmap)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*TT_cmap));
    if ( !((((const byte *)&((&*SubstNWP)->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&*SubstNWP));
    if ( !((((const byte *)&((&*GDBytes)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*GDBytes));
    if ( !((((const byte *)&((&*CIDMap)->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&*CIDMap));
    code = cid_fill_CIDMap(((gs_memory_t *)((i_ctx_p->memory).current)), Decoding, TT_cmap, SubstNWP, GDBytes->value.intval, CIDMap);
...
zfillIdentityCIDMap(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*CIDMap)->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&*CIDMap));
    code = cid_fill_Identity_CIDMap(((gs_memory_t *)((i_ctx_p->memory).current)), CIDMap);
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return code;
}
