...
static int iodev_ram_init(gx_io_device *iodev, gs_memory_t *mem);
static int ram_open_file(gx_io_device *iodev, const char *fname, uint namelen, const char *access, stream **ps, gs_memory_t *mem);
static int ram_delete(gx_io_device *iodev, const char *fname);
static int ram_rename(gx_io_device *iodev, const char *from, const char *to);
static int ram_status(gx_io_device *iodev, const char *fname, struct stat *pstat);
static file_enum *ram_enumerate_init(gx_io_device *iodev, const char *pat, uint patlen, gs_memory_t *mem);
static uint ram_enumerate_next(file_enum *pfen, char *ptr, uint maxlen);
static void ram_enumerate_close(file_enum *pfen);
static int ram_get_params(gx_io_device *iodev, gs_param_list *plist);
...
static const gc_ptr_element_t ramfs_state_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((struct ramfs_state_s *)0)->memory - (char *)((struct ramfs_state_s *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((struct ramfs_state_s *)0)->fs - (char *)((struct ramfs_state_s *)0) )) } }; static const gc_struct_data_t ramfs_state_reloc_ptrs = { (sizeof(ramfs_state_enum_ptrs) / sizeof((ramfs_state_enum_ptrs)[0])), 0, 0, ramfs_state_enum_ptrs }; static const gs_memory_struct_type_t st_ramfs_state = { sizeof(struct ramfs_state_s), "ramfs_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, ram_finalize, &ramfs_state_reloc_ptrs };
...
static const gc_ptr_element_t gsram_enum_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((struct gsram_enum_s *)0)->pattern - (char *)((struct gsram_enum_s *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((struct gsram_enum_s *)0)->e - (char *)((struct gsram_enum_s *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((struct gsram_enum_s *)0)->memory - (char *)((struct gsram_enum_s *)0) )) } }; static const gc_struct_data_t gsram_enum_reloc_ptrs = { (sizeof(gsram_enum_enum_ptrs) / sizeof((gsram_enum_enum_ptrs)[0])), 0, 0, gsram_enum_enum_ptrs }; static const gs_memory_struct_type_t st_gsram_enum = { sizeof(struct gsram_enum_s), "gsram_enum", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &gsram_enum_reloc_ptrs };
# 84 "./base/gsioram.c"
static int
    ...
    const char *file_access, stream ** ps, gs_memory_t * mem)
{
    ...
    char fmode[4];
    int openmode=1;
    ramfs * fs = (((ramfs_state*)(iodev->state))->fs);
    char * namestr = ((void*)0);


    namestr = (char *)(*(mem)->procs.alloc_bytes)(mem, len + 1, "temporary filename string");
    if(!namestr)
        return ((-25));
    __builtin_strncpy (namestr, fname, len);
    namestr[len] = 0;

    if (!iodev) {
        ((mem)->procs.free_object(mem, namestr, "free temporary filename string"));
        return ((-7));
    }
    code = file_prepare_stream(fname, len, file_access, 2048,
    ps, fmode, mem
    );
    ...
    if (fmode[0] != 'r' || fmode[1] == '+') openmode |= 4;
    if (fmode[0] != 'r') openmode |= 2;
...
s_ram_available(register stream * s, gs_offset_t *pl)
{
    ...
    long buf_avail = ((s)->cursor.r.limit - (s)->cursor.r.ptr);

    *pl = (((max_avail) < (buf_avail)) ? (max_avail) : (buf_avail));
...
s_ram_read_seek(register stream * s, gs_offset_t pos)
{
    ...
    ramfile_seek((ramhandle*)s->file, s->file_offset + pos, 0) != 0
    )
    return ((int)(-2));
    ...
    stream_cursor_write * pw, bool last)
{
    ...
    if (max_count > limit_count)
        max_count = limit_count, status = ((int)(-1));
    }
    count = ramfile_read(file,pw->ptr + 1, max_count);
    if (count < 0) return ((int)(-2));
    pw->ptr += count;

    return ramfile_eof(file) ? ((int)(-1)) : status;
}
...
s_ram_write_seek(stream * s, gs_offset_t pos)
{
    ...
    int code = (*(s)->procs.flush)(s);

    if (code < 0) return code;
    if (ramfile_seek((ramhandle*)s->file, pos, 0) != 0)
    return ((int)(-2));
    s->position = pos;
    return 0;
}
...
s_ram_switch(stream * s, bool writing)
{
    ...
    if (writing) {
    if (!(s->file_modes & 2)) return ((int)(-2));
    pos = stell(s);
    ramfile_seek(file, pos, 0);
    if (modes & 8) {
        sappend_ram(s, file, s->cbuf, s->cbsize);
    } else {
    ...
    if (!(s->file_modes & 1)) return ((int)(-2));
    pos = stell(s);
    if ((*(s)->procs.flush)(s) < 0) return ((int)(-2));
...
{
    ramfs* fs = ramfs_new(mem, 2000000);
    ramfs_state* state = (ramfs_state *)(*(mem)->procs.alloc_struct)(mem, &st_ramfs_state, "ramfs_init(state)");
    ...
    if(state) ((mem)->procs.free_object(mem, state, "iodev_ram_init(state)"));
    return (-25);
}
...
{
    ramfs* fs = (((ramfs_state*)((ramfs_state*)vptr))->fs);
...
ram_delete(gx_io_device * iodev, const char *fname)
{
    ...
    return (ramfs_errno_to_code(ramfs_error(fs)));
    }
    return 0;
}
...
ram_status(gx_io_device * iodev, const char *fname, struct stat *pstat)
{
    ...
    f = ramfs_open(((ramfs_state *)iodev->state)->memory, fs,fname,1);
    if(!f) return (ramfs_errno_to_code(ramfs_error(fs)));
...
ram_enumerate_init(gx_io_device *iodev, const char *pat, uint patlen,
    gs_memory_t *mem)
{
    gsram_enum * penum = (gsram_enum *)(*(mem)->procs.alloc_struct)(mem, &st_gsram_enum, "ram_enumerate_files_init(file_enum)");



    char *pattern = (char *)(*(mem)->procs.alloc_bytes)(mem, patlen+1, "ram_enumerate_file_init(pattern)");


    ramfs_enum * e = ramfs_enum_new((((ramfs_state*)(iodev->state))->fs));
    if(penum && pattern && e) {
    ...
    if (penum) ((mem)->procs.free_object(mem, penum, "ramfs_enum_init(ramfs_enum)"));
    if (pattern)
    ((mem)->procs.free_object(mem, pattern, "ramfs_enum_init(pattern)"));
...
ram_enumerate_close(file_enum *pfen)
{
    ...
    ((mem)->procs.free_object(mem, penum->pattern, "ramfs_enum_init(pattern)"));
    ((mem)->procs.free_object(mem, penum, "ramfs_enum_init(ramfs_enum)"));
}
