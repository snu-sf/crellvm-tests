            ...
            ref * pref, const byte ** psp, int scanner_options)
{
    const byte *sp = str;
# 50 "./psi/iscannum.c"
    static const float powers_10[6 + 1] = {
        1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6
    };
    static const double neg_powers_10[6 + 1] = {
    ...
    max_ps_int_scan = scanner_options & 128 ? (~(-1 << ((1 << 2) * 8 - 1))) : (~(-((int64_t)1) << (sizeof(int64_t) * 8 - 1)));
    min_ps_int_scan = scanner_options & 128 ? (-1 << ((1 << 2) * 8 - 1)) : (-((int64_t)1) << (sizeof(int64_t) * 8 - 1));

    max_scan = scanner_options & 64 && sign >= 0 ? ~((ps_int)0) : max_ps_int_scan;

    for (;; ival = ival * 10 + d) {
        if (sp >= end) { goto iret; } else c = *sp++;
        if (!((d = decoder[c]) < 10))
            break;
        if ((((ps_uint)ival) >= max_scan / 10 && (((ps_uint)ival) > max_scan / 10 || d > (int64_t)(max_scan % 10)))) {
            if (ival == max_ps_int_scan / 10 && d == (max_ps_int_scan % 10) + 1 && sign < 0) {
                if (sp >= end) { c = ((int)(-1)); } else c = *sp++;
        ...
        case '#':
            {
                ...
                if (!(radix & (radix - 1))) {
                    ...
                    switch (radix) {
                        case 2:
                            shift = 1, imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) >> 1;
                            break;
                        case 4:
                            shift = 2, imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) >> 2;
                            break;
                        case 8:
                            shift = 3, imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) >> 3;
                            break;
                        case 16:
                            shift = 4, imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) >> 4;
                            break;
                        case 32:
                            shift = 5, imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) >> 5;
                            break;
                        default:
                            return ((-15));
                    }
                    for (;; uval = (uval << shift) + d) {
                        if (sp >= end) { break; } else c = *sp++;
                        d = decoder[c];
                        if (d >= radix) {
                            *psp = sp;
                            code = 1;
                    ...
                    ps_int irem = ((uint64_t)~((uint64_t)0) + (uint64_t)0) % radix;

                    imax = ((uint64_t)~((uint64_t)0) + (uint64_t)0) / radix;
                ...
                if (scanner_options & 128) {
                    ps_uint32 int1 = 0;
                    int1 |= (uval & 0xffffffff);
                    ((pref)->value.intval = ((ps_int)((ps_int32)int1)), ((pref)->tas.type_attrs = ((t_integer) << 8) + (0)) );
                }
                else
                    ((pref)->value.intval = (uval), ((pref)->tas.type_attrs = ((t_integer) << 8) + (0)) );
...
iret:
    if (scanner_options & 128) {
        ((pref)->value.intval = ((sign < 0 ? (ps_int32)-ival : (ps_int32)ival)), ((pref)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    }
    else {
        ((pref)->value.intval = ((sign < 0 ? (ps_int)-ival : (ps_int)ival)), ((pref)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    }
    return code;
...
i2r:
    ...
    while (((d = decoder[c]) < 10) || c == '-') {
        ...
        if ((ival >= (~(-1 << ((1 << 2) * 8 - 1))) / 10 && (ival > (~(-1 << ((1 << 2) * 8 - 1))) / 10 || d > (int64_t)((~(-1 << ((1 << 2) * 8 - 1))) % 10)))) {
            dval = ival;
            goto fd;
        }
        ival = ival * 10 + d;
    ...
    if (!(c == 'e' || c == 'E' || exp10 < -6)) {
        if (c != ((int)(-1)))
            *psp = sp, code = 1;
        ((pref)->value.realval = (ival * neg_powers_10[-exp10]), ((pref)->tas.type_attrs = ((t_real) << 8) + (0)) );
        return code;
    }
    dval = ival;
    goto fe;
...
rret:
    ((pref)->value.realval = (dval), ((pref)->tas.type_attrs = ((t_real) << 8) + (0)) );
    return code;
}
