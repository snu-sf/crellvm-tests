...
zclosefile(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_file)) ) return (check_type_failed(&*op));
    if ((s = (op)->value.pfile, (s->read_id | s->write_id) == ((op)->tas.rsize))) {
        int status = sclose(s);

        if (status != 0 && status != ((int)(-1))) {
            if ((((s)->modes & 2) != 0))
                return handle_write_status(i_ctx_p, status, op, ((void*)0),
                                           zclosefile);
            else
                return handle_read_status(i_ctx_p, status, op, ((void*)0),
                                          zclosefile);
        }
    }
...
zread(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !(((&*(op))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(op))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(op)) : (-7))); do { s = (op)->value.pfile; if (s->read_id != ((op)->tas.rsize)) { if (s->read_id == 0 && s->write_id == ((op)->tas.rsize)) { int fcode = file_switch_to_read(op); if (fcode < 0) return(fcode); } else { s = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
# 85 "./psi/zfileio.c"
    ...
    if (ch >= 0) {
        ...
        return handle_read_status(i_ctx_p, ch, op, ((void*)0), zread);
    }
    return 0;
}
...
zwrite(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !(((&*(op - 1))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_file) << 8) + (0x10))) ) return ((!((((const byte *)&((&*(op - 1))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(op - 1)) : (-7))); do { s = (op - 1)->value.pfile; if ( s->write_id != ((op - 1)->tas.rsize) ) { int fcode = file_switch_to_write(op - 1); if ( fcode < 0 ) return(fcode); } } while (0); } while (0);
    ...
    return handle_write_status(i_ctx_p, status, op - 1, ((void*)0), zwrite);
}
...
zreadhexstring_at(i_ctx_t *i_ctx_p, os_ptr op, uint start, int odd)
{
    ...
    do { if ( !(((&*(op - 1))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(op - 1))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(op - 1)) : (-7))); do { s = (op - 1)->value.pfile; if (s->read_id != ((op - 1)->tas.rsize)) { if (s->read_id == 0 && s->write_id == ((op - 1)->tas.rsize)) { int fcode = file_switch_to_read(op - 1); if (fcode < 0) return(fcode); } else { s = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
...
zreadhexstring_continue(i_ctx_t *i_ctx_p)
{
    ...
    if (length > ((op - 1)->tas.rsize) || odd < -1 || odd > 0xF)
        return ((-15));
    if ( !(((&op[-1])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))) ) return ((!((((const byte *)&((&op[-1])->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&op[-1]) : (-7)));
...
zwritestring(i_ctx_t *i_ctx_p)
{
    ...
    return handle_write_status(i_ctx_p, status, op - 1, ((void*)0), zwritestring);
}
...
zreadline_at(i_ctx_t *i_ctx_p, os_ptr op, uint count, bool in_eol)
{
    ...
    str.size = ((op)->tas.rsize);
    status = zreadline_from(s, &str, ((void*)0), &count, &in_eol);
    switch (status) {
        case 0:
        case ((int)(-1)):
            break;
        ...
        default:
            if (count == 0 && !in_eol)
                return handle_read_status(i_ctx_p, status, op - 1, ((void*)0),
                                          zreadline);
            else {
                if (in_eol) {
                    ((op)->tas.rsize = (count));
...
zreadline(i_ctx_t *i_ctx_p)
{
    ...
    return zreadline_at(i_ctx_p, op, 0, ((bool)0));
}
...
zreadline_continue(i_ctx_t *i_ctx_p)
{
    ...
    code = (start == 0 ? zreadline_at(i_ctx_p, op - 1, size, ((bool)1)) :
            zreadline_at(i_ctx_p, op - 1, start, ((bool)0)));
    if (code >= 0)
        ((((i_ctx_p->op_stack).stack).p) -= (1));
    return code;
}
...
zreadline_from(stream *s, gs_string *buf, gs_memory_t *bufmem,
               uint *pcount, bool *pin_eol)
{
    int (*readline)(stream *s_in, stream *s_out, void *readline_data, gs_const_string *prompt, gs_string *buf, gs_memory_t *bufmem, uint *pcount, bool *pin_eol, bool (*is_stdin)(const stream *));

    if (zis_stdin(s))
        readline = gp_readline;
    else
        readline = sreadline;
    return readline(s, ((void*)0), ((void*)0) , ((void*)0), buf, bufmem,
...
zbytesavailable(i_ctx_t *i_ctx_p)
{
    ...
    if (gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current)))) {
...
zflush(i_ctx_t *i_ctx_p)
{
    ...
    return
        ((((s)->modes & 2) != 0) ?
         handle_write_status(i_ctx_p, status, &rstdout, ((void*)0), zflush) :
         handle_read_status(i_ctx_p, status, &rstdout, ((void*)0), zflush));
}
...
zflushfile(i_ctx_t *i_ctx_p)
{
    ...
    return
        ((((s)->modes & 2) != 0) ?
         handle_write_status(i_ctx_p, status, op, ((void*)0), zflushfile) :
         handle_read_status(i_ctx_p, status, op, ((void*)0), zflushfile));
}
...
zprint(i_ctx_t *i_ctx_p)
{
    ...
    make_stream_file(&rstdout, s, "w");
    code = handle_write_status(i_ctx_p, status, &rstdout, ((void*)0),
                               zwritestring);
    if (code != 5)
        return code;
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
...
zfileposition(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&*(op))->tas.type_attrs))[1]) == (t_file)) ) return (check_type_failed(&*(op))); if ( (s = (op)->value.pfile, (s->read_id | s->write_id) != ((op)->tas.rsize)) ) return ((-7)); } while (0);
...
zsetfileposition(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op));
    do { if ( !((((const byte *)&((&*(op - 1))->tas.type_attrs))[1]) == (t_file)) ) return (check_type_failed(&*(op - 1))); if ( (s = (op - 1)->value.pfile, (s->read_id | s->write_id) != ((op - 1)->tas.rsize)) ) return ((-7)); } while (0);
    if (spseek(s, (gs_offset_t)((gs_offset_t)op->value.intval)) < 0)
        return ((-12));
    ((((i_ctx_p->op_stack).stack).p) -= (2));
    return 0;
}
...
zfilename(i_ctx_t *i_ctx_p)
{
    ...
    str = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), fname.size, "filename");
    if (str == 0)
        return ((-25));
    memcpy(str, fname.data, fname.size);
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op - 1)->value.const_bytes = (str), ((op - 1)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | imemory_space((const struct gs_ref_memory_s*) ((gs_memory_t *)((i_ctx_p->memory).current))))), ((op - 1)->tas.rsize = (fname.size)));
...
zisprocfilter(i_ctx_t *i_ctx_p)
{
    ...
    while (s->strm != 0)
        s = s->strm;
    ((op)->value.boolval = (s_is_proc(s)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
...
zpeekstring(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op) : (-7)));
    len = ((op)->tas.rsize);
    while ((rlen = ((s)->cursor.r.limit - (s)->cursor.r.ptr)) < len) {
    ...
    memcpy(op->value.bytes, ((s)->cursor.r.ptr + 1), rlen);
    ((op)->tas.rsize = (rlen));
    op[-1] = *op;
    ((op)->value.boolval = ((rlen == len ? 1 : 0)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
...
zwritecvp_at(i_ctx_t *i_ctx_p, os_ptr op, uint start, bool first)
{
    ...
    do { if ( !(((&*(op - 2))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_file) << 8) + (0x10))) ) return ((!((((const byte *)&((&*(op - 2))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(op - 2)) : (-7))); do { s = (op - 2)->value.pfile; if ( s->write_id != ((op - 2)->tas.rsize) ) { int fcode = file_switch_to_write(op - 2); if ( fcode < 0 ) return(fcode); } } while (0); } while (0);
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op));
    code = obj_cvp(op - 1, str, sizeof(str), &len, (int)op->value.intval,
                   start, ((gs_memory_t *)((i_ctx_p->memory).current)), ((bool)1));
    if (code == (-15)) {
        code = obj_string_data(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, &data, &len);
        if (len < start)
            return ((-15));
        data += start;
        len -= start;
    ...
    if (code == 1) {
        if (first)
            if ( (((i_ctx_p->op_stack).stack).top) - (((i_ctx_p->op_stack).stack).p) < (1) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); };
        (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (zwritecvp_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
        if (first)
            do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
        (((((i_ctx_p->op_stack).stack).p))->value.intval = (start + len), (((((i_ctx_p->op_stack).stack).p))->tas.type_attrs = ((t_integer) << 8) + (0)) );
        return 5;
    }
    if (first)
...
{
    return zwritecvp_at(i_ctx_p, (((i_ctx_p->op_stack).stack).p), 0, ((bool)1));
}
...
zwritecvp_continue(i_ctx_t *i_ctx_p)
{
    ...
    if (op->value.intval != (uint) op->value.intval)
        return ((-15));
    return zwritecvp_at(i_ctx_p, op - 1, (uint) op->value.intval, ((bool)0));
}
...
copy_error_string(i_ctx_t *i_ctx_p, const ref *fop)
{
    ...
    for (s = (fop)->value.pfile; s->strm != 0 && s->state->error_string[0] == 0;)
        s = s->strm;
    ...
    switch (ch) {
        ...
        case ((int)(-4)):
            if (pindex) {
                ...
                return s_handle_read_exception(i_ctx_p, ch, fop, ((void*)0), 0,
                                               cont);
    }
}
    ...
    switch (ch) {
        ...
        case ((int)(-4)):
            if (pindex) {
                ...
                return s_handle_write_exception(i_ctx_p, ch, fop, ((void*)0), 0,
                                                cont);
    }
}
