# 1 "./openjpeg/libopenjpeg/openjpeg.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./openjpeg/libopenjpeg/openjpeg.c" 2
# 33 "./openjpeg/libopenjpeg/openjpeg.c"
...
# 34 "./openjpeg/libopenjpeg/openjpeg.c" 2
...
{
 if (fseek(p_user_data,p_nb_bytes,1)) {
  return -1;
 }
...
{
 if (fseek(p_user_data,p_nb_bytes,0)) {
  return 0;
 }

 return 1;
}
# 269 "./openjpeg/libopenjpeg/openjpeg.c"
...
opj_codec_t* opj_create_decompress(OPJ_CODEC_FORMAT p_format)
{
 ...
 l_codec = (opj_codec_private_t*) ((size_t)(1) != 0 && (size_t)(1) >= (size_t)-0x100 / (size_t)(sizeof(opj_codec_private_t)) ? ((void*)0) : calloc(1, sizeof(opj_codec_private_t)));
 if (!l_codec){
  return 00;
 }
 ...
 switch (p_format) {
  ...
  case OPJ_CODEC_JP2:
   ...
   l_codec->m_codec = opj_jp2_create(1);
          ...
          )
{
 if (p_codec && parameters) {
  ...
  if (! l_codec->is_decompressor) {
   opj_event_msg(&(l_codec->m_event_mgr), 1,
...
OPJ_BOOL opj_decode( opj_codec_t *p_codec,
                                    opj_stream_t *p_stream,
                                    opj_image_t* p_image)
{
 if (p_codec && p_stream) {
...
OPJ_BOOL opj_get_decoded_tile( opj_codec_t *p_codec,
           opj_stream_t *p_stream,
           opj_image_t *p_image,
           OPJ_UINT32 tile_index)
{
...
OPJ_BOOL opj_set_decoded_resolution_factor(opj_codec_t *p_codec,
              OPJ_UINT32 res_factor )
{
 opj_codec_private_t * l_codec = (opj_codec_private_t *) p_codec;
...
opj_codec_t* opj_create_compress(OPJ_CODEC_FORMAT p_format)
{
 ...
 switch(p_format) {
  ...
  case OPJ_CODEC_JP2:
   ...
   l_codec->m_codec = opj_jp2_create(0);
   if (! l_codec->m_codec) {
    free(l_codec);
    return 00;
   }
...
# 805 "./openjpeg/libopenjpeg/openjpeg.c"
 }
}
                                  ...
                                  OPJ_INT32 * p_dc_shift,OPJ_UINT32 pNbComp)
{
 ...
 parameters->mct_data = ((size_t)(l_mct_total_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_mct_total_size));
...
void opj_dump_codec( opj_codec_t *p_codec,
         OPJ_INT32 info_flag,
         FILE* output_stream)
{
 if (p_codec) {
...
{
 return opj_stream_create_file_stream(p_file,0x100000,p_is_read_stream);
}

opj_stream_t* opj_stream_create_default_file_stream_v3 (const char *fname, OPJ_BOOL p_is_read_stream)
{
    return opj_stream_create_file_stream_v3(fname, 0x100000, p_is_read_stream);
}

opj_stream_t* opj_stream_create_file_stream ( FILE * p_file,
               OPJ_SIZE_T p_size,
               OPJ_BOOL p_is_read_stream)
{
...
opj_stream_t* opj_stream_create_file_stream_v3 (
        const char *fname,
  OPJ_SIZE_T p_size,
        OPJ_BOOL p_is_read_stream)
{
