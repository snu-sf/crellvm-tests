...
# 55 "./base/memory_.h" 2
# 20 "./base/gxclutil.c" 2
# 1 "./base/string_.h" 1
# 21 "./base/gxclutil.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
clist_update_trans_bbox(gx_device_clist_writer *cldev, gs_int_rect *bbox)
{
    ...
    first_band = (((0) > (bbox->p.y / cldev->page_info.band_params.BandHeight)) ? (0) : (bbox->p.y / cldev->page_info.band_params.BandHeight));
    p_y = bbox->p.y - (first_band * cldev->page_info.band_params.BandHeight);
    last_band = ((((cldev->nbands - 1)) < (bbox->q.y / cldev->page_info.band_params.BandHeight)) ? ((cldev->nbands - 1)) : (bbox->q.y / cldev->page_info.band_params.BandHeight));
...
cmd_put_list_op(gx_device_clist_writer * cldev, cmd_list * pcl, uint size)
{
    ...
    if (size + (sizeof(cmd_prefix) + 8) > cldev->cend - dp) {
        if ((cldev->error_code =
             cmd_write_buffer(cldev, cmd_opv_end_run)) != 0 ||
            (size + (sizeof(cmd_prefix) + 8) > cldev->cend - cldev->cnext)) {
...
{
    if (size + (sizeof(cmd_prefix) + 8) > cldev->cend - cldev->cnext) {
    ...
    return cldev->cend - cldev->cnext - (sizeof(cmd_prefix) + 8);
}
# 390 "./base/gxclutil.c"
byte *
...
const gx_color_index cmd_delta_offsets[] = {
        ((0) & (~((gx_color_index) 0))),
        ((0) & (~((gx_color_index) 0))),
        ((0x0808) & (~((gx_color_index) 0))),
        ((0x102010) & (~((gx_color_index) 0))),
        ((0x08080808) & (~((gx_color_index) 0)))

        ,
        ((0x1020100808) & (~((gx_color_index) 0))),
        ((0x080808080808) & (~((gx_color_index) 0))),
        ((0x10201008080808) & (~((gx_color_index) 0))),
        ((0x0808080808080808) & (~((gx_color_index) 0)))
...
static const gx_color_index cmd_delta_masks[] = {
        ((0) & (~((gx_color_index) 0))),
        ((0) & (~((gx_color_index) 0))),
        ((0x0f0f) & (~((gx_color_index) 0))),
        ((0x1f3f1f) & (~((gx_color_index) 0))),
        ((0x0f0f0f0f) & (~((gx_color_index) 0)))

        ,
        ((0x1f3f1f0f0f) & (~((gx_color_index) 0))),
        ((0x0f0f0f0f0f0f) & (~((gx_color_index) 0))),
        ((0x1f3f1f0f0f0f0f) & (~((gx_color_index) 0))),
        ((0x0f0f0f0f0f0f0f0f) & (~((gx_color_index) 0)))
              ...
              gx_color_index color, gx_color_index * pcolor)
{
    ...
    if (select->tile_color) {
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_tile_color), 0) );
        if (code < 0)
            return code;
    }
    op = select->set_op;
    ...
    if (color == ((gx_color_index)(~0))) {
        ...
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (op + 15), 0) );
        if (code < 0)
            return code;
    } else {
        ...
        if (use_delta && delta_bytes < (num_bytes - bytes_dropped)) {
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, delta_bytes + 1)) == 0 ? (cldev)->error_code : (*dp = (op_delta), 0) );
            ...
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, num_bytes + 1)) == 0 ? (cldev)->error_code : (*dp = ((byte)(op + bytes_dropped)), 0) );
                   ...
                   int px, int py, bool all_bands)
{
    ...
    pcsize = 1 + ((!(((px) | (py)) & ~0x7f)) ? 2 : cmd_size_w((uint)(px)) + cmd_size_w((uint)(py)));
    if (all_bands)
        code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, pcsize)) == 0 ? (cldev)->error_code : (*dp = ((byte)cmd_opv_set_tile_phase), 0) );
    else
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, pcsize)) == 0 ? (cldev)->error_code : (*dp = ((byte)cmd_opv_set_tile_phase), 0) );
    if (code < 0)
        return code;
    ++dp;
...
cmd_set_tile_phase(gx_device_clist_writer * cldev, gx_clist_state * pcls,
                   int px, int py)
{
    return cmd_set_tile_phase_generic(cldev, pcls, px, py, ((bool)0));
}
...
{
    byte *dp;
    int code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = ((byte)(enable ? cmd_opv_enable_lop : cmd_opv_disable_lop)), 0) );
...
{
    byte *dp;
    int code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = ((byte)(enable ? cmd_opv_enable_clip : cmd_opv_disable_clip)), 0) );
            ...
            gs_logical_operation_t lop)
{
    byte *dp;
    uint lop_msb = lop >> 6;
    int code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2 + cmd_size_w(lop_msb))) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_misc), 0) );
               ...
               gs_logical_operation_t lop)
{
    ...
    if (lop == (rop3_default | (((bool)0) ? 0x100 : 0) | (((bool)0) ? 0x200 : 0)))
        return ( (pcls)->lop_enabled == ((0) ^ 1) && cmd_put_enable_lop(cldev, pcls, 0) < 0 ? (cldev)->error_code : 0 );
    code = cmd_set_lop(cldev, pcls, lop);
    if (code < 0)
        return code;
    return ( (pcls)->lop_enabled == ((1) ^ 1) && cmd_put_enable_lop(cldev, pcls, 1) < 0 ? (cldev)->error_code : 0 );
}
               ...
               gs_param_list *param_list)
{
    ...
    if (param_length > 0) {
        ...
        code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, 2 + sizeof(unsigned) + param_length)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_extend), 0) );
        ...
        if (param_length > sizeof(local_buf)) {
            ...
            if (code < 0) {
                ...
                ((&cldev->band_range_list)->tail->size -= (old_param_length), (cldev)->cnext -= (old_param_length));

            }
        } else
            memcpy(dp, local_buf, param_length);
    }
    return code;
}
...
clist_cf_init(stream_CF_state *ss, int width)
{
    ...
    ss->EndOfBlock = ((bool)1);

    ss->BlackIs1 = ((bool)1);
    ss->DecodedByteAlign = (1 << 3);
}
void
...
clist_rle_init(stream_RLE_state *ss)
{
    ...
    ((ss)->EndOfData = ((bool)1), (ss)->record_size = 0);
    ((ss)->record_left = ((ss)->record_size == 0 ? ((ss)->record_size = ((unsigned int)~0 + (unsigned int)0)) : (ss)->record_size), (ss)->copy_left = 0);
}
void
