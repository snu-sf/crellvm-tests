                                ...
                                pdf14_device * p14dev, byte * out)
{
    ...
    if (p14dev->devn_params.compressed_color_list == ((void*)0)) {
                                ...
                                pdf14_device * p14dev, byte * out)
{
    ...
    gx_color_value cm_values[(8 * 8)];

    ((tdev)->procs.decode_color)(tdev, color, cm_values);
    for (i = 0; i < num_comp; i++)
        out[i] = 0xff - ((gx_color_value)((((unsigned int)cm_values[i])*0xFF01 + 0x800000)>>24));
}
                           ...
                           bool knockout_buff)
{
    ...
    int x0 = (((buf->rect.p.x) > (tos->rect.p.x)) ? (buf->rect.p.x) : (tos->rect.p.x));
    int x1 = (((buf->rect.q.x) < (tos->rect.q.x)) ? (buf->rect.q.x) : (tos->rect.q.x));
    int y0 = (((buf->rect.p.y) > (tos->rect.p.y)) ? (buf->rect.p.y) : (tos->rect.p.y));
    int y1 = (((buf->rect.q.y) < (tos->rect.q.y)) ? (buf->rect.q.y) : (tos->rect.q.y));
    ...
    if (x0 < x1 && y0 < y1) {
        ...
        icc_link = gsicc_get_link_profile(pis, dev, tos_profile, group_profile,
                                          &rendering_params, memory, ((bool)0));
        if (icc_link == ((void*)0))
            return gs_throw_imp(__func__, "./base/gxblend1.c", 212, 0, (-1), "ICC link failed.  Trans backdrop");
        ...
        if (icc_link->is_identity) {
            ...
            gsicc_init_buffer(&input_buff_desc, tos_profile->num_comps, 1, ((bool)0),
                              ((bool)0), ((bool)1), tos->planestride, tos->rowstride, height,
                              width);
            gsicc_init_buffer(&output_buff_desc, group_profile->num_comps, 1, ((bool)0),
                              ((bool)0), ((bool)1), buf->planestride, buf->rowstride, height,
                              width);
              ...
              gs_memory_t *memory)
{
    ...
    byte *mask_row_ptr = ((void*)0);
    byte *mask_curr_ptr = ((void*)0);
    ...
    int x, y;
    int i;
    byte tos_pixel[(8 * 8)+3];
    byte nos_pixel[(8 * 8)+3];
    byte back_drop[(8 * 8)+3];
    ...
    byte *mask_tr_fn = ((void*)0);
    gx_color_index comps;
    bool has_mask = ((bool)0);
    bool in_mask_rect_y = ((bool)0);
    bool in_mask_rect = ((bool)0);
    byte pix_alpha;
    bool has_backdrop = ((bool)0);
    byte *backdrop_ptr = ((void*)0);
    ...
    if ((tos->n_chan == 0) || (nos->n_chan == 0))
        return;
    do { if ((tos->dirty).p.x < (nos->dirty).p.x) (nos->dirty).p.x = (tos->dirty).p.x; if ((tos->dirty).q.x > (nos->dirty).q.x) (nos->dirty).q.x = (tos->dirty).q.x; if ((tos->dirty).p.y < (nos->dirty).p.y) (nos->dirty).p.y = (tos->dirty).p.y; if ((tos->dirty).q.y > (nos->dirty).q.y) (nos->dirty).q.y = (tos->dirty).q.y; } while (0);
    do { } while (0);
    ...
    if (nos->has_alpha_g) {
        ...
        nos_alpha_g_ptr = ((void*)0);
    if (nos->backdrop != ((void*)0)) {
        has_backdrop = ((bool)1);
...
# 428 "./base/gxblend1.c"
    for (y = y1 - y0; y > 0; --y) {
        ...
        if (has_mask && y1 - y >= maskbuf->rect.p.y && y1 - y < maskbuf->rect.q.y) {
            in_mask_rect_y = ((bool)1);
        } else {
            in_mask_rect_y = ((bool)0);
        }
        for (x = 0; x < width; x++) {
            ...
            if (nos_knockout) {
                ...
                byte *nos_shape_ptr = nos_shape_offset ?
                    &nos_ptr[nos_shape_offset] : ((void*)0);
                byte *nos_tag_ptr = nos_tag_offset ?
                    &nos_ptr[nos_tag_offset] : ((void*)0);
                ...
                if (tos_isolated || (nos->saved == ((void*)0) && tos->knockout)) {
...
pdf14_encode_color(gx_device *dev, const gx_color_value colors[])
{
    ...
    do { COLROUND_M = (((1<<8)-1)<<((sizeof(gx_color_value) * 8)-8))+1; COLROUND_S = ((sizeof(gx_color_value) * 8)*2-8); COLROUND_R = 1<<(COLROUND_S-1); } while (0 == 1);
    for (i = 0; i < ncomp; i++) {
        color <<= 8;
...
{
    gs_devn_params *pdevn_params = ((void*)0);

    if (dev->procs.ret_devn_params != ((void*)0))
        pdevn_params = ((dev)->procs.ret_devn_params)(dev);
...
# 984 "./base/gxblend1.c"
    for (comp_num = 0; comp_num < 4; comp_num++) {
        ...
        output_comp_num = ((target)->procs.get_color_comp_index)
                (target, pcomp_name, strlen(pcomp_name), 0);
    ...
    for (comp_num = 0; comp_num < num_sep; comp_num++) {
        output_comp_num = ((target)->procs.get_color_comp_index)
               (target, (const char *)(pseparations->names[comp_num].data),
                pseparations->names[comp_num].size, 0);
        if (output_comp_num >= 0 &&
                output_comp_num < (8 * 8)) {
            output_map[num_known_comp] = output_comp_num;
...
# 1037 "./base/gxblend1.c"
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            color = ((target)->procs.encode_color)(target, cv);
            code = ((target)->procs.fill_rectangle)(target, x + x0,
                                                            y + y0, 1, 1, color);
            if (code < 0)
                return code;
        }
