...
# 94 "./base/gdevmr2n.c"
...

    {
        ...
        int (*strip_tile_rectangle)(gx_device *dev, const gx_strip_bitmap *tiles, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1, int phase_x, int phase_y) =
            ((dev)->procs.strip_tile_rectangle);

        ((dev)->procs.fill_rectangle = (mem_gray_rop_fill_rectangle));
        ((dev)->procs.copy_mono = (mem_gray_rop_copy_mono));
        ((dev)->procs.strip_tile_rectangle = (mem_gray_rop_strip_tile_rectangle));
        ...
        ((dev)->procs.fill_rectangle = (fill_rectangle));
        ((dev)->procs.copy_mono = (copy_mono));
        ((dev)->procs.strip_tile_rectangle = (strip_tile_rectangle));
        dev->width >>= log2_depth;
    }
