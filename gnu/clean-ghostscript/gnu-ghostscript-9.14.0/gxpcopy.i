                      ...
                      gx_path_copy_options options)
{
    ...
    if (options & pco_for_stroke) {
        ...
        expansion.x =
            ((fixed)(((fabs(pis->ctm.xx) + fabs(pis->ctm.yx)) * width)*(float)(1<<8))) * 2;
        expansion.y =
            ((fixed)(((fabs(pis->ctm.xy) + fabs(pis->ctm.yy)) * width)*(float)(1<<8))) * 2;
    } else
        expansion.x = expansion.y = 0;
    do { } while (0);
    pseg = (const segment *)(ppath_old->segments->contents.subpath_first);
    while (pseg) {
        switch (pseg->type) {
            case s_start:
...
# 165 "./base/gxpcopy.c"
                            do { if (x0 < pc->p1.x) min01 = x0, max01 = pc->p1.x; else min01 = pc->p1.x, max01 = x0; if (pc->p2.x < pc->pt.x) min23 = pc->p2.x, max23 = pc->pt.x; else min23 = pc->pt.x, max23 = pc->p2.x; ex = (((max01) > (max23)) ? (max01) : (max23)) - (((min01) < (min23)) ? (min01) : (min23)); } while (0);
                            do { if (y0 < pc->p1.y) min01 = y0, max01 = pc->p1.y; else min01 = pc->p1.y, max01 = y0; if (pc->p2.y < pc->pt.y) min23 = pc->p2.y, max23 = pc->pt.y; else min23 = pc->pt.y, max23 = pc->p2.y; ey = (((max01) > (max23)) ? (max01) : (max23)) - (((min01) < (min23)) ? (min01) : (min23)); } while (0);
                            ...
                            if (ex == 0 || ey == 0)
                                ...
                                flat_y =
                                    fixed_mult_quo(fixed_flatness, ey,
                                                   ey + expansion.y);
                                flat = (((flat_x) < (flat_y)) ? (flat_x) : (flat_y));
                                k = gx_curve_log2_samples(x0, y0, pc, flat);
                            }
                        } else
                            k = gx_curve_log2_samples(x0, y0, pc, flat);
                        if (options & pco_accurate) {
                            ...
                            if (code < 0)
                                break;
                            do { } while (0);
                            start = ppath->segments->contents.subpath_current->last;
                            notes |= sn_not_first;
                            cseg = *pc;
                            ...
                            end = ppath->segments->contents.subpath_current->last;
                            do { } while (0);
    ...
    if ((((ppath_old)->state_flags & ~psf_outside_range) == psf_last_moveto))
        gx_path_add_point(ppath, ppath_old->position.x,
                          ppath_old->position.y);
    if (ppath_old->bbox_set) {
        if (ppath->bbox_set) {
            ppath->bbox.p.x = (((ppath_old->bbox.p.x) < (ppath->bbox.p.x)) ? (ppath_old->bbox.p.x) : (ppath->bbox.p.x));
            ppath->bbox.p.y = (((ppath_old->bbox.p.y) < (ppath->bbox.p.y)) ? (ppath_old->bbox.p.y) : (ppath->bbox.p.y));
            ppath->bbox.q.x = (((ppath_old->bbox.q.x) > (ppath->bbox.q.x)) ? (ppath_old->bbox.q.x) : (ppath->bbox.q.x));
            ppath->bbox.q.y = (((ppath_old->bbox.q.y) > (ppath->bbox.q.y)) ? (ppath_old->bbox.q.y) : (ppath->bbox.q.y));
        } else {
            ppath->bbox_set = ((bool)1);
                        ...
                        const gs_fixed_point * p1)
{
    ...
    if (fC == 0) {
        ...
        if (T > 0) {
            ...
            pseg->pt.x = (((fixed) (C * T)) >> (2)) + x0;
            pseg->pt.y = (((fixed) (D * T)) >> (2)) + y0;
        }
    }
}
...
gx_path__check_curves(const gx_path * ppath, gx_path_copy_options options, fixed fixed_flat)
{
    const segment *pseg = (const segment *)(ppath->segments->contents.subpath_first);
    gs_fixed_point pt0;
...
gx_curve_monotonize(gx_path * ppath, const curve_segment * pc)
{
    ...
    for (i = k + 1, j = k; i < n && t[k] < 1 - delta; i++)
        if (((t[i] - t[j]) < 0 ? -(t[i] - t[j]) : (t[i] - t[j])) < delta) {
            t[j] = (t[j] + t[i]) / 2;
            c[j] |= c[i];
        } else {
            j++;
    ...
    ( v01 = (pc->p1.x) - (x0), cx = (v01 << 1) + v01, v12 = (pc->p2.x) - (pc->p1.x), bx = (v12 << 1) + v12 - cx, ax = (pc->pt.x) - bx - cx - (x0));
    ( v01 = (pc->p1.y) - (y0), cy = (v01 << 1) + v01, v12 = (pc->p2.y) - (pc->p1.y), by = (v12 << 1) + v12 - cy, ay = (pc->pt.y) - by - cy - (y0));
    ax *= 3, bx *= 2;
    ay *= 3, by *= 2;
    px = x0;
    py = y0;
    ...
    for (i = k; i < n; i++) {
        ...
        double ddx = (c[i] & 1 ? 0 : ax * t2 + bx * ti + cx);
...
# 598 "./base/gxpcopy.c"
    ...
    ( v01 = (v1) - (v0), c = (v01 << 1) + v01, v12 = (v2) - (v1), b = (v12 << 1) + v12 - c, a = (v3) - b - c - (v0));
    b2 = b << 1;
    a3 = (a << 1) + a;
    ...
    if (a == 0) {
        if ((b ^ c) < 0 && ((c) < 0 ? -(c) : (c)) < ((b2) < 0 ? -(b2) : (b2)) && c != 0) {
            *pst = (double)(-c) / b2;
            return 1;
        } else
            return 0;
    ...
    if (c == 0) {
        if ((a ^ b) < 0 && ((b2) < 0 ? -(b2) : (b2)) < ((a3) < 0 ? -(a3) : (a3)) && b != 0) {
            *pst = (double)(-b2) / a3;
            return 1;
        } else
            return 0;
        ...
        if ((a ^ b) < 0 &&
            (b2abs = ((b2) < 0 ? -(b2) : (b2))) > (a3abs = ((a3) < 0 ? -(a3) : (a3))) &&
            b2abs < a3abs << 1
            ) {
            *pst = (double)(-b2 - a3) / a3;
            return 1;
                         ...
                         segment **sc0, segment **sc1)
{
    ...
    int min_length = (1L << 8) * 1;
# 735 "./base/gxpcopy.c"
    for (s0 = sp0last, count0 = 0; count0 < search_limit && s0 != (segment *)sp0; s0 = s0->prev, count0++) {
        s0s = s0->prev;
        if ((s0->type == s_line || s0->type == s_gap) &&
            (s0s->pt.x == s0->pt.x ||
             (((s0s->pt.x - s0->pt.x) < 0 ? -(s0s->pt.x - s0->pt.x) : (s0s->pt.x - s0->pt.x)) == 1 &&
              ((s0s->pt.y - s0->pt.y) < 0 ? -(s0s->pt.y - s0->pt.y) : (s0s->pt.y - s0->pt.y)) > min_length))) {
            for (s1 = sp1last, count1 = 0; count1 < search_limit && s1 != (segment *)sp1; s1 = s1->prev, count1++) {
                s1s = s1->prev;
                if ((s1->type == s_line || s1->type == s_gap) &&
                    (s1s->pt.x == s1->pt.x ||
                     (((s1s->pt.x - s1->pt.x) < 0 ? -(s1s->pt.x - s1->pt.x) : (s1s->pt.x - s1->pt.x)) == 1 && ((s1s->pt.y - s1->pt.y) < 0 ? -(s1s->pt.y - s1->pt.y) : (s1s->pt.y - s1->pt.y)) > min_length))) {
                    if (s0s->pt.x == s1s->pt.x || s0->pt.x == s1->pt.x || s0->pt.x == s1s->pt.x || s0s->pt.x == s1->pt.x) {
                        if (s0s->pt.y < s0->pt.y && s1s->pt.y > s1->pt.y) {
                            fixed y0 = (((s0s->pt.y) > (s1->pt.y)) ? (s0s->pt.y) : (s1->pt.y));
                            fixed y1 = (((s0->pt.y) < (s1s->pt.y)) ? (s0->pt.y) : (s1s->pt.y));
                        ...
                        if (s0s->pt.y > s0->pt.y && s1s->pt.y < s1->pt.y) {
                            fixed y0 = (((s0->pt.y) > (s1s->pt.y)) ? (s0->pt.y) : (s1s->pt.y));
                            fixed y1 = (((s0s->pt.y) < (s1->pt.y)) ? (s0s->pt.y) : (s1->pt.y));
...
gx_path_merge_contacting_contours(gx_path *ppath)
{
    ...
    for (; sp0 != ((void*)0); sp0 = (subpath *)sp0->last->next) {
        segment *sp0last = sp0->last;
        subpath *sp1 = (subpath *)sp0last->next, *spnext;
        subpath *sp1p = sp0;
        int count;

        for (count = 0; sp1 != ((void*)0) && count < window; sp1 = spnext, count++) {
            ...
            if (find_contacting_segments(sp0, sp0last, sp1, sp1last, &sc0, &sc1)) {
                ...
                if (sp1last->type == s_line_close) {
                    ...
                    ((((ppath->memory)->procs.stable(ppath->memory)))->procs.free_object(((ppath->memory)->procs.stable(ppath->memory)), sp1, "gx_path_merge_contacting_contours"));
