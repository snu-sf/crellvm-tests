...
static gs_ptr_type_t gs_glyph_data_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_glyph_data_t *pgd = vptr; switch ( index ) { default:
    case 0: return enum_const_bytestring(pep, &pgd->bits);
    case 1: return (pep->ptr = (const void *)(pgd->proc_data), (&ptr_struct_procs));
} }
static void gs_glyph_data_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_glyph_data_t *pgd = vptr;
{
    reloc_const_bytestring(&(pgd->bits), gcst);
    (pgd->proc_data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(pgd->proc_data), gcst)));
} }
static gs_ptr_type_t gs_glyph_data_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void gs_glyph_data_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_glyph_data = { sizeof(gs_glyph_data_t), "gs_glyph_data_t", 0, 0, gs_glyph_data_enum_ptrs, gs_glyph_data_reloc_ptrs, 0, 0 };
...
glyph_data_substring_by_font(gs_glyph_data_t *pgd, uint offset, uint size)
{
    ...
    pgd->bits.data =
        (*(font->memory)->procs.resize_string)(font->memory, data, pgd->bits.size, size, "glyph_data_substring");
...
gs_glyph_data_from_string(gs_glyph_data_t *pgd, const byte *data,
                          uint size, gs_font *font)
{
    ((&pgd->bits)->data = (data), (&pgd->bits)->size = (size), (&pgd->bits)->bytes = 0);
    pgd->proc_data = font;
...
gs_glyph_data_from_bytes(gs_glyph_data_t *pgd, const byte *bytes,
                         uint offset, uint size, gs_font *font)
{
    ((&pgd->bits)->data = ((&pgd->bits)->bytes = (bytes)) + (offset), (&pgd->bits)->size = (size));
    pgd->proc_data = font;
    pgd->procs = (font ? &free_by_font_procs : &no_free_procs);
}
void
...
{
    gs_glyph_data_from_string(pgd, ((void*)0), 0, ((void*)0));
}
