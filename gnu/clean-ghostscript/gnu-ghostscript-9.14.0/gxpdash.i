                           ...
                           const gs_imager_state * pis)
{
    ...
    if (dash->pattern_size == 0)
        return gx_path_copy_reducing(ppath_old, ppath, (~(-1 << ((1 << 2) * 8 - 1))), ((void*)0), pco_none);
    for (psub = ppath_old->segments->contents.subpath_first; psub != 0 && code >= 0;
  ...
  top:count = dash->pattern_size;
    ...
    while ((pseg = pseg->next) != 0 && pseg->type != s_start) {
        ...
        if (!(udx | udy)) {
            ...
            length = sqrt((double)(d.x)*(d.x)+(double)(d.y)*(d.y)) * (1.0 / (1L << 8));
        ...
        while (left > elt_length) {
            ...
            if (ink_on && !gap) {
                if (drawing >= 0) {
                    if (left >= elt_length && ((fx) < 0 ? -(fx) : (fx)) + ((fy) < 0 ? -(fy) : (fy)) < ((1L << 8) >> 1))
                ...
                if (pseg->type == s_line_close && drawing > 0)
                    code = gx_path_close_subpath_notes(ppath,
                                                       ((notes & pseg->notes)|
                                                        start_notes |
                                                        end_notes));
                else if ((((sx - x) < 0 ? -(sx - x) : (sx - x)) + ((sy - y) < 0 ? -(sy - y) : (sy - y)) < ((1L << 8) >> 1)) &&
                         (udx | udy))
