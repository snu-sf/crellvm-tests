...
# 41 "./base/gxshade6.c" 2
# 80 "./base/gxshade6.c"
enum {
    min_linear_grades = 255
...
{
    if (pfs->color_stack != ((void*)0))
        return 0;
    pfs->color_stack_step = ((int) ( (char *)&((patch_color_t *)0)->cc.paint.values[pfs->num_components] - (char *)((patch_color_t *)0) ));
    pfs->color_stack_step = (pfs->color_stack_step + sizeof(void *) - 1) / sizeof(void *) * sizeof(void *);

    pfs->color_stack_size = pfs->color_stack_step * 200;;
    pfs->color_stack = (*(memory)->procs.alloc_bytes)(memory, pfs->color_stack_size, "allocate_color_stack");
    if (pfs->color_stack == ((void*)0))
        return ((-25));
...
shade_next_curve(shade_coord_stream_t * cs, patch_curve_t * curve)
{
    ...
    if (code >= 0)
        code = shade_next_coords(cs, curve->control,
                                 (sizeof(curve->control) / sizeof((curve->control)[0])));
    return code;
}
...
alloc_patch_fill_memory(patch_fill_state_t *pfs, gs_memory_t *memory, const gs_color_space *pcs)
{
    ...
    if (code < 0)
        return code;
    if (pfs->unlinear || pcs == ((void*)0))
        pfs->pcic = ((void*)0);
    else {
        pfs->pcic = gs_color_index_cache_create(memory, pcs, pfs->dev, pfs->pis, ((bool)1), pfs->trans_device);
        if (pfs->pcic == ((void*)0))
            return ((-25));
    }
    return 0;
...
init_patch_fill_state(patch_fill_state_t *pfs)
{
    ...
    for (i = 0; i < pfs->num_components; i++)
        pfs->color_domain.paint.values[i] = (((fcc1.paint.values[i] - fcc0.paint.values[i]) > (1)) ? (fcc1.paint.values[i] - fcc0.paint.values[i]) : (1));
    pfs->vectorization = ((bool)0);
    pfs->maybe_self_intersecting = ((bool)1);
    pfs->monotonic_color = (pfs->Function == ((void*)0));
    pfs->function_arg_shift = 0;
    pfs->linear_color = ((bool)0);
    pfs->inside = ((bool)0);
    pfs->n_color_args = 1;
    ...
    pfs->decomposition_limit = (1L << 8);

    pfs->fixed_flat = ((fixed)((pfs->pis->flatness)*(float)(1<<8)));




    pfs->smoothness = (((pfs->pis->smoothness) > (1.0 / min_linear_grades)) ? (pfs->pis->smoothness) : (1.0 / min_linear_grades));
    pfs->color_stack_size = 0;
    pfs->color_stack_step = 0;
    pfs->color_stack_ptr = ((void*)0);
    pfs->color_stack = ((void*)0);
    pfs->color_stack_limit = ((void*)0);
...
term_patch_fill_state(patch_fill_state_t *pfs)
{
    ...
    if (pfs->color_stack)
        ((pfs->memory)->procs.free_object(pfs->memory, pfs->color_stack, "term_patch_fill_state"));
    if (pfs->pcic != ((void*)0))
...
patch_resolve_color_inline(patch_color_t * ppcr, const patch_fill_state_t *pfs)
{
    if (pfs->Function) {
        ...
        ((pfs->Function)->head.procs.evaluate)(pfs->Function, ppcr->t, ppcr->cc.paint.values);
           ...
           const gs_fixed_point * p3, double t)
{
    ...
    d = p0->x;
    ( t01 = (p1->x) - (d), c = (t01 << 1) + t01, t12 = (p2->x) - (p1->x), b = (t12 << 1) + t12 - c, a = (p3->x) - b - c - (d));

    pt->x = (fixed) (((a * t + b) * t + c) * t + d);
    d = p0->y;
    ( t01 = (p1->y) - (d), c = (t01 << 1) + t01, t12 = (p2->y) - (p1->y), b = (t12 << 1) + t12 - c, a = (p3->y) - b - c - (d));
             ...
             const gs_fixed_point ignore_interior[4], double u, double v)
{
    ...
    curve_eval(&c1u, &curve[0].vertex.p,
               &curve[3].control[1], &curve[3].control[0],
               &curve[3].vertex.p, u);
    curve_eval(&d1v, &curve[0].vertex.p,
               &curve[0].control[0], &curve[0].control[1],
               &curve[1].vertex.p, v);
    curve_eval(&c2u, &curve[1].vertex.p,
               &curve[1].control[0], &curve[1].control[1],
               &curve[2].vertex.p, u);
    curve_eval(&d2v, &curve[3].vertex.p,
               &curve[2].control[1], &curve[2].control[0],
               &curve[2].vertex.p, v);
    ...
    pt->x = (fixed) ((co_v * c1u.x + v * c2u.x) + (co_u * d1v.x + u * d2v.x) - (co_v * (co_u * curve[0].vertex.p.x + u * curve[3].vertex.p.x) + v * (co_u * curve[1].vertex.p.x + u * curve[2].vertex.p.x)));
    pt->y = (fixed) ((co_v * c1u.y + v * c2u.y) + (co_u * d1v.y + u * d2v.y) - (co_v * (co_u * curve[0].vertex.p.y + u * curve[3].vertex.p.y) + v * (co_u * curve[1].vertex.p.y + u * curve[2].vertex.p.y)));
                             ...
                             gx_device * dev, gs_imager_state * pis)
{
    ...
    curve[0].straight = curve[1].straight = curve[2].straight = curve[3].straight = ((bool)0);
    shade_next_init(&cs, (const gs_shading_mesh_params_t *)&psh->params, pis);
    while ((code = shade_next_patch(&cs, psh->params.BitsPerFlag,
                                    curve, ((void*)0))) == 0 &&
           (code = patch_fill(&state, curve, ((void*)0), Cp_transform)) >= 0
        ) {
        do { } while (0);
    }
    if (1 && ((bool)0))
...
# 668 "./base/gxshade6.c"
typedef struct {
    gs_fixed_point pole[4][4];
    patch_color_t *c[2][2];
} tensor_patch;

typedef enum {
    interpatch_padding = 1,
    inpatch_wedge = 2
} wedge_type_t;
...
# 694 "./base/gxshade6.c"
    ...
    pfs->wedge_vertex_list_elem_buffer = (wedge_vertex_list_elem_t *)(*(memory)->procs.alloc_bytes)(memory, sizeof(wedge_vertex_list_elem_t) * pfs->wedge_vertex_list_elem_count_max, "alloc_wedge_vertex_list_elem_buffer");


    if (pfs->wedge_vertex_list_elem_buffer == ((void*)0))
        return ((-25));
    pfs->free_wedge_vertex = ((void*)0);
...
wedge_vertex_list_elem_buffer_free(patch_fill_state_t *pfs)
{
    ...
    ((memory)->procs.free_object(memory, pfs->wedge_vertex_list_elem_buffer, "wedge_vertex_list_elem_buffer_free"));

    pfs->wedge_vertex_list_elem_buffer = ((void*)0);
    pfs->free_wedge_vertex = ((void*)0);
}

static __inline__ wedge_vertex_list_elem_t *
                ...
                const gs_fixed_point *pole, int pole_step, fixed fixed_flat)
{
    ...
    k = gx_curve_log2_samples(pole[0].x, pole[0].y, &s, fixed_flat);
    {
            ...
            int k1;
            fixed L = ((pole[pole_step * 1].x - pole[pole_step * 0].x) < 0 ? -(pole[pole_step * 1].x - pole[pole_step * 0].x) : (pole[pole_step * 1].x - pole[pole_step * 0].x)) + ((pole[pole_step * 1].y - pole[pole_step * 0].y) < 0 ? -(pole[pole_step * 1].y - pole[pole_step * 0].y) : (pole[pole_step * 1].y - pole[pole_step * 0].y)) +
                      ((pole[pole_step * 2].x - pole[pole_step * 1].x) < 0 ? -(pole[pole_step * 2].x - pole[pole_step * 1].x) : (pole[pole_step * 2].x - pole[pole_step * 1].x)) + ((pole[pole_step * 2].y - pole[pole_step * 1].y) < 0 ? -(pole[pole_step * 2].y - pole[pole_step * 1].y) : (pole[pole_step * 2].y - pole[pole_step * 1].y)) +
                      ((pole[pole_step * 3].x - pole[pole_step * 2].x) < 0 ? -(pole[pole_step * 3].x - pole[pole_step * 2].x) : (pole[pole_step * 3].x - pole[pole_step * 2].x)) + ((pole[pole_step * 3].y - pole[pole_step * 2].y) < 0 ? -(pole[pole_step * 3].y - pole[pole_step * 2].y) : (pole[pole_step * 3].y - pole[pole_step * 2].y));




            k1 = ilog2(L / (1L << 8) / (1 << (9 - 1)));
            k = (((k) > (k1)) ? (k) : (k1));
        ...
        bool swap_axes, const gx_device_color *pdevc, bool orient)
{
    gs_fixed_edge le, re;
    int code;
    fixed ybot = (((ybot0) > (swap_axes ? pfs->rect.p.x : pfs->rect.p.y)) ? (ybot0) : (swap_axes ? pfs->rect.p.x : pfs->rect.p.y));
    fixed ytop = (((ytop0) < (swap_axes ? pfs->rect.q.x : pfs->rect.q.y)) ? (ytop0) : (swap_axes ? pfs->rect.q.x : pfs->rect.q.y));
...
# 953 "./base/gxshade6.c"
        ...
        if (clip)
        {
            ...
            if (ytl < ybr) {
# 1197 "./base/gxshade6.c"
                code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
                                        &lenew, &re, ybot, ytl,
                ...
                if (code < 0)
                    return code;
                ybot = ybr;
                return ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
                                        &le, &renew, ybr, ytop,
                                        swap_axes, pdevc, pfs->pis->log_op);
            } else if (ytr < ybl) {
# 1220 "./base/gxshade6.c"
                code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
                                        &le, &renew, ybot, ytr,
                                        swap_axes, pdevc, pfs->pis->log_op);
                if (code < 0)
            ...
            frac31 fc[(8 * 8)])
{
    ...
    if (pdevc->type == &gx_dc_type_data_pure) {
        ...
        for (j = 0; j < cinfo->num_components; j++) {
                fc[j] = ((frac)( ((pdevc->colors.devn.values[j]) >> (((1 << 1) * 8) - 15)) - ((pdevc->colors.devn.values[j]) >> (((1 << 1) * 8) - 3)) ));
        }
    }
}
...
{
    return patch_color_to_device_color_inline(pfs, c, pdevc, ((void*)0));
}
...
color_span(const patch_fill_state_t *pfs, const patch_color_t *c0, const patch_color_t *c1)
{
    ...
    m = ((c1->cc.paint.values[0] - c0->cc.paint.values[0]) < 0 ? -(c1->cc.paint.values[0] - c0->cc.paint.values[0]) : (c1->cc.paint.values[0] - c0->cc.paint.values[0])) / pfs->color_domain.paint.values[0];
    for (i = 1; i < n; i++)
        m = (((m) > (((c1->cc.paint.values[i] - c0->cc.paint.values[i]) < 0 ? -(c1->cc.paint.values[i] - c0->cc.paint.values[i]) : (c1->cc.paint.values[i] - c0->cc.paint.values[i])) / pfs->color_domain.paint.values[i])) ? (m) : (((c1->cc.paint.values[i] - c0->cc.paint.values[i]) < 0 ? -(c1->cc.paint.values[i] - c0->cc.paint.values[i]) : (c1->cc.paint.values[i] - c0->cc.paint.values[i])) / pfs->color_domain.paint.values[i]));
    return m;
}
...
color_norm(const patch_fill_state_t *pfs, const patch_color_t *c)
{
    ...
    m = ((c->cc.paint.values[0]) < 0 ? -(c->cc.paint.values[0]) : (c->cc.paint.values[0])) / pfs->color_domain.paint.values[0];
    for (i = 1; i < n; i++)
        m = (((m) > (((c->cc.paint.values[i]) < 0 ? -(c->cc.paint.values[i]) : (c->cc.paint.values[i])) / pfs->color_domain.paint.values[i])) ? (m) : (((c->cc.paint.values[i]) < 0 ? -(c->cc.paint.values[i]) : (c->cc.paint.values[i])) / pfs->color_domain.paint.values[i]));
    return m;
}
...
{
# 1490 "./base/gxshade6.c"
    uint mask;
    int code = ((pfs->Function)->head.procs.is_monotonic)(pfs->Function, c0->t, c1->t, &mask);
...
{
    return (((ybot)+(((1<<8)>>1) - 1L))&(-1L<<8)) < (((ytop)+(((1<<8)>>1) - 1L))&(-1L<<8));
}
        ...
        fixed ybot, fixed ytop, bool swap_axes, const patch_color_t *c)
{
    ...
    code = patch_color_to_device_color_inline(pfs, &c1, &dc, ((void*)0));
    if (code < 0)
        return code;
    if (!1)
        do { } while (0);
...
function_linearity(const patch_fill_state_t *pfs, const patch_color_t *c0, const patch_color_t *c1)
{
    ...
    if (pfs->Function != ((void*)0)) {
        ...
        for (j = 0; j < (((int)(sizeof(q))) / ((int)(sizeof((q)[0])))); j++) {
            ...
            for (i = 0; i < pfs->num_components; i++) {
                ...
                float s1 = ((d) < 0 ? -(d) : (d)) / pfs->color_domain.paint.values[i];
...
is_color_linear(const patch_fill_state_t *pfs, const patch_color_t *c0, const patch_color_t *c1)
{
    if (pfs->unlinear)
        ...
        if (s > pfs->smoothness)
            return 0;
        code = (*(cs)->type->is_linear)(cs, pfs->pis, pfs->trans_device, &c0->cc, &c1->cc, ((void*)0), ((void*)0), pfs->smoothness - s, pfs->icclink);
        ...
        const patch_color_t *c1)
{
    ...
    if (!pfs->inside) {
        ...
        if(swap_axes) {
            r.p.y = (((le->start.x) < (le->end.x)) ? (le->start.x) : (le->end.x));
            r.p.x = (((le->start.y) < (le->end.y)) ? (le->start.y) : (le->end.y));
            r.q.y = (((re->start.x) > (re->end.x)) ? (re->start.x) : (re->end.x));
            r.q.x = (((re->start.y) > (re->end.y)) ? (re->start.y) : (re->end.y));
        } else {
            r.p.x = (((le->start.x) < (le->end.x)) ? (le->start.x) : (le->end.x));
            r.p.y = (((le->start.y) < (le->end.y)) ? (le->start.y) : (le->end.y));
            r.q.x = (((re->start.x) > (re->end.x)) ? (re->start.x) : (re->end.x));
            r.q.y = (((re->start.y) > (re->end.y)) ? (re->start.y) : (re->end.y));
        }
        r1 = r;
        do { if ((pfs->rect).p.x > (r).p.x) (r).p.x = (pfs->rect).p.x; if ((pfs->rect).q.x < (r).q.x) (r).q.x = (pfs->rect).q.x; if ((pfs->rect).p.y > (r).p.y) (r).p.y = (pfs->rect).p.y; if ((pfs->rect).q.y < (r).q.y) (r).q.y = (pfs->rect).q.y; } while (0);
    ...
    if (color_stack_ptr == ((void*)0))
        return ((-28));



    patch_interpolate_color(c, c0, c1, pfs, 0.5);
    if (ytop - ybot < pfs->decomposition_limit)
        ...
        if (!pfs->unlinear && pfs->linear_color) {
            ...
            clip.p.y = (((clip.p.y) > (ybot)) ? (clip.p.y) : (ybot));
            clip.q.y = (((clip.q.y) < (ytop)) ? (clip.q.y) : (ytop));
            fa.clip = &clip;
            fa.ht = ((void*)0);
            ...
            fa.ystart = ybot;
            fa.yend = ytop;
            code = patch_color_to_device_color_inline(pfs, c0, ((void*)0), fc[0]);
            if (code < 0)
                goto out;
            if (code == 2) {
            ...
            code = patch_color_to_device_color_inline(pfs, c1, ((void*)0), fc[1]);
            if (code < 0)
                goto out;
            if (fa.swap_axes) {
                do { } while (0);
            ...
            code = ((pdev)->procs.fill_linear_color_trapezoid)(pdev, &fa,
                            &le->start, &le->end, &re->start, &re->end,
                            fc[0], fc[1], ((void*)0), ((void*)0));
            if (code == 1) {
...
{
# 1795 "./base/gxshade6.c"
    fixed x12 = (((pole[1 * pole_step].x) >> 1) + ((pole[2 * pole_step].x) >> 1) + (((pole[1 * pole_step].x) | (pole[2 * pole_step].x)) & 1));
    fixed y12 = (((pole[1 * pole_step].y) >> 1) + ((pole[2 * pole_step].y) >> 1) + (((pole[1 * pole_step].y) | (pole[2 * pole_step].y)) & 1));


    q0[1 * pole_step].x = (((pole[0 * pole_step].x) >> 1) + ((pole[1 * pole_step].x) >> 1) + (((pole[0 * pole_step].x) | (pole[1 * pole_step].x)) & 1));
    q0[1 * pole_step].y = (((pole[0 * pole_step].y) >> 1) + ((pole[1 * pole_step].y) >> 1) + (((pole[0 * pole_step].y) | (pole[1 * pole_step].y)) & 1));
    q1[2 * pole_step].x = (((pole[2 * pole_step].x) >> 1) + ((pole[3 * pole_step].x) >> 1) + (((pole[2 * pole_step].x) | (pole[3 * pole_step].x)) & 1));
    q1[2 * pole_step].y = (((pole[2 * pole_step].y) >> 1) + ((pole[3 * pole_step].y) >> 1) + (((pole[2 * pole_step].y) | (pole[3 * pole_step].y)) & 1));
    q0[2 * pole_step].x = (((q0[1 * pole_step].x) >> 1) + ((x12) >> 1) + (((q0[1 * pole_step].x) | (x12)) & 1));
    q0[2 * pole_step].y = (((q0[1 * pole_step].y) >> 1) + ((y12) >> 1) + (((q0[1 * pole_step].y) | (y12)) & 1));
    q1[1 * pole_step].x = (((x12) >> 1) + ((q1[2 * pole_step].x) >> 1) + (((x12) | (q1[2 * pole_step].x)) & 1));
    q1[1 * pole_step].y = (((y12) >> 1) + ((q1[2 * pole_step].y) >> 1) + (((y12) | (q1[2 * pole_step].y)) & 1));
    q0[0 * pole_step].x = pole[0 * pole_step].x;
    q0[0 * pole_step].y = pole[0 * pole_step].y;
    q0[3 * pole_step].x = q1[0 * pole_step].x = (((q0[2 * pole_step].x) >> 1) + ((q1[1 * pole_step].x) >> 1) + (((q0[2 * pole_step].x) | (q1[1 * pole_step].x)) & 1));
    q0[3 * pole_step].y = q1[0 * pole_step].y = (((q0[2 * pole_step].y) >> 1) + ((q1[1 * pole_step].y) >> 1) + (((q0[2 * pole_step].y) | (q1[1 * pole_step].y)) & 1));
...
{
# 1889 "./base/gxshade6.c"
}

static __inline__ fixed
manhattan_dist(const gs_fixed_point *p0, const gs_fixed_point *p1)
{
    fixed dx = ((p1->x - p0->x) < 0 ? -(p1->x - p0->x) : (p1->x - p0->x)), dy = ((p1->y - p0->y) < 0 ? -(p1->y - p0->y) : (p1->y - p0->y));
                              ...
                              const gs_fixed_point *p, wedge_vertex_list_elem_t **r)
{
    ...
    e->prev = l->beg;
    e->p = *p;
    e->level = (((l->beg->level) > (l->end->level)) ? (l->beg->level) : (l->end->level)) + 1;
    e->divide_count = 0;
    ...
    if (!l->last_side) {
        ...
        if (l->beg->next == l->end) {
            ...
            e = wedge_vertex_list_find(l->beg, l->end,
                        (((l->beg->level) > (l->end->level)) ? (l->beg->level) : (l->end->level)) + 1);
            if (e == ((void*)0))
                return ((-28));
...
{ int code;
    ...
    fixed dx = ((q0->p.x - q1->p.x) < 0 ? -(q0->p.x - q1->p.x) : (q0->p.x - q1->p.x)), dy = ((q0->p.y - q1->p.y) < 0 ? -(q0->p.y - q1->p.y) : (q0->p.y - q1->p.y));
    bool swap_axes;
    ...
    if (p0->y < p1->y) {
        code = fill_wedge_trap(pfs, p0, p2, p0, p1, q0->c, q2->c, swap_axes, ((bool)0));
        if (code < 0)
            return code;
        return fill_wedge_trap(pfs, p2, p1, p0, p1, q2->c, q1->c, swap_axes, ((bool)0));
    } else {
        code = fill_wedge_trap(pfs, p0, p2, p1, p0, q0->c, q2->c, swap_axes, ((bool)0));
        if (code < 0)
            return code;
        return fill_wedge_trap(pfs, p2, p1, p1, p0, q2->c, q1->c, swap_axes, ((bool)0));
    }
}
        ...
        const shading_vertex_t *p2)
{
    ...
    if (!wedge) {
        ...
        if (cs != ((void*)0)) {
            ...
            s01 = (((s0) > (s1)) ? (s0) : (s1));
            s012 = (((s01) > (s2)) ? (s01) : (s2));
            code = (*(cs)->type->is_linear)(cs, pfs->pis, pfs->trans_device, &p0->c->cc, &p1->c->cc, &p2->c->cc, ((void*)0), pfs->smoothness - s012, pfs->icclink);
    ...
    { gx_device *pdev = pfs->dev;
        ...
        if (code != 0)
            return code;
        draw_triangle(&p0->p, &p1->p, &p2->p, ((unsigned long)(255) * 65536L + (unsigned long)(0) * 256L + (unsigned long)(0)));
        if (!1)
            do { } while (0);
        code = ((pdev)->procs.fill_linear_color_triangle)(pdev, &fa,
                        &p0->p, &p1->p, &p2->p,
                        fc[0], (wedge ? ((void*)0) : fc[1]), fc[2]);
        do { } while (0);
        if (code == 1)
            return 0;
        if (code < 0)
            ...
            const shading_vertex_t *q0, const shading_vertex_t *q1, const shading_vertex_t *q2)
{
    if ((int64_t)(q1->p.x - q0->p.x) * (q2->p.y - q0->p.y) ==
        ...
        return 0;
    draw_triangle(&q0->p, &q1->p, &q2->p, ((unsigned long)(255) * 65536L + (unsigned long)(255) * 256L + (unsigned long)(0)));
            ...
            const patch_color_t *c0, const patch_color_t *c1)
{
    return fill_wedge_from_list_rec(pfs, l->beg, l->end,
                    (((l->beg->level) > (l->end->level)) ? (l->beg->level) : (l->end->level)), c0, c1);
}
            ...
            const patch_color_t *c0, const patch_color_t *c1)
{
    ...
    fixed dx = p1->x - p0->x;
    fixed dy = p1->y - p0->y;
    bool swap_axes = (((dx) < 0 ? -(dx) : (dx)) > ((dy) < 0 ? -(dy) : (dy)));
    gs_fixed_edge le, re;
    const fixed adjust = ((1L << 8) / 2);
        ...
        int wedge_type)
{
    ...
    if (k > 1) {
        ...
        if (((1L << 8) / 2) && (wedge_type & interpatch_padding)) {
            do { } while (0);
...
ordered_triangle(patch_fill_state_t *pfs, gs_fixed_edge *le, gs_fixed_edge *re, patch_color_t *c)
{
    ...
    if (!1)
        do { } while (0);
    code = patch_color_to_device_color_inline(pfs, c, &dc, ((void*)0));
    if (code < 0)
        return code;
    if (le->end.y < re->end.y) {
        code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
            le, re, le->start.y, le->end.y, ((bool)0), &dc, pfs->pis->log_op);
        if (code >= 0) {
            ue.start = le->end;
            ue.end = re->end;
            code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
                &ue, re, le->end.y, re->end.y, ((bool)0), &dc, pfs->pis->log_op);
        }
    } else if (le->end.y > re->end.y) {
        code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
            le, re, le->start.y, re->end.y, ((bool)0), &dc, pfs->pis->log_op);
        if (code >= 0) {
            ue.start = re->end;
            ue.end = le->end;
            code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
                le, &ue, re->end.y, le->end.y, ((bool)0), &dc, pfs->pis->log_op);
        }
    } else
        code = ((pfs->dev)->procs.fill_trapezoid)(pfs->dev,
            le, re, le->start.y, le->end.y, ((bool)0), &dc, pfs->pis->log_op);
        ...
        patch_color_t *c[3])
{
    ...
    fixed ry, ey;
    int code;
    bool swap_axes = ((bool)0);
    gx_device_color dc;
    bool orient;

    draw_quadrangle(p, ((unsigned long)(0) * 65536L + (unsigned long)(255) * 256L + (unsigned long)(0)));
    ...
    code = patch_color_to_device_color_inline(pfs, c[0], &dc, ((void*)0));
    if (code < 0)
        return code;
    { gs_fixed_point qq[4];

        make_vertices(qq, p);
# 2647 "./base/gxshade6.c"
                              ...
                              bool *not_monotonic_by_u, bool *not_monotonic_by_v)
{
    ...
    if (r & 1)
        *not_monotonic_by_u = ((bool)1);
    if (r & 2)
        *not_monotonic_by_v = ((bool)1);
    return !code;
}
...
quadrangle_bbox_covers_pixel_centers(const quadrangle_patch *p)
{
    ...
    xbot = ((((((p->p[0][0]->p.x) < (p->p[0][1]->p.x)) ? (p->p[0][0]->p.x) : (p->p[0][1]->p.x))) < ((((p->p[1][0]->p.x) < (p->p[1][1]->p.x)) ? (p->p[1][0]->p.x) : (p->p[1][1]->p.x)))) ? ((((p->p[0][0]->p.x) < (p->p[0][1]->p.x)) ? (p->p[0][0]->p.x) : (p->p[0][1]->p.x))) : ((((p->p[1][0]->p.x) < (p->p[1][1]->p.x)) ? (p->p[1][0]->p.x) : (p->p[1][1]->p.x))));

    xtop = ((((((p->p[0][0]->p.x) > (p->p[0][1]->p.x)) ? (p->p[0][0]->p.x) : (p->p[0][1]->p.x))) > ((((p->p[1][0]->p.x) > (p->p[1][1]->p.x)) ? (p->p[1][0]->p.x) : (p->p[1][1]->p.x)))) ? ((((p->p[0][0]->p.x) > (p->p[0][1]->p.x)) ? (p->p[0][0]->p.x) : (p->p[0][1]->p.x))) : ((((p->p[1][0]->p.x) > (p->p[1][1]->p.x)) ? (p->p[1][0]->p.x) : (p->p[1][1]->p.x))));

    if (covers_pixel_centers(xbot, xtop))
        return ((bool)1);
    ybot = ((((((p->p[0][0]->p.y) < (p->p[0][1]->p.y)) ? (p->p[0][0]->p.y) : (p->p[0][1]->p.y))) < ((((p->p[1][0]->p.y) < (p->p[1][1]->p.y)) ? (p->p[1][0]->p.y) : (p->p[1][1]->p.y)))) ? ((((p->p[0][0]->p.y) < (p->p[0][1]->p.y)) ? (p->p[0][0]->p.y) : (p->p[0][1]->p.y))) : ((((p->p[1][0]->p.y) < (p->p[1][1]->p.y)) ? (p->p[1][0]->p.y) : (p->p[1][1]->p.y))));

    ytop = ((((((p->p[0][0]->p.y) > (p->p[0][1]->p.y)) ? (p->p[0][0]->p.y) : (p->p[0][1]->p.y))) > ((((p->p[1][0]->p.y) > (p->p[1][1]->p.y)) ? (p->p[1][0]->p.y) : (p->p[1][1]->p.y)))) ? ((((p->p[0][0]->p.y) > (p->p[0][1]->p.y)) ? (p->p[0][0]->p.y) : (p->p[0][1]->p.y))) : ((((p->p[1][0]->p.y) > (p->p[1][1]->p.y)) ? (p->p[1][0]->p.y) : (p->p[1][1]->p.y))));
        ...
        double cd, fixed sd)
{
    ...
    int code = 0;
    byte *color_stack_ptr;
    const bool inside = pfs->inside;

    if (!inside) {
        bbox_of_points(&r, &p0->p, &p1->p, &p2->p, ((void*)0));
        r1 = r;
        do { if ((pfs->rect).p.x > (r).p.x) (r).p.x = (pfs->rect).p.x; if ((pfs->rect).q.x < (r).q.x) (r).q.x = (pfs->rect).q.x; if ((pfs->rect).p.y > (r).p.y) (r).p.y = (pfs->rect).p.y; if ((pfs->rect).q.y < (r).q.y) (r).q.y = (pfs->rect).q.y; } while (0);
        if (r.q.x <= r.p.x || r.q.y <= r.p.y)
            return 0;
    ...
    p12.c = c[1];
    p20.c = c[2];
    code = try_device_linear_color(pfs, ((bool)0), p0, p1, p2);
    switch(code) {
        case 0:
            goto out;
        case 2:
            ...
            if (pfs->Function != ((void*)0)) {
                ...
                if (d01 <= pfs->smoothness / 1 &&
                    d12 <= pfs->smoothness / 1 &&
                    d20 <= pfs->smoothness / 1) {
                    code = constant_color_triangle(pfs, p2, p0, p1);
                    goto out;
                }
            } else if (cd <= pfs->smoothness / 1) {
                code = constant_color_triangle(pfs, p2, p0, p1);
                goto out;
            }
            break;
    ...
    if (1) {
        init_wedge_vertex_list(L, (((int)(sizeof(L))) / ((int)(sizeof((L)[0])))));
        code = make_wedge_median(pfs, &L01, l01, ((bool)1), &p0->p, &p1->p, &p01.p);
        if (code >= 0)
            code = make_wedge_median(pfs, &L12, l12, ((bool)1), &p1->p, &p2->p, &p12.p);
        if (code >= 0)
            code = make_wedge_median(pfs, &L20, l20, ((bool)0), &p2->p, &p0->p, &p20.p);
    } else {
    ...
    if (code >= 0) {
        if (1) {
            move_wedge(&L01, l01, ((bool)1));
            move_wedge(&L20, l20, ((bool)0));
        }
        code = triangle_by_4(pfs, p1, &p12, &p01, &L12, &L[1], &L01, cd / 2, sd / 2);
    }
    if (code >= 0) {
        if (1)
            move_wedge(&L12, l12, ((bool)1));
        code = triangle_by_4(pfs, p2, &p20, &p12, &L20, &L[2], &L12, cd / 2, sd / 2);
    }
    if (code >= 0) {
        L[0].last_side = L[1].last_side = L[2].last_side = ((bool)1);
...
fill_triangle(patch_fill_state_t *pfs,
        ...
        wedge_vertex_list_t *l01, wedge_vertex_list_t *l12, wedge_vertex_list_t *l20)
{
    fixed sd01 = (((((p1->p.x - p0->p.x) < 0 ? -(p1->p.x - p0->p.x) : (p1->p.x - p0->p.x))) > (((p1->p.y - p0->p.y) < 0 ? -(p1->p.y - p0->p.y) : (p1->p.y - p0->p.y)))) ? (((p1->p.x - p0->p.x) < 0 ? -(p1->p.x - p0->p.x) : (p1->p.x - p0->p.x))) : (((p1->p.y - p0->p.y) < 0 ? -(p1->p.y - p0->p.y) : (p1->p.y - p0->p.y))));
    fixed sd12 = (((((p2->p.x - p1->p.x) < 0 ? -(p2->p.x - p1->p.x) : (p2->p.x - p1->p.x))) > (((p2->p.y - p1->p.y) < 0 ? -(p2->p.y - p1->p.y) : (p2->p.y - p1->p.y)))) ? (((p2->p.x - p1->p.x) < 0 ? -(p2->p.x - p1->p.x) : (p2->p.x - p1->p.x))) : (((p2->p.y - p1->p.y) < 0 ? -(p2->p.y - p1->p.y) : (p2->p.y - p1->p.y))));
    fixed sd20 = (((((p0->p.x - p2->p.x) < 0 ? -(p0->p.x - p2->p.x) : (p0->p.x - p2->p.x))) > (((p0->p.y - p2->p.y) < 0 ? -(p0->p.y - p2->p.y) : (p0->p.y - p2->p.y)))) ? (((p0->p.x - p2->p.x) < 0 ? -(p0->p.x - p2->p.x) : (p0->p.x - p2->p.x))) : (((p0->p.y - p2->p.y) < 0 ? -(p0->p.y - p2->p.y) : (p0->p.y - p2->p.y))));
    fixed sd1 = (((sd01) > (sd12)) ? (sd01) : (sd12));
    fixed sd = (((sd1) > (sd20)) ? (sd1) : (sd20));
    double cd = 0;
...
gx_init_patch_fill_state_for_clist(gx_device *dev, patch_fill_state_t *pfs, gs_memory_t *memory)
{
    ...
    pfs->vectorization = ((bool)0);
    pfs->n_color_args = 1;
    pfs->max_small_coord = 0;
    pfs->wedge_vertex_list_elem_buffer = ((void*)0);
    pfs->free_wedge_vertex = ((void*)0);
    pfs->wedge_vertex_list_elem_count = 0;
    pfs->wedge_vertex_list_elem_count_max = 0;
    for (i = 0; i < pfs->num_components; i++)
    ...
    pfs->fixed_flat = 0;
    pfs->smoothness = 0;
    pfs->maybe_self_intersecting = ((bool)0);
    pfs->monotonic_color = ((bool)1);
    pfs->linear_color = ((bool)1);
    pfs->unlinear = ((bool)0);
    pfs->inside = ((bool)0);
    ...
    pfs->color_stack_limit = ((void*)0);
    pfs->pcic = ((void*)0);
    pfs->trans_device = ((void*)0);
    pfs->icclink = ((void*)0);
    return alloc_patch_fill_memory(pfs, memory, ((void*)0));
}
        ...
        const frac31 *c0, const frac31 *c1, const frac31 *c2)
{
    ...
    p[1].c = &c[1];
    p[2].c = &c[2];
    c[0].t[0] = c[0].t[1] = c[1].t[0] = c[1].t[1] = c[2].t[0] = c[2].t[1] = 0;
...
mesh_triangle(patch_fill_state_t *pfs,
        const shading_vertex_t *p0, const shading_vertex_t *p1, const shading_vertex_t *p2)
{
    if ((*((pfs->dev)->procs.dev_spec_op))(pfs->dev,
            gxdso_pattern_shading_area, ((void*)0), 0) > 0) {
        ...
        (void)gx_path_init_local_shared(&path, ((void*)0), pdev->memory);
        code = gx_path_add_point(&path, p0->p.x, p0->p.y);
        if (code >= 0 && s1 >= 0)
            code = gx_path_add_line_notes(&path, p1->p.x, p1->p.y, sn_none);
        if (code >= 0)
            code = gx_path_add_line_notes(&path, p2->p.x, p2->p.y, sn_none);
        if (code >= 0 && s1 < 0)
            code = gx_path_add_line_notes(&path, p1->p.x, p1->p.y, sn_none);
        if (code >= 0)
            code = gx_path_close_subpath_notes(&path, sn_none);
        if (code >= 0)
            code = (*((pfs->dev)->procs.fill_path))(pdev, ((void*)0), &path, ((void*)0), ((void*)0), ((void*)0));
...
static __inline__ color_change_type_t
                        ...
                        bool *divide_u, bool *divide_v)
{
    ...
    Du = (((D0001) > (D1011)) ? (D0001) : (D1011));
    Dv = (((D0010) > (D0111)) ? (D0010) : (D0111));
...
fill_quadrangle(patch_fill_state_t *pfs, const quadrangle_patch *p, bool big)
{
    ...
    int code;
    bool divide_u = ((bool)0), divide_v = ((bool)0), big1 = big;
    ...
    if (big) {
        ...
        fixed size_u = ((((((((p->p[0][0]->p.x - p->p[0][1]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[0][1]->p.x) : (p->p[0][0]->p.x - p->p[0][1]->p.x))) > (((p->p[1][0]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[1][0]->p.x - p->p[1][1]->p.x) : (p->p[1][0]->p.x - p->p[1][1]->p.x)))) ? (((p->p[0][0]->p.x - p->p[0][1]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[0][1]->p.x) : (p->p[0][0]->p.x - p->p[0][1]->p.x))) : (((p->p[1][0]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[1][0]->p.x - p->p[1][1]->p.x) : (p->p[1][0]->p.x - p->p[1][1]->p.x))))) > ((((((p->p[0][0]->p.y - p->p[0][1]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[0][1]->p.y) : (p->p[0][0]->p.y - p->p[0][1]->p.y))) > (((p->p[1][0]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[1][0]->p.y - p->p[1][1]->p.y) : (p->p[1][0]->p.y - p->p[1][1]->p.y)))) ? (((p->p[0][0]->p.y - p->p[0][1]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[0][1]->p.y) : (p->p[0][0]->p.y - p->p[0][1]->p.y))) : (((p->p[1][0]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[1][0]->p.y - p->p[1][1]->p.y) : (p->p[1][0]->p.y - p->p[1][1]->p.y)))))) ? ((((((p->p[0][0]->p.x - p->p[0][1]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[0][1]->p.x) : (p->p[0][0]->p.x - p->p[0][1]->p.x))) > (((p->p[1][0]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[1][0]->p.x - p->p[1][1]->p.x) : (p->p[1][0]->p.x - p->p[1][1]->p.x)))) ? (((p->p[0][0]->p.x - p->p[0][1]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[0][1]->p.x) : (p->p[0][0]->p.x - p->p[0][1]->p.x))) : (((p->p[1][0]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[1][0]->p.x - p->p[1][1]->p.x) : (p->p[1][0]->p.x - p->p[1][1]->p.x))))) : ((((((p->p[0][0]->p.y - p->p[0][1]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[0][1]->p.y) : (p->p[0][0]->p.y - p->p[0][1]->p.y))) > (((p->p[1][0]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[1][0]->p.y - p->p[1][1]->p.y) : (p->p[1][0]->p.y - p->p[1][1]->p.y)))) ? (((p->p[0][0]->p.y - p->p[0][1]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[0][1]->p.y) : (p->p[0][0]->p.y - p->p[0][1]->p.y))) : (((p->p[1][0]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[1][0]->p.y - p->p[1][1]->p.y) : (p->p[1][0]->p.y - p->p[1][1]->p.y))))));



        fixed size_v = ((((((((p->p[0][0]->p.x - p->p[1][0]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[1][0]->p.x) : (p->p[0][0]->p.x - p->p[1][0]->p.x))) > (((p->p[0][1]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[0][1]->p.x - p->p[1][1]->p.x) : (p->p[0][1]->p.x - p->p[1][1]->p.x)))) ? (((p->p[0][0]->p.x - p->p[1][0]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[1][0]->p.x) : (p->p[0][0]->p.x - p->p[1][0]->p.x))) : (((p->p[0][1]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[0][1]->p.x - p->p[1][1]->p.x) : (p->p[0][1]->p.x - p->p[1][1]->p.x))))) > ((((((p->p[0][0]->p.y - p->p[1][0]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[1][0]->p.y) : (p->p[0][0]->p.y - p->p[1][0]->p.y))) > (((p->p[0][1]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[0][1]->p.y - p->p[1][1]->p.y) : (p->p[0][1]->p.y - p->p[1][1]->p.y)))) ? (((p->p[0][0]->p.y - p->p[1][0]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[1][0]->p.y) : (p->p[0][0]->p.y - p->p[1][0]->p.y))) : (((p->p[0][1]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[0][1]->p.y - p->p[1][1]->p.y) : (p->p[0][1]->p.y - p->p[1][1]->p.y)))))) ? ((((((p->p[0][0]->p.x - p->p[1][0]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[1][0]->p.x) : (p->p[0][0]->p.x - p->p[1][0]->p.x))) > (((p->p[0][1]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[0][1]->p.x - p->p[1][1]->p.x) : (p->p[0][1]->p.x - p->p[1][1]->p.x)))) ? (((p->p[0][0]->p.x - p->p[1][0]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[1][0]->p.x) : (p->p[0][0]->p.x - p->p[1][0]->p.x))) : (((p->p[0][1]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[0][1]->p.x - p->p[1][1]->p.x) : (p->p[0][1]->p.x - p->p[1][1]->p.x))))) : ((((((p->p[0][0]->p.y - p->p[1][0]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[1][0]->p.y) : (p->p[0][0]->p.y - p->p[1][0]->p.y))) > (((p->p[0][1]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[0][1]->p.y - p->p[1][1]->p.y) : (p->p[0][1]->p.y - p->p[1][1]->p.y)))) ? (((p->p[0][0]->p.y - p->p[1][0]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[1][0]->p.y) : (p->p[0][0]->p.y - p->p[1][0]->p.y))) : (((p->p[0][1]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[0][1]->p.y - p->p[1][1]->p.y) : (p->p[0][1]->p.y - p->p[1][1]->p.y))))));




        if (0 && pfs->maybe_self_intersecting) {
    ...
    if (!big1) {
        bool is_big_u = ((bool)0), is_big_v = ((bool)0);
        double d0001x = ((p->p[0][0]->p.x - p->p[0][1]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[0][1]->p.x) : (p->p[0][0]->p.x - p->p[0][1]->p.x));
        double d1011x = ((p->p[1][0]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[1][0]->p.x - p->p[1][1]->p.x) : (p->p[1][0]->p.x - p->p[1][1]->p.x));
        double d0001y = ((p->p[0][0]->p.y - p->p[0][1]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[0][1]->p.y) : (p->p[0][0]->p.y - p->p[0][1]->p.y));
        double d1011y = ((p->p[1][0]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[1][0]->p.y - p->p[1][1]->p.y) : (p->p[1][0]->p.y - p->p[1][1]->p.y));
        double d0010x = ((p->p[0][0]->p.x - p->p[1][0]->p.x) < 0 ? -(p->p[0][0]->p.x - p->p[1][0]->p.x) : (p->p[0][0]->p.x - p->p[1][0]->p.x));
        double d0111x = ((p->p[0][1]->p.x - p->p[1][1]->p.x) < 0 ? -(p->p[0][1]->p.x - p->p[1][1]->p.x) : (p->p[0][1]->p.x - p->p[1][1]->p.x));
        double d0010y = ((p->p[0][0]->p.y - p->p[1][0]->p.y) < 0 ? -(p->p[0][0]->p.y - p->p[1][0]->p.y) : (p->p[0][0]->p.y - p->p[1][0]->p.y));
        double d0111y = ((p->p[0][1]->p.y - p->p[1][1]->p.y) < 0 ? -(p->p[0][1]->p.y - p->p[1][1]->p.y) : (p->p[0][1]->p.y - p->p[1][1]->p.y));
        double size_u = ((((((d0001x) > (d1011x)) ? (d0001x) : (d1011x))) > ((((d0001y) > (d1011y)) ? (d0001y) : (d1011y)))) ? ((((d0001x) > (d1011x)) ? (d0001x) : (d1011x))) : ((((d0001y) > (d1011y)) ? (d0001y) : (d1011y))));
        double size_v = ((((((d0010x) > (d0111x)) ? (d0010x) : (d0111x))) > ((((d0010y) > (d0111y)) ? (d0010y) : (d0111y)))) ? ((((d0010x) > (d0111x)) ? (d0010x) : (d0111x))) : ((((d0010y) > (d0111y)) ? (d0010y) : (d0111y))));
        ...
        if (size_v > pfs->decomposition_limit)
            ...
            return (0 || !pfs->maybe_self_intersecting ?
                        constant_color_quadrangle : triangles4)(pfs, p,
                            pfs->maybe_self_intersecting);
        if (!pfs->monotonic_color) {
            bool not_monotonic_by_u = ((bool)0), not_monotonic_by_v = ((bool)0);
        ...
        if (!pfs->linear_color) {
            ...
            case color_change_small:
                code = (0 || !pfs->maybe_self_intersecting ?
                            constant_color_quadrangle : triangles4)(pfs, p,
                                pfs->maybe_self_intersecting);
                pfs->monotonic_color = monotonic_color_save;
                pfs->linear_color = linear_color_save;
    ...
    if (divide_v) {
        ...
        if (1) {
            code = make_wedge_median(pfs, &l1, p->l0111, ((bool)1), &p->p[0][1]->p, &p->p[1][1]->p, &s0.p[1][1]->p);
            if (code >= 0)
                code = make_wedge_median(pfs, &l2, p->l1000, ((bool)0), &p->p[1][0]->p, &p->p[0][0]->p, &s0.p[1][0]->p);
            if (code >= 0) {
        ...
        if (code >= 0) {
            if (1) {
                l0.last_side = ((bool)1);
                move_wedge(&l1, p->l0111, ((bool)1));
                move_wedge(&l2, p->l1000, ((bool)0));
            }
        ...
        if (1) {
            code = make_wedge_median(pfs, &l1, p->l0001, ((bool)1), &p->p[0][0]->p, &p->p[0][1]->p, &s0.p[0][1]->p);
            if (code >= 0)
                code = make_wedge_median(pfs, &l2, p->l1110, ((bool)0), &p->p[1][1]->p, &p->p[1][0]->p, &s0.p[1][1]->p);
            if (code >= 0) {
        ...
        if (code >= 0) {
            if (1) {
                l0.last_side = ((bool)1);
                move_wedge(&l1, p->l0001, ((bool)1));
                move_wedge(&l2, p->l1110, ((bool)0));
            }
            code = fill_quadrangle(pfs, &s1, big1);
        }
        if (1) {
...
decompose_stripe(patch_fill_state_t *pfs, const tensor_patch *p, int ku)
{
    if (ku > 1) {
        ...
        if (0) {
            draw_patch(&s0, ((bool)1), ((unsigned long)(0) * 65536L + (unsigned long)(128) * 256L + (unsigned long)(128)));
            draw_patch(&s1, ((bool)1), ((unsigned long)(0) * 65536L + (unsigned long)(128) * 256L + (unsigned long)(128)));
        }
        ...
        code = fill_quadrangle(pfs, &q, ((bool)1));
        if (code < 0)
            return code;
        if (1) {
...
fill_stripe(patch_fill_state_t *pfs, const tensor_patch *p)
{
    ...
    if (0)
        draw_patch(p, ((bool)1), ((unsigned long)(0) * 65536L + (unsigned long)(255) * 256L + (unsigned long)(255)));
...
static __inline__ bool neqs(int *a, int b)
{
    if (*a * b < 0)
        return ((bool)1);
    if (!*a)
...
{
    fixed xmin0 = (((pole[0 * pole_step].x) < (pole[1 * pole_step].x)) ? (pole[0 * pole_step].x) : (pole[1 * pole_step].x));
    fixed xmin1 = (((pole[2 * pole_step].x) < (pole[3 * pole_step].x)) ? (pole[2 * pole_step].x) : (pole[3 * pole_step].x));
    fixed xmin = (((xmin0) < (xmin1)) ? (xmin0) : (xmin1));
    fixed xmax0 = (((pole[0 * pole_step].x) > (pole[1 * pole_step].x)) ? (pole[0 * pole_step].x) : (pole[1 * pole_step].x));
    fixed xmax1 = (((pole[2 * pole_step].x) > (pole[3 * pole_step].x)) ? (pole[2 * pole_step].x) : (pole[3 * pole_step].x));
    fixed xmax = (((xmax0) > (xmax1)) ? (xmax0) : (xmax1));
...
{
    fixed ymin0 = (((pole[0 * pole_step].y) < (pole[1 * pole_step].y)) ? (pole[0 * pole_step].y) : (pole[1 * pole_step].y));
    fixed ymin1 = (((pole[2 * pole_step].y) < (pole[3 * pole_step].y)) ? (pole[2 * pole_step].y) : (pole[3 * pole_step].y));
    fixed ymin = (((ymin0) < (ymin1)) ? (ymin0) : (ymin1));
    fixed ymax0 = (((pole[0 * pole_step].y) > (pole[1 * pole_step].y)) ? (pole[0 * pole_step].y) : (pole[1 * pole_step].y));
    fixed ymax1 = (((pole[2 * pole_step].y) > (pole[3 * pole_step].y)) ? (pole[2 * pole_step].y) : (pole[3 * pole_step].y));
    fixed ymax = (((ymax0) > (ymax1)) ? (ymax0) : (ymax1));
...
fill_patch(patch_fill_state_t *pfs, const tensor_patch *p, int kv, int kv0, int kv1)
{
    ...
    { tensor_patch s0, s1;
        ...
        if(color_stack_ptr == ((void*)0))
            return ((-28));
        if (0)
            draw_patch(p, ((bool)1), ((unsigned long)(255) * 65536L + (unsigned long)(255) * 256L + (unsigned long)(0)));
        ...
        const gx_device_color *pdevc, gs_logical_operation_t log_op)
{
    ...
    le.start.x = rect->p.x - ((1L << 8) / 2);
    le.start.y = rect->p.y - ((1L << 8) / 2);
    le.end.x = rect->p.x - ((1L << 8) / 2);
    le.end.y = rect->q.y + ((1L << 8) / 2);
    re.start.x = rect->q.x + ((1L << 8) / 2);
    re.start.y = rect->p.y - ((1L << 8) / 2);
    re.end.x = rect->q.x + ((1L << 8) / 2);
    re.end.y = rect->q.y + ((1L << 8) / 2);
    return ((pdev)->procs.fill_trapezoid)(pdev,
            &le, &re, le.start.y, le.end.y, ((bool)0), pdevc, log_op);
}
                              ...
                              const gs_fixed_point[4], double, double))
{
    ...
    int code = 0;
    byte *color_stack_ptr = reserve_colors_inline(pfs, c, 4);
        ...
        if (is_x_bended(&p) || is_y_bended(&p)) {
            ...
            for (i = k = 0; k < 4 && code >= 0; i = j, k++) {
                ...
                if (curve[jj].straight)
                    code = gx_path_add_line_notes(&path, curve[j].vertex.p.x, curve[j].vertex.p.y, sn_none);

                else
                    code = gx_path_add_curve_notes(&path, curve[jj].control[l].x, curve[jj].control[l].y, curve[jj].control[(l + 1) & 1].x, curve[jj].control[(l + 1) & 1].y, curve[j].vertex.p.x, curve[j].vertex.p.y, sn_none);
    ...
    kvm = ((((((kv[0]) > (kv[1])) ? (kv[0]) : (kv[1]))) > ((((kv[2]) > (kv[3])) ? (kv[2]) : (kv[3])))) ? ((((kv[0]) > (kv[1])) ? (kv[0]) : (kv[1]))) : ((((kv[2]) > (kv[3])) ? (kv[2]) : (kv[3]))));
    ku[0] = curve_samples(pfs, p.pole[0], 1, pfs->fixed_flat);
    ku[1] = curve_samples(pfs, p.pole[1], 1, pfs->fixed_flat);
    ku[2] = curve_samples(pfs, p.pole[2], 1, pfs->fixed_flat);
    ku[3] = curve_samples(pfs, p.pole[3], 1, pfs->fixed_flat);
    kum = ((((((ku[0]) > (ku[1])) ? (ku[0]) : (ku[1]))) > ((((ku[2]) > (ku[3])) ? (ku[2]) : (ku[3])))) ? ((((ku[0]) > (ku[1])) ? (ku[0]) : (ku[1]))) : ((((ku[2]) > (ku[3])) ? (ku[2]) : (ku[3]))));
