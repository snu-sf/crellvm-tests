...
static gs_ptr_type_t cs_Separation_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void cs_Separation_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_color_space_Separation = { sizeof(gs_color_space), "gs_color_space_Separation", 0, 0, cs_Separation_enum_ptrs, cs_Separation_reloc_ptrs, 0, 0 };




static void gx_init_Separation(gs_client_color *, const gs_color_space *);
static const gs_color_space *gx_concrete_space_Separation(const gs_color_space *, const gs_imager_state *);
static int gx_concretize_Separation(const gs_client_color *, const gs_color_space *, frac *, const gs_imager_state *, gx_device *);
static int gx_remap_concrete_Separation(const frac *, const gs_color_space * pcs, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static int gx_remap_Separation(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static int gx_install_Separation(gs_color_space *, gs_state *);
static int gx_set_overprint_Separation(const gs_color_space *, gs_state *);
static void gx_final_Separation(const gs_color_space *);
static int gx_serialize_Separation(const gs_color_space *, stream *);
const gs_color_space_type gs_color_space_type_Separation = {
    gs_color_space_index_Separation, ((bool)1), ((bool)0),
...
static
gs_ptr_type_t cs_Separation_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { switch ( index ) { default: return 0;
    case 0: return (pep->ptr = (const void *)(((const gs_color_space *)vptr)->params.separation.map), (&ptr_struct_procs));
    case 1 : return (pep->size = (uint)(((const gs_color_space *)vptr)->params.separation.sep_name), (&ptr_name_index_procs));
} }
static void cs_Separation_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) {
{
    (((gs_color_space *)vptr)->params.separation.map = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_color_space *)vptr)->params.separation.map), gcst)));
}
}
...
gx_install_Separation(gs_color_space * pcs, gs_state * pgs)
{
    ...
    if (code < 0)
       return code;
    ((pgs)->color[0].color_space)->params.separation.use_alt_cspace =
        using_alt_color_space(pgs);
    if (((pgs)->color[0].color_space)->params.separation.use_alt_cspace) {
...
{
    do { if (pcs->params.separation.map) { do { do { } while (0); (pcs->params.separation.map)->rc.ref_count += (-1); } while (0); if (!(pcs->params.separation.map)->rc.ref_count) { do { do { } while (0); (pcs->params.separation.map)->rc.free((pcs->params.separation.map)->rc.memory, (void *)(pcs->params.separation.map), "gx_adjust_Separation"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
...
)
{
    ...
    if (pcs == ((void*)0))
        return ((-25));
    pcs->params.separation.map = ((void*)0);

    code = alloc_device_n_map(&pcs->params.separation.map, pmem,
                              "gs_cspace_build_Separation");
    if (code < 0) {
        ((pmem)->procs.free_object(pmem, pcs, "gs_cspace_build_Separation"));
        return (code);
    }
    ...
    if (pcs->params.separation.sep_type == SEP_OTHER &&
        ...
        if (pis->icc_manager->device_named != ((void*)0)) {
            ...
            code = gsicc_transform_named_color(pc->paint.values, &named_color,
                                               1, device_values, pis, dev, ((void*)0),
                                               &rendering_params);
            if (code == 0) {
        ...
        return (*(pacs)->type->concretize_color)(&cc, pacs, pconc, pis, dev);
    }
    else {
...
# 417 "./base/gscsepr.c"
    if (pis->color_component_map.use_alt_cspace) {
        ...
        ((pis)->cmap_procs->map_separation)(pconc[0], pdc, pis, dev, select);
        return 0;
    }
}
...
check_Separation_component_name(const gs_color_space * pcs, gs_state * pgs)
{
    ...
    if (pcs->params.separation.sep_type != SEP_OTHER) {
        pcolor_component_map->use_alt_cspace = ((bool)0);
        return 0;
    }
    ...
    if (colorant_number >= 0 && colorant_number < dev->color_info.max_components) {
