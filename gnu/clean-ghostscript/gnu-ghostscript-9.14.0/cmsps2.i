# 1 "./lcms2/src/cmsps2.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./lcms2/src/cmsps2.c" 2
# 27 "./lcms2/src/cmsps2.c"
...
# 28 "./lcms2/src/cmsps2.c" 2
# 268 "./lcms2/src/cmsps2.c"
typedef struct {
...
char* RemoveCR(const char* txt)
{
    ...
    __builtin_strncpy (Buffer, txt, 2047);
    Buffer[2047] = 0;
    for (pt = Buffer; *pt; pt++)
            if (*pt == '\n' || *pt == '\r') *pt = ' ';
...
void EmitHeader(cmsIOHANDLER* m, const char* Title, cmsHPROFILE hProfile)
{
    ...
    if (Description != ((void*)0)) cmsMLUgetASCII(Description, "\0\0", "\0\0", DescASCII, 255);
    if (Copyright != ((void*)0)) cmsMLUgetASCII(Copyright, "\0\0", "\0\0", CopyrightASCII, 255);
...
void EmitIntent(cmsIOHANDLER* m, int RenderingIntent)
{
    ...
    switch (RenderingIntent) {
        ...
        case 0: intent = "Perceptual"; break;
        case 1: intent = "RelativeColorimetric"; break;
        case 3: intent = "AbsoluteColorimetric"; break;
        case 2: intent = "Saturation"; break;
                                             ...
                                             cmsColorSpaceSignature ColorSpace)
{
    ...
    cmsStageSampleCLut16bit(mpe, OutputValueSampler, (void*) &sc, 0x01000000);
...
int EmitCIEBasedDEF(cmsIOHANDLER* m, cmsPipeline* Pipeline, int Intent, cmsCIEXYZ* BlackPoint)
{
    ...
    if (cmsStageType(mpe) == cmsSigCLutElemType) {
            ...
            WriteCLUT(m, mpe, PreMaj, PostMaj, PreMin, PostMin, 0, (cmsColorSpaceSignature) 0);
            _cmsIOPrintf(m, "]\n");
    }
    ...
    cmsToneCurve* ExtractGray2Y(cmsContext ContextID, cmsHPROFILE hProfile, int Intent)
{
    ...
    cmsHTRANSFORM xform = cmsCreateTransformTHR(ContextID, hProfile, (((3) << 16)|((1) << 3)|(1)), hXYZ, (((1) << 22)|((9) << 16)|((3) << 3)|(0)), Intent, 0x0100);
    int i;
...
int WriteInputLUT(cmsIOHANDLER* m, cmsHPROFILE hProfile, int Intent, cmsUInt32Number dwFlags)
{
    ...
    InputFormat = cmsFormatterForColorspaceOfProfile(hProfile, 2, 0);
    nChannels = (((InputFormat)>>3)&15);


    cmsDetectBlackPoint(&BlackPointAdaptedToD50, hProfile, Intent, 0);


    hLab = cmsCreateLab4ProfileTHR(m ->ContextID, ((void*)0));

    Profiles[0] = hProfile;
    Profiles[1] = hLab;

    xform = cmsCreateMultiprofileTransform(Profiles, 2, InputFormat, (((1) << 22)|((10) << 16)|((3) << 3)|(0)), Intent, 0);
    cmsCloseProfile(hLab);

    if (xform == ((void*)0)) {

        cmsSignalError(m ->ContextID, 9, "Cannot create transform Profile -> Lab");
        return 0;
    }
    ...
    case 3:
    case 4: {
            cmsUInt32Number OutFrm = (((10) << 16)|((3) << 3)|(2));
    ...
    default:
        ...
        cmsSignalError(m ->ContextID, 9, "Only 3, 4 channels supported for CSA. This profile has %d channels.", nChannels);
        return 0;
    }
...
int WriteInputMatrixShaper(cmsIOHANDLER* m, cmsHPROFILE hProfile, cmsStage* Matrix, cmsStage* Shaper)
{
    ...
    cmsDetectBlackPoint(&BlackPointAdaptedToD50, hProfile, 1, 0);
    ...
    if (ColorSpace == cmsSigGrayData) {
        ...
        if (ColorSpace == cmsSigRgbData) {
            ...
            for (i=0; i < 3; i++)
                for (j=0; j < 3; j++)
                    Mat.v[i].n[j] *= (1.0 + 32767.0/32768.0);
            ...
            cmsSignalError(m ->ContextID, 9, "Profile is not suitable for CSA. Unsupported colorspace.");
            return 0;
        }
...
int WriteNamedColorCSA(cmsIOHANDLER* m, cmsHPROFILE hNamedColor, int Intent)
{
    ...
    hLab = cmsCreateLab4ProfileTHR(m ->ContextID, ((void*)0));
    xform = cmsCreateTransform(hNamedColor, (((1) << 3)|(2)), hLab, (((1) << 22)|((10) << 16)|((3) << 3)|(0)), Intent, 0);
    ...
    for (i=0; i < nColors; i++) {
        ...
        if (!cmsNamedColorInfo(NamedColorList, i, ColorName, ((void*)0), ((void*)0), ((void*)0), ((void*)0)))
                continue;
    ...
    if (cmsGetDeviceClass(hProfile) == cmsSigNamedColorClass) {
        ...
        if (ColorSpace != cmsSigXYZData &&
            ColorSpace != cmsSigLabData) {
                ...
                cmsSignalError(ContextID, 9, "Invalid output color space");
                goto Error;
        }
...
int WriteOutputLUT(cmsIOHANDLER* m, cmsHPROFILE hProfile, int Intent, cmsUInt32Number dwFlags)
{
    ...
    cmsCIEXYZ BlackPointAdaptedToD50;
    cmsBool lDoBPC = (dwFlags & 0x2000);
    cmsBool lFixWhite = !(dwFlags & 0x0004);
    cmsUInt32Number InFrm = (((10) << 16)|((3) << 3)|(2));
    ...
    OutputFormat = cmsFormatterForColorspaceOfProfile(hProfile, 2, 0);
    nChannels = (((OutputFormat)>>3)&15);
    ...
    if (RelativeEncodingIntent == 3)
        RelativeEncodingIntent = 1;
    ...
    xform = cmsCreateMultiprofileTransformTHR(m ->ContextID,
                                              Profiles, 2, (((1) << 22)|((10) << 16)|((3) << 3)|(0)),
                                              OutputFormat, RelativeEncodingIntent, 0);
    cmsCloseProfile(hLab);

    if (xform == ((void*)0)) {

        cmsSignalError(m ->ContextID, 9, "Cannot create transform Lab -> Profile in CRD creation");
        return 0;
    }
    ...
    EmitPQRStage(m, hProfile, lDoBPC, Intent == 3);
    EmitXYZ2Lab(m);
# 1353 "./lcms2/src/cmsps2.c"
    if (Intent == 3)
            lFixWhite = 0;
...
int WriteNamedColorCRD(cmsIOHANDLER* m, cmsHPROFILE hNamedColor, int Intent, cmsUInt32Number dwFlags)
{
    ...
    OutputFormat = cmsFormatterForColorspaceOfProfile(hNamedColor, 2, 0);
    nColorant = (((OutputFormat)>>3)&15);


    xform = cmsCreateTransform(hNamedColor, (((1) << 3)|(2)), ((void*)0), OutputFormat, Intent, dwFlags);
    if (xform == ((void*)0)) return 0;
