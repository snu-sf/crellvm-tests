...
int file_close_finish(stream *);
int file_close_disable(stream *);
# 41 "./base/gspcolor.c" 2


static const gc_ptr_element_t pattern_template_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_pattern_template_t *)0)->uid.xvalues - (char *)((gs_pattern_template_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_pattern_template_t *)0)->client_data - (char *)((gs_pattern_template_t *)0) )) } }; static const gc_struct_data_t pattern_template_reloc_ptrs = { (sizeof(pattern_template_enum_ptrs) / sizeof((pattern_template_enum_ptrs)[0])), 0, 0, pattern_template_enum_ptrs }; const gs_memory_struct_type_t st_pattern_template = { sizeof(gs_pattern_template_t), "gs_pattern_template_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pattern_template_reloc_ptrs };
static const gc_ptr_element_t pattern_instance_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_pattern_instance_t *)0)->saved - (char *)((gs_pattern_instance_t *)0) )) } }; static const gc_struct_data_t pattern_instance_reloc_ptrs = { (sizeof(pattern_instance_enum_ptrs) / sizeof((pattern_instance_enum_ptrs)[0])), 0, 0, pattern_instance_enum_ptrs }; const gs_memory_struct_type_t st_pattern_instance = { sizeof(gs_pattern_instance_t), "gs_pattern_instance_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pattern_instance_reloc_ptrs };


static gs_ptr_type_t cs_Pattern_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void cs_Pattern_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_color_space_Pattern = { sizeof(gs_color_space), "gs_color_space_Pattern", 0, 0, cs_Pattern_enum_ptrs, cs_Pattern_reloc_ptrs, 0, 0 };

static int gx_num_components_Pattern(const gs_color_space *);
static int gx_remap_Pattern(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static void gx_init_Pattern(gs_client_color *, const gs_color_space *);
static void gx_restrict_Pattern(gs_client_color *, const gs_color_space *);
static int gx_install_Pattern(gs_color_space *, gs_state *);
static int gx_set_overprint_Pattern(const gs_color_space *, gs_state *);
static void gx_final_Pattern(const gs_color_space *);
static void gx_adjust_color_Pattern(const gs_client_color *, const gs_color_space *, int);
static int gx_serialize_Pattern(const gs_color_space *, stream *);
const gs_color_space_type gs_color_space_type_Pattern = {
    gs_color_space_index_Pattern, ((bool)0), ((bool)0),
    &st_color_space_Pattern, gx_num_components_Pattern,
    gx_init_Pattern, gx_restrict_Pattern,
    gx_no_concrete_space,
    gx_no_concretize_color, ((void*)0),
                       ...
                       const gs_pattern_type_t *type)
{
    ...
    ((&ppat->uid)->id = (~(-1L << ((1 << 3) * 8 - 1))), (&ppat->uid)->xvalues = 0);
    ppat->client_data = 0;
}
                       ...
                       gs_memory_type_ptr_t pstype)
{
    ...
    do { if ( ((pinst) = (gs_pattern_instance_t *)(*(mem)->procs.alloc_struct)(mem, pstype, "gs_make_pattern_common")) == 0 ) { return ((-25)); } else { do { (pinst)->rc.ref_count = 1; (pinst)->rc.memory = mem; (pinst)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
    ...
    if (saved == 0) {
        ((mem)->procs.free_object(mem, pinst, "gs_make_pattern_common"));
        return ((-25));
    }
...
gs_setpatternspace(gs_state * pgs)
{
    ...
    if (pgs->in_cachedevice)
        return ((-21));
    ccs_old = ((pgs)->color[0].color_space);
    if (ccs_old->type->index != gs_color_space_index_Pattern) {
        ...
        pcs->params.pattern.has_base_space = ((bool)1);
...
{
    if (pcc->pattern != 0)
        do { if (pcc->pattern) { do { do { } while (0); (pcc->pattern)->rc.ref_count += (delta); } while (0); if (!(pcc->pattern)->rc.ref_count) { do { do { } while (0); (pcc->pattern)->rc.free((pcc->pattern)->rc.memory, (void *)(pcc->pattern), "gs_pattern_reference"); } while (0); (pcc->pattern) = 0; } else do { } while (0); } } while (0);
}
...
gx_num_components_Pattern(const gs_color_space * pcs)
{
    return
        (pcs->params.pattern.has_base_space ?
         -1 - (*(pcs->base_space)->type->num_components)(pcs->base_space) :
         -1 );
}
...
{
    if (pc->pattern == 0) {
        pdc->ccolor_valid = ((bool)0);
        pdc->ccolor.pattern = 0;
                        ...
                        const gs_color_space * pcs, int delta)
{
    ...
    do { if (pinst) { do { do { } while (0); (pinst)->rc.ref_count += (delta); } while (0); if (!(pinst)->rc.ref_count) { do { do { } while (0); (pinst)->rc.free((pinst)->rc.memory, (void *)(pinst), "gx_adjust_color_Pattern"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
...
static
gs_ptr_type_t cs_Pattern_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst)
{
    return 0;

}

static void cs_Pattern_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) {
{
    return;
}
}
...
gx_serialize_Pattern(const gs_color_space * pcs, stream * s)
{
    ...
    if (!p->has_base_space)
        return 0;
    return (*(pcs->base_space)->type->serialize)(pcs->base_space, s);
}
