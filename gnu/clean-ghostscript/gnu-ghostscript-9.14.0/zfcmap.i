    ...
    if (pcmap->lookup) {
        ...
        for (i = 0; i < pcmap->num_lookup; ++i) {
            ...
            if (pclr->value_type == CODE_VALUE_GLYPH)
                (*(mem)->procs.free_string)(mem, pclr->values.data, pclr->values.size, "free_code_map(values)");

        }
        ((mem)->procs.free_object(mem, pcmap->lookup, "free_code_map(map)"));
    }
}
...
acquire_code_ranges(gs_cmap_adobe1_t *cmap, const ref *pref, gs_memory_t *mem)
{
    ...
    ranges = (gx_code_space_range_t *)
        (*(mem)->procs.alloc_byte_array)(mem, num_ranges, sizeof(gx_code_space_range_t), "acquire_code_ranges");
    ...
    for (i = 0; i < ((pref)->tas.rsize); i++) {
        ...
        if (!((((const byte *)&((&rfirst)->tas.type_attrs))[1]) == (t_string)) ||
            !((((const byte *)&((&rlast)->tas.type_attrs))[1]) == (t_string)) ||
            (size = ((&rfirst)->tas.rsize)) == 0 || size > 4 ||
            ((&rlast)->tas.rsize) != size ||
                 ...
                 gs_memory_t *mem)
{
    ...
    num_lookup /= 5;
    pclr = (gx_cmap_lookup_range_t *)(*(mem)->procs.alloc_struct_array)(mem, num_lookup, &st_cmap_lookup_range_element, "acquire_code_map(lookup ranges)");
    ...
    for (i = 0; i < ((pref)->tas.rsize); i++) {
        ...
        if (!((((const byte *)&((&rprefix)->tas.type_attrs))[1]) == (t_string)) ||
            !((((const byte *)&((&rmisc)->tas.type_attrs))[1]) == (t_string)) ||
            !((((const byte *)&((&rkeys)->tas.type_attrs))[1]) == (t_string)) ||
            !(((((const byte *)&((&rvalues)->tas.type_attrs))[1]) == (t_string)) || (((&rvalues)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))) ||
            !((((const byte *)&((&rfxs)->tas.type_attrs))[1]) == (t_integer))
            )
            return ((-20));
        if (((&rmisc)->tas.rsize) != 4 ||
            rmisc.value.bytes[0] > 4 - ((&rprefix)->tas.rsize) ||
            rmisc.value.bytes[1] > 1 ||
            rmisc.value.bytes[2] > CODE_VALUE_NOTDEF ||
            rmisc.value.bytes[3] == 0)
            return ((-15));
        pclr->cmap = root;
        pclr->key_size = rmisc.value.bytes[0];
        pclr->key_prefix_size = ((&rprefix)->tas.rsize);
        memcpy(pclr->key_prefix, rprefix.value.bytes, pclr->key_prefix_size);
        pclr->key_is_range = rmisc.value.bytes[1];
        if (pclr->key_size == 0) {

            if (((&rkeys)->tas.rsize) != 0)
                return ((-15));
            pclr->num_entries = 1;
        } else {
            int step = pclr->key_size * (pclr->key_is_range ? 2 : 1);

            if (((&rkeys)->tas.rsize) % step != 0)
                return ((-15));
            pclr->num_entries = ((&rkeys)->tas.rsize) / step;
        }
        pclr->keys.data = rkeys.value.bytes,
            pclr->keys.size = ((&rkeys)->tas.rsize);
        pclr->value_type = rmisc.value.bytes[2];
        pclr->value_size = rmisc.value.bytes[3];
        if (((((const byte *)&((&rvalues)->tas.type_attrs))[1]) == (t_string))) {
            if (pclr->value_type == CODE_VALUE_GLYPH)
                return ((-15));
            if (((&rvalues)->tas.rsize) % pclr->num_entries != 0 ||
                ((&rvalues)->tas.rsize) / pclr->num_entries != pclr->value_size)
                return ((-15));
            pclr->values.data = rvalues.value.bytes,
                pclr->values.size = ((&rvalues)->tas.rsize);
        } else {
            ...
            if (pclr->value_type != CODE_VALUE_GLYPH ||
                ((&rvalues)->tas.rsize) != pclr->num_entries ||
                pclr->value_size > sizeof(gs_glyph))
                return ((-15));
            pclr->values.data = (*(mem)->procs.alloc_string)(mem, values_size, "acquire_code_map(values)");
            ...
            for (k = 0; k < pclr->num_entries; ++k) {
                ...
                if (!((((const byte *)&((&rvalue)->tas.type_attrs))[1]) == (t_name)))
                    return ((-15));
                value = names_index(mem->gs_lib_ctx->gs_name_table, &rvalue);
        ...
        do { do { if ( !((((const byte *)&((&rfxs)->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0); if ( (ulong)(rfxs).value.intval > (0xff) ) return ((-15)); } while (0);
...
acquire_cid_system_info(ref *psia, const ref *op)
{
    ...
    if (dict_find_string(op, "CIDSystemInfo", &prcidsi) <= 0) {
        ((psia)->value.refs = ((ref *)((void*)0)), ((psia)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((psia)->tas.rsize = (0)));
        return 1;
    }
    if (((((const byte *)&((prcidsi)->tas.type_attrs))[1]) == (t_dictionary))) {
        ((psia)->value.refs = (prcidsi), ((psia)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((psia)->tas.rsize = (1)));
        return 0;
    }
    if (!(((prcidsi)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return ((-20));
    *psia = *prcidsi;
    return 0;
}
...
get_cid_system_info(const gs_memory_t *mem, gs_cid_system_info_t *pcidsi, const ref *psia, uint index)
{
    ...
    if (code < 0 || ((((const byte *)&((&rcidsi)->tas.type_attrs))[1]) == (t_null))) {
                ...
                const ref *op, gs_memory_t *imem)
{
    ...
    if (dict_find_string(op, "CMap", &prcmap) <= 0 ||
        !((((const byte *)&((prcmap)->tas.type_attrs))[1]) == (t_dictionary)) ||
        dict_find_string(prcmap, "CodeMap", &pcodemap) <= 0 ||
        !(((pcodemap)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) ||
        (*(imem)->procs.object_size)(imem, ((gs_cmap_t *)((pcodemap)->value.pstruct))) < sizeof(gs_cmap_t)
        )
        return ((-10));
    pcmap = ((gs_cmap_t *)((pcodemap)->value.pstruct));
    num_fonts = ((pfdepvector)->tas.rsize);
                  ...
                  gs_glyph glyph, gs_const_string *pstr, void *proc_data)
{
    ...
    names_index_ref(mem->gs_lib_ctx->gs_name_table, (uint)glyph, &nref);
    names_string_ref(mem->gs_lib_ctx->gs_name_table, &nref, &nsref);
    pstr->data = nsref.value.const_bytes;
    pstr->size = ((&nsref)->tas.rsize);
    return 0;
}
static int
zbuildcmap(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
    if ((code = dict_find_string(op, "CMapName", &pcmapname)) <= 0) {
        code = ((-15));
        goto fail;
    }
    if (!((((const byte *)&((pcmapname)->tas.type_attrs))[1]) == (t_name))) {
        code = ((-20));
        goto fail;
    }
    names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, pcmapname, &rname);
    if (dict_find_string(op, ".CodeMapData", &pcodemapdata) <= 0 ||
        !((((const byte *)&((pcodemapdata)->tas.type_attrs))[1]) == (t_array)) ||
        ((pcodemapdata)->tas.rsize) != 3 ||
        dict_find_string(op, "CodeMap", &pcodemap) <= 0 ||
        !((((const byte *)&((pcodemap)->tas.type_attrs))[1]) == (t_null))
        ) {
        code = ((-15));
        goto fail;
    }
    ...
    if ((code = gs_cmap_adobe1_alloc(&pcmap, 0, rname.value.const_bytes,
                                     ((&rname)->tas.rsize), ((&rcidsi)->tas.rsize),
                                     0, 0, 0, 0, 0, ((gs_memory_t *)((i_ctx_p->memory).current)))) < 0)
        goto fail;
    if ((code = dict_int_param(op, "CMapType", 0, 1, 0, &pcmap->CMapType)) < 0 ||
        (code = dict_float_param(op, "CMapVersion", 0.0, &pcmap->CMapVersion)) < 0 ||
        (code = dict_uid_param(op, &pcmap->uid, 0, ((gs_memory_t *)((i_ctx_p->memory).current)), i_ctx_p)) < 0 ||
        (code = dict_int_param(op, "WMode", 0, 1, 0, &pcmap->WMode)) < 0
        )
        goto fail;
    if (dict_find_string(op, "UIDOffset", &puidoffset) > 0) {
        if (!((((const byte *)&((puidoffset)->tas.type_attrs))[1]) == (t_integer))) {
            code = ((-20));
            goto fail;
        }
        pcmap->UIDOffset = puidoffset->value.intval;
    }
    for (i = 0; i < ((&rcidsi)->tas.rsize); ++i) {
        code = get_cid_system_info(((gs_memory_t *)((i_ctx_p->memory).current)), pcmap->CIDSystemInfo + i, &rcidsi, i);
        if (code < 0)
            goto fail;
    }
    array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pcodemapdata, 0L, &rcoderanges);
    array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pcodemapdata, 1L, &rdefs);
    array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pcodemapdata, 2L, &rnotdefs);
    if ((code = acquire_code_ranges(pcmap, &rcoderanges, ((gs_memory_t *)((i_ctx_p->memory).current)))) < 0)
        goto fail;
    if ((code = acquire_code_map(&pcmap->def, &rdefs, pcmap, ((gs_memory_t *)((i_ctx_p->memory).current)))) < 0)
        goto fail;
    if ((code = acquire_code_map(&pcmap->notdef, &rnotdefs, pcmap, ((gs_memory_t *)((i_ctx_p->memory).current)))) < 0)
        goto fail;
    ...
    ((&rcmap)->value.pstruct = ((obj_header_t *)(pcmap)), ((&rcmap)->tas.type_attrs = ((t_struct) << 8) + ((((0x20 + 0x40)) | (((&(i_ctx_p->memory)))->current_space))|((&(i_ctx_p->memory))->new_mask))) );
    code = dict_put_string(op, "CodeMap", &rcmap, &(i_ctx_p->dict_stack));
    if (code < 0)
        goto fail;
    return zreadonly(i_ctx_p);
fail:
    if (pcmap) {
        free_code_map(&pcmap->notdef, ((gs_memory_t *)((i_ctx_p->memory).current)));
        free_code_map(&pcmap->def, ((gs_memory_t *)((i_ctx_p->memory).current)));
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pcmap->CIDSystemInfo, "zbuildcmap(CIDSystemInfo)"));
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pcmap, "zbuildcmap(cmap)"));
    }
    return code;
}
