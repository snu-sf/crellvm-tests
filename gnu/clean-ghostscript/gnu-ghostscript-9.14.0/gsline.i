...
{
    (((&(pgs)->line_params))->half_width = fabs(width) / 2);
    return 0;
}
...
{
    return (((&(pgs)->line_params))->half_width * 2);
}
...
gs_setlinecap(gs_state * pgs, gs_line_cap cap)
{
    if ((uint) cap > 3)
        return ((-15));
    (&(pgs)->line_params)->start_cap = cap;
    (&(pgs)->line_params)->end_cap = cap;
    (&(pgs)->line_params)->dash_cap = cap;
    return 0;
}
...
gs_currentlinecap(const gs_state * pgs)
{
    ...
    return (&(pgs)->line_params)->start_cap;
}
...
{
    return gx_set_miter_limit((&(pgs)->line_params), limit);
}
            ...
            double offset, gs_memory_t * mem)
{
    ...
    if (length == 0) {
        dist_left = 0.0;
        if (mem && ppat) {
            ((mem)->procs.free_object(mem, ppat, "gx_set_dash(old pattern)"));
            ppat = 0;
        }
    } else {
        ...
        if (length & 1) {
            ...
            dist_left = ((offset) - floor((offset) / (length2)) * (length2));

            dist_left = ((dist_left) - floor((dist_left) / (length2)) * (length2));
            if (dist_left >= pattern_length)
                dist_left -= pattern_length, ink = !ink;
        } else {
            dist_left = ((offset) - floor((offset) / (pattern_length)) * (pattern_length));

            dist_left = ((dist_left) - floor((dist_left) / (pattern_length)) * (pattern_length));
        }
        ...
        if (mem) {
            if (ppat == 0)
                ppat = (float *)(*(mem)->procs.alloc_bytes)(mem, size, "gx_set_dash(pattern)");

            else if (length != dash->pattern_size)
                ppat = ((mem)->procs.resize_object(mem, ppat, size, "gx_set_dash(pattern)"));
...
{
    return gx_set_dash(&(&(pgs)->line_params)->dash, pattern, length, offset,
                       pgs->memory);
}
...
{
    (&(pgs)->line_params)->dash.adapt = adapt;
}
...
{
    return (&(pis)->line_params)->dash.adapt;
}
bool
...
gs_setcurvejoin(gs_state * pgs, int join)
{
    if (join < -1 || join > 4)
        return ((-15));
    (&(pgs)->line_params)->curve_join = join;
    return 0;
}
...
{
    return (&(pgs)->line_params)->curve_join;
}
...
{
    return gx_set_dot_length((&(pgs)->line_params), length, absolute);
}
...
{
    return (&(pgs)->line_params)->dot_length;
}
bool
gs_currentdotlength_absolute(const gs_state * pgs)
{
    return (&(pgs)->line_params)->dot_length_absolute;
}
...
gs_setdotorientation(gs_state *pgs)
{
    if ((((&pgs->ctm)->xy) == 0.0 && ((&pgs->ctm)->yx) == 0.0) || (((&pgs->ctm)->xx) == 0.0 && ((&pgs->ctm)->yy) == 0.0))
        return gs_currentmatrix(pgs, &(&(pgs)->line_params)->dot_orientation);
    return ((-15));
}
...
{
    return gs_setmatrix(pgs, &(&(pgs)->line_params)->dot_orientation);
}
