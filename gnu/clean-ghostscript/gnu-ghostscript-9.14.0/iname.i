...
const uint name_max_string = ((1 << (14 - 4)) - 1);
...
static const byte nt_1char_names[128] = {
...
static const gs_memory_struct_type_t st_name_sub_table = { sizeof(name_sub_table), "name_sub_table", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
static gs_ptr_type_t name_string_sub_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void name_string_sub_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_name_string_sub_table = { sizeof(name_string_sub_table_t), "name_string_sub_table_t", 0, 0, name_string_sub_enum_ptrs, name_string_sub_reloc_ptrs, 0, 0 };


static void gs_names_finalize(const gs_memory_t *cmem, void *ptr); static gs_ptr_type_t name_table_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void name_table_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_name_table = { sizeof(name_table), "name_table", 0, 0, name_table_enum_ptrs, name_table_reloc_ptrs, gs_names_finalize, 0 };
...
names_init(ulong count, gs_ref_memory_t *imem)
{
    ...
    if (count == 0)
        count = (uint)((0x10000 << 4) - 1) + 1L;
    else if (count - 1 > (uint)((0x10000 << 4) - 1))
        return 0;
    ...
    nt->max_sub_count =
        ((count - 1) | ((1 << (8 + (4 >= 2))) - 1)) >> (8 + (4 >= 2));
    nt->name_string_attrs = imemory_space(imem) | (0x20 + 0x40);
    nt->memory = mem;


    for (i = 0; i < 2 + 128; i += (1 << (8 + (4 >= 2)))) {
        int code = name_alloc_sub(nt);

        if (code < 0) {
            while (nt->sub_next > 0)
                name_free_sub(nt, --(nt->sub_next), ((bool)0));
            ((mem)->procs.free_object(mem, nt, "name_init(nt)"));
            return 0;
        }
    }
    for (i = -1; i < 128; i++) {
        uint ncnt = 2 + i;
        uint nidx = (((ncnt) & (-(1 << (8 + (4 >= 2))))) + (((ncnt) * 23) & ((1 << (8 + (4 >= 2))) - 1)));
        name *pname = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].names->names + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
        name_string_t *pnstr = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].strings->strings + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
...
{
    (void)vptr;
    cmem->gs_lib_ctx->gs_name_table = ((void*)0);
}
...
names_ref(name_table *nt, const byte *ptr, uint size, ref *pref, int enterflag)
{
    ...
    switch (size) {
    case 0:
        nidx = (((1) & (-(1 << (8 + (4 >= 2))))) + (((1) * 23) & ((1 << (8 + (4 >= 2))) - 1)));
        pname = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].names->names + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
        goto mkn;
    case 1:
        if (*ptr < 128) {
            uint hash = *ptr + 2;

            nidx = (((hash) & (-(1 << (8 + (4 >= 2))))) + (((hash) * 23) & ((1 << (8 + (4 >= 2))) - 1)));
            pname = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].names->names + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
            goto mkn;
        }
    ...
    default: {
        ...
        do { const byte *p = ptr; uint n = size; hash = hash_permutation[*p++]; while (--n > 0) hash = (hash << 8) | hash_permutation[(byte)hash ^ *p++]; } while (0);
        phash = nt->hash + (hash & ((1024 << (4 / 2)) - 1));
    }
    }

    for (nidx = *phash; nidx != 0;
         nidx = ((pnstr)->next_index)
        ) {
        pnstr = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].strings->strings + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
        if (pnstr->string_size == size &&
            !memcmp(ptr,pnstr->string_bytes,size)
            ) {
            pname = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].names->names + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
            goto mkn;
        }
    ...
    pnstr = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].strings->strings + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
    if (enterflag == 1) {
        byte *cptr = (byte *)(*(nt->memory)->procs.alloc_string)(nt->memory, size, "names_ref(string)");
    ...
    pname = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].names->names + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
    pname->pvalue = ((ref *)0);
    nt->free = ((pnstr)->next_index);
    ((pnstr)->next_index = (*phash));
    *phash = nidx;
    do { } while (0);
 mkn:
    ((pref)->value.pname = (pname), ((pref)->tas.type_attrs = ((t_name) << 8) + (avm_system)), ((pref)->tas.rsize = ((ushort)(nidx))));
    return 0;
}
...
names_string_ref(const name_table * nt, const ref * pnref ,
                 ref * psref )
{
    const name_string_t *pnstr = ((nt)->sub[(( ((const name_sub_table *) ((pnref)->value.pname - (((pnref)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((pnref)->tas.rsize) )) >> (8 + (4 >= 2))].strings->strings + ((( ((const name_sub_table *) ((pnref)->value.pname - (((pnref)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((pnref)->tas.rsize) )) & ((1 << (8 + (4 >= 2))) - 1)));

    ((psref)->value.const_bytes = ((const byte *)pnstr->string_bytes), ((psref)->tas.type_attrs = ((t_string) << 8) + ((pnstr->foreign_string ? avm_foreign | (0x20 + 0x40) : nt->name_string_attrs))), ((psref)->tas.rsize = (pnstr->string_size)));
...
names_from_string(name_table * nt, const ref * psref, ref * pnref)
{
    ...
    int code = names_ref(nt, psref->value.bytes, ((psref)->tas.rsize), pnref, 1);
...
{
    return ( ((const name_sub_table *) ((pnref)->value.pname - (((pnref)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((pnref)->tas.rsize) );
}
void
names_index_ref(const name_table * nt, name_index_t index, ref * pnref)
{
    ((pnref)->value.pname = (((nt)->sub[(index) >> (8 + (4 >= 2))].names->names + ((index) & ((1 << (8 + (4 >= 2))) - 1)))), ((pnref)->tas.type_attrs = ((t_name) << 8) + (avm_system)), ((pnref)->tas.rsize = ((ushort)(index))));;
}
name *
names_index_ptr(const name_table * nt, name_index_t index)
{
    return ((nt)->sub[(index) >> (8 + (4 >= 2))].names->names + ((index) & ((1 << (8 + (4 >= 2))) - 1)));
}
...
names_next_valid_index(name_table * nt, name_index_t nidx)
{
    ...
    do {
        ...
        if ((nidx & ((1 << (8 + (4 >= 2))) - 1)) == 0)
            for (;; nidx += (1 << (8 + (4 >= 2)))) {
                if ((nidx >> (8 + (4 >= 2))) >= nt->sub_count)
                    return 0;
                ssub = nt->sub[nidx >> (8 + (4 >= 2))].strings;
                if (ssub != 0)
                    break;
            }
        pnstr = &ssub->strings[nidx & ((1 << (8 + (4 >= 2))) - 1)];
    }
...
names_unmark_all(name_table * nt)
{
    ...
    for (si = 0; si < nt->sub_count; ++si)
        if ((ssub = nt->sub[si].strings) != 0) {
            ...
            for (i = 0; i < (1 << (8 + (4 >= 2))); ++i)
                if (((((si << (8 + (4 >= 2))) + i) & (-(1 << (8 + (4 >= 2))))) + ((((si << (8 + (4 >= 2))) + i) * 1959) & ((1 << (8 + (4 >= 2))) - 1))) >=
                    nt->perm_count)
...
names_trace_finish(name_table * nt, gc_state_t * gcst)
{
    ...
    for (i = 0; i < (1024 << (4 / 2)); phash++, i++) {
        ...
        while (nidx != 0) {
            name_string_t *pnstr = ((nt)->sub[(nidx) >> (8 + (4 >= 2))].strings->strings + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
            name_index_t next = ((pnstr)->next_index);
    ...
    for (i = nt->sub_count; --i >= 0;) {
        ...
        if (sub != 0) {
            name_scan_sub(nt, i, ((bool)1), ((bool)1) && (gcst != 0));
        }
    }
    nt->sub_next = 0;
}
...
names_restore(name_table * nt, alloc_save_t * save)
{
    ...
    for (si = 0; si < nt->sub_count; ++si)
        if (nt->sub[si].strings != 0) {
            ...
            for (i = 0; i < (1 << (8 + (4 >= 2))); ++i) {
                name_string_t *pnstr =
                    ((nt)->sub[((si << (8 + (4 >= 2))) + i) >> (8 + (4 >= 2))].strings->strings + (((si << (8 + (4 >= 2))) + i) & ((1 << (8 + (4 >= 2))) - 1)));
...
name_alloc_sub(name_table * nt)
{
    ...
    sub = (name_sub_table *)(*(mem)->procs.alloc_struct)(mem, &st_name_sub_table, "name_alloc_sub(sub-table)");

    ssub = (name_string_sub_table_t *)(*(mem)->procs.alloc_struct)(mem, &st_name_string_sub_table, "name_alloc_sub(string sub-table)");


    if (sub == 0 || ssub == 0) {
        ((mem)->procs.free_object(mem, ssub, "name_alloc_sub(string sub-table)"));
        ((mem)->procs.free_object(mem, sub, "name_alloc_sub(sub-table)"));
        return ((-25));
    }
    ...
    sub->high_index = (sub_index >> (16 - (8 + (4 >= 2)))) << 16;
    ...
    name_scan_sub(nt, sub_index, ((bool)0), ((bool)0));
# 532 "./psi/iname.c"
    return 0;
}
...
name_free_sub(name_table * nt, uint sub_index, bool unmark)
{
    ...
    if (unmark) {
        ...
        (((obj_header_t *)ssub - 1)->d.o.f.m.smark = (((uint)1 << ((1 << 2) * 8 - 1)) - 1));
    }

    ((nt->memory)->procs.free_object(nt->memory, nt->sub[sub_index].strings, "name_free_sub(string sub-table)"));

    ((nt->memory)->procs.free_object(nt->memory, nt->sub[sub_index].names, "name_free_sub(sub-table)"));
...
name_scan_sub(name_table * nt, uint sub_index, bool free_empty, bool unmark)
{
    name_string_sub_table_t *ssub = nt->sub[sub_index].strings;
    uint free = nt->free;
    uint nbase = sub_index << (8 + (4 >= 2));
    uint ncnt = nbase + ((1 << (8 + (4 >= 2))) - 1);
    ...
    for (;; --ncnt) {
        uint nidx = (((ncnt) & (-(1 << (8 + (4 >= 2))))) + (((ncnt) * 23) & ((1 << (8 + (4 >= 2))) - 1)));
        name_string_t *pnstr = &ssub->strings[nidx & ((1 << (8 + (4 >= 2))) - 1)];

        if (pnstr->mark)
            keep = ((bool)1);
        else {
            ((pnstr)->next_index = (free));
            free = nidx;
        }
        if (ncnt == nbase)
            break;
...
static
gs_ptr_type_t name_table_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst)
{
    const name_table *const nt = vptr;
    uint i = index >> 1;

    if (i >= nt->sub_count)
        return 0;
    if (index & 1)
        return (pep->ptr = (const void *)(nt->sub[i].strings), (&ptr_struct_procs));
    else
        return (pep->ptr = (const void *)(nt->sub[i].names), (&ptr_struct_procs));
}

static void name_table_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { name_table *nt = vptr;
{
    ...
    for (i = 0; i < sub_count; i++) {
        (nt->sub[i].names = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(nt->sub[i].names), gcst)));
        (nt->sub[i].strings = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(nt->sub[i].strings), gcst)));
    }
...
static gs_ptr_type_t name_string_sub_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst)
{
    return 0;
}

static void name_string_sub_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) {
{
    name_string_t *pnstr = ((name_string_sub_table_t *)vptr)->strings;
    uint i;

    for (i = 0; i < (1 << (8 + (4 >= 2))); ++pnstr, ++i) {
        if (pnstr->string_bytes != 0 && !pnstr->foreign_string) {
            gs_const_string nstr;

            nstr.data = pnstr->string_bytes;
            nstr.size = pnstr->string_size;
            (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(nstr), gcst));
