...
s_write_ps_string(stream * s, const byte * str, uint size, int print_ok)
{
    ...
    for (i = 0; i < size; ++i) {
        ...
        if (ch == 0 || ch >= 127)
            added += 3;
        else if ((__extension__ (__builtin_constant_p (ch) && !__builtin_constant_p ("()\\\n\r\t\b\f") && (ch) == '\0' ? (char *) __rawmemchr ("()\\\n\r\t\b\f", ch) : __builtin_strchr ("()\\\n\r\t\b\f", ch))) != 0)
            ++added;
        else if (ch < 32)
            added += 3;
    }

    if (added < size || (print_ok & 4)) {
        ...
        ((&state)->EndOfData = ((bool)1), (&state)->count = 0);
        spputc(s, '<');
    }

    {
        ...
        do {
            ...
            w.ptr = buf; w.ptr--;
            status = (*templat->process) (st, &r, &w, ((bool)1));
...
static const gc_ptr_element_t printer_plist_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((printer_param_list_t *)0)->strm - (char *)((printer_param_list_t *)0) )) } }; static const gc_struct_data_t printer_plist_reloc_ptrs = { (sizeof(printer_plist_enum_ptrs) / sizeof((printer_plist_enum_ptrs)[0])), 0, 0, printer_plist_enum_ptrs }; static const gs_memory_struct_type_t st_printer_param_list = { sizeof(printer_param_list_t), "printer_param_list_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &printer_plist_reloc_ptrs };
const param_printer_params_t param_printer_params_default = {
    0, 0, 0, "\n", 0
};


static int param_print_typed(gs_param_list *, gs_param_name, gs_param_typed_value *);
                      ...
                      gs_memory_t * mem)
{
    printer_param_list_t *prlist =
        (printer_param_list_t *)(*(mem)->procs.alloc_struct)(mem, &st_printer_param_list, "s_alloc_param_printer");
...
s_free_param_printer(gs_param_list * plist)
{
    if (plist) {
        ...
        ((prlist->memory)->procs.free_object(prlist->memory, plist, "s_free_param_printer"));
    }
}
