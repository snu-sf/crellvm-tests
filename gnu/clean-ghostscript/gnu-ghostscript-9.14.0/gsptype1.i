...
typedef struct gx_device_mask_clip_s {
    ...
    union _b {
        byte bytes[((16384 / (1 << 3)) * (1 << 3))];
        ulong longs[((16384 / (1 << 3)) * (1 << 3)) / (1 << 3)];
    } buffer;
} gx_device_mask_clip;

extern const gs_memory_struct_type_t st_device_mask_clip;
# 67 "./base/gxmclip.h"
...
# 38 "./base/gsptype1.c" 2
# 1 "./base/gxp1impl.h" 1
# 28 "./base/gxp1impl.h"
int gx_dc_pattern_fill_rectangle(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
int gx_dc_pure_masked_fill_rect(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
int gx_dc_devn_masked_fill_rect(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
int gx_dc_binary_masked_fill_rect(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
int gx_dc_colored_masked_fill_rect(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
int gx_dc_pat_trans_fill_rectangle(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
...
int gx_pattern_load(gx_device_color *, const gs_imager_state *,
                    gx_device *, gs_color_select_t);
int gs_pattern1_remap_color(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
# 39 "./base/gsptype1.c" 2
# 1 "./base/gxclist.h" 1
# 24 "./base/gxclist.h"
# 1 "./base/gxband.h" 1
# 22 "./base/gxband.h"
...
static const gc_struct_data_t pattern1_template_reloc_ptrs = { 0, 0, &st_pattern_template, 0 }; static const gs_memory_struct_type_t st_pattern1_template = { sizeof(gs_pattern1_template_t), "gs_pattern1_template_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pattern1_template_reloc_ptrs };
static gs_ptr_type_t pattern1_instance_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void pattern1_instance_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_pattern1_instance = { sizeof(gs_pattern1_instance_t), "gs_pattern1_instance_t", 0, 0, pattern1_instance_enum_ptrs, pattern1_instance_reloc_ptrs, 0, 0 };


static gs_ptr_type_t pattern1_instance_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { switch ( index ) { default: {
    if (index < 2) {
        gs_ptr_type_t ptype =
            (*(st_pattern1_template).enum_ptrs)(mem, &((const gs_pattern1_instance_t *)vptr)->templat, sizeof(((const gs_pattern1_instance_t *)vptr)->templat), index-(0), pep, &(st_pattern1_template), gcst);


        if (ptype)
            return ptype;
        return (pep->ptr = (const void *)(((void*)0)), (&ptr_struct_procs));
    }
    return (*(st_pattern_instance).enum_ptrs)(mem, vptr, size, index-(2), pep, &(st_pattern_instance), gcst);
} } }
static void pattern1_instance_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { {
    (*(st_pattern_instance).reloc_ptrs)(vptr, size, &(st_pattern_instance), gcst);
    (*(st_pattern1_template).reloc_ptrs)(&((gs_pattern1_instance_t *)vptr)->templat, sizeof(((gs_pattern1_instance_t *)vptr)->templat), &(st_pattern1_template), gcst);
} }


static bool gs_pattern1_uses_base_space(const gs_pattern_template_t *);
static int gs_pattern1_make_pattern(gs_client_color *, const gs_pattern_template_t *, const gs_matrix *, gs_state *, gs_memory_t *);
static const gs_pattern_template_t *gs_pattern1_get_pattern(const gs_pattern_instance_t *);
static int gs_pattern1_set_color(const gs_client_color *, gs_state *);
                    ...
                    gs_color_space * pbase_cspace, gs_memory_t * pmem)
{
    ...
    if (pcspace == ((void*)0))
        return ((-25));
    if (pbase_cspace != 0) {
        pcspace->params.pattern.has_base_space = ((bool)1);

        pcspace->base_space = pbase_cspace;
    } else
        pcspace->params.pattern.has_base_space = ((bool)0);
    *ppcspace = pcspace;
    return 0;
}
                         ...
                         gs_memory_t * mem)
{
    ...
    *(gs_pattern_instance_t *)&inst = *(gs_pattern_instance_t *)pinst;
    ...
    switch (pcp->PaintType) {
        case 1:
            gs_set_logical_op(saved, (rop3_default | (((bool)0) ? 0x100 : 0) | (((bool)0) ? 0x200 : 0)));
            break;
        case 2:
    ...
    { float width = inst.templat.BBox.q.x - inst.templat.BBox.p.x;
      ...
      if ( inst.templat.XStep < width || inst.templat.YStep < height || (*(const gs_matrix *)&(saved)->ctm).xy != 0 ||
          (*(const gs_matrix *)&(saved)->ctm).yx != 0 ){

          inst.has_overlap = ((bool)1);

      } else {

          inst.has_overlap = ((bool)0);
        ...
        if (inst.size.x == 0 || inst.size.y == 0) {
            ...
            if (0 &&
                ...
                if (0) {
                    ...
                    gs_scale(saved, (fabs(inst.size.x) - 1.0 / (1<<8)) / fabs(inst.size.x),
                                    (fabs(inst.size.y) - 1.0 / (1<<8)) / fabs(inst.size.y));
                }
                do { } while (0);
            ...
            } else if ((1) && (inst.templat.TilingType != 2)) {
...
# 286 "./base/gsptype1.c"
                        ...
                        if (bbw >= inst.size.x - 1.0 / (1<<8))
                            gs_scale(saved, (fabs(inst.size.x) - 1.0 / (1<<8)) / fabs(inst.size.x), 1);
...
# 307 "./base/gsptype1.c"
                        ...
                        if (bbh >= inst.size.y - 1.0 / (1<<8))
                            gs_scale(saved, 1, (fabs(inst.size.y) - 1.0 / (1<<8)) / fabs(inst.size.y));

                    }
                    code = gs_bbox_transform(&inst.templat.BBox, &(*(const gs_matrix *)&(saved)->ctm), &bbox);
                    if (code < 0)
                        goto fsaved;
                }
    ...
    inst.uses_mask = ((bool)1);
    inst.is_clist = ((bool)0);
    gx_translate_to_fixed(saved, ((fixed)floor((inst.step_matrix.tx - bbox.p.x)*(float)(1<<8) + 0.5)),
                                 ((fixed)floor((inst.step_matrix.ty - bbox.p.y)*(float)(1<<8) + 0.5)));
    inst.step_matrix.tx = bbox.p.x;
    inst.step_matrix.ty = bbox.p.y;

    cbox.p.x = 0L;
    cbox.p.y = 0L;
    cbox.q.x = ((fixed)(inst.size.x)<<8);
    cbox.q.y = ((fixed)(inst.size.y)<<8);
  ...
  fsaved:gs_state_free(saved);
    ((mem)->procs.free_object(mem, pinst, "gs_makepattern"));
    return code;
}
# 392 "./base/gsptype1.c"
static int
                    ...
                    int width, int height, const gs_matrix * pmat)
{
    ...
    double xepsilon = 1.192092896e-07F * width;
    double yepsilon = 1.192092896e-07F * height;
# 422 "./base/gsptype1.c"
    if (xstep < 0)
        xstep *= -1;
    if (ystep < 0)
                            ...
                            gs_rect * pbbox, int width, int height)
{
    ...
    code = gs_bbox_transform(&pinst->templat.BBox, &(*(const gs_matrix *)&(saved)->ctm), pbbox);
    if (code < 0)
        return code;
    ...
    if (saved->ctm.txy_fixed_valid) {
        code = gx_translate_to_fixed(saved, ((fixed)floor((saved->ctm.tx - dx)*(float)(1<<8) + 0.5)),
                                            ((fixed)floor((saved->ctm.ty - dy)*(float)(1<<8) + 0.5)));
    } else {
        saved->ctm.tx -= dx;
        saved->ctm.ty -= dy;

        code = gx_path_translate(saved->path, ((fixed)((-dx)*(float)(1<<8))), ((fixed)((-dy)*(float)(1<<8))));
    }
    if (code < 0)
        return code;
    ...
    if ((pbbox->q.x - pbbox->p.x > width || pbbox->q.y - pbbox->p.y > height))
        code = clamp_pattern_bbox(pinst, pbbox, width,
                                        height, &(*(const gs_matrix *)&(saved)->ctm));
...
{
    if (pdevc->colors.pattern.p_tile->cdev != ((void*)0)) {
...
gs_dc_get_pattern_id(const gx_device_color *pdevc)
{
    if (!(gx_dc_is_pattern1_color(pdevc)))
        return 0L;
    if (pdevc->colors.pattern.p_tile == ((void*)0))
        return 0L;
...
gs_pattern1_set_color(const gs_client_color * pcc, gs_state * pgs)
{
    ...
    if (ptmplt->PaintType == 2) {
        ...
        params.retain_any_comps = ((bool)0);
...
# 712 "./base/gsptype1.c"
static const gc_ptr_element_t bitmap_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_bitmap *)0)->data - (char *)((gs_bitmap *)0) )) } }; static const gc_struct_data_t bitmap_reloc_ptrs = { (sizeof(bitmap_enum_ptrs) / sizeof((bitmap_enum_ptrs)[0])), 0, 0, bitmap_enum_ptrs }; const gs_memory_struct_type_t st_gs_bitmap = { sizeof(gs_bitmap), "client bitmap", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bitmap_reloc_ptrs };
const gs_memory_struct_type_t st_gs_tile_bitmap = { sizeof(gs_tile_bitmap), "client tile bitmap", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bitmap_reloc_ptrs };
const gs_memory_struct_type_t st_gs_depth_bitmap = { sizeof(gs_depth_bitmap), "client depth bitmap", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bitmap_reloc_ptrs };
const gs_memory_struct_type_t st_gs_tile_depth_bitmap = { sizeof(gs_tile_depth_bitmap), "client tile_depth bitmap", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bitmap_reloc_ptrs };
const gs_memory_struct_type_t st_gx_strip_bitmap = { sizeof(gx_strip_bitmap), "gx_strip_bitmap", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bitmap_reloc_ptrs };
# 727 "./base/gsptype1.c"
...
static const gc_ptr_element_t pixmap_enum_ptr[] = { { GC_ELT_OBJ, ((int) ( (char *)&((pixmap_info *)0)->pcspace - (char *)((pixmap_info *)0) )) } }; static const gc_struct_data_t pixmap_reloc_ptr = { (sizeof(pixmap_enum_ptr) / sizeof((pixmap_enum_ptr)[0])), 0, &st_gs_depth_bitmap, pixmap_enum_ptr }; static const gs_memory_struct_type_t st_pixmap_info = { sizeof(pixmap_info), "pixmap info. struct", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pixmap_reloc_ptr };
# 754 "./base/gsptype1.c"
static void
free_pixmap_pattern(
    gs_memory_t * pmem,
...
)
{
    ...
    ((pmem)->procs.free_object(pmem, ppmap, cname));
}
...
mask_PaintProc(const gs_client_color * pcolor, gs_state * pgs)
{
    ...
    if (pen == 0)
        return ((-25));
    gs_image_t_init_mask_adjust(&mask, ((bool)1), ((bool)1));
    mask.Width = pbitmap->size.x;
    mask.Height = pbitmap->size.y;
    gs_image_init(pen, &mask, ((bool)0), pgs);
...
# 812 "./base/gsptype1.c"
    ...
    if (transparent) {
        image.i4.MaskColor_is_range = ((bool)0);
...
int pixmap_high_level_pattern(gs_state * pgs)
{
    ...
    int code;
    gx_device_color *pdc = ((pgs)->color[0].dev_color);
    ...
    ((pgs->device)->procs.get_initial_matrix)(pgs->device, &m);
    gs_setmatrix(pgs, &m);
    code = gs_bbox_transform(&ppat->BBox, &(*(const gs_matrix *)&(pgs)->ctm), &bbox);
    if (code < 0) {
        gs_grestore(pgs);
            return code;
    }
    clip_box.p.x = ((fixed)((bbox.p.x)*(float)(1<<8)));
    clip_box.p.y = ((fixed)((bbox.p.y)*(float)(1<<8)));
    clip_box.q.x = ((fixed)((bbox.q.x)*(float)(1<<8)));
    clip_box.q.y = ((fixed)((bbox.q.y)*(float)(1<<8)));
    code = gx_clip_to_rectangle(pgs, &clip_box);
    if (code < 0) {
        gs_grestore(pgs);
        return code;
    }
    code = ((pgs->device)->procs.dev_spec_op)(pgs->device,
    ...
    code = ((pgs->device)->procs.dev_spec_op)(pgs->device,
                          gxdso_pattern_finish_accum, ((void*)0), 0L);
...
static int pixmap_remap_mask_pattern(const gs_client_color *pcc, gs_state *pgs)
{
    ...
    if (pgs->have_pattern_streams)
        code = ((pcc->pattern->saved->device)->procs.dev_spec_op)(pcc->pattern->saved->device,
...
)
{
    ...
    if (mem == 0)
        mem = gs_state_memory(pgs);
    ppmap = (pixmap_info *)(*(mem)->procs.alloc_struct)(mem, &st_pixmap_info, "makepximappattern");
    ...
    ((&pat.uid)->id = (id == (~(-1L << ((1 << 3) * 8 - 1)))) ? gs_next_ids(mem, 1) : id, (&pat.uid)->xvalues = 0);
    ...
    if ((code = gs_makepattern(pcc, &pat, pmat, pgs, mem)) != 0)
        ((mem)->procs.free_object(mem, ppmap, "makebitmappattern_xform"));
    else {
...
# 1147 "./base/gsptype1.c"
static const gx_device_halftone * gx_dc_pattern_get_dev_halftone(const gx_device_color * pdevc);
static int gx_dc_pattern_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);

static bool gx_dc_pattern_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);
static int gx_dc_pure_masked_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);
static int gx_dc_devn_masked_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);
static bool gx_dc_devn_masked_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);

static const gx_device_halftone * gx_dc_pure_masked_get_dev_halftone(const gx_device_color * pdevc);

static bool gx_dc_pure_masked_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);
static int gx_dc_binary_masked_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);

static const gx_device_halftone * gx_dc_binary_masked_get_dev_halftone(const gx_device_color * pdevc);

static bool gx_dc_binary_masked_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);
static int gx_dc_colored_masked_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);

static const gx_device_halftone * gx_dc_colored_masked_get_dev_halftone(const gx_device_color * pdevc);

static bool gx_dc_colored_masked_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);


static gs_ptr_type_t dc_pattern_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void dc_pattern_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_dc_pattern = { sizeof(gx_device_color), "dc_pattern", 0, 0, dc_pattern_enum_ptrs, dc_pattern_reloc_ptrs, 0, 0 };
...
extern const gs_memory_struct_type_t st_dc_ht_binary;
static gs_ptr_type_t dc_masked_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void dc_masked_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_dc_pure_masked = { sizeof(gx_device_color), "dc_pure_masked", 0, 0, dc_masked_enum_ptrs, dc_masked_reloc_ptrs, 0, 0 };
...
static gs_ptr_type_t dc_binary_masked_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void dc_binary_masked_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_dc_binary_masked = { sizeof(gx_device_color), "dc_binary_masked", 0, 0, dc_binary_masked_enum_ptrs, dc_binary_masked_reloc_ptrs, 0, 0 };
...
static gs_ptr_type_t dc_colored_masked_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void dc_colored_masked_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_dc_colored_masked = { sizeof(gx_device_color), "dc_colored_masked", 0, 0, dc_colored_masked_enum_ptrs, dc_colored_masked_reloc_ptrs, 0, 0 };
...
static gs_ptr_type_t dc_devn_masked_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void dc_devn_masked_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_dc_devn_masked = { sizeof(gx_device_color), "dc_devn_masked", 0, 0, dc_devn_masked_enum_ptrs, dc_devn_masked_reloc_ptrs, 0, 0 };
...
static
gs_ptr_type_t dc_pattern_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_color *cptr = vptr; switch ( index ) { default:
{
    return (*(st_dc_pure_masked).enum_ptrs)(mem, vptr, size, index - 1, pep, &(st_dc_pure_masked), gcst);
}
case 0:
{
    gx_color_tile *tile = cptr->colors.pattern.p_tile;

    return (pep->ptr = (const void *)((tile == 0 ? tile : tile - tile->index)), (&ptr_struct_procs));
}
} }
static void dc_pattern_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_color *cptr = vptr;
{
    gx_color_tile *tile = cptr->colors.pattern.p_tile;
    ...
    if (tile != 0) {
        ...
        (((gx_device_color *)vptr)->colors.pattern.p_tile = (void *)(((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_device_color *)vptr)->colors.pattern.p_tile - (index)), gcst)), ((gx_device_color *)vptr)->colors.pattern.p_tile += (index));
    }
    (*(st_dc_pure_masked).reloc_ptrs)(vptr, size, &(st_dc_pure_masked), gcst);
}
}
static gs_ptr_type_t dc_masked_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_color *cptr = vptr; switch ( index ) { default:
return (*(st_client_color).enum_ptrs)(mem, &((const gx_device_color *)vptr)->ccolor, sizeof(((const gx_device_color *)vptr)->ccolor), index-(1), pep, &(st_client_color), gcst);
case 0:
{
    gx_color_tile *mask = cptr->mask.m_tile;

    return (pep->ptr = (const void *)((mask == 0 ? mask : mask - mask->index)), (&ptr_struct_procs));
}
} }
static void dc_masked_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_color *cptr = vptr;
{
    gx_color_tile *mask = cptr->mask.m_tile;

    (*(st_client_color).reloc_ptrs)(&((gx_device_color *)vptr)->ccolor, sizeof(((gx_device_color *)vptr)->ccolor), &(st_client_color), gcst);
    if (mask != 0) {
        uint index = mask->index;

        (((gx_device_color *)vptr)->mask.m_tile = (void *)(((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_device_color *)vptr)->mask.m_tile - (index)), gcst)), ((gx_device_color *)vptr)->mask.m_tile += (index));
    }
}
}
static gs_ptr_type_t dc_colored_masked_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_color *cptr = vptr; switch ( index ) { default:
return (*(st_client_color).enum_ptrs)(mem, &((const gx_device_color *)vptr)->ccolor, sizeof(((const gx_device_color *)vptr)->ccolor), index-(1), pep, &(st_client_color), gcst);
case 0:
{
    return (pep->ptr = (const void *)(cptr->colors.colored.c_ht), (&ptr_struct_procs));
}
} }
static void dc_colored_masked_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_color *cptr = vptr;
{
    (*(st_client_color).reloc_ptrs)(&((gx_device_color *)vptr)->ccolor, sizeof(((gx_device_color *)vptr)->ccolor), &(st_client_color), gcst);
    if (cptr->colors.colored.c_ht != 0) {
        (((gx_device_color *)vptr)->colors.colored.c_ht = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_device_color *)vptr)->colors.colored.c_ht), gcst)));
    }
}
}
static gs_ptr_type_t dc_devn_masked_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_color *cptr = vptr; switch ( index ) { default:
return (*(st_client_color).enum_ptrs)(mem, &((const gx_device_color *)vptr)->ccolor, sizeof(((const gx_device_color *)vptr)->ccolor), index-(0), pep, &(st_client_color), gcst);
} }
static void dc_devn_masked_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_color *cptr = vptr;
{
    (*(st_client_color).reloc_ptrs)(&((gx_device_color *)vptr)->ccolor, sizeof(((gx_device_color *)vptr)->ccolor), &(st_client_color), gcst);
}
}
static gs_ptr_type_t dc_binary_masked_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { switch ( index ) { default:
{
    return (*(st_dc_ht_binary).enum_ptrs)(mem, vptr, size, index - 2, pep, &(st_dc_ht_binary), gcst);
}
case 0:
case 1:
return (*(st_dc_pure_masked).enum_ptrs)(mem, vptr, size, index, pep, &(st_dc_pure_masked), gcst);
} }
static void dc_binary_masked_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) {
{
    (*(st_dc_pure_masked).reloc_ptrs)(vptr, size, &(st_dc_pure_masked), gcst);
    (*(st_dc_ht_binary).reloc_ptrs)(vptr, size, &(st_dc_ht_binary), gcst);
}
}
# 1340 "./base/gsptype1.c"
void
                   ...
                   gx_device * dev, gs_color_select_t select)
{
    ...
    while ( !gx_pattern_cache_lookup(pdevc, pis, dev, select) ) { code = gx_pattern_load(pdevc, pis, dev, select); if ( code < 0 ) break; } return code;
}
                        ...
                        gx_device * dev, gs_color_select_t select)
{
    ...
    if (id == 0L) {
        ((pdevc)->mask.id = 0L, (pdevc)->mask.m_tile = 0, (pdevc)->colors.pattern.p_tile = 0, (pdevc)->type = (&gx_dc_pattern));
        return ((bool)1);
    }
    if (pcache != 0) {
        gx_color_tile *ctile = &pcache->tiles[id % pcache->num_tiles];
        bool internal_accum = ((bool)1);
        if (pis->have_pattern_streams) {
            int code = ((dev)->procs.dev_spec_op)(dev, gxdso_pattern_load, ((void*)0), id);
...
# 1488 "./base/gsptype1.c"
                ((pdevc)->phase.x = (-px), (pdevc)->phase.y = (-py));
...
gx_dc_devn_masked_equal(const gx_device_color * pdevc1,
                        const gx_device_color * pdevc2)
{
    return (*(&gx_dc_type_data_devn)->equal) (pdevc1, pdevc2) &&
        pdevc1->mask.id == pdevc2->mask.id;
}
static bool
gx_dc_pure_masked_equal(const gx_device_color * pdevc1,
                        const gx_device_color * pdevc2)
{
    return (*(&gx_dc_type_data_pure)->equal) (pdevc1, pdevc2) &&
        pdevc1->mask.id == pdevc2->mask.id;
}
static bool
gx_dc_binary_masked_equal(const gx_device_color * pdevc1,
                          const gx_device_color * pdevc2)
{
    return (*(&gx_dc_type_data_ht_binary)->equal) (pdevc1, pdevc2) &&
        pdevc1->mask.id == pdevc2->mask.id;
}
static bool
gx_dc_colored_masked_equal(const gx_device_color * pdevc1,
                           const gx_device_color * pdevc2)
{
    return (*(&gx_dc_type_data_ht_colored)->equal) (pdevc1, pdevc2) &&
        pdevc1->mask.id == pdevc2->mask.id;
}
                           ...
                           uint *psize, const gx_device *dev)
{
    ...
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b) {
        int l = ((((size_b - sizeof(gx_strip_bitmap)) - (offset1 - sizeof(gx_dc_serialized_tile_t) - sizeof(gx_strip_bitmap))) < (left)) ? ((size_b - sizeof(gx_strip_bitmap)) - (offset1 - sizeof(gx_dc_serialized_tile_t) - sizeof(gx_strip_bitmap))) : (left));
    ...
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b + size_c) {
        int l = (((size_c - sizeof(gx_strip_bitmap)) < (left)) ? (size_c - sizeof(gx_strip_bitmap)) : (left));
    ...
    uint * psize )
{
 ...
 if (ptile->ttrans != ((void*)0)) {
  ...
  return gx_dc_pattern_trans_write_raster(ptile, offset, data, psize);
    ...
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b) {
        l = (((left) < (size_b - (offset1 - sizeof(gx_dc_serialized_tile_t)))) ? (left) : (size_b - (offset1 - sizeof(gx_dc_serialized_tile_t))));
    ...
    if (left > 0) {
        l = (((left) < (size_c - (offset1 - sizeof(gx_dc_serialized_tile_t) - size_b))) ? (left) : (size_c - (offset1 - sizeof(gx_dc_serialized_tile_t) - size_b)));
                          ...
                          int64_t offset, const byte *data, uint size, gs_memory_t *mem)
{
    ...
    if (buf != ((void*)0)) {
        size_b = buf->size_b;
        size_c = buf->size_c;
        ptile->tbits.data = (*(mem)->procs.alloc_bytes)(mem, size_b - sizeof(gx_strip_bitmap), "gx_dc_pattern_read_raster");
        if (ptile->tbits.data == ((void*)0))
            return ((-25));
        if (size_c) {
            ptile->tmask.data = (*(mem)->procs.alloc_bytes)(mem, size_c - sizeof(gx_strip_bitmap), "gx_dc_pattern_read_raster");
            if (ptile->tmask.data == ((void*)0))
                return ((-25));
        } else
            ptile->tmask.data = ((void*)0);
        ptile->cdev = ((void*)0);
    } else {
        size_b = (*(mem)->procs.object_size)(mem, ptile->tbits.data) + sizeof(gx_strip_bitmap);
        size_c = ptile->tmask.data != ((void*)0) ? (*(mem)->procs.object_size)(mem, ptile->tmask.data) + sizeof(gx_strip_bitmap) : 0;
    }

    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + sizeof(gx_strip_bitmap)) {
        int l = (((sizeof(gx_strip_bitmap)) < (left)) ? (sizeof(gx_strip_bitmap)) : (left));
    ...
    if (left == 0)
        return size;
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b) {
        int l = (((sizeof(gx_dc_serialized_tile_t) + size_b - offset1) < (left)) ? (sizeof(gx_dc_serialized_tile_t) + size_b - offset1) : (left));
    ...
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b + sizeof(gx_strip_bitmap)) {
        int l = (((sizeof(gx_dc_serialized_tile_t) + size_b + sizeof(gx_strip_bitmap) - offset1) < (left)) ? (sizeof(gx_dc_serialized_tile_t) + size_b + sizeof(gx_strip_bitmap) - offset1) : (left));
    ...
    if (left == 0)
        return size;
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + size_b + size_c) {
        int l = (((sizeof(gx_dc_serialized_tile_t) + size_b + size_c - offset1) < (left)) ? (sizeof(gx_dc_serialized_tile_t) + size_b + size_c - offset1) : (left));
                              ...
                              const byte *data, uint size, gs_memory_t *mem)
{
    ...
    if (trans_pat->transbytes == ((void*)0)){
        trans_pat->transbytes = (*(mem)->procs.alloc_bytes)(mem, data_size, "gx_dc_pattern_read_raster");
        trans_pat->mem = mem;
        if (trans_pat->transbytes == ((void*)0))
                return ((-25));
    }
    ...
    if (offset1 <= sizeof(gx_dc_serialized_tile_t) + sizeof(tile_trans_clist_info_t) + data_size ) {
        ...
        int u = (((data_size) < (left)) ? (data_size) : (left));
    ...
    gs_memory_t * mem )
{
    ...
    if (offset == 0) {
        pdevc->mask.id = 0L;
        pdevc->mask.m_tile = ((void*)0);
        if (size == 0) {

            pdevc->type = &gx_dc_pattern;
            pdevc->colors.pattern.p_tile = ((void*)0);
            pdevc->mask.id = 0L;
            return 0;
        }
        ...
        code = gx_pattern_cache_get_entry((gs_imager_state *)pis,
                        buf.id, &ptile);
        if (code < 0)
            return code;
    ...
    do { } while (0);
        ...
        ptile->tbits.size.y = size_c;
        {
            ...
            code = ((&ptile->cdev->writer)->procs.open_device)((gx_device *)&ptile->cdev->writer);
            if (code < 0)
                return code;
        }
    } else {
        ...
        if (ptile->cdev == ((void*)0))
            return gx_dc_pattern_read_raster(ptile, ((void*)0), offset1, dp, left, mem);

        size_b = ptile->tbits.size.x;
        size_c = ptile->tbits.size.y;
    }
    if (offset1 <= sizeof(buf) + size_b) {
        l = (((left) < (size_b - (offset1 - sizeof(buf)))) ? (left) : (size_b - (offset1 - sizeof(buf))));
...
gx_dc_is_pattern1_color_with_trans(const gx_device_color *pdevc)
{
    ...
    return(gx_pattern1_get_transptr(pdevc) != ((void*)0));
}
