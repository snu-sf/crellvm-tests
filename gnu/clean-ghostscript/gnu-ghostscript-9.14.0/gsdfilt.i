...
static const gc_ptr_element_t gs_device_filter_stack_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_device_filter_stack_t *)0)->next - (char *)((gs_device_filter_stack_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_device_filter_stack_t *)0)->df - (char *)((gs_device_filter_stack_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_device_filter_stack_t *)0)->next_device - (char *)((gs_device_filter_stack_t *)0) )) } }; static const gc_struct_data_t gs_device_filter_stack_reloc_ptrs = { (sizeof(gs_device_filter_stack_enum_ptrs) / sizeof((gs_device_filter_stack_enum_ptrs)[0])), 0, 0, gs_device_filter_stack_enum_ptrs }; static const gs_memory_struct_type_t st_gs_device_filter_stack = { sizeof(gs_device_filter_stack_t), "gs_device_filter_stack", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &gs_device_filter_stack_reloc_ptrs };





const gs_memory_struct_type_t st_gs_device_filter = { sizeof(gs_device_filter_t), "gs_device_filter", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
gs_push_device_filter(gs_memory_t *mem, gs_state *pgs, gs_device_filter_t *df)
{
    ...
    gx_device *new_dev = ((void*)0);
    int code;

    dfs = (gs_device_filter_stack_t *)(*(mem)->procs.alloc_struct)(mem, &st_gs_device_filter_stack, "gs_push_device_filter");
    ...
    if (code < 0) {
        ((mem)->procs.free_object(mem, dfs, "gs_push_device_filter"));
        return code;
    }
    dfs->next = pgs->dfilter_stack;
    pgs->dfilter_stack = dfs;
    dfs->df = df;
    do { (dfs)->rc.ref_count = 1; (dfs)->rc.memory = mem; (dfs)->rc.free = rc_free_struct_only; do { } while (0); } while (0);
    gs_setdevice_no_init(pgs, new_dev);
    do { if (new_dev) { do { do { } while (0); (new_dev)->rc.ref_count += (-1); } while (0); if (!(new_dev)->rc.ref_count) { do { do { } while (0); (new_dev)->rc.free((new_dev)->rc.memory, (void *)(new_dev), "gs_push_device_filter"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
    return code;
}
...
gs_pop_device_filter(gs_memory_t *mem, gs_state *pgs)
{
    ...
    do { if (tos_device) do { (tos_device)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    gs_setdevice_no_init(pgs, dfs_tos->next_device);
    do { if (dfs_tos->next_device) { do { do { } while (0); (dfs_tos->next_device)->rc.ref_count += (-1); } while (0); if (!(dfs_tos->next_device)->rc.ref_count) { do { do { } while (0); (dfs_tos->next_device)->rc.free((dfs_tos->next_device)->rc.memory, (void *)(dfs_tos->next_device), "gs_pop_device_filter"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
    dfs_tos->df = ((void*)0);
    do { if (dfs_tos) { do { do { } while (0); (dfs_tos)->rc.ref_count += (-1); } while (0); if (!(dfs_tos)->rc.ref_count) { do { do { } while (0); (dfs_tos)->rc.free((dfs_tos)->rc.memory, (void *)(dfs_tos), "gs_pop_device_filter"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
    code = df->postpop(df, mem, pgs, tos_device);
    do { if (tos_device) { do { do { } while (0); (tos_device)->rc.ref_count += (-1); } while (0); if (!(tos_device)->rc.ref_count) { do { do { } while (0); (tos_device)->rc.free((tos_device)->rc.memory, (void *)(tos_device), "gs_pop_device_filter"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
    return code;
}
...
gs_clear_device_filters(gs_memory_t *mem, gs_state *pgs)
{
    ...
    while (pgs->dfilter_stack != ((void*)0)) {
