...
const gx_io_device gs_iodev_stdin =
    { "%stdin%", iodev_dtype_stdio, { stdin_init, stdin_open, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, ((void*)0), ((void*)0), iodev_no_get_params, iodev_no_put_params }};


static int stdout_open(gx_io_device *iodev, const char *access, stream **ps, gs_memory_t *mem);
const gx_io_device gs_iodev_stdout =
    { "%stdout%", iodev_dtype_stdio, { iodev_no_init, stdout_open, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, ((void*)0), ((void*)0), iodev_no_get_params, iodev_no_put_params }};


static int stderr_open(gx_io_device *iodev, const char *access, stream **ps, gs_memory_t *mem);
const gx_io_device gs_iodev_stderr =
    { "%stderr%", iodev_dtype_stdio, { iodev_no_init, stderr_open, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, ((void*)0), ((void*)0), iodev_no_get_params, iodev_no_put_params }};
# 76 "./psi/ziodevsc.c"
static int
...
{
    mem->gs_lib_ctx->stdin_is_interactive = ((bool)1);
    return 0;
}
                     ...
                     stream_cursor_write * pw, bool last)
{
    ...
    return ((count < 0) ? ((int)(-2)) : (count == 0) ? ((int)(-1)) : count);
}
           ...
           gs_memory_t * mem)
{
    ...
    if (!((access)[0] == ('r') && (access)[1] == 0))
        return ((-9));
    if ((s = (&(i_ctx_p->stdio)[0])->value.pfile, (s->read_id | s->write_id) != ((&(i_ctx_p->stdio)[0])->tas.rsize))) {

        gs_memory_t *sysmem = ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system));
        byte *buf;
        ...
        buf = (*(sysmem)->procs.alloc_bytes)(sysmem, 1024, "stdin_open(buffer)");
        if (s == 0 || buf == 0)
            return ((-25));

        s_std_init(s, buf, 1024, &p, 1);
        s->file = 0;
        s->file_modes = s->modes;
        s->file_offset = 0;
        s->file_limit = (~(-1L << ((1 << 3) * 8 - 1)));
        s->save_close = s_std_null;
        ((&(i_ctx_p->stdio)[0])->value.pfile = (s), ((&(i_ctx_p->stdio)[0])->tas.type_attrs = ((t_file) << 8) + ((0x20 + 0x40) | avm_system)), ((&(i_ctx_p->stdio)[0])->tas.rsize = (s->read_id)));
        *ps = s;
        return 1;
    }
    *ps = s;
...
zget_stdin(i_ctx_t *i_ctx_p, stream ** ps)
{
    ...
    iodev = gs_findiodevice(((gs_memory_t *)((i_ctx_p->memory).current)), (const byte *)"%stdin", 6);
    iodev->state = i_ctx_p;
    code = (*iodev->procs.open_device)(iodev, "r", ps, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)));
    iodev->state = ((void*)0);
...
{
    return (((s)->modes != 0) && s->procs.process == s_stdin_read_process);
}
            ...
            gs_memory_t * mem)
{
    ...
    if (!((access)[0] == ('w') && (access)[1] == 0))
        return ((-9));
    if ((s = (&(i_ctx_p->stdio)[1])->value.pfile, (s->read_id | s->write_id) != ((&(i_ctx_p->stdio)[1])->tas.rsize))) {
        gs_memory_t *sysmem = ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system));
        byte *buf;
        static const stream_procs p = {
        ...
        buf = (*(sysmem)->procs.alloc_bytes)(sysmem, 128, "stdout_open(buffer)");
        if (s == 0 || buf == 0)
            return ((-25));
        s_std_init(s, buf, 128, &p, 2);
        s->file = 0;
        s->file_modes = s->modes;
        s->file_offset = 0;
        s->file_limit = (~(-1L << ((1 << 3) * 8 - 1)));
        s->save_close = s->procs.flush;
        ((&(i_ctx_p->stdio)[1])->value.pfile = (s), ((&(i_ctx_p->stdio)[1])->tas.type_attrs = ((t_file) << 8) + (0x10 | avm_system)), ((&(i_ctx_p->stdio)[1])->tas.rsize = (s->write_id)));
        *ps = s;
        return 1;
    }
    *ps = s;
...
zget_stdout(i_ctx_t *i_ctx_p, stream ** ps)
{
    ...
    iodev = gs_findiodevice(((gs_memory_t *)((i_ctx_p->memory).current)), (const byte *)"%stdout", 7);
    iodev->state = i_ctx_p;
    code = (*iodev->procs.open_device)(iodev, "w", ps, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)));
    iodev->state = ((void*)0);
    return (((code) < (0)) ? (code) : (0));
}
            ...
            gs_memory_t * mem)
{
    ...
    if (!((access)[0] == ('w') && (access)[1] == 0))
        return ((-9));
    if ((s = (&(i_ctx_p->stdio)[2])->value.pfile, (s->read_id | s->write_id) != ((&(i_ctx_p->stdio)[2])->tas.rsize))) {
        gs_memory_t *sysmem = ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system));
        byte *buf;
        static const stream_procs p = {
            s_std_noavailable, s_std_noseek, s_std_write_reset,
        ...
        buf = (*(sysmem)->procs.alloc_bytes)(sysmem, 128, "stderr_open(buffer)");
        if (s == 0 || buf == 0)
            return ((-25));
        s_std_init(s, buf, 128, &p, 2);
        s->file = 0;
        ...
        s->file_limit = (~(-1L << ((1 << 3) * 8 - 1)));
        s->save_close = s->procs.flush;
        ((&(i_ctx_p->stdio)[2])->value.pfile = (s), ((&(i_ctx_p->stdio)[2])->tas.type_attrs = ((t_file) << 8) + (0x10 | avm_system)), ((&(i_ctx_p->stdio)[2])->tas.rsize = (s->write_id)));
        *ps = s;
        return 1;
    }
    *ps = s;
...
zget_stderr(i_ctx_t *i_ctx_p, stream ** ps)
{
    ...
    iodev = gs_findiodevice(((gs_memory_t *)((i_ctx_p->memory).current)), (const byte *)"%stderr", 7);
    iodev->state = i_ctx_p;
    code = (*iodev->procs.open_device)(iodev, "w", ps, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)));
    iodev->state = ((void*)0);
    return (((code) < (0)) ? (code) : (0));
