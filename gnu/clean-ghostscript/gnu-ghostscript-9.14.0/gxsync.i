...
)
{
    ...
    if (gp_semaphore_open(0) == 0)
        ...
        sema = (gx_semaphore_t *) (*(memory)->procs.alloc_bytes)(memory, semaSizeof, "gx_semaphore (create)");

    else

        sema = (gx_semaphore_t *) ((memory)->procs.alloc_bytes_immovable(memory, semaSizeof, "gx_semaphore (create)"));
    ...
    if (gp_semaphore_open(&sema->native) < 0) {
        ((memory)->procs.free_object(memory, sema, "gx_semaphore (alloc)"));
        return 0;
    }
    return sema;
}
...
)
{
    if (sema) {
        ...
        ((sema->memory)->procs.free_object(sema->memory, sema, "gx_semaphore (free)"));
    }
}
# 88 "./base/gxsync.c"
gx_monitor_t *
...
)
{
    ...
    if (gp_monitor_open(0) == 0)
        ...
        mon = (gx_monitor_t *) (*(memory)->procs.alloc_bytes)(memory, monSizeof, "gx_monitor (create)");

    else

        mon = (gx_monitor_t *) ((memory)->procs.alloc_bytes_immovable(memory, monSizeof, "gx_monitor (create)"));
    ...
    if (gp_monitor_open(&mon->native) < 0) {
        ((memory)->procs.free_object(memory, mon, "gx_monitor (alloc)"));
        return 0;
    }
    return mon;
}
...
)
{
    if (mon) {
        ...
        ((mon->memory)->procs.free_object(mon->memory, mon, "gx_monitor (free)"));
    }
}
