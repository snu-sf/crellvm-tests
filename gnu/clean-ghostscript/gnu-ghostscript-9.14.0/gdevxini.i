# 1 "./devices/gdevxini.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 318 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/gdevxini.c" 2
# 18 "./devices/gdevxini.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
gdev_x_open(gx_device_X * xdev)
{
    ...
    int code;
# 108 "./devices/gdevxini.c"
    if (!(xdev->dpy = XOpenDisplay((char *)((void*)0)))) {
        char *dispname = getenv("DISPLAY");

        (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Cannot open X display `%s'.\n", (dispname == ((void*)0) ? "(null)" : dispname)));
    ...
    if ((window_id = getenv("GHOSTVIEW"))) {
        if (!(xdev->ghostview = sscanf(window_id, "%ld %ld",
                                       &(xdev->win), &(xdev->dest)))) {
            (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Cannot get Window ID from ghostview.\n"));
            return ((-12));
        }
    }
    if (xdev->pwin != (Window) 0L) {
        ...
        char *buf;
        Atom ghostview_atom = XInternAtom(xdev->dpy, "GHOSTVIEW", 0);
        ...
        if (XGetWindowProperty(xdev->dpy, xdev->win, ghostview_atom, 0,
            ...
            if (!(nitems == 8 || nitems == 12)) {
                (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Cannot get ghostview property.\n"));
                return ((-12));
            }
            if (xdev->dest && xdev->bpixmap) {
                (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Both destination and backing pixmap specified.\n"));
            ...
            xppp = xdev->HWResolution[0] / 72.0;
            yppp = xdev->HWResolution[1] / 72.0;
            ...
            xdev->ImagingBBox_set = ((bool)1);

        } else if (xdev->pwin == (Window) 0L) {
            (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Cannot get ghostview property.\n"));
            return ((-12));
        }
    } else {
        Screen *scr = (&((_XPrivDisplay)xdev->dpy)->screens[(((_XPrivDisplay)xdev->dpy)->default_screen)]);

        xdev->scr = scr;
        xvinfo.visual = ((scr)->root_visual);
        xdev->cmap = ((scr)->cmap);
        if (xvinfo.visual->class != 4) {
            int scrno = (((_XPrivDisplay)xdev->dpy)->default_screen);
            if ( XMatchVisualInfo(xdev->dpy, scrno, 24, 4, &xvinfo) ||
                 XMatchVisualInfo(xdev->dpy, scrno, 32, 4, &xvinfo) ||
                 XMatchVisualInfo(xdev->dpy, scrno, 16, 4, &xvinfo) ||
                 XMatchVisualInfo(xdev->dpy, scrno, 15, 4, &xvinfo) ) {
                xdev->cmap = XCreateColormap (xdev->dpy,
                                              ((&((_XPrivDisplay)xdev->dpy)->screens[(((_XPrivDisplay)xdev->dpy)->default_screen)])->root),
                                              xvinfo.visual, 0 );
            }
        }
    }
    xvinfo.visualid = XVisualIDFromVisual(xvinfo.visual);
    xdev->vinfo = XGetVisualInfo(xdev->dpy, 0x1, &xvinfo, &nitems);
    if (xdev->vinfo == ((void*)0)) {
        (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Cannot get XVisualInfo.\n"));
        return ((-12));
    }
    ...
    dpy = XtOpenDisplay(app_con, ((void*)0), "ghostscript", "Ghostscript",
                        ((void*)0), 0, &zero, ((void*)0));
    toplevel = XtAppCreateShell(((void*)0), "Ghostscript",
                                applicationShellWidgetClass, dpy, ((void*)0), 0);
    ...
    XQueryColor(xdev->dpy, ((&((_XPrivDisplay)xdev->dpy)->screens[(((_XPrivDisplay)xdev->dpy)->default_screen)])->cmap), &xc);
    ...
    if (!xdev->ghostview) {
        ...
        if (xdev->HWResolution[0] == (16*72) ||
            xdev->HWResolution[1] == (16*72)) {
            float xsize = (float)xdev->width / xdev->HWResolution[0];
            float ysize = (float)xdev->height / xdev->HWResolution[1];
            int workarea_width = ((xdev->scr)->width), workarea_height = ((xdev->scr)->height);
            ...
            if (xdev->xResolution == 0.0 && xdev->yResolution == 0.0) {
                ...
                xdpi = 25.4 * ((xdev->scr)->width) /
                    ((xdev->scr)->mwidth);
                ydpi = 25.4 * ((xdev->scr)->height) /
                    ((xdev->scr)->mheight);
                dpi = (((xdpi) < (ydpi)) ? (xdpi) : (ydpi));
# 340 "./devices/gdevxini.c"
                if (dpi < 30)
                    dpi = 75;
                else {
                    while (xsize * dpi > ((xdev->scr)->width) - 32 ||
                           ysize * dpi > ((xdev->scr)->height) - 32)
                        dpi *= 0.95;
                }
                xdev->HWResolution[0] = dpi;
                xdev->HWResolution[1] = dpi;
            } else {
                xdev->HWResolution[0] = xdev->xResolution;
                xdev->HWResolution[1] = xdev->yResolution;
            }

            if (xdev->width > workarea_width) {
                xdev->width = (((xsize * xdev->HWResolution[0]) < (workarea_width)) ? (xsize * xdev->HWResolution[0]) : (workarea_width));
            }
            if (xdev->height > workarea_height) {
                xdev->height = (((ysize * xdev->HWResolution[1]) < (workarea_height)) ? (ysize * xdev->HWResolution[1]) : (workarea_height));
            }
            xdev->MediaSize[0] =
                (float)xdev->width / xdev->HWResolution[0] * 72;
            xdev->MediaSize[1] =
                (float)xdev->height / xdev->HWResolution[1] * 72;
        }
        sizehints.x = 0;
        sizehints.y = 0;
        ...
        if (xdev->geometry != ((void*)0)) {
            ...
            bitmask = XWMGeometry(xdev->dpy, (((_XPrivDisplay)xdev->dpy)->default_screen),
...
# 452 "./devices/gdevxini.c"
    ...
    xdev->gc = XCreateGC(xdev->dpy, xdev->win, 0, (XGCValues *) ((void*)0));
    XSetFunction(xdev->dpy, xdev->gc, 0x3);
    ...
    if (!xdev->ghostview) {
        ...
        XSetGraphicsExposures(xdev->dpy, xdev->gc, 0);
        XSelectInput(xdev->dpy, xdev->win, 0L);
    } else {
        ...
        xdev->mwin = XCreateWindow(xdev->dpy, ((xdev->scr)->root),
                                   0, 0, 1, 1, 0, 0L,
                                   0L, 0L,
                                   (1L<<9), &attributes);
        xdev->NEXT = XInternAtom(xdev->dpy, "NEXT", 0);
        xdev->PAGE = XInternAtom(xdev->dpy, "PAGE", 0);
        xdev->DONE = XInternAtom(xdev->dpy, "DONE", 0);
    }
...
x_get_work_area(gx_device_X *xdev, int *pwidth, int *pheight)
{
    ...
    if ((area = x_get_win_property(xdev, "_NET_WORKAREA")) != ((void*)0) ||
            (area = x_get_win_property(xdev, "_WIN_WORKAREA")) != ((void*)0)) {
...
x_get_win_property(gx_device_X *xdev, const char *atom_name)
{
    ...
    if (XGetWindowProperty(xdev->dpy, ((xdev->scr)->root),
                           XInternAtom(xdev->dpy, atom_name, 0),
                           0, 4, 0, ((Atom) 6),
                           &r_type, &r_format,
                           &count, &bytes_remain, &prop) == 0 &&
                           prop &&
                           r_type == ((Atom) 6) &&
                           r_format == 32 &&
                           count == 4 &&
...
x_set_buffer(gx_device_X * xdev)
{
    ...
    gs_memory_t *mem = ((xdev->memory)->procs.stable(xdev->memory));
    ...
    if (buffered) {
        ...
        if (mdev == 0 || mdev->color_info.depth != xdev->color_info.depth) {
            ...
            if (mdev) {
                ...
                gx_device_set_target((gx_device_forward *)mdev, ((void*)0));
            } else {
                mdev = (gx_device_memory *)(*(mem)->procs.alloc_struct)(mem, &st_device_memory, "memory device");
        ...
        if (mdev->width != xdev->width || mdev->height != xdev->height) {
            ...
            buffer =
                (xdev->buffer ?
                 (byte *)((mem)->procs.resize_object(mem, xdev->buffer, space, "buffer")) :
                 (*(mem)->procs.alloc_bytes)(mem, space, "buffer"));
            if (!buffer) {
                buffered = ((bool)0);
                goto setup;
            }
            ...
            do { if (xdev->icc_struct) do { (xdev->icc_struct)->rc.ref_count++; do { } while (0); } while (0); } while (0);
        ...
        ((mem)->procs.free_object(mem, xdev->buffer, "buffer"));
        xdev->buffer = 0;
        xdev->buffer_size = 0;
        if (!xdev->is_buffered)
            return 0;
        gx_device_set_target((gx_device_forward *)xdev->target, ((void*)0));
        gx_device_set_target((gx_device_forward *)xdev, ((void*)0));
        xdev->is_buffered = ((bool)0);
        procs = &gs_x11_device.procs;
    }
    if (((xdev)->procs.fill_rectangle) != procs->fill_rectangle) {

        ((xdev)->procs.fill_rectangle = (procs->fill_rectangle));
        ((xdev)->procs.copy_mono = (procs->copy_mono));
        ((xdev)->procs.copy_color = (procs->copy_color));
        ((xdev)->procs.copy_alpha = (procs->copy_alpha));
        ((xdev)->procs.fill_path = (procs->fill_path));
        ((xdev)->procs.stroke_path = (procs->stroke_path));
        ((xdev)->procs.fill_mask = (procs->fill_mask));
        ((xdev)->procs.fill_trapezoid = (procs->fill_trapezoid));
        ((xdev)->procs.fill_parallelogram = (procs->fill_parallelogram));
        ((xdev)->procs.fill_triangle = (procs->fill_triangle));
        ((xdev)->procs.draw_thin_line = (procs->draw_thin_line));
        ((xdev)->procs.strip_tile_rectangle = (procs->strip_tile_rectangle));
        ((xdev)->procs.strip_copy_rop = (procs->strip_copy_rop));
        ((xdev)->procs.begin_typed_image = (procs->begin_typed_image));
        ((xdev)->procs.text_begin = (procs->text_begin));
...
gdev_x_finish_copydevice(gx_device *dev, const gx_device *from_dev)
{
    ...
    ((xdev)->procs.fill_rectangle = (((&gs_x11_device)->procs.fill_rectangle)));
...
gdev_x_put_params(gx_device * dev, gs_param_list * plist)
{
    ...
    if (xdev->is_open && !xdev->ghostview &&
        ...
        int area_width = ((xdev->scr)->width), area_height = ((xdev->scr)->height);
        int dw, dh;
        ...
        dev->width = (((dev->width) < (area_width)) ? (dev->width) : (area_width));
        dev->height = (((dev->height) < (area_height)) ? (dev->height) : (area_height));

        if (dev->width <= 0 || dev->height <= 0) {
            (emprintf_program_ident(dev->memory, gs_program_name(), gs_revision_number()), errprintf(dev->memory, "Requested pagesize %d x %d not supported by %s device\n", dev->width, dev->height, dev->dname));

            return ((-15));
        }


        dev->MediaSize[0] = (float)dev->width / xdev->HWResolution[0] * 72;
        dev->MediaSize[1] = (float)dev->height / xdev->HWResolution[1] * 72;
