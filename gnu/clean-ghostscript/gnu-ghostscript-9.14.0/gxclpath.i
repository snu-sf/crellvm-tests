                             ...
                             const gx_drawing_color *pdcolor)
{
    ...
    if (((cldev)->procs.dev_spec_op)((gx_device *)cldev,
                                     gxdso_is_std_cmyk_1bit, ((void*)0), 0) <= 0)
    ...
    const gx_drawing_color *pdcolor)
{
    ...
    if (pdcolor != 0 && ((pdcolor)->type == (&gx_dc_type_data_pure))) {
                      ...
                      dc_devn_cl_type devn_type)
{
    ...
    if ( pdht != ((void*)0) &&
        ...
        if ((code = cmd_put_halftone(cldev, pdht)) < 0)
            return code;
        ((psdc)->type = (&gx_dc_type_data_none));
    }
# 143 "./base/gxclpath.c"
    ...
    if (code > 0 && devn_type == devn_not_tile)
        ...
        return code;
    if (!all_bands && dc_size * pre->rect_nbands > 1024*1024 )
        all_bands = ((bool)1);
    ...
    if (is_pattern) {
        ...
        if (pattern_id != 0L && pcls->pattern_id == pattern_id) {
    ...
    do {
        prefix_size = 2 + 1 + (offset > 0 ? ( (uint)(offset) < (1U << 7) ? 1 : (uint)(offset) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(offset) ) : 0);
        req_size = left + prefix_size + ( (uint)(left) < (1U << 7) ? 1 : (uint)(left) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(left) );
        code = cmd_get_buffer_space(cldev, pcls, req_size);
        if (code < 0)
            return code;
        buffer_space = (((code) < (req_size)) ? (code) : (req_size));
        portion_size = buffer_space - prefix_size - ( (uint)(left) < (1U << 7) ? 1 : (uint)(left) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(left) );
        req_size_final = portion_size + prefix_size + ( (uint)(portion_size) < (1U << 7) ? 1 : (uint)(portion_size) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(portion_size) );
        if (req_size_final > buffer_space)
            return ((-28));
        if (all_bands)
            code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, req_size_final)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_extend), 0) );
        else
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, req_size_final)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_extend), 0) );
        if (code < 0)
            return code;
        dp0 = dp;
        switch (devn_type) {
        ...
        *dp++ = di | (offset > 0 ? 0x80 : 0);
        if (offset > 0)
            do { if ((uint)(offset) < (1U << 7)) *(dp)++ = (byte)(offset); else if ((uint)(offset) < (1U << (2 * 7))) { *(dp)++ = (1U << 7) | ((offset) & ((1U << 7) - 1)); *(dp)++ = (offset) >> 7; } else (dp) = enc_u_put_uint((offset), (dp)); } while (0);
        do { if ((uint)(portion_size) < (1U << 7)) *(dp)++ = (byte)(portion_size); else if ((uint)(portion_size) < (1U << (2 * 7))) { *(dp)++ = (1U << 7) | ((portion_size) & ((1U << 7) - 1)); *(dp)++ = (portion_size) >> 7; } else (dp) = enc_u_put_uint((portion_size), (dp)); } while (0);
        code = pdcolor->type->write( pdcolor,
                                     &pcls->sdc,
                                     (gx_device *)cldev,
                                     offset,
    ...
    if (is_pattern) {
        ...
        if (pattern_id &&
            ...
            if (all_bands) {
                ...
                pcls->color_usage.trans_bbox.q.x = cldev->width;
                        ...
                        const gx_drawing_color * pdcolor)
{
    if (((pdcolor)->type == (&gx_dc_type_data_pure)))
        return gx_color_index2usage((gx_device *)cldev, ((pdcolor)->colors.pure));
    else if (((pdcolor)->type == (&gx_dc_type_data_ht_binary)))
        return gx_color_index2usage((gx_device *)cldev,
                                    gx_color_index2usage((gx_device *)cldev, ((pdcolor)->colors.binary.color[0])) |
                                    gx_color_index2usage((gx_device *)cldev, ((pdcolor)->colors.binary.color[1])));
    else if (((pdcolor)->type == (&gx_dc_type_data_ht_colored)))
        return gx_color_index2usage((gx_device *)cldev, colored_halftone_color_usage(cldev, pdcolor));
    else if (((pdcolor)->type == (&gx_dc_type_data_devn))) {
        ...
        return (((gx_color_usage_bits)1 << (cldev)->color_info.num_components) - 1);
}
                     ...
                     const gx_clip_path *pcpath, uint *punknown)
{
    ...
    if ((cdev->imager_state.line_params.curve_join != pis->line_params.curve_join) || (cdev->imager_state.accurate_curves != pis->accurate_curves) ||
        (cdev->imager_state.stroke_adjust != pis->stroke_adjust)
        ) {
        *punknown |= (1<<1);
        (cdev->imager_state.line_params.curve_join = pis->line_params.curve_join);
        (cdev->imager_state.accurate_curves = pis->accurate_curves);
        (cdev->imager_state.stroke_adjust = pis->stroke_adjust);
    }
    ...
    if ((cdev->imager_state.overprint != pis->overprint) || (cdev->imager_state.overprint_mode != pis->overprint_mode) ||
        (cdev->imager_state.blend_mode != pis->blend_mode) || (cdev->imager_state.text_knockout != pis->text_knockout) ||
                  ...
                  uint must_know)
{
    ...
    uint misc2_unknown = unknown & ((1<<10)-1);
    byte *dp;
    int code;
    ...
    if (misc2_unknown) {
        ...
        if (unknown & (1<<3)) {
            float width =
                ((&cldev->imager_state.line_params)->half_width * 2);
        ...
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1 + ((!((misc2_unknown) & ~0x7f)) ? 1 : (!((misc2_unknown) & ~0x3fff)) ? 2 : cmd_size_w((uint)(misc2_unknown))) + bp - buf)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_misc2), 0) );
    ...
    if (unknown & (1<<10)) {
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1 + sizeof(fixed) * 2)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_fill_adjust), 0) );
    ...
    if (unknown & (1<<11)) {
        int len = cmd_write_ctm_return_length(cldev, &(*(const gs_matrix *)&(&cldev->imager_state)->ctm));

        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, len + 1)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_ctm), 0) );
        if (code < 0)
            return code;
        code = cmd_write_ctm(&(*(const gs_matrix *)&(&cldev->imager_state)->ctm), dp, len);
        if (code < 0)
            return code;
        pcls->known |= (1<<11);
    }
    if (unknown & (1<<12)) {
        ...
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2 + (n + 2) * sizeof(float))) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_dash), 0) );
    ...
    if (unknown & (1<<13)) {
        ...
        int ymax = (((ymin + band_height) < (cldev->height)) ? (ymin + band_height) : (cldev->height));
        gs_fixed_rect box;
        bool punt_to_outer_box = ((bool)0);
        int code;

        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_begin_clip), 0) );
        if (code < 0)
            return code;
        if (pcpath->path_valid) {
            if ((gx_path_is_rectangular(&pcpath->path, &box) != prt_none) &&
                !((box.p.x | box.p.y | box.q.x | box.q.y)&((1<<8)-1))
                ) {

                code = cmd_write_rect_cmd(cldev, pcls, cmd_op_fill_rect,
                                          ((int)((box.p.x) >> (8))),
                                          ((int)((box.p.y) >> (8))),
                                          ((int)((box.q.x - box.p.x) >> (8))),
                                          ((int)((box.q.y - box.p.y) >> (8))));
            } else if ( !(cldev->disable_mask & (1 << 3)) ) {
        ...
        if (punt_to_outer_box) {
            ...
            box.p.x = ((box.p.x)&(-1L<<8));
            box.p.y = ((box.p.y)&(-1L<<8));
            code = cmd_write_rect_cmd(cldev, pcls, cmd_op_fill_rect,
                                      ((int)((box.p.x) >> (8))),
                                      ((int)((box.p.y) >> (8))),
                                      ((int)(((box.q.x - box.p.x)+((1<<8)-1)) >> (8))),
                                      ((int)(((box.q.y - box.p.y)+((1<<8)-1)) >> (8))));
        }
        {
            int end_code =
                ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_end_clip), 0) );
    ...
    if (unknown & (1<<14)) {
        ...
        if (cldev->color_space.byte1 & 8) {
            ...
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2 + ((!((hival) & ~0x7f)) ? 1 : (!((hival) & ~0x3fff)) ? 2 : cmd_size_w((uint)(hival))) + map_size + sizeof(clist_icc_color_t))) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_color_space), 0) );
            ...
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2 + sizeof(clist_icc_color_t))) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_color_space), 0) );
                ...
                const gx_clip_path * pcpath)
{
    ...
    byte op = (byte)
        (params->rule == 1 ?
         cmd_opv_eofill : cmd_opv_fill);
    gs_fixed_point adjust;
    bool slow_rop = cmd_slow_rop(dev, (( (((lop) & (rop3_1 - 0xcc)) << 2) | ((lop) & ~0xcc) ) & ~0x100), pdcolor);
    cmd_rects_enum_t re;
    int code;
    ...
    adjust = params->adjust;
    {
        ...
        ry = ((int)((bbox.p.y) >> (8))) - 1;
        rheight = ((int)(((bbox.q.y)+((1<<8)-1)) >> (8))) - ry + 1;
        ...
        if (rheight <= 0)
            return 0;
        rx = ((int)((bbox.p.x) >> (8))) - 1;
        rwidth = ((int)(((bbox.q.x)+((1<<8)-1)) >> (8))) - rx + 1;
        do { if ( rwidth > (cdev)->width - rx ) rwidth = (cdev)->width - rx; } while (0);
    }
    if ( (cdev->disable_mask & (1 << 0)) ||
         gs_debug_c(',')
         ) {
...
# 759 "./base/gxclpath.c"
    ...
    if (pdcolor == ((void*)0)) {
        ...
        cdev->cropping_saved = ((bool)1);
        cdev->save_cropping_min = cdev->cropping_min;
        cdev->save_cropping_max = cdev->cropping_max;
        cdev->cropping_min = (((ry) > (cdev->cropping_min)) ? (ry) : (cdev->cropping_min));
        cdev->cropping_max = (((ry + rheight) < (cdev->cropping_max)) ? (ry + rheight) : (cdev->cropping_max));
        do { } while (0);
        ...
        do {
            re.band = re.y / re.band_height; re.pcls = cdev->states + re.band; re.band_end = (re.band + 1) * re.band_height; re.height = (((re.band_end) < (re.yend)) ? (re.band_end) : (re.yend)) - re.y;;
            code = ( ~(re.pcls)->known & (((1<<1) | (1<<2) | (1<<5) | (1<<7) | (1<<8) | (1<<10) | (1<<9) | (1<<13))) ? cmd_write_unknown(cdev, re.pcls, ((1<<1) | (1<<2) | (1<<5) | (1<<7) | (1<<8) | (1<<10) | (1<<9) | (1<<13))) : 0 );
            if (code < 0)
                return code;
            if ((code = ( (re.pcls)->clip_enabled == ((pcpath != ((void*)0)) ^ 1) && cmd_put_enable_clip(cdev, re.pcls, pcpath != ((void*)0)) < 0 ? (cdev)->error_code : 0 )) < 0 ||
            ...
            code = cmd_put_path(cdev, re.pcls, ppath,
                                ((fixed)((((re.y - 1) > (y0)) ? (re.y - 1) : (y0)))<<8),
                                ((fixed)((((re.y + re.height + 1) < (y1)) ? (re.y + re.height + 1) : (y1)))<<8),
                                op,
                                ((bool)1), sn_none );
            if (code < 0)
                return code;
              ...
              const gx_drawing_color * pdcolor, const gx_clip_path * pcpath)
{
    ...
    if (expansion_code < 0) {
        ...
        adjust_y = ((int)(((expansion.y)+((1<<8)-1)) >> (8))) + 1;
        ry = ((int)((bbox.p.y) >> (8))) - adjust_y;
        rheight = ((int)(((bbox.q.y)+((1<<8)-1)) >> (8))) - ry + adjust_y;
        do { if ( ry < 0 ) rheight += ry, ry = 0; } while (0);
        do { if ( rheight > (dev)->height - ry ) rheight = (dev)->height - ry; } while (0);
        if (rheight <= 0)
            return 0;
    }
    ...
    if (cdev->imager_state.line_params.dash.pattern_size != pattern_size ||
        ...
        gx_set_dash(&cdev->imager_state.line_params.dash,
                    ...
                    pis->line_params.dash.offset, ((void*)0));
        cdev->imager_state.line_params.dash.pattern = 0;
        ((&cdev->imager_state.line_params.dash)->adapt = (pis->line_params.dash.adapt));
    ...
    if ((cdev->imager_state.line_params.start_cap != pis->line_params.start_cap) || (cdev->imager_state.line_params.join != pis->line_params.join) ||
        (cdev->imager_state.line_params.end_cap != pis->line_params.end_cap) || (cdev->imager_state.line_params.dash_cap != pis->line_params.dash_cap)) {
        unknown |= (1<<0);
        (cdev->imager_state.line_params.start_cap = pis->line_params.start_cap);
        (cdev->imager_state.line_params.end_cap = pis->line_params.end_cap);
        (cdev->imager_state.line_params.dash_cap = pis->line_params.dash_cap);
        (cdev->imager_state.line_params.join = pis->line_params.join);
    }
    cmd_check_fill_known(cdev, pis, params->flatness, &pis->fill_adjust,
                         pcpath, &unknown);
    if ((cdev->imager_state.line_params.half_width != pis->line_params.half_width)) {
        unknown |= (1<<3);
        (cdev->imager_state.line_params.half_width = pis->line_params.half_width);
    }
    if ((cdev->imager_state.line_params.miter_limit != pis->line_params.miter_limit)) {
        unknown |= (1<<4);
        gx_set_miter_limit(&cdev->imager_state.line_params,
                           pis->line_params.miter_limit);
    }
    if ((cdev->imager_state.ctm.xx != pis->ctm.xx) || (cdev->imager_state.ctm.xy != pis->ctm.xy) ||
        (cdev->imager_state.ctm.yx != pis->ctm.yx) || (cdev->imager_state.ctm.yy != pis->ctm.yy) ||


        (cdev->imager_state.ctm.tx != pis->ctm.tx) || (cdev->imager_state.ctm.ty != pis->ctm.ty)
        ) {
        unknown |= (1<<11);
    ...
    if (cdev->pdf14_needed) {
        gs_int_rect trans_bbox;
        int rx = ((int)((bbox.p.x) >> (8))) - 1;
        int rwidth = ((int)(((bbox.q.x)+((1<<8)-1)) >> (8))) - rx + 1;
    ...
    do {
        ...
        re.band = re.y / re.band_height; re.pcls = cdev->states + re.band; re.band_end = (re.band + 1) * re.band_height; re.height = (((re.band_end) < (re.yend)) ? (re.band_end) : (re.yend)) - re.y;;
        if ((code = ( ~(re.pcls)->known & (((1<<14)-1)) ? cmd_write_unknown(cdev, re.pcls, ((1<<14)-1)) : 0 )) < 0 ||
            (code = ( (re.pcls)->clip_enabled == ((pcpath != ((void*)0)) ^ 1) && cmd_put_enable_clip(cdev, re.pcls, pcpath != ((void*)0)) < 0 ? (cdev)->error_code : 0 )) < 0 ||
            (code = cmd_update_lop(cdev, re.pcls, lop)) < 0
            )
            return code;
        ...
        {
            fixed ymin, ymax;
# 1005 "./base/gxclpath.c"
            if (pattern_size || expansion_code < 0 ) {
                ymin = (-1 << ((1 << 2) * 8 - 1));
                ymax = (~(-1 << ((1 << 2) * 8 - 1)));
            } else {
                ymin = ((fixed)(re.y - adjust_y)<<8);
                ymax = ((fixed)(re.y + re.height + adjust_y)<<8);
            }
            code = cmd_put_path(cdev, re.pcls, ppath, ymin, ymax,
                                cmd_opv_stroke,
                                ((bool)0), (segment_notes)~0);
            if (code < 0)
                return code;
        }
        re.y += re.height;
                   ...
                   const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
    ...
    if ((code = gx_path_add_point(&path, px, py)) < 0 ||
        (code = gx_path_add_lines_notes(&path, points, num_points, sn_none)) < 0
        )
        goto out;
                         ...
                         gs_logical_operation_t lop)
{
    ...
    if (( ((ax) | (by)) == 0 || ((bx) | (ay)) == 0 )) {
        ...
        do { int px_ = ((int)((((px)+(((1<<8)>>1) - 1L))) >> (8))); int py_ = ((int)((((py)+(((1<<8)>>1) - 1L))) >> (8))); int qx_ = ((int)(((((px) + (ax) + (bx))+(((1<<8)>>1) - 1L))) >> (8))); int qy_ = ((int)(((((py) + (ay) + (by))+(((1<<8)>>1) - 1L))) >> (8))); if (qx_ < px_) (&r)->p.x = qx_, (&r)->q.x = px_; else (&r)->p.x = px_, (&r)->q.x = qx_; if (qy_ < py_) (&r)->p.y = qy_, (&r)->q.y = py_; else (&r)->p.y = py_, (&r)->q.y = qy_; } while (0);
        return (*(pdcolor)->type->fill_rectangle)(pdcolor, r.p.x, r.p.y, r.q.x - r.p.x, r.q.y - r.p.y, dev, lop, ((void*)0));
...
# 1185 "./base/gxclpath.c"
        ...
        if (operands[0] == 0)
            ...
            switch (*psw->dp) {
                case cmd_opv_rmoveto:
                    ...
                    op = cmd_opv_rmlineto;
                  merge:do { } while (0);
                    ((&(psw->pcls)->list)->tail->size -= (psw->len), (psw->cldev)->cnext -= (psw->len));
                    q += psw->len - 1;
                    break;
    ...
    for (; --i >= 0; ++optr) {
        ...
        if (!(((d) + ((fixed)1 << ((8 + 11) - 1))) & (-(fixed)1 << (8 + 11))) &&
            !(d & (((fixed)((0.25)*(float)(1<<8))) - 1))
            ) {
            do { } while (0);
            d = ((d >> (8 - 2)) & 0x1fff) + 0xc000;
            q += 2;
        } else if (!(((d) + ((fixed)1 << ((19) - 1))) & (-(fixed)1 << (19))) && i > 0 && !(((d2 = optr[1]) + ((fixed)1 << ((19) - 1))) & (-(fixed)1 << (19)))) {
            do { } while (0);
            q[1] = (byte) ((d >> 13) & 0x3f);
            q[2] = (byte) (d >> 5);
            ...
            --i, ++optr;
            continue;
        } else if (!(((d) + ((fixed)1 << ((22) - 1))) & (-(fixed)1 << (22)))) {
            do { } while (0);
            q[1] = (byte) (((d >> 16) & 0x3f) + 0x40);
            q += 3;
        } else if (!(((d) + ((fixed)1 << ((30) - 1))) & (-(fixed)1 << (30)))) {
            do { } while (0);
    ...
    if (notes != psw->notes) {
        byte *dp;
        int code =
            ( (dp = cmd_put_list_op(psw->cldev, &(psw->pcls)->list, 3)) == 0 ? (psw->cldev)->error_code : (*dp = (cmd_opv_set_misc2), 0) );
        ...
        int len = q + 2 - psw->cmd;
        byte *dp;
        int code = ( (dp = cmd_put_list_op(psw->cldev, &(psw->pcls)->list, len)) == 0 ? (psw->cldev)->error_code : (*dp = (op), 0) );
...
# 1310 "./base/gxclpath.c"
    byte initial_op = cmd_opv_end_run;
# 1332 "./base/gxclpath.c"
    gs_fixed_point start;
    int start_side = 0x7badf00d;





    bool start_skip = 0x7badf00d;
    ...
    segment_notes out_notes = 0x7badf00d;
    ...
    fixed px = ((fixed)(pcls->rect.x)<<8);
    fixed py = ((fixed)(pcls->rect.y)<<8);
    ...
    first.x = first.y = out.x = out.y = start.x = start.y = 0;
    do { } while (0);
    ...
    for (;;) {
        ...
        switch (pe_op) {
            case 0:
                ...
                pcls->rect.x = ((int)((px) >> (8)));
                pcls->rect.y = ((int)((py) >> (8)));
                do { } while (0);

                return ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (path_op), 0) );
            case 1:
                ...
                open = -1;
                start.x = vs[0], start.y = vs[1];
                start_skip = ((bool)0);
                if ((start_side = side = ((vs[1]) < ymin ? -1 : (vs[1]) >= ymax ? 1 : 0)) != 0) {
                    out.x = vs[0], out.y = vs[1];
                    do { } while (0);


                    continue;
                }
                vs[2] = vs[0] - px, vs[3] = vs[1] - py;
                first.x = px = vs[0], first.y = py = vs[1];
                code = cmd_put_segment(&writer, cmd_opv_rmoveto, &vs[2], sn_none);
                do { } while (0);

                break;
            case 5:
                {
                    int next_side = ((vs[1]) < ymin ? -1 : (vs[1]) >= ymax ? 1 : 0);
                    segment_notes notes =
                    ...
                    if (side && (px != out.x || py != out.y || (writer.dp == &initial_op))) {
                        vs[2] = out.x - px, vs[3] = out.y - py;
                        if (open < 0) {
                            first = out;
                            code = cmd_put_segment(&writer, cmd_opv_rmoveto, &vs[2], sn_none);
                        } else
                            code = cmd_put_segment(&writer, cmd_opv_rlineto, &vs[2], out_notes);
                        if (code < 0)
                            return code;
                        px = out.x, py = out.y;
                        do { } while (0);
                    ...
                    px = vs[0], py = vs[1];
                    open = 1;
                    code = cmd_put_segment(&writer, cmd_opv_rgapto, &vs[2], notes);
                }
                do { } while (0);
            ...
            case 2:
                {
                    ...
                    px = vs[0], py = vs[1];
                    open = 1;
                    code = cmd_put_segment(&writer, cmd_opv_rlineto, &vs[2], notes);
                }
                do { } while (0);

                break;
            case 4:
# 1531 "./base/gxclpath.c"
              close:if (side != start_side) {
                    ...
                    if (open > 0 && start_skip) {
                        vs[2] = start.x - px, vs[3] = start.y - py;
                        code = cmd_put_segment(&writer, cmd_opv_rlineto, &vs[2], sn_none);
                        if (code < 0)
                            return code;
...
# 1572 "./base/gxclpath.c"
                ...
                code = cmd_put_segment(&writer, cmd_opv_closepath, &vs[0], sn_none);
                do { } while (0);
                break;
            case 3:
                {
...

                    {
                        ...
                        if (all_side != 0) {
                            ...
                            out_side = ((vs[5]) < ymin ? -1 : (vs[5]) >= ymax ? 1 : 0);
                        ...
                        if (side && (px != out.x || py != out.y || (writer.dp == &initial_op))) {
                            ...
                            if (open < 0) {
                                first = out;
                                code = cmd_put_segment(&writer, cmd_opv_rmoveto, diff, sn_none);
                            } else
                                code = cmd_put_segment(&writer, cmd_opv_rlineto, diff, out_notes);
                            if (code < 0)
                                return code;
                            px = out.x, py = out.y;
                            do { } while (0);
                    ...
                    }
                    {
                        ...
                        if (*writer.dp >= cmd_opv_min_curveto &&
                            *writer.dp <= cmd_opv_max_curveto &&
                            ((prev.vs[0] == 0 &&
                              vs[0] == prev.vs[4] && vs[2] == prev.vs[2] && vs[4] == prev.vs[0] &&
                              vs[1] == -prev.vs[5] && vs[3] == -prev.vs[3] && vs[5] == -prev.vs[1]) ||
                             (prev.vs[0] != 0 &&
                              vs[0] == -prev.vs[4] && vs[2] == -prev.vs[2] && vs[4] == -prev.vs[0] &&
                              vs[1] == prev.vs[5] && vs[3] == prev.vs[3] && vs[5] == prev.vs[1]))
                            )
                            op = cmd_opv_scurveto;
                        else if (vs[1] == 0 && vs[4] == 0) {
                            vs[1] = vs[0], vs[4] = vs[5], optr++, op = cmd_opv_hvcurveto;
                            if ((vs[1] ^ vs[3]) >= 0) {
                                if (vs[2] == vs[3] && vs[4] == vs[1])
                                    op = cmd_opv_hqcurveto;
                            } else if (vs[2] == -vs[3] && vs[4] == -vs[1])
                                vs[2] = vs[3], op = cmd_opv_hqcurveto;
                        } else if (vs[0] == 0 && vs[5] == 0) {
                            optr++, op = cmd_opv_vhcurveto;
                            if ((vs[1] ^ vs[2]) >= 0) {
                                if (vs[3] == vs[2] && vs[4] == vs[1])
                                    op = cmd_opv_vqcurveto;
                            } else if (vs[3] == -vs[2] && vs[4] == -vs[1])
                                op = cmd_opv_vqcurveto;
                        } else if (vs[0] == 0 && vs[1] == 0)
                            optr += 2, op = cmd_opv_nrcurveto;
                        else if (vs[4] == 0 && vs[5] == 0)
