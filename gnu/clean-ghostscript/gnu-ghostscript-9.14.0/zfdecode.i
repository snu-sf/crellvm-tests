...
zA85D(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        ...
        if ((code = dict_bool_param(op, "PDFRules", ((bool)0), &ss.pdf_rules)) < 0)
            return code;
    } else {
        ss.pdf_rules = ((bool)0);
    }
...
zcf_setup(os_ptr op, stream_CF_state *pcfs, gs_ref_memory_t *imem)
{
    ...
    int code = dict_param_list_read(&list, op, ((void*)0), ((bool)0), imem);

    if (code < 0)
        return code;
    ((pcfs)->Uncompressed = ((bool)0), (pcfs)->K = 0, (pcfs)->EndOfLine = ((bool)0), (pcfs)->EncodedByteAlign = ((bool)0), (pcfs)->Columns = 1728, (pcfs)->Rows = 0, (pcfs)->EndOfBlock = ((bool)1), (pcfs)->BlackIs1 = ((bool)0), (pcfs)->DamagedRowsBeforeError = 0, (pcfs)->FirstBitLowOrder = ((bool)0), (pcfs)->DecodedByteAlign = 1, (pcfs)->lbuf = 0, (pcfs)->lprev = 0, (pcfs)->error_string[0] = 0);
...
zCFD(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        ...
        dop = 0;
    code = zcf_setup(dop, (stream_CF_state *)&cfs, ((i_ctx_p->memory).current));
    if (code < 0)
        return code;
...
zlz_setup(os_ptr op, stream_LZW_state * plzs)
{
    ...
    if ( (code = dict_int_param(dop, "EarlyChange", 0, 1, 1,
           ...
           (code = dict_bool_param(dop, "FirstBitLowOrder", ((bool)0),
                                   &plzs->FirstBitLowOrder)) < 0 ||
           (code = dict_bool_param(dop, "BlockData", ((bool)0),
                                   &plzs->BlockData)) < 0
        )
        return code;
    return 0;
...
zpd_setup(os_ptr op, stream_PDiff_state * ppds)
{
    ...
    do { if ( !(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if ((code = dict_int_param(op, "Colors", 1, 60, 1,
                               &ppds->Colors)) < 0 ||
        (code = dict_int_param(op, "BitsPerComponent", 1, 16, 8,
                               &bpc)) < 0 ||
        (bpc & (bpc - 1)) != 0 ||
        (code = dict_int_param(op, "Columns", 1, (~(-1 << ((1 << 2) * 8 - 1))), 1,
                               &ppds->Columns)) < 0
        )
...
zpp_setup(os_ptr op, stream_PNGP_state * ppps)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*op));
    do { if ( !(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if ((code = dict_int_param(op, "Colors", 1, 256, 1,
                               &ppps->Colors)) < 0 ||
        (code = dict_int_param(op, "BitsPerComponent", 1, 16, 8,
                               &bpc)) < 0 ||
        (bpc & (bpc - 1)) != 0 ||
        (code = dict_uint_param(op, "Columns", 1, ((unsigned int)~0 + (unsigned int)0), 1,
                                &ppps->Columns)) < 0 ||
