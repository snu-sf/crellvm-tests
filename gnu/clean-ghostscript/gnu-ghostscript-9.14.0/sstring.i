...
static const gs_memory_struct_type_t st_AXE_state = { sizeof(stream_AXE_state), "ASCIIHexEncode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_AXE_init(stream_state * st)
{
    ...
    return ((ss)->EndOfData = ((bool)1), (ss)->count = 0);
}
...
static const gs_memory_struct_type_t st_AXD_state = { sizeof(stream_AXD_state), "ASCIIHexDecode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_AXD_init(stream_state * st)
{
    ...
    return ((ss)->min_left = 1, (ss)->odd = -1, 0);
}
    ...
    switch (code) {
        ...
        case 1:
            ...
            for (; pr->ptr < pr->limit; pr->ptr++)
                if ((&scan_char_array[4])[pr->ptr[1]] != 102) {
                    if (pr->ptr[1] == '>') {
                        pr->ptr++;
                        goto eod;
                    }
               ...
               stream_cursor_write * pw, bool last)
{
    ...
    while (p < rlimit) {
        ...
        if (c < 32 || c >= 127) {
            ...
            if (c < 32 && c != 0 && (pesc = (__extension__ (__builtin_constant_p (c) && !__builtin_constant_p (esc) && (c) == '\0' ? (char *) __rawmemchr (esc, c) : __builtin_strchr (esc, c)))) != 0) {
                if (wlimit - q < 2) {
                    --p;
                    status = 1;
                    break;
...
{&st_stream_state, ((void*)0), s_PSSE_process, 1, 4
};



static const gs_memory_struct_type_t st_PSSD_state = { sizeof(stream_PSSD_state), "PSStringDecode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
        ...
        if (c == '\\' && !ss->from_string) {
            ...
            switch ((c = *++p)) {
                ...
                case 'f':
                    ...
                    goto put;
                default:
                  put:if ( q == wlimit ) { p -= 2; status = 1; goto out; };
                    *++q = c;
                    continue;
                case '\r':
                    if ( p == rlimit ) { p -= 2; goto out; };
                    if (p[1] == '\n')
                        p++;
                    continue;
                case '\n':
                ...
                case '7':
                    {
                        ...
                        if (d >= '0' && d <= '7') {
                            ...
                            if ( q == wlimit ) { p -= 2; status = 1; goto out; };
                            c = (c << 3) + d - '0';
                            d = p[2];
                            if (d >= '0' && d <= '7') {
                                c = (c << 3) + d - '0';
            ...
            switch (c) {
                case '(':
                    if ( q == wlimit ) { p -= 1; status = 1; goto out; };
                    ss->depth++;
                    break;
                case ')':
                    if (ss->depth == 0) {
