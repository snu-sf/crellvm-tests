...
{
    os_ptr sop = (((i_ctx_p->op_stack).stack).p);
    stream_PFBD_state state;

    if ( !((((const byte *)&((&*sop)->tas.type_attrs))[1]) == (t_boolean)) ) return (check_type_failed(&*sop));
...
{
    if (((((const byte *)&((dop)->tas.type_attrs))[1]) == (t_dictionary))) {
        int code;

        do { if ( !(((&*(&(&*dop)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        if ((code = dict_bool_param(dop, "EndOfData", ((bool)1), eod)) < 0)
            return code;
        return 1;
    } else {
        *eod = ((bool)1);
...
zSFD(i_ctx_t *i_ctx_p)
{
    ...
    if (((i_ctx_p->language_level) >= 3) && ((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        int count;
        int code;
        ...
        if ((code = dict_int_param(op, "EODCount", 0, (~(-1 << ((1 << 2) * 8 - 1))), 0, &count)) < 0)
            return code;
        ...
        if ( !((((const byte *)&((&sop[-1])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&sop[-1]));
        if (sop[-1].value.intval < 0)
            return ((-15));
        state.count = sop[-1].value.intval;
        npop = 2;
    }
    if ( !(((&*sop)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*sop)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*sop) : (-7)));
    state.eod.data = sop->value.const_bytes;
    state.eod.size = ((sop)->tas.rsize);
    return filter_read(i_ctx_p, npop, &s_SFD_template,
                       (stream_state *)&state, (avm_space)(((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    uint min_size = templat->min_out_size + 1;
    uint save_space = (((&(i_ctx_p->memory)))->current_space);

    uint use_space = (((space) > (avm_system)) ? (space) : (avm_system));
    os_ptr sop = op - npop;
    stream *s;
    stream *sstrm;
    bool close = ((bool)0);
    ...
    if (((((const byte *)&((sop)->tas.type_attrs))[1]) == (t_dictionary))) {
        do { if ( !(((&*(&(&*sop)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        if ((code = dict_bool_param(sop, "CloseSource", ((bool)0), &close)) < 0)
            return code;
        --sop;
    }
    ...
    use_space = (((use_space) > ((avm_space)(((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2)))) ? (use_space) : ((avm_space)(((sop)->tas.type_attrs) & (((1 << 2) - 1) << 2))));
    switch ((((const byte *)&((sop)->tas.type_attrs))[1])) {
        case t_string:
            do { if ( !(((&*sop)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            sstrm = file_alloc_stream(((gs_memory_t *)((i_ctx_p->memory).current)), "filter_read(string stream)");
            if (sstrm == 0) {
                code = ((-25));
                goto out;
            }
            sread_string(sstrm, sop->value.bytes, ((sop)->tas.rsize));
            sstrm->is_temp = 1;
            break;
        case t_file:
            do { sstrm = (sop)->value.pfile; if (sstrm->read_id != ((sop)->tas.rsize)) { if (sstrm->read_id == 0 && sstrm->write_id == ((sop)->tas.rsize)) { int fcode = file_switch_to_read(sop); if (fcode < 0) return(fcode); } else { do { } while (0); } } } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            goto ens;
        default:
            do { if ( !(((&*sop)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*sop)); } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            code = sread_proc(sop, &sstrm, ((i_ctx_p->memory).current));
            if (code < 0)
                goto out;
            sstrm->is_temp = 2;
          ens:
            code = filter_ensure_buf(&sstrm,
                                     ...
                                     ((i_ctx_p->memory).current), ((bool)0), close);
            if (code < 0)
                goto out;
            break;
    }
    ...
    code = filter_open("r", min_size, (ref *) sop,
                       &s_filter_read_procs, templat, st, ((gs_memory_t *)((i_ctx_p->memory).current)));
    if (code < 0)
        goto out;
    s = (sop)->value.pfile;
    s->strm = sstrm;
    s->close_strm = close;
    ((((i_ctx_p->op_stack).stack).p) -= (op - sop));
out:
...
{
    return filter_read(i_ctx_p, 0, templat, ((void*)0), 0);
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    uint min_size = templat->min_in_size + 1;
    uint save_space = (((&(i_ctx_p->memory)))->current_space);
    ...
    if (((((const byte *)&((sop)->tas.type_attrs))[1]) == (t_dictionary))) {
        do { if ( !(((&*(&(&*sop)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        if ((code = dict_bool_param(sop, "CloseTarget", ((bool)0), &close)) < 0)
            return code;
        --sop;
    }
    ...
    switch ((((const byte *)&((sop)->tas.type_attrs))[1])) {
        case t_string:
            do { if ( !(((&*sop)->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            sstrm = file_alloc_stream(((gs_memory_t *)((i_ctx_p->memory).current)), "filter_write(string)");
            if (sstrm == 0) {
                code = ((-25));
                goto out;
            }
            swrite_string(sstrm, sop->value.bytes, ((sop)->tas.rsize));
            sstrm->is_temp = 1;
            break;
        case t_file:
            do { sstrm = (sop)->value.pfile; if ( sstrm->write_id != ((sop)->tas.rsize) ) { int fcode = file_switch_to_write(sop); if ( fcode < 0 ) return(fcode); } } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            goto ens;
        default:
            do { if ( !(((&*sop)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*sop)); } while (0);
            ialloc_set_space((&(i_ctx_p->memory)), use_space);
            code = swrite_proc(sop, &sstrm, ((i_ctx_p->memory).current));
            if (code < 0)
                goto out;
            sstrm->is_temp = 2;
          ens:
            code = filter_ensure_buf(&sstrm,
                                     ...
                                     ((i_ctx_p->memory).current), ((bool)1), close);
            if (code < 0)
                goto out;
            break;
    }
    ...
    code = filter_open("w", min_size, (ref *) sop,
                       &s_filter_write_procs, templat, st, ((gs_memory_t *)((i_ctx_p->memory).current)));
    if (code < 0)
        goto out;
    s = (sop)->value.pfile;
    s->strm = sstrm;
...
{
    return filter_write(i_ctx_p, 0, templat, ((void*)0), 0);
}
...
static const stream_template s_Null1D_template = {
    &st_stream_state, ((void*)0), s_Null1D_process, 1, 1
};
...
static const stream_template s_EOFD_template = {
    &st_stream_state, ((void*)0), s_EOFD_process, 1, 1
};
                  ...
                  bool writing, int close)
{
    ...
    if (s->cbuf == 0) {
        ...
        uint len = (((min_size) > (128)) ? (min_size) : (128));
        byte *buf = (*((gs_memory_t *)imem)->procs.alloc_bytes)((gs_memory_t *)imem, len, "filter_ensure_buf");
        ...
        if (writing)
            code = filter_open("w", min_size, &bsop, &s_filter_write_procs,
                               &s_NullE_template, ((void*)0), (gs_memory_t *)imem);
        else
            code = filter_open("r", min_size, &bsop, &s_filter_read_procs,
                               &s_Null1D_template, ((void*)0), (gs_memory_t *)imem);
        if (code < 0)
            return code;
        bs = (&bsop)->value.pfile;
        bs->strm = s;
...
{
    (fop)->value.pfile->is_temp = is_temp;
}
