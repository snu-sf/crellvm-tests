...
{
    os_ptr optr, op = (((i_ctx_p->op_stack).stack).p);
    ...
    if ((code = real_param(optr + 1, &y1)) < 0)
        return code;
    if ((code = gs_moveto((i_ctx_p->pgs), x1, y1)) < 0)
        return code;
    if (ocount == 2)
          goto pop;
    ...
    if ((code = real_param(optr + 3, &y2)) < 0)
        return code;
    if (ocount == 4) {
        if((code = gs_lineto((i_ctx_p->pgs), x2, y2)) < 0)
            return code;
        goto pop;
    }
    x0 = 2*x1 - x2;
    ...
    for (i = 4; i <= ocount; i += 2) {
        ...
        len1 = sqrt((double)(x1 - x0)*(x1 - x0)+(double)(y1 - y0)*(y1 - y0));
        len2 = sqrt((double)(x2 - x1)*(x2 - x1)+(double)(y2 - y1)*(y2 - y1));
        len3 = sqrt((double)(x3 - x2)*(x3 - x2)+(double)(y3 - y2)*(y3 - y2));
        ...
        code = gs_curveto((i_ctx_p->pgs), ctrl1_x, ctrl1_y, ctrl2_x, ctrl2_y, x2, y2);
        if (code < 0)
            return code;
        x0 = x1, x1 = x2, x2 = x3;
        y0 = y1, y1 = y2, y2 = y3;
