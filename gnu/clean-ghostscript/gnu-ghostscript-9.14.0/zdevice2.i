...
zcurrentshowpagecount(i_ctx_t *i_ctx_p)
{
    ...
    if ((*((dev)->procs.get_page_device))(dev) == 0) {
        ...
        ((op - 1)->value.intval = (dev->ShowpageCount), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
...
zcurrentpagedevice(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( (op += (2)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (2); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    if ((*((dev)->procs.get_page_device))(dev) != 0) {
        op[-1] = ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->pagedevice;
        ((op)->value.boolval = (1), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    } else {
        (((op - 1)->tas.type_attrs = ((t_null) << 8) + (0)) );
        ((op)->value.boolval = (0), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    }
    return 0;
}
...
# 116 "./psi/zdevice2.c"
        ...
        if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_null)) ) return (check_type_failed(&*op));
    }
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->pagedevice = *op;
...
zcallinstall(i_ctx_t *i_ctx_p)
{
    ...
    if ((dev = (*((dev)->procs.get_page_device))(dev)) != 0) {
        int code = (*dev->page_procs.install) (dev, (i_ctx_p->pgs));
...
zcallbeginpage(i_ctx_t *i_ctx_p)
{
    ...
    if ((dev = (*((dev)->procs.get_page_device))(dev)) != 0) {
        int code = (*dev->page_procs.begin_page)(dev, (i_ctx_p->pgs));
...
zcallendpage(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op));
    if ((dev = (*((dev)->procs.get_page_device))(dev)) != 0) {
        code = (*dev->page_procs.end_page)(dev, (int)op->value.intval, (i_ctx_p->pgs));
        if (code < 0)
            return code;
        if (code > 1)
            return ((-15));
    ...
    ((op - 1)->value.boolval = (code), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
# 196 "./psi/zdevice2.c"
...
{
    return
        (((((const byte *)&((&((int_gstate *)gs_state_client_data(pgs))->pagedevice)->tas.type_attrs))[1]) == (t_null)) &&
         (*((gs_currentdevice(pgs))->procs.get_page_device))(gs_currentdevice(pgs)) != 0);
}
...
{
    if (!save_page_device((i_ctx_p->pgs)))
...
restore_page_device(const gs_state * pgs_old, const gs_state * pgs_new)
{
    ...
    gx_device *dev_t1;
    gx_device *dev_t2;
    bool samepagedevice = obj_eq(dev_old->memory, &((int_gstate *)gs_state_client_data(pgs_old))->pagedevice,
        &((int_gstate *)gs_state_client_data(pgs_new))->pagedevice);

    if ((dev_t1 = (*((dev_old)->procs.get_page_device)) (dev_old)) == 0)
        return ((bool)0);



    if (!samepagedevice)
        dev_old->LockSafetyParams = ((bool)0);
    dev_new = gs_currentdevice(pgs_new);
    if (dev_old != dev_new) {
        if ((dev_t2 = (*((dev_new)->procs.get_page_device)) (dev_new)) == 0)
            return ((bool)0);
        if (dev_t1 != dev_t2)
            return ((bool)1);
    }
...
{
    if (!restore_page_device((i_ctx_p->pgs), gs_state_saved((i_ctx_p->pgs))))
...
z2grestoreall(i_ctx_t *i_ctx_p)
{
    for (;;) {
        if (!restore_page_device((i_ctx_p->pgs), gs_state_saved((i_ctx_p->pgs)))) {
            bool done = !gs_state_saved(gs_state_saved((i_ctx_p->pgs)));
...
{
    while (gs_state_saved(gs_state_saved((i_ctx_p->pgs)))) {
        if (restore_page_device((i_ctx_p->pgs), gs_state_saved((i_ctx_p->pgs))))
...
z2setgstate(i_ctx_t *i_ctx_p)
{
    ...
    if (!restore_page_device((i_ctx_p->pgs), ((gs_state *)((&((igstate_obj *)((op)->value.pstruct))->gstate)->value.pstruct))))
...
push_callout(i_ctx_t *i_ctx_p, const char *callout_name)
{
    ...
    code = names_enter_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, callout_name, (((i_ctx_p->exec_stack).stack).p) + 1);
    if (code < 0)
        return code;
