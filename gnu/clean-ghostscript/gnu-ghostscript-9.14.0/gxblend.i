            ...
            for ( y = 0; y < num_rows; y++ ) {
               for ( x = 0; x < num_cols; x++ ){
                    ...
                    if (src[x + mask_alpha_offset] != 0x00) {
                        ...
                        temp = temp * (1.0 / 255.0 );
                        dstptr[x] = ( (0.0 < temp && temp < 1.0) ? ((unsigned char) (temp*255.0)) : ( (temp <= 0.0) ? 0x00 : 0xFF ));
                    }
                }
               dstptr += row_stride;
               mask_alpha_offset += row_stride;
...
# 218 "./base/gxblend.c"
    gsicc_init_buffer(&input_buff_desc, n_chan-1, 1,
                  ((bool)0), ((bool)0), ((bool)1), plane_stride, row_stride,
                  num_rows, num_cols);
    gsicc_init_buffer(&output_buff_desc, 1, 1,
                  ((bool)0), ((bool)0), ((bool)1), plane_stride,
                           ...
                           const byte *src)
{
    ...
    for (i = 1; i < n_chan; i++) {
        temp = src[i];
        mins = (((minb) < (temp)) ? (minb) : (temp));
        maxs = (((minb) > (temp)) ? (minb) : (temp));
    }
                  ...
                  const pdf14_nonseparable_blending_procs_t * pblend_procs)
{
    ...
    switch (blend_mode) {
        ...
        default:
            ...
            (dprintf_file_and_line("./base/gxblend.c", 732), errprintf_nomem("art_blend_pixel_8: blend mode %d not implemented\n", blend_mode));
                ...
                gs_blend_mode_t blend_mode)
{
    ...
    switch (blend_mode) {
        ...
        default:
            ...
            (dprintf_file_and_line("./base/gxblend.c", 859), errprintf_nomem("art_blend_pixel: blend mode %d not implemented\n", blend_mode));
