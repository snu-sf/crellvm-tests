# 1 "./devices/vector/gdevpdfr.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdfr.c" 2
# 18 "./devices/vector/gdevpdfr.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 55 "./base/memory_.h" 2
# 19 "./devices/vector/gdevpdfr.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
# 20 "./devices/vector/gdevpdfr.c" 2
...
# 22 "./devices/vector/gdevpdfr.c" 2
...
# 23 "./devices/vector/gdevpdfr.c" 2
...
# 24 "./devices/vector/gdevpdfr.c" 2
# 1 "./base/scanchar.h" 1
# 38 "./base/scanchar.h"
extern const byte scan_char_array[4 + 256];
# 25 "./devices/vector/gdevpdfr.c" 2
...
# 27 "./devices/vector/gdevpdfr.c" 2
...
{
    cos_object_t *pco;
    int code = pdf_create_named(pdev, pname, (&cos_dict_procs), &pco, id);
                ...
                cos_object_t **ppco)
{
    ...
    if (pdf_key_eq(pname, "{ThisPage}"))
        ...
        code = pdf_create_named(pdev, pname, (&cos_generic_procs), ppco, 0L);
        return (code < 0 ? code : 1);
    }
    if (page_number <= 0)
        return code;
    gs_sprintf(page_name_chars, "{Page%d}", page_number);
    ((pnstr).data = (const byte *)(page_name_chars), (pnstr).size = strlen((const char *)(pnstr).data), (pnstr).persistent = ((bool)1));
    pname = &pnstr;
 ...
 cpage:
    if (pdf_page_id(pdev, page_number) <= 0)
        return ((-15));
    *ppco = ((cos_object_t *)&((pdev->pages[page_number - 1].Page)->cos_procs));
    return 0;
}
...
{
    cos_object_t *pco;
    int code = pdf_make_named(pdev, pname, (&cos_dict_procs), &pco, assign_id);
...
pdf_push_namespace(gx_device_pdf *pdev)
{
    int code = cos_array_add_object(pdev->Namespace_stack,
                                    ((cos_object_t *)&((pdev->local_named_objects)->cos_procs)));
    cos_dict_t *pcd =
    ...
    if (code < 0 ||
        (code = cos_array_add_object(pdev->Namespace_stack,
                                     ((cos_object_t *)&((pdev->NI_stack)->cos_procs)))) < 0
        )
        return code;
    if (pcd == 0 || pca == 0)
        return ((-25));
...
pdf_pop_namespace(gx_device_pdf *pdev)
{
    ...
    if (code < 0 ||
        ...
        return code;
    cos_free(((cos_object_t *)&((pdev->local_named_objects)->cos_procs)), "pdf_pop_namespace(local_named_objects)");

    pdev->local_named_objects = (cos_dict_t *)lno_value.contents.object;
    cos_free(((cos_object_t *)&((pdev->NI_stack)->cos_procs)), "pdf_pop_namespace(NI_stack)");
    pdev->NI_stack = (cos_array_t *)nis_value.contents.object;
    return 0;
}
# 278 "./devices/vector/gdevpdfr.c"
int
pdf_scan_token(const byte **pscan, const byte * end, const byte **ptoken)
{
    const byte *p = *pscan;

    while (p < end && (&scan_char_array[4])[*p] == 102) {
        ++p;
...
m2: *pscan = p + 2;
    ...
    while (p < end && (&scan_char_array[4])[*p] <= 100)
        ++p;
    *pscan = p;
    if (p == *ptoken)
        return ((-15));
                  ...
                  gs_param_string * to)
{
    ...
    to->persistent = ((bool)1);
    if (!any) {
        to->data = start;
        to->size = size;
        return 0;
    }
    sto = (*(pdev->v_memory)->procs.alloc_bytes)(pdev->v_memory, size, "pdf_replace_names");
    if (sto == 0)
        return ((-25));
    to->data = sto;
    to->size = size;
