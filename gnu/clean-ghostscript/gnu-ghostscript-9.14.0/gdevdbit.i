                          ...
                          int px, int py)
{
    ...
    *(gx_tile_bitmap *) & tiles = *tile;
    ...
    tiles.num_planes = 1;
    return (*((dev)->procs.strip_tile_rectangle))
                     ...
                     gx_color_index zero, gx_color_index one)
{
    ...
    if (!data)
        return gs_throw_imp(__func__, "./base/gdevdbit.c", 58, 0, ((-1)), "%s", gs_errstr(((-1))));
    do { do { if ( (x | y) < 0 ) { if ( x < 0 ) w += x, dx -= x, x = 0; if ( y < 0 ) h += y, data -= (int)(y * raster), id = 0L, y = 0; } if ( w > (dev)->width - x ) w = (dev)->width - x; } while (0); if ( h > (dev)->height - y ) h = (dev)->height - y; if ( w <= 0 || h <= 0 ) return 0; } while (0);
    if (!data)
        return gs_throw_imp(__func__, "./base/gdevdbit.c", 61, 0, ((-1)), "%s", gs_errstr(((-1))));
    if (one != ((gx_color_index)(~0))) {
        invert = ((bool)0);
        color = one;
        if (zero != ((gx_color_index)(~0))) {
            int code = (*((dev)->procs.fill_rectangle))
            (dev, x, y, w, h, zero);
    ...
    if (!data)
        return gs_throw_imp(__func__, "./base/gdevdbit.c", 77, 0, ((-1)), "%s", gs_errstr(((-1))));
    ((&devc)->colors.pure = (color), (&devc)->type = (&gx_dc_type_data_pure)); (&devc)->ccolor_valid = ((bool)0);
    if (!data)
        return gs_throw_imp(__func__, "./base/gdevdbit.c", 80, 0, ((-1)), "%s", gs_errstr(((-1))));
    return gx_dc_default_fill_masked
        (&devc, data, dx, raster, id, x, y, w, h, dev, 0xf0, invert);
}
                      ...
                      int x, int y, int w, int h)
{
    ...
    if (depth == 1)
        return (*((dev)->procs.copy_mono)) (dev, data, dx, raster, id,
                                            x, y, w, h,
                                    (gx_color_index) 0, (gx_color_index) 1);
    do { do { if ( (x | y) < 0 ) { if ( x < 0 ) w += x, dx -= x, x = 0; if ( y < 0 ) h += y, data -= (int)(y * raster), id = 0L, y = 0; } if ( w > (dev)->width - x ) w = (dev)->width - x; } while (0); if ( h > (dev)->height - y ) h = (dev)->height - y; if ( w <= 0 || h <= 0 ) return 0; } while (0);
    fill = ((dev)->procs.fill_rectangle);
    mask = (byte) ((1 << depth) - 1);
    for (row = data, iy = 0; iy < h; row += raster, ++iy) {
        int ix;
        gx_color_index c0 = ((gx_color_index)(~0));
                      ...
                      const gx_drawing_color *pdcolor, int depth)
{
    ...
    uint out_raster;
    int code = 0;
    gx_color_value src_cv[(64)];
    gx_color_value curr_cv[(64)];
    gx_color_value blend_cv[(64)];
    int ry;
    int k, j;
    ...
    row_alpha = data;
    out_raster = ((uint)((((width * byte_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    gb_buff = (*(mem)->procs.alloc_bytes)(mem, out_raster * ncomps, "copy_alpha_hl_color(gb_buff)");
    ...
    for (ry = y; ry < y + height; row_alpha += raster, ++ry) {
        ...
        for (k = 0; k < ncomps; k++) {
            ...
            code = ((dev)->procs.get_bits_rectangle) (dev, &gb_rect,
                                                      &gb_params, 0);
            src_planes[k] = gb_params.data[k];
            if (code < 0) {
                ((mem)->procs.free_object(mem, gb_buff, "copy_alpha_hl_color"));
                return code;
            }
        }
        ...
        for (sx = data_x, rx = x; sx < data_x + width; ++sx, ++rx) {
            ...
            if (alpha == 0) {
                ...
                code = ((dev)->procs.copy_planes)(dev, &(gb_buff[gb_buff_start]),
                                                  0, out_raster, 0L,
        ...
        for (ry = y; ry < y + height; row += raster, ++ry) {
            ...
            for (sx = data_x, rx = x; sx < data_x + width; ++sx, ++rx) {
              ...
              blend:
                if (alpha == 0) {
# 404 "./base/gdevdbit.c"
                    { if ( (rx) > l_xprev ) { int code; if ( (l_dbit) != 0 ) *(l_dptr) = l_dbyte | (*(l_dptr) & (0xff >> (l_dbit)));; code = (*((dev)->procs.copy_color)) (dev, lout, l_xprev - (lx), out_size, 0L, l_xprev, ry, (rx) - l_xprev, 1); if ( code < 0 ) return code; }; l_dptr = (lout); l_dbit = (0); l_dbyte = ((l_dbit) ? (byte)(*(l_dptr) & (0xff00 >> (l_dbit))) : 0); l_xprev = rx+1; };
                    lx = rx+1;
                } else {
                    if (alpha == 15) {
                        composite = color;
                                ...
                                switch (bpp >> 3) {
                                    case 8:
                                        previous += (gx_color_index) * src++
                                            << ((56) >= 8 * sizeof(previous) ? (56) & (8 * sizeof(previous) - 1) : (56));
                                    case 7:
                                        previous += (gx_color_index) * src++
                                            << ((48) >= 8 * sizeof(previous) ? (48) & (8 * sizeof(previous) - 1) : (48));
                                    case 6:
                                        previous += (gx_color_index) * src++
                                            << ((40) >= 8 * sizeof(previous) ? (40) & (8 * sizeof(previous) - 1) : (40));
                                    case 5:
                                        previous += (gx_color_index) * src++
                                            << ((32) >= 8 * sizeof(previous) ? (32) & (8 * sizeof(previous) - 1) : (32));
                                    case 4:
                        ...
                        for (i=0; i<ncomps; i++)
                            cv[i] = (cv[i]) + (((int)(color_cv[i]) - (int)(cv[i])) * (alpha) / (15));


                        composite =
                            (*((dev)->procs.encode_color)) (dev, cv);
                        if (composite == ((gx_color_index)(~0))) {
            ...
            if ( (rx) > l_xprev ) { int code; if ( (l_dbit) != 0 ) *(l_dptr) = l_dbyte | (*(l_dptr) & (0xff >> (l_dbit)));; code = (*((dev)->procs.copy_color)) (dev, lout, l_xprev - (lx), out_size, 0L, l_xprev, ry, (rx) - l_xprev, 1); if ( code < 0 ) return code; };
        }
                     ...
                     gs_logical_operation_t lop, const gx_clip_path * pcpath)
{
    ...
    if (pcpath != 0)
    {
        ...
        rect.p.x = ((fixed)(x)<<8);
        rect.p.y = ((fixed)(y)<<8);
        rect.q.x = ((fixed)(x+w)<<8);
        rect.q.y = ((fixed)(y+h)<<8);
                                ...
                                int px, int py)
{
    ...
    if (rwidth == 0 || rheight == 0)
        return ((-28));
    do { if ( (x | y) < 0 ) { if ( x < 0 ) w += x, x = 0; if ( y < 0 ) h += y, y = 0; } } while (0);
# 597 "./base/gdevdbit.c"
    if (((dev)->procs.tile_rectangle) != gx_default_tile_rectangle) {
        if (shift == 0) {
            ...
            int (*tile_proc)(gx_device *dev, const gx_tile_bitmap *tile, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1, int phase_x, int phase_y) =
                ((dev)->procs.tile_rectangle);
            int code;

            ((dev)->procs.tile_rectangle = (gx_default_tile_rectangle));
            code = (*tile_proc)
                (dev, (const gx_tile_bitmap *)tiles, x, y, w, h,
                 color0, color1, px, py);
            ((dev)->procs.tile_rectangle = (tile_proc));
            return code;
        }
        ...
        int (*proc_planes)(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, int plane_height);
        int code;
        ...
        if (color0 == ((gx_color_index)(~0)) && color1 == ((gx_color_index)(~0))) {
            if (tiles->num_planes > 1) {
                proc_mono = 0;
                proc_color = 0;
                proc_planes = ((dev)->procs.copy_planes);
            } else {
                proc_planes = 0;
...
# 668 "./base/gdevdbit.c"
        if (ch >= h) {
            if (icw >= w) {
                do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, (w == width && h == height ? tile_id : 0L), x, y, w, h, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, (w == width && h == height ? tile_id : 0L), x, y, w, h) : (*proc_mono)(dev, row, irx, raster, (w == width && h == height ? tile_id : 0L), x, y, w, h, color0, color1))); if (code < 0) return (code); do { } while (0);
                ...
                int fex = ex - width;
                int cx = x + icw;
                ulong id = (h == height ? tile_id : 0L);

                do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, 0L, x, y, icw, h, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, 0L, x, y, icw, h) : (*proc_mono)(dev, row, irx, raster, 0L, x, y, icw, h, color0, color1))); if (code < 0) return (code); do { } while (0);
                while (cx <= fex) {
                    do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, 0, raster, id, cx, y, width, h, height) : (proc_color != 0 ? (*proc_color)(dev, row, 0, raster, id, cx, y, width, h) : (*proc_mono)(dev, row, 0, raster, id, cx, y, width, h, color0, color1))); if (code < 0) return (code); do { } while (0);
                    cx += width;
                }
                if (cx < ex) {
                    do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, 0, raster, 0L, cx, y, ex - cx, h, height) : (proc_color != 0 ? (*proc_color)(dev, row, 0, raster, 0L, cx, y, ex - cx, h) : (*proc_mono)(dev, row, 0, raster, 0L, cx, y, ex - cx, h, color0, color1))); if (code < 0) return (code); do { } while (0);
                }
            }
            ...
            int fey = ey - height;
            int cy = y + ch;
            ulong id = (w == width ? tile_id : 0L);

            do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, (ch == height ? id : 0L), x, y, w, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, (ch == height ? id : 0L), x, y, w, ch) : (*proc_mono)(dev, row, irx, raster, (ch == height ? id : 0L), x, y, w, ch, color0, color1))); if (code < 0) return (code); do { } while (0);
            row = tiles->data;
            do {
                ch = (cy > fey ? ey - cy : height);
                do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, (ch == height ? id : 0L), x, cy, w, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, (ch == height ? id : 0L), x, cy, w, ch) : (*proc_mono)(dev, row, irx, raster, (ch == height ? id : 0L), x, cy, w, ch, color0, color1))); if (code < 0) return (code); do { } while (0);
            ...
            for (cy = y;;) {
                ulong id = (ch == height ? tile_id : 0L);

                if (icw >= w) {
                    do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, (w == width ? id : 0L), x, cy, w, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, (w == width ? id : 0L), x, cy, w, ch) : (*proc_mono)(dev, row, irx, raster, (w == width ? id : 0L), x, cy, w, ch, color0, color1))); if (code < 0) return (code); do { } while (0);

                } else {
                    do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, irx, raster, 0L, x, cy, icw, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, irx, raster, 0L, x, cy, icw, ch) : (*proc_mono)(dev, row, irx, raster, 0L, x, cy, icw, ch, color0, color1))); if (code < 0) return (code); do { } while (0);
                    cx = x + icw;
                    while (cx <= fex) {
                        do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, 0, raster, id, cx, cy, width, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, 0, raster, id, cx, cy, width, ch) : (*proc_mono)(dev, row, 0, raster, id, cx, cy, width, ch, color0, color1))); if (code < 0) return (code); do { } while (0);
                        cx += width;
                    }
                    if (cx < ex) {
                        do { } while (0); code = (tiles->num_planes > 1 ? (*proc_planes)(dev, row, 0, raster, 0L, cx, cy, ex - cx, ch, height) : (proc_color != 0 ? (*proc_color)(dev, row, 0, raster, 0L, cx, cy, ex - cx, ch) : (*proc_mono)(dev, row, 0, raster, 0L, cx, cy, ex - cx, ch, color0, color1))); if (code < 0) return (code); do { } while (0);
                    }
                }
                if ((cy += ch) >= ey)
                    break;
...
gx_copy_mono_unaligned(gx_device * dev, const byte * data,
            int dx, int raster, gx_bitmap_id id, int x, int y, int w, int h,
                       gx_color_index zero, gx_color_index one)
{
    int (*copy_mono)(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1) = ((dev)->procs.copy_mono);
    uint offset = ((uint)( ((const char *)(data) - (const char *)0) & (((1 << 3)) - 1) ));
    int step = raster & ((1 << 3) - 1);
...

    {
        ...
        for (i = 0; i < h && code >= 0;
            ...
            code = (*copy_mono) (dev, p, d, raster, 0L,
                                 x, y + i, w, 1, zero, one);
        return code;
    }
}
...
gx_copy_color_unaligned(gx_device * dev, const byte * data,
                        ...
                        int x, int y, int width, int height)
{
    int (*copy_color)(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height) = ((dev)->procs.copy_color);
    int depth = dev->color_info.depth;
    uint offset = (uint) (data - (const byte *)0) & ((1 << 3) - 1);
    int step = raster & ((1 << 3) - 1);
# 816 "./base/gdevdbit.c"
    if (depth == 24)
        offset += (offset % 3) *
...

    {
        ...
        for (i = 0; i < height && code >= 0;
            ...
            code = (*copy_color) (dev, p, d, raster, 0L,
                                  x, y + i, width, 1);
        return code;
    }
}
...
gx_copy_alpha_unaligned(gx_device * dev, const byte * data, int data_x,
           int raster, gx_bitmap_id id, int x, int y, int width, int height,
                        gx_color_index color, int depth)
{
    int (*copy_alpha)(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color, int depth) = ((dev)->procs.copy_alpha);
    uint offset = (uint) (data - (const byte *)0) & ((1 << 3) - 1);
    int step = raster & ((1 << 3) - 1);
...

    {
        ...
        for (i = 0; i < height && code >= 0;
            ...
            code = (*copy_alpha) (dev, p, d, raster, 0L,
