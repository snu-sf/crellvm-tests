...
flip4x1(byte * buffer, const byte ** planes, int offset, int nbytes)
{
    ...
    for (; n > 0; out += 4, ++in1, ++in2, ++in3, ++in4, --n) {
        ...
        b1 ^= (temp = ((b2 >> 1) ^ b1) & 0x55); b2 ^= temp << 1;
        b3 ^= (temp = ((b4 >> 1) ^ b3) & 0x55); b4 ^= temp << 1;

        b1 ^= (temp = ((b3 >> 2) ^ b1) & 0x33); b3 ^= temp << 2;
        b2 ^= (temp = ((b4 >> 2) ^ b2) & 0x33); b4 ^= temp << 2;
...
flip4x2(byte * buffer, const byte ** planes, int offset, int nbytes)
{
    ...
    for (; n > 0; out += 4, ++in1, ++in2, ++in3, ++in4, --n) {
        ...
        b1 ^= (temp = ((b3 >> 4) ^ b1) & 0x0f); b3 ^= temp << 4;
        b2 ^= (temp = ((b4 >> 4) ^ b2) & 0x0f); b4 ^= temp << 4;

        b1 ^= (temp = ((b2 >> 2) ^ b1) & 0x33); b2 ^= temp << 2;
        b3 ^= (temp = ((b4 >> 2) ^ b3) & 0x33); b4 ^= temp << 2;
        out[0] = b1;
        out[1] = b2;
        out[2] = b3;
        out[3] = b4;
           ...
           int num_planes, int bits_per_sample)
{
    ...
    int bi, pi;
    byte *dptr = (buffer); int dbit = (0); byte dbbyte = ((dbit) ? (byte)(*(dptr) & (0xff00 >> (dbit))) : 0);
    ...
    for (bi = 0; bi < nbytes * 8; bi += bits_per_sample) {
        for (pi = 0; pi < num_planes; ++pi) {
            ...
            do { switch ( (bits_per_sample) >> 2 ) { case 0: if ( (dbit += (bits_per_sample)) == 8 ) *(dptr)++ = dbbyte | (byte)(value), dbbyte = 0, dbit = 0; else dbbyte |= (byte)((value) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbbyte = (byte)((value) << 4); else *(dptr)++ = dbbyte | ((byte)(value)); break; case 2: *(dptr)++ = (byte)(value); break; default: return ((-15)); } } while (0);
        }
    }
         ...
         int num_planes, int ignore_bits_per_sample)
{
    ...
    for (bi = 0; bi < nbytes * 8; bi += 12) {
        for (pi = 0; pi < num_planes; ++pi) {
            ...
            do { if ( dbit ^= 4 ) *(dptr)++ = (byte)((value) >> 4), dbbyte = (byte)((value) << 4); else *(dptr) = dbbyte | (byte)((value) >> 8), (dptr)[1] = (byte)(value), dptr += 2; } while (0);
        }
    }
    if ( (dbit) != 0 ) *(dptr) = dbbyte | (*(dptr) & (0xff >> (dbit)));;
    return 0;
