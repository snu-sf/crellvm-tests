# 1 "./openjpeg/libopenjpeg/tcd.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./openjpeg/libopenjpeg/tcd.c" 2
# 35 "./openjpeg/libopenjpeg/tcd.c"
...
# 36 "./openjpeg/libopenjpeg/tcd.c" 2
# 103 "./openjpeg/libopenjpeg/tcd.c"
...
opj_tcd_t* opj_tcd_create(OPJ_BOOL p_is_decoder)
{
        ...
        l_tcd = (opj_tcd_t*) ((size_t)(sizeof(opj_tcd_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(sizeof(opj_tcd_t)));
        if (!l_tcd) {
                return 00;
        }
        ...
        l_tcd->tcd_image = (opj_tcd_image_t*)((size_t)(sizeof(opj_tcd_image_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(sizeof(opj_tcd_image_t)));
        if (!l_tcd->tcd_image) {
                free(l_tcd);
                return 00;
        }
                        ...
                        for (bandno = 0; bandno < res->numbands; bandno++) {
                                ...
                                for (precno = 0; precno < res->pw * res->ph; precno++) {
                                        ...
                                        for (cblkno = 0; cblkno < prc->cw * prc->ch; cblkno++) {
                                                ...
                                                opj_tcd_layer_t *layer = &cblk->layers[layno];
                                                OPJ_UINT32 n;
                                                OPJ_INT32 imsb = tcd->image->comps[compno].prec - cblk->numbps;
                                                                ...
                                                                opj_codestream_info_t *cstr_info)
{
        ...
        if(cstr_info) {
                ...
                tile_info->thresh = (OPJ_FLOAT64 *) ((size_t)(tcd_tcp->numlayers * sizeof(OPJ_FLOAT64)) >= (size_t)-0x100 ? ((void*)0) : malloc(tcd_tcp->numlayers * sizeof(OPJ_FLOAT64)));
        }
                                           ...
                                           opj_cp_t * p_cp )
{
        ...
        p_tcd->tcd_image->tiles = (opj_tcd_tile_t *) ((size_t)(sizeof(opj_tcd_tile_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(sizeof(opj_tcd_tile_t)));
        ...
        p_tcd->tcd_image->tiles->comps = (opj_tcd_tilecomp_t *) ((size_t)(l_tile_comp_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_tile_comp_size));
...
# 997 "./openjpeg/libopenjpeg/tcd.c"
OPJ_BOOL opj_tcd_init_encode_tile ( opj_tcd_t *p_tcd, OPJ_UINT32 p_tile_no ) { OPJ_UINT32 (*l_gain_ptr)(OPJ_UINT32) = 00; OPJ_UINT32 compno, resno, bandno, precno, cblkno; opj_tcp_t * l_tcp = 00; opj_cp_t * l_cp = 00; opj_tcd_tile_t * l_tile = 00; opj_tccp_t *l_tccp = 00; opj_tcd_tilecomp_t *l_tilec = 00; opj_image_comp_t * l_image_comp = 00; opj_tcd_resolution_t *l_res = 00; opj_tcd_band_t *l_band = 00; opj_stepsize_t * l_step_size = 00; opj_tcd_precinct_t *l_current_precinct = 00; opj_tcd_cblk_enc_t* l_code_block = 00; opj_image_t *l_image = 00; OPJ_UINT32 p,q; OPJ_UINT32 l_level_no; OPJ_UINT32 l_pdx, l_pdy; OPJ_UINT32 l_gain; OPJ_INT32 l_x0b, l_y0b; OPJ_INT32 l_tl_prc_x_start, l_tl_prc_y_start, l_br_prc_x_end, l_br_prc_y_end; OPJ_UINT32 l_nb_precincts; OPJ_UINT32 l_nb_precinct_size; OPJ_UINT32 l_nb_code_blocks; OPJ_UINT32 l_nb_code_blocks_size; OPJ_UINT32 l_data_size; l_cp = p_tcd->cp; l_tcp = &(l_cp->tcps[p_tile_no]); l_tile = p_tcd->tcd_image->tiles; l_tccp = l_tcp->tccps; l_tilec = l_tile->comps; l_image = p_tcd->image; l_image_comp = p_tcd->image->comps; p = p_tile_no % l_cp->tw; q = p_tile_no / l_cp->tw; l_tile->x0 = opj_int_max(l_cp->tx0 + p * l_cp->tdx, l_image->x0); l_tile->y0 = opj_int_max(l_cp->ty0 + q * l_cp->tdy, l_image->y0); l_tile->x1 = opj_int_min(l_cp->tx0 + (p + 1) * l_cp->tdx, l_image->x1); l_tile->y1 = opj_int_min(l_cp->ty0 + (q + 1) * l_cp->tdy, l_image->y1); if (l_tccp->numresolutions == 0) { fprintf(stderr, "tiles require at least one resolution\n"); return 0; } for(compno = 0; compno < l_tile->numcomps; ++compno) { l_tilec->x0 = opj_int_ceildiv(l_tile->x0, l_image_comp->dx); l_tilec->y0 = opj_int_ceildiv(l_tile->y0, l_image_comp->dy); l_tilec->x1 = opj_int_ceildiv(l_tile->x1, l_image_comp->dx); l_tilec->y1 = opj_int_ceildiv(l_tile->y1, l_image_comp->dy); l_data_size = (l_tilec->x1 - l_tilec->x0) * (l_tilec->y1 - l_tilec->y0) * sizeof(OPJ_UINT32 ); l_tilec->numresolutions = l_tccp->numresolutions; if (l_tccp->numresolutions < l_cp->m_specific_param.m_dec.m_reduce) { l_tilec->minimum_num_resolutions = 1; } else { l_tilec->minimum_num_resolutions = l_tccp->numresolutions - l_cp->m_specific_param.m_dec.m_reduce; } if (l_tilec->data == 00) { l_tilec->data = (OPJ_INT32 *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_data_size)); if (! l_tilec->data ) { return 0; } l_tilec->data_size = l_data_size; } else if (l_data_size > l_tilec->data_size) { OPJ_INT32 * new_data = (OPJ_INT32 *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_tilec->data, l_data_size)); fprintf(stderr, "Not enough memory to handle tile data\n"); if (! new_data) { free(l_tilec->data); l_tilec->data = ((void*)0); l_tilec->data_size = 0; return 0; } l_tilec->data = new_data; l_tilec->data_size = l_data_size; } l_data_size = l_tilec->numresolutions * sizeof(opj_tcd_resolution_t); if (l_tilec->resolutions == 00) { l_tilec->resolutions = (opj_tcd_resolution_t *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_data_size)); if (! l_tilec->resolutions ) { return 0; } l_tilec->resolutions_size = l_data_size; memset(l_tilec->resolutions,0,l_data_size); } else if (l_data_size > l_tilec->resolutions_size) { opj_tcd_resolution_t* new_resolutions = (opj_tcd_resolution_t *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_tilec->resolutions, l_data_size)); if (! new_resolutions) { fprintf(stderr, "Not enough memory to tile resolutions\n"); free(l_tilec->resolutions); l_tilec->resolutions = ((void*)0); l_tilec->resolutions_size = 0; return 0; } l_tilec->resolutions = new_resolutions; memset(((OPJ_BYTE*) l_tilec->resolutions)+l_tilec->resolutions_size,0,l_data_size - l_tilec->resolutions_size); l_tilec->resolutions_size = l_data_size; } l_level_no = l_tilec->numresolutions - 1; l_res = l_tilec->resolutions; l_step_size = l_tccp->stepsizes; if (l_tccp->qmfbid == 0) { l_gain_ptr = &opj_dwt_getgain_real; } else { l_gain_ptr = &opj_dwt_getgain; } for(resno = 0; resno < l_tilec->numresolutions; ++resno) { OPJ_INT32 tlcbgxstart, tlcbgystart ; OPJ_UINT32 cbgwidthexpn, cbgheightexpn; OPJ_UINT32 cblkwidthexpn, cblkheightexpn; l_res->x0 = opj_int_ceildivpow2(l_tilec->x0, l_level_no); l_res->y0 = opj_int_ceildivpow2(l_tilec->y0, l_level_no); l_res->x1 = opj_int_ceildivpow2(l_tilec->x1, l_level_no); l_res->y1 = opj_int_ceildivpow2(l_tilec->y1, l_level_no); l_pdx = l_tccp->prcw[resno]; l_pdy = l_tccp->prch[resno]; l_tl_prc_x_start = opj_int_floordivpow2(l_res->x0, l_pdx) << l_pdx; l_tl_prc_y_start = opj_int_floordivpow2(l_res->y0, l_pdy) << l_pdy; l_br_prc_x_end = opj_int_ceildivpow2(l_res->x1, l_pdx) << l_pdx; l_br_prc_y_end = opj_int_ceildivpow2(l_res->y1, l_pdy) << l_pdy; l_res->pw = (l_res->x0 == l_res->x1) ? 0 : ((l_br_prc_x_end - l_tl_prc_x_start) >> l_pdx); l_res->ph = (l_res->y0 == l_res->y1) ? 0 : ((l_br_prc_y_end - l_tl_prc_y_start) >> l_pdy); l_nb_precincts = l_res->pw * l_res->ph; l_nb_precinct_size = l_nb_precincts * sizeof(opj_tcd_precinct_t); if (resno == 0) { tlcbgxstart = l_tl_prc_x_start; tlcbgystart = l_tl_prc_y_start; cbgwidthexpn = l_pdx; cbgheightexpn = l_pdy; l_res->numbands = 1; } else { tlcbgxstart = opj_int_ceildivpow2(l_tl_prc_x_start, 1); tlcbgystart = opj_int_ceildivpow2(l_tl_prc_y_start, 1); cbgwidthexpn = l_pdx - 1; cbgheightexpn = l_pdy - 1; l_res->numbands = 3; } cblkwidthexpn = opj_uint_min(l_tccp->cblkw, cbgwidthexpn); cblkheightexpn = opj_uint_min(l_tccp->cblkh, cbgheightexpn); l_band = l_res->bands; for (bandno = 0; bandno < l_res->numbands; ++bandno) { OPJ_INT32 numbps; if (resno == 0) { l_band->bandno = 0 ; l_band->x0 = opj_int_ceildivpow2(l_tilec->x0, l_level_no); l_band->y0 = opj_int_ceildivpow2(l_tilec->y0, l_level_no); l_band->x1 = opj_int_ceildivpow2(l_tilec->x1, l_level_no); l_band->y1 = opj_int_ceildivpow2(l_tilec->y1, l_level_no); } else { l_band->bandno = bandno + 1; l_x0b = l_band->bandno&1; l_y0b = (l_band->bandno)>>1; l_band->x0 = opj_int_ceildivpow2(l_tilec->x0 - (1 << l_level_no) * l_x0b, l_level_no + 1); l_band->y0 = opj_int_ceildivpow2(l_tilec->y0 - (1 << l_level_no) * l_y0b, l_level_no + 1); l_band->x1 = opj_int_ceildivpow2(l_tilec->x1 - (1 << l_level_no) * l_x0b, l_level_no + 1); l_band->y1 = opj_int_ceildivpow2(l_tilec->y1 - (1 << l_level_no) * l_y0b, l_level_no + 1); } l_gain = (*l_gain_ptr) (l_band->bandno); numbps = l_image_comp->prec + l_gain; l_band->stepsize = (OPJ_FLOAT32)(((1.0 + l_step_size->mant / 2048.0) * pow(2.0, (OPJ_INT32) (numbps - l_step_size->expn)))) * 1.f; l_band->numbps = l_step_size->expn + l_tccp->numgbits - 1; if (! l_band->precincts) { l_band->precincts = (opj_tcd_precinct_t *) ((size_t)(l_nb_precinct_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_nb_precinct_size)); if (! l_band->precincts) { return 0; } memset(l_band->precincts,0,l_nb_precinct_size); l_band->precincts_data_size = l_nb_precinct_size; } else if (l_band->precincts_data_size < l_nb_precinct_size) { opj_tcd_precinct_t * new_precincts = (opj_tcd_precinct_t *) ((size_t)(l_nb_precinct_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_band->precincts, l_nb_precinct_size)); if (! new_precincts) { fprintf(stderr, "Not enough memory to handle band precints\n"); free(l_band->precincts); l_band->precincts = ((void*)0); l_band->precincts_data_size = 0; return 0; } l_band->precincts = new_precincts; memset(((OPJ_BYTE *) l_band->precincts) + l_band->precincts_data_size,0,l_nb_precinct_size - l_band->precincts_data_size); l_band->precincts_data_size = l_nb_precinct_size; } l_current_precinct = l_band->precincts; for (precno = 0; precno < l_nb_precincts; ++precno) { OPJ_INT32 tlcblkxstart, tlcblkystart, brcblkxend, brcblkyend; OPJ_INT32 cbgxstart = tlcbgxstart + (precno % l_res->pw) * (1 << cbgwidthexpn); OPJ_INT32 cbgystart = tlcbgystart + (precno / l_res->pw) * (1 << cbgheightexpn); OPJ_INT32 cbgxend = cbgxstart + (1 << cbgwidthexpn); OPJ_INT32 cbgyend = cbgystart + (1 << cbgheightexpn); l_current_precinct->x0 = opj_int_max(cbgxstart, l_band->x0); l_current_precinct->y0 = opj_int_max(cbgystart, l_band->y0); l_current_precinct->x1 = opj_int_min(cbgxend, l_band->x1); l_current_precinct->y1 = opj_int_min(cbgyend, l_band->y1); tlcblkxstart = opj_int_floordivpow2(l_current_precinct->x0, cblkwidthexpn) << cblkwidthexpn; tlcblkystart = opj_int_floordivpow2(l_current_precinct->y0, cblkheightexpn) << cblkheightexpn; brcblkxend = opj_int_ceildivpow2(l_current_precinct->x1, cblkwidthexpn) << cblkwidthexpn; brcblkyend = opj_int_ceildivpow2(l_current_precinct->y1, cblkheightexpn) << cblkheightexpn; l_current_precinct->cw = (brcblkxend - tlcblkxstart) >> cblkwidthexpn; l_current_precinct->ch = (brcblkyend - tlcblkystart) >> cblkheightexpn; l_nb_code_blocks = l_current_precinct->cw * l_current_precinct->ch; l_nb_code_blocks_size = l_nb_code_blocks * sizeof(opj_tcd_cblk_enc_t); if (! l_current_precinct->cblks.enc) { l_current_precinct->cblks.enc = (opj_tcd_cblk_enc_t*) ((size_t)(l_nb_code_blocks_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_nb_code_blocks_size)); if (! l_current_precinct->cblks.enc ) { return 0; } memset(l_current_precinct->cblks.enc,0,l_nb_code_blocks_size); l_current_precinct->block_size = l_nb_code_blocks_size; } else if (l_nb_code_blocks_size > l_current_precinct->block_size) { opj_tcd_cblk_enc_t *new_ELEMENT = (opj_tcd_cblk_enc_t*) ((size_t)(l_nb_code_blocks_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_current_precinct->cblks.enc, l_nb_code_blocks_size)); if (! new_ELEMENT) { free(l_current_precinct->cblks.enc); l_current_precinct->cblks.enc = ((void*)0); l_current_precinct->block_size = 0; fprintf(stderr, "Not enough memory for current precinct codeblock element\n"); return 0; } l_current_precinct->cblks.enc = new_ELEMENT; memset(((OPJ_BYTE *) l_current_precinct->cblks.enc) + l_current_precinct->block_size ,0 ,l_nb_code_blocks_size - l_current_precinct->block_size); l_current_precinct->block_size = l_nb_code_blocks_size; } if (! l_current_precinct->incltree) { l_current_precinct->incltree = opj_tgt_create(l_current_precinct->cw, l_current_precinct->ch); } else{ l_current_precinct->incltree = opj_tgt_init(l_current_precinct->incltree, l_current_precinct->cw, l_current_precinct->ch); } if (! l_current_precinct->incltree) { fprintf(stderr, "WARNING: No incltree created.\n"); } if (! l_current_precinct->imsbtree) { l_current_precinct->imsbtree = opj_tgt_create( l_current_precinct->cw, l_current_precinct->ch); } else { l_current_precinct->imsbtree = opj_tgt_init( l_current_precinct->imsbtree, l_current_precinct->cw, l_current_precinct->ch); } if (! l_current_precinct->imsbtree) { fprintf(stderr, "WARNING: No imsbtree created.\n"); } l_code_block = l_current_precinct->cblks.enc; for (cblkno = 0; cblkno < l_nb_code_blocks; ++cblkno) { OPJ_INT32 cblkxstart = tlcblkxstart + (cblkno % l_current_precinct->cw) * (1 << cblkwidthexpn); OPJ_INT32 cblkystart = tlcblkystart + (cblkno / l_current_precinct->cw) * (1 << cblkheightexpn); OPJ_INT32 cblkxend = cblkxstart + (1 << cblkwidthexpn); OPJ_INT32 cblkyend = cblkystart + (1 << cblkheightexpn); l_code_block->x0 = opj_int_max(cblkxstart, l_current_precinct->x0); l_code_block->y0 = opj_int_max(cblkystart, l_current_precinct->y0); l_code_block->x1 = opj_int_min(cblkxend, l_current_precinct->x1); l_code_block->y1 = opj_int_min(cblkyend, l_current_precinct->y1); if (! opj_tcd_code_block_enc_allocate(l_code_block)) { return 0; } ++l_code_block; } ++l_current_precinct; } ++l_band; ++l_step_size; } ++l_res; --l_level_no; } ++l_tccp; ++l_tilec; ++l_image_comp; } return 1; }
OPJ_BOOL opj_tcd_init_decode_tile ( opj_tcd_t *p_tcd, OPJ_UINT32 p_tile_no ) { OPJ_UINT32 (*l_gain_ptr)(OPJ_UINT32) = 00; OPJ_UINT32 compno, resno, bandno, precno, cblkno; opj_tcp_t * l_tcp = 00; opj_cp_t * l_cp = 00; opj_tcd_tile_t * l_tile = 00; opj_tccp_t *l_tccp = 00; opj_tcd_tilecomp_t *l_tilec = 00; opj_image_comp_t * l_image_comp = 00; opj_tcd_resolution_t *l_res = 00; opj_tcd_band_t *l_band = 00; opj_stepsize_t * l_step_size = 00; opj_tcd_precinct_t *l_current_precinct = 00; opj_tcd_cblk_dec_t* l_code_block = 00; opj_image_t *l_image = 00; OPJ_UINT32 p,q; OPJ_UINT32 l_level_no; OPJ_UINT32 l_pdx, l_pdy; OPJ_UINT32 l_gain; OPJ_INT32 l_x0b, l_y0b; OPJ_INT32 l_tl_prc_x_start, l_tl_prc_y_start, l_br_prc_x_end, l_br_prc_y_end; OPJ_UINT32 l_nb_precincts; OPJ_UINT32 l_nb_precinct_size; OPJ_UINT32 l_nb_code_blocks; OPJ_UINT32 l_nb_code_blocks_size; OPJ_UINT32 l_data_size; l_cp = p_tcd->cp; l_tcp = &(l_cp->tcps[p_tile_no]); l_tile = p_tcd->tcd_image->tiles; l_tccp = l_tcp->tccps; l_tilec = l_tile->comps; l_image = p_tcd->image; l_image_comp = p_tcd->image->comps; p = p_tile_no % l_cp->tw; q = p_tile_no / l_cp->tw; l_tile->x0 = opj_int_max(l_cp->tx0 + p * l_cp->tdx, l_image->x0); l_tile->y0 = opj_int_max(l_cp->ty0 + q * l_cp->tdy, l_image->y0); l_tile->x1 = opj_int_min(l_cp->tx0 + (p + 1) * l_cp->tdx, l_image->x1); l_tile->y1 = opj_int_min(l_cp->ty0 + (q + 1) * l_cp->tdy, l_image->y1); if (l_tccp->numresolutions == 0) { fprintf(stderr, "tiles require at least one resolution\n"); return 0; } for(compno = 0; compno < l_tile->numcomps; ++compno) { l_tilec->x0 = opj_int_ceildiv(l_tile->x0, l_image_comp->dx); l_tilec->y0 = opj_int_ceildiv(l_tile->y0, l_image_comp->dy); l_tilec->x1 = opj_int_ceildiv(l_tile->x1, l_image_comp->dx); l_tilec->y1 = opj_int_ceildiv(l_tile->y1, l_image_comp->dy); l_data_size = (l_tilec->x1 - l_tilec->x0) * (l_tilec->y1 - l_tilec->y0) * sizeof(OPJ_UINT32 ); l_tilec->numresolutions = l_tccp->numresolutions; if (l_tccp->numresolutions < l_cp->m_specific_param.m_dec.m_reduce) { l_tilec->minimum_num_resolutions = 1; } else { l_tilec->minimum_num_resolutions = l_tccp->numresolutions - l_cp->m_specific_param.m_dec.m_reduce; } if (l_tilec->data == 00) { l_tilec->data = (OPJ_INT32 *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_data_size)); if (! l_tilec->data ) { return 0; } l_tilec->data_size = l_data_size; } else if (l_data_size > l_tilec->data_size) { OPJ_INT32 * new_data = (OPJ_INT32 *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_tilec->data, l_data_size)); fprintf(stderr, "Not enough memory to handle tile data\n"); if (! new_data) { free(l_tilec->data); l_tilec->data = ((void*)0); l_tilec->data_size = 0; return 0; } l_tilec->data = new_data; l_tilec->data_size = l_data_size; } l_data_size = l_tilec->numresolutions * sizeof(opj_tcd_resolution_t); if (l_tilec->resolutions == 00) { l_tilec->resolutions = (opj_tcd_resolution_t *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_data_size)); if (! l_tilec->resolutions ) { return 0; } l_tilec->resolutions_size = l_data_size; memset(l_tilec->resolutions,0,l_data_size); } else if (l_data_size > l_tilec->resolutions_size) { opj_tcd_resolution_t* new_resolutions = (opj_tcd_resolution_t *) ((size_t)(l_data_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_tilec->resolutions, l_data_size)); if (! new_resolutions) { fprintf(stderr, "Not enough memory to tile resolutions\n"); free(l_tilec->resolutions); l_tilec->resolutions = ((void*)0); l_tilec->resolutions_size = 0; return 0; } l_tilec->resolutions = new_resolutions; memset(((OPJ_BYTE*) l_tilec->resolutions)+l_tilec->resolutions_size,0,l_data_size - l_tilec->resolutions_size); l_tilec->resolutions_size = l_data_size; } l_level_no = l_tilec->numresolutions - 1; l_res = l_tilec->resolutions; l_step_size = l_tccp->stepsizes; if (l_tccp->qmfbid == 0) { l_gain_ptr = &opj_dwt_getgain_real; } else { l_gain_ptr = &opj_dwt_getgain; } for(resno = 0; resno < l_tilec->numresolutions; ++resno) { OPJ_INT32 tlcbgxstart, tlcbgystart ; OPJ_UINT32 cbgwidthexpn, cbgheightexpn; OPJ_UINT32 cblkwidthexpn, cblkheightexpn; l_res->x0 = opj_int_ceildivpow2(l_tilec->x0, l_level_no); l_res->y0 = opj_int_ceildivpow2(l_tilec->y0, l_level_no); l_res->x1 = opj_int_ceildivpow2(l_tilec->x1, l_level_no); l_res->y1 = opj_int_ceildivpow2(l_tilec->y1, l_level_no); l_pdx = l_tccp->prcw[resno]; l_pdy = l_tccp->prch[resno]; l_tl_prc_x_start = opj_int_floordivpow2(l_res->x0, l_pdx) << l_pdx; l_tl_prc_y_start = opj_int_floordivpow2(l_res->y0, l_pdy) << l_pdy; l_br_prc_x_end = opj_int_ceildivpow2(l_res->x1, l_pdx) << l_pdx; l_br_prc_y_end = opj_int_ceildivpow2(l_res->y1, l_pdy) << l_pdy; l_res->pw = (l_res->x0 == l_res->x1) ? 0 : ((l_br_prc_x_end - l_tl_prc_x_start) >> l_pdx); l_res->ph = (l_res->y0 == l_res->y1) ? 0 : ((l_br_prc_y_end - l_tl_prc_y_start) >> l_pdy); l_nb_precincts = l_res->pw * l_res->ph; l_nb_precinct_size = l_nb_precincts * sizeof(opj_tcd_precinct_t); if (resno == 0) { tlcbgxstart = l_tl_prc_x_start; tlcbgystart = l_tl_prc_y_start; cbgwidthexpn = l_pdx; cbgheightexpn = l_pdy; l_res->numbands = 1; } else { tlcbgxstart = opj_int_ceildivpow2(l_tl_prc_x_start, 1); tlcbgystart = opj_int_ceildivpow2(l_tl_prc_y_start, 1); cbgwidthexpn = l_pdx - 1; cbgheightexpn = l_pdy - 1; l_res->numbands = 3; } cblkwidthexpn = opj_uint_min(l_tccp->cblkw, cbgwidthexpn); cblkheightexpn = opj_uint_min(l_tccp->cblkh, cbgheightexpn); l_band = l_res->bands; for (bandno = 0; bandno < l_res->numbands; ++bandno) { OPJ_INT32 numbps; if (resno == 0) { l_band->bandno = 0 ; l_band->x0 = opj_int_ceildivpow2(l_tilec->x0, l_level_no); l_band->y0 = opj_int_ceildivpow2(l_tilec->y0, l_level_no); l_band->x1 = opj_int_ceildivpow2(l_tilec->x1, l_level_no); l_band->y1 = opj_int_ceildivpow2(l_tilec->y1, l_level_no); } else { l_band->bandno = bandno + 1; l_x0b = l_band->bandno&1; l_y0b = (l_band->bandno)>>1; l_band->x0 = opj_int_ceildivpow2(l_tilec->x0 - (1 << l_level_no) * l_x0b, l_level_no + 1); l_band->y0 = opj_int_ceildivpow2(l_tilec->y0 - (1 << l_level_no) * l_y0b, l_level_no + 1); l_band->x1 = opj_int_ceildivpow2(l_tilec->x1 - (1 << l_level_no) * l_x0b, l_level_no + 1); l_band->y1 = opj_int_ceildivpow2(l_tilec->y1 - (1 << l_level_no) * l_y0b, l_level_no + 1); } l_gain = (*l_gain_ptr) (l_band->bandno); numbps = l_image_comp->prec + l_gain; l_band->stepsize = (OPJ_FLOAT32)(((1.0 + l_step_size->mant / 2048.0) * pow(2.0, (OPJ_INT32) (numbps - l_step_size->expn)))) * 0.5f; l_band->numbps = l_step_size->expn + l_tccp->numgbits - 1; if (! l_band->precincts) { l_band->precincts = (opj_tcd_precinct_t *) ((size_t)(l_nb_precinct_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_nb_precinct_size)); if (! l_band->precincts) { return 0; } memset(l_band->precincts,0,l_nb_precinct_size); l_band->precincts_data_size = l_nb_precinct_size; } else if (l_band->precincts_data_size < l_nb_precinct_size) { opj_tcd_precinct_t * new_precincts = (opj_tcd_precinct_t *) ((size_t)(l_nb_precinct_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_band->precincts, l_nb_precinct_size)); if (! new_precincts) { fprintf(stderr, "Not enough memory to handle band precints\n"); free(l_band->precincts); l_band->precincts = ((void*)0); l_band->precincts_data_size = 0; return 0; } l_band->precincts = new_precincts; memset(((OPJ_BYTE *) l_band->precincts) + l_band->precincts_data_size,0,l_nb_precinct_size - l_band->precincts_data_size); l_band->precincts_data_size = l_nb_precinct_size; } l_current_precinct = l_band->precincts; for (precno = 0; precno < l_nb_precincts; ++precno) { OPJ_INT32 tlcblkxstart, tlcblkystart, brcblkxend, brcblkyend; OPJ_INT32 cbgxstart = tlcbgxstart + (precno % l_res->pw) * (1 << cbgwidthexpn); OPJ_INT32 cbgystart = tlcbgystart + (precno / l_res->pw) * (1 << cbgheightexpn); OPJ_INT32 cbgxend = cbgxstart + (1 << cbgwidthexpn); OPJ_INT32 cbgyend = cbgystart + (1 << cbgheightexpn); l_current_precinct->x0 = opj_int_max(cbgxstart, l_band->x0); l_current_precinct->y0 = opj_int_max(cbgystart, l_band->y0); l_current_precinct->x1 = opj_int_min(cbgxend, l_band->x1); l_current_precinct->y1 = opj_int_min(cbgyend, l_band->y1); tlcblkxstart = opj_int_floordivpow2(l_current_precinct->x0, cblkwidthexpn) << cblkwidthexpn; tlcblkystart = opj_int_floordivpow2(l_current_precinct->y0, cblkheightexpn) << cblkheightexpn; brcblkxend = opj_int_ceildivpow2(l_current_precinct->x1, cblkwidthexpn) << cblkwidthexpn; brcblkyend = opj_int_ceildivpow2(l_current_precinct->y1, cblkheightexpn) << cblkheightexpn; l_current_precinct->cw = (brcblkxend - tlcblkxstart) >> cblkwidthexpn; l_current_precinct->ch = (brcblkyend - tlcblkystart) >> cblkheightexpn; l_nb_code_blocks = l_current_precinct->cw * l_current_precinct->ch; l_nb_code_blocks_size = l_nb_code_blocks * sizeof(opj_tcd_cblk_dec_t); if (! l_current_precinct->cblks.dec) { l_current_precinct->cblks.dec = (opj_tcd_cblk_dec_t*) ((size_t)(l_nb_code_blocks_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_nb_code_blocks_size)); if (! l_current_precinct->cblks.dec ) { return 0; } memset(l_current_precinct->cblks.dec,0,l_nb_code_blocks_size); l_current_precinct->block_size = l_nb_code_blocks_size; } else if (l_nb_code_blocks_size > l_current_precinct->block_size) { opj_tcd_cblk_dec_t *new_ELEMENT = (opj_tcd_cblk_dec_t*) ((size_t)(l_nb_code_blocks_size) >= (size_t)-0x100 ? ((void*)0) : realloc(l_current_precinct->cblks.dec, l_nb_code_blocks_size)); if (! new_ELEMENT) { free(l_current_precinct->cblks.dec); l_current_precinct->cblks.dec = ((void*)0); l_current_precinct->block_size = 0; fprintf(stderr, "Not enough memory for current precinct codeblock element\n"); return 0; } l_current_precinct->cblks.dec = new_ELEMENT; memset(((OPJ_BYTE *) l_current_precinct->cblks.dec) + l_current_precinct->block_size ,0 ,l_nb_code_blocks_size - l_current_precinct->block_size); l_current_precinct->block_size = l_nb_code_blocks_size; } if (! l_current_precinct->incltree) { l_current_precinct->incltree = opj_tgt_create(l_current_precinct->cw, l_current_precinct->ch); } else{ l_current_precinct->incltree = opj_tgt_init(l_current_precinct->incltree, l_current_precinct->cw, l_current_precinct->ch); } if (! l_current_precinct->incltree) { fprintf(stderr, "WARNING: No incltree created.\n"); } if (! l_current_precinct->imsbtree) { l_current_precinct->imsbtree = opj_tgt_create( l_current_precinct->cw, l_current_precinct->ch); } else { l_current_precinct->imsbtree = opj_tgt_init( l_current_precinct->imsbtree, l_current_precinct->cw, l_current_precinct->ch); } if (! l_current_precinct->imsbtree) { fprintf(stderr, "WARNING: No imsbtree created.\n"); } l_code_block = l_current_precinct->cblks.dec; for (cblkno = 0; cblkno < l_nb_code_blocks; ++cblkno) { OPJ_INT32 cblkxstart = tlcblkxstart + (cblkno % l_current_precinct->cw) * (1 << cblkwidthexpn); OPJ_INT32 cblkystart = tlcblkystart + (cblkno / l_current_precinct->cw) * (1 << cblkheightexpn); OPJ_INT32 cblkxend = cblkxstart + (1 << cblkwidthexpn); OPJ_INT32 cblkyend = cblkystart + (1 << cblkheightexpn); l_code_block->x0 = opj_int_max(cblkxstart, l_current_precinct->x0); l_code_block->y0 = opj_int_max(cblkystart, l_current_precinct->y0); l_code_block->x1 = opj_int_min(cblkxend, l_current_precinct->x1); l_code_block->y1 = opj_int_min(cblkyend, l_current_precinct->y1); if (! opj_tcd_code_block_dec_allocate(l_code_block)) { return 0; } ++l_code_block; } ++l_current_precinct; } ++l_band; ++l_step_size; } ++l_res; --l_level_no; } ++l_tccp; ++l_tilec; ++l_image_comp; } return 1; }
...
OPJ_BOOL opj_tcd_code_block_enc_allocate (opj_tcd_cblk_enc_t * p_code_block)
{
        if (! p_code_block->data) {
                ...
                p_code_block->data = (OPJ_BYTE*) ((size_t)(8192) >= (size_t)-0x100 ? ((void*)0) : malloc(8192));
                if(! p_code_block->data) {
                        return 0;
                }
                ...
                p_code_block->layers = (opj_tcd_layer_t*) ((size_t)(100 * sizeof(opj_tcd_layer_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(100 * sizeof(opj_tcd_layer_t)));
                if (! p_code_block->layers) {
                        return 0;
                }

                p_code_block->passes = (opj_tcd_pass_t*) ((size_t)(100 * sizeof(opj_tcd_pass_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(100 * sizeof(opj_tcd_pass_t)));
...
OPJ_BOOL opj_tcd_code_block_dec_allocate (opj_tcd_cblk_dec_t * p_code_block)
{
        ...
        if (! p_code_block->data) {
                ...
                p_code_block->data_max_size = 8192;


                l_seg_size = 10 * sizeof(opj_tcd_seg_t);
                p_code_block->segs = (opj_tcd_seg_t *) ((size_t)(l_seg_size) >= (size_t)-0x100 ? ((void*)0) : malloc(l_seg_size));
                                                        ...
                                                        opj_codestream_info_t *p_cstr_info)
{
        ...
        if (p_tcd->cur_tp_num == 0) {
                ...
                if(p_cstr_info) {
                        ...
                        OPJ_UINT32 i;
                        opj_tcd_tilecomp_t *l_tilec_idx = &p_tcd->tcd_image->tiles->comps[0];
                        ...
                        p_cstr_info->tile[p_tile_no].packet = (opj_packet_info_t*) ((size_t)(p_cstr_info->numcomps * p_cstr_info->numlayers * l_num_packs) != 0 && (size_t)(p_cstr_info->numcomps * p_cstr_info->numlayers * l_num_packs) >= (size_t)-0x100 / (size_t)(sizeof(opj_packet_info_t)) ? ((void*)0) : calloc(p_cstr_info->numcomps * p_cstr_info->numlayers * l_num_packs, sizeof(opj_packet_info_t)));
                }
                                ...
                                )
{
        ...
        p_tcd->tcd_tileno = p_tile_no;
        p_tcd->tcp = &(p_tcd->cp->tcps[p_tile_no]);
# 1221 "./openjpeg/libopenjpeg/tcd.c"
        l_data_read = 0;
...
OPJ_BOOL opj_tcd_t1_decode ( opj_tcd_t *p_tcd )
{
        ...
        for (compno = 0; compno < l_tile->numcomps; ++compno) {
                ...
                if (0 == opj_t1_decode_cblks(l_t1, l_tile_comp, l_tccp)) {
                        opj_t1_destroy(l_t1);
                        return 0;
                }
                ++l_tile_comp;
...
OPJ_BOOL opj_tcd_dwt_decode ( opj_tcd_t *p_tcd )
{
        ...
        for (compno = 0; compno < l_tile->numcomps; compno++) {
# 1549 "./openjpeg/libopenjpeg/tcd.c"
                if (l_tccp->qmfbid == 1) {
        ...
        if (l_tile->numcomps >= 3 ){
                ...
                if ((l_tile->comps[0].x1 - l_tile->comps[0].x0) * (l_tile->comps[0].y1 - l_tile->comps[0].y0) < (OPJ_INT32)l_samples ||
                    ...
                    (l_tile->comps[2].x1 - l_tile->comps[2].x0) * (l_tile->comps[2].y1 - l_tile->comps[2].y0) < (OPJ_INT32)l_samples) {
                        ...
                        l_data = (OPJ_BYTE **) ((size_t)(l_tile->numcomps*sizeof(OPJ_BYTE*)) >= (size_t)-0x100 ? ((void*)0) : malloc(l_tile->numcomps*sizeof(OPJ_BYTE*)));
                        if (! l_data) {
                                return 0;
                        }
...
OPJ_BOOL opj_tcd_dc_level_shift_decode ( opj_tcd_t *p_tcd )
{
        ...
        for (compno = 0; compno < l_tile->numcomps; compno++) {
                ...
                ((l_height == 0 || l_width + l_stride <= l_tile_comp->data_size / l_height) ? (void) (0) : __assert_fail ("l_height == 0 || l_width + l_stride <= l_tile_comp->data_size / l_height", "./openjpeg/libopenjpeg/tcd.c", 1668, __PRETTY_FUNCTION__));
