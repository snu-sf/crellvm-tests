...
static gs_ptr_type_t device_mask_clip_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void device_mask_clip_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_device_mask_clip = { sizeof(gx_device_mask_clip), "gx_device_mask_clip", 0, 0, device_mask_clip_enum_ptrs, device_mask_clip_reloc_ptrs, gx_device_finalize, 0 };


static gs_ptr_type_t device_mask_clip_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_mask_clip *mcdev = vptr; switch ( index ) { default:
{
    if (index < 1) {
        return (*(st_gx_strip_bitmap).enum_ptrs)(mem, &mcdev->tiles, sizeof(mcdev->tiles), index, pep, &(st_gx_strip_bitmap), gcst);

    }
    index -= 1;
    if (index < ((0 + 1) + 2)) {
        return (*(st_device_memory).enum_ptrs)(mem, &mcdev->mdev, sizeof(mcdev->mdev), index, pep, &(st_device_memory), gcst);

    }
    return (*(st_device_forward).enum_ptrs)(mem, vptr, size, index-(((0 + 1) + 2)), pep, &(st_device_forward), gcst);
}
} }
static void device_mask_clip_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_mask_clip *mcdev = vptr;
{
    (*(st_device_forward).reloc_ptrs)(vptr, size, &(st_device_forward), gcst);
    (*(st_gx_strip_bitmap).reloc_ptrs)(&mcdev->tiles, sizeof(mcdev->tiles), &(st_gx_strip_bitmap), gcst);
    (*(st_device_memory).reloc_ptrs)(&mcdev->mdev, sizeof(mcdev->mdev), &(st_device_memory), gcst);
    if (mcdev->mdev.base != 0) {




        long diff = (char *)(((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(mcdev), gcst)) - (char *)mcdev;
        int i;
                        ...
                        int tx, int ty, gs_memory_t *mem)
{
    ...
    int buffer_height =
        ((16384 / (1 << 3)) * (1 << 3)) / (bits->raster + sizeof(byte *));

    if (mem == ((void*)0))
        gx_device_init_on_stack((gx_device *)cdev, (const gx_device *)proto,
                                tdev->memory);
    else
        gx_device_init((gx_device *)cdev, (const gx_device *)proto, mem, ((bool)1));
    ...
    for (;;) {
        ulong bitmap_size = ((unsigned long)~0L + (unsigned long)0);
        ...
        gdev_mem_data_size(&cdev->mdev, (&cdev->mdev)->width, (&cdev->mdev)->height, &bitmap_size);
        if (bitmap_size <= ((16384 / (1 << 3)) * (1 << 3)))
            break;
        buffer_height--;
    }
    cdev->mdev.base = cdev->buffer.bytes;
    return (*((&cdev->mdev)->procs.open_device))((gx_device *)&cdev->mdev);
}
