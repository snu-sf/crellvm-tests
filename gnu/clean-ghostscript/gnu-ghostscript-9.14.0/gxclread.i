...
s_band_read_init(stream_state * st)
{
    ...
    ss->b_this.pos = 0;
    io_procs->rewind(ss->page_info.bfile, ((bool)0), ss->page_info.bfname);
    return 0;
}
# 133 "./base/gxclread.c"
static int
...
rb:
        ...
        if (ss->b_this.band_min == (-1) &&
            ...
            if (!(ss->band_last >= bmin && ss->band_first <= bmax))
                goto rb;
            io_procs->fseek(cfile, pos, 0, ss->page_info.cfname);
...
static const stream_template s_band_read_template = {
    &st_stream_state, s_band_read_init, s_band_read_process, 1, 4096
};
# 287 "./base/gxclread.c"
static int
...
clist_close_writer_and_init_reader(gx_device_clist *cldev)
{
    ...
    if (crdev->ymin < 0) {
        ...
        ((base_mem)->procs.status(base_mem, &mem_status));
        if (mem_status.is_thread_safe == ((bool)0)) {
            return ((-25));
        }

        code = (crdev->icc_cache_cl = gsicc_cache_new(base_mem)) == ((void*)0) ? (-25) : code;
    }
    return code;
}
...
clist_find_pseudoband(gx_device_clist_reader *crdev, int band, cmd_block *cb)
{
    ...
    if (bfile == ((void*)0)) {
        ...
        if ((code=page_info->io_procs->DO_NOT_USE_FOPEN(page_info->cfname, fmode,
                            &page_info->cfile,
                            crdev->memory, crdev->memory, ((bool)1))) < 0 ||
             (code=page_info->io_procs->DO_NOT_USE_FOPEN(page_info->bfname, fmode,
                            &page_info->bfile,
                            crdev->memory, crdev->memory, ((bool)0))) < 0)
            return code;
            bfile = page_info->bfile;
    }

    start_pos = page_info->bfile_end_pos - sizeof(cmd_block);
    page_info->io_procs->fseek(bfile, start_pos, 0, page_info->bfname);
    while( 1 ) {
        page_info->io_procs->fread_chars(cb, sizeof(cmd_block), bfile);
        if (cb->band_max == band && cb->band_min == band) {
            page_info->io_procs->fseek(bfile, save_pos, 0, page_info->bfname);
            return(0);
        }
...
clist_read_chunk(gx_device_clist_reader *crdev, int64_t position, int size, unsigned char *buf)
{
    ...
    crdev->page_info.io_procs->fseek(cfile, position, 0, crdev->page_info.cfname);

    crdev->page_info.io_procs->fread_chars(buf, size, cfile);

    crdev->page_info.io_procs->fseek(cfile, save_pos, 0, crdev->page_info.cfname);
    return 0;
}
...
clist_read_color_usage_array(gx_device_clist_reader *crdev)
{
    ...
    if (crdev->color_usage_array != ((void*)0))
        ((crdev->memory)->procs.free_object(crdev->memory, crdev->color_usage_array, "clist reader color_usage_array"));

    crdev->color_usage_array = (gx_color_usage_t *)(*(crdev->memory)->procs.alloc_bytes)(crdev->memory, size_data, "clist reader color_usage_array");
...
clist_unserialize_icctable(gx_device_clist_reader *crdev, cmd_block *cb)
{
    ...
    if ( icc_table != ((void*)0) )
        return(0);
    save_pos = crdev->page_info.io_procs->ftell(cfile);
    crdev->page_info.io_procs->fseek(cfile, cb->pos, 0, crdev->page_info.cfname);
    ...
    buf = (*(crdev->memory)->procs.alloc_bytes)(crdev->memory, size_data, "clist_read_icctable");
    buf_start = buf;
    if (buf == ((void*)0))
        return gs_throw_imp(__func__, "./base/gxclread.c", 489, 1, -1, "insufficient memory for icc table buffer reader");

    clist_read_chunk(crdev, cb->pos + 4, size_data, buf);
    icc_table = (clist_icctable_t *)(*(stable_mem)->procs.alloc_struct)(stable_mem, &st_clist_icctable, "clist_read_icctable");

    if (icc_table == ((void*)0)) {
        ((stable_mem)->procs.free_object(stable_mem, buf_start, "clist_read_icctable"));
        return gs_throw_imp(__func__, "./base/gxclread.c", 496, 1, -1, "insufficient memory for icc table buffer reader");
    }
    icc_table->memory = stable_mem;
    icc_table->head = ((void*)0);
    icc_table->final = ((void*)0);

    icc_table->tablesize = number_entries;
    crdev->icc_table = icc_table;
    for (k = 0; k < number_entries; k++) {
        curr_entry = (clist_icctable_entry_t *)(*(stable_mem)->procs.alloc_struct)(stable_mem, &st_clist_icctable_entry, "clist_read_icctable");

        if (curr_entry == ((void*)0)) {
            ((stable_mem)->procs.free_object(stable_mem, buf_start, "clist_read_icctable"));
            return gs_throw_imp(__func__, "./base/gxclread.c", 509, 1, -1, "insufficient memory for icc table entry");
        }
        ...
        curr_entry->next = ((void*)0);
    }
    ((crdev->memory)->procs.free_object(crdev->memory, buf_start, "clist_read_icctable"));
    crdev->page_info.io_procs->fseek(cfile, save_pos, 0, crdev->page_info.cfname);
    return 0;
}
                         ...
                         gs_get_bits_params_t *params, gs_int_rect **unread)
{
    ...
    int num_planes =
        (options & (1L<<16) ? 1 :
         options & (1L<<17) ? dev->color_info.num_components :
         options & (1L<<18) ? dev->color_info.depth :
         0 );
                       ...
                       const gx_render_plane_t *render_plane, bool clear)
{
    ...
    crdev->icc_struct->pageneutralcolor = ((bool)0);

    for (i = 0; i < num_pages && code >= 0; ++i) {
        bool pdf14_needed = ((bool)0);
        int band;
        ...
        if (ppages == ((void*)0)) {
            ...
            page_info.cfile = page_info.bfile = ((void*)0);
            __builtin_strncpy (page_info.cfname, ppage->page->cfname, sizeof(page_info.cfname));
            __builtin_strncpy (page_info.bfname, ppage->page->bfname, sizeof(page_info.bfname));
...
# 836 "./base/gxclread.c"
        ...
        for (band=band_first; band <= band_last; band++)
            ...
            crdev->color_usage_array[band].trans_bbox.q.y) ? ((bool)1) : ((bool)0);
    ...
    crdev->icc_struct->pageneutralcolor = save_pageneutralcolor;
    return code;
}
...
{
    int code = 0;
    bool opened_bfile = ((bool)0);
    bool opened_cfile = ((bool)0);
    ...
    if (rs.page_info.cfile == 0) {
        code = crdev->page_info.io_procs->DO_NOT_USE_FOPEN(rs.page_info.cfname,
                           gp_fmode_rb, &rs.page_info.cfile, crdev->bandlist_memory,
                           crdev->bandlist_memory, ((bool)1));
        opened_cfile = (code >= 0);
    }
    if (rs.page_info.bfile == 0 && code >= 0) {
        code = crdev->page_info.io_procs->DO_NOT_USE_FOPEN(rs.page_info.bfname,
    ...
    if (rs.page_info.cfile != 0 && rs.page_info.bfile != 0) {
        ...
        s_init(&s, mem);
        s_std_init(&s, sbuf, 4096, &no_procs, 1);
        s.foreign = 1;
    ...
    if (opened_bfile && rs.page_info.bfile != 0)
        crdev->page_info.io_procs->fclose(rs.page_info.bfile, rs.page_info.bfname, ((bool)0));
    if (opened_cfile && rs.page_info.cfile != 0)
        crdev->page_info.io_procs->fclose(rs.page_info.cfile, rs.page_info.cfname, ((bool)0));
