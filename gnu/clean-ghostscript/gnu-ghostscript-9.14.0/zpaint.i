...
{
    return gs_fillpage((i_ctx_p->pgs));
}
...
zimagepath(i_ctx_t *i_ctx_p)
{
    ...
    if (((op)->tas.rsize) < ((op[-2].value.intval + 7) >> 3) * op[-1].value.intval)
        return ((-15));
    code = gs_imagepath((i_ctx_p->pgs),
