...
struct gs_image_enum_s {
    ...
    bool error;
    byte wanted[((64) + 1)];
    byte client_wanted[((64) + 1)];
    image_enum_plane_t planes[((64) + 1)];
    ...
    gx_image_plane_t image_planes[((64) + 1)];
};

static gs_ptr_type_t gs_image_enum_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void gs_image_enum_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_gs_image_enum = { sizeof(gs_image_enum), "gs_image_enum", 0, 0, gs_image_enum_enum_ptrs, gs_image_enum_reloc_ptrs, 0, 0 };




static
gs_ptr_type_t gs_image_enum_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_image_enum *eptr = vptr; switch ( index ) { default:
{

    index -= 2;
    if (index < eptr->num_planes)
        return (pep->ptr = (&((const gs_image_enum *)vptr)->planes[index].source)->data, pep->size = (&((const gs_image_enum *)vptr)->planes[index].source)->size, (&ptr_string_procs));
    index -= eptr->num_planes;
    if (index < eptr->num_planes)
        return (pep->ptr = (&((const gs_image_enum *)vptr)->planes[index].row)->data, pep->size = (&((const gs_image_enum *)vptr)->planes[index].row)->size, (&ptr_string_procs));
    return 0;
}
case 0: return (pep->ptr = (const void *)(((const gs_image_enum *)vptr)->dev), (&ptr_struct_procs));
case 1: return (pep->ptr = (const void *)(((const gs_image_enum *)vptr)->info), (&ptr_struct_procs));
} }
static void gs_image_enum_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_image_enum *eptr = vptr;
{
    int i;

    (((gs_image_enum *)vptr)->dev = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_image_enum *)vptr)->dev), gcst)));
    (((gs_image_enum *)vptr)->info = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_image_enum *)vptr)->info), gcst)));
    for (i = 0; i < eptr->num_planes; i++)
        (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(((gs_image_enum *)vptr)->planes[i].source), gcst));
    for (i = 0; i < eptr->num_planes; i++)
        (((*(const gc_procs_common_t **)(gcst))->reloc_string)(&(((gs_image_enum *)vptr)->planes[i].row), gcst));
}
}
...
is_image_visible(const gs_image_common_t * pic, gs_state * pgs, gx_clip_path *pcpath)
{
    ...
    if (pic->type->begin_typed_image == gx_begin_image1) {
        ...
        if (pic->ImageMatrix.xx == (*(const gs_matrix *)&(pgs)->ctm).xx &&
            pic->ImageMatrix.xy == (*(const gs_matrix *)&(pgs)->ctm).xy &&
            pic->ImageMatrix.yx == (*(const gs_matrix *)&(pgs)->ctm).yx &&
            pic->ImageMatrix.yy == (*(const gs_matrix *)&(pgs)->ctm).yy) {

            mat.xx = mat.yy = 1.;
            mat.yx = mat.xy = 0.;
            mat.tx = (*(const gs_matrix *)&(pgs)->ctm).tx - pic->ImageMatrix.tx;
            mat.ty = (*(const gs_matrix *)&(pgs)->ctm).ty - pic->ImageMatrix.ty;
        } else {
            code = gs_matrix_invert(&pic->ImageMatrix, &mat);
            if (code < 0)
                return code;
            code = gs_matrix_multiply(&mat, &(*(const gs_matrix *)&(pgs)->ctm), &mat);
            if (code < 0)
                return code;
        }
                     ...
                     bool uses_color, gx_image_enum_common_t ** ppie)
{
    ...
    gx_device *dev2 = dev;
    gx_device_color dc_temp, *pdevc = ((pgs)->color[0].dev_color);

    if (code < 0)
        return code;

    ((pgs->device)->procs.set_graphics_type_tag)(pgs->device, GS_IMAGE_TAG);
    ...
    if (pic->type->begin_typed_image == &gx_begin_image1) {
        ...
        if(image->ImageMask) {
            code = gx_image_fill_masked_start(dev, ((pgs)->color[0].dev_color), pcpath, pgs->memory, &dev2);
            if (code < 0)
                return code;
        }
        if (dev2 != dev) {
            ((&dc_temp)->colors.pure = (1), (&dc_temp)->type = (&gx_dc_type_data_pure)); (&dc_temp)->ccolor_valid = ((bool)0);
            pdevc = &dc_temp;
        }
    }
    code = ((*((dev2)->procs.begin_typed_image)) (dev2, (const gs_imager_state *)pgs, ((void*)0), pic, ((void*)0), pdevc, pcpath, pgs->memory, ppie));
...
{
    gs_image_enum *penum =
        (gs_image_enum *)(*(mem)->procs.alloc_struct)(mem, &st_gs_image_enum, cname);
...
{
    return ((penum->memory)->procs.stable(penum->memory));
}
...
free_row_buffers(gs_image_enum *penum, int num_planes, client_name_t cname)
{
    ...
    for (i = num_planes - 1; i >= 0; --i) {
        ...
        (*(gs_image_row_memory(penum))->procs.free_string)(gs_image_row_memory(penum), penum->planes[i].row.data, penum->planes[i].row.size, cname);
...
# 517 "./base/gsimage.c"
    ...
    for (;;) {
        ...
        int h = (penum->wanted_varies ? 1 : (~(-1 << ((1 << 2) * 8 - 1))));
        ...
        for (i = 0; i < num_planes; ++i) {
            ...
            if (size > 0) {
                if (pos < raster && (pos != 0 || size < raster)) {
                    ...
                    int copy = (((size) < (raster - pos)) ? (size) : (raster - pos));
                    ...
                    if (raster > old_size) {
                        ...
                        byte *row =
                            (old_data == 0 ?
                             (*(mem)->procs.alloc_string)(mem, raster, "gs_image_next(row)") :

                             (*(mem)->procs.resize_string)(mem, old_data, old_size, raster, "gs_image_next(row)"));
...
gs_image_cleanup(gs_image_enum * penum, gs_state *pgs)
{
    ...
    if (penum->info != 0) {
        if (((penum->info->dev)->procs.dev_spec_op)(penum->info->dev,
                    gxdso_pattern_is_cpath_accum, ((void*)0), 0)) {

            gx_device *cdev = penum->info->dev;

            code = gx_image_end(penum->info, !penum->error);
            code1 = gx_image_fill_masked_end(cdev, penum->dev, ((pgs)->color[0].dev_color));
            if (code == 0)
                code = code1;
        } else
            code = gx_image_end(penum->info, !penum->error);
...
gs_image_cleanup_and_free_enum(gs_image_enum * penum, gs_state *pgs)
{
    ...
    ((penum->memory)->procs.free_object(penum->memory, penum, "gs_image_cleanup_and_free_enum"));
    return code;
}
