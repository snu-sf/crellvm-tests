...
zinustroke(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    if ((spop = upath_stroke(i_ctx_p, &mat, ((bool)0))) < 0) {
        gs_grestore((i_ctx_p->pgs));
        return spop;
    }
    ...
    if (npop > 1)
        code = gs_concat((i_ctx_p->pgs), &mat);
    if (code >= 0)
        code = gs_stroke((i_ctx_p->pgs));
...
in_path(os_ptr oppath, i_ctx_t *i_ctx_p, gx_device * phdev)
{
    ...
    if (code >= 0) {
        ...
        gs_transform((i_ctx_p->pgs), uxy[0], uxy[1], &dxy);
        fr.p.x = ((((fixed)((dxy.x)*(float)(1<<8))))&(-1L<<8));
        fr.p.y = ((((fixed)((dxy.y)*(float)(1<<8))))&(-1L<<8));
        fr.q.x = fr.p.x + (1L << 8);
        fr.q.y = fr.p.y + (1L << 8);
        code = gx_clip_to_rectangle((i_ctx_p->pgs), &fr);
        npop = 2;
    } else if (code == (-17)) {
        ...
        gx_path *ipath = (i_ctx_p->pgs)->path;
        gx_path save;

        (void)gx_path_init_local_shared(&save, ((void*)0), ((gs_memory_t *)((i_ctx_p->memory).current)));
        gx_path_assign_preserve(&save, ipath);
        gs_newpath((i_ctx_p->pgs));
        code = upath_append(oppath, i_ctx_p, ((bool)0));
        if (code >= 0)
            code = gx_clip_to_path((i_ctx_p->pgs));
        gx_path_assign_free((i_ctx_p->pgs)->path, &save);
        npop = 1;
    }
    if (code < 0) {
        gs_grestore((i_ctx_p->pgs));
    ...
    gx_set_device_color_1((i_ctx_p->pgs));
    gx_device_init_on_stack((gx_device *) phdev, (const gx_device *)&gs_hit_device,
                            ((gs_memory_t *)((i_ctx_p->memory).current)));
    phdev->width = phdev->height = (~(-1 << ((1 << 2) * 8 - 1)));
    gx_device_fill_in_procs(phdev);
    gx_set_device_only((i_ctx_p->pgs), phdev);
    return npop;
}
...
in_path_result(i_ctx_t *i_ctx_p, int npop, int code)
{
    ...
    op -= npop;
    ((op)->value.boolval = (result), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
...
in_upath(i_ctx_t *i_ctx_p, gx_device * phdev)
{
    ...
    if (code < 0)
        return code;
    if ((code = upath_append(op, i_ctx_p, ((bool)0))) < 0 ||
...
static const up_data_t up_data[11 + 1] = {
    {4, UPS_INITIAL | UPS_UCACHE, UPS_SETBBOX},
    {2, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {2, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {2, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {2, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {6, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {6, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {5, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {5, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {5, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {0, UPS_SETBBOX | UPS_PATH, UPS_PATH},
    {0, UPS_INITIAL, UPS_UCACHE}
};
...
static const op_proc_t up_ops[11 + 1] = {
...
zuappend(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    if ((code = upath_append(op, i_ctx_p, ((bool)0))) >= 0)
        code = gs_upmergepath((i_ctx_p->pgs));
    gs_grestore((i_ctx_p->pgs));
    if (code < 0)
        return code;
...
zueofill(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    if ((code = upath_append(op, i_ctx_p, gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current))))) >= 0)
        code = gs_eofill((i_ctx_p->pgs));
    gs_grestore((i_ctx_p->pgs));
    if (code < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
...
zustroke(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    if ((code = npop = upath_stroke(i_ctx_p, ((void*)0), gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current))))) >= 0)
        code = gs_stroke((i_ctx_p->pgs));
    gs_grestore((i_ctx_p->pgs));
    if (code < 0)
        return code;
...
zustrokepath(i_ctx_t *i_ctx_p)
{
    ...
    int npop, code = gs_currentmatrix((i_ctx_p->pgs), &saved_matrix);

    if (code < 0)
        return code;

    (void)gx_path_init_local_shared(&save, ((void*)0), ((gs_memory_t *)((i_ctx_p->memory).current)));
    gx_path_assign_preserve(&save, (i_ctx_p->pgs)->path);
    if ((code = npop = upath_stroke(i_ctx_p, ((void*)0), ((bool)0))) < 0 ||
        (code = gs_strokepath((i_ctx_p->pgs))) < 0
        ) {
        gx_path_assign_free((i_ctx_p->pgs)->path, &save);
        return code;
    }
    ...
    if (npop > 1) {
        if ((code = gs_setmatrix((i_ctx_p->pgs), &saved_matrix)) < 0) {
            gx_path_assign_free((i_ctx_p->pgs)->path, &save);
            return code;
        }
    }
...
zupath(i_ctx_t *i_ctx_p)
{
    ...
    return make_upath(i_ctx_p, op, (i_ctx_p->pgs), (i_ctx_p->pgs)->path, op->value.boolval);
}
           ...
           bool with_ucache)
{
    ...
    if ((code = gs_upathbbox(pgs, &bbox, ((bool)1))) < 0) {
        ...
        if (code != (-14) || gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current))))
            return code;
    ...
    code = gs_alloc_ref_array(((i_ctx_p->memory).current), rupath, (0x10 + 0x20 +0x40) | 0x80, size, "make_upath");
    ...
    if (with_ucache) {
        if ((code = names_enter_string(pgs->memory->gs_lib_ctx->gs_name_table, "ucache", next)) < 0)
            return code;
        ((next)->tas.type_attrs |= (0x80 | 2));
        ++next;
    }
    ((next)->value.realval = (bbox.p.x), ((next)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
    ((next + 1)->value.realval = (bbox.p.y), ((next + 1)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
    ((next + 2)->value.realval = (bbox.q.x), ((next + 2)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
    ((next + 3)->value.realval = (bbox.q.y), ((next + 3)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
    next += 4;
    if ((code = names_enter_string(pgs->memory->gs_lib_ctx->gs_name_table, "setbbox", next)) < 0)
        return code;
    ((next)->tas.type_attrs |= (0x80 | 2));
    ++next;
    {
        ...
        pgs->path = ppath;
        gs_path_enum_copy_init(pgs->memory, &penum, pgs, ((bool)0));
        pgs->path = save_path;
        while ((op = gs_path_enum_next(&penum, pts)) != 0) {
            ...
            switch (op) {
                ...
                case 2:
                    opstr = "lineto";
                  ml:((next)->value.realval = (pts[0].x), ((next)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 1)->value.realval = (pts[0].y), ((next + 1)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    next += 2;
                    break;
                case 3:
                    opstr = "curveto";
                    ((next)->value.realval = (pts[0].x), ((next)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 1)->value.realval = (pts[0].y), ((next + 1)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 2)->value.realval = (pts[1].x), ((next + 2)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 3)->value.realval = (pts[1].y), ((next + 3)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 4)->value.realval = (pts[2].x), ((next + 4)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    ((next + 5)->value.realval = (pts[2].y), ((next + 5)->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                    next += 6;
                    break;
                case 4:
                    opstr = "closepath";
            ...
            if ((code = names_enter_string(pgs->memory->gs_lib_ctx->gs_name_table, opstr, next)) < 0)
                return code;
            ((next)->tas.type_attrs |= (0x80));
            ++next;
        }
    }
    return 0;
...
zgetpath(i_ctx_t *i_ctx_p)
{
    ...
    path_size = code = path_length_for_upath((i_ctx_p->pgs)->path);
    if (code < 0)
        return code;
    leaf_count = (path_size + (16*1024*1024) - 1) / (16*1024*1024);
    code = gs_alloc_ref_array(((i_ctx_p->memory).current), op, (0x10 + 0x20 +0x40), leaf_count, "zgetpath_master");
    if (code < 0)
        return code;
    if (path_size == 0)
        return 0;

    if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "moveto", &operators[1]) <= 0 ||
        dict_find_string((&(i_ctx_p->dict_stack).system_dict), "lineto", &operators[2]) <= 0 ||
        dict_find_string((&(i_ctx_p->dict_stack).system_dict), "curveto", &operators[3]) <= 0 ||
        dict_find_string((&(i_ctx_p->dict_stack).system_dict), "closepath", &operators[4]) <= 0)
          return ((-21));

    main_ref = op->value.refs;
    for (i = 0; i < leaf_count; i++) {
       int leaf_size = ( i == leaf_count - 1) ? path_size - i * (16*1024*1024) : (16*1024*1024);
       code = gs_alloc_ref_array(((i_ctx_p->memory).current), &main_ref[i], (0x10 + 0x20 +0x40) | 0x80, leaf_size, "zgetpath_leaf");
       if (code < 0)
            return code;
    }

    {
        ...
        main_ref = op->value.refs;
        gs_path_enum_copy_init((i_ctx_p->pgs)->memory, &penum, (i_ctx_p->pgs), ((bool)0));
        ...
        for (i = 0; i < leaf_count; i++) {
            ...
            for (j = 0; j < leaf_size; j++) {
                if (k < oper_count[pe])
                   ((&leaf_ref[j])->value.realval = ((float)*fts[k++]), ((&leaf_ref[j])->tas.type_attrs = ((t_real) << 8) + ((0)|((&(i_ctx_p->memory))->new_mask))) );
                else {
                    k = 0;
                    (*(&leaf_ref[j]) = *(operators[pe]));
                    pe = gs_path_enum_next(&penum, pts);
                    if (pe <= 0)
                        return pe;
                    if (pe >= 5)
...
upath_append_aux(os_ptr oppath, i_ctx_t *i_ctx_p, int *pnargs, bool upath_compat)
{
    ...
    if (((((const byte *)&((oppath)->tas.type_attrs))[1]) == (t__invalid)))
        return ((-17));
    if (!(((oppath)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return ((-20));
    do { if ( !(((&*oppath)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    gs_newpath((i_ctx_p->pgs));


    if ( ((oppath)->tas.rsize) == 2 &&
         array_get(((gs_memory_t *)((i_ctx_p->memory).current)), oppath, 1, &opcodes) >= 0 &&
         ((((const byte *)&((&opcodes)->tas.type_attrs))[1]) == (t_string))
        ) {
        ref operands;
        int code, format;
        int repcount = 1;
        ...
        array_get(((gs_memory_t *)((i_ctx_p->memory).current)), oppath, 0, &operands);
        code = num_array_format(&operands);
        if (code < 0)
            return code;
        format = code;
        do { if ( !(((&opcodes)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        opp = opcodes.value.bytes;
        ocount = ((&opcodes)->tas.rsize);
        while (ocount--) {
            byte opx = *opp++;
                ...
                *pnargs = 0;
                ...
                do {
                    ...
                    while (opargs--) {
                        do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
                        (*pnargs)++;
                        code = num_array_get(((gs_memory_t *)((i_ctx_p->memory).current)), &operands, format, i++, op);
                        switch (code) {
                            case t_integer:
                                ((op)->tas.type_attrs = ((t_integer) << 8) + (0));
                                break;
                            case t_real:
                                ((op)->tas.type_attrs = ((t_real) << 8) + (0));
                                break;
                            default:
                                return ((-20));
                        }
        ...
        for (; index < ocount; index++) {
            ...
            *pnargs = argcount;
            array_get(((gs_memory_t *)((i_ctx_p->memory).current)), arp, index, &rup);
            switch ((((const byte *)&((&rup)->tas.type_attrs))[1])) {
                case t_integer:
                case t_real:
                    argcount++;
                    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
                    *op = rup;
                    break;
                case t_name:
                    if (!(((&rup)->tas.type_attrs) & (0x80)) ||
                        dict_find((&(i_ctx_p->dict_stack).system_dict), &rup, &defp) <= 0 ||
                        ((defp)->tas.type_attrs >= (t_next_index << 8) ? t_operator : (((const byte *)&((defp)->tas.type_attrs))[1])) != t_operator)
                        return ((-20));
                    goto xop;
                case t_operator:
                    defp = &rup;
                  xop:if (!(((defp)->tas.type_attrs) & (0x80)))
                        return ((-20));
                    oproc = ((defp)->value.opproc);
                    for (opx = 0; opx <= 11; opx++)
                        if (oproc == up_ops[opx])
                            break;
                    if (opx > 11)
                        return ((-20));
...
upath_append(os_ptr oppath, i_ctx_t *i_ctx_p, bool upath_compat)
{
    ...
    if (code < 0) {
        ...
        ((((i_ctx_p->op_stack).stack).p) -= (nargs));
        return code;
    }
    return 0;
}
