...
sfopen(const char *path, const char *mode, gs_memory_t *mem)
{
    ...
    if (code < 0) {
        ...
        errwrite(mem, "sfopen: gs_parse_file_name failed.\n", strlen("sfopen: gs_parse_file_name failed.\n"));

        return ((void*)0);
    }
    if (pfn.fname == ((void*)0)) {

        errwrite(mem, "sfopen: not allowed with %device only.\n", strlen("sfopen: not allowed with %device only.\n"));
    ...
    if (code < 0) {
        ...
        sclose(s);
        ((s->memory)->procs.free_object(s->memory, s, "sfopen: allocation error"));

        errwrite(mem, "sfopen: allocation error setting path name into stream.\n", strlen("sfopen: allocation error setting path name into stream.\n"));
...
sfgetc(stream *s)
{
    int code = ((int)((s)->cursor.r.limit - (s)->cursor.r.ptr > 1 ? (++((s)->cursor.r.ptr), (int)*(s)->cursor.r.ptr) : spgetcc(s, ((bool)1))));
...
sfseek(stream *s, gs_offset_t offset, int whence)
{
    ...
    if ((((s)->modes & 4) != 0) || newpos == stell(s)) {
        return spseek(s, (gs_offset_t)(newpos));
    }
    return -1;
}
...
sfclose(stream *s)
{
    ...
    sclose(s);
    ((mem)->procs.free_object(mem, s, "sfclose(stream)"));
    return 0;
}
