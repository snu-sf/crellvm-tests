...
int z1_glyph_info(gs_font *font, gs_glyph glyph, const gs_matrix *pmat, int members, gs_glyph_info_t *info);

static int z1_same_font(const gs_font *font, const gs_font *ofont, int mask);
                   ...
                   gs_glyph * pglyph)
{
    ...
    const ref *pcsdict = &((font_data *)((pfont1)->client_data))->CharStrings;
...
charstring_font_get_refs(const_os_ptr op, charstring_font_refs_t *pfr)
{
    ...
    if (dict_find_string(op, "Private", &pfr->Private) <= 0 ||
        !((((const byte *)&((pfr->Private)->tas.type_attrs))[1]) == (t_dictionary))
        )
        return ((-10));
    ((&pfr->no_subrs)->value.refs = ((ref *)((void*)0)), ((&pfr->no_subrs)->tas.type_attrs = ((t_array) << 8) + (0)), ((&pfr->no_subrs)->tas.rsize = (0)));
    if (dict_find_string(pfr->Private, "OtherSubrs", &pfr->OtherSubrs) > 0) {
        if (!(((pfr->OtherSubrs)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
            return ((-20));
    } else
        pfr->OtherSubrs = &pfr->no_subrs;
    if (dict_find_string(pfr->Private, "Subrs", &pfr->Subrs) > 0) {
        if (!(((pfr->Subrs)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
            return ((-20));
    } else
                       ...
                       gs_type1_data *pdata1)
{
    ...
    if ((code = dict_uint_param(pprivate, "subroutineNumberBias",
         0, ((unsigned int)~0 + (unsigned int)0), pdata1->subroutineNumberBias, &pdata1->subroutineNumberBias)) < 0)
        return code;
    ...
    if ((code = pdata1->BlueValues.count = dict_float_array_param(mem, pprivate, "BlueValues",
                7 * 2, &pdata1->BlueValues.values[0], ((void*)0))) < 0)
        return code;
                     ...
                     const gs_type1_data *pdata1)
{
    ...
    pfont->data = *pdata1;
    pfont->data.parent = ((void*)0);
    (*(&pdata->u.type1.OtherSubrs) = *(pfr->OtherSubrs));
    (*(&pdata->u.type1.Subrs) = *(pfr->Subrs));
    (*(&pdata->u.type1.GlobalSubrs) = *(pfr->GlobalSubrs));
...
{
    int code = charstring_font_params(((gs_memory_t *)((i_ctx_p->memory).current)), op, pfr, pdata1);
    gs_font_type1 *pfont;
...
zbuildfont1(i_ctx_t *i_ctx_p)
{
    ...
    int code = build_proc_name_refs(((gs_memory_t *)((i_ctx_p->memory).current)), &build,
               ...
               const char *key)
{
    ...
    return (present == opresent &&
            (present <= 0 || obj_eq(((gs_memory_t *)((&(pdict)->memory)->value.pstruct)), pvalue, povalue)));
}
static int
z1_same_font(const gs_font *font, const gs_font *ofont, int mask)
{
        ...
        return mask;
    {
        ...
        const font_data *const pdata = ((font_data *)((pfont1)->client_data));
        const gs_font_type1 *pofont1 = (const gs_font_type1 *)ofont;
        const font_data *const podata = ((font_data *)((pofont1)->client_data));
