# 1 "./base/gsicc_replacecm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gsicc_replacecm.c" 2
# 19 "./base/gsicc_replacecm.c"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
# 1 "./base/stdpre.h" 1
# 253 "./base/stdpre.h"
                             ...
                             int num_bytes_in, int num_bytes_out)
{
    ...
    if (num_bytes_in == 2) {
        ...
        for (k = 0; k < num_in; k++) {
            frac_in[k] = ((frac)0x7ff8) - ((frac)( ((data[k]) >> (((1 << 1) * 8) - 15)) - ((data[k]) >> (((1 << 1) * 8) - 3)) ));
        }
    } else {
        byte *data = (byte *) inputcolor;
        for (k = 0; k < num_in; k++) {
            frac_in[k] = ((frac)0x7ff8) - ((frac)( ((frac)(data[k]) << (15 - (8))) + ((data[k]) >> ((8) * 2 - 15)) - ((data[k]) >> ((8) - 3)) ));
        }
    }
    ...
    switch (num_in) {
        case 1:
            ...
            break;
        case 3:
            (link->cm_procs.map_rgb)(dev, ((void*)0), frac_in[0], frac_in[1],
                                 frac_in[2], frac_out);
            break;
        case 4:
...
gsicc_rcm_freelink(gsicc_link_t *icclink)
{
    ...
    ((rcm_link->memory)->procs.free_object(rcm_link->memory, rcm_link, "gsicc_rcm_freelink"));
}
                   ...
                   gsicc_colorbuffer_t data_cs)
{
    ...
    rcm_link = (rcm_link_t *) (*(mem)->procs.alloc_bytes)(mem, sizeof(rcm_link_t), "gsicc_rcm_get_link");

    if (rcm_link == ((void*)0))
        return ((void*)0);
    result->link_handle = (void*) rcm_link;
    rcm_link->memory = mem;
    rcm_link->num_out = (((dev->color_info.num_components) < ((64))) ? (dev->color_info.num_components) : ((64)));
