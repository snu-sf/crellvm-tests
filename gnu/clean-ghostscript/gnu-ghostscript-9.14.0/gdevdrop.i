...
# 143 "./base/gdevdrop.c"
    ...
    draster = ((uint)((((width * depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    max_height = max_rop_bitmap / draster;
    if (max_height == 0)
        max_height = 1;
    block_height = (((height) < (max_height)) ? (height) : (max_height));
    if (planar_height > 0)
    ...
    if (dev->is_planar)
    {
        ...
        draster = ((uint)((((width * planes[0].depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
        code = gdev_mem_set_planar(pmdev, num_comp, planes);
        if (code < 0)
            return code;
        is_planar = 1;
    }
    code = (*((pmdev)->procs.open_device))((gx_device *)pmdev);
    pmdev->is_open = ((bool)1);
    if (code < 0)
        return code;
    if (( ((((gs_transparent_rop(lop)) << 1) ^ (gs_transparent_rop(lop))) & rop3_D) != 0 )) {
        row = (*(mem)->procs.alloc_bytes)(mem, draster * block_height, "copy_rop row");
        if (row == 0) {
            code = ((-25));
            goto out;
        }
    ...
    for (py = y; py < y + height; py += block_height) {
        ...
        if (row ) {
            ...
            bit_params.options =
                (1L<<0) | (1L<<4) | ((1L<<8) | (1L<<9) | (1L<<10) | (1L<<11) | (1L<<12) | (1L<<13)) |
                (1L<<16) | ((1L<<20) | (1L<<21)) | (1L<<22) |
                (1L<<24) | (1L<<26) | (1L<<28);
            bit_params.data[0] = row;
            bit_params.x_offset = 0;
            code = (*((dev)->procs.get_bits_rectangle))
                (dev, &rect, &bit_params, ((void*)0));
            if (code < 0)
                break;
            code = (*((pmdev)->procs.copy_color))
                ((gx_device *)pmdev, bit_params.data[0], bit_params.x_offset,
                 draster, 0L, 0, 0, width,
                 block_height);
            if (code < 0)
                return code;
        }
        if (planar_height == 0) {
            code = (*((pmdev)->procs.strip_copy_rop))
                        ((gx_device *)pmdev,
            ...
            code = (*((pmdev)->procs.strip_copy_rop2))
                        ((gx_device *)pmdev,
                         sdata + (py - y) * sraster, sourcex, sraster,
                         0L, scolors, textures, tcolors,
                         0, 0, width, block_height,
        ...
        if (code < 0)
            break;
        if (is_planar) {
            code = (*((dev)->procs.copy_planes))
                            (dev, ((pmdev)->line_ptrs[0]), 0,
                             draster, 0L,
...
out:
    ((mem)->procs.free_object(mem, row, "copy_rop row"));
    gx_device_retain((gx_device *)pmdev, ((bool)0));
    return code;
}
                          ...
                          const gx_color_index * colors, int depth)
{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        (*((dev)->procs.map_color_rgb)) (dev, colors[i], rgb);
        pixel = ((gx_color_value)((((unsigned int)rgb[0])*0xFF01 + 0x800000)>>24));
        if (depth > 8) {
            pixel = (pixel << 16) +
                (((gx_color_value)((((unsigned int)rgb[1])*0xFF01 + 0x800000)>>24)) << 8) +
                ((gx_color_value)((((unsigned int)rgb[2])*0xFF01 + 0x800000)>>24));
        }
        real_colors[i] = pixel;
    }
}
                                    ...
                                    int src_depth)
{
    ...
    for (plane = 0; plane < 4; plane++) {
        byte *dest = ((dev)->line_ptrs[y + plane * dev->height]);
...
map_rgb_to_color_via_cmyk(gx_device * dev, const gx_color_value rgbcv[])
{
    ...
    cmykcv[0] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - rgbcv[0];
    cmykcv[1] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - rgbcv[1];
    cmykcv[2] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - rgbcv[2];
    cmykcv[3] = (cmykcv[0] < cmykcv[1] ? (((cmykcv[0]) < (cmykcv[2])) ? (cmykcv[0]) : (cmykcv[2])) : (((cmykcv[1]) < (cmykcv[2])) ? (cmykcv[1]) : (cmykcv[2])));

    cmykcv[0] -= cmykcv[3];
    cmykcv[1] -= cmykcv[3];
    cmykcv[2] -= cmykcv[3];

    return (*((dev)->procs.map_cmyk_color)) (dev, cmykcv);
}
static void
...
{
    byte *dest = ((dev)->line_ptrs[y]);
    gx_color_index (*map)(gx_device *dev, const gx_color_value cv[]) =
        (dev->color_info.num_components == 4 ?
         map_rgb_to_color_via_cmyk : ((dev)->procs.map_rgb_color));
    ...
    for (x = width; --x >= 0;) {
        ...
        for (;;) {
            gx_color_value cv[3];
            cv[0] = (((vr) << ((sizeof(gx_color_value) * 8) - 8)) + ((vr) >> (16 - (sizeof(gx_color_value) * 8))));
            cv[1] = (((vg) << ((sizeof(gx_color_value) * 8) - 8)) + ((vg) >> (16 - (sizeof(gx_color_value) * 8))));
            cv[2] = (((vb) << ((sizeof(gx_color_value) * 8) - 8)) + ((vb) >> (16 - (sizeof(gx_color_value) * 8))));
            pixel = (*map) ((gx_device *)dev, cv);
            if (pixel != ((gx_color_index)(~0)))
                break;
...
mem_default_strip_copy_rop2(gx_device * dev,
                            ...
                            uint planar_height)
{
    (dmprintf_file_and_line(dev->memory,"./base/gdevdrop.c", 606), errprintf(dev->memory,"mem_default_strip_copy_rop2 should never be called!\n"));
    return (-100);
}
...
{
    int depth = dev->color_info.depth;
    int rop_depth = (( (dev)->color_info.num_components > 1 || (dev)->color_info.gray_index == 0xff ) ? 24 : 8);
    ...
    const gx_bitmap_format_t no_expand_t_options =
        ...
        ((1L<<20) | (1L<<21)) | (1L<<22) |
        (1L<<24) | (1L<<26) | (1L<<28) |
        ((textures && textures->num_planes > 1) ? (1L<<17) : (1L<<16));
    const gx_bitmap_format_t expand_options =
        (rop_depth > 8 ? (1L<<2) : (1L<<1)) |
        (1L<<4) | (1L<<11) |
        (1L<<16) | (1L<<20) | (1L<<22) |
        (1L<<24) | (1L<<28);
    gs_memory_t *mem = dev->memory;
    ...
    uint row_raster = ((uint)((((width * depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    ulong size_from_mem_device;
    gs_rop3_t trans_rop = gs_transparent_rop(lop);
    bool uses_d = ( ((((trans_rop) << 1) ^ (trans_rop)) & rop3_D) != 0 );
    bool uses_s = ( ((((trans_rop) << 2) ^ (trans_rop)) & 0xcc) != 0 );
    bool uses_t = ( ((((trans_rop) << 4) ^ (trans_rop)) & 0xf0) != 0 );
...
# 695 "./base/gdevdrop.c"
    if (!tdev->is_planar || tdev->color_info.num_components <= 1) {
        if ((rop_depth == 24) && (((dev)->procs.dev_spec_op)(dev,
    ...
    gs_make_mem_device(&mdev, mdproto, mem, -1, ((void*)0));
    gx_device_retain((gx_device *)&mdev, ((bool)1));
    mdev.width = width;
    ...
    code = (*((&mdev)->procs.open_device))((gx_device *)&mdev);
    if (code < 0)
        return code;
    do { uint num_bytes = (row_raster) * block_height; if (num_bytes <= sizeof(dest_buffer)) row = (byte *)dest_buffer; else { row = (*(mem)->procs.alloc_bytes)(mem, num_bytes, "copy_rop row"); if (row == 0) { code = ((-25)); goto out; } } } while (0);

    if (expand_s) {
        source_row_raster = ((uint)((((width * rop_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
        do { uint num_bytes = (source_row_raster) * block_height; if (num_bytes <= sizeof(source_buffer)) source_row = (byte *)source_buffer; else { source_row = (*(mem)->procs.alloc_bytes)(mem, num_bytes, "copy_rop source_row"); if (source_row == 0) { code = ((-25)); goto out; } } } while (0);
    ...
    if (expand_t) {
        texture_row_raster = ((uint)((((textures->rep_width * rop_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
        do { uint num_bytes = (texture_row_raster) * block_height; if (num_bytes <= sizeof(texture_buffer)) texture_row = (byte *)texture_buffer; else { texture_row = (*(mem)->procs.alloc_bytes)(mem, num_bytes, "copy_rop texture_row"); if (texture_row == 0) { code = ((-25)); goto out; } } } while (0);
        ...
        if (rop_texture.size.y > rop_texture.rep_height)
            rop_texture.size.y = rop_texture.rep_height;
    }
    if (tcolors && uses_t) {
    ...
    for (py = y; py < y + height; py += loop_height) {
        ...
        if (expand_t) {
            ...
            loop_height = (((block_height) < (rop_texture.size.y - rep_y)) ? (block_height) : (rop_texture.size.y - rep_y));
        ...
        if (uses_d) {
            ...
            bit_params.data[0] = ((&mdev)->line_ptrs[0]);
            bit_params.x_offset = 0;
        ...
        code = (*((&mdev)->procs.strip_copy_rop))
            ((gx_device *)&mdev, source_data, sx, source_raster,
             0L, real_scolors, real_texture, real_tcolors,
        ...
        {
            int i;
            const byte *unpacked = ((&mdev)->line_ptrs[0]);
...
out:
    if (texture_row != 0 && texture_row != (byte *)texture_buffer)
        ((mem)->procs.free_object(mem, texture_row, "copy_rop texture_row"));
    if (source_row != 0 && source_row != (byte *)source_buffer)
        ((mem)->procs.free_object(mem, source_row, "copy_rop source_row"));
    if (row != 0 && row != (byte *)dest_buffer)
        ((mem)->procs.free_object(mem, row, "copy_rop row"));
    (*((&mdev)->procs.close_device)) ((gx_device *) & mdev);
    return code;
}
...
gx_strip_copy_rop_unaligned(gx_device * dev,
           ...
           const gx_strip_bitmap * textures, const gx_color_index * tcolors,
                       ...
                       int phase_x, int phase_y, gs_logical_operation_t lop)
{
    int (*copy_rop)(gx_device *dev, const byte *sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index *scolors, const gx_strip_bitmap *textures, const gx_color_index *tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop) = ((dev)->procs.strip_copy_rop);
    int depth = (scolors == 0 ? dev->color_info.depth : 1);
    int step = sraster & ((1 << 3) - 1);


    if (sdata != 0) {
        uint offset =
        (uint) (sdata - (const byte *)0) & ((1 << 3) - 1);
...

    {
        ...
        for (i = 0; i < height && code >= 0;
            ...
            code = (*copy_rop) (dev, p, d, sraster, 0L, scolors,
...
# 1006 "./base/gdevdrop.c"
    gs_rop3_t mask =
    (((lop & 0x100) && ( ((((rop) << 2) ^ (rop)) & 0xcc) != 0 )) ?
     (((lop & 0x200) && ( ((((rop) << 4) ^ (rop)) & 0xf0) != 0 )) ? ((0xcc) ^ rop3_1) & ((0xf0) ^ rop3_1) : ((0xcc) ^ rop3_1)) :
     (((lop & 0x200) && ( ((((rop) << 4) ^ (rop)) & 0xf0) != 0 )) ? (((((0xcc) ^ rop3_1)) ^ rop3_1) | ((0xf0) ^ rop3_1)) : rop3_1));
