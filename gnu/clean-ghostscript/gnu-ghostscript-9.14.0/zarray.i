...
zarray(i_ctx_t *i_ctx_p)
{
    ...
    code = gs_alloc_ref_array(((i_ctx_p->memory).current), (ref *)op, (0x10 + 0x20 +0x40), size, "array");
    if (code < 0)
        return code;
    refset_null_new(op->value.refs, size, ((&(i_ctx_p->memory))->new_mask));
    return 0;
}
...
zaload(i_ctx_t *i_ctx_p)
{
    ...
    (*(&aref) = *(op));
    if (!(((&aref)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return (check_type_failed(op));
    do { if ( !(((&aref)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    asize = ((&aref)->tas.rsize);
    if (asize > (((i_ctx_p->op_stack).stack).top) - op) {
        int code = ref_stack_push(&((i_ctx_p->op_stack).stack), asize);
        uint i;
        const ref_packed *packed = aref.value.packed;

        if (code < 0)
            return code;
        for (i = asize; i > 0; i--, packed = ((*(const ref_packed *)(packed) >= ((ref_packed)(2) << 13)) ? packed + 1 : packed + (sizeof(ref) / sizeof(ref_packed))))
            packed_get(((gs_memory_t *)((i_ctx_p->memory).current)), packed, ref_stack_index(&((i_ctx_p->op_stack).stack), i));
        *(((i_ctx_p->op_stack).stack).p) = aref;
        return 0;
    }
    if (((((const byte *)&((&aref)->tas.type_attrs))[1]) == (t_array)))
        ...
        for (i = 0; i < asize; i++, pdest++, packed = ((*(const ref_packed *)(packed) >= ((ref_packed)(2) << 13)) ? packed + 1 : packed + (sizeof(ref) / sizeof(ref_packed))))
            packed_get(((gs_memory_t *)((i_ctx_p->memory).current)), packed, pdest);
    }
    do { if ( (op += (asize)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (asize); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    (*(op) = *(&aref));
    return 0;
}
...
zastore(i_ctx_t *i_ctx_p)
{
    ...
    if (!(((op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_array) << 8) + (0x10))))
        return ((-7));
    if (size > op - (((i_ctx_p->op_stack).stack).bot)) {

        ref arr;

        if (size >= ref_stack_count(&((i_ctx_p->op_stack).stack)))
            return ((-17));
        arr = *op;
        code = ref_stack_store(&((i_ctx_p->op_stack).stack), &arr, size, 1, 0, ((bool)1), (&(i_ctx_p->memory)),
                               "astore");
        if (code < 0)
            return code;
        ref_stack_pop(&((i_ctx_p->op_stack).stack), size);
        *ref_stack_index(&((i_ctx_p->op_stack).stack), 0) = arr;
    } else {
        code = refcpy_to_old(op, 0, op - size, size, (&(i_ctx_p->memory)), "astore");
        if (code < 0)
            return code;
        op[-(int)size] = *op;
        ((((i_ctx_p->op_stack).stack).p) -= (size));
    }
    return 0;
}
