# 1 "./devices/vector/gdevpdtc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdtc.c" 2
# 18 "./devices/vector/gdevpdtc.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 55 "./base/memory_.h" 2
# 19 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
# 20 "./devices/vector/gdevpdtc.c" 2
...
# 22 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gxfont.h" 1
# 24 "./base/gxfont.h"
# 1 "./base/gsfont.h" 1
# 24 "./base/gsfont.h"
...
# 23 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gxfont0.h" 1
# 25 "./base/gxfont0.h"
typedef enum {
    fmap_8_8 = 2,
...
# 24 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gxfont0c.h" 1
# 23 "./base/gxfont0c.h"
# 1 "./base/gxfcid.h" 1
# 24 "./base/gxfcid.h"
...
int gs_cmap_from_type42_cmap(gs_cmap_t **ppcmap, gs_font_type42 *pfont42,
                             int wmode, gs_memory_t *mem);
# 25 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gzpath.h" 1
# 23 "./base/gzpath.h"
# 1 "./base/gxpath.h" 1
# 23 "./base/gxpath.h"
...
# 26 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gxchar.h" 1
# 23 "./base/gxchar.h"
# 1 "./base/gschar.h" 1
# 39 "./base/gschar.h"
...
# 27 "./devices/vector/gdevpdtc.c" 2
...
int psf_convert_type1_to_type2(stream *s, const gs_glyph_data_t *pgd,
                               gs_font_type1 *pfont);
# 28 "./devices/vector/gdevpdtc.c" 2
...
# 29 "./devices/vector/gdevpdtc.c" 2
...
# 30 "./devices/vector/gdevpdtc.c" 2
...
# 31 "./devices/vector/gdevpdtc.c" 2
...
# 32 "./devices/vector/gdevpdtc.c" 2
...
# 33 "./devices/vector/gdevpdtc.c" 2
...
# 34 "./devices/vector/gdevpdtc.c" 2
...
# 36 "./devices/vector/gdevpdtc.c" 2
# 1 "./base/gxcpath.h" 1
# 44 "./base/gxcpath.h"
...
# 37 "./devices/vector/gdevpdtc.c" 2
...
process_composite_text(gs_text_enum_t *pte, void *vbuf, uint bsize)
{
    ...
    gs_font *prev_font = 0;
    gs_char chr, char_code = 0x0badf00d, space_char = ((gs_char)~0L);
    int buf_index = 0;
    bool return_width = (penum->text.operation & 0x20000);
    ...
    if (pte->text.operation &
        ((0x00001 | 0x00002 | 0x00004 | 0x00008 | (0x00010 | 0x00020)) - (0x00001 | 0x00002))
        )
        return ((-15));
    if (pte->text.operation & 0x10000) {
    ...
    for ( ; ; ) {
        ...
        if (buf_index) {
            ...
            out.index = 0;
            code = pdf_process_string_aux(&out, &str, ((void*)0), &fmat, &text_state);
            if (code < 0)
                return code;
            ...
            if (out.index < str.size) {
                ...
                gs_text_enum_copy_dynamic(pte, (gs_text_enum_t *)&prev, ((bool)1));
            }
            pte->xy_index = out.xy_index;
            if (return_width) {
# 170 "./devices/vector/gdevpdtc.c"
                if (pte->text.operation & 0x00200) {
        ...
        buf[0] = (byte)chr;
        buf_index = 1;
        space_char = ((pte->text.operation & 0x00080) &&
                ...
                const gs_cmap_t *pcmap, int font_index_only)
{
    const char *const *pcmn =
        standard_cmap_names +
        (pdev->CompatibilityLevel < 1.3 ? 39 :
         pdev->CompatibilityLevel < 1.4 ? 18 :
         pdev->CompatibilityLevel < 1.5 ? 12 : 0);
    bool is_identity = ((bool)0);
    ...
    if (pcmres || is_identity) {
        ...
        byte *chars = (*(pdev->v_memory)->procs.alloc_string)(pdev->v_memory, size, "pdf_font_resource_t(CMapName)");
        ...
        pdfont->u.type0.cmap_is_standard = ((bool)1);
    }
                          ...
                          gs_rect *text_bbox)
{
    ...
    m.tx = ((pte->origin.x)*(1.0/(1<<8)));
    m.ty = ((pte->origin.y)*(1.0/(1<<8)));
...
scan_cmap_text(pdf_text_enum_t *pte, void *vbuf)
{
    ...
    pdf_font_resource_t *pdsubf0 = ((void*)0);
    gs_font *subfont0 = ((void*)0), *saved_subfont = ((void*)0);
    ...
    for (;;) {
        ...
        do {
            ...
            if (subfont->FontType == ft_encrypted || subfont->FontType == ft_encrypted2) {
                font_change = (subfont != subfont0 && subfont0 != ((void*)0));
                if (font_change) {
                ...
                font_change = (pdsubf != pdsubf0 && pdsubf0 != ((void*)0));
            if (!font_change) {
                pdsubf0 = pdsubf;
            ...
            if (subfont->FontType != ft_encrypted && subfont->FontType != ft_encrypted2) {
                ...
                if (pdsubf->FontType == ft_user_defined || pdsubf->FontType == ft_encrypted ||
                    ...
                    if (pdf_is_CID_font(subfont)) {
                        if (subfont->procs.decode_glyph((gs_font *)subfont, glyph, -1) != ((gs_char)~0L)) {




                            code = pdf_add_ToUnicode(pdev, subfont, pdfont,
                                chr + ((gs_glyph)0x80000000L), chr, ((void*)0));
                        } else {
                        ...
                        code = pdf_add_ToUnicode(pdev, subfont, pdfont, glyph, cid, ((void*)0));
                    if (code < 0)
                        return code;
                    ...
                    if (code == (-15)) {
                        if (!(pdsubf->used[cid >> 3] & (0x80 >> (cid & 7)))) {
                            char buf[47 + 1];
                            int l = (((sizeof(buf) - 1) < (subfont->font_name.size)) ? (sizeof(buf) - 1) : (subfont->font_name.size));

                            memcpy(buf, subfont->font_name.chars, l);
                            buf[l] = 0;
                            (emprintf_program_ident(pdev->memory, gs_program_name(), gs_revision_number()), errprintf(pdev->memory, "Missing glyph CID=%d, glyph=%04x in the font %s . The output PDF may fail with some viewers.\n", (int)cid, (unsigned int)(glyph - ((gs_glyph)0x80000000L)), buf));
                        ...
                        return code;
                    if ((code == 0 || pdsubf->Widths[cid] == 0) && !notdef_subst) {
                        pdf_glyph_widths_t widths;
        ...
        if (break_index > index) {
            pdf_font_resource_t *pdfont;
            gs_matrix m3;
            int xy_index_step = (!(pte->text.operation & 0x00100) ? 0 :
            ...
            if (pte->text.operation & 0x00100) {
                if (pte->text.x_widths != ((void*)0))
                    pte->text.x_widths += xy_index * xy_index_step;
                if (pte->text.y_widths != ((void*)0))
            ...
            if (subfont && (subfont->FontType == ft_encrypted ||
                ...
                code = process_text_modify_width((pdf_text_enum_t *)pte,
                    (gs_font *)subfont, &text_state, &str, &wxy, type1_glyphs, ((bool)0), scan.index - index);
                if (code < 0)
                    return(code);
                if(font_change) {
                ...
                code = process_text_modify_width((pdf_text_enum_t *)pte, (gs_font *)font,
                    &text_state, &str, &wxy, ((void*)0), ((bool)1), scan.index - index);
            }
            if (pte->text.operation & 0x00100) {
                if (pte->text.x_widths != ((void*)0))
                    pte->text.x_widths -= xy_index * xy_index_step;
            ...
            if (pdev->Eps2Write) {
                ...
                estimate_fontbbox(pte, (gs_font_base *)font, ((void*)0), &text_bbox);
                text_bbox.p.x = ((pte->origin.x)*(1.0/(1<<8)));
...
process_cid_text(gs_text_enum_t *pte, void *vbuf, uint bsize)
{
        ...
        return ((-28));
    {
        ...
        for (i = 0; i < size; ++i) {
            ulong gnum = glyphs[i] - ((gs_glyph)0x80000000L);
    ...
    code = pdf_obtain_font_resource(penum, ((void*)0), &pdsubf);
    if (code < 0)
        return code;
    ...
    pte->text.operation = (operation & ~(0x00001 | 0x00002 | 0x00004 | 0x00008 | (0x00010 | 0x00020))) | 0x00002;
