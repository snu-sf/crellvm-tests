...
# 114 "./base/gsparams.c"
    while ((code = (*(list)->procs->next_key)(list, &key_enum, &key)) == 0) {
        ...
        if ((code = ((&value)->type = ((gs_param_type)-1), param_read_requested_typed(list, string_key, &value))) != 0) {
            code = code > 0 ? ((-1)) : code;
            break;
        }
        ...
        switch (value.type) {
            ...
            case gs_param_type_dict_int_keys:
...

                {
                    ...
                    temp_code = (*(list)->procs->end_xmit_collection)(list, (const char *)key.data, &value.value.d);
    ...
    do {
        ...
        switch (type) {
            ...
            case gs_param_type_float_array:
                ...
                typed.value.s.persistent = ((bool)0);
            ...
            case gs_param_type_name_array:
                ...
                typed.value.sa.persistent = ((bool)0);
                buf += typed.value.s.size * value_base_sizeof;
                {
                    int str_count;
                    gs_param_string *sa;
            ...
            case gs_param_type_dict_int_keys:
                ...
                code = (*(list)->procs->begin_xmit_collection)(list, key, &typed.value.d, (type == gs_param_type_dict_int_keys ? gs_param_collection_dict_int_keys : gs_param_collection_dict_any));
                ...
                temp_code = (*(list)->procs->end_xmit_collection)(list, key, &typed.value.d);
                if (code >= 0) {
                    buf += code;
                    code = temp_code;
                }
        ...
        if (typed.type != gs_param_type_dict && typed.type != gs_param_type_dict_int_keys)
            code = (*(list)->procs->xmit_typed)(list, key, &typed);
    }
    while (code >= 0);
...
{
    *src += -(int)((uint)( ((const char *)(*src) - (const char *)0) & ((alignment) - 1) )) & (alignment - 1);
}
