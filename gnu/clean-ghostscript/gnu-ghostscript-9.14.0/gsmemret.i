...
# 21 "./base/gsmemret.c" 2



static byte *gs_retrying_alloc_bytes_immovable(gs_memory_t *mem, uint nbytes, client_name_t cname);
static void *gs_retrying_resize_object(gs_memory_t *mem, void *obj, uint new_num_elements, client_name_t cname);
static void gs_forward_free_object(gs_memory_t *mem, void *data, client_name_t cname);
static gs_memory_t *gs_retrying_stable(gs_memory_t *mem);
static void gs_forward_status(gs_memory_t *mem, gs_memory_status_t *status);
static void gs_forward_free_all(gs_memory_t *mem, uint free_mask, client_name_t cname);
static void gs_forward_consolidate_free(gs_memory_t *mem);


static byte *gs_retrying_alloc_bytes(gs_memory_t *mem, uint nbytes, client_name_t cname);
static void *gs_retrying_alloc_struct(gs_memory_t *mem, gs_memory_type_ptr_t pstype, client_name_t cname);
static void *gs_retrying_alloc_struct_immovable(gs_memory_t *mem, gs_memory_type_ptr_t pstype, client_name_t cname);
static byte *gs_retrying_alloc_byte_array(gs_memory_t *mem, uint num_elements, uint elt_size, client_name_t cname);
static byte *gs_retrying_alloc_byte_array_immovable(gs_memory_t *mem, uint num_elements, uint elt_size, client_name_t cname);
static void *gs_retrying_alloc_struct_array(gs_memory_t *mem, uint num_elements, gs_memory_type_ptr_t pstype, client_name_t cname);
static void *gs_retrying_alloc_struct_array_immovable(gs_memory_t *mem, uint num_elements, gs_memory_type_ptr_t pstype, client_name_t cname);
static uint gs_forward_object_size(gs_memory_t *mem, const void *obj);
static gs_memory_type_ptr_t gs_forward_object_type(const gs_memory_t *mem, const void *obj);
static byte *gs_retrying_alloc_string(gs_memory_t *mem, uint nbytes, client_name_t cname);
static byte *gs_retrying_alloc_string_immovable(gs_memory_t *mem, uint nbytes, client_name_t cname);
static byte *gs_retrying_resize_string(gs_memory_t *mem, byte *data, uint old_num, uint new_num, client_name_t cname);
static void gs_forward_free_string(gs_memory_t *mem, byte *data, uint nbytes, client_name_t cname);
static int gs_retrying_register_root(gs_memory_t *mem, gs_gc_root_t *root, gs_ptr_type_t ptype, void **pp, client_name_t cname);
static void gs_forward_unregister_root(gs_memory_t *mem, gs_gc_root_t *root, client_name_t cname);
static void gs_forward_enable_free(gs_memory_t *mem, bool enable);
...
)
{
    ...
    rmem->thread_safe_memory = (gs_memory_t *)target->thread_safe_memory;
    gs_memory_retrying_set_recover(rmem, no_recover_proc, ((void*)0));
    return 0;
}
...
{
    (((gs_memory_t *)rmem)->procs.free_all((gs_memory_t *)rmem, 2, "gs_memory_retrying_release"));
...
gs_forward_free_all(gs_memory_t * mem, uint free_mask, client_name_t cname)
{
    ...
    rmem->target = 0;
    if (free_mask & 4)
        ((target)->procs.free_object(target, rmem, cname));
}
static void
gs_forward_consolidate_free(gs_memory_t * mem)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.consolidate_free(target);
}
static byte *
gs_retrying_alloc_bytes(gs_memory_t * mem, uint size, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_bytes(target, size, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_bytes_immovable(gs_memory_t * mem, uint size,
                                client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_bytes_immovable(target, size, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_struct(gs_memory_t * mem, gs_memory_type_ptr_t pstype,
                       client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; void * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_struct(target, pstype, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; void * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_struct_immovable(target, pstype, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_byte_array(gs_memory_t * mem, uint num_elements, uint elt_size,
                           client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_byte_array(target, num_elements, elt_size, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_byte_array_immovable(target, num_elements, elt_size, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_struct_array(gs_memory_t * mem, uint num_elements,
                           gs_memory_type_ptr_t pstype, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; void * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_struct_array(target, num_elements, pstype, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_struct_array_immovable(gs_memory_t * mem, uint num_elements,
                           gs_memory_type_ptr_t pstype, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; void * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_struct_array_immovable(target, num_elements, pstype, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_resize_object(gs_memory_t * mem, void *obj, uint new_num_elements,
                        client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; void * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.resize_object(target, obj, new_num_elements, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; return target->procs.object_size(target, ptr);
}
static gs_memory_type_ptr_t
gs_forward_object_type(const gs_memory_t * mem, const void *ptr)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; return target->procs.object_type(target, ptr);
}
static void
gs_forward_free_object(gs_memory_t * mem, void *ptr, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.free_object(target, ptr, cname);
}
static byte *
gs_retrying_alloc_string(gs_memory_t * mem, uint nbytes, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_string(target, nbytes, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_alloc_string_immovable(gs_memory_t * mem, uint nbytes,
                                 client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.alloc_string_immovable(target, nbytes, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_retrying_resize_string(gs_memory_t * mem, byte * data, uint old_num,
                        ...
                        client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; byte * temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.resize_string(target, data, old_num, new_num, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_forward_free_string(gs_memory_t * mem, byte * data, uint nbytes,
                      client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.free_string(target, data, nbytes, cname);
}
static int
gs_retrying_register_root(gs_memory_t * mem, gs_gc_root_t * rp,
                        gs_ptr_type_t ptype, void **up, client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; int temp; gs_memory_recover_status_t retry = RECOVER_STATUS_RETRY_OK; for (;;) { temp = target->procs.register_root(target, rp, ptype, up, cname); if (temp != 0 || retry != RECOVER_STATUS_RETRY_OK) break; retry = rmem->recover_proc(rmem, rmem->recover_proc_data); } return temp;
...
gs_forward_unregister_root(gs_memory_t * mem, gs_gc_root_t * rp,
                          client_name_t cname)
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.unregister_root(target, rp, cname);
}
static gs_memory_t *
gs_retrying_stable(gs_memory_t * mem)
{
    if (!mem->stable_memory) {
        gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem;
        gs_memory_t *stable = ((rmem->target)->procs.stable(rmem->target));

        if (stable == rmem->target)
            mem->stable_memory = mem;
        else {
            gs_memory_retrying_t *retrying_stable = (gs_memory_retrying_t *)
                (*(stable)->procs.alloc_bytes)(stable, sizeof(*rmem), "gs_retrying_stable");

            if (retrying_stable) {
                int code = gs_memory_retrying_init(retrying_stable, stable);

                if (code < 0)
                    ((stable)->procs.free_object(stable, retrying_stable, "gs_retrying_stable"));
                else
...
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.status(target, pstat);
...
{
    gs_memory_retrying_t * const rmem = (gs_memory_retrying_t *)mem; gs_memory_t *const target = rmem->target; target->procs.enable_free(target, enable);
}
