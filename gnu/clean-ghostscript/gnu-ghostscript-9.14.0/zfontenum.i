...
# 55 "./base/memory_.h" 2
# 25 "./psi/zfontenum.c" 2
# 1 "./base/string_.h" 1
# 26 "./psi/zfontenum.c" 2
# 1 "/usr/include/stdlib.h" 1 3 4
# 32 "/usr/include/stdlib.h" 3 4
# 1 "/home/juneyoung.lee/llvm-prototypes/llvm-install-3.7.0/bin/../lib/clang/3.7.0/include/stddef.h" 1 3 4
...
z_fontenum(i_ctx_t *i_ctx_p)
{
    ...
    enum_state = gp_enumerate_fonts_init(((gs_memory_t *)((i_ctx_p->memory).current)));
    if (enum_state == ((void*)0)) {
      ...
      ((op)->value.boolval = (((bool)0)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
      return code;
    }

    r = results = (void *)(*(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, 1, sizeof(fontenum_t), "fontenum list");
    elements = 0;
    while((code = gp_enumerate_fonts_next(enum_state, &fontname, &path )) > 0) {
        if (fontname == ((void*)0) || path == ((void*)0)) {
            gp_enumerate_fonts_free(enum_state);
            return ((-12));
        }

        length = strlen(fontname) + 1;
        r->fontname = (void *)(*(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, length, 1, "native font name");
        memcpy(r->fontname, fontname, length);

        length = strlen(path) + 1;
            r->path = (void *)(*(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, length, 1, "native font path");
            memcpy(r->path, path, length);

            r->next = (void *)(*(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, 1, sizeof(fontenum_t), "fontenum list");
            r = r->next;
            elements += 1;
        }
        ...
        code = gs_alloc_ref_array(((i_ctx_p->memory).current), &array, (0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space), elements, "native fontmap");

        r = results;
        for (e = 0; e < elements; e++) {
            ref mapping;

            code = gs_alloc_ref_array(((i_ctx_p->memory).current), &mapping, (0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space), 2, "native font mapping");

            length = strlen(r->fontname);
            string = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), length, "native font name");
            if (string == ((void*)0))
                return ((-25));
            memcpy(string, r->fontname, length);
            ((&(mapping.value.refs[0]))->value.bytes = (string), ((&(mapping.value.refs[0]))->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((&(mapping.value.refs[0]))->tas.rsize = (length)));

            length = strlen(r->path);
            string = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), length, "native font path");
            if (string == ((void*)0))
                return ((-25));
            memcpy(string, r->path, length);
            ((&(mapping.value.refs[1]))->value.bytes = (string), ((&(mapping.value.refs[1]))->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((&(mapping.value.refs[1]))->tas.rsize = (length)));

            (*(&(array.value.refs[e])) = *(&mapping));
            results = r;
            r = r->next;

            ((((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, results->fontname, "native font name"));

            ((((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, results->path, "native font path"));

            ((((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory)->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory->non_gc_memory, results, "fontenum list"));
    ...
    (*(op-1) = *(&array));
    ((op)->value.boolval = (((bool)1)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
