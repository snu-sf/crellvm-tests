# 1 "./base/gxdhtserial.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gxdhtserial.c" 2
# 19 "./base/gxdhtserial.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
    ...
    gs_memory_t * mem )
{
    ...
    do { if ( ((pmap) = (gx_transfer_map *)(*(mem)->procs.alloc_struct)(mem, &st_transfer_map, "gx_ht_read_tf")) == 0 ) { return ((-25)); } else { do { (pmap)->rc.ref_count = 1; (pmap)->rc.memory = mem; (pmap)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
    ...
    if (tf_type == gx_ht_tf_identity) {
        ...
        *ppmap = pmap;
        ...
        do { if (pmap) { do { do { } while (0); (pmap)->rc.ref_count += (-1); } while (0); if (!(pmap)->rc.ref_count) { do { do { } while (0); (pmap)->rc.free((pmap)->rc.memory, (void *)(pmap), "gx_ht_read_tf"); } while (0); (pmap) = 0; } else do { } while (0); } } while (0);
        return ((-15));
    }
}
# 159 "./base/gxdhtserial.c"
static int
    ...
    uint * psize )
{
    ...
    uint tmp_size = 0;
    int req_size;
# 208 "./base/gxdhtserial.c"
    levels_size = porder->num_levels * sizeof(porder->levels[0]);
    bits_size = porder->num_bits * porder->procs->bit_data_elt_size;
    req_size = ( (uint)(porder->width) < (1U << 7) ? 1 : (uint)(porder->width) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(porder->width) )
               + ( (uint)(porder->height) < (1U << 7) ? 1 : (uint)(porder->height) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(porder->height) )
               + ( (uint)(porder->shift) < (1U << 7) ? 1 : (uint)(porder->shift) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(porder->shift) )
               + ( (uint)(porder->num_levels) < (1U << 7) ? 1 : (uint)(porder->num_levels) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(porder->num_levels) )
               + ( (uint)(porder->num_bits) < (1U << 7) ? 1 : (uint)(porder->num_bits) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(porder->num_bits) )
               + 1
               + levels_size
               + bits_size;
    ...
    do { if ((uint)(porder->width) < (1U << 7)) *(data)++ = (byte)(porder->width); else if ((uint)(porder->width) < (1U << (2 * 7))) { *(data)++ = (1U << 7) | ((porder->width) & ((1U << 7) - 1)); *(data)++ = (porder->width) >> 7; } else (data) = enc_u_put_uint((porder->width), (data)); } while (0);
    do { if ((uint)(porder->height) < (1U << 7)) *(data)++ = (byte)(porder->height); else if ((uint)(porder->height) < (1U << (2 * 7))) { *(data)++ = (1U << 7) | ((porder->height) & ((1U << 7) - 1)); *(data)++ = (porder->height) >> 7; } else (data) = enc_u_put_uint((porder->height), (data)); } while (0);
    do { if ((uint)(porder->shift) < (1U << 7)) *(data)++ = (byte)(porder->shift); else if ((uint)(porder->shift) < (1U << (2 * 7))) { *(data)++ = (1U << 7) | ((porder->shift) & ((1U << 7) - 1)); *(data)++ = (porder->shift) >> 7; } else (data) = enc_u_put_uint((porder->shift), (data)); } while (0);
    do { if ((uint)(porder->num_levels) < (1U << 7)) *(data)++ = (byte)(porder->num_levels); else if ((uint)(porder->num_levels) < (1U << (2 * 7))) { *(data)++ = (1U << 7) | ((porder->num_levels) & ((1U << 7) - 1)); *(data)++ = (porder->num_levels) >> 7; } else (data) = enc_u_put_uint((porder->num_levels), (data)); } while (0);
    do { if ((uint)(porder->num_bits) < (1U << 7)) *(data)++ = (byte)(porder->num_bits); else if ((uint)(porder->num_bits) < (1U << (2 * 7))) { *(data)++ = (1U << 7) | ((porder->num_bits) & ((1U << 7) - 1)); *(data)++ = (porder->num_bits) >> 7; } else (data) = enc_u_put_uint((porder->num_bits), (data)); } while (0);
    ...
    gs_memory_t * mem )
{
    ...
    if (size == 0)
        return ((-15));
# 290 "./base/gxdhtserial.c"
    if (size < 7)
        return ((-15));
    do { if (((new_order.width) = *(data)) >= (1U << 7)) { uint tmp_w; (data) = enc_u_get_uint(&tmp_w, (data)); (new_order.width) = tmp_w; } else ++(data); } while (0);
    do { if (((new_order.height) = *(data)) >= (1U << 7)) { uint tmp_w; (data) = enc_u_get_uint(&tmp_w, (data)); (new_order.height) = tmp_w; } else ++(data); } while (0);
    do { if (((new_order.shift) = *(data)) >= (1U << 7)) { uint tmp_w; (data) = enc_u_get_uint(&tmp_w, (data)); (new_order.shift) = tmp_w; } else ++(data); } while (0);
    do { if (((new_order.num_levels) = *(data)) >= (1U << 7)) { uint tmp_w; (data) = enc_u_get_uint(&tmp_w, (data)); (new_order.num_levels) = tmp_w; } else ++(data); } while (0);
    do { if (((new_order.num_bits) = *(data)) >= (1U << 7)) { uint tmp_w; (data) = enc_u_get_uint(&tmp_w, (data)); (new_order.num_bits) = tmp_w; } else ++(data); } while (0);
    if (data >= data_lim)
        return ((-15));
...
# 393 "./base/gxdhtserial.c"
            if ( phtr->num_levels * sizeof(phtr->levels[0]) >= levels_size &&
                ...
                ((mem)->procs.free_object(mem, new_order.bit_data, "gx_ht_read_component"));
                new_order.bit_data = (void *)phtr->bit_data;
                ((mem)->procs.free_object(mem, new_order.levels, "gx_ht_read_component"));
...
# 449 "./base/gxdhtserial.c"
    if (pdht == 0 || pdht->components == 0)
        return ((-28));
    num_dev_comps = pdht->num_dev_comp;
# 477 "./base/gxdhtserial.c"
    for ( i = 0, code = (-15);
          i < num_dev_comps && code == (-15);
          i++) {
        uint tmp_size = 0;
...
gx_ht_read_and_install(
    ...
    gs_memory_t * mem )
{
    gx_ht_order_component components[(8 * 8)];
    gx_ht_order_component components_save[(8 * 8)];
    const byte * data0 = data;
    ...
    if (code >= 0) {
        ...
        for (i = 0; i < num_dev_comps; i++)
            gx_ht_order_release(&components_save[i].corder, mem, ((bool)0));
    }
    ...
    if (code < 0) {
        for (i = 0; i < num_dev_comps; i++)
            gx_ht_order_release(&components[i].corder, mem, ((bool)0));
    }
