...
static gs_ptr_type_t device_memory_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void device_memory_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_device_memory = { sizeof(gx_device_memory), "gx_device_memory", 0, 0, device_memory_enum_ptrs, device_memory_reloc_ptrs, gx_device_finalize, 0 };


static
gs_ptr_type_t device_memory_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_device_memory *mptr = vptr; switch ( index ) { default:
{
    return (*(st_device_forward).enum_ptrs)(mem, vptr, sizeof(gx_device_forward), index - 3, pep, &(st_device_forward), gcst);
}
case 0: return (pep->ptr = (const void *)((mptr->foreign_bits ? ((void*)0) : (void *)mptr->base)), (&ptr_struct_procs));
case 1: return (pep->ptr = (const void *)((mptr->foreign_line_pointers ? ((void*)0) : (void *)mptr->line_ptrs)), (&ptr_struct_procs));
case 2: return (pep->ptr = (&((const gx_device_memory *)vptr)->palette)->data, pep->size = (&((const gx_device_memory *)vptr)->palette)->size, (&ptr_string_procs));
} }
static
void device_memory_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_device_memory *mptr = vptr;
{
    if (!mptr->foreign_bits) {
        ...
        (((gx_device_memory *)vptr)->base = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_device_memory *)vptr)->base), gcst)));
        ...
        (((gx_device_memory *)vptr)->line_ptrs = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_device_memory *)vptr)->line_ptrs), gcst)));
    }
    (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(((gx_device_memory *)vptr)->palette), gcst));
...
gs_device_is_memory(const gx_device * dev)
{
    ...
    if (mdproto != 0 && ((dev)->procs.draw_thin_line) == ((mdproto)->procs.draw_thin_line))
        return ((bool)1);
    mdproto = gdev_mem_word_device_for_bits(bits_per_pixel);
    return (mdproto != 0 && ((dev)->procs.draw_thin_line) == ((mdproto)->procs.draw_thin_line));
}
                   ...
                   gs_memory_t * mem, int page_device, gx_device * target)
{
    ...
    switch (page_device) {
        case -1:
            ((dev)->procs.get_page_device = (gx_default_get_page_device));
            break;
        case 1:
            ((dev)->procs.get_page_device = (gx_page_device_get_page_device));
            break;
    }
    ...
    if (target == 0) {
        ...
        ((dev)->procs.put_image = (target->procs.put_image));
    ...
    if (dev->color_info.depth == 1) {
       ...
       gdev_mem_mono_set_inverted(dev, (target == 0 ||
                                   (*((dev)->procs.encode_color))((gx_device *)dev, cv) != 0));
    }
                                   ...
                                   gx_device * target)
{
    ...
    switch (page_device) {
        case -1:
            ((pdev)->procs.get_page_device = (gx_default_get_page_device));
            break;
        case 1:
            ((pdev)->procs.get_page_device = (gx_page_device_get_page_device));
            break;
    }
    ...
    if (pdev->color_info.depth == 1) {
       ...
       cv[0] = cv[1] = cv[2] = 0;
        gdev_mem_mono_set_inverted(pdev, (target == 0 ||
                                   (*((pdev)->procs.encode_color))((gx_device *)pdev, cv) != 0));
    }
                                        ...
                                        gx_device * target)
{
    ...
    ((pdev)->procs.get_profile = (gx_forward_get_profile));

    gdev_mem_mono_set_inverted(pdev, ((bool)1));
    check_device_separable((gx_device *)pdev);
    gx_device_fill_in_procs((gx_device *)pdev);
    *ppdev = pdev;
    return 0;
                        ...
                        gx_device * target)
{
    ...
    gdev_mem_mono_set_inverted(dev, ((bool)1));
    check_device_separable((gx_device *)dev);
    gx_device_fill_in_procs((gx_device *)dev);
...
gdev_mem_bits_size(const gx_device_memory * dev, int width, int height, ulong *psize)
{
    ...
    for (size = 0, pi = 0; pi < num_planes; ++pi)
        size += ((uint)((((width * planes[pi].depth) + (dev->pad<<3) + ((8<<(dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod)) - 1)) >> ((dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod) + 3)) << (dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod)));
    if (height != 0)
        if (size > (((unsigned long)~0L + (unsigned long)0) - 8) / (ulong)height)
            return ((-25));
    size = ( (8) & ((8) - 1) ? ((size * height) + ((8) - 1)) / (8) * (8) : ((size * height) + ((8) - 1)) & -(8) );
    if (dev->log2_align_mod > 3)
...
gdev_mem_data_size(const gx_device_memory * dev, int width, int height, ulong *psize)
{
    ...
    if (gdev_mem_bits_size(dev, width, height, &bits_size) < 0 ||
        bits_size > ((unsigned long)~0L + (unsigned long)0) - line_ptrs_size)
        return ((-25));
                ...
                bool page_uses_transparency)
{
    ...
    if (page_uses_transparency) {
        ...
        max_height = size / (((uint)((((width * dev->color_info.depth + (3 * ((width) * 8 * (1 + 4)))) + (dev->pad<<3) + ((8<<(dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod)) - 1)) >> ((dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod) + 3)) << (dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod))) + sizeof(byte *) * (dev->is_planar ? dev->color_info.num_components : 1));


        height = (int)(((max_height) < ((~(-1 << ((1 << 2) * 8 - 1))))) ? (max_height) : ((~(-1 << ((1 << 2) * 8 - 1)))));
    } else {

        max_height = size /
            (((uint)((((width * dev->color_info.depth) + (dev->pad<<3) + ((8<<(dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod)) - 1)) >> ((dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod) + 3)) << (dev->log2_align_mod < 3 ? 3 : dev->log2_align_mod))) +
...
{
    bool line_pointers_adjacent = ((bool)1);
    ulong size;
    ...
    if (mdev->bitmap_memory != 0) {
        ...
        if (gdev_mem_data_size(mdev, (mdev)->width, (mdev)->height, &size) < 0)
            return ((-25));

        if ((uint) size != size)
            return ((-13));
        mdev->base = (*(mdev->bitmap_memory)->procs.alloc_bytes)(mdev->bitmap_memory, (uint)size, "mem_open");

        if (mdev->base == 0)
            return ((-25));
        align = 1<<mdev->log2_align_mod;
        mdev->base += (-(int)mdev->base) & (align-1);
        mdev->foreign_bits = ((bool)0);
    } else if (mdev->line_pointer_memory != 0) {


        mdev->line_ptrs = (byte **)
            (*(mdev->line_pointer_memory)->procs.alloc_byte_array)(mdev->line_pointer_memory, mdev->height, sizeof(byte *) * (mdev->is_planar ? mdev->color_info.num_components : 1), "gdev_mem_open_scan_lines");


        if (mdev->line_ptrs == 0)
            return ((-25));
        mdev->foreign_line_pointers = ((bool)0);
        line_pointers_adjacent = ((bool)0);
    }
    ...
    return gdev_mem_set_line_ptrs(mdev, ((void*)0), 0, ((void*)0), setup_height);
}
...
mem_close(gx_device * dev)
{
    ...
    if (mdev->bitmap_memory != 0) {
        ((mdev->bitmap_memory)->procs.free_object(mdev->bitmap_memory, mdev->base, "mem_close"));
...
mem_swap_byte_rect(byte * base, uint raster, int x, int w, int h, bool store)
{
    ...
    if (store) {
        if (xbit + w > 64) {
            ...
            if (xbit != 0)
                mem_swap_byte_rect(base, raster, x, 1, h, ((bool)0));
            x += w - 1;
            xbit = x & 31;
            if (xbit == 31)
                return;
                       ...
                       gs_get_bits_params_t * params, gs_int_rect ** unread)
{
    ...
    src = ((mdev)->line_ptrs[y]);
    mem_swap_byte_rect(src, dev_raster, bit_x, bit_w, h, ((bool)0));
...
{
    gx_device_memory * const mdev = (gx_device_memory *)dev;
    byte br = ((gx_color_value)((((unsigned int)cv[0])*0xFF01 + 0x800000)>>24));
    ...
    if (mdev->color_info.num_components != 1) {
        ...
        byte bg = ((gx_color_value)((((unsigned int)cv[1])*0xFF01 + 0x800000)>>24));
        byte bb = ((gx_color_value)((((unsigned int)cv[2])*0xFF01 + 0x800000)>>24));
                         ...
                         gx_color_value prgb[3])
{
    ...
    prgb[0] = (((pptr[0]) << ((sizeof(gx_color_value) * 8) - 8)) + ((pptr[0]) >> (16 - (sizeof(gx_color_value) * 8))));
    prgb[1] = (((pptr[1]) << ((sizeof(gx_color_value) * 8) - 8)) + ((pptr[1]) >> (16 - (sizeof(gx_color_value) * 8))));
    prgb[2] = (((pptr[2]) << ((sizeof(gx_color_value) * 8) - 8)) + ((pptr[2]) >> (16 - (sizeof(gx_color_value) * 8))));
    return 0;
}
