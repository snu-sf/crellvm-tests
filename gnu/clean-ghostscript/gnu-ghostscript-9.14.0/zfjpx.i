...
# 1 "./base/sjpx_openjpeg.h" 1
# 25 "./base/sjpx_openjpeg.h"
...
z_jpx_decode(i_ctx_t * i_ctx_p)
{
    ...
    state.memory = ((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory;
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        ...
        if ( dict_find_string(op, "ColorSpace", &sop) > 0) {
            ...
            if ((((sop)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))) {

                csname = sop->value.refs;
            } else if (((((const byte *)&((sop)->tas.type_attrs))[1]) == (t_name))) {

                csname = sop;
            } else {
                errprintf(((gs_memory_t *)((i_ctx_p->memory).current)),"warning: JPX ColorSpace value is an unhandled type!\n");
            }
            if (csname != ((void*)0)) {
                ref sref;

                names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, csname, &sref);

                if (!(memcmp((&sref)->value.const_bytes, "Indexed", (((strlen("Indexed")) < (((&sref)->tas.rsize))) ? (strlen("Indexed")) : (((&sref)->tas.rsize))))))
                    state.colorspace = gs_jpx_cs_indexed;

                else if (!(memcmp((&sref)->value.const_bytes, "DeviceGray", (((strlen("DeviceGray")) < (((&sref)->tas.rsize))) ? (strlen("DeviceGray")) : (((&sref)->tas.rsize))))))
                    state.colorspace = gs_jpx_cs_gray;
                else if (!(memcmp((&sref)->value.const_bytes, "DeviceRGB", (((strlen("DeviceRGB")) < (((&sref)->tas.rsize))) ? (strlen("DeviceRGB")) : (((&sref)->tas.rsize))))))
                    state.colorspace = gs_jpx_cs_rgb;
                else if (!(memcmp((&sref)->value.const_bytes, "DeviceCMYK", (((strlen("DeviceCMYK")) < (((&sref)->tas.rsize))) ? (strlen("DeviceCMYK")) : (((&sref)->tas.rsize))))))
                    state.colorspace = gs_jpx_cs_cmyk;
                else if (!(memcmp((&sref)->value.const_bytes, "ICCBased", (((strlen("ICCBased")) < (((&sref)->tas.rsize))) ? (strlen("ICCBased")) : (((&sref)->tas.rsize)))))) {


                    ref *csdict = sop->value.refs + 1;
                    ref *nref;
                    ref altname;
                    if ((((sop)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))) && (((sop)->tas.rsize) > 1) &&
                      ((((const byte *)&((csdict)->tas.type_attrs))[1]) == (t_dictionary))) {
                        do { if ( !(((&*(&(&*csdict)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);

                        if (dict_find_string(csdict, "Alternate", &nref) > 0) {
                          names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, csname, &altname);
                          if (!(memcmp((&altname)->value.const_bytes, "DeviceGray", (((strlen("DeviceGray")) < (((&altname)->tas.rsize))) ? (strlen("DeviceGray")) : (((&altname)->tas.rsize))))))
                            state.colorspace = gs_jpx_cs_gray;
                          else if (!(memcmp((&altname)->value.const_bytes, "DeviceRGB", (((strlen("DeviceRGB")) < (((&altname)->tas.rsize))) ? (strlen("DeviceRGB")) : (((&altname)->tas.rsize))))))
                            state.colorspace = gs_jpx_cs_rgb;
                          else if (!(memcmp((&altname)->value.const_bytes, "DeviceCMYK", (((strlen("DeviceCMYK")) < (((&altname)->tas.rsize))) ? (strlen("DeviceCMYK")) : (((&altname)->tas.rsize))))))
                            state.colorspace = gs_jpx_cs_cmyk;
                        }
