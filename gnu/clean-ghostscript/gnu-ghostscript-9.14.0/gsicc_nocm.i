...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gsicc_nocm.c" 2
# 19 "./base/gsicc_nocm.c"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
# 1 "./base/stdpre.h" 1
# 253 "./base/stdpre.h"
                             ...
                             int num_bytes_in, int num_bytes_out)
{
    ...
    if (num_bytes_in == 2) {
        ...
        for (k = 0; k < num_in; k++) {
            frac_in[k] = ((frac)( ((data[k]) >> (((1 << 1) * 8) - 15)) - ((data[k]) >> (((1 << 1) * 8) - 3)) ));
        }
    } else {
        byte *data = (byte *) inputcolor;
        for (k = 0; k < num_in; k++) {
            frac_in[k] = ((frac)( ((frac)(data[k]) << (15 - (8))) + ((data[k]) >> ((8) * 2 - 15)) - ((data[k]) >> ((8) - 3)) ));
        }
    }
    ...
    if (num_bytes_out == 2) {
        ...
        for (k = 0; k < num_out; k++) {
            data[k] = ((ushort)( ((frac_out[k]) << (((1 << 1) * 8) - 15)) + ((frac_out[k]) >> (15 * 2 - ((1 << 1) * 8) - 3)) ));
        }
    } else {
        byte *data = (byte *) outputcolor;
        for (k = 0; k < num_out; k++) {
            data[k] = ((byte)((uint)((((frac_out[k]) >> (15 - 3)) + (frac_out[k])) >> (15 - (8)))));
        }
    }
    return;
}
...
gsicc_nocm_freelink(gsicc_link_t *icclink)
{
    ...
    if (nocm_link->pis != ((void*)0)) {
        if (nocm_link->pis->black_generation != ((void*)0)) {
            ((nocm_link->memory)->procs.free_object(nocm_link->memory, nocm_link->pis->black_generation, "gsicc_nocm_freelink"));

        }
        if (nocm_link->pis->undercolor_removal != ((void*)0)) {
            ((nocm_link->memory)->procs.free_object(nocm_link->memory, nocm_link->pis->undercolor_removal, "gsicc_nocm_freelink"));

        }
        ((nocm_link->memory)->procs.free_object(nocm_link->memory, nocm_link->pis, "gsicc_nocm_freelink"));
    }
    ((nocm_link->memory)->procs.free_object(nocm_link->memory, nocm_link, "gsicc_nocm_freelink"));
}
...
gsicc_nocm_copy_curve(gx_transfer_map *in_map, gs_memory_t *mem)
{
    ...
    if (in_map == ((void*)0)) {
        ...
        out_map = (gx_transfer_map*) (*(mem)->procs.alloc_bytes)(mem, sizeof(gx_transfer_map), "gsicc_nocm_copy_curve");
                    ...
                    gs_color_space_index src_index)
{
    ...
    result = gsicc_findcachelink(hash, pis->icc_link_cache, ((bool)0), ((bool)0));
    if (result != ((void*)0)) {
        return result;
    }


    if (gsicc_alloc_link_entry(pis->icc_link_cache, &result, hash, ((bool)0), ((bool)0)))
        return result;
    ...
    gp_monitor_enter(&(pis->icc_link_cache->lock)->native);
    ...
    nocm_link = (nocm_link_t *) (*(mem)->procs.alloc_bytes)(mem, sizeof(nocm_link_t), "gsicc_nocm_get_link");
    ...
    if (pis == ((void*)0) ||
       (pis->black_generation == ((void*)0) && pis->undercolor_removal == ((void*)0))) {
        nocm_link->pis = ((void*)0);
    } else {
        nocm_link->pis = (gs_imager_state*)
                          (*(mem)->procs.alloc_bytes)(mem, sizeof(gs_imager_state), "gsicc_nocm_get_link");
    ...
    nocm_link->num_out = (((dev->color_info.num_components) < ((64))) ? (dev->color_info.num_components) : ((64)));
    ...
    result->includes_softproof = ((bool)0);
    result->includes_devlink = ((bool)0);
    if (hash.src_hash == hash.des_hash) {
        result->is_identity = ((bool)1);
    } else {
    ...
    while (result->num_waiting > 0) {
        gp_semaphore_signal(&(result->wait)->native);
        result->num_waiting--;
    }
    gp_monitor_leave(&(pis->icc_link_cache->lock)->native);
