...
glyph_show_setup(i_ctx_t *i_ctx_p, gs_glyph *pglyph)
{
    ...
    switch (gs_currentfont((i_ctx_p->pgs))->FontType) {
        case ft_CID_encrypted:
        case ft_CID_user_defined:
        case ft_CID_TrueType:
        case ft_CID_bitmap:
            do { if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op)); if ( (ulong)(*op).value.intval > (((unsigned long)~0L + (unsigned long)0) - ((gs_glyph)0x80000000L)) ) return ((-15)); } while (0);
            *pglyph = (gs_glyph) op->value.intval + ((gs_glyph)0x80000000L);
            break;
        default:
            if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_name)) ) return (check_type_failed(&*op));
            *pglyph = names_index(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op);
    }
...
zglyphshow(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = glyph_show_setup(i_ctx_p, &glyph)) != 0 ||
        (code = gs_glyphshow_begin((i_ctx_p->pgs), glyph, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0)
        return code;
    *(op_proc_t *)&penum->enum_client_data = zglyphshow;
    if ((code = op_show_finish_setup(i_ctx_p, penum, 1, ((void*)0))) < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), penum, "zglyphshow"));
        return code;
    }
...
zglyphwidth(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = glyph_show_setup(i_ctx_p, &glyph)) != 0 ||
        (code = gs_glyphwidth_begin((i_ctx_p->pgs), glyph, ((gs_memory_t *)((i_ctx_p->memory).current)), &penum)) < 0)
        return code;
    if ((code = op_show_finish_setup(i_ctx_p, penum, 1, finish_stringwidth)) < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), penum, "zglyphwidth"));
        return code;
    }
    return op_show_continue_pop(i_ctx_p, 1);
}
...
moveshow(i_ctx_t *i_ctx_p, bool have_x, bool have_y)
{
    ...
    values = (float *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current)), size, sizeof(float), "moveshow");
    if (values == 0)
        return ((-25));
    if (CPSI_mode)
        memset(values, 0, size * sizeof(values[0]));
    if ((code = gs_xyshow_begin((i_ctx_p->pgs), op[-1].value.bytes, ((op - 1)->tas.rsize),
                                (have_x ? values : (float *)0),
                                (have_y ? values : (float *)0),
                                size, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), values, "moveshow"));
        if (penum)
            penum->text.x_widths = penum->text.y_widths = ((void*)0);
        return code;
    }
    if (CPSI_mode) {
        ...
        code = gs_text_count_chars((i_ctx_p->pgs), gs_get_text_params(penum), ((gs_memory_t *)((i_ctx_p->memory).current)));
        if (code < 0)
            return code;
        widths_needed = code;
        if (have_x && have_y)
    ...
    for (i = 0; i < widths_needed; ++i) {
        ...
        switch (code = num_array_get(((gs_memory_t *)((i_ctx_p->memory).current)), op, format, i, &value)) {
        case t_integer:
    ...
    if ((code = op_show_finish_setup(i_ctx_p, penum, 2, ((void*)0))) < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), values, "moveshow"));
        penum->text.x_widths = penum->text.y_widths = ((void*)0);
        return code;
    }
...
{
    return moveshow(i_ctx_p, ((bool)1), ((bool)0));
}
static int
zyshow(i_ctx_t *i_ctx_p)
{
    return moveshow(i_ctx_p, ((bool)0), ((bool)1));
}
static int
zxyshow(i_ctx_t *i_ctx_p)
{
    return moveshow(i_ctx_p, ((bool)1), ((bool)1));
}
