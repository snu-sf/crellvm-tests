...
zcurrentalpha(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.realval = (gs_currentalpha((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_real) << 8) + (0)) );
    return 0;
}
...
zsetalpha(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = gs_setalpha((i_ctx_p->pgs), alpha)) < 0)
        return code;
...
{
    return image1_setup(i_ctx_p, ((bool)1));
}
...
zcompositerect(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    do { if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op)); if ( (ulong)(*op).value.intval > (composite_Highlight) ) return ((-15)); } while (0);
        ...
        return code;
    {
        ...
        code = gs_rectfill((i_ctx_p->pgs), &rect, 1);
    }
    end_composite(i_ctx_p, &cstate);
    if (code >= 0)
        ((((i_ctx_p->op_stack).stack).p) -= (5));
    return code;
}
...
composite_image(i_ctx_t *i_ctx_p, const gs_composite_alpha_params_t * params)
{
    ...
    if (code < 0 ||
        ...
        return code;
    if (((((const byte *)&((op - 3)->tas.type_attrs))[1]) == (t_null)))
        image.DataSource = (i_ctx_p->pgs);
    else {
        if ( !((((&op[-3])->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&op[-3])->value.pstruct) == &st_igstate_obj) ) return (check_type_failed(&op[-3]));
        do { if ( !(((&op[-3])->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        image.DataSource = ((gs_state *)((&((igstate_obj *)((op - 3)->value.pstruct))->gstate)->value.pstruct));
    }
    image.XOrigin = src_rect[0];
    image.YOrigin = src_rect[1];
    image.Width = src_rect[2];
    image.Height = src_rect[3];
    image.PixelCopy = ((bool)1);

    gs_currentmatrix((i_ctx_p->pgs), &save_ctm);
    gs_translate((i_ctx_p->pgs), dest_pt[0], dest_pt[1]);
    gs_make_identity(&image.ImageMatrix);
    if (image.DataSource == (i_ctx_p->pgs)) {
    ...
    if (code >= 0) {
        ...
        end_composite(i_ctx_p, &cstate);
        if (code >= 0)
            ((((i_ctx_p->op_stack).stack).p) -= (8));
    }
    gs_setmatrix((i_ctx_p->pgs), &save_ctm);
    return code;
}
...
zcomposite(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op)); if ( (ulong)(*op).value.intval > (composite_PlusL) ) return ((-15)); } while (0);
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    const gx_device *dev = gs_currentdevice((i_ctx_p->pgs));
    gs_rect srect, drect;
    gs_matrix mat;
    gs_int_rect rect;
    int w, h;
    ...
    if ( !((((const byte *)&((&op[-4])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-4]));
    ...
    gs_currentmatrix((i_ctx_p->pgs), &mat);
    ...
    mat.tx -= rect.p.x;
    mat.ty -= rect.p.y;
    code = write_matrix_in(op, &mat, (&(i_ctx_p->memory)), ((void*)0));
    if (code < 0)
        return code;
    ((op - 4)->value.intval = (rect.p.x), ((op - 4)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op - 3)->value.intval = (rect.p.y), ((op - 3)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op - 2)->value.intval = (w), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op - 1)->value.intval = (h), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
static void
...
zsizeimageparams(i_ctx_t *i_ctx_p)
{
    ...
    if (device_is_true_color(dev))
        ...
        gx_color_value max_value =
            (dev->color_info.num_components == 1 ?
             ...
             (((dev->color_info.max_gray) > (dev->color_info.max_color)) ? (dev->color_info.max_gray) : (dev->color_info.max_color)));
        static const gx_color_value sizes[] = {
            1, 2, 4, 8, 12, sizeof(((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1))) * 8
        };
        int i;
    ...
    ((op - 2)->value.intval = (bps), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op - 1)->value.boolval = (0), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((op)->value.intval = (ncomp), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
begin_composite(i_ctx_t *i_ctx_p, alpha_composite_state_t * pcp)
{
    gx_device *dev = gs_currentdevice((i_ctx_p->pgs));
    int code =
        gs_create_composite_alpha(&pcp->pcte, &pcp->params, ((gs_memory_t *)((i_ctx_p->memory).current)));

    if (code < 0)
        return code;
    pcp->orig_dev = pcp->cdev = dev;
    code = (*((dev)->procs.create_compositor))
        (dev, &pcp->cdev, pcp->pcte, (gs_imager_state *)(i_ctx_p->pgs), ((gs_memory_t *)((i_ctx_p->memory).current)), ((void*)0));
    if (code < 0) {
        end_composite(i_ctx_p, pcp);
        return code;
    }
    gs_setdevice_no_init((i_ctx_p->pgs), pcp->cdev);
    return 0;
}
...
end_composite(i_ctx_t *i_ctx_p, alpha_composite_state_t * pcp)
{
    ...
    if (pcp->cdev != pcp->orig_dev) {
        gs_closedevice(pcp->cdev);
        gs_setdevice_no_init((i_ctx_p->pgs), pcp->orig_dev);
    }
    ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pcp->pcte, "end_composite(gs_composite_t)"));
}
...
device_is_true_color(gx_device * dev)
{
    ...
    switch (ncomp) {
        case 1:
            ...
            for (i = 0; i <= max_v; ++i) {
                gx_color_value v[3];
                v[0] = v[1] = v[2] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v);
                if ((*((dev)->procs.map_rgb_color)) (dev, v) != i)
                    return 0;
            }
            return ((bool)1);
        case 3:
                ...
                return ((bool)0);
            {
                ...
                for (i = 0; i <= max_v; ++i) {
                    ...
                    red[0] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); red[1] = ((gx_color_value)0), red[2] = ((gx_color_value)0);
                    green[0] = ((gx_color_value)0); green[1] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); green[2] = ((gx_color_value)0);
                    blue[0] = ((gx_color_value)0); blue[1] = ((gx_color_value)0); blue[2] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v);
                    if ((*((dev)->procs.map_rgb_color)) (dev, red) !=
                        i << rs ||
                        (*((dev)->procs.map_rgb_color)) (dev, green) !=
                        i << gs ||
                        (*((dev)->procs.map_rgb_color)) (dev, blue) !=
                        i
                        )
                        return 0;
                }
        ...
        case 4:
                ...
                return ((bool)0);
            {
                ...
                for (i = 0; i <= max_v; ++i) {
                    ...
                    cyan[0] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); cyan[1] = cyan[2] = cyan[3] = ((gx_color_value)0);
                    magenta[1] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); magenta[0] = magenta[2] = magenta[3] = ((gx_color_value)0);
                    yellow[2] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); yellow[0] = yellow[1] = yellow[3] = ((gx_color_value)0);
                    black[3] = (gx_color_value)((ulong)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) * i / max_v); black[0] = black[1] = black[2] = ((gx_color_value)0);
                    if ((*((dev)->procs.map_cmyk_color)) (dev, cyan) !=
                        i << cs ||
                        (*((dev)->procs.map_cmyk_color)) (dev, magenta) !=
                        i << ms ||
                        (*((dev)->procs.map_cmyk_color)) (dev, yellow) !=
                        i << ys ||
                        (*((dev)->procs.map_cmyk_color)) (dev, black) !=
                        i
                        )
                        return 0;
                }
