...
# 21 "./base/siinterp.c" 2
# 1 "./base/gxfrac.h" 1
# 27 "./base/gxfrac.h"
typedef short frac;
typedef short signed_frac;
# 22 "./base/siinterp.c" 2
# 1 "./base/strimpl.h" 1
# 23 "./base/strimpl.h"
# 1 "./base/scommon.h" 1
# 22 "./base/scommon.h"
...
typedef struct stream_IIEncode_state_s {
    ...
    int dst_x;
    gx_dda_int_t dda_x;
    gx_dda_int_t dda_x_init;
    int src_y, dst_y;
    gx_dda_int_t dda_y;
    int src_offset, dst_offset;
} stream_IIEncode_state;

static const gc_ptr_element_t iiencode_state_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_IIEncode_state *)0)->prev - (char *)((stream_IIEncode_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IIEncode_state *)0)->cur - (char *)((stream_IIEncode_state *)0) )) } }; static const gc_struct_data_t iiencode_state_reloc_ptrs = { (sizeof(iiencode_state_enum_ptrs) / sizeof((iiencode_state_enum_ptrs)[0])), 0, 0, iiencode_state_enum_ptrs }; static const gs_memory_struct_type_t st_IIEncode_state = { sizeof(stream_IIEncode_state), "ImageInterpolateEncode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &iiencode_state_reloc_ptrs };
...
s_IIEncode_init(stream_state * st)
{
    ...
    do { ((ss->dda_x).state).Q = (0), ((ss->dda_x).state).R = ((ss->params.WidthOut)-1); do { if ( (ss->params.WidthOut) == 0 ) ((ss->dda_x).step).dQ = 0, ((ss->dda_x).step).dR = 0; else if ( (ss->params.WidthIn) < 0 ) { ((ss->dda_x).step).dQ = -(int)((uint)-(ss->params.WidthIn) / (ss->params.WidthOut)); if ( (((ss->dda_x).step).dR = -(ss->params.WidthIn) % (ss->params.WidthOut)) != 0 ) --((ss->dda_x).step).dQ, ((ss->dda_x).step).dR = (ss->params.WidthOut) - ((ss->dda_x).step).dR; } else { ((ss->dda_x).step).dQ = (ss->params.WidthIn) / (ss->params.WidthOut); ((ss->dda_x).step).dR = (ss->params.WidthIn) % (ss->params.WidthOut); } ((ss->dda_x).step).N = (ss->params.WidthOut); } while (0); } while (0);
    ss->dda_x_init = ss->dda_x;
    ss->src_y = ss->dst_y = 0;
    do { ((ss->dda_y).state).Q = (0), ((ss->dda_y).state).R = ((ss->params.HeightIn)-1); do { if ( (ss->params.HeightIn) == 0 ) ((ss->dda_y).step).dQ = 0, ((ss->dda_y).step).dR = 0; else if ( (ss->params.HeightOut) < 0 ) { ((ss->dda_y).step).dQ = -(int)((uint)-(ss->params.HeightOut) / (ss->params.HeightIn)); if ( (((ss->dda_y).step).dR = -(ss->params.HeightOut) % (ss->params.HeightIn)) != 0 ) --((ss->dda_y).step).dQ, ((ss->dda_y).step).dR = (ss->params.HeightIn) - ((ss->dda_y).step).dR; } else { ((ss->dda_y).step).dQ = (ss->params.HeightOut) / (ss->params.HeightIn); ((ss->dda_y).step).dR = (ss->params.HeightOut) % (ss->params.HeightIn); } ((ss->dda_y).step).N = (ss->params.HeightIn); } while (0); } while (0);


    ss->prev = (*(mem)->procs.alloc_byte_array)(mem, ss->params.WidthIn, ss->sizeofPixelOut, "IIEncode prev");

    ss->cur = (*(mem)->procs.alloc_byte_array)(mem, ss->params.WidthIn, ss->sizeofPixelOut, "IIEncode cur");
    ...
    ss->scale_case =
        (ss->params.BitsPerComponentIn == 8 ?
         (ss->params.BitsPerComponentOut == 8 ?
          ...
          (ss->params.MaxValueIn == 255 && ss->params.MaxValueOut == ((frac)0x7ff8) ?
                   ...
                   stream_cursor_write * pw, bool last)
{
    ...
    const scale_case_t scale_case = ss->scale_case +
        ((uint)( ((const char *)(pw->ptr) - (const char *)0) & ((2) - 1) ));
    byte *out = pw->ptr + 1;
...
top:
    if (((ss->dda_y).state).Q > ss->dst_y) {

        while (ss->dst_x < ss->params.WidthOut) {
            uint sx = ((ss->dda_x).state).Q * ss->sizeofPixelIn;
            ...
            case SCALE_8_16_BYTE2FRAC_3:
                for (c = ss->params.spp_interp; --c >= 0; ++in, out += 2) {
                    uint b = *in;
                    uint value = ((frac)( ((frac)(b) << (15 - (8))) + ((b) >> ((8) * 2 - 15)) - ((b) >> ((8) - 3)) ));
            ...
            case SCALE_8_16_BYTE2FRAC_3_ALIGNED:
                {
                    ...
                    ((bits16 *)out)[0] = ((frac)( ((frac)(b) << (15 - (8))) + ((b) >> ((8) * 2 - 15)) - ((b) >> ((8) - 3)) ));
                    b = in[1];
                    ((bits16 *)out)[1] = ((frac)( ((frac)(b) << (15 - (8))) + ((b) >> ((8) * 2 - 15)) - ((b) >> ((8) - 3)) ));
                    b = in[2];
                    ((bits16 *)out)[2] = ((frac)( ((frac)(b) << (15 - (8))) + ((b) >> ((8) * 2 - 15)) - ((b) >> ((8) - 3)) ));
                }
                out += 6;
                break;
            case SCALE_8_16_GENERAL:
            case SCALE_8_16_GENERAL_ALIGNED:
                for (c = ss->params.spp_interp; --c >= 0; ++in, out += 2) {
            ...
            do { ((ss->dda_x).state).R -= ((ss->dda_x).step).dR; if ((signed)((ss->dda_x).state).R < 0) { ((ss->dda_x).state).Q++; ((ss->dda_x).state).R += ((ss->dda_x).step).N; }; ((ss->dda_x).state).Q += ((ss->dda_x).step).dQ; } while (0);
            ss->dst_x++;
        }
        ss->dst_x = 0;
        ss->dst_y++;
    ...
    if (ss->src_offset < ss->src_size) {
        uint count = (((ss->src_size - ss->src_offset) < (pr->limit - pr->ptr)) ? (ss->src_size - ss->src_offset) : (pr->limit - pr->ptr));
    ...
    ss->dst_x = 0;
    ss->dda_x = ss->dda_x_init;
    do { ((ss->dda_y).state).R -= ((ss->dda_y).step).dR; if ((signed)((ss->dda_y).state).R < 0) { ((ss->dda_y).state).Q++; ((ss->dda_y).state).R += ((ss->dda_y).step).N; }; ((ss->dda_y).state).Q += ((ss->dda_y).step).dQ; } while (0);
    goto top;
}
...
s_IIEncode_release(stream_state * st)
{
    ...
    ((mem)->procs.free_object(mem, ss->cur, "IIEncode cur"));
    ss->cur = 0;
    ((mem)->procs.free_object(mem, ss->prev, "IIEncode prev"));
    ss->prev = 0;
}
