...
# 26 "./base/gsclipsr.c" 2


static const gc_ptr_element_t clip_stack_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gx_clip_stack_t *)0)->clip_path - (char *)((gx_clip_stack_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gx_clip_stack_t *)0)->next - (char *)((gx_clip_stack_t *)0) )) } }; static const gc_struct_data_t clip_stack_reloc_ptrs = { (sizeof(clip_stack_enum_ptrs) / sizeof((clip_stack_enum_ptrs)[0])), 0, 0, clip_stack_enum_ptrs }; static const gs_memory_struct_type_t st_clip_stack = { sizeof(gx_clip_stack_t), "gx_clip_stack_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &clip_stack_reloc_ptrs };
...
rc_free_clip_stack(gs_memory_t * mem, void *vstack, client_name_t cname)
{
    ...
    if (stack->rc.ref_count <= 1 ) {
        ...
        ((stack->rc.memory)->procs.free_object(stack->rc.memory, stack, cname));
...
gs_clipsave(gs_state *pgs)
{
    ...
    gx_clip_stack_t *stack =
        (gx_clip_stack_t *)(*(mem)->procs.alloc_struct)(mem, &st_clip_stack, "gs_clipsave(stack)");


    if (copy == 0 || stack == 0) {
        ((mem)->procs.free_object(mem, stack, "gs_clipsave(stack)"));
        ((mem)->procs.free_object(mem, copy, "gs_clipsave(clip_path)"));
        return ((-25));
    }
    do { (stack)->rc.ref_count = 1; (stack)->rc.memory = mem; (stack)->rc.free = rc_free_clip_stack; do { } while (0); } while (0);
...
gs_cliprestore(gs_state *pgs)
{
    ...
    if (stack) {
        ...
        if (stack->rc.ref_count == 1) {
            ...
            ((stack->rc.memory)->procs.free_object(stack->rc.memory, stack, "cliprestore"));
