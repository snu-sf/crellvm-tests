...
# 26 "./base/gsmemlok.c" 2



static byte *gs_locked_alloc_bytes_immovable(gs_memory_t *mem, uint nbytes, client_name_t cname);
static void *gs_locked_resize_object(gs_memory_t *mem, void *obj, uint new_num_elements, client_name_t cname);
static void gs_locked_free_object(gs_memory_t *mem, void *data, client_name_t cname);
static gs_memory_t *gs_locked_stable(gs_memory_t *mem);
static void gs_locked_status(gs_memory_t *mem, gs_memory_status_t *status);
static void gs_locked_free_all(gs_memory_t *mem, uint free_mask, client_name_t cname);
static void gs_locked_consolidate_free(gs_memory_t *mem);


static byte *gs_locked_alloc_bytes(gs_memory_t *mem, uint nbytes, client_name_t cname);
static void *gs_locked_alloc_struct(gs_memory_t *mem, gs_memory_type_ptr_t pstype, client_name_t cname);
static void *gs_locked_alloc_struct_immovable(gs_memory_t *mem, gs_memory_type_ptr_t pstype, client_name_t cname);
static byte *gs_locked_alloc_byte_array(gs_memory_t *mem, uint num_elements, uint elt_size, client_name_t cname);
static byte *gs_locked_alloc_byte_array_immovable(gs_memory_t *mem, uint num_elements, uint elt_size, client_name_t cname);
static void *gs_locked_alloc_struct_array(gs_memory_t *mem, uint num_elements, gs_memory_type_ptr_t pstype, client_name_t cname);
static void *gs_locked_alloc_struct_array_immovable(gs_memory_t *mem, uint num_elements, gs_memory_type_ptr_t pstype, client_name_t cname);
static uint gs_locked_object_size(gs_memory_t *mem, const void *obj);
static gs_memory_type_ptr_t gs_locked_object_type(const gs_memory_t *mem, const void *obj);
static byte *gs_locked_alloc_string(gs_memory_t *mem, uint nbytes, client_name_t cname);
static byte *gs_locked_alloc_string_immovable(gs_memory_t *mem, uint nbytes, client_name_t cname);
static byte *gs_locked_resize_string(gs_memory_t *mem, byte *data, uint old_num, uint new_num, client_name_t cname);
static void gs_locked_free_string(gs_memory_t *mem, byte *data, uint nbytes, client_name_t cname);
static int gs_locked_register_root(gs_memory_t *mem, gs_gc_root_t *root, gs_ptr_type_t ptype, void **pp, client_name_t cname);
static void gs_locked_unregister_root(gs_memory_t *mem, gs_gc_root_t *root, client_name_t cname);
static void gs_locked_enable_free(gs_memory_t *mem, bool enable);
...
)
{
    ...
    lmem->thread_safe_memory = (gs_memory_t *)lmem;


    lmem->monitor = gx_monitor_alloc(target);
    return (lmem->monitor ? 0 : ((-25)));
}
...
{
    (((gs_memory_t *)lmem)->procs.free_all((gs_memory_t *)lmem, 2, "gs_memory_locked_release"));
...
gs_locked_free_all(gs_memory_t * mem, uint free_mask, client_name_t cname)
{
    ...
    if (free_mask & 4)
        ((target)->procs.free_object(target, lmem, cname));
}
static void
gs_locked_consolidate_free(gs_memory_t * mem)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.consolidate_free)(lmem->target); gp_monitor_leave(&(lmem->monitor)->native);
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_bytes) (lmem->target, size, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_bytes_immovable(gs_memory_t * mem, uint size,
                                client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_bytes_immovable) (lmem->target, size, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_struct(gs_memory_t * mem, gs_memory_type_ptr_t pstype,
                       client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; void * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_struct) (lmem->target, pstype, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; void * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_struct_immovable) (lmem->target, pstype, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_byte_array(gs_memory_t * mem, uint num_elements, uint elt_size,
                           client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_byte_array) (lmem->target, num_elements, elt_size, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_byte_array_immovable) (lmem->target, num_elements, elt_size, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_struct_array(gs_memory_t * mem, uint num_elements,
                           gs_memory_type_ptr_t pstype, client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; void * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_struct_array) (lmem->target, num_elements, pstype, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_struct_array_immovable(gs_memory_t * mem, uint num_elements,
                           gs_memory_type_ptr_t pstype, client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; void * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_struct_array_immovable) (lmem->target, num_elements, pstype, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_resize_object(gs_memory_t * mem, void *obj, uint new_num_elements,
                        client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; void * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.resize_object) (lmem->target, obj, new_num_elements, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; uint temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.object_size) (lmem->target, ptr); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gs_memory_type_ptr_t temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.object_type) (lmem->target, ptr); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.free_object) (lmem->target, ptr, cname); gp_monitor_leave(&(lmem->monitor)->native);
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_string) (lmem->target, nbytes, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_alloc_string_immovable(gs_memory_t * mem, uint nbytes,
                                 client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.alloc_string_immovable) (lmem->target, nbytes, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_resize_string(gs_memory_t * mem, byte * data, uint old_num,
                        ...
                        client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; byte * temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.resize_string) (lmem->target, data, old_num, new_num, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_free_string(gs_memory_t * mem, byte * data, uint nbytes,
                      client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.free_string) (lmem->target, data, nbytes, cname); gp_monitor_leave(&(lmem->monitor)->native);
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; int temp; gp_monitor_enter(&(lmem->monitor)->native); temp = (*lmem->target->procs.register_root) (lmem->target, rp, ptype, up, cname); gp_monitor_leave(&(lmem->monitor)->native); return temp;
...
gs_locked_unregister_root(gs_memory_t * mem, gs_gc_root_t * rp,
                          client_name_t cname)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.unregister_root) (lmem->target, rp, cname); gp_monitor_leave(&(lmem->monitor)->native);
...
gs_locked_stable(gs_memory_t * mem)
{
    if (!mem->stable_memory) {
        ...
        gp_monitor_enter(&(lmem->monitor)->native);
        stable = ((lmem->target)->procs.stable(lmem->target));
        if (stable == lmem->target)
            mem->stable_memory = mem;
        else {
            gs_memory_locked_t *locked_stable = (gs_memory_locked_t *)
                (*(stable)->procs.alloc_bytes)(stable, sizeof(*lmem), "gs_locked_stable");

            if (locked_stable) {
                int code = gs_memory_locked_init(locked_stable, stable);

                if (code < 0)
                    ((stable)->procs.free_object(stable, locked_stable, "gs_locked_stable"));
                else
                    mem->stable_memory = (gs_memory_t *)locked_stable;
            }
        }
        gp_monitor_leave(&(lmem->monitor)->native);
    }
...
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.status)(lmem->target, pstat); gp_monitor_leave(&(lmem->monitor)->native);


    pstat->is_thread_safe = ((bool)1);
}
static void
gs_locked_enable_free(gs_memory_t * mem, bool enable)
{
    gs_memory_locked_t * const lmem = (gs_memory_locked_t *)mem; gp_monitor_enter(&(lmem->monitor)->native); (*lmem->target->procs.enable_free)(lmem->target, enable); gp_monitor_leave(&(lmem->monitor)->native);
