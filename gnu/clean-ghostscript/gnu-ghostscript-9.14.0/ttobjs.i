...
# 185 "./base/ttobjs.c"
  TT_Error Clear_CodeRange( PExecution_Context exec, Int range )
  {
    ...
    exec->codeRangeTable[range - 1].Base = (Byte*)((void*)0);
...
static int free_aux(ttfMemory *mem, void *ptr)
{
 ...
 TT_Error Context_Destroy( void* _context )
 {
   ...
   { mem->free(mem, exec->pts.cur_y, "ttobjs.c"); exec->pts.cur_y = ((void*)0); };
   { mem->free(mem, exec->pts.cur_x, "ttobjs.c"); exec->pts.cur_x = ((void*)0); };
   { mem->free(mem, exec->pts.org_y, "ttobjs.c"); exec->pts.org_y = ((void*)0); };
   { mem->free(mem, exec->pts.org_x, "ttobjs.c"); exec->pts.org_x = ((void*)0); };
   { mem->free(mem, exec->pts.touch, "ttobjs.c"); exec->pts.touch = ((void*)0); };
   { mem->free(mem, exec->pts.contours, "ttobjs.c"); exec->pts.contours = ((void*)0); };
   exec->pts.n_points = 0;
   exec->pts.n_contours = 0;


   { mem->free(mem, exec->twilight.touch, "ttobjs.c"); exec->twilight.touch = ((void*)0); };
   { mem->free(mem, exec->twilight.cur_y, "ttobjs.c"); exec->twilight.cur_y = ((void*)0); };
   { mem->free(mem, exec->twilight.cur_x, "ttobjs.c"); exec->twilight.cur_x = ((void*)0); };
   { mem->free(mem, exec->twilight.org_y, "ttobjs.c"); exec->twilight.org_y = ((void*)0); };
   { mem->free(mem, exec->twilight.org_x, "ttobjs.c"); exec->twilight.org_x = ((void*)0); };
   { mem->free(mem, exec->twilight.contours, "ttobjs.c"); exec->twilight.contours = ((void*)0); };
   exec->twilight.n_points = 0;
   exec->twilight.n_contours = 0;


   { mem->free(mem, exec->stack, "ttobjs.c"); exec->stack = ((void*)0); };
   exec->stackSize = 0;


   { mem->free(mem, exec->callStack, "ttobjs.c"); exec->callStack = ((void*)0); };
   exec->callSize = 0;
   exec->callTop = 0;
   ...
   exec->current_face = (PFace)((void*)0);
 ...
 TT_Error Context_Create( void* _context, void* _face )
 {
   ...
   if ( (exec->callSize >= callSize ? 0 : !(free_aux(mem, exec->callStack), exec->callStack = mem->alloc_bytes(mem, (callSize) * sizeof(TCallRecord), "ttobjs.c"))) ||


        (exec->stackSize >= stackSize ? 0 : !(free_aux(mem, exec->stack), exec->stack = mem->alloc_bytes(mem, (stackSize) * sizeof(Long), "ttobjs.c"))) ||


        (exec->n_points >= n_points ? 0 : !(free_aux(mem, exec->pts.org_x), exec->pts.org_x = mem->alloc_bytes(mem, (n_points) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->n_points >= n_points ? 0 : !(free_aux(mem, exec->pts.org_y), exec->pts.org_y = mem->alloc_bytes(mem, (n_points) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->n_points >= n_points ? 0 : !(free_aux(mem, exec->pts.cur_x), exec->pts.cur_x = mem->alloc_bytes(mem, (n_points) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->n_points >= n_points ? 0 : !(free_aux(mem, exec->pts.cur_y), exec->pts.cur_y = mem->alloc_bytes(mem, (n_points) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->n_points >= n_points ? 0 : !(free_aux(mem, exec->pts.touch), exec->pts.touch = mem->alloc_bytes(mem, (n_points) * sizeof(Byte), "ttobjs.c"))) ||


        (exec->twilight.n_points >= n_twilight ? 0 : !(free_aux(mem, exec->twilight.org_x), exec->twilight.org_x = mem->alloc_bytes(mem, (n_twilight) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->twilight.n_points >= n_twilight ? 0 : !(free_aux(mem, exec->twilight.org_y), exec->twilight.org_y = mem->alloc_bytes(mem, (n_twilight) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->twilight.n_points >= n_twilight ? 0 : !(free_aux(mem, exec->twilight.cur_x), exec->twilight.cur_x = mem->alloc_bytes(mem, (n_twilight) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->twilight.n_points >= n_twilight ? 0 : !(free_aux(mem, exec->twilight.cur_y), exec->twilight.cur_y = mem->alloc_bytes(mem, (n_twilight) * sizeof(TT_F26Dot6), "ttobjs.c"))) ||
        (exec->twilight.n_points >= n_twilight ? 0 : !(free_aux(mem, exec->twilight.touch), exec->twilight.touch = mem->alloc_bytes(mem, (n_twilight) * sizeof(Byte), "ttobjs.c"))) ||


        (exec->n_contours >= face->maxContours ? 0 : !(free_aux(mem, exec->pts.contours), exec->pts.contours = mem->alloc_bytes(mem, (face->maxContours) * sizeof(UShort), "ttobjs.c")))

      )
     goto Fail_Memory;

     exec->callSize = (exec->callSize > callSize ? exec->callSize : callSize);
     exec->stackSize = (exec->stackSize > stackSize ? exec->stackSize : stackSize);
     exec->twilight.n_points = (exec->twilight.n_points > n_twilight ? exec->twilight.n_points : n_twilight);
     exec->maxGlyphSize = (exec->maxGlyphSize > maxp->maxSizeOfInstructions ? exec->maxGlyphSize : maxp->maxSizeOfInstructions);
     exec->n_contours = (exec->n_contours > face->maxContours ? exec->n_contours : face->maxContours);
     exec->n_points = (exec->n_points > n_points ? exec->n_points : n_points);
     exec->lock++;
                        ...
                        Bool debug )
 {
   ...
   if ( ( error = Goto_CodeRange( exec, 3, 0 ) ) )
     return error;
...
# 494 "./base/ttobjs.c"
  TT_Error Instance_Destroy( void* _instance )
  {
    ...
    { mem->free(mem, ins->cvt, "ttobjs.c"); ins->cvt = ((void*)0); };
    ins->cvtSize = 0;

    { mem->free(mem, ins->FDefs, "ttobjs.c"); ins->FDefs = ((void*)0); };
    { mem->free(mem, ins->IDefs, "ttobjs.c"); ins->IDefs = ((void*)0); };
    { mem->free(mem, ins->storage, "ttobjs.c"); ins->storage = ((void*)0); };
    ins->numFDefs = 0;
    ins->numIDefs = 0;

    ins->face = (PFace)((void*)0);
    ins->valid = 0;
...
# 538 "./base/ttobjs.c"
                             ...
                             void* _face )
  {
    ...
    ins->metrics.rotated = 0;
    ins->metrics.stretched = 0;
    ...
    if ( (0 >= ins->numFDefs ? 0 : !(free_aux(mem, ins->FDefs), ins->FDefs = mem->alloc_bytes(mem, (ins->numFDefs) * sizeof(TDefRecord), "ttobjs.c"))) ||
         (0 >= ins->numIDefs ? 0 : !(free_aux(mem, ins->IDefs), ins->IDefs = mem->alloc_bytes(mem, (ins->numIDefs) * sizeof(TDefRecord), "ttobjs.c"))) ||
         (0 >= ins->cvtSize ? 0 : !(free_aux(mem, ins->cvt), ins->cvt = mem->alloc_bytes(mem, (ins->cvtSize) * sizeof(Long), "ttobjs.c"))) ||
         (0 >= ins->storeSize ? 0 : !(free_aux(mem, ins->storage), ins->storage = mem->alloc_bytes(mem, (ins->storeSize) * sizeof(Long), "ttobjs.c"))) )
      goto Fail_Memory;
...
# 612 "./base/ttobjs.c"
  TT_Error Instance_Init( PInstance ins )
  {
    ...
    Clear_CodeRange( exec, 2 );
    Clear_CodeRange( exec, 3 );

    if ( face->fontPgmSize > 0 )
    {
      error = Goto_CodeRange( exec, 1, 0 );
      if ( error )
        goto Fin;
...
# 821 "./base/ttobjs.c"
  TT_Error Face_Destroy( PFace face )
  {
    if ( face ) {
        ...
        { mem->free(mem, face->cvt, "ttobjs.c"); face->cvt = ((void*)0); };
        face->cvtSize = 0;


        { mem->free(mem, face->fontProgram, "ttobjs.c"); face->fontProgram = ((void*)0); };
        { mem->free(mem, face->cvtProgram, "ttobjs.c"); face->cvtProgram = ((void*)0); };
        face->fontPgmSize = 0;
        face->cvtPgmSize = 0;
    }
    return 0;
...
# 858 "./base/ttobjs.c"
  TT_Error Face_Create( PFace face)
  {
    ...
    if (
         ( error = Load_TrueType_MaxProfile (face) ) ||


         ( error = Load_TrueType_CVT (face) ) ||

         ( error = Load_TrueType_Programs (face) )
