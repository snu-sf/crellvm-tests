...
static const gs_memory_struct_type_t st_exE_state = { sizeof(stream_exE_state), "eexecEncode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
const stream_template s_exE_template = {
    &st_exE_state, ((void*)0), s_exE_process, 1, 2
};



static const gc_ptr_element_t exd_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_exD_state *)0)->pfb_state - (char *)((stream_exD_state *)0) )) } }; static const gc_struct_data_t exd_reloc_ptrs = { (sizeof(exd_enum_ptrs) / sizeof((exd_enum_ptrs)[0])), 0, 0, exd_enum_ptrs }; static const gs_memory_struct_type_t st_exD_state = { sizeof(stream_exD_state), "eexecDecode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &exd_reloc_ptrs };
...
s_exD_set_defaults(stream_state * st)
{
    ...
    ss->binary = -1;
    ss->lenIV = 4;
    ss->hex_left = (~(-1L << ((1 << 3) * 8 - 1)));

    ss->pfb_state = 0;
    ss->keep_spaces = ((bool)0);
    ss->is_leading_space = ((bool)1);
}
              ...
              stream_cursor_write * pw, bool last)
{
    ...
    if (ss->binary < 0) {
        ...
        if (ss->pfb_state == 0 && !ss->keep_spaces) {
            ...
            pr->ptr = p;
            count = (((wcount) < (rcount)) ? (wcount) : (rcount));
        }
# 134 "./base/seexec.c"
        if (rcount < 8 && !last)
            return 0;
