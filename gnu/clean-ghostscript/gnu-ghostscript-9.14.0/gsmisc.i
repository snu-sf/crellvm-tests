...
dprintf_file_tail(const char *file)
{
    ...
    while (tail > file &&
           (((*__ctype_b_loc ())[(int) (((unsigned char)tail[-1]))] & (unsigned short int) _ISalnum) || tail[-1] == '.' || tail[-1] == '_')
        )
        --tail;
    return tail;
}
...
{
    if (gs_debug['/'])
        errprintf_nomem(dprintf_file_and_line_format,
                      ...
                      long revision_number)
{
    if (program_name) {
        ...
        if (revision_number) {
            ...
            errprintf_nomem("%d.%02d.%d", (int)(revision_number / 10000 % 100),
...
{
    if (gs_debug['/'])
        errprintf(mem, dprintf_file_and_line_format,
                dprintf_file_tail(file), line);
}
# 226 "./base/gsmisc.c"
void
                       ...
                       long revision_number)
{
    if (program_name) {
        ...
        if (revision_number) {
            ...
            errprintf(mem, "%d.%02d.%d", (int)(revision_number / 10000 % 100),
        (int)(revision_number / 100 % 100), fpart);
        }
        errprintf(mem, ": ");
    }
...
gs_return_check_interrupt(const gs_memory_t *mem, int code)
{
        ...
        return code;
    {
        ...
        return (icode == 0 ? code :
                gs_log_error((icode > 0 ? (-6) : icode), "./base/gsmisc.c", 294));
    }
}
...
const char *gs_errstr(int code)
{
    ...
    case (-6): return "interrupt";
    case (-7): return "invalidaccess";
    case (-9): return "invalidfileaccess";
    case (-10): return "invalidfont";
    case (-12): return "ioerror";
    case (-13): return "limitcheck";
    case (-14): return "nocurrentpoint";
    case (-15): return "rangecheck";
    case (-20): return "typecheck";
    case (-21): return "undefined";
    case (-22): return "undefinedfilename";
    case (-23): return "undefinedresult";
    case (-25): return "vmerror";
    case (-28): return "unregistered";
    case (-99): return "hit_detected";
    case (-100): return "Fatal";
    }
}
# 509 "./base/gsmisc.c"
void *
...
debug_dump_bytes(const gs_memory_t *mem, const byte * from, const byte * to, const char *msg)
{
    ...
    if (from < to && msg)
        errprintf(mem,"%s:\n", msg);
    while (p != to) {
        const byte *q = (((p + 16) < (to)) ? (p + 16) : (to));

        errprintf(mem,"0x%lx:", (ulong) p);
        while (p != q)
            errprintf(mem," %02x", *p++);
        errprintf(mem,"%c", '\n');
    }
...
debug_dump_bitmap(const gs_memory_t *mem, const byte * bits, uint raster, uint height, const char *msg)
{
    ...
    for (y = 0; y < height; ++y, data += raster)
        debug_dump_bytes(mem, data, data + raster, (y == 0 ? msg : ((void*)0)));
}
...
fixed_mult_quo(fixed signed_A, fixed B, fixed C)
{
    ...
    if (B < (1L << (53 - (1 << 2) * 8)) || ((signed_A) < 0 ? -(signed_A) : (signed_A)) < (1L << (53 - (1 << 2) * 8))) {
...
# 900 "./base/gsmisc.c"
        fixed q =
            (fixed)floor(((double)signed_A * B + (1L << ((1 << 2) * 8 * 2 - 53))) / C);
...
gs_sin_degrees(double ang)
{
    ...
    return sin(ang * (3.14159265358979323846 / 180));
}
...
gs_cos_degrees(double ang)
{
    ...
    return cos(ang * (3.14159265358979323846 / 180));
}
...
gs_sincos_degrees(double ang, gs_sincos_t * psincos)
{
    ...
    if (floor(quot) == quot) {
        ...
        psincos->orthogonal = ((bool)1);
    } else {
        double arad = ang * (3.14159265358979323846 / 180);

        psincos->sin = sin(arad);
        psincos->cos = cos(arad);
        psincos->orthogonal = ((bool)0);
    }
}
...
gs_atan2_degrees(double y, double x, double *pangle)
{
    if (y == 0) {
        if (x == 0)
            return gs_log_error((-23), "./base/gsmisc.c", 1002);
        *pangle = (x < 0 ? 180 : 0);
    } else {
        double result = atan2(y, x) * (180.0 / 3.14159265358979323846);
...
{ 0, "", "" },
{1, "icc" ,"ICC profile"},
{1, "validate_chunks" ,"Validate chunks during interpretation"},
{1, "gc_disable" ,"Disable Garbage Collection (completely)"},
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
...
{ 0, "", "" },
{ 0, "", "" },
{1, "ps_op_names" ,"Postscript operator names"},
{1, "contexts_detail" ,"Contexts (detail)"},
{1, "trace_errors" ,"Turn on tracing of error returns from operators"},
{1, "memfill_obj" ,"Fill unused parts of object with identifiable garbage values"},
{1, "ext_commands" ,"Externally processed commands"},
{ 0, "", "" },
{1, "contexts" ,"Contexts (create/destroy)"},
{ 0, "", "" },
{ 0, "", "" },
{1, "image" ,"Image and rasterop parameters"},
{1, "min_stack" ,"Use minimum_size stack blocks"},
{1, "no_path_banding" ,"Don't use path-based banding"},
{ 0, "", "" },
{1, "small_mem_tables" ,"Use small-memory table sizes even on large-memory machines"},
{1, "file_line" ,"Include file/line info on all trace output"},
{1, "gc" ,"Garbage collection, minimal detail"},
{1, "type1" ,"Type 1 and type 43 font interpreter"},
{1, "curve" ,"Curve subdivider/rasterizer"},
{1, "curve_detail" ,"Curve subdivider/rasterizer (detail)"},
{1, "gc_strings" ,"Garbage collector (strings)"},
{1, "gc_strings_detail" ,"Garbage collector (strings, detail)"},
{1, "gc_chunks" ,"Garbage collector (chunks, roots)"},
{1, "gc_objects" ,"Garbage collector (objects)"},
{1, "gc_refs" ,"Garbage collector (refs)"},
{1, "gc_pointers" ,"Garbage collector (pointers)"},
{1, "time" ,"Command list and allocator time summary"},
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{1, "validate_pointers" ,"Validate pointers before/during/after garbage collection/save and restore"},
{1, "memfill_empty" ,"Fill empty storage with a distinctive bit pattern for debugging"},
{1, "alloc_detail" ,"Allocator (all calls)"},
{1, "bitmap_detail" ,"Bitmap images (detail)"},
{1, "color_detail" ,"Color mapping (detail)"},
{1, "dict_detail" ,"Dictionary (every lookup)"},
{ 0, "", "" },
{1, "fill_detail" ,"Fill algorithm (detail)"},
{ 0, "", "" },
{1, "halftones_detail" ,"Halftones (detail)"},
{1, "interp_detail" ,"Interpreter (detail)"},
{ 0, "", "" },
{1, "char_cache_detail" ,"Character cache (detail)"},
{1, "clist_detail" ,"Command list (detail)"},
{ 0, "", "" },
{ 0, "", "" },
{1, "stroke_detail" ,"Stroke (detail)"},
{1, "paths_detail" ,"Paths (detail)"},
{ 0, "", "" },
{ 0, "", "" },
{1, "scanner" ,"Scanner"},
{ 0, "", "" },
{1, "undo_detail" ,"Undo saver (for save/restore) (detail)"},
{1, "compositors_detail" ,"Compositors (alpha/transparency/overprint/rop) (detail)"},
{ 0, "", "" },
{ 0, "", "" },
{1, "type1_hints_detail" ,"Type 1 hints (detail)"},
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{1, "ref_counts" ,"Reference counting"},
{1, "high_level" ,"High level output"},
{1, "no_hl_img_banding" ,"Don't use high_level banded images"},
{1, "alloc" ,"Allocator (large blocks)"},
{1, "bitmap" ,"Bitmap images"},
{1, "color_halftones" ,"Color mapping"},
{1, "dict" ,"Dictionary (put/undef)"},
{1, "external_calls" ,"External (OS related) calls"},
{1, "fill" ,"Fill algorithm"},
{1, "gsave" ,"gsave/grestore"},
{1, "halftones" ,"Halftones"},
{1, "interp" ,"Interpreter (names only)"},
{1, "comp_fonts" ,"Composite fonts"},
{1, "char_cache" ,"Character cache"},
{1, "clist" ,"Command list (bands)"},
{1, "makefont" ,"makefont and font cache"},
{1, "names" ,"Name lookup (new names only)"},
{1, "stroke" ,"Stroke"},
{1, "paths" ,"Paths (band list)"},
{1, "clipping" ,"Clipping"},
{1, "arcs" ,"Arcs"},
{1, "streams" ,"Streams"},
{1, "tiling" ,"Tiling algorithm"},
{1, "undo" ,"Undo saver (for save/restore)"},
{1, "compositors" ,"Compositors (alpha/transparency/overprint/rop)"},
{1, "compress" ,"Compression encoder/decoder"},
{1, "transforms" ,"Transformations"},
{1, "type1_hints" ,"Type 1 hints"},
{1, "trapezoid_fill" ,"Trapezoid fill"},
{ 0, "", "" },
{ 0, "", "" },
{ 0, "", "" },
{1, "math" ,"Math functions and Functions"}
# 1134 "./base/gsmisc.c" 2
