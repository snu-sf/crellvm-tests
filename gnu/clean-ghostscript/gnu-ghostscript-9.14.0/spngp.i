...
static const gc_ptr_element_t pngp_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_PNGP_state *)0)->prev_row - (char *)((stream_PNGP_state *)0) )) } }; static const gc_struct_data_t pngp_reloc_ptrs = { (sizeof(pngp_enum_ptrs) / sizeof((pngp_enum_ptrs)[0])), 0, 0, pngp_enum_ptrs }; static const gs_memory_struct_type_t st_PNGP_state = { sizeof(stream_PNGP_state), "PNGPredictorEncode/Decode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pngp_reloc_ptrs };
# 35 "./base/spngp.c"
...
s_PNGP_set_defaults(stream_state * st)
{
    ...
    ((ss)->Colors = 1, (ss)->BitsPerComponent = 8, (ss)->Columns = 1, (ss)->Predictor = 15, (ss)->prev_row = 0);
}
...
s_pngp_init(stream_state * st, bool need_prev)
{
    ...
    if (bits_per_row > ((unsigned int)~0 + (unsigned int)0) * 7L)
        return ((int)(-2));
    ...
    if (need_prev) {
        prev_row = (*(st->memory)->procs.alloc_bytes)(st->memory, ss->bpp + ss->row_count, "PNGPredictor prev row");
...
s_PNGPE_init(stream_state * st)
{
    ...
    return s_pngp_init(st, pngp_case_needs_prev[ss->Predictor - 10]);
}
...
s_PNGP_release(stream_state *st)
{
    ...
    if (ss->prev_row)
        ((st->memory)->procs.free_object(st->memory, ss->prev_row, "PNGPredictor prev row"));
}
# 127 "./base/spngp.c"
static int
