...
static const gs_memory_struct_type_t st_int_pattern = { sizeof(int_pattern), "int_pattern", 0, ref_struct_clear_marks, ref_struct_enum_ptrs, ref_struct_reloc_ptrs, 0, 0 };


static int
zpcolor_init(i_ctx_t *i_ctx_p)
{
    gx_pattern_cache *pc = gx_pattern_alloc_cache(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)),
                                                  gx_pat_cache_default_tiles(),
                                                  gx_pat_cache_default_bits());
    if (pc == ((void*)0))
 return ((-25));
    gstate_set_pattern_cache((i_ctx_p->pgs), pc);
    return 0;
}
...
{
    int_pattern *pdata =
        (int_pattern *)(*(mem)->procs.alloc_struct)(mem, &st_int_pattern, "int_pattern");
...
zbuildpattern1(i_ctx_t *i_ctx_p)
{
    ...
    code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op, &mat);
    if (code < 0)
        return code;
    if ( !((((const byte *)&((&*op1)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*op1));
    do { if ( !(((&*(&(&*op1)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    gs_pattern1_init(&templat);

    code = dict_uid_param(op1, &templat.uid, 1, ((gs_memory_t *)((i_ctx_p->memory).current)), i_ctx_p);
    if (code < 0)
        return code;
    if (code != 1)
        return ((-15));
    ...
    code = dict_floats_param(((gs_memory_t *)((i_ctx_p->memory).current)), op1, "BBox", 4, BBox, ((void*)0));
    if (code < 0)
        return code;
    if (code == 0)
       return ((-21));
    ...
    do { if ( !(((&*pPaintProc)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*pPaintProc)); } while (0);
    ...
    code = int_pattern_alloc(&pdata, op1, ((gs_memory_t *)((i_ctx_p->memory).current)));
    if (code < 0)
        return code;
    templat.client_data = pdata;
    code = gs_makepattern(&cc_instance, &templat, &mat, (i_ctx_p->pgs), ((gs_memory_t *)((i_ctx_p->memory).current)));
    if (code < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pdata, "int_pattern"));
        return code;
    }
    ((op)->value.pstruct = ((obj_header_t *)(cc_instance.pattern)), ((op)->tas.type_attrs = ((t_struct) << 8) + (((0x20 + 0x40)) | (((&(i_ctx_p->memory)))->current_space))) );
    return code;
}
...
zPaintProc(const gs_client_color * pcc, gs_state * pgs)
{
    ...
    ((int_remap_color_info_t *)((&((int_gstate *)((pgs)->client_data))->remap_color_info)->value.pstruct))->proc =
...
pattern_paint_prepare(i_ctx_t *i_ctx_p)
{
    ...
    if (pgs->have_pattern_streams) {
        code = ((cdev)->procs.dev_spec_op)(cdev, gxdso_pattern_can_accum,
                                pinst, pinst->id);
        if (code < 0)
            return code;
    ...
    if (internal_accum) {
        ...
        pdev = gx_pattern_accum_alloc(((gs_memory_t *)((i_ctx_p->memory).current)), storage_memory, pinst, "pattern_paint_prepare");
        if (pdev == 0)
            return ((-25));
        code = (*((pdev)->procs.open_device)) ((gx_device *) pdev);
        if (code < 0) {
            ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pdev, "pattern_paint_prepare"));
            return code;
        }
    } else {
        code = gx_pattern_cache_add_dummy_entry((gs_imager_state *)(i_ctx_p->pgs),
    ...
    if (internal_accum) {
        ...
        if (pinst->templat.uses_transparency) {
            do { } while (0);
            if ((code = gs_push_pdf14trans_device(pgs, ((bool)1))) < 0)
                return code;
        } else {
        ...
        gs_setmatrix((i_ctx_p->pgs), &m);
        code = gs_bbox_transform(&pinst->templat.BBox, &(*(const gs_matrix *)&(pgs)->ctm), &bbox);
        if (code < 0) {
            gs_grestore(pgs);
            return code;
        }
        ...
        code = gx_clip_to_rectangle((i_ctx_p->pgs), &clip_box);
        if (code < 0) {
            gs_grestore(pgs);
            return code;
        }
        code = ((cdev)->procs.dev_spec_op)(cdev, gxdso_pattern_start_accum,
                                pinst, pinst->id);
        if (code < 0) {
            gs_grestore(pgs);
            return code;
    ...
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (pattern_paint_cleanup), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    ++(((i_ctx_p->exec_stack).stack).p);
    (((((i_ctx_p->exec_stack).stack).p))->value.pstruct = ((obj_header_t *)(pdev)), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
    ++(((i_ctx_p->exec_stack).stack).p);

    (((((i_ctx_p->exec_stack).stack).p))->value.intval = (ref_stack_count(&((i_ctx_p->op_stack).stack))), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (pattern_paint_finish), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    dict_find_string(pdict, "PaintProc", &ppp);
    *++(((i_ctx_p->exec_stack).stack).p) = *ppp;
    *++(((i_ctx_p->exec_stack).stack).p) = *pdict;
    return 5;
}
...
{
    int o_stack_adjust = ref_stack_count(&((i_ctx_p->op_stack).stack)) - (((i_ctx_p->exec_stack).stack).p)->value.intval;
    gx_device_forward *pdev = ((gx_device_forward *)(((((i_ctx_p->exec_stack).stack).p) - 1)->value.pstruct));
    gs_pattern1_instance_t *pinst =
        (gs_pattern1_instance_t *)gs_currentcolor((i_ctx_p->pgs)->saved)->pattern;
    ...
    if (pdev != ((void*)0)) {
        ...
        if (pinst->templat.uses_transparency) {
            ...
            if (pinst->is_clist) {
                ...
                code = (gs_pop_pdf14trans_device(pgs, ((bool)1)) < 0);
                if (code < 0)
                    return code;
            } else {
        ...
        code = gx_pattern_cache_add_entry((gs_imager_state *)(i_ctx_p->pgs), pdev, &ctile);
        if (code < 0)
            return code;
    }
    if (o_stack_adjust > 0) {
        ...
        ((((i_ctx_p->op_stack).stack).p) -= (o_stack_adjust));
    }
...
pattern_paint_cleanup(i_ctx_t *i_ctx_p)
{
    gx_device_pattern_accum *const pdev =
        ((gx_device_pattern_accum *)(((((i_ctx_p->exec_stack).stack).p) + 2)->value.pstruct));
    int code;

    if (pdev != ((void*)0)) {

        (*((pdev)->procs.close_device)) ((gx_device *) pdev);
    }
    code = gs_grestore((i_ctx_p->pgs));
    (((((i_ctx_p->pgs))->color[0].dev_color))->type = (&gx_dc_type_data_none));
    if (pdev == ((void*)0)) {
        gx_device *cdev = (((i_ctx_p->pgs))->device);
        int code1 = ((cdev)->procs.dev_spec_op)(cdev,
