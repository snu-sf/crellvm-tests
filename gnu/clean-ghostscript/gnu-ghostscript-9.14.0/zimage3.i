...
zimage3(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = dict_int_param(op, "InterleaveType", 1, 3, -1,
        ...
        return code;
    gs_image3_t_init(&image, ((void*)0), interleave_type);
    ...
    if ((code = pixel_image_params(i_ctx_p, pDataDict,
                        ...
                        12, ((bool)0), gs_currentcolorspace((i_ctx_p->pgs)))) < 0 ||
        (mcode = code = data_image_params(((gs_memory_t *)((i_ctx_p->memory).current)), pMaskDict, &image.MaskDict,
                                   &ip_mask, ((bool)0), 1, 12, ((bool)0), ((bool)0))) < 0 ||
    ...
    if (image.InterleaveType == 3) {
        ...
        memmove(&ip_data.DataSource[1], &ip_data.DataSource[0],
                ((sizeof(ip_data.DataSource) / sizeof((ip_data.DataSource)[0])) - 1) *
...
zimage4(i_ctx_t *i_ctx_p)
{
    ...
    int colors[(sizeof(image.MaskColor) / sizeof((image.MaskColor)[0]))];
    int code;
    int i;

    gs_image4_t_init(&image, ((void*)0));
    code = pixel_image_params(i_ctx_p, op, (gs_pixel_image_t *)&image, &ip,
                              12, ((bool)0), gs_currentcolorspace((i_ctx_p->pgs)));
    if (code < 0)
        return code;
    code = dict_int_array_check_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, "MaskColor",
       num_components * 2, colors, 0, (-15));

    if (code == num_components) {
        image.MaskColor_is_range = ((bool)0);
        for (i = 0; i < code; ++i)
            image.MaskColor[i] = (colors[i] < 0 ? ~(uint)0 : colors[i]);
    }
    else if (code == num_components * 2) {
        image.MaskColor_is_range = ((bool)1);
        for (i = 0; i < code; i += 2) {
            if (colors[i+1] < 0)
                image.MaskColor[i] = 1, image.MaskColor[i+1] = 0;
            else {
                image.MaskColor[i+1] = colors[i+1];
                image.MaskColor[i] = (((colors[i]) > (0)) ? (colors[i]) : (0));
            }
        }
    } else
