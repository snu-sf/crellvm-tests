...
{
    float *p = (float *)
        (*(mem)->procs.alloc_byte_array)(mem, size, sizeof(float), "write_floats");
            ...
            const gs_range3 * domain, gs_memory_t * mem)
{
    ...
    if (!memcmp(procs, &Encode_default, sizeof(*procs)))
        return 0;
    values = (float *)(*(mem)->procs.alloc_byte_array)(mem, size * 3, sizeof(float), "write_proc3");
                        ...
                        gs_cie_render * pcrd, gs_memory_t * mem)
{
    ...
    dict.size = 20;
    if ((code = (*(plist)->procs->begin_xmit_collection)(plist, key, &dict, (((bool)0) ? gs_param_collection_dict_int_keys : gs_param_collection_dict_any))) < 0)
        return code;
    code = param_put_cie_render1(dict.list, pcrd, mem);
    dcode = (*(plist)->procs->end_xmit_collection)(plist, key, &dict);
...
{
    int crd_type = 101;
    int code = gs_cie_render_sample(pcrd);
    ...
    if (pcrd->TransformPQR.proc_name) {
        ...
        ((pn).data = (const byte *)(pcrd->TransformPQR.proc_name), (pn).size = strlen((const char *)(pn).data), (pn).persistent = ((bool)1));
        pn.size++;
        pd.data = pcrd->TransformPQR.proc_data.data;
        pd.size = pcrd->TransformPQR.proc_data.size;
        pd.persistent = ((bool)1);
    ...
    if (pcrd->RenderTable.lookup.table) {
        ...
        int *size = (int *)
            (*(mem)->procs.alloc_byte_array)(mem, n + 1, sizeof(int), "RenderTableSize");
# 202 "./base/gscrdp.c"
        gs_param_string *table =
            (gs_param_string *)
            (*(mem)->procs.alloc_byte_array)(mem, na, sizeof(gs_param_string), "RenderTableTable");
        ...
        if (code >= 0) {
            ...
            for (a = 0; a < na; ++a)
                table[a].data = pcrd->RenderTable.lookup.table[a].data,
                    ...
                    table[a].persistent = ((bool)1);
            sa.data = table;
            sa.size = na;
            sa.persistent = ((bool)1);
            code = param_write_string_array(plist, "RenderTableTable", &sa);
            if (code >= 0 && !pcrd->caches.RenderTableT_is_identity) {

                uint size = (1 << 9);
                float *values =
                    (float *)(*(mem)->procs.alloc_byte_array)(mem, size * m, sizeof(float), "write_proc3");
                ...
                for (i = 0; i < m; ++i) {
                    ...
                    for (j = 0; j < size; ++j)
                        values[i * size + j] =
                            (((*pcrd->RenderTable.T.procs[i]) ((byte)(j * scale), pcrd)) / ((float)((frac)0x7ff8)));
        ...
        if (code < 0) {
            ((mem)->procs.free_object(mem, table, "RenderTableTable"));
            ((mem)->procs.free_object(mem, size, "RenderTableSize"));
            return code;
        }
    }
    return code;
...
read_proc3(gs_param_list * plist, gs_param_name key,
           float values[(1 << 9) * 3])
{
    return read_floats(plist, key, values, (1 << 9) * 3);
}
...
{
    gs_param_dict dict;
    int code = (*(plist)->procs->begin_xmit_collection)(plist, key, &dict, (((bool)0) ? gs_param_collection_dict_int_keys : gs_param_collection_dict_any));
    int dcode;
...
static float
encode_from_data(double v, const float values[(1 << 9)],
                 const gs_range * range)
{
    return (v <= range->rmin ? values[0] :
            v >= range->rmax ? values[(1 << 9) - 1] :
...
encode_lmn_1_from_data(double v, const gs_cie_render * pcrd)
{
    ...
    return encode_from_data(v, &data->lmn[(1 << 9)],
...
encode_lmn_2_from_data(double v, const gs_cie_render * pcrd)
{
    ...
    return encode_from_data(v, &data->lmn[(1 << 9) * 2],
...
encode_abc_1_from_data(double v, const gs_cie_render * pcrd)
{
    ...
    return encode_from_data(v, &data->abc[(1 << 9)],
...
encode_abc_2_from_data(double v, const gs_cie_render * pcrd)
{
    ...
    return encode_from_data(v, &data->abc[(1 << 9) * 2],
...
render_table_t_0_from_data(byte v, const gs_cie_render * pcrd)
{
    ...
    return ((frac)(((encode_from_data(v / 255.0, &data->t[0], &Range3_default.ranges[0])) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
...
render_table_t_1_from_data(byte v, const gs_cie_render * pcrd)
{
    ...
    return ((frac)(((encode_from_data(v / 255.0, &data->t[(1 << 9)], &Range3_default.ranges[0])) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
...
render_table_t_2_from_data(byte v, const gs_cie_render * pcrd)
{
    ...
    return ((frac)(((encode_from_data(v / 255.0, &data->t[(1 << 9) * 2], &Range3_default.ranges[0])) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
...
render_table_t_3_from_data(byte v, const gs_cie_render * pcrd)
{
    ...
    return ((frac)(((encode_from_data(v / 255.0, &data->t[(1 << 9) * 3], &Range3_default.ranges[0])) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
                      ...
                      gx_device * dev)
{
    ...
    if (code == 1) {
        if (pcrd->RenderTable.lookup.table) {
            ((pcrd->rc.memory)->procs.free_object(pcrd->rc.memory, (void *)pcrd->RenderTable.lookup.table, "param_get_cie_render1(RenderTable)"));
        ...
        table =
            (gs_const_string *)(*(pcrd->rc.memory)->procs.alloc_struct_array)(pcrd->rc.memory, pcrd->RenderTable.lookup.dims[0], &st_const_string_element, "RenderTable table");
