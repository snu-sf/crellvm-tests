...
# 27 "./base/gsmalloc.c" 2
# 36 "./base/gsmalloc.c"
static byte *gs_heap_alloc_bytes(gs_memory_t *mem, uint nbytes, client_name_t cname);
static void *gs_heap_resize_object(gs_memory_t *mem, void *obj, uint new_num_elements, client_name_t cname);
static void gs_heap_free_object(gs_memory_t *mem, void *data, client_name_t cname);
static gs_memory_t *gs_heap_stable(gs_memory_t *mem);
static void gs_heap_status(gs_memory_t *mem, gs_memory_status_t *status);
...
static uint gs_heap_object_size(gs_memory_t *mem, const void *obj);
static gs_memory_type_ptr_t gs_heap_object_type(const gs_memory_t *mem, const void *obj);
static byte *gs_heap_alloc_string(gs_memory_t *mem, uint nbytes, client_name_t cname);
static byte *gs_heap_resize_string(gs_memory_t *mem, byte *data, uint old_num, uint new_num, client_name_t cname);
static void gs_heap_free_string(gs_memory_t *mem, byte *data, uint nbytes, client_name_t cname);
static int gs_heap_register_root(gs_memory_t *mem, gs_gc_root_t *root, gs_ptr_type_t ptype, void **pp, client_name_t cname);
static void gs_heap_unregister_root(gs_memory_t *mem, gs_gc_root_t *root, client_name_t cname);
...
struct malloc_block_data_s {
    gs_malloc_block_t *next; gs_malloc_block_t *prev; uint size; gs_memory_type_ptr_t type; client_name_t cname;
};
struct gs_malloc_block_s {
    ...
    byte _pad[((-((int)(sizeof(struct malloc_block_data_s))) & ((((8 - 1) | (8 - 1) | (8 - 1)) + 1) - 1)) == 0 ? (((8 - 1) | (8 - 1) | (8 - 1)) + 1) : (-((int)(sizeof(struct malloc_block_data_s))) & ((((8 - 1) | (8 - 1) | (8 - 1)) + 1) - 1)))];
...
# 173 "./base/gsmalloc.c"
    if (mmem->monitor)
        gp_monitor_enter(&(mmem->monitor)->native);
    ...
    if (mmem->monitor)
        gp_monitor_leave(&(mmem->monitor)->native);
                      ...
                      client_name_t cname)
{
    ...
    uint old_size = (*(mem)->procs.object_size)(mem, obj) + sizeof(gs_malloc_block_t);
    uint new_size =
                      ...
                      client_name_t cname)
{
    if (gs_heap_object_type(mem, data) != &st_bytes)
        (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./base/gsmalloc.c", 405), errprintf_nomem("%s: resizing non-string 0x%lx!\n", (cname), (ulong) data));
...
gs_malloc_init(void)
{
    ...
    if (malloc_memory_default == ((void*)0))
        return ((void*)0);
...
gs_malloc_release(gs_memory_t *mem)
{
    ...
    if (gs_debug[':']) {
        ...
        gs_debug['a'] = 1;
        temp = (char *)((mem)->procs.alloc_bytes_immovable(mem, 8, "gs_malloc_release"));
        gs_debug['a'] = save_debug_a;
        ((mem)->procs.free_object(mem, temp, "gs_malloc_release"));
    }
    ...
    (((gs_memory_t *)malloc_memory_default)->procs.free_all((gs_memory_t *)malloc_memory_default, (1 | 2 | 4), "gs_malloc_memory_release"));
}
