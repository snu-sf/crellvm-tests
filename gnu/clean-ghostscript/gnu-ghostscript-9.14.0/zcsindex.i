              ...
              op_proc_t map1)
{
    ...
    int num_components = (*(base_space)->type->num_components)(base_space);
    ...
    *pmap = map;
    ...
    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (5 + 1) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), 5 + 1); if ( es_code_ < 0 ) return es_code_; };
    ep = (((i_ctx_p->exec_stack).stack).p) += 5;
    ((ep + (-4))->value.intval = (num_components), ((ep + (-4))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((ep + (-3))->value.pstruct = ((obj_header_t *)(map)), ((ep + (-3))->tas.type_attrs = ((t_struct) << 8) + (space)) );
    ep[(-2)] = *pproc;
    ((ep + (-1))->value.intval = (num_entries - 1), ((ep + (-1))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((ep + 0)->value.intval = (-1), ((ep + 0)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (map1), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    return 5;
}
