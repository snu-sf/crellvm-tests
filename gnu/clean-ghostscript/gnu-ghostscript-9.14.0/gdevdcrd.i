...
# 55 "./base/memory_.h" 2
# 19 "./base/gdevdcrd.c" 2
# 1 "./base/string_.h" 1
# 20 "./base/gdevdcrd.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
{
    *out = (in <= 0.5 ? in * dent_PQR : (in - 0.5) * (1 - (0.5 * dent_PQR)) / 0.5 + 0.5 * dent_PQR);
    return 0;
}
...
{
    return (in <= 0.5 ? in * dent_LMN : (in - 0.5) * (1 - (0.5 * dent_LMN)) / 0.5 + 0.5 * dent_LMN);
}
...
{
    return ((frac)0x7ff8) * in / 255;
}
static const gs_cie_render_table_t bit_RenderTable = {
                             ...
                             const char *crd_param_name)
{
    ...
    if ((*(plist)->procs->requested)(plist, "CRDName") > 0) {
        gs_param_string cns;
        int code;

        cns.data = (const byte *)crd_param_name;
        cns.size = strlen(crd_param_name);
        cns.persistent = ((bool)1);
        code = param_write_string(plist, "CRDName", &cns);
        if (code < 0)
            ecode = code;
    }
    if ((*(plist)->procs->requested)(plist, crd_param_name) > 0) {
        gs_cie_render *pcrd;
        ...
        if (code >= 0) {
            ...
            code = gs_cie_render1_initialize(pdev->memory, pcrd, ((void*)0),
                        &bit_WhitePoint, ((void*)0) ,
                        ((void*)0) , &bit_RangePQR, &tpqr,
                        ((void*)0) , &bit_EncodeLMN, &bit_RangeLMN,
                        &bit_MatrixABC, &bit_EncodeABC, ((void*)0) ,
                        &bit_RenderTable);
            if (code >= 0) {
                code = param_write_cie_render1(plist, crd_param_name, pcrd,
                                               pdev->memory);
            }
            do { if (pcrd) { do { do { } while (0); (pcrd)->rc.ref_count += (-1); } while (0); if (!(pcrd)->rc.ref_count) { do { do { } while (0); (pcrd)->rc.free((pcrd)->rc.memory, (void *)(pcrd), "sample_device_crd_get_params"); } while (0); (pcrd) = 0; } else do { } while (0); } } while (0);
        }
        if (code < 0)
            ecode = code;
    }
    if ((*(plist)->procs->requested)(plist, bit_TransformPQR.proc_name) > 0) {




        gs_cie_transform_proc my_proc = bit_TransformPQR_proc;
        byte *my_addr = (*(pdev->memory)->procs.alloc_string)(pdev->memory, sizeof(my_proc), "sd_crd_get_params(proc)");
