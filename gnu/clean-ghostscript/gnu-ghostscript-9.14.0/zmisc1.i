           ...
           int (*proc)(byte *, const byte *, uint, ushort *))
{
    ...
    ssize = ((op - 1)->tas.rsize);
    if (((op)->tas.rsize) < ssize)
        return ((-15));
    ((void)((*proc)(op->value.bytes, op[-1].value.const_bytes, ssize, &state)));

    op[-2].value.intval = state;
    op[-1] = *op;
    ((op - 1)->tas.rsize = (ssize));
...
zexD(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        ...
        if ((code = dict_uint_param(op, "seed", 0, 0xffff, 0x10000,
                                    &cstate)) < 0 ||
            (code = dict_int_param(op, "lenIV", 0, (~(-1 << ((1 << 2) * 8 - 1))), 4,
                                   &state.lenIV)) < 0 ||
            (code = dict_bool_param(op, "eexec", ((bool)0),
                                   &is_eexec)) < 0 ||
            (code = dict_bool_param(op, "keep_spaces", ((bool)0),
                                   &state.keep_spaces)) < 0
            )
            return code;
        state.cstate = cstate;
    ...
    if (((((const byte *)&((op - 1)->tas.type_attrs))[1]) == (t_file))) {
        ...
        if (s->state != 0 && s->state->templat == &s_PFBD_template) {
            ...
            if (pss->record_type == 2) {
                ...
                if (pss->binary_to_hex && ((s)->cursor.r.limit - (s)->cursor.r.ptr) > 0) {
                    state.binary = 0;
                    state.hex_left = ((s)->cursor.r.limit - (s)->cursor.r.ptr);
                } else {
                    state.binary = 1;
                }
                pss->binary_to_hex = 0;
