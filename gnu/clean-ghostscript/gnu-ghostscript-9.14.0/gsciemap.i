...
check_range(gs_range *ranges, int num_colorants)
{
    ...
    for (k = 0; k < num_colorants; k++) {
        if (ranges[k].rmin != 0) return ((bool)0);
        if (ranges[k].rmax != 1) return ((bool)0);
    }
    return ((bool)1);
}
...
gx_ciedefg_to_icc(gs_color_space **ppcs_icc, gs_color_space *pcs, gs_memory_t *memory)
{
    ...
    code = gs_cspace_build_ICC(ppcs_icc, ((void*)0), memory->stable_memory);
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 245, 1, code, "Failed to build ICC color space");

    (*ppcs_icc)->base_space = palt_cs;
    rc_increment_cs(palt_cs);
    (*ppcs_icc)->cmm_icc_profile_data = gsicc_profile_new(((void*)0), memory, ((void*)0), 0);
    ...
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 254, 1, code, "Failed to create ICC profile from CIEDEFG");
                ...
                gs_color_select_t select)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        ...
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 281, 1, code, "Failed to create ICC profile from CIEDEFG");
    } else {
                      ...
                      frac * pconc, const gs_imager_state * pis, gx_device *dev)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        code = gx_ciedefg_to_icc(&pcs_icc, pcs, pis->memory->stable_memory);
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 320, 1, code, "Failed to create ICC profile from CIEDEFG");
    } else {
        pcs_icc = pcs->icc_equivalent;
    }
                      ...
                      frac * pconc, float * cie_xyz, const gs_imager_state * pis)
{
    ...
    if (!pis->cie_joint_caches->skipDecodeABC)
        vlmn = *(&(&pcie->caches.DecodeA)->vecs.values[(int)(cie_cached_value) ((a) <= (&pcie->caches.DecodeA)->vecs.params.base ? 0 : (a) >= (&pcie->caches.DecodeA)->vecs.params.limit ? ((1 << 9) - 1) << (0) : ((int)(((((a) - (&pcie->caches.DecodeA)->vecs.params.base)) * ((&pcie->caches.DecodeA)->vecs.params.factor)) * (1L << (0)))))]);
    else
        vlmn.u = vlmn.v = vlmn.w = a;
    ((pis)->cie_joint_caches->remap_finish(vlmn, pconc, cie_xyz, pis, pcs));
    return 0;
}
                      ...
                      frac * pconc, float * cie_xyz, const gs_imager_state * pis)
{
    ...
    if (!pis->cie_joint_caches->skipDecodeABC)
        cie_lookup_mult3(&vec3, &pcie->caches.DecodeABC);
                      ...
                      frac * pconc, float * cie_xyz, const gs_imager_state * pis)
{
    ...
    vec3.u = ( (((pcie->RangeABC.ranges[0]).rmax - (pcie->RangeABC.ranges[0]).rmin) * ((abc[0]) / ((float)((frac)0x7ff8))) + (pcie->RangeABC.ranges[0]).rmin) );
    vec3.v = ( (((pcie->RangeABC.ranges[1]).rmax - (pcie->RangeABC.ranges[1]).rmin) * ((abc[1]) / ((float)((frac)0x7ff8))) + (pcie->RangeABC.ranges[1]).rmin) );
    vec3.w = ( (((pcie->RangeABC.ranges[2]).rmax - (pcie->RangeABC.ranges[2]).rmin) * ((abc[2]) / ((float)((frac)0x7ff8))) + (pcie->RangeABC.ranges[2]).rmin) );
...
gx_ciedef_to_icc(gs_color_space **ppcs_icc, gs_color_space *pcs, gs_memory_t *memory)
{
    ...
    code = gs_cspace_build_ICC(ppcs_icc, ((void*)0), memory->stable_memory);
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 524, 1, code, "Failed to build ICC color space");
    ...
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 533, 1, code, "Failed to build ICC profile from CIEDEF");
    gsicc_init_profile_info((*ppcs_icc)->cmm_icc_profile_data);
                ...
                gs_color_select_t select)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        ...
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 561, 1, code, "Failed to build ICC profile from CIEDEF");
    } else {
        pcs_icc = pcs->icc_equivalent;
    }
                     ...
                     frac * pconc, const gs_imager_state * pis, gx_device *dev)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        code = gx_ciedef_to_icc(&pcs_icc, pcs, pis->memory->stable_memory);
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 600, 1, code, "Failed to build ICC profile from CIEDEF");
    } else {
        pcs_icc = pcs->icc_equivalent;
    }
                 ...
                 gs_memory_t *memory)
{
    ...
    code = gs_cspace_build_ICC(ppcs_icc, ((void*)0), memory);
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 630, 1, code, "Failed to create ICC profile");
    ...
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 639, 1, code, "Failed to build ICC profile from CIEABC");
    gsicc_init_profile_info((*ppcs_icc)->cmm_icc_profile_data);
                ...
                gs_color_select_t select)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        ...
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 671, 1, code, "Failed to create ICC profile from CIEABC");
    } else {
        pcs_icc = pcs->icc_equivalent;
    }
                     ...
                     frac * pconc, const gs_imager_state * pis, gx_device *dev)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        code = gx_cieabc_to_icc(&pcs_icc, pcs, &islab, pis->memory->stable_memory);
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 711, 1, code, "Failed to create ICC profile from CIEABC");
    } else {
        pcs_icc = pcs->icc_equivalent;
    }
...
gx_ciea_to_icc(gs_color_space **ppcs_icc, gs_color_space *pcs, gs_memory_t *memory)
{
    ...
    code = gs_cspace_build_ICC(ppcs_icc, ((void*)0), memory);
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 740, 1, code, "Failed to create ICC profile");
    ...
    code = gsicc_create_froma(pcs, &((*ppcs_icc)->cmm_icc_profile_data->buffer),
                    ...
                    a_cache, lmn_caches);
    if (code < 0)
        gs_throw_imp(__func__, "./base/gsciemap.c", 749, 1, code, "Failed to create ICC profile from CIEA");
    gsicc_init_profile_info((*ppcs_icc)->cmm_icc_profile_data);
                ...
                gs_color_select_t select)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        ...
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 775, 1, code, "Failed to create ICC profile from CIEA");
    } else {
                   ...
                   frac * pconc, const gs_imager_state * pis, gx_device *dev)
{
    ...
    if (pcs->icc_equivalent == ((void*)0)) {
        code = gx_ciea_to_icc(&pcs_icc, pcs, pis->memory->stable_memory);
        if (code < 0)
            gs_throw_imp(__func__, "./base/gsciemap.c", 812, 1, code, "Failed to create ICC profile from CIEA");
    } else {
                                 ...
                                 gs_memory_t *memory)
{
    ...
    *islab = ((bool)0);
    if (pcs->icc_equivalent != ((void*)0) || !gs_color_space_is_PSCIE(pcs))
        return 0;
                         ...
                         const gs_color_space *pcs)
{
    ...
    if (!pjc->skipDecodeLMN)
        cie_lookup_mult3(&vec3, &pjc->DecodeLMN);



    if (!pjc->skipPQR)
        cie_lookup_mult3(&vec3, &pjc->TransformPQR);



    if (!pjc->skipEncodeLMN)
        cie_lookup_mult3(&vec3, &pcrd->caches.EncodeLMN);
# 908 "./base/gsciemap.c"
    do { tabc[0] = ((int)((vec3 .u - pcrd->EncodeABC_base[0]) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))); if ((uint)tabc[0] > ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) tabc[0] = (tabc[0] < 0 ? 0 : ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))); } while (0);
    do { tabc[1] = ((int)((vec3 .v - pcrd->EncodeABC_base[1]) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))); if ((uint)tabc[1] > ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) tabc[1] = (tabc[1] < 0 ? 0 : ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))); } while (0);
    do { tabc[2] = ((int)((vec3 .w - pcrd->EncodeABC_base[2]) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))); if ((uint)tabc[2] > ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) tabc[2] = (tabc[2] < 0 ? 0 : ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))); } while (0);
    ...
    if (table == 0) {
        ...
        pconc[0] = ((tabc[0]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[0].fixeds.fracs.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[0].fixeds.fracs.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[0].fixeds.fracs.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[0].fixeds.fracs.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[0]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))));
        pconc[1] = ((tabc[1]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[1].fixeds.fracs.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[1].fixeds.fracs.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[1].fixeds.fracs.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[1].fixeds.fracs.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[1]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))));
        pconc[2] = ((tabc[2]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[2].fixeds.fracs.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[2].fixeds.fracs.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[2].fixeds.fracs.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[2].fixeds.fracs.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[2]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))));
...
# 938 "./base/gsciemap.c"
        fixed rfix[3];
        const int s = 8 - ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10));





        rfix[0] = ((s) > 0) ? (((tabc[0]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[0].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[0]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) << (s)) : (((tabc[0]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[0].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[0].fixeds.ints.values)[((tabc[0]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[0]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) >> -(s));
        rfix[1] = ((s) > 0) ? (((tabc[1]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[1].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[1]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) << (s)) : (((tabc[1]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[1].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[1].fixeds.ints.values)[((tabc[1]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[1]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) >> -(s));
        rfix[2] = ((s) > 0) ? (((tabc[2]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[2].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[2]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) << (s)) : (((tabc[2]) >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? (pcrd->caches.EncodeABC[2].fixeds.ints.values)[(1 << 9) - 1] : ((pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))] + (frac)(((long)((pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) + 1] - (pcrd->caches.EncodeABC[2].fixeds.ints.values)[((tabc[2]) >> ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))]) * ((int)(tabc[2]) & ((1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1))) >> (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))) >> -(s));
        ...
        if (!pcrd->caches.RenderTableT_is_identity) {
            ...
            pconc[0] = pcrd->caches.RenderTableT[0].fracs.values[((uint)((((pconc[0]) >> (15 - 3)) + (pconc[0])) >> (15 - (9))))];
            pconc[1] = pcrd->caches.RenderTableT[1].fracs.values[((uint)((((pconc[1]) >> (15 - 3)) + (pconc[1])) >> (15 - (9))))];
            pconc[2] = pcrd->caches.RenderTableT[2].fracs.values[((uint)((((pconc[2]) >> (15 - 3)) + (pconc[2])) >> (15 - (9))))];
            if (m > 3)
                pconc[3] = pcrd->caches.RenderTableT[3].fracs.values[((uint)((((pconc[3]) >> (15 - 3)) + (pconc[3])) >> (15 - (9))))];

        }
# 1017 "./base/gsciemap.c"
        return m;
    }
}
...
{
    return ((frac)((((x <= 0 ? 0 : x >= 1 ? 1 : x)) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
}
int
                 ...
                 const gx_cie_vector_cache3_t * pc)
{
    ...
    float ftemp;
# 1093 "./base/gsciemap.c"
    if ((pvec->u >= pc->interpolation_ranges[0].rmin && pvec->u < pc->interpolation_ranges[0].rmax)) {
        cie_cached_value i = (cie_cached_value) ((pvec->u) <= (&pc->caches[0])->vecs.params.base ? 0 : (pvec->u) >= (&pc->caches[0])->vecs.params.limit ? ((1 << 9) - 1) << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) : ((int)(((((pvec->u) - (&pc->caches[0])->vecs.params.base)) * ((&pc->caches[0])->vecs.params.factor)) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))));
        const cie_cached_vector3 *p = &pc->caches[0].vecs.values[(int)((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))];
        const cie_cached_vector3 *p1 = (i >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? p : p + 1);

        do { } while (0);
        u = ((p->u) + ((p1->u) - (p->u)) * ((ftemp = ((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))), ftemp - (int)ftemp));
        v = ((p->v) + ((p1->v) - (p->v)) * ((ftemp = ((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))), ftemp - (int)ftemp));
        w = ((p->w) + ((p1->w) - (p->w)) * ((ftemp = ((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))), ftemp - (int)ftemp));
    } else {
        const cie_cached_vector3 *p = (&(&pc->caches[0])->vecs.values[(int)(cie_cached_value) ((pvec->u) <= (&pc->caches[0])->vecs.params.base ? 0 : (pvec->u) >= (&pc->caches[0])->vecs.params.limit ? ((1 << 9) - 1) << (0) : ((int)(((((pvec->u) - (&pc->caches[0])->vecs.params.base)) * ((&pc->caches[0])->vecs.params.factor)) * (1L << (0)))))]);

        do { } while (0);
        u = p->u, v = p->v, w = p->w;
    }

    if ((pvec->v >= pc->interpolation_ranges[1].rmin && pvec->v < pc->interpolation_ranges[1].rmax)) {
        cie_cached_value i = (cie_cached_value) ((pvec->v) <= (&pc->caches[1])->vecs.params.base ? 0 : (pvec->v) >= (&pc->caches[1])->vecs.params.limit ? ((1 << 9) - 1) << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) : ((int)(((((pvec->v) - (&pc->caches[1])->vecs.params.base)) * ((&pc->caches[1])->vecs.params.factor)) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))));
        const cie_cached_vector3 *p = &pc->caches[1].vecs.values[(int)((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))];
        const cie_cached_vector3 *p1 = (i >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? p : p + 1);
        ...
        const cie_cached_vector3 *p = (&(&pc->caches[1])->vecs.values[(int)(cie_cached_value) ((pvec->v) <= (&pc->caches[1])->vecs.params.base ? 0 : (pvec->v) >= (&pc->caches[1])->vecs.params.limit ? ((1 << 9) - 1) << (0) : ((int)(((((pvec->v) - (&pc->caches[1])->vecs.params.base)) * ((&pc->caches[1])->vecs.params.factor)) * (1L << (0)))))]);

        do { } while (0);
        u += p->u, v += p->v, w += p->w;
    }

    if ((pvec->w >= pc->interpolation_ranges[2].rmin && pvec->w < pc->interpolation_ranges[2].rmax)) {
        cie_cached_value i = (cie_cached_value) ((pvec->w) <= (&pc->caches[2])->vecs.params.base ? 0 : (pvec->w) >= (&pc->caches[2])->vecs.params.limit ? ((1 << 9) - 1) << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) : ((int)(((((pvec->w) - (&pc->caches[2])->vecs.params.base)) * ((&pc->caches[2])->vecs.params.factor)) * (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)))))));
        const cie_cached_vector3 *p = &pc->caches[2].vecs.values[(int)((i) * (1.0 / (1L << (((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))))];
        const cie_cached_vector3 *p1 = (i >= ((1 << 9) - 1) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10)) ? p : p + 1);
        ...
        const cie_cached_vector3 *p = (&(&pc->caches[2])->vecs.values[(int)(cie_cached_value) ((pvec->w) <= (&pc->caches[2])->vecs.params.base ? 0 : (pvec->w) >= (&pc->caches[2])->vecs.params.limit ? ((1 << 9) - 1) << (0) : ((int)(((((pvec->w) - (&pc->caches[2])->vecs.params.base)) * ((&pc->caches[2])->vecs.params.factor)) * (1L << (0)))))]);
