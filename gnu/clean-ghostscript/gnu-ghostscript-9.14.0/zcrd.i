...
zcurrentcolorrendering(i_ctx_t *i_ctx_p)
{
    ...
    *op = ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.dict;
    return 0;
}
...
zbuildcolorrendering1(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_dictionary) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_dictionary)) ? check_type_failed(&*op) : (-7)));
    ...
    if (code < 0 ) {
        do { do { } while (0); (pcrd)->rc.free((pcrd)->rc.memory, (void *)(pcrd), ".buildcolorrendering1"); } while (0);
        (((i_ctx_p->exec_stack).stack).p) = ep;
        return code;
    }


    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.dict = *op;
    ((op)->value.pstruct = ((obj_header_t *)(pcrd)), ((op)->tas.type_attrs = ((t_struct) << 8) + ((((0x20 + 0x40)) | (((&(i_ctx_p->memory)))->current_space))|((&(i_ctx_p->memory))->new_mask))) );
    return ((((i_ctx_p->exec_stack).stack).p) == ep ? 0 : 5);
}
...
zbuilddevicecolorrendering1(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0) {
        do { do { } while (0); (pcrd)->rc.free((pcrd)->rc.memory, (void *)(pcrd), ".builddevicecolorrendering1"); } while (0);
        return code;
    }
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.dict = *op;
    ((op)->value.pstruct = ((obj_header_t *)(pcrd)), ((op)->tas.type_attrs = ((t_struct) << 8) + ((((0x20 + 0x40)) | (((&(i_ctx_p->memory)))->current_space))|((&(i_ctx_p->memory))->new_mask))) );
...
zsetcolorrendering1(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&*op)->value.pstruct) == &st_cie_render1) ) return (check_type_failed(&*op));
    code = zcrd1_proc_params(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, &procs);
    if (code < 0)
        return code;
    code = gs_setcolorrendering((i_ctx_p->pgs), ((gs_cie_render *)((op)->value.pstruct)));
    if (code < 0)
        return code;
    if (gs_cie_cs_common((i_ctx_p->pgs)) != 0 &&
        (code = cie_cache_joint(i_ctx_p, &procs, gs_cie_cs_common((i_ctx_p->pgs)), (i_ctx_p->pgs))) < 0
        )
        return code;
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.dict = op[-1];
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.procs = procs;
...
zsetdevicecolorrendering1(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    refset_null_new((ref *)&procs, sizeof(procs) / sizeof(ref), ((&(i_ctx_p->memory))->new_mask));
    if (gs_cie_cs_common((i_ctx_p->pgs)) != 0 &&
        (code = cie_cache_joint(i_ctx_p, &procs, gs_cie_cs_common((i_ctx_p->pgs)), (i_ctx_p->pgs))) < 0
        )
        return code;
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.dict = op[-1];
    refset_null_new((ref *)&((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.procs, sizeof(((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->colorrendering.procs) / sizeof(ref), ((&(i_ctx_p->memory))->new_mask));
                  ...
                  os_ptr op, ref_cie_render_procs * pcprocs)
{
    ...
    if (dict_find_string(op, "RenderTable", &pRT) > 0) {
        ...
        if ( !(((&*pRT)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_array) << 8) + (0x20))) ) return ((!((((const byte *)&((&*pRT)->tas.type_attrs))[1]) == (t_array)) ? check_type_failed(&*pRT) : (-7)));
        size = ((pRT)->tas.rsize);
        if (size < 5)
            return ((-15));
        prte = pRT->value.const_refs;
        for (i = 5; i < size; i++)
            do { if ( !(((&prte[i])->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&prte[i])); } while (0);
        ((&pcprocs->RenderTableT)->value.const_refs = (prte + 5), ((&pcprocs->RenderTableT)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) | (avm_space)(((pRT)->tas.type_attrs) & (((1 << 2) - 1) << 2)))), ((&pcprocs->RenderTableT)->tas.rsize = (size - 5)));
             ...
             ref_cie_render_procs * pcprocs, gs_memory_t * mem)
{
 ...
 if (dict_find_string(op, "RenderTable", &pRT) > 0) {
        ...
        do { if ( !((((const byte *)&((&prte[4])->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0);
        if (!(prte[4].value.intval == 3 || prte[4].value.intval == 4))
            return ((-15));
        prtl->n = 3;
        prtl->m = prte[4].value.intval;
        if (((pRT)->tas.rsize) != prtl->m + 5)
            return ((-15));
                ...
                const gs_cie_common *pcie, gs_state * pgs)
{
    ...
    if (pjc == 0)
        return ((-25));
    if (((((const byte *)&((&pcrprocs->TransformPQR)->tas.type_attrs))[1]) == (t_null))) {
    ...
    code = gs_alloc_ref_array(((i_ctx_p->memory).current), &pqr_procs, (0x20 + 0x40), 3 * (1 + 4 + 4 * 6), "cie_cache_common");
    ...
    *++(((i_ctx_p->exec_stack).stack).p) = pqr_procs;
    ...
    return cie_prepare_caches_4(i_ctx_p, (&pcrd->RangePQR)->ranges, pqr_procs.value.const_refs, &(pjc->TransformPQR.caches)->floats, &(pjc->TransformPQR.caches)[1].floats, &(pjc->TransformPQR.caches)[2].floats, ((void*)0), pjc, imem, "Transform.PQR");
...
cie_tpqr_finish(i_ctx_t *i_ctx_p)
{
    ...
    gs_cie_render *pcrd =
        (gs_cie_render *)gs_currentcolorrendering(pgs);
    int code;

    gs_free_ref_array(((i_ctx_p->memory).current), op - 1, "cie_tpqr_finish");
    pcrd->TransformPQR = TransformPQR_from_cache;
    code = gs_cie_cs_complete(pgs, ((bool)0));
    ((((i_ctx_p->op_stack).stack).p) -= (2));
    return code;
}
# 379 "./psi/zcrd.c"
...
ztpqr_scale_wb_common(i_ctx_t *i_ctx_p, int idx)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        code = array_get(((gs_memory_t *)((i_ctx_p->memory).current)), op - 4 + i, idx, &tmp);
        if (code >= 0)
    ...
    ((op - 4)->value.realval = (result), ((op - 4)->tas.type_attrs = ((t_real) << 8) + (0)) );
