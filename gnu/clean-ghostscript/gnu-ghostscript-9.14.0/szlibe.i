...
s_zlibE_init(stream_state * st)
{
    ...
    if (code < 0)
        return ((int)(-2));
    if (deflateInit2_((&ss->dynamic->zstate),(ss->level),(ss->method),((ss->no_wrapper ? -ss->windowBits : ss->windowBits)),(ss->memLevel), (ss->strategy), "1.2.7", (int)sizeof(z_stream)) != 0)
...
s_zlibE_reset(stream_state * st)
{
    ...
    if (deflateReset(&ss->dynamic->zstate) != 0)
        return ((int)(-2));
    return 0;
}
                ...
                stream_cursor_write * pw, bool last)
{
    ...
    switch (status) {
        ...
        case 1:
            return (last && pr->ptr == pr->limit ? 0 : ((int)(-2)));
        default:
            return ((int)(-2));
    }
}
