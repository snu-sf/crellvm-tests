        ...
        const int32_t *cg_num, int32_t cg_den)
{
    ...
    if (j < ((int)((fa->clip->p.y) >> (8))) ||
            j > ((int)(((fa->clip->q.y)+((1<<8)-1)) >> (8))))
        return 0;
    for (k = 0; k < n; k++) {
        curr[k] = c[k] = c0[k];
        f[k] = c0f[k];
    ...
    for (i = i0 + 1, di = 1; i < i1; i += di) {
        ...
        if (gx_devn_diff(c, curr, n)) {
            si = (((bi) > (((int)((fa->clip->p.x) >> (8))))) ? (bi) : (((int)((fa->clip->p.x) >> (8)))));
            ei = (((i) < (((int)(((fa->clip->q.x)+((1<<8)-1)) >> (8))))) ? (i) : (((int)(((fa->clip->q.x)+((1<<8)-1)) >> (8)))));
            if (si < ei) {
                if (fa->swap_axes) {
                    rect.p.x = ((fixed)(j)<<8);
                    rect.p.y = ((fixed)(si)<<8);
                    rect.q.x = ((fixed)(j + 1)<<8);
                    rect.q.y = ((fixed)(ei)<<8);
                } else {
                    rect.p.x = ((fixed)(si)<<8);
                    rect.p.y = ((fixed)(j)<<8);
                    rect.q.x = ((fixed)(ei)<<8);
                    rect.q.y = ((fixed)(j + 1)<<8);
                }
                for (k = 0; k < n; k++) {
                    devc.colors.devn.values[k] = ((ushort)( ((curr[k]) << (((1 << 1) * 8) - 15)) + ((curr[k]) >> (15 * 2 - ((1 << 1) * 8) - 3)) ));
                }
                code = ((dev)->procs.fill_rectangle_hl_color) (dev, &rect, ((void*)0), &devc, ((void*)0));
                if (code < 0)
                    return code;
            }
            bi = i;
            ...
            for (k = 0; k < n; k++) {
                int32_t a;
                int64_t x;
                frac31 v = 1 << (31 - cinfo->comp_bits[k]);
                frac31 u = c[k] & (v - 1);
    ...
    if (si < ei) {
        ...
        return ((dev)->procs.fill_rectangle_hl_color) (dev, &rect, ((void*)0), &devc, ((void*)0));
    }
    return 0;
}
        ...
        const frac31 *c0, const int32_t *c0f, const int32_t *cg_num, int32_t cg_den)
{
    ...
    bool devn = ((dev)->procs.dev_spec_op)(dev, gxdso_supports_devn, ((void*)0), 0);
    frac31 c[(8 * 8)];
    ulong f[(8 * 8)];
    int i, i1 = i0 + w, bi = i0, k;
    ...
    for (i = i0 + 1, di = 1; i < i1; i += di) {
        ...
        if (ci1 != ci0) {
            ...
            if (si < ei) {
                if (fa->swap_axes) {
                    do { } while (0);
                    code = ((dev)->procs.fill_rectangle)(dev, j, si, 1, ei - si, ci0);
                } else {
                    do { } while (0);
                    code = ((dev)->procs.fill_rectangle)(dev, si, j, ei - si, 1, ci0);
                }
                if (code < 0)
                    return code;
            }
    ...
    if (si < ei) {
        if (fa->swap_axes) {
            do { } while (0);
            return ((dev)->procs.fill_rectangle)(dev, j, si, 1, ei - si, ci0);
        } else {
            do { } while (0);
            return ((dev)->procs.fill_rectangle)(dev, si, j, ei - si, 1, ci0);
        }
    }
    return 0;
}
