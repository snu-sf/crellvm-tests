...
gx_bits_cache_alloc(gx_bits_cache * bc, ulong lsize, gx_cached_bits_head ** pcbh)
{
    ...
    while (fsize < ((uint)lsize1) && fsize != ((uint)lsize)) {
        if (!((cbh_next)->depth == 0)) {
            if (fsize)
                cbh->size = fsize;
            *pcbh = cbh_next;
    ...
    if (fsize > ((uint)lsize)) {
        cbh_next = (gx_cached_bits_head *) ((byte *) cbh + ((uint)lsize));
        cbh_next->size = fsize - ((uint)lsize);
        ((cbh_next)->depth = 0);
        do { } while (0);
