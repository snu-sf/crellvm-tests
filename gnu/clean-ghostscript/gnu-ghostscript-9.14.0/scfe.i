...
{
    register uint bits; register int bits_left;
    cfe_run rr;
...
# 113 "./base/scfe.c"
static const gc_ptr_element_t cfe_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_CFE_state *)0)->lbuf - (char *)((stream_CFE_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_CFE_state *)0)->lprev - (char *)((stream_CFE_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_CFE_state *)0)->lcode - (char *)((stream_CFE_state *)0) )) } }; static const gc_struct_data_t cfe_reloc_ptrs = { (sizeof(cfe_enum_ptrs) / sizeof((cfe_enum_ptrs)[0])), 0, 0, cfe_enum_ptrs }; static const gs_memory_struct_type_t st_CFE_state = { sizeof(stream_CFE_state), "CCITTFaxEncode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &cfe_reloc_ptrs };
...
s_CFE_set_defaults(register stream_state * st)
{
    ...
    (((ss)->Uncompressed = ((bool)0), (ss)->K = 0, (ss)->EndOfLine = ((bool)0), (ss)->EncodedByteAlign = ((bool)0), (ss)->Columns = 1728, (ss)->Rows = 0, (ss)->EndOfBlock = ((bool)1), (ss)->BlackIs1 = ((bool)0), (ss)->DamagedRowsBeforeError = 0, (ss)->FirstBitLowOrder = ((bool)0), (ss)->DecodedByteAlign = 1, (ss)->lbuf = 0, (ss)->lprev = 0, (ss)->error_string[0] = 0), (ss)->lcode = 0);
}
...
s_CFE_init(register stream_state * st)
{
    ...
    int code_bytes =
    ((columns * (ss->K == 0 ? 9 : 12)) >> 4) + 20;
    int raster = ss->raster =
        ( (ss->DecodedByteAlign) & ((ss->DecodedByteAlign) - 1) ? (((columns + 7) >> 3) + ((ss->DecodedByteAlign) - 1)) / (ss->DecodedByteAlign) * (ss->DecodedByteAlign) : (((columns + 7) >> 3) + ((ss->DecodedByteAlign) - 1)) & -(ss->DecodedByteAlign) );

    ((ss)->bits = 0, (ss)->bits_left = ((1 << 2) * 8));
    ...
    ss->lbuf = (*(st->memory)->procs.alloc_bytes)(st->memory, raster + 4, "CFE lbuf");
    ss->lcode = (*(st->memory)->procs.alloc_bytes)(st->memory, code_bytes, "CFE lcode");
    if (ss->lbuf == 0 || ss->lcode == 0) {
        s_CFE_release(st);
        return ((int)(-2));

    }
    memset(ss->lbuf + raster, 0, 4);
    if (ss->K != 0) {
        ss->lprev = (*(st->memory)->procs.alloc_bytes)(st->memory, raster + 4, "CFE lprev");
        if (ss->lprev == 0) {
            s_CFE_release(st);
            return ((int)(-2));
        ...
        memset(ss->lprev, (ss->BlackIs1 ? 0 : 0xff), raster + 4);
        if (columns & 7)
...
s_CFE_release(stream_state * st)
{
    ...
    ((st->memory)->procs.free_object(st->memory, ss->lprev, "CFE lprev(close)"));
    ((st->memory)->procs.free_object(st->memory, ss->lcode, "CFE lcode(close)"));
    ((st->memory)->procs.free_object(st->memory, ss->lbuf, "CFE lbuf(close)"));
}
              ...
              stream_cursor_write * pw, bool last)
{
    ...
    for (;;) {
        ...
        if (ss->write_count) {
            ...
            int ccount = (((wcount) < (ss->write_count)) ? (wcount) : (ss->write_count));
        ...
        if (ss->read_count) {
            ...
            int ccount = (((rcount) < (ss->read_count)) ? (rcount) : (ss->read_count));
...
# 280 "./base/scfe.c"
        if (ss->EndOfLine) {
            ...
            ( ((bits_left -= ((rp)->code_length)) >= 0 ? (bits += ((rp)->code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, w.ptr += ((1 << 2) * 8) >> 3, (bits + (((rp)->code) >> -bits_left))), bits = ((rp)->code) << (bits_left += ((1 << 2) * 8)))));
            ss->bits = bits, ss->bits_left = bits_left;
    ...
    if (last && status == 0) {
        ...
        uint bits_to_write =
        ((1 << 2) * 8) - ss->bits_left + i * rp->code_length;
        byte *q = pw->ptr;
        ...
        if (ss->EncodedByteAlign)
            bits_left &= ~7;
        while (--i >= 0)
            ( ((bits_left -= ((rp)->code_length)) >= 0 ? (bits += ((rp)->code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + (((rp)->code) >> -bits_left))), bits = ((rp)->code) << (bits_left += ((1 << 2) * 8)))));

        pw->ptr = hc_put_last_bits_proc((stream_hc_state *) ss, q, bits, bits_left);
    }
  out:
    do { } while (0);
# 377 "./base/scfe.c"
...
cf_encode_1d(stream_CFE_state * ss, const byte * lbuf, stream_cursor_write * pw)
{
    ...
    while (count != end_count) {
        ...
        do { rlen = byte_bit_run_length_neg[count & 7][data ^ 0xff]; if ( rlen >= 8 ) { if ( invert == 0 ) { if ( p[0] ) { data = p[0]; p += 1; rlen -= 8; } else if ( p[1] ) { data = p[1]; p += 2; } else { while ( !(p[2] | p[3] | p[4] | p[5]) ) p += 4, rlen += 32; if ( p[2] ) { data = p[2]; p += 3; rlen += 8; } else if ( p[3] ) { data = p[3]; p += 4; rlen += 16; } else if ( p[4] ) { data = p[4]; p += 5; rlen += 24; } else { data = p[5]; p += 6; rlen += 32; } } } else { if ( p[0] != 0xff ) { data = (byte)~p[0]; p += 1; rlen -= 8; } else if ( p[1] != 0xff ) { data = (byte)~p[1]; p += 2; } else { while ( (p[2] & p[3] & p[4] & p[5]) == 0xff ) p += 4, rlen += 32; if ( p[2] != 0xff ) { data = (byte)~p[2]; p += 3; rlen += 8; } else if ( p[3] != 0xff ) { data = (byte)~p[3]; p += 4; rlen += 16; } else if ( p[4] != 0xff ) { data = (byte)~p[4]; p += 5; rlen += 24; } else { data = (byte)~p[5]; p += 6; rlen += 32; } } } rlen += byte_bit_run_length_0[data ^ 0xff]; } count -= rlen;} while (0);
        do { cfe_run rr; if ( rlen >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, rlen, &cf_white_runs); bits = ss->bits, bits_left = ss->bits_left; rlen &= 63; } rr = cf_white_runs.termination[rlen]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
        if (count == end_count)
            break;

        do { rlen = byte_bit_run_length_neg[count & 7][data]; if ( rlen >= 8 ) { if ( invert == 0 ) for ( ; ; p += 4, rlen += 32 ) { if ( p[0] != 0xff ) { data = p[0]; p += 1; rlen -= 8; break; } if ( p[1] != 0xff ) { data = p[1]; p += 2; break; } if ( p[2] != 0xff ) { data = p[2]; p += 3; rlen += 8; break; } if ( p[3] != 0xff ) { data = p[3]; p += 4; rlen += 16; break; } } else for ( ; ; p += 4, rlen += 32 ) { if ( p[0] ) { data = (byte)~p[0]; p += 1; rlen -= 8; break; } if ( p[1] ) { data = (byte)~p[1]; p += 2; break; } if ( p[2] ) { data = (byte)~p[2]; p += 3; rlen += 8; break; } if ( p[3] ) { data = (byte)~p[3]; p += 4; rlen += 16; break; } } rlen += byte_bit_run_length_0[data]; } count -= rlen;} while (0);
        do { cfe_run rr; if ( rlen >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, rlen, &cf_black_runs); bits = ss->bits, bits_left = ss->bits_left; rlen &= 63; } rr = cf_black_runs.termination[rlen]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
    }
    ss->bits = bits, ss->bits_left = bits_left;
    pw->ptr = q;
}
             ...
             const byte * lprev)
{
    ...
    while (count != end_count) {
        ...
        if ((prev_data & count_bit[prev_count & 7])) {
            ...
            do { rlen = byte_bit_run_length_neg[prev_count & 7][prev_data]; if ( rlen >= 8 ) { if ( invert == 0 ) for ( ; ; prev_p += 4, rlen += 32 ) { if ( prev_p[0] != 0xff ) { prev_data = prev_p[0]; prev_p += 1; rlen -= 8; break; } if ( prev_p[1] != 0xff ) { prev_data = prev_p[1]; prev_p += 2; break; } if ( prev_p[2] != 0xff ) { prev_data = prev_p[2]; prev_p += 3; rlen += 8; break; } if ( prev_p[3] != 0xff ) { prev_data = prev_p[3]; prev_p += 4; rlen += 16; break; } } else for ( ; ; prev_p += 4, rlen += 32 ) { if ( prev_p[0] ) { prev_data = (byte)~prev_p[0]; prev_p += 1; rlen -= 8; break; } if ( prev_p[1] ) { prev_data = (byte)~prev_p[1]; prev_p += 2; break; } if ( prev_p[2] ) { prev_data = (byte)~prev_p[2]; prev_p += 3; rlen += 8; break; } if ( prev_p[3] ) { prev_data = (byte)~prev_p[3]; prev_p += 4; rlen += 16; break; } } rlen += byte_bit_run_length_0[prev_data]; } prev_count -= rlen;} while (0);
        }
        count_bit = further_count_bit;
      pass:
        if (prev_count != end_count)
            do { rlen = byte_bit_run_length_neg[prev_count & 7][prev_data ^ 0xff]; if ( rlen >= 8 ) { if ( invert == 0 ) { if ( prev_p[0] ) { prev_data = prev_p[0]; prev_p += 1; rlen -= 8; } else if ( prev_p[1] ) { prev_data = prev_p[1]; prev_p += 2; } else { while ( !(prev_p[2] | prev_p[3] | prev_p[4] | prev_p[5]) ) prev_p += 4, rlen += 32; if ( prev_p[2] ) { prev_data = prev_p[2]; prev_p += 3; rlen += 8; } else if ( prev_p[3] ) { prev_data = prev_p[3]; prev_p += 4; rlen += 16; } else if ( prev_p[4] ) { prev_data = prev_p[4]; prev_p += 5; rlen += 24; } else { prev_data = prev_p[5]; prev_p += 6; rlen += 32; } } } else { if ( prev_p[0] != 0xff ) { prev_data = (byte)~prev_p[0]; prev_p += 1; rlen -= 8; } else if ( prev_p[1] != 0xff ) { prev_data = (byte)~prev_p[1]; prev_p += 2; } else { while ( (prev_p[2] & prev_p[3] & prev_p[4] & prev_p[5]) == 0xff ) prev_p += 4, rlen += 32; if ( prev_p[2] != 0xff ) { prev_data = (byte)~prev_p[2]; prev_p += 3; rlen += 8; } else if ( prev_p[3] != 0xff ) { prev_data = (byte)~prev_p[3]; prev_p += 4; rlen += 16; } else if ( prev_p[4] != 0xff ) { prev_data = (byte)~prev_p[4]; prev_p += 5; rlen += 24; } else { prev_data = (byte)~prev_p[5]; prev_p += 6; rlen += 32; } } } rlen += byte_bit_run_length_0[prev_data ^ 0xff]; } prev_count -= rlen;} while (0);
        diff = a1 - prev_count;
        ...
        if (invert == invert_white) {
            ...
            do { cfe_run rr; if ( a0 >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, a0, &cf_white_runs); bits = ss->bits, bits_left = ss->bits_left; a0 &= 63; } rr = cf_white_runs.termination[a0]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
            do { cfe_run rr; if ( a1 >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, a1, &cf_black_runs); bits = ss->bits, bits_left = ss->bits_left; a1 &= 63; } rr = cf_black_runs.termination[a1]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
        } else {
            do { } while (0);

            do { cfe_run rr; if ( a0 >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, a0, &cf_black_runs); bits = ss->bits, bits_left = ss->bits_left; a0 &= 63; } rr = cf_black_runs.termination[a0]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
            do { cfe_run rr; if ( a1 >= 64 ) { ss->bits = bits, ss->bits_left = bits_left; q = cf_put_long_run(ss, q, a1, &cf_white_runs); bits = ss->bits, bits_left = ss->bits_left; a1 &= 63; } rr = cf_white_runs.termination[a1]; do { } while (0); ( ((bits_left -= (rr.code_length)) >= 0 ? (bits += (rr.code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder, q += ((1 << 2) * 8) >> 3, (bits + ((rr.code) >> -bits_left))), bits = (rr.code) << (bits_left += ((1 << 2) * 8)))));} while (0);
