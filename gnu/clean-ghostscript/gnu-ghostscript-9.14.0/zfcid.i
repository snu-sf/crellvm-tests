...
cid_font_data_param(os_ptr op, gs_font_cid_data *pdata, ref *pGlyphDirectory)
{
    ...
    if ((code = cid_font_system_info_param(&pdata->CIDSystemInfo, op)) < 0 ||
        (code = dict_int_param(op, "CIDCount", 0, (~(-1 << ((1 << 2) * 8 - 1))), -1,
                               &pdata->CIDCount)) < 0
        )
        return code;
    ...
    if (dict_find_string(op, "GlyphDirectory", &pgdir) <= 0) {
        ...
        return dict_int_param(op, "GDBytes", 1, 4, 0,
                              &pdata->GDBytes);
    }
    if (((((const byte *)&((pgdir)->tas.type_attrs))[1]) == (t_dictionary)) || (((pgdir)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))) {
        int index;
        ref element[2];


        *pGlyphDirectory = *pgdir;
        code = dict_int_param(op, "GDBytes", 0, 4, 0,
                              &pdata->GDBytes);
