# 1 "./lcms2/src/cmsgmt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./lcms2/src/cmsgmt.c" 2
# 27 "./lcms2/src/cmsgmt.c"
                               ...
                               cmsUInt32Number dwFlags)
{
    ...
    xform = _cmsChain2Lab(ContextID, nProfiles, (((1) << 22)|((6) << 16)|((4) << 3)|(4)), (((1) << 22)|((10) << 16)|((3) << 3)|(0)), Intents, hProfiles, BPC, AdaptationStates, dwFlags);
    if (xform == ((void*)0)) return ((void*)0);

    SampledPoints = (cmsFloat32Number*) _cmsCalloc(ContextID, nPoints, sizeof(cmsFloat32Number));
    if (SampledPoints == ((void*)0)) goto Error;
                                   ...
                                   cmsUInt32Number dwFlags)
{
    ...
    if (cmsGetColorSpace(hProfiles[0]) != cmsSigCmykData ||
        cmsGetColorSpace(hProfiles[nProfiles-1])!= cmsSigCmykData) return ((void*)0);



    if (cmsGetDeviceClass(hProfiles[nProfiles - 1]) != cmsSigOutputClass) return ((void*)0);
...
int GamutSampler(register const cmsUInt16Number In[], register cmsUInt16Number Out[], register void* Cargo)
{
    ...
    cmsUInt16Number Proof[16], Proof2[16];
                                          ...
                                          cmsHPROFILE hGamut)
{
    ...
    if (nGamutPCSposition <= 0 || nGamutPCSposition > 255) {
        cmsSignalError(ContextID, 2, "Wrong position of PCS. 1..255 expected, %d found.", nGamutPCSposition);
        return ((void*)0);
    }
    ...
    IntentList[nGamutPCSposition] = 1;


    ColorSpace = cmsGetColorSpace(hGamut);

    nChannels = cmsChannelsOf(ColorSpace);
    nGridpoints = _cmsReasonableGridpointsByColorspace(ColorSpace, 0x0400);
    dwFormat = (((nChannels) << 3)|(2));
...
cmsFloat64Number cmsDetectTAC(cmsHPROFILE hProfile)
{
    ...
    dwFormatter = cmsFormatterForColorspaceOfProfile(hProfile, 4, 1);

    bp.nOutputChans = (((dwFormatter)>>3)&15);
    bp.MaxTAC = 0;


    if (bp.nOutputChans >= 16) return 0;

    hLab = cmsCreateLab4ProfileTHR(ContextID, ((void*)0));
    if (hLab == ((void*)0)) return 0;

    bp.hRoundTrip = cmsCreateTransformTHR(ContextID, hLab, (((10) << 16)|((3) << 3)|(2)),
                                          hProfile, dwFormatter, 0, 0x0100|0x0040);

    cmsCloseProfile(hLab);
    if (bp.hRoundTrip == ((void*)0)) return 0;
                ...
                if (h >= 45. && h < 135.)
                {
                    ...
                    if (h >= 135. && h < 225.) {
                        ...
                        if (h >= 225. && h < 315.) {
                            ...
                            cmsSignalError(0, 2, "Invalid angle");
                            return 0;
                        }
