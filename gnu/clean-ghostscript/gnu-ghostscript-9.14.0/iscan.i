...
int (*gs_scan_dsc_proc) (const byte *, uint) = ((void*)0);





int (*gs_scan_comment_proc) (const byte *, uint) = ((void*)0);
# 73 "./psi/iscan.c"
static __inline__ void
dynamic_init(da_ptr pda, gs_memory_t *mem)
{
    pda->is_dynamic = ((bool)0);
...
{
    if (pda->is_dynamic)
        (*(pda->memory)->procs.free_string)(pda->memory, pda->base, ((uint)((pda)->limit - (pda)->base)), "scanner");
}
...
{
    uint old_size = ((uint)((pda)->limit - (pda)->base));
    uint pos = pda->next - pda->base;
    gs_memory_t *mem = pda->memory;
    byte *base;

    if (pda->is_dynamic) {
        base = (*(mem)->procs.resize_string)(mem, pda->base, old_size, new_size, "scanner");

        if (base == 0)
            return ((-25));
    } else {
        base = (*(mem)->procs.alloc_string)(mem, new_size, "scanner");
        if (base == 0)
            return ((-25));
        memcpy(base, pda->base, (((old_size) < (new_size)) ? (old_size) : (new_size)));
        pda->is_dynamic = ((bool)1);
    }
    pda->base = base;
    pda->next = base + pos;
...
dynamic_save(da_ptr pda)
{
    if (!pda->is_dynamic && pda->base != pda->buf) {
        int len = ((uint)((pda)->limit - (pda)->base));
...
dynamic_make_string(i_ctx_t *i_ctx_p, ref * pref, da_ptr pda, byte * next)
{
    ...
    if (code < 0)
        return code;
    ((pref)->value.bytes = (pda->base), ((pref)->tas.type_attrs = ((t_string) << 8) + (((0x10 + 0x20 +0x40) | imemory_space((gs_ref_memory_t *) pda->memory))|((&(i_ctx_p->memory))->new_mask))), ((pref)->tas.rsize = (size)));
...
static
void scanner_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
    scanner_state *const ssptr = vptr;

    ((&ssptr->s_file)->tas.type_attrs &= ~(1));
    ((&ssptr->s_ss.binary.bin_array)->tas.type_attrs &= ~(1));
    ((&ssptr->s_error.object)->tas.type_attrs &= ~(1));
}
static
gs_ptr_type_t scanner_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const scanner_state *ssptr = vptr; switch ( index ) { default: return 0;
case 0:
    return (pep->ptr = (const void *)(&ssptr->s_file), (&ptr_ref_procs));
case 1:
    return (pep->ptr = (const void *)(&ssptr->s_error.object), (&ptr_ref_procs));
case 2:
    if (ssptr->s_scan_type == scanning_none ||
        !ssptr->s_da.is_dynamic
        )
        return (pep->ptr = (const void *)(0), (&ptr_struct_procs));
    return (pep->ptr = ssptr->s_da.base, pep->size = ((uint)((&ssptr->s_da)->limit - (&ssptr->s_da)->base)), (&ptr_string_procs));
case 3:
    if (ssptr->s_scan_type != scanning_binary)
        return 0;
    return (pep->ptr = (const void *)(&ssptr->s_ss.binary.bin_array), (&ptr_ref_procs));
} }
static void scanner_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { scanner_state *ssptr = vptr;
{
    (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(ssptr->s_file)), (ref_packed *)(&(ssptr->s_file) + 1), gcst);
    ((&ssptr->s_file)->tas.type_attrs &= ~(1));
    if (ssptr->s_scan_type != scanning_none && ssptr->s_da.is_dynamic) {
        gs_string sda;

        sda.data = ssptr->s_da.base;
        sda.size = ((uint)((&ssptr->s_da)->limit - (&ssptr->s_da)->base));
        (((*(const gc_procs_with_refs_t **)(gcst))->reloc_string)(&(sda), gcst));
        ssptr->s_da.limit = sda.data + sda.size;
        ssptr->s_da.next = sda.data + (ssptr->s_da.next - ssptr->s_da.base);
        ssptr->s_da.base = sda.data;
    }
    if (ssptr->s_scan_type == scanning_binary) {
        (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(ssptr->s_ss.binary.bin_array)), (ref_packed *)(&(ssptr->s_ss.binary.bin_array) + 1), gcst);
        ((&ssptr->s_ss.binary.bin_array)->tas.type_attrs &= ~(1));
    }
    (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(ssptr->s_error.object)), (ref_packed *)(&(ssptr->s_error.object) + 1), gcst);
    ((&ssptr->s_error.object)->tas.type_attrs &= ~(1));
}
}

const gs_memory_struct_type_t st_scanner_state_dynamic = { sizeof(scanner_state_dynamic), "scanner state", 0, scanner_clear_marks, scanner_enum_ptrs, scanner_reloc_ptrs, 0, 0 };
...
gs_scanner_init_options(scanner_state *sstate, const ref *fop, int options)
{
    ...
    ((((&(sstate)->s_error.object)->tas.type_attrs = ((t__invalid) << 8) + (0)) ), (sstate)->s_error.is_name = ((bool)0), (sstate)->s_error.string[0] = 0);
}
void gs_scanner_init_stream_options(scanner_state *sstate, stream *s,
                                 int options)
{
    ...
    ((&fobj)->value.pfile = (s), ((&fobj)->tas.type_attrs = ((t_file) << 8) + (0x20)), ((&fobj)->tas.rsize = (0)));
...
gs_scanner_error_object(i_ctx_t *i_ctx_p, const scanner_state *pstate,
                     ref *pseo)
{
    if (!((((const byte *)&((&pstate->s_error.object)->tas.type_attrs))[1]) == (t__invalid))) {
        (*(pseo) = *(&pstate->s_error.object));
        return 0;
    }
    if (pstate->s_error.string[0]) {
        ...
        if (pstate->s_error.is_name) {
            int code = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, (const byte *)pstate->s_error.string, len, pseo, 1);

            if (code < 0)
                return code;
            ((pseo)->tas.type_attrs |= (0x80));
            return 0;
        } else {
            byte *estr = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), len, "gs_scanner_error_object");

            if (estr == 0)
                return -1;
            memcpy(estr, (const byte *)pstate->s_error.string, len);
            ((pseo)->value.bytes = (estr), ((pseo)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((pseo)->tas.rsize = (len)));
            return 0;
        }
    }
    return -1;
                   ...
                   bool save, op_proc_t cont)
{
    ...
    stream *s = (fop)->value.pfile;
    uint avail = ((s)->cursor.r.limit - (s)->cursor.r.ptr);
    int status;

    if (s->end_status == ((int)(-1))) {

        return ((-18));
    }
    status = s_process_read_buf(s);
    if (((s)->cursor.r.limit - (s)->cursor.r.ptr) > avail)
        return 0;
    if (status == 0)
        status = s->end_status;
    switch (status) {
        ...
        case ((int)(-4)):
            {
                ...
                if (save) {
                    pstate = (scanner_state *)
                        (scanner_state_dynamic *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_scanner_state_dynamic, "gs_scan_handle_refill");

                    if (pstate == 0)
                        return ((-25));
                    ((scanner_state_dynamic *)pstate)->mem = ((gs_memory_t *)((i_ctx_p->memory).current));
                    *pstate = *sstate;
                } else
                    pstate = sstate;
                ((&rstate[0])->value.pstruct = ((obj_header_t *)(pstate)), ((&rstate[0])->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
...
# 363 "./psi/iscan.c"
        if (gs_scan_dsc_proc != ((void*)0)) {
            code = gs_scan_dsc_proc(base, len);
            return (code < 0 ? code : 0);
        }
        if (pstate->s_options & 8) {
            code = 5;
            goto comment;
        }
...
# 382 "./psi/iscan.c"
    if (gs_scan_comment_proc != ((void*)0)) {
        code = gs_scan_comment_proc(base, len);
        return (code < 0 ? code : 0);
    }
    if (pstate->s_options & 4) {
        code = 4;
        goto comment;
    }
    return 0;
 comment:
    {
        byte *cstr = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), len, "scan_comment");

        if (cstr == 0)
            return ((-25));
        memcpy(cstr, base, len);
        ((pref)->value.bytes = (cstr), ((pref)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((pref)->tas.rsize = (len)));
    }
    return code;
}
                             ...
                             int options)
{
    ...
    if (!(((pstr)->tas.type_attrs) & (0x20)))
        return ((-7));
    s_init(s, ((void*)0));
    sread_string(s, pstr->value.bytes, ((pstr)->tas.rsize));
    gs_scanner_init_stream_options(&state, s, options | 1);
...
# 478 "./psi/iscan.c"
    int max_name_ctype =
        (((i_ctx_p->binary_object_format).value.intval != 0 && ((i_ctx_p->language_level) >= 2))? 100 : 101);
# 493 "./psi/iscan.c"
    byte s1[2];
    const byte *const decoder = (&scan_char_array[4]);
    int status;
    int sign;
    const bool check_only = (pstate->s_options & 2) != 0;
    const bool PDFScanRules = (i_ctx_p->scanner_options & 16) != 0;





    const int ctrld = (pstate->s_options & 1 ||
                      PDFScanRules ? 0x04 : 0xffff);
    scanner_state sstate;
# 515 "./psi/iscan.c"
    sptr = endptr = ((void*)0);
    if (pstate->s_pstack != 0) {
        if ( (((i_ctx_p->op_stack).stack).p) < (((i_ctx_p->op_stack).stack).top) ) (((i_ctx_p->op_stack).stack).p)++; else if ( (retcode = ref_stack_push(&((i_ctx_p->op_stack).stack), 1)) >= 0 ) ; else
            return retcode;
        myref = (((i_ctx_p->op_stack).stack).p);
    }
  ...
  top:c = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr));
    do { } while (0);
    switch (c) {
        case ' ':
        case '\f':
        ...
        case ']':
            s1[0] = (byte) c;
            retcode = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, s1, 1, myref, 1);
            ((myref)->tas.type_attrs |= (0x80));
            break;
        case '<':
            if (((i_ctx_p->language_level) >= 2)) {
                if ( sptr >= endptr ) do { sptr -= 1; sstate.s_scan_type = scanning_none; goto pause; } while (0);
                c = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr));
                switch (c) {
                    case '<':
                        --sptr;
                        sstate.s_ss.s_name.s_name_type = 0;
                        sstate.s_ss.s_name.s_try_number = ((bool)0);
                        goto try_funny_name;
                    case '~':
                        do { (&sstate.s_ss.a85d)->min_left = 1; (&sstate.s_ss.a85d)->word = 0; (&sstate.s_ss.a85d)->odd = 0; (&sstate.s_ss.a85d)->require_eod=((bool)0); } while (0);
                        sstate.s_ss.st.templat = &s_A85D_template;
                        sstate.s_ss.a85d.require_eod = ((bool)1);
                        goto str;
                }
                --sptr;
            }
            ((&sstate.s_ss.axd)->min_left = 1, (&sstate.s_ss.axd)->odd = -1, 0);
            sstate.s_ss.st.templat = &s_AXD_template;
          str:(s)->cursor.r.ptr = sptr;
            dynamic_init(&sstate.s_da, ((gs_memory_t *)((i_ctx_p->memory).current)));
          cont_string:for (;;) {
                stream_cursor_write w;
                ...
                switch (status) {
                    ...
                    case 1:
                        if (!check_only) {
                            retcode = dynamic_grow(&sstate.s_da, sstate.s_da.next, (16*1024*1024));
                            if (retcode == (-25)) {
                                sstate.s_scan_type = scanning_string;
                                goto suspend;
                            } else if (retcode < 0)
                                { retcode = (retcode); goto sret; };
                        }
                        continue;
                }
                break;
        ...
        case '(':
            sstate.s_ss.pssd.from_string =
                ((pstate->s_options & 1) != 0) &&
                !((i_ctx_p->language_level) >= 2);
        ...
        case '{':
            if (sstate.s_pstack == 0) {
                ...
                sstate.s_pdepth = ((&((i_ctx_p->op_stack).stack))->p + 1 - (&((i_ctx_p->op_stack).stack))->bot + (&((i_ctx_p->op_stack).stack))->extension_used);
            }
            (((((i_ctx_p->op_stack).stack).p))->value.intval = (sstate.s_pstack), (((((i_ctx_p->op_stack).stack).p))->tas.type_attrs = ((t_integer) << 8) + (0)) );
            sstate.s_pstack = ((&((i_ctx_p->op_stack).stack))->p + 1 - (&((i_ctx_p->op_stack).stack))->bot + (&((i_ctx_p->op_stack).stack))->extension_used);
            do { } while (0);
        ...
        case '}':
            if (sstate.s_pstack == 0)
                { retcode = ((-18)); goto sret; };
            (((i_ctx_p->op_stack).stack).p)--;
            {
                uint size = ((&((i_ctx_p->op_stack).stack))->p + 1 - (&((i_ctx_p->op_stack).stack))->bot + (&((i_ctx_p->op_stack).stack))->extension_used) - sstate.s_pstack;
                ref arr;
                ...
                if (size > (16*1024*1024))
                    { retcode = ((-13)); goto sret; };
                myref = (sstate.s_pstack == sstate.s_pdepth ? pref : &arr);
                if (check_only) {
                    ((myref)->value.refs = ((ref *)((void*)0)), ((myref)->tas.type_attrs = ((t_array) << 8) + (0)), ((myref)->tas.rsize = (0)));
                    ref_stack_pop(&((i_ctx_p->op_stack).stack), size);
                } else if ((i_ctx_p->array_packing).value.boolval) {
                    retcode = make_packed_array(myref, &((i_ctx_p->op_stack).stack), size,
                                                (&(i_ctx_p->memory)), "scanner(packed)");
                    if (retcode < 0) {
                        (((i_ctx_p->op_stack).stack).p)++;
                        --sptr;
                        sstate.s_scan_type = scanning_none;
                    ...
                    retcode = gs_alloc_ref_array(((i_ctx_p->memory).current), myref, 0x80 + (0x10 + 0x20 +0x40), size, "scanner(proc)");
                    ...
                    retcode = ref_stack_store(&((i_ctx_p->op_stack).stack), myref, size, 0, 1,
                                              ((bool)0), (&(i_ctx_p->memory)), "scanner");
                    if (retcode < 0) {
                        gs_free_ref_array(((i_ctx_p->memory).current), myref, "scanner(proc)");
                        { retcode = (retcode); goto sret; };
                    }
                    ref_stack_pop(&((i_ctx_p->op_stack).stack), size);
                }
                if (sstate.s_pstack == sstate.s_pdepth) {
                    if ( (((i_ctx_p->op_stack).stack).p) >= (((i_ctx_p->op_stack).stack).bot) ) (((i_ctx_p->op_stack).stack).p)--; else ref_stack_pop(&((i_ctx_p->op_stack).stack), 1);
                    sstate.s_pstack = 0;
                } else {
                    if ((((i_ctx_p->op_stack).stack).p) < (((i_ctx_p->op_stack).stack).bot))
                        ref_stack_pop_block(&((i_ctx_p->op_stack).stack));
                    sstate.s_pstack = (((i_ctx_p->op_stack).stack).p)->value.intval;
                    *(((i_ctx_p->op_stack).stack).p) = arr;
                    goto snext;
                }
            }
            break;
        case '/':
            ...
            if (sptr >= endptr && s->end_status != ((int)(-1))) {
                do { sptr -= 1; sstate.s_scan_type = scanning_none; goto pause; } while (0);
            }
            c = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr));
            if (!PDFScanRules && (c == '/')) {
            ...
            switch (decoder[c]) {
                ...
                default:
                    goto do_name;
                case 101:
                    if (!((i_ctx_p->binary_object_format).value.intval != 0 && ((i_ctx_p->language_level) >= 2)))
                        goto do_name;
        ...
        case '%':
          ...
          cont_comment:for (;;) {
                switch ((c = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr)))) {
                    default:
                        if (c < 0)
                            switch (c) {
                                case ((int)(-3)):
        ...
        case ((int)(-1)):
            ...
            break;
        case ((int)(-2)):
            { retcode = ((-12)); goto sret; };



          try_funny_name:
            {
                int c1 = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr));

                if (c1 == c) {
                    s1[0] = s1[1] = c;
                    names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, s1, 2, myref, 1);
                    goto have_name;
                }
                --sptr;
            }
    ...
    nr:
            ...
            if (retcode == 1 && decoder[newptr[-1]] == 102) {
                sptr = newptr - 1;
                if (*sptr == '\r' && sptr[1] == '\n')
                    sptr++;
                retcode = 0;
                ((myref)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask);
                break;
            }
            sstate.s_ss.s_name.s_name_type = 0;
            sstate.s_ss.s_name.s_try_number = ((bool)1);
            goto do_name;
        case '+':
            sign = 1;
            goto nr;
        case '-':
          ...
          case 128: case 128 +1: case 128 +2: case 128 +3: case 132: case 132 +1: case 132 +2: case 132 +3: case 136: case 136 +1: case 136 +2: case 136 +3: case 140: case 140 +1: case 140 +2: case 140 +3:
          case 144: case 144 +1: case 144 +2: case 144 +3: case 148: case 148 +1: case 148 +2: case 148 +3: case 152: case 152 +1: case 152 +2: case 152 +3: case 156: case 156 +1: case 156 +2: case 156 +3:
        ...
        default:
            if (c < 0) {
                dynamic_init(&sstate.s_da, names_memory(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table));
                sstate.s_scan_type = scanning_none;
                goto pause;
            }
        ...
        case '~':
          ...
          do_name:
            ...
            sstate.s_da.is_dynamic = ((bool)0);
            {
                ...
                while (decoder[*++sptr] <= max_name_ctype || *sptr == ctrld);
            }
          ...
          dyn_name:
            ...
            sstate.s_da.memory = names_memory(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table);
          ...
          cont_name:sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit;
            while (decoder[c = ((int)((sptr >= endptr) ? ((s)->cursor.r.ptr = sptr, (s)->inline_temp = spgetcc(s, ((bool)1)), sptr = (s)->cursor.r.ptr, endptr = (s)->cursor.r.limit, (s)->inline_temp) : *++sptr))] <= max_name_ctype || c == ctrld) {
          ...
          nx:switch (decoder[c]) {
            ...
            if (sstate.s_ss.s_name.s_try_number) {
                ...
                switch ( *base ) { case '-': sign = -1; base++; break; case '+': sign = 1; base++; break; default: sign = 0; };
            ...
            if (sstate.s_da.is_dynamic) {
                ...
                retcode = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, sstate.s_da.base, size, myref, -1);
                if (retcode >= 0) {
                    dynamic_free(&sstate.s_da);
                } else {
                    ...
                    retcode = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, sstate.s_da.base, size, myref, 2);
                }
            } else {
                retcode = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, sstate.s_da.base, (uint) (daptr - sstate.s_da.base), myref, !s->foreign);
          ...
          have_name:switch (sstate.s_ss.s_name.s_name_type) {
                case 0:
                    if (((((const byte *)&((myref)->tas.type_attrs))[1]) == (t_name)))
                        ((myref)->tas.type_attrs |= (0x80));
                case 1:
                    break;
                case 2:
                    {
                        ...
                        if (!((((const byte *)&((myref)->tas.type_attrs))[1]) == (t_name)) ||
                            (pvalue = dstack_find_name_by_index(&(i_ctx_p->dict_stack), names_index(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, myref))) == 0) {
                            (*(&sstate.s_error.object) = *(myref));
                            ((&sstate.s_error.object)->tas.type_attrs |= (0x80));

                            { retcode = ((-21)); goto sret; };
                        }
                        if (sstate.s_pstack != 0 &&
                            (avm_space)(((pvalue)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > (((&(i_ctx_p->memory)))->current_space)
                            )
                            { retcode = ((-7)); goto sret; };
                        ((void)(((*(myref) = *(pvalue)), ((myref)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
                    }
            }
    }
  sret:if (retcode < 0) {
        ...
        if (sstate.s_pstack != 0) {
            if (retcode == (-21))
                *pref = *(((i_ctx_p->op_stack).stack).p);
            ref_stack_pop(&((i_ctx_p->op_stack).stack),
                          ref_stack_count(&((i_ctx_p->op_stack).stack)) - (sstate.s_pdepth - 1));
        }
        return retcode;
    }
  ...
  snext:if ( (((i_ctx_p->op_stack).stack).p) < (((i_ctx_p->op_stack).stack).top) ) (((i_ctx_p->op_stack).stack).p)++; else if ( (retcode = ref_stack_push(&((i_ctx_p->op_stack).stack), 1)) >= 0 ) ; else {
        (s)->cursor.r.ptr = sptr;
        sstate.s_scan_type = scanning_none;
        goto save;
    }
