...
# 22 "./base/gxsample.c" 2
# 72 "./base/gxsample.c"
const bits32 lookup4x1to32_identity[16] = {
    0, 0^0xff000000U, 0^0x00ff0000U, 0^(0xff000000U +0x00ff0000U), 0^0x0000ff00U, 0^(0xff000000U +0x0000ff00U), 0^(0x00ff0000U +0x0000ff00U), 0^(0xff000000U +0x00ff0000U +0x0000ff00U), 0^0xff, 0^(0xff000000U +0xff), 0^(0x00ff0000U +0xff), 0^(0xff000000U +0x00ff0000U +0xff), 0^(0x0000ff00U +0xff), 0^(0xff000000U +0x0000ff00U +0xff), 0^(0x00ff0000U +0x0000ff00U +0xff), 0^(0xff000000U +0x00ff0000U +0x0000ff00U +0xff)
};
const bits32 lookup4x1to32_inverted[16] = {
    0xffffffffU, 0xffffffffU^0xff000000U, 0xffffffffU^0x00ff0000U, 0xffffffffU^(0xff000000U +0x00ff0000U), 0xffffffffU^0x0000ff00U, 0xffffffffU^(0xff000000U +0x0000ff00U), 0xffffffffU^(0x00ff0000U +0x0000ff00U), 0xffffffffU^(0xff000000U +0x00ff0000U +0x0000ff00U), 0xffffffffU^0xff, 0xffffffffU^(0xff000000U +0xff), 0xffffffffU^(0x00ff0000U +0xff), 0xffffffffU^(0xff000000U +0x00ff0000U +0xff), 0xffffffffU^(0x0000ff00U +0xff), 0xffffffffU^(0xff000000U +0x0000ff00U +0xff), 0xffffffffU^(0x00ff0000U +0x0000ff00U +0xff), 0xffffffffU^(0xff000000U +0x00ff0000U +0x0000ff00U +0xff)
};
# 86 "./base/gxsample.c"
const byte *
...
# 102 "./base/gxsample.c" 2
# 115 "./base/gxsample.c"
# 1 "./base/gxsamplp.h" 1
# 39 "./base/gxsamplp.h"
const byte *
sample_unpack_1_interleaved(byte * bptr, int *pdata_x, const byte * data, int data_x,
                uint dsize, const sample_map *smap, int spread,
                int num_components_per_plane)
{
    const sample_lookup_t * ptab = &smap->table;
    ...
    if (spread == 1) {
        ...
        if (left & 1) {
            b = psrc[0];
            bufp[0] = map[b >> 4];
            map = smap[++smap_index % num_components_per_plane].table.lookup4x1to32;
            bufp[1] = map[b & 0xf];
            map = smap[++smap_index % num_components_per_plane].table.lookup4x1to32;
            psrc++, bufp += 2;
        }
        ...
        while (left--) {
            ...
            *bufp = map[b >> 7];
            map = smap[++smap_index % num_components_per_plane].table.lookup8;
            bufp += spread;
            *bufp = map[(b >> 6) & 1];
            map = smap[++smap_index % num_components_per_plane].table.lookup8;
            bufp += spread;
...
# 121 "./base/gxsamplp.h"
const byte *
sample_unpack_2_interleaved(byte * bptr, int *pdata_x, const byte * data, int data_x,
                uint dsize, const sample_map *smap, int spread,
                int num_components_per_plane)
{
    const sample_lookup_t * ptab = &smap->table;
    ...
    if (spread == 1) {
        ...
        while (left--) {
            ...
            *bufp++ = map[b >> 4];
            map = smap[++smap_index % num_components_per_plane].table.lookup2x2to16;
            *bufp++ = map[b & 0xf];
            map = smap[++smap_index % num_components_per_plane].table.lookup2x2to16;
        }
    } else {
...
const byte *
sample_unpack_4_interleaved(byte * bptr, int *pdata_x, const byte * data, int data_x,
                uint dsize, const sample_map *smap, int spread,
                int num_components_per_plane)
{
    const sample_lookup_t * ptab = &smap->table;
...
const byte *
sample_unpack_8_interleaved(byte * bptr, int *pdata_x, const byte * data, int data_x,
                uint dsize, const sample_map *smap, int spread,
                int num_components_per_plane)
{
    const sample_lookup_t * ptab = &smap->table;
