...
static const gc_ptr_element_t imscale_state_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_imscale_state *)0)->window - (char *)((stream_imscale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_imscale_state *)0)->dst - (char *)((stream_imscale_state *)0) )) } }; static const gc_struct_data_t imscale_state_reloc_ptrs = { (sizeof(imscale_state_enum_ptrs) / sizeof((imscale_state_enum_ptrs)[0])), 0, 0, imscale_state_enum_ptrs }; static const gs_memory_struct_type_t st_imscale_state = { sizeof(stream_imscale_state), "ImscaleDecode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &imscale_state_reloc_ptrs };
...
s_imscale_release(stream_state *st)
{
    ...
    ((mem)->procs.free_object(mem, (void *)ss->window, "imscale window"));
    ss->window = 0;
    ((mem)->procs.free_object(mem, (void *)ss->dst, "imscale dst"));
    ss->dst = 0;
}
...
s_imscale_init(stream_state *st)
{
    ...
    ss->window = (byte *)(*(mem)->procs.alloc_byte_array)(mem, bytesin + 2, 4, "imscale window");


    ss->dst = (byte *)(*(mem)->procs.alloc_bytes)(mem, bytesout + 1, "imscale dst");
                ...
                stream_cursor_write *pw, bool last)
{
    ...
    while (1) {
        ...
        if (ss->dst_offset < ss->dst_size) {
            uint ncopy = (((pw->limit - pw->ptr) < (ss->dst_size - ss->dst_offset)) ? (pw->limit - pw->ptr) : (ss->dst_size - ss->dst_offset));
        ...
        if (ss->dst_y >= ss->src_y * 4 - 6) {
            ...
            uint ncopy = (((rleft) < (ss->src_size - ss->src_offset)) ? (rleft) : (ss->src_size - ss->src_offset));
