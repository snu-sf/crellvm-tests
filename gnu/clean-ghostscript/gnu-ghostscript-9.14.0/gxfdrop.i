...
# 29 "./base/gxfdrop.c" 2
# 40 "./base/gxfdrop.c"
static const gs_memory_struct_type_t st_margin = { sizeof(margin), "margin", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
const gs_memory_struct_type_t st_section = { sizeof(section), "section", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
{ margin *m;
    ...
    } else if (ll->local_margin_alloc_count < 20) {
        m = ll->local_margins + ll->local_margin_alloc_count;
        ++ ll->local_margin_alloc_count;
    } else {
        m = (margin *)(*(ll->memory)->procs.alloc_struct)(ll->memory, &st_margin, "filling contiguity margin");
...
{ margin * m = ll->free_margin_list;
    ...
    while (m != 0) {
        ...
        if (m < ll->local_margins || m >= ll->local_margins + 20)
            ((ll->memory)->procs.free_object(ll->memory, m, "filling contiguity margin"));
        m = m1;
    }
}
...
static int store_margin(line_list * ll, margin_set * set, int ii0, int ii1)
{
    ...
    if (m0 != 0) {
        ...
        if (mb != 0 && mb != m1) {
            ...
            if (me != 0) {
                 if (mb != me && me->prev != 0) {
                    ...
                    i0 = (((i0) < (mb->ibeg)) ? (i0) : (mb->ibeg));
                    i1 = (((i1) > (mf->iend)) ? (i1) : (mf->iend));
        ...
        if (me == 0)
            ...
            me->ibeg = (((i0) < (me->ibeg)) ? (i0) : (me->ibeg));
            me->iend = (((i1) > (me->iend)) ? (i1) : (me->iend));
...
static __inline__ int to_interval(int x, int l, int u)
{ return x < l ? l : x > u ? u : x;
}

static __inline__ fixed Y_AT_X(active_line *alp, fixed xp)
...
{ section *sect = set->sect;
    ...
    if (alp == 0)
        ...
        x0 = (yy0 == y0 ? alp->x_current : ((yy0) == (alp)->end.y ? (alp)->end.x : ((yy0) <= (alp)->y_fast_max ? ((((yy0) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (yy0) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
        x1 = (yy1 == y1 ? alp->x_next : ((yy1) == (alp)->end.y ? (alp)->end.x : ((yy1) <= (alp)->y_fast_max ? ((((yy1) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (yy1) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
    }
    xmin = (((x0) < (x1)) ? (x0) : (x1));
    xmax = (((x0) > (x1)) ? (x0) : (x1));
# 273 "./base/gxfdrop.c"
        xp0 = ((xmin)&(-1L<<8)) + ((1L << 8) >> 1);
        i0 = ((int)((xp0) >> (8))) - ll->bbox_left;
        if (xp0 < xmin) {
            i0++;
            xp0 += (1L << 8);
        }
        for (i = i0, xp = xp0; xp < xmax; xp += (1L << 8), i++) {
            section *s = &sect[i];
...
{ int code;
    ...
    section *sect = set->sect;
    fixed yy0 = ((((((y0) > (alp->start.y)) ? (y0) : (alp->start.y))) > (set->y)) ? ((((y0) > (alp->start.y)) ? (y0) : (alp->start.y))) : (set->y));
    fixed yy1 = ((((((y1) < (alp->end.y)) ? (y1) : (alp->end.y))) < (set->y + (1L << 8))) ? ((((y1) < (alp->end.y)) ? (y1) : (alp->end.y))) : (set->y + (1L << 8)));

    if (yy0 <= yy1) {
        fixed x00 = (yy0 == y0 ? flp->x_current : ((yy0) == (flp)->end.y ? (flp)->end.x : ((yy0) <= (flp)->y_fast_max ? ((((yy0) - (flp)->start.y) * (flp)->diff.x) + (flp)->num_adjust) / (flp)->diff.y : (((void)(0)), fixed_mult_quo((flp)->diff.x, (yy0) - (flp)->start.y, (flp)->diff.y))) + (flp)->start.x));
        fixed x10 = (yy0 == y0 ? alp->x_current : ((yy0) == (alp)->end.y ? (alp)->end.x : ((yy0) <= (alp)->y_fast_max ? ((((yy0) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (yy0) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
        fixed x01 = (yy1 == y1 ? flp->x_next : ((yy1) == (flp)->end.y ? (flp)->end.x : ((yy1) <= (flp)->y_fast_max ? ((((yy1) - (flp)->start.y) * (flp)->diff.x) + (flp)->num_adjust) / (flp)->diff.y : (((void)(0)), fixed_mult_quo((flp)->diff.x, (yy1) - (flp)->start.y, (flp)->diff.y))) + (flp)->start.x));
        fixed x11 = (yy1 == y1 ? alp->x_next : ((yy1) == (alp)->end.y ? (alp)->end.x : ((yy1) <= (alp)->y_fast_max ? ((((yy1) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (yy1) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
        fixed xmin = (((x00) < (x01)) ? (x00) : (x01)), xmax = (((x10) > (x11)) ? (x10) : (x11));

        int i0 = ((int)((xmin) >> (8))) - ll->bbox_left, i;
        int i1 = ((int)(((xmax)+((1<<8)-1)) >> (8))) - ll->bbox_left;

        for (i = i0; i < i1; i++) {
            section *s = &sect[i];
            int x_pixel = ((fixed)(i + ll->bbox_left)<<8);
            int xl = (((xmin - x_pixel) > (0)) ? (xmin - x_pixel) : (0));
            int xu = (((xmax - x_pixel) < ((1L << 8))) ? (xmax - x_pixel) : ((1L << 8)));

            s->x0 = (((s->x0) < (xl)) ? (s->x0) : (xl));
            s->x1 = (((s->x1) > (xu)) ? (s->x1) : (xu));
            x_pixel+=0;
        }
...
static __inline__ int mark_margin_interior(line_list * ll, margin_set * set, active_line * flp, active_line * alp, fixed y, fixed y0, fixed y1)
{
    section *sect = set->sect;
    fixed x0 = (y == y0 ? flp->x_current : y == y1 ? flp->x_next : ((y) == (flp)->end.y ? (flp)->end.x : ((y) <= (flp)->y_fast_max ? ((((y) - (flp)->start.y) * (flp)->diff.x) + (flp)->num_adjust) / (flp)->diff.y : (((void)(0)), fixed_mult_quo((flp)->diff.x, (y) - (flp)->start.y, (flp)->diff.y))) + (flp)->start.x));
    fixed x1 = (y == y0 ? alp->x_current : y == y1 ? alp->x_next : ((y) == (alp)->end.y ? (alp)->end.x : ((y) <= (alp)->y_fast_max ? ((((y) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
    int i0 = ((int)((x0) >> (8))), ii0, ii1, i, code;

    if (((fixed)(i0)<<8) + ((1L << 8) >> 1) < x0)
        i0++;
    ii0 = i0 - ll->bbox_left;
    ii1 = ((int)((((x1)+(((1<<8)>>1) - 1L))) >> (8))) - ll->bbox_left;
    if (ii0 < ii1) {
        if (ii0 < 0 || ii1 > ll->bbox_width)
            return ((-28));
        for (i = ii0; i < ii1; i++) {
...
static __inline__ int process_h_sect(line_list * ll, margin_set * set, active_line * hlp0,
    active_line * plp, active_line * flp, int side, fixed y0, fixed y1)
{
    active_line *hlp = hlp0;
    fixed y = hlp->start.y;
    fixed x0 = (plp != 0 ? (y == y0 ? plp->x_current : y == y1 ? plp->x_next : ((y) == (plp)->end.y ? (plp)->end.x : ((y) <= (plp)->y_fast_max ? ((((y) - (plp)->start.y) * (plp)->diff.x) + (plp)->num_adjust) / (plp)->diff.y : (((void)(0)), fixed_mult_quo((plp)->diff.x, (y) - (plp)->start.y, (plp)->diff.y))) + (plp)->start.x))
                         : ((fixed)(ll->bbox_left)<<8));
    fixed x1 = (flp != 0 ? (y == y0 ? flp->x_current : y == y1 ? flp->x_next : ((y) == (flp)->end.y ? (flp)->end.x : ((y) <= (flp)->y_fast_max ? ((((y) - (flp)->start.y) * (flp)->diff.x) + (flp)->num_adjust) / (flp)->diff.y : (((void)(0)), fixed_mult_quo((flp)->diff.x, (y) - (flp)->start.y, (flp)->diff.y))) + (flp)->start.x))
                         : ((fixed)(ll->bbox_left + ll->bbox_width)<<8));
    int code;

    for (; hlp != 0; hlp = hlp->next) {
        fixed xx0 = (((x0) > ((((hlp->start.x) < (hlp->end.x)) ? (hlp->start.x) : (hlp->end.x)))) ? (x0) : ((((hlp->start.x) < (hlp->end.x)) ? (hlp->start.x) : (hlp->end.x))));
        fixed xx1 = (((x1) < ((((hlp->start.x) > (hlp->end.x)) ? (hlp->start.x) : (hlp->end.x)))) ? (x1) : ((((hlp->start.x) > (hlp->end.x)) ? (hlp->start.x) : (hlp->end.x))));
...
static __inline__ int process_h_side(line_list * ll, margin_set * set, active_line * hlp,
...
static __inline__ int process_h_list(line_list * ll, active_line * hlp, active_line * plp,
    active_line * flp, active_line * alp, int side, fixed y0, fixed y1)
{ fixed y = hlp->start.y;

    if (ll->margin_set0.y <= y && y <= ll->margin_set0.y + (1L << 8)) {
        int code = process_h_side(ll, &ll->margin_set0, hlp, plp, flp, alp, side, y0, y1);

        if (code < 0)
            return code;
    }
    if (ll->margin_set1.y <= y && y <= ll->margin_set1.y + (1L << 8)) {
...
static int fill_margin(gx_device * dev, const line_list * ll, margin_set *ms, int i0, int i1)
{
    ...
    for (i = i0; i < i1; i++) {
        ...
        hh = compute_padding(&sect[i]);
            ...
            if (hh >= 0) {
# 517 "./base/gxfdrop.c"
                    if (sect[i].x0 > 0 && sect[i].x1 == (1L << 8) && i + 1 < i1) {



                        hh = (i + 1 < i1 ? compute_padding(&sect[i + 1]) : -2);
# 537 "./base/gxfdrop.c"
                    } else if (sect[i].x0 == 0 && sect[i].x1 < (1L << 8)) {
...
# 554 "./base/gxfdrop.c"
        ...
        if (h != hh) {
            if (h >= 0) {
                do { } while (0);
                code = ( FILL_DIRECT ? (fo)->fill_rect((fo)->dev, ir + ll->bbox_left, iy + h, i - ir, 1, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, ir + ll->bbox_left, iy + h, i - ir, 1, (fo)->dev, (fo)->lop, ((void*)0)));
                if (code < 0)
                    return code;
            }
            ir = i;
...
int start_margin_set(gx_device * dev, line_list * ll, fixed y0)
{ int code;
    fixed ym = (((y0)+(((1<<8)>>1) - 1L))&(-1L<<8)) - ((1L << 8) >> 1);
    margin_set s;
