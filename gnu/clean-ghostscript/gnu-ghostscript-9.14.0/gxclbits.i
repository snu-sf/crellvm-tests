...
{
    uint width_bytes = ((uint)((((width_bits) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    int status = 0;
    stream_cursor_read r;
    ...
    if (raster == width_bytes) {
        ...
        status = (*st->templat->process) (st, &r, pw, ((bool)1));
    } else {
        uint y;

        for (y = 1; (r.limit = r.ptr + width_bytes), y < height; ++y) {
            status = (*st->templat->process) (st, &r, pw, ((bool)0));
            if (status)
                break;
            if (r.ptr != r.limit) {
             ...
             int compression_mask, byte ** pdp, uint * psize)
{
    ...
    if (short_size >= 50 &&
        (compression_mask & ((1<<1) | (1<<2))) != 0 &&
        (uncompressed_size <= max_size ||
         (compression_mask & 0x100) != 0)
        ) {
        union ss_ {
            stream_state ss;
            stream_CFE_state cf;
        ...
        int try_size = op_size + (((uncompressed_size) < (max_size)) ? (uncompressed_size) : (max_size));

        *psize = try_size;
        code = (pcls != 0 ?
                ( (dp = cmd_put_list_op(cldev, &(pcls)->list, try_size)) == 0 ? (cldev)->error_code : (*dp = (0), 0) ) :
                ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, try_size)) == 0 ? (cldev)->error_code : (*dp = (0), 0) ));
        if (code < 0)
            return code;
        do { } while (0);
        ...
        if (compress) {
            ...
            if (status == 0) {
                ...
                (((pcls ? &pcls->list : &cldev->band_range_list))->tail->size -= (try_size - (op_size + wcount)), (cldev)->cnext -= (try_size - (op_size + wcount)));
        ...
        if (uncompressed_size > max_size) {
            ...
            (((pcls ? &pcls->list : &cldev->band_range_list))->tail->size -= (try_size), (cldev)->cnext -= (try_size));
        ...
        if (uncompressed_size != short_size) {
            ...
            (((pcls ? &pcls->list : &cldev->band_range_list))->tail->size -= (try_size - (op_size + short_size)), (cldev)->cnext -= (try_size - (op_size + short_size)));
        ...
        *psize = op_size + short_size;
        code = (pcls != 0 ?
                ( (dp = cmd_put_list_op(cldev, &(pcls)->list, *psize)) == 0 ? (cldev)->error_code : (*dp = (0), 0) ) :
                ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, *psize)) == 0 ? (cldev)->error_code : (*dp = (0), 0) ));
        if (code < 0)
            return code;
        do { } while (0);
    }
    if ((compression_mask & (1 << 3)) &&
        (code = bytes_rectangle_is_const(data, raster, uncompressed_raster << 3, height)) >= 0) {
        (((pcls ? &pcls->list : &cldev->band_range_list))->tail->size -= (*psize - (op_size + 1)), (cldev)->cnext -= (*psize - (op_size + 1)));
...
{
    byte *p = dp + 2;
    byte bd = ((depth) > 8 ? 8 | (((depth)-5) >> 3) : ((depth)-1));
                   ...
                   uint indx)
{
    ...
    if (!(idelta & ~15)) {
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (cmd_op_delta_tile_index + idelta), 0) );

        if (code < 0)
            return code;
    } else {
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2)) == 0 ? (cldev)->error_code : (*dp = (cmd_op_set_tile_index + (indx >> 8)), 0) );
        ...
        int comp_num, const gx_transfer_map * map, gs_id * pid)
{
    ...
    if (map == 0) {
        if (pid && *pid == 0L)
            return 0;
        code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, 3)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_misc), 0) );
        if (code < 0)
            return code;
        dp[1] = (2 << 6) + (cmd_map_none << 4) + map_index;
        dp[2] = comp_num;
        if (pid)
            *pid = 0L;
    } else {
        ...
        if (map->proc == gs_identity_transfer) {
            ...
            if (code < 0)
                return code;
            dp[1] = (2 << 6) + (cmd_map_identity << 4) + map_index;
            dp[2] = comp_num;
        } else {
            code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, 3 + sizeof(map->values))) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_set_misc), 0) );

            if (code < 0)
                return code;
            dp[1] = (2 << 6) + (cmd_map_other << 4) + map_index;
            dp[2] = comp_num;
               ...
               uint sraster, int depth)
{
    ...
    ((tile_slot *)slot_head)->num_planes = (byte)tiles->num_planes;
    if (((tile_slot *)slot_head)->num_planes != 1)
        depth /= ((tile_slot *)slot_head)->num_planes;
    memset((byte *)((((tile_slot *)slot_head)) + 1), 0, cldev->tile_band_mask_size);
    bytes_copy_rectangle_zero_padding(((byte *)((((tile_slot *)slot_head)) + 1) + (cldev)->tile_band_mask_size), raster,
                                      tiles->data, sraster,
                                      (tiles->rep_width * depth + 7) >> 3,
                                      tiles->rep_height * ((tile_slot *)slot_head)->num_planes);
    ...
    *new_tile = *tiles;
    {
        ...
        uint reps_x =
        (((max_bits_x) < (32 * 8)) ? (max_bits_x) : (32 * 8)) / rep_width_bits;
        uint reps_y;

        while (reps_x > 255)
            reps_x >>= 1;
        new_tile->size.x = (((reps_x) > (1)) ? (reps_x) : (1)) * rep_width;
        new_tile->raster = ((uint)((((new_tile->size.x * depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
...
# 650 "./base/gxclbits.c"
                ...
                code = cmd_put_bits(cldev, pcls, ((byte *)((loc.tile) + 1) + (cldev)->tile_band_mask_size),
   ...
   top:if (clist_find_bits(cldev, tiles->id, &loc)) {
        ...
        if (*bptr & bmask) {
            ...
            if (loc.tile->num_bands == ((unsigned short)0xffff + (unsigned short)0))
                bit_pcls = ((void*)0);
            code = cmd_put_bits(cldev, bit_pcls, ((byte *)((loc.tile) + 1) + (cldev)->tile_band_mask_size),
                                loc.tile->width * pdepth,
                                loc.tile->height * loc.tile->num_planes, loc.tile->raster,
                                rsize,
                              0x100 | (((gx_device_printer *)cldev->target)->BLS_force_memory ? (1 << 2) : 0),
                                &dp, &csize);
