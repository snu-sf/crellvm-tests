                        ...
                        gs_memory_t *mem)
{
    gs_font_type0 *font0 = (gs_font_type0 *)
        gs_font_alloc(mem, &st_gs_font_type0, &gs_font_procs_default, ((void*)0),
                      "gs_type0_from_cidfont(font)");

    uint *encoding = (uint *)
        (*(mem)->procs.alloc_bytes)(mem, sizeof(uint), "gs_type0_from_cidfont(Encoding)");
    gs_font **fdep =
        (gs_font * *)(*(mem)->procs.alloc_struct_array)(mem, 1, &st_gs_font_ptr_element, "gs_type0_from_cidfont(FDepVector)");

    int code;

    if (font0 == 0 || encoding == 0 || fdep == 0) {
        ((mem)->procs.free_object(mem, fdep, "gs_type0_from_cidfont(FDepVector)"));
        ((mem)->procs.free_object(mem, encoding, "gs_type0_from_cidfont(Encoding)"));
        ((mem)->procs.free_object(mem, font0, "gs_type0_from_cidfont(font)"));
        return ((-25));
    }
    if (psmat)
                           ...
                           const gs_matrix *psmat, gs_memory_t *mem)
{
    ...
    if (code < 0) {
        ((mem)->procs.free_object(mem, pcmap, "gs_font_type0_from_cidfont(CMap)"));
                          ...
                          int wmode, bool use_cmap, gs_memory_t *mem)
{
    ...
    if (code < 0) {
        ((mem)->procs.free_object(mem, pfcid, "gs_type0_from_type42(CIDFont)"));
