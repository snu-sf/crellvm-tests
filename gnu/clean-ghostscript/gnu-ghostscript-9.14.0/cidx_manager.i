# 1 "./openjpeg/libopenjpeg/cidx_manager.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./openjpeg/libopenjpeg/cidx_manager.c" 2
# 32 "./openjpeg/libopenjpeg/cidx_manager.c"
...
# 33 "./openjpeg/libopenjpeg/cidx_manager.c" 2
# 43 "./openjpeg/libopenjpeg/cidx_manager.c"
              ...
              opj_event_mgr_t * p_manager )
{
  ...
  lenp = -1;
  box = (opj_jp2_box_t *)((size_t)(32) != 0 && (size_t)(32) >= (size_t)-0x100 / (size_t)(sizeof(opj_jp2_box_t)) ? ((void*)0) : calloc(32, sizeof(opj_jp2_box_t)));
  ...
  for (i=0;i<2;i++){
    ...
    opj_write_bytes_LE(l_data_header,0x63696478,4);
              ...
              opj_event_mgr_t * p_manager )
{
  ...
  lenp = opj_stream_tell(cio);
  opj_stream_skip( cio, 4, p_manager);
  opj_write_bytes_LE( l_data_header, 0x63707472, 4);
  opj_write_bytes_LE( l_data_header+4, 0, 2);
  opj_write_bytes_LE( l_data_header+6, 0, 2);
  opj_write_bytes_LE( l_data_header+8, coff, 8);
  opj_write_bytes_LE( l_data_header+16, clen, 8);
                    ...
                    opj_event_mgr_t * p_manager )
{
  ...
  lenp = opj_stream_tell(cio);
  opj_stream_skip( cio, 4, p_manager);
  opj_write_bytes_LE( l_data_header, 0x6d616e66, 4);
  opj_stream_write_data(cio,l_data_header,4,p_manager);

  if (second){
    for( i=0; i<v; i++){
      opj_write_bytes_LE( l_data_header, box[i].length, 4);
      opj_stream_write_data(cio,l_data_header,4,p_manager);
      opj_write_bytes_LE( l_data_header, box[i].type, 4);
      opj_stream_write_data(cio,l_data_header,4,p_manager);
    }
  }
              ...
              opj_event_mgr_t * p_manager )
{
  ...
  opj_write_bytes_LE(l_data_header, cstr_info.main_head_end-cstr_info.main_head_start+1, 8);
  opj_stream_write_data(cio,l_data_header,8,p_manager);

  for(i = 1; i < cstr_info.marknum; i++){
    opj_write_bytes_LE( l_data_header, cstr_info.marker[i].type, 2);
    opj_write_bytes_LE( l_data_header+2, 0, 2);
    opj_stream_write_data(cio,l_data_header,4,p_manager);
    opj_write_bytes_LE( l_data_header,(OPJ_UINT32) (cstr_info.marker[i].pos-coff), 8);
    opj_stream_write_data(cio,l_data_header,8,p_manager);
    opj_write_bytes_LE( l_data_header, cstr_info.marker[i].len, 2);
              ...
              opj_event_mgr_t * p_manager )
{
  ...
  for(i = 0; i < marknum; i++)
    ...
    if( markers[i].type == 0xff52)
      ...
      opj_read_bytes_LE(l_data_header,&Scod,1);
      if( ((Scod >> 2) & 1))
        EPHused = 1;
