...
char* pSHA256_End(SHA256_CTX*, char[(32 * 2 + 1)]);
char* pSHA256_Data(const uint8_t*, size_t, char[(32 * 2 + 1)]);

void pSHA384_Init(SHA384_CTX*);
void pSHA384_Update(SHA384_CTX*, const uint8_t*, size_t);
void pSHA384_Final(uint8_t[48], SHA384_CTX*);
char* pSHA384_End(SHA384_CTX*, char[(48 * 2 + 1)]);
char* pSHA384_Data(const uint8_t*, size_t, char[(48 * 2 + 1)]);

void pSHA512_Init(SHA512_CTX*);
void pSHA512_Update(SHA512_CTX*, const uint8_t*, size_t);
void pSHA512_Final(uint8_t[64], SHA512_CTX*);
char* pSHA512_End(SHA512_CTX*, char[(64 * 2 + 1)]);
char* pSHA512_Data(const uint8_t*, size_t, char[(64 * 2 + 1)]);
# 24 "./base/ssha2.h" 2
...
static const gs_memory_struct_type_t st_SHA256E_state = { sizeof(stream_SHA256E_state), "SHA256Encode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_SHA256E_make_stream(gs_memory_t *mem, byte *digest, int digest_size)
{
    ...
    if (ss == ((void*)0) || s == ((void*)0))
        goto err;
    ss->templat = &s_SHA256E_template;
    if (s_init_filter(s, ss, digest, digest_size, ((void*)0)) < 0)
        goto err;
    s->strm = s;
    return s;
err:
    ((mem)->procs.free_object(mem, ss, "s_SHA256E_make_stream"));
    ((mem)->procs.free_object(mem, s, "s_SHA256E_make_stream"));
    return ((void*)0);
}
