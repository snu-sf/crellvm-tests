...
# 1 "./base/gxht_thresh.h" 1
# 32 "./base/gxht_thresh.h"
...
# 46 "./base/gximono.c" 2
# 58 "./base/gximono.c"
irender_proc_t gs_image_class_3_mono(gx_image_enum *penum);

static int image_render_mono(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_mono_ht(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);

irender_proc_t
gs_image_class_3_mono(gx_image_enum * penum)
{

    bool use_fast_code = ((bool)1);
...
# 91 "./base/gximono.c"
        ...
        if (use_fast_code && penum->pcs != ((void*)0) && dev_color_ok &&
            ...
            if (gs_color_space_is_PSCIE(penum->pcs) &&
                penum->pcs->icc_equivalent != ((void*)0)) {

                if ( !check_cie_range(penum->pcs) ) {


                    penum->use_cie_range = (get_cie_range(penum->pcs) != ((void*)0));
                }
            }
...
not_fast_halftoning:
        ...
        penum->slow_loop =
            (penum->masked && !((penum->icolor0)->type == (&gx_dc_type_data_pure))) ||
            penum->use_rop;

        if (!(penum->slow_loop || penum->posture != image_portrait))
            penum->clip_image &= ~(1 | 2);
        do { } while (0);

        penum->dxx =
            ((fixed)((penum->matrix.xx + ((1L)*(1.0/(1<<8))) / 2)*(float)(1<<8)));
        ...
        if (penum->use_mask_color) {
            ...
            if (penum->mask_color.values[0] <= 0)
                ((penum->icolor0)->type = (&gx_dc_type_data_null));
            if (penum->mask_color.values[1] >= 255)
                ((penum->icolor1)->type = (&gx_dc_type_data_null));
        }
                ...
                gx_image_enum * penum)
{
   ...
   if (!((pdevc)->type != (&gx_dc_type_data_none))) {
       if ((uint)(sample_value - mask_base) < mask_limit) {
           ((pdevc)->type = (&gx_dc_type_data_null));
       } else {
            switch ( penum->map[0].decoding )
            {
            case sd_none:
            cc->paint.values[0] = (sample_value) * (1.0f / 255.0f);
            break;
            case sd_lookup:
            cc->paint.values[0] =
            ...
            case sd_compute:
            cc->paint.values[0] =
              penum->map[0].decode_lookup[0] + (sample_value) * penum->map[0].decode_factor;
            }
        ...
        code = (*(pdevc)->type->load)(pdevc, pis, dev, gs_color_select_source);
        if (code < 0)
            return(code);
    }
    return(0);
                  ...
                  uint w, int h, gx_device * dev)
{
    ...
    const gs_color_space *pcs = ((void*)0);
    int (*remap_color)(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t) = ((void*)0);
    gs_client_color cc;
...
# 301 "./base/gximono.c"
    ...
    if (penum->slow_loop || penum->posture != image_portrait) {
        ...
        fixed yrun;
        const fixed pdyx = ((penum->dda.row.x).state).Q - penum->cur.x;
        const fixed pdyy = ((penum->dda.row.y).state).Q - penum->cur.y;
        int (*fill_pgram)(gx_device *dev, fixed px, fixed py, fixed ax, fixed ay, fixed bx, fixed by, const gx_drawing_color *pdcolor, gs_logical_operation_t lop) =
            ((dev)->procs.fill_parallelogram);
        ...
        if (masked) {
            ...
            pdevc = penum->icolor1;
            code = (*(pdevc)->type->load)(pdevc, pis, dev, gs_color_select_texture);
            if (code < 0)
                return code;
...
# 379 "./base/gximono.c"
                ...
                ax <<= 1;
                dxx2 = next.x.step;
                do { (dxx2).dR += (next.x.step).dR; if ((dxx2).dR >= (dxx2).N) { (dxx2).dQ ++; (dxx2).dR -= (dxx2).N; } (dxx2).dQ += (next.x.step).dQ; } while (0);
                dxx3 = dxx2;
                do { (dxx3).dR += (next.x.step).dR; if ((dxx3).dR >= (dxx3).N) { (dxx3).dQ ++; (dxx3).dR -= (dxx3).N; } (dxx3).dQ += (next.x.step).dQ; } while (0);
                dxx4 = dxx3;
                do { (dxx4).dR += (next.x.step).dR; if ((dxx4).dR >= (dxx4).N) { (dxx4).dQ ++; (dxx4).dR -= (dxx4).N; } (dxx4).dQ += (next.x.step).dQ; } while (0);
                for (;;) {
                    while (!psrc[0])
                        if (psrc + 4 <= endp) {
                            ...
                            if (!psrc[1]) {
                                if (!psrc[2]) {
                                    if (!psrc[3]) {
                                        psrc += 4;
                                        do { (next.x.state).R -= (dxx4).dR; if ((signed)(next.x.state).R < 0) { (next.x.state).Q++; (next.x.state).R += (dxx4).N; }; (next.x.state).Q += (dxx4).dQ; } while (0);
                                        if (psrc >= endp)
                                            break;
                                        continue;
                                    }
                                    psrc += 3;
                                    do { (next.x.state).R -= (dxx3).dR; if ((signed)(next.x.state).R < 0) { (next.x.state).Q++; (next.x.state).R += (dxx3).N; }; (next.x.state).Q += (dxx3).dQ; } while (0);
                                    break;
                                }
                                psrc += 2;
                                do { (next.x.state).R -= (dxx2).dR; if ((signed)(next.x.state).R < 0) { (next.x.state).Q++; (next.x.state).R += (dxx2).N; }; (next.x.state).Q += (dxx2).dQ; } while (0);
                                break;
                            } else {
                                ++psrc;
                                do { ((next.x).state).R -= ((next.x).step).dR; if ((signed)((next.x).state).R < 0) { ((next.x).state).Q++; ((next.x).state).R += ((next.x).step).N; }; ((next.x).state).Q += ((next.x).step).dQ; } while (0);
                                break;
                            }
                        } else {
                    ...
                    code = (*fill_pgram)(dev, xrun, yrun,
                                         ((next.x).state).Q - xrun + ax, 0L, 0L, dyy,
                                         pdevc, lop);
                    if (code < 0)
                        goto err;
                    rsrc = psrc;
...
# 456 "./base/gximono.c"
                ...
                for (;;) {
                    for (; psrc < endp && !*psrc; ++psrc)
                        do { ((next.y).state).R -= ((next.y).step).dR; if ((signed)((next.y).state).R < 0) { ((next.y).state).Q++; ((next.y).state).R += ((next.y).step).N; }; ((next.y).state).Q += ((next.y).step).dQ; } while (0);
                    yrun = ((next.y).state).Q;
                    if (psrc >= stop)
                        break;
                    for (; psrc < endp && *psrc; ++psrc)
                        do { ((next.y).state).R -= ((next.y).step).dR; if ((signed)((next.y).state).R < 0) { ((next.y).state).Q++; ((next.y).state).R += ((next.y).step).N; }; ((next.y).state).Q += ((next.y).step).dQ; } while (0);
                    code = (*fill_pgram)(dev, xrun, yrun, 0L,
                                         ((next.y).state).Q - yrun + ay, dyx, 0L,
                                         pdevc, lop);
                    if (code < 0)
                        goto err;
                    rsrc = psrc;
...
# 497 "./base/gximono.c"
                ...
                for (;;) {
                    ...
                    do {
                        ...
                        do { ((next.x).state).R -= ((next.x).step).dR; if ((signed)((next.x).state).R < 0) { ((next.x).state).Q++; ((next.x).state).R += ((next.x).step).N; }; ((next.x).state).Q += ((next.x).step).dQ; } while (0);
                        do { ((next.y).state).R -= ((next.y).step).dR; if ((signed)((next.y).state).R < 0) { ((next.y).state).Q++; ((next.y).state).R += ((next.y).step).N; }; ((next.y).state).Q += ((next.y).step).dQ; } while (0);
                        code = (*fill_pgram)(dev, xrun, yrun, ((next.x).state).Q - xrun,
                                             ((next.y).state).Q - yrun, pdyx, pdyy, pdevc, lop);
                        if (code < 0)
                            goto err;
                        psrc++;
                        rsrc = psrc;
            ...
            for (;;) {
                if (*psrc != run) {
                    if (run != htrun) {
                        ...
                        do { pdevc = &penum->clues[run].dev_color; if (!((pdevc)->type != (&gx_dc_type_data_none))) { if ((uint)(run - mask_base) < mask_limit) ((pdevc)->type = (&gx_dc_type_data_null)); else { switch ( penum->map[0].decoding ) { case sd_none: cc.paint.values[0] = (run) * (1.0f / 255.0f); break; case sd_lookup: cc.paint.values[0] = penum->map[0].decode_lookup[(run) >> 4]; break; case sd_compute: cc.paint.values[0] = penum->map[0].decode_lookup[0] + (run) * penum->map[0].decode_factor; }; code = (*remap_color)(&cc, pcs, pdevc, pis, dev, gs_color_select_source); if (code < 0) goto err; } } else if (!((pdevc)->type == (&gx_dc_type_data_pure))) { code = (*(pdevc)->type->load)(pdevc, pis, dev, gs_color_select_source); if (code < 0) goto err; } } while (0);

                    }
                    code = (*fill_pgram)(dev, xrun, yrun, ((next.x).state).Q - xrun,
                                         ((next.y).state).Q - yrun, pdyx, pdyy,
                                         pdevc, lop);
                    if (code < 0)
                        goto err;
                    yrun = ((next.y).state).Q;
...
# 579 "./base/gximono.c"
      ...
      last:if (stop < endp && (*stop || !masked)) {
            if (!masked) {
                    ...
                    do { pdevc = &penum->clues[*stop].dev_color; if (!((pdevc)->type != (&gx_dc_type_data_none))) { if ((uint)(*stop - mask_base) < mask_limit) ((pdevc)->type = (&gx_dc_type_data_null)); else { switch ( penum->map[0].decoding ) { case sd_none: cc.paint.values[0] = (*stop) * (1.0f / 255.0f); break; case sd_lookup: cc.paint.values[0] = penum->map[0].decode_lookup[(*stop) >> 4]; break; case sd_compute: cc.paint.values[0] = penum->map[0].decode_lookup[0] + (*stop) * penum->map[0].decode_factor; }; code = (*remap_color)(&cc, pcs, pdevc, pis, dev, gs_color_select_source); if (code < 0) goto err; } } else if (!((pdevc)->type == (&gx_dc_type_data_pure))) { code = (*(pdevc)->type->load)(pdevc, pis, dev, gs_color_select_source); if (code < 0) goto err; } } while (0);

            }
            do { uint n_ = (endp - stop); ((next.x).state).Q += ((next.x).step).dQ * (endp - stop); while ( n_-- ) { ((next.x).state).R -= ((next.x).step).dR; if ((signed int)((next.x).state).R < 0) { ((next.x).state).Q ++; ((next.x).state).R += ((next.x).step).N; } } } while (0);
            do { uint n_ = (endp - stop); ((next.y).state).Q += ((next.y).step).dQ * (endp - stop); while ( n_-- ) { ((next.y).state).R -= ((next.y).step).dR; if ((signed int)((next.y).state).R < 0) { ((next.y).state).Q ++; ((next.y).state).R += ((next.y).step).N; } } } while (0);
            code = (*fill_pgram) (dev, xrun, yrun, ((next.x).state).Q - xrun,
                                  ((next.y).state).Q - yrun, pdyx, pdyy, pdevc, lop);
        }
        ...
        int (*fill_proc)(gx_device *dev, int x, int y, int width, int height, gx_color_index color) =
            ((dev)->procs.fill_rectangle);
        int xmin = ((int)((((penum->clip_outer.p.x)+(((1<<8)>>1) - 1L))) >> (8)));
        int xmax = ((int)((((penum->clip_outer.q.x)+(((1<<8)>>1) - 1L))) >> (8)));
        ...
        xrun = xrun - xa + (((1L << 8) >> 1) - 1L);
        (((next.x).state).Q += (xa + (((1L << 8) >> 1) - 1L)));
        xa <<= 1;
        ...
        if (stop > psrc)
            for (;;) {
              ...
              skf:if (psrc[0] == run) {
                ...
                {
                    int xi = ((int)((xrun) >> (8)));
                    int wi = ((int)((((next.x).state).Q) >> (8))) - xi;
                    int xei;
                    ...
                    switch (run) {
                        case 0:
                            if (masked)
                                goto mt;
                            if (!((penum->icolor0)->type == (&gx_dc_type_data_pure)))
                                goto ht;
                            code = (*fill_proc) (dev, xi, yt, wi, iht,
                                                 penum->icolor0->colors.pure);
                            break;
                        case 255:
                            if (!((penum->icolor1)->type == (&gx_dc_type_data_pure)))
                                goto ht;
                            code = (*fill_proc) (dev, xi, yt, wi, iht,
                        ...
                        default:
                          ht:
                            ...
                            code = (*(pdevc)->type->fill_rectangle)(pdevc, xi, yt, wi, iht, dev, lop, ((void*)0));
                  ...
                  uint w, int h, gx_device * dev)
{
    ...
    int contone_stride = 0;
    fixed scale_factor, offset;
    int src_size;
    bool flush_buff = ((bool)0);
    int offset_contone[(8 * 8)];
    ...
    if (h == 0) {
        if (penum->ht_landscape.count == 0 || posture == image_portrait) {
            ...
            penum->ht_landscape.offset_set = ((bool)1);
            flush_buff = ((bool)1);
        }
    }
    ...
    switch (posture) {
        case image_portrait:
            ...
            xrun = ((penum->dda.pixel0.x).state).Q;
            xrun = xrun - penum->adjust + (((1L << 8) >> 1) - 1L);
            dest_width = ((int)(((((penum->x_extent.x) < 0 ? -(penum->x_extent.x) : (penum->x_extent.x)))+((1<<8)>>1)) >> (8)));
            ...
            dest_height = ((int)(((((penum->y_extent.y) < 0 ? -(penum->y_extent.y) : (penum->y_extent.y)))+((1<<8)>>1)) >> (8)));
            scale_factor = ((fixed)floor(((float) src_size / (float) dest_width)*(float)(1<<8) + 0.5));
        ...
        default:
            ...
            vdi = penum->wci;
            contone_stride = penum->line_size;
            dest_width = ((int)(((((penum->y_extent.x) < 0 ? -(penum->y_extent.x) : (penum->y_extent.x)))+((1<<8)>>1)) >> (8)));
            dest_height = ((int)(((((penum->x_extent.y) < 0 ? -(penum->x_extent.y) : (penum->x_extent.y)))+((1<<8)>>1)) >> (8)));
            data_length = dest_height;
            scale_factor = ((fixed)floor(((float) src_size / (float) dest_height)*(float)(1<<8) + 0.5));
...
# 892 "./base/gximono.c"
            if (penum->ht_landscape.count == 0) {
                ...
                if (offset_bits == 0 || offset_bits == 16) {
                    penum->ht_landscape.offset_set = ((bool)0);
    ...
    for (k = 0; k < spp_out; k++) {
        if (posture == image_portrait) {
            ...
            devc_contone[k] = penum->line + offset_contone[k] +
                              ((1 << 3)*8) * k * contone_stride;
        }
    }
    if (flush_buff) goto flush;

    do { do { if ( (data_length) == 0 ) ((dda_ht).step).dQ = 0, ((dda_ht).step).dR = 0; else if ( (src_size) < 0 ) { ((dda_ht).step).dQ = -(int)((uint)-(src_size) / (data_length)); if ( (((dda_ht).step).dR = -(src_size) % (data_length)) != 0 ) --((dda_ht).step).dQ, ((dda_ht).step).dR = (data_length) - ((dda_ht).step).dR; } else { ((dda_ht).step).dQ = (src_size) / (data_length); ((dda_ht).step).dR = (src_size) % (data_length); } ((dda_ht).step).N = (data_length); } while (0); ((dda_ht).state).Q = (0) + (((dda_ht).step).dQ>>1); ((dda_ht).state).R = ((data_length)-1) - ((((dda_ht).step).dR + (((dda_ht).step).dQ & 1 ? ((dda_ht).step).N : 0))>>1); if ((signed)((dda_ht).state).R < 0) { ((dda_ht).state).Q++; ((dda_ht).state).R += ((dda_ht).step).N; }; } while (0);
    devc_contone_gray = devc_contone[0];
    if (penum->color_cache == ((void*)0)) {
        ...
        switch (posture) {
                    ...
                    if (src_size == dest_width) {
                        ...
                        for (k = 0; k < data_length; k++, devc_contone_gray++) {
                            *devc_contone_gray = psrc[dda_ht.state.Q];
                            do { ((dda_ht).state).R -= ((dda_ht).step).dR; if ((signed)((dda_ht).state).R < 0) { ((dda_ht).state).Q++; ((dda_ht).state).R += ((dda_ht).step).N; }; ((dda_ht).state).Q += ((dda_ht).step).dQ; } while (0);
                        }
                    }
                } else {
            ...
            case image_landscape:
                ...
                if (penum->ht_landscape.flipy) {
                    ...
                    } else if (scale_factor == ((1L << 8) >> 1)) {
                        for (k = 0; k < data_length; k+=2) {
                            offset = ((int)(((scale_factor * k)+((1<<8)>>1)) >> (8)));
                            devc_contone_gray[position] =
                                devc_contone_gray[position + ((1 << 3)*8)] = psrc[offset];
                            position += ((1 << 3)*8)*2;
                        }
                    } else {
