...
static gs_ptr_type_t halftone_component_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void halftone_component_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_halftone_component = { sizeof(gs_halftone_component), "gs_halftone_component", 0, 0, halftone_component_enum_ptrs, halftone_component_reloc_ptrs, 0, 0 };
static gs_ptr_type_t ht_comp_elt_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { uint count = size / (uint)sizeof(gs_halftone_component); if ( count == 0 ) return 0; return (*(st_halftone_component).enum_ptrs)(mem, (const char *)vptr + (index % count) * sizeof(gs_halftone_component), sizeof(gs_halftone_component), index / count, pep, &(st_halftone_component), gcst); } static void ht_comp_elt_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { { uint count = size / (uint)sizeof(gs_halftone_component); for ( ; count; count--, vptr = (char *)vptr + sizeof(gs_halftone_component) ) (*(st_halftone_component).reloc_ptrs)(vptr, sizeof(gs_halftone_component), &(st_halftone_component), gcst); } } const gs_memory_struct_type_t st_ht_component_element = { sizeof(gs_halftone_component), "gs_halftone_component[]", 0, 0, ht_comp_elt_enum_ptrs, ht_comp_elt_reloc_ptrs, 0, 0 };



static
gs_ptr_type_t halftone_component_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_halftone_component *hptr = vptr; switch ( index ) { default: return 0;
case 0:
switch (hptr->type)
{
    case ht_type_spot:
...
return (pep->ptr = (&((const gs_halftone_component *)vptr)->params.threshold.thresholds)->data, pep->size = (&((const gs_halftone_component *)vptr)->params.threshold.thresholds)->size, (&ptr_const_string_procs));
...
static void halftone_component_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_halftone_component *hptr = vptr;
{
    switch (hptr->type) {
        case ht_type_spot:
            if (hptr->params.spot.transfer == 0)
                (hptr->params.spot.transfer_closure.data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(hptr->params.spot.transfer_closure.data), gcst)));
            break;
        case ht_type_threshold:
            (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(hptr->params.threshold.thresholds), gcst));
            if (hptr->params.threshold.transfer == 0)
                (hptr->params.threshold.transfer_closure.data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(hptr->params.threshold.transfer_closure.data), gcst)));
            break;
        case ht_type_threshold2:
            ...
            break;
        case ht_type_client_order:
            (hptr->params.client_order.client_data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(hptr->params.client_order.client_data), gcst)));
            (hptr->params.client_order.transfer_closure.data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(hptr->params.client_order.transfer_closure.data), gcst)));
            break;
        default:
            break;
    }
        ...
        case ht_type_colorscreen:
            {
                ...
                pocs = (gx_ht_order_component *)(*(mem)->procs.alloc_struct_array)(mem, 4, &st_ht_order_component_element, "gs_sethalftone");
        ...
        case ht_type_multiple_colorscreen:
            {
                ...
                bool have_Default = ((bool)0);
                uint i;
                gs_halftone_component *phc = pht->params.multiple.components;
                gx_ht_order_component *poc_next;

                pocs = (gx_ht_order_component *)(*(mem)->procs.alloc_struct_array)(mem, count, &st_ht_order_component_element, "gs_sethalftone");
                ...
                if (count == 1) {
                    ...
                    ((mem)->procs.free_object(mem, pocs, "gs_sethalftone"));
                    pdht->components = 0;
                    pdht->num_comp = 0;
                } else {
                    pdht->components = pocs;
                 ...
                 gs_memory_t * mem)
{
    ...
    do { if ( ((pmap) = (gx_transfer_map *)(*(mem)->procs.alloc_struct)(mem, &st_transfer_map, "process_transfer")) == 0 ) { return ((-25)); } else { do { (pmap)->rc.ref_count = 1; (pmap)->rc.memory = mem; (pmap)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
...
# 431 "./base/gsht1.c"
...

    {
        ...
        for (i = 0; i < size; ++i) {
            ...
            mask |= thr;
            max_thr = (((max_thr) > (thr)) ? (max_thr) : (thr));
        }
        if (mask == 0)
            mask = 1, max_thr = 1;
        while (!(mask & 1) || max_thr > (1 << 14))
        ...
        for (row = 0, di = 0; row < d; ++row) {
            ...
            for (dx = 0; dx < sod; dx += w) {
                ...
                for (j = 0; j < w; ++j, ++si, ++di) {
                    ...
                    do { } while (0);
                    bits[di].mask = (((thr) > (1)) ? (thr) : (1));
                }
            }
        }
    }
    gx_ht_complete_threshold_order(porder);
    return process_transfer(porder, pgs, ((void*)0), &phtp->transfer_closure, mem);
                     ...
                     gs_client_order_halftone * phcop, gs_memory_t * mem)
{
    ...
    if (code < 0)
        return code;
    return process_transfer(porder, pgs, ((void*)0),
