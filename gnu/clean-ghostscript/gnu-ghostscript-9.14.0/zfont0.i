...
static int ztype0_define_font(gs_font_dir *, gs_font *);
static int ztype0_make_font(gs_font_dir *, const gs_font *, const gs_matrix *, gs_font **);
...
zbuildfont0(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*op));
    {
        ...
        if (dict_find_string(op, "FMapType", &pfmaptype) <= 0 ||
            !((((const byte *)&((pfmaptype)->tas.type_attrs))[1]) == (t_integer)) ||
            pfmaptype->value.intval < (int)2 ||
            pfmaptype->value.intval > (int)9 ||
            dict_find_string(op, "FDepVector", &pfdepvector) <= 0 ||
            !(((pfdepvector)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))
            )
            return ((-10));
    ...
    data.fdep_size = ((&fdepvector)->tas.rsize);
    for (i = 0; i < data.fdep_size; i++) {
        ref fdep;
        gs_font *psub;

        array_get(((gs_memory_t *)((i_ctx_p->memory).current)), &fdepvector, i, &fdep);
    ...
    switch (data.FMapType) {
        ...
        case fmap_SubsVector:
            {
                ...
                if (dict_find_string(op, "SubsVector", &psubsvector) <= 0 ||
                    !((((const byte *)&((psubsvector)->tas.type_attrs))[1]) == (t_string)) ||
                    (svsize = ((psubsvector)->tas.rsize)) == 0 ||
        ...
        case fmap_CMap:
            code = ztype0_get_cmap(&data.CMap, (const ref *)&fdepvector,
                                   (const ref *)op, ((gs_memory_t *)((i_ctx_p->memory).current)));
            break;
        default:
            ;
    }
...

    {
        ...
        if (dict_find_string(op, "FID", &pfid) <= 0)
            (((&save_FID)->tas.type_attrs = ((t_null) << 8) + (0)) );
        else
            save_FID = *pfid;
    }
    {
    ...
    if (dict_find_string(op, "PrefEnc", &pprefenc) <= 0) {
        ...
        (((&nul)->tas.type_attrs = ((t_null) << 8) + (((&(i_ctx_p->memory))->new_mask))) );
        if ((code = dict_put_string(op, "PrefEnc", &nul, &(i_ctx_p->dict_stack))) < 0)
            goto fail;
    }
    get_GlyphNames2Unicode(i_ctx_p, (gs_font *)pfont, op);

    pdata = ((font_data *)((pfont)->client_data));
    data.encoding_size = ((&pdata->Encoding)->tas.rsize);
# 202 "./psi/zfont0.c"
    if (data.FMapType == fmap_SubsVector) {
        if (data.subs_size >= ((&pdata->Encoding)->tas.rsize)) {
            code = ((-15));
            goto fail;
        }
    }
    data.Encoding =
        (uint *) (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current)), data.encoding_size, sizeof(uint), "buildfont0(Encoding)");
    ...
    for (i = 0; i < data.encoding_size; i++) {
        ...
        array_get(((gs_memory_t *)((i_ctx_p->memory).current)), &pdata->Encoding, i, &enc);
        if (!((((const byte *)&((&enc)->tas.type_attrs))[1]) == (t_integer))) {
            code = ((-20));
            goto fail;
        }
    ...
    data.FDepVector =
        (gs_font * *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct_array)(((gs_memory_t *)((i_ctx_p->memory).current)), data.fdep_size, &st_gs_font_ptr_element, "buildfont0(FDepVector)");
    ...
    for (i = 0; i < data.fdep_size; i++) {
        ...
        data.FDepVector[i] = ((gs_font *)((pfid)->value.pstruct));
    }
    pfont->data = data;
...
fail:
    ...
    if (((((const byte *)&((&save_FID)->tas.type_attrs))[1]) == (t_null))) {
        ref rnfid;

        names_enter_string(pfont->memory->gs_lib_ctx->gs_name_table, "FID", &rnfid);
        dict_undef(op, &rnfid, &(i_ctx_p->dict_stack));
    } else
        dict_put_string(op, "FID", &save_FID, &(i_ctx_p->dict_stack));
    ((pfont->memory)->procs.free_object(pfont->memory, pfont, "buildfont0(font)"));
    return code;
}
...
ztype0_adjust_FDepVector(gs_font_type0 * pfont)
{
    ...
    ref *prdep;
    uint i;
    int code = gs_alloc_ref_array(imem, &newdep, (0x20 + 0x40), fdep_size,
                                  "ztype0_adjust_matrix");

    if (code < 0)
        return code;
    for (prdep = newdep.value.refs, i = 0; i < fdep_size; i++, prdep++) {
        const ref *pdict = (&((font_data *)((pdep[i])->client_data))->dict);

        (*(prdep) = *(pdict));
        ((prdep)->tas.type_attrs |= (imemory_new_mask(imem)));
    }




    return dict_put_string((&((font_data *)((pfont)->client_data))->dict), "FDepVector", &newdep, ((void*)0));
}
static int
                  ...
                  byte * pvalue, int default_value)
{
    ...
    if (dict_find_string(op, kstr, &pentry) <= 0) {
        ...
        ((&ent)->value.intval = (default_value), ((&ent)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        *pvalue = (byte) default_value;
        return dict_put_string(op, kstr, &ent, &(i_ctx_p->dict_stack));
    } else {
        do { do { if ( !((((const byte *)&((&*pentry)->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0); if ( (ulong)(*pentry).value.intval > (255) ) return ((-15)); } while (0);
