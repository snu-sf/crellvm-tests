...
static int z42_enumerate_glyph(gs_font *font, int *pindex, gs_glyph_space_t glyph_space, gs_glyph *pglyph);
static int z42_gdir_enumerate_glyph(gs_font *font, int *pindex, gs_glyph_space_t glyph_space, gs_glyph *pglyph);
static gs_glyph z42_encode_char(gs_font *, gs_char, gs_glyph_space_t);
static int z42_glyph_info(gs_font *font, gs_glyph glyph, const gs_matrix *pmat, int members, gs_glyph_info_t *info);
static int z42_glyph_outline(gs_font *font, int WMode, gs_glyph glyph, const gs_matrix *pmat, gx_path *ppath, double sbw[4]);
static int z42_font_info(gs_font *font, const gs_point *pscale, int members, gs_font_info_t *info);
                       ...
                       build_font_options_t options)
{
    ...
    code = build_proc_name_refs(((gs_memory_t *)((i_ctx_p->memory).current)), &build, bcstr, bgstr);
    if (code < 0)
        return code;
    ...
    if ((code = font_string_array_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, "sfnts", &sfnts)) < 0 ||
    ...
    (*(&pdata->u.type42.sfnts) = *(&sfnts));
    pdata->u.type42.mru_sfnts_index = 0;
    pdata->u.type42.mru_sfnts_pos = 0;
    (((&pdata->u.type42.CIDMap)->tas.type_attrs = ((t_null) << 8) + (((&(i_ctx_p->memory))->new_mask))) );
    (*(&pdata->u.type42.GlyphDirectory) = *(&GlyphDirectory));
    pfont->data.string_proc = z42_string_proc;
    pfont->data.proc_data = (char *)pdata;
    pfont->is_resource = (options & bf_has_font_file ? ((bool)1) : ((bool)0));
...
font_string_array_param(const gs_memory_t *mem, os_ptr op, const char *kstr, ref *psa)
{
    ...
    *psa = *pvsa;
    ...
    if ((code = array_get(mem, pvsa, 0L, &rstr0)) < 0)
        return code;
    if (!((((const byte *)&((&rstr0)->tas.type_attrs))[1]) == (t_string)))
        return ((-20));
    return 0;
}
...
font_GlyphDirectory_param(os_ptr op, ref *pGlyphDirectory)
{
    ...
    if (dict_find_string(op, "GlyphDirectory", &pgdir) <= 0)
        (((pGlyphDirectory)->tas.type_attrs = ((t_null) << 8) + (0)) );
    else if (!((((const byte *)&((pgdir)->tas.type_attrs))[1]) == (t_dictionary)) && !(((pgdir)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return ((-20));
    else
...
glyph_to_index(const gs_font *font, gs_glyph glyph)
{
    ...
    if (glyph >= (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)))
        return glyph;
    names_index_ref(font->memory->gs_lib_ctx->gs_name_table, glyph, &gref);
    if (dict_find(&((font_data *)((font)->client_data))->CharStrings, &gref, &pcstr) > 0 &&
        ((((const byte *)&((pcstr)->tas.type_attrs))[1]) == (t_integer))
        ) {
        gs_glyph index_glyph = pcstr->value.intval + (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));

        if (index_glyph >= (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)) && index_glyph <= ((unsigned long)~0L + (unsigned long)0))
            return index_glyph;
    }
    return (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
}
static uint
z42_get_glyph_index(gs_font_type42 *pfont, gs_glyph glyph)
{
    ...
    return gid - (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
}
                      ...
                      gs_glyph_data_t *pgd)
{
    ...
    if (((((const byte *)&((pgdir)->tas.type_attrs))[1]) == (t_dictionary))) {
        ((&iglyph)->value.intval = (glyph_index), ((&iglyph)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        code = dict_find(pgdir, &iglyph, &pgdef) - 1;
    } else {
    ...
    } else if (!((((const byte *)&((pgdef)->tas.type_attrs))[1]) == (t_string))) {
        return ((-20));
    } else {
        gs_glyph_data_from_string(pgd, pgdef->value.const_bytes, ((pgdef)->tas.rsize),
                                  ((void*)0));
    }
    return 0;
}
                    ...
                    gs_glyph *pglyph)
{
    if (glyph_space == GLYPH_SPACE_INDEX)
        ...
        const ref *pcsdict = &((font_data *)((font)->client_data))->CharStrings;
                         ...
                         gs_glyph_space_t glyph_space, gs_glyph *pglyph)
{
    ...
    if (glyph_space == GLYPH_SPACE_INDEX) {
        pgdict = &((font_data *)((font)->client_data))->u.type42.GlyphDirectory;
        if (!((((const byte *)&((pgdict)->tas.type_attrs))[1]) == (t_dictionary))) {
            ref gdef;
            ...
            for (;; (*pindex)++) {
                ...
                if (!((((const byte *)&((&gdef)->tas.type_attrs))[1]) == (t_null))) {
                    *pglyph = (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)) + (*pindex)++;
                    return 0;
                }
            }
        }
    } else
        pgdict = &((font_data *)((font)->client_data))->CharStrings;

    code = zchar_enumerate_glyph(font->memory, pgdict, pindex, pglyph);
    if (*pindex != 0 && *pglyph >= ((gs_glyph)0x80000000L))
        *pglyph = *pglyph - ((gs_glyph)0x80000000L) + (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
    return code;
}
...
z42_encode_char(gs_font *font, gs_char chr, gs_glyph_space_t glyph_space)
{
    ...
    return (glyph_space == GLYPH_SPACE_INDEX && glyph != ((gs_glyph)0x7fffffff) ?
...
z42_string_proc(gs_font_type42 * pfont, ulong offset, uint length,
                const byte ** pdata)
{
    return string_array_access_proc(pfont->memory, &((font_data *)((pfont)->client_data))->u.type42.sfnts, 2,
                                    offset, length, &((font_data *)((pfont)->client_data))->u.type42.mru_sfnts_index,
                                    &((font_data *)((pfont)->client_data))->u.type42.mru_sfnts_pos, pdata);
}
