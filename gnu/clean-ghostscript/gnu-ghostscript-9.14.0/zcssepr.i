...
zcurrentoverprint(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.boolval = (gs_currentoverprint((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
...
zsetoverprint(i_ctx_t *i_ctx_p)
{
    ...
    gs_setoverprint((i_ctx_p->pgs), op->value.boolval);
...
zcurrentoverprintmode(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.intval = (gs_currentoverprintmode((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zsetoverprintmode(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0 || (code = gs_setoverprintmode((i_ctx_p->pgs), param)) < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
