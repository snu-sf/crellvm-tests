...
static const gc_ptr_element_t client_color_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_client_color *)0)->pattern - (char *)((gs_client_color *)0) )) } }; static const gc_struct_data_t client_color_reloc_ptrs = { (sizeof(client_color_enum_ptrs) / sizeof((client_color_enum_ptrs)[0])), 0, 0, client_color_enum_ptrs }; const gs_memory_struct_type_t st_client_color = { sizeof(gs_client_color), "gs_client_color", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &client_color_reloc_ptrs };
static gs_ptr_type_t transfer_map_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void transfer_map_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_transfer_map = { sizeof(gx_transfer_map), "gx_transfer_map", 0, 0, transfer_map_enum_ptrs, transfer_map_reloc_ptrs, 0, 0 };


static
gs_ptr_type_t transfer_map_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gx_transfer_map *mptr = vptr; switch ( index ) { default: return 0;
case 0: return (pep->ptr = (const void *)((mptr->proc == 0 ? mptr->closure.data : 0)), (&ptr_struct_procs));
} }
static void transfer_map_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gx_transfer_map *mptr = vptr;
{
    if (mptr->proc == 0)
        (((gx_transfer_map *)vptr)->closure.data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gx_transfer_map *)vptr)->closure.data), gcst)));
}
}
...
{
    pcc->paint.values[0] = (pcc->paint.values[0] <= 0.0 ? 0.0 : pcc->paint.values[0] >= 1.0 ? 1.0 : pcc->paint.values[0]);
}
void
gx_restrict01_paint_3(gs_client_color * pcc, const gs_color_space * pcs)
{
    pcc->paint.values[2] = (pcc->paint.values[2] <= 0.0 ? 0.0 : pcc->paint.values[2] >= 1.0 ? 1.0 : pcc->paint.values[2]);
    pcc->paint.values[1] = (pcc->paint.values[1] <= 0.0 ? 0.0 : pcc->paint.values[1] >= 1.0 ? 1.0 : pcc->paint.values[1]);
    pcc->paint.values[0] = (pcc->paint.values[0] <= 0.0 ? 0.0 : pcc->paint.values[0] >= 1.0 ? 1.0 : pcc->paint.values[0]);
}
void
gx_restrict01_paint_4(gs_client_color * pcc, const gs_color_space * pcs)
{
    pcc->paint.values[3] = (pcc->paint.values[3] <= 0.0 ? 0.0 : pcc->paint.values[3] >= 1.0 ? 1.0 : pcc->paint.values[3]);
...
gs_setgray(gs_state * pgs, double gray)
{
    ...
    if ((code = gs_setcolorspace(pgs, pcs)) >= 0) {
        ...
        pcc->paint.values[0] = (gray <= 0.0 ? 0.0 : gray >= 1.0 ? 1.0 : gray);
        pcc->pattern = 0;
...
gs_setrgbcolor(gs_state * pgs, double r, double g, double b)
{
    ...
    if ((code = gs_setcolorspace(pgs, pcs)) >= 0) {
       gs_client_color *pcc = ((pgs)->color[0].ccolor);
        ...
        (*((pgs)->color[0].color_space)->type->adjust_color_count) (((pgs)->color[0].ccolor), ((pgs)->color[0].color_space), -1);
        pcc->paint.values[0] = (r <= 0.0 ? 0.0 : r >= 1.0 ? 1.0 : r);
        pcc->paint.values[1] = (g <= 0.0 ? 0.0 : g >= 1.0 ? 1.0 : g);
        pcc->paint.values[2] = (b <= 0.0 ? 0.0 : b >= 1.0 ? 1.0 : b);
        pcc->pattern = 0;
        ((((pgs)->color[0].dev_color))->type = (&gx_dc_type_data_none));
    }
...
gs_setnullcolor(gs_state * pgs)
{
    if (pgs->in_cachedevice)
        return ((-21));
    gs_setgray(pgs, 0.0);
    ((((pgs)->color[0].dev_color))->type = (&gx_dc_type_data_null));
    return 0;
}
...
{
    return gs_settransfer_remap(pgs, tproc, ((bool)1));
}
int
gs_settransfer_remap(gs_state * pgs, gs_mapping_proc tproc, bool remap)
{
    ...
    do { if (ptran->red) { do { do { } while (0); (ptran->red)->rc.ref_count += (-1); } while (0); if (!(ptran->red)->rc.ref_count) { do { do { } while (0); (ptran->red)->rc.free((ptran->red)->rc.memory, (void *)(ptran->red), "gs_settransfer"); } while (0); (ptran->red) = 0; } else do { } while (0); } } while (0);
    do { if (ptran->green) { do { do { } while (0); (ptran->green)->rc.ref_count += (-1); } while (0); if (!(ptran->green)->rc.ref_count) { do { do { } while (0); (ptran->green)->rc.free((ptran->green)->rc.memory, (void *)(ptran->green), "gs_settransfer"); } while (0); (ptran->green) = 0; } else do { } while (0); } } while (0);
    do { if (ptran->blue) { do { do { } while (0); (ptran->blue)->rc.ref_count += (-1); } while (0); if (!(ptran->blue)->rc.ref_count) { do { do { } while (0); (ptran->blue)->rc.free((ptran->blue)->rc.memory, (void *)(ptran->blue), "gs_settransfer"); } while (0); (ptran->blue) = 0; } else do { } while (0); } } while (0);
    do { if ( (ptran->gray) == 0 || (ptran->gray)->rc.ref_count > 1 || (ptran->gray)->rc.memory != (pgs->memory) ) { gx_transfer_map *new; do { if ( ((new) = (gx_transfer_map *)(*(pgs->memory)->procs.alloc_struct)(pgs->memory, &st_transfer_map, "gs_settransfer")) == 0 ) { goto fail; } else { do { (new)->rc.ref_count = 1; (new)->rc.memory = pgs->memory; (new)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0); if ( ptran->gray ) do { do { } while (0); (ptran->gray)->rc.ref_count += (-1); } while (0); (ptran->gray) = new; } } while (0);
    ...
    return 0;
  fail:
    do { if (ptran->red) do { (ptran->red)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    do { if (ptran->green) do { (ptran->green)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    do { if (ptran->blue) do { (ptran->blue)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    do { if (ptran->gray) do { (ptran->gray)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    return ((-25));
}
...
gx_set_device_color_1(gs_state * pgs)
{
    ...
    ((((pgs)->color[0].dev_color))->colors.pure = (1), (((pgs)->color[0].dev_color))->type = (&gx_dc_type_data_pure)); (((pgs)->color[0].dev_color))->ccolor_valid = ((bool)0);
...
load_transfer_map(gs_state * pgs, gx_transfer_map * pmap, double min_value)
{
    ...
    frac fmin = ((frac)(((min_value) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
    int i;
    ...
    for (i = 0; i < (1 << 8); i++) {
        float fval =
        (*proc) ((float)i / ((1 << 8) - 1), pmap, proc_data);

        values[i] =
            (fval < min_value ? fmin :
             fval >= 1.0 ? ((frac)0x7ff8) :
             ((frac)(((fval) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8)))));
    }
}
