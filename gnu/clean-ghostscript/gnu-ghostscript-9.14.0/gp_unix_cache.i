# 1 "./base/gp_unix_cache.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gp_unix_cache.c" 2
# 19 "./base/gp_unix_cache.c"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 23 "./base/gp_unix_cache.c" 2
# 1 "./obj/gconfigd.h" 1
# 24 "./base/gp_unix_cache.c" 2
# 1 "./base/gp.h" 1
# 23 "./base/gp.h"
# 1 "./base/gstypes.h" 1
# 28 "./base/gstypes.h"
...
# 26 "./base/gp_unix_cache.c" 2
# 35 "./base/gp_unix_cache.c"
...
static char *gp_cache_prefix(void)
{
    ...
    if (gp_getenv("GS_CACHE_DIR", (char *)((void*)0), &plen) < 0) {
        prefix = malloc(plen);
        gp_getenv("GS_CACHE_DIR", prefix, &plen);
        plen--;
    } else {

        prefix = (__extension__ (__builtin_constant_p ("~/.ghostscript/cache/") && ((size_t)(const void *)(("~/.ghostscript/cache/") + 1) - (size_t)(const void *)("~/.ghostscript/cache/") == 1) ? (((const char *) ("~/.ghostscript/cache/"))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen ("~/.ghostscript/cache/") + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, "~/.ghostscript/cache/", __len); __retval; })) : __strdup ("~/.ghostscript/cache/")));
    ...
    if (plen >= 1 && prefix[0] == '~') {
        ...
        if (gp_getenv("HOME", (char *)((void*)0), &hlen) < 0) {
            home = malloc(hlen);
            if (home == ((void*)0)) return prefix;
            ...
            pathlen = hlen + plen + 1;
            path = malloc(pathlen);
            if (path == ((void*)0)) { free(home); return prefix; }
            result = gp_file_name_combine(home, hlen, prefix+2, plen-2, ((bool)0), path, &pathlen);
            if (result == gp_combine_success) {
                free(prefix);
                prefix = path;
            } else {
                (dprintf_file_and_line("./base/gp_unix_cache.c", 106), errprintf_nomem("file_name_combine failed with code %d\n", result));
                free(path);
            }
            free(home);
        }
...
gp_cache_indexfilename(const char *prefix)
{
    ...
    result = gp_file_name_combine(prefix, strlen(prefix), fn, strlen(fn), ((bool)1), path, &len);
    ...
    if (result != gp_combine_success) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 138), errprintf_nomem("pcache: file_name_combine for indexfilename failed with code %d\n", result));
        free(path);
        return ((void*)0);
    }
    return path;
...
static char *gp_cache_itempath(const char *prefix, gp_cache_entry *item)
{
    ...
    result = gp_file_name_combine(prefix, strlen(prefix),
        fn, strlen(fn), ((bool)0), path, &len);

    if (result != gp_combine_success) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 191), errprintf_nomem("pcache: file_name_combine failed on cache item filename with code %d\n", result));
    }
...
static int gp_cache_loaditem(FILE *file, gp_cache_entry *item, gp_cache_alloc alloc, void *userdata)
{
    ...
    if (filekey == ((void*)0)) {
        errprintf_nomem("pcache: couldn't allocate file key!\n");
        return -1;
    }
    ...
    if (item->buffer == ((void*)0)) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 258), errprintf_nomem("pcache: couldn't allocate buffer for file data!\n"));
        item->len = 0;
        return -1;
    }
...
int gp_cache_insert(int type, byte *key, int keylen, void *buffer, int buflen)
{
    ...
    in = gp_fopen(infn, "r");
    if (in == ((void*)0)) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 350), errprintf_nomem("pcache: unable to open '%s'\n", infn));
        free(prefix);
        free(infn);
        free(outfn);
        return -1;
    ...
    if (out == ((void*)0)) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 358), errprintf_nomem("pcache: unable to open '%s'\n", outfn));
        fclose(in);
        free(prefix);
        free(infn);
        free(outfn);
    ...
    gp_cache_alloc alloc, void *userdata)
{
    ...
    in = gp_fopen(infn, "r");
    if (in == ((void*)0)) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 443), errprintf_nomem("pcache: unable to open '%s'\n", infn));
        free(prefix);
        free(infn);
        free(outfn);
        return -1;
    ...
    out = gp_fopen(outfn, "w");
    if (out == ((void*)0)) {
        (dprintf_file_and_line("./base/gp_unix_cache.c", 451), errprintf_nomem("pcache: unable to open '%s'\n", outfn));
        fclose(in);
        free(prefix);
        free(infn);
        free(outfn);
