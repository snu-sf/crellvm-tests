...
const gs_memory_struct_type_t st_gs_dual_memory = { sizeof(gs_dual_memory_t), "gs_dual_memory_t", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
            ...
            bool level2)
{
    ...
    for (i = 0; i < (sizeof(dmem->spaces.memories.indexed) / sizeof((dmem->spaces.memories.indexed)[0])); i++)
        dmem->spaces.memories.indexed[i] = 0;
    dmem->spaces.memories.named.local = ilmem;
    dmem->spaces.memories.named.global = igmem;
    dmem->spaces.memories.named.system = ismem;
...
{
    gs_ref_memory_t *mem = dmem->spaces.memories.indexed[space >> 2];
...
{
    dmem->spaces.memories.named.system->gc_status.requested = 0;
    dmem->spaces.memories.named.global->gc_status.requested = 0;
    dmem->spaces.memories.named.local->gc_status.requested = 0;
}
# 143 "./psi/ialloc.c"
int
gs_register_ref_root(gs_memory_t *mem, gs_gc_root_t *root,
                     void **pp, client_name_t cname)
{
    return (*(mem)->procs.register_root)(mem, root, (&ptr_ref_procs), pp, cname);
}
# 158 "./psi/ialloc.c"
int
...
gs_free_ref_array(gs_ref_memory_t * mem, ref * parr, client_name_t cname)
{
...

    {
        ...
        switch ((((const byte *)&((parr)->tas.type_attrs))[1])) {
            case t_shortarray:
            ...
            case t_mixedarray:{
                ...
                for (; i < num_refs; ++i)
                    p = ((*(const ref_packed *)(p) >= ((ref_packed)(2) << 13)) ? p + 1 : p + (sizeof(ref) / sizeof(ref_packed)));
            ...
            case t_array:
                ...
                break;
            default:
                (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./psi/ialloc.c", 353), errprintf_nomem("Unknown type 0x%x in free_ref_array(%u,0x%lx)!", (((const byte *)&((parr)->tas.type_attrs))[1]), num_refs, (ulong) obj));
