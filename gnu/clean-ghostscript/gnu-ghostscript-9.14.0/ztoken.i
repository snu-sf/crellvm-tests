...
ztoken(i_ctx_t *i_ctx_p)
{
    ...
    switch ((((const byte *)&((op)->tas.type_attrs))[1])) {
        ...
        case t_file: {
            ...
            gs_scanner_init_options(&state, op, 0);
            return token_continue(i_ctx_p, &state, ((bool)1));
        }
        case t_string: {
            ref token;

            int orig_ostack_depth = ref_stack_count(&((i_ctx_p->op_stack).stack)) - 1;
            int code;


            if (!(((op)->tas.type_attrs) & (0x20)))
                return ((-7));
            code = gs_scan_string_token_options(i_ctx_p, op, &token, 0);
            switch (code) {
            case 2:
            ...
            default:
                if (code < 0) {
                    ...
                    if (orig_ostack_depth < ref_stack_count(&((i_ctx_p->op_stack).stack)))
                        ((((i_ctx_p->op_stack).stack).p) -= (ref_stack_count(&((i_ctx_p->op_stack).stack))- orig_ostack_depth));
                    return code;
                }
            }
...
ztoken_continue(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&*op)->value.pstruct) == &st_scanner_state_dynamic) ) return (check_type_failed(&*op));
    pstate = ((scanner_state *)((op)->value.pstruct));
    return token_continue(i_ctx_p, pstate, ((bool)0));
}
...
again:
    ...
    if (code <= 0 && !save) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pstate, "token_continue"));
    }
    return code;
}
...
ztokenexec(i_ctx_t *i_ctx_p)
{
    ...
    gs_scanner_init_options(&state, op, 0);
    return tokenexec_continue(i_ctx_p, &state, ((bool)1));
}
...
ztokenexec_continue(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&*op)->value.pstruct) == &st_scanner_state_dynamic) ) return (check_type_failed(&*op));
    pstate = ((scanner_state *)((op)->value.pstruct));
    return tokenexec_continue(i_ctx_p, pstate, ((bool)0));
}
...
again:
    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (1) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), 1); if ( es_code_ < 0 ) return es_code_; };
    code = gs_scan_token(i_ctx_p, (ref *) ((((i_ctx_p->exec_stack).stack).p) + 1), pstate);
    op = (((i_ctx_p->op_stack).stack).p);
    switch (code) {
        case 0:
            if (((((((i_ctx_p->exec_stack).stack).p) + 1)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80)))) {
                do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
        ...
        case 4:
        case 5:
            return ztoken_handle_comment(i_ctx_p, pstate, (((i_ctx_p->exec_stack).stack).p) + 1, code,
                                         save, ((bool)1), ztokenexec_continue);
        default:
            gs_scanner_error_object(i_ctx_p, pstate, &i_ctx_p->error_object);
            break;
    }
    if (!save) {
        ((((scanner_state_dynamic *)pstate)->mem)->procs.free_object(((scanner_state_dynamic *)pstate)->mem, pstate, "token_continue"));
    }
    return code;
}
                      ...
                      bool save, bool push_file, op_proc_t cont)
{
    ...
    if ((((i_ctx_p->op_stack).stack).top) - (((i_ctx_p->op_stack).stack).p) < 2) {
        code = ref_stack_extend(&((i_ctx_p->op_stack).stack), 2);
        if (code < 0)
            return code;
    }
    ...
    code = names_enter_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, proc_name, (((i_ctx_p->exec_stack).stack).p) + 3);
    if (code < 0)
        return code;
    if (save) {
        pstate = (scanner_state *)(scanner_state_dynamic *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_scanner_state_dynamic, "ztoken_handle_comment");
    ...
    if (!pstate->s_pstack)
        (((i_ctx_p->op_stack).stack).p)[2] = *ptoken;
    ...
    (((((i_ctx_p->exec_stack).stack).p) + 2)->value.pstruct = ((obj_header_t *)(pstate)), (((((i_ctx_p->exec_stack).stack).p) + 2)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
    ppcproc = dstack_find_name_by_index(&(i_ctx_p->dict_stack), names_index(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, (((i_ctx_p->exec_stack).stack).p) + 3));
    if (ppcproc == 0) {
        ...
        if (pstate->s_pstack) {
            ...
            *op = op[-1];
            ...
            op = (((i_ctx_p->op_stack).stack).p) += 2;

        }
        op[-1] = pstate->s_file;
        (((i_ctx_p->exec_stack).stack).p)[3] = *ppcproc;
...
ztoken_scanner_options(const ref *upref, int old_options)
{
    ...
    for (i = 0; i < (sizeof(named_options) / sizeof((named_options)[0])); ++i) {
        ...
        if (code >= 0) {
            if (((((const byte *)&((ppcproc)->tas.type_attrs))[1]) == (t_null)))
                options &= ~pnso->option;
            else
                options |= pnso->option;
        }
...
ztoken_get_scanner_option(const ref *psref, int options, const char **pname)
{
    ...
    for (pnso = named_options + (sizeof(named_options) / sizeof((named_options)[0])); pnso-- != named_options;) {
