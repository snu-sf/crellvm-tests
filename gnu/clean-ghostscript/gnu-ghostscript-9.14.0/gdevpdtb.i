# 1 "./devices/vector/gdevpdtb.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdtb.c" 2
# 18 "./devices/vector/gdevpdtb.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 55 "./base/memory_.h" 2
# 19 "./devices/vector/gdevpdtb.c" 2
# 1 "./base/ctype_.h" 1
# 25 "./base/ctype_.h"
...
# 26 "./base/ctype_.h" 2
# 20 "./devices/vector/gdevpdtb.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
# 21 "./devices/vector/gdevpdtb.c" 2
...
# 23 "./devices/vector/gdevpdtb.c" 2
# 1 "./base/gxfcid.h" 1
# 22 "./base/gxfcid.h"
# 1 "./base/gxcid.h" 1
# 22 "./base/gxcid.h"
...
# 24 "./devices/vector/gdevpdtb.c" 2
...
# 25 "./devices/vector/gdevpdtb.c" 2
...
int psf_convert_type1_to_type2(stream *s, const gs_glyph_data_t *pgd,
                               gs_font_type1 *pfont);
# 28 "./devices/vector/gdevpdtb.c" 2
...
# 29 "./devices/vector/gdevpdtb.c" 2
...
# 30 "./devices/vector/gdevpdtb.c" 2
...
# 31 "./devices/vector/gdevpdtb.c" 2
...
# 32 "./devices/vector/gdevpdtb.c" 2
...
# 34 "./devices/vector/gdevpdtb.c" 2
# 45 "./devices/vector/gdevpdtb.c"
static const gc_ptr_element_t pdf_base_font_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((pdf_base_font_t *)0)->copied - (char *)((pdf_base_font_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((pdf_base_font_t *)0)->complete - (char *)((pdf_base_font_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((pdf_base_font_t *)0)->CIDSet - (char *)((pdf_base_font_t *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((pdf_base_font_t *)0)->FontFile - (char *)((pdf_base_font_t *)0) )) }, { GC_ELT_STRING, ((int) ( (char *)&((pdf_base_font_t *)0)->font_name - (char *)((pdf_base_font_t *)0) )) }};
static const gc_struct_data_t pdf_base_font_data = { (sizeof(pdf_base_font_ptrs) / sizeof((pdf_base_font_ptrs)[0])), 0, 0, pdf_base_font_ptrs }; static const gs_memory_struct_type_t st_pdf_base_font = { sizeof(pdf_base_font_t), "pdf_base_font_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pdf_base_font_data };
...
{
    uint size = pstr->size;
    byte *data = (*(pdev->v_memory)->procs.resize_string)(pdev->v_memory, pstr->data, size, size + 7, "pdf_add_subset_prefix");
                    ...
                    bool is_standard)
{
    ...
    pdf_base_font_t *pbfont =
        (pdf_base_font_t *)(*(mem)->procs.alloc_struct)(mem, &st_pdf_base_font, "pdf_base_font_alloc");

    const gs_font_name *pfname = &font->font_name;
    gs_const_string font_name;
    char fnbuf[2*sizeof(long) + 3];
    ...
    case ft_encrypted2:
        ...
        if(pbfont->num_glyphs > 2048 && !is_standard) {
            ...
            if(pbfont->do_subset != DO_SUBSET_NO){
                char buf[47 + 1];
                int l = (((font->font_name.size) < (sizeof(buf) - 1)) ? (font->font_name.size) : (sizeof(buf) - 1));

                memcpy(buf, font->font_name.chars, l);
                buf[l] = 0;
                (emprintf_program_ident(pdev->memory, gs_program_name(), gs_revision_number()), errprintf(pdev->memory, "Can't embed the complete font %s as it is too large, embedding a subset.\n", buf));
    ...
    cid:
        ...
        pbfont->CIDSet =
            (*(mem)->procs.alloc_bytes)(mem, (pbfont->num_glyphs + 7) / 8, "pdf_base_font_alloc(CIDSet)");
...
# 306 "./devices/vector/gdevpdtb.c"
    ...
    if (pbfont->do_subset != DO_SUBSET_YES && reserve_glyphs == -1) {
        ...
        if (code < 0 && pbfont->do_subset == DO_SUBSET_NO) {
            char buf[47 + 1];
            int l = (((copied->font_name.size) < (sizeof(buf) - 1)) ? (copied->font_name.size) : (sizeof(buf) - 1));

            memcpy(buf, copied->font_name.chars, l);
            buf[l] = 0;
            (emprintf_program_ident(pdev->memory, gs_program_name(), gs_revision_number()), errprintf(pdev->memory, "Can't embed the complete font %s due to font error.\n", buf));
    ...
    pbfont->font_name.data =
        (*(mem)->procs.alloc_string)(mem, font_name.size, "pdf_base_font_alloc(font_name)");
    ...
    *ppbfont = pbfont;
    return 0;
 fail:
    ((mem)->procs.free_object(mem, pbfont, "pdf_base_font_alloc"));
    return code;
}
                         ...
                         gs_font_base *font)
{
    int code =
        gs_copy_glyph_options((gs_font *)font, glyph,
                              ...
                              (pbfont->is_standard ? 2 : 0));

    if (code < 0)
        return code;
    if (pbfont->CIDSet != 0 &&
        (uint)(glyph - ((gs_glyph)0x80000000L)) < pbfont->num_glyphs
        ) {
...
pdf_adjust_font_name(gx_device_pdf *pdev, long id, pdf_base_font_t *pbfont)
{
    ...
    for (i = size;
         i > 0 && ((*__ctype_b_loc ())[(int) ((chars[i - 1]))] & (unsigned short int) _ISxdigit);
         --i)
        do { } while (0);
    if (i < size && i > 0 && chars[i - 1] == '~') {
        do {
            --i;
        } while (i > 0 && chars[i - 1] == '~');
        size = i + 1;
    ...
    gs_sprintf(suffix, "%c%lx", '~', id);
    suffix_size = strlen(suffix);
    data = (*(pdev->v_memory)->procs.resize_string)(pdev->v_memory, chars, size, size + suffix_size, "pdf_adjust_font_name");
                        ...
                        gs_int_rect *FontBBox, gs_id rid, cos_dict_t **ppcd)
{
    ...
    gs_font_base *out_font =
        (do_subset || pbfont->complete == ((void*)0) ? pbfont->copied : pbfont->complete);
    ...
    if (code < 0)
        return code;
    if (pdev->PDFA != 0) {
        stream *s = s_MD5C_make_stream(pdev->v_memory, writer.binary.strm);
...
# 608 "./devices/vector/gdevpdtb.c"
    ...
    case ft_encrypted:
        ...
        if (!pdev->HaveCFF) {
            ...
            code = psf_write_type2_font(writer.binary.strm,
                                        (gs_font_type1 *)out_font,
                                        (1 | 2) |
                            (pdev->CompatibilityLevel < 1.3 ? 4 : 0),
                                        ((void*)0), 0, &fnstr, FontBBox);
            code1 = code;
        }
        goto finish;
    ...
    case ft_TrueType: {
        ...
        code = psf_write_truetype_font(&poss, pfont, options, ((void*)0), 0, &fnstr);
        if (code < 0)
            return code;
    ...
    finish:
        if (pdev->PDFA != 0) {
            (*(writer.binary.strm)->procs.flush)(writer.binary.strm);
