...
static const bool FORCE_STRIP_HALFTONES = ((bool)0);


static gs_ptr_type_t screen_enum_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void screen_enum_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_gs_screen_enum = { sizeof(gs_screen_enum), "gs_screen_enum", 0, 0, screen_enum_enum_ptrs, screen_enum_reloc_ptrs, 0, 0 };


static
gs_ptr_type_t screen_enum_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_screen_enum *eptr = vptr; switch ( index ) { default:
{
    if (index < 1 + 4) {
        gs_ptr_type_t ret =
            (*(st_ht_order).enum_ptrs)(mem, &eptr->order, sizeof(eptr->order), index - 1, pep, &(st_ht_order), gcst);
    ...
    return (*(st_halftone).enum_ptrs)(mem, &eptr->halftone, sizeof(eptr->halftone), index - (1 + 4), pep, &(st_halftone), gcst);

}
case 0: return (pep->ptr = (const void *)(((const gs_screen_enum *)vptr)->pgs), (&ptr_struct_procs));
} }
static void screen_enum_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_screen_enum *eptr = vptr;
{
    (((gs_screen_enum *)vptr)->pgs = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_screen_enum *)vptr)->pgs), gcst)));
    (*(st_halftone).reloc_ptrs)(&eptr->halftone, sizeof(gs_halftone), &(st_halftone), gcst);
    (*(st_ht_order).reloc_ptrs)(&eptr->order, sizeof(gx_ht_order), &(st_ht_order), gcst);
}
}
...
int gs_gshtscr_init(gs_memory_t *);
int
gs_gshtscr_init(gs_memory_t *mem)
{
    gs_setaccuratescreens(mem, ((bool)0));
...
gx_compute_cell_values(gx_ht_cell_params_t * phcp)
{
    ...
    const uint m = ((M) < 0 ? -(M) : (M)), n = ((N) < 0 ? -(N) : (N));
    const uint m1 = ((M1) < 0 ? -(M1) : (M1)), n1 = ((N1) < 0 ? -(N1) : (N1));
...
{
    return (gs_screen_enum *)(*(mem)->procs.alloc_struct)(mem, &st_gs_screen_enum, cname);
}
               ...
               gs_screen_halftone * phsp)
{
    ...
    return gs_screen_init_memory(penum, pgs, phsp, ctx->screen_accurate_screens, pgs->memory);
...
gs_screen_order_alloc(gx_ht_order *porder, gs_memory_t *mem)
{
    ...
    if (!FORCE_STRIP_HALFTONES &&
        ((ulong)porder->params.W1 * ((uint)((((porder->params.W) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) +
...
# 331 "./base/gshtscr.c"
    {
        ...
        gs_distance_transform(uv0.x, uv0.y, pmat, &uv0);
        do { } while (0);
    ...
    better = ((bool)0);
    {
        ...
        for (p.M = m0 + 1; p.M >= m0; p.M--)
            for (p.N = n0 + 1; p.N >= n0; p.N--) {
                ...
                raster = ((uint)((((wt) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
                if (raster > max_size / p.D || raster > (~(-1L << ((1 << 3) * 8 - 1))) / wt)
                    continue;
                wt_size = raster * wt;
                ...
                at = (ar * (180.0 / 3.14159265358979323846) - rotation) * reflection;
...
gs_screen_next(gs_screen_enum * penum, double value)
{
    ...
    if (value < -1.0 || value > 1.0)
        return ((-15));
    sample = (ht_sample_t) ((value + 1) * (ht_sample_t)(((1 << ((sizeof(ht_mask_t) * 8) - 2)) - 1) * 2 + 1));
# 590 "./base/gshtscr.c"
