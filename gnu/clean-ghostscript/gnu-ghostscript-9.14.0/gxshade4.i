               ...
               shading_vertex_t * vertex, patch_color_t *c)
{
    ...
    if (code >= 0 && psh->params.Function) {
        ...
        code = ((psh->params.Function)->head.procs.evaluate)(psh->params.Function, c->t, c->cc.paint.values);
                               ...
                               gx_device * dev, gs_imager_state * pis)
{
    ...
    if (code < 0) {
        if (pfs.icclink != ((void*)0)) gsicc_release_link(pfs.icclink);
        return code;
    }
    reserve_colors(&pfs, C, 3);
    va.c = ca = C[0];
    vb.c = cb = C[1];
    vc.c = cc = C[2];
                               ...
                               gx_device * dev, gs_imager_state * pis)
{
    ...
    const gs_shading_mesh_t *pshm = (const gs_shading_mesh_t *)psh;
    shade_coord_stream_t cs;
    shading_vertex_t *vertex = ((void*)0);
    byte *color_buffer = ((void*)0);
    patch_color_t **color_buffer_ptrs = ((void*)0);
    ...
    if (code < 0)
        goto out;
    reserve_colors(&pfs, &cn, 1);
    next.c = cn;
    shade_next_init(&cs, (const gs_shading_mesh_params_t *)&psh->params,
                    pis);
    vertex = (shading_vertex_t *)
        (*(pis->memory)->procs.alloc_byte_array)(pis->memory, per_row, sizeof(*vertex), "gs_shading_LfGt_render");

    if (vertex == ((void*)0)) {
        code = ((-25));
        goto out;
    }
    color_buffer = (*(pis->memory)->procs.alloc_bytes)(pis->memory, pfs.color_stack_step * per_row, "gs_shading_LfGt_fill_rectangle");
    if (color_buffer == ((void*)0)) {
        code = ((-25));
        goto out;
    }
    color_buffer_ptrs = (patch_color_t **)(*(pis->memory)->procs.alloc_bytes)(pis->memory, sizeof(patch_color_t *) * per_row, "gs_shading_LfGt_fill_rectangle");

    if (color_buffer_ptrs == ((void*)0)) {
        code = ((-25));
        goto out;
    }
...
out:
    if (1 && ((bool)0))
        do { } while (0);
    ((pis->memory)->procs.free_object(pis->memory, vertex, "gs_shading_LfGt_render"));
    ((pis->memory)->procs.free_object(pis->memory, color_buffer, "gs_shading_LfGt_render"));
    ((pis->memory)->procs.free_object(pis->memory, color_buffer_ptrs, "gs_shading_LfGt_render"));
