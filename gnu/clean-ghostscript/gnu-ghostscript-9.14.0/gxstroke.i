...
not_exact:
    if (result) {
        if (!((ppath)->curve_count != 0) && ((ppath)->subpath_count) <= 1 &&
            ...
            if (pis->line_params.curve_join >= 0)
                factor = (((factor) > (join_expansion_factor(pis, (gs_line_join)pis->line_params.curve_join))) ? (factor) : (join_expansion_factor(pis, (gs_line_join)pis->line_params.curve_join)));
...

    {
        float exx = expand * cx;
        float exy = expand * cy;
        int code = (((exx) >= -2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2)) && (exx) < 2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2))) ? (ppt->x = ((fixed)((exx)*(float)(1<<8))), 0) : ((-13)));

        if (code < 0)
            return code;
        code = (((exy) >= -2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2)) && (exy) < 2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2))) ? (ppt->y = ((fixed)((exy)*(float)(1<<8))), 0) : ((-13)));
        if (code < 0)
            return code;
    }
...
typedef struct endpoint_s {
    gs_fixed_point p;
    gs_fixed_point co, ce;
    gs_fixed_point cdelta;
} endpoint;
...
# 369 "./base/gxstroke.c"
typedef int (*stroke_line_proc_t)(gx_path *, gx_path *, bool ensure_closed, int, pl_ptr, pl_ptr, const gx_device_color *, gx_device *, const gs_imager_state *, const gx_stroke_params *, const gs_fixed_rect *, int, gs_line_join, bool, note_flags);

static int stroke_add(gx_path *, gx_path *, bool ensure_closed, int, pl_ptr, pl_ptr, const gx_device_color *, gx_device *, const gs_imager_state *, const gx_stroke_params *, const gs_fixed_rect *, int, gs_line_join, bool, note_flags);
static int stroke_add_compat(gx_path *, gx_path *, bool ensure_closed, int, pl_ptr, pl_ptr, const gx_device_color *, gx_device *, const gs_imager_state *, const gx_stroke_params *, const gs_fixed_rect *, int, gs_line_join, bool, note_flags);
static int stroke_add_fast(gx_path *, gx_path *, bool ensure_closed, int, pl_ptr, pl_ptr, const gx_device_color *, gx_device *, const gs_imager_state *, const gx_stroke_params *, const gs_fixed_rect *, int, gs_line_join, bool, note_flags);
static int stroke_fill(gx_path *, gx_path *, bool ensure_closed, int, pl_ptr, pl_ptr, const gx_device_color *, gx_device *, const gs_imager_state *, const gx_stroke_params *, const gs_fixed_rect *, int, gs_line_join, bool, note_flags);
...
gx_join_path_and_reverse(gx_path * path, gx_path * rpath)
{
    ...
    if (((rpath)->segments != 0 && (rpath)->segments->contents.subpath_first == 0))
        return 0;
    ...
    (void)gx_path_init_local_shared(rpath, ((void*)0), path->memory);

    return gx_path_close_subpath_notes(path, sn_none);
}
# 420 "./base/gxstroke.c"
static int
                 ...
                 const gx_device_color * pdevc, const gx_clip_path * pcpath)
{
    ...
    stroke_line_proc_t line_proc =
               ((to_path == 0 && !gx_dc_is_pattern1_color_clist_based(pdevc))
                      ? ((!( (~pis->log_op) & ((pis->log_op) << 1) & rop3_D ) && !(pis->log_op & 0x400)) ? stroke_fill : stroke_add) :
...
# 480 "./base/gxstroke.c"
    ...
    double device_line_width_scale = 0;
    double device_dot_length = pgs_lp->dot_length * (1L << 8);
    const subpath *psub;
    gs_matrix initial_matrix;
    bool initial_matrix_reflected;
    note_flags flags;

    (*((pdev)->procs.get_initial_matrix)) (pdev, &initial_matrix);
...
# 520 "./base/gxstroke.c"
...

    {
        ...
        if (gx_stroke_path_expansion(pis, ppath, &expansion) < 0) {
            ...
            ibox.p.x = ibox.p.y = (-1 << ((1 << 2) * 8 - 1));
            ibox.q.x = ibox.q.y = (~(-1 << ((1 << 2) * 8 - 1)));
        } else {
            ...
            ibox.p.x = (ibox.p.x < (-1 << ((1 << 2) * 8 - 1)) + expansion.x ? (-1 << ((1 << 2) * 8 - 1)) :
                        ibox.p.x - expansion.x);
            ibox.p.y = (ibox.p.y < (-1 << ((1 << 2) * 8 - 1)) + expansion.y ? (-1 << ((1 << 2) * 8 - 1)) :
                        ibox.p.y - expansion.y);
            ibox.q.x = (ibox.q.x > (~(-1 << ((1 << 2) * 8 - 1))) - expansion.x ? (~(-1 << ((1 << 2) * 8 - 1))) :
                        ibox.q.x + expansion.x);
            ibox.q.y = (ibox.q.y > (~(-1 << ((1 << 2) * 8 - 1))) - expansion.y ? (~(-1 << ((1 << 2) * 8 - 1))) :
    ...
    if (pcpath)
        ...
        (*((pdev)->procs.get_clipping_box)) (pdev, &cbox);
    else {

        cbox = ibox;
    }
    if (!((ibox).q.y <= (cbox).q.y && (ibox).q.x <= (cbox).q.x && (ibox).p.y >= (cbox).p.y && (ibox).p.x >= (cbox).p.x)) {
        ...
        if (pcpath) {
            ...
            do { if ((cbox).p.x > (ibox).p.x) (ibox).p.x = (cbox).p.x; if ((cbox).q.x < (ibox).q.x) (ibox).q.x = (cbox).q.x; if ((cbox).p.y > (ibox).p.y) (ibox).p.y = (cbox).p.y; if ((cbox).q.y < (ibox).q.y) (ibox).q.y = (cbox).q.y; } while (0);
...
# 591 "./base/gxstroke.c"
    ...
    line_width_and_scale = line_width * (double)((fixed)(1)<<8);
    if (((line_width) == 0.0))
        always_thin = ((bool)1);
    else {
        float xa, ya;
        ...
        switch (orient) {
            ...
            case orient_landscape:
              ...
              sat:
                ...
                if (ya < 0)
                    ya = -ya;
                always_thin = ((((xa) > (ya)) ? (xa) : (ya)) * line_width < 0.5);
...
# 636 "./base/gxstroke.c"
    ...
    if (!((ppath)->curve_count != 0) && !gx_path_has_long_segments(ppath)) {

        if (!ppath->segments->contents.subpath_first)
            return 0;
        spath = ppath;
    } else {
        (void)gx_path_init_local_shared(&fpath, ((void*)0), ppath->memory);
        if ((code = gx_path_copy_reducing(ppath, &fpath, ((fixed)((params->flatness)*(float)(1<<8))), pis, (pis->accurate_curves ? pco_accurate | pco_for_stroke : pco_for_stroke))) < 0
    ...
    if (dash_count) {
        ...
        if (expand_squared*65536.0f >= (float)(adjust*adjust)) {
            (void)gx_path_init_local_shared(&dpath, ((void*)0), ppath->memory);
    ...
    if (to_path == 0) {
        ...
        (void)gx_path_init_local_shared(&stroke_path_body, ((void*)0), ppath->memory);
    }
    if (line_proc == stroke_add_fast) {
        to_path_reverse = &stroke_path_reverse;
        (void)gx_path_init_local_shared(&stroke_path_reverse, ((void*)0), ppath->memory);
    }
    for (psub = spath->segments->contents.subpath_first; psub != 0;) {
        int index = 0;
        ...
        d1:if (pseg->type == s_dash) {
          ...
          d:flags = (((pseg->notes & sn_not_first) ?
            ...
            if (!(udx | udy) || pseg->type == s_dash || pseg->type == s_gap) {
                ...
                if ((udx | udy) == 0) {
                    if (((pgs_lp->dot_orientation.xy) == 0.0)) {
                ...
                if (sx == x && sy == y && (pseg == ((void*)0) || pseg->type == s_start)) {
                    double scale = device_dot_length /
                                sqrt((double)((double)udx)*((double)udx)+(double)((double)udy)*((double)udy));
                    fixed udx1, udy1;
                    ...
                    if (!always_thin && (pgs_lp->start_cap == gs_cap_butt ||
                        ...
                        fixed dmax = (((((udx) < 0 ? -(udx) : (udx))) > (((udy) < 0 ? -(udy) : (udy)))) ? (((udx) < 0 ? -(udx) : (udx))) : (((udy) < 0 ? -(udy) : (udy))));

                        if (dmax * scale < (1L << 8))
                            scale = (float)(1L << 8) / dmax;
                    }
            ...
            if (always_thin) {
                ...
                if (uniform != 0) {
                    ...
                    sqrt((double)(dpx)*(dpx)+(double)(dpy)*(dpy));
                    ...
                    if (code < 0) {
                        ...
                        wl = line_width_and_scale /
                            sqrt((double)(dpt.x)*(dpt.x)+(double)(dpt.y)*(dpt.y));
...
# 918 "./base/gxstroke.c"
                ...
                if (!pl.thin) {
                    if (index)
                        dev->sgr.stroke_stored = ((bool)0);
                    adjust_stroke(dev, &pl, pis, ((bool)0),
                            (pseg->prev == 0 || pseg->prev->type == s_start) &&
                            (pseg->next == 0 || pseg->next->type == s_start) &&
                            (zero_length || !is_closed),
                            (((flags>>4) | ((flags) & nf_some_from_arc)) & (((flags) & nf_all_from_arc) ? ~0 : ~nf_all_from_arc)));
                    compute_caps(&pl);
                }
            }
            if (index++) {
                ...
                ensure_closed = ((to_path == &stroke_path_body &&
                                  (!( (~pis->log_op) & ((pis->log_op) << 1) & rop3_D ) && !(pis->log_op & 0x400))) ||
                                 (lptr == ((void*)0) ? ((bool)1) : lptr->thin));
                ...
                if (code < 0)
                    goto exit;
                if(to_path==&stroke_path_body && !((&stroke_path_body)->segments != 0 && (&stroke_path_body)->segments->contents.subpath_first == 0) && (((bool)0) || (!( (~pis->log_op) & ((pis->log_op) << 1) & rop3_D ) && !(pis->log_op & 0x400)))) { fill_params.adjust.x = (always_thin ? 0L : (pis)->fill_adjust.x); fill_params.adjust.y = (always_thin ? 0L : (pis)->fill_adjust.y); if (to_path_reverse != ((void*)0)) { code = gx_join_path_and_reverse(to_path, to_path_reverse); if(code < 0) goto exit; } code = (*((pdev)->procs.fill_path))(pdev, pis, to_path, &fill_params, pdevc, pcpath); gx_path_free(&stroke_path_body, "fill_stroke_path"); if ( code < 0 ) goto exit; (void)gx_path_init_local_shared(&stroke_path_body, ((void*)0), ppath->memory); };
            } else if (pseg->type == s_gap) {
        ...
        if (index) {
            ...
            segment_notes notes = (pseg == 0 ?
                                   (const segment *)spath->segments->contents.subpath_first :
                                   pseg)->notes;
            ...
            flags = (((notes & sn_not_first) ?
                     ...
                     (flags & ~nf_all_from_arc));
            code = (*line_proc) (to_path, to_path_reverse, ((bool)1),
    ...
    if (to_path_reverse != ((void*)0))
        code = gx_join_path_and_reverse(to_path, to_path_reverse);
    if(to_path==&stroke_path_body && !((&stroke_path_body)->segments != 0 && (&stroke_path_body)->segments->contents.subpath_first == 0) && (((bool)1) || (!( (~pis->log_op) & ((pis->log_op) << 1) & rop3_D ) && !(pis->log_op & 0x400)))) { fill_params.adjust.x = (always_thin ? 0L : (pis)->fill_adjust.x); fill_params.adjust.y = (always_thin ? 0L : (pis)->fill_adjust.y); if (to_path_reverse != ((void*)0)) { code = gx_join_path_and_reverse(to_path, to_path_reverse); if(code < 0) goto exit; } code = (*((pdev)->procs.fill_path))(pdev, pis, to_path, &fill_params, pdevc, pcpath); gx_path_free(&stroke_path_body, "fill_stroke_path"); if ( code < 0 ) goto exit; (void)gx_path_init_local_shared(&stroke_path_body, ((void*)0), ppath->memory); };
  exit:
    if (dev == (gx_device *)&cdev)
        cdev.target->sgr = cdev.sgr;
    if (to_path == &stroke_path_body)
        gx_path_free(&stroke_path_body, "gx_stroke_path_only error");
...
width_is_thin(pl_ptr plp)
{
    ...
    if ((dy = plp->vector.y) == 0)
        return ((wy) < 0 ? -(wy) : (wy)) < ((1L << 8) >> 1);
    if ((dx = plp->vector.x) == 0)
        return ((wx) < 0 ? -(wx) : (wx)) < ((1L << 8) >> 1);





    if (((wx) < 0 ? -(wx) : (wx)) < ((1L << 8) >> 1) && ((wy) < 0 ? -(wy) : (wy)) < ((1L << 8) >> 1))
        return ((bool)1);
# 1117 "./base/gxstroke.c"
    {
        double C = dx, D = dy;
        double num = C * wy - D * wx;
        double denom = sqrt((double)(C)*(C)+(double)(D)*(D));
...
adjust_stroke_transversal(pl_ptr plp, const gs_imager_state * pis, bool thin, bool horiz)
{
    ...
    if (horiz) {
        ...
        adj2 = (thin ? 0L : (pis)->fill_adjust.y) << 1;
    } else {

        pw = &plp->width.x, pov = &plp->o.p.x, pev = &plp->e.p.x;
        adj2 = (thin ? 0L : (pis)->fill_adjust.x) << 1;
    }
    ...
    if (*pov == *pev) {
        ...
        if (w >= 0)
            ...
            w2 = adj2 - w2;
        if (w2 & (1L << 8))
            *pov = *pev = ((*pov)&(-1L<<8)) + ((1L << 8) >> 1);
        else
            *pov = *pev = (((*pov)+((1<<8)>>1))&(-1L<<8));
                        ...
                        gs_line_cap start_cap, gs_line_cap end_cap)
{
    ...
    if (*pow == *pew) {
        ...
        fixed length = ((*pov - *pev) < 0 ? -(*pov - *pev) : (*pov - *pev));
        fixed length_r, length_r_2;
        fixed mv = (*pov + *pev) / 2, mv_r;
        fixed adj2 = (horiz ? (thin ? 0L : (pis)->fill_adjust.x)
                            : (thin ? 0L : (pis)->fill_adjust.y)) << 1;
        ...
        if (start_cap == gs_cap_butt || end_cap == gs_cap_butt) {
            length_r = (((length)+((1<<8)>>1))&(-1L<<8));
            if (length_r < (1L << 8))
                length_r = (1L << 8);
            length_r_2 = length_r / 2;
        } else {

            fixed width = ((horiz ? plp->width.y : plp->width.x) < 0 ? -(horiz ? plp->width.y : plp->width.x) : (horiz ? plp->width.y : plp->width.x));

            length_r = (((length + width * 2 + adj2)+((1<<8)>>1))&(-1L<<8));
            length_r_2 = (((length)+((1<<8)>>1))&(-1L<<8)) / 2;
        }
        if (length_r & (1L << 8))
            mv_r = ((mv)&(-1L<<8)) + ((1L << 8) >> 1);
        else
              ...
              bool thin, bool adjust_longitude, note_flags flags)
{
    ...
    if (dev->sgr.stroke_stored &&
        ...
        if ((int64_t)(plp->o.p.x - dev->sgr.orig[0].x) * plp->vector.x ==
            ...
            if (((plp->o.p.x - dev->sgr.orig[0].x) < 0 ? -(plp->o.p.x - dev->sgr.orig[0].x) : (plp->o.p.x - dev->sgr.orig[0].x)) <= ((plp->width.x + dev->sgr.orig[2].x) < 0 ? -(plp->width.x + dev->sgr.orig[2].x) : (plp->width.x + dev->sgr.orig[2].x)) &&
                ((plp->o.p.y - dev->sgr.orig[0].y) < 0 ? -(plp->o.p.y - dev->sgr.orig[0].y) : (plp->o.p.y - dev->sgr.orig[0].y)) <= ((plp->width.y + dev->sgr.orig[2].y) < 0 ? -(plp->width.y + dev->sgr.orig[2].y) : (plp->width.y + dev->sgr.orig[2].y)) &&
                ((plp->e.p.x - dev->sgr.orig[1].x) < 0 ? -(plp->e.p.x - dev->sgr.orig[1].x) : (plp->e.p.x - dev->sgr.orig[1].x)) <= ((plp->width.x + dev->sgr.orig[2].x) < 0 ? -(plp->width.x + dev->sgr.orig[2].x) : (plp->width.x + dev->sgr.orig[2].x)) &&
                ((plp->e.p.y - dev->sgr.orig[1].y) < 0 ? -(plp->e.p.y - dev->sgr.orig[1].y) : (plp->e.p.y - dev->sgr.orig[1].y)) <= ((plp->width.y + dev->sgr.orig[2].y) < 0 ? -(plp->width.y + dev->sgr.orig[2].y) : (plp->width.y + dev->sgr.orig[2].y))) {

                if (((plp->o.p.x - dev->sgr.orig[0].x) < 0 ? -(plp->o.p.x - dev->sgr.orig[0].x) : (plp->o.p.x - dev->sgr.orig[0].x)) >= ((plp->width.x + dev->sgr.orig[2].x) < 0 ? -(plp->width.x + dev->sgr.orig[2].x) : (plp->width.x + dev->sgr.orig[2].x)) / 2 &&
                    ((plp->o.p.y - dev->sgr.orig[0].y) < 0 ? -(plp->o.p.y - dev->sgr.orig[0].y) : (plp->o.p.y - dev->sgr.orig[0].y)) >= ((plp->width.y + dev->sgr.orig[2].y) < 0 ? -(plp->width.y + dev->sgr.orig[2].y) : (plp->width.y + dev->sgr.orig[2].y)) / 2 &&
                    ((plp->e.p.x - dev->sgr.orig[1].x) < 0 ? -(plp->e.p.x - dev->sgr.orig[1].x) : (plp->e.p.x - dev->sgr.orig[1].x)) >= ((plp->width.x + dev->sgr.orig[2].x) < 0 ? -(plp->width.x + dev->sgr.orig[2].x) : (plp->width.x + dev->sgr.orig[2].x)) / 2 &&
                    ((plp->e.p.y - dev->sgr.orig[1].y) < 0 ? -(plp->e.p.y - dev->sgr.orig[1].y) : (plp->e.p.y - dev->sgr.orig[1].y)) >= ((plp->width.y + dev->sgr.orig[2].y) < 0 ? -(plp->width.y + dev->sgr.orig[2].y) : (plp->width.y + dev->sgr.orig[2].y)) / 2) {

                    if (!(((plp->o.p.x - dev->sgr.adjusted[0].x) < 0 ? -(plp->o.p.x - dev->sgr.adjusted[0].x) : (plp->o.p.x - dev->sgr.adjusted[0].x)) <= ((plp->width.x + dev->sgr.adjusted[2].x) < 0 ? -(plp->width.x + dev->sgr.adjusted[2].x) : (plp->width.x + dev->sgr.adjusted[2].x)) &&
                          ((plp->o.p.y - dev->sgr.adjusted[0].y) < 0 ? -(plp->o.p.y - dev->sgr.adjusted[0].y) : (plp->o.p.y - dev->sgr.adjusted[0].y)) <= ((plp->width.y + dev->sgr.adjusted[2].y) < 0 ? -(plp->width.y + dev->sgr.adjusted[2].y) : (plp->width.y + dev->sgr.adjusted[2].y)) &&
                          ((plp->e.p.x - dev->sgr.adjusted[1].x) < 0 ? -(plp->e.p.x - dev->sgr.adjusted[1].x) : (plp->e.p.x - dev->sgr.adjusted[1].x)) <= ((plp->width.x + dev->sgr.adjusted[2].x) < 0 ? -(plp->width.x + dev->sgr.adjusted[2].x) : (plp->width.x + dev->sgr.adjusted[2].x)) &&
                          ((plp->e.p.y - dev->sgr.adjusted[1].y) < 0 ? -(plp->e.p.y - dev->sgr.adjusted[1].y) : (plp->e.p.y - dev->sgr.adjusted[1].y)) <= ((plp->width.y + dev->sgr.adjusted[2].y) < 0 ? -(plp->width.y + dev->sgr.adjusted[2].y) : (plp->width.y + dev->sgr.adjusted[2].y)))) {
                        ...
                        fixed shift_e_x = (dev->sgr.adjusted[1].x - dev->sgr.orig[1].x);
    ...
    if ((start_cap == gs_cap_butt) || (end_cap == gs_cap_butt)) {
        ...
        dev->sgr.stroke_stored = ((bool)0);
    if (adjust) {
        horiz = (((plp->width.x) < 0 ? -(plp->width.x) : (plp->width.x)) <= ((plp->width.y) < 0 ? -(plp->width.y) : (plp->width.y)));
                  ...
                  p_ptr pi)
{
    ...
    double f1;
    double max_result = ((denom) < 0 ? -(denom) : (denom)) * (double)(~(-1 << ((1 << 2) * 8 - 1)));
# 1378 "./base/gxstroke.c"
    if (((xdiff) < 0 ? -(xdiff) : (xdiff)) >= max_result || ((ydiff) < 0 ? -(ydiff) : (ydiff)) >= max_result) {
...
set_thin_widths(register pl_ptr plp)
{
    ...
    if (((dx) < 0 ? -(dx) : (dx)) > ((dy) < 0 ? -(dy) : (dy))) {
        ...
        plp->width.y = plp->e.cdelta.x = ((dx) >= 0 ? (((1L << 8) >> 1)) : -(((1L << 8) >> 1)));
    } else {
        plp->width.y = plp->e.cdelta.x = 0;
        plp->width.x = -(plp->e.cdelta.y = ((dy) >= 0 ? (((1L << 8) >> 1)) : -(((1L << 8) >> 1))));
    }
            ...
            note_flags flags)
{
    ...
    if (plp->thin) {
        ...
        return (*((dev)->procs.draw_thin_line))(dev, lix, liy, litox, litoy,
                                                pdevc, pis->log_op,
                                                pis->fill_adjust.x,
                                                pis->fill_adjust.y);
    }
        ...
        if (!plp->thin && (nplp == 0 || !nplp->thin)
            ...
            if (nplp == 0)
                ...
                code = line_join_points(pgs_lp, plp, nplp, points + npoints,
                                        (uniform ? (gs_matrix *) 0 :
                                         &(*(const gs_matrix *)&(pis)->ctm)), join, reflected);
            if (code < 0)
                goto general;



            if ((((ax = points[0].x - points[1].x) ^ points[0].x) < 0 && (points[0].x ^ points[1].x) < 0) ||
                (((ay = points[0].y - points[1].y) ^ points[0].y) < 0 && (points[0].y ^ points[1].y) < 0) ||
                (((bx = points[2].x - points[1].x) ^ points[2].x) < 0 && (points[2].x ^ points[1].x) < 0) ||
                (((by = points[2].y - points[1].y) ^ points[2].y) < 0 && (points[2].y ^ points[1].y) < 0)
                )
                goto general;
            ...
            if (nplp != 0) {
                if (join == gs_join_miter) {
                    ...
                    code = (*((dev)->procs.fill_triangle)) (dev,
                                                         bevel->x, bevel->y,
            ...
            return (*((dev)->procs.fill_parallelogram)) (dev,
                                                   points[1].x, points[1].y,
                                                         ax, ay, bx, by,
                                                         pdevc, pis->log_op);
          fill:
            code = add_points(ppath, points, npoints + code, ((bool)1));
            if (code < 0)
                return code;
            return gx_path_close_subpath_notes(ppath, sn_none);
        }
    }
           ...
           gs_line_join join, bool reflected, note_flags flags)
{
    ...
    if (plp->thin) {
        ...
        set_thin_widths(plp);
        adjust_stroke(dev, plp, pis, ((bool)1), first == 0 && nplp == 0, flags);
        compute_caps(plp);
    }
    ...
    if (nplp == 0) {
        ...
        if (end_cap == gs_cap_round) {
            ((&points[npoints])->x = (plp->e.co).x, (&points[npoints])->y = (plp->e.co).y);
            do { } while (0);
            ++npoints;
        ...
        if ((code = add_points(ppath, points, npoints, moveto_first)) < 0)
            return code;
        code = add_pie_join(ppath, plp, nplp, reflected, ((bool)1));
        goto done;
        ...
        if ((code = add_points(ppath, points, npoints, moveto_first)) < 0)
            return code;
        code = add_pie_join(ppath, plp, nplp, reflected, ((bool)0));
        goto done;
    } else
       code = line_join_points(pgs_lp, plp, nplp, points + npoints,
                                (uniform ? (gs_matrix *) 0 : &(*(const gs_matrix *)&(pis)->ctm)),
                                join, reflected);
    if (code < 0)
        return code;
                ...
                gs_line_join join, bool reflected, note_flags flags)
{
    ...
    int nrpoints = 0;
    int code;
    bool moveto_first = ((bool)0);
    bool rmoveto_first = ((bool)0);
    ...
    if (((rpath)->segments != 0 && (rpath)->segments->contents.subpath_first == 0) && (first != 0)) {
        first = 0;
        start_cap = gs_cap_butt;
        end_cap = gs_cap_butt;
        moveto_first = ((bool)1);
        rmoveto_first = ((bool)1);
    }
    if (first == 0) {
        ...
        rmoveto_first = ((bool)1);
        ((&rpoints[0])->x = (plp->o.co).x, (&rpoints[0])->y = (plp->o.co).y);
        nrpoints = 1;
    }

    ((&points [npoints ])->x = (plp->e.co).x, (&points [npoints ])->y = (plp->e.co).y);
    ((&rpoints[nrpoints])->x = (plp->e.ce).x, (&rpoints[nrpoints])->y = (plp->e.ce).y);
    npoints++;
    nrpoints++;
    if ((code = add_points(ppath, points, npoints, moveto_first)) < 0)
        return code;
    ...
    if (nplp == 0) {
        ...
        l = (double)(plp->width.x) * (nplp->width.y) ;
        r = (double)(nplp->width.x) * (plp->width.y) ;
        ...
        if ((l == r) && (join == gs_join_round)) {
            ...
            if (code >= 0) {
                ...
                if ((double)(plp->width.x) * nplp->width.x + (double)plp->width.y * nplp->width.y >= 0)
                    code = gx_path_add_line_notes(ppath, plp->e.co.x, plp->e.co.y, sn_none);
            }
...
# 1745 "./base/gxstroke.c"
                code = gx_path_add_line_notes(rpath, nplp->o.p.x, nplp->o.p.y, sn_none);
                if (code < 0)
                    return code;
            }
            code = gx_path_add_line_notes(rpath, nplp->o.co.x, nplp->o.co.y, sn_none);
        } else {
            ...
            if (!(flags & nf_some_from_arc)) {
# 1785 "./base/gxstroke.c"
                code = gx_path_add_line_notes(ppath, nplp->o.p.x, nplp->o.p.y, sn_none);
                if (code < 0)
                    return code;
            }
            code = gx_path_add_line_notes(ppath, nplp->o.ce.x, nplp->o.ce.y, sn_none);
        }
    }
    if (code < 0)
        return code;
    if (npoints > 0) {
        code = add_points(ppath, points, npoints, ((bool)0));
        if (code < 0)
            return code;
    }
    if (nrpoints > 0) {
        code = add_points(rpath, rpoints, nrpoints, ((bool)0));
        if (code < 0)
            return code;
    }
    do { } while (0);
                  ...
                  gs_line_join join, bool reflected, note_flags flags)
{
    ...
    int npoints;
    bool const moveto_first = ((bool)1);
    int code;
    ...
    ((&points[0])->x = (plp->o.ce).x, (&points[0])->y = (plp->o.ce).y);
    ((&points[1])->x = (plp->e.co).x, (&points[1])->y = (plp->e.co).y);
    ((&points[2])->x = (plp->e.ce).x, (&points[2])->y = (plp->e.ce).y);
    ((&points[3])->x = (plp->o.co).x, (&points[3])->y = (plp->o.co).y);
    ...
    if (nplp == 0) {
        ...
        ((&points[0])->x = (plp->e.ce).x, (&points[0])->y = (plp->e.ce).y);
        ++npoints;
        ((&points[npoints])->x = (plp->e.co).x, (&points[npoints])->y = (plp->e.co).y);
        ++npoints;
        ...
        if (ccw ^ reflected) {
            ((&points[0])->x = (plp->e.co).x, (&points[0])->y = (plp->e.co).y);
            do { } while (0);
            ++npoints;
            code = line_join_points(pgs_lp, plp, nplp, points + npoints,
                                    (uniform ? (gs_matrix *) 0 : &(*(const gs_matrix *)&(pis)->ctm)),
           ...
           const gs_imager_state * pis)
{
    ...
    if (plp->thin) {
        ...
        set_thin_widths(plp);
        adjust_stroke(dev, plp, pis, ((bool)1), adlust_longitude, 0);
        compute_caps(plp);
    }
    ...
    if (pgs_lp->start_cap == gs_cap_round) {
        ...
        ((&points[0])->x = (plp->o.co).x, (&points[0])->y = (plp->o.co).y);
        ++npoints;
        if ((code = cap_points(pgs_lp->start_cap, &plp->o, points + npoints)) < 0)
            return npoints;
        npoints += code;
        ((&points[npoints])->x = (plp->o.ce).x, (&points[npoints])->y = (plp->o.ce).y);
        ++npoints;
        code = add_points(ppath, points, npoints, ((bool)1));
        if (code < 0)
            return code;
        return gx_path_close_subpath_notes(ppath, sn_none);
    }
           ...
           bool moveto_first)
{
    ...
    if (moveto_first) {
        ...
        if (code < 0)
            return code;
        do { } while (0);
        return gx_path_add_lines_notes(ppath, points + 1, npoints - 1, sn_none);
    } else {
        do { } while (0);
        return gx_path_add_lines_notes(ppath, points, npoints, sn_none);
    }
}
...
# 2008 "./base/gxstroke.c"
    ...
    if (pmat) {
        ...
        if (code < 0)
            return code;
        v2 = pt.x, u2 = pt.y;
# 2034 "./base/gxstroke.c"
        ccw0 = v1 * u2 < v2 * u1;
# 2048 "./base/gxstroke.c"
    }
    num = u1 * v2 - u2 * v1;
    denom = u1 * u2 + v1 * v2;
...
# 2174 "./base/gxstroke.c"
    ...
    ((&join_points[0])->x = (plp->e.co).x, (&join_points[0])->y = (plp->e.co).y);
    ((&join_points[3])->x = (plp->e.ce).x, (&join_points[3])->y = (plp->e.ce).y);
    ...
    if (!ccw) {
        outp = &join_points[3];
        ((&join_points[2])->x = (nplp->o.co).x, (&join_points[2])->y = (nplp->o.co).y);
        ((&join_points[1])->x = (nplp->o.p).x, (&join_points[1])->y = (nplp->o.p).y);
        np = &join_points[2];
    } else {
        outp = &join_points[0];
        ((&join_points[1])->x = (nplp->o.ce).x, (&join_points[1])->y = (nplp->o.ce).y);
        ((&join_points[2])->x = (nplp->o.p).x, (&join_points[2])->y = (nplp->o.p).y);
        np = &join_points[1];
    }
    do { } while (0);
    ...
    if (join == gs_join_triangle) {
        ...
        ((&join_points[4])->x = (join_points[3]).x, (&join_points[4])->y = (join_points[3]).y);
        if (!ccw) {

            join_points[3].x = tpx, join_points[3].y = tpy;
        } else {

            ((&join_points[3])->x = (join_points[2]).x, (&join_points[3])->y = (join_points[2]).y);
            ((&join_points[2])->x = (join_points[1]).x, (&join_points[2])->y = (join_points[1]).y);
    ...
    if (join == gs_join_miter &&
        !(((long)((outp->x << 1) >> (8))) == ((long)((np->x << 1) >> (8))) &&
          ((long)((outp->y << 1) >> (8))) == ((long)((np->y << 1) >> (8))))
        ) {
        gs_fixed_point mpt;
                         ...
                         gs_line_join join)
{
    ...
    if (join == gs_join_triangle)
    {
        ...
        ((&rjoin_points[0])->x = (tp).x, (&rjoin_points[0])->y = (tp).y);
        ((&rjoin_points[1])->x = (nplp->o.co).x, (&rjoin_points[1])->y = (nplp->o.co).y);
        return 2;
    }


    ((&rjoin_points[0])->x = (nplp->o.co).x, (&rjoin_points[0])->y = (nplp->o.co).y);
    ...
    if (join == gs_join_miter &&
        !(((long)((plp->e.ce.x << 1) >> (8))) == ((long)((nplp->o.co.x << 1) >> (8))) &&
          ((long)((plp->e.ce.y << 1) >> (8))) == ((long)((nplp->o.co.y << 1) >> (8))))
        ) {
        gs_fixed_point mpt;
        int code = check_miter(pgs_lp, plp, nplp, pmat, &plp->e.ce,
                               &nplp->o.co, &mpt, ((bool)0));
        if (code < 0)
            return code;
        if (code == 0) {
            ((&rjoin_points[0])->x = (mpt).x, (&rjoin_points[0])->y = (mpt).y);
            ((&rjoin_points[1])->x = (nplp->o.co).x, (&rjoin_points[1])->y = (nplp->o.co).y);
            return 2;
        }
    }
                          ...
                          gs_line_join join)
{
    ...
    if (join == gs_join_triangle)
    {
        ...
        ((&join_points[0])->x = (tp).x, (&join_points[0])->y = (tp).y);
        ((&join_points[1])->x = (nplp->o.ce).x, (&join_points[1])->y = (nplp->o.ce).y);
        return 2;
    }


    ((&join_points[0])->x = (nplp->o.ce).x, (&join_points[0])->y = (nplp->o.ce).y);
    ...
    if (join == gs_join_miter &&
        !(((long)((plp->e.co.x << 1) >> (8))) == ((long)((nplp->o.ce.x << 1) >> (8))) &&
          ((long)((plp->e.co.y << 1) >> (8))) == ((long)((nplp->o.ce.y << 1) >> (8))))
        ) {
        gs_fixed_point mpt;
        int code = check_miter(pgs_lp, plp, nplp, pmat, &plp->e.co,
                               &nplp->o.ce, &mpt, ((bool)1));
        if (code < 0)
            return code;
        if (code == 0) {
            ((&join_points[0])->x = (mpt).x, (&join_points[0])->y = (mpt).y);
            ((&join_points[1])->x = (nplp->o.ce).x, (&join_points[1])->y = (nplp->o.ce).y);
            return 2;
        }
    }
...
# 2362 "./base/gxstroke.c"
}
# 2375 "./base/gxstroke.c"
static int
add_round_cap(gx_path * ppath, const_ep_ptr endp)
{
    ...
    if ((code = gx_path_add_partial_arc_notes(ppath, endp->p.x + endp->cdelta.x, endp->p.y + endp->cdelta.y, endp->co.x + endp->cdelta.x, endp->co.y + endp->cdelta.y, 0.55228474983079334, sn_none)) < 0 ||


        (code = gx_path_add_partial_arc_notes(ppath, endp->ce.x, endp->ce.y, endp->ce.x + endp->cdelta.x, endp->ce.y + endp->cdelta.y, 0.55228474983079334, sn_none)) < 0 ||

        (code = gx_path_add_partial_arc_notes(ppath, endp->p.x - endp->cdelta.x, endp->p.y - endp->cdelta.y, endp->ce.x - endp->cdelta.x, endp->ce.y - endp->cdelta.y, 0.55228474983079334, sn_none)) < 0 ||


        (code = gx_path_add_partial_arc_notes(ppath, endp->co.x, endp->co.y, endp->co.x - endp->cdelta.x, endp->co.y - endp->cdelta.y, 0.55228474983079334, sn_none)) < 0 ||


        (code = gx_path_add_line_notes(ppath, endp->ce.x, endp->ce.y, sn_none)) < 0
        )
        return code;
    do { } while (0);
    return 0;
            ...
            gs_fixed_point *width)
{
    ...
    if ((double)tangent.x * (double)final_tangent->x +
        ...
        code = gx_path_add_partial_arc_notes(ppath, centre->x + tangent.x, centre->y + tangent.y, current.x + tangent.x, current.y + tangent.y, 0.55228474983079334, sn_none);
    ...
    if (line_intersect(&current, &tangent,
        ...
        return gx_path_add_line_notes(ppath, final->x, final->y, sn_none);
    }
    ...
    return gx_path_add_partial_arc_notes(ppath, final->x, final->y, tangmeet.x, tangmeet.y, F, sn_none);
             ...
             bool cap)
{
    ...
    if (l == r) {
        if (cap)
            ...
            return gx_path_add_line_notes(ppath, plp->e.ce.x, plp->e.ce.y, sn_none);
    }
    ...
    if (ccw) {
        ...
        code = gx_path_add_line_notes(ppath, plp->e.p.x, plp->e.p.y, sn_none);
        if (code < 0)
            return code;
        code = gx_path_add_line_notes(ppath, current->x, current->y, sn_none);
        if (code < 0)
            return code;
        if (current->x == final->x && current->y == final->y)
            return 0;
    ...
    if ((code = do_pie_join(ppath, &plp->e.p, current, tangent,
        ...
        return code;
    if (ccw &&
        ((code = gx_path_add_line_notes(ppath, plp->e.p.x, plp->e.p.y, sn_none)) < 0 ||
         (code = gx_path_add_line_notes(ppath, plp->e.ce.x, plp->e.ce.y, sn_none)) < 0))
        return code;
...
join_under_pie(gx_path * ppath, pl_ptr plp, pl_ptr nplp, bool reflected)
{
    ...
    if (ccw) {
        ...
        if (line_intersect(& plp->o.co, &dirn1,
            ...
            return 0;
        if ((code = gx_path_close_subpath_notes(ppath, sn_none)) < 0 ||
            (code = gx_path_add_point(ppath, tangmeet.x, tangmeet.y)) < 0 ||
            (code = gx_path_add_line_notes(ppath, plp->o.co.x, plp->o.co.y, sn_none)) < 0 ||
        ...
        if ((code = gx_path_close_subpath_notes(ppath, sn_none)) < 0 ||
            (code = gx_path_add_point(ppath, tangmeet.x, tangmeet.y)) < 0 ||
            (code = gx_path_add_line_notes(ppath, nplp->e.co.x, nplp->e.co.y, sn_none)) < 0 ||
...
static int
cap_points(gs_line_cap type, const_ep_ptr endp, gs_fixed_point *pts )
{


    switch (type) {
        case gs_cap_butt:
            pts[0].x = (endp->co.x), pts[0].y = (endp->co.y);
            pts[1].x = (endp->ce.x), pts[1].y = (endp->ce.y);
            return 2;
        case gs_cap_square:
            pts[0].x = (endp->co.x + endp->cdelta.x), pts[0].y = (endp->co.y + endp->cdelta.y);
            pts[1].x = (endp->ce.x + endp->cdelta.x), pts[1].y = (endp->ce.y + endp->cdelta.y);
            return 2;
        case gs_cap_triangle:
            pts[0].x = (endp->co.x), pts[0].y = (endp->co.y);
            pts[1].x = (endp->p.x + endp->cdelta.x), pts[1].y = (endp->p.y + endp->cdelta.y);
            pts[2].x = (endp->ce.x), pts[2].y = (endp->ce.y);
            return 3;
        default:
            return ((-28));
    }
