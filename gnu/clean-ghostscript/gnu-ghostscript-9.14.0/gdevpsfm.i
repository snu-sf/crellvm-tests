# 1 "./devices/vector/gdevpsfm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpsfm.c" 2
# 18 "./devices/vector/gdevpsfm.c"
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
...
# 19 "./devices/vector/gdevpsfm.c" 2
...
# 21 "./devices/vector/gdevpsfm.c" 2
# 1 "./base/stream.h" 1
# 23 "./base/stream.h"
# 1 "./base/scommon.h" 1
# 25 "./base/scommon.h"
...
int file_close_finish(stream *);
int file_close_disable(stream *);
# 22 "./devices/vector/gdevpsfm.c" 2
# 1 "./base/spprint.h" 1
# 32 "./base/spprint.h"
...
# 23 "./devices/vector/gdevpsfm.c" 2
# 1 "./base/spsdf.h" 1
# 22 "./base/spsdf.h"
# 1 "./base/gsparam.h" 1
# 40 "./base/gsparam.h"
...
# 24 "./devices/vector/gdevpsfm.c" 2
# 1 "./devices/vector/gdevpsf.h" 1
# 23 "./devices/vector/gdevpsf.h"
# 1 "./base/gsgdata.h" 1
# 55 "./base/gsgdata.h"
...
# 24 "./devices/vector/gdevpsf.h" 2
# 33 "./devices/vector/gdevpsf.h"
...
int psf_convert_type1_to_type2(stream *s, const gs_glyph_data_t *pgd,
                               gs_font_type1 *pfont);
# 25 "./devices/vector/gdevpsfm.c" 2
...
pput_hex(stream *s, const byte *pcid, int size)
{
    ...
    for (i = 0; i < size; ++i) {
        spputc(s, hex_digits[pcid[i] >> 4]);
        spputc(s, hex_digits[pcid[i] & 0xf]);
    }
}
                  ...
                  int font_index_only)
{
    ...
    for (gs_cmap_lookups_enum_init(pcmap, which, &lenum);
        ...
        for (gi = 0; gi < num_entries; gi += 100) {
            int i = gi, ni = (((i + 100) < (num_entries)) ? (i + 100) : (num_entries));
            const char *end;
            ...
            for (; i < ni; ++i) {
                ...
                ((void)(gs_cmap_enum_next_entry(&lenum)));
               ...
               const gs_const_string *alt_cmap_name, int font_index_only)
{
    ...
    if (!pcmap->ToUnicode) {
        ...
        stream_puts(s, " def\n");
        if (((&pcmap->uid)->id < 0)) {
            uint i, n = ((uint)(-(&pcmap->uid)->id));
            const long *values = ((&pcmap->uid)->xvalues);
...

    {
        ...
        gx_code_space_range_t ranges[100];
        int code, count = 0;
