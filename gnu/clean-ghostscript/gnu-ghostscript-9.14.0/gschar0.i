...
gs_stack_modal_fonts(gs_text_enum_t *pte)
{
    ...
    while (cfont->FontType == ft_composite) {
        ...
        if (!((cmfont->data.FMapType) == fmap_escape || (cmfont->data.FMapType) == fmap_double_escape || (cmfont->data.FMapType) == fmap_shift))
            break;
        if (fdepth == 5)
            return ((-10));
        fdepth++;
...
# 131 "./base/gschar0.c"
    if (pte->index == 0) {
        ...
        for (; pfont->FontType == ft_composite;) {
            fmap_type fmt = (pdata = &((gs_font_type0 *)pfont)->data)->FMapType;
            ...
            switch (fmt) {
                case fmap_escape:
                    if (chr != root_esc_char(pte))
                        break;
                    if ( end - p < 2 ) return ((-15));
                    fidx = p[1];
                    p += 2;
                    do { } while (0);
                  rdown:if (fidx >= pdata->encoding_size) return ((-15)); if (idepth == 5) return ((-10)); pfont = pdata->FDepVector[pdata->Encoding[fidx]]; pte->fstack.items[idepth].index = fidx; if (++idepth > orig_depth || pfont != pte->fstack.items[idepth].font) { pte->fstack.items[idepth].font = pfont; pte->fstack.items[idepth].index = 0; changed = 1; } else { };
                    do { } while (0);
  ...
  up:if (p == end)
    ...
    while (fdepth > 0) {
        ...
        pdata = &((gs_font_type0 *)pfont)->data;
        switch (pdata->FMapType) {
            ...
            case fmap_escape:
              ...
              down:if (++p == end)
                ...
                fdepth--;
                do {
                    if (fidx >= pdata->encoding_size) return ((-15)); if (fdepth == 5) return ((-10)); pfont = pdata->FDepVector[pdata->Encoding[fidx]]; pte->fstack.items[fdepth].index = fidx; if (++fdepth > orig_depth || pfont != pte->fstack.items[fdepth].font) { pte->fstack.items[fdepth].font = pfont; pte->fstack.items[fdepth].index = 0; changed = 1; } else { };
                    do { } while (0);
                        ...
                        case 1:
                            {
                                ...
                                while ( subs_count != 0 && tchr >= (schr = *psv) ) subs_count--, tchr -= schr, psv += 1; chr = tchr; p += 1 - 1; break;
                            }
                        case 2:
                            if ( end - p < 1 ) return ((-15));

                            {
                                ...
                                while ( subs_count != 0 && tchr >= (schr = (((ushort)*psv << 8) + psv[1])) ) subs_count--, tchr -= schr, psv += 2; chr = tchr; p += 2 - 1; break;
                            }
                        case 3:
                            if ( end - p < 2 ) return ((-15));

                            {
                                ulong tchr = ((ulong) chr << 16) + (((ushort)*p << 8) + p[1]),
                                      schr;

                                while ( subs_count != 0 && tchr >= (schr = (((ulong)*psv << 16) + ((uint)psv[1] << 8) + psv[2])) ) subs_count--, tchr -= schr, psv += 3; chr = tchr; p += 3 - 1; break;
                            }
                        case 4:
                            if ( end - p < 3 ) return ((-15));

                            {
                                ulong tchr = ((ulong) chr << 24) + (((ulong)*p << 16) + ((uint)p[1] << 8) + p[2]),
                                      schr;

                                while ( subs_count != 0 && tchr >= (schr = (((ulong)*psv << 24) + ((ulong)psv[1] << 16) + ((uint)psv[2] << 8) + psv[3])) ) subs_count--, tchr -= schr, psv += 4; chr = tchr; p += 4 - 1; break;
                            }
...
# 374 "./base/gschar0.c"
                    if (*(p - 1) != chr) {
                        ...
                        substr[0] = chr;
                        cstr.data = substr;
                        cstr.size = (((4) < (end - p + 1)) ? (4) : (end - p + 1));
                        if (gs_debug_c('j')) {
                            (dmprintf_file_and_line(pfont->memory,"./base/gschar0.c", 386), errprintf(pfont->memory,"[j] original str("));
                            debug_print_string_hex(pfont->memory, str, end - str);
                            (dmprintf_file_and_line(pfont->memory,"./base/gschar0.c", 388), errprintf(pfont->memory,") -> modified substr("));
                            debug_print_string_hex(pfont->memory, cstr.data, cstr.size);
                            (dmprintf_file_and_line(pfont->memory,"./base/gschar0.c", 390), errprintf(pfont->memory,")\n"));
                        }
                    ...
                    if (code == 0) {
                        ...
                        chr = (gs_char) glyph, glyph = ((gs_glyph)0x7fffffff);
    ...
    if (pte->fstack.items[fdepth].font->FontType == ft_CID_encrypted) {
        ...
        code = ((gs_font_cid0 *)pfont)->cidata.glyph_data((gs_font_base *)pfont,
                            glyph, ((void*)0), &font_index);
        if (code < 0) {
           code = ((gs_font_cid0 *)pfont)->cidata.glyph_data((gs_font_base *)pfont,
                        (gs_glyph)(((gs_glyph)0x80000000L) + 0), ((void*)0), &font_index);
           if (code < 0)
               return ((-10));
        }
