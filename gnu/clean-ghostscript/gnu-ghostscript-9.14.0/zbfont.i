...
static void font_data_clear_marks(const gs_memory_t *cmem, void *pre, uint size, const gs_memory_struct_type_t *pstype); static gs_ptr_type_t font_data_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void font_data_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_font_data = { sizeof(font_data), "font_data", 0, font_data_clear_marks, font_data_enum_ptrs, font_data_reloc_ptrs, 0, 0 };
static
void font_data_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
    ref_struct_clear_marks(cmem, vptr, ((int) ( (char *)&((font_data *)0)->u.type42.mru_sfnts_index - (char *)((font_data *)0) )) , pstype);
}
static
gs_ptr_type_t font_data_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst)
{
    return ref_struct_enum_ptrs(mem, vptr, ((int) ( (char *)&((font_data *)0)->u.type42.mru_sfnts_index - (char *)((font_data *)0) )) , index, pep, pstype, gcst);
}

static
void font_data_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) {
{
    ref_struct_reloc_ptrs(vptr, ((int) ( (char *)&((font_data *)0)->u.type42.mru_sfnts_index - (char *)((font_data *)0) )) , pstype, gcst);
}
}
...
{
    font_data *pdata = ((font_data *)((pfont)->client_data));
    ...
    if (pfont->FontType == ft_user_defined && (((const byte *)&((&pdata->BuildGlyph)->tas.type_attrs))[1]) == t_null) {
        ref nsref, tname;

        names_string_ref(pfont->memory->gs_lib_ctx->gs_name_table, &cname, &nsref);
        if (((&nsref)->tas.rsize) == 7 &&
            !memcmp(nsref.value.const_bytes, ".notdef", ((&nsref)->tas.rsize))) {
# 107 "./psi/zbfont.c"
            char buf[20];
            int code;
            ...
            if (gspace == GLYPH_SPACE_NOGEN)
                return ((gs_glyph)0x7fffffff);
            gs_sprintf(buf, "j%ld", chr);
            code = names_ref(pfont->memory->gs_lib_ctx->gs_name_table, (const byte *)buf, strlen(buf), &tname, 1);
            if (code < 0) {
    ...
    return (gs_glyph)( ((const name_sub_table *) ((&cname)->value.pname - (((&cname)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((&cname)->tas.rsize) );
}
...
zfont_glyph_name(gs_font *font, gs_glyph index, gs_const_string *pstr)
{
    ...
    if (index >= ((gs_glyph)0x80000000L)) {
        char cid_name[sizeof(gs_glyph) * 3 + 1];
        int code;

        gs_sprintf(cid_name, "%lu", (ulong) index);
        code = names_ref(font->memory->gs_lib_ctx->gs_name_table, (const byte *)cid_name, strlen(cid_name), &nref, 1);

        if (code < 0)
            return code;
    } else
        names_index_ref(font->memory->gs_lib_ctx->gs_name_table, index, &nref);
    names_string_ref(font->memory->gs_lib_ctx->gs_name_table, &nref, &sref);
    pstr->data = sref.value.const_bytes;
    pstr->size = ((&sref)->tas.rsize);
    return 0;
}
...
gs_font_map_glyph_by_dict(const gs_memory_t *mem, const ref *map, gs_glyph glyph)
{
    ...
    if (glyph >= ((gs_glyph)0x80000000L)) {
        ...
        if (dict_find_string(map, "CIDCount", &v) > 0) {
            ...
            ((&n)->value.intval = (cid / 256), ((&n)->tas.type_attrs = ((t_integer) << 8) + (0)) );
            if (dict_find(map, &n, &v) > 0) {
                ref vv;

                if (array_get(mem, v, cid % 256, &vv) == 0 && (((const byte *)&((&vv)->tas.type_attrs))[1]) == t_integer)
                    return vv.value.intval;
            }
            return ((gs_char)~0L);
        }

        ((&n)->value.intval = (cid), ((&n)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    } else
        names_index_ref(mem->gs_lib_ctx->gs_name_table, glyph, &n);
     if (dict_find(map, &n, &v) > 0) {
        ...
        if ((((const byte *)&((v)->tas.type_attrs))[1]) == t_integer)
...
{
    font_data *pdata = ((font_data *)((font)->client_data));
    const ref *UnicodeDecoding;

    if ((((const byte *)&((&pdata->GlyphNames2Unicode)->tas.type_attrs))[1]) == t_dictionary) {
        ...
        if (ch != -1) {
            ...
            ((&n)->value.intval = (ch), ((&n)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ...
    if (glyph <= ((gs_glyph)0x80000000L)) {
        ...
        if (UnicodeDecoding != ((void*)0) && (((const byte *)&((UnicodeDecoding)->tas.type_attrs))[1]) == t_dictionary)
                     ...
                     const char *bcstr, const char *bgstr)
{
    ...
    if (!bcstr)
        ...
        if ((code = names_ref(mem->gs_lib_ctx->gs_name_table, (const byte *)bcstr, strlen(bcstr), &pbuild->BuildChar, 0)) < 0)
    ...
    if (!bgstr)
        ...
        if ((code = names_ref(mem->gs_lib_ctx->gs_name_table, (const byte *)bgstr, strlen(bgstr), &pbuild->BuildGlyph, 0)) < 0)
...
build_gs_font_procs(os_ptr op, build_proc_refs * pbuild)
{
    ...
    if (ccode <= 0) {
        ...
        do { if ( !(((&*pBuildChar)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*pBuildChar)); } while (0);
        pbuild->BuildChar = *pBuildChar;
    }
    if (gcode <= 0)
        (((&pbuild->BuildGlyph)->tas.type_attrs = ((t_null) << 8) + (0)) );
    else {
        do { if ( !(((&*pBuildGlyph)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*pBuildGlyph)); } while (0);
...
static int font_with_same_UID_and_another_metrics(const gs_font *pfont0, const gs_font *pfont1)
{
    ...
    if (uid_equal(&pbfont0->UID, &pbfont1->UID)) {
        const ref *pfdict0 = &((font_data *)((gs_font_parent(pbfont0))->client_data))->dict;
        const ref *pfdict1 = &((font_data *)((gs_font_parent(pbfont1))->client_data))->dict;
        ref *pmdict0, *pmdict1;
        ...
        if (!pmdict0 != !pmdict1)
            return 1;
        if (pmdict0 != ((void*)0) && !obj_eq(pfont0->memory, pmdict0, pmdict1))
            return 1;
                        ...
                        build_font_options_t options)
{
    ...
    if (pcharstrings)
        (*(&pdata->CharStrings) = *(&CharStrings));
    else
        (((&pdata->CharStrings)->tas.type_attrs = ((t_null) << 8) + (0)) );


    if (((&pfont->UID)->id != (~(-1L << ((1 << 3) * 8 - 1)))) &&
    ...
    if (((&pfont->UID)->id != (~(-1L << ((1 << 3) * 8 - 1))))) {
        ...
        code = gs_font_find_similar((gs_lib_ctx_get_interp_instance(((gs_memory_t *)((i_ctx_p->memory).current)))->font_dir), &pfont0,
                      ...
                      const build_proc_refs * pbuild)
{
    ...
    ((&uid)->id = (~(-1L << ((1 << 3) * 8 - 1))), (&uid)->xvalues = 0);
...
{
    ref *pfontinfo = ((void*)0), *g2u = ((void*)0);
    font_data *pdata;

    if (dict_find_string(pdref, "FontInfo", &pfontinfo) <= 0 ||
            !((((const byte *)&((pfontinfo)->tas.type_attrs))[1]) == (t_dictionary)) ||
    ...
    pdata = ((font_data *)((pfont)->client_data));
    ((void)(((*(&pdata->GlyphNames2Unicode) = *(g2u)), ((&pdata->GlyphNames2Unicode)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
}
                     ...
                     build_font_options_t options)
{
    ...
    code = font_bbox_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, bbox);
    if (code < 0)
        return code;
    ...
    ialloc_set_space((&(i_ctx_p->memory)), (avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
    code = dict_uid_param(op, &uid, 0, ((gs_memory_t *)((i_ctx_p->memory).current)), i_ctx_p);
...
lookup_gs_simple_font_encoding(gs_font_base * pfont)
{
    ...
    if ((((const byte *)&((pfe)->tas.type_attrs))[1]) == t_array && ((pfe)->tas.rsize) <= 256) {
        ...
        for (i = 0; i < esize; ++i) {
            ...
            if (array_get(pfont->memory, pfe, (long)i, &fchar) < 0 ||
                !((((const byte *)&((&fchar)->tas.type_attrs))[1]) == (t_name))
                )
                fstrs[i].data = 0, fstrs[i].size = 0;
            else {
                ref nsref;

                names_string_ref(pfont->memory->gs_lib_ctx->gs_name_table, &fchar, &nsref);
                fstrs[i].data = nsref.value.const_bytes;
                fstrs[i].size = ((&nsref)->tas.rsize);
            }
        }
...
sub_font_params(gs_memory_t *mem, const ref *op, gs_matrix *pmat, gs_matrix *pomat, ref *pfname)
{
    ...
    if ((dict_find_string((porigfont != ((void*)0) ? porigfont : op), "FontInfo", &pfontinfo) > 0) &&
        ((((const byte *)&((pfontinfo)->tas.type_attrs))[1]) == (t_dictionary)) &&
        (dict_find_string(pfontinfo, "OrigFontName", &pfontname) > 0)) {
        if ((dict_find_string(pfontinfo, "OrigFontStyle", &pfontstyle) > 0) &&
                ((pfontstyle)->tas.rsize) > 0) {
            const byte *tmpStr1 = pfontname->value.const_bytes;
            const byte *tmpStr2 = pfontstyle->value.const_bytes;
            int fssize1 = ((pfontname)->tas.rsize), fssize2 = ((pfontstyle)->tas.rsize), fssize = fssize1 + fssize2 + 1;
            byte *sfname = (*(mem)->procs.alloc_string)(mem, fssize, "sub_font_params");
            ...
            ((pfname)->value.bytes = (sfname), ((pfname)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40))), ((pfname)->tas.rsize = (fssize)));
        } else
            get_font_name(mem, pfname, pfontname);
    } else if (dict_find_string((porigfont != ((void*)0) ? porigfont : op), ".Alias", &pfontname) > 0) {

        get_font_name(mem, pfname, pfontname);
    } else if (dict_find_string((porigfont != ((void*)0) ? porigfont : op), "FontName", &pfontname) > 0) {
        get_font_name(mem, pfname, pfontname);
    } else
              ...
              build_font_options_t options)
{
    ...
    bool cpsi_mode = gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current)));

    get_font_name(((gs_memory_t *)((i_ctx_p->memory).current)), &kname, op - 1);
    if (dict_find_string(op, "FontType", &pftype) <= 0 ||
        !((((const byte *)&((pftype)->tas.type_attrs))[1]) == (t_integer)) ||
    ...
    if (pencoding) {
        int count = ((pencoding)->tas.rsize);
        int type = ftype ? t_name : t_integer;
        bool fixit = ((bool)0);

        while (count--) {
           ref r;
           if ((code = array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pencoding, count, &r)) < 0 ||
             !(((((const byte *)&((&r)->tas.type_attrs))[1]) == (type)) || ((((const byte *)&((&r)->tas.type_attrs))[1]) == (t_null)))) {
               if (!cpsi_mode && ftype == ft_user_defined) {
                   if (code < 0 || ((((const byte *)&((&r)->tas.type_attrs))[1]) == (t_null))) {
                       return ((-20));
                   }
                   fixit = ((bool)1);
                   break;
        ...
        if (fixit) {
            ...
            uint size = 0;
            char buf[32], *bptr;
            avm_space curglob = (((&(i_ctx_p->memory)))->current_space);
            avm_space useglob = ((avm_space)(((pencoding)->tas.type_attrs) & (((1 << 2) - 1) << 2)) == avm_local) ? avm_local : avm_global;

            ialloc_set_space((&(i_ctx_p->memory)), useglob);

            count = ((pencoding)->tas.rsize);
            if ((code = gs_alloc_ref_array(((i_ctx_p->memory).current), &penc, (((pencoding)->tas.type_attrs) & (0x20 + 0x40)), count, "build_gs_font")) < 0)
                 return code;

            while (count--) {
               ref r;
               if (array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pencoding, count, &r) < 0){
                   return ((-20));
               }

               if (((((const byte *)&((&r)->tas.type_attrs))[1]) == (t_name))){
                   (*(&(penc.value.refs[count])) = *(&r));
               }
               else {

                   if ((code = obj_cvs(((gs_memory_t *)((i_ctx_p->memory).current)), &r, (byte *)buf, 32, &size, (const byte **)(&bptr))) < 0) {
                       return(code);
                   }
                   if ((code = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, (const byte *)bptr, size, &r, ((bool)1))) < 0)
                        return code;
                   (*(&(penc.value.refs[count])) = *(&r));
               }
            }

            if ((code = dict_put_string((((i_ctx_p->op_stack).stack).p), "Encoding", &penc, ((void*)0))) < 0)
               return code;
            ialloc_set_space((&(i_ctx_p->memory)), curglob);
        }
    }
    if ((code = dict_int_param(op, "WMode", 0, 1, 0, &wmode)) < 0 ||
        (code = dict_bool_param(op, "BitmapWidths", ((bool)0), &bitmapwidths)) < 0 ||
    ...
    if (code > 0 && ((((const byte *)&((pfid)->tas.type_attrs))[1]) == (t_fontID))) {
        ...
        if (obj_eq(pfont->memory, (&((font_data *)((pfont)->client_data))->dict), op)) {
            if (pfont->base == pfont) {
                ...
                *ppfont = pfont;
                ...
                code = sub_font_params(((gs_memory_t *)((i_ctx_p->memory).current)), op, &mat, ((void*)0), &fname);
                if (code < 0)
                    return code;
                code = 1;
                  ...
                  ref *fid_op)
{
    ...
    uint space = (((&(i_ctx_p->memory)))->current_space);
    int code = sub_font_params(((gs_memory_t *)((i_ctx_p->memory).current)), op, &mat, &omat, &fname);

    if (code < 0)
        return code;
    ialloc_set_space((&(i_ctx_p->memory)), (avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)));
    pfont = gs_font_alloc(((gs_memory_t *)((i_ctx_p->memory).current)), pstype, &gs_font_procs_default, ((void*)0),
                          "buildfont(font)");
    pdata = (font_data *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_font_data, "buildfont(data)");

    if (pfont == 0 || pdata == 0)
        code = ((-25));
    else if (fid_op)
        code = add_FID(i_ctx_p, fid_op, pfont, ((i_ctx_p->memory).current));
    if (code < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pdata, "buildfont(data)"));
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), pfont, "buildfont(font)"));
        ialloc_set_space((&(i_ctx_p->memory)), space);
        return code;
    }
    refset_null_new((ref *) pdata, sizeof(font_data) / sizeof(ref), ((&(i_ctx_p->memory))->new_mask));
    ((void)(((*(&pdata->dict) = *(op)), ((&pdata->dict)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    ((void)(((*(&pdata->BuildChar) = *(&pbuild->BuildChar)), ((&pdata->BuildChar)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    ((void)(((*(&pdata->BuildGlyph) = *(&pbuild->BuildGlyph)), ((&pdata->BuildGlyph)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    if (pencoding)
        ((void)(((*(&pdata->Encoding) = *(pencoding)), ((&pdata->Encoding)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    pfont->client_data = pdata;
    pfont->FontType = ftype;
    pfont->FontMatrix = mat;
    pfont->orig_FontMatrix = omat;
    pfont->BitmapWidths = ((bool)0);
    ...
    switch ((((const byte *)&((op)->tas.type_attrs))[1])) {
        case t_string:
            *pfname = *op;
            break;
        case t_name:
            names_string_ref(mem->gs_lib_ctx->gs_name_table, op, pfname);
            break;
        default:
...
define_gs_font(i_ctx_t *i_ctx_p, gs_font * pfont)
{
    return (pfont->base == pfont && pfont->dir == 0 ?
            ...
            gs_definefont((gs_lib_ctx_get_interp_instance(((gs_memory_t *)((i_ctx_p->memory).current)))->font_dir), pfont) :
            0);
}
