...
# 28 "./base/sddparam.c" 2



int s_DCTD_get_params(gs_param_list *plist, const stream_DCT_state *ss, bool all);
...
int s_DCTD_put_params(gs_param_list *plist, stream_DCT_state *ss);
...
s_DCTD_put_params(gs_param_list * plist, stream_DCT_state * pdct)
{
    ...
    if ((code = s_DCT_put_params(plist, pdct)) < 0 ||
        ...
        (code = s_DCT_put_huffman_tables(plist, pdct, ((bool)0))) < 0 ||
        (code = s_DCT_put_quantization_tables(plist, pdct, ((bool)0))) < 0
        )
        do { } while (0);
    return code;
}
