# 1 "./openjpeg/libopenjpeg/t2.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./openjpeg/libopenjpeg/t2.c" 2
# 34 "./openjpeg/libopenjpeg/t2.c"
...
# 35 "./openjpeg/libopenjpeg/t2.c" 2
...
# 347 "./openjpeg/libopenjpeg/t2.c"
        ...
        for (pino = 0; pino <= l_tcp->numpocs; ++pino) {
                ...
                OPJ_BOOL* first_pass_failed = (OPJ_BOOL*)((size_t)(l_image->numcomps * sizeof(OPJ_BOOL)) >= (size_t)-0x100 ? ((void*)0) : malloc(l_image->numcomps * sizeof(OPJ_BOOL)));
                if (!first_pass_failed)
                {
                    opj_pi_destroy(l_pi,l_nb_pocs);
                    return 0;
                }
                memset(first_pass_failed, 1, l_image->numcomps * sizeof(OPJ_BOOL));
...
# 428 "./openjpeg/libopenjpeg/t2.c"
                }
                ++l_current_pi;

                free(first_pass_failed);
        }
# 443 "./openjpeg/libopenjpeg/t2.c"
        opj_pi_destroy(l_pi,l_nb_pocs);
        *p_data_read = l_current_data - p_src;
        return 1;
}
# 457 "./openjpeg/libopenjpeg/t2.c"
opj_t2_t* opj_t2_create(opj_image_t *p_image, opj_cp_t *p_cp)
{

        opj_t2_t *l_t2 = (opj_t2_t*)((size_t)(sizeof(opj_t2_t)) >= (size_t)-0x100 ? ((void*)0) : malloc(sizeof(opj_t2_t)));
        if (!l_t2) {
                return ((void*)0);
        }
...
# 1128 "./openjpeg/libopenjpeg/t2.c"
                                if ((l_cblk->data_current_size + l_seg->newlen) > l_cblk->data_max_size) {
                                    OPJ_BYTE* new_cblk_data = (OPJ_BYTE*) ((size_t)(l_cblk->data_current_size + l_seg->newlen) >= (size_t)-0x100 ? ((void*)0) : realloc(l_cblk->data, l_cblk->data_current_size + l_seg->newlen));
                                    if(! new_cblk_data) {
                                        free(l_cblk->data);
                                        l_cblk->data = ((void*)0);
...
# 1251 "./openjpeg/libopenjpeg/t2.c"
                            ...
                            OPJ_UINT32 first)
{
        ...
        if (l_nb_segs > cblk->m_current_max_segs) {
                ...
                cblk->m_current_max_segs += 10;

                new_segs = (opj_tcd_seg_t*) ((size_t)(cblk->m_current_max_segs * sizeof(opj_tcd_seg_t)) >= (size_t)-0x100 ? ((void*)0) : realloc(cblk->segs, cblk->m_current_max_segs * sizeof(opj_tcd_seg_t)));
                if(! new_segs) {
                        free(cblk->segs);
                        cblk->segs = ((void*)0);
