...
static const gs_memory_struct_type_t st_joint_caches = { sizeof(gx_cie_joint_caches), "gx_cie_joint_caches", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
cache_is_linear(cie_linear_params_t *params, const cie_cache_floats *pcf)
{
    ...
    double diff = pcf->values[(sizeof(pcf->values) / sizeof((pcf->values)[0])) - 1] - origin;
    double scale = diff / ((sizeof(pcf->values) / sizeof((pcf->values)[0])) - 1);
    int i;
    double test = origin + scale;

    for (i = 1; i < (sizeof(pcf->values) / sizeof((pcf->values)[0])) - 1; ++i, test += scale)
        if (fabs(pcf->values[i] - test) >= 0.5 / (sizeof(pcf->values) / sizeof((pcf->values)[0])))
            return (params->is_linear = ((bool)0));
    params->origin = origin - pcf->params.base;
    params->scale =
        diff * pcf->params.factor / ((sizeof(pcf->values) / sizeof((pcf->values)[0])) - 1);
    return (params->is_linear = ((bool)1));
}
...
cache_set_linear(cie_cache_floats *pcf)
{
        if (pcf->params.is_identity) {
            ...
            pcf->params.linear.is_linear = ((bool)1);
...
gs_cie_cached_value(double in, const cie_cache_floats *pcache)
{
    ...
    index = (index < 0 ? 0 : index >= (1 << 9) ? (1 << 9) - 1 : index);
...
gx_install_cie_abc(gs_cie_abc *pcie, gs_state * pgs)
{
    ...
    do { int j; for (j = 0; j < (sizeof(pcie->caches.DecodeABC.caches) / sizeof((pcie->caches.DecodeABC.caches)[0])); j++) { cie_cache_floats *pcf = &(pcie->caches.DecodeABC.caches)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcie->RangeABC.ranges)[j], "DecodeABC"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcie->DecodeABC)->procs[j])(v, pcie); do { } while (0); } pcf->params.is_identity = (&pcie->DecodeABC)->procs[j] == (DecodeABC_default).procs[j]; } } while (0);


    gx_cie_load_common_cache(&pcie->common, pgs);
    gs_cie_abc_complete(pcie);
    return gs_cie_cs_complete(pgs, ((bool)1));
}
...
gx_install_CIEDEFG(gs_color_space * pcs, gs_state * pgs)
{
    ...
    do { int j; for (j = 0; j < (sizeof(pcie->caches_defg.DecodeDEFG) / sizeof((pcie->caches_defg.DecodeDEFG)[0])); j++) { cie_cache_floats *pcf = &(pcie->caches_defg.DecodeDEFG)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcie->RangeDEFG.ranges)[j], "DecodeDEFG"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcie->DecodeDEFG)->procs[j])(v, pcie); do { } while (0); } pcf->params.is_identity = (&pcie->DecodeDEFG)->procs[j] == (DecodeDEFG_default).procs[j]; } } while (0);
...
gx_install_CIEDEF(gs_color_space * pcs, gs_state * pgs)
{
    ...
    do { int j; for (j = 0; j < (sizeof(pcie->caches_def.DecodeDEF) / sizeof((pcie->caches_def.DecodeDEF)[0])); j++) { cie_cache_floats *pcf = &(pcie->caches_def.DecodeDEF)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcie->RangeDEF.ranges)[j], "DecodeDEF"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcie->DecodeDEF)->procs[j])(v, pcie); do { } while (0); } pcf->params.is_identity = (&pcie->DecodeDEF)->procs[j] == (DecodeDEF_default).procs[j]; } } while (0);
...
gx_install_CIEA(gs_color_space * pcs, gs_state * pgs)
{
    ...
    gs_cie_cache_init(&pcie->caches.DecodeA.floats.params, &lp,
                      &pcie->RangeA, "DecodeA");
    for (i = 0; i <= lp.N; ++i) {
        float in = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N );
...
gx_cie_load_common_cache(gs_cie_common * pcie, gs_state * pgs)
{
    ...
    do { int j; for (j = 0; j < (sizeof(pcie->caches.DecodeLMN) / sizeof((pcie->caches.DecodeLMN)[0])); j++) { cie_cache_floats *pcf = &(pcie->caches.DecodeLMN)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcie->RangeLMN.ranges)[j], "DecodeLMN"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcie->DecodeLMN)->procs[j])(v, pcie); do { } while (0); } pcf->params.is_identity = (&pcie->DecodeLMN)->procs[j] == (DecodeLMN_default).procs[j]; } } while (0);
                             ...
                             cie_cached_value cur, int i, double threshold)
{
    ...
    if (fabs(cur - prev) > threshold * (((fabs(prev)) < (fabs(cur))) ? (fabs(prev)) : (fabs(cur)))) {
        if (i - 1 < pccr->imin)
            pccr->imin = i - 1;
        if (i > pccr->imax)
            pccr->imax = i;
               ...
               const cie_cache_floats * pcf, double threshold)
{
    ...
    pcache->vecs.params.base = (pcf->params.base);
    pcache->vecs.params.factor = (pcf->params.factor);
    pcache->vecs.params.limit =
        (((1 << 9) - 1) / pcf->params.factor + pcf->params.base);

    for (i = 0; i < (1 << 9); ++i) {
        float f = pcf->values[i];

        pcache->vecs.values[i].u = (f * u);
        pcache->vecs.values[i].v = (f * v);
        pcache->vecs.values[i].w = (f * w);
    }
...
cie_cache3_set_interpolation(gx_cie_vector_cache3_t * pvc)
{
    ...
    for (j = 0; j < 3; ++j) {
        ...
        for (k = 1; k < 3; ++k) {
            ...
            rmin = (((rmin) < (p->rmin)) ? (rmin) : (p->rmin)), rmax = (((rmax) > (p->rmax)) ? (rmax) : (p->rmax));
        }
...
gs_setcolorrendering(gs_state * pgs, gs_cie_render * pcrd)
{
    ...
    joint_ok =
        ...
        !memcmp(&pcrd->points.WhitePoint, &pcrd_old->points.WhitePoint, sizeof(pcrd->points.WhitePoint)) && !memcmp(&pcrd->points.BlackPoint, &pcrd_old->points.BlackPoint, sizeof(pcrd->points.BlackPoint)) &&
        !memcmp(&pcrd->MatrixPQR, &pcrd_old->MatrixPQR, sizeof(pcrd->MatrixPQR)) && !memcmp(&pcrd->RangePQR, &pcrd_old->RangePQR, sizeof(pcrd->RangePQR)) &&
        !memcmp(&pcrd->TransformPQR, &pcrd_old->TransformPQR, sizeof(pcrd->TransformPQR));

    do { if ((pgs->cie_render) != (pcrd)) { do { if (pcrd) do { (pcrd)->rc.ref_count++; do { } while (0); } while (0); } while (0); do { if (pgs->cie_render) { do { do { } while (0); (pgs->cie_render)->rc.ref_count += (-1); } while (0); if (!(pgs->cie_render)->rc.ref_count) { do { do { } while (0); (pgs->cie_render)->rc.free((pgs->cie_render)->rc.memory, (void *)(pgs->cie_render), "gs_setcolorrendering"); } while (0); do { } while (0); } else do { } while (0); } } while (0); (pgs->cie_render) = (pcrd); } } while (0);

    if (!joint_ok)
        code = gs_cie_cs_complete(pgs, ((bool)1));
...
gx_unshare_cie_caches(gs_state * pgs)
{
    ...
    do { if ( (pgs->cie_joint_caches) == 0 || (pgs->cie_joint_caches)->rc.ref_count > 1 || (pgs->cie_joint_caches)->rc.memory != (pgs->memory) ) { gx_cie_joint_caches *new; do { if ( ((new) = (gx_cie_joint_caches *)(*(pgs->memory)->procs.alloc_struct)(pgs->memory, &st_joint_caches, "gx_unshare_cie_caches")) == 0 ) { return 0; } else { do { (new)->rc.ref_count = 1; (new)->rc.memory = pgs->memory; (new)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0); if ( pgs->cie_joint_caches ) do { do { } while (0); (pgs->cie_joint_caches)->rc.ref_count += (-1); } while (0); (pgs->cie_joint_caches) = new; } } while (0);


    if (pgs->cie_joint_caches != pjc) {
        pjc = pgs->cie_joint_caches;
        pjc->cspace_id = pjc->render_id = 0L;
...
# 768 "./base/gscie.c"
    ...
    if (A < 0 && B >= 0) {
        const double X = -((1 << 9) - 1) * A / R;

        const int K = (int)(A + B < 0 ? floor(X) : ceil(X));
        const double Ca = -A / K, Cb = B / (((1 << 9) - 1) - K);
        double C = (((Ca) > (Cb)) ? (Ca) : (Cb));
        const int M = 24 - 9;
        int cexp;
    ...
    pcache->base = A;
# 807 "./base/gscie.c"
    pcache->factor = (((delta) < 0 ? -(delta) : (delta)) < 1e-30 ? 1.0 : ((1 << 9) - 1) / R);
    do { } while (0);
...
gs_cie_render_sample(gs_cie_render * pcrd)
{
    ...
    if (code < 0)
        return code;
    do { int j; for (j = 0; j < (sizeof(pcrd->caches.EncodeLMN.caches) / sizeof((pcrd->caches.EncodeLMN.caches)[0])); j++) { cie_cache_floats *pcf = &(pcrd->caches.EncodeLMN.caches)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcrd->DomainLMN.ranges)[j], "EncodeLMN"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcrd->EncodeLMN)->procs[j])(v, pcrd); do { } while (0); } pcf->params.is_identity = (&pcrd->EncodeLMN)->procs[j] == (Encode_default).procs[j]; } } while (0);

    cache3_set_linear(&pcrd->caches.EncodeLMN);
    do { int j; for (j = 0; j < (sizeof(pcrd->caches.EncodeABC) / sizeof((pcrd->caches.EncodeABC)[0])); j++) { cie_cache_floats *pcf = &(pcrd->caches.EncodeABC)[j].floats; int i; gs_sample_loop_params_t lp; gs_cie_cache_init(&pcf->params, &lp, &(pcrd->DomainABC.ranges)[j], "EncodeABC"); for (i = 0; i <= lp.N; ++i) { float v = ( (((lp).N - (i)) * (lp).A + (i) * (lp).B) / (lp).N ); pcf->values[i] = (*(&pcrd->EncodeABC)->procs[j])(v, pcrd); do { } while (0); } pcf->params.is_identity = (&pcrd->EncodeABC)->procs[j] == (Encode_default).procs[j]; } } while (0);
    ...
    pcrd->MatrixABCEncode = pcrd->MatrixABC;
    {
        ...
        for (c = 0; c < 3; c++) {
            ...
            if (pcrd->RenderTable.lookup.table == 0) {
                ...
                pcache->fixeds.fracs.params.is_identity = ((bool)0);
            } else {
                int i;
...
# 1000 "./base/gscie.c"
                ...
                for (i = 0; i < (1 << 9); ++i) {
                    ...
                    pcache->fixeds.ints.values[i] =
                        ((uint)(itemp = (int)(v * (1 << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))))) >= ((n) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) ? (itemp < 0 ? 0 : ((n) << ((((1 << 2) * 8 - 9 - 2) < (10)) ? ((1 << 2) * 8 - 9 - 2) : (10))) - 1) : itemp);
                }
                pcache->fixeds.ints.params = pcache->floats.params;
                pcache->fixeds.ints.params.is_identity = ((bool)0);

            }
        }
# 1033 "./base/gscie.c"
        f = pcrd->caches.EncodeABC[0].floats.params.factor; pcrd->MatrixABCEncode.cu.u *= f; pcrd->MatrixABCEncode.cv.u *= f; pcrd->MatrixABCEncode.cw.u *= f; pcrd->EncodeABC_base[0] = (pcrd->caches.EncodeABC[0].floats.params.base * f);
        f = pcrd->caches.EncodeABC[1].floats.params.factor; pcrd->MatrixABCEncode.cu.v *= f; pcrd->MatrixABCEncode.cv.v *= f; pcrd->MatrixABCEncode.cw.v *= f; pcrd->EncodeABC_base[1] = (pcrd->caches.EncodeABC[1].floats.params.base * f);
        f = pcrd->caches.EncodeABC[2].floats.params.factor; pcrd->MatrixABCEncode.cu.w *= f; pcrd->MatrixABCEncode.cv.w *= f; pcrd->MatrixABCEncode.cw.w *= f; pcrd->EncodeABC_base[2] = (pcrd->caches.EncodeABC[2].floats.params.base * f);
...
gs_cie_cache_to_fracs(const cie_cache_floats *pfloats, cie_cache_fracs *pfracs)
{
    ...
    for (i = 0; i < (1 << 9); ++i)
        pfracs->values[i] = ((frac)(((pfloats->values[i]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
...
gs_cie_cs_common(const gs_state * pgs)
{
    ...
    return cie_cs_common_abc(((pgs)->color[0].color_space), &ignore_pabc);
}
...
# 1258 "./base/gscie.c"
    if (pcrd->caches.EncodeLMN.caches[0].floats.params.is_identity &&
        ...
        cie_matrix_mult3(&pcrd->MatrixABCEncode, &pcrd->MatrixPQR_inverse_LMN,
                         &mat3);
        pjc->skipEncodeLMN = ((bool)1);
    } else

    {
        do { } while (0);
        mat3 = pcrd->MatrixPQR_inverse_LMN;
        pjc->skipEncodeLMN = ((bool)0);
    }
...

    {
        ...
        cie_cache_mult3(&pjc->TransformPQR, &mat3, 0.001);
        pjc->skipPQR = ((bool)0);
    }
    ...
    if (pcie->caches.DecodeLMN[0].floats.params.is_identity &
        ...
        if (!pabc) {
            ...
            cie_cache3_set_interpolation(&pjc->DecodeLMN);
            pjc->skipDecodeLMN = ((bool)0);
            pjc->skipDecodeABC = ((bool)1);
        }
    } else
                    ...
                    gs_memory_t *mem)
{
    ...
    gs_imager_state *pis =
        (gs_imager_state *)(*(mem)->procs.alloc_struct)(mem, &st_imager_state, "gx_cie_to_xyz_alloc(imager state)");
    ...
    pjc = (gx_cie_joint_caches *)(*(mem)->procs.alloc_struct)(mem, &st_joint_caches, "gx_cie_to_xyz_free(joint caches)");

    if (pjc == 0) {
        ((mem)->procs.free_object(mem, pis, "gx_cie_to_xyz_alloc(imager state)"));
        return ((-25));
    }
...
gx_cie_to_xyz_free(gs_imager_state *pis)
{
    ...
    ((mem)->procs.free_object(mem, pis->cie_joint_caches, "gx_cie_to_xyz_free(joint caches)"));


    if (pis->icc_link_cache != ((void*)0)) {
        do { if (pis->icc_link_cache) { do { do { } while (0); (pis->icc_link_cache)->rc.ref_count += (-1); } while (0); if (!(pis->icc_link_cache)->rc.ref_count) { do { do { } while (0); (pis->icc_link_cache)->rc.free((pis->icc_link_cache)->rc.memory, (void *)(pis->icc_link_cache), "gx_cie_to_xyz_free"); } while (0); (pis->icc_link_cache) = 0; } else do { } while (0); } } while (0);
    }
    if (pis->icc_manager != ((void*)0)) {
        do { if (pis->icc_manager) { do { do { } while (0); (pis->icc_manager)->rc.ref_count += (-1); } while (0); if (!(pis->icc_manager)->rc.ref_count) { do { do { } while (0); (pis->icc_manager)->rc.free((pis->icc_manager)->rc.memory, (void *)(pis->icc_manager), "gx_cie_to_xyz_free"); } while (0); (pis->icc_manager) = 0; } else do { } while (0); } } while (0);
    }
    if (pis->icc_profile_cache != ((void*)0)) {
        do { if (pis->icc_profile_cache) { do { do { } while (0); (pis->icc_profile_cache)->rc.ref_count += (-1); } while (0); if (!(pis->icc_profile_cache)->rc.ref_count) { do { do { } while (0); (pis->icc_profile_cache)->rc.free((pis->icc_profile_cache)->rc.memory, (void *)(pis->icc_profile_cache), "gx_cie_to_xyz_free"); } while (0); (pis->icc_profile_cache) = 0; } else do { } while (0); } } while (0);
    }
    ((mem)->procs.free_object(mem, pis, "gx_cie_to_xyz_free(imager state)"));
}
