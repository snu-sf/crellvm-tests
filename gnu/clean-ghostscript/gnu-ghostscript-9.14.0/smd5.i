...
static const gs_memory_struct_type_t st_MD5E_state = { sizeof(stream_MD5E_state), "MD5Encode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
goto err;
    ...
    return s;
err:
    ((mem)->procs.free_object(mem, ss, "s_MD5E_make_stream"));
    ((mem)->procs.free_object(mem, s, "s_MD5E_make_stream"));
    return ((void*)0);
}
...
s_MD5C_make_stream(gs_memory_t *mem, stream *strm)
{
    ...
    byte *buffer = (*(mem)->procs.alloc_bytes)(mem, buffer_size, "s_MD5E_make_stream(buffer)");

    if (ss == ((void*)0) || s == ((void*)0) || buffer == ((void*)0))
        goto err;
    ss->templat = &s_MD5C_template;
    if (s_init_filter(s, ss, buffer, buffer_size, ((void*)0)) < 0)
        goto err;
    s->strm = strm;
    s->close_strm = ((bool)1);
    return s;
err:
    ((mem)->procs.free_object(mem, ss, "s_MD5E_make_stream"));
    ((mem)->procs.free_object(mem, s, "s_MD5E_make_stream"));
    ((mem)->procs.free_object(mem, buffer, "s_MD5E_make_stream"));
    return ((void*)0);
}
...
s_MD5C_get_digest(stream *s, byte *buf, int buf_length)
{
    ...
    gs_md5_state_t md5;
    byte b[16], *p;
    int l = (((16) < (buf_length)) ? (16) : (buf_length)), k;
