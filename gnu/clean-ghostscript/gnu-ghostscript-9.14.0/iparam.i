...
ref_param_key(const iparam_list * plist, gs_param_name pkey, ref * pkref)
{
    if (plist->int_keys) {
        ...
        return names_ref(plist->memory->gs_lib_ctx->gs_name_table, (const byte *)pkey, strlen(pkey), pkref, 0);
}
...
{
    if (((((const byte *)&((pref)->tas.type_attrs))[1]) == (t_name))) {
        ref nref;

        names_string_ref(plist->memory->gs_lib_ctx->gs_name_table, pref, &nref);
        key->data = nref.value.const_bytes;
        key->size = ((&nref)->tas.rsize);
        key->persistent = ((bool)0);
    } else if (((((const byte *)&((pref)->tas.type_attrs))[1]) == (t_integer))) {
        char istr[sizeof(long) * 8 / 3 + 2];
        int len;
        byte *buf;

        gs_sprintf(istr, "%""l" "d", pref->value.intval);
        len = strlen(istr);

        buf = (*(plist->memory)->procs.alloc_string)(plist->memory, len, "ref_to_key");
        if (!buf)
            return ((-25));
        key->data = buf;
        key->size = len;
        key->persistent = ((bool)1);
    } else
        return ((-20));
    return 0;
}
...
static int ref_param_begin_write_collection(gs_param_list *, gs_param_name, gs_param_dict *, gs_param_collection_type_t);
static int ref_param_end_write_collection(gs_param_list *, gs_param_name, gs_param_dict *);
static int ref_param_write_typed(gs_param_list *, gs_param_name, gs_param_typed_value *);
static int ref_param_get_next_key(gs_param_list *, gs_param_enumerator_t *, gs_param_key_t *);
static int ref_param_requested(const gs_param_list *, gs_param_name);
...
{
    ((pe)->value.intval = (((const gs_param_int_array *)pvalue)->data[i]), ((pe)->tas.type_attrs = ((t_integer) << 8) + (imemory_new_mask(imem))) );
...
{
    ((pe)->value.realval = (((const gs_param_float_array *)pvalue)->data[i]), ((pe)->tas.type_attrs = ((t_real) << 8) + (imemory_new_mask(imem))) );
                                 ...
                                 gs_param_collection_type_t coll_type)
{
    ...
    dict_param_list *dlist = (dict_param_list *)
        (*(plist->memory)->procs.alloc_bytes)(plist->memory, ((int)(sizeof(dict_param_list))), "ref_param_begin_write_collection");
    ...
    if (coll_type != gs_param_collection_array) {
        ...
        if (code >= 0) {
            code = dict_param_list_write(dlist, &dref, ((void*)0), imem);
            dlist->int_keys = coll_type == gs_param_collection_dict_int_keys;
        }
    } else {
        ref aref;

        code = gs_alloc_ref_array(imem, &aref, (0x10 + 0x20 +0x40), pvalue->size,
                                  "ref_param_begin_write_collection");
        if (code >= 0)
            code = array_new_indexed_plist_write(dlist, &aref, ((void*)0), imem);
    }
    if (code < 0)
        ((plist->memory)->procs.free_object(plist->memory, dlist, "ref_param_begin_write_collection"));
    else
                               ...
                               gs_param_dict * pvalue)
{
    ...
    ((plist->memory)->procs.free_object(plist->memory, pvalue->list, "ref_param_end_write_collection"));
    pvalue->list = 0;
    return code;
}
static int
                      ...
                      gs_param_typed_value * pvalue)
{
    ...
    switch (pvalue->type) {
        ...
        case gs_param_type_bool:
            ((&value)->value.boolval = (pvalue->value.b), ((&value)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
            break;
        case gs_param_type_int:
            ((&value)->value.intval = (pvalue->value.i), ((&value)->tas.type_attrs = ((t_integer) << 8) + (0)) );
            break;
        case gs_param_type_long:
            ((&value)->value.intval = (pvalue->value.l), ((&value)->tas.type_attrs = ((t_integer) << 8) + (0)) );
            break;
        case gs_param_type_float:
            ((&value)->value.realval = (pvalue->value.f), ((&value)->tas.type_attrs = ((t_real) << 8) + (0)) );
            break;
...
ref_param_requested(const gs_param_list * plist, gs_param_name pkey)
{
    ...
    if (!((((const byte *)&((&ciplist->u.w.wanted)->tas.type_attrs))[1]) == (t_dictionary)))
        return -1;
                          ...
                          ref *pvalue, uint size, client_name_t cname)
{
    ...
    if (!ref_param_requested((const gs_param_list *)iplist, pkey))
        return 0;
    code = gs_alloc_ref_array(iplist->ref_memory, pvalue, (0x10 + 0x20 +0x40), size, cname);
                             ...
                             gs_ref_memory_t *imem)
{
    ...
    if (pvalue->persistent)
        ...
        byte *pstr = (*((gs_memory_t *)imem)->procs.alloc_string)((gs_memory_t *)imem, n, "ref_param_write_string");


        if (pstr == 0)
            return ((-25));
        memcpy(pstr, pdata, n);
        ((pref)->value.bytes = (pstr), ((pref)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40) | imemory_space(imem))), ((pref)->tas.rsize = (n)));
    }
    return 0;
}
...
{
    return names_ref(mem->gs_lib_ctx->gs_name_table, pvalue->data, pvalue->size, pref, (pvalue->persistent ? 0 : 1));
                      ...
                      gs_param_key_t * key, ref_type * type)
{
    ...
    do {
        ...
        if (!stack_element)
            return 1;
    } while (index += 2, !((((const byte *)&((stack_element)->tas.type_attrs))[1]) == (t_name)));
    *type = (((const byte *)&((stack_element)->tas.type_attrs))[1]);
...
{
    int code =
        dict_put(&((dict_param_list *) plist)->dict, pkey, pvalue, ((void*)0));
                     ...
                     gs_param_key_t * key, ref_type * type)
{
    ...
    if (index < 0)
        return 1;
    *type = (((const byte *)&((&elt[1])->tas.type_attrs))[1]);
...
dict_param_list_write(dict_param_list *plist, ref *pdict, const ref *pwanted,
                      gs_ref_memory_t *imem)
{
    do { if ( !(((&*(&(&*pdict)->value.pdict->values))->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
                          ...
                          const ref * pvalue)
{
    ...
    if (!((((const byte *)&((pkey)->tas.type_attrs))[1]) == (t_integer)))
        return ((-20));
    do { if ( !((((const byte *)&((&*pkey)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*pkey)); if ( (ulong)(*pkey).value.intval >= (((arr)->tas.rsize)) ) return ((-15)); } while (0);
    if ( (avm_space)(((pvalue)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > ((avm_space)(((arr)->tas.type_attrs) & (((1 << 2) - 1) << 2))) ) return ((-7));
    eltp = arr->value.refs + pkey->value.intval;

    (*(eltp) = *(pvalue));
    ((eltp)->tas.type_attrs |= (imemory_new_mask(iplist->ref_memory)));
    return 0;
}
static int
array_new_indexed_plist_write(dict_param_list * plist, ref * parray,
                              const ref * pwanted, gs_ref_memory_t *imem)
{
    do { if ( !((((const byte *)&((&*parray)->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&*parray)); } while (0);
    do { if ( !(((&*parray)->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
    plist->u.w.write = array_new_indexed_param_write;
    ref_param_write_init((iparam_list *) plist, pwanted, imem);
    plist->dict = *parray;
    plist->int_keys = ((bool)1);
    return 0;
}
...
static int ref_param_begin_read_collection(gs_param_list *, gs_param_name, gs_param_dict *, gs_param_collection_type_t);
static int ref_param_end_read_collection(gs_param_list *, gs_param_name, gs_param_dict *);
static int ref_param_read_typed(gs_param_list *, gs_param_name, gs_param_typed_value *);
                         ...
                         gs_param_int_array * pvalue)
{
    ...
    piv = (int *)(*(plist->memory)->procs.alloc_byte_array)(plist->memory, size, sizeof(int), "ref_param_read_int_array");
    ...
    for (i = 0; i < size; i++) {
        ...
        if (!((((const byte *)&((&elt)->tas.type_attrs))[1]) == (t_integer))) {
            code = ((-20));
            break;
        }
    ...
    if (code < 0) {
        ((plist->memory)->procs.free_object(plist->memory, piv, "ref_param_read_int_array"));
                           ...
                           gs_param_float_array * pvalue)
{
    ...
    if (code != 0)
        return code;
    size = ((loc.pvalue)->tas.rsize);
    pfv = (float *)(*(plist->memory)->procs.alloc_byte_array)(plist->memory, size, sizeof(float), "ref_param_read_float_array");
    ...
    if (code < 0) {
        ((plist->memory)->procs.free_object(plist->memory, pfv, "ref_read_float_array_param"));
        return (*loc.presult = code);
    }
    pvalue->data = pfv;
    pvalue->size = size;
                            ...
                            gs_param_string_array * pvalue)
{
    ...
    size = ((loc.pvalue)->tas.rsize);
    psv = (gs_param_string *)
        (*(plist->memory)->procs.alloc_byte_array)(plist->memory, size, sizeof(gs_param_string), "ref_param_read_string_array");

    if (psv == 0)
        return ((-25));
    aref = *loc.pvalue;
    if (((((const byte *)&((&aref)->tas.type_attrs))[1]) == (t_array))) {
        for (i = 0; code >= 0 && i < size; i++) {
    ...
    if (code < 0) {
        ((plist->memory)->procs.free_object(plist->memory, psv, "ref_param_read_string_array"));
        return (*loc.presult = code);
    }
    pvalue->data = psv;
    pvalue->size = size;
                                ...
                                gs_param_collection_type_t coll_type)
{
    ...
    dlist = (dict_param_list *)
        (*(plist->memory)->procs.alloc_bytes)(plist->memory, ((int)(sizeof(dict_param_list))), "ref_param_begin_read_collection");

    if (dlist == 0)
        return ((-25));
    if (((((const byte *)&((loc.pvalue)->tas.type_attrs))[1]) == (t_dictionary))) {
        code = dict_param_list_read(dlist, loc.pvalue, ((void*)0), ((bool)0),
                                    iplist->ref_memory);
        dlist->int_keys = int_keys;
        if (code >= 0)
            pvalue->size = dict_length(loc.pvalue);
    } else if (int_keys && (((loc.pvalue)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))) {
        code = array_indexed_param_list_read(dlist, loc.pvalue, ((void*)0), ((bool)0),
                                             iplist->ref_memory);
        if (code >= 0)
            pvalue->size = ((loc.pvalue)->tas.rsize);
    } else
        code = ((-20));
    if (code < 0) {
        ((plist->memory)->procs.free_object(plist->memory, dlist, "ref_param_begin_write_collection"));
...
ref_param_end_read_collection(gs_param_list * plist, gs_param_name pkey,
                              gs_param_dict * pvalue)
{
    ((((dict_param_list *) pvalue->list)->memory)->procs.free_object(((dict_param_list *) pvalue->list)->memory, ((dict_param_list *) pvalue->list)->results, "iparam_list_release"));
    ((plist->memory)->procs.free_object(plist->memory, pvalue->list, "ref_param_end_read_collection"));
                     ...
                     gs_param_typed_value * pvalue)
{
    ...
    switch ((((const byte *)&((loc.pvalue)->tas.type_attrs))[1])) {
        ...
        case t_mixedarray:
        case t_shortarray:
            if ( !((((loc).pvalue)->tas.type_attrs) & (0x20)) ) return (*(loc).presult = (-7));
            if (((loc.pvalue)->tas.rsize) <= 0) {
...
ref_param_read_get_policy(gs_param_list * plist, gs_param_name pkey)
{
    ...
    if (!(((((const byte *)&((&iplist->u.r.policies)->tas.type_attrs))[1]) == (t_dictionary)) &&
          dict_find_string(&iplist->u.r.policies, pkey, &pvalue) > 0 &&
          ((((const byte *)&((pvalue)->tas.type_attrs))[1]) == (t_integer)))
        )
        return 1;
...
ref_param_read_string_value(gs_memory_t *mem, const iparam_loc * ploc, gs_param_string * pvalue)
{
    ...
    switch ((((const byte *)&((pref)->tas.type_attrs))[1])) {
        case t_name: {
            ...
            names_string_ref(mem->gs_lib_ctx->gs_name_table, pref, &nref);
            pvalue->data = nref.value.const_bytes;
            pvalue->size = ((&nref)->tas.rsize);
            pvalue->persistent = ((bool)1);
        }
            break;
        case t_string:
            if ( !((((*ploc).pvalue)->tas.type_attrs) & (0x20)) ) return (*(*ploc).presult = (-7));
        ...
        case t_astruct:
            ...
            if ( !((((*ploc).pvalue)->tas.type_attrs) & (0x20)) ) return (*(*ploc).presult = (-7));
            if ((*(mem)->procs.object_type)(mem, pref->value.pstruct) != &st_bytes)
                return (*(*ploc).presult = (-20));
            pvalue->data = ((byte *)((pref)->value.pstruct));
            pvalue->size = (*(mem)->procs.object_size)(mem, pref->value.pstruct);
            pvalue->persistent = ((bool)0);
            break;
        default:
            return (*(*ploc).presult = (-20));
...
ref_param_read_array(iparam_list * plist, gs_param_name pkey, iparam_loc * ploc)
{
    ...
    if (code != 0)
        return code;
    if (!(((ploc->pvalue)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return (*(*ploc).presult = (-20));
    if ( !((((*ploc).pvalue)->tas.type_attrs) & (0x20)) ) return (*(*ploc).presult = (-7));
    return 0;
}
               ...
               int type)
{
    ...
    if (code != 0)
        return code;
    if (type >= 0)
        if ( !((((const byte *)&(((*ploc).pvalue)->tas.type_attrs))[1]) == (type)) ) return (*(*ploc).presult = (-20));
    return 0;
}
                    ...
                    bool require_all, gs_ref_memory_t *imem)
{
    ...
    plist->count = count;
    plist->results = (int *)
        (*(plist->memory)->procs.alloc_byte_array)(plist->memory, count, sizeof(int), "ref_param_read_init");
...
array_indexed_param_read(iparam_list * plist, const ref * pkey, iparam_loc * ploc)
{
    ...
    if ( !((((const byte *)&((&*pkey)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*pkey));
    if (pkey->value.intval < 0 || pkey->value.intval >= ((arr)->tas.rsize))
        return 1;
                              ...
                              gs_ref_memory_t *ref_memory)
{
    ...
    if ( !(((&*parray)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_array) << 8) + (0x20))) ) return ((!((((const byte *)&((&*parray)->tas.type_attrs))[1]) == (t_array)) ? check_type_failed(&*parray) : (-7)));
    plist->u.r.read = array_indexed_param_read;
    plist->dict = *parray;
    code = ref_param_read_init(iplist, ((parray)->tas.rsize), ppolicies,
                      ...
                      gs_param_key_t * key, ref_type * type)
{
    ...
    for (; ptr < top; ptr += 2) {
        ...
        if (((((const byte *)&((ptr)->tas.type_attrs))[1]) == (t_name))) {
            int code = ref_to_key(ptr, key, plist);

            *type = (((const byte *)&((ptr)->tas.type_attrs))[1]);
            penum->intval = index;
            return code;
        }
    }
                     ...
                     gs_ref_memory_t *imem)
{
    ...
    if (pdict == 0) {
        ...
        do { if ( !(((&*(&(&*pdict)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
