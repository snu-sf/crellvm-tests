...
{
    (i_ctx_p->memory).reclaim = ireclaim;
    return 0;
}
...
ireclaim(gs_dual_memory_t * dmem, int space)
{
    ...
    if (space < 0) {
        ...
        mem = dmem->spaces.memories.indexed[space >> 2];
    }
    do { } while (0);
    ...
    if (space < 0) {
        ...
        (((gs_memory_t *) mem)->procs.status((gs_memory_t *) mem, &stats));
        allocated = stats.allocated;
        if (mem->stable_memory != (gs_memory_t *)mem) {
            ((mem->stable_memory)->procs.status(mem->stable_memory, &stats));
            allocated += stats.allocated;
        }
...
gs_vmreclaim(gs_dual_memory_t *dmem, bool global)
{
    ...
    i_ctx_t *i_ctx_p =
        (i_ctx_t *)((char *)dmem - ((int) ( (char *)&((i_ctx_t *)0)->memory - (char *)((i_ctx_t *)0) )));
    ...
    memories[0] = dmem->spaces.memories.named.system;
    memories[1] = mem = dmem->spaces.memories.named.global;
    nmem = 2;
    if (lmem != dmem->spaces.memories.named.global)
        memories[nmem++] = lmem;
    for (i = nmem; --i >= 0;) {
        mem = memories[i];
        if (mem->stable_memory != (gs_memory_t *)mem)
    ...
    for (i = (global ? i_vm_system : i_vm_local);
         i < (sizeof(dmem->spaces.memories.indexed) / sizeof((dmem->spaces.memories.indexed)[0]));
         ++i
         ) {
...

    {
        ...
        gs_register_struct_root((gs_memory_t *)lmem, &context_root,
                                &ctxp, "i_ctx_p root");
        ((&dmem->spaces)->vm_reclaim(&dmem->spaces, global));
        (*((gs_memory_t *)lmem)->procs.unregister_root)((gs_memory_t *)lmem, &context_root, "i_ctx_p root");
        i_ctx_p = ctxp;
        dmem = &i_ctx_p->memory;
    }
    ...
    *(&(i_ctx_p->dict_stack).system_dict) = *ref_stack_index(&((i_ctx_p->dict_stack).stack), ref_stack_count(&((i_ctx_p->dict_stack).stack)) - 1);



    dstack_gc_cleanup(&(i_ctx_p->dict_stack));
