...
{
    return gs_initmatrix((i_ctx_p->pgs));
}
...
zdefaultmatrix(i_ctx_t *i_ctx_p)
{
    ...
    gs_defaultmatrix((i_ctx_p->pgs), &mat);
    return write_matrix_in(op, &mat, (&(i_ctx_p->memory)), ((void*)0));
}
...
zcurrentmatrix(i_ctx_t *i_ctx_p)
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    gs_matrix mat;
    int code = gs_currentmatrix((i_ctx_p->pgs), &mat);

    if (code < 0)
        return code;
    do { if ( (op += (6)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (6); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
...
zsetmatrix(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    if ((code = gs_setmatrix((i_ctx_p->pgs), &mat)) < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (6));
    return 0;
}
...
zsetdefaultmatrix(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_null)))
        code = gs_setdefaultmatrix((i_ctx_p->pgs), ((void*)0));
    else {
        gs_matrix mat;
        ...
        if (code < 0)
            return code;
        code = gs_setdefaultmatrix((i_ctx_p->pgs), &mat);
    }
    if (code < 0)
        return code;
...
ztranslate(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = num_params(op, 2, trans)) >= 0) {
        code = gs_translate((i_ctx_p->pgs), trans[0], trans[1]);
        if (code < 0)
            return code;
    } else {
        gs_matrix mat;
        ...
        if ((code = num_params(op - 1, 2, trans)) < 0 ||
            ...
            (code = write_matrix_in(op, &mat, (&(i_ctx_p->memory)), ((void*)0))) < 0
            ) {
...
zscale(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = num_params(op, 2, scale)) >= 0) {
        code = gs_scale((i_ctx_p->pgs), scale[0], scale[1]);
        if (code < 0)
            return code;
    } else {
        gs_matrix mat;
...
zrotate(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = real_param(op, &ang)) >= 0) {
        code = gs_rotate((i_ctx_p->pgs), ang);
        if (code < 0)
            return code;
    } else {
        gs_matrix mat;
...
zconcat(i_ctx_t *i_ctx_p)
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    gs_matrix mat;
    int code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op, &mat);
...
zconcatmatrix(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op - 2, &m1)) < 0 ||
        (code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, &m2)) < 0 ||
        (code = gs_matrix_multiply(&m1, &m2, &mp)) < 0 ||
        (code = write_matrix_in(op, &mp, (&(i_ctx_p->memory)), ((void*)0))) < 0
        )
        return code;
    op[-2] = *op;
    ((((i_ctx_p->op_stack).stack).p) -= (2));
        ...
        int (*matproc)(double, double, const gs_matrix *, gs_point *))
{
    ...
    switch ((((const byte *)&((op)->tas.type_attrs))[1])) {
        ...
        case t_mixedarray: {
            ...
            if ((code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op, pmat)) < 0 ||
    ...
    if ((code = (*ptproc) ((i_ctx_p->pgs), opxy[0], opxy[1], &pt)) < 0)
        return code;
out:
...
zinvertmatrix(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, &m)) < 0 ||
        (code = gs_matrix_invert(&m, &m)) < 0 ||
        (code = write_matrix_in(op, &m, (&(i_ctx_p->memory)), ((void*)0))) < 0
        )
        return code;
    op[-1] = *op;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
...
zbbox_transform(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op, &m)) < 0)
        return code;

    if (!(((op - 1)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return (check_type_failed(op - 1));
    do { if ( !(((&op[-1])->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if (((op - 1)->tas.rsize) != 4)
        return ((-15));
    if ((code = process_float_array(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, 4, bbox) < 0))
        return code;
    ...
    ((op - 3)->value.realval = ((float)aa.x), ((op - 3)->tas.type_attrs = ((t_real) << 8) + (0)) );
    ((op - 2)->value.realval = ((float)aa.y), ((op - 2)->tas.type_attrs = ((t_real) << 8) + (0)) );
    ((op - 1)->value.realval = ((float)zz.x), ((op - 1)->tas.type_attrs = ((t_real) << 8) + (0)) );
    ((op)->value.realval = ((float)zz.y), ((op)->tas.type_attrs = ((t_real) << 8) + (0)) );
    return 0;
}
