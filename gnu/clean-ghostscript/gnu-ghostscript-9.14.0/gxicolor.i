...
# 24 "./base/gxicolor.c" 2
# 1 "./base/gxfrac.h" 1
# 27 "./base/gxfrac.h"
typedef short frac;
typedef short signed_frac;
# 25 "./base/gxicolor.c" 2
# 1 "./base/gxarith.h" 1
# 27 "./base/gxarith.h"
...
irender_proc_t gs_image_class_4_color(gx_image_enum *penum);

static int image_render_color_DeviceN(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_color_icc(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_color_thresh(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
...
gs_image_class_4_color(gx_image_enum * penum)
{
    ...
    bool use_fast_thresh = ((bool)1);



    bool is_planar_dev = ((bool)0);
    ...
    if ( (gs_color_space_get_index(penum->pcs) == gs_color_space_index_DeviceN &&
        penum->pcs->cmm_icc_profile_data == ((void*)0)) || penum->use_mask_color ||
        !std_cmap_procs) {
        ...
        if (((( (penum->dev)->color_info.num_components > 1 || (penum->dev)->color_info.gray_index == 0xff ) ? (penum->dev)->color_info.max_color : (penum->dev)->color_info.max_gray) < 31) && use_fast_thresh &&
            ...
            switch ( penum->map[k].decoding ) {
                ...
                case sd_lookup:
                    ...
                    *curr_pos = (unsigned char) temp;
                    break;
                case sd_compute:
                    temp = penum->map[k].decode_lookup[0] +
...
{
    const gx_image_enum *const penum = penum_orig;
    ...
    if (penum->icc_link == ((void*)0)) {
        return gs_throw_imp(__func__, "./base/gxicolor.c", 318, 1, -1, "ICC Link not created during image render color");
    }
    ...
    if (num_des_comps != 1 && planar_out == ((bool)1)) {
        force_planar = ((bool)1);
    }
    if (penum->icc_link->is_identity && !need_decode && !force_planar) {
        ...
        *psrc_cm = (unsigned char *) psrc;
        spp_cm = spp;
        *bufend = *psrc_cm + w;
        *psrc_cm_start = ((void*)0);
    } else {
        spp_cm = num_des_comps;
        *psrc_cm = (*(pis->memory)->procs.alloc_bytes)(pis->memory, w * spp_cm/spp, "image_color_icc_prep");
        ...
        if (penum->icc_link->is_identity) {
            if (!force_planar) {
                ...
                if (need_decode) {
                    ...
                    psrc_decode = (*(pis->memory)->procs.alloc_bytes)(pis->memory, w, "image_color_icc_prep");
                ...
                if (psrc_decode != ((void*)0)) {
                    ((pis->memory)->procs.free_object(pis->memory, (byte *) psrc_decode, "image_render_color_icc"));
            ...
            gsicc_init_buffer(&input_buff_desc, spp, 1,
                          ((bool)0), ((bool)0), ((bool)0), 0, w,
                          1, num_pixels);
            if (!force_planar) {
                gsicc_init_buffer(&output_buff_desc, spp_cm, 1,
                              ((bool)0), ((bool)0), ((bool)0), 0, num_pixels * spp_cm,
                              1, num_pixels);
            } else {
                gsicc_init_buffer(&output_buff_desc, spp_cm, 1,
                              ((bool)0), ((bool)0), ((bool)1), w/spp, w/spp,
                              1, num_pixels);
            }
            ...
            if (need_decode) {
                ...
                (penum->icc_link->procs.map_buffer)(dev, penum->icc_link,
                                                    ...
                                                    (void*) psrc_decode,
                                                    (void*) *psrc_cm);
                ((pis->memory)->procs.free_object(pis->memory, psrc_decode, "image_color_icc_prep"));
            } else {
                   ...
                   uint w, int h, gx_device * dev)
{
    ...
    int code = 0;
    int spp_cm = 0;
    byte *psrc_cm = ((void*)0), *psrc_cm_start = ((void*)0);
    byte *bufend = ((void*)0), *curr_ptr;
        ...
        if (penum->icc_setup.has_transfer) {
            for (k = 0; k < num_des_comp; k++) {
                ...
                for (j = 0; j < psrc_planestride; j++, curr_ptr++) {
                    conc = (((curr_ptr[0]) << ((sizeof(gx_color_value) * 8) - 8)) + ((curr_ptr[0]) >> (16 - (sizeof(gx_color_value) * 8))));
                    cmap_transfer_plane(&(conc), penum->pis, penum->dev, k);
                    curr_ptr[0] = ((gx_color_value)((((unsigned int)conc)*0xFF01 + 0x800000)>>24));
                }
            }
        }
    } else {
                        ...
                        if (src_size == dest_height) {
                            ...
                            for (k = 0; k < data_length; k+=2) {
                                ...
                                devc_contone_gray[position] =
                                    devc_contone_gray[position + ((1 << 3)*8)] =
                                    psrc_cm[offset];
                                position += 2*((1 << 3)*8);
                            }
                        } else {
...
flush:
    ...
    if (psrc_cm_start != ((void*)0)) {
        ((penum->pis->memory)->procs.free_object(penum->pis->memory, (byte *)psrc_cm_start, "image_render_color_thresh"));
                   ...
                   uint w, int h, gx_device * dev)
{
    ...
    pdevc->type = (&gx_dc_type_data_none);
    pdevc_next->type = (&gx_dc_type_data_none);
    if (h == 0)
        return 0;
    code = image_color_icc_prep(penum_orig, psrc, w, dev, &spp_cm, &psrc_cm,
                                &psrc_cm_start, &bufend, ((bool)0));
    if (code < 0) return code;

    memset(&(conc[0]), 0, sizeof(gx_color_value[(8 * 8)]));
    pnext = penum->dda.pixel0;
    xrun = xprev = ((pnext.x).state).Q;
    yrun = yprev = ((pnext.y).state).Q;
    pdyx = ((penum->dda.row.x).state).Q - penum->cur.x;
    pdyy = ((penum->dda.row.y).state).Q - penum->cur.y;
    switch (posture) {
        case image_portrait:
            vci = penum->yci, vdi = penum->hci;
            irun = ((int)(((xrun)+((1<<8)>>1)) >> (8)));
            break;
        case image_landscape:
        default:
            vci = penum->xci, vdi = penum->wci;
            irun = ((int)(((yrun)+((1<<8)>>1)) >> (8)));
            break;
    }
    do { } while (0);
    ...
    while (psrc_cm < bufend) {
        do { ((pnext.x).state).R -= ((pnext.x).step).dR; if ((signed)((pnext.x).state).R < 0) { ((pnext.x).state).Q++; ((pnext.x).state).R += ((pnext.x).step).N; }; ((pnext.x).state).Q += ((pnext.x).step).dQ; } while (0);
        do { ((pnext.y).state).R -= ((pnext.y).step).dR; if ((signed)((pnext.y).state).R < 0) { ((pnext.y).state).Q++; ((pnext.y).state).R += ((pnext.y).step).N; }; ((pnext.y).state).Q += ((pnext.y).step).dQ; } while (0);
        if ( penum->alpha ) {
        ...
        for ( k = 0; k < spp_cm; k++ ) {
            conc[k] = (((next.v[k]) << ((sizeof(gx_color_value) * 8) - 8)) + ((next.v[k]) >> (16 - (sizeof(gx_color_value) * 8))));
        }
        ...
        if (must_halftone || has_transfer) {
            ...
            color = ((dev)->procs.encode_color)(dev, &(conc[0]));

            if (color != ((gx_color_index)(~0)))
                ((pdevc_next)->colors.pure = (color), (pdevc_next)->type = (&gx_dc_type_data_pure));
        }
# 928 "./base/gxicolor.c"
        switch (posture) {
        case image_portrait:
            {
                int xi = irun;
                int wi = (irun = ((int)(((xprev)+((1<<8)>>1)) >> (8)))) - xi;

                if (wi < 0)
                    xi += wi, wi = -wi;
                if (wi > 0)
                    code = (*(pdevc)->type->fill_rectangle)(pdevc, xi, vci, wi, vdi, dev, lop, ((void*)0));
        ...
        case image_landscape:
            {
                int yi = irun;
                int hi = (irun = ((int)(((yprev)+((1<<8)>>1)) >> (8)))) - yi;

                if (hi < 0)
                    yi += hi, hi = -hi;
                if (hi > 0)
                    code = (*(pdevc)->type->fill_rectangle)(pdevc, vci, yi, vdi, hi, dev, lop, ((void*)0));

            }
            break;
        default:
            {
                code = (*((dev)->procs.fill_parallelogram))
        ...
        pdevc_next = ptemp;
        run = next;
inc: xprev = ((pnext.x).state).Q;
        yprev = ((pnext.y).state).Q;
    }
    ...
    if (psrc_cm_start != ((void*)0)) {
        ((pis->memory)->procs.free_object(pis->memory, (byte *)psrc_cm_start, "image_render_color_icc"));
    }
                   ...
                   uint w, int h, gx_device * dev)
{
    ...
    if (gs_color_space_is_ICC(pcs) && pcs->cmm_icc_profile_data != ((void*)0) &&
    ...
    memset(&run, 0, sizeof(run));
    memset(&next, 0, sizeof(next));
    ((&cc)->pattern = 0, (*(pcs)->type->init_color)(&cc, pcs));
    run.v[0] = ~psrc[0];
    while (psrc < bufend) {
        do { ((pnext.x).state).R -= ((pnext.x).step).dR; if ((signed)((pnext.x).state).R < 0) { ((pnext.x).state).Q++; ((pnext.x).state).R += ((pnext.x).step).N; }; ((pnext.x).state).Q += ((pnext.x).step).dQ; } while (0);
        do { ((pnext.y).state).R -= ((pnext.y).step).dR; if ((signed)((pnext.y).state).R < 0) { ((pnext.y).state).Q++; ((pnext.y).state).R += ((pnext.y).step).N; }; ((pnext.y).state).Q += ((pnext.y).step).dQ; } while (0);
        ...
        if ((next.all[0] & mask) == test &&
            ...
            ((pdevc_next)->type = (&gx_dc_type_data_null));
            goto mapped;
        }
        ...
        if (lab_case)
            ...
            for (i = 0; i < spp; ++i)
                switch ( penum->map[i].decoding ) { case sd_none: cc.paint.values[i] = (next.v[i]) * (1.0f / 255.0f); break; case sd_lookup: cc.paint.values[i] = penum->map[i].decode_lookup[(next.v[i]) >> 4]; break; case sd_compute: cc.paint.values[i] = penum->map[i].decode_lookup[0] + (next.v[i]) * penum->map[i].decode_factor; };
# 1142 "./base/gxicolor.c"
...
mapped: if (mcode < 0)
        ...
        if (sizeof(pdevc_next->colors.binary.color[0]) <= sizeof(ulong))
            ...
            do { } while (0);
# 1165 "./base/gxicolor.c"
        if (posture != image_skewed && gx_device_color_equal(&(*pdevc), &(*pdevc_next)))
            goto set;
fill:
