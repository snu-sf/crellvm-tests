...
{
    gx_color_index (*encode_proc)(gx_device * dev, const gx_color_value colors[]);


    if ((encode_proc = ((dev)->procs.encode_color)) == 0) {
        if (dev->color_info.num_components == 1 &&
            ((dev)->procs.map_rgb_color) != 0) {
            ...
            encode_proc = gx_backwards_compatible_gray_encode;
             ...
             (encode_proc = ((dev)->procs.map_rgb_color)) != 0 )
            set_cinfo_polarity(dev, GX_CINFO_POLARITY_ADDITIVE);
        else if ( dev->color_info.num_components == 4 &&
                 (encode_proc = ((dev)->procs.map_cmyk_color)) != 0 )
...
is_like_DeviceRGB(gx_device * dev)
{
    ...
    if ( dev->color_info.num_components != 3 ||
        ...
        return ((bool)0);
    cm_procs = ((dev)->procs.get_color_mapping_procs)(dev);
    ...
    cm_comp_fracs[0] -= ((frac)0x7ff8) / 4;
    cm_comp_fracs[1] -= ((frac)0x7ff8) / 3;
    cm_comp_fracs[2] -= 3 * ((frac)0x7ff8) / 4;
    for ( i = 0;
           i < 3 &&
           -((frac)0x7ff8) / 100 < cm_comp_fracs[i] &&
           cm_comp_fracs[i] < ((frac)0x7ff8) / 100;
          i++ )
        ;
    return i == 3;
}
    ...
    gx_color_value cv[1] )
{
    ...
    int code = ((dev)->procs.map_color_rgb)(dev, color, rgb);
    ...
    gx_color_value cv[1] )
{
    ...
    cv[0] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - rgb[0];
    return code;
}
# 232 "./base/gdevdflt.c"
static int
    ...
    gx_color_value cv[4] )
{
    ...
    if (dev->color_info.separable_and_linear == GX_CINFO_SEP_LIN)
        ...
        int i, code = ((dev)->procs.map_color_rgb)(dev, color, cv);
        gx_color_value min_val = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1));

        for (i = 0; i < 3; i++) {
            if ((cv[i] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - cv[i]) < min_val)
                min_val = cv[i];
        }
        for (i = 0; i < 3; i++)
            cv[i] -= min_val;
...
gx_1bit_cmyk_decode_color(
    ...
    gx_color_value cv[4] )
{
    cv[0] = ((color & 0x8) != 0 ? ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) : 0);
    cv[1] = ((color & 0x4) != 0 ? ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) : 0);
    cv[2] = ((color & 0x2) != 0 ? ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) : 0);
    cv[3] = ((color & 0x1) != 0 ? ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) : 0);
    return 0;
}
...
(*get_decode_color(gx_device * dev))(gx_device *, gx_color_index, gx_color_value *)
{
    ...
    if (((dev)->procs.decode_color) != 0)
        return ((dev)->procs.decode_color);
    ...
    if (((dev)->procs.map_color_rgb) != 0) {


        if (is_like_DeviceRGB(dev))
            return ((dev)->procs.map_color_rgb);
...
# 317 "./base/gdevdflt.c"
        if (is_like_DeviceCMYK(dev)) {
            if (((dev)->procs.map_color_rgb) == cmyk_1bit_map_color_rgb)
...
set_linear_color_bits_mask_shift(gx_device * dev)
{
    ...
    (dev->color_info.comp_shift)[num_components - 1] = 0;
    for ( i = num_components - 1 - 1; i >= 0; i-- ) {
        (dev->color_info.comp_shift)[i] = (dev->color_info.comp_shift)[i + 1] +
            ( i == gray_index ? ilog2(max_gray + 1) : ilog2(max_color + 1) );
    }
    for ( i = 0; i < num_components; i++ ) {
        (dev->color_info.comp_bits)[i] = ( i == gray_index ?
                         ilog2(max_gray + 1) :
                         ilog2(max_color + 1) );
        (dev->color_info.comp_mask)[i] = (((gx_color_index)1 << (dev->color_info.comp_bits)[i]) - 1)
                                               << (dev->color_info.comp_shift)[i];
    }
...
check_device_separable(gx_device * dev)
{
    ...
    gx_color_value colorants[(8 * 8)] = { 0 };


    if (pinfo->separable_and_linear != GX_CINFO_UNKNOWN_SEP_LIN)
        return;

    if (((dev)->procs.encode_color) == ((void*)0))
        return;
    ...
    if (!((((pinfo->dither_grays) - 1) & (pinfo->dither_grays)) == 0)
                    || !((((pinfo->dither_colors) - 1) & (pinfo->dither_colors)) == 0))
        return;




    color_index = ((dev)->procs.encode_color)(dev, colorants);
    if (color_index != 0)
        return;
    for (i = 0; i < num_components; i++) {
        ...
        for (j = 0; j < num_components; j++)
            colorants[j] = 0;
        colorants[i] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1));
        color_index = ((dev)->procs.encode_color)(dev, colorants);
        if (color_index == 0)
            return;
        if (color_index & current_bits)
            return;
        ...
        for (j = 0; j < num_components; j++)
            colorants[j] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1));
        colorants[i] = 0;
        color_index = ((dev)->procs.encode_color)(dev, colorants);
        if (color_index & comp_mask[i])
            return;
    }
...
gx_device_fill_in_procs(register gx_device * dev)
{
    ...
    if ( ((dev)->procs.open_device) == 0 ) ((dev)->procs.open_device = (gx_default_open_device));
    if ( ((dev)->procs.get_initial_matrix) == 0 ) ((dev)->procs.get_initial_matrix = (gx_default_get_initial_matrix));
    if ( ((dev)->procs.sync_output) == 0 ) ((dev)->procs.sync_output = (gx_default_sync_output));
    if ( ((dev)->procs.output_page) == 0 ) ((dev)->procs.output_page = (gx_default_output_page));
    if ( ((dev)->procs.close_device) == 0 ) ((dev)->procs.close_device = (gx_default_close_device));

    if ( ((dev)->procs.map_color_rgb) == 0 ) ((dev)->procs.map_color_rgb = (gx_default_w_b_map_color_rgb));

    if ( ((dev)->procs.tile_rectangle) == 0 ) ((dev)->procs.tile_rectangle = (gx_default_tile_rectangle));
    if ( ((dev)->procs.copy_mono) == 0 ) ((dev)->procs.copy_mono = (gx_default_copy_mono));
    if ( ((dev)->procs.copy_color) == 0 ) ((dev)->procs.copy_color = (gx_default_copy_color));
    if ( ((dev)->procs.obsolete_draw_line) == 0 ) ((dev)->procs.obsolete_draw_line = (gx_default_draw_line));
    if ( ((dev)->procs.get_bits) == 0 ) ((dev)->procs.get_bits = (gx_default_get_bits));
    ...
    if ( ((dev)->procs.get_xfont_procs) == 0 ) ((dev)->procs.get_xfont_procs = (gx_default_get_xfont_procs));
    if ( ((dev)->procs.get_xfont_device) == 0 ) ((dev)->procs.get_xfont_device = (gx_default_get_xfont_device));
    if ( ((dev)->procs.map_rgb_alpha_color) == 0 ) ((dev)->procs.map_rgb_alpha_color = (gx_default_map_rgb_alpha_color));
    ...
    if ( ((dev)->procs.copy_alpha) == 0 ) ((dev)->procs.copy_alpha = (gx_default_copy_alpha));
    if ( ((dev)->procs.get_band) == 0 ) ((dev)->procs.get_band = (gx_default_get_band));
    if ( ((dev)->procs.copy_rop) == 0 ) ((dev)->procs.copy_rop = (gx_default_copy_rop));
    if ( ((dev)->procs.fill_path) == 0 ) ((dev)->procs.fill_path = (gx_default_fill_path));
    if ( ((dev)->procs.stroke_path) == 0 ) ((dev)->procs.stroke_path = (gx_default_stroke_path));
    if ( ((dev)->procs.fill_mask) == 0 ) ((dev)->procs.fill_mask = (gx_default_fill_mask));
    if ( ((dev)->procs.fill_trapezoid) == 0 ) ((dev)->procs.fill_trapezoid = (gx_default_fill_trapezoid));
    if ( ((dev)->procs.fill_parallelogram) == 0 ) ((dev)->procs.fill_parallelogram = (gx_default_fill_parallelogram));
    if ( ((dev)->procs.fill_triangle) == 0 ) ((dev)->procs.fill_triangle = (gx_default_fill_triangle));
    if ( ((dev)->procs.draw_thin_line) == 0 ) ((dev)->procs.draw_thin_line = (gx_default_draw_thin_line));
    if ( ((dev)->procs.begin_image) == 0 ) ((dev)->procs.begin_image = (gx_default_begin_image));
# 554 "./base/gdevdflt.c"
    do { } while (0);

    ((dev)->procs.get_alpha_bits = (gx_default_get_alpha_bits));
    do { } while (0);
    ((dev)->procs.image_data = (gx_default_image_data));
    do { } while (0);
    ((dev)->procs.end_image = (gx_default_end_image));

    if ( ((dev)->procs.strip_tile_rectangle) == 0 ) ((dev)->procs.strip_tile_rectangle = (gx_default_strip_tile_rectangle));
    if ( ((dev)->procs.strip_copy_rop) == 0 ) ((dev)->procs.strip_copy_rop = (gx_default_strip_copy_rop));
    if ( ((dev)->procs.strip_copy_rop2) == 0 ) ((dev)->procs.strip_copy_rop2 = (gx_default_strip_copy_rop2));
    if ( ((dev)->procs.strip_tile_rect_devn) == 0 ) ((dev)->procs.strip_tile_rect_devn = (gx_default_strip_tile_rect_devn));
    if ( ((dev)->procs.get_clipping_box) == 0 ) ((dev)->procs.get_clipping_box = (gx_default_get_clipping_box));
    if ( ((dev)->procs.begin_typed_image) == 0 ) ((dev)->procs.begin_typed_image = (gx_default_begin_typed_image));
    if ( ((dev)->procs.get_bits_rectangle) == 0 ) ((dev)->procs.get_bits_rectangle = (gx_default_get_bits_rectangle));
    if ( ((dev)->procs.map_color_rgb_alpha) == 0 ) ((dev)->procs.map_color_rgb_alpha = (gx_default_map_color_rgb_alpha));
    if ( ((dev)->procs.create_compositor) == 0 ) ((dev)->procs.create_compositor = (gx_default_create_compositor));
    if ( ((dev)->procs.get_hardware_params) == 0 ) ((dev)->procs.get_hardware_params = (gx_default_get_hardware_params));
    if ( ((dev)->procs.text_begin) == 0 ) ((dev)->procs.text_begin = (gx_default_text_begin));
    if ( ((dev)->procs.finish_copydevice) == 0 ) ((dev)->procs.finish_copydevice = (gx_default_finish_copydevice));

    ((dev)->procs.encode_color = (get_encode_color(dev)));
    if (dev->color_info.num_components == 3)
        ((dev)->procs.map_rgb_color = (((dev)->procs.encode_color)));
    if (dev->color_info.num_components == 4)
        ((dev)->procs.map_cmyk_color = (((dev)->procs.encode_color)));

    if ( dev->color_info.separable_and_linear == GX_CINFO_SEP_LIN ) {
        if ( ((dev)->procs.encode_color) == 0 ) ((dev)->procs.encode_color = (gx_default_encode_color));
        if ( ((dev)->procs.map_cmyk_color) == 0 ) ((dev)->procs.map_cmyk_color = (gx_default_encode_color));
        if ( ((dev)->procs.map_rgb_color) == 0 ) ((dev)->procs.map_rgb_color = (gx_default_encode_color));
    } else {

        if ( ((dev)->procs.encode_color) == 0 ) ((dev)->procs.encode_color = (gx_error_encode_color));
        if ( ((dev)->procs.map_cmyk_color) == 0 ) ((dev)->procs.map_cmyk_color = (gx_error_encode_color));
        if ( ((dev)->procs.map_rgb_color) == 0 ) ((dev)->procs.map_rgb_color = (gx_error_encode_color));
    }
# 600 "./base/gdevdflt.c"
    switch (dev->color_info.num_components) {
    case 1:
        if (((dev)->procs.get_color_mapping_procs) == ((void*)0)) {




            if (dev->color_info.polarity == GX_CINFO_POLARITY_ADDITIVE) {
                if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_default_DevGray_get_color_mapping_procs));


            } else
                if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_error_get_color_mapping_procs));
        }
        if ( ((dev)->procs.get_color_comp_index) == 0 ) ((dev)->procs.get_color_comp_index = (gx_default_DevGray_get_color_comp_index));
    ...
    case 3:
        if (((dev)->procs.get_color_mapping_procs) == ((void*)0)) {
            if (dev->color_info.polarity == GX_CINFO_POLARITY_ADDITIVE) {
                if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_default_DevRGB_get_color_mapping_procs));


                if ( ((dev)->procs.get_color_comp_index) == 0 ) ((dev)->procs.get_color_comp_index = (gx_default_DevRGB_get_color_comp_index));


            } else {
                if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_error_get_color_mapping_procs));
                if ( ((dev)->procs.get_color_comp_index) == 0 ) ((dev)->procs.get_color_comp_index = (gx_error_get_color_comp_index));
            }
        }
        break;

    case 4:
        if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_default_DevCMYK_get_color_mapping_procs));
        if ( ((dev)->procs.get_color_comp_index) == 0 ) ((dev)->procs.get_color_comp_index = (gx_default_DevCMYK_get_color_comp_index));
        break;
    default:
        if (((dev)->procs.get_color_mapping_procs) == ((void*)0)) {
            if ( ((dev)->procs.get_color_mapping_procs) == 0 ) ((dev)->procs.get_color_mapping_procs = (gx_error_get_color_mapping_procs));
    ...
    ((dev)->procs.decode_color = (get_decode_color(dev)));
    if ( ((dev)->procs.map_color_rgb) == 0 ) ((dev)->procs.map_color_rgb = (gx_default_w_b_map_color_rgb));
    if ( ((dev)->procs.get_profile) == 0 ) ((dev)->procs.get_profile = (gx_default_get_profile));
    if ( ((dev)->procs.set_graphics_type_tag) == 0 ) ((dev)->procs.set_graphics_type_tag = (gx_default_set_graphics_type_tag));
# 658 "./base/gdevdflt.c"
    if ( dev->color_info.opmode == GX_CINFO_OPMODE_UNKNOWN &&
         (dev->color_info.num_components < 4 ||
          dev->color_info.polarity == GX_CINFO_POLARITY_ADDITIVE ||
          dev->color_info.gray_index == 0xff ) )
        dev->color_info.opmode = GX_CINFO_OPMODE_NOT;

    if ( ((dev)->procs.fill_rectangle_hl_color) == 0 ) ((dev)->procs.fill_rectangle_hl_color = (gx_default_fill_rectangle_hl_color));
    if ( ((dev)->procs.include_color_space) == 0 ) ((dev)->procs.include_color_space = (gx_default_include_color_space));
    if ( ((dev)->procs.fill_linear_color_scanline) == 0 ) ((dev)->procs.fill_linear_color_scanline = (gx_default_fill_linear_color_scanline));
    if ( ((dev)->procs.fill_linear_color_trapezoid) == 0 ) ((dev)->procs.fill_linear_color_trapezoid = (gx_default_fill_linear_color_trapezoid));
    if ( ((dev)->procs.fill_linear_color_triangle) == 0 ) ((dev)->procs.fill_linear_color_triangle = (gx_default_fill_linear_color_triangle));
    if ( ((dev)->procs.update_spot_equivalent_colors) == 0 ) ((dev)->procs.update_spot_equivalent_colors = (gx_default_update_spot_equivalent_colors));
    if ( ((dev)->procs.ret_devn_params) == 0 ) ((dev)->procs.ret_devn_params = (gx_default_ret_devn_params));
    if ( ((dev)->procs.fillpage) == 0 ) ((dev)->procs.fillpage = (gx_default_fillpage));
    if ( ((dev)->procs.copy_alpha_hl_color) == 0 ) ((dev)->procs.copy_alpha_hl_color = (gx_default_copy_alpha_hl_color));




    if ( ((dev)->procs.dev_spec_op) == 0 ) ((dev)->procs.dev_spec_op = (gx_default_dev_spec_op));

    if ( ((dev)->procs.process_page) == 0 ) ((dev)->procs.process_page = (gx_default_process_page));
}
...
{
    int code = ((dev)->procs.sync_output)(dev);
...
gx_default_get_clipping_box(gx_device * dev, gs_fixed_rect * pbox)
{
    pbox->p.x = 0;
    pbox->p.y = 0;
    pbox->q.x = ((fixed)(dev->width)<<8);
    pbox->q.y = ((fixed)(dev->height)<<8);
}
void
