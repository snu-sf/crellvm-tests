...
# 55 "./psi/ztype.c"
static const double min_int_real = ((((ps_int)-1) << ((sizeof(ps_int) * 8) - 1)) * 1.0 - 1);
static const double max_int_real = ((~((((ps_int)-1) << ((sizeof(ps_int) * 8) - 1)))) * 1.0 + 1);
# 66 "./psi/ztype.c"
static int
ztype(i_ctx_t *i_ctx_p)
{
    ...
    ref tnref;
    int code = array_get(((gs_memory_t *)((i_ctx_p->memory).current)), op, (long)((op - 1)->tas.type_attrs >= (t_next_index << 8) ? t_operator : (((const byte *)&((op - 1)->tas.type_attrs))[1])), &tnref);

    if (code < 0)
        return code;
    if (!((((const byte *)&((&tnref)->tas.type_attrs))[1]) == (t_name))) {

        if ( op < (((i_ctx_p->op_stack).stack).bot) + ((2) - 1) ) return ((-17));
        {
            const char *sname =
                ((const char *)gs_struct_type_name((*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), op[-1].value.pstruct)));

            int code = names_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, (const byte *)sname, strlen(sname), (ref *) (op - 1), 0);


            if (code < 0)
                return code;
        }
        ((op - 1)->tas.type_attrs |= (0x80));
    } else {
        (*(op - 1) = *(&tnref));
    }
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    static const char *const tnames[] = { 0,"booleantype","dicttype","filetype", "arraytype","packedarraytype","packedarraytype","arraytype", 0,0, "fonttype","integertype","marktype","nametype","nulltype", "operatortype","realtype","savetype","stringtype", "devicetype","operatortype" };
    int i;

    if ( (((i_ctx_p->op_stack).stack).top) - (((i_ctx_p->op_stack).stack).p) < (t_next_index) ) { ((i_ctx_p->op_stack).stack).requested = (t_next_index); return ((-16)); };
    for (i = 0; i < t_next_index; i++) {
        ref *const rtnp = op + 1 + i;

        if (i >= (sizeof(tnames) / sizeof((tnames)[0])) || tnames[i] == 0)
            (((rtnp)->tas.type_attrs = ((t_null) << 8) + (0)) );
        else {
            int code = names_enter_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, tnames[i], rtnp);

            if (code < 0)
                return code;
            ((rtnp)->tas.type_attrs |= (0x80));
        }
    }
    (((i_ctx_p->op_stack).stack).p) += t_next_index;
    return 0;
}
...
zcvx(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_operator)) &&
        ((opidx = (((op)->tas.rsize) == 0 ? op_find_index(op) : ((op)->tas.rsize))) == 0 ||
         (((&op_defs_all[(opidx) >> 4] [(opidx) & ((1 << 4) - 1)]))->oname[1] == '%'))
        )
        return ((-15));
    aop = (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary)) ? (&(op)->value.pdict->values) : op);
    ((aop)->tas.type_attrs |= (0x80));
...
zexecuteonly(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary)))
        return ((-20));
    return access_check(i_ctx_p, 0x40, ((bool)1));
}
...
znoaccess(i_ctx_t *i_ctx_p)
{
    ...
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_dictionary))) {
        ...
        if (!!(~((aop)->tas.type_attrs) & (0x10))) {
            if (!!(~((aop)->tas.type_attrs) & (0x20)) && !!(~((aop)->tas.type_attrs) & (0x40))) {
        ...
        if (dstack_dict_is_permanent(&(i_ctx_p->dict_stack), op))
            return ((-7));
    }
    return access_check(i_ctx_p, 0, ((bool)1));
}
...
{
    return access_check(i_ctx_p, (0x20 + 0x40), ((bool)1));
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    int code = access_check(i_ctx_p, 0x20, ((bool)0));
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    int code = access_check(i_ctx_p, 0x10, ((bool)0));
        ...
        case t_string:
            {
                ...
                (*(&str) = *(op));
                code = gs_scan_string_token_options(i_ctx_p, &str, &token, 0);
                if (code > 0)
                    code = ((-18));
                if (code < 0)
                    return code;
...
zcvn(i_ctx_t *i_ctx_p)
{
    ...
    return names_from_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op, op);
}
...
zcvr(i_ctx_t *i_ctx_p)
{
    ...
    switch ((((const byte *)&((op)->tas.type_attrs))[1])) {
        case t_integer:
            ((op)->value.realval = ((float)op->value.intval), ((op)->tas.type_attrs = ((t_real) << 8) + (0)) );
        case t_real:
            return 0;
        default:
            return (check_type_failed(op));
        case t_string:
            {
                ...
                switch ((((const byte *)&((&token)->tas.type_attrs))[1])) {
                    case t_integer:
                        ((op)->value.realval = ((float)token.value.intval), ((op)->tas.type_attrs = ((t_real) << 8) + (0)) );
                        return 0;
                    case t_real:
                        *op = token;
                        return 0;
...
zcvrs(i_ctx_t *i_ctx_p)
{
    ...
    if (radix == 10) {
        switch ((((const byte *)&((op - 2)->tas.type_attrs))[1])) {
            ...
            case t_real:
                {
                    int code = convert_to_string(((gs_memory_t *)((i_ctx_p->memory).current)), op - 2, op);
...
zcvs(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op) : (-7)));
    if ( op < (((i_ctx_p->op_stack).stack).bot) + ((2) - 1) ) return ((-17));
    code = convert_to_string(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, op);
    if (code >= 0)
        ((((i_ctx_p->op_stack).stack).p) -= (1));
    return code;
}
        ...
        case t_dictionary:
            ...
            if (modify) {
                if (!!(~((aop)->tas.type_attrs) & (access)))
                    return ((-7));
                ((void)((((((aop)->tas.type_attrs) & ((&(i_ctx_p->memory)))->test_mask) == 0) ? alloc_save_change((&(i_ctx_p->memory)), op, (ref_packed *)(aop), "access_check(modify)") : 0)));
...
convert_to_string(const gs_memory_t *mem, os_ptr op1, os_ptr op)
{
    uint len;
    const byte *pstr = 0;
    int code = obj_cvs(mem, op1, op->value.bytes, ((op)->tas.rsize), &len, &pstr);
