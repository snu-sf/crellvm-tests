...
jpeg_calc_trans_dimensions (j_compress_ptr cinfo)
{
  if (cinfo->min_DCT_h_scaled_size != cinfo->min_DCT_v_scaled_size)
    ((cinfo)->err->msg_code = (JERR_BAD_DCTSIZE), (cinfo)->err->msg_parm.i[0] = (cinfo->min_DCT_h_scaled_size), (cinfo)->err->msg_parm.i[1] = (cinfo->min_DCT_v_scaled_size), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
...

{
  ...
  if (cinfo->block_size < 1 || cinfo->block_size > 16)
    ((cinfo)->err->msg_code = (JERR_BAD_DCTSIZE), (cinfo)->err->msg_parm.i[0] = (cinfo->block_size), (cinfo)->err->msg_parm.i[1] = (cinfo->block_size), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
  ...
  cinfo->lim_Se = cinfo->block_size < 8 ?
    cinfo->block_size * cinfo->block_size - 1 : 64 -1;
  ...
  if ((long) cinfo->jpeg_height > (long) 65500L ||
      (long) cinfo->jpeg_width > (long) 65500L)
    ((cinfo)->err->msg_code = (JERR_IMAGE_TOO_BIG), (cinfo)->err->msg_parm.i[0] = ((unsigned int) 65500L), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
 ...
 compptr->v_samp_factor<=0 || compptr->v_samp_factor>4)
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    (cinfo->do_fancy_downsampling ? 8 : 8 / 2) &&
...
jinit_c_master_control (j_compress_ptr cinfo, boolean transcode_only)
{
  ...
  if (cinfo->optimize_coding)
    ...
    (cinfo->progressive_mode ||
     (cinfo->block_size > 1 && cinfo->block_size < 8)))
