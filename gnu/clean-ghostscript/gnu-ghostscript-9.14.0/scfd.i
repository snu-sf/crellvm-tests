...
static const gc_ptr_element_t cfd_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_CFD_state *)0)->lbuf - (char *)((stream_CFD_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_CFD_state *)0)->lprev - (char *)((stream_CFD_state *)0) )) } }; static const gc_struct_data_t cfd_reloc_ptrs = { (sizeof(cfd_enum_ptrs) / sizeof((cfd_enum_ptrs)[0])), 0, 0, cfd_enum_ptrs }; static const gs_memory_struct_type_t st_CFD_state = { sizeof(stream_CFD_state), "CCITTFaxDecode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &cfd_reloc_ptrs };
...
s_CFD_set_defaults(register stream_state * st)
{
    ...
    ((ss)->Uncompressed = ((bool)0), (ss)->K = 0, (ss)->EndOfLine = ((bool)0), (ss)->EncodedByteAlign = ((bool)0), (ss)->Columns = 1728, (ss)->Rows = 0, (ss)->EndOfBlock = ((bool)1), (ss)->BlackIs1 = ((bool)0), (ss)->DamagedRowsBeforeError = 0, (ss)->FirstBitLowOrder = ((bool)0), (ss)->DecodedByteAlign = 1, (ss)->lbuf = 0, (ss)->lprev = 0, (ss)->error_string[0] = 0);
}
...
s_CFD_init(stream_state * st)
{
    ...
    int raster = ss->raster =
        ( (ss->DecodedByteAlign) & ((ss->DecodedByteAlign) - 1) ? (((ss->Columns + 7) >> 3) + ((ss->DecodedByteAlign) - 1)) / (ss->DecodedByteAlign) * (ss->DecodedByteAlign) : (((ss->Columns + 7) >> 3) + ((ss->DecodedByteAlign) - 1)) & -(ss->DecodedByteAlign) );
    byte white = (ss->BlackIs1 ? 0 : 0xff);

    ((ss)->bits = 0, (ss)->bits_left = 0);


    ss->lbuf = (*(st->memory)->procs.alloc_bytes)(st->memory, raster + 4, "CFD lbuf");
    ss->lprev = 0;
    if (ss->lbuf == 0)
        return ((int)(-2));
    memset(ss->lbuf, white, raster);
    memset(ss->lbuf + raster, 0xaa, 4);
    if (ss->K != 0) {
        ss->lprev = (*(st->memory)->procs.alloc_bytes)(st->memory, raster + 4, "CFD lprev");
        if (ss->lprev == 0)
            return ((int)(-2));
    ...
    ss->k_left = (((ss->K) < (0)) ? (ss->K) : (0));
    ss->run_color = 0;
    ss->damaged_rows = 0;
    ss->skipping_damage = ((bool)0);
    ss->cbit = 0;
    ss->uncomp_run = 0;
...
s_CFD_release(stream_state * st)
{
    ...
    ((st->memory)->procs.free_object(st->memory, ss->lprev, "CFD lprev(close)"));
    ((st->memory)->procs.free_object(st->memory, ss->lbuf, "CFD lbuf(close)"));
}
# 131 "./base/scfd.c"
static __inline__ int get_run(stream_CFD_state *ss, stream_cursor_read *pr, const cfd_node decode[],
           int initial_bits, int min_bits, int *runlen, const char *str)
{
    register const byte *p; const byte *rlimit; uint bits; int bits_left; register byte *q; int qbit;
    const cfd_node *np;
    int clen;
    p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;

    if (bits_left >= initial_bits) do { } while (0); else if (rlimit - p >= 3) { if (ss->FirstBitLowOrder) bits = (bits << 24) + ((uint)byte_reverse_bits[p[1]] << 16) + ((uint)byte_reverse_bits[p[2]] << 8) + byte_reverse_bits[p[3]]; else bits = (bits << 24) + ((uint)p[1] << 16) + ((uint)p[2] << 8) + p[3]; bits_left += 24, p += 3; } else if (p < rlimit) { int c = *++p; if (ss->FirstBitLowOrder) c = byte_reverse_bits[c]; bits = (bits << 8) + c, bits_left += 8; } else {

        if (bits_left < min_bits)
            goto outl;
        np = &decode[(bits & byte_right_mask[bits_left]) << (initial_bits - bits_left)];
        if ((clen = np->code_length) > bits_left)
            goto outl;
        goto locl;
    }
    np = &decode[((bits >> (bits_left - (initial_bits))) & ((1 << (initial_bits)) - 1))];
    if ((clen = np->code_length) > initial_bits) {
            do { } while (0);
            if (!(bits_left >= (clen) || rlimit - p > ((clen) - bits_left - 1) >> 3)) goto outl;
            clen -= initial_bits;
            (bits_left -= (initial_bits));
            do { if (bits_left >= clen) do { } while (0); else if (rlimit - p >= 3) { if (ss->FirstBitLowOrder) bits = (bits << 24) + ((uint)byte_reverse_bits[p[1]] << 16) + ((uint)byte_reverse_bits[p[2]] << 8) + byte_reverse_bits[p[3]]; else bits = (bits << 24) + ((uint)p[1] << 16) + ((uint)p[2] << 8) + p[3]; bits_left += 24, p += 3; } else if (p < rlimit) { int c = *++p; if (ss->FirstBitLowOrder) c = byte_reverse_bits[c]; bits = (bits << 8) + c, bits_left += 8; } else goto outl; } while (0);
            np = &decode[np->value + ((bits >> (bits_left - (clen))) & byte_right_mask[clen])];
            do { } while (0);
...
static __inline__ int skip_data(stream_CFD_state *ss, stream_cursor_read *pr, int rlen)
{
    register const byte *p; const byte *rlimit; uint bits; int bits_left; register byte *q; int qbit;
    p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;
    (void)rlimit;
...
static __inline__ int invert_data(stream_CFD_state *ss, stream_cursor_read *pr, int *rlen, byte black_byte)
{
    register const byte *p; const byte *rlimit; uint bits; int bits_left; register byte *q; int qbit;
    p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;
    (void)rlimit;
...
# 304 "./base/scfd.c"
    if (ss->skipping_damage) {
        register const byte *p; const byte *rlimit; uint bits; int bits_left;
        int skip;
        (void)rlimit;
...
ck_eol:
        while ((status = cf_decode_eol(ss, pr)) > 0) {
            ...
            if (ss->K > 0) {
                register const byte *p; const byte *rlimit; uint bits; int bits_left;
                p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left;
                do { if (bits_left >= 1) do { } while (0); else if (rlimit - p >= 3) { if (ss->FirstBitLowOrder) bits = (bits << 24) + ((uint)byte_reverse_bits[p[1]] << 16) + ((uint)byte_reverse_bits[p[2]] << 8) + byte_reverse_bits[p[3]]; else bits = (bits << 24) + ((uint)p[1] << 16) + ((uint)p[2] << 8) + p[3]; bits_left += 24, p += 3; } else if (p < rlimit) { int c = *++p; if (ss->FirstBitLowOrder) c = byte_reverse_bits[c]; bits = (bits << 8) + c, bits_left += 8; } else goto out; } while (0);
...
cf_decode_eol(stream_CFD_state * ss, stream_cursor_read * pr)
{
    ...
    for (;;) {
        do { if (bits_left >= look_ahead) do { } while (0); else if (rlimit - p >= 3) { if (ss->FirstBitLowOrder) bits = (bits << 24) + ((uint)byte_reverse_bits[p[1]] << 16) + ((uint)byte_reverse_bits[p[2]] << 8) + byte_reverse_bits[p[3]]; else bits = (bits << 24) + ((uint)p[1] << 16) + ((uint)p[2] << 8) + p[3]; bits_left += 24, p += 3; } else if (p < rlimit) { int c = *++p; if (ss->FirstBitLowOrder) c = byte_reverse_bits[c]; bits = (bits << 8) + c, bits_left += 8; } else goto back; } while (0);
        if (((bits >> (bits_left - (1))) & ((1 << (1)) - 1)))
            break;
        (bits_left -= (1));
    }
...
cf_decode_1d(stream_CFD_state * ss, stream_cursor_read * pr)
{
  ...
  dw:
    ...
    pr->ptr = p -= (bits_left >> 3), ss->bits = bits >>= (bits_left & ~7), ss->bits_left = bits_left &= 7, ss->wpos = q - ss->lbuf, ss->cbit = qbit;
    status = get_run(ss, pr, cf_white_decode, 8, 4,
            &bcnt, "[w1]white");
    p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;
    if (status < 0) {
        goto out0;
  ...
  db:
    ...
    pr->ptr = p -= (bits_left >> 3), ss->bits = bits >>= (bits_left & ~7), ss->bits_left = bits_left &= 7, ss->wpos = q - ss->lbuf, ss->cbit = qbit;
    status = get_run(ss, pr, cf_black_decode, 7, 2,
            &bcnt, "[w1]black");
    p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;
    if (status < 0) {
        goto out1;
...
cf_decode_2d(stream_CFD_state * ss, stream_cursor_read * pr)
{
  ...
  top:if (count <= end_count) {
    ...
    do { } while (0);
# 687 "./base/scfd.c"
    do { if (bits_left >= 3) do { } while (0); else if (rlimit - p >= 3) { if (ss->FirstBitLowOrder) bits = (bits << 24) + ((uint)byte_reverse_bits[p[1]] << 16) + ((uint)byte_reverse_bits[p[2]] << 8) + byte_reverse_bits[p[3]]; else bits = (bits << 24) + ((uint)p[1] << 16) + ((uint)p[2] << 8) + p[3]; bits_left += 24, p += 3; } else if (p < rlimit) { int c = *++p; if (ss->FirstBitLowOrder) c = byte_reverse_bits[c]; bits = (bits << 8) + c, bits_left += 8; } else goto out3; } while (0);
...
v0: (bits_left -= (1));
        ...
        case 0:
            pr->ptr = p -= (bits_left >> 3), ss->bits = bits >>= (bits_left & ~7), ss->bits_left = bits_left &= 7, ss->wpos = q - ss->lbuf, ss->cbit = qbit;
            status = get_run(ss, pr, cf_2d_decode, 7, 4,
                    &rlen, "[w2]");
            p = pr->ptr, rlimit = pr->limit, bits = ss->bits, bits_left = ss->bits_left, q = ss->lbuf + ss->wpos, qbit = ss->cbit;
            if (status < 0) {
                goto out0;
...
rlen_lt_zero:
...

    {
        ...
        if ((prev_data & count_bit[prev_count & 7]) &&
            ...
            do { } while (0);
            do { plen = byte_bit_run_length_neg[prev_count & 7][prev_data]; if ( plen >= 8 ) { if ( invert == 0 ) for ( ; ; prev_q += 4, plen += 32 ) { if ( prev_q[0] != 0xff ) { prev_data = prev_q[0]; prev_q += 1; plen -= 8; break; } if ( prev_q[1] != 0xff ) { prev_data = prev_q[1]; prev_q += 2; break; } if ( prev_q[2] != 0xff ) { prev_data = prev_q[2]; prev_q += 3; plen += 8; break; } if ( prev_q[3] != 0xff ) { prev_data = prev_q[3]; prev_q += 4; plen += 16; break; } } else for ( ; ; prev_q += 4, plen += 32 ) { if ( prev_q[0] ) { prev_data = (byte)~prev_q[0]; prev_q += 1; plen -= 8; break; } if ( prev_q[1] ) { prev_data = (byte)~prev_q[1]; prev_q += 2; break; } if ( prev_q[2] ) { prev_data = (byte)~prev_q[2]; prev_q += 3; plen += 8; break; } if ( prev_q[3] ) { prev_data = (byte)~prev_q[3]; prev_q += 4; plen += 16; break; } } plen += byte_bit_run_length_0[prev_data]; } prev_count -= plen;} while (0);
        ...
        if (prev_count != end_count) {
            do { } while (0);
            do { plen = byte_bit_run_length_neg[prev_count & 7][prev_data ^ 0xff]; if ( plen >= 8 ) { if ( invert == 0 ) { if ( prev_q[0] ) { prev_data = prev_q[0]; prev_q += 1; plen -= 8; } else if ( prev_q[1] ) { prev_data = prev_q[1]; prev_q += 2; } else { while ( !(prev_q[2] | prev_q[3] | prev_q[4] | prev_q[5]) ) prev_q += 4, plen += 32; if ( prev_q[2] ) { prev_data = prev_q[2]; prev_q += 3; plen += 8; } else if ( prev_q[3] ) { prev_data = prev_q[3]; prev_q += 4; plen += 16; } else if ( prev_q[4] ) { prev_data = prev_q[4]; prev_q += 5; plen += 24; } else { prev_data = prev_q[5]; prev_q += 6; plen += 32; } } } else { if ( prev_q[0] != 0xff ) { prev_data = (byte)~prev_q[0]; prev_q += 1; plen -= 8; } else if ( prev_q[1] != 0xff ) { prev_data = (byte)~prev_q[1]; prev_q += 2; } else { while ( (prev_q[2] & prev_q[3] & prev_q[4] & prev_q[5]) == 0xff ) prev_q += 4, plen += 32; if ( prev_q[2] != 0xff ) { prev_data = (byte)~prev_q[2]; prev_q += 3; plen += 8; } else if ( prev_q[3] != 0xff ) { prev_data = (byte)~prev_q[3]; prev_q += 4; plen += 16; } else if ( prev_q[4] != 0xff ) { prev_data = (byte)~prev_q[4]; prev_q += 5; plen += 24; } else { prev_data = (byte)~prev_q[5]; prev_q += 6; plen += 32; } } } plen += byte_bit_run_length_0[prev_data ^ 0xff]; } prev_count -= plen;} while (0);
    ...
    goto top;
 out3:
    if (bits_left > 0 && ((bits >> (bits_left - (1))) & ((1 << (1)) - 1))) {
  ...
  out:pr->ptr = p -= (bits_left >> 3), ss->bits = bits >>= (bits_left & ~7), ss->bits_left = bits_left &= 7, ss->wpos = q - ss->lbuf, ss->cbit = qbit;
    ...
    if (status == ((int)(-2)) && ss->Rows > 0 && ss->row > ss->Rows)
        status = ((int)(-1));
    return status;
