...
# 38 "./base/gxifast.c" 2
# 47 "./base/gxifast.c"
irender_proc_t gs_image_class_1_simple(gx_image_enum *penum);


static int image_render_skip(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_simple(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_landscape(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
irender_proc_t
gs_image_class_1_simple(gx_image_enum * penum)
{
    irender_proc_t rproc;
    fixed ox = ((penum->dda.pixel0.x).state).Q;
    fixed oy = ((penum->dda.pixel0.y).state).Q;
    ...
    switch (penum->posture) {
        case image_portrait:
            {
                long dev_width =
                    ((long)(((ox + penum->x_extent.x)+(((1<<8)>>1) - 1L)) >> (8))) -
                    ((long)(((ox)+(((1<<8)>>1) - 1L)) >> (8)));
                ...
                if (dev_width != penum->rect.w) {
                    ...
                    long line_size =
                        ((uint)((((((dev_width) < 0 ? -(dev_width) : (dev_width))) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) + (1 << 3);

                    if (penum->adjust != 0 || line_size > ((unsigned int)~0 + (unsigned int)0))
                        return 0;

                    penum->line_width = ((dev_width) < 0 ? -(dev_width) : (dev_width));
                    penum->line_size = (uint) line_size;
                    penum->line = (*(penum->memory)->procs.alloc_bytes)(penum->memory, penum->line_size, "image line");
        ...
        case image_landscape:
            {
                long dev_width =
                    ((long)(((oy + penum->x_extent.y)+(((1<<8)>>1) - 1L)) >> (8))) -
                    ((long)(((oy)+(((1<<8)>>1) - 1L)) >> (8)));
                long line_size =
                    (dev_width = ((dev_width) < 0 ? -(dev_width) : (dev_width)),
                     ((uint)((((dev_width) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) * 8 +
                     ( (8) & ((8) - 1) ? ((dev_width) + ((8) - 1)) / (8) * (8) : ((dev_width) + ((8) - 1)) & -(8) ) * (1 << 3));

                if ((dev_width != penum->rect.w && penum->adjust != 0) ||
                    line_size > ((unsigned int)~0 + (unsigned int)0)
                    )
                    return 0;
                ...
                penum->xi_next = penum->line_xy = ((int)(((ox)+((1<<8)>>1)) >> (8)));
                do { } while (0);
                ...
                penum->dxy =
                    ((fixed)((penum->matrix.xy + ((1L)*(1.0/(1<<8))) / 2)*(float)(1<<8)));
    ...
    if (penum->use_mask_color) {
        ...
        if (penum->mask_color.values[0] == 1) {
            ...
            ((penum->map[0].inverted ? penum->icolor0 : penum->icolor1)->colors.pure = (((gx_color_index)(~0))), (penum->map[0].inverted ? penum->icolor0 : penum->icolor1)->type = (&gx_dc_type_data_pure)); (penum->map[0].inverted ? penum->icolor0 : penum->icolor1)->ccolor_valid = ((bool)0);

        } else if (penum->mask_color.values[1] == 0) {

            ((penum->map[0].inverted ? penum->icolor1 : penum->icolor0)->colors.pure = (((gx_color_index)(~0))), (penum->map[0].inverted ? penum->icolor1 : penum->icolor0)->type = (&gx_dc_type_data_pure)); (penum->map[0].inverted ? penum->icolor1 : penum->icolor0)->ccolor_valid = ((bool)0);
...
# 244 "./base/gxifast.c"
    ...
    xl0 =
        (x_extent >= 0 ?
         ((int)(((xcur)+(((1<<8)>>1) - 1L)))&((1<<8)-1)) :
         ((int)(((xcur + x_extent)+(((1<<8)>>1) - 1L)))&((1<<8)-1)) - x_extent);
    xl0 += ((fixed)(line_x)<<8);
    do { ((xl).state).Q = (xl0), ((xl).state).R = ((w)-1); do { if ( (w) == 0 ) ((xl).step).dQ = 0, ((xl).step).dR = 0; else if ( (x_extent) < 0 ) { ((xl).step).dQ = -(int)((uint)-(x_extent) / (w)); if ( (((xl).step).dR = -(x_extent) % (w)) != 0 ) --((xl).step).dQ, ((xl).step).dR = (w) - ((xl).step).dR; } else { ((xl).step).dQ = (x_extent) / (w); ((xl).step).dR = (x_extent) % (w); } ((xl).step).N = (w); } while (0); } while (0);
    dxx4 = xl.step;
    do { (dxx4).dR += (xl.step).dR; if ((dxx4).dR >= (dxx4).N) { (dxx4).dQ ++; (dxx4).dR -= (dxx4).N; } (dxx4).dQ += (xl.step).dQ; } while (0);




    dxx8 = dxx4;
    do { (dxx4).dR += (dxx8).dR; if ((dxx4).dR >= (dxx4).N) { (dxx4).dQ ++; (dxx4).dR -= (dxx4).N; } (dxx4).dQ += (dxx8).dQ; } while (0);
    dxx8 = dxx4;
    do { (dxx8).dR += (dxx4).dR; if ((dxx8).dR >= (dxx8).N) { (dxx8).dQ ++; (dxx8).dR -= (dxx8).N; } (dxx8).dQ += (dxx4).dQ; } while (0);
    dxx16 = dxx8;
    do { (dxx16).dR += (dxx8).dR; if ((dxx16).dR >= (dxx16).N) { (dxx16).dQ ++; (dxx16).dR -= (dxx16).N; } (dxx16).dQ += (dxx8).dQ; } while (0);
    dxx24 = dxx16;
    do { (dxx24).dR += (dxx8).dR; if ((dxx24).dR >= (dxx24).N) { (dxx24).dQ ++; (dxx24).dR -= (dxx24).N; } (dxx24).dQ += (dxx8).dQ; } while (0);
    dxx32 = dxx24;
    do { (dxx32).dR += (dxx8).dR; if ((dxx32).dR >= (dxx32).N) { (dxx32).dQ ++; (dxx32).dR -= (dxx32).N; } (dxx32).dQ += (dxx8).dQ; } while (0);
    ...
    for (data = *psrc;;) {
        ...
        data ^= 0xff;
        while (data & sbit) {
            do { ((xl).state).R -= ((xl).step).dR; if ((signed)((xl).state).R < 0) { ((xl).state).Q++; ((xl).state).R += ((xl).step).N; }; ((xl).state).Q += ((xl).step).dQ; } while (0);
            sbit >>= 1;
            do { } while (0);
        }
        if (!sbit) {
sw: if ((data = psrc[1]) != 0) {
                ...
                do { (xl.state).R -= (dxx8).dR; if ((signed)(xl.state).R < 0) { (xl.state).Q++; (xl.state).R += (dxx8).N; }; (xl.state).Q += (dxx8).dQ; } while (0);
                psrc += 2;
                do { } while (0);
            } else if ((data = psrc[3]) != 0) {
                do { (xl.state).R -= (dxx16).dR; if ((signed)(xl.state).R < 0) { (xl.state).Q++; (xl.state).R += (dxx16).N; }; (xl.state).Q += (dxx16).dQ; } while (0);
                psrc += 3;
                do { } while (0);
            } else if ((data = psrc[4]) != 0) {
                do { (xl.state).R -= (dxx24).dR; if ((signed)(xl.state).R < 0) { (xl.state).Q++; (xl.state).R += (dxx24).N; }; (xl.state).Q += (dxx24).dQ; } while (0);
                psrc += 4;
                do { } while (0);
            } else {
                do { (xl.state).R -= (dxx32).dR; if ((signed)(xl.state).R < 0) { (xl.state).Q++; (xl.state).R += (dxx32).N; }; (xl.state).Q += (dxx32).dQ; } while (0);
                psrc += 4;
                do { } while (0);
                goto sw;
            }
            if (data > 0xf)
                ...
                sbit = 0x08;
                do { (xl.state).R -= (dxx4).dR; if ((signed)(xl.state).R < 0) { (xl.state).Q++; (xl.state).R += (dxx4).N; }; (xl.state).Q += (dxx4).dQ; } while (0);
            }
            data ^= 0xff;
            while (data & sbit) {
                do { ((xl).state).R -= ((xl).step).dR; if ((signed)((xl).state).R < 0) { ((xl).state).Q++; ((xl).state).R += ((xl).step).N; }; ((xl).state).Q += ((xl).step).dQ; } while (0);
        ...
        if (psrc >= stop && sbit == stopbit) {
            ...
            n = ((int)(((int64_t)xl0 + (int64_t)x_extent) >> (8))) - x0;
        } else {
              ...
              int x, int y, int w, int h, gx_device * dev)
{
    ...
    uint align = ((uint)( ((const char *)(data) - (const char *)0) & (((1 << 3)) - 1) ));
    ...
    data -= align;
    dx += align << 3;
    if (((pdc0)->type == (&gx_dc_type_data_pure)) && ((pdc1)->type == (&gx_dc_type_data_pure))) {
        ...
        int (*copy_mono)(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1) =
            (h == 1 || (raster & ((1 << 3) - 1)) == 0 ?
             ((dev)->procs.copy_mono) : gx_copy_mono_unaligned);
        return (*copy_mono)
            (dev, data, dx, raster, 0L,
...

    {
        ...
        if ((((pdc1)->type == (&gx_dc_type_data_pure)) && (pdc1)->colors.pure == ((gx_color_index)(~0)))) {
            pdc = pdc0;
            invert = ((bool)1);
        } else {
            if (!(((pdc0)->type == (&gx_dc_type_data_pure)) && (pdc0)->colors.pure == ((gx_color_index)(~0)))) {
                int code = (*(pdc0)->type->fill_rectangle)(pdc0, x, y, w, h, dev, (rop3_default | (((bool)0) ? 0x100 : 0) | (((bool)0) ? 0x200 : 0)), ((void*)0));
        ...
        return (*pdc->type->fill_masked)
            (pdc, data, dx, raster, 0L, x, y, w, h,
             dev, (rop3_default | (((bool)0) ? 0x100 : 0) | (((bool)0) ? 0x200 : 0)), invert);
                    ...
                    uint w, int h, gx_device * dev)
{
    ...
    int line_x;
    fixed xcur = ((penum->dda.pixel0.x).state).Q;
    int ix = ((int)((((xcur)+(((1<<8)>>1) - 1L))) >> (8)));
    int ixr;
    ...
    if (h == 0)
        return 0;
    if ((!(((pdc0)->type == (&gx_dc_type_data_pure)) && (pdc0)->colors.pure == ((gx_color_index)(~0))) &&
         (code = (*(pdc0)->type->load)(pdc0, penum->pis, dev, gs_color_select_texture)) < 0) ||
        (!(((pdc1)->type == (&gx_dc_type_data_pure)) && (pdc1)->colors.pure == ((gx_color_index)(~0))) &&
         (code = (*(pdc1)->type->load)(pdc1, penum->pis, dev, gs_color_select_texture)) < 0)
        )
        return code;
    if (penum->line == 0) {
        line = buffer;
        ...
        line_width = w;
        line_x = 0;
    } else if (copy_mono == ((&mem_mono_device)->procs.copy_mono) &&
               dxx > 0 && ((pdc1)->type == (&gx_dc_type_data_pure)) && ((pdc0)->type == (&gx_dc_type_data_pure)) &&
               ...
               ix >= 0 &&
               (ixr = ((int)((((xcur + penum->x_extent.x)+(((1<<8)>>1) - 1L))) >> (8))) - 1) <
                 dev->width &&
        ...
        byte *scan_line = (((gx_device_memory *) dev)->line_ptrs[iy]);
                       ...
                       uint w, int h, gx_device * dev)
{
    ...
    uint raster = ((uint)((((penum->line_width) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
               ...
               gx_device * dev)
{
    ...
    uint raster = ((uint)((((line_width) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    byte *flipped = line + raster * 8;
    int w = x1 - x0;
    int y = ((int)((((((penum->dda.pixel0.y).state).Q)+(((1<<8)>>1) - 1L))) >> (8)));
    ...
    if (y_neg)
        y -= line_width;
    return copy_portrait(penum, flipped, x0 & 7, (1 << 3),
