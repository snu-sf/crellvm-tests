# 1 "./base/gsicc_monitorcm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gsicc_monitorcm.c" 2
# 19 "./base/gsicc_monitorcm.c"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
# 1 "./base/stdpre.h" 1
# 253 "./base/stdpre.h"
...
gsicc_mcm_transform_general(gx_device *dev, gsicc_link_t *icclink,
                             ...
                             int num_bytes_in, int num_bytes_out)
{
    bool is_neutral = ((bool)0);
    unsigned short outputcolor_cm[(8 * 8)];
        ...
        if (num_bytes_in == num_bytes_out) {
            ...
            if (num_bytes_in == 2) {
                ...
                for (k = 0; k < icclink->num_input; k++) {
                    out_ptr[k] = ((gx_color_value)((((unsigned int)in_ptr[k])*0xFF01 + 0x800000)>>24));
                }
            } else {
...
gsicc_mcm_set_link(gsicc_link_t* link)
{
    ...
    link->is_monitored = ((bool)1);
    link->is_identity = ((bool)0);
...
gsicc_mcm_end_monitor(gsicc_link_cache_t *cache, gx_device *dev)
{
    ...
    if (((dev)->procs.dev_spec_op)(dev, gxdso_is_pdf14_device, ((void*)0), 0) > 0) {
        gs_pdf14_device_color_mon_set(dev, ((bool)0));
    }
    ...
    while (curr != ((void*)0) ) {
        if (curr->is_monitored) {
            ...
            if (curr->hashcode.des_hash == curr->hashcode.src_hash)
                curr->is_identity = ((bool)1);
            curr->is_monitored = ((bool)0);
        }

        while (curr->num_waiting > 0) {
            gp_semaphore_signal(&(curr->wait)->native);
            curr->num_waiting--;
        }
        curr = curr->next;
    }
...
gsicc_mcm_begin_monitor(gsicc_link_cache_t *cache, gx_device *dev)
{
    ...
    if (((dev)->procs.dev_spec_op)(dev, gxdso_is_pdf14_device, ((void*)0), 0) > 0) {
        gs_pdf14_device_color_mon_set(dev, ((bool)1));
    }
