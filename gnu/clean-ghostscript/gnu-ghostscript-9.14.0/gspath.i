...
{
    pgs->current_point_valid = ((bool)0);
...
gs_closepath(gs_state * pgs)
{
    ...
    int code = gx_path_close_subpath_notes(ppath, sn_none);
...
gs_upmergepath(gs_state * pgs)
{
    ...
    if (pgs->current_point_valid) {
        ...
        saved->current_point_valid = ((bool)1);
    }
    return code;
}
...
{
    ppt->x = (x > ((((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8)))*(1.0/(1<<8))) ? ((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8)) : x < (((-((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8))))*(1.0/(1<<8))) ? (-((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8))) : ((fixed)((x)*(float)(1<<8))));
    ppt->y = (y > ((((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8)))*(1.0/(1<<8))) ? ((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8)) : y < (((-((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8))))*(1.0/(1<<8))) ? (-((~(-1 << ((1 << 2) * 8 - 1))) - ((fixed)(1000)<<8))) : ((fixed)((y)*(float)(1<<8))));
}
...
{
    if (!((x) >= -2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2)) && (x) < 2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2))) || !((y) >= -2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2)) && (y) < 2.0 * (1L << (((sizeof(fixed) * 8 - 8)) - 2)))) {
        ...
        ppt->x = ((fixed)floor((x)*(float)(1<<8) + 0.5));
        ppt->y = ((fixed)floor((y)*(float)(1<<8) + 0.5));
    }
    return 0;
}
...
gs_moveto_aux(gs_imager_state *pis, gx_path *ppath, double x, double y)
{
    ...
    if (code < 0)
        return code;
    if (pis->hpgl_path_mode && (((ppath)->state_flags & psf_subpath_open) != 0))
    {
        code = gx_path_add_gap_notes(ppath, pt.x, pt.y, 0);
        if (code < 0)
            return code;
        (pis)->current_point.x = x; (pis)->current_point.y = y;;
    }
    else
    {
    ...
    pis->current_point_valid = ((bool)1);
    return 0;
}
...
gs_lineto_aux(gs_state * pgs, double x, double y)
{
    ...
    if (code < 0)
        return code;
    (pgs)->current_point.x = x; (pgs)->current_point.y = y;;
    return 0;
}
           ...
           double x1, double y1, double x2, double y2, double x3, double y3)
{
    ...
    if (code < 0)
        return code;
    code = gx_path_add_curve_notes(ppath, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, sn_none);
    if (code < 0)
        return code;
    (pgs)->current_point.x = x3; (pgs)->current_point.y = y3;;
    return 0;
}
...
gx_curr_bbox(gs_state * pgs, gs_rect *bbox, gs_bbox_comp_t comp_type)
{
    ...
    int expansion_code;
    bool include_path = ((bool)1);
    gs_fixed_point expansion;

    code = gx_effective_clip_path(pgs, &clip_path);
        if (code < 0) return code;
    if (comp_type == NO_PATH) {
        bbox->p.x = ((clip_path->outer_box.p.x)*(1.0/(1<<8)));
        bbox->p.y = ((clip_path->outer_box.p.y)*(1.0/(1<<8)));
        bbox->q.x = ((clip_path->outer_box.q.x)*(1.0/(1<<8)));
        bbox->q.y = ((clip_path->outer_box.q.y)*(1.0/(1<<8)));
        return 0;
    }
    ...
    if (include_path) {
        do { if ((clip_path->outer_box).p.x > (path_bbox).p.x) (path_bbox).p.x = (clip_path->outer_box).p.x; if ((clip_path->outer_box).q.x < (path_bbox).q.x) (path_bbox).q.x = (clip_path->outer_box).q.x; if ((clip_path->outer_box).p.y > (path_bbox).p.y) (path_bbox).p.y = (clip_path->outer_box).p.y; if ((clip_path->outer_box).q.y < (path_bbox).q.y) (path_bbox).q.y = (clip_path->outer_box).q.y; } while (0);

        bbox->p.x = ((path_bbox.p.x)*(1.0/(1<<8)));
        bbox->p.y = ((path_bbox.p.y)*(1.0/(1<<8)));
        bbox->q.x = ((path_bbox.q.x)*(1.0/(1<<8)));
        bbox->q.y = ((path_bbox.q.y)*(1.0/(1<<8)));
    } else {
...
{
    gs_id view_clip_id =
        (pgs->view_clip == 0 || pgs->view_clip->rule == 0 ? 0L :
         pgs->view_clip->id);
    ...
    if (view_clip_id == 0L) {
        ...
        if (((vcbox).q.y <= (cbox).q.y && (vcbox).q.x <= (cbox).q.x && (vcbox).p.y >= (cbox).p.y && (vcbox).p.x >= (cbox).p.x)) {
            if (!pgs->effective_clip_shared)
            ...
            if (pgs->effective_clip_shared) {
                npath = gx_cpath_alloc_shared(((void*)0), pgs->memory, "gx_effective_clip_path");
                if (npath == 0)
                    return ((-25));
            }
            (void)gx_cpath_init_local_shared(&ipath, ((void*)0), pgs->memory);
            code = gx_cpath_assign_preserve(&ipath, pgs->clip_path);
            if (code < 0)
                return code;
            (void)gx_path_init_local_shared(&vpath, ((void*)0), pgs->memory);
            ...
            pgs->effective_clip_shared = ((bool)0);
        }
    }
...
gs_clippath(gs_state * pgs)
{
    ...
    (void)gx_path_init_local_shared(&cpath, ((void*)0), pgs->path->memory);
    code = gx_cpath_to_path(pgs->clip_path, &cpath);
    if (code >= 0) {
        code = gx_path_assign_free(pgs->path, &cpath);
        pgs->current_point.x = ((pgs->path->position.x)*(1.0/(1<<8)));
        pgs->current_point.y = ((pgs->path->position.y)*(1.0/(1<<8)));
...
gx_default_clip_box(const gs_state * pgs, gs_fixed_rect * pbox)
{
    ...
    pbox->p.x = (((((fixed)((bbox.p.x)*(float)(1<<8))))+((1<<8)>>1))&(-1L<<8));
    pbox->p.y = (((((fixed)((bbox.p.y)*(float)(1<<8))))+((1<<8)>>1))&(-1L<<8));
    pbox->q.x = (((((fixed)((bbox.q.x)*(float)(1<<8))))+((1<<8)>>1))&(-1L<<8));
    pbox->q.y = (((((fixed)((bbox.q.y)*(float)(1<<8))))+((1<<8)>>1))&(-1L<<8));
    return 0;
}
