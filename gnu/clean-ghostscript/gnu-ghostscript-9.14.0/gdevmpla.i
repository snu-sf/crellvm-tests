...
# 29 "./base/gdevmpla.c" 2


static int mem_planar_open(gx_device *dev);
static int mem_planar_copy_mono(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1); static int mem_planar_copy_color(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height); static int mem_planar_fill_rectangle(gx_device *dev, int x, int y, int width, int height, gx_color_index color);
static int mem_planar_copy_color_24to8(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem_planar_copy_color_4to1(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int mem_planar_copy_planes(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, int plane_height);

static int mem_planar_strip_tile_rectangle(gx_device *dev, const gx_strip_bitmap *tiles, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1, int phase_x, int phase_y);
static int mem_planar_strip_tile_rect_devn(gx_device *dev, const gx_strip_bitmap *tiles, int x, int y, int width, int height, const gx_drawing_color *pdcolor0, const gx_drawing_color *pdcolor1, int phase_x, int phase_y);
static int mem_planar_strip_copy_rop(gx_device *dev, const byte *sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index *scolors, const gx_strip_bitmap *textures, const gx_color_index *tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop);
static int mem_planar_strip_copy_rop2(gx_device *dev, const byte *sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index *scolors, const gx_strip_bitmap *textures, const gx_color_index *tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop, uint planar_height);
...
static int mem_planar_fill_rectangle_hl_color(gx_device *dev, const gs_fixed_rect *rect, const gs_imager_state *pis, const gx_drawing_color *pdcolor, const gx_clip_path *pcpath);
static int mem_planar_put_image(gx_device *dev, const byte *buffer, int num_chan, int x, int y, int width, int height, int row_stride, int plane_stride, int alpha_plane_index, int tag_plane_index);
                       ...
                       void *data, int size)
{
    ...
    if (dev_spec_op == gxdso_supports_devn) {
        ((pdev)->procs.get_profile)(pdev, &dev_profile);
        if (dev_profile != ((void*)0) && dev_profile->supports_devn &&
                    ...
                    const gx_render_plane_t *planes )
{
    ...
    if (num_planes < 1 || num_planes > (8 * 8) || num_planes != mdev->color_info.num_components)
        return ((-15));
    ...
    ((mdev)->procs.open_device = (mem_planar_open));
    if (num_planes == 1) {

        ((mdev)->procs.fill_rectangle = (((mdproto)->procs.fill_rectangle)));
        ((mdev)->procs.copy_mono = (((mdproto)->procs.copy_mono)));
        ((mdev)->procs.copy_color = (((mdproto)->procs.copy_color)));
        ((mdev)->procs.copy_alpha = (((mdproto)->procs.copy_alpha)));
        ((mdev)->procs.strip_tile_rectangle = (((mdproto)->procs.strip_tile_rectangle)));
        ((mdev)->procs.strip_copy_rop = (((mdproto)->procs.strip_copy_rop)));
        ((mdev)->procs.strip_copy_rop2 = (((mdproto)->procs.strip_copy_rop2)));
        ((mdev)->procs.get_bits_rectangle = (((mdproto)->procs.get_bits_rectangle)));
    } else {
        ...
        if (num_planes >= 4) {
            ((mdev)->procs.fill_rectangle_hl_color = (mem_planar_fill_rectangle_hl_color));

            ((mdev)->procs.put_image = (mem_planar_put_image));
        }
        ((mdev)->procs.fill_rectangle = (mem_planar_fill_rectangle));
        ((mdev)->procs.copy_alpha_hl_color = (gx_default_copy_alpha_hl_color));
        ((mdev)->procs.copy_mono = (mem_planar_copy_mono));
        ((mdev)->procs.dev_spec_op = (mem_planar_dev_spec_op));
        if ((mdev->color_info.depth == 24) &&
            ...
            ((mdev)->procs.copy_color = (mem_planar_copy_color_24to8));
            ...
            ((mdev)->procs.copy_color = (mem_planar_copy_color_4to1));
            ((mdev)->procs.dev_spec_op = (mem_planar_dev_spec_op_cmyk4));
        } else
            ((mdev)->procs.copy_color = (mem_planar_copy_color));
        ((mdev)->procs.copy_alpha = (gx_default_copy_alpha));
        ((mdev)->procs.copy_planes = (mem_planar_copy_planes));
        ((mdev)->procs.strip_tile_rectangle = (mem_planar_strip_tile_rectangle));
        ((mdev)->procs.strip_tile_rect_devn = (mem_planar_strip_tile_rect_devn));
        ((mdev)->procs.strip_copy_rop = (mem_planar_strip_copy_rop));
        ((mdev)->procs.strip_copy_rop2 = (mem_planar_strip_copy_rop2));
        ((mdev)->procs.get_bits_rectangle = (mem_planar_get_bits_rectangle));
    }
    return 0;
}
              ...
              int plane_stride, int alpha_plane_index, int tag_plane_index)
{
    ...
    (*((pdev)->procs.copy_planes)) (pdev, buffer, 0, row_stride,
                                 0L, xstart, ystart,
    ...
    const gx_clip_path *pcpath)
{
    ...
    if (pdcolor->type != (&gx_dc_type_data_devn) && pdcolor->type != &gx_dc_devn_masked) {
        return (*(pdcolor)->type->fill_rectangle)(pdcolor, x, y, w, h, dev, (rop3_default | (((bool)0) ? 0x100 : 0) | (((bool)0) ? 0x200 : 0)), ((void*)0));
    }
    (save.depth = mdev->color_info.depth, save.base = mdev->base, save.line_ptrs = mdev->line_ptrs);
    for (pi = 0; pi < mdev->color_info.num_components; ++pi) {
        ...
        (mdev->color_info.depth = plane_depth, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * plane_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
        ((mdproto)->procs.fill_rectangle)(dev, x, y, w, h,
                                          (pdcolor->colors.devn.values[pi]) >> shift & mask);
        mdev->line_ptrs += mdev->height;
    }
    (mdev->color_info.depth = save.depth, mdev->base = save.base, mdev->line_ptrs = save.line_ptrs);
    return 0;
}
                     ...
                     gx_color_index color0, gx_color_index color1)
{
    ...
    for (pi = 0; pi < mdev->color_info.num_components; ++pi) {
        ...
        const gx_device_memory *mdproto =
            gdev_mem_device_for_bits(plane_depth);
        gx_color_index c0 =
            (color0 == ((gx_color_index)(~0)) ? ((gx_color_index)(~0)) :
             (color0 >> shift) & mask);
        gx_color_index c1 =
            (color1 == ((gx_color_index)(~0)) ? ((gx_color_index)(~0)) :
             (color1 >> shift) & mask);

        (mdev->color_info.depth = plane_depth, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * plane_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
        if (c0 == c1)
            ((mdproto)->procs.fill_rectangle)(dev, x, y, w, h, c0);
        else
            ((mdproto)->procs.copy_mono)
                            ...
                            int x, int y, int w, int h)
{
    ...
    uint plane_raster = ((uint)((((w<<3) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    ...
    (save.depth = mdev->color_info.depth, save.base = mdev->base, save.line_ptrs = mdev->line_ptrs);
    (mdev->color_info.depth = 8, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * 8) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
    if (plane_raster > (100 * (1 << 3))) {
        br = (100 * (1 << 3));
        bw = (100 * (1 << 3));
        bh = 1;
    } else {
    ...
    for (cy = y; cy < y + h; cy += ch) {
        ...
        for (cx = x; cx < x + w; cx += cw) {
            ...
            ((mdproto)->procs.copy_color)
                        (dev, buf.b, 0, br, 0L, cx, cy, cw, ch);
            mdev->line_ptrs += mdev->height;
            ((mdproto)->procs.copy_color)
                    (dev, buf1.b, 0, br, 0L, cx, cy, cw, ch);
            mdev->line_ptrs += mdev->height;
            ((mdproto)->procs.copy_color)
                    (dev, buf2.b, 0, br, 0L, cx, cy, cw, ch);
                            ...
                            int x, int y, int w, int h)
{
    ...
    uint plane_raster = ((uint)((((w) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    int br, bw, bh, cx, cy, cw, ch, ix, iy;

    do { do { if ( (x | y) < 0 ) { if ( x < 0 ) w += x, sourcex -= x, x = 0; if ( y < 0 ) h += y, base -= (int)(y * sraster), id = 0L, y = 0; } if ( w > (dev)->width - x ) w = (dev)->width - x; } while (0); if ( h > (dev)->height - y ) h = (dev)->height - y; if ( w <= 0 || h <= 0 ) return 0; } while (0);
    (save.depth = mdev->color_info.depth, save.base = mdev->base, save.line_ptrs = mdev->line_ptrs);
    (mdev->color_info.depth = 1, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * 1) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
    if (plane_raster > (100 * (1 << 3))) {
        br = (100 * (1 << 3));
        bw = (100 * (1 << 3))<<3;
        bh = 1;
...
loop_entry:
            ...
            ((mdproto)->procs.copy_mono)
                        (dev, buf0.b, 0, br, 0L, cx, cy, cw, ch,
            ...
            mdev->line_ptrs += mdev->height;
            ((mdproto)->procs.copy_mono)
                        (dev, buf3.b, 0, br, 0L, cx, cy, cw, ch,
                         (gx_color_index)0, (gx_color_index)1);
                      ...
                      int x, int y, int w, int h)
{
    ...
    for (pi = 0; pi < mdev->color_info.num_components; ++pi) {
        ...
        uint plane_raster = ((uint)((((plane_depth * w) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
        int br, bw, bh, cx, cy, cw, ch, ix, iy;

        (mdev->color_info.depth = plane_depth, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * plane_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
        if (plane_raster > (100 * (1 << 3))) {
            br = (100 * (1 << 3));
            bw = (100 * (1 << 3)) * 8 / plane_depth;
            bh = 1;
        } else {
            br = plane_raster;
            bw = w;
        ...
        for (cy = y; cy < y + h; cy += ch) {
            ...
            for (cx = x; cx < x + w; cx += cw) {
                ...
                for (iy = 0; iy < ch; ++iy) {
                    const byte *sptr = (source_base); int sbit = (source_bit);

                    byte *dptr = (buf.b + br * iy); int dbit = (0); byte dbbyte = ((dbit) ? (byte)(*(dptr) & (0xff00 >> (dbit))) : 0);
                    ...
                    for (ix = 0; ix < cw; ++ix) {
                        ...
                        if (sizeof(value) > 4) do { switch ( (source_depth) >> 2 ) { case 0: value = (*(sptr) >> (8 - (sbit) - (source_depth))) & ((source_depth) | 1); break; case 1: value = (*(sptr) >> (4 - (sbit))) & 0xf; break; case 2: value = *(sptr); break; case 3: value = ((sbit) ? ((*(sptr) & 0xf) << 8) | (sptr)[1] : (*(sptr) << 4) | ((sptr)[1] >> 4)); break; case 4: value = (*(sptr) << 8) | (sptr)[1]; break; case 6: value = (*(sptr) << 16) | ((sptr)[1] << 8) | (sptr)[2]; break; case 8: value = (*(sptr) << 24) | ((sptr)[1] << 16) | ((sptr)[2] << 8) | sptr[3]; break;; case 10: value = ((gx_color_index)((sptr)[0]) << ((32) >= 8 * sizeof((value)) ? (32) & (8 * sizeof((value)) - 1) : (32))) | ((gx_color_index)((sptr)[1]) << 24) | ((gx_color_index)((sptr)[2]) << 16) | ((gx_color_index)((sptr)[3]) << 8) | (gx_color_index)((sptr)[4]); break; case 12: value = ((gx_color_index)((sptr)[0]) << ((40) >= 8 * sizeof((value)) ? (40) & (8 * sizeof((value)) - 1) : (40))) | ((gx_color_index)((sptr)[1]) << ((32) >= 8 * sizeof((value)) ? (32) & (8 * sizeof((value)) - 1) : (32))) | ((gx_color_index)((sptr)[2]) << 24) | ((gx_color_index)((sptr)[3]) << 16) | ((gx_color_index)((sptr)[4]) << 8) | (gx_color_index)((sptr)[5]); break; case 14: value = ((gx_color_index)((sptr)[0]) << ((48) >= 8 * sizeof((value)) ? (48) & (8 * sizeof((value)) - 1) : (48))) | ((gx_color_index)((sptr)[1]) << ((40) >= 8 * sizeof((value)) ? (40) & (8 * sizeof((value)) - 1) : (40))) | ((gx_color_index)((sptr)[2]) << ((32) >= 8 * sizeof((value)) ? (32) & (8 * sizeof((value)) - 1) : (32))) | ((gx_color_index)((sptr)[3]) << 24) | ((gx_color_index)((sptr)[4]) << 16) | ((gx_color_index)((sptr)[5]) << 8) | (gx_color_index)((sptr)[6]); break; case 16: value = ((gx_color_index)((sptr)[0]) << ((56) >= 8 * sizeof((value)) ? (56) & (8 * sizeof((value)) - 1) : (56))) | ((gx_color_index)((sptr)[1]) << ((48) >= 8 * sizeof((value)) ? (48) & (8 * sizeof((value)) - 1) : (48))) | ((gx_color_index)((sptr)[2]) << ((40) >= 8 * sizeof((value)) ? (40) & (8 * sizeof((value)) - 1) : (40))) | ((gx_color_index)((sptr)[3]) << ((32) >= 8 * sizeof((value)) ? (32) & (8 * sizeof((value)) - 1) : (32))) | ((gx_color_index)((sptr)[4]) << 24) | ((gx_color_index)((sptr)[5]) << 16) | ((gx_color_index)((sptr)[6]) << 8) | (gx_color_index)((sptr)[7]); break;; default: return ((-15)); } } while (0); else do { switch ( (source_depth) >> 2 ) { case 0: value = (*(sptr) >> (8 - (sbit) - (source_depth))) & ((source_depth) | 1); break; case 1: value = (*(sptr) >> (4 - (sbit))) & 0xf; break; case 2: value = *(sptr); break; case 3: value = ((sbit) ? ((*(sptr) & 0xf) << 8) | (sptr)[1] : (*(sptr) << 4) | ((sptr)[1] >> 4)); break; case 4: value = (*(sptr) << 8) | (sptr)[1]; break; case 6: value = (*(sptr) << 16) | ((sptr)[1] << 8) | (sptr)[2]; break; case 8: value = (*(sptr) << 24) | ((sptr)[1] << 16) | ((sptr)[2] << 8) | sptr[3]; break;; default: return ((-15)); } } while (0); do { sbit += (source_depth); sptr += sbit >> 3; sbit &= 7; } while (0);
                        value = (value >> shift) & mask;
                        do { switch ( (plane_depth) >> 2 ) { case 0: if ( (dbit += (plane_depth)) == 8 ) *(dptr)++ = dbbyte | (byte)(value), dbbyte = 0, dbit = 0; else dbbyte |= (byte)((value) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbbyte = (byte)((value) << 4); else *(dptr)++ = dbbyte | ((byte)(value)); break; case 3: if ( dbit ^= 4 ) *(dptr)++ = (byte)((value) >> 4), dbbyte = (byte)((value) << 4); else *(dptr) = dbbyte | (byte)((value) >> 8), (dptr)[1] = (byte)(value), dptr += 2; break; case 4: *(dptr)++ = (byte)((value) >> 8); case 2: *(dptr)++ = (byte)(value); break; default: return ((-15)); } } while (0);
                       ...
                       int x, int y, int w, int h, int plane_height)
{
    ...
    for (plane = 0; plane < mdev->color_info.num_components; plane++)
    {
        ...
        if (plane_depth == 1)
            code = ((mdproto)->procs.copy_mono)(dev, base, sourcex, sraster, id,
                                                x, y, w, h,
                                                (gx_color_index)0,
                                                (gx_color_index)1);
        else
            code = ((mdproto)->procs.copy_color)(dev, base, sourcex, sraster,
                                                 id, x, y, w, h);
                                ...
                                const gx_drawing_color *pdcolor1, int px, int py)
{
    ...
    for (pi = 0; pi < mdev->color_info.num_components; ++pi) {
        ...
        if (pdcolor0->type == (&gx_dc_type_data_devn)) {
            c0 = (pdcolor0->colors.devn.values[pi]) >> shift & mask;
        } else {
            c0 = ((gx_color_index)(~0));
        }
        if (pdcolor1->type == (&gx_dc_type_data_devn)) {
        ...
        if (c0 == c1)
            ...
            ((dev)->procs.copy_mono = (((mdproto)->procs.copy_mono)));
            ((mdproto)->procs.strip_tile_rectangle)
                (dev, tiles, x, y, w, h, c0, c1, px, py);
        }
        mdev->line_ptrs += mdev->height;
    }
    (mdev->color_info.depth = save.depth, mdev->base = save.base, mdev->line_ptrs = save.line_ptrs);
    ((dev)->procs.copy_mono = (mem_planar_copy_mono));
    return 0;
}
                               ...
                               gs_logical_operation_t lop)
{
    ...
    do { do { if ( (x | y) < 0 ) { if ( x < 0 ) width += x, sourcex -= x, x = 0; if ( y < 0 ) height += y, srow -= (int)(y * sraster), id = 0L, y = 0; } if ( width > (mdev)->width - x ) width = (mdev)->width - x; } while (0); if ( height > (mdev)->height - y ) height = (mdev)->height - y; if ( width <= 0 || height <= 0 ) return 0; } while (0);
    ...
    if (scolors)
    {
        if (scolors[0] == scolors[1]) {
            ...
            switch (cscolor) {
                case 0:
                    cproc = rop_proc_table[( (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~0xcc) )];
                    break;
                case 1:
                    cproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~rop3_1) )];
                    break;
                case 2:
                    cproc = rop_proc_table[rop];
                    break;
                default:
                    cproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | ((rop) & ~(rop3_1 - 0xcc)) )];
                    break;
            }
            switch (mscolor) {
                case 0:
                    mproc = rop_proc_table[( (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~0xcc) )];
                    break;
                case 1:
                    mproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~rop3_1) )];
                    break;
                case 2:
                    mproc = rop_proc_table[rop];
                    break;
                default:
                    mproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | ((rop) & ~(rop3_1 - 0xcc)) )];
                    break;
            }
            switch (yscolor) {
                case 0:
                    yproc = rop_proc_table[( (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~0xcc) )];
                    break;
                case 1:
                    yproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | (((rop) & (rop3_1 - 0xcc)) << 2) | ((rop) & ~rop3_1) )];
                    break;
                case 2:
                    yproc = rop_proc_table[rop];
                    break;
                default:
                    yproc = rop_proc_table[( (((rop) & 0xcc) >> 2) | ((rop) & ~(rop3_1 - 0xcc)) )];
                    break;
            }
        }
    }
    ...
    line_count = height;
    if ((( ((((lop) << 4) ^ (lop)) & 0xf0) != 0 ) || ((lop) & 0x200)) && (tcolors == ((void*)0))) {
        ...
        for (; line_count-- > 0; cdrow += draster, mdrow += draster, ydrow += draster, kdrow += draster, srow += sraster, ++ty) {
            ...
            for (; w > 0; sx += nw, dx += nw, w -= nw, tx = 0) {
                ...
                int tbit = tx & 1;
                int tskew = tbit - dbit;
                int left = (nw = (((w) < (textures->size.x - tx)) ? (w) : (textures->size.x - tx)))-8+dbit;
                int sbit = sx & 1;
                int sskew = sbit - dbit;
                     ...
                     gs_logical_operation_t lop, int plane)
{
    ...
    ((mdev)->procs.get_bits_rectangle = (((mdproto)->procs.get_bits_rectangle)));
    ((mdev)->procs.fill_rectangle = (((mdproto)->procs.fill_rectangle)));
    code = ((mdproto)->procs.strip_copy_rop)((gx_device *)mdev, sdata, sourcex, sraster,
                 ...
                 int plane_height)
{
    ...
    const byte *sptr[(8 * 8)]; int sbit[(8 * 8)];

    byte *dptr; int dbit; byte dbbyte;
    ...
    for (iy = y; iy < y + h; ++iy) {
        ...
        for (ix = w; ix > 0; --ix) {
            ...
            for (pi = 0; pi < num_planes; ++pi) {
                ...
                do { switch ( (plane_depth) >> 2 ) { case 0: value = (*(sptr[pi]) >> (8 - (sbit[pi]) - (plane_depth))) & ((plane_depth) | 1); break; case 1: value = (*(sptr[pi]) >> (4 - (sbit[pi]))) & 0xf; break; case 2: value = *(sptr[pi]); break; case 3: value = ((sbit[pi]) ? ((*(sptr[pi]) & 0xf) << 8) | (sptr[pi])[1] : (*(sptr[pi]) << 4) | ((sptr[pi])[1] >> 4)); break; case 4: value = (*(sptr[pi]) << 8) | (sptr[pi])[1]; break; default: return ((-15)); } } while (0); do { sbit[pi] += (plane_depth); sptr[pi] += sbit[pi] >> 3; sbit[pi] &= 7; } while (0);
                color |= (gx_color_index)value << mdev->planes[pi].shift;
            }
            if (sizeof(color) > 4) do { switch ( (ddepth) >> 2 ) { case 0: if ( (dbit += (ddepth)) == 8 ) *(dptr)++ = dbbyte | (byte)(color), dbbyte = 0, dbit = 0; else dbbyte |= (byte)((color) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbbyte = (byte)((color) << 4); else *(dptr)++ = dbbyte | ((byte)(color)); break; case 3: if ( dbit ^= 4 ) *(dptr)++ = (byte)((color) >> 4), dbbyte = (byte)((color) << 4); else *(dptr) = dbbyte | (byte)((color) >> 8), (dptr)[1] = (byte)(color), dptr += 2; break; case 16: *(dptr)++ = (byte)((color) >> ((56) >= 8 * sizeof((color)) ? (56) & (8 * sizeof((color)) - 1) : (56))); case 14: *(dptr)++ = (byte)((color) >> ((48) >= 8 * sizeof((color)) ? (48) & (8 * sizeof((color)) - 1) : (48))); case 12: *(dptr)++ = (byte)((color) >> ((40) >= 8 * sizeof((color)) ? (40) & (8 * sizeof((color)) - 1) : (40))); case 10: *(dptr)++ = (byte)((color) >> ((32) >= 8 * sizeof((color)) ? (32) & (8 * sizeof((color)) - 1) : (32))); case 8: *(dptr)++ = (byte)((color) >> 24); case 6: *(dptr)++ = (byte)((color) >> 16); case 4: *(dptr)++ = (byte)((color) >> 8); case 2: *(dptr)++ = (byte)(color); break; default: return ((-15)); } } while (0); else do { switch ( (ddepth) >> 2 ) { case 0: if ( (dbit += (ddepth)) == 8 ) *(dptr)++ = dbbyte | (byte)(color), dbbyte = 0, dbit = 0; else dbbyte |= (byte)((color) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbbyte = (byte)((color) << 4); else *(dptr)++ = dbbyte | ((byte)(color)); break; case 3: if ( dbit ^= 4 ) *(dptr)++ = (byte)((color) >> 4), dbbyte = (byte)((color) << 4); else *(dptr) = dbbyte | (byte)((color) >> 8), (dptr)[1] = (byte)(color), dptr += 2; break; case 8: *(dptr)++ = (byte)((color) >> 24); case 6: *(dptr)++ = (byte)((color) >> 16); case 4: *(dptr)++ = (byte)((color) >> 8); case 2: *(dptr)++ = (byte)(color); break; default: return ((-15)); } } while (0);
        }
        if ( (dbit) != 0 ) *(dptr) = dbbyte | (*(dptr) & (0xff >> (dbit)));;
    }
    return 0;
                           ...
                           uint planar_height)
{
    ...
    if (planar_height != 0) {
        ...
        buf = (*(mdev->memory)->procs.alloc_bytes)(mdev->memory, nbytes, "mem_planar_strip_copy_rop(buf)");
        if (buf == ((void*)0)) {
            return ((-25));
        }
        nbytes = sizeof(byte *) * mdev->color_info.num_components * height;
        line_ptrs = (byte **)(*(mdev->memory)->procs.alloc_bytes)(mdev->memory, nbytes, "mem_planar_strip_copy_rop(line_ptrs)");
        if (line_ptrs == ((void*)0)) {
            ((mdev->memory)->procs.free_object(mdev->memory, buf, "mem_planar_strip_copy_rop(buf)"));
            return ((-25));
        }
        ...
        ((mdev->memory)->procs.free_object(mdev->memory, line_ptrs, "mem_planar_strip_copy_rop(line_ptrs)"));
...
# 1876 "./base/gdevmpla.c"
        ...
        chunky_t_raster = ((uint)((((textures->rep_width * mdev->color_info.depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    ...
    if (!(( ((((lop) << 4) ^ (lop)) & 0xf0) != 0 ) || ((lop) & 0x200)) || (tcolors && (tcolors[0] == tcolors[1]))) {

        if ((!(( ((((lop) << 2) ^ (lop)) & 0xcc) != 0 ) || ((lop) & (0x100 | 0x200))) || (scolors && (scolors[0] == scolors[1]))) &&
            ...
            for (plane=0; plane < mdev->color_info.num_components; plane++)
            {
                ...
                code = plane_strip_copy_rop(mdev, sdata, sourcex, sraster,
                                            id, (scolors ? scolors2 : ((void*)0)),
                                            textures, (tcolors ? tcolors2 : ((void*)0)),
                                            x, y, width, height,
                              ...
                              gs_int_rect ** unread)
{
    ...
    {
        gs_get_bits_params_t copy_params;
        byte **base = &((mdev)->line_ptrs[y]);
        int code;

        copy_params.options =
            (1L<<0) | (1L<<17) | (1L<<4) |
            (mdev->raster ==
             ((uint)((((mdev->width * mdev->color_info.depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) ?
             (1L<<28) : (1L<<29));
    ...
    if (!(~options & ((1L<<17) | (1L<<19)))) {
        ...
        if (pi < num_planes) {
            ...
            if (pi == num_planes) {
                ...
                copy_params = *params;
                copy_params.options =
                    (options & ~(((1L<<16) | (1L<<17) | (1L<<18)) | (1L<<19))) |
                    (1L<<16);
                copy_params.data[0] = copy_params.data[plane];
                (save.depth = mdev->color_info.depth, save.base = mdev->base, save.line_ptrs = mdev->line_ptrs);
                mdev->line_ptrs += mdev->height * plane;
                (mdev->color_info.depth = mdev->planes[plane].depth, mdev->base = mdev->line_ptrs[0], mdev->raster = (mdev->height > 1 ? mdev->line_ptrs[1]-mdev->line_ptrs[0] : ((uint)((((mdev->width * mdev->planes[plane].depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))));
...
# 2106 "./base/gdevmpla.c"
    if (!(~options & ((1L<<0) | (1L<<4) |
                      (1L<<16) | (1L<<20)))) {
        int offset = (options & (1L<<25) ? params->x_offset : 0);
        uint draster =
            (options & (1L<<29) ? params->raster :
             ((uint)(((((offset + w) * mdev->color_info.depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)));

        planar_to_chunky(mdev, x, y, w, h, offset, draster, params->data[0],
                         mdev->line_ptrs, mdev->height);
    } else {
# 2126 "./base/gdevmpla.c"
        union b_ {
            ulong l[(((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long)))];
            byte b[((((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) * (1 << 3))];
        } buf;
        int br, bw, bh, cx, cy, cw, ch;
        int ddepth = mdev->color_info.depth;
        uint raster = ((uint)((((ddepth * mdev->width) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
        gs_get_bits_params_t dest_params;
        int dest_bytes;

        if (raster > ((((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) * (1 << 3))) {
            br = ((((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) * (1 << 3));
            bw = ((((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) * (1 << 3)) * 8 / ddepth;
            bh = 1;
        } else {
            br = raster;
            bw = w;
            bh = ((((100) > (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) ? (100) : (((8 * 8) * 2 + sizeof(long) - 1) / sizeof(long))) * (1 << 3)) / raster;
        }
        copy_params.options =
            (1L<<0) | (1L<<16) | (1L<<4) |
            (1L<<28);
        ...
        if (options & (1L<<2))
            dest_bytes = 3;
        else if (options & (1L<<3))
            dest_bytes = 4;
        else if (options & (1L<<1))
            dest_bytes = 1;
        else
