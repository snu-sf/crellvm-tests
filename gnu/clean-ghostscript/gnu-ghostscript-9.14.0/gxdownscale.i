# 1 "./base/gxdownscale.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./base/gxdownscale.c" 2
# 17 "./base/gxdownscale.c"
...
# 20 "./base/gxdownscale.c" 2
# 33 "./base/gxdownscale.c"
enum {
    mfs_clear = 0,
    mfs_force_off = 1,
    mfs_above_is_0 = 2,
...
int gx_downscaler_init_planar(gx_downscaler_t *ds,
                              ...
                              int dst_bpc)
{
    int span = ((uint)((((dev->width * src_bpc) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    int width;
    int code;
    ...
    ds->src_bpc = src_bpc;
    ds->scaled_data = ((void*)0);
    ds->scaled_span = ((uint)(((((dst_bpc*dev->width*upfactor + downfactor-1)/downfactor) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));

    memcpy(&ds->params, params, sizeof(*params));
    ds->params.raster = span;
    for (i = 0; i < num_comps; i++)
    {
        ds->params.data[i] = (*(dev->memory)->procs.alloc_bytes)(dev->memory, span * downfactor, "gx_downscaler(planar_data)");

        if (ds->params.data[i] == ((void*)0)) {
            code = ((-25));
            goto cleanup;
        }
    }
    if (upfactor > 1)
    {
        ds->scaled_data = (*(dev->memory)->procs.alloc_bytes)(dev->memory, ds->scaled_span * upfactor * num_comps, "gx_downscaler(scaled_data)");

        if (ds->scaled_data == ((void*)0)) {
            code = ((-25));
            goto cleanup;
        }
    }
    ...
    if (mfs > 1) {
        ds->mfs_data = (byte *)(*(dev->memory)->procs.alloc_bytes)(dev->memory, (width+1) * num_comps, "gx_downscaler(mfs)");


        if (ds->mfs_data == ((void*)0)) {
            code = ((-25));
            goto cleanup;
        }
    ...
    if (dst_bpc == 1)
    {
        ds->errors = (int *)(*(dev->memory)->procs.alloc_bytes)(dev->memory, num_comps*(width+3)*sizeof(int), "gx_downscaler(errors)");
...
int gx_downscaler_init(gx_downscaler_t *ds,
                       ...
                       int adjust_width)
{
    int size = gx_device_raster((gx_device *)((gx_device_printer *)((gx_device *)dev)), 0);
    int span;
    int width;
    int awidth;
    int pad_white;
    ...
    if (core != ((void*)0)) {
        ds->data = (*(dev->memory)->procs.alloc_bytes)(dev->memory, span * downfactor, "gx_downscaler(data)");
        ...
        if (mfs > 1) {
            ds->mfs_data = (byte *)(*(dev->memory)->procs.alloc_bytes)(dev->memory, awidth+1, "gx_downscaler(mfs)");
        ...
        if (dst_bpc == 1)
        {
            ds->errors = (int *)(*(dev->memory)->procs.alloc_bytes)(dev->memory, num_comps*(awidth+3)*sizeof(int), "gx_downscaler(errors)");
...
void gx_downscaler_fin(gx_downscaler_t *ds)
{
    ...
    {
        ((ds->dev->memory)->procs.free_object(ds->dev->memory, ds->params.data[plane], "gx_downscaler(planar_data)"));

    }
    ds->num_planes = 0;

    ((ds->dev->memory)->procs.free_object(ds->dev->memory, ds->mfs_data, "gx_downscaler(mfs)"));
    ds->mfs_data = ((void*)0);
    ((ds->dev->memory)->procs.free_object(ds->dev->memory, ds->errors, "gx_downscaler(errors)"));
    ds->errors = ((void*)0);
    ((ds->dev->memory)->procs.free_object(ds->dev->memory, ds->data, "gx_downscaler(data)"));
    ds->data = ((void*)0);
    ((ds->dev->memory)->procs.free_object(ds->dev->memory, ds->scaled_data, "gx_downscaler(scaled_data)"));
    ds->scaled_data = ((void*)0);
}
                          ...
                          int row)
{
    ...
    if (ds->down_core == ((void*)0)) {
        return (*((ds->dev)->procs.get_bits))(ds->dev, row, out_data, ((void*)0));
    }
    ...
    data_ptr = ds->data;
    do {
        code = (*((ds->dev)->procs.get_bits))(ds->dev, y, data_ptr, ((void*)0));
        if (code < 0)
            return code;
        data_ptr += ds->span;
        y++;
                                     ...
                                     int row)
{
    ...
    if (ds->down_core == ((void*)0)) {
        return (*((ds->dev)->procs.get_bits_rectangle))(ds->dev, &rect, params, ((void*)0));
    }
    ...
    code = (*((ds->dev)->procs.get_bits_rectangle))(ds->dev, &rect, &params2, ((void*)0));
    if (code == (-15))
    {
        int i, j;
                              ...
                              byte * str, uint size)
{
    ...
    count = (((count) < (height - y)) ? (count) : (height - y));
...
static int downscaler_init_fn(void *arg_, gx_device *dev, gs_memory_t *memory, int w, int h, void **pbuffer)
{
    ...
    buffer = (downscaler_process_page_buffer_t *)(*(memory)->procs.alloc_bytes)(memory, sizeof(*buffer), "downscaler process_page buffer");
    if (buffer == ((void*)0))
        return (-25);
    ...
    if (arg->upfactor > arg->downfactor) {
        code = gx_default_create_buf_device(&buffer->bdev, dev,
                          ...
                          ((void*)0), memory, ((void*)0));
        if (code < 0) {
            ((memory)->procs.free_object(memory, buffer, "downscaler process_page buffer"));
            return code;
        }
    }
...
static int downscaler_process_fn(void *arg_, gx_device *dev, gx_device *bdev, const gs_int_rect *rect, void *buffer_)
{
    ...
    params.options = (1L<<0) | (1L<<4) | (1L<<16) | (1L<<21) | (1L<<23) | (1L<<24) | (1L<<30);
    code = ((bdev)->procs.get_bits_rectangle)(bdev, &in_rect, &params, ((void*)0));
    if (code < 0)
        return code;
    ...
    if (buffer->bdev) {
        code = ((bdev)->procs.get_bits_rectangle)(buffer->bdev, &out_rect, &params, ((void*)0));
        if (code < 0)
            return code;
                               ...
                               int factor)
{
    ...
    arg.ds.span = ((uint)((((dev->width * num_comps * src_bpc) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    scaled_w = (dev->width * arg.upfactor + arg.downfactor-1)/arg.downfactor;
    arg.ds.factor = factor;
    arg.ds.src_bpc = src_bpc;
    arg.ds.scaled_span = ((uint)((((scaled_w * num_comps * src_bpc) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    arg.ds.num_planes = 0;
    ...
    return ((dev)->procs.process_page)(dev, &my_options);
}
