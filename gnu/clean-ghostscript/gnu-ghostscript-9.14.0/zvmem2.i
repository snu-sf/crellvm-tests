...
zsetglobal(i_ctx_t *i_ctx_p)
{
    ...
    ialloc_set_space((&(i_ctx_p->memory)),
...
zcurrentglobal(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.boolval = ((((&(i_ctx_p->memory)))->current_space) != avm_local), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
...
zgcheck(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.boolval = ((((avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) == avm_local) ? ((bool)0) : ((bool)1))), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
# 77 "./psi/zvmem2.c"
int
set_vm_threshold(i_ctx_t *i_ctx_p, long val)
{
    if (val < -1)
        ...
        val = (gs_debug_c('.') ? 100000 :
               8000000);
    else if (val < 1)
        val = 1;
    else if (val > (~(-1L << ((1 << 3) * 8 - 1))))
        val = (~(-1L << ((1 << 3) * 8 - 1)));
    gs_memory_set_vm_threshold((&(i_ctx_p->memory))->spaces.memories.named.global, val);
    gs_memory_set_vm_threshold((&(i_ctx_p->memory))->spaces.memories.named.local, val);
    return 0;
}
...
{
    if (val >= -2 && val <= 0) {
        gs_memory_set_vm_reclaim((&(i_ctx_p->memory))->spaces.memories.named.system, (val >= -1));
        gs_memory_set_vm_reclaim((&(i_ctx_p->memory))->spaces.memories.named.global, (val >= -1));
        gs_memory_set_vm_reclaim((&(i_ctx_p->memory))->spaces.memories.named.local, (val == 0));
        return 0;
    } else
        return ((-15));
}
