...
common_viewclip(gs_state * pgs, int rule)
{
    ...
    if (pcpath == 0) {
        pcpath = gx_cpath_alloc_shared(((void*)0), pgs->memory, "gs_[eo]viewclip");
        if (pcpath == 0)
            return ((-25));
    ...
    if ((code = gx_path_bbox(pgs->path, &bbox)) < 0)
        return code;
    (void)gx_cpath_init_local_shared(&rpath, ((void*)0), pgs->memory);
