# 1 "./devices/vector/gdevpdfe.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdfe.c" 2
# 18 "./devices/vector/gdevpdfe.c"
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
...
# 19 "./devices/vector/gdevpdfe.c" 2
...
# 33 "./base/string_.h" 2
# 21 "./devices/vector/gdevpdfe.c" 2
# 1 "./base/time_.h" 1
# 30 "./base/time_.h"
# 1 "./obj/gconfig_.h" 1
# 31 "./base/time_.h" 2
...
# 83 "./base/time_.h" 2
# 22 "./devices/vector/gdevpdfe.c" 2
# 1 "./base/stream.h" 1
# 23 "./base/stream.h"
# 1 "./base/scommon.h" 1
# 24 "./base/scommon.h"
...
int file_close_finish(stream *);
int file_close_disable(stream *);
# 23 "./devices/vector/gdevpdfe.c" 2
# 1 "./base/gp.h" 1
# 24 "./base/gp.h"
# 1 "./base/gscdefs.h" 1
# 31 "./base/gscdefs.h"
...
# 24 "./devices/vector/gdevpdfe.c" 2
# 1 "./base/smd5.h" 1
# 23 "./base/smd5.h"
# 1 "./base/md5.h" 1
# 63 "./base/md5.h"
...
# 25 "./devices/vector/gdevpdfe.c" 2
...
# 27 "./devices/vector/gdevpdfe.c" 2
...
# 28 "./devices/vector/gdevpdfe.c" 2
...
# 29 "./devices/vector/gdevpdfe.c" 2
...
# 30 "./devices/vector/gdevpdfe.c" 2
...
pdf_xmp_time(char *buf, int buf_length)
{
    ...
    __builtin_strncpy (buf, buf1, buf_length);
    return strlen(buf);
}
...
pdf_xmp_convert_time(char *dt, int dtl, char *buf, int bufl)
{
    ...
    dt[10] = 'T';
    memcpy(dt + 11, buf + 8, 2);
    dt[13] = ':';
    memcpy(dt + 14, buf + 10, 2);
    if (l <= 12) {
        dt[16] = 'Z';
        return 17;
    }
    ...
    if (l <= 15)
        return 20;
    memcpy(dt + 20, buf + 15, 2);
    if (l <= 17)
        return 22;

    dt[22] = ':';

    memcpy(dt + 23, buf + 18, 2);
    return 25;
}
...
pdf_get_docinfo_item(gx_device_pdf *pdev, const char *key, char *buf, int buf_length)
{
    ...
    if (v != ((void*)0) && (v->value_type == COS_VALUE_SCALAR ||
                         ...
                         void(*write)(stream *s, const byte *data, int data_length))
{
    if (pdev->DSCEncodingToUnicode.data == 0) {
        ...
        buf0 = (unsigned char *)(*(pdev->memory)->procs.alloc_bytes)(pdev->memory, data_length * sizeof(unsigned char), "pdf_xmp_write_translated");
        ...
        if (buf0[0] != 0xfe || buf0[1] != 0xff) {
            ...
            buf1 = (unsigned char *)(*(pdev->memory)->procs.alloc_bytes)(pdev->memory, (j * sizeof(UTF16)) + 2, "pdf_xmp_write_translated");
            ...
            for (i = 0; i < j; i++) {
                if (buf0[i] <= 0x7f || buf0[i] >= 0xAE) {
                    if (buf0[i] == 0x7f) {
                        (emprintf_program_ident(pdev->memory, gs_program_name(), gs_revision_number()), errprintf(pdev->memory, "PDFDocEncoding %x cannot be represented in Unicode\n", buf0[i]));
        ...
        }
        {
            ...
            buf1 = (UTF8 *)(*(pdev->memory)->procs.alloc_bytes)(pdev->memory, data_length * 2 * sizeof(unsigned char), "pdf_xmp_write_translated");
            ...
            switch (ConvertUTF16toUTF8((const UTF16**)&buf0b, (UTF16 *)(buf0 + j),
                ...
                case conversionOK:
                    ...
                    ((pdev->memory)->procs.free_object(pdev->memory, buf1, "pdf_xmp_write_translated"));
                    break;
        ...
        buf0 = (UTF16 *)(*(pdev->memory)->procs.alloc_bytes)(pdev->memory, data_length * sizeof(UTF16), "pdf_xmp_write_translated");

        if (buf0 == ((void*)0))
            return ((-25));
        buf1 = (UTF8 *)(*(pdev->memory)->procs.alloc_bytes)(pdev->memory, data_length * 2, "pdf_xmp_write_translated");
...
{
    long *dt = pdev->uuid_time;
    uint64_t t;


    t = (uint64_t)10000000 * dt[0] + dt[0] / 100;
    t += (uint64_t) (1000*1000*10)
       * (uint64_t) (60 * 60 * 24)
       * (uint64_t) (17+30+31+365*397+99);
    return t;
}
...
{
    char b[45], *p = b;
    ulong uuid_time_lo = (ulong)(uuid_time & 0xFFFFFFFF);
    ushort uuid_time_md = (ushort)((uuid_time >> 32) & 0xFFFF);
    ushort uuid_time_hi = (ushort)((uuid_time >> 48) & 0x0FFF);

    writehex(&p, uuid_time_lo, 4);
    *(p++) = '-';
    writehex(&p, uuid_time_md, 2);
    *(p++) = '-';
    writehex(&p, uuid_time_hi | (ushort)(1 << 12), 2);
    *(p++) = '-';
    writehex(&p, (time_seq & 0x3F00) >> 8, 1);
    writehex(&p, time_seq & 0xFF, 1);
    *(p++) = '-';
    writehex(&p, node[0], 1);
    writehex(&p, node[1], 1);
    writehex(&p, node[2], 1);
    ...
    writehex(&p, node[5], 1);
    *p = 0;
    __builtin_strncpy (buf, b, buf_length);
}
...
pdf_make_instance_uuid(gx_device_pdf *pdev, const byte digest[6], char *buf, int buf_length)
{
    ...
    if (pdev->InstanceUUID.size) {
        int l = (((buf_length - 6) < (pdev->InstanceUUID.size)) ? (buf_length - 6) : (pdev->InstanceUUID.size));
...
pdf_make_document_uuid(gx_device_pdf *pdev, const byte digest[6], char *buf, int buf_length)
{
    ...
    if (pdev->DocumentUUID.size) {
        int l = (((buf_length - 6) < (pdev->DocumentUUID.size)) ? (buf_length - 6) : (pdev->DocumentUUID.size));
...
pdf_document_metadata(gx_device_pdf *pdev)
{
    ...
    if (pdev->ParseDSCCommentsForDocInfo || pdev->PreserveEPSInfo) {
        ...
        if (pdev->EncryptMetadata)
            options |= 8;
        code = pdf_open_aside(pdev, resourceMetadata, 0L, &pres, ((bool)1), options);
        if (code < 0)
            return code;
        ...
        if (code < 0)
            return code;
        code = cos_write_object(((cos_object_t *)&((pres->object)->cos_procs)), pdev, resourceNone);
        if (code < 0)
            return code;
        gs_sprintf(buf, "%ld 0 R", pres->object->id);
        pdf_record_usage(pdev, pres->object->id, -3);
