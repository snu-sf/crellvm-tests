...
# 22 "./base/slzwe.c" 2
# 35 "./base/slzwe.c"
extern void s_LZW_release(stream_state *);
...
struct lzw_encode_table_s {
        lzw_encode encode[4095];
        ushort hashed[(4095 + 4095 / 4)];
};
static const gs_memory_struct_type_t st_lzwe_table = { sizeof(lzw_encode_table), "lzw_encode_table", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
# 61 "./base/slzwe.c"
static byte *
...
{ register int c;
        ...
        ss->code_size = 9;
        ss->prev_code = 257;
        for ( c = 0; c < (4095 + 4095 / 4); c++ )
                table->hashed[c] = 257;
        for ( c = 0; c < 256; c++ )
        { lzw_encode *ec = &table->encode[c];
                register ushort *tc = &table->hashed[((uint)((257) * 59 + (c) * (((4095 + 4095 / 4) / 256) | 1)) % (4095 + 4095 / 4))];
                while ( *tc != 257 )
                  if ( ++tc == &table->hashed[(4095 + 4095 / 4)] )
                    tc = &table->hashed[0];
                *tc = c;
                ec->datum = c, ec->prefix = 257;
        }
        table->encode[257].prefix = 256;
}
...
{ ((stream_LZW_state *)st)->bits_left = 8;
        ((stream_LZW_state *)st)->bits = 0;
        ((stream_LZW_state *)st)->table.encode = (lzw_encode_table *)(*(st->memory)->procs.alloc_struct)(st->memory, &st_lzwe_table, "LZWEncode init");

        if ( ((stream_LZW_state *)st)->table.encode == 0 )
                return ((int)(-2));
        ((stream_LZW_state *)st)->first = ((bool)1);
        lzw_reset_encode(((stream_LZW_state *)st));
        return 0;
}
...
{ register const byte *p = pr->ptr;
        ...
        int code = ((stream_LZW_state *)st)->prev_code;
        lzw_encode_table *table = ((stream_LZW_state *)st)->table.encode;
        ushort *table_end = &table->hashed[(4095 + 4095 / 4)];
        int status = 0;
        int limit_code;



        limit_code = (1 << ((stream_LZW_state *)st)->code_size) - ((stream_LZW_state *)st)->EarlyChange; if ( limit_code > 4095 ) limit_code = 4095;
        if ( ((stream_LZW_state *)st)->first )
        {
                if ( wlimit - q < 2 )
                        return 1;
                q = lzw_put_code(((stream_LZW_state *)st), q, 256);
                ((stream_LZW_state *)st)->first = ((bool)0);
        }
        while ( p < rlimit )
        { byte c = p[1];
                ushort *tp;
                for ( tp = &table->hashed[((uint)((code) * 59 + (c) * (((4095 + 4095 / 4) / 256) | 1)) % (4095 + 4095 / 4))]; ; )
                { lzw_encode *ep = &table->encode[*tp];
                        ...
                        else
                        {
                                ...
                                q = lzw_put_code(((stream_LZW_state *)st), q, code);
                                if ( ((stream_LZW_state *)st)->next_code == limit_code )
                                {

                                        if ( ((stream_LZW_state *)st)->next_code == 4095 )
                                        { q = lzw_put_code(((stream_LZW_state *)st), q, 256);
                                                lzw_reset_encode(((stream_LZW_state *)st));
                                                limit_code = (1 << ((stream_LZW_state *)st)->code_size) - ((stream_LZW_state *)st)->EarlyChange; if ( limit_code > 4095 ) limit_code = 4095;
                                                goto cx;
                                        }
                                        ((stream_LZW_state *)st)->code_size++;
                                        limit_code = (1 << ((stream_LZW_state *)st)->code_size) - ((stream_LZW_state *)st)->EarlyChange; if ( limit_code > 4095 ) limit_code = 4095;
                                }
                                do { } while (0);

                                *tp = ((stream_LZW_state *)st)->next_code++;
                                ep = &table->encode[*tp];
                                ep->datum = c;
                                ep->prefix = code;
cx: code = 257;
        ...
        { if ( wlimit - q < 4 )
                  ...
                  { if ( code != 257 )
                          { q = lzw_put_code(((stream_LZW_state *)st), q, code);
                                if (((stream_LZW_state *)st)->next_code == limit_code && ((stream_LZW_state *)st)->next_code != 4095)
                                    ((stream_LZW_state *)st)->code_size++;
                          }
                        q = lzw_put_code(((stream_LZW_state *)st), q, 257);
                        if ( ((stream_LZW_state *)st)->bits_left < 8 )
                          *++q = ((stream_LZW_state *)st)->bits << ((stream_LZW_state *)st)->bits_left;
                  }
        }
out: ((stream_LZW_state *)st)->prev_code = code;
        pr->ptr = p;
        pw->ptr = q;
        return status;
}
