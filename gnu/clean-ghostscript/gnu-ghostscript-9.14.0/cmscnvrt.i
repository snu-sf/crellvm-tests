# 1 "./lcms2/src/cmscnvrt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./lcms2/src/cmscnvrt.c" 2
# 27 "./lcms2/src/cmscnvrt.c"
...
static cmsIntentsList DefaultIntents[] = {
    ...
    { 0, "Perceptual", DefaultICCintents, &DefaultIntents[1] },
    { 1, "Relative colorimetric", DefaultICCintents, &DefaultIntents[2] },
    { 2, "Saturation", DefaultICCintents, &DefaultIntents[3] },
    { 3, "Absolute colorimetric", DefaultICCintents, &DefaultIntents[4] },
    { 10, "Perceptual preserving black ink", BlackPreservingKOnlyIntents, &DefaultIntents[5] },
    { 11, "Relative colorimetric preserving black ink", BlackPreservingKOnlyIntents, &DefaultIntents[6] },
    { 12, "Saturation preserving black ink", BlackPreservingKOnlyIntents, &DefaultIntents[7] },
    { 13, "Perceptual preserving black plane", BlackPreservingKPlaneIntents, &DefaultIntents[8] },
    { 14,"Relative colorimetric preserving black plane", BlackPreservingKPlaneIntents, &DefaultIntents[9] },
    { 15, "Saturation preserving black plane", BlackPreservingKPlaneIntents, ((void*)0) }
};



_cmsIntentsPluginChunkType _cmsIntentsPluginChunk = { ((void*)0) };
...
cmsFloat64Number CHAD2Temp(const cmsMAT3* Chad)
{
    ...
    m1 = *Chad;
    if (!_cmsMAT3inverse(&m1, &m2)) return 0;
    ...
    void Temp2CHAD(cmsMAT3* Chad, cmsFloat64Number Temp)
{
    ...
    _cmsAdaptationMatrix(Chad, ((void*)0), &White, cmsD50_XYZ());
}
                               ...
                               cmsMAT3* m)
{
    ...
    if (AdaptationState == 1.0) {
        ...
        if (AdaptationState == 0.0) {
            ...
            if (!_cmsMAT3inverse(&m3, &m4)) return 0;
            _cmsMAT3per(m, &m2, &m4);
            ...
            if (TempSrc < 0.0 || TempDest < 0.0) return 0;
...
cmsBool IsEmptyLayer(cmsMAT3* m, cmsVEC3* off)
{
    ...
    if (m == ((void*)0) && off == ((void*)0)) return 1;
    if (m == ((void*)0) && off != ((void*)0)) return 0;
                                 ...
                                 cmsMAT3* m, cmsVEC3* off)
{
    ...
    if (Intent == 3) {
        ...
        if (!ComputeAbsoluteIntent(AdaptationState,
                                  ...
                                  &WhitePointOut, &ChromaticAdaptationMatrixOut, m)) return 0;
...
# 420 "./lcms2/src/cmscnvrt.c"
    for (k=0; k < 3; k++) {
        off ->n[k] /= (1.0 + 32767.0/32768.0);
    }
...
cmsBool ColorSpaceIsCompatible(cmsColorSpaceSignature a, cmsColorSpaceSignature b)
{
    ...
    if ((a == cmsSig4colorData) && (b == cmsSigCmykData)) return 1;
    if ((a == cmsSigCmykData) && (b == cmsSig4colorData)) return 1;


    if ((a == cmsSigXYZData) && (b == cmsSigLabData)) return 1;
    if ((a == cmsSigLabData) && (b == cmsSigXYZData)) return 1;
                               ...
                               cmsUInt32Number dwFlags)
{
    ...
    for (i=0; i < nProfiles; i++) {
        ...
        if (!ColorSpaceIsCompatible(ColorSpaceIn, CurrentColorSpace)) {
            ...
            cmsSignalError(ContextID, 9, "ColorSpace mismatch");
            goto Error;
        }
...
Error:
    ...
    if (Result != ((void*)0)) cmsPipelineFree(Result);
    return ((void*)0);
...
Error:
    ...
    if (bp.cmyk2cmyk != ((void*)0)) cmsPipelineFree(bp.cmyk2cmyk);
    if (bp.KTone != ((void*)0)) cmsFreeToneCurve(bp.KTone);
    if (Result != ((void*)0)) cmsPipelineFree(Result);
    return ((void*)0);
                                          ...
                                          cmsUInt32Number dwFlags)
{
    ...
    bp.LabK2cmyk = _cmsReadInputLUT(hProfiles[nProfiles-1], 1);
    ...
    hLab = cmsCreateLab4ProfileTHR(ContextID, ((void*)0));
    bp.hProofOutput = cmsCreateTransformTHR(ContextID, hProfiles[nProfiles-1],
                                         ((4) << 3)|(2), hLab, (((1) << 22)|((10) << 16)|((3) << 3)|(0)),
                                         1,
                                         0x0040|0x0100);
                              ...
                              cmsUInt32Number dwFlags)
{
    ...
    if (nProfiles <= 0 || nProfiles > 255) {
         cmsSignalError(ContextID, 2, "Couldn't link '%d' profiles", nProfiles);
        return ((void*)0);
    }
    ...
    for (i=0; i < nProfiles; i++) {
        ...
        if (TheIntents[i] == 0 || TheIntents[i] == 2) {
    ...
    if (Intent == ((void*)0)) {
        cmsSignalError(ContextID, 8, "Unsupported intent '%d'", TheIntents[0]);
        return ((void*)0);
    }
...
cmsUInt32Number cmsGetSupportedIntentsTHR(cmsContext ContextID, cmsUInt32Number nMax, cmsUInt32Number* Codes, char** Descriptions)
{
    ...
    for (nIntents=0, pt = ctx->Intents; pt != ((void*)0); pt = pt -> Next)
    {
        if (nIntents < nMax) {
            if (Codes != ((void*)0))
    ...
    for (nIntents=0, pt = DefaultIntents; pt != ((void*)0); pt = pt -> Next)
    {
        if (nIntents < nMax) {
            if (Codes != ((void*)0))
                Codes[nIntents] = pt ->Intent;
...
cmsUInt32Number cmsGetSupportedIntents(cmsUInt32Number nMax, cmsUInt32Number* Codes, char** Descriptions)
{
    return cmsGetSupportedIntentsTHR(((void*)0), nMax, Codes, Descriptions);
}
...
cmsBool _cmsRegisterRenderingIntentPlugin(cmsContext id, cmsPluginBase* Data)
{
    ...
    __builtin_strncpy (fl ->Description, Plugin ->Description, sizeof(fl ->Description)-1);
