...
cid_system_info_param(gs_cid_system_info_t *pcidsi, const ref *prcidsi)
{
    ...
    if (dict_find_string(prcidsi, "Registry", &pregistry) <= 0 ||
        ...
        return ((-15));
    do { if ( !(((&*pregistry)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*pregistry)->tas.type_attrs))[1]) == (t_string)) ? (-20) : (-7))); } while (0);
    do { if ( !(((&*pordering)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*pordering)->tas.type_attrs))[1]) == (t_string)) ? (-20) : (-7))); } while (0);
    ...
    code = dict_int_param(prcidsi, "Supplement", 0, (~(-1 << ((1 << 2) * 8 - 1))), -1,
                          &pcidsi->Supplement);
...
{ ref *DecodingArray, char_code, char_code1, ih, *glyph_index;
    ...
    ((&ih)->value.intval = (nCID / 256), ((&ih)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ...
    for (;n--; i++) {
        ...
        if (dict_find(TT_cmap, &char_code1, &glyph_index) >= 0 &&
                ((((const byte *)&((glyph_index)->tas.type_attrs))[1]) == (t_integer))) {
...
cid_to_TT_charcode(const gs_memory_t *mem,
                   ...
                   uint nCID, uint *c, ref *src_type, ref *dst_type)
{
    int SubstNWP_length = ((SubstNWP)->tas.rsize), i, code;

    if (TT_char_code_from_CID_no_subst(mem, Decoding, TT_cmap, nCID, c)) {
        (((src_type)->tas.type_attrs = ((t_null) << 8) + (0)) );
...
set_CIDMap_element(const gs_memory_t *mem, ref *CIDMap, uint cid, uint glyph_index)
{
    ...
    int offset = cid * 2;
    int count = ((CIDMap)->tas.rsize), size, i;
    ref s;
    uchar *c;
...
{ int dict_enum;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (code < 0)
            return code;
        if ( !((((const byte *)&((&s)->tas.type_attrs))[1]) == (t_string)) ) return (check_type_failed(&s));
    }
    ...
    for (;;) {
        ...
        if ((dict_enum = dict_next(Decoding, dict_enum, el)) == -1)
            break;
        if (!((((const byte *)&((&el[0])->tas.type_attrs))[1]) == (t_integer)))
            continue;
        if (!((((const byte *)&((&el[1])->tas.type_attrs))[1]) == (t_array)))
            return ((-20));
        index = el[0].value.intval;
        count = ((&el[1])->tas.rsize);
        for (i = 0; i < count; i++) {
