...
static const gc_ptr_element_t iscale_state_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->dst - (char *)((stream_IScale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->src - (char *)((stream_IScale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->tmp - (char *)((stream_IScale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->contrib - (char *)((stream_IScale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->items - (char *)((stream_IScale_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_IScale_state *)0)->dst_items - (char *)((stream_IScale_state *)0) )) } }; static const gc_struct_data_t iscale_state_reloc_ptrs = { (sizeof(iscale_state_enum_ptrs) / sizeof((iscale_state_enum_ptrs)[0])), 0, 0, iscale_state_enum_ptrs }; static const gs_memory_struct_type_t st_IScale_state = { sizeof(stream_IScale_state), "ImageScaleEncode/Decode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &iscale_state_reloc_ptrs };
# 86 "./base/siscale.c"
static double
Mitchell_filter(double t)
{
    ...
    if (t < 1)
        return
            ((12 - 9 * (1.0 / 3.0) - 6 * (1.0 / 3.0)) * (t * t2) +
             (-18 + 12 * (1.0 / 3.0) + 6 * (1.0 / 3.0)) * t2 +
             (6 - 2 * (1.0 / 3.0))) / 6;
    else if (t < 2)
        return
            ((-1 * (1.0 / 3.0) - 6 * (1.0 / 3.0)) * (t * t2) +
             (6 * (1.0 / 3.0) + 30 * (1.0 / 3.0)) * t2 +
             (-12 * (1.0 / 3.0) - 48 * (1.0 / 3.0)) * t +
             (8 * (1.0 / 3.0) + 24 * (1.0 / 3.0))) / 6;
    else
        return 0;
}
...
Interp_contrib_pixels(double scale)
{
    if (scale == 0.0)
        return 1;
    return (int)(((float)1) / (scale >= 1.0 ? 1.0 : scale)
                 * 2 + 1.5);
}
...
Mitchell_contrib_pixels(double scale)
{
    if (scale == 0.0)
        return 1;
    return (int)(((float)2) / (scale >= 1.0 ? 1.0 : (((scale) > (((2 * 2) / ((1 << 3) - 1.01)))) ? (scale) : (((2 * 2) / ((1 << 3) - 1.01)))))
                 * 2 + 1.5);
}
...
)
{
    ...
    do { } while (0);
    if (scale < 1.0) {
        double clamped_scale = (((scale) > (min_scale)) ? (scale) : (min_scale));
...
# 225 "./base/siscale.c"
        ...
        int first_pixel = (left < 0 ? 0 : left >= limit ? limit - 1 : left);
        int last_pixel = (right < 0 ? 0 : right >= limit ? limit - 1 : right);
        CONTRIB *p;
...
static void
zoom_x(byte * tmp, const void *src, int sizeofPixelIn,
    ...
    for (c = 0; c < Colors; ++c) {
        ...
        if (sizeofPixelIn == 1) {
            ...
            for ( i = 0; i < tmp_width; tp += Colors, ++clp, ++i ) {
                ...
                do { } while (0);
                *tp = (byte)(pixel < 0 ? 0 : pixel > 255 ? 255 : pixel);
            }
        } else {
        ...
        const filter_defn_s *vert)
{
    ...
    ss->tmp = (byte *) (*(mem)->procs.alloc_byte_array)(mem, ss->max_support, (ss->params.WidthOut * ss->params.spp_interp), "image_scale tmp");




    ss->contrib = (CLIST *) (*(mem)->procs.alloc_byte_array)(mem, (((ss->params.WidthOut) > (ss->params.HeightOut)) ? (ss->params.WidthOut) : (ss->params.HeightOut)), sizeof(CLIST), "image_scale contrib");




    ss->items = (CONTRIB *)
                    (*(mem)->procs.alloc_byte_array)(mem, (horiz->contrib_pixels( (double)ss->params.EntireWidthOut / ss->params.EntireWidthIn) * ss->params.WidthOut), sizeof(CONTRIB), "image_scale contrib[*]");
    ...
    ss->dst_items = (CONTRIB *) (*(mem)->procs.alloc_byte_array)(mem, ss->max_support*2, sizeof(CONTRIB), "image_scale contrib_dst[*]");



    ss->dst =
        (*(mem)->procs.alloc_byte_array)(mem, ss->params.WidthOut * ss->params.spp_interp, ss->sizeofPixelOut, "image_scale dst");

    ss->src =
        (*(mem)->procs.alloc_byte_array)(mem, ss->params.WidthIn * ss->params.spp_interp, ss->sizeofPixelIn, "image_scale src");
        ...
        if (ss->dst_offset == 0) {
            ...
            if (ss->params.Active)
                zoom_y(row,
                       ...
                       ss->tmp,
                       ss->params.LeftMarginOut,
                       ss->params.PatchWidthOut,
                       ss->params.WidthOut,
...
s_IScale_release(stream_state * st)
{
    ...
    ((mem)->procs.free_object(mem, (void *)ss->src, "image_scale src"));
    ss->src = 0;
    ((mem)->procs.free_object(mem, ss->dst, "image_scale dst"));
    ss->dst = 0;
    ((mem)->procs.free_object(mem, ss->items, "image_scale contrib[*]"));
    ss->items = 0;
    ((mem)->procs.free_object(mem, ss->items, "image_scale contrib_dst[*]"));
    ss->dst_items = 0;
    ((mem)->procs.free_object(mem, ss->contrib, "image_scale contrib"));
    ss->contrib = 0;
    ((mem)->procs.free_object(mem, ss->tmp, "image_scale tmp"));
    ss->tmp = 0;
}
