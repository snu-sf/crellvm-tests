...
# 87 "./base/gshtx.h"
extern int gs_ht_build(gs_halftone ** ppht, uint num_comps, gs_memory_t * pmem);
...
extern void gs_ht_reference(gs_halftone * pht);
extern void gs_ht_release(gs_halftone * pht);
# 144 "./base/gshtx.h"
extern int gs_ht_install(gs_state * pgs, gs_halftone * pht);
# 27 "./base/gshtx.c" 2
...
{
    gs_halftone *pht = (gs_halftone *) pvht;

    ((pmem)->procs.free_object(pmem, pht->params.multiple.components, cname));
    ((pmem)->procs.free_object(pmem, pvht, cname));
}
...
null_closure_transfer(
                         double val,
                         const gx_transfer_map * pmap_dummy,
                         const void *dummy
)
{
    return val;
...
)
{
    ...
    *ppht = 0;
    ...
    do { if ( ((pht) = (gs_halftone *)(*(pmem)->procs.alloc_struct)(pmem, &st_halftone, "gs_ht_build")) == 0 ) { return ((-25)); } else { do { (pht)->rc.ref_count = 1; (pht)->rc.memory = pmem; (pht)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
    ...
    phtc = (gs_halftone_component *)(*(pmem)->procs.alloc_struct_array)(pmem, num_comps, &st_ht_component_element, "gs_ht_build");
    ...
    if (phtc == 0) {
        ((pmem)->procs.free_object(pmem, pht, "gs_ht_build"));
        return ((-25));
    }
    ...
    pht->params.multiple.num_comp = num_comps;
...
{
    gs_halftone_component *phtc = &(pht->params.multiple.components[comp]);

    if (comp >= pht->params.multiple.num_comp)
        return ((-15));
    if (phtc->type != ht_type_none)
        return ((-7));

    phtc->type = ht_type_spot;
    phtc->params.spot.screen.frequency = freq;
    phtc->params.spot.screen.angle = angle;
    phtc->params.spot.screen.spot_function = spot_func;
    phtc->params.spot.accurate_screens = accurate;
    phtc->params.spot.transfer = gs_mapped_transfer;

    phtc->params.spot.transfer_closure.proc =
        (transfer == 0 ? null_closure_transfer : transfer);
    phtc->params.spot.transfer_closure.data = client_data;
...
)
{
    ...
    phtc->params.threshold.width = width;
    phtc->params.threshold.height = height;
    phtc->params.threshold.thresholds = *thresholds;
    phtc->params.threshold.transfer = gs_mapped_transfer;

    phtc->params.threshold.transfer_closure.proc =
        (transfer == 0 ? null_closure_transfer : transfer);
    phtc->params.threshold.transfer_closure.data = client_data;
...
{
    do { if (pht) do { (pht)->rc.ref_count++; do { } while (0); } while (0); } while (0);
}
...
{
    do { if (pht) { do { do { } while (0); (pht)->rc.ref_count += (-1); } while (0); if (!(pht)->rc.ref_count) { do { do { } while (0); (pht)->rc.free((pht)->rc.memory, (void *)(pht), "gs_ht_release"); } while (0); do { } while (0); } else do { } while (0); } } while (0);
}
...
{
    int i;
    int num_comps = pht->params.multiple.num_comp;

    if (pht->type != ht_type_multiple)
        return ((-28));
    for (i = 0; i < num_comps; i++) {
        gs_halftone_component *phtc = &(pht->params.multiple.components[i]);
...
)
{
    ...
    if (phtc->type == ht_type_spot) {
        proc = phtc->params.spot.transfer_closure.proc;
        client_info = phtc->params.spot.transfer_closure.data;
    } else {
        proc = phtc->params.threshold.transfer_closure.proc;
        client_info = phtc->params.threshold.transfer_closure.data;
    }

    for (i = 0; i < (1 << 8); i++) {
        float fval =
            proc(i * (1 / (double)((1 << 8) - 1)), pmap, client_info);

        values[i] =
            (fval <= 0.0 ? ((frac)0) : fval >= 1.0 ? ((frac)0x7ff8) :
...
alloc_ht_order(
                  const gs_halftone * pht,
                  gs_memory_t * pmem,
                  byte * comp2order
)
{
    int num_comps = pht->params.multiple.num_comp;
    gx_ht_order_component *pocs = (gx_ht_order_component *)(*(pmem)->procs.alloc_struct_array)(pmem, pht->params.multiple.num_comp, &st_ht_order_component_element, "alloc_ht_order");
    ...
    for (i = 0; i < num_comps; i++) {
        gs_halftone_component *phtc = &(pht->params.multiple.components[i]);
        gx_transfer_map *pmap = (gx_transfer_map *)(*(pmem)->procs.alloc_struct)(pmem, &st_transfer_map, "alloc_ht_order");
        ...
        if (pmap == 0) {
            ...
            for (j = 0; j < inext; j++)
                ((pmem)->procs.free_object(pmem, pocs[j].corder.transfer, "alloc_ht_order"));
            ((pmem)->procs.free_object(pmem, pocs, "alloc_ht_order"));
            return 0;
        }
...
)
{
    if (phtc->type == ht_type_spot) {
        ...
        code = gx_ht_process_screen_memory(&senum,
                                           pgs,
                                           &phtc->params.spot.screen,
                                      phtc->params.spot.accurate_screens,
                                           pmem
            );
        if (code < 0)
            return code;
        ...
        *porder = senum.order;
        ...
        porder->params.M = phtc->params.threshold.width;
        porder->params.N = 0;
        porder->params.R = 1;
        porder->params.M1 = phtc->params.threshold.height;
        porder->params.N1 = 0;
        porder->params.R1 = 1;
        code = gx_ht_alloc_threshold_order(porder,
                                           phtc->params.threshold.width,
                                           phtc->params.threshold.height,
                                           256,
                                           pmem
            );
        if (code < 0)
        ...
        gx_ht_construct_threshold_order(
                                porder,
                                phtc->params.threshold.thresholds.data
            );
...
)
{
    ...
    for (i = 0; i < num_comps; i++)
        gx_ht_order_release(&(pocs[i].corder), pmem, ((bool)1));
    ((pmem)->procs.free_object(pmem, pocs, "gs_ht_install"));
}
...
)
{
    ...
    for (i = 0; i < num_comps; i++) {
        ...
        code = build_component(&(pht->params.multiple.components[i]),
                               &(pocs[j].corder),
                               pgs,
                               pmem
            );
                  ...
                  gs_memory_t * mem)
{
    ...
    for (prev_mask = masks, num_bits = 0, i = 0;
        ...
        num_bits += create_mask_bits(prev_mask, prev_mask + bytes_per_mask,
                                     phcop->width, phcop->height, ((void*)0));
...
int
gs_ht_set_mask_comp(gs_halftone * pht,
                    int component_index,
...
{
    gs_halftone_component *phtc =
    &(pht->params.multiple.components[component_index]);

    if (component_index >= pht->params.multiple.num_comp)
        return ((-15));
    if (phtc->type != ht_type_none)
        return ((-7));
