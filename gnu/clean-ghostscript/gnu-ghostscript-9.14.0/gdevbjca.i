# 1 "./contrib/gdevbjca.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./contrib/gdevbjca.c" 2
# 44 "./contrib/gdevbjca.c"
# 1 "./base/gdevprn.h" 1
# 22 "./base/gdevprn.h"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
...
{ bool ret=((bool)0);
    ...
    for(; raster > 1; rowC++, rowM++, rowY++, rowK++, raster--) {
        ...
        if(*rowC) skip->skipC=((bool)1);
        if(*rowM) skip->skipM=((bool)1);
        if(*rowY) skip->skipY=((bool)1);
        if(*rowK) skip->skipK=((bool)1);
        if(*rowC|*rowM|*rowY|*rowK) ret = ((bool)1);
    }
    return ret;
}
...
FloydSteinbergInitG(gx_device_printer * pdev)
{
    ...
    dev->FloydSteinbergErrorsG = (int *) (*(pdev->memory)->procs.alloc_bytes)(pdev->memory, sizeof(int)*(pdev->width+3), "bjc error buffer");


    if (dev->FloydSteinbergErrorsG == 0)
        return -1;
    dev->FloydSteinbergDirectionForward=((bool)1);
    ...
    bjc_rgb_to_gray(dev->paperColor.red,
                    ...
                    dev->paperColor.blue,
                    &dev->FloydSteinbergG);
    dev->FloydSteinbergG = (255 - dev->FloydSteinbergG) << 4;
                         ...
                         uint raster, bool limit_extr)
{
    ...
    if (dev->FloydSteinbergDirectionForward) {
        ...
        for( i=width; i>0; i--, row++, err_vect++) {
            err_corr = dev->bjc_gamma_tableC[255-(*row)] + dev->FloydSteinbergG;
        ...
        dev->FloydSteinbergDirectionForward=((bool)0);
    }
    else {
        row += width - 1;
        dithered += raster - 1;
...
{
    gx_device_bjc_printer *dev = (gx_device_bjc_printer *)pdev;
    ((pdev->memory)->procs.free_object(pdev->memory, dev->FloydSteinbergErrorsG, "bjc error buffer"));
}
...
FloydSteinbergInitC(gx_device_printer * pdev)
{
    ...
    dev-> FloydSteinbergErrorsC = (int *) (*(pdev->memory)->procs.alloc_bytes)(pdev->memory, 3*sizeof(int)*(pdev->width+3), "bjc CMY error buffer");


    if (dev->FloydSteinbergErrorsC == 0 )
        return -1;
...
{ byte byteC=0, byteM=0, byteY=0, byteK=0, bitmask = 0x80;
    int i;
    ...
    if (dev->FloydSteinbergDirectionForward) {
        err_vect = dev->FloydSteinbergErrorsC + 3;
        ...
        for( i=width; i>0; i--, row+=4, err_vect+=3) {
            ...
            *(err_vect+4) = (errorM + 8) >> 4;
            delta = errorM << 1;
            errorM += delta;
            *(err_vect-2) += (errorM + 8) >> 4;
            errorM += delta;
            *(err_vect+1) += (errorM + 8) >> 4;
            errorM += delta + 8;
            errorM >>= 4;
        ...
        row += (width << 2) - 4;
        dithered += raster - 1;
        err_vect = dev->FloydSteinbergErrorsC + 3 * width + 3;
        bitmask = 1 << ((raster << 3 ) - width) ;
...
{
    gx_device_bjc_printer *dev = (gx_device_bjc_printer *)pdev;
    ((pdev->memory)->procs.free_object(pdev->memory, dev->FloydSteinbergErrorsC, "bjc CMY error buffer"));
