...
typedef struct {
  ...
  unsigned char * dc_stats[16];
  unsigned char * ac_stats[16];
...
process_restart (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (! cinfo->progressive_mode || (cinfo->Ss == 0 && cinfo->Ah == 0)) {
      memset((void *)(entropy->dc_stats[compptr->dc_tbl_no]), 0, (size_t)(64));
 ...
 (cinfo->progressive_mode && cinfo->Ss)) {
      memset((void *)(entropy->ac_stats[compptr->ac_tbl_no]), 0, (size_t)(256));
    }
  }
...
decode_mcu_DC_first (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 while (arith_decode(cinfo, st)) {
   if ((m <<= 1) == 0x8000) {
     ((cinfo)->err->msg_code = (JWRN_ARITH_BAD_CODE), (*(cinfo)->err->emit_message) ((j_common_ptr) (cinfo), -1));
     entropy->ct = -1;
     return TRUE;
   }
   st += 1;
...
start_pass (j_decompress_ptr cinfo)
{
 ...
 (cinfo->Se < 64 && cinfo->Se != cinfo->lim_Se))
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (! cinfo->progressive_mode || (cinfo->Ss == 0 && cinfo->Ah == 0)) {
      tbl = compptr->dc_tbl_no;
      if (tbl < 0 || tbl >= 16)
 ((cinfo)->err->msg_code = (JERR_NO_ARITH_TABLE), (cinfo)->err->msg_parm.i[0] = (tbl), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
      if (entropy->dc_stats[tbl] == ((void*)0))
 entropy->dc_stats[tbl] = (unsigned char *) (*cinfo->mem->alloc_small)
   ((j_common_ptr) cinfo, 1, 64);
      memset((void *)(entropy->dc_stats[tbl]), 0, (size_t)(64));
 ...
 (cinfo->progressive_mode && cinfo->Ss)) {
      ...
      if (tbl < 0 || tbl >= 16)
 ((cinfo)->err->msg_code = (JERR_NO_ARITH_TABLE), (cinfo)->err->msg_parm.i[0] = (tbl), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
      if (entropy->ac_stats[tbl] == ((void*)0))
 entropy->ac_stats[tbl] = (unsigned char *) (*cinfo->mem->alloc_small)
   ((j_common_ptr) cinfo, 1, 256);
      memset((void *)(entropy->ac_stats[tbl]), 0, (size_t)(256));
    }
  }
...
jinit_arith_decoder (j_decompress_ptr cinfo)
{
  ...
  entropy = (arith_entropy_ptr)
    ...
    ((size_t) sizeof(arith_entropy_decoder)));
  ...
  for (i = 0; i < 16; i++) {
    entropy->dc_stats[i] = ((void*)0);
    entropy->ac_stats[i] = ((void*)0);
  }
