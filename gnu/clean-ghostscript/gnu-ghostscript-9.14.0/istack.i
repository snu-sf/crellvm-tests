...
static const gs_memory_struct_type_t st_ref_stack_params = { sizeof(ref_stack_params_t), "ref_stack_params_t", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
static
void ref_stack_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
    ref_stack_t *const sptr = vptr;

    ((&sptr->current)->tas.type_attrs &= ~(1));
}
static
gs_ptr_type_t ref_stack_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const ref_stack_t *sptr = vptr; switch ( index ) { default: return 0;
case 0: return (pep->ptr = (const void *)(&sptr->current), (&ptr_ref_procs));
case 1: return (pep->ptr = (const void *)(sptr->params), (&ptr_struct_procs));
} }
static void ref_stack_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { ref_stack_t *sptr = vptr;
{
    ...
    (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(sptr->current)), (ref_packed *)(&(sptr->current) + 1), gcst);
    ((&sptr->current)->tas.type_attrs &= ~(1));
    reloc = bot - (ref_packed *) sptr->current.value.refs;


    sptr->p = (ref *)((ref_packed *)sptr->p - reloc);;
    sptr->bot = (ref *)((ref_packed *)sptr->bot - reloc);;
    sptr->top = (ref *)((ref_packed *)sptr->top - reloc);;

    (sptr->params = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(sptr->params), gcst)));
} }

const gs_memory_struct_type_t st_ref_stack = { sizeof(ref_stack_t), "ref_stack_t", 0, ref_stack_clear_marks, ref_stack_enum_ptrs, ref_stack_reloc_ptrs, 0, 0 };
...
{
    uint size = ((pblock_array)->tas.rsize);
    uint avail = size - ((sizeof(ref_stack_block) / sizeof(ref)) + bot_guard + top_guard);
    ref_stack_block *pblock = (ref_stack_block *)pblock_array->value.refs;
    s_ptr body = (s_ptr)(pblock + 1);

    if (params == 0) {
        params = (ref_stack_params_t *)(*((gs_memory_t *)mem)->procs.alloc_struct)((gs_memory_t *)mem, &st_ref_stack_params, "ref_stack_alloc(stack.params)");
    ...
    ((&pstack->max_stack)->value.intval = (avail), ((&pstack->max_stack)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    pstack->requested = 0;
    pstack->margin = 0;
    ...
    if (pguard_value != 0)
        ...
        ((&params->guard_value)->value.intval = (0), ((&params->guard_value)->tas.type_attrs = ((t__invalid) << 8) + (0)) );
    params->underflow_error = -1;
    params->overflow_error = -1;
    params->allow_expansion = ((bool)1);
    init_block(pstack, pblock_array, 0);
    refset_null_new(pstack->bot, avail, 0);
    ((&pblock->next)->value.refs = ((ref *)((void*)0)), ((&pblock->next)->tas.type_attrs = ((t_array) << 8) + (0)), ((&pblock->next)->tas.rsize = (0)));
    return 0;
}
...
{
    uint nmin = ((pstack)->p + 1 - (pstack)->bot + (pstack)->extension_used);

    if (nmax < nmin)
        nmax = nmin;
    if (nmax > ((unsigned int)~0 + (unsigned int)0) / sizeof(ref))
        nmax = ((unsigned int)~0 + (unsigned int)0) / sizeof(ref);
...
{
    return ((pstack)->p + 1 - (pstack)->bot + (pstack)->extension_used);
}
...
ref_stack_counttomark(const ref_stack_t *pstack)
{
    ...
    do {
        ...
        for (; count; count--, p--)
            if (((((const byte *)&((p)->tas.type_attrs))[1]) == (t_mark)))
...
ref_stack_store_check(const ref_stack_t *pstack, ref *parray, uint count,
                      uint skip)
{
    uint space = (avm_space)(((parray)->tas.type_attrs) & (((1 << 2) - 1) << 2));
                ...
                client_name_t cname)
{
    ...
    if (count > ref_stack_count(pstack) || count > ((parray)->tas.rsize))
        return ((-15));
    if (check) {
    ...
    do {
        ...
        if (size <= pass)
            ...
            case 0:
                while (size--) {
                    from--, to--;
                    (((void)((((((to)->tas.type_attrs) & (idmemory)->test_mask) == 0) ? alloc_save_change(idmemory, parray, (ref_packed *)(to), cname) : 0))), ((void)(((*(to) = *(from)), ((to)->tas.type_attrs |= (idmemory)->new_mask)))));
                }
                break;
            case 1:
                while (size--) {
                    from--, to--;
                    ((void)(((*(to) = *(from)), ((to)->tas.type_attrs |= (idmemory)->new_mask))));
                }
                break;
            }
            if (left == 0)
...
ref_stack_pop_block(ref_stack_t *pstack)
{
    ...
    if (pnext == 0)
        return (pstack->params->underflow_error);
    used = ((&pnext->used)->tas.rsize);
...
ref_stack_enum_next(ref_stack_enum_t *prse)
{
    ...
    prse->size = ((&block->used)->tas.rsize);
    return ((bool)1);
}
...
ref_stack_cleanup(ref_stack_t *pstack)
{
    ...
    ((&pblock->used)->tas.rsize = (pstack->p + 1 - pstack->bot));
}
...
ref_stack_release(ref_stack_t *pstack)
{
    ...
    ref_stack_pop(pstack, ref_stack_count(pstack));

    (((gs_memory_t *)mem)->procs.free_object((gs_memory_t *)mem, pstack->params, "ref_stack_release(stack.params)"));
...
ref_stack_free(ref_stack_t *pstack)
{
    ...
    ((mem)->procs.free_object(mem, pstack, "ref_stack_free"));
}
...
init_block(ref_stack_t *pstack, const ref *psb, uint used)
{
    ...
    for (i = params->bot_guard, p = brefs + (sizeof(ref_stack_block) / sizeof(ref));
         i != 0; i--, p++
        )
        (*(p) = *(&params->guard_value));
    ...
    if (params->top_guard) {
        ...
        pblock->used = *psb;
        pblock->used.value.refs = brefs + (sizeof(ref_stack_block) / sizeof(ref)) + params->bot_guard;
        ((&pblock->used)->tas.rsize = (0));
    }
}
