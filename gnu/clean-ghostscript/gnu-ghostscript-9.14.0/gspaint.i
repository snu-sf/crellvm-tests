...
gs_fillpage(gs_state * pgs)
{
    ...
    if (((dev)->procs.get_color_mapping_procs) == ((void*)0) ||
        ((dev)->procs.get_color_mapping_procs) == gx_error_get_color_mapping_procs) {
        (emprintf_program_ident(dev->memory, gs_program_name(), gs_revision_number()), errprintf(dev->memory, "\n   *** Error: No get_color_mapping_procs for device: %s\n", dev->dname));


        return ((-100));
    }

    ((pgs->device)->procs.set_graphics_type_tag)(pgs->device, GS_PATH_TAG);

    code = ((((pgs)->color[0].dev_color))->type != (&gx_dc_type_data_none)) ? 0 : gx_remap_color(pgs);;
    if (code != 0)
        return code;

    code = (*((dev)->procs.fillpage))(dev, (gs_imager_state *)pgs,
                  ...
                  bool devn)
{
    ...
    ibox.p.x = ((int)((bbox.p.x - extra_x) >> (8))) - 1;
    ibox.p.y = ((int)((bbox.p.y - extra_y) >> (8))) - 1;
    ibox.q.x = ((int)(((bbox.q.x + extra_x)+((1<<8)-1)) >> (8))) + 1;
    ibox.q.y = ((int)(((bbox.q.y + extra_y)+((1<<8)-1)) >> (8))) + 1;
    width = (ibox.q.x - ibox.p.x) << log2_scale.x;
    raster = ((uint)((((width) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    band_space = raster << log2_scale.y;
    height = (2000 / band_space) << log2_scale.y;
    if (height == 0)
        height = 1 << log2_scale.y;
    mem = pgs->memory;
    mdev = (gx_device_memory *)(*(mem)->procs.alloc_struct)(mem, &st_device_memory, "alpha_buffer_init");

    if (mdev == 0)
        return 0;


    if (((dev)->procs.dev_spec_op)(dev, gxdso_is_pdf14_device, ((void*)0), 0) > 0) {
    ...
    if ((*((mdev)->procs.open_device)) ((gx_device *) mdev) < 0) {

        ((mem)->procs.free_object(mem, mdev, "alpha_buffer_init"));
        return 0;
    }
    gx_set_device_only(pgs, (gx_device *) mdev);
    scale_paths(pgs, log2_scale.x, log2_scale.y, ((bool)1));
    return 1;
}
...
{
    gx_device_memory *mdev =
        (gx_device_memory *) ((pgs)->device);
    int code = (*((mdev)->procs.close_device)) ((gx_device *) mdev);

    if (code >= 0)
        scale_paths(pgs, -mdev->log2_scale.x, -mdev->log2_scale.y,
                !(newpath && !((pgs->path)->segments != 0 && (pgs->path)->segments->rc.ref_count > 1)));
...
static int do_fill(gs_state *pgs, int rule)
{
    ...
    bool devn;
# 275 "./base/gspaint.c"
    if (( (gs_currentdevice(pgs))->color_info.num_components > 1 || (gs_currentdevice(pgs))->color_info.gray_index == 0xff )) {
        ((pgs->device)->procs.set_graphics_type_tag)(pgs->device, GS_PATH_TAG);
    }
    else {
    ...
    abits = 0;
    {
        gx_device_color *col = ((pgs)->color[0].dev_color);
        devn = ((col)->type == (&gx_dc_type_data_devn));
        if (((col)->type == (&gx_dc_type_data_pure)) || devn)
    ...
    if (abits > 1) {
        ...
        acode = 0;
    code = gx_fill_path(pgs->path, ((pgs)->color[0].dev_color), pgs, rule,
...
# 388 "./base/gspaint.c"
    ...
    if (abits > 1) {
        ...
        fixed extra_adjust =
                ((fixed)(((((xxyy) > (xyyx)) ? (xxyy) : (xyyx)) * new_width / 2)*(float)(1<<8)));
        float orig_flatness = gs_currentflat(pgs);
        gx_path spath;


        if (extra_adjust < (1L << 8))
            extra_adjust = (1L << 8);
        ...
        (void)gx_path_init_local_shared(&spath, ((void*)0), pgs->memory);
        code = gx_stroke_add(pgs->path, &spath, pgs, ((bool)0));
        gs_setlinewidth(pgs, orig_width);
        scale_dash_pattern(pgs, 1.0 / scale);
        if (code >= 0)
            code = gx_fill_path(&spath, ((pgs)->color[0].dev_color), pgs,
                                (-1),
                                pgs->fill_adjust.x,
                                pgs->fill_adjust.y);
...
gs_strokepath_aux(gs_state * pgs, bool traditional)
{
    ...
    (void)gx_path_init_local_shared(&spath, ((void*)0), pgs->path->memory);
    ...
    if (((pgs->path)->segments != 0 && (pgs->path)->segments->contents.subpath_first == 0))
        pgs->current_point_valid = ((bool)0);
    else
        (pgs)->current_point.x = ((spath.position.x)*(1.0/(1<<8))); (pgs)->current_point.y = ((spath.position.y)*(1.0/(1<<8)));;
    return 0;
...
{
    return gs_strokepath_aux(pgs, ((bool)1));
}

int
gs_strokepath2(gs_state * pgs)
{
    return gs_strokepath_aux(pgs, ((bool)0));
}
