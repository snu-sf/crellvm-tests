...
static
void sproc_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
    stream_proc_state *const pptr = vptr;

    ((&pptr->proc)->tas.type_attrs &= ~(1));
    ((&pptr->data)->tas.type_attrs &= ~(1));
}
static
gs_ptr_type_t sproc_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const stream_proc_state *pptr = vptr; switch ( index ) { default: return 0;
case 0:
return (pep->ptr = (const void *)(&pptr->proc), (&ptr_ref_procs));
case 1:
return (pep->ptr = (const void *)(&pptr->data), (&ptr_ref_procs));
} }
static void sproc_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { stream_proc_state *pptr = vptr;;
(*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(pptr->proc)), (ref_packed *)(&(pptr->proc) + 1), gcst);
((&pptr->proc)->tas.type_attrs &= ~(1));
...
static const gs_memory_struct_type_t st_sproc_state = { sizeof(stream_proc_state), "procedure stream state", 0, sproc_clear_marks, sproc_enum_ptrs, sproc_reloc_ptrs, 0, 0 };
            ...
            gs_ref_memory_t *imem)
{
    ...
    if (sstrm == 0 || state == 0) {
        ((mem)->procs.free_object(mem, state, "s_proc_init(state)"));

        return ((-25));
    }
    s_std_init(sstrm, ((void*)0), 0, procs, mode);
    sstrm->procs.process = temp->process;
    state->templat = temp;
    state->memory = mem;
    state->eof = 0;
    state->proc = *sop;
    ((&state->data)->value.bytes = ((byte *)((void*)0)), ((&state->data)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40))), ((&state->data)->tas.rsize = (0)));
    state->index = 0;
              ...
              op_proc_t cont)
{
    ...
    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (npush) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), npush); if ( es_code_ < 0 ) return es_code_; };
    if (nstate)
        memcpy((((i_ctx_p->exec_stack).stack).p) + 2, pstate, nstate * sizeof(ref));
# 110 "./psi/zfproc.c"
    npush--;

    (((((i_ctx_p->exec_stack).stack).p) + 1)->value.opproc = (cont), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.rsize = (0)));
    (((i_ctx_p->exec_stack).stack).p) += npush;
    return 5;
}
...
s_proc_set_defaults(stream_state * st)
{
    ...
    (((&ss->proc)->tas.type_attrs = ((t_null) << 8) + (0)) );
    (((&ss->data)->tas.type_attrs = ((t_null) << 8) + (0)) );
}




static int s_proc_read_process(stream_state *, stream_cursor_read *, stream_cursor_write *, bool);
static int s_proc_read_continue(i_ctx_t *);


static const stream_template s_proc_read_template = {
    &st_sproc_state, ((void*)0), s_proc_read_process, 1, 1,
    ((void*)0), s_proc_set_defaults
};
static const stream_procs s_proc_read_procs = {
    s_std_noavailable, s_std_noseek, s_std_read_reset,
    s_std_read_flush, s_std_null, ((void*)0)
};
...
{
    int code =
        s_proc_init(sop, psstrm, 1, &s_proc_read_template,
                    &s_proc_read_procs, imem);

    if (code < 0)
        return code;
    (*psstrm)->end_status = ((int)(-4));
    return code;
}
                    ...
                    stream_cursor_write * pw, bool last)
{
    ...
    stream_proc_state *const ss = (stream_proc_state *) st;
    uint count = ((&ss->data)->tas.rsize) - ss->index;
                        ...
                        const ref * pstate, int nstate, op_proc_t cont)
{
    ...
    for (ps = (fop)->value.pfile; ps->strm != 0;)
        ps = ps->strm;
    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (npush) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), npush); if ( es_code_ < 0 ) return es_code_; };
    if (nstate)
        memcpy((((i_ctx_p->exec_stack).stack).p) + 2, pstate, nstate * sizeof(ref));
    (((((i_ctx_p->exec_stack).stack).p) + 1)->value.opproc = (cont), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.rsize = (0)));
    (((i_ctx_p->exec_stack).stack).p) += npush;
    (((((i_ctx_p->exec_stack).stack).p) - 2)->value.opproc = (s_proc_read_continue), (((((i_ctx_p->exec_stack).stack).p) - 2)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) - 2)->tas.rsize = (0)));
    (((i_ctx_p->exec_stack).stack).p)[-1] = *fop;
    (((((i_ctx_p->exec_stack).stack).p) - 1)->tas.type_attrs &= ~(0x80));
    *(((i_ctx_p->exec_stack).stack).p) = ((stream_proc_state *) ps->state)->proc;
    return 5;
}
...
s_proc_read_continue(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&*(op))->tas.type_attrs))[1]) == (t_file)) ) return (check_type_failed(&*(op))); if ( (ps = (op)->value.pfile, (ps->read_id | ps->write_id) != ((op)->tas.rsize)) ) return ((-7)); } while (0);
    if ( !(((&*opbuf)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*opbuf)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*opbuf) : (-7)));
    while ((ps->end_status = 0, ps->strm) != 0)
        ps = ps->strm;
    ss = (stream_proc_state *) ps->state;
    ss->data = *opbuf;
    ss->index = 0;
    if (((opbuf)->tas.rsize) == 0)
        ss->eof = ((bool)1);
...
static int s_proc_write_flush(stream *);
static int s_proc_write_process(stream_state *, stream_cursor_read *, stream_cursor_write *, bool);
static int s_proc_write_continue(i_ctx_t *);


static const stream_template s_proc_write_template = {
    &st_sproc_state, ((void*)0), s_proc_write_process, 1, 1,
    ((void*)0), s_proc_set_defaults
};
static const stream_procs s_proc_write_procs = {
    s_std_noavailable, s_std_noseek, s_std_write_reset,
    s_proc_write_flush, s_std_null, ((void*)0)
};
...
{
    return s_proc_init(sop, psstrm, 2, &s_proc_write_template,
                     ...
                     stream_cursor_write * ignore_pw, bool last)
{
    ...
    if (rcount > 0 || (last && !ss->eof)) {
        uint wcount = ((&ss->data)->tas.rsize) - ss->index;
        uint count = (((rcount) < (wcount)) ? (rcount) : (wcount));
...
{
    int result = s_process_write_buf(s, ((bool)0));
    stream_proc_state *const ss = (stream_proc_state *)s->state;

    return (result < 0 || ss->index == 0 ? result : ((int)(-4)));
}
                         ...
                         const ref * pstate, int nstate, op_proc_t cont)
{
    ...
    psst = (stream_proc_state *) ps->state;
    {
        ...
        (((((i_ctx_p->exec_stack).stack).p) + 1)->value.opproc = (cont), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) + 1)->tas.rsize = (0)));
        (((i_ctx_p->exec_stack).stack).p) += npush;
        (((((i_ctx_p->exec_stack).stack).p) - 4)->value.opproc = (s_proc_write_continue), (((((i_ctx_p->exec_stack).stack).p) - 4)->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) - 4)->tas.rsize = (0)));
        (((i_ctx_p->exec_stack).stack).p)[-3] = *fop;
        (((((i_ctx_p->exec_stack).stack).p) - 3)->tas.type_attrs &= ~(0x80));
        (((((i_ctx_p->exec_stack).stack).p) - 1)->value.boolval = (!psst->eof), (((((i_ctx_p->exec_stack).stack).p) - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    }
    (((i_ctx_p->exec_stack).stack).p)[-2] = psst->proc;
    *(((i_ctx_p->exec_stack).stack).p) = psst->data;
    (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (psst->index));
    return 5;
}
...
s_proc_write_continue(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&*(op))->tas.type_attrs))[1]) == (t_file)) ) return (check_type_failed(&*(op))); if ( (ps = (op)->value.pfile, (ps->read_id | ps->write_id) != ((op)->tas.rsize)) ) return ((-7)); } while (0);
    if ( !(((&*opbuf)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))) ) return ((!((((const byte *)&((&*opbuf)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*opbuf) : (-7)));
    while (ps->strm != 0) {
        if (ps->end_status == ((int)(-4)))
            ps->end_status = 0;
        ps = ps->strm;
    }
    ps->end_status = 0;
