...
sread_file(register stream * s, FILE * file, byte * buf, uint len)
{
    ...
    bool seekable = (curpos != -1L && gp_fseek_64(file, curpos, 0) == 0);
    ...
    s->file_offset = 0;
    s->file_limit = sizeof(gs_offset_t) > 4 ? (~(-((int64_t)1) << (sizeof(int64_t) * 8 - 1))) : (~(-1L << ((1 << 3) * 8 - 1)));
}
...
{
    if (s->file == 0 || s->modes != 1 + 4 ||
        s->file_offset != 0 || s->file_limit != (~(-1L << ((1 << 3) * 8 - 1))) ||
        ((s->position < start || s->position > start + length) &&
         spseek(s, (gs_offset_t)(start)) < 0)
        )
        return ((int)(-2));
    s->position -= start;
    s->file_offset = start;
...
s_file_available(register stream * s, gs_offset_t *pl)
{
    ...
    gs_offset_t buf_avail = ((s)->cursor.r.limit - (s)->cursor.r.ptr);
    ...
    *pl = (((max_avail) < (buf_avail)) ? (max_avail) : (buf_avail));
    if ((((s)->modes & 4) != 0)) {
        ...
        if (gp_fseek_64(s->file, 0, 2))
            return ((int)(-2));
        end = gp_ftell_64(s->file);
        if (gp_fseek_64(s->file, pos, 0))
            return ((int)(-2));
        buf_avail += end - pos;
        *pl = (((max_avail) < (buf_avail)) ? (max_avail) : (buf_avail));
        if (*pl == 0)
...
s_file_read_seek(register stream * s, gs_offset_t pos)
{
    ...
    if (pos < 0 || pos > s->file_limit ||
        gp_fseek_64(s->file, s->file_offset + pos, 0) != 0
        )
        return ((int)(-2));
                    ...
                    stream_cursor_write * pw, bool last)
{
    ...
    pw->ptr += count;
    do { } while (0);
    return (ferror(file) ? ((int)(-2)) : feof(file) ? ((int)(-1)) : status);
}
...
s_file_switch(stream * s, bool writing)
{
    ...
    if (writing) {
        ...
        if ((*(s)->procs.flush)(s) < 0)
            return ((int)(-2));
        gp_fseek_64(file, 0L, 1);
