...
static __inline__ gs_context_t *
...
static
void context_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
...
static
gs_ptr_type_t context_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_context_t *pctx = vptr; switch ( index ) { default:
return (*(st_context_state).enum_ptrs)(mem, vptr, size, index-(2), pep, &(st_context_state), gcst);
case 0: return (pep->ptr = (const void *)(pctx->scheduler), (&ptr_struct_procs));
case 1: {
    ...
    return (pep->ptr = (const void *)(next), (&ptr_struct_procs));
}
} }
static void context_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_context_t *pctx = vptr;
    (*(st_context_state).reloc_ptrs)(vptr, size, &(st_context_state), gcst);
    (pctx->scheduler = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(pctx->scheduler), gcst)));

}
static const gs_memory_struct_type_t st_context = { sizeof(gs_context_t), "gs_context_t", 0, context_clear_marks, context_enum_ptrs, context_reloc_ptrs, 0, 0 };
...
static const gs_memory_struct_type_t st_condition = { sizeof(gs_condition_t), "conditiontype", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
static const gc_ptr_element_t lock_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_lock_t *)0)->scheduler - (char *)((gs_lock_t *)0) )) } }; static const gc_struct_data_t lock_reloc_ptrs = { (sizeof(lock_enum_ptrs) / sizeof((lock_enum_ptrs)[0])), 0, 0, lock_enum_ptrs }; static const gs_memory_struct_type_t st_lock = { sizeof(gs_lock_t), "locktype", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &lock_reloc_ptrs };
...
struct gs_scheduler_s {
    ...
    ctx_list_t active;
    void (*save_vm_reclaim)(vm_spaces *pspaces, bool global);
    ctx_index_t dead_index;

    gs_context_t *table[19];
};
...
index_context(const gs_scheduler_t *psched, long index)
{
    ...
    if (index == 0)
        return ((void*)0);
    pctx = psched->table[index % 19];
    while (pctx != ((void*)0) && pctx->index != index)
        pctx = pctx->table_next;
    return pctx;
}


static gs_ptr_type_t scheduler_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void scheduler_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_scheduler = { sizeof(gs_scheduler_t), "gs_scheduler", 0, 0, scheduler_enum_ptrs, scheduler_reloc_ptrs, 0, 0 };
...
static gs_ptr_type_t scheduler_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_scheduler_t *psched = vptr; switch ( index ) { default:
{
    index -= 1;
    if (index < 19) {
        ...
        while (pctx && !pctx->visible)
            pctx = pctx->table_next;
        return (pep->ptr = (const void *)(pctx), (&ptr_struct_procs));
    }
    return 0;
}
case 0: return (pep->ptr = (const void *)(visible_context(psched->current)), (&ptr_struct_procs));
} }
static void scheduler_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_scheduler_t *psched = vptr;
{
    if (psched->current->visible)
        (psched->current = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(psched->current), gcst)));
    {
        int i;
        ...
        for (i = 0; i < 19; ++i) {
            ...
            for (; *ppctx; ppctx = pnext) {
                ...
                if ((*ppctx)->visible)
                    (*ppctx = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(*ppctx), gcst)));
            }
        }
    }
}
...
context_reclaim(vm_spaces * pspaces, bool global)
{
    ...
    for (i = (sizeof(pspaces->memories.indexed) / sizeof((pspaces->memories.indexed)[0])) - 1; psched == 0 && i > 0; --i) {
        gs_ref_memory_t *mem = pspaces->memories.indexed[i];
        const gs_gc_root_t *root = mem->roots;

        for (; root; root = root->next) {
            if ((*((gs_memory_t *)mem)->procs.object_type)((gs_memory_t *)mem, *root->p) == &st_context) {
                pctx = *root->p;
    ...
    loc.memory = (gs_ref_memory_t *)(((gs_memory_t *)lmem)->procs.stable((gs_memory_t *)lmem));
    loc.cp = 0;
    for (i = 0; i < 19; ++i)
...
ctx_initialize(i_ctx_t **pi_ctx_p)
{
    ...
    gs_ref_memory_t *imem = ((i_ctx_p->memory).spaces.memories.named.system);
    gs_scheduler_t *psched =
        (gs_scheduler_t *)(*((gs_memory_t *) imem)->procs.alloc_struct_immovable)((gs_memory_t *) imem, &st_scheduler, "gs_scheduler");
    ...
    if (context_create(psched, &psched->current, &(i_ctx_p->memory), *pi_ctx_p, ((bool)1)) < 0) {
        (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./psi/zcontext.c", 379), errprintf_nomem("Can't create initial context!"));
        gs_abort(((gs_memory_t *)((i_ctx_p->memory).current)));
    }
...
# 413 "./psi/zcontext.c"
    ...
    if (current != 0)
        current->saved_local_vm =
            current->state.memory.spaces.memories.named.local->saved != 0;

    {
        ...
        for (ready = index_context(psched, psched->active.head_index);;
            ...
            if (ready == 0) {
                ...
                (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./psi/zcontext.c", 440), errprintf_nomem("No context to run!"));
                return ((-100));
            }

            if (ready->state.memory.spaces.memories.named.local->saved != 0 &&
...
zcurrentcontext(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.intval = (current->index), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    uint mcount = ref_stack_counttomark(&((i_ctx_p->op_stack).stack));
    ref rnull;

    if (mcount == 0)
        return ((-24));
    (((&rnull)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return do_fork(i_ctx_p, op, &rnull, &rnull, mcount, ((bool)0));
}
static int
zlocalfork(i_ctx_t *i_ctx_p)
{
    ...
    if (mcount == 0)
        return ((-24));
    code = values_older_than(&((i_ctx_p->op_stack).stack), 1, mcount - 1, avm_local);
    if (code < 0)
        return code;
    code = do_fork(i_ctx_p, op - 2, op - 1, op, mcount - 2, ((bool)1));
    if (code < 0)
        return code;
        ...
        uint mcount, bool local)
{
    ...
    if (((i_ctx_p->memory).spaces.memories.named.local)->save_level)
        return ((-29));
    if (((((const byte *)&((pstdout)->tas.type_attrs))[1]) == (t_null))) {
        code = zget_stdout(i_ctx_p, &s);
        if (code < 0)
            return code;
        pstdout = &(i_ctx_p->stdio)[1];
    } else
        do { if ( !(((&*(pstdout))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(pstdout))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(pstdout)) : (-7))); do { s = (pstdout)->value.pfile; if (s->read_id != ((pstdout)->tas.rsize)) { if (s->read_id == 0 && s->write_id == ((pstdout)->tas.rsize)) { int fcode = file_switch_to_read(pstdout); if (fcode < 0) return(fcode); } else { s = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
    if (((((const byte *)&((pstdin)->tas.type_attrs))[1]) == (t_null))) {
        code = zget_stdin(i_ctx_p, &s);
        if (code < 0)
            return code;
        pstdin = &(i_ctx_p->stdio)[0];
    } else
        do { if ( !(((&*(pstdin))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(pstdin))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(pstdin)) : (-7))); do { s = (pstdin)->value.pfile; if (s->read_id != ((pstdin)->tas.rsize)) { if (s->read_id == 0 && s->write_id == ((pstdin)->tas.rsize)) { int fcode = file_switch_to_read(pstdin); if (fcode < 0) return(fcode); } else { s = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
    dmem = (i_ctx_p->memory);
    if (local) {

        ref *puserdict;
        uint userdict_size;
        gs_memory_t *parent = ((i_ctx_p->memory).spaces.memories.named.local)->non_gc_memory;
        gs_ref_memory_t *lmem;
        gs_ref_memory_t *lmem_stable;

        if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "userdict", &puserdict) <= 0 ||
            !((((const byte *)&((puserdict)->tas.type_attrs))[1]) == (t_dictionary))
            )
            return ((-100));
        old_userdict = *puserdict;
        userdict_size = dict_maxlength(&old_userdict);
        lmem = ialloc_alloc_state(parent, ((i_ctx_p->memory).spaces.memories.named.local)->chunk_size);
        lmem_stable = ialloc_alloc_state(parent, ((i_ctx_p->memory).spaces.memories.named.local)->chunk_size);
        if (lmem == 0 || lmem_stable == 0) {
            ((parent)->procs.free_object(parent, lmem_stable, "do_fork"));
            ((parent)->procs.free_object(parent, lmem, "do_fork"));
            return ((-25));
        }
        lmem->space = avm_local;
        lmem_stable->space = avm_local;
        lmem->stable_memory = (gs_memory_t *)lmem_stable;
        dmem.spaces.memories.named.local = lmem;
        code = context_create(psched, &pctx, &dmem, &pcur->state, ((bool)0));
        if (code < 0) {
...

        {
            ...
            for (n = 0, old = (i_ctx_p->pgs); old != 0; old = gs_state_saved(old))
                ++n;
            ...
            for (old = (i_ctx_p->pgs), new = pctx->state.pgs;
                 old != 0 && code >= 0;
    ...
    pctx->state.dict_stack.min_size = (i_ctx_p->dict_stack).min_size;
    pctx->state.dict_stack.userdict_index = (i_ctx_p->dict_stack).userdict_index;
...

    {
        ...
        uint count = ref_stack_count(&((i_ctx_p->dict_stack).stack));
        uint copy = (local ? ((i_ctx_p->dict_stack).min_size) : count);

        ref_stack_push(dstack, copy);
        stack_copy(dstack, &((i_ctx_p->dict_stack).stack), copy, count - copy);
        if (local) {
            ...
            for (i = 0; i < copy; ++i) {
                ...
                if (obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), pdref, &old_userdict))
                    *pdref = new_userdict;
            }
        }
    }
    {
        ...
        ((estack->p - 2)->value.opproc = (fork_done_with_error), ((estack->p - 2)->tas.type_attrs = ((t_null) << 8) + (0x80)), ((estack->p - 2)->tas.rsize = (0)));
        ((estack->p - 1)->value.opproc = (fork_done), ((estack->p - 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((estack->p - 1)->tas.rsize = (0)));
        *estack->p = *op;
    }
    {
        ...
        stack_copy(ostack, &((i_ctx_p->op_stack).stack), count, (((i_ctx_p->op_stack).stack).p) - op + 1);
    }
    pctx->state.binary_object_format = pcur->state.binary_object_format;
    add_last(psched, &psched->active, pctx);
    ((((i_ctx_p->op_stack).stack).p) -= (mcount - 1));
    op = (((i_ctx_p->op_stack).stack).p);
    ((op)->value.intval = (pctx->index), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
                  ...
                  int next_space)
{
    ...
    for (i = first; i <= last; ++i)
        if ((avm_space)(((ref_stack_index(pstack, (long)i))->tas.type_attrs) & (((1 << 2) - 1) << 2)) >= next_space)
            return ((-7));
    return 0;
}
...
fork_done(i_ctx_t *i_ctx_p)
{
    ...
    pop_estack(&pcur->state, ref_stack_count(&((i_ctx_p->exec_stack).stack)) - 1);
    gs_grestoreall((i_ctx_p->pgs));
    ...
    if (((i_ctx_p->memory).spaces.memories.named.local)->save_level) {
        ...
        if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "restore", &prestore) <= 0) {
            (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./psi/zcontext.c", 796), errprintf_nomem("restore not found in systemdict!"));
            return ((-100));
        }
        if (pcur->detach) {
        ...
        do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
        ((op)->value.saveid = (alloc_save_current_id(&(i_ctx_p->memory))), ((op)->tas.type_attrs = ((t_save) << 8) + (0)) );
        (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (fork_done), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
        ++(((i_ctx_p->exec_stack).stack).p);
        (*((((i_ctx_p->exec_stack).stack).p)) = *(prestore));
        return 5;
    }
    if (pcur->detach) {
...
zjoin(i_ctx_t *i_ctx_p)
{
    ...
    if (pctx->joiner_index != 0 || pctx->detach || pctx == current ||
        pctx->state.memory.spaces.memories.named.global !=
          current->state.memory.spaces.memories.named.global ||
        pctx->state.memory.spaces.memories.named.local !=
          current->state.memory.spaces.memories.named.local ||
        ((i_ctx_p->memory).spaces.memories.named.local)->save_level != 0
        )
        return ((-29));
    switch (pctx->status) {
        case cs_active:
            ...
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (finish_join), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (reschedule_now), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
        ...
        case cs_done:
            {
                ...
                do { if ( (op += (count)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (count); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
                {
                    ref *rp = ref_stack_index(&((i_ctx_p->op_stack).stack), count);

                    (((rp)->tas.type_attrs = ((t_mark) << 8) + (0)) );
                }
                stack_copy(&((i_ctx_p->op_stack).stack), ostack, count, 0);
                context_destroy(pctx);
            }
    }
    return 0;
...
zcondition(i_ctx_t *i_ctx_p)
{
    ...
    gs_condition_t *pcond =
        (gs_condition_t *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_condition, "zcondition");

    if (pcond == 0)
        return ((-25));
    pcond->waiting.head_index = pcond->waiting.tail_index = 0;
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.pstruct = ((obj_header_t *)(pcond)), ((op)->tas.type_attrs = ((t_struct) << 8) + (((0x10 + 0x20 +0x40)) | (((&(i_ctx_p->memory)))->current_space))) );
    return 0;
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    gs_lock_t *plock = (gs_lock_t *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_lock, "zlock");
    ...
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.pstruct = ((obj_header_t *)(plock)), ((op)->tas.type_attrs = ((t_struct) << 8) + (((0x10 + 0x20 +0x40)) | (((&(i_ctx_p->memory)))->current_space))) );
    return 0;
}
...
zmonitor(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((&op[-1])->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&op[-1])->value.pstruct) == &st_lock) ) return (check_type_failed(&op[-1]));
    do { if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*op)); } while (0);
    plock = ((gs_lock_t *)((op - 1)->value.pstruct));
...
# 1015 "./psi/zcontext.c"
    ...
    if (code != 0) {
        (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (zmonitor), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
        return code;
    }
    *++(((i_ctx_p->exec_stack).stack).p) = op[-1];
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (monitor_cleanup), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (monitor_release), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
...
{
    int code = lock_release((((i_ctx_p->exec_stack).stack).p));
...
{
    int code = lock_release((((i_ctx_p->exec_stack).stack).p) - 1);
...
znotify(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((&*op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) && (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), (&*op)->value.pstruct) == &st_condition) ) return (check_type_failed(&*op));
    pcond = ((gs_condition_t *)((op)->value.pstruct));
    do { } while (0);
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    op--;
...
zwait(i_ctx_t *i_ctx_p)
{
    ...
    if (pctx == 0 || pctx != psched->current ||
        (((i_ctx_p->memory).spaces.memories.named.local)->save_level != 0 &&
         ((avm_space)(((op - 1)->tas.type_attrs) & (((1 << 2) - 1) << 2)) == avm_local || (avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) == avm_local))
        )
        return ((-29));
    ...
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (await_lock), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    return 22;
}
...
zusertime_context(i_ctx_t *i_ctx_p)
{
    ...
    if (!current->state.keep_usertime) {
        ...
        current->state.keep_usertime = ((bool)1);
    }
    ((op)->value.intval = (current->state.usertime_total + utime - psched->usertime_initial), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
...
{
# 1171 "./psi/zcontext.c"
    gs_memory_t *mem = (((gs_memory_t *)dmem->spaces.memories.named.local)->procs.stable((gs_memory_t *)dmem->spaces.memories.named.local));
    gs_context_t *pctx;
    int code;
    long ctx_index;
    gs_context_t **pte;
    ...
    pctx = (gs_context_t *)(*(mem)->procs.alloc_struct)(mem, &st_context, "context_create");
    if (pctx == 0)
        return ((-25));
    if (copy_state) {
        pctx->state = *i_ctx_p;
        ...
        code = context_state_alloc(&pctx_st, (&(i_ctx_p->dict_stack).system_dict), dmem);
        if (code < 0) {
            ((mem)->procs.free_object(mem, pctx, "context_create"));
            return code;
        }
    }

    if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "TIME_SLICE_INTERVAL", &slice_ref) > 0) {
    ...
    pctx->detach = ((bool)0);
    pctx->saved_local_vm = ((bool)0);
    pctx->visible = ((bool)1);
    pctx->next_index = 0;
    pctx->joiner_index = 0;
    pte = &psched->table[ctx_index % 19];
    pctx->table_next = *pte;
    *pte = pctx;
    *ppctx = pctx;
    if (gs_debug_c('\'') | gs_debug_c('"'))
        (dmprintf_file_and_line(((gs_memory_t *)((i_ctx_p->memory).current)),"./psi/zcontext.c", 1211), errprintf(((gs_memory_t *)((i_ctx_p->memory).current)),"[']create %ld at 0x%lx\n", ctx_index, (ulong) pctx));
    return 0;
}
...
{
    gs_ref_memory_t *mem = pctx->state.memory.spaces.memories.named.local;
    gs_scheduler_t *psched = pctx->scheduler;
    gs_context_t **ppctx = &psched->table[pctx->index % 19];

    while (*ppctx != pctx)
        ppctx = &(*ppctx)->table_next;
    *ppctx = (*ppctx)->table_next;
    if (gs_debug_c('\'') | gs_debug_c('"'))
        (dmprintf_file_and_line((const gs_memory_t *)mem,"./psi/zcontext.c", 1253), errprintf((const gs_memory_t *)mem,"[']destroy %ld at 0x%lx, status = %d\n", pctx->index, (ulong) pctx, pctx->status));


    if (!context_state_free(&pctx->state))
        (((gs_memory_t *) mem)->procs.free_object((gs_memory_t *) mem, pctx, "context_destroy"));
}
...
{
    gs_lock_t *plock = ((gs_lock_t *)((op)->value.pstruct));
