...
int file_close_finish(stream *);
int file_close_disable(stream *);
# 26 "./base/sfxcommon.c" 2


const uint file_default_buffer_size = 2048;
                 ...
                 iodev_proc_fopen_t fopen_proc, gs_memory_t *mem)
{
    ...
    if (code < 0) {
        ...
        ((mem)->procs.free_object(mem, (*ps)->cbuf, "file_close(buffer)"));
        ((mem)->procs.free_object(mem, *ps, "file_prepare_stream(stream)"));
        *ps = ((void*)0);
        return code;
    }
    ...
    case 'r':

        {
            ...
            sread_file(s, file, buffer,
                       (((((rstat.st_mode)) & 0170000) == (0020000)) ? 1 : buffer_size));
        }
        break;
    case 'w':
                 ...
                 uint buffer_size, stream ** ps, char fmode[4], gs_memory_t *mem)
{
    ...
    buffer = (*(mem)->procs.alloc_bytes)(mem, buffer_size, "file_prepare_stream(buffer)");
    if (buffer == 0)
        return ((-25));
    if (fname != 0) {
