...
zrsdparams(i_ctx_t *i_ctx_p)
{
    ...
    ((&empty_array)->value.refs = ((ref *)((void*)0)), ((&empty_array)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((&empty_array)->tas.rsize = (0)));
    if (dict_find_string(op, "Filter", &pFilter) > 0) {
        if (!(((pFilter)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0)))) {
            if (!((((const byte *)&((pFilter)->tas.type_attrs))[1]) == (t_name)))
                return ((-20));
            ((&filter1_array)->value.refs = (pFilter), ((&filter1_array)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((&filter1_array)->tas.rsize = (1)));
    ...
    if (pFilter != &empty_array &&
        ...
        if (pFilter == &filter1_array) {
            ((&parms1_array)->value.refs = (pDecodeParms), ((&parms1_array)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((&parms1_array)->tas.rsize = (1)));
            pDecodeParms = &parms1_array;
        } else if (!(((pDecodeParms)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
            return ((-20));
        else if (((pFilter)->tas.rsize) != ((pDecodeParms)->tas.rsize))
            return ((-15));
    } else
        pDecodeParms = 0;
    for (i = 0; i < ((pFilter)->tas.rsize); ++i) {
        ref f, fname, dp;

        array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pFilter, (long)i, &f);
        if (!((((const byte *)&((&f)->tas.type_attrs))[1]) == (t_name)))
            return ((-20));
        names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, &f, &fname);
        if (((&fname)->tas.rsize) < 6 ||
            memcmp(fname.value.bytes + ((&fname)->tas.rsize) - 6, "Decode", 6)
            )
            return ((-15));
        if (pDecodeParms) {
            array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pDecodeParms, (long)i, &dp);
            if (!(((((const byte *)&((&dp)->tas.type_attrs))[1]) == (t_dictionary)) || ((((const byte *)&((&dp)->tas.type_attrs))[1]) == (t_null))))
                return ((-20));
        }
    }
    ...
    if (code < 0 && code != (-15))
        return code;
    if ((code = dict_bool_param(op, "AsyncRead", ((bool)0), &AsyncRead)) < 0
        )
        return code;
...
zreusablestream(i_ctx_t *i_ctx_p)
{
    ...
    long length = (~(-1L << ((1 << 3) * 8 - 1)));
    bool close_source;
    int code;

    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_boolean)) ) return (check_type_failed(&*op));
    close_source = op->value.boolval;
    if (((((const byte *)&((source_op)->tas.type_attrs))[1]) == (t_string))) {
        uint size = ((source_op)->tas.rsize);

        do { if ( !(((&*source_op)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        code = make_rss(i_ctx_p, source_op, source_op->value.const_bytes,
                        size, (avm_space)(((source_op)->tas.type_attrs) & (((1 << 2) - 1) << 2)), 0L, size, ((bool)0));
    } else if (((((const byte *)&((source_op)->tas.type_attrs))[1]) == (t_astruct))) {
        uint size = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_size)(((gs_memory_t *)((i_ctx_p->memory).current)), source_op->value.pstruct);

        if ((*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.object_type)(((gs_memory_t *)((i_ctx_p->memory).current)), source_op->value.pstruct) != &st_bytes)
            return ((-15));
        do { if ( !(((&*source_op)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        code = make_rss(i_ctx_p, source_op,
                        (const byte *)source_op->value.pstruct, size,
                        (avm_space)(((source_op)->tas.type_attrs) & (((1 << 2) - 1) << 2)), 0L, size, ((bool)1));
    } else if (((((const byte *)&((source_op)->tas.type_attrs))[1]) == (t_array))) {
        int i, blk_cnt, blk_sz;
        ref *blk_ref;
        ulong filelen = 0;

        do { if ( !(((&*source_op)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
        blk_cnt = ((source_op)->tas.rsize);
        blk_ref = source_op->value.refs;
        if (blk_cnt > 0) {
            blk_sz = ((blk_ref)->tas.rsize);
            for (i = 0; i < blk_cnt; i++) {
                int len;

                if ( !(((&blk_ref[i])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&blk_ref[i])->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&blk_ref[i]) : (-7)));
                len = ((&blk_ref[i])->tas.rsize);
        ...
        if (filelen == 0) {
           code = make_rss(i_ctx_p, source_op, (unsigned char *)"", 0,
               (avm_space)(((source_op)->tas.type_attrs) & (((1 << 2) - 1) << 2)), 0, 0, ((bool)0));
        } else {
           code = make_aos(i_ctx_p, source_op, blk_sz, ((&blk_ref[blk_cnt - 1])->tas.rsize), filelen);
        }
    } else {
        long offset = 0;
        stream *source;
        ...
        do { if ( !(((&*(source_op))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(source_op))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(source_op)) : (-7))); do { source = (source_op)->value.pfile; if (source->read_id != ((source_op)->tas.rsize)) { if (source->read_id == 0 && source->write_id == ((source_op)->tas.rsize)) { int fcode = file_switch_to_read(source_op); if (fcode < 0) return(fcode); } else { source = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);
        s = source;
rs:
        if (s->cbuf_string.data != 0) {
            long pos = stell(s);
            long avail = ((s)->cursor.r.limit - (s)->cursor.r.ptr) + pos;

            offset += pos;
            code = make_rss(i_ctx_p, source_op, s->cbuf_string.data,
                            s->cbuf_string.size,
                            imemory_space((const gs_ref_memory_t *)s->memory),
                            offset, (((avail) < (length)) ? (avail) : (length)), ((bool)0));
        } else if (s->file != 0) {
            if (~s->modes & (1 | 4))
                return ((-12));
            ...
            if (sfd_state->eod.size != 0)
                return ((-15));
            offset += sfd_state->skip_count - ((s)->cursor.r.limit - (s)->cursor.r.ptr);
            if (sfd_state->count != 0) {
                long left = (((sfd_state->count) > (0)) ? (sfd_state->count) : (0)) + ((s)->cursor.r.limit - (s)->cursor.r.ptr);
            ...
            return ((-15));
        if (close_source) {
            stream *rs = (source_op)->value.pfile;

            rs->strm = source;
            rs->close_strm = ((bool)1);
        }
    }
    if (code >= 0)
         ...
         uint string_space, long offset, long length, bool is_bytestring)
{
    ...
    stream *s;
    long left = (((length) < (size - offset)) ? (length) : (size - offset));

    ialloc_set_space((&(i_ctx_p->memory)), string_space);
    s = file_alloc_stream(((gs_memory_t *)((i_ctx_p->memory).current)), "make_rss");
    ialloc_set_space((&(i_ctx_p->memory)), save_space);
    if (s == 0)
        return ((-25));
    sread_string_reusable(s, data + offset, (((left) > (0)) ? (left) : (0)));
    if (is_bytestring)
...
make_rfs(i_ctx_t *i_ctx_p, os_ptr op, stream *fs, long offset, long length)
{
    ...
    ialloc_set_space((&(i_ctx_p->memory)), stream_space);
...
static
void aos_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{ aos_state_t *const pptr = vptr;

    ((&pptr->blocks)->tas.type_attrs &= ~(1));
}
static
gs_ptr_type_t aos_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const aos_state_t *pptr = vptr; switch ( index ) { default: return 0;
case 0: return (pep->ptr = (const void *)(((const aos_state_t *)vptr)->s), (&ptr_struct_procs));
case 1:
return (pep->ptr = (const void *)(&pptr->blocks), (&ptr_ref_procs));
} }
static void aos_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { aos_state_t *pptr = vptr;;
(((aos_state_t *)vptr)->s = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(((aos_state_t *)vptr)->s), gcst)));
(*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(pptr->blocks)), (ref_packed *)(&(pptr->blocks) + 1), gcst);
((&pptr->blocks)->tas.type_attrs &= ~(1));
}

static const gs_memory_struct_type_t st_aos_state = { sizeof(aos_state_t), "aos_state", 0, aos_clear_marks, aos_enum_ptrs, aos_reloc_ptrs, 0, 0 };
...
make_aos(i_ctx_t *i_ctx_p, os_ptr op, int blk_sz, int blk_sz_last, uint file_sz)
{
    ...
    s = s_alloc(((gs_memory_t *)((i_ctx_p->memory).current)), "aos_stream");
    ss = (aos_state_t *)s_alloc_state(((gs_memory_t *)((i_ctx_p->memory).current)), &st_aos_state, "st_aos_state");
    buf = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_bytes)(((gs_memory_t *)((i_ctx_p->memory).current)), aos_buf_size, "aos_stream_buf");
    if (s == 0 || ss == 0 || buf == 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), buf, "aos_stream_buf"));
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), ss, "st_aos_state"));
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), s, "aos_stream"));
        ialloc_set_space((&(i_ctx_p->memory)), save_space);
        return ((-25));
    }
    ialloc_set_space((&(i_ctx_p->memory)), save_space);
    ...
    ss->file_sz = file_sz;
    s_std_init(s, buf, aos_buf_size, &s_aos_procs, 1 + 4);
    s->state = (stream_state *)ss;
    s->file_offset = 0;
    s->file_limit = (~(-1L << ((1 << 3) * 8 - 1)));
    s->close_at_eod = ((bool)0);
    s->read_id = 1;
    make_stream_file(op, s, "r");
    return 0;
...
{
    ((s->memory)->procs.free_object(s->memory, s->cbuf, "s_aos_close(buffer)"));
    s->cbuf = 0;
                      ...
                      stream_cursor_write * pw, bool last)
{
    ...
    pos += ((ss->s)->cursor.r.limit - (ss->s)->cursor.r.ptr);
    if (pos >= ss->file_sz)
        return ((int)(-1));
    blk_i = pos / ss->blk_sz;
    blk_off = pos % ss->blk_sz;
    blk_cnt = ((&ss->blocks)->tas.rsize);
