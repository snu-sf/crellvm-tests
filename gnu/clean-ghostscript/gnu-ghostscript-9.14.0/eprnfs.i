# 1 "./contrib/pcl3/eprn/eprnfs.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./contrib/pcl3/eprn/eprnfs.c" 2
# 27 "./contrib/pcl3/eprn/eprnfs.c"
static const char
  cm_id[] = "@(#)$Id: eprnfs.c,v 1.6 2001/05/01 07:02:01 Martin Rel $";
# 37 "./contrib/pcl3/eprn/eprnfs.c"
...
# 185 "./contrib/pcl3/eprn/gdeveprn.h" 2
...
# 38 "./contrib/pcl3/eprn/eprnfs.c" 2
# 157 "./contrib/pcl3/eprn/eprnfs.c"
  ...
  int max_octets, eprn_OctetString bitplanes[])
{
  ...
  for (pixel = 0, pixel_mod_8 = 8; pixel < pixels; pixel++, pixel_mod_8++) {
    ...
    *ptr = (*ptr << 1) | approx;
    ...
    if (error != 0) { remaining_error = error; correction = (7*error)/16; remaining_error -= correction; if (pixel < max_pixel) { to = from + 1; new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (pixel == pixels - 1 && *to > 0) { pixels++; line->length++; } } correction = (3*error)/16; remaining_error -= correction; if (pixel > 0) { to = next_line->str + (pixel - 1); new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (next_line->length < pixel && *to > 0) next_line->length = pixel; } correction = (5*error)/16; remaining_error -= correction; to = next_line->str + pixel; new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (next_line->length <= pixel && *to > 0) next_line->length = pixel + 1; if (pixel < max_pixel) { to = next_line->str + (pixel+1); new_value = *to + remaining_error; *to = ((new_value) < 0? 0: (new_value) > 255? 255: (new_value)); if (next_line->length < pixel + 2 && *to > 0) next_line->length = pixel + 2; } }

    if (pixel_mod_8 == 7) ptr++;
  }

  eprn_finalize(((bool)0), 0, 1, bitplanes, &ptr, pixels);

  return;
}
# 215 "./contrib/pcl3/eprn/eprnfs.c"
  ...
  int max_octets, unsigned int black_levels, eprn_OctetString bitplanes[])
{
  ...
  eprn_finalize(((bool)0), 0, planes, bitplanes, ptr, pixels);

  return;
}
# 293 "./contrib/pcl3/eprn/eprnfs.c"
  ...
  eprn_OctetString *next_line, int max_octets, eprn_OctetString bitplanes[])
{
  ...
  for (plane = 0; plane < 4; plane++) ptr[plane] = bitplanes[plane].str;
  ...
  for (pixel = 0, pixel_mod_8 = 8; pixel < pixels; pixel++, pixel_mod_8++) {
    if (pixel_mod_8 == 8) {
      pixel_mod_8 = 0;
      for (plane = 0; plane < 4; plane++) *ptr[plane] = 0;
    }



    for (colorant = 3; colorant >= 0; colorant--) {
      from = line->str + pixel*4 + colorant;
      ...
      *ptr[plane] = (*ptr[plane] << 1) | approx;
      ...
      if (error != 0) { remaining_error = error; correction = (7*error)/16; remaining_error -= correction; if (pixel < max_pixel) { to = from + 4; new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (pixel == pixels - 1 && *to > 0) { pixels++; line->length += 4; } } correction = (3*error)/16; remaining_error -= correction; if (pixel > 0) { to = next_line->str + (pixel - 1)*4 + colorant; new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (next_line->length < pixel*4 && *to > 0) next_line->length = pixel*4; } correction = (5*error)/16; remaining_error -= correction; to = next_line->str + pixel*4 + colorant; new_value = *to + correction; if (new_value < 0) { *to = 0; remaining_error += new_value; } else if (255 < new_value) { *to = 255; remaining_error += new_value - 255; } else *to = new_value; if (next_line->length <= pixel*4 && *to > 0) next_line->length = (pixel + 1)*4; if (pixel < max_pixel) { to = next_line->str + (pixel+1)*4 + colorant; new_value = *to + remaining_error; *to = ((new_value) < 0? 0: (new_value) > 255? 255: (new_value)); if (next_line->length < (pixel + 2)*4 && *to > 0) next_line->length = (pixel + 2)*4; } }
    }

    if (pixel_mod_8 == 7)
      for (plane = 0; plane < 4; plane++) ptr[plane]++;
  }

  eprn_finalize(((bool)0), 2, 4, bitplanes, ptr, pixels);

  return;
}
# 361 "./contrib/pcl3/eprn/eprnfs.c"
...
# 445 "./contrib/pcl3/eprn/eprnfs.c"
  ...
  eprn_OctetString bitplanes[])
{
  const int
    ...
    last_colorant = black_levels > 0? 3: 2,
    max_pixel = max_octets/4 - 1,
  ...
  if (black_levels > 0) {
    divisor[3] = 256/black_levels;
    max_level[3] = black_levels - 1;
  }
  else {
    divisor[3] = 0;
    max_level[3] = 0;
  }
  next_plane[3] = black_planes;
  ...
  for (pixel = 0, pixel_mod_8 = 8; pixel < pixels; pixel++, pixel_mod_8++) {
    ...
    if (colour_model == eprn_DeviceCMY_plus_K &&
      ...
      if (value % (non_black_levels - 1) == 0) {
        ...
        approx[3] = value/(non_black_levels - 1);
        approx[0] = approx[1] = approx[2] = 0;
      }
    }
...
# 568 "./contrib/pcl3/eprn/eprnfs.c"
