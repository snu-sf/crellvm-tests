...
# 33 "./base/gsptype2.c" 2


static const gc_ptr_element_t pattern2_template_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_pattern2_template_t *)0)->Shading - (char *)((gs_pattern2_template_t *)0) )) } }; static const gc_struct_data_t pattern2_template_reloc_ptrs = { (sizeof(pattern2_template_enum_ptrs) / sizeof((pattern2_template_enum_ptrs)[0])), 0, &st_pattern_template, pattern2_template_enum_ptrs }; static const gs_memory_struct_type_t st_pattern2_template = { sizeof(gs_pattern2_template_t), "gs_pattern2_template_t", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &pattern2_template_reloc_ptrs };
static gs_ptr_type_t pattern2_instance_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void pattern2_instance_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_pattern2_instance = { sizeof(gs_pattern2_instance_t), "gs_pattern2_instance_t", 0, 0, pattern2_instance_enum_ptrs, pattern2_instance_reloc_ptrs, 0, 0 };


static gs_ptr_type_t pattern2_instance_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { switch ( index ) { default: {
    if (index < (2 + 1)) {
        gs_ptr_type_t ptype =
            (*(st_pattern2_template).enum_ptrs)(mem, &((const gs_pattern2_instance_t *)vptr)->templat, sizeof(((const gs_pattern2_instance_t *)vptr)->templat), index-(0), pep, &(st_pattern2_template), gcst);
    ...
    return (*(st_pattern_instance).enum_ptrs)(mem, vptr, size, index-((2 + 1)), pep, &(st_pattern_instance), gcst);
}
} }
static void pattern2_instance_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { {
    (*(st_pattern_instance).reloc_ptrs)(vptr, size, &(st_pattern_instance), gcst);
    (*(st_pattern2_template).reloc_ptrs)(&((gs_pattern2_instance_t *)vptr)->templat, sizeof(((gs_pattern2_instance_t *)vptr)->templat), &(st_pattern2_template), gcst);
} }


static bool gs_pattern2_uses_base_space(const gs_pattern_template_t *);
static int gs_pattern2_make_pattern(gs_client_color *, const gs_pattern_template_t *, const gs_matrix *, gs_state *, gs_memory_t *);
static const gs_pattern_template_t *gs_pattern2_get_pattern(const gs_pattern_instance_t *);
static int gs_pattern2_remap_color(const gs_client_color *, const gs_color_space *, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t);
static int gs_pattern2_set_color(const gs_client_color *, gs_state *);
...
static const gc_struct_data_t dc_pattern2_reloc_ptrs = { 0, ((int) ( (char *)&((gx_device_color *)0)->ccolor - (char *)((gx_device_color *)0) )), &st_client_color, 0 }; static const gs_memory_struct_type_t st_dc_pattern2 = { sizeof(gx_device_color), "dc_pattern2", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &dc_pattern2_reloc_ptrs };



static const gx_device_halftone * gx_dc_pattern2_get_dev_halftone(const gx_device_color * pdevc);
static int gx_dc_pattern2_load(gx_device_color *pdevc, const gs_imager_state *pis, gx_device *dev, gs_color_select_t select);
static int gx_dc_pattern2_fill_rectangle(const gx_device_color *pdevc, int x, int y, int w, int h, gx_device *dev, gs_logical_operation_t lop, const gx_rop_source_t *source);
static bool gx_dc_pattern2_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);
static void gx_dc_pattern2_save_dc(const gx_device_color * pdevc, gx_device_color_saved * psdc);
...
{
    if (((dev)->procs.dev_spec_op)(dev, gxdso_pattern_is_cpath_accum, ((void*)0), 0)) {


        return ((dev)->procs.fill_rectangle)(dev, x, y, w, h, (gx_color_index)0 );
    } else {
        gs_fixed_rect rect;
        gs_pattern2_instance_t *pinst =
...
{
    gs_rect dev_rect;
    int code = gs_bbox_transform(rect, &(*(const gs_matrix *)&(pis)->ctm), &dev_rect);

    if (code >= 0) {
        rfixed->p.x = ((fixed)((dev_rect.p.x)*(float)(1<<8)));
        rfixed->p.y = ((fixed)((dev_rect.p.y)*(float)(1<<8)));
        rfixed->q.x = ((fixed)((dev_rect.q.x)*(float)(1<<8)));
        rfixed->q.y = ((fixed)((dev_rect.q.y)*(float)(1<<8)));
    }
    return code;
}
                              ...
                              gx_clip_path *cpath_local, const gx_clip_path **ppcpath1)
{
    if (gx_dc_is_pattern2_color(pdevc) && gx_dc_pattern2_color_has_bbox(pdevc) &&
            (*((pdev)->procs.dev_spec_op))(pdev, gxdso_pattern_shading_area, ((void*)0), 0) == 0) {
        gs_pattern2_instance_t *pinst = (gs_pattern2_instance_t *)pdevc->ccolor.pattern;
        gx_path box_path;
        gs_memory_t *mem = (*ppcpath1 != ((void*)0) ? (*ppcpath1)->path.memory : pdev->memory);
        int code;

        (void)gx_path_init_local_shared(&box_path, ((void*)0), mem);
        ...
        if (code == (-13)) {
            ...
            if (code >= 0) {
                ...
                code = gx_cpath_intersect(cpath_local, &box_path, (-1), (gs_imager_state *)pinst->saved);
...
gx_dc_pattern2_is_rectangular_cell(const gx_device_color * pdevc, gx_device * pdev, gs_fixed_rect *rect)
{
    if (gx_dc_is_pattern2_color(pdevc) && gx_dc_pattern2_color_has_bbox(pdevc) &&
        ...
        if ((((&(*(const gs_matrix *)&(pinst->saved)->ctm))->xy) == 0.0 && ((&(*(const gs_matrix *)&(pinst->saved)->ctm))->yx) == 0.0))
            if (psh->params.have_BBox) {
...
bool gx_dc_pattern2_has_background(const gx_device_color *pdevc)
{
    ...
    return !pinst->shfill && Shading->params.Background != ((void*)0);
}
