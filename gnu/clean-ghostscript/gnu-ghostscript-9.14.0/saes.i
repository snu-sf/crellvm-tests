...
static const gc_ptr_element_t aes_state_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_aes_state *)0)->ctx - (char *)((stream_aes_state *)0) )) } }; static const gc_struct_data_t aes_state_reloc_ptrs = { (sizeof(aes_state_enum_ptrs) / sizeof((aes_state_enum_ptrs)[0])), 0, 0, aes_state_enum_ptrs }; static const gs_memory_struct_type_t st_aes_state = { sizeof(stream_aes_state), "aes filter state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &aes_state_reloc_ptrs };
                  ...
                  int keylength)
{
    ...
    if (code) {
      return gs_throw_imp(__func__, "./base/saes.c", 50, 0, (-15), "could not set AES key");
    }
...
s_aes_release(stream_state *ss)
{
    ...
    if (state->ctx != ((void*)0))
        ((state->memory)->procs.free_object(state->memory, state->ctx, "aes context structure"));
}
                  ...
                  stream_cursor_write * pw, bool last)
{
    ...
    if (state->ctx == ((void*)0)) {
      ...
      state->ctx = (aes_context *)((state->memory)->procs.alloc_bytes_immovable(state->memory, sizeof(aes_context), "aes context structure"));

      if (state->ctx == ((void*)0)) {
        gs_throw_imp(__func__, "./base/saes.c", 120, 0, (-25), "could not allocate aes context");
        return ((int)(-2));
      }
      if (state->keylength < 1 || state->keylength > 32) {
        gs_throw_imp(__func__, "./base/saes.c", 125, 0, (-15), "invalid aes key length (%d bytes)", state->keylength);
    ...
    while (pr->ptr + 16 <= limit) {
      aes_crypt_cbc(state->ctx, 0, 16, state->iv,
                                pr->ptr + 1, temp);
      pr->ptr += 16;
      if (last && pr->ptr == pr->limit) {
        ...
        if (state->use_padding) {
          ...
          if (pad < 1 || pad > 16) {
            ...
            (void)gs_throw_imp(__func__, "./base/saes.c", 156, 3, 0, "invalid aes padding byte (0x%02x)", (unsigned char)pad);
    ...
    if (status == ((int)(-1))) {
      gs_throw_imp(__func__, "./base/saes.c", 176, 0, (-15), "aes stream isn't a multiple of 16 bytes");
      return 0;
    }
