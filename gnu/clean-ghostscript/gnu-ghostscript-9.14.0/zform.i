...
static int zbeginform(i_ctx_t *i_ctx_p)
{
    ...
    code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, &tmplate.CTM);
    if (code < 0)
        return code;

    code = dict_floats_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, "BBox", 4, BBox, ((void*)0));
    if (code < 0)
        return code;
    if (code == 0)
       return ((-21));
    ...
    code = dict_floats_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, "Matrix", 6, Matrix, ((void*)0));
    if (code < 0)
        return code;
    if (code == 0)
       return ((-21));
    ...
    tmplate.pcpath = (i_ctx_p->pgs)->clip_path;
    code = ((cdev)->procs.dev_spec_op)(cdev, gxdso_form_begin,
                            &tmplate, 0);
    ...
    if (code > 0)
    {
        ...
        gs_setmatrix((i_ctx_p->pgs), &tmplate.CTM);
# 95 "./psi/zform.c"
        code = gx_default_clip_box((i_ctx_p->pgs), &box);
        if (code < 0)
            return code;
        ...
        code = gx_clip_to_rectangle((i_ctx_p->pgs), &box);
    }
...
static int zendform(i_ctx_t *i_ctx_p)
{
    ...
    code = ((cdev)->procs.dev_spec_op)(cdev, gxdso_form_end,
                            0, 0);
    return code;
}
