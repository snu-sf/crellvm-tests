...
static const bool I_VALIDATE_BEFORE_SAVE = ((bool)1);
static const bool I_VALIDATE_AFTER_SAVE = ((bool)1);
static const bool I_VALIDATE_BEFORE_RESTORE = ((bool)1);
static const bool I_VALIDATE_AFTER_RESTORE = ((bool)1);
...
static const gc_ptr_element_t vm_save_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((vm_save_t *)0)->gsave - (char *)((vm_save_t *)0) )) } }; static const gc_struct_data_t vm_save_reloc_ptrs = { (sizeof(vm_save_enum_ptrs) / sizeof((vm_save_enum_ptrs)[0])), 0, 0, vm_save_enum_ptrs }; static const gs_memory_struct_type_t st_vm_save = { sizeof(vm_save_t), "savetype", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &vm_save_reloc_ptrs };
...
{
    if (gs_debug_c('?')) {
        ref_stack_cleanup(&((i_ctx_p->dict_stack).stack));
        ref_stack_cleanup(&((i_ctx_p->exec_stack).stack));
        ref_stack_cleanup(&((i_ctx_p->op_stack).stack));
        ialloc_validate_spaces((&(i_ctx_p->memory)));
    }
}
...
zsave(i_ctx_t *i_ctx_p)
{
    ...
    vmsave = (vm_save_t *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_struct)(((gs_memory_t *)((i_ctx_p->memory).current)), &st_vm_save, "zsave");
    ialloc_set_space((&(i_ctx_p->memory)), space);
    if (vmsave == 0)
        return ((-25));
    vmsave->gsave = ((void*)0);
    code = alloc_save_state((&(i_ctx_p->memory)), vmsave, &sid);
    if (code < 0)
        return code;
    if (sid == 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), vmsave, "zsave"));
        return ((-25));
    }
    do { } while (0);

    code = gs_gsave_for_save((i_ctx_p->pgs), &prev);
    if (code < 0)
        return code;
    ...
    ((op)->value.saveid = (sid), ((op)->tas.type_attrs = ((t_save) << 8) + (0)) );
...
zrestore(i_ctx_t *i_ctx_p)
{
    ...
    bool last;
    vm_save_t *vmsave;
    int code = restore_check_operand(op, &asave, (&(i_ctx_p->memory)));
    ...
    (((i_ctx_p->op_stack).stack).p)--;
    {
        ...
        if ((code = restore_check_stack(i_ctx_p, &((i_ctx_p->op_stack).stack), asave, ((bool)0))) < 0 ||
            (code = restore_check_stack(i_ctx_p, &((i_ctx_p->exec_stack).stack), asave, ((bool)1))) < 0 ||
            (code = restore_check_stack(i_ctx_p, &((i_ctx_p->dict_stack).stack), asave, ((bool)0))) < 0
            ) {
            (((i_ctx_p->op_stack).stack).p)++;
            return code;
        }
    ...
    restore_fix_stack(i_ctx_p, &((i_ctx_p->op_stack).stack), asave, ((bool)0));
    restore_fix_stack(i_ctx_p, &((i_ctx_p->exec_stack).stack), asave, ((bool)1));
    restore_fix_stack(i_ctx_p, &((i_ctx_p->dict_stack).stack), asave, ((bool)0));


    do {
        vmsave = alloc_save_client_data(alloc_save_current((&(i_ctx_p->memory))));

        gs_grestoreall_for_restore((i_ctx_p->pgs), vmsave->gsave);
# 156 "./psi/zvmem.c"
        vmsave->gsave = 0;

        code = alloc_restore_step_in((&(i_ctx_p->memory)), asave);
        if (code < 0)
            return code;
        last = code;
    }
    while (!last);
    {
        ...
        ialloc_set_space((&(i_ctx_p->memory)), avm_local);
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), vmsave, "zrestore"));
        ialloc_set_space((&(i_ctx_p->memory)), space);
    }
    ...
    i_ctx_p->LockFilePermissions = ((bool)0);
    return 0;
}
                      ...
                      gs_dual_memory_t *idmem)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_save)) ) return (check_type_failed(&*op));
    vmsave = ((vm_save_t *)((op)->value.pstruct));
    if (vmsave == 0)
        return ((-11));
    sid = op->value.saveid;
                    ...
                    const alloc_save_t * asave, bool is_estack)
{
    ...
    do {
        ...
        for (; size; stkp++, size--) {
            ...
            switch ((((const byte *)&((stkp)->tas.type_attrs))[1])) {
                case t_array:
                ...
                case t_file:
...

                    {
                        ...
                        if (is_estack &&
                            ((((stkp)->tas.type_attrs) & (0x80)) ||
                             (s = (stkp)->value.pfile, (s->read_id | s->write_id) != ((stkp)->tas.rsize)))
                            )
                            continue;
                    }
                ...
                case t_string:
                    ...
                    if (((stkp)->tas.rsize) == 0 &&
                        (((stkp)->tas.type_attrs) & (0x80)) && is_estack
                        )
                        continue;
                ...
                case t_save:
                    ...
                    if (i_ctx_p->language_level <= 2)
                        continue;
                    ptr = alloc_find_save(&(i_ctx_p->memory), stkp->value.saveid);
    ...
    do {
        ...
        for (; size; stkp++, size--) {
            ...
            if (is_estack) {
                ...
                switch ((((const byte *)&((stkp)->tas.type_attrs))[1])) {
                    case t_string:
                        if (((stkp)->tas.rsize) == 0 &&
                            ...
                            ((stkp)->value.const_bytes = ((const byte *)((void*)0)), ((stkp)->tas.type_attrs = ((t_string) << 8) + (avm_foreign)), ((stkp)->tas.rsize = (0)));
                ...
                ((stkp)->tas.type_attrs = ((stkp)->tas.type_attrs & ~((0x10 + 0x20 +0x40) | 0x80)) | ((&ofile)->tas.type_attrs & ((0x10 + 0x20 +0x40) | 0x80)));
...
zvmstatus(i_ctx_t *i_ctx_p)
{
    ...
    ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.status(((gs_memory_t *)((i_ctx_p->memory).current)), &mstat));
    if (((gs_memory_t *)((i_ctx_p->memory).current)) == ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.global))) {
        gs_memory_status_t sstat;

        ((((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)))->procs.status(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), &sstat));
        mstat.allocated += sstat.allocated;
        mstat.used += sstat.used;
    }
    ((((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory)->procs.status(((gs_memory_t *)((i_ctx_p->memory).current))->non_gc_memory, &dstat));
    ...
    ((op - 2)->value.intval = (imemory_save_level(((i_ctx_p->memory).spaces.memories.named.local))), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op - 1)->value.intval = (mstat.used), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op)->value.intval = (mstat.allocated + dstat.allocated - dstat.used), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zforgetsave(i_ctx_t *i_ctx_p)
{
    ...
    restore_fix_stack(i_ctx_p, &((i_ctx_p->op_stack).stack), asave, ((bool)0));
    restore_fix_stack(i_ctx_p, &((i_ctx_p->exec_stack).stack), asave, ((bool)0));
    restore_fix_stack(i_ctx_p, &((i_ctx_p->dict_stack).stack), asave, ((bool)0));
    ...
    code = alloc_forget_save_in((&(i_ctx_p->memory)), asave);
    if (code < 0)
        return code;
    {
        uint space = (((&(i_ctx_p->memory)))->current_space);
