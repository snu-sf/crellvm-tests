# 1 "./devices/vector/gdevpdtw.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdtw.c" 2
# 18 "./devices/vector/gdevpdtw.c"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 55 "./base/memory_.h" 2
# 19 "./devices/vector/gdevpdtw.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
# 20 "./devices/vector/gdevpdtw.c" 2
...
# 22 "./devices/vector/gdevpdtw.c" 2
# 1 "./base/gxfont.h" 1
# 24 "./base/gxfont.h"
# 1 "./base/gsfont.h" 1
# 24 "./base/gsfont.h"
...
# 23 "./devices/vector/gdevpdtw.c" 2
...
# 24 "./devices/vector/gdevpdtw.c" 2
# 1 "./base/gscencs.h" 1
# 45 "./base/gscencs.h"
...
# 25 "./devices/vector/gdevpdtw.c" 2
...
int psf_convert_type1_to_type2(stream *s, const gs_glyph_data_t *pgd,
                               gs_font_type1 *pfont);
# 26 "./devices/vector/gdevpdtw.c" 2
...
# 27 "./devices/vector/gdevpdtw.c" 2
...
# 28 "./devices/vector/gdevpdtw.c" 2
...
# 29 "./devices/vector/gdevpdtw.c" 2
...
# 30 "./devices/vector/gdevpdtw.c" 2
...
# 31 "./devices/vector/gdevpdtw.c" 2
...
# 32 "./devices/vector/gdevpdtw.c" 2
# 1 "./devices/vector/gdevpdtv.h" 1
# 35 "./devices/vector/gdevpdtv.h"
extern const unsigned char gs_c_pdf_glyph_type[];
# 33 "./devices/vector/gdevpdtw.c" 2


static const char *const encoding_names[] = {
    "StandardEncoding", "ISOLatin1Encoding", "SymbolEncoding", "DingbatsEncoding", "WinAnsiEncoding", "MacRomanEncoding", "MacExpertEncoding"
};
...
pdf_simple_font_needs_ToUnicode(const pdf_font_resource_t *pdfont)
{
    ...
    for (ch = 0; ch < 256; ++ch) {
        ...
        if (glyph == ((gs_glyph)0x7fffffff))
            continue;
        if (glyph < gs_c_min_std_encoding_glyph || glyph >= ((gs_glyph)0x80000000L)) {
            if (pet->str.size == 0)
                return ((bool)1);
...
# 270 "./devices/vector/gdevpdtw.c"
    psf_enumerate_bits_begin(&genum, ((void*)0),
                             wmode ? pdfont->u.cidfont.used2 : pdfont->used,
                             pdfont->count, GLYPH_SPACE_INDEX);
    memset(counts, 0, sizeof(counts));
    while (!psf_enumerate_glyphs_next(&genum, &glyph)) {
        int i = glyph - ((gs_glyph)0x80000000L);

        if ( i < pdfont->count) {
            int width = (int)(w[i] + 0.5);

            counts[(((((width) < 0 ? -(width) : (width))) < ((sizeof(counts) / sizeof((counts)[0])) - 1)) ? (((width) < 0 ? -(width) : (width))) : ((sizeof(counts) / sizeof((counts)[0])) - 1))]++;
            if (width > 0)
                pos_count++;
            else if (width < 0)
                neg_count++;
    ...
    if (wmode) {
        ...
        while (!psf_enumerate_glyphs_next(&genum, &glyph)) {
            ...
            if ( i < pdfont->count) {
                ...
                if ((((((width) < 0 ? -(width) : (width))) < ((sizeof(counts) / sizeof((counts)[0])) - 1)) ? (((width) < 0 ? -(width) : (width))) : ((sizeof(counts) / sizeof((counts)[0])) - 1)) == ((dwi) < 0 ? -(dwi) : (dwi))) {
                         ...
                         const pdf_font_resource_t *pdfont, int wmode)
{
    ...
    {
        while (!psf_enumerate_glyphs_next(&genum, &glyph)) {
            int cid = glyph - ((gs_glyph)0x80000000L);
...

            {
                ...
                gs_font_base *pfont = pdf_font_resource_font(pdfont, ((bool)0));
                ...
                if (pfont->FontType == ft_TrueType) {
                    ...
                    if (cid == 0 && glyph1 == ((gs_glyph)0x7fffffff))
                        glyph1 = copied_get_notdef((gs_font *)pdf_font_resource_font(pdfont, ((bool)0)));
                    if (glyph1 == ((gs_glyph)0x7fffffff))
                        continue;
                    if (pfont->procs.glyph_info((gs_font *)pfont, glyph1, ((void*)0), 0, &info) < 0)
                        continue;
                } else if (pfont->procs.glyph_info((gs_font *)pfont, glyph, ((void*)0), 0, &info) < 0)
                    continue;
            }
...
pdf_write_contents_cid2(gx_device_pdf *pdev, pdf_font_resource_t *pdfont)
{
    ...
    psf_enumerate_bits_begin(&genum, ((void*)0), pdfont->used, count,
                             GLYPH_SPACE_INDEX);
    while (!psf_enumerate_glyphs_next(&genum, &glyph)) {
        int cid = glyph - ((gs_glyph)0x80000000L);
    ...
    if (map_id && pdf_font_descriptor_embedding(pdfont->FontDescriptor)) {
        ...
        for (i = 0; i < pdfont->u.cidfont.CIDToGIDMapLength; ++i) {
            ...
            spputc(writer.binary.strm, (byte)(gid >> 8));
            spputc(writer.binary.strm, (byte)(gid));
        }
...
{
    stream *s;
    cos_dict_t *pcd_Resources = ((void*)0);
    char *base14_name = ((void*)0);
    int id;

    if (pdfont->cmap_ToUnicode != ((void*)0) && pdfont->res_ToUnicode == ((void*)0))
    ...
    if (pdev->CompatibilityLevel >= 1.2 &&
            ...
            pdfont->FontType == ft_GL2_stick_user_defined ||
            pdfont->FontType == ft_GL2_531) &&
            pdfont->u.simple.s.type3.Resources != ((void*)0) &&
            pdfont->u.simple.s.type3.Resources->elements != ((void*)0)) {
        int code;
        ...
        code = cos_write(((const cos_object_t *)&((pcd_Resources)->cos_procs)), pdev, (pcd_Resources)->id);
        if (code < 0)
            return code;
        pdf_end_separate(pdev, resourceFont);
    }
    ...
    for (j = 0; j < 16; ++j)
        for (pres = prlist->chains[j]; pres != 0; pres = pres->next) {
            ...
            if (pdf_resource_id(pres) != -1) {
                int code = pdf_compute_BaseFont(pdev, pdfont, ((bool)1));
                          ...
                          const gs_cid_system_info_t *pcidsi, gs_id object_id)
{
    ...
    Registry = (*(pdev->v_memory)->procs.alloc_bytes)(pdev->v_memory, pcidsi->Registry.size, "temporary buffer for Registry");
    if (!Registry)
        return(((-25)));
    Ordering = (*(pdev->v_memory)->procs.alloc_bytes)(pdev->v_memory, pcidsi->Ordering.size, "temporary buffer for Registry");
    if (!Ordering) {
        ((pdev->v_memory)->procs.free_object(pdev->v_memory, Registry, "free temporary Registry buffer"));
        return(((-25)));
    }
    ...
    if (pdev->KeyLength && object_id != 0) {
        ...
        if (code < 0) {
            ((pdev->v_memory)->procs.free_object(pdev->v_memory, Registry, "free temporary Registry buffer"));
            ((pdev->v_memory)->procs.free_object(pdev->v_memory, Ordering, "free temporary Ordering buffer"));
            return((code));
        }
    ...
    stream_puts(s, "<<\n/Registry");
    s_write_ps_string(s, Registry, pcidsi->Registry.size, 4);
    stream_puts(s, "\n/Ordering");
    s_write_ps_string(s, Ordering, pcidsi->Ordering.size, 4);
    pprintd1(s, "\n/Supplement %d\n>>\n", pcidsi->Supplement);
    ((pdev->v_memory)->procs.free_object(pdev->v_memory, Registry, "free temporary Registry buffer"));
    ((pdev->v_memory)->procs.free_object(pdev->v_memory, Ordering, "free temporary Ordering buffer"));
    return 0;
