...
# 28 "./base/sdcparam.c" 2



static const gs_param_item_t s_DCT_param_items[] =
{
{ "ColorTransform", gs_param_type_int, ((int) ( (char *)&((stream_DCT_state *)0)->ColorTransform - (char *)((stream_DCT_state *)0) )) },
    { "QFactor", gs_param_type_float, ((int) ( (char *)&((stream_DCT_state *)0)->QFactor - (char *)((stream_DCT_state *)0) )) },
    { 0 }
};
static const gs_param_item_t jsd_param_items[] =
{
    { "Picky", gs_param_type_int, ((int) ( (char *)&((jpeg_stream_data *)0)->Picky - (char *)((jpeg_stream_data *)0) )) },
    { "Relax", gs_param_type_int, ((int) ( (char *)&((jpeg_stream_data *)0)->Relax - (char *)((jpeg_stream_data *)0) )) },
    { 0 }
};
# 53 "./base/sdcparam.c"
static const byte natural_order[64] =
{
    0, 1, 8, 16, 9, 2, 3, 10,
...
static const byte inverse_natural_order[64] =
{
    0, 1, 5, 6, 14, 15, 27, 28,
                   ...
                   double QFactor, gs_memory_t * mem)
{
    ...
    data = (*(mem)->procs.alloc_string)(mem, count, "quant_param_string");
    if (data == 0)
        return ((-25));
    for (i = 0; i < count; ++i) {
        double val = pvals[inverse_natural_order[i]] / QFactor;
                  ...
                  double QFactor, gs_memory_t * mem)
{
    ...
    data = (float *)(*(mem)->procs.alloc_byte_array)(mem, count, sizeof(float), "quant_param_array");


    if (data == 0)
        return ((-25));
    for (i = 0; i < count; ++i)
        data[i] = pvals[inverse_natural_order[i]] / QFactor;
    pfa->data = data;
    pfa->size = count;
    pfa->persistent = ((bool)1);
    return 0;
}
                              ...
                              bool is_encode)
{
    ...
    if (is_encode) {
        ...
        quant_tables.size = (((int)(sizeof(d_comp_info))) / ((int)(sizeof((d_comp_info)[0]))));
    ...
    code = (*(plist)->procs->begin_xmit_collection)(plist, "QuantTables", &quant_tables, gs_param_collection_array);
    ...
    for (i = 0; i < num_in_tables; ++i) {
        ...
        gs_sprintf(key, "%d", i);
        if (QFactor == 1.0) {
            code = quant_param_string(&str, 64,
    ...
    return (*(plist)->procs->end_xmit_collection)(plist, "QuantTables", &quant_tables);
}
                ...
                gs_memory_t * mem)
{
    ...
    for (i = 1, total = 0; i <= 16; ++i)
        total += table->bits[i];
    data = (*(mem)->procs.alloc_string)(mem, 16 + total, "pack_huff_table");
    if (data == 0)
        return ((-25));
                         ...
                         bool is_encode)
{
    ...
    huff_data = (gs_param_string *)
        (*(mem)->procs.alloc_byte_array)(mem, num_in_tables, sizeof(gs_param_string), "get huffman tables");
...
int s_DCT_put_params(gs_param_list *plist, stream_DCT_state *ss);
                  ...
                  int count, UINT8 * pvals)
{
    ...
    if (code < 0)
        (*(plist)->procs->signal_error)(plist, key, code);
    return code;
}
                              ...
                              bool is_encode)
{
    ...
    switch ((code = (*(plist)->procs->begin_xmit_collection)(plist, "QuantTables", &quant_tables, (((bool)1) ? gs_param_collection_dict_int_keys : gs_param_collection_dict_any)))

        ) {
        case 1:
            return 1;
        default:
            return (*(plist)->procs->signal_error)(plist, "QuantTables", code);
        case 0:
            ;
    }
    if (is_encode) {
    ...
    for (i = 0; i < num_in_tables; ++i) {
        ...
        gs_sprintf(istr, "%d", i);
        code = quant_params(quant_tables.list, istr, 64, values,
                            pdct->QFactor);
        if (code < 0)
            return code;
                         ...
                         bool is_encode)
{
    ...
    switch ((code = (*(plist)->procs->begin_xmit_collection)(plist, "HuffTables", &huff_tables, (((bool)1) ? gs_param_collection_dict_int_keys : gs_param_collection_dict_any)))

        ) {
        case 1:
            return 0;
        default:
            return (*(plist)->procs->signal_error)(plist, "HuffTables", code);
        case 0:
            ;
    }
    if (is_encode) {
        ...
        if (pdct->data.common->Relax)
            max_tables = (((pdct->data.compress->cinfo.input_components) > (2)) ? (pdct->data.compress->cinfo.input_components) : (2));
    } else {
