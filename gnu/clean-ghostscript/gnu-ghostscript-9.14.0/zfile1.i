...
zfile_name_combine(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&op[ 0])->tas.type_attrs))[1]) == (t_boolean)) ) return (check_type_failed(&op[ 0]));
    if ( !((((const byte *)&((&op[-1])->tas.type_attrs))[1]) == (t_string)) ) return (check_type_failed(&op[-1]));
    if ( !((((const byte *)&((&op[-2])->tas.type_attrs))[1]) == (t_string)) ) return (check_type_failed(&op[-2]));
    plen = ((op - 2)->tas.rsize);
    flen = ((op - 1)->tas.rsize);
    blen = blen0 = plen + flen + 2;
    buffer = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), blen, "zfile_name_combine");
    if (buffer == 0)
        return ((-25));
    ...
    if (gp_file_name_combine((const char *)prefix, plen,
        ...
        buffer = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.resize_string)(((gs_memory_t *)((i_ctx_p->memory).current)), buffer, blen0, blen, "zfile_name_combine");
        if (buffer == 0)
            return ((-25));
        ((op - 2)->value.bytes = (buffer), ((op - 2)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((op - 2)->tas.rsize = (blen)));
        ((op - 1)->value.boolval = (((bool)1)), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
...
{ os_ptr op = (((i_ctx_p->op_stack).stack).p);
    ...
    if ( !((((const byte *)&((&op[0])->tas.type_attrs))[1]) == (t_string)) ) return (check_type_failed(&op[0]));
    ((op)->value.boolval = (gp_file_name_is_absolute((const char *)op->value.const_bytes, ((op)->tas.rsize))), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
...
{ os_ptr op = (((i_ctx_p->op_stack).stack).p);
    ...
    ((op)->value.const_bytes = ((const byte *)v), ((op)->tas.type_attrs = ((t_string) << 8) + (avm_foreign | (0x20 + 0x40))), ((op)->tas.rsize = (len)));
