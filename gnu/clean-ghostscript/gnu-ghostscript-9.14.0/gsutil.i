...
memflip8x8(const byte * inp, int line_size, byte * outp, int dist)
{
    ...
    if (aceg == bdfh && (aceg >> 8) == (aceg & 0xffffff)) {
        ...
        *outp = (byte)-(int)((aceg >> 1) & 1);
        ...
        (aceg ^= (temp = ((aceg >> 20) ^ aceg) & 0x00000f0f), aceg ^= temp << 20);
        (bdfh ^= (temp = ((bdfh >> 20) ^ bdfh) & 0x00000f0f), bdfh ^= temp << 20);


        (aceg ^= (temp = ((aceg >> 10) ^ aceg) & 0x00330033), aceg ^= temp << 10);
        (bdfh ^= (temp = ((bdfh >> 10) ^ bdfh) & 0x00330033), bdfh ^= temp << 10);


        (aceg ^= (temp = ((bdfh >> 1) ^ aceg) & 0x55555555), bdfh ^= temp << 1);
...
{
    if (((puid)->id < 0)) {
        uint xsize = ((uint)(-(puid)->id));
        long *xvalues = (long *)
            (*(mem)->procs.alloc_byte_array)(mem, xsize, sizeof(long), cname);

        if (xvalues == 0)
            return ((-25));
        memcpy(xvalues, ((puid)->xvalues), xsize * sizeof(long));
                    ...
                    gs_int_rect * diffs )
{
    ...
    if (y0 < inner->p.y) {
        ...
        diffs[0].q.x = x1, diffs[0].q.y = (((y1) < (inner->p.y)) ? (y1) : (inner->p.y));
        outer->p.y = y0 = diffs[0].q.y;
        ++count;
    }
    if (y1 > inner->q.y) {
        diffs[count].p.x = x0, diffs[count].p.y = (((y0) > (inner->q.y)) ? (y0) : (inner->q.y));
    ...
    if (x0 < inner->p.x) {
        diffs[0].p.x = x0, diffs[0].p.y = y0;
        diffs[0].q.x = (((x1) < (inner->p.x)) ? (x1) : (inner->p.x)), diffs[0].q.y = y1;
        outer->p.x = x0 = diffs[count].q.x;
        ++count;
    }
    if (x1 > inner->q.x) {
        diffs[count].p.x = (((x0) > (inner->q.x)) ? (x0) : (inner->q.x)), diffs[count].p.y = y0;
        diffs[count].q.x = x1, diffs[count].q.y = y1;
