...
# 26 "./base/gxclrect.c" 2

extern int gdev_prn_forwarding_dev_spec_op(gx_device *dev, int op, void *data, int datasize);
# 36 "./base/gxclrect.c"
static int
cmd_size_rect(register const gx_cmd_rect * prect)
{
    return
        ((!((prect->x) & ~0x7f)) ? 1 : (!((prect->x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(prect->x))) + ((!((prect->y) & ~0x7f)) ? 1 : (!((prect->y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(prect->y))) +
        ((!((prect->width) & ~0x7f)) ? 1 : (!((prect->width) & ~0x3fff)) ? 2 : cmd_size_w((uint)(prect->width))) + ((!((prect->height) & ~0x7f)) ? 1 : (!((prect->height) & ~0x3fff)) ? 2 : cmd_size_w((uint)(prect->height)));
}
static byte *
cmd_put_rect(register const gx_cmd_rect * prect, register byte * dp)
{
    ((!((prect->x) & ~0x7f)) ? (*dp = prect->x, ++dp) : (!((prect->x) & ~0x3fff)) ? (*dp = (prect->x) | 0x80, dp[1] = (prect->x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(prect->x), dp)));
    ((!((prect->y) & ~0x7f)) ? (*dp = prect->y, ++dp) : (!((prect->y) & ~0x3fff)) ? (*dp = (prect->y) | 0x80, dp[1] = (prect->y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(prect->y), dp)));
    ((!((prect->width) & ~0x7f)) ? (*dp = prect->width, ++dp) : (!((prect->width) & ~0x3fff)) ? (*dp = (prect->width) | 0x80, dp[1] = (prect->width) >> 7, dp += 2) : (dp = cmd_put_w((uint)(prect->width), dp)));
    ((!((prect->height) & ~0x7f)) ? (*dp = prect->height, ++dp) : (!((prect->height) & ~0x3fff)) ? (*dp = (prect->height) | 0x80, dp[1] = (prect->height) >> 7, dp += 2) : (dp = cmd_put_w((uint)(prect->height), dp)));
    return dp;
}
                      ...
                      bool extended_command)
{
    ...
    ((pcls->rect).x = x, (pcls->rect).y = y, (pcls->rect).width = width, (pcls->rect).height = height);
    if (extended_command) {
        rcsize = 2 + cmd_size_rect(&pcls->rect);
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, rcsize)) == 0 ? (cldev)->error_code : (*dp = (cmd_opv_extend), 0) );
        dp[1] = op;
        dp += 2;
    } else {
        rcsize = 1 + cmd_size_rect(&pcls->rect);
        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, rcsize)) == 0 ? (cldev)->error_code : (*dp = (op), 0) );
        dp += 1;
    }
    if (code < 0)
        return code;
...
# 101 "./base/gxclrect.c"
    ((pcls->rect).x = x, (pcls->rect).y = y, (pcls->rect).width = width, (pcls->rect).height = height);
    if (dheight == 0 && ((unsigned)(dwidth - (-4)) <= (3 - (-4))) &&
        ((unsigned)(dx - (-8)) <= (7 - (-8)) && (unsigned)(dy - (-8)) <= (7 - (-8)))
        ) {
        byte op_tiny = op + 0x20 + dwidth - (-4);

        if (dx == width - dwidth && dy == 0) {
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 1)) == 0 ? (cldev)->error_code : (*dp = (op_tiny + 8), 0) );
            if (code < 0)
                return code;
        } else {
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 2)) == 0 ? (cldev)->error_code : (*dp = (op_tiny), 0) );
            if (code < 0)
                return code;
            dp[1] = (dx << 4) + dy - ((-8) * 0x11);
        }
    ...
    else if ((((unsigned)(dx - (-128)) <= (127 - (-128)) && (unsigned)(dy - (-128)) <= (127 - (-128))) && ((unsigned)(dwidth - (-128)) <= (127 - (-128))) && (unsigned)(dheight - (-128)) <= (127 - (-128)))) {
        int dh = dheight - (-8);
        ...
        if ((unsigned)dh <= 7 - (-8) &&
            ...
            op += dh;
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 3)) == 0 ? (cldev)->error_code : (*dp = (op + 0x10), 0) );
            if (code < 0)
                return code;
            do { } while (0);

        } else {
            code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, 5)) == 0 ? (cldev)->error_code : (*dp = (op + 0x10), 0) );
            if (code < 0)
                return code;
            do { } while (0);
        ...
        int rcsize = 1 + ((!((x) & ~0x7f)) ? 1 : (!((x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(x))) + ((!((width) & ~0x7f)) ? 1 : (!((width) & ~0x3fff)) ? 2 : cmd_size_w((uint)(width)));

        code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, rcsize)) == 0 ? (cldev)->error_code : (*dp = (op + ((dy + 2) << 2) + dheight + 2), 0) );

        if (code < 0)
            return code;
        ++dp;
        ((!(((x) | (width)) & ~0x7f)) ? (dp[0] = (x), dp[1] = (width), dp += 2) : (dp = cmd_put_w((uint)(width), cmd_put_w((uint)(x), dp))));
    } else {
...
cmd_write_page_rect_cmd(gx_device_clist_writer * cldev, int op)
{
    ...
    int rcsize = 1 + 4 * ((!((0) & ~0x7f)) ? 1 : (!((0) & ~0x3fff)) ? 2 : cmd_size_w((uint)(0)));
    byte *dp;
    ...
    do { } while (0);
    code = ( (dp = cmd_put_range_op(cldev, 0, (cldev)->nbands - 1, rcsize)) == 0 ? (cldev)->error_code : (*dp = (op), 0) );
    if (code < 0)
        return code;
    for (pcls1 = cldev->states; pcls1 < cldev->states + cldev->nbands; pcls1++)
        ((pcls1->rect).x = x, (pcls1->rect).y = y, (pcls1->rect).width = width, (pcls1->rect).height = height);
    ++dp;
    ((!((0) & ~0x7f)) ? (*dp = 0, ++dp) : (!((0) & ~0x3fff)) ? (*dp = (0) | 0x80, dp[1] = (0) >> 7, dp += 2) : (dp = cmd_put_w((uint)(0), dp)));
    ((!((0) & ~0x7f)) ? (*dp = 0, ++dp) : (!((0) & ~0x3fff)) ? (*dp = (0) | 0x80, dp[1] = (0) >> 7, dp += 2) : (dp = cmd_put_w((uint)(0), dp)));
    ((!((0) & ~0x7f)) ? (*dp = 0, ++dp) : (!((0) & ~0x3fff)) ? (*dp = (0) | 0x80, dp[1] = (0) >> 7, dp += 2) : (dp = cmd_put_w((uint)(0), dp)));
    ((!((0) & ~0x7f)) ? (*dp = 0, ++dp) : (!((0) & ~0x3fff)) ? (*dp = (0) | 0x80, dp[1] = (0) >> 7, dp += 2) : (dp = cmd_put_w((uint)(0), dp)));
    do { } while (0);
                    ...
                    const frac31 *c2, const frac31 *c3)
{
    ...
    rcsize = 1 + ((!((left->start.x) & ~0x7f)) ? 1 : (!((left->start.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(left->start.x))) + ((!((left->start.y) & ~0x7f)) ? 1 : (!((left->start.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(left->start.y)))
               + ((!((left->end.x) & ~0x7f)) ? 1 : (!((left->end.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(left->end.x))) + ((!((left->end.y) & ~0x7f)) ? 1 : (!((left->end.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(left->end.y)))
               + ((!((right->start.x) & ~0x7f)) ? 1 : (!((right->start.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(right->start.x))) + ((!((right->start.y) & ~0x7f)) ? 1 : (!((right->start.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(right->start.y)))
               + ((!((right->end.x) & ~0x7f)) ? 1 : (!((right->end.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(right->end.x))) + ((!((right->end.y) & ~0x7f)) ? 1 : (!((right->end.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(right->end.y)))
               + ((!((options) & ~0x7f)) ? 1 : (!((options) & ~0x3fff)) ? 2 : cmd_size_w((uint)(options)));

    if (!(options & 4))
        rcsize += ((!((ybot) & ~0x7f)) ? 1 : (!((ybot) & ~0x3fff)) ? 2 : cmd_size_w((uint)(ybot))) + ((!((ytop) & ~0x7f)) ? 1 : (!((ytop) & ~0x3fff)) ? 2 : cmd_size_w((uint)(ytop)));
    if (options & 2) {
        rcsize += ((!((fa->clip->p.x) & ~0x7f)) ? 1 : (!((fa->clip->p.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(fa->clip->p.x))) + ((!((fa->clip->p.y) & ~0x7f)) ? 1 : (!((fa->clip->p.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(fa->clip->p.y)))
                + ((!((fa->clip->q.x) & ~0x7f)) ? 1 : (!((fa->clip->q.x) & ~0x3fff)) ? 2 : cmd_size_w((uint)(fa->clip->q.x))) + ((!((fa->clip->q.y) & ~0x7f)) ? 1 : (!((fa->clip->q.y) & ~0x3fff)) ? 2 : cmd_size_w((uint)(fa->clip->q.y)));
        if (c0 != ((void*)0)) {
            colors_mask += 1;
        ...
        rcsize += ((!((colors_mask) & ~0x7f)) ? 1 : (!((colors_mask) & ~0x3fff)) ? 2 : cmd_size_w((uint)(colors_mask)));
    }
    code = ( (dp = cmd_put_list_op(cldev, &(pcls)->list, rcsize)) == 0 ? (cldev)->error_code : (*dp = (op), 0) );
    if (code < 0)
        return code;
    dp++;
    ((!((left->start.x) & ~0x7f)) ? (*dp = left->start.x, ++dp) : (!((left->start.x) & ~0x3fff)) ? (*dp = (left->start.x) | 0x80, dp[1] = (left->start.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(left->start.x), dp)));
    ((!((left->start.y) & ~0x7f)) ? (*dp = left->start.y, ++dp) : (!((left->start.y) & ~0x3fff)) ? (*dp = (left->start.y) | 0x80, dp[1] = (left->start.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(left->start.y), dp)));
    ((!((left->end.x) & ~0x7f)) ? (*dp = left->end.x, ++dp) : (!((left->end.x) & ~0x3fff)) ? (*dp = (left->end.x) | 0x80, dp[1] = (left->end.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(left->end.x), dp)));
    ((!((left->end.y) & ~0x7f)) ? (*dp = left->end.y, ++dp) : (!((left->end.y) & ~0x3fff)) ? (*dp = (left->end.y) | 0x80, dp[1] = (left->end.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(left->end.y), dp)));
    ((!((right->start.x) & ~0x7f)) ? (*dp = right->start.x, ++dp) : (!((right->start.x) & ~0x3fff)) ? (*dp = (right->start.x) | 0x80, dp[1] = (right->start.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(right->start.x), dp)));
    ((!((right->start.y) & ~0x7f)) ? (*dp = right->start.y, ++dp) : (!((right->start.y) & ~0x3fff)) ? (*dp = (right->start.y) | 0x80, dp[1] = (right->start.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(right->start.y), dp)));
    ((!((right->end.x) & ~0x7f)) ? (*dp = right->end.x, ++dp) : (!((right->end.x) & ~0x3fff)) ? (*dp = (right->end.x) | 0x80, dp[1] = (right->end.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(right->end.x), dp)));
    ((!((right->end.y) & ~0x7f)) ? (*dp = right->end.y, ++dp) : (!((right->end.y) & ~0x3fff)) ? (*dp = (right->end.y) | 0x80, dp[1] = (right->end.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(right->end.y), dp)));
    ((!((options) & ~0x7f)) ? (*dp = options, ++dp) : (!((options) & ~0x3fff)) ? (*dp = (options) | 0x80, dp[1] = (options) >> 7, dp += 2) : (dp = cmd_put_w((uint)(options), dp)));
    if (!(options & 4)) {
        ((!((ybot) & ~0x7f)) ? (*dp = ybot, ++dp) : (!((ybot) & ~0x3fff)) ? (*dp = (ybot) | 0x80, dp[1] = (ybot) >> 7, dp += 2) : (dp = cmd_put_w((uint)(ybot), dp)));
        ((!((ytop) & ~0x7f)) ? (*dp = ytop, ++dp) : (!((ytop) & ~0x3fff)) ? (*dp = (ytop) | 0x80, dp[1] = (ytop) >> 7, dp += 2) : (dp = cmd_put_w((uint)(ytop), dp)));
    }
    do { } while (0);
    ...
    if (options & 2) {
        ((!((fa->clip->p.x) & ~0x7f)) ? (*dp = fa->clip->p.x, ++dp) : (!((fa->clip->p.x) & ~0x3fff)) ? (*dp = (fa->clip->p.x) | 0x80, dp[1] = (fa->clip->p.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(fa->clip->p.x), dp)));
        ((!((fa->clip->p.y) & ~0x7f)) ? (*dp = fa->clip->p.y, ++dp) : (!((fa->clip->p.y) & ~0x3fff)) ? (*dp = (fa->clip->p.y) | 0x80, dp[1] = (fa->clip->p.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(fa->clip->p.y), dp)));
        ((!((fa->clip->q.x) & ~0x7f)) ? (*dp = fa->clip->q.x, ++dp) : (!((fa->clip->q.x) & ~0x3fff)) ? (*dp = (fa->clip->q.x) | 0x80, dp[1] = (fa->clip->q.x) >> 7, dp += 2) : (dp = cmd_put_w((uint)(fa->clip->q.x), dp)));
        ((!((fa->clip->q.y) & ~0x7f)) ? (*dp = fa->clip->q.y, ++dp) : (!((fa->clip->q.y) & ~0x3fff)) ? (*dp = (fa->clip->q.y) | 0x80, dp[1] = (fa->clip->q.y) >> 7, dp += 2) : (dp = cmd_put_w((uint)(fa->clip->q.y), dp)));
        ((!((colors_mask) & ~0x7f)) ? (*dp = colors_mask, ++dp) : (!((colors_mask) & ~0x3fff)) ? (*dp = (colors_mask) | 0x80, dp[1] = (colors_mask) >> 7, dp += 2) : (dp = cmd_put_w((uint)(colors_mask), dp)));
        if (c0 != ((void*)0))
...
clist_fillpage(gx_device * dev, gs_imager_state *pis, gx_drawing_color *pdcolor)
{
    ...
    if ((code = clist_close_writer_and_init_reader(cldev) < 0) ||
        (code = clist_finish_page(dev, ((bool)1))) < 0)
        return code;;

    pcls = cdev->states;

    do {
        code = cmd_put_drawing_color(cdev, pcls, pdcolor, ((void*)0), devn_not_tile);
        if (code >= 0)
            code = cmd_write_page_rect_cmd(cdev, cmd_op_fill_rect);
    } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
    return code;
}
                     ...
                     gx_color_index color)
{
    ...
    do { if ( rx < 0 ) rwidth += rx, rx = 0; do { if ( rwidth > (cdev)->width - rx ) rwidth = (cdev)->width - rx; } while (0); do { if (ry < cdev->cropping_min) { rheight = ry + rheight - cdev->cropping_min; ry = cdev->cropping_min; } if (ry + rheight > cdev->cropping_max) rheight = cdev->cropping_max - ry; } while (0); } while (0);
    ...
    re.y = ry; re.height = rheight; re.yend = re.y + re.height; re.band_height = cdev->page_info.band_params.BandHeight; re.rect_nbands = (re.yend - re.y + re.band_height - 1) / re.band_height;;
    color_usage = gx_color_index2usage(dev, color);
    do {
        re.band = re.y / re.band_height; re.pcls = cdev->states + re.band; re.band_end = (re.band + 1) * re.band_height; re.height = (((re.band_end) < (re.yend)) ? (re.band_end) : (re.yend)) - re.y;;
        re.pcls->color_usage.or |= color_usage;
        do {
            code = ( (re.pcls)->lop_enabled == ((0) ^ 1) && cmd_put_enable_lop(cdev, re.pcls, 0) < 0 ? (cdev)->error_code : 0 );
        ...
        if (code < 0 && (re.band_code = code))
            goto error_in_rect;
        re.y += re.height;
        continue;
error_in_rect:
        if (!(cdev->error_is_retryable && cdev->driver_call_nesting == 0 &&
                (re.band_code = clist_VMerror_recover_flush(cdev, re.band_code)) >= 0))
            return re.band_code;
    } while (re.y < re.yend);
    return 0;
}
    ...
    const gx_clip_path *pcpath)
{
    ...
    rx = ((int)((rect->p.x) >> (8)));
    ry = ((int)((rect->p.y) >> (8)));
    rwidth = ((int)((rect->q.x) >> (8))) - rx;
    rheight = ((int)((rect->q.y) >> (8))) - ry;
        ...
        do {
            ...
            if (code >= 0) {
                code = cmd_write_rect_hl_cmd(cdev, re.pcls, cmd_op_fill_rect_hl,
                                             rx, re.y, rwidth, re.height, ((bool)0));
            }
        } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
        if (code < 0 && (re.band_code = code))
            goto error_in_rect;
    ...
    const frac31 *c2, const frac31 *c3)
{
    ...
    if (options & 4) {
        if (swap_axes) {
            rx = ((int)(((((((((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) < (right->start.y)) ? ((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) > (fa->clip->p.y)) ? (((((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) < (right->start.y)) ? ((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) : (fa->clip->p.y))) >> (8)));
            rxe = ((int)((((((((((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) > (right->start.y)) ? ((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) < (fa->clip->q.y)) ? (((((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) > (right->start.y)) ? ((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) : (fa->clip->q.y)))+((1<<8)-1)) >> (8)));
            ry = ((int)(((((((((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) < (right->start.x)) ? ((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) > (fa->clip->p.x)) ? (((((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) < (right->start.x)) ? ((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) : (fa->clip->p.x))) >> (8)));
            rheight = ((int)((((((((((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) > (right->start.x)) ? ((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) < (fa->clip->q.x)) ? (((((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) > (right->start.x)) ? ((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) : (fa->clip->q.x)))+((1<<8)-1)) >> (8))) - ry;
        } else {
            rx = ((int)(((((((((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) < (right->start.x)) ? ((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) > (fa->clip->p.x)) ? (((((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) < (right->start.x)) ? ((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) : (fa->clip->p.x))) >> (8)));
            rxe = ((int)((((((((((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) > (right->start.x)) ? ((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) < (fa->clip->q.x)) ? (((((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) > (right->start.x)) ? ((((left->start.x) > (left->end.x)) ? (left->start.x) : (left->end.x))) : (right->start.x))) : (fa->clip->q.x)))+((1<<8)-1)) >> (8)));
            ry = ((int)(((((((((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) < (right->start.y)) ? ((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) > (fa->clip->p.y)) ? (((((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) < (right->start.y)) ? ((((left->start.y) < (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) : (fa->clip->p.y))) >> (8)));
            rheight = ((int)((((((((((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) > (right->start.y)) ? ((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) < (fa->clip->q.y)) ? (((((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) > (right->start.y)) ? ((((left->start.y) > (left->end.y)) ? (left->start.y) : (left->end.y))) : (right->start.y))) : (fa->clip->q.y)))+((1<<8)-1)) >> (8))) - ry;
        }
    } else {
        ...
        if (swap_axes) {
            rx = ((int)((ybot) >> (8)));
            rxe = ((int)(((ytop)+((1<<8)-1)) >> (8)));
            ry = ((int)(((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) >> (8)));
            rheight = ((int)((((((right->start.x) > (right->end.x)) ? (right->start.x) : (right->end.x)))+((1<<8)-1)) >> (8))) - ry;
        } else {
            rx = ((int)(((((left->start.x) < (left->end.x)) ? (left->start.x) : (left->end.x))) >> (8)));
            rxe = ((int)((((((right->start.x) > (right->end.x)) ? (right->start.x) : (right->end.x)))+((1<<8)-1)) >> (8)));
            ry = ((int)((ybot) >> (8)));
            rheight = ((int)(((ytop)+((1<<8)-1)) >> (8))) - ry;
        }
    }
    do { if (ry < cdev->cropping_min) { rheight = ry + rheight - cdev->cropping_min; ry = cdev->cropping_min; } if (ry + rheight > cdev->cropping_max) rheight = cdev->cropping_max - ry; } while (0);
    if (rheight <= 0)
        return 0;
    if (cdev->permanent_error < 0)
        return (cdev->permanent_error);
    ...
    const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
    return clist_write_fill_trapezoid(dev, left, right,
        ybot, ytop, swap_axes, pdcolor, lop, ((void*)0), ((void*)0), ((void*)0), ((void*)0), ((void*)0));
}
        ...
        const frac31 *c2, const frac31 *c3)
{
    ...
    code = clist_write_fill_trapezoid(dev, &left, &right,
        fa->ystart, fa->yend, fa->swap_axes | 2, ((void*)0), fa->lop, fa, c0, c1, c2, c3);
    if (code < 0)
        return code;
# 572 "./base/gxclrect.c"
    return 1;
        ...
        const frac31 *c0, const frac31 *c1, const frac31 *c2)
{
    ...
    left.end = *p1;
    right.start = *p2;
    right.end.x = right.end.y = 0;

    code = clist_write_fill_trapezoid(dev, &left, &right,
        0, 0, fa->swap_axes | 2 | 4, ((void*)0), fa->lop, fa, c0, c1, c2, ((void*)0));
    if (code < 0)
        return code;
# 601 "./base/gxclrect.c"
    return 1;
...
extern int pattern_clist_open_device(gx_device *dev);
...
clist_dev_spec_op(gx_device *pdev, int dev_spec_op, void *data, int size)
{
    ...
    if (dev_spec_op == gxdso_supports_devn) {
        ...
        int code;
        code = ((cdev)->procs.get_profile)((gx_device*) cdev, &dev_profile);
        if (code == 0) {
                           ...
                           const gx_drawing_color *pdcolor1, int px, int py)
{
    ...
    do {
        ...
        if (!((re.pcls)->tile_id == (tile->id) && (offset_temp = cdev->tile_table[(re.pcls)->tile_index].offset) != 0 && ((tile_slot *)(cdev->data + offset_temp))->id == (tile->id))) {
            if (tile->id != 0L) {
                do {
                    code = clist_change_tile(cdev, re.pcls, tile, depth);
                } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
                if (code < 0 && !(code != (-25) || !cdev->error_is_retryable) && (re.band_code = code))
                    goto error_in_rect;
            } else
                code = -1;
            if (code < 0) {
               ...
               gx_color_index color0, gx_color_index color1, int px, int py)
{
    gx_device_clist_writer * const cdev =
        &((gx_device_clist *)dev)->writer;
    int depth =
        (color1 == ((gx_color_index)(~0)) && color0 == ((gx_color_index)(~0)) ?
         cdev->clist_color_info.depth : 1);
    gx_color_usage_bits color_usage =
        (color1 == ((gx_color_index)(~0)) && color0 == ((gx_color_index)(~0)) ?

         (((gx_color_usage_bits)1 << (dev)->color_info.num_components) - 1) :
         (color0 == ((gx_color_index)(~0)) ? 0 : gx_color_index2usage(dev, color0)) |
         (color1 == ((gx_color_index)(~0)) ? 0 : gx_color_index2usage(dev, color1)));
    int code;
    cmd_rects_enum_t re;
                ...
                gx_color_index color0, gx_color_index color1)
{
    ...
    do { do { if ( (rx | ry) < 0 ) { if ( rx < 0 ) rwidth += rx, data_x -= rx, rx = 0; if ( ry < 0 ) rheight += ry, data -= (int)(ry * raster), id = 0L, ry = 0; } if ( rwidth > (dev)->width - rx ) rwidth = (dev)->width - rx; } while (0); if ( rheight > (dev)->height - ry ) rheight = (dev)->height - ry; if ( rwidth <= 0 || rheight <= 0 ) return 0; } while (0);
    y0 = ry;
    if (cdev->permanent_error < 0)
      return (cdev->permanent_error);
    ...
    do {
        ...
        do {
            code = ( (re.pcls)->lop_enabled == ((0) ^ 1) && cmd_put_enable_lop(cdev, re.pcls, 0) < 0 ? (cdev)->error_code : 0 );
            if (code >= 0)
                code = ( (re.pcls)->clip_enabled == ((0) ^ 1) && cmd_put_enable_clip(cdev, re.pcls, 0) < 0 ? (cdev)->error_code : 0 );
            if (color0 != re.pcls->colors[0] && code >= 0)
                code = cmd_put_color(cdev, re.pcls, &clist_select_color0, color0, &(re.pcls)->colors[0]);
            if (color1 != re.pcls->colors[1] && code >= 0)
                code = cmd_put_color(cdev, re.pcls, &clist_select_color1, color1, &(re.pcls)->colors[1]);
        } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
        if (code < 0 && (re.band_code = code))
            goto error_in_rect;
...
copy:{
        ...
        rsize = (dx ? 3 : 1) + cmd_size_rect(&rect) + ((!((0) & ~0x7f)) ? 1 : (!((0) & ~0x3fff)) ? 2 : cmd_size_w((uint)(0)));
        do {
        ...
        if (code < 0 && !(code == (-13)) && (re.band_code = code))
            goto error_in_rect;
        compress = (uint)code;
        if (code < 0) {
        ...
        *dp++ = (op);
        ...
        ((!((0) & ~0x7f)) ? (*dp = 0, ++dp) : (!((0) & ~0x3fff)) ? (*dp = (0) | 0x80, dp[1] = (0) >> 7, dp += 2) : (dp = cmd_put_w((uint)(0), dp)));
        ((!(((rx) | (re.y)) & ~0x7f)) ? (dp[0] = (rx), dp[1] = (re.y), dp += 2) : (dp = cmd_put_w((uint)(re.y), cmd_put_w((uint)(rx), dp))));
        ((!(((w1) | (re.height)) & ~0x7f)) ? (dp[0] = (w1), dp[1] = (re.height), dp += 2) : (dp = cmd_put_w((uint)(re.height), cmd_put_w((uint)(w1), dp))));
        re.pcls->rect = rect;
        }
        continue;
error_in_rect:
...
# 968 "./base/gxclrect.c"
    ...
    do {
        ...
        int maxheight = 4096 / bytes_row / cdev->color_info.num_components;
        ...
        if ((cdev->cend - cdev->cnext) < 0x100 + (re.height * bytes_row * cdev->color_info.num_components))
            cmd_write_buffer(cdev, cmd_opv_end_run);

        rect.x = rx, rect.y = re.y;
        rect.width = w1, rect.height = re.height;
        rsize = (dx ? 3 : 1) + cmd_size_rect(&rect) + ((!((plane_height) & ~0x7f)) ? 1 : (!((plane_height) & ~0x3fff)) ? 2 : cmd_size_w((uint)(plane_height)));
        do {
            int plane;

            code = cmd_put_bits(cdev, re.pcls, row, w1*bpc, re.height, raster,
                                rsize, (bpc == 1 ? (orig_id == 0L ?
                                                    1 << 1 :
                                                    ((1 << 1) | (1 << 2) | (1 << 3))) : 0),
                                &dp, &csize);
            if (code < 0)
            ...
            dp2 = dp;
            if (dx) {
                *dp2++ = (cmd_opv_set_misc);
                *dp2++ = (1 << 6) + dx;
            }
            *dp2++ = (op + code);
            ((!((plane_height) & ~0x7f)) ? (*dp2 = plane_height, ++dp2) : (!((plane_height) & ~0x3fff)) ? (*dp2 = (plane_height) | 0x80, dp2[1] = (plane_height) >> 7, dp2 += 2) : (dp2 = cmd_put_w((uint)(plane_height), dp2)));
            ((!(((rx) | (re.y)) & ~0x7f)) ? (dp2[0] = (rx), dp2[1] = (re.y), dp2 += 2) : (dp2 = cmd_put_w((uint)(re.y), cmd_put_w((uint)(rx), dp2))));
            ((!(((w1) | (re.height)) & ~0x7f)) ? (dp2[0] = (w1), dp2[1] = (re.height), dp2 += 2) : (dp2 = cmd_put_w((uint)(re.height), cmd_put_w((uint)(w1), dp2))));
                 ...
                 int rx, int ry, int rwidth, int rheight)
{
    ...
    gx_color_usage_bits all = (((gx_color_usage_bits)1 << (cdev)->color_info.num_components) - 1);
    cmd_rects_enum_t re;
    ...
    do {
        ...
        if (re.pcls->color_is_alpha) {
            ...
            do {
                code =
                    ( (dp = cmd_put_list_op(cdev, &(re.pcls)->list, 1)) == 0 ? (cdev)->error_code : (*dp = (cmd_opv_set_copy_color), 0) );
            } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
            if (code < 0 && (re.band_code = code))
                goto error_in_rect;
                 ...
                 const gx_drawing_color *pdcolor, int depth)
{
    ...
    if (depth > 1 && (cdev->disable_mask & (1 << 6)) != 0)
        return ((-1));
    ...
    do {
        ...
        if (!re.pcls->color_is_alpha) {
            ...
            do {
                code =
                    ( (dp = cmd_put_list_op(cdev, &(re.pcls)->list, 1)) == 0 ? (cdev)->error_code : (*dp = (cmd_opv_set_copy_alpha), 0) );
            } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
            if (code < 0 && (re.band_code = code))
                goto error_in_rect;
        ...
        if (!re.pcls->color_is_devn) {
            ...
            do {
                code = ( (dp = cmd_put_list_op(cdev, &(re.pcls)->list, 2)) == 0 ? (cdev)->error_code : (*dp = (cmd_opv_extend), 0) );
                 ...
                 gx_color_index color, int depth)
{
    ...
    do {
        ...
        if (re.pcls->color_is_devn) {
            ...
            do {
                code = ( (dp = cmd_put_list_op(cdev, &(re.pcls)->list, 1)) == 0 ? (cdev)->error_code : (*dp = (cmd_opv_extend), 0) );
                code = ( (dp = cmd_put_list_op(cdev, &(re.pcls)->list, 1)) == 0 ? (cdev)->error_code : (*dp = (cmd_opv_ext_unset_color_is_devn), 0) );
...
# 1454 "./base/gxclrect.c"
        if (color != re.pcls->colors[1]) {
            do {
                code = cmd_put_color(cdev, re.pcls, &clist_select_color1, color, &(re.pcls)->colors[1]);
            } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
            if (code < 0 && (re.band_code = code))
                goto error_in_rect;
        }
                     ...
                     uint planar_height)
{
    ...
    gx_color_usage_bits all = (((gx_color_usage_bits)1 << (cdev)->color_info.num_components) - 1);
    bool subtractive = dev->color_info.num_components >= 4;
    ...
    if (scolors != 0 && scolors[0] != scolors[1]) {
        ...
        do { if ( rx < 0 ) rwidth += rx, sourcex -= rx, rx = 0; do { if ( rwidth > (cdev)->width - rx ) rwidth = (cdev)->width - rx; } while (0); do { if (ry < cdev->cropping_min) { rheight = ry + rheight - cdev->cropping_min; sdata += (cdev->cropping_min - ry) * sraster; id = 0L; ry = cdev->cropping_min; } if (ry + rheight > cdev->cropping_max) rheight = cdev->cropping_max - ry; } while (0); } while (0);
    }
    if (rwidth <= 0 || rheight <= 0)
        return 0;
...

    {
        ...
        if (scolors && (scolors[0] == scolors[1]))
            rop_used = (scolors[0] == gx_device_black(dev) ?
                        ( (((rop_used) & (rop3_1 - 0xcc)) << 2) | ((rop_used) & ~0xcc) ) :
                        scolors[0] == gx_device_white(dev) ?
                        ( (((rop_used) & 0xcc) >> 2) | ((rop_used) & ~(rop3_1 - 0xcc)) ) : rop_used);
        if (tcolors && (tcolors[0] == tcolors[1]))
            rop_used = (tcolors[0] == gx_device_black(dev) ?
                        ( (((rop_used) & (rop3_1 - 0xf0)) << 4) | ((rop_used) & ~0xf0) ) :
                        tcolors[0] == gx_device_white(dev) ?
                        ( (((rop_used) & 0xf0) >> 4) | ((rop_used) & ~(rop3_1 - 0xf0)) ) : rop_used);
    ...
    do {
        ...
        if (sizeof(rop_operand) >= sizeof(gx_color_usage_bits)) {
            ...
            D = ((rop_operand)(((re.pcls->color_usage.or)>>(4*sizeof(rop_operand)))>>(4*sizeof(rop_operand))));
            re.pcls->color_usage.or |=
                    ((((gx_color_usage_bits)((rop_proc_table[color_rop])(D, ((rop_operand)(((S)>>(4*sizeof(rop_operand)))>>(4*sizeof(rop_operand)))), ((rop_operand)(((T)>>(4*sizeof(rop_operand)))>>(4*sizeof(rop_operand)))))))<<(4*sizeof(rop_operand)))<<(4*sizeof(rop_operand))) & all;
        ...
        if (( ((((rop) << 4) ^ (rop)) & 0xf0) != 0 )) {
            if (tcolors == 0 || tcolors[0] != tcolors[1]) {
                ...
                if (!((re.pcls)->tile_id == (tiles->id) && (offset_temp = cdev->tile_table[(re.pcls)->tile_index].offset) != 0 && ((tile_slot *)(cdev->data + offset_temp))->id == (tiles->id))) {
...
# 1682 "./base/gxclrect.c"
                        ...
                        ids = gs_next_ids(dev->memory, (((re.height) < (rep_height)) ? (re.height) : (rep_height)));
                        line_tile = *tiles;
                        line_tile.size.y = 1;
                        ...
                        if (tile_space_phase + rwidth <= tiles->rep_width) {
                            ...
                            data_shift = tile_space_phase * depth / 8;
            ...
            do {
                code =
                    (tcolors != 0 ?
                     ...
                     cmd_set_tile_colors(cdev, re.pcls, ((gx_color_index)(~0)),
                                         ((gx_color_index)(~0))));
            } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
            if (code < 0 && (re.band_code = code))
                goto error_in_rect;
        ...
        do {
            ...
            if (code >= 0)
                code = ( (re.pcls)->lop_enabled == ((1) ^ 1) && cmd_put_enable_lop(cdev, re.pcls, 1) < 0 ? (cdev)->error_code : 0 );
        } while ((code < 0 && (code = clist_VMerror_recover(cdev, code)) >= 0));
        if (code < 0 && (re.band_code = code))
            goto error_in_rect;
