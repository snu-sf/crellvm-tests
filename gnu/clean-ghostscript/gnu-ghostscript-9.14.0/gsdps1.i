...
gs_setbbox(gs_state * pgs, double llx, double lly, double urx, double ury)
{
    ...
    if (dbox.p.x < (((-1 << ((1 << 2) * 8 - 1)) + (1L * 3))*(1.0/(1<<8))) ||
        dbox.p.y < (((-1 << ((1 << 2) * 8 - 1)) + (1L * 3))*(1.0/(1<<8))) ||
        dbox.q.x >= (((~(-1 << ((1 << 2) * 8 - 1))) - (1L * 3) + 1L)*(1.0/(1<<8))) ||
        dbox.q.y >= (((~(-1 << ((1 << 2) * 8 - 1))) - (1L * 3) + 1L)*(1.0/(1<<8)))
        )
        return ((-13));
    bbox.p.x =
        (fixed) floor(dbox.p.x * (1<<8)) - (1L * 3);
    bbox.p.y =
        (fixed) floor(dbox.p.y * (1<<8)) - (1L * 3);
    bbox.q.x =
        (fixed) ceil(dbox.q.x * (1<<8)) + (1L * 3);
    bbox.q.y =
        (fixed) ceil(dbox.q.y * (1<<8)) + (1L * 3);
    if (gx_path_bbox_set(ppath, &obox) >= 0) {
...
{
    return gs_rectappend_compat(pgs, pr, count, ((bool)0));
}
...
gs_rectclip(gs_state * pgs, const gs_rect * pr, uint count)
{
    ...
    (void)gx_path_init_local_shared(&save, ((void*)0), pgs->memory);
    gx_path_assign_preserve(&save, pgs->path);
    gs_newpath(pgs);
    if ((code = gs_rectappend_compat(pgs, pr, count, ((bool)1))) < 0 ||
        (code = gs_clip(pgs)) < 0
        ) {
...
gs_rectfill(gs_state * pgs, const gs_rect * pr, uint count)
{
    ...
    bool hl_color = (hl_color_available &&
                ((pdev)->procs.dev_spec_op)(pdev, gxdso_supports_hlcolor,
                                  ((void*)0), 0));
    ...
    if (code != 0)
        return code;
    if ((((pgs->ctm.xy) == 0.0 && (pgs->ctm.yx) == 0.0) ||
         ((pgs->ctm.xx) == 0.0 && (pgs->ctm.yy) == 0.0)) &&
        gx_effective_clip_path(pgs, &pcpath) >= 0 &&
        ((gx_cpath_list(pcpath))->count <= 1) &&
        (hl_color ||
         pdc->type == (&gx_dc_type_data_pure) ||
         pdc->type == (&gx_dc_type_data_ht_binary) ||
         pdc->type == (&gx_dc_type_data_ht_colored)) &&
        (*(((pgs)->color[0].dev_color))->type->load)(((pgs)->color[0].dev_color), (const gs_imager_state *)(pgs), (pgs)->device, gs_color_select_texture) >= 0 &&
        (*((pdev)->procs.get_alpha_bits)) (pdev, go_graphics)
        <= 1 &&
        ...
        for (i = 0; i < count; ++i) {
            ...
            draw_rect.p.x = (((p.x) < (q.x)) ? (p.x) : (q.x));
            draw_rect.p.y = (((p.y) < (q.y)) ? (p.y) : (q.y));
            draw_rect.q.x = (((p.x) > (q.x)) ? (p.x) : (q.x));
            draw_rect.q.y = (((p.y) > (q.y)) ? (p.y) : (q.y));
            if (hl_color) {
                do { if ((clip_rect).p.x > (draw_rect).p.x) (draw_rect).p.x = (clip_rect).p.x; if ((clip_rect).q.x < (draw_rect).q.x) (draw_rect).q.x = (clip_rect).q.x; if ((clip_rect).p.y > (draw_rect).p.y) (draw_rect).p.y = (clip_rect).p.y; if ((clip_rect).q.y < (draw_rect).q.y) (draw_rect).q.y = (clip_rect).q.y; } while (0);
                ...
                if (draw_rect.p.x <= draw_rect.q.x &&
                    ...
                    code = ((pdev)->procs.fill_rectangle_hl_color)(pdev,
                ...
                do { if ((clip_rect).p.x > (draw_rect).p.x) (draw_rect).p.x = (clip_rect).p.x; if ((clip_rect).q.x < (draw_rect).q.x) (draw_rect).q.x = (clip_rect).q.x; if ((clip_rect).p.y > (draw_rect).p.y) (draw_rect).p.y = (clip_rect).p.y; if ((clip_rect).q.y < (draw_rect).q.y) (draw_rect).q.y = (clip_rect).q.y; } while (0);
                if (center_of_pixel) {
                    draw_rect.p.x = (((draw_rect.p.x)+((1<<8)>>1))&(-1L<<8));
                    draw_rect.p.y = (((draw_rect.p.y)+((1<<8)>>1))&(-1L<<8));
                    draw_rect.q.x = (((draw_rect.q.x)+((1<<8)>>1))&(-1L<<8));
                    draw_rect.q.y = (((draw_rect.q.y)+((1<<8)>>1))&(-1L<<8));
                } else {
                    draw_rect.p.x = ((draw_rect.p.x)&(-1L<<8));
                    draw_rect.p.y = ((draw_rect.p.y)&(-1L<<8));
                    draw_rect.q.x = (((draw_rect.q.x)+((1<<8)-1))&(-1L<<8));
                    draw_rect.q.y = (((draw_rect.q.y)+((1<<8)-1))&(-1L<<8));
                }
                x = ((int)((draw_rect.p.x) >> (8)));
                y = ((int)((draw_rect.p.y) >> (8)));
                w = ((int)((draw_rect.q.x) >> (8))) - x;
                h = ((int)((draw_rect.q.y) >> (8))) - y;
                ...
                if ((*(pdc)->type->fill_rectangle)(pdc, x, y, w, h, (pgs)->device, (pgs)->log_op, ((void*)0)) < 0)
                    goto slow;
            }
        }
        return 0;
              ...
              const gs_matrix * pmat)
{
    ...
    if ((code = gs_rectappend(pgs, pr, count)) < 0 ||
        (pmat != ((void*)0) && (code = gs_concat(pgs, pmat)) < 0) ||
