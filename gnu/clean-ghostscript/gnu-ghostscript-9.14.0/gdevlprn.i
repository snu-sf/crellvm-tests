# 1 "./contrib/lips4/gdevlprn.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./contrib/lips4/gdevlprn.c" 2
# 43 "./contrib/lips4/gdevlprn.c"
...
lprn_put_params(gx_device * dev, gs_param_list * plist)
{
    ...
    switch (code = param_read_int(plist,
        ...
        default:
            ecode = code;
          bwidthe:(*(plist)->procs->signal_error)(plist, param_name, ecode = code);
        case 1:
            break;
    }
    ...
    switch (code = param_read_int(plist,
        ...
        default:
            ecode = code;
          crowe:(*(plist)->procs->signal_error)(plist, param_name, ecode = code);
        case 1:
            break;
    }
    ...
    switch (code = param_read_int(plist,
        ...
        default:
            ecode = code;
          bheighte:(*(plist)->procs->signal_error)(plist, param_name, ecode = code);
        case 1:
            break;
    }
...
lprn_print_image(gx_device_printer * pdev, FILE * fp)
{
    ...
    if (!(lprn->ImageBuf = (void *)(*(pdev->memory->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(pdev->memory->non_gc_memory->non_gc_memory, bpl, maxY, "lprn_print_image(ImageBuf)")))
        return ((-25));
    if (!(lprn->TmpBuf = (void *)(*(pdev->memory->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(pdev->memory->non_gc_memory->non_gc_memory, bpl, maxY, "lprn_print_iamge(TmpBuf)")))
        return ((-25));
    if (!(lprn->bubbleTbl = (void *)(*(pdev->memory->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(pdev->memory->non_gc_memory->non_gc_memory, sizeof(Bubble *), maxBx, "lprn_print_image(bubbleTbl)")))
        return ((-25));
    if (!(bubbleBuffer = (void *)(*(pdev->memory->non_gc_memory->non_gc_memory)->procs.alloc_byte_array)(pdev->memory->non_gc_memory->non_gc_memory, sizeof(Bubble), maxBx, "lprn_print_image(bubbleBuffer)")))
        return ((-25));

    for (i = 0; i < maxBx; i++)
        lprn->bubbleTbl[i] = ((void*)0);
    bbtbl = bubbleBuffer;
    ...
    for (y = 0; y < maxBy; y++) {
        if (num_y_blocks + lprn->nBh > maxY) {
            rmin = start_y_block + lprn->nBh;
            for (i = 0; i < maxBx; i++) {
                bbl = lprn->bubbleTbl[i];
                if (bbl != ((void*)0) && bbl->brect.p.y < rmin)
    ...
    lprn_bubble_flush_all(pdev, fp);

    ((pdev->memory->non_gc_memory->non_gc_memory)->procs.free_object(pdev->memory->non_gc_memory->non_gc_memory, lprn->ImageBuf, "lprn_print_image(ImageBuf)"));
    ((pdev->memory->non_gc_memory->non_gc_memory)->procs.free_object(pdev->memory->non_gc_memory->non_gc_memory, lprn->TmpBuf, "lprn_print_iamge(TmpBuf)"));
    ((pdev->memory->non_gc_memory->non_gc_memory)->procs.free_object(pdev->memory->non_gc_memory->non_gc_memory, lprn->bubbleTbl, "lprn_print_image(bubbleTbl)"));
    ((pdev->memory->non_gc_memory->non_gc_memory)->procs.free_object(pdev->memory->non_gc_memory->non_gc_memory, bubbleBuffer, "lprn_print_image(bubbleBuffer)"));
...
lprn_rect_add(gx_device_printer * pdev, FILE * fp, int r, int h, int start, int end)
{
    ...
    if ((bbl = lprn->bubbleTbl[start]) != ((void*)0) &&
        bbl->brect.q.y == y0 - 1 &&
        bbl->brect.p.x == x0 &&
        bbl->brect.q.x == x1) {
        bbl->brect.q.y = y1;
