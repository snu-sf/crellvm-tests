...
zdict(i_ctx_t *i_ctx_p)
{
    ...
    if (op->value.intval < 0)
        return ((-15));
    return dict_alloc(((i_ctx_p->memory).current), (uint) op->value.intval, op);
}
...
zbegin(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*op));
    do { if ( !(((&*(&(&*op)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if ( (((i_ctx_p->dict_stack).stack).p) == (((i_ctx_p->dict_stack).stack).top) ) {
        int code = ref_stack_extend(&((i_ctx_p->dict_stack).stack), 1);
...
{
    if (((&((i_ctx_p->dict_stack).stack))->p + 1 - (&((i_ctx_p->dict_stack).stack))->bot + (&((i_ctx_p->dict_stack).stack))->extension_used) == ((i_ctx_p->dict_stack).min_size)) {

        return ((-4));
    }
    while ((((i_ctx_p->dict_stack).stack).p) == (((i_ctx_p->dict_stack).stack).bot)) {

        ref_stack_pop_block(&((i_ctx_p->dict_stack).stack));
    }
    (((i_ctx_p->dict_stack).stack).p)--;
    dstack_set_top(&(i_ctx_p->dict_stack));;
    return 0;
...
zop_def(i_ctx_t *i_ctx_p)
{
    ...
    switch ((((const byte *)&((op1)->tas.type_attrs))[1])) {
        case t_name: {
            ...
            uint nidx = names_index(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op1);
            uint htemp;

            if ( (((&(i_ctx_p->dict_stack))->top_keys[htemp = (((nidx)) & (((&(i_ctx_p->dict_stack))->top_npairs) - 1)) + 1] == ((ref_packed)(pt_literal_name) << 13) + (nidx)) ? ((pvslot) = (&(i_ctx_p->dict_stack))->top_values + (htemp), 1) : 0) ) {
                if (((int)(avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) <= ((i_ctx_p->dict_stack).def_space)))
                    goto ra;
            }
            break;
            }
    ...
    if (!((int)(avm_space)(((op)->tas.type_attrs) & (((1 << 2) - 1) << 2)) <= ((i_ctx_p->dict_stack).def_space))) {
        do { if ( !(((&*(&(&*(((i_ctx_p->dict_stack).stack).p))->value.pdict->values))->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
    ...
    if (dict_find((((i_ctx_p->dict_stack).stack).p), op1, &pvslot) <= 0)
        return dict_put((((i_ctx_p->dict_stack).stack).p), op1, op, &(i_ctx_p->dict_stack));
ra:
    if ((pvslot->tas.type_attrs & (&i_ctx_p->memory)->test_mask) == 0)
        alloc_save_change((&(i_ctx_p->memory)), &(((i_ctx_p->dict_stack).stack).p)->value.pdict->values, (ref_packed *)pvslot, "dict_put(value)");
    ((void)(((*(pvslot) = *(op)), ((pvslot)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
...
zload(i_ctx_t *i_ctx_p)
{
    ...
    switch ((((const byte *)&((op)->tas.type_attrs))[1])) {
        case t_name:
            ...
            if ((pvalue = dstack_find_name_by_index(&(i_ctx_p->dict_stack), names_index(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op))) == 0)
                return ((-21));
            (*(op) = *(pvalue));
            return 0;
        case t_null:
        ...
        default: {
                ...
                uint size = ref_stack_count(&((i_ctx_p->dict_stack).stack));
                uint i;

                for (i = 0; i < size; i++) {
                    ref *dp = ref_stack_index(&((i_ctx_p->dict_stack).stack), i);

                    do { if ( !(((&*(&(&*dp)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
...
zundef(i_ctx_t *i_ctx_p)
{
    ...
    if (i_ctx_p->in_superexec == 0)
        do { if ( !(((&*(&(&*op1)->value.pdict->values))->tas.type_attrs) & (0x10)) ) return ((-7)); } while (0);
    code = dict_undef(op1, op, &(i_ctx_p->dict_stack));
    if (code < 0 && code != (-21))
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (2));
    return 0;
...
zknown(i_ctx_t *i_ctx_p)
{
    ...
    ((op1)->value.boolval = (code), ((op1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
...
zwhere(i_ctx_t *i_ctx_p)
{
    ...
    ref_stack_enum_begin(&rsenum, &((i_ctx_p->dict_stack).stack));
    do {
        ...
        while (pdref-- > bot) {
            do { if ( !(((&*(&(&*pdref)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
...
zcopy_dict(i_ctx_t *i_ctx_p)
{
    ...
    if (!((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->dict_auto_expand &&
        (dict_length(op) != 0 || dict_maxlength(op) < dict_length(op1))
        )
        return ((-15));
    code = dict_copy_entries(op1, op, ((bool)0), &(i_ctx_p->dict_stack));
    if (code < 0)
        return code;
...
zdictstack(i_ctx_t *i_ctx_p)
{
    ...
    if (((op)->tas.rsize) < count)
        return ((-15));
    if (!(((op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_array) << 8) + (0x10))))
        return ((-7));
    return ref_stack_store(&((i_ctx_p->dict_stack).stack), op, count, 0, 0, ((bool)1), (&(i_ctx_p->memory)),
                           "dictstack");
}
...
{
    uint count2 = ref_stack_counttomark(&((i_ctx_p->op_stack).stack));
    ref rdict;
    int code;
    uint idx;
    ...
    if ((count2 & 1) != 0)
        return ((-15));
    code = dict_alloc(((i_ctx_p->memory).current), count2 >> 1, &rdict);
    if (code < 0)
        return code;
    if ((i_ctx_p->scanner_options & 16) != 0) {
       for (idx = count2; idx > 0; idx -= 2) {
           code = dict_put(&rdict, ref_stack_index(&((i_ctx_p->op_stack).stack), idx - 1), ref_stack_index(&((i_ctx_p->op_stack).stack), idx - 2), &(i_ctx_p->dict_stack));
       ...
       for (idx = 0; idx < count2; idx += 2) {
           code = dict_put(&rdict, ref_stack_index(&((i_ctx_p->op_stack).stack), idx + 1), ref_stack_index(&((i_ctx_p->op_stack).stack), idx), &(i_ctx_p->dict_stack));
    ...
    ref_stack_pop(&((i_ctx_p->op_stack).stack), count2);
    (*((((i_ctx_p->op_stack).stack).p)) = *(&rdict));
    return code;
}
# 420 "./psi/zdict.c"
static int
zforcecopynew(i_ctx_t *i_ctx_p)
{
    ...
    if (!((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->dict_auto_expand)
        return ((-21));
    code = dict_copy_entries(op1, op, ((bool)1), &(i_ctx_p->dict_stack));
    if (code < 0)
        return code;
    (*(op1) = *(op));
    ((((i_ctx_p->op_stack).stack).p) -= (1));
...
zforceundef(i_ctx_t *i_ctx_p)
{
    ...
    dict_undef(op - 1, op, &(i_ctx_p->dict_stack));
    ((((i_ctx_p->op_stack).stack).p) -= (2));
    return 0;
}
...
zsetmaxlength(i_ctx_t *i_ctx_p)
{
    ...
    if (dict_length(op - 1) > new_size)
        return ((-2));
    code = dict_resize(op - 1, new_size, &(i_ctx_p->dict_stack));
    if (code >= 0)
        ((((i_ctx_p->op_stack).stack).p) -= (2));
    return code;
}
