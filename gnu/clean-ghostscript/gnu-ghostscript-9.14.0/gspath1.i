...
gx_setcurrentpoint_from_path(gs_imager_state *pis, gx_path *path)
{
    ...
    pt.x = ((path->position.x)*(1.0/(1<<8)));
    pt.y = ((path->position.y)*(1.0/(1<<8)));
    (pis)->current_point.x = pt.x; (pis)->current_point.y = pt.y;;
                    ...
                    double a1, double a2, bool add)
{
    ...
    return gs_point_transform(p3.x, p3.y, &(*(const gs_matrix *)&(pgs)->ctm), &pgs->current_point);
...
{
    return gs_arc_add_inline(pgs, ((bool)0), xc, yc, r, ang1, ang2, ((bool)1));
}
...
gs_arcn(gs_state * pgs,
        double xc, double yc, double r, double ang1, double ang2)
{
    return gs_arc_add_inline(pgs, ((bool)1), xc, yc, r, ang1, ang2, ((bool)1));
}
...
next_arc_curve(arc_curve_params_t * arc, double anext)
{
    ...
    double trad = arc->radius *
        tan((anext - arc->angle) *
            ((3.14159265358979323846 / 180.0) / 2));
...
next_arc_quadrant(arc_curve_params_t * arc, double anext)
{
    ...
    if (!arc->fast_quadrant) {
        ...
        if (((pis->ctm.xy) == 0.0 && (pis->ctm.yx) == 0.0) ?
            ...
            arc->scaled_radius = ((fixed)((scaled_radius)*(float)(1<<8)));
            arc->quadrant_delta =
                ((fixed)((scaled_radius * 0.55228474983079334)*(float)(1<<8)));
            arc->fast_quadrant = 1;
        } else {
            arc->fast_quadrant = -1;
        }
...
arc_add(const arc_curve_params_t * arc, bool is_quadrant)
{
    ...
    if (!(((path)->state_flags & psf_position_valid) != 0))
        gs_point_transform(arc->p0.x, arc->p0.y, &(*(const gs_matrix *)&(arc->pis)->ctm), &pis->subpath_start);

    code = (arc->action == arc_nothing ?
          (p0.x = path->position.x, p0.y = path->position.y, 0) :
          arc->action == arc_lineto && (((path)->state_flags & psf_position_valid) != 0) ?
          gx_path_add_line_notes(path, p0.x, p0.y, sn_none) :
        ...
        const gs_point *p0, const gs_point *p1, double r)
{
    p[0].x = c->x + p0->x * r;
    p[0].y = c->y + p0->y * r;
    p[1].x = c->x + p0->x * r + p1->x * r * 0.55228474983079334;
    p[1].y = c->y + p0->y * r + p1->y * r * 0.55228474983079334;
    p[2].x = c->x + p0->x * r * 0.55228474983079334 + p1->x * r;
    p[2].y = c->y + p0->y * r * 0.55228474983079334 + p1->y * r;
    p[3].x = c->x + p1->x * r;
    p[3].y = c->y + p1->y * r;
}
...
gs_flattenpath(gs_state * pgs)
{
    ...
    if (!((ppath)->curve_count != 0))
        return 0;
    (void)gx_path_init_local_shared(&fpath, ((void*)0), ppath->memory);
    code = gx_path_copy_reducing(ppath, &fpath, ((fixed)((pgs->flatness)*(float)(1<<8))), ((void*)0), (pgs->accurate_curves ? pco_accurate : pco_none));
...
gs_reversepath(gs_state * pgs)
{
    ...
    (void)gx_path_init_local_shared(&rpath, ((void*)0), ppath->memory);
    ...
    if (pgs->current_point_valid) {
        ...
        (pgs)->current_point.x = ((rpath.position.x)*(1.0/(1<<8))); (pgs)->current_point.y = ((rpath.position.y)*(1.0/(1<<8)));;

        if (rpath.segments->contents.subpath_first != 0) {
            pgs->subpath_start.x = ((rpath.segments->contents.subpath_current->pt.x)*(1.0/(1<<8)));
            pgs->subpath_start.y = ((rpath.segments->contents.subpath_current->pt.y)*(1.0/(1<<8)));
        }
    }
...
gs_upathbbox(gs_state * pgs, gs_rect * pbox, bool include_moveto)
{
    ...
    if ((((pgs->path)->state_flags & ~psf_outside_range) == psf_last_moveto) && include_moveto) {
        gs_fixed_point pt;

        code = ( !pgs->current_point_valid ? ((-14)) : ((&pt)->x = ((fixed)floor((pgs->current_point.x)*(float)(1<<8) + 0.5)), (&pt)->y = ((fixed)floor((pgs->current_point.y)*(float)(1<<8) + 0.5)), 0) );
        if (code < 0)
            return code;
        if (pt.x < fbox.p.x)
            fbox.p.x = pt.x;
    ...
    dbox.p.x = ((fbox.p.x)*(1.0/(1<<8)));
    dbox.p.y = ((fbox.p.y)*(1.0/(1<<8)));
    dbox.q.x = ((fbox.q.x)*(1.0/(1<<8)));
    dbox.q.y = ((fbox.q.y)*(1.0/(1<<8)));
    return gs_bbox_transform_inverse(&dbox, &(*(const gs_matrix *)&(pgs)->ctm), pbox);
}
...
gs_path_enum_copy_init(gs_memory_t *mem, gs_path_enum * penum, const gs_state * pgs, bool copy)
{
    if (copy) {
        gx_path *copied_path =
        gx_path_alloc_shared(((void*)0), mem, "gs_path_enum_init");
        int code;

        if (copied_path == 0)
            return ((-25));
        code = gx_path_copy_reducing(pgs->path, copied_path, (~(-1 << ((1 << 2) * 8 - 1))), ((void*)0), pco_none);
        if (code < 0) {
