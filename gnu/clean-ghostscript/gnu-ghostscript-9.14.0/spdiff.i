...
static const gs_memory_struct_type_t st_PDiff_state = { sizeof(stream_PDiff_state), "PixelDifferenceEncode/Decode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
# 37 "./base/spdiff.c"
static void
...
row:
    if (ss->row_left == 0) {
        ...
        s0 = 0;
        memset(&ss->prev[1], 0, sizeof(uint) * (60 - 1));
    }
    {
...
# 169 "./base/spdiff.c"
        ...
        case 0 + 0 + 2:
            if (colors < 8) {
                ...
                for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = t ^ ((s0 << cshift) | (t >> colors)), s0 = t); break;
            } else if (colors & 7) {
                int shift = colors & 7;
                int cshift = 8 - shift;

                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p; t = (s0 << (cshift)) | (ss->prev[ci] >> (shift)); *++q = *p ^ t; s0 = ss->prev[ci]; ss->prev[ci] = *p; } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, *++q = t ^ ss->prev[ci], ss->prev[ci] = t; } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 0 + 0 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = t ^ ((s0 << 7) | (t >> 1)), s0 = t); break;
        case 0 + 0 + 3:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = t ^ ((s0 << 5) | (t >> 3)), s0 = t); break;
        case 0 + 0 + 4:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = t ^ ((s0 << 4) | (t >> 4)), s0 = t); break;
# 219 "./base/spdiff.c"
        case 25 + 0 + 0:
            if (colors < 8) {
                int cshift = 8 - colors;

                for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p ^ (s0 << cshift), s0 = *q = t ^ (t >> colors)); break;
            } else if (colors & 7) {
                int shift = colors & 7;
                int cshift = 8 - shift;

                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p, ++q; t = (s0 << (cshift)) | (ss->prev[ci] >> (shift)); s0 = ss->prev[ci]; ss->prev[ci] = *q = *p ^ t; } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, ss->prev[ci] = *++q = t ^ ss->prev[ci]; } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 25 + 0 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p ^ (s0 << 7), s0 = *q = (t ^= t >> 1, t ^= t >> 2, t ^ (t >> 4))); break;

        case 25 + 0 + 2:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p ^ (s0 << 6), s0 = *q = (t ^= (t >> 2), t ^ (t >> 4))); break;

        case 25 + 0 + 3:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p ^ (s0 << 5), s0 = *q = t ^ (t >> 3) ^ (t >> 6)); break;

        case 25 + 0 + 4:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p ^ (s0 << 4), s0 = *q = t ^ (t >> 4)); break;
# 253 "./base/spdiff.c"
        case 0 + 5 + 0:
            if (colors & 7) {
                ...
                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p; t = (s0 << (cshift)) | (ss->prev[ci] >> (shift)); *++q = ( ((~(*p) & (t) & 0x55) << 1) ^ (*p) ^ (t) ); s0 = ss->prev[ci]; ss->prev[ci] = *p; } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, *++q = ( ((~(t) & (ss->prev[ci]) & 0x55) << 1) ^ (t) ^ (ss->prev[ci]) ), ss->prev[ci] = t; } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 0 + 5 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = (s0 = (s0 << 6) | (t >> 2), ( ((~(t) & (s0) & 0x55) << 1) ^ (t) ^ (s0) )), s0 = t); break;
        case 0 + 5 + 2:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = (s0 = (s0 << 4) | (t >> 4), ( ((~(t) & (s0) & 0x55) << 1) ^ (t) ^ (s0) )), s0 = t); break;
        case 0 + 5 + 3:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = (s0 = (s0 << 2) | (t >> 6), ( ((~(t) & (s0) & 0x55) << 1) ^ (t) ^ (s0) )), s0 = t); break;
        case 0 + 5 + 4:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = ( ((~(t) & (s0) & 0x55) << 1) ^ (t) ^ (s0) ), s0 = t); break;
        ...
        case 25 + 5 + 0:
            if (colors & 7) {
                ...
                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p, ++q; t = (s0 << (cshift)) | (ss->prev[ci] >> (shift)); s0 = ss->prev[ci]; ss->prev[ci] = *q = ( (((*p) & (t) & 0x55) << 1) ^ (*p) ^ (t) ); } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, ss->prev[ci] = *++q = ( (((t) & (ss->prev[ci]) & 0x55) << 1) ^ (t) ^ (ss->prev[ci]) ); } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 25 + 5 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p + (s0 << 6), s0 = *q = (t = ( (((t >> 2) & (t) & 0x55) << 1) ^ (t >> 2) ^ (t) ), ( (((t >> 4) & (t) & 0x55) << 1) ^ (t >> 4) ^ (t) ))); break;

        case 25 + 5 + 2:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, s0 = *q = (t = ( (((t) & (s0 << 4) & 0x55) << 1) ^ (t) ^ (s0 << 4) ), ( (((t >> 4) & (t) & 0x55) << 1) ^ (t >> 4) ^ (t) ))); break;
        case 25 + 5 + 3:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, s0 = *q = (t = ( (((t) & (s0 << 2) & 0x55) << 1) ^ (t) ^ (s0 << 2) ), ( (((t >> 6) & (t) & 0x55) << 1) ^ (t >> 6) ^ (t) ))); break;
        case 25 + 5 + 4:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, s0 = *q = ( (((t) & (s0) & 0x55) << 1) ^ (t) ^ (s0) )); break;
# 304 "./base/spdiff.c"
        case 0 + 10 + 0:
        case 0 + 10 + 2:
    enc4:
            if (colors & 1) {
                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p; t = (s0 << (4)) | (ss->prev[ci] >> (4)); *++q = ( (((*p) - (t)) & 0xf) + ((*p) & 0xf0) - ((t) & 0xf0) ); s0 = ss->prev[ci]; ss->prev[ci] = *p; } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, *++q = ( (((t) - (ss->prev[ci])) & 0xf) + ((t) & 0xf0) - ((ss->prev[ci]) & 0xf0) ), ss->prev[ci] = t; } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 0 + 10 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = ((t - (s0 << 4)) & 0xf0) | ((t - (t >> 4)) & 0xf), s0 = t); break;

        case 0 + 10 + 3: {
            uint s1 = ss->prev[1];

            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p, *q = ((t - (s0 << 4)) & 0xf0) | ((t - (s1 >> 4)) & 0xf), s0 = s1, s1 = t);
        ...
        case 0 + 10 + 4: {
            ...
            for (; count >= 2; count -= 2) p += 2, q += 2, (t = p[-1], q[-1] = ( (((t) - (s0)) & 0xf) + ((t) & 0xf0) - ((s0) & 0xf0) ), s0 = t, t = *p, *q = ( (((t) - (s1)) & 0xf) + ((t) & 0xf0) - ((s1) & 0xf0) ), s1 = t);
    ...
    dec4:
            if (colors & 1) {
                do { for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 1; ci <= ndone; ++ci) { ++p, ++q; t = (s0 << (4)) | (ss->prev[ci] >> (4)); s0 = ss->prev[ci]; ss->prev[ci] = *q = ( (((*p) + (t)) & 0xf) + ((*p) & 0xf0) + ((t) & 0xf0) ); } } } while (0);
            } else {
                do { ss->prev[0] = s0; for (; count >= final; count -= ndone) { ndone = (((count) < (nb)) ? (count) : (nb)); for (ci = 0; ci < ndone; ++ci) t = *++p, ss->prev[ci] = *++q = ( (((t) + (ss->prev[ci])) & 0xf) + ((t) & 0xf0) + ((ss->prev[ci]) & 0xf0) ); } s0 = ss->prev[0]; } while (0);
            }
            break;

        case 25 + 10 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (t = *p + (s0 << 4), s0 = *q = ( (((t) + ((t) >> 4)) & 0xf) + ((t) & 0xf0) )); break;

        case 25 + 10 + 3: {
            uint s1 = ss->prev[1];

            for (; count >= 1; count -= 1) p += 1, q += 1, (t = (s0 << 4) + (s1 >> 4), s0 = s1, s1 = *q = ( (((*p) + (t)) & 0xf) + ((*p) & 0xf0) + ((t) & 0xf0) ));
        ...
        case 25 + 10 + 4: {
            ...
            for (; count >= 2; count -= 2) p += 2, q += 2, (t = p[-1], s0 = q[-1] = ( (((s0) + (t)) & 0xf) + ((s0) & 0xf0) + ((t) & 0xf0) ), t = *p, s1 = *q = ( (((s1) + (t)) & 0xf) + ((s1) & 0xf0) + ((t) & 0xf0) ));
...
# 378 "./base/spdiff.c"
    ...
    dec8:
        ...
        case 0 + 15 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, (q[0] = p[0] - s0, s0 = p[0]);
            break;

        case 25 + 15 + 1:
            for (; count >= 1; count -= 1) p += 1, q += 1, q[0] = s0 += p[0];
            break;

        case 0 + 15 + 3: {
            uint s1 = ss->prev[1], s2 = ss->prev[2];

            for (; count >= 3; count -= 3) p += 3, q += 3, ((q[-2] = p[-2] - s0, s0 = p[-2]), (q[-1] = p[-1] - s1, s1 = p[-1]), (q[0] = p[0] - s2, s2 = p[0]));
        ...
        case 25 + 15 + 3: {
            ...
            for (; count >= 3; count -= 3) p += 3, q += 3, (q[-2] = s0 += p[-2], q[-1] = s1 += p[-1], q[0] = s2 += p[0]);
        ...
        case 0 + 15 + 4: {
            ...
            for (; count >= 4; count -= 4) p += 4, q += 4, ((q[-3] = p[-3] - s0, s0 = p[-3]), (q[-2] = p[-2] - s1, s1 = p[-2]), (q[-1] = p[-1] - s2, s2 = p[-1]), (q[0] = p[0] - s3, s3 = p[0]));
        ...
        case 25 + 15 + 4: {
            ...
            for (; count >= 4; count -= 4) p += 4, q += 4, (q[-3] = s0 += p[-3], q[-2] = s1 += p[-2], q[-1] = s2 += p[-1], q[0] = s3 += p[0]);
...
# 461 "./base/spdiff.c"
    ...
    dec16:
        ...
        case 0 + 20 + 1:
           for (; count >= 2; count -= 2) p += 2, q += 2, (ti = ((p[0 -1] << 8) + p[0]), s0 = ti - s0, q[0 -1] = s0 >> 8, q[0] = s0 & 0xff, s0 = ti);
           break;

        case 25 + 20 + 1:
           for (; count >= 2; count -= 2) p += 2, q += 2, (s0 = 0xffff & (s0 + ((p[0 -1] << 8) + p[0])), q[0 -1] = s0 >> 8, q[0] = s0 & 0xff);
           break;

        case 0 + 20 + 3: {
            uint s1 = ss->prev[1], s2 = ss->prev[2];

            for (; count >= 6; count -= 6) p += 6, q += 6, ((ti = ((p[-4 -1] << 8) + p[-4]), s0 = ti - s0, q[-4 -1] = s0 >> 8, q[-4] = s0 & 0xff, s0 = ti), (ti = ((p[-2 -1] << 8) + p[-2]), s1 = ti - s1, q[-2 -1] = s1 >> 8, q[-2] = s1 & 0xff, s1 = ti), (ti = ((p[0 -1] << 8) + p[0]), s2 = ti - s2, q[0 -1] = s2 >> 8, q[0] = s2 & 0xff, s2 = ti));
        ...
        case 25 + 20 + 3: {
            ...
            for (; count >= 6; count -= 6) p += 6, q += 6, ((s0 = 0xffff & (s0 + ((p[-4 -1] << 8) + p[-4])), q[-4 -1] = s0 >> 8, q[-4] = s0 & 0xff), (s1 = 0xffff & (s1 + ((p[-2 -1] << 8) + p[-2])), q[-2 -1] = s1 >> 8, q[-2] = s1 & 0xff), (s2 = 0xffff & (s2 + ((p[0 -1] << 8) + p[0])), q[0 -1] = s2 >> 8, q[0] = s2 & 0xff));
        ...
        case 0 + 20 + 4: {
            ...
            for (; count >= 8; count -= 8) p += 8, q += 8, ((ti = ((p[-6 -1] << 8) + p[-6]), s0 = ti - s0, q[-6 -1] = s0 >> 8, q[-6] = s0 & 0xff, s0 = ti), (ti = ((p[-4 -1] << 8) + p[-4]), s1 = ti - s1, q[-4 -1] = s1 >> 8, q[-4] = s1 & 0xff, s1 = ti), (ti = ((p[-2 -1] << 8) + p[-2]), s2 = ti - s2, q[-2 -1] = s2 >> 8, q[-2] = s2 & 0xff, s2 = ti), (ti = ((p[0 -1] << 8) + p[0]), s3 = ti - s3, q[0 -1] = s3 >> 8, q[0] = s3 & 0xff, s3 = ti));
        ...
        case 25 + 20 + 4: {
            ...
            for (; count >= 8; count -= 8) p += 8, q += 8, ((s0 = 0xffff & (s0 + ((p[-6 -1] << 8) + p[-6])), q[-6 -1] = s0 >> 8, q[-6] = s0 & 0xff), (s1 = 0xffff & (s1 + ((p[-4 -1] << 8) + p[-4])), q[-4 -1] = s1 >> 8, q[-4] = s1 & 0xff), (s2 = 0xffff & (s2 + ((p[-2 -1] << 8) + p[-2])), q[-2 -1] = s2 >> 8, q[-2] = s2 & 0xff), (s3 = 0xffff & (s3 + ((p[0 -1] << 8) + p[0])), q[0 -1] = s3 >> 8, q[0] = s3 & 0xff));
...
const stream_template s_PDiffE_template = {
    &st_PDiff_state, s_PDiffE_init, s_PDiff_process, 1, 1, ((void*)0),
    s_PDiff_set_defaults, s_PDiff_reinit
};
const stream_template s_PDiffD_template = {
    &st_PDiff_state, s_PDiffD_init, s_PDiff_process, 1, 1, ((void*)0),
    s_PDiff_set_defaults, s_PDiff_reinit
};
