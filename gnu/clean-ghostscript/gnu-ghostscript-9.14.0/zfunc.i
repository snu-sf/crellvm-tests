...
make_function_proc(i_ctx_t *i_ctx_p, ref *op, gs_function_t *pfn)
{
    ...
    code = gs_alloc_ref_array(((i_ctx_p->memory).current), &cref, 0x80 | 0x40, 2, ".buildfunction");
...
zbuildfunction(i_ctx_t *i_ctx_p)
{
    ...
    gs_function_t *pfn;
    int code = fn_build_function(i_ctx_p, op, &pfn, ((gs_memory_t *)((i_ctx_p->memory).current)), 0, 0);

    if (code < 0)
        return code;
    code = make_function_proc(i_ctx_p, op, pfn);
    if (code < 0)
        ((pfn)->head.procs.free(pfn, ((bool)1), ((gs_memory_t *)((i_ctx_p->memory).current))));
    return 0;
}
...
# 158 "./psi/zfunc.c"
    if (!(((op)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) ||
        !((((op)->tas.type_attrs) & (0x80 | (0x10 + 0x20 +0x40))) == (0x80 | 0x40))
        )
        return ((-20));
    {
        ...
        if (diff > 0)
            if ( (((i_ctx_p->op_stack).stack).top) - (((i_ctx_p->op_stack).stack).p) < (diff) ) { ((i_ctx_p->op_stack).stack).requested = (diff); return ((-16)); };
        {
            float params[20];
            float *in;
            float *out;
            ...
            if (m + n <= (sizeof(params) / sizeof((params)[0]))) {
                in = params;
            } else {
                in = (float *)(*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_byte_array)(((gs_memory_t *)((i_ctx_p->memory).current)), m + n, sizeof(float), "%execfunction(in/out)");
            ...
            if (code < 0 ||
                ...
                (code = ((pfn)->head.procs.evaluate)(pfn, in, out)) < 0
                )
                do { } while (0);
            else {
                if (diff > 0)
                    do { if ( (op += (diff)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (diff); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
                else if (diff < 0) {
                    ((((i_ctx_p->op_stack).stack).p) -= (-diff));
                    op = (((i_ctx_p->op_stack).stack).p);
                }
                code = make_floats(op + 1 - n, out, n);
            }
            if (in != params)
                ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), in, "%execfunction(in)"));
            return code;
        }
    }
}
...
zisencapfunction(i_ctx_t *i_ctx_p)
{
    ...
    pfn = ref_function(op);
    ((op)->value.boolval = (pfn != ((void*)0)), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
    ...
    int depth, gs_memory_t *mem, const float *shading_domain, const int num_inputs)
{
    ...
    code = dict_int_param(op, "FunctionType", 0, (~(-1 << ((1 << 2) * 8 - 1))), -1, &type);
    if (code < 0)
        return code;
    ...
    params.Domain = 0;
    params.Range = 0;
    code = fn_build_float_array(op, "Domain", ((bool)1), ((bool)1), &params.Domain, mem);
    if (code < 0) {
    ...
    code = fn_build_float_array(op, "Range", ((bool)0), ((bool)1), &params.Range, mem);
    if (code < 0)
        goto fail;
    params.n = code >> 1;
                     ...
                     bool even, const float **pparray, gs_memory_t *mem)
{
    ...
    *pparray = 0;
    if (dict_find_string(op, kstr, &par) <= 0)
        return (required ? ((-15)) : 0);
    if (!(((par)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return ((-20));
    {
        uint size = ((par)->tas.rsize);
        float *ptr = (float *)
            (*(mem)->procs.alloc_byte_array)(mem, size, sizeof(float), kstr);
        ...
        if (code < 0 || (even && (code & 1) != 0)) {
            ((mem)->procs.free_object(mem, ptr, kstr));
                     ...
                     const float **pparray, gs_memory_t *mem)
{
    ...
    *pparray = 0;
    ...
    if( (((par)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))) )
        size = ((par)->tas.rsize);
    else if((((const byte *)&((par)->tas.type_attrs))[1]) == t_integer || (((const byte *)&((par)->tas.type_attrs))[1]) == t_real)
        size = 1;
    else
        return ((-20));
    ptr = (float *)(*(mem)->procs.alloc_byte_array)(mem, size, sizeof(float), kstr);
...
{
    if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_array)) &&
        ((((op)->tas.type_attrs) & (0x80 | (0x10 + 0x20 +0x40))) == (0x80 | 0x40)) &&

        ((op)->tas.rsize) == 2 &&
        (((op->value.refs + 1)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x80))) == (((t_operator) << 8) + (0x80))) &&
        op->value.refs[1].value.opproc == zexecfunction &&
        (((op->value.refs)->tas.type_attrs & ((((1 << 6) - (2)) << 8) + (0))) == (((t_struct) << 8) + (0))) &&
        ((((op->value.refs)->tas.type_attrs) & (0x80 | (0x10 + 0x20 +0x40))) == (0x80 | 0x40))
