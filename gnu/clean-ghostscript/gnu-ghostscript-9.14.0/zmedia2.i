...
zmatchmedia(i_ctx_t *i_ctx_p)
{
    ...
    os_ptr pkeys = op;
    int policy_default;
    float best_mismatch = (float)(~(-1L << ((1 << 3) * 8 - 1)));
    float mepos_penalty;
    ...
    if (((((const byte *)&((pattr)->tas.type_attrs))[1]) == (t_null))) {
        if ( op < (((i_ctx_p->op_stack).stack).bot) + ((4) - 1) ) return ((-17));
        (((op - 3)->tas.type_attrs = ((t_null) << 8) + (0)) );
        ((op - 2)->value.boolval = (1), ((op - 2)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
        ((((i_ctx_p->op_stack).stack).p) -= (2));
        return 0;
    }
    if ( !((((const byte *)&((&*preq)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*preq));
    do { if ( !(((&*(&(&*preq)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if ( !((((const byte *)&((&*pattr)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*pattr));
    do { if ( !(((&*(&(&*pattr)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    if ( !((((const byte *)&((&*ppol)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*ppol));
    do { if ( !(((&*(&(&*ppol)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    do { if ( !((((const byte *)&((&*pkeys)->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&*pkeys)); } while (0);
    do { if ( !(((&*pkeys)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    ...
    code = dict_bool_param(preq, "RollFedMedia", ((bool)0), &roll);
    if (code < 0)
        return code;
    ...
    if (dict_find_string(pattr, "Priority", &ppriority) > 0) {
        do { if ( !((((const byte *)&((&*ppriority)->tas.type_attrs))[1]) == (t_array)) ) return ((-20)); } while (0);
        do { if ( !(((&*ppriority)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    } else {
        ((&no_priority)->value.refs = ((ref *)((void*)0)), ((&no_priority)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40))), ((&no_priority)->tas.rsize = (0)));
        ppriority = &no_priority;
    }
    match.no_match_priority = ((ppriority)->tas.rsize);
    reset_match(&match);
    for (ai = dict_first(pattr);
         (ai = dict_next(pattr, ai, (ref * )&aelt)) >= 0;
         ) {
        if (((((const byte *)&((&aelt.dict)->tas.type_attrs))[1]) == (t_dictionary)) &&
            ((((&(&aelt.dict)->value.pdict->values))->tas.type_attrs) & (0x20)) &&
            ((((const byte *)&((&aelt.key)->tas.type_attrs))[1]) == (t_integer))
            ) {
            bool match_all;
            uint ki, pi;

            code = dict_bool_param(&aelt.dict, "MatchAll", ((bool)0),
                                   &match_all);
            if (code < 0)
                return code;
            for (ki = 0; ki < ((pkeys)->tas.rsize); ki++) {
                ref key;
                ref kstr;
                ref *prvalue;
                ref *pmvalue;
                ...
                array_get(((gs_memory_t *)((i_ctx_p->memory).current)), pkeys, ki, &key);
                if (dict_find(&aelt.dict, &key, &pmvalue) <= 0)
                    continue;
                if (dict_find(preq, &key, &prvalue) <= 0 ||
                    ((((const byte *)&((prvalue)->tas.type_attrs))[1]) == (t_null))
                    ) {
                    if (match_all)
                        goto no;
                    else
                ...
                if (dict_find(ppol, &key, &ppvalue) > 0) {
                    do { if ( !((((const byte *)&((&*ppvalue)->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0);
                ...
                if (((((const byte *)&((&key)->tas.type_attrs))[1]) == (t_name)) &&
                    (names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, &key, &kstr),
                     ((&kstr)->tas.rsize) == 8 &&
                    ...
                    if (zmatch_page_size(((gs_memory_t *)((i_ctx_p->memory).current)), prvalue, pmvalue,
                                         policy, orient, roll,
                                         &best_mismatch,
                                         &ignore_mat,
                                         &ignore_msize)
                        <= 0)
                        goto no;
                } else if (!obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), prvalue, pmvalue))
                    goto no;
            }
            ...
            if (((((const byte *)&((&match.match_key)->tas.type_attrs))[1]) == (t_null)))
            ...
            if (best_mismatch + mepos_penalty <= mbest) {
                if (best_mismatch + mepos_penalty < mbest ||
                    (((((const byte *)&((&match.match_key)->tas.type_attrs))[1]) == (t_integer)) &&
            ...
            for (pi = match.priority; pi > 0;) {
                ...
                pi--;
                array_get(((gs_memory_t *)((i_ctx_p->memory).current)), ppriority, pi, &pri);
                if (obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), &aelt.key, &pri)) {
...
no:;
    ...
    if (((((const byte *)&((&match.match_key)->tas.type_attrs))[1]) == (t_null))) {
        ((op - 3)->value.boolval = (0), ((op - 3)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
        ((((i_ctx_p->op_stack).stack).p) -= (3));
    } else {
        if (((((const byte *)&((&match.best_key)->tas.type_attrs))[1]) == (t_null)))
...
zmatchpagesize(i_ctx_t *i_ctx_p)
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    gs_matrix mat;
    float ignore_mismatch = (float)(~(-1L << ((1 << 3) * 8 - 1)));
    gs_point media_size;
    int orient;
    bool roll;
    int code;
    ...
    if (((((const byte *)&((op - 2)->tas.type_attrs))[1]) == (t_null)))
        orient = -1;
    else {
        do { if ( !((((const byte *)&((&op[-2])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-2])); if ( (ulong)(op[-2]).value.intval > (3) ) return ((-15)); } while (0);
    ...
    code = zmatch_page_size(((gs_memory_t *)((i_ctx_p->memory).current)),
    ...
    switch (code) {
        default:
            return code;
        case 0:
            ((op - 5)->value.boolval = (0), ((op - 5)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
            ((((i_ctx_p->op_stack).stack).p) -= (5));
            break;
        case 1:
            code = write_matrix_in(op, &mat, (&(i_ctx_p->memory)), ((void*)0));
            if (code < 0 && !((((const byte *)&((op)->tas.type_attrs))[1]) == (t_null)))
                return code;
            op[-5] = *op;
            ((op - 4)->value.realval = (media_size.x), ((op - 4)->tas.type_attrs = ((t_real) << 8) + (0)) );
            ((op - 3)->value.realval = (media_size.y), ((op - 3)->tas.type_attrs = ((t_real) << 8) + (0)) );
            ((op - 2)->value.boolval = (1), ((op - 2)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
            ((((i_ctx_p->op_stack).stack).p) -= (2));
            break;
    }
...
# 372 "./psi/zmedia2.c"
        if ( fit_direct && fit_rotated) {
            make_adjustment_matrix(request, medium, pmat, ((bool)0), orient < 0 ? 0 : orient);
            if (medium->p.x < medium->q.x || medium->p.y < medium->q.y)
                *best_mismatch = (float)0.001;
            else
            ...
            make_adjustment_matrix(request, medium, pmat, ((bool)0), (rotate + 1) & 2);
            *best_mismatch = fabs((medium->p.x - rx) * (medium->q.x - rx)) +
                                fabs((medium->p.y - ry) * (medium->q.y - ry)) +
                                    (pmat->xx == 0.0 || (rotate & 1) == 1 ? 0.01 : 0);
        } else if ( fit_rotated ) {
            int rotate = (orient < 0 ? 1 : orient);

            make_adjustment_matrix(request, medium, pmat, ((bool)0), rotate | 1);
            *best_mismatch = fabs((medium->p.y - rx) * (medium->q.y - rx)) +
            ...
            if (adjust)
                ...
                make_adjustment_matrix(request, &req_rect, pmat, ((bool)0), rotate);
            }
        ...
        pmsize->x = (rx < medium->p.x ? medium->p.x : rx > medium->q.x ? medium->q.x : rx);
        pmsize->y = (ry < medium->p.y ? medium->p.y : ry > medium->q.y ? medium->q.y : ry);
