...
# 32 "./base/gxcht.c" 2
# 53 "./base/gxcht.c"
static const gc_ptr_element_t dc_ht_colored_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gx_device_color *)0)->colors.colored.c_ht - (char *)((gx_device_color *)0) )) } }; static const gc_struct_data_t dc_ht_colored_reloc_ptrs = { (sizeof(dc_ht_colored_enum_ptrs) / sizeof((dc_ht_colored_enum_ptrs)[0])), 0, 0, dc_ht_colored_enum_ptrs }; static const gs_memory_struct_type_t st_dc_ht_colored = { sizeof(gx_device_color), "dc_ht_colored", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &dc_ht_colored_reloc_ptrs };

static void gx_dc_ht_colored_save_dc(const gx_device_color * pdevc, gx_device_color_saved * psdc);
static const gx_device_halftone * gx_dc_ht_colored_get_dev_halftone(const gx_device_color * pdevc);
...
static bool gx_dc_ht_colored_equal(const gx_device_color *pdevc1, const gx_device_color *pdevc2);
static int gx_dc_ht_colored_write(const gx_device_color *pdevc, const gx_device_color_saved *psdc, const gx_device * dev, int64_t offset, byte *data, uint *psize);
static int gx_dc_ht_colored_read(gx_device_color *pdevc, const gs_imager_state * pis, const gx_device_color *prior_devc, const gx_device * dev, int64_t offset, const byte *data, uint size, gs_memory_t *mem);
    ...
    uint * psize )
{
    ...
    if ( psdc == 0 ||
        ...
        if (num_comps > 8 * sizeof(uint)) {
            ...
            req_size += ( (uint)(tmp_mask) < (1U << 7) ? 1 : (uint)(tmp_mask) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(tmp_mask) );
        ...
        for (i = 0, comp_bit = 0x1; i < num_comps; i++, comp_bit <<= 1) {
            if ((plane_mask & comp_bit) != 0)
                req_size += ( (uint)(pdevc->colors.colored.c_level[i]) < (1U << 7) ? 1 : (uint)(pdevc->colors.colored.c_level[i]) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(pdevc->colors.colored.c_level[i]) );
        }
    }
    ...
    if (psdc == 0 || alpha != psdc->colors.colored.alpha) {
        if (alpha == ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)))
            ...
            req_size += ( (uint)(alpha) < (1U << 7) ? 1 : (uint)(alpha) < (1U << (2 * 7)) ? 2 : enc_u_size_uint(alpha) );
        }
    }
    ...
    *pdata++ = (byte)flag_bits;
    ...
    if ((flag_bits & dc_ht_colored_has_level) != 0) {
        ...
        if (num_comps > 8 * sizeof(uint)) {
            tmp_mask = (uint)plane_mask;
            do { if ((uint)(tmp_mask) < (1U << 7)) *(pdata)++ = (byte)(tmp_mask); else if ((uint)(tmp_mask) < (1U << (2 * 7))) { *(pdata)++ = (1U << 7) | ((tmp_mask) & ((1U << 7) - 1)); *(pdata)++ = (tmp_mask) >> 7; } else (pdata) = enc_u_put_uint((tmp_mask), (pdata)); } while (0);
            tmp_mask = (uint)((plane_mask >> (8*sizeof(uint)-1))>>1);
            do { if ((uint)(tmp_mask) < (1U << 7)) *(pdata)++ = (byte)(tmp_mask); else if ((uint)(tmp_mask) < (1U << (2 * 7))) { *(pdata)++ = (1U << 7) | ((tmp_mask) & ((1U << 7) - 1)); *(pdata)++ = (tmp_mask) >> 7; } else (pdata) = enc_u_put_uint((tmp_mask), (pdata)); } while (0);
        } else {
            tmp_mask = (uint)plane_mask;
        ...
        for (i = 0, code_bit = 0x1; i < num_comps; i++, code_bit <<= 1) {
            if ((plane_mask & code_bit) != 0)
                do { if ((uint)(pdevc->colors.colored.c_level[i]) < (1U << 7)) *(pdata)++ = (byte)(pdevc->colors.colored.c_level[i]); else if ((uint)(pdevc->colors.colored.c_level[i]) < (1U << (2 * 7))) { *(pdata)++ = (1U << 7) | ((pdevc->colors.colored.c_level[i]) & ((1U << 7) - 1)); *(pdata)++ = (pdevc->colors.colored.c_level[i]) >> 7; } else (pdata) = enc_u_put_uint((pdevc->colors.colored.c_level[i]), (pdata)); } while (0);
        }
    }

    if ((flag_bits & dc_ht_colored_has_alpha) != 0)
        do { if ((uint)(alpha) < (1U << 7)) *(pdata)++ = (byte)(alpha); else if ((uint)(alpha) < (1U << (2 * 7))) { *(pdata)++ = (1U << 7) | ((alpha) & ((1U << 7) - 1)); *(pdata)++ = (alpha) >> 7; } else (pdata) = enc_u_put_uint((alpha), (pdata)); } while (0);
    ...
    gs_memory_t * mem )
{
    ...
    if (prior_devc != 0 && prior_devc->type == (&gx_dc_type_data_ht_colored))
        devc = *prior_devc;
    else
        memset(&devc, 0, sizeof(devc));
    devc.type = (&gx_dc_type_data_ht_colored);
...
# 392 "./base/gxcht.c"
    ...
    if ((flag_bits & dc_ht_colored_has_level) != 0) {
        ...
        if (num_comps > 8 * sizeof(uint)) {
            do { if (((tmp_mask) = *(pdata)) >= (1U << 7)) { uint tmp_w; (pdata) = enc_u_get_uint(&tmp_w, (pdata)); (tmp_mask) = tmp_w; } else ++(pdata); } while (0);
        ...
        for (i = 0; i < num_comps; i++, plane_mask >>= 1) {
            if ((plane_mask & 0x1) != 0) {
                if (size - (pdata - pdata_start) < 1)
                    return ((-15));
                do { if (((devc.colors.colored.c_level[i]) = *(pdata)) >= (1U << 7)) { uint tmp_w; (pdata) = enc_u_get_uint(&tmp_w, (pdata)); (devc.colors.colored.c_level[i]) = tmp_w; } else ++(pdata); } while (0);
            } else
    ...
    if ((flag_bits & dc_ht_colored_alpha_is_max) != 0)
        devc.colors.colored.alpha = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1));
    else if ((flag_bits & dc_ht_colored_has_alpha) != 0) {
        const byte * pdata_start = pdata;

        if (size < 1)
            return ((-15));
        do { if (((devc.colors.colored.alpha) = *(pdata)) >= (1U << 7)) { uint tmp_w; (pdata) = enc_u_get_uint(&tmp_w, (pdata)); (devc.colors.colored.alpha) = tmp_w; } else ++(pdata); } while (0);
        size -= pdata - pdata_start;
    }


    ((&devc)->phase.x = (imod(-(pis->screen_phase[0].x), pis->dev_ht->lcm_width)), (&devc)->phase.y = (imod(-(pis->screen_phase[0].y), pis->dev_ht->lcm_height)));
...
# 516 "./base/gxcht.c"
typedef gx_color_value gx_color_value_array[(8 * 8)];
typedef struct color_values_pair_s {
    gx_color_value_array values[2];
} color_values_pair_t;
# 531 "./base/gxcht.c"
static int set_ht_colors_le_4( color_values_pair_t *pvp, gx_color_index colors[(2 * (8 * 8) < 16 ? 16 : 2 * (8 * 8))], const gx_const_strip_bitmap *sbits[(8 * 8)], const gx_device_color *pdevc, gx_device *dev, gx_ht_cache *caches[(8 * 8)], int nplanes );
static int set_cmyk_1bit_colors( color_values_pair_t *pvp, gx_color_index colors[(2 * (8 * 8) < 16 ? 16 : 2 * (8 * 8))], const gx_const_strip_bitmap *sbits[(8 * 8)], const gx_device_color *pdevc, gx_device *dev, gx_ht_cache *caches[(8 * 8)], int nplanes );
static int set_ht_colors_gt_4( color_values_pair_t *pvp, gx_color_index colors[(2 * (8 * 8) < 16 ? 16 : 2 * (8 * 8))], const gx_const_strip_bitmap *sbits[(8 * 8)], const gx_device_color *pdevc, gx_device *dev, gx_ht_cache *caches[(8 * 8)], int nplanes );
# 555 "./base/gxcht.c"
static void set_color_ht_le_4( byte *dest_data, uint dest_raster, int px, int py, int w, int h, int depth, int special, int nplanes, gx_color_index plane_mask, gx_device *dev, const color_values_pair_t *pvp, gx_color_index colors[(8 * 8)], const gx_const_strip_bitmap * sbits[(8 * 8)] );
static void set_color_ht_gt_4( byte *dest_data, uint dest_raster, int px, int py, int w, int h, int depth, int special, int nplanes, gx_color_index plane_mask, gx_device *dev, const color_values_pair_t *pvp, gx_color_index colors[(8 * 8)], const gx_const_strip_bitmap * sbits[(8 * 8)] );
                                ...
                                const gx_rop_source_t * source)
{
    ...
    const uint tile_bytes = 256 * ((int)(sizeof(long)));
    gx_strip_bitmap tiles;
    ...
    int (*set_ht_colors)( color_values_pair_t *pvp, gx_color_index colors[(2 * (8 * 8) < 16 ? 16 : 2 * (8 * 8))], const gx_const_strip_bitmap *sbits[(8 * 8)], const gx_device_color *pdevc, gx_device *dev, gx_ht_cache *caches[(8 * 8)], int nplanes ) =
        (



         (((dev)->procs.dev_spec_op)(dev, gxdso_is_std_cmyk_1bit, ((void*)0), 0) > 0) ?
            set_cmyk_1bit_colors :
            nplanes <= 4 ? set_ht_colors_le_4 :
                          set_ht_colors_gt_4

         );
    void (*set_color_ht)( byte *dest_data, uint dest_raster, int px, int py, int w, int h, int depth, int special, int nplanes, gx_color_index plane_mask, gx_device *dev, const color_values_pair_t *pvp, gx_color_index colors[(8 * 8)], const gx_const_strip_bitmap * sbits[(8 * 8)] ) =
        (
    ...
    gx_color_index colors[(2 * (8 * 8) < 16 ? 16 : 2 * (8 * 8))];
    const gx_const_strip_bitmap *sbits[(8 * 8)];
    gx_ht_cache *caches[(8 * 8)];
    int special;
    int code = 0;
    int raster;
    uint size_x;
    ...
    if ((w | h) >= 16) {
        ...
        ((dev)->procs.get_clipping_box)(dev, &cbox);
        if ((t = ((int)((cbox.p.x) >> (8)))) > x) {
            if ((w += x - t) <= 0)
                return 0;
            x = t;
        }
        if ((t = ((int)((cbox.p.y) >> (8)))) > y) {
            if ((h += y - t) <= 0)
                return 0;
            y = t;
        }
        if ((t = ((int)((cbox.q.x) >> (8)))) < x + w)
            if ((w = t - x) <= 0)
                return 0;
        if ((t = ((int)((cbox.q.y) >> (8)))) < y + h)
            if ((h = t - y) <= 0)
                return 0;
    }
    ...
    no_rop = source == ((void*)0) && (((lop) & (0x200 | (rop3_1 - 0xcc))) == (0xf0 & ~0xcc));
    if ((!no_rop) && (source == ((void*)0)))
    ...
    if ((w > lw || h > lh) &&
        (raster = ((uint)((((lw * depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3))) <= tile_bytes / lh
        ) {
        ...
        do { do { do { if ( (x | y) < 0 ) { if ( x < 0 ) w += x, x = 0; if ( y < 0 ) h += y, y = 0; } } while (0); do { if ( w > (dev)->width - x ) w = (dev)->width - x; } while (0); do { if ( h > (dev)->height - y ) h = (dev)->height - y; } while (0); } while (0); if ( w <= 0 || h <= 0 ) return 0; } while (0);
        ...
        if (w > lw || h > lh) {
            ...
            set_color_ht((byte *)tbits, raster, 0, 0, lw, lh, depth,
                         ...
                         dev, &vp, colors, sbits);
            if (no_rop)
                return (*((dev)->procs.strip_tile_rectangle)) (dev, &tiles,
                                                               x, y, w, h,
                                                               ((gx_color_index)(~0)),
                                                               ((gx_color_index)(~0)),
                                                               pdevc->phase.x,
    ...
    size_x = w * depth;
    raster = ((uint)((((size_x) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    ...
    while (w) {
        ...
        for (;;) {
            set_color_ht((byte *)tbits, raster,
                         ...
                         dev, &vp, colors, sbits);
            if (no_rop) {
                code = (*((dev)->procs.copy_color))
                    (dev, (byte *)tbits, 0, raster, 0L,
                     x, cy, dw, ch);
            } else {
                tiles.rep_height = tiles.size.y = ch;
                if (source->planar_height == 0)
                    code = (*((dev)->procs.strip_copy_rop))
                          (dev, source->sdata + source->sraster * (cy-origy),
                           source->sourcex + (x - origx),
                           source->sraster,
                           source->id,
                           (source->use_scolors ? source->scolors : ((void*)0)),
                           &tiles, ((void*)0), x, cy, dw, ch, 0, 0, lop);
                else
                    code = (*((dev)->procs.strip_copy_rop2))
                          (dev, source->sdata + source->sraster * (cy-origy),
                           source->sourcex + (x - origx),
                           source->sraster,
                           source->id,
...
set_ht_colors_le_4(color_values_pair_t *pvp ,
                   ...
                   gx_ht_cache * caches[(8 * 8)], int nplanes)
{
...
# 890 "./base/gxcht.c"
    ...
    do { uint q = pdc->colors.colored.c_base[0]; uint r = pdc->colors.colored.c_level[0]; pvp->values[0][0] = ((max_color) <= 7 ? fc_color_quo[max_color][q] : (gx_color_value)(((q) * (0xffffL * 2) + max_color) / (max_color * 2))); if (r == 0) pvp->values[1][0] = pvp->values[0][0], sbits[0] = &ht_no_bitmap; else if (!invert) { pvp->values[1][0] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[0] = (const gx_const_strip_bitmap *) &((caches[0])->render_ht(caches[0], r))->tiles; } else { const gx_device_halftone *pdht = pdc->colors.colored.c_ht; int nlevels = (pdht->components ? pdht->components[0].corder.num_levels : pdht->order.num_levels); pvp->values[1][0] = pvp->values[0][0]; pvp->values[0][0] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[0] = (const gx_const_strip_bitmap *) &((caches[0])->render_ht(caches[0], nlevels - r))->tiles; } } while (0);
    if (nplanes >= 2) {
        do { uint q = pdc->colors.colored.c_base[1]; uint r = pdc->colors.colored.c_level[1]; pvp->values[0][1] = ((max_color) <= 7 ? fc_color_quo[max_color][q] : (gx_color_value)(((q) * (0xffffL * 2) + max_color) / (max_color * 2))); if (r == 0) pvp->values[1][1] = pvp->values[0][1], sbits[1] = &ht_no_bitmap; else if (!invert) { pvp->values[1][1] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[1] = (const gx_const_strip_bitmap *) &((caches[1])->render_ht(caches[1], r))->tiles; } else { const gx_device_halftone *pdht = pdc->colors.colored.c_ht; int nlevels = (pdht->components ? pdht->components[1].corder.num_levels : pdht->order.num_levels); pvp->values[1][1] = pvp->values[0][1]; pvp->values[0][1] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[1] = (const gx_const_strip_bitmap *) &((caches[1])->render_ht(caches[1], nlevels - r))->tiles; } } while (0);
    }
    if (nplanes >= 3) {
        do { uint q = pdc->colors.colored.c_base[2]; uint r = pdc->colors.colored.c_level[2]; pvp->values[0][2] = ((max_color) <= 7 ? fc_color_quo[max_color][q] : (gx_color_value)(((q) * (0xffffL * 2) + max_color) / (max_color * 2))); if (r == 0) pvp->values[1][2] = pvp->values[0][2], sbits[2] = &ht_no_bitmap; else if (!invert) { pvp->values[1][2] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[2] = (const gx_const_strip_bitmap *) &((caches[2])->render_ht(caches[2], r))->tiles; } else { const gx_device_halftone *pdht = pdc->colors.colored.c_ht; int nlevels = (pdht->components ? pdht->components[2].corder.num_levels : pdht->order.num_levels); pvp->values[1][2] = pvp->values[0][2]; pvp->values[0][2] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[2] = (const gx_const_strip_bitmap *) &((caches[2])->render_ht(caches[2], nlevels - r))->tiles; } } while (0);
    }
    if (nplanes == 3) {
        ...
        if (alpha == ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1))) {
            ...
            cvalues[0] = pvp->values[(0) & 1][0]; cvalues[1] = pvp->values[((0) & 2) >> 1][1]; cvalues[2] = pvp->values[(0) >> 2][2]; colors[0] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(1) & 1][0]; cvalues[1] = pvp->values[((1) & 2) >> 1][1]; cvalues[2] = pvp->values[(1) >> 2][2]; colors[1] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(2) & 1][0]; cvalues[1] = pvp->values[((2) & 2) >> 1][1]; cvalues[2] = pvp->values[(2) >> 2][2]; colors[2] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(3) & 1][0]; cvalues[1] = pvp->values[((3) & 2) >> 1][1]; cvalues[2] = pvp->values[(3) >> 2][2]; colors[3] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(4) & 1][0]; cvalues[1] = pvp->values[((4) & 2) >> 1][1]; cvalues[2] = pvp->values[(4) >> 2][2]; colors[4] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(5) & 1][0]; cvalues[1] = pvp->values[((5) & 2) >> 1][1]; cvalues[2] = pvp->values[(5) >> 2][2]; colors[5] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(6) & 1][0]; cvalues[1] = pvp->values[((6) & 2) >> 1][1]; cvalues[2] = pvp->values[(6) >> 2][2]; colors[6] = ((dev)->procs.encode_color)(dev, cvalues);; cvalues[0] = pvp->values[(7) & 1][0]; cvalues[1] = pvp->values[((7) & 2) >> 1][1]; cvalues[2] = pvp->values[(7) >> 2][2]; colors[7] = ((dev)->procs.encode_color)(dev, cvalues);;
            ...
            colors[0] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(0) & 1][0], pvp->values[((0) & 2) >> 1][1], pvp->values[(0) >> 2][2], alpha); colors[1] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(1) & 1][0], pvp->values[((1) & 2) >> 1][1], pvp->values[(1) >> 2][2], alpha); colors[2] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(2) & 1][0], pvp->values[((2) & 2) >> 1][1], pvp->values[(2) >> 2][2], alpha); colors[3] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(3) & 1][0], pvp->values[((3) & 2) >> 1][1], pvp->values[(3) >> 2][2], alpha); colors[4] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(4) & 1][0], pvp->values[((4) & 2) >> 1][1], pvp->values[(4) >> 2][2], alpha); colors[5] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(5) & 1][0], pvp->values[((5) & 2) >> 1][1], pvp->values[(5) >> 2][2], alpha); colors[6] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(6) & 1][0], pvp->values[((6) & 2) >> 1][1], pvp->values[(6) >> 2][2], alpha); colors[7] = ((dev)->procs.map_rgb_alpha_color)(dev, pvp->values[(7) & 1][0], pvp->values[((7) & 2) >> 1][1], pvp->values[(7) >> 2][2], alpha);

        }
    } else if (nplanes > 3){
        do { uint q = pdc->colors.colored.c_base[3]; uint r = pdc->colors.colored.c_level[3]; pvp->values[0][3] = ((max_color) <= 7 ? fc_color_quo[max_color][q] : (gx_color_value)(((q) * (0xffffL * 2) + max_color) / (max_color * 2))); if (r == 0) pvp->values[1][3] = pvp->values[0][3], sbits[3] = &ht_no_bitmap; else if (!invert) { pvp->values[1][3] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[3] = (const gx_const_strip_bitmap *) &((caches[3])->render_ht(caches[3], r))->tiles; } else { const gx_device_halftone *pdht = pdc->colors.colored.c_ht; int nlevels = (pdht->components ? pdht->components[3].corder.num_levels : pdht->order.num_levels); pvp->values[1][3] = pvp->values[0][3]; pvp->values[0][3] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[3] = (const gx_const_strip_bitmap *) &((caches[3])->render_ht(caches[3], nlevels - r))->tiles; } } while (0);
        if (nplanes > 4) {
            ...
            for (pi = 4; pi < nplanes; ++pi)
                do { pvp->values[1][pi] = pvp->values[0][pi] = ((max_color) <= 7 ? fc_color_quo[max_color][pdc->colors.colored.c_base[pi]] : (gx_color_value)(((pdc->colors.colored.c_base[pi]) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[pi] = &ht_no_bitmap; } while (0);
        }
# 947 "./base/gxcht.c"
        switch ((int)pdc->colors.colored.plane_mask) {
            case 15:
                cvalues[0] = pvp->values[(15) & 1][0]; cvalues[1] = pvp->values[((15) & 2) >> 1][1]; cvalues[2] = pvp->values[((15) & 4) >> 2][2]; cvalues[3] = pvp->values[(15) >> 3][3]; colors[15] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(14) & 1][0]; cvalues[1] = pvp->values[((14) & 2) >> 1][1]; cvalues[2] = pvp->values[((14) & 4) >> 2][2]; cvalues[3] = pvp->values[(14) >> 3][3]; colors[14] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(13) & 1][0]; cvalues[1] = pvp->values[((13) & 2) >> 1][1]; cvalues[2] = pvp->values[((13) & 4) >> 2][2]; cvalues[3] = pvp->values[(13) >> 3][3]; colors[13] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(12) & 1][0]; cvalues[1] = pvp->values[((12) & 2) >> 1][1]; cvalues[2] = pvp->values[((12) & 4) >> 2][2]; cvalues[3] = pvp->values[(12) >> 3][3]; colors[12] = ((dev)->procs.encode_color)(dev, cvalues);
                cvalues[0] = pvp->values[(11) & 1][0]; cvalues[1] = pvp->values[((11) & 2) >> 1][1]; cvalues[2] = pvp->values[((11) & 4) >> 2][2]; cvalues[3] = pvp->values[(11) >> 3][3]; colors[11] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(10) & 1][0]; cvalues[1] = pvp->values[((10) & 2) >> 1][1]; cvalues[2] = pvp->values[((10) & 4) >> 2][2]; cvalues[3] = pvp->values[(10) >> 3][3]; colors[10] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(9) & 1][0]; cvalues[1] = pvp->values[((9) & 2) >> 1][1]; cvalues[2] = pvp->values[((9) & 4) >> 2][2]; cvalues[3] = pvp->values[(9) >> 3][3]; colors[9] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
            case 7:
                cvalues[0] = pvp->values[(7) & 1][0]; cvalues[1] = pvp->values[((7) & 2) >> 1][1]; cvalues[2] = pvp->values[((7) & 4) >> 2][2]; cvalues[3] = pvp->values[(7) >> 3][3]; colors[7] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(6) & 1][0]; cvalues[1] = pvp->values[((6) & 2) >> 1][1]; cvalues[2] = pvp->values[((6) & 4) >> 2][2]; cvalues[3] = pvp->values[(6) >> 3][3]; colors[6] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(5) & 1][0]; cvalues[1] = pvp->values[((5) & 2) >> 1][1]; cvalues[2] = pvp->values[((5) & 4) >> 2][2]; cvalues[3] = pvp->values[(5) >> 3][3]; colors[5] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(4) & 1][0]; cvalues[1] = pvp->values[((4) & 2) >> 1][1]; cvalues[2] = pvp->values[((4) & 4) >> 2][2]; cvalues[3] = pvp->values[(4) >> 3][3]; colors[4] = ((dev)->procs.encode_color)(dev, cvalues);
c3: case 3:
                cvalues[0] = pvp->values[(3) & 1][0]; cvalues[1] = pvp->values[((3) & 2) >> 1][1]; cvalues[2] = pvp->values[((3) & 4) >> 2][2]; cvalues[3] = pvp->values[(3) >> 3][3]; colors[3] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(2) & 1][0]; cvalues[1] = pvp->values[((2) & 2) >> 1][1]; cvalues[2] = pvp->values[((2) & 4) >> 2][2]; cvalues[3] = pvp->values[(2) >> 3][3]; colors[2] = ((dev)->procs.encode_color)(dev, cvalues);
c1: case 1:
                cvalues[0] = pvp->values[(1) & 1][0]; cvalues[1] = pvp->values[((1) & 2) >> 1][1]; cvalues[2] = pvp->values[((1) & 4) >> 2][2]; cvalues[3] = pvp->values[(1) >> 3][3]; colors[1] = ((dev)->procs.encode_color)(dev, cvalues);
                break;
            case 14:
                cvalues[0] = pvp->values[(14) & 1][0]; cvalues[1] = pvp->values[((14) & 2) >> 1][1]; cvalues[2] = pvp->values[((14) & 4) >> 2][2]; cvalues[3] = pvp->values[(14) >> 3][3]; colors[14] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(12) & 1][0]; cvalues[1] = pvp->values[((12) & 2) >> 1][1]; cvalues[2] = pvp->values[((12) & 4) >> 2][2]; cvalues[3] = pvp->values[(12) >> 3][3]; colors[12] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(10) & 1][0]; cvalues[1] = pvp->values[((10) & 2) >> 1][1]; cvalues[2] = pvp->values[((10) & 4) >> 2][2]; cvalues[3] = pvp->values[(10) >> 3][3]; colors[10] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
            case 6:
                cvalues[0] = pvp->values[(6) & 1][0]; cvalues[1] = pvp->values[((6) & 2) >> 1][1]; cvalues[2] = pvp->values[((6) & 4) >> 2][2]; cvalues[3] = pvp->values[(6) >> 3][3]; colors[6] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(4) & 1][0]; cvalues[1] = pvp->values[((4) & 2) >> 1][1]; cvalues[2] = pvp->values[((4) & 4) >> 2][2]; cvalues[3] = pvp->values[(4) >> 3][3]; colors[4] = ((dev)->procs.encode_color)(dev, cvalues);
c2: case 2:
                cvalues[0] = pvp->values[(2) & 1][0]; cvalues[1] = pvp->values[((2) & 2) >> 1][1]; cvalues[2] = pvp->values[((2) & 4) >> 2][2]; cvalues[3] = pvp->values[(2) >> 3][3]; colors[2] = ((dev)->procs.encode_color)(dev, cvalues);
                break;
            case 13:
                cvalues[0] = pvp->values[(13) & 1][0]; cvalues[1] = pvp->values[((13) & 2) >> 1][1]; cvalues[2] = pvp->values[((13) & 4) >> 2][2]; cvalues[3] = pvp->values[(13) >> 3][3]; colors[13] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(12) & 1][0]; cvalues[1] = pvp->values[((12) & 2) >> 1][1]; cvalues[2] = pvp->values[((12) & 4) >> 2][2]; cvalues[3] = pvp->values[(12) >> 3][3]; colors[12] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(9) & 1][0]; cvalues[1] = pvp->values[((9) & 2) >> 1][1]; cvalues[2] = pvp->values[((9) & 4) >> 2][2]; cvalues[3] = pvp->values[(9) >> 3][3]; colors[9] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
            case 5:
                cvalues[0] = pvp->values[(5) & 1][0]; cvalues[1] = pvp->values[((5) & 2) >> 1][1]; cvalues[2] = pvp->values[((5) & 4) >> 2][2]; cvalues[3] = pvp->values[(5) >> 3][3]; colors[5] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(4) & 1][0]; cvalues[1] = pvp->values[((4) & 2) >> 1][1]; cvalues[2] = pvp->values[((4) & 4) >> 2][2]; cvalues[3] = pvp->values[(4) >> 3][3]; colors[4] = ((dev)->procs.encode_color)(dev, cvalues);
                goto c1;
            case 12:
                cvalues[0] = pvp->values[(12) & 1][0]; cvalues[1] = pvp->values[((12) & 2) >> 1][1]; cvalues[2] = pvp->values[((12) & 4) >> 2][2]; cvalues[3] = pvp->values[(12) >> 3][3]; colors[12] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
            case 4:
                cvalues[0] = pvp->values[(4) & 1][0]; cvalues[1] = pvp->values[((4) & 2) >> 1][1]; cvalues[2] = pvp->values[((4) & 4) >> 2][2]; cvalues[3] = pvp->values[(4) >> 3][3]; colors[4] = ((dev)->procs.encode_color)(dev, cvalues);
                break;
            case 11:
                cvalues[0] = pvp->values[(11) & 1][0]; cvalues[1] = pvp->values[((11) & 2) >> 1][1]; cvalues[2] = pvp->values[((11) & 4) >> 2][2]; cvalues[3] = pvp->values[(11) >> 3][3]; colors[11] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(10) & 1][0]; cvalues[1] = pvp->values[((10) & 2) >> 1][1]; cvalues[2] = pvp->values[((10) & 4) >> 2][2]; cvalues[3] = pvp->values[(10) >> 3][3]; colors[10] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(9) & 1][0]; cvalues[1] = pvp->values[((9) & 2) >> 1][1]; cvalues[2] = pvp->values[((9) & 4) >> 2][2]; cvalues[3] = pvp->values[(9) >> 3][3]; colors[9] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
                goto c3;
            case 10:
                cvalues[0] = pvp->values[(10) & 1][0]; cvalues[1] = pvp->values[((10) & 2) >> 1][1]; cvalues[2] = pvp->values[((10) & 4) >> 2][2]; cvalues[3] = pvp->values[(10) >> 3][3]; colors[10] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
                goto c2;
            case 9:
                cvalues[0] = pvp->values[(9) & 1][0]; cvalues[1] = pvp->values[((9) & 2) >> 1][1]; cvalues[2] = pvp->values[((9) & 4) >> 2][2]; cvalues[3] = pvp->values[(9) >> 3][3]; colors[9] = ((dev)->procs.encode_color)(dev, cvalues); cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
                goto c1;
            case 8:
                cvalues[0] = pvp->values[(8) & 1][0]; cvalues[1] = pvp->values[((8) & 2) >> 1][1]; cvalues[2] = pvp->values[((8) & 4) >> 2][2]; cvalues[3] = pvp->values[(8) >> 3][3]; colors[8] = ((dev)->procs.encode_color)(dev, cvalues);
                break;
            case 0:;
        }
        cvalues[0] = pvp->values[(0) & 1][0]; cvalues[1] = pvp->values[((0) & 2) >> 1][1]; cvalues[2] = pvp->values[((0) & 4) >> 2][2]; cvalues[3] = pvp->values[(0) >> 3][3]; colors[0] = ((dev)->procs.encode_color)(dev, cvalues);
...
# 1038 "./base/gxcht.c"
    do { uint r = pdc->colors.colored.c_level[0]; if (r == 0) { if (pdc->colors.colored.c_base[0]) mask0 |= (bits32)~0x77777777, mask1 |= (bits32)~0x77777777; sbits[3 - 0] = &ht_no_bitmap; } else { int nlevels = (pdht->components ? pdht->components[0].corder.num_levels : pdht->order.num_levels); mask0 |= (bits32)~0x77777777; sbits[3 - 0] = (const gx_const_strip_bitmap *) &((caches[0])->render_ht(caches[0], nlevels - r))->tiles; } } while (0);
    do { uint r = pdc->colors.colored.c_level[1]; if (r == 0) { if (pdc->colors.colored.c_base[1]) mask0 |= 0x44444444, mask1 |= 0x44444444; sbits[3 - 1] = &ht_no_bitmap; } else { int nlevels = (pdht->components ? pdht->components[1].corder.num_levels : pdht->order.num_levels); mask0 |= 0x44444444; sbits[3 - 1] = (const gx_const_strip_bitmap *) &((caches[1])->render_ht(caches[1], nlevels - r))->tiles; } } while (0);
    do { uint r = pdc->colors.colored.c_level[2]; if (r == 0) { if (pdc->colors.colored.c_base[2]) mask0 |= 0x22222222, mask1 |= 0x22222222; sbits[3 - 2] = &ht_no_bitmap; } else { int nlevels = (pdht->components ? pdht->components[2].corder.num_levels : pdht->order.num_levels); mask0 |= 0x22222222; sbits[3 - 2] = (const gx_const_strip_bitmap *) &((caches[2])->render_ht(caches[2], nlevels - r))->tiles; } } while (0);
    do { uint r = pdc->colors.colored.c_level[3]; if (r == 0) { if (pdc->colors.colored.c_base[3]) mask0 |= 0x11111111, mask1 |= 0x11111111; sbits[3 - 3] = &ht_no_bitmap; } else { int nlevels = (pdht->components ? pdht->components[3].corder.num_levels : pdht->order.num_levels); mask0 |= 0x11111111; sbits[3 - 3] = (const gx_const_strip_bitmap *) &((caches[3])->render_ht(caches[3], nlevels - r))->tiles; } } while (0);
                   ...
                   gx_ht_cache * caches[(8 * 8)], int nplanes)
{
    ...
    int i;
    gx_color_value cv[(8 * 8)] = {0};


    for (i = 0; i < nplanes; ++i)
        if ((plane_mask >> i) & 1)
            do { uint q = pdc->colors.colored.c_base[i]; uint r = pdc->colors.colored.c_level[i]; pvp->values[0][i] = ((max_color) <= 7 ? fc_color_quo[max_color][q] : (gx_color_value)(((q) * (0xffffL * 2) + max_color) / (max_color * 2))); if (r == 0) pvp->values[1][i] = pvp->values[0][i], sbits[i] = &ht_no_bitmap; else if (!invert) { pvp->values[1][i] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[i] = (const gx_const_strip_bitmap *) &((caches[i])->render_ht(caches[i], r))->tiles; } else { const gx_device_halftone *pdht = pdc->colors.colored.c_ht; int nlevels = (pdht->components ? pdht->components[i].corder.num_levels : pdht->order.num_levels); pvp->values[1][i] = pvp->values[0][i]; pvp->values[0][i] = ((max_color) <= 7 ? fc_color_quo[max_color][q + 1] : (gx_color_value)(((q + 1) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[i] = (const gx_const_strip_bitmap *) &((caches[i])->render_ht(caches[i], nlevels - r))->tiles; } } while (0);
        else
            do { pvp->values[1][i] = pvp->values[0][i] = ((max_color) <= 7 ? fc_color_quo[max_color][pdc->colors.colored.c_base[i]] : (gx_color_value)(((pdc->colors.colored.c_base[i]) * (0xffffL * 2) + max_color) / (max_color * 2))); sbits[i] = &ht_no_bitmap; } while (0);
# 1088 "./base/gxcht.c"
    for (i = 0; i < nplanes; i++ ) {
        cv[i] = pvp->values[0][i];
        colors[2 * i] = ((dev)->procs.encode_color)(dev, cv);

        if ((plane_mask >> i) & 1) {
            cv[i] = pvp->values[1][i];
            colors[2 * i + 1] = ((dev)->procs.encode_color)(dev, cv);
        }
        cv[i] = 0;
    }
...
# 1261 "./base/gxcht.c"
            if (plane_mask & 1) {
                do { if (cursor[0].data > cursor[0].row) { bits = ((cursor[0].data[-1] << 8) | *cursor[0].data) >> cursor[0].bit_shift; cursor[0].data--; } else { bits = *cursor[0].data >> cursor[0].bit_shift; cursor[0].data += cursor[0].xbytes; if ((cursor[0].bit_shift -= cursor[0].xbits) < 0) { bits |= *cursor[0].data << -cursor[0].bit_shift; cursor[0].bit_shift += 8; } else { bits |= ((cursor[0].data[-1] << 8) | *cursor[0].data) >> cursor[0].bit_shift; cursor[0].data--; } } } while (0);
                indices = expand_8x1_to_8x4[bits & 0xff];
            } else
                indices = 0;
            if (plane_mask & 2) {
                do { if (cursor[1].data > cursor[1].row) { bits = ((cursor[1].data[-1] << 8) | *cursor[1].data) >> cursor[1].bit_shift; cursor[1].data--; } else { bits = *cursor[1].data >> cursor[1].bit_shift; cursor[1].data += cursor[1].xbytes; if ((cursor[1].bit_shift -= cursor[1].xbits) < 0) { bits |= *cursor[1].data << -cursor[1].bit_shift; cursor[1].bit_shift += 8; } else { bits |= ((cursor[1].data[-1] << 8) | *cursor[1].data) >> cursor[1].bit_shift; cursor[1].data--; } } } while (0);
                indices |= expand_8x1_to_8x4[bits & 0xff] << 1;
            }
            if (plane_mask & 4) {
                do { if (cursor[2].data > cursor[2].row) { bits = ((cursor[2].data[-1] << 8) | *cursor[2].data) >> cursor[2].bit_shift; cursor[2].data--; } else { bits = *cursor[2].data >> cursor[2].bit_shift; cursor[2].data += cursor[2].xbytes; if ((cursor[2].bit_shift -= cursor[2].xbits) < 0) { bits |= *cursor[2].data << -cursor[2].bit_shift; cursor[2].bit_shift += 8; } else { bits |= ((cursor[2].data[-1] << 8) | *cursor[2].data) >> cursor[2].bit_shift; cursor[2].data--; } } } while (0);
                indices |= expand_8x1_to_8x4[bits & 0xff] << 2;
            }
            if (plane_mask & 8) {
                do { if (cursor[3].data > cursor[3].row) { bits = ((cursor[3].data[-1] << 8) | *cursor[3].data) >> cursor[3].bit_shift; cursor[3].data--; } else { bits = *cursor[3].data >> cursor[3].bit_shift; cursor[3].data += cursor[3].xbytes; if ((cursor[3].bit_shift -= cursor[3].xbits) < 0) { bits |= *cursor[3].data << -cursor[3].bit_shift; cursor[3].bit_shift += 8; } else { bits |= ((cursor[3].data[-1] << 8) | *cursor[3].data) >> cursor[3].bit_shift; cursor[3].data--; } } } while (0);
        ...
        if (plane_mask & 1)
            do { if (cursor[0].row > cursor[0].tdata) cursor[0].row -= cursor[0].raster; else { wrap_shifted_cursor(&cursor[0], sbits[0]); } cursor[0].data = cursor[0].row + cursor[0].xoffset; cursor[0].bit_shift = cursor[0].xshift; } while (0);
        if (plane_mask & 2)
            do { if (cursor[1].row > cursor[1].tdata) cursor[1].row -= cursor[1].raster; else { wrap_shifted_cursor(&cursor[1], sbits[1]); } cursor[1].data = cursor[1].row + cursor[1].xoffset; cursor[1].bit_shift = cursor[1].xshift; } while (0);
        if (plane_mask & 4)
            do { if (cursor[2].row > cursor[2].tdata) cursor[2].row -= cursor[2].raster; else { wrap_shifted_cursor(&cursor[2], sbits[2]); } cursor[2].data = cursor[2].row + cursor[2].xoffset; cursor[2].bit_shift = cursor[2].xshift; } while (0);
        if (plane_mask & 8)
            do { if (cursor[3].row > cursor[3].tdata) cursor[3].row -= cursor[3].raster; else { wrap_shifted_cursor(&cursor[3], sbits[3]); } cursor[3].data = cursor[3].row + cursor[3].xoffset; cursor[3].bit_shift = cursor[3].xshift; } while (0);
    }
}
...
b: if (ptc->bit_shift < 8)
        ...
        for (i = pmin; i <= pmax; ++i)
            if ((plane_mask >> i) & 1)
                do { if (cursor[i].row > cursor[i].tdata) cursor[i].row -= cursor[i].raster; else { wrap_shifted_cursor(&cursor[i], sbits[i]); } cursor[i].data = cursor[i].row + cursor[i].xoffset; cursor[i].bit_shift = cursor[i].xshift; } while (0);
    }
}
