# 1 "./lcms2/src/cmsopt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./lcms2/src/cmsopt.c" 2
# 28 "./lcms2/src/cmsopt.c"
...
typedef struct {
    ...
    cmsUInt32Number X0[256], Y0[256], Z0[256];
...
typedef struct {
    ...
    _cmsInterpFn16 EvalCurveIn16[8];
    cmsInterpParams* ParamsCurveIn16[8];

    _cmsInterpFn16 EvalCLUT;
    const cmsInterpParams* CLUTparams;


    _cmsInterpFn16* EvalCurveOut16;
    cmsInterpParams** ParamsCurveOut16;
                  ...
                  register const void* D)
{
    ...
    cmsUInt16Number StageABC[8];
    cmsUInt16Number StageDEF[16];
    int i;
...
# 323 "./lcms2/src/cmsopt.c"
static
int XFormSampler16(register const cmsUInt16Number In[], register cmsUInt16Number Out[], register void* Cargo)
{
    cmsPipeline* Lut = (cmsPipeline*) Cargo;
    cmsFloat32Number InFloat[16], OutFloat[16];
    cmsUInt32Number i;

    (((Lut -> InputChannels < 16)) ? (void) (0) : __assert_fail ("(Lut -> InputChannels < 16)", "./lcms2/src/cmsopt.c", 330, __PRETTY_FUNCTION__));
    (((Lut -> OutputChannels < 16)) ? (void) (0) : __assert_fail ("(Lut -> OutputChannels < 16)", "./lcms2/src/cmsopt.c", 331, __PRETTY_FUNCTION__));
...
cmsBool AllCurvesAreLinear(cmsStage* mpe)
{
    ...
    if (Curves == ((void*)0)) return 0;

    n = cmsStageOutputChannels(mpe);

    for (i=0; i < n; i++) {
        if (!cmsIsToneCurveLinear(Curves[i])) return 0;
    }
                  ...
                  int nChannelsOut, int nChannelsIn)
{
    ...
    if (CLUT -> Type != cmsSigCLutElemType) {
        cmsSignalError(CLUT->ContextID, 3, "(internal) Attempt to PatchLUT on non-lut stage");
        return 0;
    }
    ...
    if (nChannelsIn == 4) {
        ...
        if (nChannelsIn == 3) {
            ...
            if (nChannelsIn == 1) {
                ...
                cmsSignalError(CLUT->ContextID, 3, "(internal) %d Channels are not supported on PatchLUT", nChannelsIn);
                return 0;
            }
...
cmsBool WhitesAreEqual(int n, cmsUInt16Number White1[], cmsUInt16Number White2[] )
{
    ...
    for (i=0; i < n; i++) {
        ...
        if (abs(White1[i] - White2[i]) > 0xf000) return 1;
        if (White1[i] != White2[i]) return 0;
    }
    return 1;
}
...
cmsBool FixWhiteMisalignment(cmsPipeline* Lut, cmsColorSpaceSignature EntryColorSpace, cmsColorSpaceSignature ExitColorSpace)
{
    ...
    cmsUInt16Number WhiteIn[16], WhiteOut[16], ObtainedOut[16];
    cmsUInt32Number i, nOuts, nIns;
    cmsStage *PreLin = ((void*)0), *CLUT = ((void*)0), *PostLin = ((void*)0);

    if (!_cmsEndPointsBySpace(EntryColorSpace,
        &WhitePointIn, ((void*)0), &nIns)) return 0;

    if (!_cmsEndPointsBySpace(ExitColorSpace,
        &WhitePointOut, ((void*)0), &nOuts)) return 0;


    if (Lut ->InputChannels != nIns) return 0;
    if (Lut ->OutputChannels != nOuts) return 0;

    cmsPipelineEval16(WhitePointIn, ObtainedOut, Lut);

    if (WhitesAreEqual(nOuts, WhitePointOut, ObtainedOut)) return 1;
...
# 545 "./lcms2/src/cmsopt.c"
static
cmsBool OptimizeByResampling(cmsPipeline** Lut, cmsUInt32Number Intent, cmsUInt32Number* InputFormat, cmsUInt32Number* OutputFormat, cmsUInt32Number* dwFlags)
{
    cmsPipeline* Src = ((void*)0);
    cmsPipeline* Dest = ((void*)0);
    cmsStage* mpe;
    cmsStage* CLUT;
    cmsStage *KeepPreLin = ((void*)0), *KeepPostLin = ((void*)0);
    int nGridPoints;
    cmsColorSpaceSignature ColorSpace, OutputColorSpace;
    cmsStage *NewPreLin = ((void*)0);
    cmsStage *NewPostLin = ((void*)0);
    ...
    if (_cmsFormatterIsFloat(*InputFormat) || _cmsFormatterIsFloat(*OutputFormat)) return 0;

    ColorSpace = _cmsICCcolorSpace((((*InputFormat)>>16)&31));
    OutputColorSpace = _cmsICCcolorSpace((((*OutputFormat)>>16)&31));
    ...
    for (mpe = cmsPipelineGetPtrToFirstStage(Src);
        ...
        mpe = cmsStageNext(mpe)) {
            if (cmsStageType(mpe) == cmsSigNamedColorElemType) return 0;
    }
    ...
    CLUT = cmsStageAllocCLut16bit(Src ->ContextID, nGridPoints, Src ->InputChannels, Src->OutputChannels, ((void*)0));
...
Error:
        ...
        if (KeepPreLin != ((void*)0)) {
            if (!cmsPipelineInsertStage(Src, cmsAT_BEGIN, KeepPreLin)) {
                (((0)) ? (void) (0) : __assert_fail ("(0)", "./lcms2/src/cmsopt.c", 649, __PRETTY_FUNCTION__));
            }
        }
        if (KeepPostLin != ((void*)0)) {
            if (!cmsPipelineInsertStage(Src, cmsAT_END, KeepPostLin)) {
                (((0)) ? (void) (0) : __assert_fail ("(0)", "./lcms2/src/cmsopt.c", 654, __PRETTY_FUNCTION__));
            }
        }
    ...
    if (KeepPreLin != ((void*)0)) cmsStageFree(KeepPreLin);
    if (KeepPostLin != ((void*)0)) cmsStageFree(KeepPostLin);
    cmsPipelineFree(Src);

    DataCLUT = (_cmsStageCLutData*) CLUT ->Data;

    if (NewPreLin == ((void*)0)) DataSetIn = ((void*)0);
    else DataSetIn = ((_cmsStageToneCurvesData*) NewPreLin ->Data) ->TheCurves;

    if (NewPostLin == ((void*)0)) DataSetOut = ((void*)0);
    else DataSetOut = ((_cmsStageToneCurvesData*) NewPostLin ->Data) ->TheCurves;


    if (DataSetIn == ((void*)0) && DataSetOut == ((void*)0)) {

        _cmsPipelineSetOptimizationParameters(Dest, (_cmsOPTeval16Fn) DataCLUT->Params->Interpolation.Lerp16, DataCLUT->Params, ((void*)0), ((void*)0));
    }
    else {
...
Prelin8Data* PrelinOpt8alloc(cmsContext ContextID, const cmsInterpParams* p, cmsToneCurve* G[3])
{
    ...
    for (i=0; i < 256; i++) {
        ...
        if (G != ((void*)0)) {
            ...
            Input[0] = cmsEvalToneCurve16(G[0], (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i)));
            Input[1] = cmsEvalToneCurve16(G[1], (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i)));
            Input[2] = cmsEvalToneCurve16(G[2], (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i)));
        }
        else {
            Input[0] = (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i));
            Input[1] = (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i));
            Input[2] = (cmsUInt16Number) ((((cmsUInt16Number) (i)) << 8)|(i));
        }
        ...
        p8 ->X0[i] = (p->opta[2] * ((v1)>>16));
        p8 ->Y0[i] = (p->opta[1] * ((v2)>>16));
        p8 ->Z0[i] = (p->opta[0] * ((v3)>>16));


        p8 ->rx[i] = (cmsUInt16Number) ((v1)&0xFFFFU);
        p8 ->ry[i] = (cmsUInt16Number) ((v2)&0xFFFFU);
        p8 ->rz[i] = (cmsUInt16Number) ((v3)&0xFFFFU);
    }
                  ...
                  register const void* D)
{
    ...
    for (OutChan=0; OutChan < TotalOut; OutChan++) {
        ...
        c0 = (LutTable[(X0)+(Y0)+(Z0)+OutChan]);

        if (rx >= ry && ry >= rz)
        {
            c1 = (LutTable[(X1)+(Y0)+(Z0)+OutChan]) - c0;
            c2 = (LutTable[(X1)+(Y1)+(Z0)+OutChan]) - (LutTable[(X1)+(Y0)+(Z0)+OutChan]);
            c3 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X1)+(Y1)+(Z0)+OutChan]);
        }
        else
            if (rx >= rz && rz >= ry)
            {
                c1 = (LutTable[(X1)+(Y0)+(Z0)+OutChan]) - c0;
                c2 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X1)+(Y0)+(Z1)+OutChan]);
                c3 = (LutTable[(X1)+(Y0)+(Z1)+OutChan]) - (LutTable[(X1)+(Y0)+(Z0)+OutChan]);
            }
            else
                if (rz >= rx && rx >= ry)
                {
                    c1 = (LutTable[(X1)+(Y0)+(Z1)+OutChan]) - (LutTable[(X0)+(Y0)+(Z1)+OutChan]);
                    c2 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X1)+(Y0)+(Z1)+OutChan]);
                    c3 = (LutTable[(X0)+(Y0)+(Z1)+OutChan]) - c0;
                }
                else
                    if (ry >= rx && rx >= rz)
                    {
                        c1 = (LutTable[(X1)+(Y1)+(Z0)+OutChan]) - (LutTable[(X0)+(Y1)+(Z0)+OutChan]);
                        c2 = (LutTable[(X0)+(Y1)+(Z0)+OutChan]) - c0;
                        c3 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X1)+(Y1)+(Z0)+OutChan]);
                    }
                    else
                        if (ry >= rz && rz >= rx)
                        {
                            c1 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X0)+(Y1)+(Z1)+OutChan]);
                            c2 = (LutTable[(X0)+(Y1)+(Z0)+OutChan]) - c0;
                            c3 = (LutTable[(X0)+(Y1)+(Z1)+OutChan]) - (LutTable[(X0)+(Y1)+(Z0)+OutChan]);
                        }
                        else
                            if (rz >= ry && ry >= rx)
                            {
                                c1 = (LutTable[(X1)+(Y1)+(Z1)+OutChan]) - (LutTable[(X0)+(Y1)+(Z1)+OutChan]);
                                c2 = (LutTable[(X0)+(Y1)+(Z1)+OutChan]) - (LutTable[(X0)+(Y0)+(Z1)+OutChan]);
                                c3 = (LutTable[(X0)+(Y0)+(Z1)+OutChan]) - c0;
                            }
                            else {
                                c1 = c2 = c3 = 0;
...
cmsBool IsDegenerated(const cmsToneCurve* g)
{
    ...
    if (Zeros == 1 && Poles == 1) return 0;
    if (Zeros > (nEntries / 4)) return 1;
    if (Poles > (nEntries / 4)) return 1;
...
cmsBool OptimizeByComputingLinearization(cmsPipeline** Lut, cmsUInt32Number Intent, cmsUInt32Number* InputFormat, cmsUInt32Number* OutputFormat, cmsUInt32Number* dwFlags)
{
    ...
    int nGridPoints;
    cmsToneCurve *Trans[16], *TransReverse[16];
    cmsUInt32Number t, i;
    cmsFloat32Number v, In[16], Out[16];
    ...
    if ((((*InputFormat)>>16)&31) != 4) return 0;
    if ((((*OutputFormat)>>16)&31) != 4) return 0;



    if (!_cmsFormatterIs8bit(*InputFormat)) {
        if (!(*dwFlags & 0x0010)) return 0;
    }
   ...
   for (mpe = cmsPipelineGetPtrToFirstStage(OriginalLut);
    ...
    for (t = 0; t < OriginalLut ->InputChannels; t++) {
        Trans[t] = cmsBuildTabulatedToneCurve16(OriginalLut ->ContextID, 4096, ((void*)0));
    ...
    for (i=0; i < 4096; i++) {
        ...
        v = (cmsFloat32Number) ((cmsFloat64Number) i / (4096 - 1));
    ...
    for (t = 0; t < OriginalLut ->InputChannels; t++) {
        TransReverse[t] = cmsReverseToneCurveEx(4096, Trans[t]);
    ...
    OptimizedCLUTmpe = cmsStageAllocCLut16bit(OriginalLut ->ContextID, nGridPoints, OriginalLut ->InputChannels, OriginalLut ->OutputChannels, ((void*)0));
...
Error:
    ...
    if (LutPlusCurves != ((void*)0)) cmsPipelineFree(LutPlusCurves);
    if (OptimizedLUT != ((void*)0)) cmsPipelineFree(OptimizedLUT);
...
Curves16Data* CurvesAlloc(cmsContext ContextID, int nCurves, int nElements, cmsToneCurve** G)
{
    ...
    if (c16 == ((void*)0)) return ((void*)0);

    c16 ->nCurves = nCurves;
    c16 ->nElements = nElements;

    c16 ->Curves = _cmsCalloc(ContextID, nCurves, sizeof(cmsUInt16Number*));
    if (c16 ->Curves == ((void*)0)) return ((void*)0);

    for (i=0; i < nCurves; i++) {

        c16->Curves[i] = _cmsCalloc(ContextID, nElements, sizeof(cmsUInt16Number));

        if (c16->Curves[i] == ((void*)0)) {
        ...
        if (nElements == 256) {
            ...
            for (j=0; j < nElements; j++) {
                ...
                c16 ->Curves[i][j] = cmsEvalToneCurve16(G[i], (cmsUInt16Number) ((((cmsUInt16Number) (j)) << 8)|(j)));
            }
        }
        else {
...
{
    cmsToneCurve** GammaTables = ((void*)0);
    cmsFloat32Number InFloat[16], OutFloat[16];
    cmsUInt32Number i, j;
    cmsPipeline* Src = *Lut;
    cmsPipeline* Dest = ((void*)0);
    cmsStage* mpe;
    cmsStage* ObtainedCurves = ((void*)0);
    ...
    for (mpe = cmsPipelineGetPtrToFirstStage(Src);
         mpe != ((void*)0);
         mpe = cmsStageNext(mpe)) {
            if (cmsStageType(mpe) != cmsSigCurveSetElemType) return 0;
    }
    ...
    if (GammaTables == ((void*)0)) goto Error;

    for (i=0; i < Src ->InputChannels; i++) {
        GammaTables[i] = cmsBuildTabulatedToneCurve16(Src ->ContextID, 4096, ((void*)0));
        if (GammaTables[i] == ((void*)0)) goto Error;
    }
    ...
    for (i=0; i < 4096; i++) {
        ...
        for (j=0; j < Src ->InputChannels; j++)
            InFloat[j] = (cmsFloat32Number) ((cmsFloat64Number) i / (4096 - 1));
    ...
    if (ObtainedCurves == ((void*)0)) goto Error;

    for (i=0; i < Src ->InputChannels; i++) {
        cmsFreeToneCurve(GammaTables[i]);
        GammaTables[i] = ((void*)0);
    }

    if (GammaTables != ((void*)0)) _cmsFree(Src ->ContextID, GammaTables);
    ...
    if (!AllCurvesAreLinear(ObtainedCurves)) {
        ...
        *dwFlags |= 0x0040;
        _cmsPipelineSetOptimizationParameters(Dest, FastIdentity16, (void*) Dest, ((void*)0), ((void*)0));
    }
...
Error:
    ...
    if (ObtainedCurves != ((void*)0)) cmsStageFree(ObtainedCurves);
    if (GammaTables != ((void*)0)) {
        for (i=0; i < Src ->InputChannels; i++) {
            if (GammaTables[i] != ((void*)0)) cmsFreeToneCurve(GammaTables[i]);
        }

        _cmsFree(Src ->ContextID, GammaTables);
    }

    if (Dest != ((void*)0)) cmsPipelineFree(Dest);
    return 0;
...
void FillFirstShaper(cmsS1Fixed14Number* Table, cmsToneCurve* Curve)
{
    ...
    for (i=0; i < 256; i++) {
        ...
        Table[i] = ((cmsS1Fixed14Number) floor((y) * 16384.0 + 0.5));
    }
}
...
void FillSecondShaper(cmsUInt16Number* Table, cmsToneCurve* Curve, cmsBool Is8BitsOutput)
{
    ...
    for (i=0; i < 16385; i++) {
        ...
        if (Is8BitsOutput) {
            ...
            cmsUInt8Number b = (cmsUInt8Number) ((((w) * 65281 + 8388608) >> 24) & 0xFF);

            Table[i] = (cmsUInt16Number) ((((cmsUInt16Number) (b)) << 8)|(b));
        }
...
cmsBool SetMatShaper(cmsPipeline* Dest, cmsToneCurve* Curve1[3], cmsMAT3* Mat, cmsVEC3* Off, cmsToneCurve* Curve2[3], cmsUInt32Number* OutputFormat)
{
    ...
    for (i=0; i < 3; i++) {
        for (j=0; j < 3; j++) {
            p ->Mat[i][j] = ((cmsS1Fixed14Number) floor((Mat->v[i].n[j]) * 16384.0 + 0.5));
        }
    }
    ...
    for (i=0; i < 3; i++) {
        ...
        if (Off == ((void*)0)) {
            ...
            p ->Off[i] = ((cmsS1Fixed14Number) floor((Off->n[i]) * 16384.0 + 0.5));
        }
    }
...
cmsBool OptimizeMatrixShaper(cmsPipeline** Lut, cmsUInt32Number Intent, cmsUInt32Number* InputFormat, cmsUInt32Number* OutputFormat, cmsUInt32Number* dwFlags)
{
    ...
    if ((((*InputFormat)>>3)&15) != 3 || (((*OutputFormat)>>3)&15) != 3) return 0;


    if (!_cmsFormatterIs8bit(*InputFormat)) return 0;
    ...
    if (!cmsPipelineCheckAndRetreiveStages(Src, 4,
        ...
        &Curve1, &Matrix1, &Matrix2, &Curve2)) return 0;
    ...
    if (Data1 ->Offset != ((void*)0)) return 0;
    ...
    IdentityMat = 0;
    if (_cmsMAT3isIdentity(&res) && Data2 ->Offset == ((void*)0)) {
...
static _cmsOptimizationCollection DefaultOptimization[] = {
    ...
    { OptimizeByResampling, ((void*)0) }
};


_cmsOptimizationPluginChunkType _cmsOptimizationPluginChunk = { ((void*)0) };
                               ...
                               const struct _cmsContext_struct* src)
{
   ...
   _cmsOptimizationPluginChunkType* head = (_cmsOptimizationPluginChunkType*) src->chunks[OptimizationPlugin];
    ...
    (((ctx != ((void*)0))) ? (void) (0) : __assert_fail ("(ctx != ((void*)0))", "./lcms2/src/cmsopt.c", 1655, __PRETTY_FUNCTION__));
    (((head != ((void*)0))) ? (void) (0) : __assert_fail ("(head != ((void*)0))", "./lcms2/src/cmsopt.c", 1656, __PRETTY_FUNCTION__));
...
cmsBool _cmsRegisterOptimizationPlugin(cmsContext ContextID, cmsPluginBase* Data)
{
    ...
    if (Plugin ->OptimizePtr == ((void*)0)) return 0;
                             ...
                             cmsUInt32Number* dwFlags)
{
    ...
    if ((*PtrLut) ->Elements == ((void*)0)) {
        _cmsPipelineSetOptimizationParameters(*PtrLut, FastIdentity16, (void*) *PtrLut, ((void*)0), ((void*)0));
        return 1;
    }
