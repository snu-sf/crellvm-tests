...
gdev_prn_set_procs_planar(gx_device *dev)
{
    ...
    if (((pdev)->procs.dev_spec_op) == gx_default_dev_spec_op)
        ((pdev)->procs.dev_spec_op = (gdev_prn_dev_spec_op));
    return 0;
}
                         ...
                         int height, bool for_band)
{
    ...
    space->raster = ((uint)((((target->width * mdev.planes[0].depth) + (mdev.pad<<3) + ((8<<(mdev.log2_align_mod < 3 ? 3 : mdev.log2_align_mod)) - 1)) >> ((mdev.log2_align_mod < 3 ? 3 : mdev.log2_align_mod) + 3)) << (mdev.log2_align_mod < 3 ? 3 : mdev.log2_align_mod)));
    return 0;
}
