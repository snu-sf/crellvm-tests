# 1 "./jbig2dec/jbig2_page.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./jbig2dec/jbig2_page.c" 2
# 24 "./jbig2dec/jbig2_page.c"
...
jbig2_page_info (Jbig2Ctx *ctx, Jbig2Segment *segment, const uint8_t *segment_data)
{
  ...
  ctx->pages = ((Jbig2Page *)jbig2_realloc(ctx->allocator, (ctx->pages), (ctx->max_page_index <<= 2), sizeof(Jbig2Page)));

                for (j=index; j < ctx->max_page_index; j++) {
                    ctx->pages[j].state = JBIG2_PAGE_FREE;
                    ctx->pages[j].number = 0;
                    ctx->pages[j].image = ((void*)0);
    ...
    if (page->height == 0xFFFFFFFF && page->striped == 0) {
        jbig2_error(ctx, JBIG2_SEVERITY_WARNING, segment->number,
