...
const gx_io_device gs_iodev_lineedit =
    { "%lineedit%", iodev_dtype_stdio, { iodev_no_init, iodev_no_open_device, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, ((void*)0), ((void*)0), iodev_no_get_params, iodev_no_put_params }};



const gx_io_device gs_iodev_statementedit =
    { "%statementedit%", iodev_dtype_stdio, { iodev_no_init, iodev_no_open_device, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, ((void*)0), ((void*)0), iodev_no_get_params, iodev_no_put_params }};
...
zgetiodevice(i_ctx_t *i_ctx_p)
{
    ...
    iodev = gs_getiodevice(((gs_memory_t *)((i_ctx_p->memory).current)), (int)(op->value.intval));
    if (iodev == 0)
        return ((-15));
...
zfilelineedit(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !(((&*(op - 3))->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_file) << 8) + (0x20))) ) return ((!((((const byte *)&((&*(op - 3))->tas.type_attrs))[1]) == (t_file)) ? check_type_failed(&*(op - 3)) : (-7))); do { ins = (op - 3)->value.pfile; if (ins->read_id != ((op - 3)->tas.rsize)) { if (ins->read_id == 0 && ins->write_id == ((op - 3)->tas.rsize)) { int fcode = file_switch_to_read(op - 3); if (fcode < 0) return(fcode); } else { ins = (i_ctx_p->invalid_file_stream); } } } while (0); } while (0);


    initial_buf_size = statement ? 50 : 20;
    if (initial_buf_size > (16*1024*1024))
        return ((-13));
    if (!buf->data || (buf->size < initial_buf_size)) {
        count = 0;
        buf->data = (*(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), initial_buf_size, "zfilelineedit(buffer)");
...
rd:
    code = zreadline_from(ins, buf, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), &count, &in_eol);
    if (buf->size > (16*1024*1024)) {
        ...
        byte *nbuf = (*(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)))->procs.resize_string)(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), buf->data, buf->size, (16*1024*1024), "zfilelineedit(shrink buffer)");

        if (nbuf == 0)
            return ((-25));
        op->value.bytes = buf->data = nbuf;
        op->tas.rsize = buf->size = (16*1024*1024);
        return ((-13));
    }
        ...
        case ((int)(-4)):
            {
                ...
                ((&rfile)->value.pfile = (ins), ((&rfile)->tas.type_attrs = ((t_file) << 8) + ((0x20 + 0x40) | avm_system)), ((&rfile)->tas.rsize = (ins->read_id)));
                code = s_handle_read_exception(i_ctx_p, code, &rfile,
                    ((void*)0), 0, zfilelineedit);
            }
            break;
        case 1:
            {
                ...
                if (nsize >= (16*1024*1024)) {
                    code = ((-13));
                    break;
                }
                else if (nsize >= (16*1024*1024) / 2)
                    nsize= (16*1024*1024);
                else
                    nsize = buf->size * 2;
                nbuf = (*(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)))->procs.resize_string)(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), buf->data, buf->size, nsize, "zfilelineedit(grow buffer)");
    ...
    if (statement) {
        ...
        scanner_state state;
        ref ignore_value;
        uint depth = ref_stack_count(&((i_ctx_p->op_stack).stack));
        int code;
...
sc:
        gs_scanner_init_stream_options(&state, ts, 2);
        ialloc_set_space((&(i_ctx_p->memory)), avm_local);
        code = gs_scan_token(i_ctx_p, &ignore_value, &state);
        ref_stack_pop(&((i_ctx_p->op_stack).stack), ref_stack_count(&((i_ctx_p->op_stack).stack)) - (depth));
        if (code < 0)
            code = 2;
        switch (code) {
            case 0:
    ...
    buf->data = (*(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)))->procs.resize_string)(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), buf->data, buf->size, count, "zfilelineedit(resize buffer)");
    ...
    s = file_alloc_stream(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)), "zfilelineedit(stream)");
    if (s == 0)
        return ((-25));
    ...
    make_stream_file((((i_ctx_p->op_stack).stack).p), s, "r");
