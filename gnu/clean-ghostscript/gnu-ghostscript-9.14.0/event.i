# 1 "./openjpeg/libopenjpeg/event.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./openjpeg/libopenjpeg/event.c" 2
# 29 "./openjpeg/libopenjpeg/event.c"
...
# 30 "./openjpeg/libopenjpeg/event.c" 2
# 71 "./openjpeg/libopenjpeg/event.c"
...
OPJ_BOOL opj_event_msg(opj_event_mgr_t* p_event_mgr, OPJ_INT32 event_type, const char *fmt, ...) {
 ...
 if ((fmt != 00) && (p_event_mgr != 00)) {
  ...
  str_length = (strlen(fmt) > 512) ? 512 : strlen(fmt);
        (void)str_length;

  vsnprintf(message, 512, fmt, arg);
