...
zcopydevice2(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&op[-1])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))) ) return ((!((((const byte *)&((&op[-1])->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&op[-1]) : (-7)));
    ...
    if (code < 0)
        return code;
    new_dev->memory = ((gs_memory_t *)((i_ctx_p->memory).current));
    ((op - 1)->value.pdevice = (new_dev), ((op - 1)->tas.type_attrs = ((t_device) << 8) + ((((&(i_ctx_p->memory)))->current_space) | (0x10 + 0x20 +0x40))) );
...
zcurrentdevice(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.pdevice = (dev), ((op)->tas.type_attrs = ((t_device) << 8) + ((mem == 0 ? avm_foreign : imemory_space(mem)) | (0x10 + 0x20 +0x40))) );
...
zdevicename(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op) : (-7)));
    dname = op->value.pdevice->dname;
    ((op)->value.const_bytes = ((const byte *)dname), ((op)->tas.type_attrs = ((t_string) << 8) + (avm_foreign | (0x20 + 0x40))), ((op)->tas.rsize = (strlen(dname))));
...
{
    gx_device *dev = gs_currentdevice((i_ctx_p->pgs));
    gx_device *tdev = (*((dev)->procs.get_page_device)) (dev);
...
{
    return gs_flushpage((i_ctx_p->pgs));
}
...
zgetbitsrect(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&op[-7])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_device) << 8) + (0x20))) ) return ((!((((const byte *)&((&op[-7])->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&op[-7]) : (-7)));
    dev = op[-7].value.pdevice;
    do { if ( !((((const byte *)&((&op[-6])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-6])); if ( (ulong)(op[-6]).value.intval > (dev->width) ) return ((-15)); } while (0);
    rect.p.x = op[-6].value.intval;
    do { if ( !((((const byte *)&((&op[-5])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-5])); if ( (ulong)(op[-5]).value.intval > (dev->height) ) return ((-15)); } while (0);
    rect.p.y = op[-5].value.intval;
    do { if ( !((((const byte *)&((&op[-4])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-4])); if ( (ulong)(op[-4]).value.intval > (dev->width) ) return ((-15)); } while (0);
    w = op[-4].value.intval;
    do { if ( !((((const byte *)&((&op[-3])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-3])); if ( (ulong)(op[-3]).value.intval > (dev->height) ) return ((-15)); } while (0);
    h = op[-3].value.intval;
    ...
    if (((((const byte *)&((op - 1)->tas.type_attrs))[1]) == (t_null))) {
        ...
        static const gs_get_bits_options_t depths[17] = {
            0, (1L<<8), (1L<<9), 0, (1L<<10), 0, 0, 0, (1L<<11),
            0, 0, 0, (1L<<12), 0, 0, 0, (1L<<13)
        };
        gs_get_bits_options_t depth_option;
        int std_depth;

        do { if ( !((((const byte *)&((&op[-1])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-1])); if ( (ulong)(op[-1]).value.intval > (16) ) return ((-15)); } while (0);
        std_depth = (int)op[-1].value.intval;
        depth_option = depths[std_depth];
        if (depth_option == 0)
            return ((-15));
        options |= depth_option | (1L<<0);
        depth = (dev->color_info.num_components +
                 (options & (1L<<4) ? 0 : 1)) * std_depth;
    }
    if (w == 0)
        return ((-15));
    raster = (w * depth + 7) >> 3;
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_string) << 8) + (0x10))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op) : (-7)));
    num_rows = ((op)->tas.rsize) / raster;
    h = (((h) < (num_rows)) ? (h) : (num_rows));
    if (h == 0)
        return ((-15));
    rect.q.x = rect.p.x + w;
    rect.q.y = rect.p.y + h;
    ...
    code = (*((dev)->procs.get_bits_rectangle))(dev, &rect, &params, ((void*)0));
    if (code < 0)
        return code;
    ((op - 7)->value.intval = (h), ((op - 7)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    op[-6] = *op;
    ((op - 6)->tas.rsize = (h * raster));
...
zgetdevice(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.pdevice = ((gx_device *) dev), ((op)->tas.type_attrs = ((t_device) << 8) + (avm_foreign | (0x20 + 0x40))) );
...
zget_device_params(i_ctx_t *i_ctx_p, bool is_hardware)
{
    ...
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    stack_param_list_write(&list, &((i_ctx_p->op_stack).stack), &rkeys, ((i_ctx_p->memory).current));
    code = gs_get_device_or_hw_params(dev, (gs_param_list *) & list,
                                            is_hardware);
    if (code < 0) {

        if (list.count > 0)
            ref_stack_pop(&((i_ctx_p->op_stack).stack), list.count * 2 - 1);
        else
            ref_stack_push(&((i_ctx_p->op_stack).stack), 1);
        *(((i_ctx_p->op_stack).stack).p) = rkeys;
        return code;
    }
    pmark = ref_stack_index(&((i_ctx_p->op_stack).stack), list.count * 2);
...
{
    return zget_device_params(i_ctx_p, ((bool)0));
}

static int
zgethardwareparams(i_ctx_t *i_ctx_p)
{
    return zget_device_params(i_ctx_p, ((bool)1));
}
...
zmakewordimagedevice(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&op[-3])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-3])); if ( (ulong)(op[-3]).value.intval > (((unsigned int)~0 + (unsigned int)0) >> 1) ) return ((-15)); } while (0);
    do { if ( !((((const byte *)&((&op[-2])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-2])); if ( (ulong)(op[-2]).value.intval > (((unsigned int)~0 + (unsigned int)0) >> 1) ) return ((-15)); } while (0);
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_boolean)) ) return (check_type_failed(&*op));
    if (((((const byte *)&((op1)->tas.type_attrs))[1]) == (t_null))) {
        colors = 0;
        colors_size = -24;
    } else if (((((const byte *)&((op1)->tas.type_attrs))[1]) == (t_integer))) {
        ...
        if ( !((((const byte *)&((&*op1)->tas.type_attrs))[1]) == (t_string)) ) return (check_type_failed(&*op1));
        if (((op1)->tas.rsize) > 3 * 256)
            return ((-15));
        colors = op1->value.bytes;
        colors_size = ((op1)->tas.rsize);
    }
    if ((code = read_matrix(((gs_memory_t *)((i_ctx_p->memory).current)), op - 4, &imat)) < 0)
        return code;
    ...
    code = gs_makewordimagedevice(&new_dev, &imat,
                                  ...
                                  colors, colors_size,
                                  op->value.boolval, ((bool)1), ((gs_memory_t *)((i_ctx_p->memory).current)));
    if (code == 0) {
        new_dev->memory = ((gs_memory_t *)((i_ctx_p->memory).current));
        ((op - 4)->value.pdevice = (new_dev), ((op - 4)->tas.type_attrs = ((t_device) << 8) + (imemory_space(((i_ctx_p->memory).current)) | (0x10 + 0x20 +0x40))) );
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).bot);
    while (op != (((i_ctx_p->op_stack).stack).top)) {
        if (((((const byte *)&((op)->tas.type_attrs))[1]) == (t_device)))
            op->value.pdevice = 0;
        op++;
    }
}
...
{
    gs_nulldevice((i_ctx_p->pgs));
    invalidate_stack_devices(i_ctx_p);
    (((&(((int_gstate *)gs_state_client_data((i_ctx_p->pgs))))->pagedevice)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return 0;
}
...
zoutputpage(i_ctx_t *i_ctx_p)
{
    ...
    code = gs_output_page((i_ctx_p->pgs), (int)op[-1].value.intval,
                          op->value.boolval);
...
{
    uint count = ref_stack_counttomark(&((i_ctx_p->op_stack).stack));
    ref *prequire_all;
    ref *ppolicy;
    ref *pdev;
    gx_device *dev;
    ...
    if (count == 0)
        return ((-24));
    prequire_all = ref_stack_index(&((i_ctx_p->op_stack).stack), count);
    ppolicy = ref_stack_index(&((i_ctx_p->op_stack).stack), count + 1);
    pdev = ref_stack_index(&((i_ctx_p->op_stack).stack), count + 2);
    if (pdev == 0)
        return ((-17));
    do { if ( !((((const byte *)&((&*prequire_all)->tas.type_attrs))[1]) == (t_boolean)) ) return ((-20)); } while (0);
    do { if ( !(((&*pdev)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_device) << 8) + (0x10))) ) return ((!((((const byte *)&((&*pdev)->tas.type_attrs))[1]) == (t_device)) ? (-20) : (-7))); } while (0);
    dev = pdev->value.pdevice;
    code = stack_param_list_read(&list, &((i_ctx_p->op_stack).stack), 0, ppolicy,
                                 prequire_all->value.boolval, ((i_ctx_p->memory).current));
    if (code < 0)
        return code;
    old_width = dev->width;
    ...
    for (dest = count - 2, i = 0; i < count >> 1; i++)
        if (list.results[i] < 0) {
            *ref_stack_index(&((i_ctx_p->op_stack).stack), dest) =
                *ref_stack_index(&((i_ctx_p->op_stack).stack), count - (i << 1) - 2);
            gs_errorname(i_ctx_p, list.results[i],
                         ref_stack_index(&((i_ctx_p->op_stack).stack), dest - 1));
            dest -= 2;
        }
    (((&list)->memory)->procs.free_object((&list)->memory, (&list)->results, "iparam_list_release"));
    if (code < 0) {
        ref_stack_pop(&((i_ctx_p->op_stack).stack), dest + 1);
        return 0;
    }
    if (code > 0 || (code == 0 && (dev->width != old_width || dev->height != old_height))) {
        ...
        if (gs_currentdevice((i_ctx_p->pgs)) == dev) {
            bool was_open = dev->is_open;

            code = gs_setdevice_no_erase((i_ctx_p->pgs), dev);
    ...
    if (code < 0)
        return code;
    ref_stack_pop(&((i_ctx_p->op_stack).stack), count + 1);
    (((((i_ctx_p->op_stack).stack).p))->value.boolval = (code), (((((i_ctx_p->op_stack).stack).p))->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    (((&(((int_gstate *)gs_state_client_data((i_ctx_p->pgs))))->pagedevice)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return 0;
}
...
zsetdevice(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x10))) == (((t_device) << 8) + (0x10))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_device)) ? check_type_failed(&*op) : (-7)));
    ...
    code = gs_setdevice_no_erase((i_ctx_p->pgs), op->value.pdevice);
    if (code < 0)
        return code;


    ((op)->value.boolval = (code != 0), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    invalidate_stack_devices(i_ctx_p);
    (((&(((int_gstate *)gs_state_client_data((i_ctx_p->pgs))))->pagedevice)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return code;
}
