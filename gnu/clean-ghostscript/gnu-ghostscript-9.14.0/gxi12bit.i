...
# 23 "./base/gxi12bit.c" 2
# 1 "./base/gxfrac.h" 1
# 27 "./base/gxfrac.h"
typedef short frac;
typedef short signed_frac;
# 24 "./base/gxi12bit.c" 2
# 1 "./base/gxarith.h" 1
# 27 "./base/gxarith.h"
    ...
    if ((data_x & 1) && left > 0)
        switch (left) {
            default:
                ...
                *bufp = ((frac)( ((frac)(sample) << (15 - (12))) + ((sample) >> ((12) * 2 - 15)) - ((sample) >> ((12) - 3)) ));
                bufp = (frac *)((byte *)(bufp) + (spread));
                psrc += 3;
                left -= 3;
                break;
            case 2:
                *bufp = (psrc[1] & 0xf) * (((frac)0x7ff8) / 15);
            case 1:
                left = 0;
        }
    while (left >= 3) {
...
irender_proc_t gs_image_class_2_fracs(gx_image_enum *penum);


static int image_render_frac(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
static int image_render_icc16(gx_image_enum *penum, const byte *buffer, int data_x, uint w, int h, gx_device *dev);
        ...
        if (penum->use_mask_color) {
            ...
            for (i = 0; i < penum->spp * 2; ++i)
                penum->mask_color.values[i] =
                    ((frac)( ((frac)(penum->mask_color.values[i]) << (15 - (12))) + ((penum->mask_color.values[i]) >> ((12) * 2 - 15)) - ((penum->mask_color.values[i]) >> ((12) - 3)) ));
        }
                  ...
                  uint w, int h, gx_device * dev)
{
    ...
    void (*map_rgb)(frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t) = cmap_procs->map_rgb;
    void (*map_cmyk)(frac, frac, frac, frac, gx_device_color *, const gs_imager_state *, gx_device *, gs_color_select_t, const gs_color_space *) = cmap_procs->map_cmyk;
    ...
    xrun = xl = ((pnext.x).state).Q;
    irun = ((int)(((xrun)+((1<<8)>>1)) >> (8)));
    yrun = ytf = ((pnext.y).state).Q;
    ...
    ((&devc1)->colors.pure = (((gx_color_index)(~0))), (&devc1)->type = (&gx_dc_type_data_pure)); (&devc1)->ccolor_valid = ((bool)0);
    ((&devc2)->colors.pure = (((gx_color_index)(~0))), (&devc2)->type = (&gx_dc_type_data_pure)); (&devc2)->ccolor_valid = ((bool)0);
            ...
            case 4:
                ...
                if (device_color) {
                    (*map_cmyk) (next.v[0], next.v[1],
                                 next.v[2], next.v[3],
                                 pdevc_next, pis, dev,
                                 gs_color_select_source, ((void*)0));
                    goto f;
                }
                cc.paint.values[0] = penum->map[0].decode_lookup[0] + (next.v[0]) * penum->map[0].decode_factor;
                cc.paint.values[1] = penum->map[1].decode_lookup[0] + (next.v[1]) * penum->map[1].decode_factor;
                cc.paint.values[2] = penum->map[2].decode_lookup[0] + (next.v[2]) * penum->map[2].decode_factor;
                cc.paint.values[3] = penum->map[3].decode_lookup[0] + (next.v[3]) * penum->map[3].decode_factor;
                do { } while (0);
            ...
            default:
                {
                    ...
                    for (i = 0; i < spp; ++i)
                        cc.paint.values[i] = penum->map[i].decode_lookup[0] + (next.v[i]) * penum->map[i].decode_factor;
# 403 "./base/gxi12bit.c"
                }
                break;
        }
...
f:
        if (sizeof(pdevc_next->colors.binary.color[0]) <= sizeof(ulong))
            ...
            do { } while (0);
# 433 "./base/gxi12bit.c"
        if (!gx_device_color_equal(&(devc1), &(devc2))) {
...
fill:
        ...
        run = next;
inc:
        do { do { ((pnext.x).state).R -= ((pnext.x).step).dR; if ((signed)((pnext.x).state).R < 0) { ((pnext.x).state).Q++; ((pnext.x).state).R += ((pnext.x).step).N; }; ((pnext.x).state).Q += ((pnext.x).step).dQ; } while (0);(xl)=(pnext.x).state.Q; } while (0);
        do { do { ((pnext.y).state).R -= ((pnext.y).step).dR; if ((signed)((pnext.y).state).R < 0) { ((pnext.y).state).Q++; ((pnext.y).state).R += ((pnext.y).step).N; }; ((pnext.y).state).Q += ((pnext.y).step).dQ; } while (0);(ytf)=(pnext.y).state.Q; } while (0);
    }
                  ...
                  uint w, int h, gx_device * dev)
{
    ...
    const unsigned short *rsrc = psrc + spp;
    fixed xrun;
    int irun;
    fixed yrun;
    color_fracs run;
    ...
    if (penum->icc_link == ((void*)0)) {
        return gs_throw_imp(__func__, "./base/gxi12bit.c", 630, 1, -1, "ICC Link not created during image render icc16");
    }
    ...
    if (penum->icc_link->is_identity && !need_decode) {
        ...
        psrc_cm = (unsigned short*) (*(pis->memory)->procs.alloc_bytes)(pis->memory, sizeof(unsigned short) * w * spp_cm/spp, "image_render_icc16");
        ...
        if (penum->icc_link->is_identity) {
            ...
            gsicc_init_buffer(&input_buff_desc, spp, 2,
                          ((bool)0), ((bool)0), ((bool)0), 0, w * 2,
                          1, num_pixels);
            gsicc_init_buffer(&output_buff_desc, spp_cm, 2,
                          ((bool)0), ((bool)0), ((bool)0), 0, num_pixels * spp_cm * 2,
                          1, num_pixels);
            ...
            if (need_decode) {
                ...
                psrc_decode = (unsigned short*) (*(pis->memory)->procs.alloc_bytes)(pis->memory, sizeof(unsigned short) * w * spp_cm/spp, "image_render_icc16");
...
inc: xprev = ((pnext.x).state).Q;
    ...
    if (psrc_cm_start != ((void*)0)) {
        ((pis->memory)->procs.free_object(pis->memory, (byte *)psrc_cm_start, "image_render_icc16"));
    }
    return (code < 0 ? code : 1);
