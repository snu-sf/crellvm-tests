...
zlanguagelevel(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.intval = ((i_ctx_p->language_level)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zsetlanguagelevel(i_ctx_t *i_ctx_p)
{
    ...
    if (op->value.intval != (i_ctx_p->language_level)) {
        code = set_language_level(i_ctx_p, (int)op->value.intval);
        if (code < 0)
            return code;
    }
    (i_ctx_p->language_level) = op->value.intval;
...
{
    int old_level = (i_ctx_p->language_level);
    ref *pgdict =
        ref_stack_index(&((i_ctx_p->dict_stack).stack), ref_stack_count(&((i_ctx_p->dict_stack).stack)) - 2);
    ref *level2dict;
    int code = 0;

    if (new_level < 1 ||
        new_level >
        (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "ll3dict", &level2dict) > 0 ? 3 : 2)
        )
        return ((-15));
    if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "level2dict", &level2dict) <= 0)
        return ((-21));
    ...
    while (new_level != old_level) {
        switch (old_level) {
            case 1: {
                ...
                ((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->dict_auto_expand = ((bool)1);
                }
        ...
        switch (new_level) {
            case 1: {
                ...
                while ((index = dict_next(pgdict, index, &elt[0])) >= 0)
                    if (((((const byte *)&((&elt[0])->tas.type_attrs))[1]) == (t_name)))
                        names_invalidate_value_cache(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, &elt[0]);

                *pgdict = *(&(i_ctx_p->dict_stack).system_dict);

                ((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->dict_auto_expand = ((bool)0);
                }
                code = swap_level_dict(i_ctx_p, "level2dict");
                break;
            case 3:
...
swap_level_dict(i_ctx_t *i_ctx_p, const char *dict_name)
{
    ...
    if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), dict_name, &pleveldict) <= 0)
        return ((-21));
    rleveldict = *pleveldict;
    index = dict_first(&rleveldict);
    while ((index = dict_next(&rleveldict, index, &elt[0])) >= 0)
        if (((((const byte *)&((&elt[1])->tas.type_attrs))[1]) == (t_dictionary)) &&
            dict_find(&elt[1], &elt[0], &psubdict) > 0 &&
            obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), &elt[1], psubdict)
            ) {

            int isub = dict_first(&elt[1]);
            ref subelt[2];
            int found = dict_find((&(i_ctx_p->dict_stack).system_dict), &elt[0], &psubdict);
            ref rsubdict;
            ...
            while ((isub = dict_next(&elt[1], isub, &subelt[0])) >= 0)
                if (!obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), &subelt[0], &elt[0])) {
            ...
            int code = swap_entry(i_ctx_p, elt, (&(i_ctx_p->dict_stack).system_dict), &rleveldict);
...
swap_entry(i_ctx_t *i_ctx_p, ref elt[2], ref * pdict, ref * pdict2)
{
    ...
    switch (found) {
        ...
        case 0:
            (((&old_value)->tas.type_attrs = ((t_null) << 8) + (0)) );
            break;
        case 1:
            old_value = *pvalue;
    }
    ...
    {
        uint space2 = (avm_space)(((pdict2)->tas.type_attrs) & (((1 << 2) - 1) << 2));
        int code;

        ((pdict2)->tas.type_attrs = ((pdict2)->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint)avm_local));
        dict_put(pdict2, &elt[0], &old_value, &(i_ctx_p->dict_stack));
        if (((((const byte *)&((&elt[1])->tas.type_attrs))[1]) == (t_null))) {
            code = dict_undef(pdict, &elt[0], &(i_ctx_p->dict_stack));
            if (code == (-21) &&
                ((((const byte *)&((&old_value)->tas.type_attrs))[1]) == (t_null))
                )
                code = 0;
        } else {
            uint space = (avm_space)(((pdict)->tas.type_attrs) & (((1 << 2) - 1) << 2));

            ((pdict)->tas.type_attrs = ((pdict)->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint)avm_local));
            code = dict_put(pdict, &elt[0], &elt[1], &(i_ctx_p->dict_stack));
            ((pdict)->tas.type_attrs = ((pdict)->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint)space));
        }
        ((pdict2)->tas.type_attrs = ((pdict2)->tas.type_attrs & ~((((1 << 2) - 1) << 2))) | ((uint)space2));
        return code;
    }
}
