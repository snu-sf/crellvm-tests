...
gs_jpeg_create_compress(stream_DCT_state * st)
{
    ...
    do { (st->data.compress)->Picky = 0; (st->data.compress)->Relax = 0; (st->data.compress)->dummy = 0;} while (0);
    jpeg_CreateCompress((&st->data.compress->cinfo), 90, (size_t) sizeof(struct jpeg_compress_struct));
    return 0;
}
