...
zgetdevparams(i_ctx_t *i_ctx_p)
{
    ...
    iodev = gs_findiodevice(((gs_memory_t *)((i_ctx_p->memory).current)), op->value.bytes, ((op)->tas.rsize));
    if (iodev == 0)
        return ((-21));
    stack_param_list_write(&list, &((i_ctx_p->op_stack).stack), ((void*)0), ((i_ctx_p->memory).current));
    if ((code = gs_getdevparams(iodev, plist)) < 0) {
        ref_stack_pop(&((i_ctx_p->op_stack).stack), list.count * 2);
        return code;
    }
...
zputdevparams(i_ctx_t *i_ctx_p)
{
    ...
    if (iodev == 0)
        return ((-21));
    code = stack_param_list_read(&list, &((i_ctx_p->op_stack).stack), 1, ((void*)0), ((bool)0), ((i_ctx_p->memory).current));
    if (code < 0)
        return code;
    code = dict_read_password(&system_params_password, (&(i_ctx_p->dict_stack).system_dict),
    ...
    if (code < 0)
        return code;
    ref_stack_pop(&((i_ctx_p->op_stack).stack), list.count * 2 + 2);
    return 0;
}
