...
set_float_value(i_ctx_t *i_ctx_p, int (*set_value)(gs_state *, double))
{
    ...
    if ((code = set_value((i_ctx_p->pgs), value)) < 0)
        return code;
                    ...
                    float (*current_value)(const gs_state *))
{
    ...
    ((op)->value.realval = (current_value((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_real) << 8) + (0)) );
    return 0;
}
           ...
           const char *const names[])
{
    ...
    names_string_ref(mem->gs_lib_ctx->gs_name_table, pnref, &nsref);
    for (p = names; *p; ++p)
        if (((&nsref)->tas.rsize) == strlen(*p) &&
            !memcmp(*p, nsref.value.const_bytes, ((&nsref)->tas.rsize))
            )
            return p - names;
    return ((-15));
}
...
static const char *const blend_mode_names[] = {
    "Normal", "Multiply", "Screen", "Difference", "Darken", "Lighten", "ColorDodge", "ColorBurn", "Exclusion", "HardLight", "Overlay", "SoftLight", "Luminosity", "Hue", "Saturation", "Color", "Compatible", 0
};
...
zsetblendmode(i_ctx_t *i_ctx_p)
{
    ...
    if ((code = enum_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, blend_mode_names)) < 0 ||
        (code = gs_setblendmode((i_ctx_p->pgs), code)) < 0
        )
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    const char *mode_name = blend_mode_names[gs_currentblendmode((i_ctx_p->pgs))];
    ref nref;
    int code = names_enter_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, mode_name, &nref);
...
zsettextknockout(i_ctx_t *i_ctx_p)
{
    ...
    gs_settextknockout((i_ctx_p->pgs), op->value.boolval);
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
...
zcurrenttextknockout(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.boolval = (gs_currenttextknockout((i_ctx_p->pgs))), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
...
{
    int csel;
    int code = int_param((((i_ctx_p->op_stack).stack).p), 1, &csel);

    if (code < 0)
        return code;
    code = mask_proc((i_ctx_p->pgs), csel);
    if (code >= 0)
        ((((i_ctx_p->op_stack).stack).p) -= (1));
    return code;
...
zbegintransparencygroup(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*dop)->tas.type_attrs))[1]) == (t_dictionary)) ) return (check_type_failed(&*dop));
    do { if ( !(((&*(&(&*dop)->value.pdict->values))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
    gs_trans_group_params_init(&params);
    if ((code = dict_bool_param(dop, "Isolated", ((bool)0), &params.Isolated)) < 0 ||
        (code = dict_bool_param(dop, "Knockout", ((bool)0), &params.Knockout)) < 0 ||
        (code = dict_bool_param(dop, ".image_with_SMask", ((bool)0), &params.image_with_SMask)) < 0
        )
        return code;
    ...
    if (dict_find_string(dop, "CS", &dummy) <= 0) {
        params.ColorSpace = ((void*)0);
    } else {

        params.ColorSpace = gs_currentcolorspace((i_ctx_p->pgs));
        ...
        if (gs_color_space_is_PSCIE(params.ColorSpace))
            ...
            params.ColorSpace->cmm_icc_profile_data != ((void*)0) &&
            params.ColorSpace->cmm_icc_profile_data->profile_handle != ((void*)0)) {
            if (gscms_is_input(params.ColorSpace->cmm_icc_profile_data->profile_handle))
                params.ColorSpace = ((void*)0);
        }
    }
    code = gs_begin_transparency_group((i_ctx_p->pgs), &params, &bbox);
    if (code < 0)
        return code;
...
{
    return gs_end_transparency_group((i_ctx_p->pgs));
}
...
zbegintransparencymaskgroup(i_ctx_t *i_ctx_p)
{
    ...
    if (dict_find_string(dop, "Subtype", &pparam) <= 0)
        return ((-15));
    if ((code = enum_param(((gs_memory_t *)((i_ctx_p->memory).current)), pparam, subtype_names)) < 0)
        return code;
    gs_trans_mask_params_init(&params, code);
    params.replacing = ((bool)1);
    if ((code = dict_floats_param(((gs_memory_t *)((i_ctx_p->memory).current)), dop, "Background",
                    (*(gs_currentcolorspace(i_ctx_p->pgs))->type->num_components)(gs_currentcolorspace(i_ctx_p->pgs)),
                                  params.Background, ((void*)0))) < 0)
        return code;
    else if (code > 0)
        params.Background_components = code;
    if ((code = dict_floats_param(((gs_memory_t *)((i_ctx_p->memory).current)), dop, "GrayBackground",
                    1, &params.GrayBackground, ((void*)0))) < 0)
        return code;
    ...
    code = gs_begin_transparency_mask((i_ctx_p->pgs), &params, &bbox, ((bool)0));
    if (code < 0)
        return code;
...
zbegintransparencymaskimage(i_ctx_t *i_ctx_p)
{
    ...
    int code;
    gs_color_space *gray_cs = gs_cspace_new_DeviceGray(((gs_memory_t *)((i_ctx_p->memory).current)));

    if (!gray_cs)
        return ((-25));
    gs_trans_mask_params_init(&params, TRANSPARENCY_MASK_Luminosity);
    code = gs_begin_transparency_mask((i_ctx_p->pgs), &params, &bbox, ((bool)1));
    if (code < 0)
        return code;
...
zimage3x(i_ctx_t *i_ctx_p)
{
    ...
    int num_components =
        gs_color_space_num_components(gs_currentcolorspace((i_ctx_p->pgs)));
    int ignored;
    int code;
    ...
    gs_image3x_t_init(&image, ((void*)0));
    if (dict_find_string(op, "DataDict", &pDataDict) <= 0)
        return ((-15));
    if ((code = pixel_image_params(i_ctx_p, pDataDict,
                   (gs_pixel_image_t *)&image, &ip_data,
                   16, ((bool)0), gs_currentcolorspace((i_ctx_p->pgs)))) < 0 ||
    ...
    if ((code = mask_dict_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, &ip_data,
                                "ShapeMaskDict", num_components,
                                &image.Shape)) < 0 ||
        (code = mask_dict_param(((gs_memory_t *)((i_ctx_p->memory).current)), op, &ip_data,
                ...
                int num_components, gs_image3x_mask_t *pixm)
{
    ...
    if ((mcode = code = data_image_params(mem, pMaskDict, &pixm->MaskDict,
                                          &ip_mask, ((bool)0), 1, 16, ((bool)0), ((bool)0))) < 0 ||
    ...
    if (pixm->InterleaveType == 3) {
        ...
        memmove(&pip_data->DataSource[1], &pip_data->DataSource[0],
                ((sizeof(pip_data->DataSource) / sizeof((pip_data->DataSource)[0])) - 1) *
...
zpushpdf14devicefilter(i_ctx_t *i_ctx_p)
{
    ...
    code = gs_push_pdf14trans_device((i_ctx_p->pgs), ((bool)0));
    if (code < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
...
{
    return gs_pop_pdf14trans_device((i_ctx_p->pgs), ((bool)0));
}
...
{
    return gs_abort_pdf14trans_device((i_ctx_p->pgs));
}
...
{
    int code;
    code = gs_push_transparency_state((i_ctx_p->pgs));
    return(code);
}
...
{
    int code;
    code = gs_pop_transparency_state((i_ctx_p->pgs), ((bool)0));
    return(code);
}
