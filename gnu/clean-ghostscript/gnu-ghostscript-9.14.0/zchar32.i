...
zmakeglyph32(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&op[-4])->tas.type_attrs))[1]) == (t_array)) ) return (check_type_failed(&op[-4])); } while (0);
    msize = ((op - 4)->tas.rsize);
    switch (msize) {
        case 10:
            long_form = ((bool)1);
            break;
    ...
    if (~code & 0x3c)
        return ((-20));
    if ( !(((&op[-3])->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&op[-3])->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&op[-3]) : (-7)));
    llx = (int)metrics[2];
    lly = (int)metrics[3];
    urx = (int)metrics[4];
    ury = (int)metrics[5];
    ...
    height = ury - lly;
    raster = (width + 7) >> 3;
    if (width < 0 || height < 0 || ((op - 3)->tas.rsize) != raster * height)
        return ((-15));
    do { if ( !((((const byte *)&((&op[-2])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-2])); if ( (ulong)(op[-2]).value.intval > (65535) ) return ((-15)); } while (0);
...
zremoveglyphs(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&op[-2])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-2])); if ( (ulong)(op[-2]).value.intval > (65535) ) return ((-15)); } while (0);
    do { if ( !((((const byte *)&((&op[-1])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-1])); if ( (ulong)(op[-1]).value.intval > (65535) ) return ((-15)); } while (0);
    code = font_param(op, &range.font);
    if (code < 0)
        return code;
    if (range.font->FontType != ft_CID_bitmap)
        return ((-10));
    range.cid_min = ((gs_glyph)0x80000000L) + op[-2].value.intval;
    range.cid_max = ((gs_glyph)0x80000000L) + op[-1].value.intval;
...
zgetmetrics32(i_ctx_t *i_ctx_p)
{
    ...
    if (data[0]) {
        ...
        n = 6;
        size = 5;
        do { if ( (op += (8)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (8); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
        ((op - 6)->value.intval = (data[2]), ((op - 6)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        ((op - 5)->value.intval = (0), ((op - 5)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        ((op - 4)->value.intval = (llx), ((op - 4)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        ((op - 3)->value.intval = (lly), ((op - 3)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        ((op - 2)->value.intval = (llx + data[0]), ((op - 2)->tas.type_attrs = ((t_integer) << 8) + (0)) );
        ((op - 1)->value.intval = (lly + data[1]), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    } else {
        if (data[1]) {
        ...
        do { if ( (op += (2 + n)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (2 + n); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
        for (i = 0; i < n; ++i)
            ((op - n + i)->value.intval = (((int)((data[2 * i + 2] << 8) + data[2 * i + 3]) ^ 0x8000) - 0x8000), ((op - n + i)->tas.type_attrs = ((t_integer) << 8) + (0)) );


    }
    wop = op - n;
    ((wop - 2)->value.intval = (wop[4].value.intval - wop[2].value.intval), ((wop - 2)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((wop - 1)->value.intval = (wop[5].value.intval - wop[3].value.intval), ((wop - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((op)->value.intval = (size), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
