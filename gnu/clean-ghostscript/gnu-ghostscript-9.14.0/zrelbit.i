...
zeq(i_ctx_t *i_ctx_p)
{
    ...
    switch ( (((const byte *)&((op - 1)->tas.type_attrs))[1]) ) { case t_string: do { if ( !(((&*(op - 1))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0); break; default: if ( op < (((i_ctx_p->op_stack).stack).bot) + ((2) - 1) ) return ((-17)); };
    switch ( (((const byte *)&((op)->tas.type_attrs))[1]) ) { case t_string: do { if ( !(((&*(op))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0); break; default: do { } while (0); };
    ((op - 1)->value.boolval = ((obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, op) ? 1 : 0)), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
...
zne(i_ctx_t *i_ctx_p)
{
    ...
    if (!code)
        (((i_ctx_p->op_stack).stack).p)->value.boolval ^= 1;
    return code;
}
...
zgt(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    ((op - 1)->value.boolval = (code ^ 1), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
...
zbitshift(i_ctx_t *i_ctx_p)
{
    ...
    if ((op->value.intval < -max_shift) || (op->value.intval > max_shift))
        op[-1].value.intval = 0;
    else if (sizeof(ps_int) != 4 && gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current))) && (op->value.intval < -max_shift32 || op->value.intval > max_shift32))
        op[-1].value.intval = 0;
...
zidenteq(i_ctx_t *i_ctx_p)
{
    ...
    switch ( (((const byte *)&((op - 1)->tas.type_attrs))[1]) ) { case t_string: do { if ( !(((&*(op - 1))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0); break; default: if ( op < (((i_ctx_p->op_stack).stack).bot) + ((2) - 1) ) return ((-17)); };
    switch ( (((const byte *)&((op)->tas.type_attrs))[1]) ) { case t_string: do { if ( !(((&*(op))->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0); break; default: do { } while (0); };
    ((op - 1)->value.boolval = ((obj_ident_eq(((gs_memory_t *)((i_ctx_p->memory).current)), op - 1, op) ? 1 : 0)), ((op - 1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
...
obj_le(register os_ptr op1, register os_ptr op)
{
    switch ((((const byte *)&((op1)->tas.type_attrs))[1])) {
        ...
        case t_string:
            ...
            return (bytes_compare(op1->value.bytes, ((op1)->tas.rsize),
                                  op->value.bytes, ((op)->tas.rsize)) <= 0);
        default:
