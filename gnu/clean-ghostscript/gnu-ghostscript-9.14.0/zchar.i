...
zshow(i_ctx_t *i_ctx_p)
{
    ...
    if (code != 0 ||
        (code = gs_show_begin((i_ctx_p->pgs), op->value.bytes, ((op)->tas.rsize), ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0)
        return code;
    *(op_proc_t *)&penum->enum_client_data = zshow;
    if ((code = op_show_finish_setup(i_ctx_p, penum, 1, finish_show)) < 0) {
        ((((gs_memory_t *)((i_ctx_p->memory).current)))->procs.free_object(((gs_memory_t *)((i_ctx_p->memory).current)), penum, "op_show_enum_setup"));
        return code;
    }
...
zashow(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0 ||
        ...
        (code = gs_ashow_begin((i_ctx_p->pgs), axy[0], axy[1], op->value.bytes, ((op)->tas.rsize), ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0)
        return code;
...
widthshow_aux(i_ctx_t *i_ctx_p, bool single_byte_space)
{
    ...
    if (gs_currentfont((i_ctx_p->pgs))->FontType == ft_composite) {
    ...
    if ((code = num_params(op - 2, 2, cxy)) < 0 )
        return code;
    if ((code = gs_widthshow_begin((i_ctx_p->pgs), cxy[0], cxy[1],
                                   (gs_char) op[-1].value.intval,
                                   op->value.bytes, ((op)->tas.rsize),
                                   ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0)
        return code;
...
{
    return(widthshow_aux(i_ctx_p, ((bool)0)));
}
# 152 "./psi/zchar.c"
static int
zpdfwidthshow(i_ctx_t *i_ctx_p)
{
    return(widthshow_aux(i_ctx_p, ((bool)1)));
}
...
awidthshow_aux(i_ctx_t *i_ctx_p, bool single_byte_space)
{
    ...
    if ((code = num_params(op - 1, 2, axy)) < 0 )
        return code;
    if ( !((((const byte *)&((&op[-3])->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&op[-3]));
    if (gs_currentfont((i_ctx_p->pgs))->FontType == ft_composite) {
    ...
    if ((code = num_params(op - 4, 2, cxy)) < 0 )
        return code;
    if ((code = gs_awidthshow_begin((i_ctx_p->pgs), cxy[0], cxy[1],
...
{
    return(awidthshow_aux(i_ctx_p, ((bool)0)));
}
...
{
    return(awidthshow_aux(i_ctx_p, ((bool)1)));
}
...
zkshow(i_ctx_t *i_ctx_p)
{
    ...
    if (gs_currentfont((i_ctx_p->pgs))->FontType == ft_composite)
        return ((-10));
    if ((code = op_show_setup(i_ctx_p, op)) != 0 ||
        (code = gs_kshow_begin((i_ctx_p->pgs), op->value.bytes, ((op)->tas.rsize),
                               ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), &penum)) < 0)
        return code;
    *(op_proc_t *)&penum->enum_client_data = zkshow;
    ...
    (((((i_ctx_p->exec_stack).stack).p))[-1]) = op[-1];
...
zstringwidth(i_ctx_t *i_ctx_p)
{
    ...
    if (code != 0 ||
        (code = gs_stringwidth_begin((i_ctx_p->pgs), op->value.bytes, ((op)->tas.rsize),
                                     ((gs_memory_t *)((i_ctx_p->memory).current)), &penum)) < 0)
        return code;
...
finish_stringwidth(i_ctx_t *i_ctx_p)
{
    ...
    gs_text_total_width(((gs_text_enum_t *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct)), &width);
                        ...
                        bool, gs_memory_t *, gs_text_enum_t **))
{
    ...
    if (code != 0 ||
        (code = begin((i_ctx_p->pgs), op[-1].value.bytes, ((op - 1)->tas.rsize),
                      op->value.boolval, ((gs_memory_t *)((i_ctx_p->memory).current)), &penum)) < 0)
        return code;
...
zsetcachedevice(i_ctx_t *i_ctx_p)
{
    ...
    if (code < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (6));
    if (code == 1)
        (((&(((int_gstate *)(((i_ctx_p->pgs))->client_data)))->pagedevice)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return 0;
}
...
zsetcachedevice2(i_ctx_t *i_ctx_p)
{
    ...
    if (zchar_show_width_only(penum))
        return op_show_return_width(i_ctx_p, 10,
                                    (gs_rootfont((i_ctx_p->pgs))->WMode ?
                                     &wbox[6] : &wbox[0]));
    code = gs_text_setcachedevice2(penum, wbox);
    if (code < 0)
        return code;
    ((((i_ctx_p->op_stack).stack).p) -= (10));
    if (code == 1)
        (((&(((int_gstate *)(((i_ctx_p->pgs))->client_data)))->pagedevice)->tas.type_attrs = ((t_null) << 8) + (0)) );
    return 0;
}
...
zincachedevice(i_ctx_t *i_ctx_p)
{
    ...
    ((op)->value.boolval = (!!(i_ctx_p->pgs)->in_cachedevice), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    return 0;
}
...
glyph_ref(const gs_memory_t *mem, gs_glyph glyph, ref * gref)
{
    if (glyph < ((gs_glyph)0x80000000L))
        names_index_ref(mem->gs_lib_ctx->gs_name_table, glyph, gref);
    else
        ((gref)->value.intval = (glyph - ((gs_glyph)0x80000000L)), ((gref)->tas.type_attrs = ((t_integer) << 8) + (0)) );
}
...
{
    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (9 + 2) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), 9 + 2); if ( es_code_ < 0 ) return es_code_; };
    return 0;
}
                     ...
                     op_proc_t endproc )
{
    ...
    es_ptr ep = (((i_ctx_p->exec_stack).stack).p) + 9;
    gs_glyph glyph;

    if (gs_currentcpsimode((i_ctx_p->pgs)->memory)) {



        int code = gs_text_count_chars((i_ctx_p->pgs), gs_get_text_params(penum), ((gs_memory_t *)((i_ctx_p->memory).current)));
    ...
    if (osenum &&
        (((osenum)->text.operation & (0x00001 | 0x00200 | 0x10000)) == (0x00001 | 0x00200 | 0x10000)) &&

        (((penum)->text.operation & (0x00001 | 0x20000)) == (0x00001 | 0x20000)) &&
        (glyph = gs_text_current_glyph(osenum)) != ((gs_glyph)0x7fffffff) &&
        glyph >= ((gs_glyph)0x80000000L) &&



        gs_default_same_font (gs_text_current_font(osenum),
                              gs_text_current_font(penum), ((bool)1))
        ) {
    ...
    if (osenum == ((void*)0) && !(penum->text.operation & (0x00008 | 0x00020))) {
        int ft = (i_ctx_p->pgs)->root_font->FontType;

        if ((ft >= ft_CID_encrypted && ft <= ft_CID_TrueType) || ft == ft_CID_bitmap)
            return ((-20));
    }
    ((ep - (9 - 1))->value.opproc = (op_show_cleanup), ((ep - (9 - 1))->tas.type_attrs = ((t_null) << 8) + (0x80)), ((ep - (9 - 1))->tas.rsize = (1)));
    if (endproc == ((void*)0))
        endproc = finish_show;
    (((&((ep)[-1]))->tas.type_attrs = ((t_null) << 8) + (0)) );
    ((&((ep)[-2]))->value.intval = (((&((i_ctx_p->op_stack).stack))->p + 1 - (&((i_ctx_p->op_stack).stack))->bot + (&((i_ctx_p->op_stack).stack))->extension_used) - npop), ((&((ep)[-2]))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((&((ep)[-3]))->value.intval = (((&((i_ctx_p->dict_stack).stack))->p + 1 - (&((i_ctx_p->dict_stack).stack))->bot + (&((i_ctx_p->dict_stack).stack))->extension_used)), ((&((ep)[-3]))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    ((&((ep)[-4]))->value.intval = ((i_ctx_p->pgs)->level), ((&((ep)[-4]))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    (((&((ep)[-5]))->tas.type_attrs = ((t_null) << 8) + (0)) );
    (((&((ep)[-6]))->tas.type_attrs = ((t_null) << 8) + (0)) );
    ((&((ep)[-7]))->value.opproc = (endproc), ((&((ep)[-7]))->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((&((ep)[-7]))->tas.rsize = (0)));
    ((ep)->value.pstruct = ((obj_header_t *)(penum)), ((ep)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
...
{
    int code = gs_text_update_dev_color((i_ctx_p->pgs), ((gs_text_enum_t *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct)));

    if (code >= 0)
        code = op_show_continue_dispatch(i_ctx_p, 0, gs_text_process(((gs_text_enum_t *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct))));
    return code;
}
int
op_show_continue_pop(i_ctx_t *i_ctx_p, int npop)
{
    return op_show_continue_dispatch(i_ctx_p, npop, gs_text_process(((gs_text_enum_t *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct))));
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p) - npop;
    gs_text_enum_t *penum = ((gs_text_enum_t *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct));

    switch (code) {
        case 0: {
            os_ptr save_osp = (((i_ctx_p->op_stack).stack).p);

            (((i_ctx_p->op_stack).stack).p) = op;
            code = (*((&(((((i_ctx_p->exec_stack).stack).p))[-7]))->value.opproc)) (i_ctx_p);
            op_show_free(i_ctx_p, code);
            if (code < 0) {
                (((i_ctx_p->op_stack).stack).p) = save_osp;
                return code;
            }
            return 14;
        }
        case 2: {
            ref *pslot = &(((((i_ctx_p->exec_stack).stack).p))[-1]);

            do { if ( (op += (2)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (2); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
            ((op - 1)->value.intval = (gs_text_current_char(penum)), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
            ((op)->value.intval = (gs_text_next_char(penum)), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (op_show_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
            *++(((i_ctx_p->exec_stack).stack).p) = *pslot;
            return 5;
        }
        case 1: {
            gs_font *pfont = gs_currentfont((i_ctx_p->pgs));
...
# 639 "./psi/zchar.c"
            if (pfont->FontType == ft_user_defined) {
                ...
                if (((i_ctx_p->language_level) >= 2) &&
                    !((((const byte *)&((&pfdata->BuildGlyph)->tas.type_attrs))[1]) == (t_null)) &&
                    glyph != ((gs_glyph)0x7fffffff)
                    ) {
                    glyph_ref(((gs_memory_t *)((i_ctx_p->memory).current)), glyph, op);
                    (((i_ctx_p->exec_stack).stack).p)[2] = pfdata->BuildGlyph;
                    ...
                    glyph_ref(((gs_memory_t *)((i_ctx_p->memory).current)), glyph, &gref);
                    if (!map_glyph_to_char(((gs_memory_t *)((i_ctx_p->memory).current)), &gref, pencoding,
                                           (ref *) op)
                        ) {
                        names_enter_string(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, ".notdef", &gref);
                        if (!map_glyph_to_char(((gs_memory_t *)((i_ctx_p->memory).current)), &gref,
                                               pencoding,
                                               (ref *) op)
                            )
                            goto err;
                    }
                    (((i_ctx_p->exec_stack).stack).p)[2] = pfdata->BuildChar;
                } else {
                    ((op)->value.intval = (chr & 0xff), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
                    (((i_ctx_p->exec_stack).stack).p)[2] = pfdata->BuildChar;
                }
            } else {
                ...
                if (chr != ((gs_char)~0L) &&
                    !((((const byte *)&((&pfdata->BuildChar)->tas.type_attrs))[1]) == (t_null)) &&
                    (glyph == ((gs_glyph)0x7fffffff) ||
                     (!((((const byte *)&((&pfdata->Encoding)->tas.type_attrs))[1]) == (t_null)) &&
                       array_get(((gs_memory_t *)((i_ctx_p->memory).current)), &pfdata->Encoding, (long)(chr & 0xff), &eref) >= 0 &&
                      (glyph_ref(((gs_memory_t *)((i_ctx_p->memory).current)), glyph, &gref), obj_eq(((gs_memory_t *)((i_ctx_p->memory).current)), &gref, &eref))))
                    ) {
                    ((op)->value.intval = (chr & 0xff), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
                    (((i_ctx_p->exec_stack).stack).p)[2] = pfdata->BuildChar;
                } else {
            ...
            (((((i_ctx_p->exec_stack).stack).p))[-2]).value.intval = ref_stack_count(&((i_ctx_p->op_stack).stack)) - 2;
            (((((i_ctx_p->exec_stack).stack).p))[-3]).value.intval = ref_stack_count(&((i_ctx_p->dict_stack).stack));
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (op_show_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
        ...
        case 3:
            { gs_font *pfont = penum->current_font;
                ...
                ((((i_ctx_p->op_stack).stack).p) -= (npop));
                op = (((i_ctx_p->op_stack).stack).p);
                glyph_ref(((gs_memory_t *)((i_ctx_p->memory).current)), glyph, &cnref);
...
op_show_find(i_ctx_t *i_ctx_p)
{
    ...
    if (index == 0)
        return 0;
    return ((gs_text_enum_t *)((ref_stack_index(&((i_ctx_p->exec_stack).stack), index - (9 - 1)))->value.pstruct));
...
{
    uint index = op_show_find_index(i_ctx_p);
    es_ptr ep = (es_ptr) ref_stack_index(&((i_ctx_p->exec_stack).stack), index - (9 - 1));
    int code = gs_text_setcharwidth(((gs_text_enum_t *)((ep)->value.pstruct)), pwidth);
    uint ocount, dsaved, dcount;

    if (code < 0)
        return code;

    ocount = ref_stack_count(&((i_ctx_p->op_stack).stack)) - (uint) ((ep)[-2]).value.intval;
    if (ocount < npop)
        return ((-17));
    dsaved = (uint) ((ep)[-3]).value.intval;
    dcount = ref_stack_count(&((i_ctx_p->dict_stack).stack));
    if (dcount < dsaved)
        return ((-4));
    while (dcount > dsaved) {
        code = zend(i_ctx_p);
    ...
    ref_stack_pop(&((i_ctx_p->op_stack).stack), ocount);


    pop_estack(i_ctx_p, index - 9);
    return 14;
}
...
{
    register es_ptr ep = (((i_ctx_p->exec_stack).stack).p) + 9;
    gs_text_enum_t *penum = ((gs_text_enum_t *)((ep)->value.pstruct));
    int saved_level = ((ep)[-4]).value.intval;
    int code = 0;

    if (for_error) {
# 868 "./psi/zchar.c"
        if (ep[1].value.opproc == op_show_continue && penum->enum_client_data != ((void*)0)) {

            op_proc_t proc;

            *(void **)&proc = penum->enum_client_data;
            ((ep + 1)->value.opproc = (proc), ((ep + 1)->tas.type_attrs = ((t_operator) << 8) + (0x80)), ((ep + 1)->tas.rsize = (0)));
        }
    }
    if ((((penum)->text.operation & (0x00200 | 0x20000)) == (0x00200 | 0x20000)) && (i_ctx_p->pgs)->text_rendering_mode != 3) {
    ...
    gs_set_currentfont((i_ctx_p->pgs), penum->orig_font);
    while ((i_ctx_p->pgs)->level > saved_level && code >= 0) {
        if ((i_ctx_p->pgs)->saved == 0 || (i_ctx_p->pgs)->saved->saved == 0) {
...
{
    return op_show_restore(i_ctx_p, ((bool)1));
}
