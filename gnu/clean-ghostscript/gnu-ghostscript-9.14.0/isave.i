...
static const gc_ptr_element_t save_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((alloc_save_t *)0)->client_data - (char *)((alloc_save_t *)0) )) } }; static const gc_struct_data_t save_reloc_ptrs = { (sizeof(save_enum_ptrs) / sizeof((save_enum_ptrs)[0])), 0, &st_ref_memory, save_enum_ptrs }; static const gs_memory_struct_type_t st_alloc_save = { sizeof(alloc_save_t), "alloc_save", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &save_reloc_ptrs };
...
ref_packed *igc_reloc_ref_ptr_nocheck(const ref_packed *, gc_state_t *);

static
void change_clear_marks(const gs_memory_t *cmem, void *vptr, uint size, const gs_memory_struct_type_t *pstype)
{
    alloc_change_t *const ptr = (alloc_change_t *)vptr;

    if ((*(const ref_packed *)(&ptr->contents) >= ((ref_packed)(2) << 13)))
        (*((ref_packed *) & ptr->contents) &= ~(1 << 12));
    else
        ((&ptr->contents)->tas.type_attrs &= ~(1));
}
static
gs_ptr_type_t change_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const alloc_change_t *ptr = vptr; switch ( index ) { default: return 0;
case 0: return (pep->ptr = (const void *)(((const alloc_change_t *)vptr)->next), (&ptr_struct_procs));
case 1:
    if (ptr->offset >= 0)
        return (pep->ptr = (const void *)((byte *) ptr->where - ptr->offset), (&ptr_struct_procs));
    else
        if (ptr->offset != (-3))
            return (pep->ptr = (const void *)(ptr->where), (&ptr_ref_procs));
        else {
...
case 2:
    return (pep->ptr = (const void *)(&ptr->contents), (&ptr_ref_procs));
} }
static void change_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { alloc_change_t *ptr = vptr;
{
    (ptr->next = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(ptr->next), gcst)));
    switch (ptr->offset) {
        case (-2):
            break;
        case (-1):
            ptr->where = (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_ref_ptr))((const void *)(ptr->where), gcst);
            break;
        case (-3):
# 207 "./psi/isave.c"
            {
        ...
        default:
            {
                ...
                (obj = (((*(const gc_procs_with_refs_t **)(gcst))->reloc_struct_ptr)((const void *)(obj), gcst)));
    ...
    if ((*(const ref_packed *)(&ptr->contents) >= ((ref_packed)(2) << 13)))
        ...
        (*((*(const gc_procs_with_refs_t **)(gcst))->reloc_refs))((ref_packed *)(&(ptr->contents)), (ref_packed *)(&(ptr->contents) + 1), gcst);
        ((&ptr->contents)->tas.type_attrs &= ~(1));
    }
}
}
static const gs_memory_struct_type_t st_alloc_change = { sizeof(alloc_change_t), "alloc_change", 0, change_clear_marks, change_enum_ptrs, change_reloc_ptrs, 0, 0 };
# 265 "./psi/isave.c"
...
alloc_set_masks(gs_dual_memory_t *dmem, uint new_mask, uint test_mask)
{
    ...
    for (i = 0; i < (sizeof(dmem->spaces.memories.indexed) / sizeof((dmem->spaces.memories.indexed)[0])); ++i)
...
alloc_free_save(gs_ref_memory_t *mem, alloc_save_t *save, const char *scn)
{
    ...
    (((gs_memory_t *)mem)->procs.free_object((gs_memory_t *)mem, save, scn));
...
{
    gs_ref_memory_t *lmem = dmem->spaces.memories.named.local;
    gs_ref_memory_t *gmem = dmem->spaces.memories.named.global;
    ...
    if (lmem->save_level > 1) {
        ulong scanned;
        int code = save_set_new(&lsave->state, ((bool)0), ((bool)1), &scanned);
...
alloc_save_space(gs_ref_memory_t * mem, gs_dual_memory_t * dmem, ulong sid)
{
    ...
    save->client_data = ((void*)0);
    save->state = save_mem;
    save->spaces = dmem->spaces;
    save->restore_names = (names_memory(mem->gs_lib_ctx->gs_name_table) == (gs_memory_t *) mem);
                  ...
                  ref_packed * where, client_name_t cname)
{
    ...
    if (pcont == ((void*)0))
        ...
        (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./psi/isave.c", 488), errprintf_nomem("Bad type %u for save!  pcont = 0x%lx, where = 0x%lx\n", (((const byte *)&((pcont)->tas.type_attrs))[1]), (ulong) pcont, (ulong) where));
...
{
    gs_ref_memory_t *mem =
        (pcont == ((void*)0) ? dmem->spaces.memories.named.local :
         dmem->spaces.memories.indexed[(avm_space)(((pcont)->tas.type_attrs) & (((1 << 2) - 1) << 2)) >> 2]);
...
alloc_save_change_alloc(gs_ref_memory_t *mem, client_name_t cname, alloc_change_t **pcp)
{
    ...
    cp->where = 0;
    cp->offset = (-3);
    (((&cp->contents)->tas.type_attrs = ((t_null) << 8) + (0)) );
    *pcp = cp;
    return 1;
}
...
alloc_save_remove(gs_ref_memory_t *mem, ref_packed *obj, client_name_t cname)
{
    ...
    for (; *cpp != ((void*)0);) {
        ...
        if (cp->offset == (-3) && cp->where == obj) {
            if (mem->scan_limit == cp)
                mem->scan_limit = cp->next;
            *cpp = cp->next;
            (((gs_memory_t *)mem)->procs.free_object((gs_memory_t *)mem, cp, "alloc_save_remove"));
        } else
            cpp = &(*cpp)->next;
    }
}
...
alloc_save__filter_changes_in_space(gs_ref_memory_t *mem)
{
    ...
    for (; *cpp != ((void*)0); ) {
        ...
        if (cp->offset == (-3) && !check_l_mark(cp->where)) {
...
{
    const alloc_save_t *save = dmem->spaces.memories.named.local->saved;
...
alloc_is_since_save(const void *vptr, const alloc_save_t * save)
{
    ...
    register const gs_ref_memory_t *mem = save->spaces.memories.named.local;
    ...
    for (;; mem = &mem->saved->state) {
        ...
        for (cp = mem->cfirst; cp != 0; cp = cp->cnext) {
            if ((((ptr_ord_t)((const byte *)(ptr)) >= (ptr_ord_t)((cp)->cbase)) && ((ptr_ord_t)((const byte *)(ptr)) < (ptr_ord_t)((cp)->cend)))) {
                do { } while (0);
...
# 653 "./psi/isave.c"
    if (save->state.save_level == 0 &&
        (mem = save->spaces.memories.named.global) != save->spaces.memories.named.local &&
        save->spaces.memories.named.global->num_contexts == 1
        ) {
        const chunk_t *cp;
                         ...
                         const ref * pnref, const alloc_save_t * save)
{
    ...
    if (!save->restore_names)
        return ((bool)0);
    pnstr = ((mem->gs_lib_ctx->gs_name_table)->sub[(( ((const name_sub_table *) ((pnref)->value.pname - (((pnref)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((pnref)->tas.rsize) )) >> (8 + (4 >= 2))].strings->strings + ((( ((const name_sub_table *) ((pnref)->value.pname - (((pnref)->tas.rsize) & ((1 << (8 + (4 >= 2))) - 1))))->high_index + ((pnref)->tas.rsize) )) & ((1 << (8 + (4 >= 2))) - 1)));
                               ...
                               uint nidx, const alloc_save_t *save)
{
    ...
    if (!save->restore_names)
        return ((bool)0);
    pnstr = ((mem->gs_lib_ctx->gs_name_table)->sub[(nidx) >> (8 + (4 >= 2))].strings->strings + ((nidx) & ((1 << (8 + (4 >= 2))) - 1)));
    if (pnstr->foreign_string)
        return ((bool)0);
    return alloc_is_since_save(pnstr->string_bytes, save);
}
...
{
    alloc_save_t *sprev = dmem->spaces.memories.named.local->saved;
...
alloc_restore_step_in(gs_dual_memory_t *dmem, alloc_save_t * save)
{
    ...
    gs_ref_memory_t *lmem = save->spaces.memories.named.local;
    gs_ref_memory_t *gmem = save->spaces.memories.named.global;
    ...
    do {
        ...
        sid = sprev->id;
        sprev->state.gc_status.psignal = mem->gc_status.psignal;
        code = restore_resources(sprev, mem);
        if (code < 0)
            return code;
    ...
    if (mem->save_level == 0) {
        ...
        code = save_set_new(mem, ((bool)1), ((bool)0), &scanned);
        if (code < 0)
            return code;
    }
...
alloc_restore_all(gs_dual_memory_t * dmem)
{
    ...
    gs_ref_memory_t *lmem = dmem->spaces.memories.named.local;
    gs_ref_memory_t *gmem = dmem->spaces.memories.named.global;
    gs_ref_memory_t *smem = dmem->spaces.memories.named.system;
    gs_ref_memory_t *mem;
    int code;
...

    {
        ...
        code = restore_resources(&empty_save, ((void*)0));
        if (code < 0)
            return code;
    }
...
restore_free(gs_ref_memory_t * mem)
{
    ...
    (((gs_memory_t *) mem)->procs.free_all((gs_memory_t *) mem, 1, "(free_all)"));
}
...
{
    gs_ref_memory_t *mem = save->spaces.memories.named.local;
    alloc_save_t *sprev;
    ulong scanned;
    int code;
    ...
    do {
        ...
        if (mem->save_level != 0) {
            ...
            code = save_set_new(&sprev->state, ((bool)1), ((bool)0), &scanned);
            if (code < 0)
                return code;
            ...
            forget_changes(mem);
            code = save_set_new(mem, ((bool)0), ((bool)0), &scanned);
            if (code < 0)
                return code;
            file_forget_save(mem);
            combine_space(mem);
            ...
            mem = save->spaces.memories.named.global;
            if (mem != save->spaces.memories.named.local && mem->saved != 0) {
                forget_changes(mem);
                code = save_set_new(mem, ((bool)0), ((bool)0), &scanned);
                if (code < 0)
                    return code;
...
combine_space(gs_ref_memory_t * mem)
{
    ...
    for (cp = mem->cfirst; cp != 0; cp = csucc) {
        ...
        if (cp->outer == 0)
            ...
            ((mem->non_gc_memory)->procs.free_object(mem->non_gc_memory, cp, "combine_space(inner)"));
...
save_set_new(gs_ref_memory_t * mem, bool to_new, bool set_limit, ulong *pscanned)
{
    ...
    { chunk_t *cp = (mem)->cfirst; for ( ; cp != 0; cp = cp->cnext ) { {
        if (cp->has_refs) {
...
{
    register alloc_change_t *chp = mem->changes, *chp_back = ((void*)0), *chp_forth;
# 1323 "./psi/isave.c"
    alloc_open_chunk(mem);
    ...
    for (chp = mem->changes; chp; chp = chp_forth) {
        ...
        if (chp->offset != (-3)) {
            ...
            if (!(*(const ref_packed *)(prp) >= ((ref_packed)(2) << 13))) {
                ...
                if ((rp->tas.type_attrs & 2) == 0) {
                    if (mem->scan_limit == chp)
                        mem->scan_limit = chp_back;
                    if (mem->changes == chp)
                        mem->changes = chp_back;
                    (((gs_memory_t *)mem)->procs.free_object((gs_memory_t *)mem, chp, "alloc_save_remove"));
                    continue;
                } else
                    rp->tas.type_attrs &= ~2;
            }
...
{
    return((gs_memory_t *)save->spaces.memories.named.local);
}
