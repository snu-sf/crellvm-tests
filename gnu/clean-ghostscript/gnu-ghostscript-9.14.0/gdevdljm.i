# 1 "./devices/gdevdljm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/gdevdljm.c" 2
# 17 "./devices/gdevdljm.c"
# 1 "./base/gdevprn.h" 1
# 22 "./base/gdevprn.h"
# 1 "./base/memory_.h" 1
# 23 "./base/memory_.h"
...
# 19 "./devices/gdevdljm.c" 2
# 47 "./devices/gdevdljm.c"
int
                      ...
                      int dots_per_inch, int features, const char *page_init)
{
    return dljet_mono_print_page_copies(pdev, prn_stream, 1, dots_per_inch,
                                        features, page_init, page_init, ((bool)0));
}
int
                             ...
                             const char *odd_page_init, const char *even_page_init, bool tumble)
{
    ...
    if (num_copies != 1 && !(features & 256))
        return gx_default_print_page_copies(pdev, prn_stream, num_copies);
    storage =
        (ulong *)(*(pdev->memory)->procs.alloc_byte_array)(pdev->memory, storage_size_words, sizeof(word), "hpjet_print_page");
...
# 142 "./devices/gdevdljm.c"
    if ((features & 64) && dupset && dup) {
...
# 241 "./devices/gdevdljm.c"
                if ((num_blank_lines < 7 && compression != 3) ||
                    !(features & (1 | 2 | 4))
                    ) {
                    bool mode_3ns =
                        (features & 16) &&
                ...
                memset(((byte *)prev_row_words), 0, line_size);
            }
            num_blank_lines = 0;
            ...
            if (features & 16) {
                ...
                int count3 = gdev_pcl_mode3compress(line_size, ((byte *)data_words),
                                                    ((byte *)prev_row_words), ((byte *)out_row_words));
                int count2 = gdev_pcl_mode2compress(data_words, end_data,
                                                    ((byte *)out_row_alt_words));
                int penalty3 =
                ...
                if (count3 + penalty3 < count2 + penalty2) {
                    ...
                    compression = 3;
                    out_data = ((byte *)out_row_words);
                    out_count = count3;
                } else {
                    if (compression != 2)
                        fputs(from3to2, prn_stream);
                    compression = 2;
                    out_data = ((byte *)out_row_alt_words);
                    out_count = count2;
                }
            } else if (features & 8) {
                out_data = ((byte *)out_row_words);
    ...
    ((pdev->memory)->procs.free_object(pdev->memory, storage, "hpjet_print_page"));
