...

{
  ...
  while (put_bits >= 8) {
    ...
    { *(state)->next_output_byte++ = (JOCTET) (c); if (--(state)->free_in_buffer == 0) if (! dump_buffer_s(state)) { return FALSE; } };
    if (c == 0xFF) {
      { *(state)->next_output_byte++ = (JOCTET) (0); if (--(state)->free_in_buffer == 0) if (! dump_buffer_s(state)) { return FALSE; } };
    }
    put_buffer <<= 8;
    put_bits -= 8;
  }
...

{
  ...
  while (put_bits >= 8) {
    ...
    { *(entropy)->next_output_byte++ = (JOCTET) (c); if (--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); };
    if (c == 0xFF) {
      { *(entropy)->next_output_byte++ = (JOCTET) (0); if (--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); };
    }
    put_buffer <<= 8;
    put_bits -= 8;
  }
...
emit_restart_s (working_state * state, int restart_num)
{
  ...
  { *(state)->next_output_byte++ = (JOCTET) (0xFF); if (--(state)->free_in_buffer == 0) if (! dump_buffer_s(state)) { return FALSE; } };
  { *(state)->next_output_byte++ = (JOCTET) (0xD0 + restart_num); if (--(state)->free_in_buffer == 0) if (! dump_buffer_s(state)) { return FALSE; } };
...
emit_restart_e (huff_entropy_ptr entropy, int restart_num)
{
  ...
  if (! entropy->gather_statistics) {
    ...
    { *(entropy)->next_output_byte++ = (JOCTET) (0xFF); if (--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); };
    { *(entropy)->next_output_byte++ = (JOCTET) (0xD0 + restart_num); if (--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); };
  }
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
 ...
 if (entropy->dc_count_ptrs[tbl] == ((void*)0))
 ...
 if (tbl < 0 || tbl >= 4)
   ((cinfo)->err->msg_code = (JERR_NO_HUFF_TABLE), (cinfo)->err->msg_parm.i[0] = (tbl), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
 if (entropy->ac_count_ptrs[tbl] == ((void*)0))
