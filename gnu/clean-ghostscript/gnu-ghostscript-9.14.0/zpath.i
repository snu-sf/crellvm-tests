...
zcurrentpoint(i_ctx_t *i_ctx_p)
{
    ...
    gs_point pt;
    int code = gs_currentpoint((i_ctx_p->pgs), &pt);
          ...
          int (*add_proc)(gs_state *, double, double))
{
    ...
    if ((code = num_params(op, 2, opxy)) < 0 ||
        (code = (*add_proc)((i_ctx_p->pgs), opxy[0], opxy[1])) < 0
        )
        return code;
             ...
             int (*add_proc)(gs_state *, double, double, double, double, double, double))
{
    ...
    if ((code = num_params(op, 6, opxy)) < 0)
        return code;
    code = (*add_proc)((i_ctx_p->pgs), opxy[0], opxy[1], opxy[2], opxy[3], opxy[4], opxy[5]);
    if (code >= 0)
...
{
    return gs_closepath((i_ctx_p->pgs));
}
...
{
    return gs_initclip((i_ctx_p->pgs));
}
