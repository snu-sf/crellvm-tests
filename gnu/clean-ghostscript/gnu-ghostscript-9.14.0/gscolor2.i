...
gs_setcolorspace(gs_state * pgs, gs_color_space * pcs)
{
    ...
    if (code >= 0) {
        ...
        ((pgs->color[0].ccolor)->pattern = 0, (*(pcs)->type->init_color)(pgs->color[0].ccolor, pcs));
...
static const gc_ptr_element_t indexed_map_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_indexed_map *)0)->proc_data - (char *)((gs_indexed_map *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_indexed_map *)0)->values - (char *)((gs_indexed_map *)0) )) } }; static const gc_struct_data_t indexed_map_reloc_ptrs = { (sizeof(indexed_map_enum_ptrs) / sizeof((indexed_map_enum_ptrs)[0])), 0, 0, indexed_map_enum_ptrs }; static const gs_memory_struct_type_t st_indexed_map = { sizeof(gs_indexed_map), "gs_indexed_map", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &indexed_map_reloc_ptrs };


int
lookup_indexed_map(const gs_color_space * pcs, int index, float *values)
{
    int m = (*(pcs->base_space)->type->num_components)(pcs->base_space);
...
{
    ((pmem)->procs.free_object(pmem, ((gs_indexed_map *) pmap)->values, cname));
    ((pmem)->procs.free_object(pmem, pmap, cname));
}
                  ...
                  client_name_t cname)
{
    ...
    do { if ( ((pimap) = (gs_indexed_map *)(*(pmem)->procs.alloc_struct)(pmem, &st_indexed_map, cname)) == 0 ) { return ((-25)); } else { do { (pimap)->rc.ref_count = 1; (pimap)->rc.memory = pmem; (pimap)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);

    if (nvals > 0) {
        pimap->values =
            (float *)(*(pmem)->procs.alloc_byte_array)(pmem, nvals, sizeof(float), cname);

        if (pimap->values == 0) {
            ((pmem)->procs.free_object(pmem, pimap, cname));
            return ((-25));
        }
    } else
        pimap->values = 0;
...
static gs_ptr_type_t cs_Indexed_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void cs_Indexed_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static const gs_memory_struct_type_t st_color_space_Indexed = { sizeof(gs_color_space), "gs_color_space_Indexed", 0, 0, cs_Indexed_enum_ptrs, cs_Indexed_reloc_ptrs, 0, 0 };





static void gx_restrict_Indexed(gs_client_color *, const gs_color_space *);
static const gs_color_space *gx_concrete_space_Indexed(const gs_color_space *, const gs_imager_state *);
static int gx_concretize_Indexed(const gs_client_color *, const gs_color_space *, frac *, const gs_imager_state *, gx_device *);
static int gx_install_Indexed(gs_color_space *, gs_state *);
static int gx_set_overprint_Indexed(const gs_color_space *, gs_state *);
static void gx_final_Indexed(const gs_color_space *);
static int gx_serialize_Indexed(const gs_color_space *, stream *);
static gx_color_polarity_t gx_polarity_Indexed(const gs_color_space *);
const gs_color_space_type gs_color_space_type_Indexed = {
    gs_color_space_index_Indexed, ((bool)0), ((bool)0),
    &st_color_space_Indexed, gx_num_components_1,
    gx_init_paint_1, gx_restrict_Indexed,
    gx_concrete_space_Indexed,
    gx_concretize_Indexed, ((void*)0),
...
static
gs_ptr_type_t cs_Indexed_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_color_space *pcs = vptr; switch ( index ) { default: return 0;
case 0:
if (pcs->params.indexed.use_proc)
    return (pep->ptr = (const void *)((void *)pcs->params.indexed.lookup.map), (&ptr_struct_procs));
else
    return (pep->ptr = pcs->params.indexed.lookup.table.data, pep->size = indexed_table_size(pcs), (&ptr_const_string_procs));

} }
static void cs_Indexed_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_color_space *pcs = vptr;
{
    if (pcs->params.indexed.use_proc)
        (((gs_color_space *)vptr)->params.indexed.lookup.map = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_color_space *)vptr)->params.indexed.lookup.map), gcst)));
    else {
        gs_const_string table;

        table.data = pcs->params.indexed.lookup.table.data;
        table.size = indexed_table_size(pcs);
        (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(table), gcst));
...
{
    if (pcs->params.indexed.use_proc) {
        do { if (pcs->params.indexed.lookup.map) { do { do { } while (0); (pcs->params.indexed.lookup.map)->rc.ref_count += (-1); } while (0); if (!(pcs->params.indexed.lookup.map)->rc.ref_count) { do { do { } while (0); (pcs->params.indexed.lookup.map)->rc.free((pcs->params.indexed.lookup.map)->rc.memory, (void *)(pcs->params.indexed.lookup.map), "gx_adjust_Indexed"); } while (0); do { } while (0); } else do { } while (0); } } while (0);

    } else {
        byte *data = (byte *)pcs->params.indexed.lookup.table.data;

        (*(pcs->rc.memory)->procs.free_string)(pcs->rc.memory, data, pcs->params.indexed.lookup.table.size, "gx_final_Indexed");
...
)
{
    ...
    if (ptbl == 0) {
        ...
        if (pindexed->lookup.map == 0) {
            ((pmem)->procs.free_object(pmem, pcspace, "gs_cspace_build_Indexed"));
            return ((-25));
        }
        pindexed->use_proc = ((bool)1);
    } else {
        pindexed->lookup.table = *ptbl;
        pindexed->use_proc = ((bool)0);
    }
    pcspace->base_space = pbase_cspace;
    rc_increment_cs(pbase_cspace);
    pindexed->hival = num_entries - 1;
    pindexed->n_comps = (*(pbase_cspace)->type->num_components)(pbase_cspace);
    *ppcspace = pcspace;
    return 0;
}
                         ...
                         gs_client_color *pcc)
{
    ...
    if (pip->use_proc) {
        ...
        int m = (*(pbcs)->type->num_components)(pbcs);
    ...
    if (pip->use_proc) {
        ...
        switch (m) {
            default: {
                ...
                for (i = 0; i < m; ++i)
                    output[i] = ( (0.0 < values[i] && values[i] < 1.0) ? ((unsigned char) (values[i]*255.0)) : ( (values[i] <= 0.0) ? 0x00 : 0xFF ));
            }
                break;
            case 4:
                output[3] = ( (0.0 < values[3] && values[3] < 1.0) ? ((unsigned char) (values[3]*255.0)) : ( (values[3] <= 0.0) ? 0x00 : 0xFF ));
            case 3:
                output[2] = ( (0.0 < values[2] && values[2] < 1.0) ? ((unsigned char) (values[2]*255.0)) : ( (values[2] <= 0.0) ? 0x00 : 0xFF ));
            case 2:
                output[1] = ( (0.0 < values[1] && values[1] < 1.0) ? ((unsigned char) (values[1]*255.0)) : ( (values[1] <= 0.0) ? 0x00 : 0xFF ));
            case 1:
                output[0] = ( (0.0 < values[0] && values[0] < 1.0) ? ((unsigned char) (values[0]*255.0)) : ( (values[0] <= 0.0) ? 0x00 : 0xFF ));
        }
    ...
    if (pip->use_proc) {
        ...
        switch (m) {
            default: {
                ...
                for (i = 0; i < m; ++i)
                    output[i] = ((frac)(((values[i]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
            }
                break;
            case 4:
                output[3] = ((frac)(((values[3]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
            case 3:
                output[2] = ((frac)(((values[2]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
            case 2:
                output[1] = ((frac)(((values[1]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
            case 1:
                output[0] = ((frac)(((values[0]) + 0.5 / ((float)((frac)0x7ff8))) * ((float)((frac)0x7ff8))));
        }
        ...
        switch (m) {
            default: {
                ...
                for (i = 0; i < m; ++i)
                    output[i] = ((frac)( ((frac)(pcomp[i]) << (15 - (8))) + ((pcomp[i]) >> ((8) * 2 - 15)) - ((pcomp[i]) >> ((8) - 3)) ));
            }
                break;
            case 4:
                output[3] = ((frac)( ((frac)(pcomp[3]) << (15 - (8))) + ((pcomp[3]) >> ((8) * 2 - 15)) - ((pcomp[3]) >> ((8) - 3)) ));
            case 3:
                output[2] = ((frac)( ((frac)(pcomp[2]) << (15 - (8))) + ((pcomp[2]) >> ((8) * 2 - 15)) - ((pcomp[2]) >> ((8) - 3)) ));
            case 2:
                output[1] = ((frac)( ((frac)(pcomp[1]) << (15 - (8))) + ((pcomp[1]) >> ((8) * 2 - 15)) - ((pcomp[1]) >> ((8) - 3)) ));
            case 1:
                output[0] = ((frac)( ((frac)(pcomp[0]) << (15 - (8))) + ((pcomp[0]) >> ((8) * 2 - 15)) - ((pcomp[0]) >> ((8) - 3)) ));
        }
        return 0;
    }
}
...
{
    return (*((pgs->device)->procs.include_color_space))(pgs->device, ((pgs)->color[0].color_space), res_name, name_length);
}
