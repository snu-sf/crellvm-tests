# 1 "./devices/vector/gdevpdfv.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdfv.c" 2
# 18 "./devices/vector/gdevpdfv.c"
# 1 "./base/math_.h" 1
# 23 "./base/math_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 19 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/string_.h" 1
# 32 "./base/string_.h"
...
# 33 "./base/string_.h" 2
# 20 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 44 "./base/stdio_.h"
...
# 21 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gscindex.h" 1
# 22 "./base/gscindex.h"
# 1 "./base/gscspace.h" 1
# 23 "./base/gscspace.h"
...
int gs_cspace_indexed_lookup_frac(const gs_color_space *pcs, float index_float,
                        frac *output);
# 22 "./devices/vector/gdevpdfv.c" 2
...
# 24 "./devices/vector/gdevpdfv.c" 2
...
# 26 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gsptype2.h" 1
# 22 "./base/gsptype2.h"
# 1 "./base/gspcolor.h" 1
# 24 "./base/gspcolor.h"
...
# 27 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gxcolor2.h" 1
# 23 "./base/gxcolor2.h"
# 1 "./base/gscolor2.h" 1
# 24 "./base/gscolor2.h"
...
# 28 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gxdcolor.h" 1
# 22 "./base/gxdcolor.h"
# 1 "./base/gscsel.h" 1
# 27 "./base/gscsel.h"
...
extern int gx_dc_read_color( gx_color_index * pcolor,
                                  ...
                                  const byte * pdata,
                                  int size );
# 29 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gxpcolor.h" 1
# 24 "./base/gxpcolor.h"
# 1 "./base/gxcspace.h" 1
# 48 "./base/gxcspace.h"
...
# 30 "./devices/vector/gdevpdfv.c" 2
# 1 "./base/gxshade.h" 1
# 22 "./base/gxshade.h"
# 1 "./base/gsshade.h" 1
# 24 "./base/gsshade.h"
...
# 31 "./devices/vector/gdevpdfv.c" 2
...
# 32 "./devices/vector/gdevpdfv.c" 2
...
# 33 "./devices/vector/gdevpdfv.c" 2
...
# 34 "./devices/vector/gdevpdfv.c" 2


extern const gx_device_color_type_t gx_dc_pattern2;
# 64 "./devices/vector/gdevpdfv.c"
static int
             ...
             const gx_color_tile *m_tile)
{
    ...
    if (pdev->CompatibilityLevel < 1.4)
        return ((((p_size) > (m_size)) ? (p_size) : (m_size)) <= 65500);
    else
        return 1;
}
            ...
            cos_stream_t *pcs_image, pdf_resource_t **ppres)
{
    ...
    if ((((&tile->step_matrix)->xy) == 0.0 && ((&tile->step_matrix)->yx) == 0.0))
        step.x = tile->step_matrix.xx, step.y = tile->step_matrix.yy;
    else if ((((&tile->step_matrix)->xx) == 0.0 && ((&tile->step_matrix)->yy) == 0.0))
...
# 170 "./devices/vector/gdevpdfv.c"
    ...
    if ((code = cos_dict_put_c_key_int(pcd, "/PatternType", 1)) < 0 ||
        ...
        (code = cos_dict_put_c_key_object(pcd, "/Resources",
                                          ((cos_object_t *)&((pcd_Resources)->cos_procs)))) < 0 ||
    ...
    {
        char buf[((sizeof(long) * 8 + 2) / 3) + 6 + 1];
                        ...
                        gs_pattern1_instance_t *pinst)
{
    ...
    gs_matrix smat2 = (*(const gs_matrix *)&((gs_imager_state *)pinst->saved)->ctm), smat;
    ...
    if (((smat.tx) < 0 ? -(smat.tx) : (smat.tx)) < 0.0001)
        smat.tx = 0;
    if (((smat.ty) < 0 ? -(smat.ty) : (smat.ty)) < 0.0001)
        smat.ty = 0;
                     ...
                     cos_stream_t **ppcs_mask)
{
    ...
    gs_image_t_init_mask_adjust(&image, ((bool)1), ((bool)0));
    pdf_set_pattern_image((gs_data_image_t *)&image, &m_tile->tmask);
    pdf_image_writer_init(&writer);
    if ((code = pdf_begin_write_image(pdev, &writer, 0L, w, h, ((void*)0), ((bool)0))) < 0 ||
        (pdev->params.MonoImage.Encode &&
         (code = psdf_CFE_binary(&writer.binary[0], w, h, ((bool)1))) < 0) ||
        (code = pdf_begin_image_data(pdev, &writer, (const gs_pixel_image_t *)&image, ((void*)0), 0)) < 0
        )
        return code;
                          ...
                          bool have_pattern_streams, pdf_resource_t **ppres)
{
    ...
    if (!have_pattern_streams && m_tile == 0) {
        ...
        *ppres = 0;
        ((&dc_pure)->colors.pure = (((pdc)->colors.pure)), (&dc_pure)->type = (&gx_dc_type_data_pure)); (&dc_pure)->ccolor_valid = ((bool)0);
                        ...
                        bool have_pattern_streams, pdf_resource_t **ppres)
{
    ...
    int code;
    gs_image1_t image;
    const gx_color_tile *m_tile = ((void*)0);
    pdf_image_writer writer;
    int w = p_tile->tbits.rep_width, h = p_tile->tbits.rep_height;

    if (!have_pattern_streams) {
# 409 "./devices/vector/gdevpdfv.c"
        m_tile = pdc->mask.m_tile;
        if (m_tile) {
            if (p_tile && !(p_tile->depth & 7) && p_tile->depth <= sizeof(gx_color_index_data) * 8) {
    ...
    code = pdf_color_space_named(pdev, &cs_value, ((void*)0), pcs_Device,
    ...
    if (!have_pattern_streams) {
        ...
        gs_image_t_init_adjust(&image, pcs_Device, ((bool)0));
        ...
        if ((code = pdf_begin_write_image(pdev, &writer, 0L, w, h, ((void*)0), ((bool)0))) < 0 ||
            (code = psdf_setup_lossless_filters((gx_device_psdf *)pdev,
                                                &writer.binary[0],
                                                (gs_pixel_image_t *)&image, ((bool)0))) < 0 ||
        ...
        if ((pcs_mask != 0 &&
             (code = cos_dict_put_c_key_object(cos_stream_dict(pcs_image), "/Mask",
                                               ((cos_object_t *)&((pcs_mask)->cos_procs)))) < 0) ||
            (code = pdf_end_write_image(pdev, &writer)) < 0
            )
            return code;
        pcs_image = (cos_stream_t *)writer.pres->object;
...
{
    gs_shading_type_t type = ((psh)->head.type);
...
pdf_put_linear_shading(cos_dict_t *pscd, const float *Coords,
                       int num_coords, const float *Domain ,
...
{
    put_clamped(p, ( ((v) - ((-0x400000 / 256.0))) * ((double)(0xffffff) / ((( 0x3fffff / 256.0)) - ((-0x400000 / 256.0)))) ), num_bytes);
}
                    ...
                    int flag, const pdf_mesh_data_params_t *pmdp)
{
    ...
    for (i = 0; i < num_points; ++i) {
        put_clamped_coord(b + 1 + i * 6, ((pts[i].x)*(1.0/(1<<8))), 3);
        put_clamped_coord(b + 4 + i * 6, ((pts[i].y)*(1.0/(1<<8))), 3);
    }
    ...
    for (i = 0; i < pmdp->num_components; ++i) {
        ...
        if (pmdp->is_indexed)
            ...
            v = ( ((c) - (vmin)) * ((double)(0xffff) / ((vmax) - (vmin))) );
        }
        put_clamped(b, v, 2);
                     ...
                     const gs_range_t *pranges)
{
    ...
    int num_comp;
    bool from_array = ((pmp->DataSource).type == data_source_type_floats);
    ...
    if (pmp->Function) {
        ...
        data_params.Domain = (pmp->Decode != 0 ? pmp->Decode + 4 : ((void*)0));
        num_comp = gs_color_space_num_components(pcs);
    }
    data_params.ranges = pranges;


    shade_next_init(&cs, pmp, ((void*)0));
    if (from_array) {
        ...
        if (pca == 0)
            return ((-25));
        for (i = 0; i < 2; ++i)
            if ((code = pdf_array_add_real2(pca, (-0x400000 / 256.0),
                                            ( 0x3fffff / 256.0))) < 0)
                return code;
        data_params.is_indexed = ((bool)0);
        if (gs_color_space_get_index(pcs) == gs_color_space_index_Indexed) {
            data_params.is_indexed = ((bool)1);
            if ((code = pdf_array_add_real2(pca, 0,
                                            0xffff)) < 0)
                return code;
        } else {
        ...
        code = cos_dict_put_c_key_object(pscd, "/Decode", ((cos_object_t *)&((pca)->cos_procs)));
        if (code < 0)
            return code;
        ...
        ctm_ident.txy_fixed_valid = ((bool)1);
        cs.pctm = &ctm_ident;
        if (pmp->Function)
            data_params.ranges = 0;
    } else {
        ...
        if (from_array) {
            while ((flag = shade_next_flag(&cs, 0)) >= 0)
                if ((code = put_float_mesh_data(pscs, &cs, flag,
                                                &data_params)) < 0)
                    return code;
            if (!(((cs.s)->cursor.r.ptr >= (cs.s)->cursor.r.limit) && (cs.s)->end_status == ((int)(-1))))
                code = ((-15));
        }
        if (bits_per_flag < 0)
    ...
    case shading_type_Lattice_form_Gouraud_triangle: {
        ...
        data_params.num_components = num_comp;
        if (from_array)
            while (!(((cs.s)->cursor.r.ptr >= (cs.s)->cursor.r.limit) && (cs.s)->end_status == ((int)(-1))))
                 ...
                 pdf_resource_t **ppres)
{
    ...
    if (code < 0)
        return code;
    code = pdf_alloc_resource(pdev, resourcePattern, 0L, ppres, -1);
    if (code < 0)
        return code;
    pres = *ppres;
    cos_become(pres->object, (&cos_dict_procs));
    pcd = (cos_dict_t *)pres->object;
    code = pdf_alloc_resource(pdev, resourceShading, 0L, &psres, -1);
    if (code < 0)
        return code;
    psco = psres->object;
    if (((psh)->head.type) >= 4) {

        cos_become(psco, (&cos_stream_procs));
        ...
        cos_become(psco, (&cos_dict_procs));
    ...
    if (psres->where_used) {
        code = pdf_substitute_resource(pdev, &psres, resourceShading, ((void*)0), ((bool)0));
        if (code < 0)
            return code;
        psco = psres->object;
    ...
    if (code < 0 ||
        ...
        return code;
    code = pdf_substitute_resource(pdev, &pres, resourcePattern, ((void*)0), ((bool)0));
    if (code < 0)
        return code;
...
gdev_pdf_include_color_space(gx_device *dev, gs_color_space *cspace, const byte *res_name, int name_length)
{
    ...
    return pdf_color_space_named(pdev, &cs_value, ((void*)0), cspace,
                                &pdf_color_space_names, ((bool)1), res_name, name_length, ((bool)0));
}
