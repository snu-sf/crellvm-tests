...
zsetcolorscreen(i_ctx_t *i_ctx_p)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        sprocs[i] = *op1;
        space = (((space) > (((int)(avm_space)(((op1)->tas.type_attrs) & (((1 << 2) - 1) << 2)) >> 2))) ? (space) : (((int)(avm_space)(((op1)->tas.type_attrs) & (((1 << 2) - 1) << 2)) >> 2)));
    }
    mem = (gs_memory_t *)(&(i_ctx_p->memory))->spaces.memories.indexed[space];
    ...
    do { if ( ((pht) = (gs_halftone *)(*(mem)->procs.alloc_struct)(mem, &st_halftone, "setcolorscreen(halftone)")) == 0 ) { pht = 0; } else { do { (pht)->rc.ref_count = 0; (pht)->rc.memory = mem; (pht)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);

    do { if ( ((pdht) = (gx_device_halftone *)(*(mem)->procs.alloc_struct)(mem, &st_device_halftone, "setcolorscreen(device halftone)")) == 0 ) { pdht = 0; } else { do { (pdht)->rc.ref_count = 0; (pdht)->rc.memory = mem; (pdht)->rc.free = rc_free_struct_only; do { } while (0); } while (0); } } while (0);
    ...
    if (pht == 0 || pdht == 0)
        ...
        code = gs_sethalftone_prepare((i_ctx_p->pgs), pht, pdht);
    }
    if (code >= 0) {
        es_ptr esp0 = (((i_ctx_p->exec_stack).stack).p);

        (((i_ctx_p->exec_stack).stack).p) += 8;
        (((((i_ctx_p->exec_stack).stack).p) - 7)->value.opproc = (setcolorscreen_cleanup), (((((i_ctx_p->exec_stack).stack).p) - 7)->tas.type_attrs = ((t_null) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p) - 7)->tas.rsize = (0)));
        memcpy((((i_ctx_p->exec_stack).stack).p) - 6, sprocs, sizeof(ref) * 4);
        (((((i_ctx_p->exec_stack).stack).p) - 2)->value.pstruct = ((obj_header_t *)(pht)), (((((i_ctx_p->exec_stack).stack).p) - 2)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
        (((((i_ctx_p->exec_stack).stack).p) - 1)->value.pstruct = ((obj_header_t *)(pdht)), (((((i_ctx_p->exec_stack).stack).p) - 1)->tas.type_attrs = ((t_struct) << 8) + ((0) | (((&(i_ctx_p->memory)))->current_space))) );
        (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (setcolorscreen_finish), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0)));
        for (i = 0; i < 4; i++) {
            ...
            code = zscreen_enum_init(i_ctx_p,
                                ...
                                &pht->params.colorscreen.screens.indexed[i],
                                     &sprocs[i], 0, 0, space);
            if (code < 0) {
                (((i_ctx_p->exec_stack).stack).p) = esp0;
                break;
            }
        }
    }
    if (code < 0) {
        ((mem)->procs.free_object(mem, pdht, "setcolorscreen(device halftone)"));
        ((mem)->procs.free_object(mem, pht, "setcolorscreen(halftone)"));
        return code;
    }
    ((((i_ctx_p->op_stack).stack).p) -= (12));
    return 5;
}
...
{
    gx_device_halftone *pdht = ((gx_device_halftone *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct));
    int code;

    pdht->order = pdht->components[0].corder;
    code = gx_ht_install((i_ctx_p->pgs), ((gs_halftone *)(((((i_ctx_p->exec_stack).stack).p) - 1)->value.pstruct)), pdht);
    if (code < 0)
        return code;
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->screen_procs.red = (((i_ctx_p->exec_stack).stack).p)[-5];
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->screen_procs.green = (((i_ctx_p->exec_stack).stack).p)[-4];
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->screen_procs.blue = (((i_ctx_p->exec_stack).stack).p)[-3];
    ((int_gstate *)gs_state_client_data((i_ctx_p->pgs)))->screen_procs.gray = (((i_ctx_p->exec_stack).stack).p)[-2];
...
{
    gs_halftone *pht = ((gs_halftone *)(((((i_ctx_p->exec_stack).stack).p) + 6)->value.pstruct));
    gx_device_halftone *pdht = ((gx_device_halftone *)(((((i_ctx_p->exec_stack).stack).p) + 7)->value.pstruct));

    ((pdht->rc.memory)->procs.free_object(pdht->rc.memory, pdht, "setcolorscreen_cleanup(device halftone)"));

    ((pht->rc.memory)->procs.free_object(pht->rc.memory, pht, "setcolorscreen_cleanup(halftone)"));
