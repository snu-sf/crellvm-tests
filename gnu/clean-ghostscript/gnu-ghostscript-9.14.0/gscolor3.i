...
# 75 "./base/gscolor3.c"
    ...
    if (code < 0)
        return 0;
    if (psh->params.cie_joint_caches != ((void*)0)) {
        pgs->cie_joint_caches = psh->params.cie_joint_caches;
        do { if (pgs->cie_joint_caches) do { (pgs->cie_joint_caches)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    }
    ...
    pcs->params.pattern.has_base_space = ((bool)0);
    code = pcs->type->remap_color(&cc, pcs, &devc, (gs_imager_state *)pgs,
                                  pgs->device, gs_color_select_texture);
    if (code >= 0) {
        gx_device *dev = pgs->device;
        bool need_path = !((dev)->procs.dev_spec_op)(dev,
                             gxdso_pattern_shfill_doesnt_need_path, ((void*)0), 0);

        if (need_path) {
            gx_path cpath;

            (void)gx_path_init_local_shared(&cpath, ((void*)0), pgs->memory);
            code = gx_cpath_to_path(pgs->clip_path, &cpath);
            if (code >= 0)
                code = gx_fill_path(&cpath, &devc, pgs, (-1),
                                    pgs->fill_adjust.x, pgs->fill_adjust.y);
            gx_path_free(&cpath, "gs_shfill");
        } else
            code = gx_fill_path(((void*)0), &devc, pgs, (-1),
                                pgs->fill_adjust.x, pgs->fill_adjust.y);
    }
