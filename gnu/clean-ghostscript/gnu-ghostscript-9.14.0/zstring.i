...
zbytestring(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op)); if ( (ulong)(*op).value.intval > ((~(-1 << ((1 << 2) * 8 - 1)))) ) return ((-15)); } while (0);
    size = (uint)op->value.intval;
    sbody = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_bytes)(((gs_memory_t *)((i_ctx_p->memory).current)), size, ".bytestring");
    if (sbody == 0)
        return ((-25));
    ((op)->value.pstruct = ((obj_header_t *)(sbody)), ((op)->tas.type_attrs = ((t_astruct) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))) );
    memset(sbody, 0, size);
    return 0;
}
...
zstring(i_ctx_t *i_ctx_p)
{
    ...
    if (op->value.intval < 0 )
        return ((-15));
    if (op->value.intval > (16*1024*1024) )
        return ((-13));
    size = op->value.intval;
    sbody = (*(((gs_memory_t *)((i_ctx_p->memory).current)))->procs.alloc_string)(((gs_memory_t *)((i_ctx_p->memory).current)), size, "string");
    if (sbody == 0)
        return ((-25));
    ((op)->value.bytes = (sbody), ((op)->tas.type_attrs = ((t_string) << 8) + ((0x10 + 0x20 +0x40) | (((&(i_ctx_p->memory)))->current_space))), ((op)->tas.rsize = (size)));
    memset(sbody, 0, size);
    return 0;
}
...
znamestring(i_ctx_t *i_ctx_p)
{
    ...
    names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op, op);
    return 0;
}
...
zanchorsearch(i_ctx_t *i_ctx_p)
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    os_ptr op1 = op - 1;
    uint size = ((op)->tas.rsize);

    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op) : (-7)));
    if ( !(((&*op1)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op1)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op1) : (-7)));
    if (size <= ((op1)->tas.rsize) && !memcmp(op1->value.bytes, op->value.bytes, size)) {
        os_ptr op0 = op;
...
zsearch(i_ctx_t *i_ctx_p)
{
    ...
    if ( !(((&*op1)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op1)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op1) : (-7)));
    if ( !(((&*op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20))) ) return ((!((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_string)) ? check_type_failed(&*op) : (-7)));
    if (size > ((op1)->tas.rsize)) {
        ((op)->value.boolval = (0), ((op)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
        return 0;
    }
    count = ((op1)->tas.rsize) - size;
    ptr = op1->value.bytes;
    if (size == 0)
        goto found;
    pat = op->value.bytes;
...
zstringbreak(i_ctx_t *i_ctx_p)
{
    ...
    for (i = 0; i < ((op - 1)->tas.rsize); ++i)
        for (j = 0; j < ((op)->tas.rsize); ++j)
            if (op[-1].value.const_bytes[i] == op->value.const_bytes[j]) {
                ((op - 1)->value.intval = (i), ((op - 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
                goto done;
            }
...
zstringmatch(i_ctx_t *i_ctx_p)
{
    ...
    switch ((((const byte *)&((op1)->tas.type_attrs))[1])) {
        case t_string:
            ...
            goto cmp;
        case t_name:
            names_string_ref(((gs_memory_t *)((i_ctx_p->memory).current))->gs_lib_ctx->gs_name_table, op1, op1);
cmp:
            result = string_match(op1->value.const_bytes, ((op1)->tas.rsize),
                                  op->value.const_bytes, ((op)->tas.rsize),
                                  ((void*)0));
            break;
        default:
            result = (((op)->tas.rsize) == 1 && *op->value.bytes == '*');
    }
    ((op1)->value.boolval = (result), ((op1)->tas.type_attrs = ((t_boolean) << 8) + (0)) );
    ((((i_ctx_p->op_stack).stack).p) -= (1));
    return 0;
}
