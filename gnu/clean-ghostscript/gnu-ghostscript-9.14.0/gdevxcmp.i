# 1 "./devices/gdevxcmp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 318 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/gdevxcmp.c" 2
# 18 "./devices/gdevxcmp.c"
# 1 "./base/math_.h" 1
# 23 "./base/math_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 62 "/usr/include/X11/Intrinsic.h" 2 3 4
...
{
    ((mem->non_gc_memory)->procs.free_object(mem->non_gc_memory, obj, cname));
}
...
set_cmap_values(x11_cmap_values_t *values, int maxv, int mult)
{
    ...
    if (maxv < 1 || maxv > 63 || (maxv & (maxv + 1)) ||
        ...
        return ((bool)0);
    values->cv_shift = 16 - ((uint)(05637042010L >> ((((maxv + 1) % 11) - 1) * 3)) & 7);
    for (i = 0; i <= maxv; ++i)
        values->nearest[i] = 0xffff * i / maxv;
    for (i = 0; mult != (1 << i); ++i)
        do { } while (0);
...
x_get_std_cmap(gx_device_X * xdev, Atom prop)
{
    ...
    if (XGetRGBColormaps(xdev->dpy, ((xdev->scr)->root),
                         &scmap, &nitems, prop))
...
alloc_std_cmap(gx_device_X *xdev, bool colored)
{
    ...
    xdev->cman.std_cmap.free_map = ((bool)1);
    return ((bool)1);
}
...
alloc_dynamic_colors(gx_device_X * xdev, int num_colors)
{
    if (num_colors > 0) {
        xdev->cman.dynamic.colors = (x11_color_t **)
            (void *)(*(xdev->memory->non_gc_memory)->procs.alloc_byte_array)(xdev->memory->non_gc_memory, sizeof(x11_color_t *), xdev->cman.num_rgb, "x11 cman.dynamic.colors");
        ...
        if (xdev->cman.dynamic.colors) {
            ...
            for (i = 0; i < xdev->cman.num_rgb; i++)
                xdev->cman.dynamic.colors[i] = ((void*)0);
            xdev->cman.dynamic.max_used = (((256) < (num_colors)) ? (256) : (num_colors));
...
static void
x_free_colors(gx_device_X *xdev, x_pixel *pixels , int count)
{
    int i;
    x_pixel pixel;
    ...
    for (i = 0; i < count; ++i)
        if ((pixel = pixels[i]) < xdev->cman.color_to_rgb.size)
            xdev->cman.color_to_rgb.values[pixel].defined = ((bool)0);
}
...
setup_cube(gx_device_X * xdev, int ramp_size, bool colors)
{
    ...
    if (colors) {
        ...
        step = (ramp_size + 1) * ramp_size + 1;
    }

    xdev->cman.dither_ramp =
        (x_pixel *) (void *)(*(xdev->memory->non_gc_memory)->procs.alloc_byte_array)(xdev->memory->non_gc_memory, sizeof(x_pixel), num_entries, "gdevx setup_cube");

    if (xdev->cman.dither_ramp == ((void*)0))
        return ((bool)0);
    ...
    for (index = 1; index < num_entries - 1; index++) {
        ...
        xc.red = (0xffff * r / max_rgb) & xdev->cman.color_mask.red;
        xc.green = (0xffff * g / max_rgb) & xdev->cman.color_mask.green;
        xc.blue = (0xffff * b / max_rgb) & xdev->cman.color_mask.blue;
...
gdev_x_setup_colors(gx_device_X * xdev)
{
    char palette =
        ((xdev->vinfo->class != 0) &&
         (xdev->vinfo->class != 1) ? 'C' :
         (xdev->vinfo->colormap_size > 2) ? 'G' :
         'M');

    if (xdev->ghostview) {
        Atom gv_colors = XInternAtom(xdev->dpy, "GHOSTVIEW_COLORS", 0);
        Atom type;
        int format;
        ...
        if (XGetWindowProperty(xdev->dpy, xdev->win, gv_colors, 0,
            ...
            if (nitems != 2 || (*buf != 'M' && *buf != 'G' && *buf != 'C')) {
                (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Malformed GHOSTVIEW_COLOR property.\n"));
                return ((-15));
            }
            palette = (((palette) > (*buf)) ? (palette) : (*buf));
        }
    } else {
        if (xdev->palette[0] == 'c')
            xdev->palette[0] = 'C';
            ...
            xdev->palette[0] = 'M';
        palette = (((palette) > (xdev->palette[0])) ? (palette) : (xdev->palette[0]));
    }


    xdev->cman.color_mask.red = xdev->cman.color_mask.green =
        xdev->cman.color_mask.blue = 0xffff -
          (0xffff >> xdev->vinfo->bits_per_rgb);
    xdev->cman.match_mask = xdev->cman.color_mask;
    xdev->cman.num_rgb = 1 << xdev->vinfo->bits_per_rgb;


    xdev->cman.std_cmap.map = ((void*)0);
    xdev->cman.std_cmap.free_map = ((bool)0);

    xdev->cman.dither_ramp = ((void*)0);
    xdev->cman.dynamic.colors = ((void*)0);
    ...
    case 15:
        ...
        break;
    default:
        (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Unsupported X visual depth: %d\n", xdev->vinfo->depth));


        return ((-15));
    }
    {
        int count = 1 << (((xdev->color_info.depth) < (8)) ? (xdev->color_info.depth) : (8));

        xdev->cman.color_to_rgb.values =
            (x11_rgb_t *)(void *)(*(xdev->memory->non_gc_memory)->procs.alloc_byte_array)(xdev->memory->non_gc_memory, sizeof(x11_rgb_t), count, "gdevx color_to_rgb");

        if (xdev->cman.color_to_rgb.values) {
            int i;

            for (i = 0; i < count; ++i)
                xdev->cman.color_to_rgb.values[i].defined = ((bool)0);
    ...
    case 'C':
        ...
        if (xdev->vinfo->visual == ((xdev->scr)->root_visual)) {
            xdev->cman.std_cmap.map = x_get_std_cmap(xdev, ((Atom) 27));
        } else {
            xdev->cman.std_cmap.map = x_get_std_cmap(xdev, ((Atom) 25));
        }
        if (xdev->cman.std_cmap.map ||
            (xdev->vinfo->class == 4 && alloc_std_cmap(xdev, ((bool)1)))
            ) {
            xdev->color_info.dither_grays = xdev->color_info.dither_colors =
                (((xdev->cman.std_cmap.map->red_max) < ((((xdev->cman.std_cmap.map->green_max) < (xdev->cman.std_cmap.map->blue_max)) ? (xdev->cman.std_cmap.map->green_max) : (xdev->cman.std_cmap.map->blue_max)))) ? (xdev->cman.std_cmap.map->red_max) : ((((xdev->cman.std_cmap.map->green_max) < (xdev->cman.std_cmap.map->blue_max)) ? (xdev->cman.std_cmap.map->green_max) : (xdev->cman.std_cmap.map->blue_max)))) + 1;
        ...
        {
            int ramp_size =
                ((((int)pow(xdev->vinfo->colormap_size / 2.0, 1.0/3.0)) < ((((xdev->maxRGBRamp) < (xdev->cman.num_rgb)) ? (xdev->maxRGBRamp) : (xdev->cman.num_rgb)))) ? ((int)pow(xdev->vinfo->colormap_size / 2.0, 1.0/3.0)) : ((((xdev->maxRGBRamp) < (xdev->cman.num_rgb)) ? (xdev->maxRGBRamp) : (xdev->cman.num_rgb))));


            while (!xdev->cman.dither_ramp && ramp_size >= 2) {
                xdev->color_info.dither_grays =
                    xdev->color_info.dither_colors = ramp_size;
                if (!setup_cube(xdev, ramp_size, ((bool)1))) {
        ...
        alloc_dynamic_colors(xdev, (xdev->cman.num_rgb*xdev->cman.num_rgb*xdev->cman.num_rgb) -
                             (xdev->color_info.dither_colors*xdev->color_info.dither_colors*xdev->color_info.dither_colors));
...
grayscale:
        ...
        xdev->cman.std_cmap.map = x_get_std_cmap(xdev, ((Atom) 28));
        if (xdev->cman.std_cmap.map ||
            (xdev->vinfo->class == 0 && alloc_std_cmap(xdev, ((bool)0)))
            ) {
            xdev->color_info.dither_grays =
        ...
        {
            int ramp_size = (((xdev->vinfo->colormap_size / 2) < ((((xdev->maxGrayRamp) < (xdev->cman.num_rgb)) ? (xdev->maxGrayRamp) : (xdev->cman.num_rgb)))) ? (xdev->vinfo->colormap_size / 2) : ((((xdev->maxGrayRamp) < (xdev->cman.num_rgb)) ? (xdev->maxGrayRamp) : (xdev->cman.num_rgb))));


            while (!xdev->cman.dither_ramp && ramp_size >= 3) {
                xdev->color_info.dither_grays = ramp_size;
                if (!setup_cube(xdev, ramp_size, ((bool)0))) {
...
monochrome:
        ...
        break;
    default:
        (emprintf_program_ident(xdev->memory, gs_program_name(), gs_revision_number()), errprintf(xdev->memory, "Unknown palette: %s\n", xdev->palette));
        if (xdev->cman.color_to_rgb.values) {
    ...
    if (!((( (xdev)->color_info.num_components > 1 || (xdev)->color_info.gray_index == 0xff ) ? (xdev)->color_info.max_color : (xdev)->color_info.max_gray) < 31)) {
        if (xdev->cman.std_cmap.map) {
            xdev->cman.match_mask.red &=
                0xffff << xdev->cman.std_cmap.red.cv_shift;
            xdev->cman.match_mask.green &=
                0xffff << xdev->cman.std_cmap.green.cv_shift;
            xdev->cman.match_mask.blue &=
                0xffff << xdev->cman.std_cmap.blue.cv_shift;
        }
    }
...
gdev_x_free_colors(gx_device_X *xdev)
{
    ...
    if (xdev->cman.color_to_rgb.values) {
        ...
        xdev->cman.color_to_rgb.values = ((void*)0);
        xdev->cman.color_to_rgb.size = 0;
    }
}
# 532 "./devices/gdevxcmp.c"
static const X_color_value cv_tab1[] = {
    ((X_color_value)(0xffff * (0) / (1))), ((X_color_value)(0xffff * (1) / (1)))
};
static const X_color_value cv_tab2[] = {
    ((X_color_value)(0xffff * (0) / (2))), ((X_color_value)(0xffff * (1) / (2))), ((X_color_value)(0xffff * (2) / (2)))
};
static const X_color_value cv_tab3[] = {
    ((X_color_value)(0xffff * (0) / (3))), ((X_color_value)(0xffff * (1) / (3))), ((X_color_value)(0xffff * (2) / (3))), ((X_color_value)(0xffff * (3) / (3)))
};
static const X_color_value cv_tab4[] = {
    ((X_color_value)(0xffff * (0) / (4))), ((X_color_value)(0xffff * (1) / (4))), ((X_color_value)(0xffff * (2) / (4))), ((X_color_value)(0xffff * (3) / (4))), ((X_color_value)(0xffff * (4) / (4)))
};
static const X_color_value cv_tab5[] = {
    ((X_color_value)(0xffff * (0) / (5))), ((X_color_value)(0xffff * (1) / (5))), ((X_color_value)(0xffff * (2) / (5))), ((X_color_value)(0xffff * (3) / (5))), ((X_color_value)(0xffff * (4) / (5))), ((X_color_value)(0xffff * (5) / (5)))
};
static const X_color_value cv_tab6[] = {
    ((X_color_value)(0xffff * (0) / (6))), ((X_color_value)(0xffff * (1) / (6))), ((X_color_value)(0xffff * (2) / (6))), ((X_color_value)(0xffff * (3) / (6))), ((X_color_value)(0xffff * (4) / (6))), ((X_color_value)(0xffff * (5) / (6))), ((X_color_value)(0xffff * (6) / (6)))
};
static const X_color_value cv_tab7[] = {
    ((X_color_value)(0xffff * (0) / (7))), ((X_color_value)(0xffff * (1) / (7))), ((X_color_value)(0xffff * (2) / (7))), ((X_color_value)(0xffff * (3) / (7))), ((X_color_value)(0xffff * (4) / (7))), ((X_color_value)(0xffff * (5) / (7))), ((X_color_value)(0xffff * (6) / (7))), ((X_color_value)(0xffff * (7) / (7)))
};
...
gdev_x_map_rgb_color(gx_device * dev, const gx_color_value cv[])
{
    ...
    if (xdev->cman.std_cmap.map) {
        ...
        if (( (xdev)->color_info.num_components > 1 || (xdev)->color_info.gray_index == 0xff )) {
            uint cr, cg, cb;
            X_color_value cvr, cvg, cvb;
            ...
            if (xdev->cman.std_cmap.fast) {
                ...
                cr = r * (cmap->red_max + 1) / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
                cg = g * (cmap->green_max + 1) / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
                cb = b * (cmap->blue_max + 1) / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
    ...
    if (xdev->cman.dither_ramp) {
        if (( (xdev)->color_info.num_components > 1 || (xdev)->color_info.gray_index == 0xff )) {
            ...
            cr = r * dither_rgb / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
            cg = g * dither_rgb / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
            cb = b * dither_rgb / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
            if (max_rgb < (sizeof(cv_tables) / sizeof((cv_tables)[0]))) {
                ...
                cvr = ((X_color_value)(0xffff * (cr) / (max_rgb)));
                cvg = ((X_color_value)(0xffff * (cg) / (max_rgb)));
                cvb = ((X_color_value)(0xffff * (cb) / (max_rgb)));
            }
            if ((iabs((int)r - (int)cvr) & xdev->cman.match_mask.red) == 0 &&
                (iabs((int)g - (int)cvg) & xdev->cman.match_mask.green) == 0 &&
                (iabs((int)b - (int)cvb) & xdev->cman.match_mask.blue) == 0) {
                gx_color_index pixel =
                    xdev->cman.dither_ramp[(((cr) * xdev->color_info.dither_colors + (cg)) * xdev->color_info.dither_colors + (cb))];
            ...
            cr = r * dither_grays / (((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) + 1);
            cvr = (0xffff * cr / max_gray);
            if ((iabs((int)r - (int)cvr) & xdev->cman.match_mask.red) == 0) {
    ...
    if (xdev->cman.dynamic.colors) {
        ...
        xcp = (x11_color_t *)
            (void *)(*(xdev->memory->non_gc_memory)->procs.alloc_byte_array)(xdev->memory->non_gc_memory, sizeof(x11_color_t), 1, "x11_dynamic_color");
        if (!xcp)
            return ((gx_color_index)(~0));
        ...
        if (x_alloc_color(xdev, &xc)) {
            ...
            xcp->color.pad = ((bool)1);
            do { } while (0);


            return xc.pixel;
        } else {
            xcp->color.pad = ((bool)0);
            do { } while (0);
                     ...
                     gx_color_value prgb[3])
{
    ...
    if (color == xdev->background) {
        prgb[0] = prgb[1] = prgb[2] = ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1));
        return 0;
    }
    ...
    if (cmap) {
        if (color >= cmap->base_pixel) {
            ...
            if (value == r * cmap->red_mult + g * cmap->green_mult +
                b * cmap->blue_mult) {
# 827 "./devices/gdevxcmp.c"
                prgb[0] = r * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / cmap->red_max;
                prgb[1] = g * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / cmap->green_max;
                prgb[2] = b * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / cmap->blue_max;
                return 0;
            }
        }
    }
        ...
        if (( (xdev)->color_info.num_components > 1 || (xdev)->color_info.gray_index == 0xff )) {
            ...
            for (i = 0; i < size3; ++i)
                if (xdev->cman.dither_ramp[i] == color) {
                    ...
                    prgb[0] = r * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / max_rgb;
                    prgb[1] = g * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / max_rgb;
                    prgb[2] = b * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / max_rgb;
                    return 0;
                }
        } else {
            ...
            for (i = 0; i < size; ++i)
                if (xdev->cman.dither_ramp[i] == color) {
                    prgb[0] = prgb[1] = prgb[2] =
                        i * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / (size - 1);
                    return 0;
                }
        }
    }
