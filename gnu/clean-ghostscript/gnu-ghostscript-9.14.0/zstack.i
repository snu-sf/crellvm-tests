...
zindex(i_ctx_t *i_ctx_p)
{
    ...
    if ((ulong)op->value.intval >= (ulong)(op - (((i_ctx_p->op_stack).stack).bot))) {

        ref *elt;

        if (op->value.intval < 0)
            return ((-15));
        elt = ref_stack_index(&((i_ctx_p->op_stack).stack), op->value.intval + 1);
        if (elt == 0)
            return ((-17));
        (*(op) = *(elt));
        return 0;
...
zargindex(i_ctx_t *i_ctx_p)
{
    ...
    if (code == (-15) && (((i_ctx_p->op_stack).stack).p)->value.intval >= 0)
        code = ((-17));
    return code;
}
...
zroll(i_ctx_t *i_ctx_p)
{
    ...
    if ( !((((const byte *)&((&*op1)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op1));
    if ( !((((const byte *)&((&*op)->tas.type_attrs))[1]) == (t_integer)) ) return (check_type_failed(&*op));
    if ((uint) op1->value.intval > (uint)(op1 - (((i_ctx_p->op_stack).stack).bot))) {
        ...
        if (op1->value.intval < 0)
            return ((-15));
        if (op1->value.intval + 2 > (int)ref_stack_count(&((i_ctx_p->op_stack).stack)))
            return ((-17));
        ...
        for (i = 0, left = count; left; i++) {
            ref *elt = ref_stack_index(&((i_ctx_p->op_stack).stack), i + 2);
            ref save;
            int j, k;
            ref *next;
            ...
            for (j = i, left--;; j = k, elt = next, left--) {
                ...
                if (k == i)
                    break;
                next = ref_stack_index(&((i_ctx_p->op_stack).stack), k + 2);
                (*(elt) = *(next));
            }
            *elt = save;
        }
...
# 177 "./psi/zstack.c"
    ...
    if (mod <= count >> 1) {
        ...
        if (mod >= (((i_ctx_p->op_stack).stack).top) - op) {
            ((i_ctx_p->op_stack).stack).requested = mod;
            return ((-16));
        }
        ((((i_ctx_p->op_stack).stack).p) -= (2));
        op -= 2;
...
zcount(i_ctx_t *i_ctx_p)
{
    ...
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.intval = (ref_stack_count(&((i_ctx_p->op_stack).stack)) - 1), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
...
zcleartomark(i_ctx_t *i_ctx_p)
{
    ...
    if (count == 0)
        return ((-24));
    ref_stack_pop(&((i_ctx_p->op_stack).stack), count);
    return 0;
}
...
zcounttomark(i_ctx_t *i_ctx_p)
{
    ...
    if (count == 0)
        return ((-24));
    do { if ( (op += (1)) > (((i_ctx_p->op_stack).stack).top) ) { ((i_ctx_p->op_stack).stack).requested = (1); return ((-16)); } else (((i_ctx_p->op_stack).stack).p) = op; } while (0);
    ((op)->value.intval = (count - 1), ((op)->tas.type_attrs = ((t_integer) << 8) + (0)) );
    return 0;
}
