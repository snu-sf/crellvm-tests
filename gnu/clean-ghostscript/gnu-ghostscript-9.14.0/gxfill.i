...
extern const gs_memory_struct_type_t st_section;
...
typedef struct fill_options_s {
    ...
    bool is_spotan;
    int rule;
    int (*fill_rect)(gx_device *dev, int x, int y, int width, int height, gx_color_index color);
    int (*fill_trap)(gx_device *dev, const gs_fixed_edge *left, const gs_fixed_edge *right, fixed ybot, fixed ytop, bool swap_axes, const gx_drawing_color *pdcolor, gs_logical_operation_t lop);
} fill_options;
...
struct line_list_s {
    ...
    int contour_count;
# 147 "./base/gxfill.h"
    active_line local_active[20];
    margin local_margins[20];
    section local_section0[100];
    section local_section1[100];
    int local_windings[20];
};
# 46 "./base/gxfill.c" 2
...
extern const gs_memory_struct_type_t st_device_spot_analyzer;
# 119 "./base/gzspotan.h"
...
static const gs_memory_struct_type_t st_active_line = { sizeof(active_line), "active_line", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
# 196 "./base/gxfill.c"
...
static int spot_into_scan_lines(line_list *, fixed band_mask);
static int spot_into_trapezoids(line_list *, fixed band_mask);
# 237 "./base/gxfill.c"
...
unclose_path(gx_path * ppath, int count)
{
    ...
    for (psub = ppath->segments->contents.subpath_first; count != 0;
...
# 314 "./base/gxfill.c"
    ...
    pseudo_rasterization = (is_character &&
                            ...
                            ibox.q.y - ibox.p.y < 500 * (1<<8) &&
                            ibox.q.x - ibox.p.x < 500 * (1<<8));
    if (is_character)
        adjust.x = adjust.y = 0;
    else
    ...
    lst.contour_count = 0;
    lst.windings = ((void*)0);
    lst.bbox_left = ((int)((ibox.p.x - adjust.x - 1L) >> (8)));
    lst.bbox_width = ((int)(((((ibox.q.x + adjust.x)+((1<<8)-1))&(-1L<<8))) >> (8))) - lst.bbox_left;
    if (0) {
        fixed x0 = ((fixed)(((int)((ibox.p.x - adjust.x - 1L) >> (8))))<<8);
        fixed x1 = ((fixed)(((int)((ibox.q.x + adjust.x + (1<<8) - 1L) >> (8))))<<8);
        fixed y0 = ((fixed)(((int)((ibox.p.y - adjust.y - 1L) >> (8))))<<8);
        fixed y1 = ((fixed)(((int)((ibox.q.y + adjust.y + (1<<8) - 1L) >> (8))))<<8), k;
    ...
    if (pcpath)
        ...
        (*((dev)->procs.get_clipping_box)) (dev, &bbox);
    if (!((ibox).q.y <= (bbox).q.y && (ibox).q.x <= (bbox).q.x && (ibox).p.y >= (bbox).p.y && (ibox).p.x >= (bbox).p.x)) {
        ...
        do { if ((bbox).p.x > (ibox).p.x) (ibox).p.x = (bbox).p.x; if ((bbox).q.x < (ibox).q.x) (ibox).q.x = (bbox).q.x; if ((bbox).p.y > (ibox).p.y) (ibox).p.y = (bbox).p.y; if ((bbox).q.y < (ibox).q.y) (ibox).q.y = (bbox).q.y; } while (0);
...
# 390 "./base/gxfill.c"
    if (adjust.x == ((1L << 8) >> 1))
        fo.adjust_left = ((1L << 8) >> 1) - 1L,
            fo.adjust_right = ((1L << 8) >> 1) ;
    else
        fo.adjust_left = fo.adjust_right = adjust.x;
    if (adjust.y == ((1L << 8) >> 1))
        fo.adjust_below = ((1L << 8) >> 1) - 1L,
            fo.adjust_above = ((1L << 8) >> 1) ;
    else
    ...
    fo.pdevc = pdevc;
    fo.lop = lop;
    fo.fixed_flat = ((fixed)((params->flatness)*(float)(1<<8)));
    fo.ymin = ibox.p.y;
    fo.ymax = ibox.q.y;
    fo.dev = dev;
    fo.pbox = &sbox;
    ...
    fo.fill_direct = (((pdevc)->type == (&gx_dc_type_data_pure)) && (((lop) & (0x200 | (rop3_1 - 0xcc))) == (0xf0 & ~0xcc)));
    fo.fill_rect = (fo.fill_direct ? ((dev)->procs.fill_rectangle) : ((void*)0));
    fo.fill_trap = ((dev)->procs.fill_trapezoid);
# 436 "./base/gxfill.c"
    fill_by_trapezoids =
        (pseudo_rasterization || !((ppath)->curve_count != 0) ||
         params->flatness >= 1.0 || fo.is_spotan);
    if (fill_by_trapezoids && !fo.is_spotan && !(!( (~lop) & ((lop) << 1) & rop3_D ) && !(lop & 0x400))) {
        gs_fixed_rect rbox;

        if (gx_path_is_rectangular(ppath, &rbox)) {
            int x0 = ((int)((((rbox.p.x - fo.adjust_left)+(((1<<8)>>1) - 1L))) >> (8)));
            int y0 = ((int)((((rbox.p.y - fo.adjust_below)+(((1<<8)>>1) - 1L))) >> (8)));
            int x1 = ((int)((((rbox.q.x + fo.adjust_right)+(((1<<8)>>1) - 1L))) >> (8)));
            int y1 = ((int)((((rbox.q.y + fo.adjust_above)+(((1<<8)>>1) - 1L))) >> (8)));

            return (*(pdevc)->type->fill_rectangle)(pdevc, x0, y0, x1 - x0, y1 - y0, dev, lop, ((void*)0));

        }
        if (fo.adjust_left | fo.adjust_right | fo.adjust_below | fo.adjust_above)
            fill_by_trapezoids = ((bool)0);
    }
    (void)gx_path_init_local_shared(&ffpath, ((void*)0), ppath->memory);
    if (!big_path && !((ppath)->curve_count != 0))
        pfpath = ppath;
        ...
        code = gx_path_copy_reducing(ppath, &ffpath, fo.fixed_flat, ((void*)0),
                            pco_small_curves);
        if (code < 0)
            return code;
        pfpath = &ffpath;
    ...
    if ((code = add_y_list(pfpath, &lst)) < 0)
        goto nope;
    {
        int (*fill_loop)(line_list *, fixed band_mask);
        ...
        if (lst.bbox_width > 100 && fo.pseudo_rasterization) {
            ...
            lst.margin_set0.sect = (section *)(section *)(*(pdev->memory)->procs.alloc_struct_array)(pdev->memory, lst.bbox_width * 2, &st_section, "gx_general_fill_path");
        ...
        if (gs_currentcpsimode(pis->memory) && is_character) {
            if (lst.contour_count > (sizeof(lst.local_windings) / sizeof((lst.local_windings)[0]))) {
                lst.windings = (int *)(*(pdev->memory)->procs.alloc_byte_array)(pdev->memory, lst.contour_count, sizeof(int), "gx_general_fill_path");
        ...
        code = (*fill_loop)
            (&lst, (max_fill_band == 0 ? ((fixed)(-1) << (sizeof(fixed) * 8 - 1)) : ((fixed)(-max_fill_band)<<8)));
        if (lst.margin_set0.sect != lst.local_section0 &&
            lst.margin_set0.sect != lst.local_section1)
            ((pdev->memory)->procs.free_object(pdev->memory, (((lst.margin_set0.sect) < (lst.margin_set1.sect)) ? (lst.margin_set0.sect) : (lst.margin_set1.sect)), "gx_general_fill_path"));
        if (lst.windings != ((void*)0) && lst.windings != lst.local_windings)
            ((pdev->memory)->procs.free_object(pdev->memory, lst.windings, "gx_general_fill_path"));
    }
                 ...
                 const gx_device_color * pdevc, const gx_clip_path * pcpath)
{
    if (pdevc == ((void*)0)) {
        ...
        return ((cdev->target)->procs.fill_path)(cdev->target, pis, ppath, params, pdevc, pcpath);
    }
...
# 592 "./base/gxfill.c"
        ...
        if (ppath != ((void*)0)) {
            ...
            if (pcpath == ((void*)0)) {
                ...
                (*((pdev)->procs.get_clipping_box)) (pdev, &clip_box1);
        ...
        if (code >= 0) {
            ...
            gs_int_rect cb;
            const gx_rop_source_t *rs = ((void*)0);
            gx_device *dev;
            gx_device_clip cdev;

            gx_cpath_outer_box(pcpath1, &clip_box);
            cb.p.x = ((int)((((clip_box.p.x)+(((1<<8)>>1) - 1L))) >> (8)));
            cb.p.y = ((int)((((clip_box.p.y)+(((1<<8)>>1) - 1L))) >> (8)));
            cb.q.x = ((int)((((clip_box.q.x)+(((1<<8)>>1) - 1L))) >> (8)));
            cb.q.y = ((int)((((clip_box.q.y)+(((1<<8)>>1) - 1L))) >> (8)));
            if (gx_dc_is_pattern2_color(pdevc) &&
                    (*((pdev)->procs.dev_spec_op))(pdev,
                         gxdso_pattern_handles_clip_path, ((void*)0), 0) > 0) {



                code = (*((pdev)->procs.fill_path))(pdev, pis, ppath, params, ((void*)0), pcpath1);
                dev = pdev;
            } else {
                gx_make_clip_device_on_stack(&cdev, pcpath1, pdev);
                dev = (gx_device *)&cdev;
                if ((*((pdev)->procs.dev_spec_op))(pdev,
                        gxdso_pattern_shading_area, ((void*)0), 0) > 0)
                    ((&cdev)->procs.fill_path = (pass_shading_area_through_clip_path_device));
                code = 0;
            }
            if (code >= 0)
...
free_line_list(line_list *ll)
{
    ...
    while ((alp = ll->active_area) != 0) {
        ...
        ((mem)->procs.free_object(mem, alp, "active line"));
        ll->active_area = next;
    }
    free_all_margins(ll);
}

static __inline__ active_line *
make_al(line_list *ll)
{
    active_line *alp = ll->next_active;

    if (alp == ll->limit) {
        alp = (active_line *)(*(ll->memory)->procs.alloc_struct)(ll->memory, &st_active_line, "active line");
...
{
    if ((((y0) > (y1)) ? (y0) : (y1)) < fo->ymin)
        return 2;
    if ((((y0) < (y1)) ? (y0) : (y1)) > fo->ymax)
        return 2;
    return (y0 < y1 ? 1 :
            y0 > y1 ? (-1) : 0);
}
    ...
    gx_flattened_iterator *fi, bool more1, bool step_back, bool monotonic_x)
{
    ...
    code = step_al(alp, ((bool)0));
    if (code < 0)
        return code;
    alp->monotonic_y = ((bool)0);
...
start_al_pair(line_list *ll, contour_cursor *q, contour_cursor *p)
{
    ...
    if (q->monotonic_y)
        code = add_y_line(q->prev, q->pseg, (-1), ll);
    else
        code = add_y_curve_part(ll, q->prev, q->pseg, (-1), q->fi,
                            !q->first_flattened, ((bool)0), q->monotonic_x);
    if (code < 0)
        return code;
    if (p->monotonic_y)
        code = add_y_line(p->prev, p->pseg, 1, ll);
    else
        code = add_y_curve_part(ll, p->prev, p->pseg, 1, p->fi,
                            p->more_flattened, ((bool)0), p->monotonic_x);
    return code;
}
...
start_al_pair_from_min(line_list *ll, contour_cursor *q)
{
    ...
    do {
        ...
        if (q->fi->ly1 > fo->ymax && ll->y_break > q->fi->ly1)
            ll->y_break = q->fi->ly1;
        if (dir == 1 && ll->main_dir == (-1) && q->fi->ly0 >= fo->ymin) {
            code = add_y_curve_part(ll, q->prev, q->pseg, (-1), q->fi,
                            ((bool)1), ((bool)1), q->monotonic_x);
            if (code < 0)
                return code;
            code = add_y_curve_part(ll, q->prev, q->pseg, 1, q->fi,
                            q->more_flattened, ((bool)0), q->monotonic_x);
            if (code < 0)
                return code;
            ...
            code = add_y_curve_part(ll, q->prev, q->pseg, (-1), q->fi,
                            ((bool)1), ((bool)0), q->monotonic_x);
            if (code < 0)
                return code;
        }
        q->first_flattened = ((bool)0);
        q->dir = dir;
        ll->main_dir = (dir == (-1) ? (-1) :
                        dir == 1 ? 1 : ll->main_dir);
        if (!q->more_flattened)
...
init_contour_cursor(line_list *ll, contour_cursor *q)
{
    ...
    if (q->pseg->type == s_curve) {
        ...
        fixed ymin = ((((((q->prev->pt.y) < (s->p1.y)) ? (q->prev->pt.y) : (s->p1.y))) < ((((s->p2.y) < (s->pt.y)) ? (s->p2.y) : (s->pt.y)))) ? ((((q->prev->pt.y) < (s->p1.y)) ? (q->prev->pt.y) : (s->p1.y))) : ((((s->p2.y) < (s->pt.y)) ? (s->p2.y) : (s->pt.y))));
        fixed ymax = ((((((q->prev->pt.y) > (s->p1.y)) ? (q->prev->pt.y) : (s->p1.y))) > ((((s->p2.y) > (s->pt.y)) ? (s->p2.y) : (s->pt.y)))) ? ((((q->prev->pt.y) > (s->p1.y)) ? (q->prev->pt.y) : (s->p1.y))) : ((((s->p2.y) > (s->pt.y)) ? (s->p2.y) : (s->pt.y))));
...
scan_contour(line_list *ll, contour_cursor *q)
{
    ...
    segment *pseg;
    int code;
    bool only_horizontal = ((bool)1), saved = ((bool)0);
    const fill_options * const fo = ll->fo;
    contour_cursor save_q;

    memset(&save_q, 0, sizeof(save_q));
    p.monotonic_x = ((bool)0);
    p.fi = &fi;
    save_q.dir = 2;
    ll->main_dir = 0;
    for (; ; q->pseg = q->prev, q->prev = q->prev->prev) {
        ...
        for (;;) {
            ...
            ll->main_dir = (q->dir == (-1) ? (-1) :
                            q->dir == 1 ? 1 : ll->main_dir);
        }
        q->dir = compute_dir(fo, q->fi->ly0, q->fi->ly1);
        q->more_flattened = ((bool)0);
        ll->main_dir = (q->dir == (-1) ? (-1) :
                        q->dir == 1 ? 1 : ll->main_dir);
        if (ll->main_dir != 0) {
            only_horizontal = ((bool)0);
            break;
        }
        if (!saved && q->dir != 2) {
            save_q = *q;
    ...
    for (pseg = q->pfirst; pseg != q->plast; pseg = pseg->next) {
        ...
        if (!fo->pseudo_rasterization || only_horizontal
            ...
            if (p.fi->ly1 > fo->ymax && ll->y_break > p.fi->ly1)
                ll->y_break = p.fi->ly1;
            if (p.monotonic_y && p.dir == 0 &&
                    !fo->pseudo_rasterization &&



                    (fo->adjust_below + fo->adjust_above >= ((1L << 8) - 1L) ||
                    ((int)((((p.pseg->pt.y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))) <
                    ((int)((((p.pseg->pt.y + fo->adjust_above)+(((1<<8)>>1) - 1L))) >> (8))))) {


                code = add_y_line(p.prev, p.pseg, 0, ll);
                if (code < 0)
                    return code;
            }
            if (p.monotonic_y && p.dir == 0 &&
            ...
            if (p.fi->ly0 >= fo->ymin && p.dir == 1 && ll->main_dir == (-1)) {
            ...
            if (p.fi->ly0 >= fo->ymin && p.fi->ly1 < fo->ymin) {
                if (p.monotonic_y)
                    ...
                    code = add_y_curve_part(ll, p.prev, p.pseg, (-1), p.fi,
                                        !p.first_flattened, ((bool)0), p.monotonic_x);
                if (code < 0)
                    return code;
            }
            ll->main_dir = (p.dir == (-1) ? (-1) :
                            p.dir == 1 ? 1 : ll->main_dir);
...
{
    subpath *psub = ppath->segments->contents.subpath_first;
    int close_count = 0;
    int code;
    contour_cursor q;
    gx_flattened_iterator fi;

    q.monotonic_x = ((bool)0);
    ll->y_break = (~(-1 << ((1 << 2) * 8 - 1)));
...
{
    bool forth = (alp->direction == 1 || !alp->fi.curve);
    ...
    (alp)->num_adjust = ((alp)->diff.x >= 0 ? 0 : -(alp)->diff.y + 1L);
    (alp)->y_fast_max = (((~(-1 << ((1 << 2) * 8 - 1)))) + (alp)->num_adjust) /
...
{
    const segment *ss = (alp->direction == 1 ? s1 : s0);

    bool curve = (ss != ((void*)0) && ss->type == s_curve);
    int code;
    ...
    if (curve) {
        ...
        if (code < 0)
            return code;
        alp->monotonic_x = alp->monotonic_y = ((bool)1);
    }
    alp->pseg = s1;
    return 0;
}
        ...
        const active_line *flp, const active_line *alp, fixed y, fixed y1)
{
    ...
    if (flp->start.x < flp->end.x) {
        ...
        if (0)
        {
            ...
            cy = (((vert_right.start.y)+(((1<<8)>>1) - 1L))&(-1L<<8))+((1L << 8) >> 1);
            cx = (((vert_right.start.x)+(((1<<8)>>1) - 1L))&(-1L<<8))+((1L << 8) >> 1);
            ...
            cy = (((vert_left.end.y)+(((1<<8)>>1) - 1L))&(-1L<<8))-((1L << 8) >> 1);
            cx = (((vert_left.end.x)+(((1<<8)>>1) - 1L))&(-1L<<8))-((1L << 8) >> 1);
        ...
        if (0)
        {
            ...
            cy = (((vert_left.start.y)+(((1<<8)>>1) - 1L))&(-1L<<8))+((1L << 8) >> 1);
            cx = (((vert_left.start.x)+(((1<<8)>>1) - 1L))&(-1L<<8))-((1L << 8) >> 1);
            dy = slant_left.end.y-slant_left.start.y;
            x = vert_left.start.x -
            ...
            cy = (((vert_right.end.y)+(((1<<8)>>1) - 1L))&(-1L<<8))-((1L << 8) >> 1);
            cx = (((vert_right.end.x)+(((1<<8)>>1) - 1L))&(-1L<<8))-((1L << 8) >> 1);
            dy = slant_right.end.y-slant_right.start.y;
            x = vert_right.end.x +
    ...
    if (Ya >= Y1b) {
        ...
        int iYb = ((int)((((Yb)+(((1<<8)>>1) - 1L))) >> (8)));
        int iYa = ((int)((((Ya)+(((1<<8)>>1) - 1L))) >> (8)));
        int iY1b = ((int)((((Y1b)+(((1<<8)>>1) - 1L))) >> (8)));
        int iY1a = ((int)((((Y1a)+(((1<<8)>>1) - 1L))) >> (8)));

        do { } while (0);
        if (iY1b > iYb) {
            code = fo->fill_trap(fo->dev, plbot, prbot,
                                 Yb, Y1b, ((bool)0), fo->pdevc, fo->lop);
            if (code < 0)
                return code;
        }
        if (iYa > iY1b) {
            int ix = ((int)((((vert_left.start.x)+(((1<<8)>>1) - 1L))) >> (8)));
            int iw = ((int)((((vert_right.start.x)+(((1<<8)>>1) - 1L))) >> (8))) - ix;

            code = (*(fo->pdevc)->type->fill_rectangle)(fo->pdevc, ix, iY1b, iw, iYa - iY1b, fo->dev, fo->lop, ((void*)0));
        ...
        if (fo->pbox->q.y > Y1b) {
            code = fo->fill_trap(fo->dev, &slant_left, &slant_right,
                                 Yac, Y1b, ((bool)0), fo->pdevc, fo->lop);
            if (code < 0)
                return code;
            code = fo->fill_trap(fo->dev, pltop, prtop,
                                 Y1b, Y1a, ((bool)0), fo->pdevc, fo->lop);
        } else
            code = fo->fill_trap(fo->dev, &slant_left, &slant_right,
                                 Yac, fo->pbox->q.y, ((bool)0), fo->pdevc, fo->lop);
    }
    return code;
}
...
move_al_by_y(line_list *ll, fixed y1)
{
    ...
    for (x = (-1 << ((1 << 2) * 8 - 1)), alp = ll->x_head.next; alp != 0; alp = nlp) {
        bool notend = ((bool)0);
    ...
    if (ll->x_head.next != 0 && ll->fo->pseudo_rasterization) {
        ...
        for (alp = ll->x_head.next; alp->next != 0; ) {
    ...
    for (alp = ll->x_head.next; alp != 0; alp = nlp) {
        ...
        if (alp->start.y == y && alp->end.y == y) {
            if (ll->fo->pseudo_rasterization) {
                code = add_y_line_aux(((void*)0), ((void*)0), &alp->start, &alp->end, 0, ll);
                if (code < 0)
                    return code;
            }
            inserted = 1;
...
{
    const fill_options * const fo = ll->fo;
    fixed ybot = (((y) > (fo->pbox->p.y)) ? (y) : (fo->pbox->p.y));
    fixed ytop = (((y1) < (fo->pbox->q.y)) ? (y1) : (fo->pbox->q.y));

    if (ybot >= ytop)
        return 0;
    do { } while (0);
    return (*fo->fill_trap)
        (fo->dev, le, re, ybot, ytop, ((bool)0), fo->pdevc, fo->lop);
}
...
# 30 "./base/gxfillts.h"
static __inline__ int
slant_into_trapezoids__fd (const line_list *ll,
        const active_line *flp, const active_line *alp, fixed y, fixed y1)
{
# 42 "./base/gxfillts.h"
    const fill_options * const fo = ll->fo;
    ...
    int code = 0;
# 67 "./base/gxfillts.h"
    fixed y_span_delta = (((1<<8)>>1) - 1L) + fo->adjust_above;
    ...
    if ((le.end.x == le.start.x) && (re.end.x == re.start.x)) {
        ...
        le.start.x = ((int)((((le.start.x)+(((1<<8)>>1) - 1L))) >> (8)));
        le.start.y = ((int)((((y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8)));
        re.start.x = ((int)((((re.start.x)+(((1<<8)>>1) - 1L))) >> (8)));
        re.start.y = ((int)((((y1 + fo->adjust_above)+(((1<<8)>>1) - 1L))) >> (8)));
        return ( 1 ? (fo)->fill_rect((fo)->dev, le.start.x, le.start.y, re.start.x-le.start.x, re.start.y-le.start.y, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, le.start.x, le.start.y, re.start.x-le.start.x, re.start.y-le.start.y, (fo)->dev, (fo)->lop, ((void*)0)));
        ...
        if ((((int)((y1) + y_span_delta)&((1<<8)-1)) < y_span_limit)) {



            int xli = ((int)((((flp->x_next - fo->adjust_left)+(((1<<8)>>1) - 1L))) >> (8)));
            int xri = ((int)((((alp->x_next + fo->adjust_right)+(((1<<8)>>1) - 1L))) >> (8)));
            ...
            if (0) {
                ...
                if ((((int)((((newxl)+(((1<<8)>>1) - 1L))) >> (8))) == xli) &&
                    (((int)((((newxr)+(((1<<8)>>1) - 1L))) >> (8))) == xri)) {
            ...
            do { } while (0);
            code = ( 1 ? (fo)->fill_rect((fo)->dev, xli, ((int)((((y1 - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, xli, ((int)((((y1 - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->dev, (fo)->lop, ((void*)0)));
        ...
        if ((((int)((y) + y_span_delta)&((1<<8)-1)) < y_span_limit)) {



            int xli = ((int)((((flp->x_current - fo->adjust_left)+(((1<<8)>>1) - 1L))) >> (8)));
            int xri = ((int)((((alp->x_current + fo->adjust_right)+(((1<<8)>>1) - 1L))) >> (8)));
            ...
            do { } while (0);
            code = ( 1 ? (fo)->fill_rect((fo)->dev, xli, ((int)((((y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, xli, ((int)((((y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->dev, (fo)->lop, ((void*)0)));
...
# 30 "./base/gxfillts.h"
static __inline__ int
slant_into_trapezoids__nd (const line_list *ll,
        const active_line *flp, const active_line *alp, fixed y, fixed y1)
{
# 42 "./base/gxfillts.h"
    const fill_options * const fo = ll->fo;
...
# 67 "./base/gxfillts.h"
    ...
    if ((le.end.x == le.start.x) && (re.end.x == re.start.x)) {
        ...
        re.start.x = ((int)((((re.start.x)+(((1<<8)>>1) - 1L))) >> (8)));
        re.start.y = ((int)((((y1 + fo->adjust_above)+(((1<<8)>>1) - 1L))) >> (8)));
        return ( 0 ? (fo)->fill_rect((fo)->dev, le.start.x, le.start.y, re.start.x-le.start.x, re.start.y-le.start.y, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, le.start.x, le.start.y, re.start.x-le.start.x, re.start.y-le.start.y, (fo)->dev, (fo)->lop, ((void*)0)));
        ...
        if ((((int)((y1) + y_span_delta)&((1<<8)-1)) < y_span_limit)) {
            ...
            do { } while (0);
            code = ( 0 ? (fo)->fill_rect((fo)->dev, xli, ((int)((((y1 - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, xli, ((int)((((y1 - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->dev, (fo)->lop, ((void*)0)));
        ...
        if ((((int)((y) + y_span_delta)&((1<<8)-1)) < y_span_limit)) {
            ...
            do { } while (0);
            code = ( 0 ? (fo)->fill_rect((fo)->dev, xli, ((int)((((y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->pdevc->colors.pure) : (*((fo)->pdevc)->type->fill_rectangle)((fo)->pdevc, xli, ((int)((((y - fo->adjust_below)+(((1<<8)>>1) - 1L))) >> (8))), xri - xli, 1, (fo)->dev, (fo)->lop, ((void*)0)));
...
intersect(active_line *endp, active_line *alp, fixed y, fixed y1, fixed *p_y_new)
{
    ...
    y_new =
        ((dy | dx_old) < 1L << (((int)(sizeof(fixed))) * 4 - 1) ?
         dy * dx_old / dx_den :
         (((void)(0)), fixed_mult_quo(dy, dx_old, dx_den)))
        + y;
    ...
    if (y_new <= y) {
        ...
        if (endp->start.y < alp->start.y)
            ys = alp->start.y,
                xs0 = ((ys) == (endp)->end.y ? (endp)->end.x : ((ys) <= (endp)->y_fast_max ? ((((ys) - (endp)->start.y) * (endp)->diff.x) + (endp)->num_adjust) / (endp)->diff.y : (((void)(0)), fixed_mult_quo((endp)->diff.x, (ys) - (endp)->start.y, (endp)->diff.y))) + (endp)->start.x), xs1 = alp->start.x;
        else
            ys = endp->start.y,
                xs0 = endp->start.x, xs1 = ((ys) == (alp)->end.y ? (alp)->end.x : ((ys) <= (alp)->y_fast_max ? ((((ys) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (ys) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);
        if (endp->end.y > alp->end.y)
            ye = alp->end.y,
                xe0 = ((ye) == (endp)->end.y ? (endp)->end.x : ((ye) <= (endp)->y_fast_max ? ((((ye) - (endp)->start.y) * (endp)->diff.x) + (endp)->num_adjust) / (endp)->diff.y : (((void)(0)), fixed_mult_quo((endp)->diff.x, (ye) - (endp)->start.y, (endp)->diff.y))) + (endp)->start.x), xe1 = alp->end.x;
        else
            ye = endp->end.y,
                xe0 = endp->end.x, xe1 = ((ye) == (alp)->end.y ? (alp)->end.x : ((ye) <= (alp)->y_fast_max ? ((((ye) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (ye) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);
        dy = ye - ys;
        dx0 = xe0 - xs0;
        dx1 = xe1 - xs1;
...
{
    return 1 + ((((((int)((int64_t)alp->diff.y * 8 / alp->diff.x)) < 0 ? -((int)((int64_t)alp->diff.y * 8 / alp->diff.x)) : ((int)((int64_t)alp->diff.y * 8 / alp->diff.x)))) < (256)) ? ((((int)((int64_t)alp->diff.y * 8 / alp->diff.x)) < 0 ? -((int)((int64_t)alp->diff.y * 8 / alp->diff.x)) : ((int)((int64_t)alp->diff.y * 8 / alp->diff.x)))) : (256));
}
...
{
    fixed x = (-1 << ((1 << 2) * 8 - 1)), y1 = *y_top;
    active_line *alp, *stopx = ((void*)0);
    active_line *endp = ((void*)0);
    ...
    if (y == y1) {
        ...
        for (alp = stopx = ll->x_head.next;
             ((void)(0)), alp != 0;
             endp = alp, alp = alp->next
            ) {
            fixed nx = ((y1) == (alp)->end.y ? (alp)->end.x : ((y1) <= (alp)->y_fast_max ? ((((y1) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y1) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);
            fixed y_new;
            ...
            if (nx >= x)
                ...
                if (y_new <= y1) {
                    ...
                    if (endp->diff.x == 0)
                        ...
                        fixed nx0 = ((y1) == (endp)->end.y ? (endp)->end.x : ((y1) <= (endp)->y_fast_max ? ((((y1) - (endp)->start.y) * (endp)->diff.x) + (endp)->num_adjust) / (endp)->diff.y : (((void)(0)), fixed_mult_quo((endp)->diff.x, (y1) - (endp)->start.y, (endp)->diff.y))) + (endp)->start.x);
                        fixed nx1 = ((y1) == (alp)->end.y ? (alp)->end.x : ((y1) <= (alp)->y_fast_max ? ((((y1) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y1) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);
                        if (nx0 != nx1) {
        ...
        for (alp = ll->x_head.next; alp != stopx; alp = alp->next)
            alp->x_next = ((y1) == (alp)->end.y ? (alp)->end.x : ((y1) <= (alp)->y_fast_max ? ((((y1) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y1) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);

        if (ll->x_head.next != ((void*)0)) {
            for (;;) {
                fixed x1;
                ...
                endp = ll->x_head.next;
                x1 = endp->x_next;
                for (alp = endp->next; alp != ((void*)0); x1 = alp->x_next, alp = alp->next)
                    if (alp->x_next < x1)
                        break;
                if (alp == ((void*)0))
                    break;
                x1 = endp->x_next;
                n = 0;
                for (alp = endp->next; alp != ((void*)0); alp = alp->next) {
                     x = alp->x_next;
                     if (x < x1) {
                        if (n == 0) {
                            if (endp->diff.x == 0) {
...
static __inline__ int sign(int a)
{
...
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__pr_fd_sw (line_list *ll, fixed band_mask)
{
    ...
    y = yll->start.y;
    ll->x_head.next = 0;
    ll->x_head.x_current = (-1 << ((1 << 2) * 8 - 1));
    ll->margin_set0.y = (((y)+(((1<<8)>>1) - 1L))&(-1L<<8)) - ((1L << 8) >> 1);
    ll->margin_set1.y = (((y)+(((1<<8)>>1) - 1L))&(-1L<<8)) - (1L << 8) - ((1L << 8) >> 1);
    while (1) {
        fixed y1;
        active_line *alp, *plp = ((void*)0);
        ...
        while (yll != 0 && yll->start.y == y) {
            ...
            if (yll->direction == 0) {
                if (!1) {
                    ...
                    int yi = ((int)((((y - (!0 ? 0 : fo.adjust_below))+(((1<<8)>>1) - 1L))) >> (8)));
                    int xi, wi;

                    if (yll->start.x <= yll->end.x) {
                        xi = ((int)((((yll->start.x - (!0 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                        wi = ((int)((((yll->end.x + (!0 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8))) - xi;
                    } else {
                        xi = ((int)((((yll->end.x - (!0 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                        wi = ((int)((((yll->start.x + (!0 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8))) - xi;
                    }
                    do { } while (0);
                    code = ( 1 ? (&fo)->fill_rect((&fo)->dev, xi, yi, wi, 1, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, xi, yi, wi, 1, (&fo)->dev, (&fo)->lop, ((void*)0)));
                    if (code < 0)
                        return code;
                } else if (1)
                    insert_h_new(yll, ll);
        ...
        covering_pixel_centers = ((((y - (!0 ? 0 : fo.adjust_below))+(((1<<8)>>1) - 1L))&(-1L<<8)) < (((y1 + (!0 ? 0 : fo.adjust_above))+(((1<<8)>>1) - 1L))&(-1L<<8)));
        ...
        if (covering_pixel_centers || all_bands) {
            ...
            for (alp = ll->x_head.next; alp != 0; alp = alp->next) {
                ...
                do { } while (0);
                if (!((inside & rule) != 0)) {
                    { int k = alp->contour_count; int v = ll->windings[k]; inside -= ((v) < 0 ? -((v) & 1) : (v) > 0 ? ((v) & 1) : 0); v = ll->windings[k] += alp->direction; inside += ((v) < 0 ? -((v) & 1) : (v) > 0 ? ((v) & 1) : 0); };
                    if (((inside & rule) != 0))
                        flp = alp;
                    continue;
                }
...
# 210 "./base/gxfilltr.h"
                ...
                do { } while (0);
                if (0 && !(flp->end.x == flp->start.x && alp->end.x == alp->start.x) &&
                    ...
                    if (0) {
                        ...
                        if (flp->end.x == flp->start.x && alp->end.x == alp->start.x) {
                            if (0) {
                                ybot = (((y - fo.adjust_below) > (fo.pbox->p.y)) ? (y - fo.adjust_below) : (fo.pbox->p.y));
                                ytop = (((y1 + fo.adjust_above) < (fo.pbox->q.y)) ? (y1 + fo.adjust_above) : (fo.pbox->q.y));
                            }
                            if (ytop > ybot) {
                                int yi = ((int)((((ybot)+(((1<<8)>>1) - 1L))) >> (8)));
                                int hi = ((int)((((ytop)+(((1<<8)>>1) - 1L))) >> (8))) - yi;
                                int xli = ((int)((((flp->end.x - (!0 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                                int xi = ((int)((((alp->end.x + (!0 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8)));
                                ...
                                do { } while (0);
                                code = ( 1 ? (&fo)->fill_rect((&fo)->dev, xli, yi, xi - xli, hi, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, xli, yi, xi - xli, hi, (&fo)->dev, (&fo)->lop, ((void*)0)));
                            } else
                                code = 0;
                        } else if (ybot < ytop) {
                            gs_fixed_edge le, re;
                            ...
                            if (1) {
                                ...
                                code = fo.fill_trap(fo.dev,
                                        &le, &re, ybot, ytop, ((bool)0), fo.pdevc, fo.lop);
                        } else
                            code = 0;
                    }
                    if (1) {
...
# 2004 "./base/gxfill.c" 2
# 2017 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__pr_nd_sw (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
        ...
        while (yll != 0 && yll->start.y == y) {
            ...
            if (yll->direction == 0) {
                if (!1) {
                    ...
                    do { } while (0);
                    code = ( 0 ? (&fo)->fill_rect((&fo)->dev, xi, yi, wi, 1, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, xi, yi, wi, 1, (&fo)->dev, (&fo)->lop, ((void*)0)));
                    if (code < 0)
                        return code;
                } else if (1)
                    insert_h_new(yll, ll);
                    ...
                    if (0) {
                        ...
                        if (flp->end.x == flp->start.x && alp->end.x == alp->start.x) {
                            ...
                            if (ytop > ybot) {
                                ...
                                do { } while (0);
                                code = ( 0 ? (&fo)->fill_rect((&fo)->dev, xli, yi, xi - xli, hi, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, xli, yi, xi - xli, hi, (&fo)->dev, (&fo)->lop, ((void*)0)));
                            } else
                                code = 0;
                        } else if (ybot < ytop) {
                            gs_fixed_edge le, re;
...
# 2018 "./base/gxfill.c" 2
# 2031 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__nj_fd_sw (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2032 "./base/gxfill.c" 2
# 2045 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__nj_nd_sw (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2046 "./base/gxfill.c" 2
# 2064 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__spotan (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2065 "./base/gxfill.c" 2
# 2078 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__pr_fd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2079 "./base/gxfill.c" 2
# 2092 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__pr_nd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2093 "./base/gxfill.c" 2
# 2106 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__aj_fd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
    ...
    while (1) {
        ...
        while (yll != 0 && yll->start.y == y) {
            ...
            if (yll->direction == 0) {
                if (!0) {
                    ...
                    int yi = ((int)((((y - (!1 ? 0 : fo.adjust_below))+(((1<<8)>>1) - 1L))) >> (8)));
                    int xi, wi;

                    if (yll->start.x <= yll->end.x) {
                        xi = ((int)((((yll->start.x - (!1 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                        wi = ((int)((((yll->end.x + (!1 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8))) - xi;
                    } else {
                        xi = ((int)((((yll->end.x - (!1 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                        wi = ((int)((((yll->start.x + (!1 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8))) - xi;
                    }
                    do { } while (0);
                    code = ( 1 ? (&fo)->fill_rect((&fo)->dev, xi, yi, wi, 1, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, xi, yi, wi, 1, (&fo)->dev, (&fo)->lop, ((void*)0)));
                    if (code < 0)
        ...
        covering_pixel_centers = ((((y - (!1 ? 0 : fo.adjust_below))+(((1<<8)>>1) - 1L))&(-1L<<8)) < (((y1 + (!1 ? 0 : fo.adjust_above))+(((1<<8)>>1) - 1L))&(-1L<<8)));
...
# 210 "./base/gxfilltr.h"
                ...
                do { } while (0);
                if (1 && !(flp->end.x == flp->start.x && alp->end.x == alp->start.x) &&
                    (fo.adjust_below | fo.adjust_above) != 0) {
                    ...
                    if (0) {
                        ...
                        if (flp->end.x == flp->start.x && alp->end.x == alp->start.x) {
                            ...
                            if (ytop > ybot) {
                                int yi = ((int)((((ybot)+(((1<<8)>>1) - 1L))) >> (8)));
                                int hi = ((int)((((ytop)+(((1<<8)>>1) - 1L))) >> (8))) - yi;
                                int xli = ((int)((((flp->end.x - (!1 ? 0 : fo.adjust_left))+(((1<<8)>>1) - 1L))) >> (8)));
                                int xi = ((int)((((alp->end.x + (!1 ? 0 : fo.adjust_right))+(((1<<8)>>1) - 1L))) >> (8)));
...
# 2107 "./base/gxfill.c" 2
# 2120 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__aj_nd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2121 "./base/gxfill.c" 2
# 2134 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__nj_fd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
# 2135 "./base/gxfill.c" 2
# 2148 "./base/gxfill.c"
# 1 "./base/gxfilltr.h" 1
# 39 "./base/gxfilltr.h"
static int
spot_into_trapezoids__nj_nd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    int rule = fo.rule;
    const fixed y_limit = fo.ymax;
...
static const gs_memory_struct_type_t st_coord_range = { sizeof(coord_range_t), "coord_range_t", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
range_alloc(coord_range_list_t *pcrl)
{
    ...
    if (pcrl->freed) {
        ...
        pcr = (coord_range_t *)(*(pcrl->memory)->procs.alloc_struct)(pcrl->memory, &st_coord_range, "range_alloc");
                ...
                int num_local, gs_memory_t *mem)
{
    ...
    pcrl->first.rmin = pcrl->first.rmax = (-1 << ((1 << 2) * 8 - 1));
    pcrl->first.prev = 0;
    pcrl->last.rmin = pcrl->last.rmax = (~(-1 << ((1 << 2) * 8 - 1)));
    pcrl->last.next = 0;
...
range_list_free(coord_range_list_t *pcrl)
{
    ...
    while ((pcr = pcrl->allocated) != 0) {
        ...
        ((pcrl->memory)->procs.free_object(pcrl->memory, pcr, "range_list_free"));
        pcrl->allocated = next;
    }
}
...
merge_ranges(coord_range_list_t *pcrl, const line_list *ll, fixed y_min, fixed y_top)
{
    ...
    for (alp = ll->x_head.next; alp != 0 && code >= 0; alp = nlp) {
        ...
        if (alp->monotonic_x && alp->monotonic_y && ye <= y_top) {
            ...
            code = range_list_add(pcrl,
                                  ((int)((((x0 - ll->fo->adjust_left)+(((1<<8)>>1) - 1L))) >> (8))),
                                  ((int)(((x1 + ll->fo->adjust_right)+((1<<8)>>1)) >> (8))));
            alp->more_flattened = ((bool)0);
        } else {
            x1 = x0;
            for (;;) {
                if (alp->end.y <= y_top)
                    ...
                    xt = ((y_top) == (alp)->end.y ? (alp)->end.x : ((y_top) <= (alp)->y_fast_max ? ((((y_top) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y_top) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x);
                x0 = (((x0) < (xt)) ? (x0) : (xt));
                x1 = (((x1) > (xt)) ? (x1) : (xt));
...
# 28 "./base/gxfillsl.h"
static int
spot_into_scan_lines_fd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    active_line *yll = ll->y_list;
...
# 43 "./base/gxfillsl.h"
    fixed y_frac_min =
        (fo.adjust_above == 0L ? ((1L << 8) >> 1) :
         ((1L << 8) >> 1) + 1L - fo.adjust_above);
    fixed y_frac_max =
        ((1L << 8) >> 1) + fo.adjust_below;
    int y0 = ((int)(((-1 << ((1 << 2) * 8 - 1))) >> (8)));
    fixed y_bot = (-1 << ((1 << 2) * 8 - 1));
    fixed y_top = (-1 << ((1 << 2) * 8 - 1));
    fixed y = (-1 << ((1 << 2) * 8 - 1));
    coord_range_list_t rlist;
    coord_range_t rlocal[20];
    int code = 0;

    if (yll == 0)
        return 0;
    range_list_init(&rlist, rlocal, (sizeof(rlocal) / sizeof((rlocal)[0])), ll->memory);
    ll->x_head.next = 0;
    ll->x_head.x_current = (-1 << ((1 << 2) * 8 - 1));
    do {
        active_line *alp, *nlp;
        ...
        if (ll->x_head.next == 0)
            ...
            y = y_bot + (1L << 8);
            if (yll != 0)
                y = (((y) < (yll->start.y)) ? (y) : (yll->start.y));
            for (alp = ll->x_head.next; alp != 0; alp = alp->next) {
                fixed yy = (((alp->fi.y3) > (alp->fi.y0)) ? (alp->fi.y3) : (alp->fi.y0));

                yy = (((yy) > (alp->end.y)) ? (yy) : (alp->end.y));
                y = (((y) < (yy)) ? (y) : (yy));
            }
        }
        ...
        for (alp = ll->x_head.next; alp != 0; alp = nlp) {
          ...
          e:if (alp->end.y <= y || alp->start.y == alp->end.y) {
            ...
            nx = alp->x_current = (alp->start.y >= y ? alp->start.x : ((y) == (alp)->end.y ? (alp)->end.x : ((y) <= (alp)->y_fast_max ? ((((y) - (alp)->start.y) * (alp)->diff.x) + (alp)->num_adjust) / (alp)->diff.y : (((void)(0)), fixed_mult_quo((alp)->diff.x, (y) - (alp)->start.y, (alp)->diff.y))) + (alp)->start.x));
            if (nx < x) {
        ...
        if (y > y_top || y >= y_limit) {
            ...
            for (pcr = rlist.first.next; pcr != &rlist.last;
                ...
                do { } while (0);
                code = ( 1 ? (&fo)->fill_rect((&fo)->dev, x0, y0, x1 - x0, 1, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, x0, y0, x1 - x0, 1, (&fo)->dev, (&fo)->lop, ((void*)0)));
                do { } while (0);
                if (code < 0)
                    goto done;
            }
            ...
            y0 = ((int)((y) >> (8)));
            if (((int)(y)&((1<<8)-1)) < y_frac_min)
                --y0;
            y_bot = ((fixed)(y0)<<8) + y_frac_min;
            y_top = ((fixed)(y0)<<8) + y_frac_max;
            new_band = ((bool)1);
        } else
            new_band = ((bool)0);
        ...
        if (y <= y_top) {
            ...
            if (new_band) {
                ...
                do { } while (0);
                for (alp = ll->x_head.next; alp != 0; alp = alp->next) {
                    int x0 = ((int)((((alp->x_current - fo.adjust_left)+(((1<<8)>>1) - 1L))) >> (8)));
                    ...
                    code = range_list_add(&rlist, x0,
                                          ((int)(((alp->x_current + fo.adjust_right)+((1<<8)>>1)) >> (8))));
            ...
            for (alp = ll->h_list0; alp != ((void*)0); alp = alp->next) {
                fixed x0 = (((alp->start.x) < (alp->end.x)) ? (alp->start.x) : (alp->end.x));
                fixed x1 = (((alp->start.x) > (alp->end.x)) ? (alp->start.x) : (alp->end.x));

                code = range_list_add(&rlist, ((int)(((x0 - fo.adjust_left)+((1<<8)>>1)) >> (8))),
                                              ((int)(((x1 + fo.adjust_right)+((1<<8)>>1)) >> (8))));
                if (code < 0)
                    goto done;
            }
...
# 28 "./base/gxfillsl.h"
static int
spot_into_scan_lines_nd (line_list *ll, fixed band_mask)
{
    const fill_options fo = *ll->fo;
    active_line *yll = ll->y_list;
    fixed y_limit = fo.ymax;
    ...
    do {
        ...
        if (y > y_top || y >= y_limit) {
            ...
            for (pcr = rlist.first.next; pcr != &rlist.last;
                ...
                do { } while (0);
                code = ( 0 ? (&fo)->fill_rect((&fo)->dev, x0, y0, x1 - x0, 1, (&fo)->pdevc->colors.pure) : (*((&fo)->pdevc)->type->fill_rectangle)((&fo)->pdevc, x0, y0, x1 - x0, 1, (&fo)->dev, (&fo)->lop, ((void*)0)));
                do { } while (0);
                if (code < 0)
                    goto done;
            }
