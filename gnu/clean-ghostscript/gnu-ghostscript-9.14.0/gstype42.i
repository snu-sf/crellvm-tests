...
# 41 "./base/gstype42.c" 2


static const gc_ptr_element_t font_type42_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((gs_font_type42 *)0)->data.proc_data - (char *)((gs_font_type42 *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_font_type42 *)0)->data.len_glyphs - (char *)((gs_font_type42 *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_font_type42 *)0)->data.gdcache - (char *)((gs_font_type42 *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((gs_font_type42 *)0)->data.gsub - (char *)((gs_font_type42 *)0) )) } }; static const gc_struct_data_t font_type42_reloc_ptrs = { (sizeof(font_type42_enum_ptrs) / sizeof((font_type42_enum_ptrs)[0])), 0, &st_gs_font_base, font_type42_enum_ptrs }; const gs_memory_struct_type_t st_gs_font_type42 = { sizeof(gs_font_type42), "gs_font_type42", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, gs_font_finalize, &font_type42_reloc_ptrs };
...
static int default_get_outline(gs_font_type42 *pfont, uint glyph_index,
                                gs_glyph_data_t *pgd);
int gs_type42_font_info(gs_font *font, const gs_point *pscale, int members, gs_font_info_t *info);
int gs_truetype_font_info(gs_font *font, const gs_point *pscale, int members, gs_font_info_t *info);
# 64 "./base/gstype42.c"
int gs_len_glyphs_release(void *proc_data, void *event_data);
int gs_gsub_release(void *proc_data, void *event_data);
...
get_glyph_offset(gs_font_type42 *pfont, uint glyph_index)
{
    ...
    if (pfont->data.indexToLocFormat) {
        ...
        result = (code < 0 ? 0xffffffff : get_u32_msb(buf));
    } else {
        code = gs_type42_read_data(pfont, pfont->data.loca + glyph_index * 2, 2, buf);
        result = (code < 0 ? 0xffffffff : (ulong) (((uint)((buf)[0]) << 8) + (buf)[1]) << 1);
    }
    return result;
}
...
gs_type42_font_init(gs_font_type42 * pfont, int subfontID)
{
    ...
    if (!memcmp(OffsetTable, version_ttcf, 4)) {
        version = get_u32_msb(OffsetTable + 4);
        if (version != 0x00010000 && version !=0x00020000) {
            (emprintf_program_ident(pfont->memory, gs_program_name(), gs_revision_number()), errprintf(pfont->memory, "Unknown TTC header version %08X in the font %s.\n", version, pfont->key_name.chars));


            return ((-10));
        }
        numFonts = get_u32_msb(OffsetTable + 8);
        if (subfontID < 0 || subfontID >= numFonts)
            return ((-15));
        do { if (4 > sizeof(OffsetTable)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(12 + subfontID * 4), 4, OffsetTable); if ( code < 0 ) return code; } while (0);
        OffsetTableOffset = get_u32_msb(OffsetTable);
        do { if (12 > sizeof(OffsetTable)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(OffsetTableOffset), 12, OffsetTable); if ( code < 0 ) return code; } while (0);
    } else {
        OffsetTableOffset = 0;
    }
    ...
    if (numTables > 40)
        return ((-10));
    do { if (numTables * 16 > sizeof(TableDirectory)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(OffsetTableOffset + 12), numTables * 16, TableDirectory); if ( code < 0 ) return code; } while (0);
    ...
    for (i = 0; i < numTables; ++i) {
        ...
        if (!memcmp(tab, "cmap", 4))
            ...
            glyph_size = (uint)get_u32_msb(tab + 12);
        } else if (!memcmp(tab, "GSUB", 4)) {
            pfont->data.gsub_size = get_u32_msb(tab + 12);
            pfont->data.gsub = (*(pfont->memory)->procs.alloc_byte_array)(pfont->memory, pfont->data.gsub_size, 1, "gs_type42_font_init(GSUB)");
            ...
            do { if (54 > sizeof(head)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset), 54, head); if ( code < 0 ) return code; } while (0);
            ...
            do { if (36 > sizeof(hhea)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset), 36, hhea); if ( code < 0 ) return code; } while (0);
            pfont->data.metrics[0].numMetrics = (((uint)((hhea + 34)[0]) << 8) + (hhea + 34)[1]);
        } else if (!memcmp(tab, "hmtx", 4)) {
            pfont->data.metrics[0].offset = offset;
            pfont->data.metrics[0].length = (uint)get_u32_msb(tab + 12);
            ...
            do { if (30 > sizeof(maxp)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset), 30, maxp); if ( code < 0 ) return code; } while (0);
            pfont->data.trueNumGlyphs = (((uint)((maxp + 4)[0]) << 8) + (maxp + 4)[1]);
            pfont->data.maxPoints = (((uint)((maxp + 6)[0]) << 8) + (maxp + 6)[1]);
            pfont->data.maxContours = (((uint)((maxp + 8)[0]) << 8) + (maxp + 8)[1]);
            pfont->data.maxCPoints = (((uint)((maxp + 12)[0]) << 8) + (maxp + 12)[1]);
            pfont->data.maxCContours = (((uint)((maxp + 16)[0]) << 8) + (maxp + 16)[1]);
        } else if (!memcmp(tab, "name", 4)) {
            pfont->data.name_offset = offset;
        } else if (!memcmp(tab, "vhea", 4)) {
            byte vhea[36];

            do { if (36 > sizeof(vhea)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset), 36, vhea); if ( code < 0 ) return code; } while (0);
            pfont->data.metrics[1].numMetrics = (((uint)((vhea + 34)[0]) << 8) + (vhea + 34)[1]);
        } else if (!memcmp(tab, "vmtx", 4)) {
            pfont->data.metrics[1].offset = offset;
            pfont->data.metrics[1].length = (uint)get_u32_msb(tab + 12);
...
# 274 "./base/gstype42.c"
        ...
        if (pfont->key_name.size) {
            int l = (((sizeof(buf) - 1) < (pfont->key_name.size)) ? (sizeof(buf) - 1) : (pfont->key_name.size));

            memcpy(buf, pfont->key_name.chars, l);
            buf[l] = 0;
        } else if (pfont->font_name.size) {
            int l = (((sizeof(buf) - 1) < (pfont->font_name.size)) ? (sizeof(buf) - 1) : (pfont->font_name.size));
        ...
        (emprintf_program_ident(pfont->memory, gs_program_name(), gs_revision_number()), errprintf(pfont->memory, "Warning: 'loca' length %d is greater than numGlyphs %d in the font %s.\n", pfont->data.numGlyphs + 1, pfont->data.trueNumGlyphs, buf));
    ...
    pfont->data.warning_patented = ((bool)0);
    pfont->data.warning_bad_instruction = ((bool)0);
...
# 334 "./base/gstype42.c"
        pfont->data.len_glyphs = ((void*)0);
    } else {

        pfont->data.len_glyphs = (uint *)(*(pfont->memory)->procs.alloc_byte_array)(pfont->memory, loca_size, sizeof(uint), "gs_type42_font_init");
        ...
        if (i < loca_size) {
            ...
            gs_type42_font_init_sort_t *psortary =
                (gs_type42_font_init_sort_t *)(*(pfont->memory)->procs.alloc_byte_array)(pfont->memory, loca_size, sizeof(gs_type42_font_init_sort_t), "gs_type42_font_init(sort loca)");
            ...
            for (i = num_valid_loca_elm; i--;) {
                ...
                if (old_length < 0 || old_length > 2000 ) {
            ...
            ((pfont->memory)->procs.free_object(pfont->memory, psortary, "gs_type42_font_init(sort loca)"));
        }
    }
...
gs_len_glyphs_release(void *data, void *event)
{
    ...
    ((pfont->memory)->procs.free_object(pfont->memory, pfont->data.len_glyphs, "gs_len_glyphs_release"));
    return 0;
}
...
gs_gsub_release(void *data, void *event)
{
    ...
    ((pfont->memory)->procs.free_object(pfont->memory, pfont->data.gsub, "gs_gsub_release"));
    return 0;
}
# 472 "./base/gstype42.c"
void
                          ...
                          const gs_matrix_fixed *pmat)
{
    ...
    if (flags & (1<<1)) {
        ...
        if (flags & (1<<0))
            arg1 = (int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000), arg2 = (int)(((((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]) ^ 0x8000) - 0x8000), gdata += 4;
        else
            arg1 = (int)((((uint)((gdata)[0])) ^ 0x80) - 0x80), arg2 = (int)((((uint)((gdata + 1)[0])) ^ 0x80) - 0x80), gdata += 2;
        if (flags & (1<<2)) {
        ...
        mat.tx = ((mat.tx_fixed = pt.x)*(1.0/(1<<8)));
        mat.ty = ((mat.ty_fixed = pt.y)*(1.0/(1<<8)));
        if (pmp)
            pmp[0] = pmp[1] = -1;
    } else {
        if (flags & (1<<0)) {
            if (pmp)
                pmp[0] = (((uint)((gdata)[0]) << 8) + (gdata)[1]), pmp[1] = (int)(((((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]) ^ 0x8000) - 0x8000);
            gdata += 4;
        } else {
            if (pmp)
                pmp[0] = ((uint)((gdata)[0])), pmp[1] = ((uint)((gdata + 1)[0]));
            gdata += 2;
        }
    }

    if (flags & (1<<3)) {
        scale_mat.xx = scale_mat.yy = ((int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.xy = scale_mat.yx = 0;
        gdata += 2;
    } else if (flags & (1<<6)) {
        scale_mat.xx = ((int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.yy = ((int)(((((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.xy = scale_mat.yx = 0;
        gdata += 4;
    } else if (flags & (1<<7)) {
        scale_mat.xx = ((int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.xy = ((int)(((((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.yx = ((int)(((((uint)((gdata + 4)[0]) << 8) + (gdata + 4)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        scale_mat.yy = ((int)(((((uint)((gdata + 6)[0]) << 8) + (gdata + 6)[1]) ^ 0x8000) - 0x8000) / 16384.0);
        gdata += 8;
    } else
        goto no_scale;
...
total_points(gs_font_type42 *pfont, uint glyph_index)
{
    ...
    if ((int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000) != -1) {

        int numContours = (int)(((((uint)((gdata)[0]) << 8) + (gdata)[1]) ^ 0x8000) - 0x8000);
        const byte *pends = gdata + 10;
        const byte *pinstr = pends + numContours * 2;

        total = (numContours == 0 ? 0 : (((uint)((pinstr - 2)[0]) << 8) + (pinstr - 2)[1]) + 1);
    } else {
        ...
        total = 0;
        do {
            code = total_points(pfont, (((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]));
            if (code < 0)
                return code;
            total += code;
            gs_type42_parse_component(&gdata, &flags, &mat, ((void*)0), pfont, &mat);
        }
        while (flags & (1<<5));
    }
...
{
    return (glyph < ((gs_glyph)0x80000000L) ? 0 :
            glyph - ((gs_glyph)0x80000000L));
}
                    ...
                    gs_glyph_data_t *pgd)
{
    ...
    if (glyph_length == 0)
        ...
        if (code < 0)
            return code;
        if (code == 0)
            gs_glyph_data_from_string(pgd, data, glyph_length, ((void*)0));
        else {
# 641 "./base/gstype42.c"
            buf = (byte *)(*(pgd->memory)->procs.alloc_string)(pgd->memory, glyph_length, "default_get_outline");
            if (buf == 0)
                return ((-25));
                ...
                gs_glyph_data_t *pgd)
{
    ...
    if (pfont->data.indexToLocFormat) {
        spseek(s, (gs_offset_t)(pfont->data.loca + glyph_index * 4));
        sgets(s, lbuf, 8, &count);
        if (count < 8)
            return ((-10));
        glyph_start = get_u32_msb(lbuf);
        glyph_length = get_u32_msb(lbuf + 4) - glyph_start;
    } else {
        spseek(s, (gs_offset_t)(pfont->data.loca + glyph_index * 2));
        sgets(s, lbuf, 4, &count);
        if (count < 4)
            return ((-10));
        glyph_start = (ulong) (((uint)((lbuf)[0]) << 8) + (lbuf)[1]) << 1;
        glyph_length = ((ulong) (((uint)((lbuf + 2)[0]) << 8) + (lbuf + 2)[1]) << 1) - glyph_start;
    }
    if (glyph_length == 0)
        gs_glyph_data_from_null(pgd);
    else {
        ...
        spseek(s, (gs_offset_t)(pfont->data.glyf + glyph_start));
        buf = (byte *)(*(pgd->memory)->procs.alloc_string)(pgd->memory, glyph_length, "default_get_outline");
        if (buf == 0)
            return ((-25));
        gs_glyph_data_from_string(pgd, buf, glyph_length, (gs_font *)pfont);
...
# 743 "./base/gstype42.c"
    ...
    gsub.Version = get_u32_msb(gsub_ptr + ((int) ( (char *)&((GSUB *)0)->Version - (char *)((GSUB *)0) )));
    gsub.ScriptList = (((uint)((gsub_ptr + ((int) ( (char *)&((GSUB *)0)->ScriptList - (char *)((GSUB *)0) )))[0]) << 8) + (gsub_ptr + ((int) ( (char *)&((GSUB *)0)->ScriptList - (char *)((GSUB *)0) )))[1]);
    gsub.FeatureList = (((uint)((gsub_ptr + ((int) ( (char *)&((GSUB *)0)->FeatureList - (char *)((GSUB *)0) )))[0]) << 8) + (gsub_ptr + ((int) ( (char *)&((GSUB *)0)->FeatureList - (char *)((GSUB *)0) )))[1]);
    gsub.LookupList = (((uint)((gsub_ptr + ((int) ( (char *)&((GSUB *)0)->LookupList - (char *)((GSUB *)0) )))[0]) << 8) + (gsub_ptr + ((int) ( (char *)&((GSUB *)0)->LookupList - (char *)((GSUB *)0) )))[1]);

    lookup_list_ptr = gsub_ptr + gsub.LookupList;
    lookup_list_table.LookupCount = (((uint)((lookup_list_ptr + ((int) ( (char *)&((LookupListTable *)0)->LookupCount - (char *)((LookupListTable *)0) )))[0]) << 8) + (lookup_list_ptr + ((int) ( (char *)&((LookupListTable *)0)->LookupCount - (char *)((LookupListTable *)0) )))[1]);
    for (i = 0; i < lookup_list_table.LookupCount; i++) {
        byte *lookup_table_offset_ptr = lookup_list_ptr + ((int) ( (char *)&((LookupListTable *)0)->Lookup - (char *)((LookupListTable *)0) ))
                               + i * sizeof(uint16_t);
        byte *lookup_table_ptr = lookup_list_ptr + (((uint)((lookup_table_offset_ptr)[0]) << 8) + (lookup_table_offset_ptr)[1]);
        LookupTable lookup_table;

        lookup_table.LookupType = (((uint)((lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->LookupType - (char *)((LookupTable *)0) )))[0]) << 8) + (lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->LookupType - (char *)((LookupTable *)0) )))[1]);
        lookup_table.LookupFlag = (((uint)((lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->LookupFlag - (char *)((LookupTable *)0) )))[0]) << 8) + (lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->LookupFlag - (char *)((LookupTable *)0) )))[1]);
        lookup_table.SubTableCount = (((uint)((lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->SubTableCount - (char *)((LookupTable *)0) )))[0]) << 8) + (lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->SubTableCount - (char *)((LookupTable *)0) )))[1]);
        ...
        if (lookup_table.LookupType == 1) {
            ...
            byte *subtable_offset_ptr = lookup_table_ptr + ((int) ( (char *)&((LookupTable *)0)->SubTable - (char *)((LookupTable *)0) ));
            for (j = 0; j < lookup_table.SubTableCount; j++) {
                byte *subtable_ptr = lookup_table_ptr + (((uint)((subtable_offset_ptr + j * sizeof(uint16_t))[0]) << 8) + (subtable_offset_ptr + j * sizeof(uint16_t))[1]);
                uint16_t format = (((uint)((subtable_ptr)[0]) << 8) + (subtable_ptr)[1]);

                if (format == 1) {
                    SingleSubstFormat1 subst;

                    subst.SubstFormat = format;
                    subst.Coverage = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat1 *)0)->Coverage - (char *)((SingleSubstFormat1 *)0) )))[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat1 *)0)->Coverage - (char *)((SingleSubstFormat1 *)0) )))[1]);
                    subst.DeltaGlyphId = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat1 *)0)->DeltaGlyphId - (char *)((SingleSubstFormat1 *)0) )))[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat1 *)0)->DeltaGlyphId - (char *)((SingleSubstFormat1 *)0) )))[1]);
                } else {
                    ...
                    subst.SubstFormat = format;
                    subst.Coverage = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Coverage - (char *)((SingleSubstFormat2 *)0) )))[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Coverage - (char *)((SingleSubstFormat2 *)0) )))[1]);
                    subst.GlyphCount = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->GlyphCount - (char *)((SingleSubstFormat2 *)0) )))[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->GlyphCount - (char *)((SingleSubstFormat2 *)0) )))[1]);
                    coverage_ptr = subtable_ptr + subst.Coverage;
                    coverage_format = (((uint)((coverage_ptr)[0]) << 8) + (coverage_ptr)[1]);
                    if (coverage_format == 1) {
                        CoverageFormat1 cov;

                        cov.CoverageFormat = coverage_format;
                        cov.GlyphCount = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat1 *)0)->GlyphCount - (char *)((CoverageFormat1 *)0) )))[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat1 *)0)->GlyphCount - (char *)((CoverageFormat1 *)0) )))[1]);
                        {
                            int k0 = 0, k1 = cov.GlyphCount;

                            for (;;) {
                                int k = (k0 + k1) / 2;
                                GlyphID glyph = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat1 *)0)->GlyphArray - (char *)((CoverageFormat1 *)0) )) + sizeof(GlyphID) * k)[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat1 *)0)->GlyphArray - (char *)((CoverageFormat1 *)0) )) + sizeof(GlyphID) * k)[1]);
                                ...
                                if (glyph_index == glyph) {
                                    ...
                                    if (k >= cov.GlyphCount)
                                        ...
                                        GlyphID new_glyph = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Substitute - (char *)((SingleSubstFormat2 *)0) )) + sizeof(GlyphID) * k)[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Substitute - (char *)((SingleSubstFormat2 *)0) )) + sizeof(GlyphID) * k)[1]);
                        ...
                        cov.CoverageFormat = coverage_format;
                        cov.RangeCount = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeCount - (char *)((CoverageFormat2 *)0) )))[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeCount - (char *)((CoverageFormat2 *)0) )))[1]);
                        {
                            ...
                            for (;;) {
                                ...
                                rr.Start = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->Start - (char *)((RangeRecord *)0) )))[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->Start - (char *)((RangeRecord *)0) )))[1]);

                                rr.End = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->End - (char *)((RangeRecord *)0) )))[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->End - (char *)((RangeRecord *)0) )))[1]);

                                rr.StartCoverageIndex = (((uint)((coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->StartCoverageIndex - (char *)((RangeRecord *)0) )))[0]) << 8) + (coverage_ptr + ((int) ( (char *)&((CoverageFormat2 *)0)->RangeArray - (char *)((CoverageFormat2 *)0) )) + sizeof(RangeRecord) * k + ((int) ( (char *)&((RangeRecord *)0)->StartCoverageIndex - (char *)((RangeRecord *)0) )))[1]);
                                ...
                                if (rr.Start <= glyph_index && glyph_index <= rr.End) {
                                    ...
                                    if (k >= cov.RangeCount)
                                        ...
                                        GlyphID new_glyph = (((uint)((subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Substitute - (char *)((SingleSubstFormat2 *)0) )) + sizeof(GlyphID) * subst_index)[0]) << 8) + (subtable_ptr + ((int) ( (char *)&((SingleSubstFormat2 *)0)->Substitute - (char *)((SingleSubstFormat2 *)0) )) + sizeof(GlyphID) * subst_index)[1]);
...
parse_pieces(gs_font_type42 *pfont, gs_glyph glyph, gs_glyph *pieces,
             int *pnum_pieces)
{
    uint glyph_index = (glyph >= (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1))
    ...
    if (code < 0)
        return code;
    if (glyph_data.bits.size != 0 && (int)(((((uint)((glyph_data.bits.data)[0]) << 8) + (glyph_data.bits.data)[1]) ^ 0x8000) - 0x8000) == -1) {
        ...
        for (i = 0; flags & (1<<5); ++i) {
            if (pieces)
                pieces[i] = (((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]) + (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
            gs_type42_parse_component(&gdata, &flags, &mat, ((void*)0), pfont, &mat);
        }
        *pnum_pieces = i;
    } else
                        ...
                        gx_path *ppath, double sbw[4])
{
    ...
    static const gs_matrix imat = { (float)(1), (float)(0), (float)(0), (float)(1), (float)(0), (float)(0) };
    bool design_grid = ((bool)1);
    ...
    if (glyph >= (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)))
        glyph_index = glyph - (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
    else {
        glyph_index = pfont->data.get_glyph_index(pfont, glyph);
        if (pfont->data.gsub_size) {
            if (pfont->data.substitute_glyph_index_vertical != ((void*)0))
    ...
    if ((code = gx_path_current_point(ppath, &origin)) < 0 ||
        ...
        return code;
    return gx_path_add_point(ppath, origin.x + ((fixed)((info.width[WMode].x)*(float)(1<<8))),
                             origin.y + ((fixed)((info.width[WMode].y)*(float)(1<<8))));
}
                     ...
                     int members, gs_glyph_info_t *info, uint glyph_index)
{
    ...
    if (members & (8 | 16)) {
        gs_glyph *pieces =
            (members & 16 ? info->pieces : (gs_glyph *)0);
        int code = parse_pieces(pfont, glyph, pieces, &info->num_pieces);

        if (code < 0)
            return code;
        info->members |= members & (8 | 16);
    }
    return code;
}
int
                     ...
                     int members, gs_glyph_info_t *info)
{
    ...
    if (glyph >= (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1)))
        ...
        glyph_index = pfont->data.get_glyph_index(pfont, glyph);
        if (glyph_index == ((gs_glyph)0x7fffffff))
            return ((-21));
        if (pfont->data.gsub_size) {
            int WMode = ((members & (2 | 128)) ? 1 : 0);
                          ...
                          gs_glyph_space_t glyph_space, gs_glyph *pglyph)
{
    ...
    while (++*pindex <= pfont->data.numGlyphs) {
        ...
        if (outline.bits.data == 0)
            continue;
        *pglyph = glyph_index + (((gs_glyph)0x80000000L) | (((gs_glyph)0x80000000L) >> 1));
                     ...
                     float sbw[4])
{
...

    {
        ...
        if (glyph_index < num_metrics) {
            do { if (4 > sizeof(pmetrics)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(pmtx->offset + glyph_index * 4), 4, pmetrics); if ( code < 0 ) return code; } while (0);
            width = (((uint)((pmetrics)[0]) << 8) + (pmetrics)[1]);
            lsb = (int)(((((uint)((pmetrics + 2)[0]) << 8) + (pmetrics + 2)[1]) ^ 0x8000) - 0x8000);
        } else {
            uint offset = pmtx->offset + num_metrics * 4;
            uint glyph_offset = (glyph_index - num_metrics) * 2;
            byte plsb[2];

            do { if (4 > sizeof(pmetrics)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset - 4), 4, pmetrics); if ( code < 0 ) return code; } while (0);
            width = (((uint)((pmetrics)[0]) << 8) + (pmetrics)[1]);
            if (glyph_offset >= pmtx->length)
                glyph_offset = pmtx->length - 2;
            do { if (2 > sizeof(plsb)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(offset + glyph_offset), 2, plsb); if ( code < 0 ) return code; } while (0);
            lsb = (int)(((((uint)((plsb)[0]) << 8) + (plsb)[1]) ^ 0x8000) - 0x8000);
        }
    }
    if (wmode) {
        factor = -factor;
                              ...
                              gs_type42_metrics_options_t options, float sbw[4])
{
    ...
    int sbw_requested = (~(options)&gs_type42_metrics_options_BBOX);
    int bbox_requested = ((options)&6);
    ...
    if (bbox_requested) {
        if (glyph_data.bits.size >= 10 && bbox_requested) {
            ...
            sbw[4] = (int)(((((uint)((glyph_data.bits.data + 2)[0]) << 8) + (glyph_data.bits.data + 2)[1]) ^ 0x8000) - 0x8000) * factor;
            sbw[5] = (int)(((((uint)((glyph_data.bits.data + 4)[0]) << 8) + (glyph_data.bits.data + 4)[1]) ^ 0x8000) - 0x8000) * factor;
            sbw[6] = (int)(((((uint)((glyph_data.bits.data + 6)[0]) << 8) + (glyph_data.bits.data + 6)[1]) ^ 0x8000) - 0x8000) * factor;
            sbw[7] = (int)(((((uint)((glyph_data.bits.data + 8)[0]) << 8) + (glyph_data.bits.data + 8)[1]) ^ 0x8000) - 0x8000) * factor;
        } else
    ...
    if (sbw_requested) {
        if (glyph_data.bits.size != 0 && (int)(((((uint)((glyph_data.bits.data)[0]) << 8) + (glyph_data.bits.data)[1]) ^ 0x8000) - 0x8000) == -1) {
            ...
            memset(&mat, 0, sizeof(mat));
            do {
                uint comp_index = (((uint)((gdata + 2)[0]) << 8) + (gdata + 2)[1]);
                 ...
                 bool charpath_flag)
{
    ...
    int code = append_outline_fitted(glyph_index, &(*(const gs_matrix *)&(pgs)->ctm), ppath,
               ...
               const gs_log2_scale_point * pscale, bool design_grid)
{
    ...
    gx_ttfReader__set_font(pair->ttr, ((void*)0));
    return code;
}
                           ...
                           gs_const_string *pmember, int member, uint name_id)
{
    ...
    do { if (4 > sizeof(t)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(pfont->data.name_offset + 2), 4, t); if ( code < 0 ) return code; } while (0);
    num_records = (((uint)((t)[0]) << 8) + (t)[1]);
    strings_offset = (((uint)((t + 2)[0]) << 8) + (t + 2)[1]);
    for (i = 0; i < num_records; i++) {
        ushort platformID, specificID, languageID, nameID, length, offset;

        do { if (12 > sizeof(t)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(pfont->data.name_offset + 6 + i * 12), 12, t); if ( code < 0 ) return code; } while (0);
        platformID = (((uint)((t + 0)[0]) << 8) + (t + 0)[1]);
        specificID = (((uint)((t + 2)[0]) << 8) + (t + 2)[1]);
        languageID = (((uint)((t + 4)[0]) << 8) + (t + 4)[1]);
        nameID = (((uint)((t + 6)[0]) << 8) + (t + 6)[1]);
        length = (((uint)((t + 8)[0]) << 8) + (t + 8)[1]);
        offset = (((uint)((t + 10)[0]) << 8) + (t + 10)[1]);
        if (nameID == name_id) {
            ((void)(platformID));
            ((void)(specificID));
           ...
           gs_font_info_t *info)
{
    ...
    if (!(info->members & 0x4000) && (members & 0x4000)) {
        if(pfont->data.os2_offset != 0) {
            unsigned char fstype[2];

            do { if (2 > sizeof(fstype)) return ((-28)); code = gs_type42_read_data(pfont, (ulong)(pfont->data.os2_offset + 8), 2, fstype); if ( code < 0 ) return code; } while (0);
            info->EmbeddingRights = (((uint)((fstype)[0]) << 8) + (fstype)[1]);
            info->members |= 0x4000;
        }
    }
    if (pfont->data.name_offset == 0)
        return 0;
    if (!(info->members & 0x0040) && (members & 0x0040)) {
        code = get_from_names_table(pfont, info, &info->Copyright, 0x0040, 0);
        if (code < 0)
            return code;
    }
    if (!(info->members & 0x1000) && (members & 0x1000)) {
        code = get_from_names_table(pfont, info, &info->FamilyName, 0x1000, 1);
        if (code < 0)
            return code;
    }
    if (!(info->members & 0x2000) && (members & 0x2000)) {
        code = get_from_names_table(pfont, info, &info->FullName, 0x2000, 4);
        if (code < 0)
            return code;
    }
