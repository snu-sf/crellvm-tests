# 1 "./devices/vector/gdevpdft.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpdft.c" 2
# 18 "./devices/vector/gdevpdft.c"
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
...
# 19 "./devices/vector/gdevpdft.c" 2
# 1 "./base/string_.h" 1
# 32 "./base/string_.h"
...
# 33 "./base/string_.h" 2
# 20 "./devices/vector/gdevpdft.c" 2
...
# 22 "./devices/vector/gdevpdft.c" 2
# 1 "./base/gscolor2.h" 1
# 23 "./base/gscolor2.h"
# 1 "./base/gscindex.h" 1
# 48 "./base/gscindex.h"
...
# 23 "./devices/vector/gdevpdft.c" 2
# 1 "./base/gzstate.h" 1
# 25 "./base/gzstate.h"
# 1 "./base/gxdcolor.h" 1
# 43 "./base/gxdcolor.h"
...
# 24 "./devices/vector/gdevpdft.c" 2
...
# 25 "./devices/vector/gdevpdft.c" 2
...
# 26 "./devices/vector/gdevpdft.c" 2
...
# 27 "./devices/vector/gdevpdft.c" 2
...
pdf_make_soft_mask_dict(gx_device_pdf * pdev, const gs_pdf14trans_params_t * pparams)
{
    ...
    code = pdf_alloc_resource(pdev, resourceSoftMaskDict, 0L, &pres_soft_mask_dict, -1);
    if (code < 0)
        return code;
    cos_become(pres_soft_mask_dict->object, (&cos_dict_procs));
    ...
    if (pparams->transfer_function != ((void*)0)) {
        long id;
        char buf[20];
                            ...
                            const gs_imager_state * pis, cos_dict_t **pdict)
{
    ...
    code = pdf_alloc_resource(pdev, resourceGroup, 0L, &pres_group, -1);
    if (code < 0)
        return code;
    cos_become(pres_group->object, (&cos_dict_procs));
    ...
    if (code < 0)
        return code;
    if (pparams->Isolated) {
        code = cos_dict_put_c_key_bool(group_dict, "/I", ((bool)1));
        if (code < 0)
            return code;
    }
    if (pparams->Knockout) {
        code = cos_dict_put_c_key_bool(group_dict, "/K", ((bool)1));
        if (code < 0)
            return code;
    }
    ...
    if (gstate != ((void*)0) && pparams->group_color != UNKNOWN) {
        const gs_color_space *cs = ((gstate)->color[0].color_space);

        code = pdf_color_space_named(pdev, &cs_value, ((void*)0), cs,
                &pdf_color_space_names, ((bool)0), ((void*)0), 0, ((bool)0));
        if (code < 0)
            return code;
    ...
    group_dict = ((void*)0);
    code = pdf_substitute_resource(pdev, &pres_group, resourceGroup, ((void*)0), ((bool)0));
    if (code < 0)
        return code;
                            ...
                            const cos_dict_t *group_dict, cos_dict_t *form_dict)
{
    ...
    code = gs_bbox_transform(&pparams->bbox, &(*(const gs_matrix *)&(pis)->ctm), &bbox_rect);
    if (code < 0)
        return code;
...
{
    cos_dict_t *group_dict;
    bool in_page = ((pdev)->contents_id != 0);
    const gs_state *gstate = gx_hld_get_gstate_ptr(pis);
    int code;
    ...
    if (!in_page)
        ...
        pdf_resource_t *pres, *pres_gstate = ((void*)0);
        ...
        code = pdf_enter_substream(pdev, resourceXObject,
                0L, &pres, ((bool)0), pdev->params.CompressPages);
        if (code < 0)
            return code;
        pdev->FormDepth++;
...
pdf_end_transparency_group(gs_imager_state * pis, gx_device_pdf * pdev)
{
    ...
    if (pdev->image_with_SMask & (1 << pdev->FormDepth)) {
        ...
        (!((pdev->strm)->cursor.w.ptr >= (pdev->strm)->cursor.w.limit) ? (++((pdev->strm)->cursor.w.ptr), *(pdev->strm)->cursor.w.ptr=('/'), 0) : spputc((pdev->strm),('/')));
...
# 299 "./devices/vector/gdevpdft.c"
        pdev->image_mask_skip = ((bool)1);
        return 0;
    } else {
        int code;
                                ...
                                const gs_pdf14trans_params_t * pparams)
{
    if (pdev->image_mask_skip)
        ...
        code = pdf_substitute_resource(pdev, &pdev->pres_soft_mask_dict,
                                        resourceSoftMaskDict, ((void*)0), ((bool)0));
        if (code < 0)
            return code;
