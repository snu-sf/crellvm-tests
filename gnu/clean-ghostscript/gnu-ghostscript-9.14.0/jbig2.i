      ...
      const char *fmt, ...)
{
  ...
  __builtin_va_end(ap);
  if (n < 0 || n == sizeof(buf))
    __builtin_strncpy (buf, "jbig2_error: error in generating error string", sizeof(buf));
        ...
        void *error_callback_data)
{
  ...
  result->segments = ((Jbig2Segment* *)jbig2_alloc(result->allocator, result->n_segments_max, sizeof(Jbig2Segment*)));
  if (result->segments == ((void*)0)) {
  ...
  result->pages = ((Jbig2Page *)jbig2_alloc(result->allocator, result->max_page_index, sizeof(Jbig2Page)));
  ...
  }
  {
    ...
    for (index = 0; index < result->max_page_index; index++) {
        ...
        result->pages[index].image = ((void*)0);
    }
  }
...
{
    return (((int)(((bptr)[0] << 8) | (bptr)[1]) ^ 0x8000) - 0x8000);
}
...
{
    return ((int32_t)(((int)(((bptr)[0] << 8) | (bptr)[1]) ^ 0x8000) - 0x8000) << 16) | (((bptr + 2)[0] << 8) | (bptr + 2)[1]);
}

uint32_t
jbig2_get_uint32(const byte *bptr)
{
    return ((uint32_t)(((bptr)[0] << 8) | (bptr)[1]) << 16) | (((bptr + 2)[0] << 8) | (bptr + 2)[1]);
}
# 227 "./jbig2dec/jbig2.c"
int
jbig2_data_in (Jbig2Ctx *ctx, const unsigned char *data, size_t size)
{
 ...
 buf_size <<= 1;
      while (buf_size < size);
      ctx->buf = ((byte *)jbig2_alloc(ctx->allocator, buf_size, sizeof(byte)));
      if (ctx->buf == ((void*)0))
      {
      ...
      else
 {
   ...
   buf = ((byte *)jbig2_alloc(ctx->allocator, buf_size, sizeof(byte)));
      if (buf == ((void*)0))
      {
          return jbig2_error(ctx, JBIG2_SEVERITY_FATAL, -1,
 ...
 case JBIG2_FILE_RANDOM_HEADERS:
   ...
   if (ctx->n_segments == ctx->n_segments_max)
              ctx->segments = ((Jbig2Segment* *)jbig2_realloc(ctx->allocator, (ctx->segments), (ctx->n_segments_max <<= 2), sizeof(Jbig2Segment*)));
...
jbig2_word_stream_buf_new(Jbig2Ctx *ctx, const byte *data, size_t size)
{
  Jbig2WordStreamBuf *result = ((Jbig2WordStreamBuf *)jbig2_alloc(ctx->allocator, 1, sizeof(Jbig2WordStreamBuf)));
  if (result == ((void*)0))
  {
