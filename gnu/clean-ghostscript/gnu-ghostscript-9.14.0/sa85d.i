...
static const gs_memory_struct_type_t st_A85D_state = { sizeof(stream_A85D_state), "ASCII85Decode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_A85D_init(stream_state * st)
{
    ...
    do { (ss)->min_left = 1; (ss)->word = 0; (ss)->odd = 0; (ss)->require_eod=((bool)0); } while (0);
    return 0;
}
               ...
               stream_cursor_write * pw, bool last)
{
    ...
    const byte *rlimit = pr->limit - (last ? 0 : 7);
    const byte *r = (((p) > (rlimit)) ? (p) : (rlimit));
    ...
    while (p < rlimit) {
        ...
        if (ccode < 85) {
            ...
            q[1] = q[2] = q[3] = q[4] = 0,
                q += 4;
        } else if ((&scan_char_array[4])[ch] == 102)
            do { } while (0);
        else if (ch == '~') {
            int i = 1;
