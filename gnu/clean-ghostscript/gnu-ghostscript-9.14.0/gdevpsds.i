# 1 "./devices/vector/gdevpsds.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpsds.c" 2
# 18 "./devices/vector/gdevpsds.c"
# 1 "./base/gx.h" 1
# 22 "./base/gx.h"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
...
# 19 "./devices/vector/gdevpsds.c" 2
# 1 "./base/memory_.h" 1
# 54 "./base/memory_.h"
...
# 55 "./base/memory_.h" 2
# 20 "./devices/vector/gdevpsds.c" 2
...
void color_cmyk_to_rgb(frac c, frac m, frac y, frac k,
                       ...
                       gs_memory_t * mem);
# 22 "./devices/vector/gdevpsds.c" 2
# 1 "./devices/vector/gdevpsds.h" 1
# 22 "./devices/vector/gdevpsds.h"
# 1 "./base/strimpl.h" 1
# 23 "./base/strimpl.h"
# 1 "./base/scommon.h" 1
# 24 "./base/scommon.h"
...
# 23 "./devices/vector/gdevpsds.h" 2
# 1 "./base/gsiparam.h" 1
# 22 "./base/gsiparam.h"
# 1 "./base/gsccolor.h" 1
# 27 "./base/gsccolor.h"
...
# 24 "./devices/vector/gdevpsds.h" 2
...
# 125 "./devices/vector/gdevpsds.h"
typedef struct stream_Downsample_state_s {
    const stream_template *templat; gs_memory_t *memory; int (*report_error)(stream_state *, const char *); int min_left; char error_string[79 + 1]; int Colors; int WidthIn, HeightIn; float XFactor, YFactor; bool AntiAlias; bool padX, padY; int x, y;
...
# 23 "./devices/vector/gdevpsds.c" 2
...
# 25 "./devices/vector/gdevpsds.c" 2
# 1 "./base/gsdcolor.h" 1
# 23 "./base/gsdcolor.h"
# 1 "./base/gxarith.h" 1
# 27 "./base/gxarith.h"
...
# 26 "./devices/vector/gdevpsds.c" 2
...
# 28 "./devices/vector/gdevpsds.c" 2
# 1 "./base/gxistate.h" 1
# 29 "./base/gxistate.h"
# 1 "./base/gxline.h" 1
# 22 "./base/gxline.h"
...
# 29 "./devices/vector/gdevpsds.c" 2



static const gs_memory_struct_type_t st_1248_state = { sizeof(stream_1248_state), "stream_1248_state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_16_init(stream_state * st)
{
    ...
    ss->left = ss->samples_per_row;
    ss->bits_per_sample = 16;
    return 0;
}
# 120 "./devices/vector/gdevpsds.c"
static int
s_N_8_process(stream_state * st, stream_cursor_read * pr,
              stream_cursor_write * pw, bool last)
{
    stream_1248_state * const ss = (stream_1248_state *)st; const byte *p = pr->ptr; const byte *rlimit = pr->limit; byte *q = pw->ptr; byte *wlimit = pw->limit; uint left = ss->left; int status; int n;

    switch (ss->bits_per_sample) {

        case 1:{
                status = 0; for ( ; p < rlimit; left -= n, q += n, ++p ) { byte in = p[1]; n = (((left) < (8)) ? (left) : (8)); if ( wlimit - q < n ) { status = 1; break; } switch ( n ) { case 0: left = ss->samples_per_row; --p; continue;
        case 8:
                q[8] = (byte) - (in & 1);
        case 7:
                q[7] = (byte) - ((in >> 1) & 1);
        ...
        case 2:{
                ...
                status = 0; for ( ; p < rlimit; left -= n, q += n, ++p ) { byte in = p[1]; n = (((left) < (4)) ? (left) : (4)); if ( wlimit - q < n ) { status = 1; break; } switch ( n ) { case 0: left = ss->samples_per_row; --p; continue;
        case 4:
                q[4] = b2[in & 3];
        case 3:
                q[3] = b2[(in >> 2) & 3];
        ...
        case 4:{
                ...
                status = 0; for ( ; p < rlimit; left -= n, q += n, ++p ) { byte in = p[1]; n = (((left) < (2)) ? (left) : (2)); if ( wlimit - q < n ) { status = 1; break; } switch ( n ) { case 0: left = ss->samples_per_row; --p; continue;
        case 2:
                q[2] = b4[in & 0xf];
        case 1:
                q[1] = b4[in >> 4];
    ...
    pr->ptr = p; pw->ptr = q; ss->left = left; return status;
}
              ...
              stream_cursor_write * pw, bool last)
{
    ...
    switch (ss->bits_per_sample) {
        ...
        case 1:{
                byte out; status = 1; for ( ; q < wlimit; left -= n, p += n, ++q ) { n = (((left) < (8)) ? (left) : (8)); if ( rlimit - p < n ) { status = 0; break; } out = 0; switch ( n ) { case 0: left = ss->samples_per_row; --q; continue;
        case 8:
                out = p[8] >> 7;
        case 7:
                out |= (p[7] >> 7) << 1;
        ...
        case 2:{
                byte out; status = 1; for ( ; q < wlimit; left -= n, p += n, ++q ) { n = (((left) < (4)) ? (left) : (4)); if ( rlimit - p < n ) { status = 0; break; } out = 0; switch ( n ) { case 0: left = ss->samples_per_row; --q; continue;
        case 4:
                out |= p[4] >> 6;
        case 3:
                out |= (p[3] >> 6) << 2;
        ...
        case 4:{
                byte out; status = 1; for ( ; q < wlimit; left -= n, p += n, ++q ) { n = (((left) < (2)) ? (left) : (2)); if ( rlimit - p < n ) { status = 0; break; } out = 0; switch ( n ) { case 0: left = ss->samples_per_row; --q; continue;
        case 2:
                out |= p[2] >> 4;
        case 1:
                out |= p[1] & 0xf0;
...
static const gc_ptr_element_t c2r_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_C2R_state *)0)->pis - (char *)((stream_C2R_state *)0) )) } }; static const gc_struct_data_t c2r_reloc_ptrs = { (sizeof(c2r_enum_ptrs) / sizeof((c2r_enum_ptrs)[0])), 0, 0, c2r_enum_ptrs }; static const gs_memory_struct_type_t st_C2R_state = { sizeof(stream_C2R_state), "stream_C2R_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &c2r_reloc_ptrs };
              ...
              stream_cursor_write * pw, bool last)
{
    ...
    for (; rlimit - p >= 4 && wlimit - q >= 3; p += 4, q += 3) {
        ...
        color_cmyk_to_rgb(((frac)( ((frac)(bc) << (15 - (8))) + ((bc) >> ((8) * 2 - 15)) - ((bc) >> ((8) - 3)) )), ((frac)( ((frac)(bm) << (15 - (8))) + ((bm) >> ((8) * 2 - 15)) - ((bm) >> ((8) - 3)) )), ((frac)( ((frac)(by) << (15 - (8))) + ((by) >> ((8) * 2 - 15)) - ((by) >> ((8) - 3)) )),
                          ((frac)( ((frac)(bk) << (15 - (8))) + ((bk) >> ((8) * 2 - 15)) - ((bk) >> ((8) - 3)) )), ss->pis, rgb, ss->pis->memory);
        q[1] = ((byte)((uint)((((rgb[0]) >> (15 - 3)) + (rgb[0])) >> (15 - (8)))));
        q[2] = ((byte)((uint)((((rgb[1]) >> (15 - 3)) + (rgb[1])) >> (15 - (8)))));
        q[3] = ((byte)((uint)((((rgb[2]) >> (15 - 3)) + (rgb[2])) >> (15 - (8)))));
    }
    pr->ptr = p;
    pw->ptr = q;
...
static gs_ptr_type_t ie_state_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void ie_state_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_IE_state = { sizeof(stream_IE_state), "stream_IE_state", 0, 0, ie_state_enum_ptrs, ie_state_reloc_ptrs, 0, 0 };
static
gs_ptr_type_t ie_state_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const stream_IE_state *st = vptr; switch ( index ) { default: return 0;
case 0: return (pep->ptr = (const void *)(st->Decode), (&ptr_struct_procs));
case 1: return enum_bytestring(pep, &st->Table);
} }
static
void ie_state_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { stream_IE_state *st = vptr;
{
    (st->Decode = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->Decode), gcst)));
    reloc_bytestring(&(st->Table), gcst);
}
}
...
s_IE_set_defaults(stream_state * st)
{
    ...
    ss->Decode = 0;
    ((&ss->Table)->data = (0), (&ss->Table)->size = (0), (&ss->Table)->bytes = 0);
}
...
s_IE_init(stream_state * st)
{
    ...
    for (i = 0; i < (sizeof(ss->hash_table) / sizeof((ss->hash_table)[0])); ++i)
             ...
             stream_cursor_write * pw, bool last)
{
    ...
    for (;;) {
        ...
        for (hash %= (sizeof(ss->hash_table) / sizeof((ss->hash_table)[0]));
             memcmp(table + ss->hash_table[hash], key, num_components);
             hash = (hash + reprobe) % (sizeof(ss->hash_table) / sizeof((ss->hash_table)[0]))
             )
            do { } while (0);
...
s_Downsample_set_defaults(register stream_state * st)
{
    ...
    ((ss)->AntiAlias = (ss)->padX = (ss)->padY = ((bool)0));
}
...
static const gs_memory_struct_type_t st_Subsample_state = { sizeof(stream_Subsample_state), "stream_Subsample_state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_Subsample_init(stream_state * st)
{
    ...
    if ((float)xf != ss->XFactor) {
        errprintf(st->memory,"Subsample filter does not support non-integer downsample factor (%f)\n", ss->XFactor);
...
const stream_template s_Subsample_template = {
    ...
    0 , s_Downsample_set_defaults
};



static const gc_ptr_element_t avg_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_Average_state *)0)->sums - (char *)((stream_Average_state *)0) )) } }; static const gc_struct_data_t avg_reloc_ptrs = { (sizeof(avg_enum_ptrs) / sizeof((avg_enum_ptrs)[0])), 0, 0, avg_enum_ptrs }; static const gs_memory_struct_type_t st_Average_state = { sizeof(stream_Average_state), "stream_Average_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &avg_reloc_ptrs };
...
s_Average_init(stream_state * st)
{
    ...
    if ((float)xf != ss->XFactor) {
        errprintf(st->memory,"Average filter does not support non-integer downsample factor (%f)\n", ss->XFactor);
    ...
    if (ss->sums)
        ((st->memory)->procs.free_object(st->memory, ss->sums, "Average sums"));
    ss->sums =
        (uint *)(*(st->memory)->procs.alloc_byte_array)(st->memory, ss->sum_size, sizeof(uint), "Average sums");
...
top:
    if (y == yf || (last && p >= rlimit && ss->padY && y != 0)) {
        ...
        int ncopy = (((ss->copy_size - x) < (wlimit - q)) ? (ss->copy_size - x) : (wlimit - q));
...
static const gc_ptr_element_t bcb_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_Bicubic_state *)0)->data - (char *)((stream_Bicubic_state *)0) )) } }; static const gc_struct_data_t bcb_reloc_ptrs = { (sizeof(bcb_enum_ptrs) / sizeof((bcb_enum_ptrs)[0])), 0, 0, bcb_enum_ptrs }; static const gs_memory_struct_type_t st_Bicubic_state = { sizeof(stream_Bicubic_state), "stream_Bicubic_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &bcb_reloc_ptrs };
...
s_Bicubic_init(stream_state * st)
{
    ...
    if (ss->data)
        ((st->memory)->procs.free_object(st->memory, ss->data, "Bicubic data"));
    ss->data = (byte *)(*(st->memory)->procs.alloc_bytes)(st->memory, ss->d_size, "Bicubic data");
    if (ss->data == ((void*)0))
        return ((int)(-2));
                  ...
                  stream_cursor_write * pw, bool last)
{
    ...
    for (;;) {
        ...
        if ((ss->d_len < ss->d_size) && (pr->ptr < pr->limit)) {
            ...
            ulong copy = (((ss->d_size - ss->d_len) < (pr->limit - pr->ptr)) ? (ss->d_size - ss->d_len) : (pr->limit - pr->ptr));
...
static const gc_ptr_element_t compr_chooser_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_compr_chooser_state *)0)->sample - (char *)((stream_compr_chooser_state *)0) )) } }; static const gc_struct_data_t compr_chooser_reloc_ptrs = { (sizeof(compr_chooser_enum_ptrs) / sizeof((compr_chooser_enum_ptrs)[0])), 0, 0, compr_chooser_enum_ptrs }; static const gs_memory_struct_type_t st_compr_chooser_state = { sizeof(stream_compr_chooser_state), "stream_compr_chooser_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &compr_chooser_reloc_ptrs };
                    ...
                    int height, int depth, int bits_per_sample)
{
    ...
    ss->sample = (*(ss->memory)->procs.alloc_bytes)(ss->memory, width * depth, "s_compr_chooser_set_dimensions");
    if (ss->sample == 0)
        return ((-25));
    return 0;
}
...
s_compr_chooser_release(stream_state * st)
{
    ...
    ((ss->memory)->procs.free_object(ss->memory, ss->sample, "s_compr_chooser_release"));
}
...
{
# 982 "./devices/vector/gdevpsds.c"
    const int delta = 256 / 16;
    const int max_lineart_boundary_width = 3;
    const int max_gradient_constant = 10;
    int i, j0 = 0, j1 = 0;
    int w0 = p[0], w1 = p[0], v;
    ulong plateau_count = 0, lower_plateaus = 0;
    ulong upper_plateaus = 0, gradients = 0;
    bool lower = ((bool)0), upper = ((bool)0);
    ...
    for (i = 1; i < ss->width; i++) {
        ...
        if (!lower) {
            if (w1 < v) {
                ...
                j1 = i - max_gradient_constant;
            else if (upper && w1 - delta > v) {

                for (j0 = i - 1; j0 > j1 && w1 - delta <= p[j0]; j0--) do { } while (0);

                if(j0 > 0 && i < ss->width - 1)
                    upper_plateaus += i - j0;
                plateau_count ++;
                if (j0 > j1) {
        ...
        if (!upper) {
            if (w0 > v) {
                ...
                for (j0 = i - 1; j0 > j1 && w0 + delta >= p[j0]; j0--) do { } while (0);
    ...
    if (plateau_count > ss->width / 6) {
        ...
        plateaus = (((ss->lower_plateaus) < (ss->upper_plateaus)) ? (ss->lower_plateaus) : (ss->upper_plateaus));
...
{
    ulong plateaus = (((ss->lower_plateaus) < (ss->upper_plateaus)) ? (ss->lower_plateaus) : (ss->upper_plateaus));

    if (ss->choice)
        return ss->choice;
    if (force) {
        if (ss->gradients > plateaus / 12)
            return 1;
...
static const gc_ptr_element_t stream_image_colors_enum_ptrs[] = { { GC_ELT_OBJ, ((int) ( (char *)&((stream_image_colors_state *)0)->pcs - (char *)((stream_image_colors_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_image_colors_state *)0)->pdev - (char *)((stream_image_colors_state *)0) )) }, { GC_ELT_OBJ, ((int) ( (char *)&((stream_image_colors_state *)0)->pis - (char *)((stream_image_colors_state *)0) )) } }; static const gc_struct_data_t stream_image_colors_reloc_ptrs = { (sizeof(stream_image_colors_enum_ptrs) / sizeof((stream_image_colors_enum_ptrs)[0])), 0, 0, stream_image_colors_enum_ptrs }; static const gs_memory_struct_type_t st_stream_image_colors_state = { sizeof(stream_image_colors_state), "stream_image_colors_state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &stream_image_colors_reloc_ptrs };
                               ...
                               int width, int height, int depth, int bits_per_sample)
{
    ...
    ss->raster = ((uint)((((ss->row_bits) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    ss->row_alignment_bytes = 0;
}
                               ...
                               float *Decode)
{
    ...
    ss->output_component_index = ss->output_depth;
    ss->output_bits_per_sample = pdev->color_info.comp_bits[0];
    ...
    for (;;) {
        ...
        if (ss->output_component_index < ss->output_depth) {
            for (;ss->output_component_index < ss->output_depth;) {
                ...
                n = ss->output_bits_per_sample - ss->output_component_bits_written;
                w = v - ((v >> n) << n);
                if (fitting > n)
                    fitting = n;
                m = n - fitting;
