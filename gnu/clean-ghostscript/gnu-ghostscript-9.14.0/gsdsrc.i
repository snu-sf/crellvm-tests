...
static gs_ptr_type_t data_source_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void data_source_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_data_source = { sizeof(gs_data_source_t), "gs_data_source_t", 0, 0, data_source_enum_ptrs, data_source_reloc_ptrs, 0, 0 };
static
gs_ptr_type_t data_source_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const gs_data_source_t *psrc = vptr; switch ( index ) { default:
{
    if (psrc->type == data_source_type_string)
        return (pep->ptr = (&((const gs_data_source_t *)vptr)->data.str)->data, pep->size = (&((const gs_data_source_t *)vptr)->data.str)->size, (&ptr_const_string_procs));
    else if (psrc->type == data_source_type_stream)
        return (pep->ptr = (const void *)(((const gs_data_source_t *)vptr)->data.strm), (&ptr_struct_procs));
    else
        return (pep->ptr = (const void *)(((const gs_data_source_t *)vptr)->data.str.data), (&ptr_struct_procs));
}
} }
static void data_source_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { gs_data_source_t *psrc = vptr;
{
    if (psrc->type == data_source_type_string)
        (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(((gs_data_source_t *)vptr)->data.str), gcst));
    else if (psrc->type == data_source_type_stream)
        (((gs_data_source_t *)vptr)->data.strm = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_data_source_t *)vptr)->data.strm), gcst)));
    else
        (((gs_data_source_t *)vptr)->data.str.data = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(((gs_data_source_t *)vptr)->data.str.data), gcst)));
}
}
                          ...
                          uint length, byte * buf, const byte ** ptr)
{
    ...
    if (start >= s->position &&
        ...
        uint nread;
        int code = spseek(s, (gs_offset_t)(start));
