...
static void stream_finalize(const gs_memory_t *cmem, void *ptr); static gs_ptr_type_t stream_enum_ptrs(const gs_memory_t *mem, const void *ptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); static void stream_reloc_ptrs(void *ptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst); const gs_memory_struct_type_t st_stream = { sizeof(stream), "stream", 0, 0, stream_enum_ptrs, stream_reloc_ptrs, stream_finalize, 0 };
const gs_memory_struct_type_t st_stream_state = { sizeof(stream_state), "stream_state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };

static
gs_ptr_type_t stream_enum_ptrs(const gs_memory_t *mem, const void *vptr, uint size, int index, enum_ptr_t *pep, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { const stream *st = vptr; switch ( index ) { default: return 0;
case 0:
if (st->foreign)
    return (pep->ptr = (const void *)(((void*)0)), (&ptr_struct_procs));
else if (st->cbuf_string.data != 0)
    return (pep->ptr = (&((const stream *)vptr)->cbuf_string)->data, pep->size = (&((const stream *)vptr)->cbuf_string)->size, (&ptr_string_procs));
else
    return (pep->ptr = (const void *)(st->cbuf), (&ptr_struct_procs));
case 1: return (pep->ptr = (const void *)(((const stream *)vptr)->strm), (&ptr_struct_procs)); case (1)+1: return (pep->ptr = (const void *)(((const stream *)vptr)->prev), (&ptr_struct_procs)); case (1)+2: return (pep->ptr = (const void *)(((const stream *)vptr)->next), (&ptr_struct_procs));
case 4: return (pep->ptr = (const void *)(((const stream *)vptr)->state), (&ptr_struct_procs));
case 5: return (pep->ptr = (&st->file_name)->data, pep->size = (&st->file_name)->size, (&ptr_const_string_procs));
} }
static void stream_reloc_ptrs(void *vptr, uint size, const gs_memory_struct_type_t *pstype, gc_state_t *gcst) { stream *st = vptr;
{
    ...
    if (cbuf_old != 0 && !st->foreign) {
        ...
        if (st->cbuf_string.data != 0) {
            (((*(const gc_procs_common_t **)(gcst))->reloc_string)(&(st->cbuf_string), gcst));
            st->cbuf = st->cbuf_string.data;
        } else
            (st->cbuf = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->cbuf), gcst)));
        reloc = cbuf_old - st->cbuf;

        st->cursor.r.ptr -= reloc;
        st->cursor.r.limit -= reloc;
        st->cursor.w.limit -= reloc;
    }
    (st->strm = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->strm), gcst)));
    (st->prev = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->prev), gcst)));
    (st->next = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->next), gcst)));
    (st->state = (((*(const gc_procs_common_t **)(gcst))->reloc_struct_ptr)((const void *)(st->state), gcst)));
    (((*(const gc_procs_common_t **)(gcst))->reloc_const_string)(&(st->file_name), gcst));
}
}
...
stream_finalize(const gs_memory_t *cmem, void *vptr)
{
    ...
    if (((st)->modes != 0) && !st->is_temp && st->file != 0) {
...
ssetfilename(stream *s, const byte *data, uint size)
{
    byte *str =
        (s->file_name.data == 0 ?
         (*(s->memory)->procs.alloc_string)(s->memory, size + 1, "ssetfilename") :
         (*(s->memory)->procs.resize_string)(s->memory, (byte *)s->file_name.data, s->file_name.size, size + 1, "ssetfilename"));
...
s_filter_close(register stream * s)
{
    ...
    if ((((s)->modes & 2) != 0)) {
        ...
        if (status != 0 && status != ((int)(-1)))
            return status;
        status = (*(stemp)->procs.flush)(stemp);
        if (status != 0 && status != ((int)(-1)))
            return status;
    }
...
sclose(register stream * s)
{
    ...
    st = s->state;
    if (st != 0) {
        void (*release)(stream_state *) = st->templat->release;
        if (release != 0)
            (*release) (st);
...
{
    int status, left;
    int min_left = ((s->end_status == ((int)(-1)) || s->end_status == ((int)(-2)) ? 0 : s->state->min_left));
    ...
    while (status = s->end_status,
        ...
        s_process_read_buf(s);
    if (left <= min_left &&
        (left == 0 || (status != ((int)(-1)) && status != ((int)(-2))))
        ) {

        stream_compact(s, ((bool)1));
        if (status == ((int)(-1)) && close_at_eod && s->close_at_eod) {
            status = sclose(s);
            if (status == 0)
                status = ((int)(-1));
            s->end_status = status;
...
sgets(stream * s, byte * buf, uint nmax, uint * pn)
{
    ...
    int status = 0;
    gs_offset_t min_left = ((s->end_status == ((int)(-1)) || s->end_status == ((int)(-2)) ? 0 : s->state->min_left));
...
spskip(register stream * s, gs_offset_t nskip, gs_offset_t *pskipped)
{
    ...
    if ((((s)->modes & 4) != 0)) {
        ...
        int status = spseek(s, (gs_offset_t)(pos + n));

        *pskipped = stell(s) - pos;
        return status;
    }
    min_left = ((s->end_status == ((int)(-1)) || s->end_status == ((int)(-2)) ? 0 : s->state->min_left));
    while (((s)->cursor.r.limit - (s)->cursor.r.ptr) < n + min_left) {
        ...
        status = ((int)((s)->cursor.r.limit - (s)->cursor.r.ptr > 1 ? (++((s)->cursor.r.ptr), (int)*(s)->cursor.r.ptr) : spgetcc(s, ((bool)1))));
        if (status < 0) {
            *pskipped = nskip - n;
            return status;
        }
...
top:
    ...
    for (;;) {
        int ch = ((int)((s_in)->cursor.r.limit - (s_in)->cursor.r.ptr > 1 ? (++((s_in)->cursor.r.ptr), (int)*(s_in)->cursor.r.ptr) : spgetcc(s_in, ((bool)1))));
        ...
        if (count >= buf->size) {
            ...
            {
                uint nsize = count + (((count) > (20)) ? (count) : (20));
                byte *ndata = (*(bufmem)->procs.resize_string)(bufmem, buf->data, buf->size, nsize, "sreadline(buffer)");
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (strm == 0) {
                ...
                pr = &strm->cursor.r;
                left = ((strm->end_status == ((int)(-1)) || strm->end_status == ((int)(-2)) ? 0 : strm->state->min_left));
                left = (((left) < (pr->limit - pr->ptr)) ? (left) : (pr->limit - pr->ptr));
                pr->limit -= left;
            ...
            if (strm->end_status < 0) {
                if (strm->end_status != ((int)(-1)) || pw->ptr == oldpos)
                    status = strm->end_status;
                break;
            }
            do { stream *ahead = curr->strm; curr->strm = prev; prev = curr; curr = ahead; } while (0);
        ...
        if ((strm != 0 || curr->file) && status == ((int)(-1)) &&
        ...
        if (prev == 0)
            return status;
        do { stream *back = prev->strm; prev->strm = curr; curr = prev; prev = back; } while (0);
    }
}
    ...
    for (;;) {
        ...
        if (status < 0 || prev == 0) {
            ...
            while (prev) {
                ...
                if (status >= 0)
                    curr->end_status = 0;
                else if (status == ((int)(-2)))
                    curr->end_status = ((int)(-2));
            }
            return status;
        }
        do { stream *back = prev->strm; prev->strm = curr; curr = prev; prev = back; } while (0);
...
{
    s->cursor.r.ptr = s->cbuf - 1;
    s->cursor.r.limit = s->cursor.r.ptr + s->bsize;
}
static int
s_string_reusable_flush(stream *s)
{
    s->cursor.r.ptr = s->cursor.r.limit = s->cbuf + s->bsize - 1;
    return 0;
}
void
              ...
              stream *target)
{
    ...
    if (bsize < templat->min_in_size)
        return ((int)(-2));
    s_std_init(fs, buf, bsize, &s_filter_write_procs, 2);
             ...
             stream_state *ss, gs_memory_t *mem)
{
    stream *es;
    stream_state *ess;
    uint bsize = (((templat->min_in_size) > (256)) ? (templat->min_in_size) : (256));
    byte *buf;
    ...
    if (bsize > (*ps)->bsize && templat->process != s_NullE_template.process) {
        ...
        if (s_add_filter(ps, &null_template, ((void*)0), mem) == 0)
            return 0;
    }
    es = s_alloc(mem, "s_add_filter(stream)");
    buf = (*(mem)->procs.alloc_bytes)(mem, bsize, "s_add_filter(buf)");
    if (es == 0 || buf == 0) {
        ((mem)->procs.free_object(mem, buf, "s_add_filter(buf)"));
        ((mem)->procs.free_object(mem, es, "s_add_filter(stream)"));
        return 0;
    }
...
s_close_filters(stream **ps, stream *target)
{
    while (*ps != target) {
        ...
        if (status < 0)
            return status;
        if (mem) {
            ((mem)->procs.free_object(mem, sbuf, "s_close_filters(buf)"));
            ((mem)->procs.free_object(mem, s, "s_close_filters(stream)"));
            if (ss != (stream_state *)s)
                ((mem)->procs.free_object(mem, ss, "s_close_filters(state)"));
        }
        *ps = next;
    }
    return 0;
...
file_close_disable(stream * s)
{
    ...
    if ((*s->save_close != ((void*)0)) && ((code = (*s->save_close)(s)) != 0))
        return code;
    s_close_disable(s);
...
const stream_template s_NullE_template = {
    &st_stream_state, ((void*)0), s_Null_process, 1, 1
};
