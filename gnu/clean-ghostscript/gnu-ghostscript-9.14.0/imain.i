...
gs_main_alloc_instance(gs_memory_t *mem)
{
    ...
    minst = (gs_main_instance *)((mem)->procs.alloc_bytes_immovable(mem, sizeof(gs_main_instance), "init_main_instance"));
...
get_op_array(const gs_memory_t *mem, int size)
{
    ...
    return ((size) < minst->i_ctx_p->op_array_table_local.base_index ? &minst->i_ctx_p->op_array_table_global : &minst->i_ctx_p->op_array_table_local);
}
              ...
              int max_lib_paths)
{
    ...
    paths = (ref *) (*(minst->heap)->procs.alloc_byte_array)(minst->heap, max_lib_paths, sizeof(ref), "lib_path array");

    if (paths == 0) {
        gs_lib_finit(1, (-25), minst->heap);
        return ((-25));
    }
    array = (ref *) (*(minst->heap)->procs.alloc_byte_array)(minst->heap, max_lib_paths, sizeof(ref), "lib_path array");

    if (array == 0) {
        gs_lib_finit(1, (-25), minst->heap);
        return ((-25));
    }
    ((&minst->lib_path.container)->value.refs = (array), ((&minst->lib_path.container)->tas.type_attrs = ((t_array) << 8) + (avm_foreign)), ((&minst->lib_path.container)->tas.rsize = (max_lib_paths)));
...
gs_main_init1(gs_main_instance * minst)
{
    ...
    if (minst->init_done < 1) {
        ...
        if (code < 0)
            return code;
        code = gs_lib_init1((gs_memory_t *)idmem.spaces.memories.named.system);
        if (code < 0)
            return code;
        alloc_save_init(&idmem);
        {
            gs_memory_t *mem = (gs_memory_t *)idmem.spaces.memories.named.system;
            name_table *nt = names_init(minst->name_table_size,
                                        idmem.spaces.memories.named.system);
        ...
        code = gs_iodev_init(((gs_memory_t *)((i_ctx_p->memory).current)));
        if (code < 0)
            return code;
        minst->init_done = 1;
    }
...
int gs_main_init2aux(gs_main_instance * minst) {
    ...
    if (minst->init_done < 2) {
        ...
        ((&ifa)->value.const_bytes = (gs_init_files), ((&ifa)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40) | avm_foreign)), ((&ifa)->tas.rsize = (gs_init_files_sizeof - 2)));
        code = i_initial_enter_name(i_ctx_p, "INITFILES", &ifa);
        if (code < 0)
            return code;


        ((&ifa)->value.const_bytes = (gs_emulators), ((&ifa)->tas.type_attrs = ((t_string) << 8) + ((0x20 + 0x40) | avm_foreign)), ((&ifa)->tas.rsize = (gs_emulators_sizeof - 2)));
...
gs_main_init2(gs_main_instance * minst)
{
       ...
       gx_device_printer *ppdev = (gx_device_printer *)pdev;
        ...
        if (minst->saved_pages_test_mode) {
            if ((((pdev)->procs.dev_spec_op)(pdev, gxdso_supports_saved_pages, ((void*)0), 0) == 0)) {
                ...
                minst->saved_pages_test_mode = ((bool)0);
            } else {
        ...
        } else if (minst->saved_pages_initial_arg != ((void*)0)) {
            if (((pdev)->procs.dev_spec_op)(pdev, gxdso_supports_saved_pages, ((void*)0), 0) == 0) {
    ...
    if (gs_debug_c(':'))
        print_resource_usage(minst, &(i_ctx_p->memory), "Start");
    gp_readline_init(&minst->readline_data, ((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.system)));
    return 0;
}
...
{
    uint len = ((&minst->lib_path.container)->tas.rsize);
    ref *paths, *opaths = minst->lib_path.container.value.refs;


    paths = (ref *) (*(minst->heap)->procs.alloc_byte_array)(minst->heap, len + 5, sizeof(ref), "extend_path_list_container array");


    if (paths == 0) {
        return ((-25));
    }
    ((&minst->lib_path.container)->value.refs = (paths), ((&minst->lib_path.container)->tas.type_attrs = ((t_array) << 8) + (avm_foreign)), ((&minst->lib_path.container)->tas.rsize = (len + 5)));
    ((&minst->lib_path.list)->value.refs = (minst->lib_path.container.value.refs), ((&minst->lib_path.list)->tas.type_attrs = ((t_array) << 8) + (avm_foreign | (0x20 + 0x40))), ((&minst->lib_path.list)->tas.rsize = (0)));


    memcpy(paths, opaths, len * sizeof(ref));
    ((&minst->lib_path.list)->tas.rsize = (len));

    ((minst->heap)->procs.free_object(minst->heap, opaths, "extend_path_list_container"));
    return(0);
}
    ...
    for (;;) {
        ...
        if (npath > dpath) {
            if (len == ((&pfp->container)->tas.rsize)) {
                ...
                if (code < 0) {
                    (emprintf_program_ident(minst->heap, gs_program_name(), gs_revision_number()), errprintf(minst->heap, "\nAdding path to search paths failed.\n"));
                    return(code);
                }
            }
...
gs_main_add_lib_path(gs_main_instance * minst, const char *lpath)
{
    ...
    int first_is_here =
        (((&minst->lib_path.list)->tas.rsize) != 0 &&
...
gs_main_lib_open(gs_main_instance * minst, const char *file_name, ref * pfile)
{
    ...
    return lib_file_open(&minst->lib_path, ((gs_memory_t *)((i_ctx_p->memory).current)),
                         ((void*)0),

                          file_name, strlen(file_name), fn, 2048, &len, pfile);
}
...
gs_main_run_file_open(gs_main_instance * minst, const char *file_name, ref * pfref)
{
    ...
    if (gs_main_lib_open(minst, file_name, pfref) < 0) {
        (emprintf_program_ident(minst->heap, gs_program_name(), gs_revision_number()), errprintf(minst->heap, "Can't find initialization file %s.\n", file_name));
...
gs_run_init_file(gs_main_instance * minst, int *pexit_code, ref * perror_object)
{
    ...
    gs_scanner_init_options(&state, &ifile, 0);
    code = gs_scan_token(i_ctx_p, &first_token, &state);
    if (code != 0 || !((((const byte *)&((&first_token)->tas.type_attrs))[1]) == (t_integer))) {
        (emprintf_program_ident(minst->heap, gs_program_name(), gs_revision_number()), errprintf(minst->heap, "Initialization file %s does not begin with an integer.\n", gs_init_file));


        *pexit_code = 255;
        return ((-100));
    }
    *++(((i_ctx_p->op_stack).stack).p) = first_token;
...
{
    i_ctx_t *i_ctx_p = minst->i_ctx_p;
    int code = ref_stack_push(&((i_ctx_p->op_stack).stack), 1);

    if (code < 0)
        return code;
    *ref_stack_index(&((i_ctx_p->op_stack).stack), 0L) = *pvalue;
    return 0;
}
...
{
    if (!ref_stack_count(&((i_ctx_p->op_stack).stack)))
        return ((-17));
    *pvalue = *ref_stack_index(&((i_ctx_p->op_stack).stack), 0L);
    return 0;
}
...
gs_pop_boolean(gs_main_instance * minst, bool * result)
{
    ...
    if (code < 0)
        return code;
    do { if ( !((((const byte *)&((&vref)->tas.type_attrs))[1]) == (t_boolean)) ) return ((-20)); } while (0);
    *result = vref.value.boolval;
    ref_stack_pop(&((i_ctx_p->op_stack).stack), 1);
    return 0;
}
...
gs_pop_integer(gs_main_instance * minst, long *result)
{
    ...
    if (code < 0)
        return code;
    do { if ( !((((const byte *)&((&vref)->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0);
...
gs_pop_string(gs_main_instance * minst, gs_string * result)
{
    ...
    switch ((((const byte *)&((&vref)->tas.type_attrs))[1])) {
        case t_name:
            names_string_ref(minst->heap->gs_lib_ctx->gs_name_table, &vref, &vref);
            code = 1;
            goto rstr;
        case t_string:
...
static char *gs_main_tempnames(gs_main_instance *minst)
{
    ...
    ref *tempfiles;
    ref keyval[2];
    char *tempnames = ((void*)0);
    int i;
    int idict;
    int len = 0;
    const byte *data = ((void*)0);
    uint size;
    if (minst->init_done >= 2) {
...
# 874 "./psi/imain.c"
    ...
    if (minst->init_done >= 1) {
        ...
        if ((&(i_ctx_p->memory))->reclaim != 0) {
            code = interp_reclaim(&minst->i_ctx_p, avm_global);

            if (code < 0) {
                (emprintf_program_ident(minst->heap, gs_program_name(), gs_revision_number()), errprintf(minst->heap, "ERROR %d reclaiming the memory while the interpreter finalization.\n", code));
        ...
        if (i_ctx_p->pgs != ((void*)0) && i_ctx_p->pgs->device != ((void*)0)) {
            gx_device *pdev = i_ctx_p->pgs->device;
            const char * dname = pdev->dname;


            do { if (pdev) { do { do { } while (0); (pdev)->rc.ref_count += (1); } while (0); if (!(pdev)->rc.ref_count) { do { do { } while (0); (pdev)->rc.free((pdev)->rc.memory, (void *)(pdev), "gs_main_finit"); } while (0); (pdev) = 0; } else do { } while (0); } } while (0);
            ...
            if (code < 0)
                (emprintf_program_ident(pdev->memory, gs_program_name(), gs_revision_number()), errprintf(pdev->memory, "ERROR %d closing %s device. See gs/psi/ierrors.h for code explanation.\n", code, dname));



            do { if (pdev) { do { do { } while (0); (pdev)->rc.ref_count += (-1); } while (0); if (!(pdev)->rc.ref_count) { do { do { } while (0); (pdev)->rc.free((pdev)->rc.memory, (void *)(pdev), "gs_main_finit"); } while (0); (pdev) = 0; } else do { } while (0); } } while (0);
            if (exit_status == 0 || exit_status == (-101))
                exit_status = code;
        }
    ...
    i_ctx_p = minst->i_ctx_p;
    if (gs_debug_c(':')) {
        print_resource_usage(minst, &(i_ctx_p->memory), "Final");
        errprintf(minst->heap,"%% Exiting instance 0x%p\n", minst);
    }
    ...
    if (minst->init_done >= 1) {
        ...
        code = alloc_restore_all((&(i_ctx_p->memory)));
        if (code < 0)
            (emprintf_program_ident(mem_raw, gs_program_name(), gs_revision_number()), errprintf(mem_raw, "ERROR %d while the final restore. See gs/psi/ierrors.h for code explanation.\n", code));
                     ...
                     const char *msg)
{
    ...
    gp_get_realtime(utime);
    {
        ...
        for (i = 0; i < (sizeof(dmem->spaces.memories.indexed) / sizeof((dmem->spaces.memories.indexed)[0])); ++i) {
            gs_ref_memory_t *mem = dmem->spaces.memories.indexed[i];

            if (mem != 0 && (i == 0 || mem != dmem->spaces.memories.indexed[i - 1])) {
                gs_memory_status_t status;
                gs_ref_memory_t *mem_stable =
                    (gs_ref_memory_t *)(((gs_memory_t *)mem)->procs.stable((gs_memory_t *)mem));

                (((gs_memory_t *)mem)->procs.status((gs_memory_t *)mem, &status));
                allocated += status.allocated;
                used += status.used;
                if (mem_stable != mem) {
                    (((gs_memory_t *)mem_stable)->procs.status((gs_memory_t *)mem_stable, &status));
                    allocated += status.allocated;
                    used += status.used;
                }
            }
    ...
    errprintf(minst->heap,"%% %s time = %g, memory allocated = %lu, used = %lu\n", msg, utime[0] - minst->base_time[0] + (utime[1] - minst->base_time[1]) / 1000000000.0, allocated, used);
...
gs_main_dump_stack(gs_main_instance *minst, int code, ref * perror_object)
{
    ...
    if (perror_object != 0) {
        errprintf(minst->heap,"%s", "Error object: ");
        debug_print_ref(minst->heap, perror_object);
        errprintf(minst->heap,"%c", '\n');
    }
    debug_dump_stack(minst->heap, &((i_ctx_p->op_stack).stack), "Operand stack");
    debug_dump_stack(minst->heap, &((i_ctx_p->exec_stack).stack), "Execution stack");
    debug_dump_stack(minst->heap, &((i_ctx_p->dict_stack).stack), "Dictionary stack");
}
