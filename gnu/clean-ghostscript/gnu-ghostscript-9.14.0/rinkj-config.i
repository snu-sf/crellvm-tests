# 1 "./devices/rinkj/rinkj-config.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/rinkj/rinkj-config.c" 2
# 19 "./devices/rinkj/rinkj-config.c"
...
# 20 "./devices/rinkj/rinkj-config.c" 2
...
# 21 "./devices/rinkj/rinkj-config.c" 2
...
# 22 "./devices/rinkj/rinkj-config.c" 2
...
# 24 "./devices/rinkj/rinkj-config.c" 2
# 35 "./devices/rinkj/rinkj-config.c"
char *
rinkj_strdup_size (const char *src, int size)
{
  char *result;
...
# 55 "./devices/rinkj/rinkj-config.c"
char *
rinkj_config_get (const char *config, const char *key)
{
  int ix;
  ...
  for (ix = 0; config[ix]; ix = ix_next)
    {
      ...
      p_nl = (__extension__ (__builtin_constant_p ('\n') && !__builtin_constant_p (config + ix) && ('\n') == '\0' ? (char *) __rawmemchr (config + ix, '\n') : __builtin_strchr (config + ix, '\n')));
      if (p_nl == ((void*)0))
        {
      ...
      for (key_ix = 0; ix + key_ix < ix_eol; key_ix++)
        {
          if (key[key_ix] == 0 && config[ix + key_ix] == ':')
            {
              ix += key_ix + 1;
              while (ix < ix_eol && ((*__ctype_b_loc ())[(int) ((config[ix]))] & (unsigned short int) _ISspace))
                ix++;
...
# 108 "./devices/rinkj/rinkj-config.c"
char *
rinkj_config_keyval (const char *config, char **p_val, const char **p_next)
{
  char *key;
