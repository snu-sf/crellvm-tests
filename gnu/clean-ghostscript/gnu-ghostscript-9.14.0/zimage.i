                  ...
                  bool has_alpha, bool islab)
{
    ...
    code = dict_int_param(op, "Width", 0, ((int)(((~(-1 << ((1 << 2) * 8 - 1)))) >> (8)))/2, -1, &pim->Width);
    if (code < 0)
     return code;
    code = dict_int_param(op, "Height", 0, ((int)(((~(-1 << ((1 << 2) * 8 - 1)))) >> (8)))/2, -1, &pim->Height);
    if (code < 0)
     return code;
    code = dict_matrix_param(mem, op, "ImageMatrix", &pim->ImageMatrix);
    if (code < 0)
     return code;
    code = dict_bool_param(op, "MultipleDataSources", ((bool)0), &pip->MultipleDataSources);
    if (code < 0)
     return code;
    code = dict_int_param(op, "BitsPerComponent", 1, max_bits_per_component, -1, &pim->BitsPerComponent);
    if (code < 0)
     return code;
    code = dict_bool_param(op, "Interpolate", ((bool)0), &pim->Interpolate);
    if (code < 0)
     return code;
# 91 "./psi/zimage.c"
    if (islab) {
    ...
    if (pip->MultipleDataSources) {
        ...
        if (!(((pds)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
            return ((-20));
        if (((pds)->tas.rsize) != n)
            return ((-15));
        for (i = 0; i < n; ++i)
            array_get(mem, pds, i, &ds[i]);
        if ((((const byte *)&((&ds[0])->tas.type_attrs))[1]) == t_string) {
            ...
            if (has_alpha)
                n--;
            for (i = 1; i < n; ++i) {
                if ((((const byte *)&((&ds[i])->tas.type_attrs))[1]) == t_string && ((&ds[i])->tas.rsize) != ((&ds[0])->tas.rsize)) {
                    return ((-15));
                }
            }
        }
                   ...
                   bool has_alpha, gs_color_space *csp)
{
    ...
    if (pim->ColorSpace->cmm_icc_profile_data != ((void*)0))
        islab = pim->ColorSpace->cmm_icc_profile_data->islab;

    code = data_image_params(((gs_memory_t *)((i_ctx_p->memory).current)), op, (gs_data_image_t *) pim, pip, ((bool)1),
    ...
    return dict_bool_param(op, "CombineWithColor", ((bool)0),
             ...
             const ref * sources, bool uses_color, int npop)
{
    gx_image_enum_common_t *pie;
    int code =
        gs_image_begin_typed((const gs_image_common_t *)pim, (i_ctx_p->pgs),
                             uses_color, &pie);
...
image1_setup(i_ctx_t * i_ctx_p, bool has_alpha)
{
    ...
    image_params ip;
    int code;
    gs_color_space *csp = gs_currentcolorspace((i_ctx_p->pgs));
    ...
    if (gs_currentcpsimode(((gs_memory_t *)((i_ctx_p->memory).current))) && gs_color_space_num_components(csp) < 1) {
       gs_color_space *bsp = csp->base_space;
       if (bsp)
         csp = bsp;
    }

    gs_image_t_init_adjust(&image, csp, ((bool)1));
    code = pixel_image_params( i_ctx_p,
                               op,
                               (gs_pixel_image_t *)&image,
                               &ip,
                               (((i_ctx_p->language_level) >= 2) ? 16 : 8),
                               has_alpha, csp);
    if (code < 0)
        return code;
...
{
    return image1_setup(i_ctx_p, ((bool)0));
}
...
zimagemask1(i_ctx_t *i_ctx_p)
{
    ...
    gs_image_t_init_mask_adjust(&image, ((bool)0),
                                gs_incachedevice((i_ctx_p->pgs)) != CACHE_DEVICE_NONE);
    code = data_image_params(((gs_memory_t *)((i_ctx_p->memory).current)), op, (gs_data_image_t *) & image,
                             &ip, ((bool)1), 1, 1, ((bool)0), ((bool)0));
    if (code < 0)
        return code;
                  ...
                  gx_image_enum_common_t * pie, const ref * sources, int npop)
{
    ...
    int inumpush = ((num_sources) * 2 + 5);
    int code;
    gs_image_enum *penum;
    int px;
    const ref *pp;
    bool string_sources = ((bool)1);

    if ( (((i_ctx_p->exec_stack).stack).p) > (((i_ctx_p->exec_stack).stack).top) - (inumpush + 2) ) { int es_code_ = ref_stack_extend(&((i_ctx_p->exec_stack).stack), inumpush + 2); if ( es_code_ < 0 ) return es_code_; };
    (((((((i_ctx_p->exec_stack).stack).p)) + 2))->value.intval = (num_sources), (((((((i_ctx_p->exec_stack).stack).p)) + 2))->tas.type_attrs = ((t_integer) << 8) + (0)) );
# 337 "./psi/zimage.c"
    for (px = 0, pp = sources; px < num_sources; px++, pp++) {
        es_ptr ep = (((((i_ctx_p->exec_stack).stack).p)) + 3 + ((((((i_ctx_p->exec_stack).stack).p)) + 2)->value.intval - 1 - (px)) * 2);

        ((ep + 1)->value.intval = (1), ((ep + 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
...

                {
                    ...
                    for (pi = 0; pi < px; ++pi)
                        if (sources[pi].value.pfile == pp->value.pfile) {
                            ...
                            ((ep + 1)->value.intval = (-pi), ((ep + 1)->tas.type_attrs = ((t_integer) << 8) + (0)) );
                            (((((i_ctx_p->exec_stack).stack).p)) + 3 + ((((((i_ctx_p->exec_stack).stack).p)) + 2)->value.intval - 1 - (pi)) * 2)[1].value.intval++;
                            break;
                        }
                }
                string_sources = ((bool)0);

            case t_string:
                if ((((const byte *)&((pp)->tas.type_attrs))[1]) != (((const byte *)&((sources)->tas.type_attrs))[1])) {
                    gx_image_end(pie, ((bool)0));
                    return ((-20));
                }
                do { if ( !(((&*pp)->tas.type_attrs) & (0x20)) ) return ((-7)); } while (0);
                break;
            default:
                ...
                do { if ( !(((&*pp)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*pp)); } while (0);
                string_sources = ((bool)0);
        }
        *ep = *pp;
    }
    ...
    if ((penum = gs_image_enum_alloc(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local)), "image_setup")) == 0)
        return ((-25));
    code = gs_image_enum_init(penum, pie, (const gs_data_image_t *)pim, (i_ctx_p->pgs));
    if (code != 0 || (pie->skipping && string_sources)) {
        int code1 = gs_image_cleanup_and_free_enum(penum, (i_ctx_p->pgs));
    ...
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (image_cleanup), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_null) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    (((i_ctx_p->exec_stack).stack).p) += inumpush - 1;
    (((((((i_ctx_p->exec_stack).stack).p)) - 2))->value.intval = (0), (((((((i_ctx_p->exec_stack).stack).p)) - 2))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    (((((((i_ctx_p->exec_stack).stack).p)) - 1))->value.intval = (num_sources), (((((((i_ctx_p->exec_stack).stack).p)) - 1))->tas.type_attrs = ((t_integer) << 8) + (0)) );
    (((((i_ctx_p->exec_stack).stack).p))->value.pstruct = ((obj_header_t *)(penum)), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_struct) << 8) + (avm_local)) );
    switch ((((const byte *)&((sources)->tas.type_attrs))[1])) {
        case t_file:
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (image_file_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
            break;
        case t_string:
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (image_string_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
            break;
        default:
            (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (image_proc_process), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
            break;
    }
    ((((i_ctx_p->op_stack).stack).p) -= (npop));
    return 5;
...
{
    return tep - ((((tep) - 1)->value.intval) * 2 + 5);
}
...
{
    os_ptr op = (((i_ctx_p->op_stack).stack).p);
    gs_image_enum *penum = ((gs_image_enum *)(((((i_ctx_p->exec_stack).stack).p))->value.pstruct));
    int px = (((((i_ctx_p->exec_stack).stack).p)) - 2)->value.intval;
    int num_sources = (((((i_ctx_p->exec_stack).stack).p)) - 1)->value.intval;
    uint size, used[((64) + 1)];
    gs_const_string plane_data[((64) + 1)];
    const byte *wanted;
    int i, code;
    ...
    if (!(((op)->tas.type_attrs & ((((1 << 6) - (1)) << 8) + (0x20))) == (((t_string) << 8) + (0x20)))) {
        ...
        (((i_ctx_p->exec_stack).stack).p) = zimage_pop_estack((((i_ctx_p->exec_stack).stack).p));
        image_cleanup(i_ctx_p);
        return (!((((const byte *)&((op)->tas.type_attrs))[1]) == (t_string)) ? (-20) : (-7));
    }
    size = ((op)->tas.rsize);
    if (size == 0 && (((((i_ctx_p->exec_stack).stack).p)) - 4 - (0) * 2)[1].value.intval == 0)
        code = 1;
    else {
        ...
        if (code == (-103)) {
            ...
            ((op)->tas.rsize -= (used[px]));
            (((((i_ctx_p->exec_stack).stack).p)) - 4 - (0) * 2)[1].value.intval = 0;
            return code;
        }
    }
    if (code) {
    ...
    (((((i_ctx_p->exec_stack).stack).p)) - 2)->value.intval = px;
...
image_proc_process(i_ctx_t *i_ctx_p)
{
    ...
    pp = (((((i_ctx_p->exec_stack).stack).p)) - 4 - (px) * 2);
    (++(((i_ctx_p->exec_stack).stack).p), (((((i_ctx_p->exec_stack).stack).p))->value.opproc = (image_proc_continue), (((((i_ctx_p->exec_stack).stack).p))->tas.type_attrs = ((t_operator) << 8) + (0x80)), (((((i_ctx_p->exec_stack).stack).p))->tas.rsize = (0))));
    *++(((i_ctx_p->exec_stack).stack).p) = *pp;
    return 5;
}
...
image_file_continue(i_ctx_t *i_ctx_p)
{
    ...
    for (;;) {
        ...
        for (px = 0, pp = (((((i_ctx_p->exec_stack).stack).p)) - 4 - (0) * 2); px < num_sources;
             ++px, pp -= 2
            ) {
            ...
            if (num_aliases <= 0)
                num_aliases = (((((i_ctx_p->exec_stack).stack).p)) - 4 - (-num_aliases) * 2)[1].value.intval;
            while ((avail = ((s)->cursor.r.limit - (s)->cursor.r.ptr)) <=
                   (min_left = ((s->end_status == ((int)(-1)) || s->end_status == ((int)(-2)) ? 0 : s->state->min_left))) + num_aliases - 1) {
            ...
            if (avail < min_avail)
                min_avail = avail;
            plane_data[px].data = ((s)->cursor.r.ptr + 1);
...
# 568 "./psi/zimage.c"
        {
            ...
            total_used = 0;
            for (pi = 0, pp = (((((i_ctx_p->exec_stack).stack).p)) - 4 - (0) * 2); pi < num_sources;
                 ++pi, pp -= 2 ) {
                ((pp->value.pfile)->cursor.r.ptr += (used[pi]), 0);
                total_used += used[pi];
            }
            if (code == (-103))
                return code;
        ...
        if (code) {
            ...
            return (code < 0 ? code : code1 < 0 ? code1 : 14);
        }
    }
}
...
image_string_continue(i_ctx_t *i_ctx_p)
{
    ...
    stop_now:
        if (code) {
            (((i_ctx_p->exec_stack).stack).p) -= ((num_sources) * 2 + 5);
            image_cleanup(i_ctx_p);
            return (code < 0 ? code : 14);
        }
        for (px = 0; px < num_sources; ++px)
            if (sources[px].size == 0) {
                const ref *psrc = (((((i_ctx_p->exec_stack).stack).p)) - 4 - (px) * 2);
...
{
    es_ptr ep_top = (((i_ctx_p->exec_stack).stack).p) + (((((((i_ctx_p->exec_stack).stack).p)) + 2)->value.intval) * 2 + 5);
    gs_image_enum *penum = ((gs_image_enum *)((ep_top)->value.pstruct));

    return gs_image_cleanup_and_free_enum(penum, (i_ctx_p->pgs));
}
