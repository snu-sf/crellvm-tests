...
static const gs_memory_struct_type_t st_PFBD_state = { sizeof(stream_PFBD_state), "PFBDecode state", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
static const gc_ptr_element_t sfd_enum_ptrs[] = { { GC_ELT_CONST_STRING, ((int) ( (char *)&((stream_SFD_state *)0)->eod - (char *)((stream_SFD_state *)0) )) } }; static const gc_struct_data_t sfd_reloc_ptrs = { (sizeof(sfd_enum_ptrs) / sizeof((sfd_enum_ptrs)[0])), 0, 0, sfd_enum_ptrs }; static const gs_memory_struct_type_t st_SFD_state = { sizeof(stream_SFD_state), "SubFileDecode state", 0, 0, basic_enum_ptrs, basic_reloc_ptrs, 0, &sfd_reloc_ptrs };
              ...
              stream_cursor_write * pw, bool last)
{
    ...
    if (ss->eod.size == 0) {
        ...
        count = (((rcount) < (wcount)) ? (rcount) : (wcount));
        if (ss->count == 0)
...
cp:
        ...
        if (ss->copy_count) {
            int count = (((wlimit - q) < (ss->copy_count)) ? (wlimit - q) : (ss->copy_count));
