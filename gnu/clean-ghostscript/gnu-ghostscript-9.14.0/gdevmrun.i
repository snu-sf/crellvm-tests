...
typedef struct gx_device_run_s {
    ...
    struct sp_ {
        ...
        int (*get_bits_rectangle)(gx_device *dev, const gs_int_rect *prect, gs_get_bits_params_t *params, gs_int_rect **unread);
    } save_procs;
} gx_device_run;
...
rp_delete_next(run_ptr *prpc, run *data, run_line *line)
{
    ...
    ((rpn).ptr = (data) + ((rpn).index = ((*prpc).ptr->next)));
    ((rpn2).ptr = (data) + ((rpn2).index = ((rpn).ptr->next)));
    ((*prpc).ptr->next) = rpn2.index;
    ((rpn2).ptr->prev) = prpc->index;
    ((rpn).ptr->next) = line->free;
    ((rpn).ptr->prev) = 0;
    line->free = rpn.index;
}
static int
rp_insert_next(run_ptr *prpc, run *data, run_line *line, run_ptr *prpn)
{
    ...
    if (new == 0)
        return -1;
    ((*prpn).ptr = (data) + ((*prpn).index = ((*prpc).ptr->next)));
    ((*prpc).ptr->next) = new;
    ((*prpn).ptr->prev) = new;
...
rp_insert_prev(run_ptr *prpc, run *data, run_line *line, run_ptr *prpp)
{
    ...
    if (new == 0)
        return -1;
    ((*prpp).ptr = (data) + ((*prpp).index = ((*prpc).ptr->prev)));
    ((*prpp).ptr->next) = new;
    ((*prpc).ptr->prev) = new;
    line->free = prnew->next;
...
static int run_copy_mono(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1);
static int run_copy_color(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height);
static int run_fill_rectangle(gx_device *dev, int x, int y, int width, int height, gx_color_index color);
static int run_copy_alpha(gx_device *dev, const byte *data, int data_x, int raster, gx_bitmap_id id, int x, int y, int width, int height, gx_color_index color, int depth);
static int run_strip_tile_rectangle(gx_device *dev, const gx_strip_bitmap *tiles, int x, int y, int width, int height, gx_color_index color0, gx_color_index color1, int phase_x, int phase_y);
static int run_strip_copy_rop(gx_device *dev, const byte *sdata, int sourcex, uint sraster, gx_bitmap_id id, const gx_color_index *scolors, const gx_strip_bitmap *textures, const gx_color_index *tcolors, int x, int y, int width, int height, int phase_x, int phase_y, gs_logical_operation_t lop);
static int run_get_bits_rectangle(gx_device *dev, const gs_int_rect *prect, gs_get_bits_params_t *params, gs_int_rect **unread);
...
gdev_run_from_mem(gx_device_run *rdev, gx_device_memory *mdev)
{
    ...
    int min_runs = (mdev->width + (((1 << (32 - 2 * 10)) - 1) - 1)) / ((1 << (32 - 2 * 10)) - 1) + 2;
    int i;
    gx_color_index white = gx_device_white((gx_device *)mdev);

    rdev->md = *mdev;
    if (runs_per_line > (1 << 10))
        runs_per_line = (1 << 10);
    if (runs_per_line < min_runs)
        return 0;
    for (i = 0; i < mdev->height; ++i) {
        run_line *line = ((run_line *)((&(rdev)->md)->line_ptrs[i]));
    ...
    (rdev->save_procs.copy_mono = ((&rdev->md)->procs.copy_mono), ((&rdev->md)->procs.copy_mono = (run_copy_mono)));
    (rdev->save_procs.copy_color = ((&rdev->md)->procs.copy_color), ((&rdev->md)->procs.copy_color = (run_copy_color)));
    (rdev->save_procs.fill_rectangle = ((&rdev->md)->procs.fill_rectangle), ((&rdev->md)->procs.fill_rectangle = (run_fill_rectangle)));
    (rdev->save_procs.copy_alpha = ((&rdev->md)->procs.copy_alpha), ((&rdev->md)->procs.copy_alpha = (run_copy_alpha)));
    (rdev->save_procs.strip_tile_rectangle = ((&rdev->md)->procs.strip_tile_rectangle), ((&rdev->md)->procs.strip_tile_rectangle = (run_strip_tile_rectangle)));
    (rdev->save_procs.strip_copy_rop = ((&rdev->md)->procs.strip_copy_rop), ((&rdev->md)->procs.strip_copy_rop = (run_strip_copy_rop)));
    (rdev->save_procs.get_bits_rectangle = ((&rdev->md)->procs.get_bits_rectangle), ((&rdev->md)->procs.get_bits_rectangle = (run_get_bits_rectangle)));
...
{
    const run_line *line = ((run_line *)((&(rdev)->md)->line_ptrs[y]));
    const run *const data = ((const run *)((line) + 1));
    const_run_ptr rp;
    int n, x, w;
    ...
    gx_color_index value[(1 << 10)];

    if (line->zero != ((gx_color_index)(~0))) {
        rdev->save_procs.fill_rectangle((gx_device *)&rdev->md,
                                        0, y, rdev->md.width, 1, line->zero);
        return 0;
    }

    for (n = 0, ((rp).index = (data)[1].next, (rp).ptr = (data) + (rp).index); !((rp).index == 0);
         ++n, ((rp).ptr = (data) + ((rp).index = ((rp).ptr->next)))
        ) {
        length[n] = ((rp).ptr->length);
        value[n] = ((rp).ptr->value);
    }
...
run_standardize(gx_device_run *rdev, int y, int h)
{
    ...
    do { if ( y < 0 ) h += y, y = 0; } while (0);
    do { if ( h > (&rdev->md)->height - y ) h = (&rdev->md)->height - y; } while (0);
    ye = y + h;
    if (y < rdev->smin) {
        if (ye > rdev->smax1)
...
{
    run_line *line = ((run_line *)((&(rdev)->md)->line_ptrs[y]));
    run *data = ((run *)((line) + 1));
    int left = rdev->md.width;
    run_index index = 2;
    run *rcur;

    line->zero = ((gx_color_index)(~0));
    data[0].length = ((1 << (32 - 2 * 10)) - 1);
    data[0].value = ((gx_color_index)(~0));
    data[1].length = ((1 << (32 - 2 * 10)) - 1);
    data[1].value = ((gx_color_index)(~0));
    data[1].next = 2;
    rcur = data + index;
    for (; left > 0; index++, rcur++, left -= ((1 << (32 - 2 * 10)) - 1)) {
        rcur->length = (((left) < (((1 << (32 - 2 * 10)) - 1))) ? (left) : (((1 << (32 - 2 * 10)) - 1)));
        rcur->value = 0;
        rcur->prev = index - 1;
        rcur->next = index + 1;
    }
...
run_fill_interval(run_line *line, int xo, int xe, run_value new)
{
    ...
    if (xo < xc) {
        while (xo < xc)
            ((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->prev))), xc -= ((rpc).ptr->length);
    } else {
        while (xo >= xc + ((rpc).ptr->length))
            xc += ((rpc).ptr->length), ((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->next)));
    }
    ...
    for (; !((rpc).index == 0) && ((rpc).ptr->value) == new;
         ((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->next)))
        )
        if ((xo = xc += ((rpc).ptr->length)) >= xe)
            return 0;
    x0 = xc, rp0 = rpc;



    while (xe > xc + ((rpc).ptr->length))
        xc += ((rpc).ptr->length), ((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->next)));
    ...
    while (((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->prev))), ((rpc).ptr->value) == new)
        xe = xc -= ((rpc).ptr->length);
    ((rpc).ptr = (data) + ((rpc).index = ((rpc).ptr->next)));
# 466 "./base/gdevmrun.c"
    if (x0 < xo) {
        uint diff = xo - x0;
        run_value v0 = ((rp0).ptr->value);
        run_ptr rpp;

        ((rpp).ptr = (data) + ((rpp).index = ((rp0).ptr->prev)));
        if (((rpp).ptr->value) == v0 && ((rpp).ptr->length) + diff <= ((1 << (32 - 2 * 10)) - 1))
            ((rpp).ptr->length) += diff;
        else {
    ...
    x1 = xc + ((rpc).ptr->length);
    if (x1 > xe) {
        uint diff = x1 - xe;
        run_value vc = ((rpc).ptr->value);
        run_ptr rpn;

        ((rpn).ptr = (data) + ((rpn).index = ((rpc).ptr->next)));
        if (((rpn).ptr->value) == vc && ((rpn).ptr->length) + diff <= ((1 << (32 - 2 * 10)) - 1))
            ((rpn).ptr->length) += diff;
        else {
    ...
    ((rp0).ptr = (data) + ((rp0).index = ((rp0).ptr->prev)));
    while (((rp0).ptr->next) != ((rpc).ptr->next))
        rp_delete_next(&rp0, data, line);
# 519 "./base/gdevmrun.c"
    {
        uint left = xe - xo;

        if (xo == x0 && ((rp0).ptr->value) == new &&
            ((rp0).ptr->length) + left <= ((1 << (32 - 2 * 10)) - 1)
            )
            ((rp0).ptr->length) += left;
        else {
            ...
            if (left <= ((1 << (32 - 2 * 10)) - 1))
                ...
                int pieces = left + (((1 << (32 - 2 * 10)) - 1) - 1) / ((1 << (32 - 2 * 10)) - 1);
            ...
            do {
                ...
                ((rpn).ptr->length) = (((left) < (len)) ? (left) : (len));
                ((rpn).ptr->value) = new;
            }
            while ((left -= len) > 0);
        }
                   ...
                   gx_color_index color)
{
    ...
    if (y >= rdev->umin && ye <= rdev->umax1 &&
        color == ((run_line *)((&(rdev)->md)->line_ptrs[y]))->zero
        )
        return 0;
    ...
    if (y < rdev->smax1 && ye > rdev->smin) {
        ...
        rdev->save_procs.fill_rectangle((gx_device *)&rdev->md,
                                        x, y, w, (((ye) < (rdev->smax1)) ? (ye) : (rdev->smax1)) - y,
                                        color);
        if (ye <= rdev->smax1)
            return 0;
        y = rdev->smax1;
    ...
    xe = x + w;
    for (iy = y; iy < ye; ++iy) {
        run_line *line = ((run_line *)((&(rdev)->md)->line_ptrs[iy]));
