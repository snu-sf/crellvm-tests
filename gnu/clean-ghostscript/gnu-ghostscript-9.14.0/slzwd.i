...
static const gs_memory_struct_type_t st_lzw_decode = { sizeof(lzw_decode), "lzw_decode", 0, 0, gs_no_struct_enum_ptrs, gs_no_struct_reloc_ptrs, 0, 0 };
...
s_LZWD_reset(stream_state * st)
{
    ...
    ss->bits_left = 0;
    ss->bytes_left = 0;
    ss->next_code = (code_escape + 2);
    ss->code_size = ss->InitialCodeLength + 1;
    ss->prev_code = -1;
    ss->copy_code = -1;
    dc[(code_escape + 0)].len = 255;
    dc[(code_escape + 1)].len = 255;
    for (i = 0; i < code_escape; i++, dc++)
        dc->datum = i, dc->len = 1, dc->prefix = (code_escape + 1);
    return 0;
}
static int
s_LZWD_init(stream_state * st)
{
    stream_LZW_state *const ss = (stream_LZW_state *) st;
    lzw_decode *dc =
    (lzw_decode *)(*(st->memory)->procs.alloc_struct_array)(st->memory, 4096 + 1, &st_lzw_decode, "LZWDecode(init)");
               ...
               stream_cursor_write * pw, bool last)
{
    ...
    code_mask = (1 << code_size) - 1, switch_code = code_mask + 1 - ss->EarlyChange;
    if (!ss->BlockData)
...
reset:
    ...
    if (len == 255) {
        ...
        if (code == (code_escape + 0)) {
            do { } while (0);

            next_code = (code_escape + 2);
            dc_next = table + (code_escape + 2);
  ...
  add:
    if (prev_code >= 0) {
        ...
        dc_next->datum = b;
        dc_next->len = (((prev_len) < (254)) ? (prev_len) : (254)) + 1;
        dc_next->prefix = prev_code;
        dc_next++;
        do { } while (0);
