...
# 26 "./base/gdevpipe.c" 2


static int pipe_fopen(gx_io_device *iodev, const char *fname, const char *access, FILE **pfile, char *rfname, uint rnamelen);
static int pipe_fclose(gx_io_device *iodev, FILE *file);
const gx_io_device gs_iodev_pipe = {
    "%pipe%", "Special",
    {iodev_no_init, iodev_no_open_device,
     ((void*)0) , pipe_fopen, pipe_fclose,
           ...
           FILE ** pfile, char *rfname, uint rnamelen)
{
    ...
    if ((__extension__ (__builtin_constant_p ('+') && !__builtin_constant_p (access) && ('+') == '\0' ? (char *) __rawmemchr (access, '+') : __builtin_strchr (access, '+'))))
        return ((-9));
