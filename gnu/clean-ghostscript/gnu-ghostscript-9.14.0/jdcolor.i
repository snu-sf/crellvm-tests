...

{
  ...
  for (i = 0, x = -128; i <= 255; i++, x++) {
    ...
    cconvert->Cr_g_tab[i] = (- ((INT32) ((0.714136286) * (1L<<16) + 0.5))) * x;
...

{
  ...
  for (i = 0, x = -128; i <= 255; i++, x++) {
    ...
    cconvert->Cr_r_tab[i] = (int)
      ((((INT32) ((2.804) * (1L<<16) + 0.5)) * x + ((INT32) 1 << (16 -1))) >> (16));

    cconvert->Cb_b_tab[i] = (int)
      ((((INT32) ((3.544) * (1L<<16) + 0.5)) * x + ((INT32) 1 << (16 -1))) >> (16));

    cconvert->Cr_g_tab[i] = (- ((INT32) ((1.428272572) * (1L<<16) + 0.5))) * x;


    cconvert->Cb_g_tab[i] = (- ((INT32) ((0.688272572) * (1L<<16) + 0.5))) * x + ((INT32) 1 << (16 -1));
  }
}
# 203 "./obj/jdcolor.c"
static void
...
build_rgb_y_table (j_decompress_ptr cinfo)
{
  ...
  cconvert->rgb_y_tab = rgb_y_tab = (INT32 *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1,
    ((3*(255 +1)) * ((size_t) sizeof(INT32))));

  for (i = 0; i <= 255; i++) {
    rgb_y_tab[i+0] = ((INT32) ((0.299) * (1L<<16) + 0.5)) * i;
    rgb_y_tab[i+(1*(255 +1))] = ((INT32) ((0.587) * (1L<<16) + 0.5)) * i;
    rgb_y_tab[i+(2*(255 +1))] = ((INT32) ((0.114) * (1L<<16) + 0.5)) * i + ((INT32) 1 << (16 -1));
  }
}
