                ...
                bool defaultval, bool * pvalue)
{
    ...
    if (!((((const byte *)&((pdval)->tas.type_attrs))[1]) == (t_boolean)))
        return ((-20));
                    ...
                    int maxval, int defaultval, int *pvalue)
{
    ...
    if (pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0) {
        ...
        switch ((((const byte *)&((pdval)->tas.type_attrs))[1])) {
            case t_integer:
                ...
                uint minval, uint maxval, uint defaultval, uint * pvalue)
{
    ...
    if (pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0) {
        ...
        do { if ( !((((const byte *)&((&*pdval)->tas.type_attrs))[1]) == (t_integer)) ) return ((-20)); } while (0);
   ...
   const char *kstr, uint len, int *ivec, int under_error, int over_error)
{
    ...
    if (pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0)
        return 0;
    if (!(((pdval)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0))) == (((t_array) << 8) + (0))))
        return ((-20));
    size = ((pdval)->tas.rsize);
    if (size > len)
        return (over_error);
    for (i = 0; i < size; i++) {
        ...
        switch ((((const byte *)&((&pa)->tas.type_attrs))[1])) {
            case t_integer:
                ...
                break;
            case t_real:
                if (pa.value.realval < (-1 << ((1 << 2) * 8 - 1)) ||
                    pa.value.realval > (~(-1 << ((1 << 2) * 8 - 1))) ||
...
{
    ref *val;
    int code = dict_float_array_check_param(((gs_memory_t *)((i_ctx_p->memory).current)), pdict, kstr, maxlen,
                              fvec, defaultvec, (-15), (-15));
    if (code < 0) {
                ...
                bool defaultval)
{
    ...
    if (pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0) {
        if (defaultval)
            ((pproc)->value.const_refs = ((const ref *)((void*)0)), ((pproc)->tas.type_attrs = ((t_array) << 8) + ((0x20 + 0x40) + 0x80)), ((pproc)->tas.rsize = (0)));
        else
            (((pproc)->tas.type_attrs = ((t__invalid) << 8) + (0)) );
        return 1;
    }
    do { if ( !(((&*pdval)->tas.type_attrs & ((((1 << 6) - (4)) << 8) + (0x40 +0x80))) == (((t_array) << 8) + (0x40 +0x80))) ) return (check_proc_failed(&*pdval)); } while (0);
    *pproc = *pdval;
    return 0;
}
               ...
               gs_memory_t * mem, const i_ctx_t *i_ctx_p)
{
    ...
    if (pdict == 0) {
        ((puid)->id = (~(-1L << ((1 << 3) * 8 - 1))), (puid)->xvalues = 0);
        return defaultval;
    }
    ...
    if (((i_ctx_p->language_level) >= 2) &&
        ...
        if (!((((const byte *)&((puniqueid)->tas.type_attrs))[1]) == (t_array)))
            return ((-20));
        size = ((puniqueid)->tas.rsize);
        if (size == 0)
            return ((-15));
        xvalues = (long *)(*(mem)->procs.alloc_byte_array)(mem, size, sizeof(long), "get XUID");
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!((((const byte *)&((pvalue)->tas.type_attrs))[1]) == (t_integer))) {
                ((mem)->procs.free_object(mem, xvalues, "get XUID"));
                return ((-20));
            }
            xvalues[i] = pvalue->value.intval;
        }
        ((puid)->id = -(long)(size), (puid)->xvalues = xvalues);
        return 1;
    }
    ...
    if (dict_find_string(pdict, "UniqueID", &puniqueid) <= 0) {
        ...
        if (!((((const byte *)&((puniqueid)->tas.type_attrs))[1]) == (t_integer)))
           return ((-20));
        ...
        if (puniqueid->value.intval == 0) {
            ...
            ((puid)->id = puniqueid->value.intval, (puid)->xvalues = 0);
    }
    return 0;
}
...
dict_check_uid_param(const ref * pdict, const gs_uid * puid)
{
    ...
    if (((puid)->id < 0)) {
        uint size = ((uint)(-(puid)->id));
        uint i;

        if (dict_find_string(pdict, "XUID", &puniqueid) <= 0)
            return ((bool)0);
        if (!((((const byte *)&((puniqueid)->tas.type_attrs))[1]) == (t_array)) ||
            ((puniqueid)->tas.rsize) != size
            )
            return ((bool)0);
        for (i = 0; i < size; i++) {
            const ref *pvalue = puniqueid->value.const_refs + i;
            ...
            if (!((((const byte *)&((pvalue)->tas.type_attrs))[1]) == (t_integer)))
                return ((bool)0);
            if (pvalue->value.intval != ((puid)->xvalues)[i])
                return ((bool)0);
        }
        return ((bool)1);
    } else {
        if (dict_find_string(pdict, "UniqueID", &puniqueid) <= 0)
            return ((bool)0);
        return (((((const byte *)&((puniqueid)->tas.type_attrs))[1]) == (t_integer)) &&
...
gs_errorinfo_put_pair(i_ctx_t *i_ctx_p, const char *key, int len, const ref *any)
{
    ...
    code = names_ref(((gs_memory_t *)((i_ctx_p->memory).spaces.memories.named.local))->gs_lib_ctx->gs_name_table, (const byte *)key, len, &key_name, 0);
    if (code < 0)
        return code;
    code = gs_alloc_ref_array(((i_ctx_p->memory).spaces.memories.named.local), &pair, (0x20 + 0x40), 2, "gs_errorinfo_put_pair");
    if (code < 0)
        return code;
    aptr = pair.value.refs;
    ((void)(((*(aptr) = *(&key_name)), ((aptr)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    ((void)(((*(aptr+1) = *(any)), ((aptr+1)->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask))));
    if (dict_find_string((&(i_ctx_p->dict_stack).system_dict), "$error", &pderror) <= 0 ||
        !((((const byte *)&((pderror)->tas.type_attrs))[1]) == (t_dictionary)) ||
        dict_put_string(pderror, "errorinfo", &pair, &(i_ctx_p->dict_stack)) < 0
        )
        return ((-100));
    return 0;
}
