...
zpackedarray(i_ctx_t *i_ctx_p)
{
    ...
    if (op->value.intval < 0)
        return ((-15));
    if (op->value.intval > op - (((i_ctx_p->op_stack).stack).bot) &&
        op->value.intval >= ref_stack_count(&((i_ctx_p->op_stack).stack))
        )
        return ((-17));
    (((i_ctx_p->op_stack).stack).p)--;
    code = make_packed_array(&parr, &((i_ctx_p->op_stack).stack), (uint) op->value.intval,
                             (&(i_ctx_p->memory)), "packedarray");
    (((i_ctx_p->op_stack).stack).p)++;
    if (code >= 0)
        *(((i_ctx_p->op_stack).stack).p) = parr;
    return code;
}
...
zsetpacking(i_ctx_t *i_ctx_p)
{
    ...
    ((&cont)->value.pstruct = ((obj_header_t *)(i_ctx_p)), ((&cont)->tas.type_attrs = ((t_struct) << 8) + (avm_local)) );
    (((void)((((((&(i_ctx_p->array_packing))->tas.type_attrs) & ((&(i_ctx_p->memory)))->test_mask) == 0) ? alloc_save_change((&(i_ctx_p->memory)), &cont, (ref_packed *)(&(i_ctx_p->array_packing)), "setpacking") : 0))), ((void)(((*(&(i_ctx_p->array_packing)) = *(op)), ((&(i_ctx_p->array_packing))->tas.type_attrs |= ((&(i_ctx_p->memory)))->new_mask)))));
                  ...
                  gs_dual_memory_t *idmemory, client_name_t cname)
{
    ...
    for (i = size; i != 0; i--) {
        ...
        switch (((pref)->tas.type_attrs >= (t_next_index << 8) ? t_operator : (((const byte *)&((pref)->tas.type_attrs))[1]))) {
            case t_name:
              if (names_index(imem->gs_lib_ctx->gs_name_table, pref) >= ((1 << 12) - 1))
                    break;
                idest++;
                continue;
            case t_integer:
                if (pref->value.intval < (-(1 << (12 - 1))) ||
                    pref->value.intval > ((1 << (12 - 1)) - 1)
                    )
                    break;
                idest++;
                continue;
            case t_oparray:
                ...
                if ( (avm_space)(((pref)->tas.type_attrs) & (((1 << 2) - 1) << 2)) > (space) ) return ((-7));
            ...
            case t_operator:
                {
                    ...
                    if (!(((pref)->tas.type_attrs) & (0x80)))
                        break;
                    oidx = (((pref)->tas.rsize) == 0 ? op_find_index(pref) : ((pref)->tas.rsize));
                    if (oidx == 0 || oidx > ((1 << 12) - 1))
                        break;
                }
                idest++;
                continue;
        ...
        {
            int i = (idest - ishort) & (((((8 - 1) | (4 - 1) | (8 - 1)) + 1) / 2) - 1);

            if (ishort == 0)
                idest += skip = -i & (((((8 - 1) | (4 - 1) | (8 - 1)) + 1) / 2) - 1);
            else
                idest += ((sizeof(ref) / sizeof(ref_packed)) - 1) * i;
        }
        ishort = idest += (sizeof(ref) / sizeof(ref_packed));
    }
    pad = ((sizeof(ref) / sizeof(ref_packed)) - idest % (sizeof(ref) / sizeof(ref_packed))) % (sizeof(ref) / sizeof(ref_packed));



    code = gs_alloc_ref_array(imem, &rtemp, 0, (idest + pad) / (sizeof(ref) / sizeof(ref_packed)),
                              cname);
    if (code < 0)
        return code;
    ...
    pshort = pbody;
    for (; skip; skip--)
        *pbody++ = ((ref_packed)(pt_integer) << 13);
    pdest = pbody;
    ...
    for (i = size; i != 0; i--) {
        ...
        switch (((pref)->tas.type_attrs >= (t_next_index << 8) ? t_operator : (((const byte *)&((pref)->tas.type_attrs))[1]))) {
            case t_name:
                {
                    uint nidx = names_index(imem->gs_lib_ctx->gs_name_table, pref);

                    if (nidx >= ((1 << 12) - 1))
                        break;
                    *pdest++ = nidx +
                        ((((pref)->tas.type_attrs) & (0x80)) ?
                         ((ref_packed)(pt_executable_name) << 13) :
                         ((ref_packed)(pt_literal_name) << 13));
                }
                continue;
            case t_integer:
                if (pref->value.intval < (-(1 << (12 - 1))) ||
                    ...
                    break;
                *pdest++ = ((ref_packed)(pt_integer) << 13) +
                    ((short)pref->value.intval - (-(1 << (12 - 1))));
                continue;
            case t_oparray:
            case t_operator:
                {
                    ...
                    if (oidx == 0 || oidx > ((1 << 12) - 1))
                        break;
                    *pdest++ = ((ref_packed)(pt_executable_operator) << 13) + oidx;
                }
                continue;
        }
        ...
        {
            int i = (pdest - pshort) & (((((8 - 1) | (4 - 1) | (8 - 1)) + 1) / 2) - 1);
            const ref_packed *psrc = pdest;
            ref *pmove =
            (ref *) (pdest += ((sizeof(ref) / sizeof(ref_packed)) - 1) * i);

            ((void)(((*(pmove) = *(pref)), ((pmove)->tas.type_attrs |= (idmemory)->new_mask))));
            while (--i >= 0) {
                --psrc;
                --pmove;
        ...
        pshort = pdest += (sizeof(ref) / sizeof(ref_packed));
    }

    {
        ...
        ((parr)->value.packed = (pbody + skip), ((parr)->tas.type_attrs = ((atype) << 8) + (((0x20 + 0x40) | space)|(idmemory->new_mask))), ((parr)->tas.rsize = (size)));
