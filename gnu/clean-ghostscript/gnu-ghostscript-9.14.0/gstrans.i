...
# 53 "./base/gstrans.c"
    ...
    if (mode < 0 || mode > BLEND_MODE_Color)
        return ((-15));
    pgs->blend_mode = mode;
    return 0;
}
...
{
    ptgp->ColorSpace = ((void*)0);
    ptgp->Isolated = ((bool)0);
    ptgp->Knockout = ((bool)0);
    ptgp->image_with_SMask = ((bool)0);
    ptgp->mask_id = 0;
    ptgp->iccprofile = ((void*)0);
}
                            ...
                            const gs_rect *pbbox)
{
    ...
    blend_color_space = ((pgs)->color[0].color_space);
    if (gs_color_space_get_index(blend_color_space) > gs_color_space_index_DeviceCMYK) {
        ...
        blend_color_space = (*(blend_color_space)->type->concrete_space)(blend_color_space, pis);
    }
...
# 271 "./base/gstrans.c"
        if ( gs_color_space_is_ICC(blend_color_space) ) {
            ...
            switch ((*(blend_color_space)->type->num_components)(blend_color_space)) {
                case 1:
                ...
                default:
                    ...
                    params.group_color_numcomps = (*(blend_color_space)->type->num_components)(blend_color_space);
                break;
            }
            if (profile != ((void*)0)) {
                params.group_color = ICC;
                                ...
                                const gs_pdf14trans_params_t * pparams)
{
    ...
    bbox = pparams->bbox;
# 375 "./base/gstrans.c"
    if (((pdev)->procs.begin_transparency_group) != 0)
        return (*((pdev)->procs.begin_transparency_group)) (pdev, &tgp, &bbox, pis,
                                                            ((void*)0));
    else
        return 0;
}
...
gs_end_transparency_group(gs_state *pgs)
{
    ...
    do { } while (0);
    params.pdf14_op = PDF14_END_TRANS_GROUP;
    return gs_state_update_pdf14trans(pgs, &params);
}
...
{
    do { } while (0);
    if (((pdev)->procs.end_transparency_group) != 0)
        return (*((pdev)->procs.end_transparency_group)) (pdev, pis);
    else
        return 0;
}
...
{
    do { } while (0);
    if (((pdev)->procs.pop_transparency_state) != 0)
        return (*((pdev)->procs.pop_transparency_state)) (pdev, pis);
    else
        return 0;
}
...
{
    do { } while (0);
    if (((pdev)->procs.push_transparency_state) != 0)
        return (*((pdev)->procs.push_transparency_state)) (pdev, pis);
    else
        return 0;
}
                          ...
                          gs_transparency_mask_subtype_t subtype)
{
    ...
    ptmp->replacing = ((bool)0);
    ptmp->iccprofile = ((void*)0);
}
...
# 556 "./base/gstrans.c"
    ...
    do { if (blend_color_space->cmm_icc_profile_data) do { (blend_color_space->cmm_icc_profile_data)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    do { } while (0);
    ...
    if ( blend_color_space->cmm_icc_profile_data != ((void*)0) ) {
        ...
        params.iccprofile = blend_color_space->cmm_icc_profile_data;
        params.icc_hash = blend_color_space->cmm_icc_profile_data->hashcode;
        do { if (params.iccprofile) do { (params.iccprofile)->rc.ref_count++; do { } while (0); } while (0); } while (0);
    } else {
        params.group_color = GRAY_SCALE;
        params.group_color_numcomps = 1;
    }
                                ...
                                const gs_pdf14trans_params_t * pparams)
{
    ...
    if (tmp.group_color == ICC ) {
        ...
        tmp.iccprofile = ((void*)0);
        tmp.icc_hashcode = 0;
    }
    memcpy(tmp.transfer_fn, pparams->transfer_fn, ((int)(sizeof(tmp.transfer_fn))));
    do { } while (0);
# 641 "./base/gstrans.c"
    if (((pdev)->procs.begin_transparency_mask) != 0)
        return (*((pdev)->procs.begin_transparency_mask))
                        (pdev, &tmp, &(pparams->bbox), pis, ((void*)0));
    else
        return 0;
}
...
# 670 "./base/gstrans.c"
    pis->trans_flags.xstate_change = ((bool)1);
    do { } while (0);

    do { } while (0);


    params.pdf14_op = PDF14_END_TRANS_MASK;
    params.csel = csel;
                                ...
                                const gs_pdf14trans_params_t * pparams)
{
    ...
    if (((pdev)->procs.end_transparency_mask) != 0)
        return (*((pdev)->procs.end_transparency_mask)) (pdev, pis);
    else
        return 0;
}
...
{
    gx_device * dev = pgs->device;
    gs_devn_params * pclist_devn_params = ((dev)->procs.ret_devn_params)(dev);
# 719 "./base/gstrans.c"
...
gs_push_pdf14trans_device(gs_state * pgs, bool is_pattern)
{
    ...
    code = ((pgs->device)->procs.get_profile)(pgs->device, &dev_profile);
    if (code < 0)
        return code;
...
gs_pop_pdf14trans_device(gs_state * pgs, bool is_pattern)
{
    ...
    params.is_pattern = is_pattern;
    params.pdf14_op = PDF14_POP_DEVICE;
    return gs_state_update_pdf14trans(pgs, &params);
}
...
gs_abort_pdf14trans_device(gs_state * pgs)
{
    ...
    params.pdf14_op = PDF14_ABORT_DEVICE;
    return gs_state_update_pdf14trans(pgs, &params);
}
...
{
    do { } while (0);
    if (((pdev)->procs.discard_transparency_layer) != 0)
    return (*((pdev)->procs.discard_transparency_layer)) (pdev, pis);
    else
    return 0;
}
