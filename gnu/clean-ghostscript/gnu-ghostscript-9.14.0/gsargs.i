...
static int get_codepoint_utf8(FILE *file, const char **astr)
{
    ...
    do {
        c = (file ? fgetc(file) : (**astr ? (int)(unsigned char)*(*astr)++ : (-1)));
        if (c == (-1))
            return (-1);
        if (c < 0x80)
            return c;
...
arg_push_decoded_memory_string(arg_list * pal, char *str, bool parsed, bool decoded, gs_memory_t * mem)
{
    ...
    if (pal->depth == 10) {
        (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./base/gsargs.c", 148), errprintf_nomem("Too much nesting of @-files.\n"));
        return 1;
    }
...
arg_finit(arg_list * pal)
{
    while (pal->depth) {
        ...
        if (pas->is_file)
            ...
            ((pas->u.s.memory)->procs.free_object(pas->u.s.memory, pas->u.s.chars, "arg_finit"));
    }
}
...
arg_next(arg_list * pal, int *code, const gs_memory_t *errmem)
{
  ...
  top:pas = &pal->sources[pal->depth - 1];
    ...
    if (pas->is_file)
        ...
        if (strlen(pas->u.s.str) >= 2048) {
            ...
            *code = (-100);
            ...
            result = pal->cstr;
            if (pas->u.s.memory)
                ((pas->u.s.memory)->procs.free_object(pas->u.s.memory, pas->u.s.chars, "arg_next"));
            pal->depth--;
            pas--;
            goto at;
        }
    else
        astr = pas->u.s.str, f = ((void*)0);
    ...
    for (i = 0;;) {
        ...
        if (c > 0 && c < 256 && ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISspace)) {
            if (i == 0) {
                c = get_codepoint(f, &astr, pal, pas);
                continue;
            }
...
{
    char *sstr = (char *)(*(mem)->procs.alloc_bytes)(mem, strlen(str) + 1, "arg_copy");

    if (sstr == 0) {
        (eprintf_program_ident(gs_program_name(), gs_revision_number()), lprintf_file_and_line("./base/gsargs.c", 347), errprintf_nomem("Out of memory!\n"));
        return ((void*)0);
    }
    strcpy(sstr, str);
    return sstr;
...
{
    ((mem)->procs.free_object(mem, str, "arg_copy"));
}
...
int arg_strcmp(arg_list *pal, const char *arg, const char *match)
{
    ...
    if (!arg || !match)
        return 1;
    do {
        rune = pal->get_codepoint(((void*)0), &arg);
        if (rune == -1)
            rune = 0;
        c = *match++;
        if (rune != c)
