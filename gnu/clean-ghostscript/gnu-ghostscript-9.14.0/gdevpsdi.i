# 1 "./devices/vector/gdevpsdi.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 316 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "./devices/vector/gdevpsdi.c" 2
# 18 "./devices/vector/gdevpsdi.c"
# 1 "./base/stdio_.h" 1
# 26 "./base/stdio_.h"
# 1 "./base/std.h" 1
# 22 "./base/std.h"
...
# 83 "./base/stdio_.h" 2
# 19 "./devices/vector/gdevpsdi.c" 2
# 1 "./obj/jpeglib_.h" 1
# 25 "./obj/jpeglib_.h"
# 1 "./obj/jconfig.h" 1
# 507 "./obj/jconfig.h"
...
# 20 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/math_.h" 1
# 29 "./base/math_.h"
...
# 21 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/string_.h" 1
# 32 "./base/string_.h"
...
# 33 "./base/string_.h" 2
# 22 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/gx.h" 1
# 23 "./base/gx.h"
# 1 "./base/gserrors.h" 1
# 50 "./base/gserrors.h"
...
# 23 "./devices/vector/gdevpsdi.c" 2
...
# 25 "./devices/vector/gdevpsdi.c" 2
...
# 26 "./devices/vector/gdevpsdi.c" 2
...
# 27 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/gxdevmem.h" 1
# 81 "./base/gxdevmem.h"
...
# 28 "./devices/vector/gdevpsdi.c" 2
...
# 30 "./devices/vector/gdevpsdi.c" 2
...
# 33 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/spngpx.h" 1
# 33 "./base/spngpx.h"
...
# 34 "./devices/vector/gdevpsdi.c" 2
# 1 "./base/szlibx.h" 1
# 23 "./base/szlibx.h"
...
# 35 "./devices/vector/gdevpsdi.c" 2
...
# 36 "./devices/vector/gdevpsdi.c" 2
             ...
             int bpc_in, int bpc_out)
{
    ...
    if (code < 0) {
        ((mem)->procs.free_object(mem, st, "pixel_resize state"));
        return code;
    }
                  ...
                  stream_state *st)
{
    ...
    int code;
    float c[4][3];
    const float MIN_FLOAT = - ((0x1000000 - 1.0) * 0x1000000 * 0x1000000 * 0x10000000 * 0x10000000);
    ...
    *param = list;
    ...
    gs_make_mem_device(&mdev, gdev_mem_device_for_bits(24), pdev->memory, 0, ((void*)0));
    ...
    gsicc_init_device_profile_struct((gx_device *) &mdev, ((void*)0), 0);
    if (pis) {
        ...
        if (code < 0)
            return code;
        cc.paint.values[0] = ((0x1000000 - 1.0) * 0x1000000 * 0x1000000 * 0x10000000 * 0x10000000); cc.paint.values[1] = MIN_FLOAT; cc.paint.values[2] = MIN_FLOAT;
        code = convert_color((gx_device *)&mdev, pcs, pis, &cc, c[0]);
        if (code < 0)
            return code;
        cc.paint.values[0] = MIN_FLOAT; cc.paint.values[1] = ((0x1000000 - 1.0) * 0x1000000 * 0x1000000 * 0x10000000 * 0x10000000); cc.paint.values[2] = MIN_FLOAT;
        code = convert_color((gx_device *)&mdev, pcs, pis, &cc, c[1]);
        if (code < 0)
            return code;
        cc.paint.values[0] = MIN_FLOAT; cc.paint.values[1] = MIN_FLOAT; cc.paint.values[2] = ((0x1000000 - 1.0) * 0x1000000 * 0x1000000 * 0x10000000 * 0x10000000);
        ...
        c[0][0] = ((c[0][0]) < 0 ? -(c[0][0]) : (c[0][0])); c[0][1] = ((c[0][1]) < 0 ? -(c[0][1]) : (c[0][1])); c[0][2] = ((c[0][2]) < 0 ? -(c[0][2]) : (c[0][2]));
        c[1][0] = ((c[1][0]) < 0 ? -(c[1][0]) : (c[1][0])); c[1][1] = ((c[1][1]) < 0 ? -(c[1][1]) : (c[1][1])); c[1][2] = ((c[1][2]) < 0 ? -(c[1][2]) : (c[1][2]));
        c[2][0] = ((c[2][0]) < 0 ? -(c[2][0]) : (c[2][0])); c[2][1] = ((c[2][1]) < 0 ? -(c[2][1]) : (c[2][1])); c[2][2] = ((c[2][2]) < 0 ? -(c[2][2]) : (c[2][2]));
        ...
        cc.paint.values[0] = ((0x1000000 - 1.0) * 0x1000000 * 0x1000000 * 0x10000000 * 0x10000000); cc.paint.values[1] = cc.paint.values[2] = 0;
        ...
        c[0][1] = ((c[0][1]) < 0 ? -(c[0][1]) : (c[0][1])); c[0][2] = ((c[0][2]) < 0 ? -(c[0][2]) : (c[0][2]));
        c[1][1] = ((c[1][1]) < 0 ? -(c[1][1]) : (c[1][1])); c[1][2] = ((c[1][2]) < 0 ? -(c[1][2]) : (c[1][2]));
        c[2][1] = ((c[2][1]) < 0 ? -(c[2][1]) : (c[2][1])); c[2][2] = ((c[2][2]) < 0 ? -(c[2][2]) : (c[2][2]));
    }
                        ...
                        bool lossless)
{
    ...
    if (!pdip->Encode)
        return 0;
    if (pdip->AutoFilter) {
# 264 "./devices/vector/gdevpsdi.c"
        if (lossless) {
            templat = lossless_template;
        } else if (templat == ((void*)0) || templat == &s_zlibE_template ||
                   ...
                   double resolution, bool lossless)
{
    ...
    switch (pdip->DownsampleType) {
        ...
        case ds_Bicubic:
            ...
            break;
        default:
            errprintf(pdev->v_memory,"Unsupported downsample type %d\n", pdip->DownsampleType);
            return ((-15));
    }
        ...
        templat->set_defaults(st);
    {
        ...
        if ((code = setup_image_compression(pbw, pdip, pim, pis, lossless)) < 0 ||
            ...
            ((pdev->v_memory)->procs.free_object(pdev->v_memory, st, "setup_image_compression"));
            return code;
        }
    }
    return 0;
...
{
# 553 "./devices/vector/gdevpsdi.c"
    int code = 0;
    psdf_image_params params;
    int bpc = pim->BitsPerComponent;
    int bpc_out = pim->BitsPerComponent = (((bpc) < (8)) ? (bpc) : (8));
    int ncomp;
    double resolution;
    ...
    if (pctm == 0)
        ...
        resolution = 1.0 / sqrt((double)(pt.x / pdev->HWResolution[0])*(pt.x / pdev->HWResolution[0])+(double)(pt.y / pdev->HWResolution[1])*(pt.y / pdev->HWResolution[1]));
                            ...
                            gs_pixel_image_t *pim, bool in_line)
{
    ...
    ipdev = *pdev;
    ipdev.params.ColorImage.AutoFilter = ((bool)0);
    ipdev.params.ColorImage.Downsample = ((bool)0);
    ipdev.params.ColorImage.Filter = "FlateEncode";
    ipdev.params.ColorImage.filter_template = &s_zlibE_template;
    ipdev.params.ConvertCMYKImagesToRGB = ((bool)0);
    ipdev.params.GrayImage.AutoFilter = ((bool)0);
    ipdev.params.GrayImage.Downsample = ((bool)0);
    ipdev.params.GrayImage.Filter = "FlateEncode";
    ipdev.params.GrayImage.filter_template = &s_zlibE_template;
    return psdf_setup_image_filters(&ipdev, pbw, pim, ((void*)0), ((void*)0), ((bool)1), in_line);
}
                            ...
                            bool colour_conversion)
{
    ...
    ipdev.params.GrayImage.Filter = "FlateEncode";
    ipdev.params.GrayImage.filter_template = &s_zlibE_template;
    return new_setup_image_filters(&ipdev, pbw, pim, ((void*)0), ((void*)0), ((bool)1), in_line, colour_conversion);
}
