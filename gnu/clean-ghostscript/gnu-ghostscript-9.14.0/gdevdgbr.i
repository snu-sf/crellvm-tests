...
# 23 "./base/gdevdgbr.c" 2
# 1 "./base/gxlum.h" 1
# 24 "./base/gdevdgbr.c" 2
# 1 "./base/gdevmem.h" 1
# 21 "./base/gdevmem.h"
# 1 "./base/gxbitops.h" 1
# 108 "./base/gxbitops.h"
...
gx_default_get_bits(gx_device * dev, int y, byte * data, byte ** actual_data)
{
    ...
    int (*save_get_bits)(gx_device *dev, int y, byte *data, byte **actual_data) = ((dev)->procs.get_bits);
    gs_int_rect rect;
    gs_get_bits_params_t params;
    int code;
    ...
    params.options =
        (actual_data ? (1L<<21) : 0) | (1L<<20) |
        ((1L<<22) | (1L<<24) | (1L<<28) |

         (1L<<16) | (1L<<0) | (1L<<4));
    params.x_offset = 0;
    params.raster = ((uint)((((dev->width * dev->color_info.depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
    params.data[0] = data;
    ((dev)->procs.get_bits = (gx_no_get_bits));
    code = (*((dev)->procs.get_bits_rectangle))
        (dev, &rect, &params, ((void*)0));
    if (actual_data)
        *actual_data = params.data[0];
    ((dev)->procs.get_bits = (save_get_bits));
    return code;
}
                          ...
                          const gs_get_bits_params_t *stored)
{
    ...
    if (!(both & ((1L<<16) | (1L<<17) | (1L<<18))))
        return ((bool)0);
    if (stored->options & (1L<<19)) {




        int i;
        int n = (stored->options & (1L<<18) ?
                 dev->color_info.depth : dev->color_info.num_components);

        if (!(requested->options & (1L<<19)) ||
            !(both & ((1L<<17) || (1L<<18)))
            )
            return ((bool)0);
        for (i = 0; i < n; ++i)
    ...
    if (both & (1L<<0))
        return ((bool)1);
    if (both & ((1L<<1) | (1L<<2) | (1L<<3))) {
        if ((both & ((1L<<4) | (1L<<5) | (1L<<6))) && (both & ((1L<<8) | (1L<<9) | (1L<<10) | (1L<<11) | (1L<<12) | (1L<<13))))
            return ((bool)1);
    }
    return ((bool)0);
}
                           ...
                           byte **stored_base)
{
...

    {
        ...
        uint dev_raster = gx_device_raster(dev, ((bool)1));
        uint raster =
            (options & ((1L<<28) | (1L<<30)) ? dev_raster :
             params->raster);
        byte *base;
        ...
        if (h <= 1 || raster == dev_raster) {
            int x_offset =
                (options & (1L<<26) ? x :
                 options & (1L<<24) ? 0 : params->x_offset);
            ...
            if (x_offset == x) {
                ...
                base = stored_base[0] + ((bytes) >> (3));
                params->x_offset = (bit_offset - bytes) / depth;
            }
            params->options =
                (1L<<22) | (1L<<21) | (1L<<28) |
                (stored->options & ~((1L<<16) | (1L<<17) | (1L<<18))) |
                (params->x_offset == 0 ? (1L<<24) : (1L<<25));
            if (both & (1L<<16)) {
                params->options |= (1L<<16);
                params->data[0] = base;
            } else {
                int n =
                    (stored->options & (1L<<18) ?
                       (params->options |= (1L<<18),
                        dev->color_info.depth) :
                       (params->options |= (1L<<17),
                        dev->color_info.num_components));
                int i;

                for (i = 0; i < n; ++i) {
                    if (!(both & (1L<<19)) || stored->data[i] != 0) {
                        params->data[i] = base;
                    }
                    if (i < n-1) {
                 ...
                 const byte * src_base, uint dev_raster)
{
    ...
    int x_offset = (options & (1L<<24) ? 0 : params->x_offset);
    int depth = dev->color_info.depth;
    int bit_x = x * depth;
...
# 286 "./base/gdevdgbr.c"
    ...
    if (options & (1L<<16)) {
        ...
        uint std_raster =
            (options & (1L<<22) ? ((uint)((((end_bit) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) :
             (end_bit + 7) >> 3);
        uint raster =
            (options & (1L<<28) ? std_raster : params->raster);
        ...
        if (!(skew & 7) && direct_copy) {
            ...
            for (; h > 0; line_ptr += raster, src += dev_raster, --h) {
                ...
                int align = ((uint)( ((const char *)(line_ptr) - (const char *)0) & (((1 << 3)) - 1) ));

                tdev.base = line_ptr - align;

                tdev.width = dest_bit_x + (align << 3) + bit_w;
                tdev.height = 1;
                (*((&mem_mono_device)->procs.copy_mono))
                    ((gx_device *) & tdev, src, bit_x, dev_raster, 0L,
                     dest_bit_x + (align << 3), 0, bit_w, 1,
                     (gx_color_index) 0, (gx_color_index) 1);
            }
        } else if (options & ~stored_options & (1L<<0)) {
        ...
        params->options =
            (options & (((1L<<0) | ((1L<<1) | (1L<<2) | (1L<<3))) | ((1L<<4) | (1L<<5) | (1L<<6)))) | (1L<<16) |
            (options & (1L<<0) ? 0 : options & ((1L<<8) | (1L<<9) | (1L<<10) | (1L<<11) | (1L<<12) | (1L<<13))) |
            (options & (1L<<22) ? (1L<<22) : (1L<<23)) |
            (1L<<20) |
            (x_offset == 0 ? (1L<<24) : (1L<<25)) |
            (raster == std_raster ? (1L<<28) : (1L<<29));
    } else if (!(~options &
                 ((1L<<17) | (1L<<19) | (1L<<22))) &&
               (stored_options & (1L<<16)) &&
               ((options & stored_options) & (1L<<0))
               ) {
        ...
        dest.raster =
            (options & (1L<<28) ?
             ((uint)(((((x_offset + w) * dest_depth) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) : params->raster);
                          ...
                          int x_offset, uint raster)
{
    ...
    int ncolors =
        (stored->options & (1L<<2) ? 3 :
         stored->options & (1L<<3) ? 4 :
         stored->options & (1L<<1) ? 1 : -1);
    int ncomp = ncolors +
        ((stored->options & ((1L<<5) | (1L<<6))) != 0);
    int src_depth = ((((stored->options) >> 8) & 0xf) | "\000\000\014\020"[((stored->options) >> 12) & 3]);
    ...
    params->options &= ~((1L<<0) | ((1L<<1) | (1L<<2) | (1L<<3))) | (1L<<0);
    for (; h > 0; dest_line += raster, src_line += dev_raster, --h) {
        int i;

        const byte *src = (src_line); int sbit = (src_bit_offset & 7);

        byte *dest = (dest_line); int dbit = (dest_bit_offset & 7); byte dbyte = ((dbit) ? (byte)(*(dest) & (0xff00 >> (dbit))) : 0);
        ...
        for (i = 0; i < w; ++i) {
            int j;
            frac sc[4], dc[(8 * 8)];
            gx_color_value v[(8 * 8)], va = alpha_default;
            gx_color_index pixel;
            bool do_alpha = ((bool)0);
            const gx_cm_color_map_procs * map_procs;

            map_procs = ((dev)->procs.get_color_mapping_procs)(dev);


            if (stored->options & (1L<<5)) {
                do { switch ( (src_depth) >> 2 ) { case 0: va = (*(src) >> (8 - (sbit) - (src_depth))) & ((src_depth) | 1); break; case 1: va = (*(src) >> (4 - (sbit))) & 0xf; break; case 2: va = *(src); break; case 3: va = ((sbit) ? ((*(src) & 0xf) << 8) | (src)[1] : (*(src) << 4) | ((src)[1] >> 4)); break; case 4: va = (*(src) << 8) | (src)[1]; break; default: return ((-15)); } } while (0); do { sbit += (src_depth); src += sbit >> 3; sbit &= 7; } while (0);
                va = ((ulong)(va) * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / src_max);
                do_alpha = ((bool)1);
            }
            for (j = 0; j < ncolors; ++j) {
                gx_color_value vj;

                do { switch ( (src_depth) >> 2 ) { case 0: vj = (*(src) >> (8 - (sbit) - (src_depth))) & ((src_depth) | 1); break; case 1: vj = (*(src) >> (4 - (sbit))) & 0xf; break; case 2: vj = *(src); break; case 3: vj = ((sbit) ? ((*(src) & 0xf) << 8) | (src)[1] : (*(src) << 4) | ((src)[1] >> 4)); break; case 4: vj = (*(src) << 8) | (src)[1]; break; default: return ((-15)); } } while (0); do { sbit += (src_depth); src += sbit >> 3; sbit &= 7; } while (0);
                sc[j] = ((long)(vj) * ((frac)0x7ff8) / src_max);
            }
            if (stored->options & (1L<<6)) {
                do { switch ( (src_depth) >> 2 ) { case 0: va = (*(src) >> (8 - (sbit) - (src_depth))) & ((src_depth) | 1); break; case 1: va = (*(src) >> (4 - (sbit))) & 0xf; break; case 2: va = *(src); break; case 3: va = ((sbit) ? ((*(src) & 0xf) << 8) | (src)[1] : (*(src) << 4) | ((src)[1] >> 4)); break; case 4: va = (*(src) << 8) | (src)[1]; break; default: return ((-15)); } } while (0); do { sbit += (src_depth); src += sbit >> 3; sbit &= 7; } while (0);
                va = ((ulong)(va) * ((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) / src_max);
                do_alpha = ((bool)1);
            }
            ...
            if (do_alpha) {
                for (j = 0; j < ncolors; j++)
                    v[j] = ((ushort)( ((sc[j]) << (((1 << 1) * 8) - 15)) + ((sc[j]) >> (15 * 2 - ((1 << 1) * 8) - 3)) ));
                if (ncolors == 1)
                    v[2] = v[1] = v[0];
                pixel = ((dev)->procs.map_rgb_alpha_color)
                    (dev, v[0], v[1], v[2], va);
            } else {
                ...
                for (j = 0; j < dev->color_info.num_components; j++)
                    v[j] = ((ushort)( ((dc[j]) << (((1 << 1) * 8) - 15)) + ((dc[j]) >> (15 * 2 - ((1 << 1) * 8) - 3)) ));

                pixel = ((dev)->procs.encode_color)(dev, v);
            }
            if (sizeof(pixel) > 4) do { switch ( (depth) >> 2 ) { case 0: if ( (dbit += (depth)) == 8 ) *(dest)++ = dbyte | (byte)(pixel), dbyte = 0, dbit = 0; else dbyte |= (byte)((pixel) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbyte = (byte)((pixel) << 4); else *(dest)++ = dbyte | ((byte)(pixel)); break; case 3: if ( dbit ^= 4 ) *(dest)++ = (byte)((pixel) >> 4), dbyte = (byte)((pixel) << 4); else *(dest) = dbyte | (byte)((pixel) >> 8), (dest)[1] = (byte)(pixel), dest += 2; break; case 16: *(dest)++ = (byte)((pixel) >> ((56) >= 8 * sizeof((pixel)) ? (56) & (8 * sizeof((pixel)) - 1) : (56))); case 14: *(dest)++ = (byte)((pixel) >> ((48) >= 8 * sizeof((pixel)) ? (48) & (8 * sizeof((pixel)) - 1) : (48))); case 12: *(dest)++ = (byte)((pixel) >> ((40) >= 8 * sizeof((pixel)) ? (40) & (8 * sizeof((pixel)) - 1) : (40))); case 10: *(dest)++ = (byte)((pixel) >> ((32) >= 8 * sizeof((pixel)) ? (32) & (8 * sizeof((pixel)) - 1) : (32))); case 8: *(dest)++ = (byte)((pixel) >> 24); case 6: *(dest)++ = (byte)((pixel) >> 16); case 4: *(dest)++ = (byte)((pixel) >> 8); case 2: *(dest)++ = (byte)(pixel); break; default: return ((-15)); } } while (0); else do { switch ( (depth) >> 2 ) { case 0: if ( (dbit += (depth)) == 8 ) *(dest)++ = dbyte | (byte)(pixel), dbyte = 0, dbit = 0; else dbyte |= (byte)((pixel) << (8 - dbit)); break; case 1: if ( dbit ^= 4 ) dbyte = (byte)((pixel) << 4); else *(dest)++ = dbyte | ((byte)(pixel)); break; case 3: if ( dbit ^= 4 ) *(dest)++ = (byte)((pixel) >> 4), dbyte = (byte)((pixel) << 4); else *(dest) = dbyte | (byte)((pixel) >> 8), (dest)[1] = (byte)(pixel), dest += 2; break; case 8: *(dest)++ = (byte)((pixel) >> 24); case 6: *(dest)++ = (byte)((pixel) >> 16); case 4: *(dest)++ = (byte)((pixel) >> 8); case 2: *(dest)++ = (byte)(pixel); break; default: return ((-15)); } } while (0);
        }
        if ( (dbit) != 0 ) *(dest) = dbyte | (*(dest) & (0xff >> (dbit)));;
    }
    return 0;
}
                          ...
                          int x_offset, uint raster, uint std_raster)
{
    ...
    if (options & (1L<<2))
        params->options = options &= ~((1L<<1) | (1L<<2) | (1L<<3)) | (1L<<2),
            dest_bytes = 3;
    else if (options & (1L<<3))
        params->options = options &= ~((1L<<1) | (1L<<2) | (1L<<3)) | (1L<<3),
            dest_bytes = 4;
    else if (options & (1L<<1))
        params->options = options &= ~((1L<<1) | (1L<<2) | (1L<<3)) | (1L<<1),
            dest_bytes = 1;
    else
        return ((-15));
    ...
    if (options & (1L<<28)) {
        ...
        raster = std_raster =
            (options & (1L<<22) ?
             ((uint)((((end_byte << 3) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) : end_byte);
    }




    if (((options & ((1L<<2) | (1L<<5) | (1L<<6)))
           == (1L<<2)) &&
    ...
    if (options & ((1L<<5) | (1L<<6)))
        ++dest_bytes;

    for (i = (depth > 4 ? 16 : 1 << depth); --i >= 0; )
        mapped[i] = 0;
    for (; h > 0; dest_line += raster, src_line += dev_raster, --h) {
        const byte *src = (src_line); int bit = (src_bit_offset & 7);
        ...
        for (i = 0; i < w; ++i) {
            ...
            if (sizeof(pixel) > 4) do { switch ( (depth) >> 2 ) { case 0: pixel = (*(src) >> (8 - (bit) - (depth))) & ((depth) | 1); break; case 1: pixel = (*(src) >> (4 - (bit))) & 0xf; break; case 2: pixel = *(src); break; case 3: pixel = ((bit) ? ((*(src) & 0xf) << 8) | (src)[1] : (*(src) << 4) | ((src)[1] >> 4)); break; case 4: pixel = (*(src) << 8) | (src)[1]; break; case 6: pixel = (*(src) << 16) | ((src)[1] << 8) | (src)[2]; break; case 8: pixel = (*(src) << 24) | ((src)[1] << 16) | ((src)[2] << 8) | src[3]; break;; case 10: pixel = ((gx_color_index)((src)[0]) << ((32) >= 8 * sizeof((pixel)) ? (32) & (8 * sizeof((pixel)) - 1) : (32))) | ((gx_color_index)((src)[1]) << 24) | ((gx_color_index)((src)[2]) << 16) | ((gx_color_index)((src)[3]) << 8) | (gx_color_index)((src)[4]); break; case 12: pixel = ((gx_color_index)((src)[0]) << ((40) >= 8 * sizeof((pixel)) ? (40) & (8 * sizeof((pixel)) - 1) : (40))) | ((gx_color_index)((src)[1]) << ((32) >= 8 * sizeof((pixel)) ? (32) & (8 * sizeof((pixel)) - 1) : (32))) | ((gx_color_index)((src)[2]) << 24) | ((gx_color_index)((src)[3]) << 16) | ((gx_color_index)((src)[4]) << 8) | (gx_color_index)((src)[5]); break; case 14: pixel = ((gx_color_index)((src)[0]) << ((48) >= 8 * sizeof((pixel)) ? (48) & (8 * sizeof((pixel)) - 1) : (48))) | ((gx_color_index)((src)[1]) << ((40) >= 8 * sizeof((pixel)) ? (40) & (8 * sizeof((pixel)) - 1) : (40))) | ((gx_color_index)((src)[2]) << ((32) >= 8 * sizeof((pixel)) ? (32) & (8 * sizeof((pixel)) - 1) : (32))) | ((gx_color_index)((src)[3]) << 24) | ((gx_color_index)((src)[4]) << 16) | ((gx_color_index)((src)[5]) << 8) | (gx_color_index)((src)[6]); break; case 16: pixel = ((gx_color_index)((src)[0]) << ((56) >= 8 * sizeof((pixel)) ? (56) & (8 * sizeof((pixel)) - 1) : (56))) | ((gx_color_index)((src)[1]) << ((48) >= 8 * sizeof((pixel)) ? (48) & (8 * sizeof((pixel)) - 1) : (48))) | ((gx_color_index)((src)[2]) << ((40) >= 8 * sizeof((pixel)) ? (40) & (8 * sizeof((pixel)) - 1) : (40))) | ((gx_color_index)((src)[3]) << ((32) >= 8 * sizeof((pixel)) ? (32) & (8 * sizeof((pixel)) - 1) : (32))) | ((gx_color_index)((src)[4]) << 24) | ((gx_color_index)((src)[5]) << 16) | ((gx_color_index)((src)[6]) << 8) | (gx_color_index)((src)[7]); break;; default: return ((-15)); } } while (0); else do { switch ( (depth) >> 2 ) { case 0: pixel = (*(src) >> (8 - (bit) - (depth))) & ((depth) | 1); break; case 1: pixel = (*(src) >> (4 - (bit))) & 0xf; break; case 2: pixel = *(src); break; case 3: pixel = ((bit) ? ((*(src) & 0xf) << 8) | (src)[1] : (*(src) << 4) | ((src)[1] >> 4)); break; case 4: pixel = (*(src) << 8) | (src)[1]; break; case 6: pixel = (*(src) << 16) | ((src)[1] << 8) | (src)[2]; break; case 8: pixel = (*(src) << 24) | ((src)[1] << 16) | ((src)[2] << 8) | src[3]; break;; default: return ((-15)); } } while (0); do { bit += (depth); src += bit >> 3; bit &= 7; } while (0);
            if (pixel < 16) {
                if (mapped[pixel]) {
            ...
            (*((dev)->procs.map_color_rgb_alpha)) (dev, pixel, rgba);
            if (options & (1L<<5))
                *dest++ = ((gx_color_value)((((unsigned int)rgba[3])*0xFF01 + 0x800000)>>24));

            if (options & (1L<<2)) {
                dest[0] = ((gx_color_value)((((unsigned int)rgba[0])*0xFF01 + 0x800000)>>24));
                dest[1] = ((gx_color_value)((((unsigned int)rgba[1])*0xFF01 + 0x800000)>>24));
                dest[2] = ((gx_color_value)((((unsigned int)rgba[2])*0xFF01 + 0x800000)>>24));
                dest += 3;
            } else if (options & (1L<<3)) {


                gx_color_value white = (((rgba[0]) > ((((rgba[1]) > (rgba[2])) ? (rgba[1]) : (rgba[2])))) ? (rgba[0]) : ((((rgba[1]) > (rgba[2])) ? (rgba[1]) : (rgba[2]))));

                dest[0] = ((gx_color_value)((((unsigned int)white - rgba[0])*0xFF01 + 0x800000)>>24));
                dest[1] = ((gx_color_value)((((unsigned int)white - rgba[1])*0xFF01 + 0x800000)>>24));
                dest[2] = ((gx_color_value)((((unsigned int)white - rgba[2])*0xFF01 + 0x800000)>>24));
                dest[3] = ((gx_color_value)((((unsigned int)((gx_color_value)((1L << (sizeof(gx_color_value) * 8)) - 1)) - white)*0xFF01 + 0x800000)>>24));
                dest += 4;
            } else {

                *dest++ = ((gx_color_value)((((unsigned int)((rgba[0] * (ulong) 30) + (rgba[1] * (ulong) 59) + (rgba[2] * (ulong) 11) + ((30 + 59 + 11) / 2)) / (30 + 59 + 11))*0xFF01 + 0x800000)>>24));
...
gx_default_get_bits_rectangle(gx_device * dev, const gs_int_rect * prect,
                       gs_get_bits_params_t * params, gs_int_rect ** unread)
{
    int (*save_get_bits_rectangle)(gx_device *dev, const gs_int_rect *prect, gs_get_bits_params_t *params, gs_int_rect **unread) =
        ((dev)->procs.get_bits_rectangle);
    int depth = dev->color_info.depth;
    ...
    ((dev)->procs.get_bits_rectangle = (gx_no_get_bits_rectangle));
# 646 "./base/gdevdgbr.c"
    if (prect->q.y == prect->p.y + 1 &&
        ...
        (options & ((1L<<22) | (1L<<23))) &&
        ((options & ((1L<<24) | (1L<<26))) ||
         ((options & (1L<<25)) && params->x_offset == 0)) &&
        ((options & ((1L<<28) | (1L<<30))) ||
         ((options & (1L<<29)) &&
        ...
        if (!(prect->p.x == 0 && prect->q.x == dev->width)) {
            ...
            row = (*(dev->memory)->procs.alloc_bytes)(dev->memory, min_raster, "gx_default_get_bits_rectangle");
        ...
        code = (*((dev)->procs.get_bits)) (dev, prect->p.y, row,
                (params->options & (1L<<21)) ? &params->data[0]
                                                      : ((void*)0) );
        if (code >= 0) {
            if (row != data) {
                if (prect->p.x == 0 && params->data[0] != row
                    && params->options & (1L<<21)) {
                    ...
                    tdev.line_ptrs = &tdev.base;
                    tdev.base = data;
                    tdev.raster = ((uint)((((width_bits) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3));
                    code = (*((&mem_mono_device)->procs.copy_mono))
                        ((gx_device *) & tdev,
                         (params->options & (1L<<21)) ? params->data[0] : row,
                         prect->p.x * depth,
                         min_raster, 0L, 0, 0, width_bits, 1,
                         (gx_color_index) 0, (gx_color_index) 1);
                    params->data[0] = data;
                }
                ((dev->memory)->procs.free_object(dev->memory, row, "gx_default_get_bits_rectangle"));

            }
            params->options =
                (1L<<22) | (1L<<24) | (1L<<16) |
                (1L<<4) | (1L<<0) | (1L<<28) |
                (params->data[0] == data ? (1L<<20) : (1L<<21));
            goto ret;
        }
    } {
        ...
        if (options & ((1L<<1) | (1L<<2) | (1L<<3))) {




            int bpc = "\000\001\002\002\004\004\004\004\010\010\010\010\010\010\010\010\014\014\014\014\014\014\014\014\014\014\014\014\014\014\014\014\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020\020"[((options) >> 8) & 0x3f];
            int nc =
            (options & (1L<<3) ? 4 :
             options & (1L<<2) ? 3 : 1) +
            (options & (((1L<<4) | (1L<<5) | (1L<<6)) - (1L<<4)) ? 1 : 0);
            int bpp = bpc * nc;

            if (bpp > bits_per_pixel)
                bits_per_pixel = bpp;
        }
        row = (*(dev->memory)->procs.alloc_bytes)(dev->memory, (bits_per_pixel * w + 7) >> 3, "gx_default_get_bits_rectangle");
        ...
        if (row == 0) {
            ...
            (options & (1L<<29) ? params->raster :
             options & (1L<<22) ? ((uint)((((depth * w) + ((1 << 3) * 8 - 1)) >> (3 + 3)) << 3)) :
             (depth * w + 7) >> 3);
            gs_int_rect rect;
            ...
            gs_get_bits_options_t copy_options =
                ...
                ((1L<<24) | (1L<<26)) |
                ((1L<<28) | (1L<<30)) | (1L<<16) |
                (1L<<0) | (options & (((1L<<8) | (1L<<9) | (1L<<10) | (1L<<11) | (1L<<12) | (1L<<13)) | ((1L<<0) | ((1L<<1) | (1L<<2) | (1L<<3))))) |
                ((1L<<4) | (1L<<5) | (1L<<6));
            ...
            for (y = prect->p.y; y < prect->q.y; ++y) {
                ...
                code = (*save_get_bits_rectangle)
                    (dev, &rect, &copy_params, ((void*)0));
                if (code < 0)
                    break;
                if (copy_params.options & (1L<<24))
  ...
  ret:((dev)->procs.get_bits_rectangle = (save_get_bits_rectangle));
