...
@.str.1 = private unnamed_addr constant [11 x i8] c"2AESDecode\00", align 1
@zfaes_op_defs = constant [3 x %struct.op_def] [%struct.op_def { i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str, i32 0, i32 0), i32 (%struct.gs_context_state_s*)* null }, %struct.op_def { i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str.1, i32 0, i32 0), i32 (%struct.gs_context_state_s*)* @z_aes_d }, %struct.op_def zeroinitializer], align 16
...
@s_aes_template = external constant %struct.stream_template_s, align 8

; Function Attrs: nounwind uwtable
define internal i32 @z_aes_d(%struct.gs_context_state_s* %i_ctx_p) #0 {
entry:
  ...
  %4 = bitcast %struct.stream_aes_state_s* %state to i8*
  call void @llvm.lifetime.start(i64 176, i8* %4) #1
  %5 = bitcast i32* %use_padding to i8*
...
do.end:                                           ; preds = %do.cond
  ...
  %call7 = call i32 @dict_find_string(%struct.ref_s* %13, i8* getelementptr inbounds ([4 x i8], [4 x i8]* @.str.2, i32 0, i32 0), %struct.ref_s** %sop) #3
...
if.end.11:                                        ; preds = %do.end
  ...
  %value12 = getelementptr inbounds %struct.ref_s, %struct.ref_s* %14, i32 0, i32 1
  %const_bytes = bitcast %union.v* %value12 to i8**
  ...
  %17 = load i32, i32* %rsize, align 4, !tbaa !21
  %call14 = call i32 @s_aes_set_key(%struct.stream_aes_state_s* %state, i8* %15, i32 %17) #3
  %18 = load %struct.ref_s*, %struct.ref_s** %op, align 8, !tbaa !1
  %call15 = call i32 @dict_bool_param(%struct.ref_s* %18, i8* getelementptr inbounds ([8 x i8], [8 x i8]* @.str.3, i32 0, i32 0), i32 1, i32* %use_padding) #3
...
if.end.19:                                        ; preds = %if.end.11
  %19 = load i32, i32* %use_padding, align 4, !tbaa !22
  call void @s_aes_set_padding(%struct.stream_aes_state_s* %state, i32 %19) #3
  %20 = load %struct.gs_context_state_s*, %struct.gs_context_state_s** %i_ctx_p.addr, align 8, !tbaa !1
  %21 = bitcast %struct.stream_aes_state_s* %state to %struct.stream_state_s*
  %call20 = call i32 @filter_read(%struct.gs_context_state_s* %20, i32 0, %struct.stream_template_s* @s_aes_template, %struct.stream_state_s* %21, i32 0) #3
  store i32 %call20, i32* %retval
  store i32 1, i32* %cleanup.dest.slot
  br label %cleanup

cleanup:                                          ; preds = %if.end.19, %if.then.18, %if.then.10, %if.then.5, %if.then
  %22 = bitcast i32* %use_padding to i8*
  call void @llvm.lifetime.end(i64 4, i8* %22) #1
  %23 = bitcast %struct.stream_aes_state_s* %state to i8*
  call void @llvm.lifetime.end(i64 176, i8* %23) #1
  %24 = bitcast %struct.ref_s** %sop to i8*
...
declare i32 @s_aes_set_key(%struct.stream_aes_state_s*, i8*, i32) #2

declare i32 @dict_bool_param(%struct.ref_s*, i8*, i32, i32*) #2

declare void @s_aes_set_padding(%struct.stream_aes_state_s*, i32) #2
