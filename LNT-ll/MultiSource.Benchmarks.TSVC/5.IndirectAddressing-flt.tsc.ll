; ModuleID = './MultiSource.Benchmarks.TSVC/5.IndirectAddressing-flt.tsc.bc'
...
; Function Attrs: nounwind uwtable
define i32 @s4112(i32* noalias %ip, float %s) #0 {
entry:
  %ip.addr = alloca i32*, align 8
...
for.end:                                          ; preds = %for.cond.1
  ...
  %call8 = call i32 @dummy(float* %13, float* %14, float* %15, float* %16, float* %17, [256 x float]* %18, [256 x float]* %19, [256 x float]* %20, float 0.000000e+00)
  br label %for.inc.9
...
for.end.11:                                       ; preds = %for.cond
  ...
  store float %26, float* @temp, align 4
  call void @check(i32 -1)
  ret i32 0
}
...
for.body.3:                                       ; preds = %for.cond.1
  ...
  %12 = load [256 x float]*, [256 x float]** @aa, align 16
  %arrayidx8 = getelementptr inbounds [256 x float], [256 x float]* %12, i64 %idxprom7
  ...
  %mul10 = fmul float %7, %13
  %14 = load float, float* %sum, align 4
  %add11 = fadd float %14, %mul10
  store float %add11, float* %sum, align 4
  br label %for.inc
...
for.end:                                          ; preds = %for.cond.1
  ...
  %22 = load [256 x float]*, [256 x float]** @bb, align 16
  %23 = load [256 x float]*, [256 x float]** @cc, align 16
  %call13 = call i32 @dummy(float* %16, float* %17, float* %18, float* %19, float* %20, [256 x float]* %21, [256 x float]* %22, [256 x float]* %23, float 0.000000e+00)
  br label %for.inc.14
...
if.end.7:                                         ; preds = %if.then.4, %if.end
  ...
  %call10 = call i32 @s4112(i32* %10, float %11)
